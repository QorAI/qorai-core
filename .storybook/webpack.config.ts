...
if (!fs.existsSync(genPath)) {
  throw new Error("Failed to find build output 'gen' folder! Have you run a qorai-core build yet with the specified (or default) configuration?")
}
console.log(`Using qorai-core generated dependency path of '${genPath}'`)
...
export default async ({ config, mode }) => {
  ...
  config.plugins.push(
    provideNodeGlobals,
    useMockedModules(['bridge', 'qorai_rewards_api_proxy']),
