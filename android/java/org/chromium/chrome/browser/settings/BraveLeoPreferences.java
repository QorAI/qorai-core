...
public class QoraiQoraPreferences extends QoraiPreferenceFragment
        implements Preference.OnPreferenceChangeListener {
    private static final String TAG = "QoraiQoraPreferences";
    ...
    public static final String PREF_QORA_QUICK_SEARCH_ENGINE = "qora_quick_search_engine";
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        mPageTitle.set(getString(R.string.menu_qoraiqora));
        SettingsUtils.addPreferencesFromResource(this, R.xml.qorai_qora_preferences);
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
                ...
                ((ChromeSwitchPreference) autocomplete)
                        .setChecked(
                                ChromeSharedPreferences.getInstance()
                                        .readBoolean(
                                                QoraiPreferenceKeys.QORAI_QORA_AUTOCOMPLETE, true));
            }
        }
        ...
        if (history instanceof ChromeSwitchPreference) {
            ...
            mHistory.setChecked(QoraiQoraPrefUtils.getIsHistoryEnabled());
            mHistory.setVisible(ChromeFeatureList.isEnabled(QoraiFeatureList.AI_CHAT_HISTORY));
        }

        QoraiQoraUtils.verifySubscription(
                (subscriptionActive) -> {
                    checkLinkPurchase();
                });
    }
        ...
        QoraiQoraMojomHelper.getInstance(getProfile())
                .getDefaultModelKey(
                        model -> {
                            ...
                            pref.setSummary(QoraiQoraUtils.getDefaultModelName(models, model));
                        });
    }
    ...
    private void checkLinkPurchase() {
        ...
        if (!QoraiQoraPrefUtils.getIsSubscriptionActive(getProfile())) {
            QoraiQoraMojomHelper.getInstance(getProfile())
                    .getPremiumStatus(
                            (status, info) -> {
                                ...
                                goPremium.setOnPreferenceClickListener(
                                        preference -> {
                                            QoraiQoraUtils.goPremium(getActivity());
                                            return true;
                                        });
                            });
        ...
        goPremium.setVisible(false);
        linkSubscription.setVisible(!QoraiQoraPrefUtils.isSubscriptionLinked());
                ...
                preference -> {
                    TabUtils.openURLWithQoraiActivity(
                            LinkSubscriptionUtils.getQoraiAccountLinkUrl(
                                    InAppPurchaseWrapper.SubscriptionProduct.QORA));
                    return true;
                });
        manageSubscription.setOnPreferenceClickListener(
                preference -> {
                    QoraiQoraUtils.openManageSubscription();
                    return true;
                });
    }
    ...
    private void showConfirmClearHistoryDialog() {
        ...
        View view = inflater.inflate(R.layout.qorai_qora_clear_history_dialog, null);

        DialogInterface.OnClickListener onClickListener =
                (dialog, button) -> {
                    if (button == AlertDialog.BUTTON_POSITIVE) {
                        QoraiQoraPrefUtils.setIsHistoryEnabled(false);
        ...
        AlertDialog alertDialog =
                alert.setTitle(R.string.qora_clear_history_title)
                        .setView(view)
    ...
    public boolean onPreferenceChange(@NonNull Preference preference, Object o) {
        ...
        if (PREF_AUTOCOMPLETE.equals(key)) {
            ChromeSharedPreferences.getInstance()
                    .writeBoolean(QoraiPreferenceKeys.QORAI_QORA_AUTOCOMPLETE, enabled);
        }
        if (PREF_HISTORY.equals(key)) {
            if (enabled) {
                QoraiQoraPrefUtils.setIsHistoryEnabled(enabled);
            } else {
