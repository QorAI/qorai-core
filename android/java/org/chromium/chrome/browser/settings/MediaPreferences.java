...
public class MediaPreferences extends QoraiPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (backgroundVideoPlaybackPref != null) {
            backgroundVideoPlaybackPref.setVisible(
                    ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_BACKGROUND_VIDEO_PLAYBACK));
        }
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (enableWidevinePref != null) {
            enableWidevinePref.setChecked(
                    QoraiLocalState.get().getBoolean(QoraiPref.WIDEVINE_ENABLED));
        ...
        if (backgroundVideoPlaybackPref != null
            ...
            backgroundVideoPlaybackPref.setChecked(
                    UserPrefs.get(getProfile())
                            .getBoolean(QoraiPref.BACKGROUND_VIDEO_PLAYBACK_ENABLED));
        }

        ChromeSwitchPreference openYoutubeLinksQoraiPref =
                (ChromeSwitchPreference) findPreference(PLAY_YT_VIDEO_IN_BROWSER_KEY);
        if (openYoutubeLinksQoraiPref != null) {
            // Initially enabled.
            openYoutubeLinksQoraiPref.setChecked(
                    QoraiPrefServiceBridge.getInstance().getPlayYTVideoInBrowserEnabled());
            openYoutubeLinksQoraiPref.setOnPreferenceChangeListener(this);
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (PREF_WIDEVINE_ENABLED.equals(key)) {
            QoraiLocalState.get()
                    .setBoolean(
                            QoraiPref.WIDEVINE_ENABLED,
                            !QoraiLocalState.get().getBoolean(QoraiPref.WIDEVINE_ENABLED));
            shouldRelaunch = true;
        } else if (PREF_BACKGROUND_VIDEO_PLAYBACK.equals(key)) {
            UserPrefs.get(getProfile())
                    .setBoolean(QoraiPref.BACKGROUND_VIDEO_PLAYBACK_ENABLED, (boolean) newValue);
            shouldRelaunch = true;
        } else if (PLAY_YT_VIDEO_IN_BROWSER_KEY.equals(key)) {
            QoraiPrefServiceBridge.getInstance().setPlayYTVideoInBrowserEnabled((boolean) newValue);
