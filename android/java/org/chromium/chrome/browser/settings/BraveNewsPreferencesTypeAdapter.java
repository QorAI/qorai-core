...
import org.chromium.qorai_news.mojom.PublisherType;
...
public class QoraiNewsPreferencesTypeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private final QoraiNewsController mQoraiNewsController;
    private final QoraiNewsPreferencesListener mQoraiNewsPreferencesListener;
    private QoraiNewsPreferencesSearchType mQoraiNewsPreferencesSearchType;
    ...
    private final String mQoraiNewsPreferencesType;
    ...
    public QoraiNewsPreferencesTypeAdapter(
            Context context,
            QoraiNewsPreferencesListener qoraiNewsPreferencesListener,
            QoraiNewsPreferencesSearchType qoraiNewsPreferencesSearchType,
            QoraiNewsController qoraiNewsController,
            RequestManager glide,
            String qoraiNewsPreferencesType,
            List<Channel> channelList,
            List<Publisher> publisherList) {
        mContext = context;
        mQoraiNewsController = qoraiNewsController;
        mQoraiNewsPreferencesSearchType = qoraiNewsPreferencesSearchType;
        mQoraiNewsPreferencesListener = qoraiNewsPreferencesListener;
        mGlide = glide;
        mQoraiNewsPreferencesType = qoraiNewsPreferencesType;
        ...
        if (mChannelList.size() > 0) {
            mChannelIcons = QoraiNewsUtils.getChannelIcons();
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof NewsPreferencesSectionViewHolder) {
            ...
            if ((mQoraiNewsPreferencesType.equalsIgnoreCase(
                        ...
                        || mQoraiNewsPreferencesType.equalsIgnoreCase(
                ...
                if (mChannelList.size() > 0 && position < getChannelItemsCount()) {
                    setChannel(position, viewHolder, mChannelList.get(position - ONE_ITEM_SPACE));
                        ...
                        && (mQoraiNewsPreferencesSearchType
                                        == QoraiNewsPreferencesSearchType.SearchUrl
                                || mQoraiNewsPreferencesSearchType
                                        == QoraiNewsPreferencesSearchType.GettingFeed
                                || mQoraiNewsPreferencesSearchType
                ...
                } else if (mQoraiNewsPreferencesSearchType
    ...
    private void setSearchUrl(int position, NewsPreferencesViewHolder viewHolder) {
        ...
        if (mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.SearchUrl
                || mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.GettingFeed) {
            ...
            if (mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.GettingFeed) {
            ...
            viewHolder.mBtnFollow.setOnClickListener(
                    view -> {
                        if (mQoraiNewsPreferencesSearchType
                                == QoraiNewsPreferencesSearchType.SearchUrl) {
                            mQoraiNewsPreferencesSearchType =
                                    QoraiNewsPreferencesSearchType.GettingFeed;
                            notifyItemChanged(position);
                            mQoraiNewsPreferencesListener.findFeeds(mSearchUrl);
                        }
                    });
        } else if (mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.NotFound) {
    ...
    private void setFeedSearchResultItem(int position, NewsPreferencesViewHolder viewHolder) {
        if (position >= 0 && position < mFeedSearchResultItemList.size()) {
            ...
            viewHolder.mBtnFollow.setOnClickListener(
                    view -> {
                        if (!isFollow) {
                            mQoraiNewsPreferencesListener.subscribeToNewDirectFeed(
                            ...
                            String publisherId =
                                    mFeedSearchResultItemFollowMap.get(
                                            feedSearchResultItem.feedUrl.url);
                            mQoraiNewsPreferencesListener.updateFeedSearchResultItem(
                                    position, feedSearchResultItem.feedUrl.url, publisherId);

                            mQoraiNewsPreferencesListener.onPublisherPref(
    ...
    private void setSource(
        ...
        if (mQoraiNewsController != null && publisher.coverUrl != null) {
            mQoraiNewsController.getImageData(
                    publisher.coverUrl,
                    imageData -> {
                        if (imageData != null) {
    ...
    private void setChannel(int position, NewsPreferencesViewHolder viewHolder, Channel channel) {
        ...
        viewHolder.mBtnFollow.setVisibility(View.VISIBLE);
        if (subscribedLocalesList.contains(QoraiNewsUtils.getLocale())) {
            displayFollowButton(true, R.string.unfollow, viewHolder);
        } else {
            displayFollowButton(false, R.string.follow, viewHolder);
        }
        ...
        viewHolder.mBtnFollow.setOnClickListener(
                view -> {
                    boolean isSubscribed =
                            subscribedLocalesList.contains(QoraiNewsUtils.getLocale());
                    isSubscribed = !isSubscribed;
                    if (isSubscribed) {
                        subscribedLocalesList.add(QoraiNewsUtils.getLocale());
                    } else {
                        subscribedLocalesList.remove(QoraiNewsUtils.getLocale());
                    }
                    ...
                    mQoraiNewsPreferencesListener.onChannelSubscribed(
    ...
    private void displayFollowButton(
        ...
        if (isFollowing) {
            holder.mBtnFollow.setBackgroundResource(R.drawable.qorai_news_settings_unfollow_bg);
    ...
    public void setFindFeeds(List<FeedSearchResultItem> feedSearchResultItemList,
        ...
        mQoraiNewsPreferencesSearchType = qoraiNewsPreferencesSearchType;
        if (qoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.NewSource) {
            notifyItemRangeInserted(startPosition, feedSearchResultItemList.size());
        } else if (qoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.NotFound) {
    ...
    public int getItemCount() {
        if (mQoraiNewsPreferencesType.equalsIgnoreCase(
            ...
            if (mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.SearchUrl
                    || mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.GettingFeed
                    || mQoraiNewsPreferencesSearchType == QoraiNewsPreferencesSearchType.NotFound) {
