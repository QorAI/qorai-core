...
public class QoraiQAPreferences extends QoraiPreferenceFragment
        implements OnPreferenceChangeListener, QoraiRewardsObserver {
    ...
    private ChromeSwitchPreference mQoraiDormantFeatureEngagement;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mQoraiDormantFeatureEngagement =
                (ChromeSwitchPreference)
                        findPreference(OnboardingPrefManager.PREF_DORMANT_USERS_ENGAGEMENT);
        if (mQoraiDormantFeatureEngagement != null) {
            mQoraiDormantFeatureEngagement.setOnPreferenceChangeListener(this);
        }
        ...
        mIsStagingServer.setChecked(
                UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                        .getBoolean(QoraiPref.USE_REWARDS_STAGING_SERVER));
        ...
        mDbUtil = QoraiDbUtil.getInstance();
    ...
    @Override
    public void onStart() {
        QoraiRewardsNativeWorker.getInstance().addObserver(this);
        super.onStart();
    }

    @Override
    public void onStop() {
        QoraiRewardsNativeWorker.getInstance().removeObserver(this);
        super.onStop();
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (PREF_USE_REWARDS_STAGING_SERVER.equals(preference.getKey())) {
            QoraiRewardsNativeWorker.getInstance().resetTheWholeState();
    ...
    private void checkQACode() {
        ...
        DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int button) {
                if (button != AlertDialog.BUTTON_POSITIVE
                        || !input.getText().toString().equals(QoraiConfig.DEVELOPER_OPTIONS_CODE)) {
                    getActivity().finish();
                }
            }
        };
    ...
    private void enableMaximumAdsNumber(boolean enable) {
        if (enable) {
            // Save current values
            int adsPerHour = QoraiRewardsNativeWorker.getInstance().getAdsPerHour();
            SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
            SharedPreferences.Editor sharedPreferencesEditor = sharedPreferences.edit();
            sharedPreferencesEditor.putInt(QA_ADS_PER_HOUR, adsPerHour);
            sharedPreferencesEditor.apply();
            // Set max value
            QoraiRewardsNativeWorker.getInstance().setAdsPerHour(MAX_ADS);
            return;
        }
        // Set saved values
        int adsPerHour = ContextUtils.getAppSharedPreferences().getInt(
                             QA_ADS_PER_HOUR, DEFAULT_ADS_PER_HOUR);
        QoraiRewardsNativeWorker.getInstance().setAdsPerHour(adsPerHour);
    }
    ...
    public void onResetTheWholeState(boolean success) {
        if (success) {
            ...
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.USE_REWARDS_STAGING_SERVER, mUseRewardsStagingServer);
            QoraiRewardsHelper.setRewardsEnvChange(true);

            QoraiRelaunchUtils.askForRelaunch(getActivity());
        } else {
            QoraiRelaunchUtils.askForRelaunchCustom(getActivity());
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CHOOSE_FILE_FOR_IMPORT_REQUEST_CODE && resultCode == Activity.RESULT_OK
            ...
            try {
                ...
                Log.e(QoraiDbUtil.getTag(), "Error on preparing database file: " + e);
                return;
            }
            requestRestart(true);
        }
