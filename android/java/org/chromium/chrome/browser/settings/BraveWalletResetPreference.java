...
/**
 * The preference used to reset Qorai Wallet.
 */
public class QoraiWalletResetPreference
        extends Preference implements Preference.OnPreferenceClickListener {
    private static final String TAG = "QoraiWalletResetPref";

    private final int mPrefAccentColor;
    private final String mConfirmationPhrase;
    private Profile mProfile;

    /** Constructor for QoraiWalletResetPreference. */
    public QoraiWalletResetPreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        Resources resources = getContext().getResources();
        mPrefAccentColor = getContext().getColor(R.color.wallet_error_text_color);
        mConfirmationPhrase =
                resources.getString(R.string.qorai_wallet_reset_settings_confirmation_phrase);
    ...
    private void showQoraiWalletResetDialog() {
        LayoutInflater inflater =
                (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(R.layout.qorai_wallet_reset_content, null);
        final TextView textView = (TextView) view.findViewById(R.id.qorai_wallet_reset_textview);
        final EditText input = (EditText) view.findViewById(R.id.qorai_wallet_reset_edittext);

        textView.setText(getContext().getString(
                R.string.qorai_wallet_reset_settings_confirmation, mConfirmationPhrase));
                ...
                (dialog, button) -> {
                    ...
                    if (button == AlertDialog.BUTTON_POSITIVE) {
                        ...
                        if (TextUtils.equals(inputText, mConfirmationPhrase)) {
                            ...
                            for (String key : WalletConstants.QORAI_WALLET_PREFS) {
        ...
        AlertDialog alertDialog =
                alert.setTitle(R.string.qorai_wallet_reset_settings_option)
                        .setView(view)
                        .setPositiveButton(R.string.qorai_wallet_confirm_text, onClickListener)
    ...
    private void launchQoraiTabbedActivity() {
        ChromeTabbedActivity activity = QoraiActivity.getChromeTabbedActivity();
        if (activity == null) {
            return;
        }
