...
import org.chromium.chrome.browser.sync.QoraiSyncDevices;
import org.chromium.chrome.browser.sync.SyncServiceFactory;
import org.chromium.chrome.browser.sync.settings.QoraiManageSyncSettings;
import org.chromium.components.browser_ui.settings.SettingsNavigation;
import org.chromium.components.sync.SyncService;
import org.chromium.ui.base.QoraiClipboardHelper;
...
public class QoraiSyncScreensPreference extends QoraiPreferenceFragment
        implements View.OnClickListener,
                BackPressHelper.ObsoleteBackPressedHandler,
                BarcodeTracker.BarcodeGraphicTrackerCallback,
                QoraiSyncDevices.DeviceInfoChangedListener,
    ...
    private TextView mQoraiSyncTextDevicesTitle;
    private TextView mQoraiSyncWordCountTitle;
    private TextView mQoraiSyncAddDeviceCodeWords;
    ...
    // Below enum is matching the values of GetDeviceTypeString() in qorai_device_info.cc
    ...
    QoraiSyncWorker getQoraiSyncWorker() {
        return QoraiSyncWorker.get();
    }
    ...
    public View onCreateView(
        ...
        return mInflater.inflate(R.layout.qorai_sync_layout, container, false);
    }
    ...
    private void fillDevices() {
        try {
            ...
            ArrayList<QoraiSyncDevices.SyncDeviceInfo> deviceInfos =
                    QoraiSyncDevices.get().getSyncDeviceList();

            ViewGroup insertPoint = (ViewGroup) getView().findViewById(R.id.qorai_sync_devices);
            insertPoint.removeAllViews();
            int index = 0;
            for (QoraiSyncDevices.SyncDeviceInfo device : deviceInfos) {
                View separator = (View) mInflater.inflate(R.layout.menu_separator, null);
                View listItemView = (View) mInflater.inflate(R.layout.qorai_sync_device, null);
                if (null != listItemView && null != separator && null != insertPoint) {
                    ImageView imageView =
                            (ImageView) listItemView.findViewById(R.id.qorai_sync_device_image);
                    ...
                    TextView textView =
                            (TextView) listItemView.findViewById(R.id.qorai_sync_device_text);
                    if (null != textView) {
                        if (device.mIsCurrentDevice) {
                            String currentDevice =
                                    device.mName
                                            ...
                                            + getResources()
                                                    .getString(
                                                            R.string.qorai_sync_this_device_text);
                    ...
                    AppCompatImageView deleteButton =
                            (AppCompatImageView)
                                    listItemView.findViewById(R.id.qorai_sync_remove_device);
                    if (device.mSupportsSelfDelete || device.mIsCurrentDevice) {
                        deleteButton.setTag(device);
                        deleteButton.setOnClickListener(
                                v -> {
                                    QoraiSyncDevices.SyncDeviceInfo deviceToDelete =
                                            (QoraiSyncDevices.SyncDeviceInfo) v.getTag();
            ...
            if (index > 0) {
                mQoraiSyncTextDevicesTitle.setText(
                        getResources().getString(R.string.qorai_sync_devices_title));
                View separator = (View) mInflater.inflate(R.layout.menu_separator, null);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mScanChainCodeButton = getView().findViewById(R.id.qorai_sync_btn_scan_chain_code);
        if (mScanChainCodeButton != null) {
            mScanChainCodeButton.setOnClickListener(this);
        }

        mStartNewChainButton = getView().findViewById(R.id.qorai_sync_btn_start_new_chain);
        if (mStartNewChainButton != null) {
            mStartNewChainButton.setOnClickListener(this);
        }

        mEnterCodeWordsButton = getView().findViewById(R.id.qorai_sync_btn_enter_code_words);
        if (mEnterCodeWordsButton != null) {
            mEnterCodeWordsButton.setOnClickListener(this);
        }

        mQRCodeImage = getView().findViewById(R.id.qorai_sync_qr_code_image);

        mQRContainer = getView().findViewById(R.id.qorai_sync_qr_containter);

        mDoneButton = getView().findViewById(R.id.qorai_sync_btn_done);
        if (mDoneButton != null) {
            mDoneButton.setOnClickListener(this);
        }

        mDoneLaptopButton = getView().findViewById(R.id.qorai_sync_btn_add_laptop_done);
        if (mDoneLaptopButton != null) {
            mDoneLaptopButton.setOnClickListener(this);
        }

        mUseCameraButton = getView().findViewById(R.id.qorai_sync_btn_use_camera);
        if (mUseCameraButton != null) {
            mUseCameraButton.setOnClickListener(this);
        }

        mConfirmCodeWordsButton = getView().findViewById(R.id.qorai_sync_confirm_code_words);
        if (mConfirmCodeWordsButton != null) {
            mConfirmCodeWordsButton.setOnClickListener(this);
        }

        mMobileButton = getView().findViewById(R.id.qorai_sync_btn_mobile);
        if (mMobileButton != null) {
            mMobileButton.setOnClickListener(this);
        }

        mLaptopButton = getView().findViewById(R.id.qorai_sync_btn_laptop);
        if (mLaptopButton != null) {
            mLaptopButton.setOnClickListener(this);
        }

        mPasteButton = getView().findViewById(R.id.qorai_sync_paste_button);
        if (mPasteButton != null) {
            mPasteButton.setOnClickListener(this);
        }

        mCopyButton = getView().findViewById(R.id.qorai_sync_copy_button);
        if (mCopyButton != null) {
            mCopyButton.setOnClickListener(this);
        }

        mQoraiSyncTextDevicesTitle = getView().findViewById(R.id.qorai_sync_devices_title);
        mQoraiSyncWordCountTitle = getView().findViewById(R.id.qorai_sync_text_word_count);
        mQoraiSyncWordCountTitle.setText(getString(R.string.qorai_sync_word_count_text, 0));
        mQoraiSyncAddDeviceCodeWords =
                getView().findViewById(R.id.qorai_sync_add_device_code_words);

        mCameraSourcePreview = getView().findViewById(R.id.preview);

        mAddDeviceButton = getView().findViewById(R.id.qorai_sync_btn_add_device);
        if (null != mAddDeviceButton) {
            mAddDeviceButton.setOnClickListener(this);
        }

        mDeleteAccountButton = getView().findViewById(R.id.qorai_sync_btn_delete_account);
        if (null != mDeleteAccountButton) {
            mDeleteAccountButton.setOnClickListener(this);
        }

        mShowCategoriesButton = getView().findViewById(R.id.qorai_sync_btn_show_categories);
        ...
        mNewCodeWordsButton = getView().findViewById(R.id.qorai_sync_btn_add_laptop_new_code);
        assert mNewCodeWordsButton != null;
        mNewCodeWordsButton.setOnClickListener(this);

        mNewQrCodeButton = getView().findViewById(R.id.qorai_sync_btn_add_mobile_new_code);
        ...
        mLayoutMobile = getView().findViewById(R.id.qorai_sync_frame_mobile);
        mLayoutLaptop = getView().findViewById(R.id.qorai_sync_frame_laptop);
    ...
    private void setAppropriateView() {
        ...
        boolean firstSetupComplete = getQoraiSyncWorker().isInitialSyncFeatureSetupComplete();
    ...
    private String getWordsValidationString(String words) {
        int validationResult = getQoraiSyncWorker().getWordsValidationResult(words);
        ...
        switch (validationResult) {
            ...
            case 2:
                // kVersionDeprecated
                return getResources().getString(R.string.qorai_sync_code_from_deprecated_version);
            case 3:
                // kExpired
                return getResources().getString(R.string.qorai_sync_code_expired);
            case 4:
                // kValidForTooLong
                return getResources().getString(R.string.qorai_sync_code_valid_for_too_long);

            default:
                // These three different types of errors have the same message
                // kWrongWordsNumber
                // kNotValidPureWords
                return getResources().getString(R.string.qorai_sync_wrong_code_error);
        }
    }
        ...
        if (mScanChainCodeButton == v) {
            ...
            if (null != mCodeWords) {
                String codeWordsAtClipboard =
                        QoraiClipboardHelper.getTextFromClipboard(
            ...
            if (null != mQoraiSyncAddDeviceCodeWords) {
                QoraiClipboardHelper.saveTextToClipboard(
                        getActivity().getApplicationContext(),
                        mQoraiSyncAddDeviceCodeWords.getText().toString(),
                        R.string.qorai_sync_copied_text,
            ...
            String codephraseCandidate = getQoraiSyncWorker().getPureWordsFromTimeLimited(words);
            ...
            getActivity().setTitle(R.string.qorai_sync_code_words_title);
            if (null != mCodeWords && null != mQoraiSyncWordCountTitle) {
                mCodeWords.addTextChangedListener(
                        new TextWatcher() {
                            ...
                            public void onTextChanged(
                                ...
                                mQoraiSyncWordCountTitle.setText(
                                        getString(R.string.qorai_sync_word_count_text, wordCount));
                                mQoraiSyncWordCountTitle.invalidate();
                            }
                        });
            }
            ...
            settingsLauncher.startSettings(getContext(), QoraiManageSyncSettings.class);
    ...
    void generateNewCodeWords() {
        ...
        String timeLimitedWords = getQoraiSyncWorker().getTimeLimitedWordsFromPure(codePhrase);
        assert timeLimitedWords != null && !timeLimitedWords.isEmpty();
        mQoraiSyncAddDeviceCodeWords.setVisibility(View.VISIBLE);
        mQoraiSyncAddDeviceCodeWords.setText(timeLimitedWords);
        mCopyButton.setVisibility(View.VISIBLE);
        mNewCodeWordsButton.setVisibility(View.GONE);

        LocalDateTime notAfterTime =
                getQoraiSyncWorker().getNotAfterFromFromTimeLimitedWords(timeLimitedWords);
    ...
    void generateNewQrCode() {
        ...
        String seedHex = getQoraiSyncWorker().getSeedHexFromWords(getPureWords());
        if (null == seedHex || seedHex.isEmpty()) {
            ...
            if (!isSeedHexValid(seedHex)) {
                ...
                String qrCodeString = getQoraiSyncWorker().getQrDataJson(seedHex);
        ...
        getActivity()
                .runOnUiThread(
                        new Runnable() {
                            ...
                            public void run() {
                                String seedWords =
                                        getQoraiSyncWorker().getWordsFromSeedHex(seedHex);
                            ...
                            if (result) {
                                ...
                                onSyncError(
                                        getResources()
                                                .getString(
                                                        R.string
                                                                .qorai_sync_joining_deleted_account));
                                allowNewQrScan();
                            }
                        });

        getQoraiSyncWorker().requestSync();
        getQoraiSyncWorker().saveCodephrase(seedWords);
        getQoraiSyncWorker().finalizeSyncSetup();
    }
    ...
    public void onDestroy() {
        ...
        if (mDeviceInfoObserverSet) {
            QoraiSyncDevices.get().removeDeviceInfoChangedListener(this);
    ...
    private boolean isSeedHexValid(String barcode) {
        ...
        String seedWords = getQoraiSyncWorker().getWordsFromSeedHex(barcode);
    ...
    private String getQrCodeValidationString(String jsonQr) {
        ...
        int validationResult = getQoraiSyncWorker().getQrCodeValidationResult(jsonQr);
        Log.v(TAG, "validationResult is " + validationResult);
        switch (validationResult) {
            case 0:
                // kValid, empty string indicates there is no error
            ...
            default:
                ...
                return getResources().getString(R.string.qorai_sync_wrong_qrcode_error);
        }
    }
    ...
    public void onDetectedQrCode(Barcode barcode) {
        if (barcode != null) {
            ...
            String seedHex = getQoraiSyncWorker().getSeedHexFromQrJson(jsonQr);
    ...
    private void showEndDialog(String message, Runnable runWhenDismissed) {
        ...
        AlertDialog alertDialog =
                alertBuilder.setTitle(getResources().getString(R.string.qorai_sync_device))
                        .setMessage(message)
    ...
    private void deleteDeviceDialog(QoraiSyncDevices.SyncDeviceInfo device) {
        AlertDialog.Builder alertBuilder =
                new AlertDialog.Builder(getActivity(), R.style.ThemeOverlay_BrowserUI_AlertDialog);
                    ...
                    public void onClick(DialogInterface dialog, int button) {
                        if (button == AlertDialog.BUTTON_POSITIVE) {
                            if (device.mIsCurrentDevice) {
                                ...
                                getQoraiSyncWorker().resetSync();
                                startLeaveSyncChainOperations();
                            } else {
                                QoraiSyncDevices.get().deleteDevice(device.mGuid);
                            }
                        }
                    }
                };
        ...
        if (device.mIsCurrentDevice) {
            deviceNameToDisplay = deviceNameToDisplay + " "
                    + getResources().getString(R.string.qorai_sync_this_device_text);
        }
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.qorai_sync_remove_device_text))
                        .setMessage(
                                getString(R.string.qorai_sync_delete_device, deviceNameToDisplay))
                        .setPositiveButton(R.string.ok, onClickListener)
    ...
    private void permanentlyDeleteAccount() {
        ...
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.qorai_sync_delete_account_text))
                        .setMessage(getString(R.string.qorai_sync_delete_account_message))
                        .setPositiveButton(
                                R.string.qorai_sync_delete_account_button, onClickListener)
                        .setNegativeButton(R.string.cancel, onClickListener)
                        .create();
        alertDialog.getDelegate().setHandleNativeActionModesEnabled(false);
        alertDialog.show();
    ...
    private void permanentlyDeletedAccount(String result) {
        ...
        if (result == null || result.isEmpty()) {
            showToast(
                    activity,
                    getResources().getString(R.string.qorai_sync_delete_account_succeeded_toast)
                            + " "
                            + result);
        } else {
            showToast(
                    activity,
                    getResources().getString(R.string.qorai_sync_delete_account_failed_toast)
                            + " "
                            + result);
        }
    }
    ...
    private void startLeaveSyncChainOperations() {
        ...
        mQoraiSyncTextDevicesTitle.setText(
                getResources().getString(R.string.qorai_sync_leaving_sync_chain_title));

        mShowCategoriesButton.setVisibility(View.GONE);
        mAddDeviceButton.setVisibility(View.GONE);

        ViewGroup devicesUiGroup = (ViewGroup) getView().findViewById(R.id.qorai_sync_devices);
    ...
    private void setJoinExistingChainLayout() {
        ...
        getActivity().setTitle(R.string.qorai_sync_scan_chain_code);
        if (null != mScrollViewSyncChainCode) {
    ...
    private void setNewChainLayout() {
        getActivity().setTitle(R.string.qorai_sync_start_new_chain);
        if (null != mScrollViewSyncInitial) {
            mScrollViewSyncInitial.setVisibility(View.GONE);
        }
        if (null != mScrollViewEnterCodeWords) {
    ...
    public String getPureWords() {
        if (mCodephrase == null || mCodephrase.isEmpty()) {
            mCodephrase = getQoraiSyncWorker().getPureWords();
        }
        return mCodephrase;
    }
    ...
    private void setAddMobileDeviceLayout() {
        getActivity().setTitle(R.string.qorai_sync_btn_mobile);
    ...
    private void setQrCountDown(LocalDateTime notAfterTime) {
        FragmentContainerView the_view =
                (FragmentContainerView) getView().findViewById(R.id.qorai_sync_count_down_qr);
        QoraiSyncCodeCountdownFragment countdown = the_view.getFragment();
    ...
    private void setAddLaptopLayout() {
        getActivity().setTitle(R.string.qorai_sync_btn_laptop);
    ...
    private void setWordsCountDown(LocalDateTime notAfterTime) {
        FragmentContainerView containerView =
                (FragmentContainerView)
                        getView().findViewById(R.id.qorai_sync_count_down_code_words);
        QoraiSyncCodeCountdownFragment countdown = containerView.getFragment();
        countdown.setExpiredRunnable(
                () -> {
                    mNewCodeWordsButton.setVisibility(View.VISIBLE);
                    mQoraiSyncAddDeviceCodeWords.setVisibility(View.GONE);
    ...
    private void setSyncDoneLayout() {
        if (!mDeviceInfoObserverSet) {
            QoraiSyncDevices.get().addDeviceInfoChangedListener(this);
            mDeviceInfoObserverSet = true;
        }

        QoraiPermissionUtils.showNotificationPermissionDialog(getActivity());
        ...
        mQoraiSyncTextDevicesTitle.setText(
                getResources().getString(R.string.qorai_sync_loading_devices_title));
