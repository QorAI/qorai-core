...
public class QoraiDialogPreference extends DialogPreference {
    ...
    public QoraiDialogPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        setDialogLayoutResource(R.layout.qorai_dialog_preference);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.qorai_list_preference);

        // Sets dialog entries
        CharSequence[] entries = a.getTextArray(R.styleable.qorai_list_preference_dialog_entries);
        if (entries != null) {
            this.setDialogEntries(entries);
        }

        // Sets dialog subtitle
        String subtitle = a.getString(R.styleable.qorai_list_preference_dialog_subtitle);
        if (subtitle != null) {
            this.setDialogSubtitle(subtitle);
        }

        // Sets default checked index
        int defaultChecked = a.getInt(R.styleable.qorai_list_preference_dialog_default_index, 1);
