...
import org.chromium.components.browser_ui.media.QoraiMediaSessionHelper;
import org.chromium.components.browser_ui.media.MediaNotificationInfo;
import org.chromium.components.browser_ui.media.MediaNotificationManager;

@NullMarked
public class QoraiMediaSessionTabHelper extends MediaSessionTabHelper {
    /** Will be deleted in bytecode, value from the parent class will be used instead. */
    private @Nullable Tab mTab;

    QoraiMediaSessionTabHelper(Tab tab) {
        super(tab);
    }

    @Override
    public MediaNotificationInfo.Builder createMediaNotificationInfoBuilder() {
        if (!QoraiMediaSessionHelper.isQoraiTalk(assumeNonNull(mTab).getWebContents())) {
    ...
    public void hideMediaNotification() {
        ...
        if (!QoraiMediaSessionHelper.isQoraiTalk(mTab.getWebContents())) {
