...
public class QoraiWalletSolanaConnectedSitesPreference
    ...
    public QoraiWalletSolanaConnectedSitesPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private void updateWebSitesList() {
        mQoraiWalletService.getWebSitesWithPermission(CoinType.SOL, webSites -> {
            if (mAdapter == null) {
    ...
    public void removePermission(String webSite) {
        mQoraiWalletService.resetWebSitePermission(CoinType.SOL, webSite, success -> {
            if (success) {
                updateWebSitesList();
            }
        });
