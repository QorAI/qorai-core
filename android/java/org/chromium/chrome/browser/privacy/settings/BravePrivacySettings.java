...
import org.chromium.qorai_shields.mojom.FilterListAndroidHandler;
import org.chromium.qorai_shields.mojom.FilterListConstants;
import org.chromium.build.annotations.NullMarked;
import org.chromium.build.annotations.Nullable;
import org.chromium.chrome.R;
import org.chromium.chrome.browser.QoraiFeatureUtil;
import org.chromium.chrome.browser.QoraiLocalState;
import org.chromium.chrome.browser.QoraiRelaunchUtils;
import org.chromium.chrome.browser.browsing_data.QoraiClearBrowsingDataFragment;
...
import org.chromium.chrome.browser.settings.QoraiDialogPreference;
import org.chromium.chrome.browser.settings.QoraiPreferenceDialogFragment;
import org.chromium.chrome.browser.settings.QoraiWebrtcPolicyPreference;
...
/** Fragment to keep track of the all the qorai privacy related preferences. */
@NullMarked
public class QoraiPrivacySettings extends PrivacySettings {
    private static final String BLOCK_ALL_COOKIES_LEARN_MORE_LINK =
            "https://github.com/qorai/qorai-browser/wiki/Block-all-cookies-global-Shields-setting";
    ...
    private static final String PREF_SHIELDS_SAVE_CONTACT_INFO = "qorai_shields_save_contact_info";
    ...
    // qorai Prefs
    private static final String PREF_QORAI_SHIELDS_GLOBALS_SECTION =
            "qorai_shields_globals_section";
    private static final String PREF_CLEAR_DATA_SECTION = "clear_data_section";
    private static final String PREF_QORAI_SOCIAL_BLOCKING_SECTION =
            "qorai_social_blocking_section";
    ...
    private static final String PREF_QORAI_STATS_USAGE_PING = "qorai_stats_usage_ping";
    ...
    private static final String[] NEW_PRIVACY_PREFERENCE_ORDER = {
        PREF_QORAI_SHIELDS_GLOBALS_SECTION, //  shields globals  section
        ...
        PREF_QORAI_SOCIAL_BLOCKING_SECTION, // social blocking section
    ...
    private QoraiDialogPreference mAdsTrakersBlockPref;
    private QoraiDialogPreference mBlockCrosssiteCookies;
    ...
    private QoraiDialogPreference mHttpsUpgradePref;
    private QoraiDialogPreference mFingerprintingProtectionPref;
    private ChromeSwitchPreference mFingerprintingProtection2Pref;
    private QoraiDialogPreference mRequestOtrPref;
    ...
    private @Nullable ChromeSwitchPreference mQoraiStatsUsagePing;
    ...
    private ChromeSwitchPreference mQoraiShieldsSaveContactInfoPref;
    ...
    public void onCreatePreferences(@Nullable Bundle savedInstanceState, @Nullable String rootKey) {
        ...
        // override title
        getActivity().setTitle(R.string.qorai_shields_and_privacy);

        SettingsUtils.addPreferencesFromResource(this, R.xml.qorai_privacy_preferences);
        ...
        if (ChromeFeatureList.isEnabled(QoraiFeatureList.DEBOUNCE)) {
        ...
        boolean httpsByDefaultIsEnabled =
                ChromeFeatureList.isEnabled(QoraiFeatureList.HTTPS_BY_DEFAULT);
        ...
        mHttpsUpgradePref = (QoraiDialogPreference) findPreference(PREF_HTTPS_UPGRADE);
        ...
        mAdsTrakersBlockPref = (QoraiDialogPreference) findPreference(PREF_BLOCK_TRACKERS_ADS);
        ...
        mBlockCrosssiteCookies =
                (QoraiDialogPreference) findPreference(PREF_BLOCK_CROSS_SITE_COOKIES);
        ...
        mFingerprintingProtectionPref =
                (QoraiDialogPreference) findPreference(PREF_FINGERPRINTING_PROTECTION);
        ...
        boolean showStrictFingerprintingMode =
                ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_SHOW_STRICT_FINGERPRINTING_MODE);
        ...
        mRequestOtrPref = (QoraiDialogPreference) findPreference(PREF_REQUEST_OTR);
        ...
        mQoraiShieldsSaveContactInfoPref =
                (ChromeSwitchPreference) findPreference(PREF_SHIELDS_SAVE_CONTACT_INFO);
        mQoraiShieldsSaveContactInfoPref.setOnPreferenceChangeListener(this);
        ...
        boolean forgetFirstPartyStorageIsEnabled =
                ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_FORGET_FIRST_PARTY_STORAGE);
        ...
        if (QoraiLocalState.get().isManagedPreference(QoraiPref.P3A_ENABLED)) {
        ...
        if (QoraiLocalState.get().isManagedPreference(QoraiPref.STATS_REPORTING_ENABLED)) {
            removePreferenceIfPresent(PREF_QORAI_STATS_USAGE_PING);
            mQoraiStatsUsagePing = null;
        } else {
            mQoraiStatsUsagePing =
                    (ChromeSwitchPreference) findPreference(PREF_QORAI_STATS_USAGE_PING);
            mQoraiStatsUsagePing.setOnPreferenceChangeListener(this);
        }

        boolean surveyPanelistEnabled =
                ChromeFeatureList.isEnabled(
                        QoraiFeatureList.QORAI_NTP_BRANDED_WALLPAPER_SURVEY_PANELIST);
        ...
        ChromeClickableSpan chromeClickableSpan =
                new ChromeClickableSpan(
                        getContext().getColor(R.color.qorai_link),
                        result -> {
                            TabUtils.openUrlInCustomTab(
        ...
        mSocialBlockingCategory =
                (PreferenceCategory) findPreference(PREF_QORAI_SOCIAL_BLOCKING_SECTION);
        ...
        if (!ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_GOOGLE_SIGN_IN_PERMISSION)) {
        ...
        if (!ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_ANDROID_SAFE_BROWSING)) {
        ...
        updateQoraiPreferences();
    }

    // used for displaying QoraiDialogPreference
    @Override
    public void onDisplayPreferenceDialog(Preference preference) {
        if (preference instanceof QoraiDialogPreference) {
            ...
            if (getFragmentManager() != null) {
                dialogFragment.show(getFragmentManager(), QoraiPreferenceDialogFragment.TAG);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (PREF_HTTPS_FIRST_MODE.equals(key)) {
            ...
            switch ((int) newValue) {
                case STRICT:
                    QoraiShieldsContentSettings.setHttpsUpgradePref(
            ...
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.DE_AMP_PREF_ENABLED, (boolean) newValue);
        } else if (PREF_DEBOUNCE.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.DEBOUNCE_ENABLED, (boolean) newValue);
            ...
            if (newValue instanceof String) {
                ...
                switch (newStringValue) {
                    case QoraiShieldsContentSettings.BLOCK_RESOURCE:
                        QoraiShieldsContentSettings.setFingerprintingPref(newStringValue);
                        ...
                        break;
                    case QoraiShieldsContentSettings.DEFAULT:
                        QoraiShieldsContentSettings.setFingerprintingPref(newStringValue);
                        mFingerprintingProtectionPref.setSummary(
                                getActivity().getResources().getString(
                        ...
                        break;
                    case QoraiShieldsContentSettings.ALLOW_RESOURCE:
                    default:
                        QoraiShieldsContentSettings.setFingerprintingPref(
                                QoraiShieldsContentSettings.ALLOW_RESOURCE);
                        mFingerprintingProtectionPref.setSummary(
                                getActivity().getResources().getString(
            ...
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setInteger(QoraiPref.REQUEST_OTR_ACTION_OPTION, (int) newValue);
            updateRequestOtrPref();
        } else if (PREF_FINGERPRINT_LANGUAGE.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.REDUCE_LANGUAGE_ENABLED, (boolean) newValue);
            ...
            mBlockCrosssiteCookies.setVisibleEntry(
                    0,
                    ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_BLOCK_ALL_COOKIES_TOGGLE)
                            || (int) newValue == STRICT);
            ...
            switch ((int) newValue) {
                case STRICT:
                    QoraiShieldsContentSettings.setCookiesPref(
                            QoraiShieldsContentSettings.BLOCK_RESOURCE);
            ...
            QoraiShieldsContentSettings.setJavascriptPref((boolean) newValue);
        } else if (PREF_FORGET_FIRST_PARTY_STORAGE.equals(key)) {
            QoraiShieldsContentSettings.setForgetFirstPartyStoragePref((boolean) newValue);
        } else if (PREF_CLOSE_TABS_ON_EXIT.equals(key)) {
            preferencesManager.writeBoolean(
                    QoraiPreferenceKeys.QORAI_CLOSE_TABS_ON_EXIT, (boolean) newValue);
        } else if (PREF_SEND_P3A.equals(key)) {
            QoraiLocalState.get().setBoolean(QoraiPref.P3A_ENABLED, (boolean) newValue);
            QoraiLocalState.commitPendingWrite();
        } else if (PREF_SEND_CRASH_REPORTS.equals(key)) {
            UmaSessionStats.changeMetricsReportingConsent(
                    (boolean) newValue, ChangeMetricsReportingStateCalledFrom.UI_SETTINGS);
        } else if (PREF_QORAI_STATS_USAGE_PING.equals(key)) {
            QoraiLocalState.get().setBoolean(QoraiPref.STATS_REPORTING_ENABLED, (boolean) newValue);
        } else if (PREF_SURVEY_PANELIST.equals(key)) {
            UserPrefs.get(getProfile())
                    .setBoolean(
                            QoraiPref.NEW_TAB_PAGE_SPONSORED_IMAGES_SURVEY_PANELIST,
                            (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_GOOGLE.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.GOOGLE_LOGIN_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_FACEBOOK.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.FB_EMBED_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_TWITTER.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.TWITTER_EMBED_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_LINKEDIN.equals(key)) {
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(QoraiPref.LINKED_IN_EMBED_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_CLEAR_ON_EXIT.equals(key)) {
            preferencesManager.writeBoolean(
                    QoraiPreferenceKeys.QORAI_CLEAR_ON_EXIT, (boolean) newValue);
        } else if (PREF_APP_LINKS.equals(key)) {
            preferencesManager.writeBoolean(PREF_APP_LINKS, (boolean) newValue);
            preferencesManager.writeBoolean(QoraiPrivacySettings.PREF_APP_LINKS_RESET, false);
        } else if (PREF_INCOGNITO_SCREENSHOT.equals(key)) {
            QoraiFeatureUtil.enableFeature(
                    QoraiFeatureList.QORAI_INCOGNITO_SCREENSHOT, (boolean) newValue, false);
            ...
            if (newValue instanceof String) {
                ...
                switch (newStringValue) {
                    case QoraiShieldsContentSettings.BLOCK_RESOURCE:
                        QoraiShieldsContentSettings.setTrackersPref(newStringValue);
                    ...
                    case QoraiShieldsContentSettings.DEFAULT:
                        ...
                        break;
                    default:
                        QoraiShieldsContentSettings.setTrackersPref(
                                QoraiShieldsContentSettings.ALLOW_RESOURCE);
                        mAdsTrakersBlockPref.setSummary(getActivity().getResources().getString(
    ...
    private void updateClearBrowsingFragment() {
        ...
        if (clearBrowsingData != null) {
            clearBrowsingData.setFragment(QoraiClearBrowsingDataFragment.class.getCanonicalName());
        }
    }
    ...
    private void updateQoraiPreferences() {
        ...
        SharedPreferencesManager preferencesManager = ChromeSharedPreferences.getInstance();
        String blockAdTrackersPref = QoraiShieldsContentSettings.getTrackersPref();
        int cookiesBlockPref = preferencesManager.readInt(PREF_BLOCK_CROSS_SITE_COOKIES, 1);
        String fingerprintingPref = QoraiShieldsContentSettings.getFingerprintingPref();
        String httpsUpgradePref = QoraiShieldsContentSettings.getHttpsUpgradePref();

        mBlockScriptsPref.setChecked(QoraiShieldsContentSettings.getJavascriptPref());
        ...
        if (blockAdTrackersPref.equals(QoraiShieldsContentSettings.BLOCK_RESOURCE)) {
            mAdsTrakersBlockPref.setCheckedIndex(0);
            mAdsTrakersBlockPref.setSummary(
                    getActivity().getResources().getString(R.string.block_trackers_ads_option_1));
        } else if (blockAdTrackersPref.equals(QoraiShieldsContentSettings.DEFAULT)) {
            mAdsTrakersBlockPref.setCheckedIndex(1);
            mAdsTrakersBlockPref.setSummary(
                    getActivity().getResources().getString(R.string.block_trackers_ads_option_2));
        } else if (blockAdTrackersPref.equals(QoraiShieldsContentSettings.ALLOW_RESOURCE)) {
            mAdsTrakersBlockPref.setCheckedIndex(2);
            mAdsTrakersBlockPref.setSummary(
        ...
        if (fingerprintingPref.equals(QoraiShieldsContentSettings.BLOCK_RESOURCE)) {
            mFingerprintingProtectionPref.setCheckedIndex(0);
            mFingerprintingProtectionPref.setSummary(
                    getActivity().getResources().getString(R.string.block_fingerprinting_option_1));
            mFingerprintingProtection2Pref.setChecked(true);
        } else if (fingerprintingPref.equals(QoraiShieldsContentSettings.DEFAULT)) {
            mFingerprintingProtectionPref.setCheckedIndex(1);
            mFingerprintingProtectionPref.setSummary(
                    getActivity().getResources().getString(R.string.block_fingerprinting_option_2));
            mFingerprintingProtection2Pref.setChecked(true);
        } else if (fingerprintingPref.equals(QoraiShieldsContentSettings.ALLOW_RESOURCE)) {
            mFingerprintingProtectionPref.setCheckedIndex(2);
            mFingerprintingProtectionPref.setSummary(
        ...
        if (httpsUpgradePref.equals(QoraiShieldsContentSettings.BLOCK_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(0);
            mHttpsUpgradePref.setSummary(
                    getActivity().getResources().getString(R.string.https_upgrade_option_1));
        } else if (httpsUpgradePref.equals(QoraiShieldsContentSettings.BLOCK_THIRDPARTY_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(1);
            mHttpsUpgradePref.setSummary(
                    getActivity().getResources().getString(R.string.https_upgrade_option_2));
        } else if (httpsUpgradePref.equals(QoraiShieldsContentSettings.ALLOW_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(2);
            mHttpsUpgradePref.setSummary(
        ...
        mForgetFirstPartyStoragePref.setChecked(
                QoraiShieldsContentSettings.getForgetFirstPartyStoragePref());
        ...
        if (mSendP3A != null) {
            ...
            mSendP3A.setChecked(QoraiLocalState.get().getBoolean(QoraiPref.P3A_ENABLED));
        }
        ...
        if (mQoraiStatsUsagePing != null) {
            mQoraiStatsUsagePing.setChecked(
                    QoraiLocalState.get().getBoolean(QoraiPref.STATS_REPORTING_ENABLED));
        }

        mSurveyPanelist.setChecked(
                UserPrefs.get(getProfile())
                        .getBoolean(QoraiPref.NEW_TAB_PAGE_SPONSORED_IMAGES_SURVEY_PANELIST));

        mWebrtcPolicy.setSummary(
                webrtcPolicyToString(QoraiPrefServiceBridge.getInstance().getWebrtcPolicy()));

        mClearBrowsingDataOnExit.setChecked(
                preferencesManager.readBoolean(QoraiPreferenceKeys.QORAI_CLEAR_ON_EXIT, false));

        mFingerprntLanguagePref.setChecked(
                UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                        .getBoolean(QoraiPref.REDUCE_LANGUAGE_ENABLED));
        if (mFilterListAndroidHandler != null) {
        ...
        if (mDebouncePref != null) {
            mDebouncePref.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.DEBOUNCE_ENABLED));
        }
        ...
        if (mSocialBlockingGoogle != null) {
            mSocialBlockingGoogle.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.GOOGLE_LOGIN_CONTROL_TYPE));
        }
        if (mSocialBlockingFacebook != null) {
            mSocialBlockingFacebook.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.FB_EMBED_CONTROL_TYPE));
        }
        if (mSocialBlockingTwitter != null) {
            mSocialBlockingTwitter.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.TWITTER_EMBED_CONTROL_TYPE));
        }
        if (mSocialBlockingLinkedin != null) {
            mSocialBlockingLinkedin.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.LINKED_IN_EMBED_CONTROL_TYPE));
        }

        if (mDeAmpPref != null) {
            mDeAmpPref.setChecked(
                    UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                            .getBoolean(QoraiPref.DE_AMP_PREF_ENABLED));
        }

        if (!ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_REQUEST_OTR_TAB)) {
    ...
    private void updateRequestOtrPref() {
        int requestOtrPrefValue =
                UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                        .getInteger(QoraiPref.REQUEST_OTR_ACTION_OPTION);
        if (requestOtrPrefValue == QoraiShieldsContentSettings.ALWAYS) {
        ...
        } else if (requestOtrPrefValue == QoraiShieldsContentSettings.ASK) {
            mRequestOtrPref.setCheckedIndex(1);
            mRequestOtrPref.setSummary(
                    getActivity().getResources().getString(R.string.request_otr_option_2));
        } else if (requestOtrPrefValue == QoraiShieldsContentSettings.NEVER) {
    ...
    private String webrtcPolicyToString(@qoraiWebrtcPolicyPreference.WebrtcPolicy int policy) {
        switch (policy) {
            case QoraiWebrtcPolicyPreference.WebrtcPolicy.DEFAULT:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default);
            case QoraiWebrtcPolicyPreference.WebrtcPolicy.DEFAULT_PUBLIC_AND_PRIVATE_INTERFACES:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default_public_and_private_interfaces);
            case QoraiWebrtcPolicyPreference.WebrtcPolicy.DEFAULT_PUBLIC_INTERFACE_ONLY:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default_public_interface_only);
            case QoraiWebrtcPolicyPreference.WebrtcPolicy.DISABLE_NON_PROXIED_UDP:
                return getActivity().getResources().getString(
