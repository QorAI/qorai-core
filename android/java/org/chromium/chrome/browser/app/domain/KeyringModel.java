...
import org.chromium.qorai_wallet.mojom.KeyringId;
...
public class KeyringModel implements KeyringServiceObserver {
    ...
    public void resetService(KeyringService keyringService, QoraiWalletService qoraiWalletService) {
        synchronized (mLock) {
            mKeyringService = keyringService;
            mQoraiWalletService = qoraiWalletService;
        }
        if (mKeyringService != null && mQoraiWalletService != null) {
            init();
        }
    }
    ...
    /**
     * Restore a Qorai Wallet with a given password, showing only the collection of selected
     ...
     *
     * @param password Given password used to restore the Qorai Wallet.
     * @param recoveryPhrase Recovery phrase used to restore the Qorai Wallet.
    ...
    /**
     * Creates a new Qorai Wallet with a given password, showing only the collection of selected
     ...
     * <p><b>Note:</b> This method must be always called from main UI thread.
     *
     * @param password Given password used to create the new Qorai Wallet.
     * @param availableNetworks All available networks.
     * @param selectedNetworks Collection of selected networks that will be shown.
     * @param jsonRpcService JSON RPC service used to add and hide the networks.
    ...
    @StringDef({QoraiWalletConstants.FILECOIN_MAINNET, QoraiWalletConstants.FILECOIN_TESTNET})
