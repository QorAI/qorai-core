...
import org.chromium.qorai_wallet.mojom.JsonRpcServiceObserver;
...
public class NetworkModel implements JsonRpcServiceObserver {
    ...
    public NetworkModel(
        ...
        mMediatorChainId.setValue(QoraiWalletConstants.MAINNET_CHAIN_ID);
    ...
    private void updateChainId() {
        if (mMode == Mode.WALLET_MODE) {
            ...
            mQoraiWalletService.getNetworkForSelectedAccountOnActiveOrigin(
                    networkInfo -> {
                        if (networkInfo != null) {
                            mMediatorChainId.postValue(networkInfo.chainId);
                        }
    ...
    public void resetServices(
            QoraiWalletService qoraiWalletService, JsonRpcService jsonRpcService) {
        synchronized (mLock) {
            mQoraiWalletService = qoraiWalletService;
            mJsonRpcService = jsonRpcService;
        }
    ...
    public void init() {
        synchronized (mLock) {
            ...
            for (Map.Entry<String, Integer> entry :
                ...
                if (!AndroidUtils.isDebugBuild()
                        && entry.getKey().equals(QoraiWalletConstants.LOCALHOST_CHAIN_ID)) {
    ...
    public void setNetworkWithAccountCheck(
        ...
        mQoraiWalletService.ensureSelectedAccountForChain(
    ...
    public void setNetworkForSelectedAccountOnActiveOrigin(
        ...
        mQoraiWalletService.setNetworkForSelectedAccountOnActiveOrigin(
                networkToBeSetAsSelected.chainId,
                success -> {
