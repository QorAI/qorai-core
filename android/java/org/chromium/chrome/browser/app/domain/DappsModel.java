...
import org.chromium.qorai_wallet.mojom.KeyringServiceObserver;
...
import org.chromium.qorai_wallet.mojom.SolanaSignature;
import org.chromium.qorai_wallet.mojom.TransactionInfo;
import org.chromium.qorai_wallet.mojom.TransactionStatus;
...
public class DappsModel implements KeyringServiceObserver {
    ...
    private final MutableLiveData<QoraiWalletDAppsActivity.ActivityType>
    ...
    public final LiveData<QoraiWalletDAppsActivity.ActivityType> mProcessNextDAppsRequest =
    ...
    public void notifySignSolTransactionsRequestProcessed(
        ...
        mQoraiWalletService.notifySignSolTransactionsRequestProcessed(
                isApproved, request.id, new SolanaSignature[0], null);
        mQoraiWalletService.getPendingSignSolTransactionsRequests(
                requests -> {
                    if (requests.length == 0) {
                        mMutableProcessNextDAppsRequest.postValue(
                                QoraiWalletDAppsActivity.ActivityType.FINISH);
                    } else {
                        mMutableSignSolTransactionsRequests.postValue(
                                new ArrayList<>(Arrays.asList(requests)));
                    }
    ...
    public void processPublicEncryptionKey(String requestId, boolean isApproved) {
        synchronized (mLock) {
            ...
            mQoraiWalletService.notifyGetPublicKeyRequestProcessed(requestId, isApproved);
    ...
    public void processDecryptRequest(String requestId, boolean isApproved) {
        synchronized (mLock) {
            ...
            mQoraiWalletService.notifyDecryptRequestProcessed(requestId, isApproved);
    ...
    public void notifySignMessageRequestProcessed(boolean isApproved, int id, String error) {
        mQoraiWalletService.notifySignMessageRequestProcessed(isApproved, id, null, error);
    }

    public void getPendingSignMessageRequests(Callback1<SignMessageRequest[]> callback) {
        mQoraiWalletService.getPendingSignMessageRequests(callback::call);
    }

    private void updateWalletBadgeVisibilityInternal() {
        if (mQoraiWalletService == null || mJsonRpcService == null || mPendingTxHelper == null) {
            return;
        }
