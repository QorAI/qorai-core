...
import org.chromium.qorai_news.mojom.LocaleInfo;
...
import org.chromium.chrome.browser.settings.QoraiNewsPreferencesDataListener;
...
@NullMarked
public class QoraiNewsUtils {
    public static final int QORAI_NEWS_VIEWD_CARD_TIME = 1000; // milliseconds
    ...
    public static boolean shouldDisplayNewsFeed() {
        return QoraiPrefServiceBridge.getInstance().getShowNews()
    ...
    public static boolean shouldDisplayNewsOptin() {
        return QoraiPrefServiceBridge.getInstance().getShowNews()
                ...
                && ChromeSharedPreferences.getInstance()
                        .readBoolean(QoraiNewsPreferencesV2.PREF_SHOW_OPTIN, true);
    }

    public static void setChannelIcons() {
        sChannelIcons.put("Qorai", R.drawable.ic_channel_qorai);
    ...
    public static void getQoraiNewsSettingsDataPerProfile(Profile profile) {
        ...
        QoraiNewsControllerFactory.getInstance()
                ...
                .then(
                        qoraiNewsController -> {
                            ...
                            QoraiNewsUtils.getQoraiNewsSettingsData(
                                    qoraiNewsController,
                                    null,
                                    () -> {
                                        qoraiNewsController.close();
                                    });
                        });
    }

    public static void getQoraiNewsSettingsData(
            QoraiNewsController qoraiNewsController,
            @Nullable QoraiNewsPreferencesDataListener qoraiNewsPreferencesDataListener,
                ...
                () -> {
                    ...
                    qoraiNewsController.getLocale(
                            (locale) -> {
                                ...
                                getChannels(
                                        qoraiNewsController,
                                        qoraiNewsPreferencesDataListener,
                                        () -> {
                                            getPublishers(
                                                    qoraiNewsController,
                                                    qoraiNewsPreferencesDataListener,
    ...
    private static void getChannels(
            ...
            @Nullable QoraiNewsPreferencesDataListener qoraiNewsPreferencesDataListener,
            final Runnable postAction) {
        qoraiNewsController.getChannels(
                (channels) -> {
                    List<Channel> channelList = new ArrayList<>();
                    ...
                    if (qoraiNewsPreferencesDataListener != null) {
                        qoraiNewsPreferencesDataListener.onChannelReceived();
                    }
                    if (postAction != null) {
                        postAction.run();
                    }
    ...
    private static void getPublishers(
            ...
            @Nullable QoraiNewsPreferencesDataListener qoraiNewsPreferencesDataListener,
            final Runnable postAction) {
        qoraiNewsController.getPublishers(
                (publishers) -> {
                    setPublishers(publishers);
                    if (qoraiNewsPreferencesDataListener != null) {
                        qoraiNewsPreferencesDataListener.onPublisherReceived();
                    }
                    if (postAction != null) {
                        postAction.run();
                    }
    ...
    public static void getSuggestionsSources(
            ...
            @Nullable QoraiNewsPreferencesDataListener qoraiNewsPreferencesDataListener,
            final Runnable postAction) {
        qoraiNewsController.getSuggestedPublisherIds(
                (publisherIds) -> {
                    setSuggestionsIds(Arrays.asList(publisherIds));
                    if (qoraiNewsPreferencesDataListener != null) {
                        qoraiNewsPreferencesDataListener.onSuggestionsReceived();
                    }
                    if (postAction != null) {
                        postAction.run();
                    }
