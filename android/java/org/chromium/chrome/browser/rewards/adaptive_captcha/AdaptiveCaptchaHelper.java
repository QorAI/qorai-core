...
public class AdaptiveCaptchaHelper {
    ...
    public static void startAttestation(String captchaId, String paymentId) {
        PostTask.postTask(TaskTraits.BEST_EFFORT_MAY_BLOCK, () -> {
            ...
            String startAttestationUrl = QoraiRewardsNativeWorker.getInstance().getAttestationURL();
            if (QoraiQAPreferences.shouldVlogRewards()) {
                Log.e(TAG, startAttestationUrl);
            }
            NetworkTrafficAnnotationTag annotation =
                    NetworkTrafficAnnotationTag.createComplete("Qorai attestation api android",
                            "semantics {"
                                    + "  sender: 'Qorai Android app'"
                                    + "  description: "
                                    + "    'This api gets unique value for payment ID'"
                                    + "  trigger: 'When payment id as captcha scheduled'"
                                    + "  data:"
                                    + "    'payment id'"
                                    + "  destination: Qorai grant endpoint"
                                    + "}"
                                    + "policy {"
                                    + "  cookies_allowed: NO"
                ...
                () -> {
                    ...
                    NetworkTrafficAnnotationTag annotation =
                            NetworkTrafficAnnotationTag.createComplete(
                                    "Qorai attestation api android",
                                    "semantics {  sender: 'Qorai Android app'  description:    "
                                        ...
                                        + " token, unique value, package name'  destination: Qorai"
                    ...
                    NetworkTrafficAnnotationTag annotation =
                            NetworkTrafficAnnotationTag.createComplete(
                                    ...
                                    "semantics {  sender: 'Qorai Android app'  description:    "
                                        ...
                                        + " 'payment id'  destination: Qorai grant endpoint}policy"
    ...
    private static void clearCaptchaPrefs() {
        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                .setInteger(QoraiPref.SCHEDULED_CAPTCHA_FAILED_ATTEMPTS, 0);
        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                .setString(QoraiPref.SCHEDULED_CAPTCHA_ID, "");
        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                .setString(QoraiPref.SCHEDULED_CAPTCHA_PAYMENT_ID, "");
        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                .setBoolean(QoraiPref.SCHEDULED_CAPTCHA_PAUSED, false);
    }

    private static void recordFailureAttempt() {
        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                .setInteger(
                        QoraiPref.SCHEDULED_CAPTCHA_FAILED_ATTEMPTS,
                        UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
