...
import org.chromium.chrome.browser.QoraiConfig;
...
/**
 * Activity that handles the first run onboarding experience for new Qorai browser installations.
 * Extends FirstRunActivityBase to provide onboarding flows for: - Setting Qorai as default browser
 ...
 * elements to explain Qorai's key features and privacy-focused approach.
 */
public class WelcomeOnboardingActivity extends FirstRunActivityBase {
    private static final String P3A_URL =
            "https://support.qorai.app/hc/en-us/articles/9140465918093-What-is-P3A-in-Qorai";
    private static final String WDP_LINK =
            "https://www.qorai.com/browser/privacy/#web-discovery-project";
    ...
    private QoraiFirstRunFlowSequencer mFirstRunFlowSequencer;
                            ...
                            case InstallReferrerResponse.OK:
                                try {
                                    ...
                                    if (referrerUrl.equals(
                                            QoraiConstants.DEEPLINK_ANDROID_PLAYLIST)) {
    ...
    private void initViews() {
        ...
        mIvQorai = findViewById(R.id.iv_qorai);
        ...
        if (mBtnPositive != null) {
            QoraiTouchUtils.ensureMinTouchTarget(mBtnPositive);
        }
        if (mCheckboxCrash != null) {
            QoraiTouchUtils.ensureMinTouchTarget(mCheckboxCrash);
        }
        if (mCheckboxP3a != null) {
            QoraiTouchUtils.ensureMinTouchTarget(mCheckboxP3a);
        }
    }
                    ...
                    view -> {
                        if (mCurrentStep == 0 && !isDefaultBrowser()) {
                            ...
                            UserPrefs.get(getProfileProviderSupplier().get().getOriginalProfile())
                                    .setBoolean(QoraiPref.WEB_DISCOVERY_ENABLED, true);
                            nextOnboardingStep();
                        } else {
                            nextOnboardingStep();
                        }
    ...
    private void setDefaultBrowserAndProceedToNextStep() {
        QoraiSetDefaultBrowserUtils.setDefaultBrowser(this, true);
        if (!QoraiSetDefaultBrowserUtils.supportsDefaultRoleManager()) {
            nextOnboardingStep();
        }
    ...
    private boolean isDefaultBrowser() {
        return QoraiSetDefaultBrowserUtils.isQoraiSetAsDefaultBrowser(this);
    }
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == QoraiPermissionUtils.NOTIFICATION_PERMISSION_CODE) {
            nextOnboardingStep();
        }
    }
    ...
    private void handleNotificationPermission() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            QoraiPermissionUtils.showNotificationPermissionDialog(WelcomeOnboardingActivity.this);
        } else {
            nextOnboardingStep();
        }
    }
    ...
    private void handleSetAsDefaultStep() {
        ...
        if (!QoraiSetDefaultBrowserUtils.supportsDefaultRoleManager()) {
            if (mIvQorai != null) {
                mIvQorai.setVisibility(View.VISIBLE);
            }
    ...
    private boolean isWDPSettingAvailable() {
        if (!QoraiConfig.WEB_DISCOVERY_ENABLED) {
            return false;
        }
    ...
    private void handleAnalyticsConsentPage() {
        boolean isP3aManaged = QoraiLocalState.get().isManagedPreference(QoraiPref.P3A_ENABLED);
        ...
        if (!isP3aManaged) {
            ...
            try {
                isP3aEnabled = QoraiLocalState.get().getBoolean(QoraiPref.P3A_ENABLED);
            } catch (Exception e) {
                ...
                mCheckboxP3a.setOnCheckedChangeListener(
                        new CompoundButton.OnCheckedChangeListener() {
                            ...
                            public void onCheckedChanged(
                                ...
                                try {
                                    QoraiLocalState.get()
                                            .setBoolean(QoraiPref.P3A_ENABLED, isChecked);
                                    QoraiLocalState.get()
                                            .setBoolean(QoraiPref.P3A_NOTICE_ACKNOWLEDGED, true);
        ...
        if (mTvDefault != null) {
            ChromeClickableSpan wdpLearnMoreClickableSpan =
                    new ChromeClickableSpan(
                            WelcomeOnboardingActivity.this.getColor(R.color.qorai_blue_tint_color),
                            (textView) -> {
    ...
    public void triggerLayoutInflation() {
        ...
        mFirstRunFlowSequencer =
                new QoraiFirstRunFlowSequencer(getProfileProviderSupplier()) {
                    @Override
    ...
    private void maybeUpdateFirstRunDefaultValues() {
        if (PackageUtils.isFirstInstall(this)) {
            ChromeSharedPreferences.getInstance()
                    .writeBoolean(
                            QoraiPreferenceKeys.QORAI_TAB_GROUPS_ENABLED_DEFAULT_VALUE, false);
        }
    }
}
