...
package org.chromium.chrome.browser.qorai_origin;
...
import org.chromium.qorai.browser.skus.SkusServiceFactory;
import org.chromium.qorai.browser.util.QoraiDomainsUtils;
import org.chromium.qorai.browser.util.ServicesEnvironment;
...
/** Utility class for managing Qorai Origin subscription preferences. */
@NullMarked
public class QoraiOriginSubscriptionPrefs {
    private static final String TAG = "QoraiOriginSubsPrefs";
    ...
    public static void setIsSubscriptionActive(@Nullable Profile profile, boolean value) {
        ...
        UserPrefs.get(profile)
                .setBoolean(QoraiPref.QORAI_ORIGIN_SUBSCRIPTION_ACTIVE_ANDROID, value);
    }
    ...
    public static boolean getIsSubscriptionActive(@Nullable Profile profile) {
        ...
        return UserPrefs.get(profile)
                .getBoolean(QoraiPref.QORAI_ORIGIN_SUBSCRIPTION_ACTIVE_ANDROID);
    }
    ...
    public static void setOriginPurchaseToken(@Nullable Profile profile, String token) {
        ...
        if (prefService.getString(QoraiPref.QORAI_ORIGIN_PURCHASE_TOKEN_ANDROID).equals(token)
                && !prefService.getString(QoraiPref.QORAI_ORIGIN_ORDER_ID_ANDROID).isEmpty()) {
            return;
        }
        // It means we don't have a Play Store subscription anymore or
        // we have a new one.
        resetSubscriptionLinkedStatus(profile);
        prefService.setString(QoraiPref.QORAI_ORIGIN_ORDER_ID_ANDROID, "");
        prefService.setString(QoraiPref.QORAI_ORIGIN_PURCHASE_TOKEN_ANDROID, token);
        if (!token.isEmpty()) {
    ...
    public static void setOriginPackageName(@Nullable Profile profile) {
        ...
        UserPrefs.get(profile)
                .setString(
                        QoraiPref.QORAI_ORIGIN_PACKAGE_NAME_ANDROID,
    ...
    public static void setOriginProductId(@Nullable Profile profile, String productId) {
        ...
        UserPrefs.get(profile).setString(QoraiPref.QORAI_ORIGIN_PRODUCT_ID_ANDROID, productId);
    }
    ...
    public static boolean isSubscriptionLinked(@Nullable Profile profile) {
        ...
        return UserPrefs.get(profile)
                        .getInteger(QoraiPref.QORAI_ORIGIN_SUBSCRIPTION_LINK_STATUS_ANDROID)
                != 0;
    }
    ...
    private static void resetSubscriptionLinkedStatus(@Nullable Profile profile) {
        ...
        UserPrefs.get(profile)
                .setInteger(QoraiPref.QORAI_ORIGIN_SUBSCRIPTION_LINK_STATUS_ANDROID, 0);
    }
    ...
    private static void createFetchOrder(Profile profile, String purchaseToken) {
        PrefService prefService = UserPrefs.get(profile);
        String packageName = prefService.getString(QoraiPref.QORAI_ORIGIN_PACKAGE_NAME_ANDROID);
        String productId = prefService.getString(QoraiPref.QORAI_ORIGIN_PRODUCT_ID_ANDROID);
                ...
                () -> {
                    ...
                    PostTask.postTask(
                            ...
                            () -> {
                                ...
                                String domain =
                                        QoraiDomainsUtils.getServicesDomain(
        ...
        skusService.fetchOrderCredentials(
                ...
                (result) -> {
                    try {
                        ...
                        // Store the order ID
                        UserPrefs.get(profile)
                                .setString(QoraiPref.QORAI_ORIGIN_ORDER_ID_ANDROID, orderId);
                    } finally {
                        skusService.close();
                    }
                });
    ...
    public static void clearOriginSubscriptionPrefs(@Nullable Profile profile) {
        ...
        PrefService prefService = UserPrefs.get(profile);
        prefService.setBoolean(QoraiPref.QORAI_ORIGIN_SUBSCRIPTION_ACTIVE_ANDROID, false);
        prefService.setString(QoraiPref.QORAI_ORIGIN_PURCHASE_TOKEN_ANDROID, "");
        prefService.setString(QoraiPref.QORAI_ORIGIN_PRODUCT_ID_ANDROID, "");
        prefService.setString(QoraiPref.QORAI_ORIGIN_ORDER_ID_ANDROID, "");
        prefService.setString(QoraiPref.QORAI_ORIGIN_PACKAGE_NAME_ANDROID, "");
        resetSubscriptionLinkedStatus(profile);
    }

    /**
     * Opens the Qorai Origin preferences settings screen.
     *
    ...
    public static void openOriginPreferences(Activity activity) {
        ...
        SettingsNavigationFactory.createSettingsNavigation()
                .startSettings(activity, QoraiOriginPreferences.class);
    }
}
