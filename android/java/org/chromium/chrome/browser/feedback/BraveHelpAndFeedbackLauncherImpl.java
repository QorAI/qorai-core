...
public class QoraiHelpAndFeedbackLauncherImpl implements HelpAndFeedbackLauncher {
    private static final String FALLBACK_SUPPORT_URL = "https://community.qorai.app/";
    private static final String SAFE_BROWSING_URL =
            "https://support.qorai.app/hc/articles/15222663599629-Safe-Browsing-in-Qorai";

    public QoraiHelpAndFeedbackLauncherImpl(Profile profile) {}
    ...
    private void launchSafeBrowsingUri(Context context) {
        ...
        // Let Qorai know that this intent is from Qorai, so that it does not close the app when
        // the user presses 'back' button.
        intent.putExtra(Browser.EXTRA_APPLICATION_ID, context.getPackageName());
        intent.putExtra(Browser.EXTRA_CREATE_NEW_TAB, true);
        intent.setPackage(context.getPackageName());
