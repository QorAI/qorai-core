...
import org.chromium.chrome.browser.toolbar.bottom.QoraiBottomControlsCoordinator;
import org.chromium.chrome.browser.toolbar.bottom.QoraiScrollingBottomViewResourceFrameLayout;
...
import org.chromium.chrome.browser.toolbar.top.QoraiTopToolbarCoordinator;
...
public class QoraiToolbarManager extends ToolbarManager
        implements OnSharedPreferenceChangeListener {
    private static final String TAG = "QoraiToolbarManager";
    ...
    // Own members.
    private boolean mIsQoraiBottomControlsVisible;
    private final ObservableSupplier<Boolean> mOmniboxFocusStateSupplier;
    private final OneshotSupplier<LayoutStateProvider> mLayoutStateProviderSupplier;
    private final HomepageManager.HomepageStateListener mQoraiHomepageStateListener;
    private final AppCompatActivity mActivity;
    private final WindowAndroid mWindowAndroid;
    private final CompositorViewHolder mCompositorViewHolder;
    private final Object mLock = new Object();
    private boolean mBottomControlsEnabled;
    private QoraiScrollingBottomViewResourceFrameLayout mBottomControls;
    ...
    public QoraiToolbarManager(
        ...
        if (isToolbarPhone()) {
            updateQoraiBottomControlsVisibility();
            mLayoutStateProviderSupplier.onAvailable(
                    mCallbackController.makeCancelable(this::setLayoutStateProvider));
        }

        mQoraiHomepageStateListener =
                () -> {
                    if (mBottomControlsCoordinatorSupplier != null
                            && mBottomControlsCoordinatorSupplier.get()
                                    instanceof QoraiBottomControlsCoordinator) {
                        ((QoraiBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
                                .updateHomeButtonState();
                    }
                };
        HomepageManager.getInstance().addListener(mQoraiHomepageStateListener);
    }
    ...
    public void enableBottomControls() {
        ...
        synchronized (mLock) {
            ...
            mBottomControls =
                    (QoraiScrollingBottomViewResourceFrameLayout) bottomControlsStub.inflate();
            ...
            var bottomControlsCoordinator =
                    new QoraiBottomControlsCoordinator(
                            mLayoutStateProviderSupplier,
            ...
            updateQoraiBottomControlsVisibility();
            if (mIsQoraiBottomControlsVisible) {
    ...
    public void initializeWithNative(
        ...
        if (isToolbarPhone() && BottomToolbarConfiguration.isQoraiBottomControlsEnabled()) {
        ...
        assert toolbarLayout instanceof QoraiToolbarLayoutImpl
                : "Something has changed in the upstream!";
        if (toolbarLayout instanceof QoraiToolbarLayoutImpl) {
            final QoraiToolbarLayoutImpl qoraiToolbarLayout =
                    (QoraiToolbarLayoutImpl) toolbarLayout;
            qoraiToolbarLayout.setTabModelSelector(mTabModelSelectorSupplier.get());
        }
    }
    ...
    public void destroy() {
        ...
        super.destroy();
        HomepageManager.getInstance().removeListener(mQoraiHomepageStateListener);
    }

    private void recordNewTabClick() {
        if (!mIncognitoStateProvider.isIncognitoSelected()) {
            if (mActivity instanceof QoraiActivity) {
                MiscAndroidMetrics miscAndroidMetrics =
                        ((QoraiActivity) mActivity).getMiscAndroidMetrics();
    ...
    protected void onOrientationChange() {
        ...
        if (mBottomControlsCoordinatorSupplier != null
            ...
            boolean isQoraiBottomControlsVisible =
                    mCurrentOrientation != Configuration.ORIENTATION_LANDSCAPE;
            setQoraiBottomControlsVisible(isQoraiBottomControlsVisible);
        }
        ...
        if (mActivity instanceof QoraiActivity) {
            ((QoraiActivity) mActivity).updateBottomSheetPosition(mCurrentOrientation);
        }
    }
    ...
    private void setQoraiBottomControlsVisible(boolean visible) {
        mIsQoraiBottomControlsVisible = visible;
        boolean isMenuFromBottom =
                (mIsQoraiBottomControlsVisible
                                && BottomToolbarConfiguration.isQoraiBottomControlsEnabled())
                        || BottomToolbarConfiguration.isToolbarBottomAnchored();
        QoraiMenuButtonCoordinator.setMenuFromBottom(isMenuFromBottom);

        if (mToolbar instanceof QoraiTopToolbarCoordinator) {
            ((QoraiTopToolbarCoordinator) mToolbar).onBottomControlsVisibilityChanged(visible);
        }
        if (mBottomControlsCoordinatorSupplier != null
                && mBottomControlsCoordinatorSupplier.get()
                        instanceof QoraiBottomControlsCoordinator) {
    ...
    private void updateQoraiBottomControlsVisibility() {
        boolean isQoraiBottomControlsVisible =
                BottomToolbarConfiguration.isQoraiBottomControlsEnabled()
    ...
    private boolean isToolbarPhone() {
        assert (mToolbar instanceof QoraiTopToolbarCoordinator);
        return mToolbar instanceof QoraiTopToolbarCoordinator
                && ((QoraiTopToolbarCoordinator) mToolbar).isToolbarPhone();
    }

    private ObservableSupplier<Integer> getConstraintsProxy() {
        if (mToolbar instanceof QoraiTopToolbarCoordinator) {
            return ((QoraiTopToolbarCoordinator) mToolbar).getConstraintsProxy();
        }
    ...
    public void onSharedPreferenceChanged(
        ...
        if (ChromePreferenceKeys.TOOLBAR_TOP_ANCHORED.equals(key)) {
            if (sharedPreferences.getBoolean(
                    QoraiPreferenceKeys.QORAI_BOTTOM_TOOLBAR_ENABLED_KEY, true)) {
                updateQoraiBottomControlsVisibility();
            }
        }
    }
                ...
                new LayoutStateProvider.LayoutStateObserver() {
                    ...
                    public void onStartedShowing(@LayoutType int layoutType) {
                        if (layoutType == LayoutType.TAB_SWITCHER
                                && BottomToolbarConfiguration.isToolbarBottomAnchored()) {
                            QoraiMenuButtonCoordinator.setMenuFromBottom(false);
                        }
                    }
                    ...
                    public void onStartedHiding(@LayoutType int layoutType) {
                        if (layoutType == LayoutType.TAB_SWITCHER
                                && BottomToolbarConfiguration.isToolbarBottomAnchored()) {
                            QoraiMenuButtonCoordinator.setMenuFromBottom(true);
                        }
                    }
                };
