...
public class WalletUtils {
    ...
    public static boolean accountIdsEqual(@Nullable AccountInfo left, @Nullable AccountInfo right) {
        ...
        // This is a fix to avoid https://github.com/qorai/qorai-browser/issues/43261
     ...
     *     QoraiActivity#QORAI_WALLET_BASE_URL} before opening a new tab. Otherwise, it tries to
     *     refresh an old tab whose URL starts with @link QoraiActivity#QORAI_WALLET_BASE_URL}.
     */
    public static void openWebWallet(final boolean forceNewTab) {
        try {
            QoraiActivity activity = QoraiActivity.getQoraiActivity();
            if (forceNewTab) {
                activity.closeAllTabsByOrigin(QoraiActivity.QORAI_WALLET_ORIGIN);
            }

            activity.openNewOrRefreshExistingTab(
                    QoraiActivity.QORAI_WALLET_ORIGIN, QoraiActivity.QORAI_WALLET_URL);
    ...
    /** Closes all Wallet tabs that contains the base URL `qorai://wallet`. */
