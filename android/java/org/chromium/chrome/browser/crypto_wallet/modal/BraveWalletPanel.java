...
import org.chromium.base.QoraiUrlConstants;
import org.chromium.base.Log;
import org.chromium.base.SysUtils;
import org.chromium.qorai_wallet.mojom.AccountInfo;
import org.chromium.qorai_wallet.mojom.AllAccountsInfo;
...
import org.chromium.chrome.browser.crypto_wallet.QoraiWalletProviderDelegateImplHelper;
...
public class QoraiWalletPanel implements DialogInterface {
    private static final String TAG = "QoraiWalletPanel";
    ...
    public QoraiWalletPanel(
        ...
        try {
            ...
            Log.e(TAG, "QoraiWalletPanel Constructor", e);
        }
        ...
        mActivity = QoraiActivity.getChromeTabbedActivity();
        ...
        mAllAccountsInfoObserver =
                allAccountsInfo -> {
                    ...
                    final QoraiWalletService qoraiWalletService =
                            mWalletModel.getQoraiWalletService();
                    if (qoraiWalletService == null) {
                        return;
                    }
    ...
    private boolean handleMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.action_lock_wallet) {
            ...
            try {
                ...
                activity.openQoraiConnectedSitesSettings();
                dismiss();
            } catch (QoraiActivity.QoraiActivityNotFoundException e) {
            ...
            try {
                QoraiActivity activity = QoraiActivity.getQoraiActivity();
                activity.openQoraiWalletSettings();
                dismiss();
            } catch (QoraiActivity.QoraiActivityNotFoundException e) {
    ...
    private void updateSolanaConnected(AccountInfo selectedAccount) {
        try {
            ...
            if (activity.getActivityTab() != null) {
                QoraiWalletProviderDelegateImplHelper.isSolanaConnected(
        ...
        if (showSolStatus) {
            ...
            try {
                ...
                if (activity.getActivityTab() != null) {
                    ...
                    if (!lastCommittedUrl.getScheme().equals(QoraiUrlConstants.QORAI_SCHEME)
    ...
    private void setUpViews() {
        ...
        mPopupView = (ViewGroup) inflater.inflate(R.layout.qorai_wallet_panel_layout, null);
        if (BottomToolbarConfiguration.isToolbarBottomAnchored()) {
    ...
    private final View.OnClickListener mOnConnectedAccountClick =
            v -> {
                try {
                    QoraiActivity activity = QoraiActivity.getQoraiActivity();
                    activity.openQoraiWalletDAppsActivity(
                            QoraiWalletDAppsActivity.ActivityType.CONNECT_ACCOUNT);
                } catch (QoraiActivity.QoraiActivityNotFoundException e) {
