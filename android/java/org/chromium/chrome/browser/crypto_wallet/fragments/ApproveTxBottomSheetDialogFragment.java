...
import org.chromium.qorai_wallet.mojom.ProviderErrorUnion;
...
public class ApproveTxBottomSheetDialogFragment extends WalletBottomSheetDialogFragment {
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof TransactionListener) {
            ...
            throw new IllegalStateException("Host activity must implement TransactionListener.");
        }
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (mTxInfo.txType
            ...
            Spanned associatedSPLTokenAccountInfo =
                    Utils.createSpanForSurroundedPhrase(
                            requireContext(),
                            R.string.qorai_wallet_confirm_transaction_account_creation_fee,
                            (v) -> {
        ...
        final AccountId accountId = mTxInfo.fromAccountId;
        final String txInfoId = mTxInfo.id;
        getKeyringService()
                .getAllAccounts(
                        allAccounts -> {
                            ...
                            AccountInfo txAccountInfo = Utils.findAccount(accounts, accountId);
                                    ...
                                    tokenList -> {
                                        SolanaTransactionsGasHelper solanaTransactionsGasHelper =
                                                new SolanaTransactionsGasHelper(
                                                        (QoraiWalletBaseActivity) getActivity(),
                                                ...
                                                () -> {
                                                    HashMap<String, Long> perTxFee =
                                                            solanaTransactionsGasHelper
                                                                    .getPerTxFee();
                                                    if (perTxFee.get(txInfoId) != null) {
                                                        Long solTxFee = perTxFee.get(txInfoId);
                                                        if (solTxFee != null) {
            ...
            next.setOnClickListener(
                    v -> {
                        if (mTransactionListener != null) {
                            mTransactionListener.onNextTransaction();
                        }
                    });
        ...
        LiveDataUtil.observeOnce(
                ...
                allNetworks -> {
                    Utils.getTxExtraInfo(
                            new WeakReference<>((QoraiWalletBaseActivity) getActivity()),
                            TokenUtils.TokenType.ALL,
                            allNetworks,
                            txNetwork,
                            accounts,
    ...
    private void refreshListContentUi() {
        if (mPendingTransactions == null || mRejectAllTx == null) {
            return;
        }
        mRejectAllTx.setText(
                getString(
                        R.string.qorai_wallet_queue_reject_all,
    ...
    private ParsedTransaction fillAssetDependentControls(
        ...
        if (parsedTx.getType() == TransactionType.ERC20_APPROVE) {
            ...
            txType.setText(R.string.qorai_wallet_approve_transaction);
            ...
            txType.setText(getResources().getString(R.string.qorai_wallet_shielding));
        } else {
        ...
        if (parsedTx.isSolanaDappTransaction) {
            ...
            if (txInfo.txType == TransactionType.ERC721_TRANSFER_FROM
        ...
        setupPager(txInfo, view, txNetwork, accounts, assetPrices, fullTokenList);
        return parsedTx;
    }
