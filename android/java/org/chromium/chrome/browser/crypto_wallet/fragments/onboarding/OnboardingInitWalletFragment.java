...
/** Initial onboarding fragment to setup Qorai Wallet. */
public class OnboardingInitWalletFragment extends BaseOnboardingWalletFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        newWallet.setOnClickListener(
                v -> {
                    ...
                    checkOnQoraiActivity(true, false);
                    if (mOnNextPage != null) {
        ...
        restoreWallet.setOnClickListener(
                v -> {
                    ...
                    checkOnQoraiActivity(false, true);
                    if (mOnNextPage != null) {
                        // Add a little delay for a smooth ripple effect animation.
                        PostTask.postDelayedTask(
    ...
    // https://github.com/qorai/qorai-browser/issues/27887
    // is done.
    private void checkOnQoraiActivity(boolean setupAction, boolean restoreAction) {
        try {
            QoraiActivity.getQoraiActivity();
        } catch (QoraiActivity.QoraiActivityNotFoundException e) {
            Log.e(TAG, "checkOnQoraiActivity " + e);
            Intent intent = new Intent(getActivity(), ChromeTabbedActivity.class);
            intent.putExtra(QoraiWalletActivity.RESTART_WALLET_ACTIVITY, true);
            intent.putExtra(QoraiWalletActivity.RESTART_WALLET_ACTIVITY_SETUP, setupAction);
            intent.putExtra(QoraiWalletActivity.RESTART_WALLET_ACTIVITY_RESTORE, restoreAction);
