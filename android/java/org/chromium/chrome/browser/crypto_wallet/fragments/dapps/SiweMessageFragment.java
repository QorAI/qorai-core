...
import org.chromium.qorai_wallet.mojom.SignMessageRequest;
import org.chromium.qorai_wallet.mojom.SiweMessage;
...
public class SiweMessageFragment extends WalletBottomSheetDialogFragment {
    ...
    public View onCreateView(
        ...
        mBtSign.setText(R.string.qorai_wallet_sign_in_message_positive_button_action);
        return view;
    }
    ...
    private void updateDetails(String chainId, AccountId accountId) {
        ...
        if (mCurrentSignMessageRequest != null && mCurrentSignMessageRequest.originInfo != null) {
            ...
            builder.append(
                    getString(
                            R.string.qorai_wallet_sign_in_with_qorai_wallet_message,
                            mCurrentSignMessageRequest.originInfo.eTldPlusOne));
            builder.append(separator);
            builder.append(
                    AndroidUtils.createClickableSpanString(
                            requireContext(),
                            R.string.qorai_wallet_see_details,
    ...
    private void showSiweDetails() {
        ...
        List<TwoLineItemRecyclerViewAdapter.TwoLineItem> items = new ArrayList<>();
        addDetail(items, R.string.qorai_wallet_origin, getOriginJson(mSiweMessageData.origin));
        addDetail(items, R.string.qorai_wallet_address, mSiweMessageData.address);
        addDetail(items, R.string.qorai_wallet_statement, mSiweMessageData.statement);
        addDetail(items, R.string.qorai_wallet_uri, mSiweMessageData.uri.url);
        addDetail(items, R.string.qorai_wallet_version, Integer.toString(mSiweMessageData.version));
        addDetail(items, R.string.qorai_wallet_chain_id, Long.toString(mSiweMessageData.chainId));
        addDetail(items, R.string.qorai_wallet_nonce, mSiweMessageData.nonce);
        addDetail(items, R.string.qorai_wallet_issued_at, mSiweMessageData.issuedAt);
        addDetail(items, R.string.qorai_wallet_expiration_time, mSiweMessageData.expirationTime);
        addDetail(items, R.string.qorai_wallet_not_before, mSiweMessageData.notBefore);
        addDetail(items, R.string.qorai_wallet_request_id, mSiweMessageData.requestId);
