    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (city != null) {
            ...
            boolean isCityPrecision =
                    QoraiVpnPrefUtils.getRegionPrecision()
                            .equals(QoraiVpnConstants.REGION_PRECISION_CITY);
            boolean regionMatches = QoraiVpnPrefUtils.getRegionName().equals(mRegion.name);
            boolean cityMatches = QoraiVpnPrefUtils.getRegionName().equals(city.name);
            boolean isoCodeMatches =
                    isCountryPrecision
                            && !regionMatches
                            && !cityMatches
                            && QoraiVpnPrefUtils.getRegionIsoCode().equals(mRegion.countryIsoCode)
                            && position == 0;
