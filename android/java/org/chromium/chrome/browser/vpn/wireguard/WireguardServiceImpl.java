...
public class WireguardServiceImpl extends SplitCompatService.Impl
    ...
    private static final int QORAI_VPN_NOTIFICATION_ID = 801;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        getService().startForeground(QORAI_VPN_NOTIFICATION_ID, getQoraiVpnNotification(""));
    ...
    private Notification getQoraiVpnNotification(String notificationText) {
        ...
        NotificationCompat.Builder notificationBuilder =
                new NotificationCompat.Builder(mContext, QoraiActivity.CHANNEL_ID);
    ...
    private void updateVpnNotification(String notificationText) {
        Notification notification = getQoraiVpnNotification(notificationText);
        NotificationManager mNotificationManager =
                (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);
        mNotificationManager.notify(QORAI_VPN_NOTIFICATION_ID, notification);
    }

    private void recordSessionTimes() {
        long sessionStartTimeMs = QoraiVpnPrefUtils.getSessionStartTimeMs();
        long sessionEndTimeMs = QoraiVpnPrefUtils.getSessionEndTimeMs();
        ...
        if (sessionStartTimeMs < 0) {
            QoraiVpnPrefUtils.setSessionStartTimeMs(currTimeMs);
        }
        ...
        if (currDate.get(Calendar.YEAR) != sessionEndTimeCal.get(Calendar.YEAR)
            ...
            QoraiVpnPrefUtils.setSessionEndTimeMs(currTimeMs);
        }
    }
