...
@NullMarked
public class QoraiBookmarkManagerCoordinator extends BookmarkManagerCoordinator {
    // Overridden Chromium's BookmarkManagerCoordinator.mMediator
    private @Nullable BookmarkManagerMediator mMediator;

    public QoraiBookmarkManagerCoordinator(
            Context context,
            boolean isDialogUi,
    ...
    public void setWindow(ActivityWindowAndroid window) {
        if (mMediator instanceof QoraiBookmarkManagerMediator) {
            ((QoraiBookmarkManagerMediator) mMediator).setWindow(window);
        }
    }
}
