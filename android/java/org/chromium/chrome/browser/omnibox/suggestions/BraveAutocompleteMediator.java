...
@NullMarked
class QoraiAutocompleteMediator extends AutocompleteMediator
        implements QoraiSuggestionHost, QoraiQoraAutocompleteDelegate {
    private static final String AUTOCOMPLETE_ENABLED = "qorai.autocomplete_enabled";
    private static final String QORA_START_WORD_UPPER_CASE = "QORA";
    ...
    @Override
    public void removeQoraiSearchSuggestion() {
        if (mDropdownViewInfoListManager instanceof QoraiDropdownItemViewInfoListManager) {
            ((QoraiDropdownItemViewInfoListManager) mDropdownViewInfoListManager)
                    .removeQoraiSearchSuggestion();
        }
    }
    ...
    void initDefaultProcessors() {
        if (mDropdownViewInfoListBuilder != null
                && mDropdownViewInfoListBuilder instanceof QoraiDropdownItemViewInfoListBuilder) {
            ((QoraiDropdownItemViewInfoListBuilder) mDropdownViewInfoListBuilder)
                    .setAutocompleteDelegate(mDelegate);
            ((QoraiDropdownItemViewInfoListBuilder) mDropdownViewInfoListBuilder)
                    .setQoraAutocompleteDelegate(this);
        }
    ...
    @Override
    public boolean isQoraEnabled() {
        return QoraiQoraPrefUtils.isQoraEnabled();
    }

    @Override
    public void openQoraQuery(WebContents webContents, String conversationUuid, String query) {
        mDelegate.clearOmniboxFocus();
        QoraiQoraUtils.openQoraQuery(webContents, conversationUuid, query, true);
    }
    ...
    void onVoiceResults(@Nullable List<VoiceRecognitionHandler.VoiceResult> voiceResults) {
        ...
        if (tab != null && voiceResults != null) {
            ...
            if (topResult != null) {
                ...
                // Check if the query starts with the start word for Qora.
                if (topResultQuery
                        .toUpperCase(Locale.ENGLISH)
                        .startsWith(QORA_START_WORD_UPPER_CASE)) {
                    // Remove the start word from the query and process it.
                    topResultQuery =
                            topResultQuery.substring(QORA_START_WORD_UPPER_CASE.length()).trim();
                    if (tab.getWebContents() != null) {
                        openQoraQuery(tab.getWebContents(), "", topResultQuery);
                    }

                    // Clear the voice results to prevent the query from being processed by Chromium
                    // since it's already handled by Qora.
                    voiceResults.clear();
                }
            }
        }
    ...
    // in multi-window/split-mode or when switch away from Qorai.
