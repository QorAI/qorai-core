...
public class QoraiQoraMojomHelper implements ConnectionErrorHandler {
    private long mNativeAIChatCMHelperAndroid;
    AiChatSettingsHelper mAIChatAndroidHelper;

    private static final Object sLock = new Object();
    private static QoraiQoraMojomHelper sInstance;

    public static QoraiQoraMojomHelper getInstance(BrowserContextHandle browserContextHandle) {
        synchronized (sLock) {
            if (sInstance == null) {
                sInstance = new QoraiQoraMojomHelper(browserContextHandle);
            }
        }
        return sInstance;
    }

    private QoraiQoraMojomHelper(BrowserContextHandle browserContextHandle) {
        mNativeAIChatCMHelperAndroid = QoraiQoraMojomHelperJni.get().init(browserContextHandle);
        long nativeHandle =
                QoraiQoraMojomHelperJni.get()
    ...
    private void destroy() {
        synchronized (sLock) {
            ...
            QoraiQoraMojomHelperJni.get().destroy(mNativeAIChatCMHelperAndroid);
