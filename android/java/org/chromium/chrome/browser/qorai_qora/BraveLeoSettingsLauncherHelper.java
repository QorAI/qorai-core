...
/** Launches Qorai Qora settings page or subscription. */
public class QoraiQoraSettingsLauncherHelper {
    private static final String ACCOUNT_PAGE_URL = "https://account.qorai.com/";
    private static SettingsNavigation sLauncher;

    @CalledByNative
    private static void showQoraiQoraSettings(WebContents webContents) {
        Context context = webContents.getTopLevelNativeWindow().getActivity().get();
        if (context == null) {
            return;
        }
        getLauncher().startSettings(context, QoraiQoraPreferences.class);
    }

    @CalledByNative
    private static void goPremium(WebContents webContents) {
        QoraiQoraUtils.goPremium(webContents.getTopLevelNativeWindow().getActivity().get());
    }

    @CalledByNative
    private static void managePremium(WebContents webContents) {
        if (QoraiQoraPrefUtils.getIsSubscriptionActive(Profile.fromWebContents(webContents))) {
            QoraiQoraUtils.openManageSubscription();
        } else {
            TabUtils.openURLWithQoraiActivity(ACCOUNT_PAGE_URL);
        }
    }

    @CalledByNative
    private static void openURL(String url) {
        TabUtils.openURLWithQoraiActivity(url);
    }
