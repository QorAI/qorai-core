...
/**
 * Helper to interact with native methods. Check qorai_youtube_script_injector_native_helper.{h|cc}.
 */
@JNINamespace("youtube_script_injector")
@NullMarked
public class QoraiYouTubeScriptInjectorNativeHelper {
    private static final String TAG = "YouTubeNativeHelper";

    public static void setFullscreen(WebContents webContents) {
        QoraiYouTubeScriptInjectorNativeHelperJni.get().setFullscreen(webContents);
    }

    public static boolean hasFullscreenBeenRequested(WebContents webContents) {
        return QoraiYouTubeScriptInjectorNativeHelperJni.get()
    ...
    public static void enterPictureInPicture(WebContents webContents) {
        ...
        if (windowAndroid != null) {
            ...
            if (activity instanceof final QoraiActivity qoraiActivity) {
                // Resume the media session when the transition completes.
                qoraiActivity.resumeMediaSession(true);
                try {
                    qoraiActivity.enterPictureInPictureMode(
                            new PictureInPictureParams.Builder().build());
                } catch (IllegalStateException | IllegalArgumentException e) {
                    Log.e(TAG, "Error entering picture in picture mode.", e);
                    qoraiActivity.resumeMediaSession(false);
                }
            }
        }
    }
