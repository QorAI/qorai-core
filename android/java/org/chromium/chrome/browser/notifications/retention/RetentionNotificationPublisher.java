...
public class RetentionNotificationPublisher extends BroadcastReceiver {
    private static final String NOTIFICATION_CHANNEL_NAME = "qorai";
            ...
            if (action != null && action.equals(RETENTION_NOTIFICATION_ACTION)) {
                if (qoraiActivity != null) {
                    ...
                    switch (notificationType) {
                        ...
                        case RetentionNotificationUtil.EVERY_SUNDAY:
                            qoraiActivity.checkForQoraiStats();
                            break;
                        case RetentionNotificationUtil.DAY_6:
                            if (qoraiActivity.getActivityTab() != null
                                    && qoraiActivity.getActivityTab().getUrl().getSpec() != null
                                    && !UrlUtilities.isNtpUrl(
                                            qoraiActivity.getActivityTab().getUrl().getSpec())) {
                                qoraiActivity.getTabCreator(false).launchUrl(
                        ...
                        case RetentionNotificationUtil.DORMANT_USERS_DAY_40:
                            if (System.currentTimeMillis()
                                ...
                                qoraiActivity.showDormantUsersEngagementDialog(notificationType);
                            } else {
                ...
                switch (notificationType) {
                    ...
                    case RetentionNotificationUtil.DAY_35:
                        ...
                        try {
                            QoraiRewardsNativeWorker rewardsNativeWorker =
                        ...
                        break;
                    case RetentionNotificationUtil.EVERY_SUNDAY:
                        if (OnboardingPrefManager.getInstance().isQoraiStatsNotificationEnabled()) {
                            createNotification(context, intent);
                        }
                        break;
                    case RetentionNotificationUtil.DORMANT_USERS_DAY_14:
                    ...
                    case RetentionNotificationUtil.DORMANT_USERS_DAY_40:
                        if (System.currentTimeMillis()
                                        > OnboardingPrefManager.getInstance()
                                                .getDormantUsersNotificationTime(notificationType)
                                && !QoraiSetDefaultBrowserUtils.isQoraiSetAsDefaultBrowser(
                                        qoraiActivity)) {
                            createNotification(context, intent);
                        } else {
                            RetentionNotificationUtil.scheduleNotificationWithTime(
