...
import org.chromium.chrome.browser.cosmetic_filters.QoraiCosmeticFiltersUtils;
...
import org.chromium.components.version_info.QoraiVersionConstants;
...
/** Object responsible for handling the creation, showing, hiding of the QoraiShields menu. */
public class QoraiShieldsHandler implements QoraiRewardsHelper.LargeIconReadyCallback {
    private static final String TAG = "QoraiShieldsHandler";
    ...
    private QoraiShieldsMenuObserver mMenuObserver;
    ...
    private SwitchCompat mQoraiShieldsBlockingScriptsSwitch;
    private OnCheckedChangeListener mQoraiShieldsBlockingScriptsChangeListener;
    private SwitchCompat mQoraiShieldsForgetFirstPartyStorageSwitch;
    private OnCheckedChangeListener mQoraiShieldsForgetFirstPartyStorageChangeListener;
    private SwitchCompat mFingerprintingSwitch;
    private OnCheckedChangeListener mQoraiShieldsFingerprintingChangeListener;
    ...
    private QoraiRewardsHelper mIconFetcher;
    ...
    /**
     * Constructs a QoraiShieldsHandler object.
     *
     * @param context Context that is using the QoraiShieldsMenu.
     */
    public QoraiShieldsHandler(Context context) {
    ...
    public void addStat(int tabId, String blockType, String subResource) {
        ...
        if (blockType.equals(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_ADS)) {
            blockersInfo.mAdsBlocked++;
        } else if (blockType.equals(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_TRACKERS)) {
            blockersInfo.mTrackersBlocked++;
        } else if (blockType.equals(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_JAVASCRIPTS)) {
    ...
    public void clearQoraiShieldsCount(int tabId) {
        mTabsStat.put(tabId, new BlockersInfo());
    }

    public void addObserver(QoraiShieldsMenuObserver menuObserver) {
        mMenuObserver = menuObserver;
    }

    private void ensureInitializedForCustomTabs() {
        if (mHardwareButtonMenuAnchor == null && mContext == null) {
            mContext = QoraiActivity.getCustomTabActivity();
            mHardwareButtonMenuAnchor = ((Activity) mContext).findViewById(R.id.menu_anchor_stub);
        }
    }
    ...
    public void show(View anchorView, Tab tab) {
        ...
        mIconFetcher = new QoraiRewardsHelper(tab);
    ...
    private PopupWindow showPopupMenu(View anchorView) {
        ...
        mPopupView = inflater.inflate(R.layout.qorai_shields_main_layout, null);
        setUpViews();
                        ...
                        new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    ...
                                    // It means that the Qorairy Panel was destroyed during the
    ...
    public void hideQoraiShieldsMenu() {
        if (isShowing()) {
    ...
    private void initViews() {
        ...
        mSecondaryLayout = mPopupView.findViewById(R.id.qorai_shields_secondary_layout_id);
        mThirdLayout = mPopupView.findViewById(R.id.qorai_shields_third_layout_id);
        mAboutLayout = mPopupView.findViewById(R.id.qorai_shields_about_layout_id);
        mToggleLayout = mPopupView.findViewById(R.id.qorai_shields_toggle_layout_id);
        mSiteBlockCounterText = mPopupView.findViewById(R.id.site_block_count_text);

        mReportBrokenSiteLayout = mPopupView.findViewById(R.id.qorai_shields_report_site_layout_id);
        mReportErrorPageLayout =
                mPopupView.findViewById(R.id.qorai_shields_report_error_page_layout_id);
        mThankYouLayout = mPopupView.findViewById(R.id.qorai_shields_thank_you_layout_id);
        mPrivacyReportLayout = mPopupView.findViewById(R.id.qorai_shields_privacy_report_layout_id);
    ...
    private void setUpMainLayout() {
        ...
        String favIconURL = mQoraiRewardsNativeWorker.getPublisherFavIconURL(mTabId);
        ...
        shareImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mMainLayout.setVisibility(View.GONE);
                try {
                    if (QoraiStatsUtil.hasWritePermission(QoraiActivity.getQoraiActivity())) {
        ...
        mThirdLayout.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        QoraiShieldsContentSettings.resetCosmeticFilter(mUrlSpec);
                        hideQoraiShieldsMenu();
    ...
    private void setUpSecondaryLayout() {
        TextView shieldsText = mSecondaryLayout.findViewById(R.id.qorai_shields_text);
    ...
    private void setupDetailsLayouts() {
        ...
        detailsLayouts.add(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_TRACKERS);
        detailsLayouts.add(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_COOKIES);
        if (ChromeFeatureList.isEnabled(QoraiFeatureList.HTTPS_BY_DEFAULT)) {
            detailsLayouts.add(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_HTTPS_UPGRADE);
        } else {
            mPopupView
                    .findViewById(R.id.qorai_shields_secondary_https_upgrade_layout_id)
                    .setVisibility(View.GONE);
        }
        if (ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_SHOW_STRICT_FINGERPRINTING_MODE)) {
            detailsLayouts.add(QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_FINGERPRINTING);
        } else {
            mPopupView
                    .findViewById(R.id.qorai_shields_fingerprinting_layout_id)
                    .setVisibility(View.GONE);
        }
        ...
        for (final String layout : detailsLayouts) {
            switch (layout) {
                case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_HTTPS_UPGRADE:
                    layoutId = R.id.qorai_shields_https_upgrade_layout_id;
                    mSecondaryLayoutId = R.id.qorai_shields_secondary_https_upgrade_layout_id;
                    ...
                    break;
                case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_TRACKERS:
                    layoutId = R.id.qorai_shields_block_cross_trackers_layout_id;
                    mSecondaryLayoutId = R.id.qorai_shields_cross_site_trackers_layout_id;
                    ...
                    break;
                case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_FINGERPRINTING:
                    layoutId = R.id.qorai_shields_block_fingerprinting_layout_id;
                    mSecondaryLayoutId = R.id.qorai_shields_fingerprinting_layout_id;
                    ...
                    break;
                case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_COOKIES:
                    layoutId = R.id.qorai_shields_block_cookies_layout_id;
                    mSecondaryLayoutId = R.id.qorai_shields_cookies_layout_id;
            ...
            String settingOption =
                    QoraiShieldsContentSettings.getShieldsValue(mProfile, mUrlSpec, layout);
            if (settingOption.equals(QoraiShieldsContentSettings.BLOCK_RESOURCE)) {
                mBlockShieldsOption1.setChecked(true);
            } else if (settingOption.equals(QoraiShieldsContentSettings.ALLOW_RESOURCE)) {
                ...
                switch (layout) {
                    ...
                    // fall through
                    case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_TRACKERS:
                        checkOption2 = settingOption.equals(QoraiShieldsContentSettings.DEFAULT)
                                || settingOption.equals(
                                        QoraiShieldsContentSettings.BLOCK_THIRDPARTY_RESOURCE);
                        break;
                    case QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_FINGERPRINTING:
                        checkOption2 = settingOption.equals(QoraiShieldsContentSettings.DEFAULT);
                        break;
                        ...
                        public void onCheckedChanged(RadioGroup group, int checkedId) {
                            ...
                            if (isChecked) {
                                if (checkedId == R.id.option1) {
                                    QoraiShieldsContentSettings.setShieldsValue(mProfile, mUrlSpec,
                                            layout, QoraiShieldsContentSettings.BLOCK_RESOURCE,
                                            false);
                                    ...
                                    QoraiShieldsContentSettings.setShieldsValue(mProfile, mUrlSpec,
                                            layout, QoraiShieldsContentSettings.ALLOW_RESOURCE,
                                            false);
                                }
    ...
    private void setUpSwitchLayouts() {
        LinearLayout upgradeHttpsLayout =
                mSecondaryLayout.findViewById(R.id.qorai_shields_upgrade_https_id);
        if (ChromeFeatureList.isEnabled(QoraiFeatureList.HTTPS_BY_DEFAULT)) {
            upgradeHttpsLayout.setVisibility(View.GONE);
        }

        LinearLayout mBlockScriptsLayout = mSecondaryLayout.findViewById(R.id.qorai_shields_block_scripts_id);
        TextView mBlockScriptsText = mBlockScriptsLayout.findViewById(R.id.qorai_shields_switch_text);
        mQoraiShieldsBlockingScriptsSwitch = mBlockScriptsLayout.findViewById(R.id.qorai_shields_switch);
        mBlockScriptsText.setText(R.string.qorai_shields_blocks_scripts_switch);
        setupBlockingScriptsSwitchClick(mQoraiShieldsBlockingScriptsSwitch);

        LinearLayout forgetFirstPartyStorageLayout =
                mSecondaryLayout.findViewById(R.id.qorai_shields_forget_first_party_storage_id);
        if (ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_FORGET_FIRST_PARTY_STORAGE)) {
            TextView forgetFirstPartyStorageText =
                    forgetFirstPartyStorageLayout.findViewById(R.id.qorai_shields_switch_text);
            forgetFirstPartyStorageText.setText(R.string.qorai_forget_first_party_storage_switch);
            mQoraiShieldsForgetFirstPartyStorageSwitch =
                    forgetFirstPartyStorageLayout.findViewById(R.id.qorai_shields_switch);
            setupForgetFirstPartyStorageSwitchClick(mQoraiShieldsForgetFirstPartyStorageSwitch);
        } else {
            forgetFirstPartyStorageLayout.setVisibility(View.GONE);
        }

        LinearLayout fingerprintingSwitchLayout =
                mSecondaryLayout.findViewById(R.id.qorai_shields_fingerprinting_switch_id);
        if (!ChromeFeatureList.isEnabled(QoraiFeatureList.QORAI_SHOW_STRICT_FINGERPRINTING_MODE)) {
            TextView fingerprintingSwitchText =
                    fingerprintingSwitchLayout.findViewById(R.id.qorai_shields_switch_text);
            mFingerprintingSwitch =
                    fingerprintingSwitchLayout.findViewById(R.id.qorai_shields_switch);
        ...
        TextView blockElementsText =
                mSecondaryLayout.findViewById(R.id.qorai_shields_block_element_text);
        blockElementsText.setVisibility(
                !isPrivateWindow
                                && ChromeFeatureList.isEnabled(
                                        QoraiFeatureList.QORAI_SHIELDS_ELEMENT_PICKER)
                        ? View.VISIBLE
                        : View.GONE);
        blockElementsText.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        hideQoraiShieldsMenu();
                        Tab currentActiveTab = mIconFetcher.getTab();
                        QoraiCosmeticFiltersUtils.launchContentPickerForWebContent(
                                currentActiveTab);
                    }
                });
    }
    ...
    private void setUpAboutLayout() {
        ...
        mOptionTitle.setText(R.string.about_qorai_shields_text);
    ...
    private void setUpReportBrokenSiteLayout() {
        ...
        mReportBrokenSiteDisclaimerView.setText(
                QoraiRewardsHelper.toSpannableString(
                        reportBrokenSiteDisclaimerText,
                        R.color.qorai_link,
                        R.string.report_broken_site_text_link,
                        (context) -> {
                            CustomTabActivity.showInfoPage(
                                    context, QoraiActivity.QORAI_WEBCOMPAT_INFO_WIKI_URL);
                        }));
        ...
        QoraiShieldsScreenshotUtil qoraiShieldsScreenshotUtil =
                new QoraiShieldsScreenshotUtil(
                        mContext,
                        (byte[] pngBytes) -> {
                            try {
                                ...
                                Log.e(TAG, "QoraiShieldsScreenshot failed " + e);
                            } finally {
        ...
        mCheckBoxScreenshot.setOnCheckedChangeListener(
                (buttonView, isChecked) -> {
                    ...
                    if (isChecked) {
                        ...
                        qoraiShieldsScreenshotUtil.capture();
                    } else {
                        mViewScreenshot.setVisibility(View.GONE);
                    }
                });
    ...
    private ReportInfo getReportInfo(String siteUrl) {
        ...
        reportInfo.channel = QoraiVersionConstants.CHANNEL;
        reportInfo.qoraiVersion = QoraiVersionConstants.VERSION;
    ...
    private SpannableString getScreenshotSpinnableString(byte[] pngBytes) {
        return QoraiRewardsHelper.toSpannableString(
                mContext.getResources()
    ...
    private void setUpMainSwitchLayout(boolean isChecked) {
        ...
        String mQoraiShieldsText =
                mContext.getResources().getString(R.string.qorai_shields_onboarding_title);
        ...
        if (isChecked) {
            ...
            mThirdLayout.setVisibility(
                    QoraiShieldsContentSettings.areAnyBlockedElementsPresent(mUrlSpec)
                            ? View.VISIBLE
                            : View.GONE);

            String mUpText = mContext.getResources().getString(R.string.up);
            SpannableString mSpanString = new SpannableString(mQoraiShieldsText + " " + mUpText);
            ...
            SpannableString mSpanString = new SpannableString(mQoraiShieldsText + " " + mDownText);
    ...
    private void setupBlockingScriptsSwitchClick(SwitchCompat qoraiShieldsBlockingScriptsSwitch) {
        if (null == qoraiShieldsBlockingScriptsSwitch) {
            return;
        }
        setupBlockingScriptsSwitch(qoraiShieldsBlockingScriptsSwitch, false);

        mQoraiShieldsBlockingScriptsChangeListener = new OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView,
                                         boolean isChecked) {
                if (0 != mUrlSpec.length()) {
                    QoraiShieldsContentSettings.setShields(mProfile, mUrlSpec,
                            QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_JAVASCRIPTS, isChecked,
                            false);
                    if (null != mMenuObserver) {
                        mMenuObserver.onMenuTopShieldsChanged(isChecked, false);
                    }
        ...
        qoraiShieldsBlockingScriptsSwitch.setOnCheckedChangeListener(mQoraiShieldsBlockingScriptsChangeListener);
    }

    private void setupBlockingScriptsSwitch(
            SwitchCompat qoraiShieldsBlockingScriptsSwitch, boolean fromTopSwitch) {
        if (null == qoraiShieldsBlockingScriptsSwitch) {
            return;
        }
        if (fromTopSwitch) {
            // Prevents to fire an event when top shields changed
            qoraiShieldsBlockingScriptsSwitch.setOnCheckedChangeListener(null);
        }
        if (0 != mUrlSpec.length()) {
            if (QoraiShieldsContentSettings.getShields(mProfile, mUrlSpec,
                        QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_QORAI_SHIELDS)) {
                if (QoraiShieldsContentSettings.getShields(mProfile, mUrlSpec,
                            QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_JAVASCRIPTS)) {
                    qoraiShieldsBlockingScriptsSwitch.setChecked(true);
                } else {
                    qoraiShieldsBlockingScriptsSwitch.setChecked(false);
                }
                qoraiShieldsBlockingScriptsSwitch.setEnabled(true);
            } else {
                qoraiShieldsBlockingScriptsSwitch.setChecked(false);
                qoraiShieldsBlockingScriptsSwitch.setEnabled(false);
            }
        }
        if (fromTopSwitch) {
            qoraiShieldsBlockingScriptsSwitch.setOnCheckedChangeListener(mQoraiShieldsBlockingScriptsChangeListener);
    ...
    private void setupForgetFirstPartyStorageSwitchClick(
            SwitchCompat qoraiShieldsForgetFirstPartyStorageSwitch) {
        if (null == qoraiShieldsForgetFirstPartyStorageSwitch) {
            return;
        }
        setupForgetFirstPartyStorageSwitch(qoraiShieldsForgetFirstPartyStorageSwitch, false);

        mQoraiShieldsForgetFirstPartyStorageChangeListener = new OnCheckedChangeListener() {
            @Override
        ...
        qoraiShieldsForgetFirstPartyStorageSwitch.setOnCheckedChangeListener(
                mQoraiShieldsForgetFirstPartyStorageChangeListener);
    }

    private void setupForgetFirstPartyStorageSwitch(
            SwitchCompat qoraiShieldsForgetFirstPartyStorageSwitch, boolean fromTopSwitch) {
        if (null == qoraiShieldsForgetFirstPartyStorageSwitch) {
            return;
        }
        if (fromTopSwitch) {
            // Prevents to fire an event when top shields changed
            qoraiShieldsForgetFirstPartyStorageSwitch.setOnCheckedChangeListener(null);
        }
        if (0 != mUrlSpec.length()) {
            if (QoraiShieldsContentSettings.getShields(mProfile, mUrlSpec,
                        QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_QORAI_SHIELDS)) {
                if (QoraiShieldsContentSettings.getShields(mProfile, mUrlSpec,
                            QoraiShieldsContentSettings
                                    .RESOURCE_IDENTIFIER_FORGET_FIRST_PARTY_STORAGE)) {
                    qoraiShieldsForgetFirstPartyStorageSwitch.setChecked(true);
                } else {
                    qoraiShieldsForgetFirstPartyStorageSwitch.setChecked(false);
                }
                qoraiShieldsForgetFirstPartyStorageSwitch.setEnabled(true);
            } else {
                qoraiShieldsForgetFirstPartyStorageSwitch.setChecked(false);
                qoraiShieldsForgetFirstPartyStorageSwitch.setEnabled(false);
            }
        }
        if (fromTopSwitch) {
            qoraiShieldsForgetFirstPartyStorageSwitch.setOnCheckedChangeListener(
    ...
    private void setupFingerprintingSwitchClick(SwitchCompat fingerprintingSwitch) {
        ...
        mQoraiShieldsFingerprintingChangeListener =
        ...
        fingerprintingSwitch.setOnCheckedChangeListener(mQoraiShieldsFingerprintingChangeListener);
    }

    private void setupFingerprintingSwitch(
            SwitchCompat qoraiShieldsFingerprintingSwitch, boolean fromTopSwitch) {
        if (null == qoraiShieldsFingerprintingSwitch) {
            return;
        }
        if (fromTopSwitch) {
            // Prevents to fire an event when top shields changed
            qoraiShieldsFingerprintingSwitch.setOnCheckedChangeListener(null);
        }
        if (0 != mUrlSpec.length()) {
            if (QoraiShieldsContentSettings.getShields(
                    mProfile,
                    mUrlSpec,
                    QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_QORAI_SHIELDS)) {
                if (!QoraiShieldsContentSettings.getShieldsValue(
                                mProfile,
                                mUrlSpec,
                                QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_FINGERPRINTING)
                        .equals(QoraiShieldsContentSettings.ALLOW_RESOURCE)) {
                    qoraiShieldsFingerprintingSwitch.setChecked(true);
                } else {
                    qoraiShieldsFingerprintingSwitch.setChecked(false);
                }
                qoraiShieldsFingerprintingSwitch.setEnabled(true);
            } else {
                qoraiShieldsFingerprintingSwitch.setChecked(false);
                qoraiShieldsFingerprintingSwitch.setEnabled(false);
            }
        }
        if (fromTopSwitch) {
            qoraiShieldsFingerprintingSwitch.setOnCheckedChangeListener(
                    mQoraiShieldsFingerprintingChangeListener);
        }
    }

    private void setupMainSwitchClick(SwitchCompat qoraiShieldsSwitch) {
        if (null == qoraiShieldsSwitch) {
            return;
        }
        if (0 != mUrlSpec.length()) {
            if (QoraiShieldsContentSettings.getShields(mProfile, mUrlSpec,
                        QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_QORAI_SHIELDS)) {
                qoraiShieldsSwitch.setChecked(true);
                setUpMainSwitchLayout(true);
            } else {
                qoraiShieldsSwitch.setChecked(false);
                setUpMainSwitchLayout(false);
            }
        }
        qoraiShieldsSwitch.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView,
                                         boolean isChecked) {
                if (0 != mUrlSpec.length()) {
                    QoraiShieldsContentSettings.setShields(mProfile, mUrlSpec,
                            QoraiShieldsContentSettings.RESOURCE_IDENTIFIER_QORAI_SHIELDS,
                            isChecked, false);
                    setupBlockingScriptsSwitch(mQoraiShieldsBlockingScriptsSwitch, true);
                    setupForgetFirstPartyStorageSwitch(
                            mQoraiShieldsForgetFirstPartyStorageSwitch, true);
                    if (null != mMenuObserver) {
        ...
        if (bmp != null && mContext != null) {
            ...
            new Runnable() {
                ...
                public void run() {
                    ...
                    if (iv != null) iv.setImageBitmap(QoraiRewardsHelper.getCircularBitmap(bmp));
                }
            });
        }
    }
