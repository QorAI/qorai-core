...
public class QoraiShieldsScreenshotUtil {
    private static final String TAG = "ShieldsScreenshot";
    private final QoraiScreenshotRunnable mQoraiScreenshotRunnable;

    private static class PngConvertorTask implements Runnable {
        private static final int PNG_QUALITY = 100;
        private final Bitmap mBitmap;
        private final QoraiShieldsScreenshotUtilCallback mCallback;

        public PngConvertorTask(
                @Nullable Bitmap bitmap, @NonNull QoraiShieldsScreenshotUtilCallback callback) {
            mCallback = callback;
            mBitmap = bitmap;
        }
    ...
    private static class QoraiScreenshotRunnable implements Runnable {
        ...
        public QoraiScreenshotRunnable(
                @NonNull ScreenshotTask screenshotTask,
                @NonNull QoraiShieldsScreenshotUtilCallback callback) {
            mCallback = callback;
    ...
    public interface QoraiShieldsScreenshotUtilCallback {
        void onScreenshotReady(@Nullable byte[] pngBytes);
    }

    public QoraiShieldsScreenshotUtil(
            @NonNull Context context, @NonNull QoraiShieldsScreenshotUtilCallback callback) {
        mQoraiScreenshotRunnable =
                new QoraiScreenshotRunnable(new ScreenshotTask((Activity) context), callback);
    }

    public void capture() {
        mQoraiScreenshotRunnable.start();
    }
}
