...
@JNINamespace("android_qorai_referrer")
public class QoraiReferrer implements InstallReferrerStateListener {
    private static final String TAG = "QoraiReferrer";
    private static final String APP_CHROME_DIR = "app_chrome";
    private static final String PROMO_CODE_FILE_NAME = "promoCode";
    private static final String QORAI_REFERRER_RECEIVED = "qorai_referrer_received";
    ...
    private long mNativeQoraiReferrer;

    private QoraiReferrer(long nativeQoraiReferrer) {
        mNativeQoraiReferrer = nativeQoraiReferrer;
    }

    @CalledByNative
    private static QoraiReferrer create(long nativeQoraiReferrer) {
        return new QoraiReferrer(nativeQoraiReferrer);
    }

    @CalledByNative
    private void destroy() {
        assert mNativeQoraiReferrer != 0;
        mNativeQoraiReferrer = 0;
    }

    private class InitReferrerRunnable implements Runnable {
        private final Context mContext;
        private final QoraiReferrer mQoraiReferrer;

        public InitReferrerRunnable(Context context, QoraiReferrer qoraiReferrer) {
            mContext = context;
            mQoraiReferrer = qoraiReferrer;
        }

        @Override
        public void run() {
            if (ChromeSharedPreferences.getInstance().readBoolean(QORAI_REFERRER_RECEIVED, false)
            ...
            try {
                mReferrerClient.startConnection(mQoraiReferrer);
            } catch (Exception e) {
            ...
            case InstallReferrerResponse.OK:
                try {
                    ...
                    ChromeSharedPreferences.getInstance()
                            .writeBoolean(QORAI_REFERRER_RECEIVED, true);
    ...
    private void onReferrerReady() {
        PostTask.postTask(TaskTraits.UI_BEST_EFFORT,
                () -> { QoraiReferrerJni.get().onReferrerReady(mNativeQoraiReferrer); });
    }

    @NativeMethods
    interface Natives {
        void onReferrerReady(long nativeQoraiReferrer);
    }
}
