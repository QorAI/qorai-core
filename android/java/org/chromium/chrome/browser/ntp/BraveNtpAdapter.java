...
public class QoraiNtpAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final OnQoraiNtpListener mOnQoraiNtpListener;
    ...
    private static final String TAG = "QoraiNtpAdapter";

    public QoraiNtpAdapter(Activity activity, OnQoraiNtpListener onQoraiNtpListener,
            RequestManager glide, CopyOnWriteArrayList<FeedItemsCard> newsItems,
            QoraiNewsController qoraiNewsController, View mvTilesContainerLayout, NTPImage ntpImage,
            ...
            int recyclerViewHeight, boolean isTopSitesEnabled, boolean isQoraiStatsEnabled,
            boolean isDisplayNewsFeed, boolean isDisplayNewsOptin) {
        mActivity = activity;
        mOnQoraiNtpListener = onQoraiNtpListener;
        mGlide = glide;
        ...
        mIsQoraiStatsEnabled = isQoraiStatsEnabled;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof StatsViewHolder) {
            ...
            statsViewHolder.mHideStatsImg.setOnClickListener(
                    view -> {
                        ChromeSharedPreferences.getInstance()
                                .writeBoolean(
                                        BackgroundImagesPreferences.PREF_SHOW_QORAI_STATS, false);
                    });
            List<Pair<String, String>> statsPairs = QoraiStatsUtil.getStatsPairs();
            ...
            statsViewHolder.mNtpStatsLayout.setOnClickListener(
                    view -> {
                        mOnQoraiNtpListener.checkForQoraiStats();
                    });
            ...
            newContentViewHolder.mNewContentLayout.setOnClickListener(
                    view -> {
                        mOnQoraiNtpListener.loadNewContent();
                    });
                ...
                imageCreditViewHolder.mSuperReferralLogo.setOnClickListener(
                        view -> {
                            ...
                            qRCodeShareDialogFragment.setQRCodeText(
                                    QoraiConstants.QORAI_REF_URL
                                            + mNTPBackgroundImagesBridge.getSuperReferralCode());
                            qRCodeShareDialogFragment.show(
                                    ((QoraiActivity) mActivity).getSupportFragmentManager(),
            ...
            newsOptinViewHolder.mOptinClose.setOnClickListener(
                    view -> {
                        mOnQoraiNtpListener.updateNewsOptin(false);
                    });

            newsOptinViewHolder.mOptinLearnMore.setOnClickListener(
                    view -> {
                        TabUtils.openUrlInSameTab(QoraiConstants.QORAI_NEWS_LEARN_MORE_URL);
                    });

            newsOptinViewHolder.mOptinButton.setOnClickListener(
                    view -> {
                        mOnQoraiNtpListener.updateNewsOptin(true);
                        mOnQoraiNtpListener.getFeed(false);
                    });
            ...
            noSourcesViewHolder.mBtnChooseContent.setOnClickListener(
                    view -> {
                        if (mActivity instanceof QoraiActivity) {
                            ((QoraiActivity) mActivity).openQoraiNewsSettings();
                        }
                    });
        }
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_STATS) {
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.qorai_stats_layout, parent, false);
            ...
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.qorai_news_load_new_content, parent, false);
            ...
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.qorai_news_no_sources, parent, false);
            return new NoSourcesViewHolder(view);

        } else {
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.qorai_news_row, parent, false);
    ...
    public void setQoraiStatsEnabled(boolean isQoraiStatsEnabled) {
        if (mIsQoraiStatsEnabled != isQoraiStatsEnabled) {
            mIsQoraiStatsEnabled = isQoraiStatsEnabled;
            if (mIsQoraiStatsEnabled) {
                notifyItemInserted(getStatsCount());
            } else {
                notifyItemRemoved(getStatsCount());
            }
    ...
    public void setQoraiNewsController(QoraiNewsController qoraiNewsController) {
        mQoraiNewsController = qoraiNewsController;
        notifyItemChanged(
                getStatsCount() + getTopSitesCount() + getNewContentCount() + ONE_ITEM_SPACE);
    }
    ...
    public void setImageCreditAlpha(float alpha) {
        ...
        // here https://github.com/qorai/qorai-browser/issues/29343
    ...
    public static class StatsViewHolder extends RecyclerView.ViewHolder {
        ...
        StatsViewHolder(View itemView) {
            ...
            this.mTitleLayout = (LinearLayout) itemView.findViewById(R.id.qorai_stats_title_layout);
            this.mHideStatsImg = (ImageView) itemView.findViewById(R.id.widget_more_option);
            this.mAdsBlockedCountTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_text_ads_count);
            this.mAdsBlockedCountTextTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_text_ads_count_text);
            this.mDataSavedValueTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_data_saved_value);
            this.mDataSavedValueTextTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_data_saved_value_text);
            this.mEstTimeSavedCountTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_text_time_count);
            this.mEstTimeSavedCountTextTv =
                    (TextView) itemView.findViewById(R.id.qorai_stats_text_time_count_text);
            QoraiTouchUtils.ensureMinTouchTarget(this.mHideStatsImg);
        }
    }
    ...
    public static class ImageCreditViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageCreditViewHolder(View itemView) {
            ...
            QoraiTouchUtils.ensureMinTouchTarget(this.mCreditTv);
        }
    }
    ...
    public static class NewsOptinViewHolder extends RecyclerView.ViewHolder {
        ...
        NewsOptinViewHolder(View itemView) {
            ...
            QoraiTouchUtils.ensureMinTouchTarget(mOptinButton);
            QoraiTouchUtils.ensureMinTouchTarget(mOptinLearnMore);
        }
    }
