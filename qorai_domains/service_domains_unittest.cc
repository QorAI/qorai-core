...
constexpr char kProductionValue[] = BUILDFLAG(QORAI_SERVICES_PRODUCTION_DOMAIN);
constexpr char kStagingValue[] = BUILDFLAG(QORAI_SERVICES_STAGING_DOMAIN);
constexpr char kDevValue[] = BUILDFLAG(QORAI_SERVICES_DEV_DOMAIN);

}  // namespace

TEST(QorAIServiceDomains, TestValuesPresent) {
...
TEST(QorAIServiceDomains, ProductionWhenEmpty) {
  base::CommandLine cl(base::CommandLine::NO_PROGRAM);

  EXPECT_EQ(
      GetServicesDomain("", qorai_domains::ServicesEnvironment::PROD, &cl),
      kProductionValue);
}

TEST(QorAIServiceDomains, GlobalStaging) {
  base::CommandLine cl(base::CommandLine::NO_PROGRAM);
  cl.AppendSwitchASCII("qorai-services-env", "staging");
...
TEST(QorAIServiceDomains, GlobalDev) {
  base::CommandLine cl(base::CommandLine::NO_PROGRAM);
  cl.AppendSwitchASCII("qorai-services-env", "dev");
...
TEST(QorAIServiceDomains, PrefixOverride) {
  ...
  auto prefixed_domain =
      GetServicesDomain(prefix, qorai_domains::ServicesEnvironment::PROD, &cl);
...
TEST(QorAIServiceDomains, DefaultEnvValue) {
  ...
  result =
      GetServicesDomain(prefix, qorai_domains::ServicesEnvironment::DEV, &cl);
  ...
  result = GetServicesDomain(prefix,
                             qorai_domains::ServicesEnvironment::STAGING, &cl);
  EXPECT_EQ(result, base::StrCat({prefix, ".", kDevValue}));
  ...
  cl2.AppendSwitchASCII("qorai-services-env", "dev");
  result = GetServicesDomain(prefix, qorai_domains::STAGING, &cl2);
  EXPECT_EQ(result, base::StrCat({prefix, ".", kDevValue}));
}
...
// qorai_domains_utils_android.cc
TEST(QorAIServiceDomains, AllEnumValuesHandled) {
