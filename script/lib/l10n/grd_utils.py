...
# Map of google_chrome_strings.grd resources ids to migrate to qorai_strings.grd
# The resources and all translations will be migrated to grd and xtb files.
# key - id in google_chrome_strings.
# value - new id in qorai_stirngs.
...
def qoraiify_grd_text(text, is_main_text, branding_replacements_only):
    """Replaces text string to Qorai wording"""
...
def generate_qoraiified_node(elem, is_comment, branding_replacements_only):
    """Replaces a node and attributes to Qorai wording"""
    if elem.text:
        elem.text = qoraiify_grd_text(
            elem.text, not is_comment, branding_replacements_only)

    if elem.tail:
        elem.tail = qoraiify_grd_text(
            elem.tail, not is_comment, branding_replacements_only)

    if 'desc' in elem.keys():
        elem.attrib['desc'] = qoraiify_grd_text(
            elem.attrib['desc'], False, branding_replacements_only)
    for child in elem:
        generate_qoraiified_node(child, is_comment, branding_replacements_only)
...
def qoraiify_grd_tree(source_xml_tree, branding_replacements_only):
    """Takes in a grd(p) tree and replaces all messages and comments with Qorai
       wording"""
    for elem in source_xml_tree.xpath('//message'):
        generate_qoraiified_node(elem, False, branding_replacements_only)
    for elem in source_xml_tree.xpath('//comment()'):
        generate_qoraiified_node(elem, True, branding_replacements_only)


def qoraiify_grd_in_place(source_string_path):
    """Takes in a grd file and replaces all messages and comments with Qorai
       wording"""
    source_xml_tree = lxml.etree.parse(source_string_path)
    qoraiify_grd_tree(source_xml_tree, False)
...
def get_override_file_path(source_string_path):
    """Obtain src/qorai source string override path for local grd strings with
       replacements"""
...
def update_xtbs_locally(grd_file_path, qorai_source_root, only_for_lang):
    ...
    # Special treatment for qorai_strings.grd
    qorai_strings_string_ids = []
    if os.path.basename(grd_file_path) == 'qorai_strings.grd':
        assert len(grd_strings) == len(chromium_grd_strings) + \
            len(GOOGLE_CHROME_STRINGS_MIGRATION_MAP)
        qorai_strings_string_ids = remove_google_chrome_strings(
    ...
    for idx, xtb_file in enumerate(xtb_file_paths):
        ...
        for node in xml_tree.xpath('//translation'):
            generate_qoraiified_node(node, False, True)
...
def get_chromium_grd_src_with_fallback(grd_file_path, qorai_source_root):
    source_root = os.path.dirname(qorai_source_root)
    chromium_grd_file_path = get_original_grd(source_root, grd_file_path)
    if not chromium_grd_file_path:
        rel_path = os.path.relpath(grd_file_path, qorai_source_root)
...
def remove_google_chrome_strings(qorai_grd_strings, google_chrome_strings_map):
    string_ids = []
    string_names = [
        string_name[4:].lower()
    ...
    to_remove = []
    for string_tuple in qorai_grd_strings:
    ...
    for string_tuple in to_remove:
        qorai_grd_strings.remove(string_tuple)

    return string_ids


def add_google_chrome_translations(qorai_strings_xtb_file, xml_tree,
                                   string_ids):
    qorai_xtb_tree = lxml.etree.parse(qorai_strings_xtb_file)
    translationbundle = xml_tree.xpath('//translationbundle')[0]
    for string_id in string_ids:
        translation = qorai_xtb_tree.xpath(
...
def is_translateable_string(grd_file_path, message_tag):
    ...
    # to be translateable in Qorai. These can be found in the main function in
    # qorai/script/chromium-rebase-l10n.py
    grd_file_name = os.path.basename(grd_file_path)
