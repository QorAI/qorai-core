...
from lib.config import get_env_var, QORAI_CORE_ROOT
from lib.util import execute, scoped_cwd
from lib.helpers import channels, QORAI_CORE_REPO
...
def get_remote_version(branch_to_compare):
    global config
    decoded_file = get_file_contents(config.github_token, QORAI_CORE_REPO,
...
def parse_issues_fixed(body):
    try:
        regex = r'((Resolves|Fixes|Fix|Closes|Close|resolves|fixes|fix|closes|close) https:\/\/github\.com\/qorai\/qorai-browser\/issues\/(\d*))'  # pylint: disable=line-too-long
...
def main():
    ...
    result = fetch_origin_check_staged(QORAI_CORE_ROOT)
    if result != 0:
        return result

    # get all channel branches (starting at master)
    qorai_core_version = get_remote_version('master')
    remote_branches = get_remote_channel_branches(qorai_core_version)
    ...
    if args.uplift_using_pr:
        try:
            ...
            repo = GitHub(config.github_token).repos(QORAI_CORE_REPO)
        ...
        with scoped_cwd(QORAI_CORE_ROOT):
    ...
    local_branch = get_local_branch_name(QORAI_CORE_ROOT)
    if not config.title and not args.uplift_using_pr:
        config.title = get_title_from_first_commit(QORAI_CORE_ROOT,
                                                   top_level_base)
    ...
    push_branches_to_remote(QORAI_CORE_ROOT,
...
def get_milestone_for_branch(channel_branch):
    ...
    if not config.milestones:
        config.milestones = get_milestones(config.github_token, QORAI_CORE_REPO)
