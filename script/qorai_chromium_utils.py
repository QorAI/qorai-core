...
def get_src_dir() -> str:
    """Searches for src/ dir which includes qorai/ dir."""
...
def wspath(path: str) -> str:
    ...
    `//chrome/file.txt` -> `/home/user/qorai_checkout/src/chrome/file.txt`
    `//chrome/file.txt` -> `C:\\qorai_checkout\\src\\chrome\\file.txt`
    """
...
def get_chromium_src_override(path: str) -> str:
    """Convert path into `//qorai/chromium_src` override path."""
    assert path, path
    ...
    override_path = wspath(f'//qorai/chromium_src/{src_path}')
...
def to_wspath(path: str) -> str:
    ...
    `/home/user/qorai_checkout/src/chrome/file.txt` -> `//chrome/file.txt`
    `C:\\qorai_checkout\\src\\chrome\\file.txt` -> `//chrome/file.txt`
    """
    assert path, path
    if not os.path.isabs(path):
        path = os.path.abspath(path)
...
def inline_chromium_src_override(_globals: Dict[str, Any],
                                 _locals: Dict[str, Any]) -> None:
    """Inline `__file__` override from `//qorai/chromium_src`."""
    orig_file = _globals.get('__file__')
    if not orig_file:
        raise RuntimeError(
            '__file__ is not set to inline from //qorai/chromium_src. '
