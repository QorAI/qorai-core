...
def load_presubmit_config():
    with qorai_chromium_utils.sys_path('//third_party/pyjson5/src'):
        import json5
        return json5.load(
            open(
                qorai_chromium_utils.wspath(
                    '//qorai/chromium_presubmit_config.json5')))
...
def inline_presubmit(filename, _globals, _locals):
    class State:
        def __init__(self, filename):
            self.presubmit_dir = os.path.dirname(
                qorai_chromium_utils.wspath(filename))
    ...
    qorai_chromium_utils.inline_file(filename, _globals, _locals)
