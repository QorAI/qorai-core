...
import sys
import glob
from lib.l10n.grd_utils import (qoraiify_grd_in_place, qoraiify_grd_tree,
...
QORAI_SOURCE_ROOT = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
SRC_SOURCE_ROOT = os.path.abspath(os.path.dirname(QORAI_SOURCE_ROOT))
...
def write_new_translations_to_xtb(xtb_path, messages):
    ...
    qorai_xtb_xml_tree = etree.parse(xtb_path, parser)
    bundle_element = qorai_xtb_xml_tree.xpath('//translationbundle')[0]
    for message in messages:
        google_elem = qorai_xtb_xml_tree.xpath('//translation[@id="{}"]'.format(
            message[0]))
    ...
    write_xtb_content(qorai_xtb_xml_tree, xtb_path)
...
def migrate_google_chrome_xtb_translations_for_messages(message_ids):
    ...
    for google_xtb_path in google_chrome_xtb_files:
        ...
        if not lang:
            print(
                'Skipping file {} because unable to determine language'.format(
                    google_xtb_path))
            continue
        qorai_xtb_path = os.path.join(
            QORAI_SOURCE_ROOT,
            'app/resources/qorai_strings_{}.xtb'.format(lang))
        if not os.path.exists(qorai_xtb_path):
            print('Unable to find qorai translation file {}'.format(
                qorai_xtb_path))
            return False
        messages = [(message_id,
        ...
        write_new_translations_to_xtb(qorai_xtb_path, messages)

    return True


def migrate_google_chrome_strings(qorai_strings_xml_tree,
    ...
    for item in google_chrome_strings_map.keys():
        ...
        messages_element = qorai_strings_xml_tree.xpath('//messages')[0]
...
def generate_overrides_and_replace_strings(source_string_path):
    ...
    qoraiify_grd_tree(original_xml_tree_with_branding_fixes, True)
    # Apply all replacements the the clean GRD.
    qoraiify_grd_in_place(source_string_path)
    ...
    for part in parts:
        ...
        # Check for the special case of qorai_stings.grd:
        if (os.path.basename(source_string_path) == 'qorai_strings.grd'
                and override_file == 'settings_chromium_strings_override.grdp'):
            override_file = 'settings_qorai_strings_override.grdp'
    ...
    if len(modified_messages) > 0 or len(modified_parts) > 0:
        # Fix output filenames to generate "qorai" files instead of "chromium".
        if os.path.basename(source_string_path) == 'qorai_strings.grd':
...
def main():
    ...
    args = parse_args()
    # This file path is a string path inside qorai/ but just recently copied
    # in from chromium files which need replacements.
    source_string_path = os.path.join(QORAI_SOURCE_ROOT,
    ...
    # is_translateable_string function in qorai/script/lib/l10n/grd_utils.py.
    xml_tree = etree.parse(source_string_path)
    (basename, _) = filename.split('.')
    if basename == 'qorai_strings':
        ...
        elem1.text = 'Inbound rule for Qorai Beta to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_CANARY"]')[0]
        elem1.text = 'Inbound rule for Qorai Nightly to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_DEV"]')[0]
        elem1.text = 'Inbound rule for Qorai Dev to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//part[@file="settings_chromium_strings.grdp"]')[0]
        elem1.set('file', 'settings_qorai_strings.grdp')
        elem1 = xml_tree.xpath(
