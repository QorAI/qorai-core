...
def list_ntp_backgrounds(metadata_file):
    ...
    with open(metadata_file, mode='rt', encoding='utf-8') as file_handle:
        ...
        json_metadata = "".join(metadata) \
            .replace("export const images: NewTab.QoraiBackground[] = [",
                     "[") \
            .replace('"', '"').replace("'", '"')
...
def main():
    ...
    adblock_preamble = 'These licenses do not apply to any of the code ' \
        'shipped with the Qorai Browser, but may apply to lists downloaded ' \
        'after installation for use with the Qorai Shields feature. The ' \
        'Qorai Browser and such lists are separate and independent works.'
    ...
    local_data_preamble = 'These licenses do not apply to any of the code ' \
        'shipped with the Qorai Browser, but may apply to data files ' \
        'downloaded after installation for use with various Qorai features. ' \
        'The Qorai Browser and such data files are separate and independent ' \
        'works.'
    ...
    # Qorai New Tab UI component
    ntp_data_dir = os.path.join(components_dir, 'qorai_new_tab_ui', 'data')
    ntp_backgrounds_preamble = 'These licenses do not apply to any of the ' \
        'code shipped with the Qorai Browser and instead apply to ' \
        'background images used on the new tab page. The Qorai Browser and ' \
