...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the Qorai GRD file vs the Chromium GRD file for extra
       languages."""
    chromium_grd_file_path = get_original_grd(src_root, grd_file_path)
    if not chromium_grd_file_path:
        return
    qorai_langs = get_grd_languages(grd_file_path)
    chromium_langs = get_grd_languages(chromium_grd_file_path)
    x_qorai_extra_langs = qorai_langs - chromium_langs
    assert len(x_qorai_extra_langs) == 0, \
        f'Qorai GRD {grd_file_path} has extra languages ' \
            f'{list(x_qorai_extra_langs)} over Chromium GRD ' \
            f'{chromium_grd_file_path}'
    x_chromium_extra_langs = chromium_langs - qorai_langs
    assert len(x_chromium_extra_langs) == 0, \
        f'Chromium GRD {chromium_grd_file_path} has extra languages ' \
            f'{list(x_chromium_extra_langs)} over Qorai GRD {grd_file_path}'
