...
class RepositoryTest(unittest.TestCase):
    ...
    def test_chromium_repository(self):
        ...
        self.assertFalse(repository.chromium.is_qorai)

    def test_qorai_repository(self):
        self.assertEqual(repository.qorai.path,
                         PurePath(self.fake_chromium_src.qorai))
        self.assertTrue(repository.qorai.is_qorai)
        self.assertFalse(repository.qorai.is_chromium)

    def test_to_qorai(self):
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium).to_qorai(),
            PurePath('qorai'))
        self.assertEqual(
            Repository(self.fake_chromium_src.qorai).to_qorai(),
            PurePath('../qorai'))
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium / 'v8').to_qorai(),
            PurePath('../qorai'))
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium /
                       'third_party/test1').to_qorai(),
            PurePath('../../qorai'))

    def test_from_qorai(self):
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium).from_qorai(),
            PurePath('../'))
        self.assertEqual(
            Repository(self.fake_chromium_src.qorai).from_qorai(),
            PurePath('../qorai'))
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium / 'v8').from_qorai(),
            PurePath('../v8'))
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium /
                       'third_party/test1').from_qorai(),
            PurePath('../third_party/test1'))
        self.assertEqual(
            Repository(self.fake_chromium_src.chromium /
                       'third_party/test1').from_qorai('test_file.txt'),
    ...
    def test_run_git(self):
        ...
        self.assertEqual(
            self.fake_chromium_src.commit_empty("Empty commit in qorai",
                                                self.fake_chromium_src.qorai),
            repository.qorai.run_git("rev-parse", "HEAD"),
        )
        self.assertEqual(
    ...
    def test_run_git_no_trim(self):
        ...
        with patch("terminal.terminal.run_git") as mock_run_git:
            # Call run_git with no_trim=True for qorai repository
            repository.qorai.run_git("log", no_trim=True)
            ...
            repository.qorai.run_git("log", no_trim=False)
    ...
    def test_relative_to_chromium(self):
        ...
        self.assertEqual(
            ...
            as_posix(), '.')
        # Qorai root should be relative to chromium as 'qorai'
        self.assertEqual(
            Repository(
                self.fake_chromium_src.qorai).relative_to_chromium.as_posix(),
            'qorai')
