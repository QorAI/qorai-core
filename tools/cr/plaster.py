...
# The path to the directory containing plaster files in qorai-core.
PLASTER_FILES_PATH = repository.QORAI_CORE_PATH / 'rewrite'

# The path to the directory where patch files are stored in qorai-core.
...
class PathChecksumPair:
    ...
    # The file path relative to QORAI_CORE_PATH.
    path: Path
...
class PatchInfo:
    ...
    # Path to the plaster file, relative to QORAI_CORE_PATH.
    plaster_file: Path
    ...
    def __post_init__(self):
        ...
        self.source_with_checksum = PathChecksumPair(
            Path(repository.chromium.from_qorai(self.source)))
...
def apply():
    """Applies all plaster files to qorai.
    """
    with terminal.with_status('Applying plaster files'):
        # TODO(https://github.com/qorai/qorai-browser/issues/46880): Add support
...
def check(args):
    ...
    if hasattr(args, 'filepaths') and args.filepaths:
        ...
        # TODO(https://github.com/qorai/qorai-browser/issues/46880): For now we
...
def main():
    ...
    apply_parser = subparsers.add_parser(
        'apply', help='Apply all plaster files to the sources in qorai-core')
