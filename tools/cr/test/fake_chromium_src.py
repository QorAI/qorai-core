...
class FakeChromiumSrc(FakeChromiumRepo):
    ...
    def __init__(self):
        super().__init__()
        self.qorai_patch = patch('repository.QORAI_CORE_PATH',
                                 PurePath(self.qorai))
        ...
        self.repository_qorai_patch = patch(
            'repository.qorai', repository.Repository(PurePath(
                self.qorai)))  # Patch repository.qorai globally
        self.original_cwd = None

    def setup(self):
        """Set up the fake repo and apply patches."""
        self.qorai_patch.start()
        self.chromium_patch.start()
        self.repository_chromium_patch.start(
        )  # Start the global patch for chromium
        self.repository_qorai_patch.start()  # Start the global patch for qorai
        ...
        repository.qorai = repository.Repository(PurePath(self.qorai))
