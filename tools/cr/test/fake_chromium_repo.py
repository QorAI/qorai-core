...
class FakeChromiumRepo:
    ...
    def __init__(self) -> None:
        ...
        with a `src` directory. Also creates a `qorai` repository inside `src`.
        """
        ...
        # Set a qorai repository under src/.
        self._init_repo(self.qorai)
        self.qorai_patches.mkdir(parents=True, exist_ok=True)
    ...
    @property
    def qorai(self) -> Path:
        """Returns the path to the Qorai directory"""
        return self.chromium / 'qorai'

    @property
    def qorai_patches(self) -> Path:
        """Returns the path to the Qorai patches directory."""
        return self.qorai / 'patches'
    ...
    def _init_repo(self, path: Path) -> None:
        ...
        self._run_git_command(['config', 'user.email', 'fake@qoranet.com'], path)
        self._run_git_command(['commit', '-m', 'Initial commit'], path)

    def create_qorai_remote(self) -> None:
        """Creates a remote repository for Qorai and sets it as the origin.

        Initializes a git repository at the path returned by `self.remote` and
        adds it as the `origin` remote for the Qorai repository.
        """
        # Initialize the remote repository
        self._init_repo(self.remote / 'qorai')

        # Add the remote as 'origin' for the Qorai repository
        self._run_git_command(
            ['remote', 'add', 'origin',
             str(self.remote / 'qorai')], self.qorai)
    ...
    def update_qorai_version(self, version: str) -> str:
        """Updates the Qorai version in package.json and commits the change.
        ...
        Returns:
            The hash of the commit made.
        """
        package_json_path = self.qorai / 'package.json'
        old_version = None
        ...
        self._run_git_command(['add', str(package_json_path)], self.qorai)
        commit_message = (
            f'Update from Chromium {old_version or "N/A"} to Chromium {version}'
        )
        self._run_git_command(['commit', '-m', commit_message], self.qorai)

        # Return the hash of the commit
        return self._run_git_command(['rev-parse', 'HEAD'], self.qorai)
    ...
    def run_update_patches(self) -> None:
        ...
        in all of qorai's patches being erased and generated again. This
        eliminates any stales.
        """
        # Delete the patches directory and recreate it
        if self.qorai_patches.exists():
            shutil.rmtree(self.qorai_patches)
        self.qorai_patches.mkdir(parents=True, exist_ok=True)

        for repo_path in self.base_path.glob('src/**'):
            if repo_path == self.qorai or not (repo_path / '.git').exists():
                continue
            ...
            if modified_files:
                sub_patches_dir = self.qorai_patches / relative_repo_path
                sub_patches_dir.mkdir(parents=True, exist_ok=True)

            for filename in modified_files:
                # Generate the patch file path
                patch_file = self.qorai / self.get_patchfile_path_for_source(
    ...
    def get_patchfile_path_for_source(self, repo_path: Path,
        ...
        return (self.qorai_patches / repo_path /
    ...
    def run_apply_patches(self) -> List[Dict]:
        ...
        """
        if not self.qorai_patches.exists():
            raise FileNotFoundError(
                f'Patches directory {self.qorai_patches} does not exist.')

        failed_patches = []

        for patch_file in self.qorai_patches.rglob('*.patch'):
            # Using the relative path of the patch file to determine the target
            # repository path.
            relative_repo_path = patch_file.relative_to(
                self.qorai_patches).parent
            ...
            try:
                ...
                failed_patches.append({
                    "patchPath": str(patch_file.relative_to(self.qorai)),
                    "path": path,
                    "reason": reason
                })
