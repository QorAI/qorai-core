...
# The path to the qorai/ directory.
QORAI_CORE_PATH = next(qorai for qorai in PurePath(__file__).parents
                       if qorai.name == 'qorai')

# The path to chromium's src/ directory.
CHROMIUM_SRC_PATH = QORAI_CORE_PATH.parent
...
class Repository:
    ...
    def is_qorai(self) -> bool:
        """If this repo is qorai/.
        """
        return self.path == QORAI_CORE_PATH
    ...
    def to_qorai(self) -> PurePath:
        """ Returns the path from the repository to qorai/.
        """
        if self.is_chromium:
            return QORAI_CORE_PATH.relative_to(CHROMIUM_SRC_PATH)
        return PurePath(
            len(self.path.relative_to(CHROMIUM_SRC_PATH).parts) *
            '../') / QORAI_CORE_PATH.relative_to(CHROMIUM_SRC_PATH)

    def from_qorai(self, source: Optional[PurePath] = None) -> PurePath:
        """ Returns the path from qorai/ to the repository.
        """
        if source:
    ...
    def run_git(self, *cmd, no_trim=False) -> str:
        ...
        return terminal.run_git('-C', self.from_qorai(), *cmd, no_trim=no_trim)
...
# An instance to the qorai repository.
qorai = Repository(PurePath(QORAI_CORE_PATH))
