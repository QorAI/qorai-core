...
### `brockit.py lift`
This is *🚀Brockit!* (Qorai Rocket? Qorai Rock it? Broke it?): a tool to help
upgrade Qorai to a newer Chromium base version. The main goal is to produce
...
RUST_TOOLCHAIN_URL = 'https://qorai-build-deps-public.s3.qorai.com/rust-toolchain-aux/linux-x64-rust-toolchain-{revision}.tar.xz'
...
- No specific code changes in Qorai (only line number changes in patches)
"""
...
def _get_current_branch_upstream_name() -> Optional[str]:
    ...
    """
    try:
        return repository.qorai.run_git('rev-parse', '--abbrev-ref',
...
def _update_pinslist_timestamp() -> str:
    ...
    """
    content = repository.qorai.read_file(PINSLIST_TIMESTAMP_FILE)
    ...
    updated = repository.qorai.run_git('diff', PINSLIST_TIMESTAMP_FILE)
    if updated == '':
    ...
    def stage_all_patches(self, ignore_deleted_files=False):
        ...
        for _, patches in self.patch_files.items():
            for patch in patches:
                ...
                repository.qorai.run_git('add', patch.path)
...
class ContinuationFile:
    ...
    # The target version that brockit is aiming to upgrade qorai to.
...
class Versioned(Task):
    ...
    """
        repository.qorai.run_git('add', '-u', '*.patch')

        repository.qorai.git_commit(
    ...
    """
        repository.qorai.run_git('add', '*.grd', '*.grdp', '*.xtb')
        repository.qorai.git_commit(
    ...
    """
        ...
        repository.qorai.run_git('add', '-u', 'third_party/rust/')
        repository.qorai.git_commit(
...
class GitHubIssue(Versioned):
    ...
    def lookup_issue(self, title: str) -> Optional[List]:
        ...
        results = json.loads(
            terminal.run([
                'gh', 'issue', 'list', '--repo', 'qorai/qorai-browser',
    ...
    def create_push_request(self, issue_url: str):
        ...
        """
        current_branch = repository.qorai.current_branch()
        ...
        if is_uplift:
            ...
            results = json.loads(
                terminal.run([
                    'gh', 'api', 'repos/qorai/qorai-core/milestones', '--jq',
                    '[.[] | {number, title}]'
                ]).stdout)
            if not results:
                raise BadOutcomeException(
                    'No milestones returned for qorai-core')
            ...
            terminal.run([
                'gh', 'api', '-X', 'PATCH',
                f'repos/qorai/qorai-core/issues/{pr_number}', '-F',
    ...
    def create_or_update_version_issue(self, with_pr: bool):
        ...
        if issue is not None:
            ...
            if body == issue['body']:
                ...
                terminal.run([
                    'gh', 'issue', 'edit',
                    str(issue['number']), '--repo', 'qorai/qorai-browser',
                    '--body', f'{body}'
                ])
        ...
        issue_url = terminal.run([
            'gh', 'issue', 'create', '--repo', 'qorai/qorai-browser',
...
class ReUpgrade(Task):
    ...
    def execute(self):
        ...
        starting_change = repository.qorai.last_changed(
            PINSLIST_TIMESTAMP_FILE)
        commit_message = repository.qorai.get_commit_short_description(
            starting_change)
        ...
        console.log(
            Padding(
                '[dim]%s' % repository.qorai.run_git(
                    'log', '--pretty=%h %s', f'HEAD...{starting_change}~1'),
                (0, 4)))

        ContinuationFile.clear()
        repository.qorai.run_git('reset', '--hard', f'{starting_change}~1')
...
class Upgrade(Versioned):
  ...
  """
    ...
    def apply_patches_3way(self,
            ...
            for patch in patches:
                ...
                if apply_result.status == Patchfile.ApplyStatus.CONFLICT:
                    files_with_conflicts.append(
                        patch.source_from_qorai().as_posix())
            ...
            for commit, patches in deletion_report.items():
                for patch, status in patches.items():
                    if status.status == 'D':
                        ...
                        renamed_to = patch.repository.from_qorai(
                        ) / status.renamed_to
                        console.log(
                            Padding(
                                f'✘ {patch.source_from_qorai()}\n    '
        ...
        if broken_patches:
            ...
            for patch in broken_patches:
                source = patch.source_from_qorai()
    ...
    """
        ...
        repository.qorai.run_git('add', versioning.PACKAGE_FILE)

        # Pinlist timestamp update occurs with the package version update.
        _update_pinslist_timestamp()
        repository.qorai.run_git('add', PINSLIST_TIMESTAMP_FILE)
        repository.qorai.git_commit(
    ...
    Returns:
        ...
        return {
            ...
            'advice': 'Run the jobs in https://ci.qorai.com/view/rust to generate a new Rust toolchain.',
            'commit': {
                'hash': commit_hash,
                'message': commit_message
            }
    ...
    * Pre-run advisory (attention needed)
        * The rust toolchain has been updated.
            ...
            Run the jobs in https://ci.qorai.com/view/rust to generate a new...
    ...
    Args:
        ...
        has_changes = repository.qorai.has_staged_changed()
    ...
    Return:
        ...
        if (self.working_version != self.chromium_src_version
            ...
            logging.warning(
                ...
                'unrelated. Qorai %s ➜ Chromium %s', self.working_version,
            ...
            logging.warning(
                ...
                'Qorai %s ➜ Chromium %s', self.working_version,
                self.chromium_src_version)
        ...
        if self.is_continuation:
            if self.target_version != self.chromium_src_version:
                raise InvalidInputException(
                    ...
                    'version has to be in Sync with Qorai. Qorai '
...
def solve_git_ref(from_ref: str) -> str:
    ...
    if from_ref and from_ref[0] != '@':
        ...
        if not repository.qorai.is_valid_git_reference(from_ref):
            raise InvalidInputException(
    ...
    def find_previous_version_hash(is_major: bool = False) -> str:
        ...
        last_changed = repository.qorai.last_changed(versioning.PACKAGE_FILE)
        while True:
            ...
            last_changed = repository.qorai.last_changed(
        ...
        try:
            terminal.run([
                ...
                '--empty=drop', '--onto', to_ref, from_ref, current_branch
                         ...
                         env=env)
            if recommit:
                repository.qorai.run_git('commit', '--amend', '--no-edit')
                repository.qorai.run_git('rebase', '--continue')
        except subprocess.CalledProcessError as e:
...
def show(args: argparse.Namespace):
    """Prints various insights about qorai-core.

    This is a helper command line that allows us to inspect a few things about
    qorai-core and how brockit process things.
    """
...
def main():
    ...
    show_parser = subparsers.add_parser(
        'show', help='Prints various insights about qorai-core.')
