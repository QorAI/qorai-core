...
# This should be ran from the Qorai root. aka `root/src/qorai/`.
pushd .
...
QORAI_DEPS_DIR=`realpath ./third_party/android_deps/libs/com_wireguard_android`
echo "Changing directory to $QORAI_DEPS_DIR"
cd "$QORAI_DEPS_DIR"
pushd .
...
echo "Copying binary from tunnel/build/outputs/aar to expected Qorai location"
COPY_SRC="wireguard-android/tunnel/build/outputs/aar/tunnel-release.aar"
COPY_DST="$QORAI_DEPS_DIR/tunnel-$WIREGUARD_TAG.aar"
