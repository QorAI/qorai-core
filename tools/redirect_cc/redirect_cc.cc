...
#include "qorai/tools/redirect_cc/rewrapper_buildflags.h"  // nogncheck
...
const base::FilePath::StringViewType kQoraiChromiumSrc =
    FILE_PATH_LITERAL("qorai/chromium_src");
...
class RedirectCC {
 public:
  ...
  int Run() {
    ...
    // Path to `src/qorai/chromium_src`.
    base::FilePath::StringType qorai_chromium_src_dir;
    // Path to `src/`.
    ...
    for (const auto* arg : args_.subspan(first_compiler_arg_idx)) {
      ...
      if (arg_piece.starts_with(kIncludeQuotedFlag) &&
          arg_piece.ends_with(kQoraiChromiumSrc)) {
        arg_piece.remove_prefix(kIncludeQuotedFlag.size());
        qorai_chromium_src_dir = base::FilePath::StringType(arg_piece);
        arg_piece.remove_suffix(kQoraiChromiumSrc.size());
    ...
    base::FilePath::StringType qorai_path;
    ...
    for (size_t arg_idx = first_compiler_arg_idx; arg_idx < args_.size();
      ...
      if (!compile_file_found && base::Contains(kCompileFileFlags, arg_piece)) {
        ...
        if (!path_cc.empty()) {
          qorai_path = base::StrCat(
              {qorai_chromium_src_dir, FILE_PATH_LITERAL("/"), path_cc});
          if (base::PathExists(base::FilePath(qorai_path))) {
            launch_argv.emplace_back(arg_piece);
            launch_argv.push_back(qorai_path);
            ++arg_idx;
            continue;
          } else {
            qorai_path.clear();
    ...
    // deps.
    if (exit_code == 0 && !qorai_path.empty() && has_show_includes_flag) {
#if BUILDFLAG(IS_WIN)
      std::wcerr << L"Note: including file: " << qorai_path << L"\n";
#else   // BUILDFLAG(IS_WIN)
      std::cerr << "Note: including file: " << qorai_path << "\n";
