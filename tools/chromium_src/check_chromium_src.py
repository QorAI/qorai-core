...
QORAI_SRC = os.path.abspath(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
QORAI_CHROMIUM_SRC = os.path.join(QORAI_SRC, 'chromium_src')
CHROMIUM_SRC = os.path.abspath(os.path.dirname(QORAI_SRC))
...
def is_header_guard_define(override_filepath, target):
    ...
    """
    guard_string = ("QORAI_CHROMIUM_SRC_" +
...
def filter_chromium_src_filepaths(affected_paths, exclude_regexp=None):
    ...
    for path in affected_paths:
        # Strip the QORAI_CHROMIUM_SRC prefix to match against the
        # list of relative paths to be included and/or excluded
        relative_path = path.replace(QORAI_CHROMIUM_SRC, '')[1:]
...
def filter_all_chromium_src_filepaths(exclude_regexp):
    """
    Return a list of paths pointing to the files in |QORAI_CHROMIUM_SRC|
    after filtering out the exclusions.
    """
    result = []
    for dir_path, _dirnames, filenames in os.walk(QORAI_CHROMIUM_SRC):
        for filename in filenames:
            full_path = os.path.join(dir_path, filename)

            # Strip the QORAI_CHROMIUM_SRC prefix to match against the
            # list of relative paths to be included and/or excluded
            relative_path = full_path.replace(QORAI_CHROMIUM_SRC, '')[1:]
...
class ChromiumSrcOverridesChecker:
        ...
        if not found['undef']:
            ...
            if override_filepath.endswith('.h'):
                message += (
                    ...
                    "exceptions in " +
                    "//qorai/chromium_src/check_chromium_src_config.json5.")
        ...
        for override_filepath in self.overrides:
            ...
            if not os.path.isfile(original_filepath):
                additional_extensions = (
                    qorai_chromium_utils.get_additional_extensions())
                if any(
    ...
    def validate_exclusion_symbol(self, path, symbol):
        ...
        """
        override_path = os.path.join(QORAI_CHROMIUM_SRC, path)
    ...
    def validate_exclusion_path(self, path):
        ...
        if not os.path.isfile(override_path):
            self.AddError(
                "  Path listed in //qorai/chromium_src/" +
                "check_chromium_src_config.json5 " +
                f"cannot be found:\n  chromium_src/{path}.\n  If the file " +
    ...
    def load_exclusions(self):
        config_path = os.path.join(QORAI_CHROMIUM_SRC,
                                   'check_chromium_src_config.json5')
...
def main():
    ...
    for directory in [QORAI_SRC, gen_buildir]:
