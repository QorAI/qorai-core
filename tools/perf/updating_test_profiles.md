...
- `qorai-core` stores only sha1 hashes of these profiles.
...
- Chromium and Qorai use different profiles, so two builds for each platforms
...
- Checkout `qorai`, run `npm run sync`
...
- Each unpacked artifact contains a zip archive with the updated profile (e.g.
  `qorai-typical-win.zip`) and `.size` file (e.g.
  `qorai-typical-win.zip.sizes`). Copy these files to `./profiles/`.
