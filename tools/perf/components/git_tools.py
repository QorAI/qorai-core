...
GH_QORAI_CORE_GIT_URL = 'git@github.com:qorai/qorai-core.git'
GH_QORAI_VARIATIONS_GIT_URL = 'git@github.com:qorai/qorai-variations.git'
GH_QORAI_PERF_TEAM = 'qorai/perf-team'
...
def DoesPrOpen(branch: str, target: Optional[str] = None):
  ...
  _, output = GetProcessOutput(args, cwd=path_util.GetQoraiDir(), check=True)
...
def MakeGithubPR(branch: str, target: str, title: str, body: str,
  ...
  for arg in extra_args:
    args.append(arg)
  return GetProcessOutput(args, cwd=path_util.GetQoraiDir())


def PushChangesToBranch(files: Dict[str, str],
                        branch: str,
                        commit_message: str,
                        cwd=path_util.GetQoraiDir()):
  ...
  for attempt in range(3):
    ...
    branch_exists, _ = GetProcessOutput(
        ['git', 'fetch', GH_QORAI_CORE_GIT_URL, branch], cwd)
    if branch_exists:
    ...
    auth_url = f'https://{github_username}:{github_token}@github.com/qorai/qorai-core.git'
...
def GetFileAtRevision(
    filepath: str, revision: str, cwd=path_util.GetQoraiDir()) -> Optional[str]:
...
def Clone(url: str, branch: Optional[str], target_dir: str) -> None:
  ...
  GetProcessOutput(args, path_util.GetQoraiDir(), check=True)
...
def EnsureRevision(revision: str, cwd=path_util.GetQoraiDir()) -> None:
  ...
  if cwd == path_util.GetQoraiDir():
    if GetProcessOutput(
        ['git', 'fetch', GH_QORAI_CORE_GIT_URL, f'{revision}:{revision}'], cwd):
      return

  raise RuntimeError(f'Can\'t fetch revision {revision}')


def GetCommitDate(revision: str, cwd=path_util.GetQoraiDir()) -> str:
...
def GetGitHash(revision: str, cwd=path_util.GetQoraiDir()) -> str:
  _, git_hash_output = GetProcessOutput(
      ['git', 'rev-list', '-n', '1', revision], cwd, check=True)
  return git_hash_output.rstrip()


def GetRevisionNumber(revision: str, cwd=path_util.GetQoraiDir()) -> str:
