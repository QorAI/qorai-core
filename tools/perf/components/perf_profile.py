...
from components.version import QoraiVersion
from components.git_tools import GetFileAtRevision
from components.perf_test_utils import IsSha1Hash


with path_util.SysPath(path_util.GetQoraiScriptDir(), 0):
  from lib.util import extract_zip


def _GetProfileHash(profile: str, version: QoraiVersion) -> str:
  if IsSha1Hash(profile):  # the explicit profile hash
    return profile
  sha1_filepath = os.path.join(path_util.GetQoraiPerfProfileDir(),
                               f'{profile}.zip.sha1')
...
def GetProfilePath(profile: str, work_directory: str,
  ...
  if os.path.isdir(profile):  # local profile
    ...
    zip_path = os.path.join(path_util.GetQoraiPerfProfileDir(),
