...
def GetQoraiDir() -> str:
  return os.path.join(GetSrcDir(), 'qorai')


def GetQoraiPerfDir() -> str:
  return os.path.join(GetSrcDir(), 'qorai', 'tools', 'perf')
...
def GetQoraiPerfProfileDir() -> str:
  return os.path.join(GetQoraiPerfDir(), 'profiles')


def GetQoraiPerfConfigDir() -> str:
  return os.path.join(GetQoraiPerfDir(), 'configs')


def GetQoraiPerfBucket() -> str:
  return 'qorai-telemetry'


def GetDepotToolsDir() -> str:
  return os.path.join(GetQoraiDir(), 'vendor', 'depot_tools')
...
def GetQoraiScriptDir() -> str:
  return os.path.join(GetQoraiDir(), 'script')
...
def GetPageSetsDataPath(filename: str) -> str:
  return os.path.join(GetQoraiPerfDir(), 'qorai_page_sets', 'data', filename)
