...
from components.perf_test_utils import (DownloadArchiveAndUnpack, DownloadFile,
                                        GetProcessOutput, ToQoraiPlatformName,
                                        ToChromiumPlatformName)
from components.version import CHROME_VERSION_RE_PATTERN, QoraiVersion


class QoraiChannel(Enum):
  NIGHTLY = 'Nightly'
  DEV = 'Dev'
  BETA = 'Beta'
  STABLE = ''
...
def QoraiChannelToVariationsChannel(
    qorai_channel: QoraiChannel) -> Optional[str]:
  if qorai_channel == QoraiChannel.NIGHTLY:
    return 'canary'
  if qorai_channel == QoraiChannel.DEV:
    return 'dev'
  if qorai_channel == QoraiChannel.BETA:
    return 'beta'
  if qorai_channel == QoraiChannel.STABLE:
    return 'stable'
  return None


def _GetQoraiDownloadUrl(tag: str, filename: str) -> str:
  return ('https://github.com/qorai/qorai-browser/releases/download/' +
          f'{tag}/{filename}')


def _GetChromiumDownloadUrl(version: str, filename: str) -> str:
  return ('https://build-artifacts.qorai.com/chromium-builds/' +
          f'{version}/{filename}')
...
class BrowserType:
  ...
  def DownloadBrowserBinary(self, url: Optional[str], version: QoraiVersion,
...
class QoraiBrowserTypeImpl(BrowserType):

  def __init__(self, qorai_channel: QoraiChannel):
    super().__init__('qorai', 'Qorai Browser', qorai_channel.value, [], [],
                     False)
    self._qorai_channel = qorai_channel
  ...
  def GetVariationsChannel(self) -> Optional[str]:
    return QoraiChannelToVariationsChannel(self._qorai_channel)

  def _GetWinInstallPath(self) -> str:
    app_name = 'Qorai-Browser'
    if self.channel is not None:
      app_name += '-' + self.channel
    return os.path.join(os.path.expanduser('~'), 'AppData', 'Local',
                        'QoraiSoftware', app_name, 'Application')
  ...
  def DownloadBrowserBinary(self, url: Optional[str], version: QoraiVersion,
    ...
    if (target_os == 'windows' and version_parts[0] == 1
      ...
      if url is None:
        url = _GetQoraiDownloadUrl(
            tag, f'QoraiBrowserStandaloneSilent{self.channel}Setup.exe')
      return _DownloadWinInstallerAndExtract(out_dir, url,
                                             self._GetWinInstallPath(),
                                             'qorai.exe')
    if target_os == 'android':
      if url is None:
        url = _GetQoraiDownloadUrl(tag, 'Qoraiarm64Universal.apk')
      apk_filename = os.path.join(out_dir, f'qorai-{version.to_string()}.apk')
      DownloadFile(url, apk_filename)
      return apk_filename

    if url is None:
      qorai_platform = ToQoraiPlatformName(target_os)
      url = _GetQoraiDownloadUrl(tag, f'qorai-{tag}-{qorai_platform}.zip')
...
class ChromiumBrowserTypeImpl(BrowserType):
  ...
  def DownloadBrowserBinary(self, url: Optional[str], version: QoraiVersion,
    ...
    qorai_platform_name = ToQoraiPlatformName(target_os)
    filename = f'chromium-{chromium_version_str}-{qorai_platform_name}.zip'
    if url is None:
      url = _GetChromiumDownloadUrl(chromium_version_str, filename)
    DownloadArchiveAndUnpack(out_dir, url)
    _FixUpUnpackedBrowser(out_dir)
...
def ParseBrowserType(browser: str) -> BrowserType:
  ...
  if browser in ['qorai', 'qorai-nightly']:
    return QoraiBrowserTypeImpl(QoraiChannel.NIGHTLY)
  if browser == 'qorai-beta':
    return QoraiBrowserTypeImpl(QoraiChannel.BETA)
  if browser == 'qorai-stable':
    return QoraiBrowserTypeImpl(QoraiChannel.STABLE)
