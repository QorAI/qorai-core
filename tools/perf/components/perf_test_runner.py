...
from components.browser_type import QoraiVersion
...
def ReportToDashboardImpl(
    dashboard_bot_name: str, version: QoraiVersion, griffin_rev: Optional[str],
  ...
  build_properties['perf_dashboard_machine_group'] = 'QoraiPerf'
  ...
  env = os.environ.copy()
  env['DASHBOARD_EXTRA_DIAG_qorai_chrome_version'] = chromium_version_str
  env['DASHBOARD_EXTRA_DIAG_qorai_tag'] = version.to_string()
  env['DASHBOARD_EXTRA_DIAG_qorai_job_name'] = job_name
  env['DASHBOARD_EXTRA_DIAG_qorai_job_id'] = build_number

  if griffin_rev is not None:
    env['DASHBOARD_EXTRA_DIAG_qorai_variations_revisions'] = griffin_rev
...
class RunableConfiguration:
  ...
  def Install(self):
    if self.common_options.is_android and self.binary.binary_path is not None:
      if self.config.version is not None:
        ...
        if self.config.browser_type.is_qorai:
  ...
  def RebaseProfile(self) -> bool:
    ...
    rebase_benchmark.name = ('qorai_utils.online'
                             if online_rebase else 'qorai_utils.offline')
