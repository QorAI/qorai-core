...
class RunnerConfig:
  ...
  version: Optional[QoraiVersion] = None
...
def ParseVersionFromBinary(binary: str) -> QoraiVersion:
  ...
  return QoraiVersion('v' + '.'.join(version))


def ParseTarget(target: str) -> Tuple[Optional[QoraiVersion], str]:
  """
  Parse the version and location from the passed string `target`.
  target = [<version>:][<location>]
  <version> could be:
  1. Qorai tag (i.e. v1.62.1);
  2. Git hash;
  ...
  if not m:
    return None, target
  version = QoraiVersion(m.group(1))
  location = m.group(2)
