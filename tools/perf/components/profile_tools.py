...
from components.path_util import GetQoraiPerfProfileDir
...
_PR_SEE_DETAILS_LINK = ('https://github.com/qorai/qorai-core/blob/master/' +
...
def _FixupPreferences(profile_dir: str):
  ...
  # Cleanup Qorai News preferences. They could vary depending on the locale.
  with open(preferences_path, 'r', encoding='utf8') as f:
    preferences = json.load(f)
    if qorai := preferences.get('qorai'):
      qorai.pop('news', None)
      if new_tab_page := qorai.get('new_tab_page'):
        new_tab_page.pop('show_qorai_news', None)
...
def MakeUpdatedProfileArchive(cfg: RunnerConfig, options: CommonOptions,
  ...
  if options.upload:
    ...
    files[new_profile_sha1_path] = os.path.join(GetQoraiPerfProfileDir(),
                                                zip_filename + '.sha1')
    files[profile_zip_sizes] = os.path.join(GetQoraiPerfProfileDir(),
                                            sizes_filename)
    ...
    if git_tools.DoesPrOpen(branch):
      ...
      git_tools.MakeGithubPR(branch=branch,
                             ...
                             body=_PR_BODY,
                             reviewers=[git_tools.GH_QORAI_PERF_TEAM],
...
def _GetComponentGroup(name: str, path: str,
  ...
  if 'Wallet' in name:
    return 'Wallet'
  if 'Qorai' in name or re.match(r'[a-z]{32}', path) is not None:
    return 'Other'
...
def RunUpdateProfile(qorai_config: perf_config.PerfConfig,
                     chromium_config: perf_config.PerfConfig,
                     options: CommonOptions) -> bool:
  qorai_profile = _RunUpdateProfileForConfig(qorai_config, options, [])
  if not qorai_profile:
    return False

  # Take Safe Browsing database from Qorai profile
  safe_browsing = _GetSafeBrowsingDir(qorai_profile)
  extra_dirs_to_add = []
