...
class CommonOptions:
  ...
  def add_parser_args(cls, parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        '--variations-repo-dir',
        type=str,
        help='A path to qorai-variation repository to use Griffin in tests')
    ...
    parser.add_argument('--ci-mode',
                        action='store_true',
                        help='Used for CI (qorai-browser-test-perf-* builds).')
    ...
    parser.add_argument(
        ...
        help=(
            ...
            + 'and push the changes to qorai-core'))
    parser.add_argument(
        '--upload-branch',
        type=str,
        help=('[For profile updating] A target qorai-core branch to push the ' +
  ...
  def from_args(cls, args) -> 'CommonOptions':
    ...
    if options.config == 'auto':  # Select the config by machine_id and chromium
      ...
      prefix = 'chromium' if options.chromium else 'qorai'
      ...
      options.config = os.path.join(path_util.GetQoraiPerfConfigDir(), 'ci',
                                    config)
    elif options.config == 'smoke':
      args.no_report = True
      options.config = os.path.join(path_util.GetQoraiPerfConfigDir(),
                                    'smoke.json5')
