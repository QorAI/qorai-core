...
3. Reports the results to qorai-perf-dashboard.appspot.com or stores them as
   local .html files.
...
def load_config(options: CommonOptions) -> dict:
  ...
  if config.startswith('https://'):  # URL to download the config
    ...
    config_path = os.path.join(path_util.GetQoraiPerfConfigDir(), config)
...
On CI:
npm run perf_tests -- smoke-qorai.json5 v1.58.45
     --working-directory=e:\work\qorai\src\out\100
     --ci-mode
''')
  ...
  if options.mode == PerfMode.UPDATE_PROFILE:
    ...
    args.chromium = False
    options = CommonOptions.from_args(args)
    qorai_config = perf_config.PerfConfig(load_config(options))
    qorai_config.runners[0].label = 'qorai-rebase'
    ...
    return 0 if profile_tools.RunUpdateProfile(qorai_config, chromium_config,
                                               options) else 1
