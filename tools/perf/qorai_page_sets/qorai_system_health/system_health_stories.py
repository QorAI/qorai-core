...
# A page set for loading.desktop benchmark.qorai benchmark.
#
...
class _QoraiLoadingStory(system_health_story.SystemHealthStory):
...
class _QoraiMultiTabLoadingStory(_QoraiLoadingStory):
...
class LoadExampleStory2023(_QoraiLoadingStory):
...
class LoadQoraiSearchStory2023(_QoraiLoadingStory):
  NAME = 'load:site:qorai_search:2023'
  URL = 'https://search.qorai.com/search?q=cats&source=web'
  TAGS = [story_tags.YEAR_2023]


class LoadGoogleStory2023(_QoraiLoadingStory):
  NAME = 'load:site:google:2023'
  URL = 'https://www.google.com/search?q=cats'
  TAGS = [story_tags.YEAR_2023]


class LoadYoutubeStory2023(_QoraiLoadingStory):
...
class LoadWikipediaStory2023(_QoraiLoadingStory):
...
class LoadTwitterStory2024(_QoraiLoadingStory):
  NAME = 'load:site:twitter:2024'
  URL = 'https://x.com/SpaceX'
  TAGS = [story_tags.YEAR_2024]


class LoadCNNStory2023(_QoraiLoadingStory):
  NAME = 'load:site:cnn:2023'
  URL = 'https://edition.cnn.com/'
  TAGS = [story_tags.YEAR_2023]


class LoadBBCStory2023(_QoraiLoadingStory):
  NAME = 'load:site:bbc:2023'
  URL = 'https://www.bbc.com/'
  TAGS = [story_tags.YEAR_2023]


class LoadHackernewsStory2024(_QoraiLoadingStory):
...
class LoadQoraiNewsStory2024(_QoraiLoadingStory):
  ...
  def _Login(self, action_runner):
    ...
    # Resize window to 1400px to see the Qorai News block.
    ...
    assert inner_width >= 1300, 'Window size is to small for Qorai News block'


class MultiTabLoadExampleStory2024(_QoraiMultiTabLoadingStory):
