...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  std::string output;
  qorai_wallet::EncodeString(input, &output);
  qorai_wallet::EncodeStringArray(std::vector<std::string>(array_size, input),
                                  &output);

  qorai_wallet::DecodeString(offset, input, &output);
  return 0;
}
