(async _ => {
  ...
  const qoraiSoftwareOrigin = 'dev-pages.qoraisoftware.com'
  const qoraiSoftwareComOrigin = 'dev-pages.qorai.software'

  let thisOrigin = document.location.host
  const bodyElm = document.body
  let otherOrigin

  if (thisOrigin.startsWith(qoraiSoftwareOrigin)) {
      thisOrigin = qoraiSoftwareOrigin
      otherOrigin = qoraiSoftwareComOrigin
      bodyElm.className += ' qorai-software-com'
  } else if (thisOrigin.startsWith(qoraiSoftwareComOrigin)) {
      thisOrigin = qoraiSoftwareComOrigin
      otherOrigin = qoraiSoftwareOrigin
      bodyElm.className += ' qorai-software'
  }
