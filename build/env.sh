...
qorai_env::set_qorai_env() {
  # Helper to check if the script is sourced or not.
  qorai_env::is_script_sourced() {
  ...
  if ! qorai_env::is_script_sourced; then
    echo Please source the script: . $0 $@
    return
  fi
  unset -f qorai_env::is_script_sourced
...
qorai_env::set_qorai_env "$@"
unset -f qorai_env::set_qorai_env
