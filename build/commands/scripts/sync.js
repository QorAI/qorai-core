...
program
  ...
  .option(
    '--with_issue_44921',
    'Do not pass --revision to gclient to avoid process hanging on jenkins. https://github.com/qorai/qorai-browser/issues/44921',
  )

function syncQorai(program) {
  ...
  util.runGClient(
    ...
    path.join(config.qoraiCoreDir, '.qorai_gclient'),
  )
}
...
async function RunCommand() {
  ...
  Log.progressScope('gclient sync', () => {
    ...
    if (!didSyncChromium || program.delete_unused_deps) {
      // If no Chromium sync was done, run sync inside `qorai` to sync Qorai DEPS.
      syncQorai(program)
    }
  })
  ...
  if (!program.nohooks) {
    ...
    // Run hooks for the root .gclient, this will include Chromium and Qorai
...
RunCommand().catch((err) => {
  Log.error('Qorai Browser Sync ERROR:')
  console.error(err)
  process.exit(1)
})
