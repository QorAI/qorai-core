...
module.exports = (program) =>
  program
    ...
    .action(async (buildConfig, args) => {
      ...
      // For now - if we can - only use qorai_browser_tests binary
      ...
      // as long as their symbols are present in the qorai_browser_test binary
      const hasBBT = allTestSuites.find((x) =>
        x.includes('qorai_browser_tests'),
      )
      const testSuites = hasBBT
        ? [`${config.outputDir}/qorai_browser_tests`]
        : allTestSuites
