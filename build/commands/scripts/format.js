...
async function runFormat(options = {}) {
  ...
  const filesToFormat = options.allFiles
    ? getAllFiles()
    : util.getChangedFiles(config.qoraiCoreDir, options.base, skipLogging)
...
const runPrettier = async (files, dryRun) => {
  ...
  const options = require(path.join(config.qoraiCoreDir, '.prettierrc'))
  const ignorePath = path.join(config.qoraiCoreDir, '.prettierignore')
...
const runMojomFormatForFile = async (file, dryRun) => {
  ...
  const mojomFormatAllowList = ['**/qorai_wallet/**/*.mojom']
