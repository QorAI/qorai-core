...
/**
 * Checks to make sure the src/chrome/VERSION matches qorai-core's package.json version
 */
const checkVersionsMatch = () => {
  ...
  const qoraiVersionFromChromeFile = `${found[2]}.${found[3]}.${found[4]}`
  if (qoraiVersionFromChromeFile !== config.qoraiVersion) {
    // Only a warning. The CI environment will choose to proceed or not within its own script.
    Log.warn(
      `Version files do not match!\n`
        + `src/chrome/VERSION: ${qoraiVersionFromChromeFile}\n`
        + `qorai-core configured version: ${config.qoraiVersion}\n`
