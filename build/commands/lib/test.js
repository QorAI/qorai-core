...
const runTests = async (
  ...
  if (!isJunitTestSuite) {
    qoraiArgs.push('--enable-logging=stderr')
  }

  // Android and ios don't support --v
  if (!config.isMobile()) {
    qoraiArgs.push('--v=' + options.v)
  ...
  if (options.filter) {
    qoraiArgs.push('--gtest_filter=' + options.filter)
  }

  if (options.run_disabled_tests) {
    if (config.isIOS()) {
      qoraiArgs.push('--gtest_also_run_disabled_tests')
    } else {
      qoraiArgs.push('--output-disabled-tests')
    }
  }

  if (options.disable_qorai_extension && !config.isIOS()) {
    qoraiArgs.push('--disable-qorai-extension')
  }

  if (options.single_process && !config.isIOS()) {
    qoraiArgs.push('--single_process')
  }
  ...
  if (!isJunitTestSuite) {
    if (
      ...
      qoraiArgs.push('--test-launcher-jobs=' + options.test_launcher_jobs)
    }
    ...
    if (jsonResultsArg) {
      qoraiArgs.push(jsonResultsArg)
    }
  }

  if (suite === 'qorai_unit_tests' && config.isTeamcity && !config.isMobile()) {
  ...
  testsToRun.every((testSuite) => {
    let runArgs = qoraiArgs.slice()
    let runOptions = config.defaultOptions

    // Filter out upstream tests that are known to fail for Qorai
...
const runChromiumTestLauncherTeamcityReporterIntegrationTests = (config) => {
  const generalTestCase = {
    ...
    expectedLines: [
      "##teamcity[testSuiteStarted name='qorai_unit_tests']",
      ...
      "##teamcity[testStarted name='DISABLED_TeamcityReporterIntegrationTest.Skipped'",
      "##teamcity[testFinished name='DISABLED_TeamcityReporterIntegrationTest.Skipped'",
      "##teamcity[testSuiteFinished name='qorai_unit_tests']",
    ],
  }
  ...
  for (const testCase of [generalTestCase, ignorePreliminaryFailuresTestCase]) {
    const prog = util.run(
      path.join(config.outputDir, 'qorai_unit_tests'),
      testCase.args,
      runOptions,
    )
