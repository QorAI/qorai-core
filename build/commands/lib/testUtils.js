...
// TODO(https://github.com/qorai/qorai-browser/issues/48118): is there a better way of doing this?
const gnTargetToExecutableName = (str) => str.split(':').at(-1).split('(')[0]

const getTestBinary = (config, suite) => {
  let testBinary = suite
  if (testBinary === 'qorai_java_unit_tests') {
    testBinary = path.join('bin', 'run_qorai_java_unit_tests')
  } else if (testBinary === 'qorai_junit_tests') {
    testBinary = path.join('bin', 'run_qorai_junit_tests')
  }
