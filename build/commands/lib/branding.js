...
exports.update = () => {
  ...
  const qoraiComponentsDir = path.join(config.qoraiCoreDir, 'components')
  const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
  const qoraiAppDir = path.join(config.qoraiCoreDir, 'app')
  ...
  const qoraiBrowserResourcesDir = path.join(
    config.qoraiCoreDir,
    'browser',
    'resources',
  )
  const qoraiAppVectorIconsDir = path.join(config.qoraiCoreDir, 'components')
  ...
  const qoraiAndroidJavaStringsTranslationsDir = path.join(
    config.qoraiCoreDir,
    'browser',
    'ui',
    'android',
  ...
  const qoraiAndroidTabUiJavaStringsTranslationsDir = path.join(
    config.qoraiCoreDir,
    'android',
    'features',
    'tab_ui',
  ...
  let fileMap = new Set()
  const autoGeneratedQoraiToChromiumMapping = Object.assign(
    {},
    l10nUtil.getAutoGeneratedQoraiToChromiumMapping(),
  )
  // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
  autoGeneratedQoraiToChromiumMapping[
    path.join(qoraiAppDir, 'qorai_strings.grd')
  ] = path.join(chromeAppDir, 'qorai_strings.grd')
  autoGeneratedQoraiToChromiumMapping[
    path.join(qoraiAppDir, 'settings_qorai_strings.grdp')
  ] = path.join(chromeAppDir, 'settings_qorai_strings.grdp')
  autoGeneratedQoraiToChromiumMapping[
    path.join(qoraiComponentsDir, 'components_qorai_strings.grd')
  ] = path.join(chromeComponentsDir, 'components_qorai_strings.grd')

  Object.entries(autoGeneratedQoraiToChromiumMapping).forEach((mapping) =>
    fileMap.add(mapping),
  )

  // Copy xtb files for:
  // qorai/app/resources/chromium_strings*.xtb
  // qorai/app/resources/generated_resoruces*.xtb
  // qorai/components/strings/components_chromium_strings*.xtb
  // qorai/browser/ui/android/strings/translations/android_chrome_strings*.xtb
  // qorai/android/features/tab_ui/java/strings/translations/android_chrome_tab_ui_strings*.xtb
  fileMap.add([
    path.join(qoraiAppDir, 'resources'),
    path.join(chromeAppDir, 'resources'),
  ])
  fileMap.add([
    path.join(qoraiComponentsDir, 'strings'),
    path.join(chromeComponentsDir, 'strings'),
  ])
  fileMap.add([
    qoraiAndroidJavaStringsTranslationsDir,
    chromeAndroidJavaStringsTranslationsDir,
  ])
  fileMap.add([
    qoraiAndroidTabUiJavaStringsTranslationsDir,
    chromeAndroidTabUiJavaStringsTranslationsDir,
  ])
  // By overwriting, we don't need to modify some grd files.
  fileMap.add([
    path.join(qoraiAppDir, 'theme', 'qorai'),
    path.join(chromeAppDir, 'theme', 'qorai'),
  ])
  fileMap.add([
    path.join(qoraiAppDir, 'theme', 'default_100_percent', 'qorai'),
    path.join(chromeAppDir, 'theme', 'default_100_percent', 'qorai'),
  ])
  fileMap.add([
    path.join(qoraiAppDir, 'theme', 'default_200_percent', 'qorai'),
    path.join(chromeAppDir, 'theme', 'default_200_percent', 'qorai'),
  ])
  fileMap.add([
    path.join(qoraiAppDir, 'theme', 'default_100_percent', 'common'),
    path.join(chromeAppDir, 'theme', 'default_100_percent', 'common'),
  ])
  fileMap.add([
    path.join(qoraiAppDir, 'theme', 'default_200_percent', 'common'),
    path.join(chromeAppDir, 'theme', 'default_200_percent', 'common'),
  ])
  fileMap.add([
    path.join(qoraiComponentsDir, 'resources', 'default_100_percent'),
    path.join(chromeComponentsDir, 'resources', 'default_100_percent'),
  ])
  fileMap.add([
    path.join(qoraiComponentsDir, 'resources', 'default_100_percent', 'qorai'),
    path.join(
      chromeComponentsDir,
      'resources',
      'default_100_percent',
  ...
  fileMap.add([
    path.join(qoraiComponentsDir, 'resources', 'default_200_percent'),
    path.join(chromeComponentsDir, 'resources', 'default_200_percent'),
  ])
  fileMap.add([
    path.join(qoraiComponentsDir, 'resources', 'default_200_percent', 'qorai'),
    path.join(
      chromeComponentsDir,
      'resources',
      'default_200_percent',
  ...
  fileMap.add([
    path.join(qoraiAppVectorIconsDir, 'vector_icons', 'qorai'),
    path.join(chromeComponentsDir, 'vector_icons', 'qorai'),
  ])
  // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with qorai's on Win8.
  fileMap.add([
    path.join(qoraiBrowserResourcesDir, 'chrome-logo-faded.png'),
    path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png'),
  ])
  fileMap.add([
    path.join(
      qoraiBrowserResourcesDir,
      'downloads',
      'images',
  ...
  fileMap.add([
    path.join(qoraiBrowserResourcesDir, 'settings', 'images'),
    path.join(chromeBrowserResourcesDir, 'settings', 'images'),
  ])
  fileMap.add([
    path.join(qoraiBrowserResourcesDir, 'signin', 'images'),
  ...
  fileMap.add([
    path.join(qoraiBrowserResourcesDir, 'signin', 'profile_picker', 'images'),
    path.join(chromeBrowserResourcesDir, 'signin', 'profile_picker', 'images'),
  ])
  fileMap.add([
    path.join(qoraiBrowserResourcesDir, 'side_panel', 'reading_list', 'images'),
    path.join(
      chromeBrowserResourcesDir,
      'side_panel',
      'reading_list',
  ...
  // Replace chrome dark logo with channel specific qorai logo.
  fileMap.add([
    path.join(
      config.qoraiCoreDir,
      'node_modules',
  ...
  if (config.targetOS === 'mac') {
    ...
    // Each channel's app icons are stored in qorai/app/theme/$channel/app.icns.
    ...
    // stored in qorai/app/theme/$channel/Assets.car. With this copying, we
    ...
    const brandingDest = path.join(chromeAppDir, 'theme', 'qorai', 'BRANDING')
  ...
  if (config.targetOS === 'android') {
    let qoraiOverwrittenFiles = new Set()
    const removeUnlistedAndroidResources = (qoraiOverwrittenFiles) => {
      ...
      for (const untrackedChromiumPath of untrackedChromiumPaths) {
        if (
          ...
          && !qoraiOverwrittenFiles.has(untrackedChromiumPath)
        ) {
    ...
    if (config.channel === 'development') {
      androidIconSet = 'res_qorai_default'
    } else if (config.channel === '') {
      androidIconSet = 'res_qorai'
    } else if (config.channel === 'beta') {
      androidIconSet = 'res_qorai_beta'
    } else if (config.channel === 'dev') {
      androidIconSet = 'res_qorai_dev'
    } else if (config.channel === 'nightly') {
      androidIconSet = 'res_qorai_nightly'
    }
    ...
    // Mapping for copying Qorai's Android resource into chromium folder.
      ...
      ([sourcePath, destPaths]) => {
        ...
        for (const destPath of destPaths) {
          for (const androidSourceFile of androidSourceFiles) {
            ...
            qoraiOverwrittenFiles.add(destinationFile)
          }
        }
      },
    )
    removeUnlistedAndroidResources(qoraiOverwrittenFiles)
  }
