...
// Use the same filename as for Qorai archive.
  ...
  'win': {
    ...
    processArtifacts: () => {
      ...
      util.run(
        'python3',
        [
          path.join(config.qoraiCoreDir, 'script', 'repack-archive.py'),
          `--input=${input}`,
          `--output=${output}`,
...
// 2. Qorai Release build configuration
...
function buildChromiumRelease(buildOptions = {}) {
  if (!config.isCI && !buildOptions.force) {
    console.error(
      'Warning: the command resets all changes in src/ folder.\n'
        + 'src/qorai stays untouched. Pass --force to continue.',
    )
    return 1
  }
  ...
  Log.progressScope(`remove recursive symlinks`, () => {
    // node_modules could have a symlink to src/qorai. The recursive symlinks
