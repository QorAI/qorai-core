...
function printFailedPatchesInJsonFormat(allPatchStatus, qoraiPath) {
  ...
  const patchFailuresOutput = failedPatches.map(
    ({ path, patchPath, reason }) => {
      return {
        // Trimming the patch path to be relative to the qorai core repo. We skip
        // the first character to avoid the trailing slash from the absolute
        // path.
        patchPath: patchPath.replace(qoraiPath, '').substring(1),
