...
const asGnTarget = (file) => {
  const gnAbsolutePath = file.startsWith('//') ? file : '//qorai/' + file

  return gnAbsolutePath.replace('qorai/chromium_src/', '')
}
...
async function analyzeAffectedTests(
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/48117): analyze impact of patch files
  if (modifiedFiles.find((x) => x.startsWith('//qorai/patches'))) {
    if (!quiet) {
      console.warn(
...
async function getAffectedTests(args = {}) {
  ...
  const testAffectedDueModifiedFilterFiles = allTestExecutablesToConsider
    ...
    .filter(({ filter }) =>
      modified.has('//qorai/' + path.relative(config.srcDir, filter)),
    )
    .map(({ test }) => test)
