...
async function generateInstrumentationFile(instrumentationFile) {
  ...
  const paths = files.map(x => `../../qorai/${x}`)
...
async function applyPatches(printPatchFailuresInJson) {
  ...
  const coreRepoPath = config.qoraiCoreDir
  ...
  if (printPatchFailuresInJson) {
    Log.printFailedPatchesInJsonFormat(allPatchStatus, config.qoraiCoreDir)
  } else {
...
const util = {
  ...
  touchOverriddenFiles: () => {
    ...
    const chromiumSrcDir = path.join(config.srcDir, 'qorai', 'chromium_src')
  ...
  touchGsutilChangeLogFile: () => {
    ...
    // https://github.com/qorai/qorai-browser/issues/31641
  ...
  buildNativeRedirectCC: async () => {
    ...
    const buildArgs = {
      'import("//qorai/tools/redirect_cc/args.gni")': null,
    ...
    util.runGnGen(config.nativeRedirectCCDir, buildArgs, [
      '--root-target=//qorai/tools/redirect_cc',
    ])
    await util.buildTargets(
      ['qorai/tools/redirect_cc'],
  ...
  presubmit: (options = {}) => {
    ...
    // 'gerrit.host' from their qorai checkout.
    util.runGit(
      config.qoraiCoreDir,
    ...
    cmdOptions.cwd = config.qoraiCoreDir
