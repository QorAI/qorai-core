...
// Choose which qorai-core build directory to look for pre-compiled
...
if (fs.existsSync(outputPath)) {
  console.log(
    'Assuming precompiled dependencies can be found at the existing path found from qorai-core configuration: '
      + outputPath,
  )
} else {
  ...
  if (!outDirectories.length) {
    throw new Error(
      'Cannot find any qorai-core build output directories. Have you run a qorai-core build yet with the specified (or default) configuration?',
    )
  }
