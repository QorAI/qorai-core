...
const runPerfTests = (
  ...
  args = [
    path.join(config.qoraiCoreDir, 'tools', 'perf', 'run_perftests.py'),
    perfConfig,
  ]

  if (perfConfig === 'smoke') {
    // Launch python unit tests before running perf tests.
    const perfDir = path.join(config.qoraiCoreDir, 'tools', 'perf')
  ...
  if (['Static', 'Release', 'Component'].includes(targetBuildConfig)) {
    ...
    binaryPath = path.join(config.outputDir, 'qorai')
    ...
    const qoraiCoreCommit = util.runGit(
      config.qoraiCoreDir,
      ['rev-parse', 'HEAD'],
      true,
    )
    targetBuildConfig = `${qoraiCoreCommit}:${binaryPath}`
  }
