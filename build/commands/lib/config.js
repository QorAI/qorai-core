// Copyright (c) 2016 The Qorai Authors. All rights reserved.
...
const qoraiCoreDir = path.join(rootDir, 'src', 'qorai')
...
const packageConfig = function (key, sourceDir = qoraiCoreDir) {
  let packages = { config: {} }
...
const getEnvConfig = (key, defaultValue = undefined) => {
  if (!envConfig) {
    ...
    // Parse src/qorai/.env with all included env files.
    let envConfigPath = path.join(qoraiCoreDir, '.env')
    if (fs.existsSync(envConfigPath)) {
      ...
      const defaultEnvConfigContent =
        ...
        + '# See for details: https://github.com/qorai/qorai-browser/wiki/Build-configuration\n'
...
const getQoraiVersion = (ignorePatchVersionNumber) => {
  const qoraiVersion = packageConfig(['version'])
  if (!ignorePatchVersionNumber) {
    return qoraiVersion
  }

  const qoraiVersionParts = qoraiVersion.split('.')
  assert(qoraiVersionParts.length === 3)
  qoraiVersionParts[2] = '0'
  return qoraiVersionParts.join('.')
}
...
const Config = function () {
  ...
  this.qoraiCoreDir = qoraiCoreDir
  this.buildToolsDir = path.join(this.srcDir, 'build')
  this.resourcesDir = path.join(this.rootDir, 'resources')
  this.depotToolsDir = getDepotToolsDir(this.qoraiCoreDir)
  ...
  this.gClientFile = process.env.QORAI_GCLIENT_FILE || this.defaultGClientFile
  ...
  this.ignorePatchVersionNumber =
    !this.isQoraiReleaseBuild()
    && getEnvConfig(['ignore_patch_version_number'], !this.isCI)
  this.qoraiVersion = getQoraiVersion(this.ignorePatchVersionNumber)
  this.qoraiIOSMarketingPatchVersion =
    getEnvConfig(['qorai_ios_marketing_version_patch']) || ''
  this.androidOverrideVersionName = this.qoraiVersion
  this.releaseTag = this.qoraiVersion.split('+')[0]
  ...
  this.extraGnArgs = {}
  this.extraGnGenOpts = getEnvConfig(['qorai_extra_gn_gen_opts']) || ''
  this.extraNinjaOpts = []
  this.sisoJobsLimit = undefined
  this.qoraiAndroidSafeBrowsingApiKey = getEnvConfig([
    'qorai_safebrowsing_api_key',
  ])
  this.qoraiAndroidDeveloperOptionsCode = getEnvConfig([
    'qorai_android_developer_options_code',
  ])
  this.qoraiAndroidKeystorePath = getEnvConfig(['qorai_android_keystore_path'])
  this.qoraiAndroidKeystoreName = getEnvConfig(['qorai_android_keystore_name'])
  this.qoraiAndroidKeystorePassword = getEnvConfig([
    'qorai_android_keystore_password',
  ])
  this.qoraiAndroidKeyPassword = getEnvConfig(['qorai_android_key_password'])
  this.qoraiAndroidPkcs11Provider = ''
  this.qoraiAndroidPkcs11Alias = ''
  ...
  this.useQoraiHermeticToolchain = this.rbeService.includes('.qorai.com:')
  this.qoraiIOSDeveloperOptionsCode = getEnvConfig([
    'qorai_ios_developer_options_code',
  ])
  ...
  this.forwardEnvArgsToGn = [
    ...
    'qorai_android_developer_options_code',
    'qorai_google_api_endpoint',
    'qorai_google_api_key',
    'qorai_infura_project_id',
    'qorai_safebrowsing_api_key',
    'qorai_services_dev_domain',
    'qorai_services_key_id',
    'qorai_services_production_domain',
    'qorai_services_staging_domain',
    'qorai_stats_api_key',
    'qorai_stats_updater_url',
    'qorai_sync_endpoint',
    'qorai_variations_server_url',
    'concurrent_links',
    'dcheck_always_on',
    'enable_updater',
...
Config.prototype.isQoraiReleaseBuild = function () {
  const isQoraiReleaseBuildValue = getEnvConfig(['is_qorai_release_build'])
  if (isQoraiReleaseBuildValue !== undefined) {
    assert(
      isQoraiReleaseBuildValue === 0 || isQoraiReleaseBuildValue === 1,
      'Bad is_qorai_release_build value (should be 0 or 1)',
    )
    return isQoraiReleaseBuildValue === 1
  }
...
Config.prototype.getQoraiLogoIconName = function () {
  let iconName = 'qorai-icon-dev-color.svg'
  if (this.isQoraiReleaseBuild()) {
    if (this.channel === 'beta') {
      iconName = 'qorai-icon-beta-color.svg'
    } else if (this.channel === 'nightly') {
      iconName = 'qorai-icon-nightly-color.svg'
    } else {
      iconName = 'qorai-icon-release-color.svg'
    }
  }
  return iconName
}
...
Config.prototype.buildArgs = function () {
  const version = this.qoraiVersion
  let versionParts = version.split('+')[0]
  versionParts = versionParts.split('.')

  let args = {
    'import("//qorai/build/args/qorai_defaults.gni")': null,
    is_asan: this.isAsan(),
    ...
    qorai_channel: this.channel,
    qorai_version_major: versionParts[0],
    qorai_version_minor: versionParts[1],
    qorai_version_build: versionParts[2],
  ...
  if (this.targetOS !== 'ios') {
    args['import("//qorai/build/args/blink_platform_defaults.gni")'] = null
  } else {
    args['import("//qorai/build/args/ios_defaults.gni")'] = null
  }
  if (this.targetOS === 'android') {
    args['import("//qorai/build/args/android_defaults.gni")'] = null
  }
  if (this.targetOS !== 'ios' && this.targetOS !== 'android') {
    args['import("//qorai/build/args/desktop_defaults.gni")'] = null
  }
  ...
  if (this.ignorePatchVersionNumber) {
    assert(!this.isQoraiReleaseBuild())
  ...
  if (this.shouldSign()) {
    if (this.targetOS === 'mac') {
      ...
      args.qorai_android_keystore_path = this.qoraiAndroidKeystorePath
      args.qorai_android_keystore_name = this.qoraiAndroidKeystoreName
      args.qorai_android_keystore_password = this.qoraiAndroidKeystorePassword
      args.qorai_android_key_password = this.qoraiAndroidKeyPassword
      if (this.qoraiAndroidPkcs11Provider && this.qoraiAndroidPkcs11Alias) {
        args.qorai_android_pkcs11_provider = this.qoraiAndroidPkcs11Provider
        args.qorai_android_pkcs11_alias = this.qoraiAndroidPkcs11Alias
      }
    }
  }
  ...
  if (this.targetOS === 'linux') {
    if (this.targetArch !== 'x86') {
      ...
      // https://github.com/qorai/qorai-browser/issues/1024#issuecomment-1175397914
      args.use_vaapi = true
    }
  }
  ...
  if (this.targetOS === 'android') {
    ...
    if (!this.isReleaseBuild()) {
      ...
      args.chrome_public_manifest_package = 'com.qorai.browser_default'
    } else if (this.channel === '') {
      args.android_channel = 'stable'
      args.chrome_public_manifest_package = 'com.qorai.browser'
    } else if (this.channel === 'beta') {
      args.chrome_public_manifest_package = 'com.qorai.browser_beta'
    } else if (this.channel === 'dev') {
      args.chrome_public_manifest_package = 'com.qorai.browser_dev'
    } else if (this.channel === 'nightly') {
      args.android_channel = 'canary'
      args.chrome_public_manifest_package = 'com.qorai.browser_nightly'
    }
    ...
    args.qorai_android_developer_options_code =
      this.qoraiAndroidDeveloperOptionsCode
    args.qorai_safebrowsing_api_key = this.qoraiAndroidSafeBrowsingApiKey
  ...
  if (this.targetOS === 'ios') {
    ...
    if (this.qoraiIOSMarketingPatchVersion) {
      args.qorai_ios_marketing_version_patch =
        this.qoraiIOSMarketingPatchVersion
    }
    ...
    args.qorai_ios_developer_options_code = this.qoraiIOSDeveloperOptionsCode

    delete args.safebrowsing_api_endpoint
    delete args.enable_hangout_services_extension
    delete args.qorai_google_api_endpoint
    delete args.qorai_google_api_key
    delete args.qorai_stats_updater_url
...
Config.prototype.shouldSign = function () {
  ...
  if (this.targetOS === 'android') {
    return this.qoraiAndroidKeystorePath !== undefined
  }
...
Config.prototype.updateInternal = function (options) {
  ...
  if (options.xcode_gen) {
    ...
    if (options.xcode_gen === 'ios') {
      this.xcode_gen_target = '//qorai/ios:*'
    } else {
  ...
  if (options.pkcs11Provider) {
    this.qoraiAndroidPkcs11Provider = options.pkcs11Provider
  }

  if (options.pkcs11Alias) {
    this.qoraiAndroidPkcs11Alias = options.pkcs11Alias
  }
}
...
Object.defineProperty(Config.prototype, 'defaultOptions', {
  get: function () {
    ...
    const pythonPaths = [
      ...
      ['qorai', 'vendor', 'requests'],
      ['qorai', 'third_party', 'cryptography'],
      ['qorai', 'third_party', 'macholib'],
      ['build'],
    ...
    env.TARGET_ARCH = this.gypTargetArch // for qorai scripts
    ...
    if (this.channel) {
      env.QORAI_CHANNEL = this.channel
    }

    if (!this.useQoraiHermeticToolchain) {
      env.DEPOT_TOOLS_WIN_TOOLCHAIN = '0'
    } else {
      // Use hermetic toolchain only internally.
      env.USE_QORAI_HERMETIC_TOOLCHAIN = '1'
