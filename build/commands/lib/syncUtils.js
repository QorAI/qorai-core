...
function buildDefaultGClientConfig(
  ...
  const items = [
    {
      ...
      custom_vars: {
        ...
        'checkout_pgo_profiles': config.isQoraiReleaseBuild(),
      },
    },
  ]
  ...
  if (!onlyChromium) {
    items.push({
      managed: false,
      name: 'src/qorai',
      // We do not use gclient to manage qorai-core, so this should not
      // actually get used.
...
function syncChromium(program) {
  ...
  const latestSyncInfoFilePath = path.join(
    config.rootDir,
    '.qorai_latest_successful_sync.json',
  )
  ...
  if (deleteUnusedDeps) {
    if (util.isGitExclusionExists(config.srcDir, 'qorai/')) {
      args.push('-D')
    } else if (!config.isCI) {
      Log.warn(
        '--delete_unused_deps is ignored because sync has not yet added '
          + 'the exclusion for the src/qorai/ directory, likely because sync '
  ...
  util.addGitExclusion(config.srcDir, 'qorai/')
...
async function checkInternalDepsEndpoint() {
  if (!config.useQoraiHermeticToolchain) {
    return true
  }
