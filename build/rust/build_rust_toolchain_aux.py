...
def restore_config_toml_template():
    args = [
        'git', '-C',
        qorai_chromium_utils.get_src_dir(), 'checkout', '--',
        os.path.join(qorai_chromium_utils.wspath(TOOLS_RUST),
...
def run_xpy():
    with qorai_chromium_utils.sys_path(TOOLS_RUST):
        import build_rust
...
def main():
    ...
    os.chdir(qorai_chromium_utils.wspath(TOOLS_RUST))
