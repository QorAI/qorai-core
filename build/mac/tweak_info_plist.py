...
def _OverrideVersionKey(plist, qorai_version):
    """ `minor.build` version string is used for update.
    When we begin to use the Major version component, Qorai version string will
    ...
    version_values = qorai_version.split('.')
...
def Main():
    ...
    parser.add_argument('--qorai_channel', dest='qorai_channel', action='store',
        default=None, help='Channel (beta, dev, nightly)')
    parser.add_argument('--qorai_product_dir_name',
                        dest='qorai_product_dir_name',
                        action='store',
                        default=None,
                        help='Product directory name')
    parser.add_argument('--qorai_eddsa_key',
                        dest='qorai_eddsa_key',
                        action='store',
                        default=None,
                        help='Public EdDSA key for update')
    parser.add_argument('--qorai_version', dest='qorai_version', action='store',
        default=None, help='qorai version string')
    ...
    if args.skip_signing and args.qorai_channel != "":
        plist['KSChannelID'] = args.qorai_channel
    elif 'KSChannelID' in plist:
        # 'KSChannelID' is set at _modify_plists() of modification.py only
        # during signing
        del plist['KSChannelID']

    if args.qorai_product_dir_name:
        plist['CrProductDirName'] = args.qorai_product_dir_name

    if args.qorai_eddsa_key:
        plist['SUPublicEDKey'] = args.qorai_eddsa_key

    _OverrideVersionKey(plist, args.qorai_version)
