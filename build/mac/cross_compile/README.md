# Cross-compiling Qorai

Out of the box, it is possible to compile unsigned Qorai for macOS in Linux. The
code in this folder additionally makes it possible to codesign Qorai, and build
...
After the above, you can compile Qorai for macOS with the following command:
...
`"checkout_dmg_tool": True` in the `custom_vars` section of the `src/qorai`
...
```
npm run build -- Static --target_os=mac --target_arch=x64 \
    --target=qorai/build/mac:create_dmg
```
...
After the above steps, you should be able to create a PKG installer for Qorai
via the command:

```
npm run build -- Static --target_os=mac --target_arch=x64 \
    --target=qorai/build/mac:create_pkg
```
...
Now you can codesign Qorai via:

```
npm run build -- Static --target_os=mac --target_arch=x64 \
    --mac_signing_identifier=MyCert --target=qorai/build/mac:sign_app
```
