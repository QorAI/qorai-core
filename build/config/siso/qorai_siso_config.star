...
"""Siso configuration adjustments for QorAI browser."""
...
__RULES_TO_REMOVE = [
    ...
    # revisited when SISO becomes first class citizen in QorAI.
...
# `touchOverriddenFiles()` in //qorai/build/commands/lib/util.js .
__QORAI_CHROMIUM_SRC_FILE_EXTENSIONS = (
    '.css',
    '.html',
    '.icon',
    '.json',
...
# runhooks by //qorai/third_party/reclient_configs/qorai_custom/qorai_custom.py.
...
# runhooks by //qorai/third_party/reclient_configs/src/configure_reclient.py.
# The path should be relative to working directory which is `out/<config>`.
...
# Main configuration function that sets up SISO for QorAI-specific build
...
def __remove_rules(step_config):
    ...
    def should_remove(rule_name):
        # Remove rules that are not supported by QorAI.
...
def __redirect_cc_handler(ctx, cmd):
    ...
    if source_file.startswith('../../'):
        ...
        if len(source_file_parts) > 1 and source_file_parts[1] == 'gen':
            override_file = '../../qorai/chromium_src/' + '/'.join(
                source_file_parts[2:])
...
def __wrap_python_with_chromium_src_inputs_handler(rule, handlers,
    ...
    if new_handler_name not in handlers:
        ...
        # Create handler with QorAI-specific utility files and other inputs.
        handlers[new_handler_name] = __create_handler([
            "qorai/script/qorai_chromium_utils.py",
            "qorai/script/override_utils.py",
        ] + fixed_inputs)
...
def __chromium_src_inputs_handler(ctx, cmd, fixed_inputs):
    ...
    for input in cmd.inputs:
        ...
        if input.endswith(__QORAI_CHROMIUM_SRC_FILE_EXTENSIONS):
            override_file = "qorai/chromium_src/" + input
