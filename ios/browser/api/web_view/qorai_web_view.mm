...
#include "qorai/ios/browser/api/web_view/autofill/qorai_web_view_autofill_client.h"
...
#include "qorai/ios/browser/ui/webui/ai_chat/ai_chat_ui_page_handler_bridge_holder.h"
...
@interface QoraiNavigationAction ()
...
class QoraiWebViewWebStatePolicyDecider : public web::WebStatePolicyDecider {
 public:
  QoraiWebViewWebStatePolicyDecider(web::WebState* web_state,
                                    QoraiWebView* web_view)
  ...
  void ShouldAllowRequest(
    ...
    id<QoraiWebViewNavigationDelegate> delegate = web_view_.navigationDelegate;
    if ([delegate respondsToSelector:@selector
                  (webView:
                      decidePolicyForQoraiNavigationAction:decisionHandler:)]) {
      QoraiNavigationAction* navigationAction =
          [[QoraiNavigationAction alloc] initWithRequest:request
                                             requestInfo:request_info];

      __block web::WebStatePolicyDecider::PolicyDecisionCallback
          block_callback = std::move(callback);
      [delegate webView:web_view_
          decidePolicyForQoraiNavigationAction:navigationAction
                               decisionHandler:^(
 ...
 private:
  ...
  __weak QoraiWebView* web_view_ = nil;
};

// A WebStateUserData to hold a reference to a corresponding QoraiWebView.
class QoraiWebViewHolder : public web::WebStateUserData<QoraiWebViewHolder> {
 public:
  explicit QoraiWebViewHolder(web::WebState* web_state, QoraiWebView* web_view)
      : web_view_(web_view) {}
  QoraiWebView* web_view() const { return web_view_; }

 private:
  friend class web::WebStateUserData<QoraiWebViewHolder>;
...
@implementation QoraiNavigationAction
- (instancetype)initWithRequest:(NSURLRequest*)request
                    requestInfo:
                        (web::WebStatePolicyDecider::RequestInfo)requestInfo {
...
@interface QoraiWebView ()
@property(nonatomic, weak) id<AIChatUIHandlerBridge> aiChatUIHandler;
@end

@implementation QoraiWebView {
  std::unique_ptr<QoraiWebViewWebStatePolicyDecider> _webStatePolicyDecider;
}
...
- (void)dealloc {
  if (self.webState) {
    QoraiWebViewHolder::RemoveFromWebState(self.webState);
  }
}
...
+ (nullable QoraiWebView*)qoraiWebViewForWebState:(web::WebState*)webState {
  ...
  QoraiWebViewHolder* holder = QoraiWebViewHolder::FromWebState(webState);
  if (!holder) {
...
- (void)resetWebStateWithCoder:(NSCoder*)coder
  ...
  QoraiWebViewHolder::CreateForWebState(self.webState, /*web_view=*/self);

  _webStatePolicyDecider =
      std::make_unique<QoraiWebViewWebStatePolicyDecider>(self.webState, self);
}
...
- (CWVAutofillController*)newAutofillController {
  ...
  auto passwordManagerClient =
      std::make_unique<QoraiWebViewPasswordManagerClient>(
  ...
  return [[CWVAutofillController alloc]
       ...
       createAutofillClient:
           base::BindRepeating(&autofill::QoraiWebViewAutofillClientIOS::Create)
              autofillAgent:autofillAgent
...
@implementation QoraiWebView (AIChat)
