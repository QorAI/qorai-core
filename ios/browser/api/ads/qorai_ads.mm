...
#import "qorai/ios/browser/api/ads/qorai_ads.h"
...
#import "qorai/build/ios/mojom/cpp_transformations.h"
...
#include "qorai/components/qorai_ads/core/public/ads_client/ads_client_notifier.h"
...
#include "qorai/components/ntp_background_images/browser/new_tab_takeover_infobar_util.h"
...
#import "qorai/ios/browser/api/ads/qorai_ads.mojom.objc+private.h"
#import "qorai/ios/browser/api/ads/inline_content_ad_ios.h"
#import "qorai/ios/browser/api/ads/new_tab_page_ad_ios.h"
#import "qorai/ios/browser/api/ads/notification_ad_ios.h"
...
#include "qorai/ios/browser/qorai_ads/virtual_pref_provider_delegate_ios.h"
...
NSString* const kQoraiAdsFirstRunAtPrefName =
    base::SysUTF8ToNSString(qorai_ads::prefs::kFirstRunAt);
...
#define BATClassAdsBridge(__type, __objc_getter, __objc_setter, __cpp_var) \
  +(__type)__objc_getter {                                                 \
    return qorai_ads::__cpp_var;                                           \
  }                                                                        \
  +(void)__objc_setter : (__type)newValue {                                \
    qorai_ads::__cpp_var = newValue;                                       \
  }
...
constexpr NSString* kComponentUpdaterMetadataPrefKey =
    @"QoraiAdsComponentUpdaterMetadata";
...
- (instancetype)initWithNotificationAdInfo:
    (const qorai_ads::NotificationAdInfo&)info;
@end

@interface InlineContentAdIOS ()
- (instancetype)initWithInlineContentAdInfo:
    (const qorai_ads::InlineContentAdInfo&)info;
@end

@interface NewTabPageAdIOS ()
- (instancetype)initWithNewTabPageAdInfo:
    (const qorai_ads::NewTabPageAdInfo&)info;
@end

@interface QoraiAds () <AdsClientBridge> {
  std::unique_ptr<qorai_ads::AdsClientNotifier> adsClientNotifier;
  std::unique_ptr<qorai_ads::VirtualPrefProvider> virtualPrefProvider;
  raw_ptr<qorai_ads::AdsServiceImplIOS> adsService;
  nw_path_monitor_t networkMonitor;
  dispatch_queue_t monitorQueue;
}

// TODO(https://github.com/qorai/qorai-browser/issues/33730): Unify Qorai Ads
// common operations.
@property(nonatomic) QoraiCommonOperations* commonOps;

// TODO(https://github.com/qorai/qorai-browser/issues/33574): Unify Qorai Ads
// component updater.
...
@implementation QoraiAds

- (instancetype)initWithStateStoragePath:(NSString*)path {
  if ((self = [super init])) {
    self.storagePath = path;
    self.commonOps = [[QoraiCommonOperations alloc] initWithStoragePath:path];
    ...
    adsClientNotifier = std::make_unique<qorai_ads::AdsClientNotifier>();

    ProfileIOS* profile = [self getLastUsedProfile];
    CHECK(profile);
    virtualPrefProvider = std::make_unique<qorai_ads::VirtualPrefProvider>(
        self.profilePrefService, self.localStatePrefService,
        std::make_unique<qorai_ads::VirtualPrefProviderDelegateIOS>(*profile));
  }
  return self;
}
...
- (BOOL)shouldShowSponsoredImagesAndVideosSetting {
  const std::string country_code = qorai_l10n::GetDefaultISOCountryCodeString();
...
- (BOOL)isOptedInToSearchResultAds {
  return self.profilePrefService->GetBoolean(
      qorai_ads::prefs::kOptedInToSearchResultAds);
}

- (BOOL)shouldShowSearchResultAdClickedInfoBar {
  return self.profilePrefService->GetBoolean(
      qorai_ads::prefs::kShouldShowSearchResultAdClickedInfoBar);
}
...
- (void)notifyQoraiNewsIsEnabledPreferenceDidChange:(BOOL)isEnabled {
  [self setProfilePref:qorai_news::prefs::kQoraiNewsOptedIn
                 value:base::Value(isEnabled)];
  [self setProfilePref:qorai_news::prefs::kNewTabPageShowToday
                 value:base::Value(isEnabled)];
}
...
- (BOOL)isEnabled {
  return self.profilePrefService->GetBoolean(qorai_rewards::prefs::kEnabled);
}

- (void)setEnabled:(BOOL)enabled {
  [self setProfilePref:qorai_rewards::prefs::kEnabled
                 value:base::Value(enabled)];
  [self setProfilePref:qorai_ads::prefs::kOptedInToNotificationAds
                 value:base::Value(enabled)];
}
...
- (void)initServiceWithSysInfo:(QoraiAdsSysInfo*)sysInfo
  ...
  auto cppSysInfo =
      sysInfo ? sysInfo.cppObjPtr : qorai_ads::mojom::SysInfo::New();
  auto cppBuildChannelInfo = buildChannelInfo
                                 ? buildChannelInfo.cppObjPtr
                                 : qorai_ads::mojom::BuildChannelInfo::New();
  auto cppWalletInfo =
      walletInfo ? walletInfo.cppObjPtr : qorai_ads::mojom::WalletInfoPtr();

  // This will always be the regular profile (not private/OTR).
  ProfileIOS* profile = [self getLastUsedProfile];
  adsService = qorai_ads::AdsServiceFactoryIOS::GetForProfile(profile);
  CHECK(adsService);
...
- (BOOL)registerAdsResourcesForLanguageCode:(NSString*)languageCode {
  ...
  BLOG(1, @"Registering Qorai Ads Resources Installer (%@) with id %@",
...
- (void)downloadAdsResource:(NSString*)folderName
  ...
  NSString* baseUrl = @"https://qorai-user-model-installer-input.s3.qorai.com";
  const auto flags = qorai_rewards::RewardsFlags::ForCurrentProcess();
  if (flags.environment) {
    switch (*flags.environment) {
      ...
      case qorai_rewards::RewardsFlags::Environment::kStaging:
        baseUrl = @"https://"
                  @"qorai-user-model-installer-input-dev.s3.qoraisoftware.com";
        break;
...
- (void)showNotificationAd:(const qorai_ads::NotificationAdInfo&)ad {
...
- (void)getSiteHistory:(int)max_count
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/33681): Unify Qorai Ads
  // site history.
...
- (void)UrlRequest:(qorai_ads::mojom::UrlRequestInfoPtr)url_request
  ...
  std::map<qorai_ads::mojom::UrlRequestMethodType, std::string> methodMap{
      {qorai_ads::mojom::UrlRequestMethodType::kGet, "GET"},
      {qorai_ads::mojom::UrlRequestMethodType::kPost, "POST"},
      {qorai_ads::mojom::UrlRequestMethodType::kPut, "PUT"}};
...
- (void)getStatementOfAccounts:(void (^)(NSInteger adsReceived,
  ...
  adsService->GetStatementOfAccounts(
      base::BindOnce(^(qorai_ads::mojom::StatementInfoPtr mojom_statement) {
        if (!mojom_statement) {
          return completion(/*adsReceived=*/0, /*estimatedEarnings=*/0,
                            /*nextPaymentDate=*/nil);
        }
...
- (void)triggerInlineContentAdEvent:(NSString*)placementId
  ...
  adsService->TriggerInlineContentAdEvent(
      ...
      static_cast<qorai_ads::mojom::InlineContentAdEventType>(eventType),
...
- (void)triggerNewTabPageAdEvent:(NSString*)wallpaperId
  ...
  const qorai_ads::mojom::NewTabPageAdMetricType mojom_metric_type =
      static_cast<qorai_ads::mojom::NewTabPageAdMetricType>(metricType);

  const qorai_ads::mojom::NewTabPageAdEventType mojom_event_type =
      static_cast<qorai_ads::mojom::NewTabPageAdEventType>(eventType);
...
- (void)triggerNotificationAdEvent:(NSString*)placementId
  ...
  adsService->TriggerNotificationAdEvent(
      base::SysNSStringToUTF8(placementId),
      static_cast<qorai_ads::mojom::NotificationAdEventType>(eventType),
      base::BindOnce(completion));
}
...
- (void)triggerPromotedContentAdEvent:(NSString*)placementId
  ...
  adsService->TriggerPromotedContentAdEvent(
      base::SysNSStringToUTF8(placementId),
      base::SysNSStringToUTF8(creativeInstanceId),
      static_cast<qorai_ads::mojom::PromotedContentAdEventType>(eventType),
      base::BindOnce(completion));
}
...
- (void)triggerSearchResultAdClickedEvent:(NSString*)placementId
  ...
  adsService->MaybeGetSearchResultAd(
      base::SysNSStringToUTF8(placementId),
      base::BindOnce(^(qorai_ads::mojom::CreativeSearchResultAdInfoPtr ad) {
        if (!ad) {
          return completion(/*success=*/false);
        }
        ...
        strongSelf->adsService->TriggerSearchResultAdEvent(
            std::move(ad), qorai_ads::mojom::SearchResultAdEventType::kClicked,
...
- (void)triggerSearchResultAdViewedEvent:
  ...
  adsService->TriggerSearchResultAdEvent(
      ...
      qorai_ads::mojom::SearchResultAdEventType::kViewedImpression,
      base::BindOnce(completion));
}
...
- (void)purgeOrphanedAdEventsForType:(QoraiAdsAdType)adType
  ...
  adsService->PurgeOrphanedAdEventsForType(
      static_cast<qorai_ads::mojom::AdType>(adType),
      base::BindOnce(completion));
}
...
- (NewTabPageAdIOS*)maybeGetPrefetchedNewTabPageAd {
  ...
  const std::optional<qorai_ads::NewTabPageAdInfo> new_tab_page_ad =
