...
#import "qorai/ios/browser/api/sync/qorai_sync_api.h"
...
// MARK: - QoraiSyncAPISyncProtocolErrorResult

QoraiSyncAPISyncProtocolErrorResult const
...
QoraiSyncAPISyncProtocolErrorResult const
    QoraiSyncAPISyncProtocolErrorResultDisabledByAdmin = static_cast<NSInteger>(
...
QoraiSyncAPISyncProtocolErrorResult const
    QoraiSyncAPISyncProtocolErrorResultDataObsolete = static_cast<NSInteger>(
...
QoraiSyncAPIQrCodeDataValidationResult const
    QoraiSyncAPIQrCodeDataValidationResultValid =
        static_cast<NSInteger>(qorai_sync::QrCodeDataValidationResult::kValid);
QoraiSyncAPIQrCodeDataValidationResult const
    QoraiSyncAPIQrCodeDataValidationResultNotWellFormed =
        static_cast<NSInteger>(
            qorai_sync::QrCodeDataValidationResult::kNotWellFormed);
QoraiSyncAPIQrCodeDataValidationResult const
    QoraiSyncAPIQrCodeDataValidationResultVersionDeprecated =
        static_cast<NSInteger>(
            qorai_sync::QrCodeDataValidationResult::kVersionDeprecated);
QoraiSyncAPIQrCodeDataValidationResult const
    QoraiSyncAPIQrCodeDataValidationResultExpired = static_cast<NSInteger>(
        qorai_sync::QrCodeDataValidationResult::kExpired);
QoraiSyncAPIQrCodeDataValidationResult const
    QoraiSyncAPIQrCodeDataValidationResultValidForTooLong =
        static_cast<NSInteger>(
            qorai_sync::QrCodeDataValidationResult::kValidForTooLong);

// MARK: - TimeLimitedWords::ValidationStatus

QoraiSyncAPIWordsValidationStatus const QoraiSyncAPIWordsValidationStatusValid =
    static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kValid);
QoraiSyncAPIWordsValidationStatus const
    QoraiSyncAPIWordsValidationStatusNotValidPureWords = static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kNotValidPureWords);
QoraiSyncAPIWordsValidationStatus const
    QoraiSyncAPIWordsValidationStatusVersionDeprecated = static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kVersionDeprecated);
QoraiSyncAPIWordsValidationStatus const
    QoraiSyncAPIWordsValidationStatusExpired = static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kExpired);
QoraiSyncAPIWordsValidationStatus const
    QoraiSyncAPIWordsValidationStatusValidForTooLong = static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kValidForTooLong);
QoraiSyncAPIWordsValidationStatus const
    QoraiSyncAPIWordsValidationStatusWrongWordsNumber = static_cast<NSInteger>(
        qorai_sync::TimeLimitedWords::ValidationStatus::kWrongWordsNumber);

// MARK: - QoraiSyncDeviceObserver

@interface QoraiSyncDeviceObserver : NSObject {
  std::unique_ptr<QoraiSyncDeviceTracker> _device_observer;
}
@end

@implementation QoraiSyncDeviceObserver

- (instancetype)initWithDeviceInfoTracker:(syncer::DeviceInfoTracker*)tracker
                                 callback:(void (^)())onDeviceInfoChanged {
  if ((self = [super init])) {
    _device_observer = std::make_unique<QoraiSyncDeviceTracker>(
...
// MARK: - QoraiSyncServiceObserver

@interface QoraiSyncServiceObserver : NSObject {
  std::unique_ptr<QoraiSyncServiceTracker> _service_tracker;
}
@end

@implementation QoraiSyncServiceObserver
...
- (instancetype)initWithSyncServiceImpl:
  ...
  if ((self = [super init])) {
    _service_tracker = std::make_unique<QoraiSyncServiceTracker>(
...
@interface QoraiSyncAPI () {
  std::unique_ptr<QoraiSyncWorker> _worker;
  raw_ptr<ProfileIOS> _profile;
}
@end

@implementation QoraiSyncAPI

- (instancetype)initWithBrowserState:(ProfileIOS*)mainBrowserState {
  if ((self = [super init])) {
    _profile = mainBrowserState;
    _worker.reset(new QoraiSyncWorker(_profile));
  }
  return self;
}
...
- (QoraiSyncAPIQrCodeDataValidationResult)getQRCodeValidationResult:
    (NSString*)json {
  return static_cast<QoraiSyncAPIQrCodeDataValidationResult>(
...
- (QoraiSyncAPIWordsValidationStatus)getWordsValidationResult:
  ...
  return static_cast<QoraiSyncAPIWordsValidationStatus>(
...
- (NSDate*)getExpirationFromTimeLimitedWords:(NSString*)timeLimitedWords {
  base::Time not_after = qorai_sync::TimeLimitedWords::GetNotAfter(
      base::SysNSStringToUTF8(timeLimitedWords));
...
- (void)permanentlyDeleteAccount:
  ...
  _worker->PermanentlyDeleteAccount(base::BindOnce(
      [](void (^completion)(QoraiSyncAPISyncProtocolErrorResult),
         const syncer::SyncProtocolError& error) {
        completion(
            static_cast<QoraiSyncAPISyncProtocolErrorResult>(error.error_type));
      },
      completion));
}

- (bool)isSyncAccountDeletedNoticePending {
  qorai_sync::Prefs qorai_sync_prefs(_profile->GetPrefs());
  return qorai_sync_prefs.IsSyncAccountDeletedNoticePending();
}
...
- (bool)isFailedDecryptSeedNoticeDismissed {
  qorai_sync::Prefs qorai_sync_prefs(_profile->GetPrefs());
  return qorai_sync_prefs.IsFailedDecryptSeedNoticeDismissed();
}

- (void)dismissFailedDecryptSeedNotice {
  qorai_sync::Prefs qorai_sync_prefs(_profile->GetPrefs());
  qorai_sync_prefs.DismissFailedDecryptSeedNotice();
}
...
- (id)createSyncDeviceObserver:(void (^)())onDeviceInfoChanged {
  ...
  return [[QoraiSyncDeviceObserver alloc]
...
- (id)createSyncServiceObserver:(void (^)())onSyncServiceStateChanged
  ...
  auto* service = static_cast<syncer::SyncServiceImpl*>(
      SyncServiceFactory::GetForProfile(_profile));
  return [[QoraiSyncServiceObserver alloc]
