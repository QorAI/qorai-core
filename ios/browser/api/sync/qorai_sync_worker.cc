...
#include "qorai/ios/browser/api/sync/qorai_sync_worker.h"
...
#include "qorai/components/qorai_sync/qr_code_validator.h"
...
QoraiSyncDeviceTracker::QoraiSyncDeviceTracker(
...
void QoraiSyncDeviceTracker::OnDeviceInfoChange() {
  if (on_device_info_changed_callback_) {
    on_device_info_changed_callback_.Run();
  }
}

QoraiSyncServiceTracker::QoraiSyncServiceTracker(
...
void QoraiSyncServiceTracker::OnStateChanged(syncer::SyncService* sync) {
  if (on_state_changed_callback_) {
    on_state_changed_callback_.Run();
  }
}

void QoraiSyncServiceTracker::OnSyncShutdown(syncer::SyncService* sync) {
  if (on_sync_shutdown_callback_) {
    on_sync_shutdown_callback_.Run();
  }
}

QoraiSyncWorker::QoraiSyncWorker(ProfileIOS* profile) : profile_(profile) {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
}

QoraiSyncWorker::~QoraiSyncWorker() {
  // Observer will be removed by ScopedObservation
}

bool QoraiSyncWorker::RequestSync() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
...
const syncer::DeviceInfo* QoraiSyncWorker::GetLocalDeviceInfo() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
...
std::vector<std::unique_ptr<syncer::QoraiDeviceInfo>>
QoraiSyncWorker::GetDeviceList() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  auto* device_info_service =
      DeviceInfoSyncServiceFactory::GetForProfile(profile_);

  if (!device_info_service) {
    return std::vector<std::unique_ptr<syncer::QoraiDeviceInfo>>();
  }

  syncer::DeviceInfoTracker* tracker =
      device_info_service->GetDeviceInfoTracker();
  return tracker->GetAllQoraiDeviceInfo();
}

std::string QoraiSyncWorker::GetOrCreateSyncCode() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::QoraiSyncServiceImpl* sync_service = GetSyncService();
...
bool QoraiSyncWorker::IsValidSyncCode(const std::string& sync_code) {
  std::vector<uint8_t> seed;
  if (!qorai_sync::crypto::PassphraseToBytes32(sync_code, &seed)) {
    return false;
  }
  return seed.size() == SEED_BYTES_COUNT;
}

bool QoraiSyncWorker::SetSyncCode(const std::string& sync_code) {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  if (sync_code.empty()) {
    return false;
  }
...
std::string QoraiSyncWorker::GetSyncCodeFromHexSeed(
  ...
  if (base::HexStringToBytes(hex_code_seed, &bytes)) {
    ...
    if (bytes.size() == SEED_BYTES_COUNT) {
      sync_code_words = qorai_sync::crypto::PassphraseFromBytes32(bytes);
...
std::string QoraiSyncWorker::GetHexSeedFromSyncCode(
    const std::string& code_words) {
  DCHECK(!code_words.empty());

  std::string sync_code_hex;
  std::vector<uint8_t> bytes;
  if (qorai_sync::crypto::PassphraseToBytes32(code_words, &bytes)) {
    DCHECK_EQ(bytes.size(), SEED_BYTES_COUNT);
    if (bytes.size() == SEED_BYTES_COUNT) {
...
std::string QoraiSyncWorker::GetQrCodeJsonFromHexSeed(
    const std::string& hex_seed) {
  DCHECK(!hex_seed.empty());
  return qorai_sync::QrCodeData::CreateWithActualDate(hex_seed)->ToJson();
}

qorai_sync::QrCodeDataValidationResult
QoraiSyncWorker::GetQrCodeValidationResult(const std::string& json) {
  DCHECK(!json.empty());
  return qorai_sync::QrCodeDataValidator::ValidateQrDataJson(json);
}

qorai_sync::TimeLimitedWords::ValidationStatus
QoraiSyncWorker::GetWordsValidationResult(
    const std::string& time_limited_words) {
  DCHECK(!time_limited_words.empty());
  auto words_with_status =
      qorai_sync::TimeLimitedWords::Parse(time_limited_words);
  if (words_with_status.has_value()) {
    return qorai_sync::TimeLimitedWords::ValidationStatus::kValid;
  } else {
    return words_with_status.error();
  }
}

std::string QoraiSyncWorker::GetWordsFromTimeLimitedWords(
    const std::string& time_limited_words) {
  DCHECK(!time_limited_words.empty());
  auto words_with_status =
      qorai_sync::TimeLimitedWords::Parse(time_limited_words);
...
std::string QoraiSyncWorker::GetTimeLimitedWordsFromWords(
    const std::string& words) {
  DCHECK(!words.empty());
  auto generate_result = qorai_sync::TimeLimitedWords::GenerateForNow(words);
  CHECK(generate_result.has_value());
  return generate_result.value();
}

std::string QoraiSyncWorker::GetHexSeedFromQrCodeJson(const std::string& json) {
  DCHECK(!json.empty());
  std::unique_ptr<qorai_sync::QrCodeData> qr_data =
      qorai_sync::QrCodeData::FromJson(json);
  if (qr_data) {
...
bool QoraiSyncWorker::IsInitialSyncFeatureSetupComplete() {
  syncer::SyncService* sync_service = GetSyncService();
  return sync_service &&
         sync_service->GetUserSettings()->IsInitialSyncFeatureSetupComplete();
}

bool QoraiSyncWorker::SetSetupComplete() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::SyncService* sync_service = GetSyncService();
...
void QoraiSyncWorker::ResetSync() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::QoraiSyncServiceImpl* sync_service = GetSyncService();
  ...
  qorai_sync::ResetSync(sync_service, device_info_service,
                        base::BindOnce(&QoraiSyncWorker::OnResetDone,
                                        weak_ptr_factory_.GetWeakPtr()));
}

void QoraiSyncWorker::DeleteDevice(const std::string& device_guid) {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::QoraiSyncServiceImpl* sync_service = GetSyncService();
  ...
  qorai_sync::DeleteDevice(sync_service, device_info_service, device_guid);
}

void QoraiSyncWorker::SetJoinSyncChainCallback(
...
void QoraiSyncWorker::PermanentlyDeleteAccount(
...
syncer::QoraiSyncServiceImpl* QoraiSyncWorker::GetSyncService() const {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  return static_cast<syncer::QoraiSyncServiceImpl*>(
      SyncServiceFactory::GetForProfile(profile_));
}

void QoraiSyncWorker::SetEncryptionPassphrase(syncer::SyncService* service) {
  DCHECK(service);
  ...
  if (sync_user_settings->IsCustomPassphraseAllowed() &&
    ...
    VLOG(3) << "[QoraiSync] " << __func__ << " SYNC_CREATED_NEW_PASSPHRASE";
  }
}

void QoraiSyncWorker::SetDecryptionPassphrase(syncer::SyncService* service) {
  DCHECK(service);
  DCHECK(service->IsEngineInitialized());
  DCHECK(!this->passphrase_.empty());
  ...
  if (sync_user_settings->SetDecryptionPassphrase(this->passphrase_)) {
    VLOG(3) << "[QoraiSync] " << __func__
            << " SYNC_ENTERED_EXISTING_PASSPHRASE";
  }
}

void QoraiSyncWorker::OnStateChanged(syncer::SyncService* service) {
  // If the sync engine has shutdown for some reason, just give up
  if (!service || !service->IsEngineInitialized()) {
    VLOG(3) << "[QoraiSync] " << __func__ << " sync engine is not initialized";
    return;
  }

  if (this->passphrase_.empty()) {
    VLOG(3) << "[QoraiSync] " << __func__ << " empty passphrase";
    return;
  }
...
bool QoraiSyncWorker::CanSyncFeatureStart() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::SyncService* sync_service = GetSyncService();
...
bool QoraiSyncWorker::IsSyncFeatureActive() {
  DCHECK_CURRENTLY_ON(web::WebThread::UI);
  syncer::SyncService* sync_service = GetSyncService();
