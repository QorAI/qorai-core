...
#include "qorai/ios/browser/api/history/qorai_history_observer.h"
#include "qorai/ios/browser/api/history/history_driver_ios.h"
#include "qorai/ios/browser/api/history/history_service_listener_ios.h"
...
#pragma mark - QoraiHistoryAPI

@interface QoraiHistoryAPI () {
...
@implementation QoraiHistoryAPI {
...
- (void)removeAllWithCompletion:(void (^)())completion {
  ...
  __weak QoraiHistoryAPI* weak_history_api = self;
  auto delete_history = ^(void (^callback)()) {
    QoraiHistoryAPI* historyAPI = weak_history_api;
    if (!historyAPI) {
      callback();
      return;
    }
