...
#include "qorai/ios/browser/api/history/importer/qorai_history_importer.h"
...
#include "qorai/ios/browser/api/history/importer/history_json_reader.h"
...
@implementation QoraiImportedHistory
...
- (void)importFromFile:(NSString*)filePath
  ...
  __weak QoraiHistoryImporter* weakSelf = self;

  auto start_import = ^{
    // Import cancelled as the importer has been deallocated
    __strong QoraiHistoryImporter* importer = weakSelf;
    if (!importer) {
      listener(QoraiHistoryImporterStateStarted, nullptr);
      listener(QoraiHistoryImporterStateCancelled, nullptr);
      return;
    }
        ...
        web::GetUIThreadTaskRunner({})->PostTask(
            FROM_HERE, base::BindOnce(
                           ^(std::vector<history::URLRow> items) {
                             ...
                             listener(QoraiHistoryImporterStateAutoCompleted,
                                      nullptr);
                           },
                           std::move(history_items)));
      } else {
        listener(QoraiHistoryImporterStateCompleted,
...
- (void)importFromArray:(NSArray<QoraiImportedHistory*>*)historyItems
  ...
  auto start_import = ^{
    ...
    __strong QoraiHistoryImporter* importer = weakSelf;
    if (!importer) {
      listener(QoraiHistoryImporterStateStarted);
      listener(QoraiHistoryImporterStateCancelled);
      return;
    }
...
- (NSArray<QoraiImportedHistory*>*)convertToIOSImportedHistory:
    (const std::vector<history::URLRow>&)historyItems {
  NSMutableArray<QoraiImportedHistory*>* results =
      [[NSMutableArray alloc] init];
  for (const auto& historyItem : historyItems) {
    QoraiImportedHistory* imported_history_item = [[QoraiImportedHistory alloc]
...
- (std::vector<history::URLRow>)convertToChromiumImportedHistory:
    (NSArray<QoraiImportedHistory*>*)historyItems {
  std::vector<history::URLRow> results;
  for (QoraiImportedHistory* historyItem in historyItems) {
