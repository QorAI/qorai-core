...
#include "qorai/ios/browser/api/bookmarks/exporter/qorai_bookmarks_exporter.h"
...
#include "qorai/ios/browser/api/bookmarks/exporter/bookmark_html_writer.h"
#include "qorai/ios/browser/api/bookmarks/exporter/bookmarks_encoder.h"
...
class QoraiBookmarksExportObserver : public BookmarksExportObserver {
 public:
  QoraiBookmarksExportObserver(
      base::OnceCallback<void(QoraiBookmarksExporterState)> on_export_finished);
  void OnExportFinished(Result result) override;

 private:
  base::OnceCallback<void(QoraiBookmarksExporterState)> _on_export_finished;
};

QoraiBookmarksExportObserver::QoraiBookmarksExportObserver(
    base::OnceCallback<void(QoraiBookmarksExporterState)> on_export_finished)
    : _on_export_finished(std::move(on_export_finished)) {}

void QoraiBookmarksExportObserver::OnExportFinished(Result result) {
  switch (result) {
    case Result::kSuccess:
      std::move(_on_export_finished).Run(QoraiBookmarksExporterStateCompleted);
      break;
    case Result::kCouldNotCreateFile:
      std::move(_on_export_finished)
          .Run(QoraiBookmarksExporterStateErrorCreatingFile);
      break;
    case Result::kCouldNotWriteHeader:
      std::move(_on_export_finished)
          .Run(QoraiBookmarksExporterStateErrorWritingHeader);
      break;
    case Result::kCouldNotWriteNodes:
      std::move(_on_export_finished)
          .Run(QoraiBookmarksExporterStateErrorWritingNodes);
      break;
    default:
      delete this;
      NOTREACHED();
...
@interface QoraiBookmarksExporter () {
  scoped_refptr<base::SequencedTaskRunner> export_thread_;
}
@end

@implementation QoraiBookmarksExporter
...
- (void)exportToFile:(NSString*)filePath
  ...
  __weak QoraiBookmarksExporter* weakSelf = self;

  auto start_export = ^{
    // Export cancelled as the exporter has been deallocated
    __strong QoraiBookmarksExporter* exporter = weakSelf;
    if (!exporter) {
      listener(QoraiBookmarksExporterStateStarted);
      listener(QoraiBookmarksExporterStateCancelled);
      return;
    }
    ...
    bookmark_html_writer::WriteBookmarks(
        ...
        new QoraiBookmarksExportObserver(base::BindOnce(listener)));
  };
...
- (void)exportToFile:(NSString*)filePath
  ...
  if ([bookmarks count] == 0) {
    listener(QoraiBookmarksExporterStateStarted);
    listener(QoraiBookmarksExporterStateCompleted);
    return;
  }
