...
#include "qorai/ios/browser/api/bookmarks/importer/qorai_bookmarks_importer.h"
...
#include "qorai/ios/browser/api/bookmarks/importer/bookmark_html_reader.h"
#include "qorai/ios/browser/api/bookmarks/importer/bookmarks_importer.h"
#include "qorai/ios/browser/api/bookmarks/importer/imported_bookmark_entry.h"
...
@implementation QoraiImportedBookmark
...
- (void)importFromFile:(NSString*)filePath
  ...
  __weak QoraiBookmarksImporter* weakSelf = self;

  auto start_import = ^{
    // Import cancelled as the importer has been deallocated
    __strong QoraiBookmarksImporter* importer = weakSelf;
    if (!importer) {
      listener(QoraiBookmarksImporterStateStarted, nullptr);
      listener(QoraiBookmarksImporterStateCancelled, nullptr);
      return;
    }
        ...
        web::GetUIThreadTaskRunner({})->PostTask(
            ...
            base::BindOnce(
                ^(std::vector<ImportedBookmarkEntry> items,
                  ...
                  listener(QoraiBookmarksImporterStateAutoCompleted, nullptr);
                },
                std::move(bookmarks), top_level_folder_name));
      } else {
        listener(QoraiBookmarksImporterStateCompleted,
...
- (void)importFromArray:(NSArray<QoraiImportedBookmark*>*)bookmarks
  ...
  auto start_import = ^{
    ...
    __strong QoraiBookmarksImporter* importer = weakSelf;
    if (!importer) {
      listener(QoraiBookmarksImporterStateStarted);
      listener(QoraiBookmarksImporterStateCancelled);
      return;
    }
...
- (NSArray<QoraiImportedBookmark*>*)convertToIOSImportedBookmarks:
    (const std::vector<ImportedBookmarkEntry>&)bookmarks {
  NSMutableArray<QoraiImportedBookmark*>* results =
      [[NSMutableArray alloc] init];
  for (const auto& bookmark : bookmarks) {
    QoraiImportedBookmark* imported_bookmark = [[QoraiImportedBookmark alloc]
...
- (std::vector<ImportedBookmarkEntry>)convertToChromiumImportedBookmarks:
    (NSArray<QoraiImportedBookmark*>*)bookmarks {
  std::vector<ImportedBookmarkEntry> results;
  for (QoraiImportedBookmark* bookmark in bookmarks) {
