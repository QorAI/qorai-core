...
#include "base/uuid.h"
#include "qorai/ios/browser/api/bookmarks/bookmark_model_listener_ios.h"
...
- (void)searchWithQuery:(NSString*)queryArg
  ...
  __weak QoraiBookmarksAPI* weak_bookmarks_api = self;
  auto search_with_query = ^(NSString* query, NSUInteger maxCount,
                             void (^completion)(NSArray<IOSBookmarkNode*>*)) {
    QoraiBookmarksAPI* bookmarks_api = weak_bookmarks_api;
    if (!bookmarks_api) {
      completion(@[]);
      return;
    }
