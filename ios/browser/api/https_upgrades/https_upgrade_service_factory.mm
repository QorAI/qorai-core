...
#include "qorai/ios/browser/api/https_upgrades/https_upgrade_service_factory.h"

#include "qorai/ios/browser/api/https_upgrades/https_upgrade_service+private.h"
...
@implementation QoraiHttpsUpgradeServiceFactory
+ (nullable id)serviceForProfile:(ProfileIOS*)profile {
  auto* service = HttpsUpgradeServiceFactory::GetForProfile(profile);
  if (!service) {
    return nil;
  }
  return [[QoraiHttpsUpgradeService alloc] initWithHttpsUpgradeService:service];
}
@end
