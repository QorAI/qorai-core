...
#include "qorai/ios/browser/api/url/url_spoof_checker.h"
...
QoraiSpoofCheckerSkeletonType const QoraiSpoofCheckerSkeletonTypeFull =
    static_cast<NSInteger>(url_formatter::SkeletonType::kFull);
QoraiSpoofCheckerSkeletonType const
...
QoraiSpoofCheckerLookalikeURLMatchType const
...
QoraiSpoofCheckerLookalikeURLMatchType const
    QoraiSpoofCheckerLookalikeURLMatchTypeEditDistance = static_cast<NSInteger>(
...
- (instancetype)initWithEntry:(url_formatter::TopDomainEntry)entry {
  if ((self = [super init])) {
    ...
    _skeletonType = static_cast<QoraiSpoofCheckerLookalikeURLMatchType>(
        entry.skeleton_type);
  }
  return self;
}
...
@implementation QoraiURLSpoofChecker
...
+ (QoraiURLSpoofCheckerResult*)isLookalikeURL:(NSString*)url {
  ...
  if (!response_url.SchemeIsHTTPOrHTTPS()) {
    return [[QoraiURLSpoofCheckerResult alloc]
