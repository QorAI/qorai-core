...
#include "qorai/components/qorai_shields/core/browser/ad_block_filter_list_catalog_provider.h"
#include "qorai/components/qorai_shields/core/browser/ad_block_filters_provider_manager.h"
#include "qorai/components/qorai_shields/core/browser/ad_block_list_p3a.h"
...
#include "qorai/components/cosmetic_filters/resources/grit/cosmetic_filters_generated.h"
#include "qorai/ios/browser/api/qorai_shields/adblock_filter_list_catalog_entry+private.h"
...
@interface AdblockService () {
  ...
  std::unique_ptr<qorai_shields::AdBlockListP3A> _adblockListP3A;
  std::unique_ptr<qorai_shields::AdBlockFilterListCatalogProvider>
      _catalogProvider;
  std::unique_ptr<qorai_shields::AdBlockComponentServiceManager>
      _serviceManager;
  std::unique_ptr<qorai_shields::AdBlockDefaultResourceProvider>
      _resourceProvider;
  std::vector<std::unique_ptr<qorai_shields::AdBlockServiceObserver>>
      _serviceObservers;
  std::unique_ptr<qorai_shields::AdBlockCatalogObserver> _catalogObserver;
  std::unique_ptr<qorai_shields::AdBlockResourceObserver> _resourceObserver;
}
...
- (instancetype)initWithComponentUpdater:
  ...
  if ((self = [super init])) {
    ...
    _adblockListP3A = std::make_unique<qorai_shields::AdBlockListP3A>(
        GetApplicationContext()->GetLocalState());
    _catalogProvider =
        std::make_unique<qorai_shields::AdBlockFilterListCatalogProvider>(_cus);
    _serviceManager =
        std::make_unique<qorai_shields::AdBlockComponentServiceManager>(
            GetApplicationContext()->GetLocalState(),
            GetApplicationContext()->GetApplicationLocaleStorage()->Get(), _cus,
            _catalogProvider.get(), _adblockListP3A.get());
    _resourceProvider =
        std::make_unique<qorai_shields::AdBlockDefaultResourceProvider>(_cus);
  }
  return self;
}
...
- (void)registerFilterListChanges:(void (^)(bool isDefaultEngine))callback {
  auto _serviceObserver =
      std::make_unique<qorai_shields::AdBlockServiceObserver>(
...
- (void)registerCatalogChanges:(void (^)())callback {
  _catalogObserver = std::make_unique<qorai_shields::AdBlockCatalogObserver>(
      base::BindRepeating(callback));
  _catalogProvider->AddObserver(_catalogObserver.get());
}

- (void)registerResourcesChanges:(void (^)(NSString* resourcesJSON))callback {
  _resourceObserver = std::make_unique<qorai_shields::AdBlockResourceObserver>(
