...
#include "qorai/ios/browser/api/opentabs/sendtab_model_listener_ios.h"
...
#pragma mark - QoraiSendTabAPI

@interface QoraiSendTabAPI () {
...
@implementation QoraiSendTabAPI
  ...
  for (const auto& device : target_device_list_) {
    IOSSendTabTargetDevice* targetDevice = [[IOSSendTabTargetDevice alloc]
        initWithFullName:base::SysUTF8ToNSString(device.full_name)
              ...
              deviceType:qorai::ios::DeviceTypeFromSyncDeviceType(
                             device.form_factor)
