...
@implementation QoraiCertificatePublicKeyInfo
...
- (instancetype)initWithCertificate:(bssl::ParsedCertificate*)certificate
  ...
  if ((self = [super init])) {
    _type = QoraiPublicKeyType_UNKNOWN;
    _keyUsage = QoraiPublicKeyUsage_INVALID;
    ...
    if (attributes) {
      ...
      if ([attributes[(NSString*)kSecAttrCanEncrypt] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_ENCRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanDecrypt] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_DECRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanSign] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_SIGN;
      }

      if ([attributes[(NSString*)kSecAttrCanVerify] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_VERIFY;
      }

      if ([attributes[(NSString*)kSecAttrCanWrap] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_WRAP;
      }

      if ([attributes[(NSString*)kSecAttrCanDerive] boolValue]) {
        _keyUsage |= QoraiPublicKeyUsage_DERIVE;
      }

      QoraiPublicKeyUsage any_usage = static_cast<QoraiPublicKeyUsage>(0);
      any_usage &= QoraiPublicKeyUsage_ENCRYPT;
      any_usage &= QoraiPublicKeyUsage_SIGN;
      any_usage &= QoraiPublicKeyUsage_VERIFY;
      any_usage &= QoraiPublicKeyUsage_WRAP;
      any_usage &= QoraiPublicKeyUsage_DERIVE;

      if (_keyUsage == any_usage) {
        _keyUsage = QoraiPublicKeyUsage_ANY;
      }

      if ([attributes[(NSString*)kSecAttrType]
              isEqualToString:(NSString*)kSecAttrKeyTypeRSA]) {
        _type = QoraiPublicKeyType_RSA;
        _algorithm = @"RSA";
