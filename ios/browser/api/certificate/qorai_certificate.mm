...
#include "qorai/ios/browser/api/certificate/qorai_certificate.h"
...
#include "qorai/ios/browser/api/certificate/models/qorai_certificate_model+private.h"
...
@interface QoraiCertificateModel () {
...
@implementation QoraiCertificateModel
...
- (void)parseCertificate {
  ...
  _subjectName = [[QoraiCertificateRDNSequence alloc]
      initWithBERName:extended_cert_->tbs().subject_tlv
             uniqueId:subject_unique_id];
  _issuerName = [[QoraiCertificateRDNSequence alloc]
  ...
  _version = static_cast<std::int32_t>(extended_cert_->tbs().version) +
             1;  // version + 1
  _signature = [[QoraiCertificateSignature alloc]
  ...
  _notValidAfter = certificate::x509_utils::GeneralizedTimeToTime(
                       ...
                       .ToNSDate();
  _publicKeyInfo = [[QoraiCertificatePublicKeyInfo alloc]
      initWithCertificate:extended_cert_.get()
                  withKey:public_key_.get()];
  //  _extensions = nullptr;
  _sha1Fingerprint = [[QoraiCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:QoraiFingerprintType_SHA1];
  _sha256Fingerprint = [[QoraiCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:QoraiFingerprintType_SHA256];
