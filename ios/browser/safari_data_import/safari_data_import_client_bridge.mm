...
#import "qorai/ios/browser/safari_data_import/safari_data_import_client_bridge.h"

#include "base/strings/sys_string_conversions.h"
#include "qorai/ios/browser/safari_data_import/safari_data_import_client_delegate.h"
#include "components/password_manager/core/browser/import/import_results.h"

QoraiSafariDataImportClientBridge::QoraiSafariDataImportClientBridge() =
    default;

QoraiSafariDataImportClientBridge::~QoraiSafariDataImportClientBridge() =
    default;

void QoraiSafariDataImportClientBridge::OnTotalFailure() {
  if (delegate_) {
    [delegate_ onTotalFailure];
  }
}

void QoraiSafariDataImportClientBridge::OnBookmarksReady(
...
void QoraiSafariDataImportClientBridge::OnHistoryReady(
...
void QoraiSafariDataImportClientBridge::OnPasswordsReady(
...
void QoraiSafariDataImportClientBridge::OnPaymentCardsReady(
    user_data_importer::CountOrError result) {
  if (delegate_) {
...
void QoraiSafariDataImportClientBridge::OnBookmarksImported(size_t count) {
  if (delegate_) {
    [delegate_ onBookmarksImported:static_cast<NSInteger>(count)];
  }
}

void QoraiSafariDataImportClientBridge::OnHistoryImported(size_t count) {
  if (delegate_) {
    [delegate_ onHistoryImported:static_cast<NSInteger>(count)];
  }
}

void QoraiSafariDataImportClientBridge::OnPasswordsImported(
...
void QoraiSafariDataImportClientBridge::OnPaymentCardsImported(size_t count) {
  if (delegate_) {
...
QoraiSafariDataImportClientBridge::AsWeakPtr() {
