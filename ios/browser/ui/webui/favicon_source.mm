...
#include "qorai/ios/browser/favicon/qorai_ios_favicon_loader.h"
...
void FaviconSource::StartDataRequest(
  ...
  if (parsed.page_url.empty()) {
    ...
    qorai_favicon::QoraiFaviconLoader* qorai_icon_service =
        qorai_favicon::QoraiIOSFaviconLoaderFactory::GetForProfile(profile_);

    if (!qorai_icon_service) {
      SendDefaultResponse(std::move(callback), parsed);
      return;
    }
    base::UmaHistogramBoolean(kDefaultResponseHistogramName, false);
    qorai_icon_service->RawFaviconForPageUrlOrHost(
