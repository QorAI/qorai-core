...
#import "qorai/ios/browser/favicon/qorai_ios_web_favicon_driver.h"
...
void QoraiIOSWebFaviconDriver::SetMaximumFaviconImageSize(
...
gfx::Image QoraiIOSWebFaviconDriver::GetFavicon() const {
  DCHECK(web_state_);
  return web_state_->GetFaviconStatus().image;
}

bool QoraiIOSWebFaviconDriver::FaviconIsValid() const {
  DCHECK(web_state_);
  return web_state_->GetFaviconStatus().valid;
}

GURL QoraiIOSWebFaviconDriver::GetActiveURL() {
  DCHECK(web_state_);
...
int QoraiIOSWebFaviconDriver::DownloadImage(const GURL& url,
                                            int max_image_size,
...
void QoraiIOSWebFaviconDriver::DownloadManifest(
    const GURL& url,
...
bool QoraiIOSWebFaviconDriver::IsOffTheRecord() {
  DCHECK(web_state_);
  return web_state_->GetBrowserState()->IsOffTheRecord();
}

void QoraiIOSWebFaviconDriver::OnFaviconUpdated(
    const GURL& page_url,
...
void QoraiIOSWebFaviconDriver::OnFaviconDeleted(
    const GURL& page_url,
...
QoraiIOSWebFaviconDriver::QoraiIOSWebFaviconDriver(
...
void QoraiIOSWebFaviconDriver::DidFinishNavigation(
    web::WebState* web_state,
...
void QoraiIOSWebFaviconDriver::FaviconUrlUpdated(
    web::WebState* web_state,
...
void QoraiIOSWebFaviconDriver::WebStateDestroyed(web::WebState* web_state) {
  DCHECK_EQ(web_state_, web_state);
  web_state_->RemoveObserver(this);
  web_state_ = nullptr;
}

void QoraiIOSWebFaviconDriver::SetFaviconStatus(
    const GURL& page_url,
