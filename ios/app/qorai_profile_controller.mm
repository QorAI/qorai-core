...
#include "qorai/components/ai_chat/ios/browser/ai_chat+private.h"
#include "qorai/components/ai_chat/ios/browser/ai_chat_delegate.h"
...
#include "qorai/ios/browser/api/bookmarks/qorai_bookmarks_api+private.h"
#include "qorai/ios/browser/api/qorai_stats/qorai_stats+private.h"
#include "qorai/ios/browser/api/qorai_wallet/qorai_wallet_api+private.h"
#include "qorai/ios/browser/api/content_settings/default_host_content_settings.h"
#include "qorai/ios/browser/api/content_settings/default_host_content_settings_internal.h"
#include "qorai/ios/browser/api/de_amp/de_amp_prefs+private.h"
#include "qorai/ios/browser/api/history/qorai_history_api+private.h"
#include "qorai/ios/browser/api/ipfs/ipfs_api+private.h"
#include "qorai/ios/browser/api/ntp_background_images/ntp_background_images_service_ios+private.h"
#include "qorai/ios/browser/api/opentabs/qorai_opentabs_api+private.h"
#include "qorai/ios/browser/api/opentabs/qorai_sendtab_api+private.h"
#include "qorai/ios/browser/api/opentabs/qorai_tabgenerator_api+private.h"
#include "qorai/ios/browser/api/password/qorai_password_api+private.h"
#include "qorai/ios/browser/api/profile/profile_bridge_impl.h"
#include "qorai/ios/browser/api/sync/qorai_sync_api+private.h"
#include "qorai/ios/browser/api/sync/driver/qorai_sync_profile_service+private.h"
#include "qorai/ios/browser/api/web_image/web_image+private.h"
#include "qorai/ios/browser/api/web_view/qorai_web_view_configuration.h"
...
@interface QoraiProfileController () {
  ...
  std::unique_ptr<QoraiWebViewDownloadManager> _downloadManager;
  std::unique_ptr<QoraiWebViewDownloadManager> _otrDownloadManager;
}
@property(nonatomic) QoraiBookmarksAPI* bookmarksAPI;
@property(nonatomic) QoraiHistoryAPI* historyAPI;
@property(nonatomic) QoraiPasswordAPI* passwordAPI;
@property(nonatomic) QoraiOpenTabsAPI* openTabsAPI;
@property(nonatomic) QoraiSendTabAPI* sendTabAPI;
@property(nonatomic) QoraiSyncAPI* syncAPI;
@property(nonatomic) QoraiSyncProfileServiceIOS* syncProfileService;
@property(nonatomic) QoraiTabGeneratorAPI* tabGeneratorAPI;
@property(nonatomic) QoraiWalletAPI* qoraiWalletAPI;
...
@property(nonatomic) QoraiWebViewConfiguration* defaultWebViewConfiguration;
@property(nonatomic)
    QoraiWebViewConfiguration* nonPersistentWebViewConfiguration;
@end

@implementation QoraiProfileController
...
- (instancetype)initWithProfileKeepAlive:
  ...
  if ((self = [super init])) {
    ...
    _downloadManager = std::make_unique<QoraiWebViewDownloadManager>(_profile);
    _otrDownloadManager = std::make_unique<QoraiWebViewDownloadManager>(
        ...
        initWithBackgroundImagesService:
            std::make_unique<ntp_background_images::NTPBackgroundImagesService>(
                ...
                GetApplicationContext()->GetLocalState())
                            ads_service:qorai_ads::AdsServiceFactoryIOS::
...
- (QoraiBookmarksAPI*)bookmarksAPI {
  if (!_bookmarksAPI) {
    ...
    _bookmarksAPI =
        [[QoraiBookmarksAPI alloc] initWithBookmarkModel:bookmark_model
                                     bookmarkUndoService:bookmark_undo_service];
  }
  return _bookmarksAPI;
}

- (QoraiHistoryAPI*)historyAPI {
  if (!_historyAPI) {
    _historyAPI = [[QoraiHistoryAPI alloc] initWithBrowserState:_profile];
  }
  return _historyAPI;
}

- (QoraiOpenTabsAPI*)openTabsAPI {
  if (!_openTabsAPI) {
    ...
    _openTabsAPI =
        [[QoraiOpenTabsAPI alloc] initWithSyncService:sync_service_
                                   sessionSyncService:session_sync_service_];
  }
  return _openTabsAPI;
}

- (QoraiPasswordAPI*)passwordAPI {
  if (!_passwordAPI) {
    ...
    _passwordAPI =
        [[QoraiPasswordAPI alloc] initWithPasswordStore:password_store_];
  }
  return _passwordAPI;
}

- (QoraiSendTabAPI*)sendTabAPI {
  if (!_sendTabAPI) {
    send_tab_to_self::SendTabToSelfSyncService* sync_service_ =
        SendTabToSelfSyncServiceFactory::GetForProfile(_profile);

    _sendTabAPI = [[QoraiSendTabAPI alloc] initWithSyncService:sync_service_];
  }
  return _sendTabAPI;
}

- (QoraiSyncAPI*)syncAPI {
  if (!_syncAPI) {
    _syncAPI = [[QoraiSyncAPI alloc] initWithBrowserState:_profile];
  }
  return _syncAPI;
}

- (QoraiSyncProfileServiceIOS*)syncProfileService {
  if (!_syncProfileService) {
    syncer::SyncService* sync_service_ =
        SyncServiceFactory::GetForProfile(_profile);
    _syncProfileService = [[QoraiSyncProfileServiceIOS alloc]
        initWithProfileSyncService:sync_service_];
  }
  return _syncProfileService;
}

- (QoraiTabGeneratorAPI*)tabGeneratorAPI {
  if (!_tabGeneratorAPI) {
    _tabGeneratorAPI =
        [[QoraiTabGeneratorAPI alloc] initWithBrowser:_browser.get()
...
- (QoraiStats*)qoraiStats {
  return [[QoraiStats alloc] initWithBrowserState:_profile];
}
...
- (QoraiWalletAPI*)qoraiWalletAPI {
  if (!_qoraiWalletAPI) {
    _qoraiWalletAPI = [[QoraiWalletAPI alloc] initWithBrowserState:_profile];
  }
  return _qoraiWalletAPI;
}
...
- (QoraiWebViewConfiguration*)defaultWebViewConfiguration {
  if (!_defaultWebViewConfiguration) {
    _defaultWebViewConfiguration = [[QoraiWebViewConfiguration alloc]
...
- (QoraiWebViewConfiguration*)nonPersistentWebViewConfiguration {
  if (!_nonPersistentWebViewConfiguration) {
    _nonPersistentWebViewConfiguration = [[QoraiWebViewConfiguration alloc]
        initWithBrowserState:ios_web_view::WebViewBrowserState::
                                 FromBrowserState(
                                     _profile->GetOffTheRecordProfile())];
  }
