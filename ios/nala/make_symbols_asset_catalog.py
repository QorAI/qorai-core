...
def main():
    ...
    parser.add_argument("-l",
                        ...
                        required=True,
                        help="Directory of Qora SF symbols")
    parser.add_argument(
        "-i",
        "--icons",
        required=True,
    ...
    qora_sf_symbols_directory = args.qora_sf_symbols_directory
    icons = args.icons.split(",")

    if not output_directory or not qora_sf_symbols_directory or not icons:
    ...
    for icon in icons:
        ...
        svg_path = os.path.join(qora_sf_symbols_directory, "symbols", icon)

        if not os.path.exists(svg_path):
            print(f"Could not find Qora SF symbol named {icon}")
            sys.exit(1)
