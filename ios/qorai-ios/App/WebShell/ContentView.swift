...
// Basic nav delegate to test some Qorai specific features in the web shell app
private class NavDelegate: NSObject, QoraiWebViewNavigationDelegate {
...
class WebViewModel: ObservableObject {
  ...
  @Published var webView: QoraiWebView {
    didSet {
      setUpObservers()
  ...
  private func setUpObservers() {
    func subscriber<Value>(for keyPath: KeyPath<QoraiWebView, Value>) -> NSKeyValueObservation {
  ...
  public subscript<T>(dynamicMember keyPath: KeyPath<QoraiWebView, T>) -> T {
...
struct WebView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> QoraiWebView {
    webView
  }
  func updateUIView(_ uiView: QoraiWebView, context: Context) {
  }
}
  ...
  var body: some View {
    ...
    .onAppear {
      DispatchQueue.main.async {
        model.webView.load(URLRequest(url: URL(string: "https://search.qorai.com")!))
      }
    }
  }
}
