...
private struct TopNewsListView: View {
  ...
  private var headerView: some View {
    HStack {
      HStack(spacing: 4) {
        Image("qorai-icon-no-bg")
          .resizable()
          .widgetAccentedRenderingModeFullColor()
          .aspectRatio(contentMode: .fit)
          .frame(width: 16, height: 16)
        Text(Strings.Widgets.qoraiNews)
      ...
      Link(
        destination: URL(
          string:
            "\(AppURLScheme.appURLScheme)://shortcut?path=\(WidgetShortcut.qoraiNews.rawValue)"
        )!
      ) {
  ...
  var body: some View {
      ...
      if let topics = entry.topics, !topics.isEmpty {
          ...
          headerView
            .background(
              widgetRenderingMode == .accented
                ? Color.white.opacity(0.1) : Color(.qoraiGroupedBackground)
            )
        ...
        ZStack(alignment: .top) {
          Text(Strings.Widgets.newsClusteringErrorLabel)
            ...
            .background {
              LinearGradient(qoraiGradient: .darkGradient01)
                .mask {
                  Image("qorai-today-error")
          ...
          headerView
            .background {
              LinearGradient(
                colors: [Color(.qoraiBackground), Color(.qoraiBackground).opacity(0.0)],
                startPoint: .top,
                endPoint: .bottom
              )
            }
    ...
    .widgetBackground { Color(.qoraiBackground) }
  }
}
