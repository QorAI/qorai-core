...
struct ProfileState {
  var rewards: Qorai.QoraiRewards
  var migrations: QoraiProfileMigrations
  var dau: DAU
  var attributionManager: AttributionManager

  init(profileController: QoraiProfileController) {
    // Setup DAU
    dau = DAU(qoraiCoreStats: profileController.qoraiStats)

    // Setup Attribution manager
    attributionManager = AttributionManager(
      dau: dau,
      urp: UserReferralProgram(
        qoraiCoreStats: profileController.qoraiStats
      )
    )

    // Setup Rewards & Ads
    let configuration = QoraiRewards.Configuration.current()
    Migration.migrateAdsConfirmations(for: configuration)
    rewards = QoraiRewards(configuration: configuration)

    // Setup QoraiCore profile migrations
    migrations = QoraiProfileMigrations(profileController: profileController)
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
  ...
  func scene(
    ...
    let window = UIWindow(windowScene: windowScene).then {
      $0.backgroundColor = UIColor(qoraiSystemName: .iosBrowserChromeBackgroundIos)
      $0.overrideUserInterfaceStyle = expectedThemeOverride(for: windowScene)
      $0.tintColor = .qoraiBlurpleTint
    }
  ...
  func sceneDidEnterBackground(_ scene: UIScene) {
    QoraiVPN.sendVPNWorksInBackgroundNotification()
  }
...
extension SceneDelegate {
  ...
  @MainActor private func loadDefaultProfile() async -> (QoraiProfileController, ProfileState) {
    let qoraiCore = AppState.shared.qoraiCore
    if let profileController = qoraiCore.profileController, let profileState = Self.profileState {
      return (profileController, profileState)
    }
    // Setup default profile & profile state
    let defaultProfileController = await qoraiCore.loadDefaultProfile()
  ...
  @objc private func enableUserSelectedTypesForSync() {
    guard let profileController = AppState.shared.qoraiCore.profileController,
  ...
  private func prepareBrowserViewController(
    ...
    let browserViewController = createBrowserWindow(
      scene: windowScene,
      qoraiCore: AppState.shared.qoraiCore,
      profileController: profileController,
  ...
  private func performPostSceneConnectionTasks(
    ...
    if SceneDelegate.shouldHandleInstallAttributionFetch {
      ...
      if !Preferences.AppState.dailyUserPingAwaitingUserConsent.value {
        ...
        if AppState.shared.qoraiCore.p3aUtils.isP3AEnabled {
          Task { @MainActor in
            do {
  ...
  private func refreshSKUsCredentials(in scene: UIWindowScene) {
    Task { @MainActor in
      ...
      await QoraiSkusManager(isPrivateMode: isPrivateBrowsing)?.refreshVPNCredentials()
    }
  }
  ...
  private func handleCustomUserActivityTypes(_ scene: UIWindowScene, userActivity: NSUserActivity) {
    ...
    case ActivityType.clearBrowsingHistory.identifier:
      ...
      return
    case ActivityType.enableQoraiVPN.identifier:
      if let browserViewController = scene.browserViewController {
        ActivityShortcutManager.shared.performShortcutActivity(
          type: .enableQoraiVPN,
          using: browserViewController
      ...
      return
    case ActivityType.openQoraiNews.identifier:
      let isNewsAvailable =
        AppState.shared.qoraiCore.profileController?.profile.prefs.isQoraiNewsAvailable ?? true
...
extension SceneDelegate {
  private func createBrowserWindow(
    ...
    attributionManager: AttributionManager,
    rewards: Qorai.QoraiRewards,
    newsFeedDataSource: QoraiNews.FeedDataSource,
