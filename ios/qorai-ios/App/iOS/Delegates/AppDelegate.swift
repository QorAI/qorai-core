...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(
    ...
    QoraiVPN.fetchLastUsedRegionDetail()
    ...
    if Preferences.QoraiNews.isEnabled.value && !Preferences.QoraiNews.userOptedIn.value {
      // Opt-out any user that has not explicitly opted-in
      Preferences.QoraiNews.isEnabled.value = false
      // User now has to explicitly opt-in
      Preferences.QoraiNews.isShowingOptIn.value = true
    }
    ...
    let shouldPerformLanguageCheck =
      !Preferences.QoraiNews.languageChecked.value
      || Preferences.QoraiNews.languageWasUnavailableDuringCheck.value == true
    let isNewsEnabledOrPreviouslyOptedIn =
      Preferences.QoraiNews.isEnabled.value || Preferences.QoraiNews.userOptedIn.value
    if shouldPerformLanguageCheck, !isNewsEnabledOrPreviouslyOptedIn,
      let languageCode = Locale.preferredLanguages.first?.prefix(2)
    {
      Preferences.QoraiNews.languageChecked.value = true
      ...
      Preferences.QoraiNews.languageWasUnavailableDuringCheck.value = !languageShouldShowOptIn
      Preferences.QoraiNews.isShowingOptIn.value = languageShouldShowOptIn
    }
  ...
  func application(
    ...
    SKPaymentQueue.default().add(QoraiVPN.iapObserver)
    // Editing Product Promotion List
    Task { @MainActor in
      await QoraiVPN.updateStorePromotionOrder()
      await QoraiVPN.hideActiveStorePromotion()
    }

    // Qorai Store SDK - Initialization
    QoraiStoreSDK.shared.refreshAllSkusOrders()
    ...
    // Starting Date for Qorai Search Promotion to mark 15 days period
    if Preferences.QoraiSearch.qoraiSearchPromotionLaunchDate.value == nil {
      Preferences.QoraiSearch.qoraiSearchPromotionLaunchDate.value = Date()
    }
    ...
    if isFirstLaunch {
      ...
      QoraiVPN.clearCredentials()

      // Always load YouTube in Qorai for new users
      Preferences.General.keepYouTubeInQorai.value = true

      // Enable Search Suggestions for QoraiSearch default countries
      Preferences.Search.showSuggestions.value =
        AppState.shared.profile.searchEngines.isQoraiSearchDefaultRegion
    ...
    if let installDate = Preferences.P3A.installationDate.value, AppConstants.isOfficialBuild {
      AppState.shared.qoraiCore.initializeP3AService(
    ...
    Task(priority: .high) {
      ...
      await LaunchHelper.shared.prepareAdBlockServices(
        adBlockService: AppState.shared.qoraiCore.adblockService
      )
    }
  ...
  func application(
    ...
    guard let prefService = AppState.shared.qoraiCore.profileController?.profile.prefs else {
      return false
    }
    return QoraiTalkJitsiCoordinator.sendAppLifetimeEvent(
  ...
  func applicationWillTerminate(_ application: UIApplication) {
    SKPaymentQueue.default().remove(QoraiVPN.iapObserver)

    // Clean up QoraiCore
    AppState.shared.qoraiCore.profileController?.syncAPI.removeAllObservers()
