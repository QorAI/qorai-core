...
{
  result=$(curl -s -w "\n%{http_code}" --silent \
    -X "GET" "https://qorai-software.crowdin.com/api/v2/projects/$crowdin_project_id/translations/builds/$1" \
...
{
  result=$(curl -s -w "\n%{http_code}" --silent \
    -X "POST" "https://qorai-software.crowdin.com/api/v2/projects/$crowdin_project_id/translations/builds" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json'
  )
  http_code=$(tail -n1 <<< "$result")
...
{
  result=$(curl -s -w "\n%{http_code}" --silent \
    -X "GET" "https://qorai-software.crowdin.com/api/v2/projects/$crowdin_project_id/translations/builds/${BUILD_ID}/download" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json'
  )
  http_code=$(tail -n1 <<< "$result")
