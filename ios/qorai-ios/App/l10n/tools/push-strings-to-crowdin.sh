...
sed -i '' 's/Shared\/Supporting Files/qorai/' en.xliff >>output.log 2>&1
if [ $? != 0 ] ; then
  report_error 4 "ERROR: Failed to cleanup strings, please see output.log"
fi
...
add_storage_id()
{
  FILE_PATH="en.xliff"
  result=$(curl --silent \
    -X "POST" "https://qorai-software.crowdin.com/api/v2/storages" \
...
delete_storage_id()
{
  ...
  -X "DELETE" "https://qorai-software.crowdin.com/api/v2/storages/$1" \
  -H "Authorization: Bearer ${TOKEN}" \
...
{
  http_status_code=$(curl --silent -o /dev/null -w "%{http_code}" \
    -X "POST" "https://qorai-software.crowdin.com/api/v2/projects/$crowdin_project_id/files" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
    --data @- << EOF
      {
...
{
  http_status_code=$(curl --silent -o /dev/null -w "%{http_code}" \
    -X "DELETE" "https://qorai-software.crowdin.com/api/v2/projects/$crowdin_project_id/files/$1" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
  )
  echo $http_status_code
