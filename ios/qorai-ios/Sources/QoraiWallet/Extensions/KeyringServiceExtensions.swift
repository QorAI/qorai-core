...
extension QoraiWalletKeyringService {

  /// Check if any wallet account has been created given a coin type and a chain id
  @MainActor func isAccountAvailable(for coin: QoraiWallet.CoinType, chainId: String) async -> Bool
  {
  ...
  @MainActor func isAccountAvailable(for coin: QoraiWallet.CoinType) async -> Bool {
  ...
  func allAccountInfos(
    ...
    if checkBTCTestnet, !Preferences.Wallet.isBitcoinTestnetEnabled.value {
      accounts = accounts.filter({ $0.keyringId != QoraiWallet.KeyringId.bitcoin84Testnet })
    }
    if checkZcashTestnet, !Preferences.Wallet.isZcashTestnetEnabled.value {
      accounts = accounts.filter({ $0.keyringId != QoraiWallet.KeyringId.zCashTestnet })
    }
    return accounts
  }
}
