...
extension QoraiWalletAssetRatioService {
  ...
  @MainActor func fetchPrices(
    for tokens: [QoraiWallet.BlockchainToken],
    vsCurrency: String
  ) async -> [QoraiWallet.AssetPrice] {
...
extension QoraiWallet.AssetPrice {
  ...
  func eq(coin: QoraiWallet.CoinType, chainId: String, address: String) -> Bool {
...
extension Array where Element == QoraiWallet.AssetPrice {
  ...
  mutating func update(with newPrices: [QoraiWallet.AssetPrice]) {
