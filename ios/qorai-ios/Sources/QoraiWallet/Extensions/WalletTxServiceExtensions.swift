...
extension QoraiWalletTxService {

  // Fetches all pending transactions for all given keyrings
  func pendingTransactions(
    networksForCoin: [QoraiWallet.CoinType: [QoraiWallet.NetworkInfo]],
    for accounts: [QoraiWallet.AccountInfo]
  ) async -> [QoraiWallet.TransactionInfo] {
  ...
  func allTransactions(
    ...
    return await withTaskGroup(
      of: [QoraiWallet.TransactionInfo].self,
      body: { @MainActor group in
        ...
        var allTx: [QoraiWallet.TransactionInfo] = []
  ...
  func allTransactions(
    ...
    for accountInfo: QoraiWallet.AccountInfo
  ) async -> [QoraiWallet.TransactionInfo] {
    return await withTaskGroup(
      of: [QoraiWallet.TransactionInfo].self,
      body: { @MainActor group in
