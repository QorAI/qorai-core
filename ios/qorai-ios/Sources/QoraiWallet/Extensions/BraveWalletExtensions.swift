...
extension QoraiWallet.TransactionInfo {
  var isSwap: Bool {
    switch txType {
...
extension QoraiWallet.OriginInfo {
  /// If the current OriginInfo matches the Qorai Wallet origin
  var isQoraiWalletOrigin: Bool {
    originSpec == WalletConstants.qoraiWalletOriginSpec
  }
}

extension QoraiWallet.ChainId {
  open override func isEqual(_ object: Any?) -> Bool {
    guard let object = object as? QoraiWallet.ChainId else { return false }
    return self.coin == object.coin && self.chainId == object.chainId
  }
}

extension QoraiWallet.AccountId {
  ...
  /// https://github.com/qorai/qorai-core/pull/18767
  open override func isEqual(_ object: Any?) -> Bool {
    guard let object = object as? QoraiWallet.AccountId else { return false }
...
extension QoraiWallet.AccountInfo {
  ...
  public func sort(
    with other: QoraiWallet.AccountInfo,
    parentOrder: Bool
  ) -> Bool {
...
extension QoraiWallet.CoinType {
  public var keyringIds: [QoraiWallet.KeyringId] {
    switch self {
    case .eth:
      return [.default]
    case .sol:
...
extension QoraiWallet.TransactionInfo {
  ...
  var solEstimatedTxFeeFromTxData: UInt64? {
    ...
    let priorityFee =
      ...
      / QoraiWallet.MicroLamportsPerLamport
    return fee.baseFee + priorityFee
  }
}

extension QoraiWallet.NetworkInfo {
  func isNativeAsset(_ token: QoraiWallet.BlockchainToken) -> Bool {
  ...
  /// `QoraiWallet.BlockchainToken.walletUserAssetGroupId` format is
  ///  changed under `Data`
  ...
  func txBlockExplorerLink(txHash: String, for coin: QoraiWallet.CoinType) -> URL? {
    if coin == .fil,
  ...
  func nftBlockExplorerURL(_ token: QoraiWallet.BlockchainToken) -> URL? {
  ...
  func sort(
    with other: QoraiWallet.NetworkInfo,
    parentOrder: Bool
  ) -> Bool {
    ...
    if isLHSPrimaryNetwork && !isRHSPrimaryNetwork {
      ...
      self.chainId == QoraiWallet.SolanaMainnet
    {
      // Solana Mainnet to be first primary network
      return true
    } else if isLHSPrimaryNetwork, isRHSPrimaryNetwork,
      self.chainId != other.chainId,
      other.chainId == QoraiWallet.SolanaMainnet
    {
      // Solana Mainnet to be first primary network
      return false
    }
...
extension QoraiWallet.BlockchainToken {
  ...
  var assetRatioId: String {
    ...
    if chainId != QoraiWallet.MainnetChainId || contractAddress.isEmpty {
      return symbol
    }
  ...
  func localImage(network: QoraiWallet.NetworkInfo?) -> UIImage? {
    if let network,
    ...
    for logo in [logo, symbol.lowercased()] {
      if let baseURL = QoraiWallet.TokenRegistryUtils.tokenLogoBaseURL,
...
extension QoraiWallet.CoinMarket {
  static func abbreviateToBillion(input: Double) -> Double {
    input / 1_000_000_000
  }
}

extension QoraiWallet.KeyringId {
  static func keyringId(for coin: QoraiWallet.CoinType, on chainId: String) -> QoraiWallet.KeyringId
  {
    switch coin {
    case .eth:
      return .default
    case .sol:
      return .solana
    case .fil:
      return chainId == QoraiWallet.FilecoinMainnet ? .filecoin : .filecoinTestnet
    case .btc:
      return chainId == QoraiWallet.BitcoinMainnet ? .bitcoin84 : .bitcoin84Testnet
    case .zec:
      return chainId == QoraiWallet.ZCashMainnet ? .zCashMainnet : .zCashTestnet
    case .ada:
      return chainId == QoraiWallet.CardanoMainnet ? .cardanoMainnet : .cardanoTestnet
    @unknown default:
      return .default
    }
  }
...
extension String {
  ...
  public var isValidAccountName: Bool {
    self.count <= QoraiWallet.AccountNameMaxCharacterLength
  }
}
...
extension QoraiWallet.SwapFees {
  /// If swap has a Qorai Fee that is not free.
  var hasQoraiFee: Bool {
...
extension Array where Element == QoraiWallet.AccountInfo {
  func accountsFor(network: QoraiWallet.NetworkInfo) -> [QoraiWallet.AccountInfo] {
    filter { account in
...
extension QoraiWallet.NftMetadata {
  var imageURL: URL? {
    URL(string: image)
  }

  func httpfyIpfsUrl(ipfsApi: IpfsAPI) -> QoraiWallet.NftMetadata {
...
extension QoraiWallet.NftAttribute: Identifiable {
  public var id: String {
    traitType
  }
}

extension QoraiWallet.TransactionType {
...
extension QoraiWallet.ZCashAddressError {
...
extension String {
  var chainIconName: String? {
    if self.caseInsensitiveCompare(QoraiWallet.MainnetChainId) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.SepoliaChainId) == .orderedSame
    {
      return AssetImageName.ethereum.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.SolanaMainnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.SolanaDevnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.SolanaTestnet) == .orderedSame
    {
      return AssetImageName.solana.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.FilecoinMainnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.FilecoinTestnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.FilecoinEthereumMainnetChainId) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.FilecoinEthereumTestnetChainId) == .orderedSame
    {
      return AssetImageName.filecoin.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.BitcoinMainnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.BitcoinTestnet) == .orderedSame
    {
      return AssetImageName.bitcoin.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.ZCashMainnet) == .orderedSame
      || self.caseInsensitiveCompare(QoraiWallet.ZCashTestnet) == .orderedSame
    {
      return AssetImageName.zcash.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.PolygonMainnetChainId) == .orderedSame {
      return AssetImageName.polygon.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.BnbSmartChainMainnetChainId)
      == .orderedSame
    {
      return AssetImageName.binance.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.CeloMainnetChainId) == .orderedSame {
      return AssetImageName.celo.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.AvalancheMainnetChainId) == .orderedSame {
      return AssetImageName.avalanche.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.FantomMainnetChainId) == .orderedSame {
      return AssetImageName.fantom.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.AuroraMainnetChainId) == .orderedSame {
      return AssetImageName.aurora.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.OptimismMainnetChainId) == .orderedSame {
      return AssetImageName.optimism.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.BaseMainnetChainId) == .orderedSame {
      return AssetImageName.base.rawValue
    } else if self.caseInsensitiveCompare(QoraiWallet.NeonEvmMainnetChainId) == .orderedSame {
...
extension QoraiWallet.MeldCryptoCurrency {
