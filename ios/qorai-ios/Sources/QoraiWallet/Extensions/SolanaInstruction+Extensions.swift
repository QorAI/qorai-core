...
extension QoraiWallet.SolanaInstruction {
  ...
  var isTokenProgram: Bool {
    programId == QoraiWallet.SolanaTokenProgramId
  }

  var isAssociatedTokenProgram: Bool {
    programId == QoraiWallet.SolanaAssociatedTokenProgramId
  }

  var isSysvarRentProgram: Bool {
    programId == QoraiWallet.SolanaSysvarRentProgramId
  }
  ...
  var instructionName: String {
      ...
      )
    {
      ...
      let instructionType = QoraiWallet.SolanaTokenInstruction(
        rawValue: Int(decodedData.instructionType)
      )
    {
      let name = instructionType.name
  ...
  var toPubkey: String? {
    guard
      let index = decodedData?.accountParams.firstIndex(
        where: { $0.name == QoraiWallet.ToAccount }
      )
    else { return nil }
  ...
  var fromPubkey: String? {
    guard
      let index = decodedData?.accountParams.firstIndex(
        where: { $0.name == QoraiWallet.FromAccount }
      )
    else { return nil }
  ...
  var accountKeyValues: [SolanaTxDetails.ParsedSolanaInstruction.KeyValue] {
    ...
    return decodedData.accountParams.enumerated().compactMap {
      ...
      if param.name == QoraiWallet.Signers {  // special case
...
extension QoraiWallet.SolanaAccountMeta {
...
extension QoraiWallet.SolanaSystemInstruction {
  var name: String {
    switch self {
    case .transfer:
...
extension QoraiWallet.SolanaTokenInstruction {
  var name: String {
    switch self {
    case .initializeMint:
...
extension QoraiWallet.DecodedSolanaInstructionData {
  func paramFor(_ paramName: String) -> QoraiWallet.SolanaInstructionParam? {
