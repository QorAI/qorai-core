...
extension QoraiWalletMeldIntegrationService {
  /// - Parameters:
  ///     - token: A `QoraiWallet.BlockchainToken` from token registry that we want to convert from
  ///
  /// - Returns: A  tuple of `QoraiWallet.MeldCryptoCurrency` and a list of QoraiWallet.MeldCryptoCurrency.
  ...
  ) async -> (QoraiWallet.MeldCryptoCurrency?, [QoraiWallet.MeldCryptoCurrency]?) {
