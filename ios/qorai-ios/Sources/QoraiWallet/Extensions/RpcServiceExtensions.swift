...
extension QoraiWalletJsonRpcService {
  ...
  func balance(
    ...
    in account: QoraiWallet.AccountInfo,
  ...
  func balance(
    ...
    case .btc:
      // Bitcoin balance should be fetched using `QoraiWallet.BitcoinWalletService`
      completion(nil)
    case .zec:
      // Zcash balance should be fetched using `QoraiWallet.ZCashWalletService`
      completion(nil)
    @unknown default:
      completion(nil)
    }
  ...
  private func ethBalance(
    ...
    in accountAddress: String,
    network: QoraiWallet.NetworkInfo,
    completion: @escaping (String, QoraiWallet.ProviderError, String) -> Void
  ) {
    if network.isNativeAsset(token) {
      balance(
  ...
  @MainActor func allNetworks(
    for coins: [QoraiWallet.CoinType],
    respectHiddenNetworksPreference: Bool = true
  ) async -> [QoraiWallet.NetworkInfo] {
    ...
    let filteredNetworks = allNetworks.filter { network in
      if network.chainId == QoraiWallet.LocalhostChainId {
        // localhost not supported on iOS
        return false
      }
      if network.chainId == QoraiWallet.BitcoinTestnet {
      ...
      } else if network.chainId == QoraiWallet.ZCashTestnet {
        if respectHiddenNetworksPreference {
  ...
  @MainActor func fetchNFTMetadata(
    for token: QoraiWallet.BlockchainToken,
    ipfsApi: IpfsAPI
  ) async -> QoraiWallet.NftMetadata? {
    let nftIdentifier: QoraiWallet.NftIdentifier =
      .init(
        chainId: .init(coin: token.coin, chainId: token.chainId),
        contractAddress: token.contractAddress,
        tokenId: token.tokenId
  ...
  /// Returns a map of Token.id with its `QoraiWallet.NftMetadata`
  @MainActor func fetchNFTMetadata(
    tokens: [QoraiWallet.BlockchainToken],
    ipfsApi: IpfsAPI
  ) async -> [String: QoraiWallet.NftMetadata] {
    var tokenCoinMap = [QoraiWallet.CoinType: [QoraiWallet.BlockchainToken]]()
    ...
    return await withTaskGroup(
      of: [String: QoraiWallet.NftMetadata].self
    ) { @MainActor [weak self] group in
      guard let self = self else { return [:] }
      for (coin, tokensPerCoin) in tokenCoinMap {
        group.addTask {
          var uniqueTokensPerCoin: [QoraiWallet.BlockchainToken] = []
          ...
          var result = [String: QoraiWallet.NftMetadata]()
  ...
  ) async -> [QoraiWallet.BlockchainToken] {
    await withTaskGroup(of: [QoraiWallet.BlockchainToken?].self) { @MainActor group in
      ...
      return await group.reduce([QoraiWallet.BlockchainToken?](), { $0 + $1 })
    }.compactMap { $0 }
  }
      ...
      for coinType in coins {
        group.addTask { @MainActor in
          ...
          return chains.filter {  // localhost not supported
            $0 != QoraiWallet.LocalhostChainId
          }
        }
      }
  ...
  @MainActor func removeHiddenNetworks(
    for networks: [QoraiWallet.CoinType: [String]]
  ) async {
...
extension Array where Element == QoraiWallet.TransactionInfo {
  func unknownTokenContractAddressChainIdPairs(
    knownTokens: [QoraiWallet.BlockchainToken]
