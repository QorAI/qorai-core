...
public protocol WalletUserAssetManagerType: AnyObject {
  ...
  /// Return a `WalletUserAsset` with a given `QoraiWallet.BlockchainToken`
  func getUserAsset(_ asset: QoraiWallet.BlockchainToken) -> WalletUserAsset?
  ...
  /// `QoraiWallet.BlockchainToken` from CoreData.
  /// As well as update in `WalletService`.
  @discardableResult func removeUserAsset(_ token: QoraiWallet.BlockchainToken) async -> Bool
  ...
  /// `QoraiWallet.BlockchainToken` from CoreData
  func removeAssetBalances(for asset: QoraiWallet.BlockchainToken) async
  /// Remove a `WalletUserAssetBalance` representation of the given
  /// `QoraiWallet.NetworkInfo` from CoreData
  func removeAssetBalances(for network: QoraiWallet.NetworkInfo) async
  ...
  func removeUserAssetsAndBalances(for network: QoraiWallet.NetworkInfo?) async
}
...
public class WalletUserAssetManager: WalletUserAssetManagerType, WalletObserverStore {
  ...
  @MainActor public func getUserAssets(
    ...
    let locallyHiddenTokens: [QoraiWallet.BlockchainToken] =
  ...
  public func getUserAsset(_ asset: QoraiWallet.BlockchainToken) -> WalletUserAsset? {
  ...
  @MainActor public func removeAssetBalances(
    for network: QoraiWallet.NetworkInfo
  ) async {
    await WalletUserAssetBalance.removeBalances(
      for: network
    )
  ...
  private func allNewCoinsIntroduced(
    ...
    var newCoins: Set<QoraiWallet.CoinType> = []
  ...
  private func migrateUserAssets(for coins: [QoraiWallet.CoinType]) async {
    var fetchedUserAssets: [String: [QoraiWallet.BlockchainToken]] = [:]
    let networks: [QoraiWallet.NetworkInfo] = await rpcService.allNetworks(
      for: coins,
      respectHiddenNetworksPreference: false
    )
...
public class TestableWalletUserAssetManager: WalletUserAssetManagerType {
  public var _getUserAssets:
    ((_ networks: [QoraiWallet.NetworkInfo], _ visible: Bool) -> [NetworkAssets])?
  public var _getAllUserAssetsInNetworkAssets:
    ((_ networks: [QoraiWallet.NetworkInfo], _ includingUserDeleted: Bool) -> [NetworkAssets])?
  public var _getAllUserNFTs:
    ((_ networks: [QoraiWallet.NetworkInfo], _ spamStatus: Bool) -> [NetworkAssets])?
  public var _getAllUserDeletedUserAssets: (() -> [WalletUserAsset])?
  public var _getBalance:
    ((_ asset: QoraiWallet.BlockchainToken?, _ account: String?) -> [WalletUserAssetBalance]?)?
  ...
  public func getAllUserAssetsInNetworkAssets(
    ...
    let defaultAssets: [NetworkAssets] = [
      ...
      NetworkAssets(
        network: .mockSepolia,
        tokens: [
          (QoraiWallet.BlockchainToken.previewToken.copy() as! QoraiWallet.BlockchainToken).then {
  ...
  public func getUserAsset(_ token: QoraiWallet.BlockchainToken) -> WalletUserAsset? {
    return nil
  }
  ...
  public func removeUserAsset(_ asset: QoraiWallet.BlockchainToken) async -> Bool {
    return false
  }
  ...
  public func removeAssetBalances(
    for asset: QoraiWallet.BlockchainToken
  ) async {
  }
