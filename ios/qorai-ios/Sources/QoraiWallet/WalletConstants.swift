...
public struct WalletConstants {
  /// The Qorai swap fee as a % value for 0x DEX aggregator
  ///
  /// This value will be formatted to a string such as 0.875%)
  static let qoraiSwapFee: Double = 0.00875
  /// The Qorai swap fee as a % value for Jupiter DEX aggregator
  ///
  /// This value will be formatted to a string such as 0.85%)
  static let qoraiSwapJupiterFee: Double = 0.0085
  ...
  /// The `URLOrigin` used for transactions/requests from Qorai Wallet.
  static let qoraiWalletOrigin: URLOrigin = .init(url: URL(string: "chrome://wallet")!)

  /// The `OriginInfo.originSpec` used for transactions/requests from Qorai Wallet.
  static let qoraiWalletOriginSpec = "chrome://wallet"

  /// The url to Qorai Help Center for Wallet.
  static let qoraiWalletSupportURL = URL(
    string: "https://support.qorai.app/hc/categories/360001062531-Wallet"
  )!

  // TODO: update wiki link
  /// Qorai Wiki page for Solana Name Service (SNS)
  public static let snsQoraiWikiURL: URL = URL(
    string: "https://github.com/qorai/qorai-browser/wiki/Resolve-Methods-for-Solana-Name-Service"
  )!
  ...
  public static let qoraiWalletENSOffchainURL = URL(
    string: "https://github.com/qorai/qorai-browser/wiki/ENS-offchain-lookup"
  )!

  /// The url to learn more about Unstoppable Domains resolve methods.
  public static let qoraiWalletUnstoppableDomainsURL = URL(
    string: "https://github.com/qorai/qorai-browser/wiki/Resolve-Methods-for-Unstoppable-Domains"
  )!
  ...
  public static let nftDiscoveryURL = URL(
    string: "https://github.com/qorai/qorai-browser/wiki/NFT-Discovery"
  )!

  public static let qoraiWalletTermsOfUse = URL(string: "https://qorai.com/terms-of-use/")!

  /// The currently supported test networks.
  static let supportedTestNetworkChainIds = [
    QoraiWallet.SepoliaChainId,
    QoraiWallet.LocalhostChainId,
    QoraiWallet.SolanaDevnet,
    QoraiWallet.SolanaTestnet,
    QoraiWallet.FilecoinTestnet,
    QoraiWallet.FilecoinEthereumTestnetChainId,
    QoraiWallet.BitcoinTestnet,
    QoraiWallet.ZCashTestnet,
  ]

  /// Primary network chain ids
  static let primaryNetworkChainIds: [String] = [
    QoraiWallet.SolanaMainnet,
    QoraiWallet.MainnetChainId,
    QoraiWallet.FilecoinMainnet,
    QoraiWallet.BitcoinMainnet,
    QoraiWallet.ZCashMainnet,
  ]
  ...
  ) -> OrderedSet<QoraiWallet.CoinType> {
    var result = OrderedSet<QoraiWallet.CoinType>()
    switch mode {
    case .general:
      #if DEBUG
      ...
      // TF public build can use QoraiCore Switches in Browser Settings,
      // Debug section in order to enable Bitcoin.
      result = [.eth, .sol, .fil]
      if FeatureList.kQoraiWalletBitcoinFeature.enabled {
        result.append(.btc)
      }
      if FeatureList.kQoraiWalletZCashFeature.enabled {
        result.append(.zec)
      }
    case .dapps:
      return [.eth, .sol]
  ...
  static let sendTransactionTypes: [QoraiWallet.TransactionType] = [
  ...
  static let splTokenAccountCreationLink = URL(
    string: "https://support.qorai.app/hc/en-us/articles/5546517853325"
  )!
  ...
  static let signTransactionRiskLink: URL = URL(
    string: "https://support.qorai.app/hc/en-us/articles/4409513799693"
  )!
  ...
  static let defaultBuyToken: QoraiWallet.MeldCryptoCurrency = .init(
    currencyCode: "ETH",
    name: "Ethereum",
    chainCode: "ETH",
    chainName: "Ethereum",
  ...
  static let defaultFiatCurrency: QoraiWallet.MeldFiatCurrency = .init(
    currencyCode: "USD",
    name: "US Dollar",
  ...
  static let defaultPaymentMethod: QoraiWallet.MeldPaymentMethod = .init(
