    ...
    VStack(alignment: .leading, spacing: 12) {
      ...
      ZStack {
        ...
        LineChartShape(points: points)
          .stroke(Color(.qoraiBlurpleTint), lineWidth: 2)
      }
      .frame(maxWidth: .infinity, alignment: .leading)
      .overlay(
        Group {
          if let dragContext = dragContext {
            ZStack {
              Rectangle()
                .fill(Color(.qoraiBlurpleTint))
                .frame(width: 2)
                .frame(maxHeight: .infinity)
                .padding(.vertical, -10)
                .position(x: dragContext.location.x, y: dragContext.size.height / 2)
              Circle()
                .strokeBorder(Color(.qoraiBlurpleTint), style: .init(lineWidth: 2))
                .frame(width: 12, height: 12)
                .background(Color(.qoraiBackground).clipShape(Circle()))
