...
public struct WalletPanelContainerView: View {
  ...
  private var setupView: some View {
    ScrollView(.vertical) {
      VStack(spacing: 36) {
        VStack(spacing: 4) {
          Text(Strings.Wallet.qoraiWallet)
...
struct WalletPanelView: View {
  ...
  enum ConnectionStatus {
    ...
    func title(_ coin: QoraiWallet.CoinType) -> String {
      switch self {
      case .connected:
  ...
  private var networkPickerButton: some View {
    NetworkPicker(
      style: .init(textColor: .qoraiLabel, borderColor: .secondaryButtonTint),
      isForOrigin: true,
      keyringStore: keyringStore,
      networkStore: networkStore
    )
  ...
  private var pendingRequestsButton: some View {
    Button {
      ...
      Image(qoraiSystemName: "qora.notification.dot")
        .foregroundColor(Color(.qoraiLabel))
  ...
  private var fullscreenButton: some View {
    Button {
      ...
      Image(qoraiSystemName: "qora.expand")
        .foregroundColor(Color(.qoraiLabel))
  ...
  private var menuButton: some View {
    Menu {
      Button {
        ...
        Label(Strings.Wallet.lock, qoraiSystemImage: "qora.lock")
      }
      Button {
        presentWalletWithContext(.settings)
      } label: {
        Label(Strings.Wallet.settings, qoraiSystemImage: "qora.settings")
      }
      Divider()
      Button {
        openWalletURL(WalletConstants.qoraiWalletSupportURL)
      } label: {
        Label(Strings.Wallet.helpCenter, qoraiSystemImage: "qora.info.outline")
      }
    } label: {
    ...
    ScrollView(.vertical, showsIndicators: false) {
      VStack(spacing: 0) {
        if sizeCategory.isAccessibilityCategory {
          ...
          .padding(.vertical, 4)
          .overlay(
            Color(.qoraiLabel).opacity(0.3)  // Divider
              ...
              Blockie(address: keyringStore.selectedAccount.blockieSeed)
                ...
                .overlay(
                  RoundedRectangle(cornerRadius: 4)
                    .strokeBorder(Color(.qoraiLabel).opacity(0.6), style: .init(lineWidth: 1))
                )
                .overlay(
                  Image(systemName: "chevron.down.circle.fill")
                    .font(.footnote)
                    .background(Color(.qoraiLabel).clipShape(Circle()))
                    .offset(x: -4, y: 4),
