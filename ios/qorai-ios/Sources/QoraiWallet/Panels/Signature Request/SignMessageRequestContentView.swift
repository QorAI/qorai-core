...
struct SignMessageRequestContentView: View {
  ...
  private var requestDisplayAttributedText: NSAttributedString? {
    ...
    let regularAttributes: [NSAttributedString.Key: Any] = [
      .font: regularFont, .foregroundColor: UIColor.qoraiLabel,
    ]
    if requestDomain.isEmpty {
    ...
    let boldAttributes: [NSAttributedString.Key: Any] = [
      .font: boldFont, .foregroundColor: UIColor.qoraiLabel,
    ]
  ...
  var body: some View {
    VStack(spacing: 20) {
      ...
      .background(
        Color(.tertiaryQoraiGroupedBackground),
      ...
      .introspectTextView { textView in
        // A flash to show users message is overflowing the text view (related to issue https://github.com/qorai/qorai-ios/issues/6277)
        if showOrignalMessage[request.id] == true {
  ...
  var body: some View {
    VStack(spacing: 8) {
      if needsPilcrowFormatted {
        Text(
          "\(Image(qoraiSystemName: "qora.warning.triangle-filled")) \(Strings.Wallet.signMessageConsecutiveNewlineWarning)"
        )
      ...
      if hasUnknownUnicode {
        Text(
          "\(Image(qoraiSystemName: "qora.warning.triangle-filled"))  \(Strings.Wallet.signMessageRequestUnknownUnicodeWarning)"
        )
        .font(.subheadline.weight(.medium))
        .foregroundColor(Color(.qoraiLabel))
        .multilineTextAlignment(.center)
    ...
    .frame(maxWidth: .infinity)
    .background(
      Color(.qoraiWarningBackground)
        .overlay(
          RoundedRectangle(cornerRadius: 10, style: .continuous)
            .strokeBorder(Color(.qoraiWarningBorder), style: StrokeStyle(lineWidth: pixelLength))
        )
