...
struct SaferSignMessageRequestContainerView: View {
  ...
  let cowSwapOrder: QoraiWallet.CowSwapOrder
  ...
  private var originAndFavicon: some View {
    VStack {
      Group {
        if request.originInfo.isQoraiWalletOrigin {
          Image(uiImage: UIImage(sharedNamed: "qorai.logo")!)
            .resizable()
            .aspectRatio(contentMode: .fit)
            .foregroundColor(Color(.qoraiOrange))
        } else {
      ...
      HStack(alignment: .top) {
        VStack(alignment: .leading, spacing: 4) {
          ...
          HStack {
            ...
            Text(Strings.Wallet.qoraiSwapFree)
              .foregroundColor(Color(.qoraiLabel))
          }
