...
struct SuggestedNetworkView: View {
  enum Mode: Equatable {
    case switchNetworks(QoraiWallet.SwitchChainRequest)
    case addNetwork(QoraiWallet.AddChainRequest)
  }
  ...
  private var chain: QoraiWallet.NetworkInfo? {
    switch mode {
    case .addNetwork(let request):
    ...
    VStack {
      Menu {
        ...
        Button {
          ...
          Label(Strings.Wallet.copyAddressButtonTitle, qoraiSystemImage: "qora.copy.plain-text")
            .font(.body)
        }
      } label: {
        HStack(spacing: 8) {
      ...
      SuggestedNetworkView(
        mode: .switchNetworks(
          .init(
            ...
            chainId: QoraiWallet.NetworkInfo.mockPolygon.chainId
          )
        ),
        cryptoStore: .previewStore,
        keyringStore: .previewStoreWithWalletCreated,
