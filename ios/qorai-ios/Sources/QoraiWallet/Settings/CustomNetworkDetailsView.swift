...
class NetworkModel: ObservableObject, Identifiable {
  enum Mode {
    case add
    case edit(_ network: QoraiWallet.NetworkInfo)
    case view(_ network: QoraiWallet.NetworkInfo)
  ...
  init(mode: Mode = .add) {
    ...
    if let network {
      ...
      {
        // Eth and EVM QoraiWallet.NetworkInfo.chainId should always in hex
        ...
        Text(item.wrappedValue.input)
          ...
          .contextMenu {
            Button {
              ...
              Label(Strings.Wallet.copyToPasteboard, qoraiSystemImage: "qora.copy.plain-text")
            }
          }
      } else {
        NetworkTextField(
...
struct NetworkRadioButton: View {
  ...
  var body: some View {
    Image(qoraiSystemName: checked ? "qora.check.circle-outline" : "qora.radio.unchecked")
      .renderingMode(.template)
      .foregroundColor(Color((checked && !isDisabled) ? .qoraiBlurpleTint : .qoraiDisabled))
      .font(.title3)
      .onTapGesture {
