...
struct BuyProviderSelectionView: View {
  ...
  let sortedQuotes: [QoraiWallet.MeldCryptoQuote]
  let providers: [QoraiWallet.MeldServiceProvider]
      ...
      if sortedQuotes.isEmpty {
        VStack {
          Image(qoraiSystemName: "qora.info.outline")
            ...
            .background(
              Color(
                qoraiSystemName: .pageBackground
              ).clipShape(.circle)
            )
          Text(
...
private struct ProviderView: View {
  ...
  let quote: QoraiWallet.MeldCryptoQuote
  let provider: QoraiWallet.MeldServiceProvider
  ...
  init(
    buyTokenStore: BuyTokenStore,
    quote: QoraiWallet.MeldCryptoQuote,
    ...
    VStack {
      Button {
        ...
        HStack {
            ...
            let url = URL(string: urlString)
          {
            WebImageReader(url: url) { image in
              if let image = image {
                ...
                Rectangle()
                  .foregroundColor(Color(.tertiaryQoraiGroupedBackground))
                  .frame(width: min(iconSize, maxIconSize), height: min(iconSize, maxIconSize))
                  .shimmer(true)
              }
            }
          ...
          Spacer()
          if isBestOption {
            Label(Strings.Wallet.providerListBestOption, qoraiSystemImage: "qora.thumb.up")
              .padding(4)
              .font(.caption2.weight(.semibold))
              .foregroundColor(Color(qoraiSystemName: .green60))
              .background(Color(qoraiSystemName: .green10))
              .cornerRadius(4)
          }
          Image(qoraiSystemName: isExpanded ? "qora.carat.up" : "qora.carat.down")
