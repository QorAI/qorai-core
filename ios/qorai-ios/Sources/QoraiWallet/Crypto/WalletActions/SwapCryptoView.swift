  ...
  var body: some View {
    HStack(spacing: 8) {
      ForEach(Option.predefinedOptions, id: \.id) { option in
        ...
        Button {
          ...
          Text(option.localizedString)
            ...
            .frame(maxWidth: .infinity)
            .foregroundColor(
              Color(isPredefinedOptionSelected(option.id) ? .white : .secondaryQoraiLabel)
            )
            .background(
              WalletActionsGridBackgroundView(
                backgroundColor: Color(
                  isSelected ? .qoraiBlurpleTint : .secondaryQoraiGroupedBackground
                )
              )
            )
            .padding(.top, 8)
      ...
      TextField("%", text: $input)
        ...
        .foregroundColor(Color(customSlippage != nil ? .white : .secondaryQoraiLabel))
        .background(
          WalletActionsGridBackgroundView(
            backgroundColor: Color(
              customSlippage != nil ? .qoraiBlurpleTint : .secondaryQoraiGroupedBackground
            )
          )
        )
        .padding(.top, 8)
  ...
  var body: some View {
    HStack {
      ...
      Button {
        ...
        Label(Strings.Wallet.refreshMarketPriceLabel, qoraiSystemImage: "qora.refresh")
          .labelStyle(.iconOnly)
...
struct SwapCryptoView: View {
  ...
  var dexAggregator: DEXAggregator {
    ...
    // policy https://github.com/qorai/qorai-browser/issues/36436
  ...
  @ViewBuilder var swapFormSections: some View {
    ...
    Section(
      //      MVP only supports market price swap. Ref: https://github.com/qorai/qorai-browser/issues/18307
      //
      ...
      header: MarketPriceView(swapTokenStore: swapTokensStore)
        .listRowBackground(Color(UIColor.qoraiGroupedBackground))
        .resetListHeaderStyle()
        .padding(.horizontal)
        .padding(.bottom, 15),
      footer: Group {
  ...
  @ViewBuilder private var feesFooter: some View {
    if swapTokensStore.qoraiFeeForDisplay != nil {
      VStack(spacing: 4) {
        if let qoraiFeeForDisplay = swapTokensStore.qoraiFeeForDisplay {
          if swapTokensStore.isQoraiFeeVoided {
            Text(
              String.localizedStringWithFormat(
                Strings.Wallet.qoraiFeeLabel,
                Strings.Wallet.qoraiSwapFree
              ) + " "
            ) + Text(qoraiFeeForDisplay).strikethrough()
          } else {
            Text(String.localizedStringWithFormat(Strings.Wallet.qoraiFeeLabel, qoraiFeeForDisplay))
          }
        }
      }
      .font(.footnote)
