...
struct DepositTokenView: View {
  ...
  @State private var selectedAccount: QoraiWallet.AccountInfo?
  ...
  private func availableAccounts(
    ...
    let keyringIdForToken: QoraiWallet.KeyringId = .keyringId(
      for: token.coin,
      on: token.chainId
    )
    guard
...
private struct DepositDetailsView: View {
  ...
  var supportedNetworks: [QoraiWallet.NetworkInfo]
  ...
  enum DepositType {
    ...
    case prefilledToken(
      ...
      availableAccounts: [QoraiWallet.AccountInfo],
      bitcoinAccounts: [String: QoraiWallet.BitcoinAccountInfo],
      zcashAccounts: [String: QoraiWallet.ZCashAccountInfo]
    )
  }
  ...
  init(
    type: DepositType,
    supportedNetworks: [QoraiWallet.NetworkInfo]
  ) {
    self.type = type
  ...
  private var accountPicker: some View {
    HStack(spacing: 16) {
      ...
      Text(selectedAccount?.name ?? "")
        ...
        .font(.body)
      Image(qoraiSystemName: "qora.arrow.small-down")
    ...
    .background {
      RoundedRectangle(cornerRadius: 8)
        .fill(Color(.secondaryQoraiBackground))
    }
  }
    ...
    VStack(spacing: 12) {
      ...
      HStack {
        ...
        Button {
          ...
          HStack {
            Image(qoraiSystemName: "qora.share.macos")
            Text(Strings.share)
              .font(.footnote.weight(.semibold))
          }
          .foregroundColor(Color(.qoraiBlurpleTint))
  ...
  private var ethNetworksCombined: String {
    let networks =
      supportedNetworks
      .filter {
        $0.coin == .eth && $0.chainId.lowercased() != QoraiWallet.MainnetChainId.lowercased()
      }
      .map { $0.chainName }
...
extension Array where Element == QoraiWallet.NetworkInfo {
  fileprivate func supportedNetworks(keyringId: QoraiWallet.KeyringId) -> [QoraiWallet.NetworkInfo]
  {
    return self.filter {
