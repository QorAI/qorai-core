...
struct PortfolioHeaderView: View {
  ...
  @Binding var selectedDateRange: QoraiWallet.AssetPriceTimeframe
  var balance: String
      ...
      if let balanceDifference {
        HStack {
          Text(isShowingBalances.value ? balanceDifference.priceDifference : "****")
            ...
            .foregroundColor(
              Color(
                qoraiSystemName: balanceDifference.isBalanceUp
          ...
          Text(isShowingBalances.value ? balanceDifference.percentageChange : "****")
            ...
            .padding(4)
            .foregroundColor(
              Color(qoraiSystemName: balanceDifference.isBalanceUp ? .green50 : .red50)
            )
            .background(
              Color(qoraiSystemName: balanceDifference.isBalanceUp ? .green20 : .red20)
                .cornerRadius(4)
            )
        }
      }
      ...
      LineChartView(
        ...
        LinearGradient(
          gradient: Gradient(colors: [
            Color(.qoraiBlurpleTint).opacity(colourScheme == .dark ? 0.5 : 0.2), .clear,
          ]),
          startPoint: .top,
          endPoint: .bottom
        )
...
struct PortfolioHeaderButton: View {
  ...
  enum Style: String, Equatable {
    ...
    var iconName: String {
      switch self {
      case .buy: return "qora.coins.alt1"
      case .send: return "qora.send"
      case .swap: return "qora.currency.exchange"
      case .more: return "qora.more.horizontal"
      case .deposit: return "qora.money.bag-coins"
      }
    }
  }
  ...
  var body: some View {
    Button(action: action) {
      VStack {
        Circle()
          .fill(Color(qoraiSystemName: .buttonBackground))
          .frame(width: 48, height: 48)
          .overlay(
            Image(qoraiSystemName: style.iconName)
    ...
    Menu(
      content: {
        Picker(
          ...
          content: {
            ForEach(QoraiWallet.AssetPriceTimeframe.allCases, id: \.self) { range in
      ...
      label: {
        HStack(spacing: 4) {
          Text(verbatim: selectedDateRange.accessibilityLabel)
            .font(.footnote.weight(.semibold))
          Image(qoraiSystemName: "qora.carat.down")
        }
        .foregroundColor(Color(qoraiSystemName: .textInteractive))
        .padding(.vertical, 6)
        .padding(.horizontal, 12)
        .padding(.trailing, -4)  // whitespace on `qora.carat.down` symbol
        .background(
          Capsule()
