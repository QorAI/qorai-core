...
struct Filters {
  ...
  init(
    ...
    accounts: [Selectable<QoraiWallet.AccountInfo>],
    networks: [Selectable<QoraiWallet.NetworkInfo>]
  ) {
...
struct FiltersDisplaySettingsView: View {
  ...
  /// All accounts and if they are currently selected. Default is all accounts selected.
  @State var accounts: [Selectable<QoraiWallet.AccountInfo>]
  /// All networks and if they are currently selected. Default is all selected except known test networks.
  @State var networks: [Selectable<QoraiWallet.NetworkInfo>]
  ...
  private var sortAssets: some View {
    FilterPickerRowView(
      ...
      icon: .init(
        qoraiSystemName: "qora.sort.desc",
        iconContainerSize: min(iconContainerSize, maxIconContainerSize)
      ),
  ...
  private var hideSmallBalances: some View {
    Toggle(isOn: $isHidingSmallBalances) {
      FilterLabelView(
        ...
        icon: .init(
          qoraiSystemName: "qora.eye.on",
          iconContainerSize: min(iconContainerSize, maxIconContainerSize)
        )
      )
    }
    .tint(Color(.qoraiBlurpleTint))
  }
    ...
    NavigationLink(
      ...
      label: {
        FilterDetailRowView(
          ...
          icon: .init(
            qoraiSystemName: "qora.user.accounts",
    ...
    HStack {
      if let icon {
        Color(uiColor: WalletV2Design.containerHighlight)
          ...
          .overlay {
            Image(qoraiSystemName: icon.qoraiSystemName)
              .imageScale(.medium)
              .font(.headline)
