...
struct AddAccountView: View {
  ...
  @State private var selectedCoin: QoraiWallet.CoinType?
  @State private var accountNetwork: QoraiWallet.NetworkInfo
  ...
  var preSelectedCoin: QoraiWallet.CoinType?
  var preSelectedAccountNetwork: QoraiWallet.NetworkInfo?
  var onCreate: (() -> Void)?
  var onDismiss: (() -> Void)?

  private var selectedCoinNetworks: [QoraiWallet.NetworkInfo] {
    guard let coin: QoraiWallet.CoinType = preSelectedCoin ?? selectedCoin else { return [] }
  ...
  init(
    ...
    preSelectedCoin: QoraiWallet.CoinType? = nil,
    preSelectedAccountNetwork: QoraiWallet.NetworkInfo? = nil,
    onCreate: (() -> Void)? = nil,
    onDismiss: (() -> Void)? = nil
  ) {
  ...
  private func addAccount(for coin: QoraiWallet.CoinType) {
  ...
  private var navigationTitle: String {
    ...
    return String.localizedStringWithFormat(
      ...
      preSelectedCoin?.localizedTitle
        ?? (selectedCoin?.localizedTitle ?? QoraiWallet.CoinType.eth.localizedTitle)
    )
  }
    ...
    .navigationBarItems(
      ...
      trailing: Button {
        ...
        ZStack {  // maintain button size
          ProgressView()
            .progressViewStyle(.qoraiCircular(size: .small))
            .hidden(isHidden: !isCreatingAccount)
          Text(Strings.Wallet.add)
            .hidden(isHidden: isCreatingAccount)
        }
      }
      .buttonStyle(QoraiFilledButtonStyle(size: .small))
        ...
        SectionFooterErrorView(
          errorMessage: name.isValidAccountName
            ...
            : String.localizedStringWithFormat(
              ...
              QoraiWallet.AccountNameMaxCharacterLength
            )
        )
      }
    )
        ...
        HStack {
          ...
          Text(
            String.localizedStringWithFormat(
              ...
              accountNetwork.chainId == QoraiWallet.BitcoinMainnet ? "zprv" : "vprv"
            )
          )
          .font(.caption)
          .foregroundColor(Color(qoraiSystemName: .systemfeedbackWarningText))
        }
        .listRowBackground(Color(qoraiSystemName: .systemfeedbackWarningBackground))
      }
      Group {
