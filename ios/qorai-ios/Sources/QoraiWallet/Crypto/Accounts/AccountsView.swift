...
struct AccountsView: View {
  ...
  @State private var selectedAccountActivity: QoraiWallet.AccountInfo?
  /// When populated, account info presented modally for given account (rename, export private key)
  @State private var selectedAccountForEdit: QoraiWallet.AccountInfo?
  /// When populated, private key export is presented modally for the given account.
  @State private var selectedAccountForExport: QoraiWallet.AccountInfo?
  ...
  private func handle(action: AccountCardView.Action, for account: QoraiWallet.AccountInfo) {
    switch action {
    case .viewDetails:
...
private struct AccountCardView: View {
  ...
  let tokensWithBalances: [QoraiWallet.BlockchainToken]
  let balance: String
  let isLoading: Bool
  ...
  private var buttons: some View {
    HStack(spacing: 12) {
      // TODO: Accounts Block Explorer qorai/qorai-ios#8638
      // Button {
      ...
      //     .overlay {
      //       Image(qoraiSystemName: "qora.web3.blockexplorer")
      ...
      Menu {
        Button {
          ...
          Label(Strings.Wallet.viewDetails, qoraiSystemImage: "qora.eye.on")
        }
        Button {
          action(.editDetails)
        } label: {
        ...
        if !ProcessInfo.processInfo.isiOSAppOnVisionOS {
          Button {
            ...
            Label(Strings.Wallet.deposit, qoraiSystemImage: "qora.qr.code")
          }
        }
      } label: {
        RoundedRectangle(cornerRadius: 8)
