...
struct AssetIconView: View {
  ...
  var meldCryptoCurrency: QoraiWallet.MeldCryptoCurrency?
...
struct StackedAssetIconsView: View {
  ...
  var bottomToken: QoraiWallet.BlockchainToken?
  var topToken: QoraiWallet.BlockchainToken?
  var network: QoraiWallet.NetworkInfo
...
struct GenericAssetIconView: View {
  ...
  init(
    backgroundColor: Color = Color(qoraiSystemName: .neutral20),
  ...
  var body: some View {
    Circle()
      ...
      .frame(width: min(length, maxLength ?? length), height: min(length, maxLength ?? length))
      .overlay {
        Image(qoraiSystemName: "qora.crypto.wallets")
          .resizable()
          .aspectRatio(contentMode: .fit)
          .padding(6)
