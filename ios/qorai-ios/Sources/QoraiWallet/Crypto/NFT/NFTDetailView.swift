...
struct NFTDetailView: View {
  ...
  var onNFTMetadataRefreshed: ((QoraiWallet.NftMetadata) -> Void)?
  ...
  var body: some View {
              ...
              if nftDetailStore.nft.isSpam {
                HStack(spacing: 4) {
                  ...
                  Image(qoraiSystemName: "qora.warning.triangle-outline")
                    .padding(.vertical, 4)
                    .padding(.trailing, 6)
                    .foregroundColor(Color(.qoraiErrorBorder))
                }
    ...
    .background(Color(UIColor.qoraiGroupedBackground).ignoresSafeArea())
        ...
        Menu {
          if nftDetailStore.nft.visible {
            Button {
              ...
              Label(Strings.Wallet.nftDetailSendNFTButtonTitle, qoraiSystemImage: "qora.send")
            }
          ...
          Button {
            ...
            if nftDetailStore.nft.visible {  // a collected visible NFT
              Label(Strings.recentSearchHide, qoraiSystemImage: "qora.eye.off")
            } else if nftDetailStore.nft.isSpam {  // a spam NFT
              Label(Strings.Wallet.nftUnspam, qoraiSystemImage: "qora.disable.outline")
            } else {  // a hidden but not spam NFT
              Label(Strings.Wallet.nftUnhide, qoraiSystemImage: "qora.eye.on")
            }
          }
          Button {
            isPresentingRemoveAlert = true
          } label: {
            Label(Strings.Wallet.nftRemoveFromWallet, qoraiSystemImage: "qora.trash")
          }
        } label: {
          Label(
            Strings.Wallet.otherWalletActionsAccessibilityTitle,
            qoraiSystemImage: "qora.more.horizontal"
          )
          .labelStyle(.iconOnly)
          .foregroundColor(Color(.qoraiBlurpleTint))
        }
        ...
        content: {
          VStack(spacing: 16) {
            ...
            Text(Strings.Wallet.nftRemoveFromWalletAlertDescription)
              .font(.footnote)
              .foregroundStyle(Color(.secondaryQoraiLabel))
          }
          .padding(.bottom, 28)
        }
      )
