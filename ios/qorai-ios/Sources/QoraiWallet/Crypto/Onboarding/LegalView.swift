...
struct LegalView: View {
  ...
  var body: some View {
          ...
          Text(
            LocalizedStringKey(
              String.localizedStringWithFormat(
                ...
                WalletConstants.qoraiWalletTermsOfUse.absoluteDisplayString
              )
            )
          )
          .foregroundColor(Color(uiColor: WalletV2Design.textPrimary))
    ...
    .background(Color(.qoraiBackground).edgesIgnoringSafeArea(.all))
...
struct LegalCheckbox: View {
  ...
  var body: some View {
    Button {
      ...
      Image(qoraiSystemName: isChecked ? "qora.checkbox.checked" : "qora.checkbox.unchecked")
        .renderingMode(.template)
        .foregroundColor(Color(isChecked ? .qoraiBlurpleTint : .qoraiDisabled))
    }
  }
}
