          ...
          Text("#\(word.index + 1) \(word.value)")
            ...
            .overlay(
              RoundedRectangle(cornerRadius: 4)
                .stroke(Color(.qoraiDisabled), lineWidth: 1)
            )
        }
        Button {
          copyRecoveryPhrase()
          ...
          if hasCopied {
            Text(
              "\(Strings.Wallet.copiedToPasteboard)  \(Image(qoraiSystemName: "qora.check.normal"))"
            )
            .font(.subheadline.bold())
            .foregroundColor(Color(.qoraiSuccessLabel))
          } else {
