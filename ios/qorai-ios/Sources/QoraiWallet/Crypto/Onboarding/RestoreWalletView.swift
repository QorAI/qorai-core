  ...
  private var errorLabel: some View {
    HStack(spacing: 12) {
      Image(qoraiSystemName: "qora.warning.circle-filled")
        ...
        .foregroundColor(Color(.qoraiLighterOrange))
        ...
        VStack(spacing: 20) {
          HStack {
            ...
            Button {
              ...
              Image(qoraiSystemName: isRevealRecoveryWords ? "qora.eye.off" : "qora.eye.on")
                .foregroundColor(Color(.qoraiLabel))
            }
          }
          if !isTwelvePhrase {
            Toggle(isOn: $isLegacyEthSeedFormat) {
              Text(Strings.Wallet.restoreLegacyQoraiWalletToggleLabel)
                .fontWeight(.medium)
                .foregroundColor(Color(.qoraiBlurpleTint))
            }
