...
struct AssetDetailView: View {
  ...
  @ViewBuilder private func coinMarketInfoView(_ coinMarket: QoraiWallet.CoinMarket) -> some View {
    VStack(spacing: 16) {
      ...
      let info: [CoinMarketInfo] = {
        let computedMarketCap =
          assetDetailStore.currencyFormatter.formatAsFiat(
            QoraiWallet.CoinMarket.abbreviateToBillion(input: coinMarket.marketCap)
          ) ?? ""
        let computedTotalVolume =
          assetDetailStore.currencyFormatter.formatAsFiat(
            QoraiWallet.CoinMarket.abbreviateToBillion(input: coinMarket.totalVolume)
          ) ?? ""
        return [
  ...
  var body: some View {
    ...
    .background(
      Color(qoraiSystemName: .containerBackground).edgesIgnoringSafeArea(.all)
    )
