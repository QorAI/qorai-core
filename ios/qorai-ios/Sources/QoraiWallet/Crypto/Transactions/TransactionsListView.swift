...
enum TransactionFollowUpAction {
  ...
  var qoraiSystemImage: String {
    ...
    case .cancel:
      return "qora.close"
    case .speedUp:
      return "qora.network.speed-fast"
    }
  }
}
...
struct TransactionsListView: View {
  ...
  let transactionTapped: (QoraiWallet.TransactionInfo) -> Void
  /// Called when retry transaction is tapped from context menu.
  let transactionFollowUpActionTapped:
    (TransactionFollowUpAction, QoraiWallet.TransactionInfo) -> Void
                          ...
                          if parsedTransaction.transaction.isCancelOrSpeedUpTransactionSupported {
                            Button {
                              ...
                              Label(
                                ...
                                qoraiSystemImage: TransactionFollowUpAction.cancel.qoraiSystemImage
                              )
                            }
                            Button {
                              transactionFollowUpActionTapped(
  ...
  private var searchBarAndFiltersContainer: some View {
    VStack(spacing: 0) {
      HStack(spacing: 10) {
        ...
        if showFilter {
          WalletIconButton(qoraiSystemName: "qora.filter.settings", action: filtersButtonTapped)
            .padding(.trailing, 8)
        }
      }
      .padding(.vertical, 8)
