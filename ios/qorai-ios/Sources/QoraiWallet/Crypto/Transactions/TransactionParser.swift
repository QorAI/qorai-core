...
enum TransactionParser {
  ...
  static func gasFee(
    from transaction: QoraiWallet.TransactionInfo,
  ...
  static func token(
    ...
    let findToken: (QoraiWallet.BlockchainToken) -> Bool = {
  ...
  static func parseTransaction(
    ...
    case .erc721TransferFrom, .erc721SafeTransferFrom:
      ...
      let tokenNFTMetadata: QoraiWallet.NftMetadata?
      if let token {
    ...
    case .solanaSplTokenTransfer,
      ...
      let fromToken: QoraiWallet.BlockchainToken? = token(
    ...
    case .solanaDappSignAndSendTransaction, .solanaDappSignTransaction, .solanaSwap:
      ...
      instructions.forEach { instruction in
        ...
        if instruction.isSystemProgram,
          let instructionType = QoraiWallet.SolanaSystemInstruction(
          ...
          )
        {
          ...
          case .transfer, .transferWithSeed:
            if let instructionLamports = instruction.decodedData?.paramFor(QoraiWallet.Lamports)?
              .value,
  ...
  static func parseSolanaInstruction(
    _ instruction: QoraiWallet.SolanaInstruction,
    decimalFormatStyle: WalletAmountFormatter.DecimalFormatStyle? = nil
    ...
    if instruction.isSystemProgram {
      ...
      if let lamportsParam = decodedData.paramFor(QoraiWallet.Lamports),
      ...
      if let amountParam = decodedData.paramFor(QoraiWallet.Amount),
        let decimalsParam = decodedData.paramFor(QoraiWallet.Decimals),
...
extension QoraiWallet.TransactionStatus: Equatable {}
...
struct SendDetails: Equatable {
  ...
  let fromTokenMetadata: QoraiWallet.NftMetadata?
...
struct EthSwapDetails: Equatable {
  ...
  let fromToken: QoraiWallet.BlockchainToken?
  /// From token network
  let fromNetwork: QoraiWallet.NetworkInfo
  /// From value prior to formatting
  let fromValue: String
  /// From amount formatted
  let fromAmount: String
  ...
  /// To token network
  let toNetwork: QoraiWallet.NetworkInfo
...
struct SolanaTxDetails: Equatable {
  struct ParsedSolanaInstruction: Equatable {
    ...
    let instruction: QoraiWallet.SolanaInstruction
...
struct FilSendDetails: Equatable {
  /// Token being sent
  let sendToken: QoraiWallet.BlockchainToken?
...
extension ParsedTransaction {
  ...
  var tokens: [QoraiWallet.BlockchainToken] {
    switch details {
    case .ethSend(let details),
      .erc20Transfer(let details),
      .solSystemTransfer(let details),
