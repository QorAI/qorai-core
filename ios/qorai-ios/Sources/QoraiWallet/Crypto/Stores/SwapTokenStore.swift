...
public class SwapTokenStore: ObservableObject, WalletObserverStore {
  ...
  @Published var allTokens: [QoraiWallet.BlockchainToken] = []
  /// The current selected token to swap from. Default with nil value.
  @Published var selectedFromToken: QoraiWallet.BlockchainToken? {
    didSet {
  ...
  @Published var selectedToToken: QoraiWallet.BlockchainToken? {
    didSet {
  ...
  struct CurrentSwapQuoteInfo {
    ...
    let swapQuote: QoraiWallet.SwapQuoteUnion?
    /// The swap fees for the current price quote
    let swapFees: QoraiWallet.SwapFees?
  }
  /// The details for the current swap quote on display
  @Published var currentSwapQuoteInfo: CurrentSwapQuoteInfo?

  /// If the Qorai Fee is voided for this swap.
  var isQoraiFeeVoided: Bool {
    guard let swapFees = currentSwapQuoteInfo?.swapFees else { return false }
    return swapFees.discountCode != .none && !swapFees.hasQoraiFee
  }

  /// The qorai fee percentage for this swap.
  /// When `isQoraiFeeVoided`, will return the fee being voided (so Free can be displayed beside % value voided)
  var qoraiFeeForDisplay: String? {
    ...
    if swapFees.discountCode == .none {
      ...
      if swapFees.hasQoraiFee {
        fee = swapFees.effectiveFeePct
      } else {
        // Display as `Free ~qoraiFee.feePct%~`
        fee = swapFees.feePct
      }
    }
  ...
  private var accountInfo: QoraiWallet.AccountInfo?
  ...
  private func swapQuoteParameters(
    ...
    in network: QoraiWallet.NetworkInfo
    ...
    let swapQuoteParams = QoraiWallet.SwapQuoteParams(
      ...
      // https://github.com/qorai/qorai-browser/issues/39043
  ...
  @MainActor private func createEthSwapTransaction() async -> Bool {
    ...
    let params = QoraiWallet.NewEvmTransactionParams(
  ...
  @MainActor private func fetchEthPriceQuote(
    base: SwapParamsBase,
    swapQuoteParams: QoraiWallet.SwapQuoteParams
  ) async {
  ...
  @MainActor private func handleSwapQuote(
    base: SwapParamsBase,
    swapQuoteUnion: QoraiWallet.SwapQuoteUnion
  ) async {
  ...
  @MainActor private func handleZeroExQuote(
    base: SwapParamsBase,
    zeroExQuote: QoraiWallet.ZeroExQuote
  ) async {
    guard !Task.isCancelled else { return }
    let walletAmountFormatter = WalletAmountFormatter(decimalFormatStyle: .decimals(precision: 18))
  ...
  @MainActor private func handleJupiterQuote(
    _ jupiterQuote: QoraiWallet.JupiterQuote
  ) async {
    guard !Task.isCancelled else { return }
  ...
  @MainActor private func handleLifiQuote(
    base: SwapParamsBase,
    _ lifiQuote: QoraiWallet.LiFiQuote
  ) async {
    guard !Task.isCancelled else { return }
  ...
  @MainActor private func handleSwapQuoteError(_ swapError: QoraiWallet.SwapErrorUnion) async {
  ...
  @MainActor private func createSolSwapTransaction() async -> Bool {
    ...
    let jupiterTransactionParams: QoraiWallet.JupiterTransactionParams = .init(
      quote: jupiterQuote,
      chainId: network.chainId,
  ...
  func prepare(with accountInfo: QoraiWallet.AccountInfo, completion: (() -> Void)? = nil) {
    self.accountInfo = accountInfo

    func updateSelectedTokens(in network: QoraiWallet.NetworkInfo) {
      ...
      if let toToken = selectedToToken {
        ...
        if network.chainId == QoraiWallet.MainnetChainId {
          ...
          } else {  // if QOR is unavailable
            selectedToToken = allTokens.first(where: {
              $0.symbol.uppercased() != selectedFromToken?.symbol.uppercased()
            })
          }
        } else if network.chainId == QoraiWallet.SolanaMainnet {
          if let fromToken = selectedFromToken,
    ...
    self.rpcService.network(coin: accountInfo.coin, origin: nil) { network in
      ...
      let continueClosure: (QoraiWallet.NetworkInfo) -> Void = { [weak self] network in
  ...
  #if DEBUG
  func setUpTest(
    fromAccount: QoraiWallet.AccountInfo = .mockEthAccount,
    selectedFromToken: QoraiWallet.BlockchainToken = .previewToken,
    selectedToToken: QoraiWallet.BlockchainToken = .previewDaiToken,
