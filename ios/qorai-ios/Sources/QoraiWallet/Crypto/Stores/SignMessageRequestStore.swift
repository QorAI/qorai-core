...
class SignMessageRequestStore: ObservableObject {
  ...
  @Published var requests: [QoraiWallet.SignMessageRequest] {
    didSet {
  ...
  var currentRequest: QoraiWallet.SignMessageRequest {
  ...
  init(
    requests: [QoraiWallet.SignMessageRequest],
  ...
  func update() {
    ...
    self.updateTask = Task { @MainActor in
      ...
      let cowSwapRequests: [(id: Int32, cowSwapOrder: QoraiWallet.CowSwapOrder, chainId: String)] =
        self.requests
      ...
      let findToken: (String, String) async -> QoraiWallet.BlockchainToken? = {
      ...
      for cowSwapRequest in cowSwapRequests {
        ...
        let fromToken: QoraiWallet.BlockchainToken? = await findToken(
        ...
        let toToken: QoraiWallet.BlockchainToken? = await findToken(cowSwapOrder.buyToken, chainId)
