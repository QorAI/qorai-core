...
///
/// This wraps a KeyringService that you would obtain through QoraiCore and makes it observable
public class KeyringStore: ObservableObject, WalletObserverStore {
  ...
  @Published var selectedAccount: QoraiWallet.AccountInfo = .init() {
    didSet {
  ...
  @Published var allAccounts: [QoraiWallet.AccountInfo] = []

  /// A list of default account with all support coin types
  @Published var defaultAccounts: [QoraiWallet.AccountInfo] = []
  ...
  @MainActor func updateInfo() async {
    ...
    // this can occur if we crash prior to saving, ex. qorai-ios #8291
  ...
  private func setSelectedAccount(to account: QoraiWallet.AccountInfo) {
    Task { @MainActor in
  ...
  func reportP3AOnboarding(action: QoraiWallet.OnboardingAction) {
  ...
  @MainActor func addPrimaryAccount(
    ...
    let accountInfo = await keyringService.addAccount(
      coin: coin,
      keyringId: QoraiWallet.KeyringId.keyringId(for: coin, on: chainId),
  ...
  @MainActor func addSecondaryAccount(
    ...
    let accountInfo: QoraiWallet.AccountInfo?
    switch coin {
    case .eth:
  ...
  private func defaultAccountName(
    for coin: QoraiWallet.CoinType,
    chainId: String
  ) -> String {
    let keyringId: QoraiWallet.KeyringId = .keyringId(for: coin, on: chainId)
  ...
  func renameAccount(
    _ account: QoraiWallet.AccountInfo,
    name: String,
    completion: ((Bool) -> Void)? = nil
  ) {
  ...
  static let passwordKeychainKey = "qorai-wallet-password"
  ...
  func storePasswordInKeychain(_ password: String) -> OSStatus {
    ...
    if osStatus == errSecDuplicateItem {
      // Duplicate item is possible if qorai-browser#36669 occurs again.
