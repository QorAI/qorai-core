...
enum PendingRequest: Equatable {
  case transactions([QoraiWallet.TransactionInfo])
  case addChain(QoraiWallet.AddChainRequest)
  case switchChain(QoraiWallet.SwitchChainRequest)
  case addSuggestedToken(QoraiWallet.AddSuggestTokenRequest)
  case signMessage([QoraiWallet.SignMessageRequest])
  case signMessageError([QoraiWallet.SignMessageError])
  case getEncryptionPublicKey(QoraiWallet.GetEncryptionPublicKeyRequest)
  case decrypt(QoraiWallet.DecryptRequest)
  case signSolTransactions([QoraiWallet.SignSolTransactionsRequest])
}
...
enum WebpageRequestResponse: Equatable {
  ...
  case addSuggestedToken(approved: Bool, token: QoraiWallet.BlockchainToken)
...
public class CryptoStore: ObservableObject, WalletObserverStore {
  ...
  /// The current selected tab of Qorai Wallet with initial value `.portfolio`
  ...
  let blockchainRegistry: QoraiWalletBlockchainRegistry
  private let txService: QoraiWalletTxService
  private let ethTxManagerProxy: QoraiWalletEthTxManagerProxy
  private let solTxManagerProxy: QoraiWalletSolanaTxManagerProxy
  private let ipfsApi: IpfsAPI
  private let walletP3A: QoraiWalletQoraiWalletP3A
  ...
  private var autoDiscoveredAssets: [QoraiWallet.BlockchainToken] = []
  ...
  public init(
    ...
    ethTxManagerProxy: QoraiWalletEthTxManagerProxy,
  ...
  public func setupObservers() {
    ...
    self.walletServiceObserver = WalletServiceObserver(
      ...
      _onDiscoverAssetsCompleted: { [weak self] discoveredAssets in
        // Failsafe incase two CryptoStore's are initialized (see qorai-ios #7804) and asset
  ...
  func openBuyTokenStore(_ prefilledToken: QoraiWallet.BlockchainToken?) -> BuyTokenStore {
  ...
  func openSendTokenStore(_ prefilledToken: QoraiWallet.BlockchainToken?) -> SendTokenStore {
  ...
  func openSwapTokenStore(_ prefilledToken: QoraiWallet.BlockchainToken?) -> SwapTokenStore {
  ...
  func openDepositTokenStore(
    ...
    prefilledAccount: QoraiWallet.AccountInfo?
  ) -> DepositTokenStore {
  ...
  func accountActivityStore(
    for account: QoraiWallet.AccountInfo,
    isWalletPanel: Bool
  ...
  func closeAccountActivityStore(for account: QoraiWallet.AccountInfo) {
  ...
  func nftDetailStore(
    for nft: QoraiWallet.BlockchainToken,
    nftMetadata: QoraiWallet.NftMetadata?,
    owner: QoraiWallet.AccountInfo?
  ) -> NFTDetailStore {
  ...
  func closeNFTDetailStore(for nft: QoraiWallet.BlockchainToken) {
    if let store = nftDetailStore, store.nft.id == nft.id {
      nftDetailStore?.tearDown()
      nftDetailStore = nil
    }
  ...
  func signMessageRequestStore(
    for requests: [QoraiWallet.SignMessageRequest]
  ...
  @MainActor
  func fetchPendingTransactions() async -> [QoraiWallet.TransactionInfo] {
    let allAccounts = await keyringService.allAccounts().accounts
    var allNetworksForCoin: [QoraiWallet.CoinType: [QoraiWallet.NetworkInfo]] = [:]
  ...
  private func recordP3AActiveWallets() {
    Task { @MainActor in
      let shouldCountTestNetworks = Preferences.QoraiCore.activeSwitches.value.contains(
        QoraiWallet.P3aCountTestNetworksSwitch
      )
      let allAccounts = await keyringService.allAccounts().accounts
