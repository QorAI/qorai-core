...
class SelectAccountTokenStore: ObservableObject, WalletObserverStore {
  ...
  struct AccountSection: Equatable, Identifiable {
    ...
    let bitcoinAccountInfo: QoraiWallet.BitcoinAccountInfo?
    let zcashAccountInfo: QoraiWallet.ZCashAccountInfo?
  ...
  private var pricesForTokensCache: [QoraiWallet.AssetPrice] = []
  /// Cache of metadata for NFTs. The key(s) is the token's `id`.
  private var metadataCache: [String: QoraiWallet.NftMetadata] = [:]

  let didSelect: (QoraiWallet.AccountInfo, QoraiWallet.BlockchainToken) -> Void
  ...
  init(
    didSelect: @escaping (QoraiWallet.AccountInfo, QoraiWallet.BlockchainToken) -> Void,
  ...
  private var bitcoinAccounts: [String: QoraiWallet.BitcoinAccountInfo] = [:]
  /// All` ZCashAccountInfo` models for every Zcash account. Key is `accountId.uniqueKey` of the Account.
  private var zcashAccounts: [String: QoraiWallet.ZCashAccountInfo] = [:]
  // All user visible assets, key is `Identifiable.id` of `BlockchainToken`.
  private var userVisibleAssets: [String: QoraiWallet.BlockchainToken] = [:]
  // All user accounts.
  ...
  func setup() {
    Task { @MainActor in
      ...
      self.userVisibleAssets = allVisibleUserAssets.reduce(
        into: [String: QoraiWallet.BlockchainToken](),
        {
          $0[$1.id] = $1
        }
      )
  ...
  func fetchTokenPrices(for tokens: [QoraiWallet.BlockchainToken]) {
    guard !tokens.isEmpty else { return }
    Task { @MainActor in
      self.isLoadingPrices = true
      defer { self.isLoadingPrices = false }
      let prices: [QoraiWallet.AssetPrice] = await assetRatioService.fetchPrices(
        for: tokens,
  ...
  func fetchNFTMetadata(for userVisibleNFTs: [QoraiWallet.BlockchainToken]) {
  ...
  private func buildAccountSections(
    selectedNetworks: [QoraiWallet.NetworkInfo],
    allAccounts: [QoraiWallet.AccountInfo],
    bitcoinAccounts: [String: QoraiWallet.BitcoinAccountInfo],
    zcashAccounts: [String: QoraiWallet.ZCashAccountInfo],
    userVisibleAssets: [QoraiWallet.BlockchainToken],
    balancesCache: TokenBalanceCache,
    btcBalancesCache: [String: [BTCBalanceType: Double]],
    balancesFetched: Bool,
    pricesCache: [QoraiWallet.AssetPrice],
    metadataCache: [String: QoraiWallet.NftMetadata],
    hideZeroBalances: Bool,
    query: String,
    currencyFormatter: NumberFormatter
  ) async -> [AccountSection] {
    let accountSections: [AccountSection] = allAccounts.compactMap { account in
      let tokensForAccountCoin: [QoraiWallet.BlockchainToken] =
        userVisibleAssets
