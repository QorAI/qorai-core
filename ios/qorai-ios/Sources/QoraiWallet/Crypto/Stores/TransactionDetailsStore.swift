...
class TransactionDetailsStore: ObservableObject, WalletObserverStore {
  ...
  @Published private(set) var transaction: QoraiWallet.TransactionInfo
  ...
  private let solanaTxManagerProxy: QoraiWalletSolanaTxManagerProxy
  private let ipfsApi: IpfsAPI
  private let assetManager: WalletUserAssetManagerType
  /// Cache for storing `BlockchainToken`s that are not in user assets or our token registry.
  /// This could occur with a dapp creating a transaction.
  private var tokenInfoCache: [QoraiWallet.BlockchainToken] = []
  private var nftMetadataCache: [String: QoraiWallet.NftMetadata] = [:]
  private var solEstimatedTxFeesCache: [String: UInt64] = [:]
  private var assetRatiosCache: [QoraiWallet.AssetPrice] = []
  ...
  init(
    ...
    solanaTxManagerProxy: QoraiWalletSolanaTxManagerProxy,
    ipfsApi: IpfsAPI,
  ...
  func updateTransaction(_ transaction: QoraiWallet.TransactionInfo) {
  ...
  func update() {
    Task { @MainActor in
      ...
      let allTokens: [QoraiWallet.BlockchainToken] = await blockchainRegistry.allTokens(
        chainId: network.chainId,
        coin: network.coin
      )
      let userAssets: [QoraiWallet.BlockchainToken] =
      ...
      let nftToken: QoraiWallet.BlockchainToken?
