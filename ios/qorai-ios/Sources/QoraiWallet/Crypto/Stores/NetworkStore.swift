...
///
/// This wraps a JsonRpcService that you would obtain through QoraiCore and makes it observable
public class NetworkStore: ObservableObject, WalletObserverStore {
  ...
  @Published private(set) var allChains: [QoraiWallet.NetworkInfo] = []
  @Published private(set) var customChains: [QoraiWallet.NetworkInfo] = []
  @Published private(set) var hiddenChains: [QoraiWallet.NetworkInfo] = []

  @Published private(set) var defaultSelectedChainId: String = QoraiWallet.MainnetChainId
  var defaultSelectedChain: QoraiWallet.NetworkInfo {
  ...
  @Published private(set) var selectedChainIdForOrigin: String = QoraiWallet.MainnetChainId
  var selectedChainForOrigin: QoraiWallet.NetworkInfo {
  ...
  @Published var defaultNetworks: [QoraiWallet.CoinType: QoraiWallet.NetworkInfo] = [:]

  var visibleChains: [QoraiWallet.NetworkInfo] {
  ...
  private func unhideAllSupportedTestnets() async {
    var networks: [QoraiWallet.CoinType: [String]] = [:]
    for chainId in WalletConstants.supportedTestNetworkChainIds {
      var coin: QoraiWallet.CoinType = .eth
      switch chainId {
      case QoraiWallet.SepoliaChainId,
        QoraiWallet.FilecoinEthereumTestnetChainId:
        coin = .eth
      case QoraiWallet.SolanaDevnet,
        QoraiWallet.SolanaTestnet:
        coin = .sol
      case QoraiWallet.FilecoinTestnet:
        coin = .fil
      case QoraiWallet.BitcoinTestnet:
        coin = .btc
      default:
        break
      }
  ...
  func isCustomChain(_ network: QoraiWallet.NetworkInfo) -> Bool {
    customChains.contains(where: { $0.chainId == network.chainId })
  }

  func isHiddenChain(_ network: QoraiWallet.NetworkInfo) -> Bool {
  ...
  func subNetworks(for network: QoraiWallet.NetworkInfo) -> [QoraiWallet.NetworkInfo] {
  ...
  @MainActor func selectedNetwork(for coin: QoraiWallet.CoinType) async -> QoraiWallet.NetworkInfo {
  ...
  func network(for chainId: String) -> QoraiWallet.NetworkInfo? {
  ...
  public func preferencesDidChange(for key: String) {
    ...
    Task { @MainActor in
      if key == Preferences.Wallet.isBitcoinTestnetEnabled.key {
        ...
        if !isBitcoinTestnetEnabled, btcDefaultNetwork.chainId == QoraiWallet.BitcoinTestnet {
          ...
          if hiddenChains.contains(where: { $0.chainId == QoraiWallet.BitcoinMainnet }) {
        ...
        if !isZcashTestnetEnabled, zcashDefaultNetwork.chainId == QoraiWallet.ZCashTestnet {
          // zcash testnet is disabled but the zcash coin type
          // default network is testnet. Need to change it to zcash
          // mainnet, to prevent user hide the default network
          if hiddenChains.contains(where: { $0.chainId == QoraiWallet.ZCashMainnet }) {
...
extension Array where Element == QoraiWallet.NetworkInfo {
  /// Returns the primary networks in Self.
  var primaryNetworks: [QoraiWallet.NetworkInfo] {
    filter { WalletConstants.primaryNetworkChainIds.contains($0.chainId) }
  }

  /// Returns the secondary networks in Self.
  var secondaryNetworks: [QoraiWallet.NetworkInfo] {
    filter {
  ...
  var testNetworks: [QoraiWallet.NetworkInfo] {
    filter {
      if !Preferences.Wallet.isBitcoinTestnetEnabled.value
        && $0.chainId == QoraiWallet.BitcoinTestnet
      {
        return false
      }
      if !Preferences.Wallet.isZcashTestnetEnabled.value
        && $0.chainId == QoraiWallet.ZCashTestnet
      {
        return false
      }
