...
public class SendTokenStore: ObservableObject, WalletObserverStore {
  ...
  @Published var userVisibleAssets: [QoraiWallet.BlockchainToken] = []
  /// The current selected token to send. Default with nil value.
  @Published var selectedSendToken: QoraiWallet.BlockchainToken? {
    didSet {
  ...
  @Published var selectedSendNFTMetadata: QoraiWallet.NftMetadata?
    ...
    var errorDescription: String? {
      ...
      case .snsError:
        return String.localizedStringWithFormat(
          ...
          QoraiWallet.CoinType.sol.localizedTitle
        )
      case .ensError:
        return String.localizedStringWithFormat(
          Strings.Wallet.sendErrorDomainNotRegistered,
          QoraiWallet.CoinType.eth.localizedTitle
        )
      case .udError:
        return String.localizedStringWithFormat(
          Strings.Wallet.sendErrorDomainNotRegistered,
  ...
  private var metadataCache: [String: QoraiWallet.NftMetadata] = [:]
  ...
  func didSelect(account: QoraiWallet.AccountInfo, token: QoraiWallet.BlockchainToken) {
    Task { @MainActor in
  ...
  @MainActor private func validatePrefilledToken(on network: inout QoraiWallet.NetworkInfo) async {
  ...
  private func validateBitcoinSendAddress(fromAccount: QoraiWallet.AccountInfo) {
    guard let selectedSendToken else { return }
    let isMainnet = selectedSendToken.chainId == QoraiWallet.BitcoinMainnet
  ...
  func sendTokenOnEth(
    ...
    from fromAccountInfo: QoraiWallet.AccountInfo,
    completion: @escaping (_ success: Bool, _ errMsg: String?) -> Void
  ) {
    isMakingTx = true
    ...
    rpcService.network(coin: .sol, origin: nil) { [weak self] network in
      ...
      if network.isNativeAsset(token) {
        self.solTxManagerProxy.makeSystemProgramTransferTxData(
          ...
          let txDataUnion = QoraiWallet.TxDataUnion(solanaTxData: solanaTxData)
  ...
  private func handleSolSPLTx(
    ...
    fromAccountInfo: QoraiWallet.AccountInfo,
    solTxData: QoraiWallet.SolanaTxData,
    error: QoraiWallet.SolanaProviderError,
    errMsg: String,
    completion: @escaping (_ success: Bool, _ errMsg: String?) -> Void
  ) {
    let txDataUnion = QoraiWallet.TxDataUnion(solanaTxData: solTxData)
    self.txService.addUnapprovedTransaction(
      txDataUnion: txDataUnion,
      chainId: network.chainId,
      from: fromAccountInfo.accountId
  ...
  private func sendTokenOnFil(
    amount: String,
    token: QoraiWallet.BlockchainToken,
    from fromAccountId: QoraiWallet.AccountId,
    completion: @escaping (_ success: Bool, _ errMsg: String?) -> Void
  ) {
    isMakingTx = true
    ...
    rpcService.network(coin: .fil, origin: nil) { [weak self] network in
      guard let self = self else { return }
      let filTxData = QoraiWallet.FilTxData(
        nonce: "",
        gasPremium: "",
        gasFeeCap: "",
        gasLimit: "",
      ...
      self.txService.addUnapprovedTransaction(
        txDataUnion: QoraiWallet.TxDataUnion(filTxData: filTxData),
        chainId: network.chainId,
        from: fromAccountId
      ) { success, txMetaId, errorMessage in
        self.isMakingTx = false
  ...
  private func sendTokenOnBtc(
    ...
    let params: QoraiWallet.NewBitcoinTransactionParams = .init(
  ...
  private func sendTokenOnZec(
    ...
    let zecTxData: QoraiWallet.ZecTxData = .init(
      useShieldedPool: false,
      to: sendAddress,
      sendingMaxAmount: isSendingMaxValue,
      memo: nil,
  ...
  ) async -> [String: QoraiWallet.NftMetadata] {
