...
public enum AssetGroupType: Equatable, Identifiable {
  case none
  case network(QoraiWallet.NetworkInfo)
  case account(QoraiWallet.AccountInfo)
...
public struct AssetViewModel: Identifiable, Equatable {
  ...
  let price: String
  let history: [QoraiWallet.AssetTimePrice]
...
public class PortfolioStore: ObservableObject, WalletObserverStore {
  ...
  var allNetworks: [QoraiWallet.NetworkInfo] = []
  var filters: Filters {
  ...
  private var pricesCache: [QoraiWallet.AssetPrice] = []
  /// Cache of priceHistories. The key is the token's `assetRatioId`.
  private var priceHistoriesCache: [String: [QoraiWallet.AssetTimePrice]] = [:]
  ...
  func update() {
    ...
    self.updateTask = Task { @MainActor in
      ...
      typealias TokenNetworkAccounts = (
        token: QoraiWallet.BlockchainToken, network: QoraiWallet.NetworkInfo,
        ...
        networkAssets.tokens.map { token in
          TokenNetworkAccounts(
            ...
            accounts: selectedAccounts.filter {
              ...
              case .fil, .btc, .zec:
                return $0.keyringId
                  == QoraiWallet.KeyringId.keyringId(
                    for: token.coin,
                    on: token.chainId
                  )
              default:
      ...
      let priceHistories: [String: [QoraiWallet.AssetTimePrice]] = await fetchPriceHistory(
  ...
  private func buildAssetViewModels(
    ...
    case .none:
      return allVisibleUserAssets.flatMap { networkAssets in
        networkAssets.tokens.map { token in
          ...
          if token.coin == .btc {
            let keyringIdForToken = QoraiWallet.KeyringId.keyringId(for: .btc, on: token.chainId)
    ...
    case .network(let network):
      ...
      if network.coin == .btc {
        let keyringIdForNetwork = QoraiWallet.KeyringId.keyringId(for: .btc, on: network.chainId)
        let accountIdsForToken =
          accounts
  ...
  @MainActor func fetchPriceHistory(
    for priceIds: [String]
  ) async -> [String: [QoraiWallet.AssetTimePrice]] {
    let uniquePriceIds = Set(priceIds)
    let priceHistories = await withTaskGroup(of: [String: [QoraiWallet.AssetTimePrice]].self) {
      @MainActor group -> [String: [QoraiWallet.AssetTimePrice]] in
      ...
      return await group.reduce(
        into: [String: [QoraiWallet.AssetTimePrice]](),
        { partialResult, new in
          for key in new.keys {
