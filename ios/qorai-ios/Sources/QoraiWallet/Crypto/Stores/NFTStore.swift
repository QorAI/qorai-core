...
struct NFTAssetViewModel: Identifiable, Equatable {
  ...
  var nftMetadata: QoraiWallet.NftMetadata?
...
public class NFTStore: ObservableObject, WalletObserverStore {
  ...
  /// We record NFT gallery views with NFT count to `QoraiWalletP3A`, and we also need to
  ...
  func updateNFTMetadataCache(
    ...
    metadata: QoraiWallet.NftMetadata
  ) {
  ...
  @MainActor private func computeSpamNFTs(
    ...
    for (index, network) in selectedNetworks.enumerated() {
      let userMarkedSpamNFTsOnNetwork: [QoraiWallet.BlockchainToken] =
      ...
      var spamNFTUnion: [QoraiWallet.BlockchainToken] = userMarkedSpamNFTsOnNetwork
  ...
  @MainActor private func buildNFTGroupModels(
    ...
    selectedAccounts: [QoraiWallet.AccountInfo],
    selectedNetworks: [QoraiWallet.NetworkInfo]
  ) async -> ([NFTGroupViewModel], [QoraiWallet.BlockchainToken]) {
    // user visible NFTs
    let userVisibleNFTs =
  ...
  func removeNFT(_ token: QoraiWallet.BlockchainToken) {
    Task { @MainActor in
  ...
  func owner(for nft: QoraiWallet.BlockchainToken) -> QoraiWallet.AccountInfo? {
