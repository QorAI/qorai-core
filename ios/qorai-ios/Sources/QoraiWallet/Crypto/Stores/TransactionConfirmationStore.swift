...
public class TransactionConfirmationStore: ObservableObject, WalletObserverStore {
  ...
  @Published var eip1559GasEstimation: QoraiWallet.GasEstimation1559?
  ...
  @Published var originInfo: QoraiWallet.OriginInfo?
  /// This is an id for the unppproved transaction that is currently displayed on screen
  @Published var activeTransactionId: QoraiWallet.TransactionInfo.ID = "" {
    didSet {
  ...
  @Published var activeTxStatus: QoraiWallet.TransactionStatus = .unapproved {
    didSet {
  ...
  @Published var allTxs: [QoraiWallet.TransactionInfo] = []
  ...
  var unapprovedTxs: [QoraiWallet.TransactionInfo] {
  ...
  private var activeTransaction: QoraiWallet.TransactionInfo {
  ...
  private var selectedChain: QoraiWallet.NetworkInfo = .init()
  ...
  @MainActor func prepare() async {
    ...
    let transactionNetworks: [QoraiWallet.NetworkInfo] = Set(allTxs.map(\.chainId))
  ...
  func updateTransaction(
    with transaction: QoraiWallet.TransactionInfo,
    ...
    Task { @MainActor in
      ...
      guard let network = allNetworks.first(where: { $0.chainId == transaction.chainId }) else {
        // default `QoraiWallet.TransactionInfo()` has empty chainId
        if !transaction.chainId.isEmpty {
          // Transactions should be removed if their network is removed
          // https://github.com/qorai/qorai-browser/issues/30234
          assertionFailure(
  ...
  @MainActor func parseTransaction(
    _ txInfo: QoraiWallet.TransactionInfo
  ...
  private var assetRatios: [QoraiWallet.AssetPrice] = []
  ...
  @MainActor private func fetchAssetRatios(
    for userVisibleTokens: [QoraiWallet.BlockchainToken]
  ) async {
  ...
  @MainActor private func fetchUnknownTokens(
    for transactions: [QoraiWallet.TransactionInfo]
  ) async {
    ...
    let coin: QoraiWallet.CoinType = .eth
  ...
  @MainActor private func fetchAllTransactions() async -> [QoraiWallet.TransactionInfo] {
  ...
  @MainActor func reject(transaction: QoraiWallet.TransactionInfo) async -> Bool {
  ...
  func onUnapprovedTxUpdated(_ txInfo: QoraiWallet.TransactionInfo) {
  ...
  func onTransactionStatusChanged(_ txInfo: QoraiWallet.TransactionInfo) {
    // first update `allTxs` with the new updated txInfo(txStatus)
    if let index = allTxs.firstIndex(where: { $0.id == txInfo.id }) {
      allTxs[index] = txInfo
    }
  ...
  func onUnapprovedTxAdded(_ txInfo: QoraiWallet.TransactionInfo) {
    Task { @MainActor in
