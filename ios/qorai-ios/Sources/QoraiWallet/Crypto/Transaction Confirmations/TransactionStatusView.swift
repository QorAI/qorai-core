...
struct TransactionStatusView: View {
  ...
  @ViewBuilder private var statusIcon: some View {
    ...
    case .submitted:
      if isTxERC20Approve {
        Image(qoraiSystemName: "qora.lock.open")
      } else if isTxEthSwap || isTxSolSwap {
        Image(qoraiSystemName: "qora.swap.horizontal")
      } else {
        Image(qoraiSystemName: "qora.send.filled")
      }
    case .signed:
      Image(qoraiSystemName: "qora.arrow.up")
    case .confirmed:
  ...
  @ViewBuilder private var explorerButton: some View {
    Button {
      ...
      if txStatusStore.activeTxStatus == .signed {
        ...
        Image(qoraiSystemName: "qora.arrow.diagonal-up-right")
          .font(.subheadline)
          .foregroundColor(Color(qoraiSystemName: .iconInteractive))
      }
    }
  ...
  @ViewBuilder private var txStatusIconView: some View {
    ...
    case .submitted:
      ZStack {
        Group {
          Circle()
            .stroke(
              Color(qoraiSystemName: .primary20),
              lineWidth: 6
            )
          Circle()
            .trim(from: 0, to: 0.25)
            .stroke(
              Color(qoraiSystemName: .primary40),
        ...
        statusIcon
          .font(.title)
          .foregroundColor(Color(qoraiSystemName: .primary40))
      }
      .onAppear {
        isSpinning = true
      }
    case .signed:
      ZStack {
        Circle()
          .stroke(Color(qoraiSystemName: .primary40), lineWidth: 6)
          .frame(width: 100, height: 100)
        statusIcon
          .font(.title)
          .foregroundColor(Color(qoraiSystemName: .primary40))
    ...
    case .error:
      ZStack {
        Circle()
          .fill(Color(qoraiSystemName: .systemfeedbackErrorBackground))
          .frame(width: 100, height: 100)
        statusIcon
    ...
    case .confirmed:
      ZStack {
        Group {
          Circle()
            .stroke(Color(qoraiSystemName: .green20), lineWidth: 6)
          Circle()
            .trim(from: 0, to: 0.25)
            .stroke(
              Color(qoraiSystemName: .green40),
              style: StrokeStyle(lineWidth: 6, lineCap: .round)
            )
        ...
        statusIcon
          .font(.title)
          .foregroundColor(Color(qoraiSystemName: .green30))
          .background(
            Color(qoraiSystemName: .green10)
              .clipShape(Circle())
              .frame(width: 100, height: 100)
          )
          .hidden(isHidden: !isConfirmSpinningFinished)
  ...
  private var activeTxNetwork: QoraiWallet.NetworkInfo? {
  ...
  private var speedUpView: some View {
    HStack(spacing: 4) {
      Text(Strings.Wallet.speedUpBannerTitle)
        .foregroundColor(Color(qoraiSystemName: .systemfeedbackInfoText))
        .font(.subheadline)
      ...
      .overlay {
        RoundedRectangle(cornerRadius: 8)
          .stroke(Color(qoraiSystemName: .dividerInteractive), lineWidth: pixelLength)
      }
    }
    .padding(16)
    .background(
      Color(qoraiSystemName: .systemfeedbackInfoBackground)
  ...
  var body: some View {
    NavigationStack {
      ...
      .background(Color(qoraiSystemName: .containerBackground).ignoresSafeArea())
