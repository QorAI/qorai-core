...
struct SaferSignTransactionView: View {
        ...
        AddressView(address: fromAddress ?? "") {
          ...
          .overlay(
            RoundedRectangle(cornerSize: CGSize(width: 4, height: 4))
              .stroke(Color(.qoraiSeparator), lineWidth: pixelLength)
          )
        }
      }
      TokenRow(
  ...
  private var arrowView: some View {
    Circle()
      .stroke(Color(.qoraiSeparator), lineWidth: pixelLength)
      .background(Color(.secondaryQoraiGroupedBackground))
  ...
  private var tokensView: some View {
    ...
    .background(Color(.secondaryQoraiGroupedBackground).cornerRadius(10))
  }
}
  ...
  var body: some View {
    HStack {
      if let token = token, let network = network {
        ...
        Circle()
          .stroke(Color(.qoraiSeparator))
          .frame(width: assetIconSize, height: assetIconSize)
          .background(Color(qoraiSystemName: .iconInteractive).clipShape(Circle()))
      }
...
struct SaferSignTransactionView_Previews: PreviewProvider {
  static var transaction: QoraiWallet.TransactionInfo {
    let transaction: QoraiWallet.TransactionInfo = .init()
  ...
  static var previews: some View {
    let parsedTransaction: ParsedTransaction = .init(
      ...
      fromAccountInfo: QoraiWallet.AccountInfo.previewAccount,
      ...
      ForEach(ColorScheme.allCases, id: \.self) { colorScheme in
        ScrollView {
          SaferSignTransactionView(
            ...
            fromToken: .mockUSDCToken,
            fromTokenContractAddress: QoraiWallet.BlockchainToken.mockUSDCToken.contractAddress,
            fromAmount: "1",
            toToken: .previewDaiToken,
            toTokenContractAddress: QoraiWallet.BlockchainToken.previewDaiToken.contractAddress,
            minBuyAmount: "0.994798"
          )
        }
