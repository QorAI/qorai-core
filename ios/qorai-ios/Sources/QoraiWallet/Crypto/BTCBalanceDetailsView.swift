...
struct UnavailableBTCBalanceView: View {
  ...
  // need to pass details to root for presentation qorai-ios#8709.
  ...
  var body: some View {
    Button(
      ...
      label: {
        HStack(spacing: 16) {
          ProgressView()
            .progressViewStyle(.qoraiCircular(size: .small, tint: .qoraiBlurpleTint))
          Text(Strings.Wallet.btcPendingBalancesBannerDesc)
            .font(.footnote)
            .multilineTextAlignment(.leading)
            .foregroundColor(Color(qoraiSystemName: .blue50))
          Spacer(minLength: 16)
        ...
        .background(Color(qoraiSystemName: .blue20).cornerRadius(12))
      }
    )
  }
}
    ...
    VStack(spacing: 16) {
      ...
      .overlay {
        ContainerRelativeShape()
          .strokeBorder(Color(qoraiSystemName: .dividerSubtle))
      }
