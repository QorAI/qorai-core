...
public class AIChatViewModel: NSObject, ObservableObject {
  ...
  private let qoraiTalkScript: AIChatQoraiTalkJavascript?
  ...
  public var isCurrentModelPremium: Bool {
    ...
    return currentModel.options.tag == .qoraModelOptions
      && currentModel.options.qoraModelOptions?.access == .premium
  }
  ...
  public init(
    ...
    qoraiTalkScript: AIChatQoraiTalkJavascript?,
    querySubmited: String? = nil
  ) {
    self.webDelegate = webDelegate
    self.qoraiTalkScript = qoraiTalkScript
    self.querySubmited = querySubmited

    super.init()

    // Initialize
    api = qoraiCore.aiChatAPI(with: self)
  }
...
extension AIChatViewModel: AIChatDelegate {
  ...
  public func pageContent() async -> (String?, Bool) {
    ...
    if let transcript = await qoraiTalkScript?.getTranscript() {
