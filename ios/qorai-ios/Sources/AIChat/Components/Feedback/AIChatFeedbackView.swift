    ...
    Button {
      ...
      .overlay(
        ContainerRelativeShape()
          .strokeBorder(Color(qoraiSystemName: .iconDefault), lineWidth: 0.5)
      )
      .background(
  ...
  var body: some View {
    VStack {
      HStack(spacing: 0.0) {
        ...
        Text(verbatim: "*")
          .font(.caption.weight(.semibold))
          .foregroundStyle(Color(qoraiSystemName: .systemfeedbackErrorText))
      }
      .frame(maxWidth: .infinity, alignment: .leading)
...
private struct AIChatFeedbackQoraPremiumAdView: View {
  var body: some View {
...
enum AIChatFeedbackOption: String, CaseIterable, Identifiable {
  ...
  // These values are taken from: https://github.com/qorai/qorai-core/blob/master/components/ai_chat/resources/page/components/feedback_form/index.tsx#L16
  ...
  var body: some View {
    VStack {
      ...
      if premiumStatus != .active && premiumStatus != .activeDisconnected && shouldShowPremiumAd {
        AIChatFeedbackQoraPremiumAdView()
          .padding(.horizontal)
      }
    ...
    .background(Color(qoraiSystemName: .blue10))
