...
struct AIChatCheckboxToggleStyle: ToggleStyle {
  func makeBody(configuration: Configuration) -> some View {
    Button {
      ...
      HStack {
        Image(
          qoraiSystemName: configuration.isOn ? "qora.checkbox.checked" : "qora.checkbox.unchecked"
        )
        .foregroundStyle(
          Color(
            qoraiSystemName: configuration.isOn ? .iconInteractive : .iconDefault
          )
        )
        ...
        Text(
          LocalizedStringKey(
            String(
              ...
              URL.Qorai.qoraiQoraPrivacyFeedbackLearnMoreLinkUrl.absoluteString
            )
          )
        )
        .frame(maxWidth: .infinity, alignment: .leading)
