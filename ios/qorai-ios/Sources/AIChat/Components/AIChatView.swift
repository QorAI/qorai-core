...
public struct AIChatView: View {
  ...
  public var body: some View {
                        ...
                        dismissAction: {
                          if model.apiError == .rateLimitReached {
                            if let nonPremiumModel = model.models.first(where: {
                              guard let leoModelOptions = $0.options.qoraModelOptions else {
                                return false
                              }
    ...
    .qoraiPopup(isPresented: $shouldShowFeedbackPrivacyWarningAlert) {
      ...
      AIChatMenuView(
        ...
        onOptionSelected: { option in
          ...
          case .managePremium:
            ...
            if QoraiStoreSDK.shared.qoraSubscriptionStatus?.state != nil {
              ...
              // The purchase was through the Qorai Account Website
              if QoraiStoreSDK.shared.environment != .production {
                openURL(.qorai.qoraiQoraManageSubscriptionStaging)
              } else {
                openURL(.qorai.qoraiQoraManageSubscriptionProd)
              }
            }
            dismiss()
    ...
    if turn.characterType == .human {
      if isEditingMessageAllowed {
        AIChatResponseMessageViewContextMenuButton(
          ...
          icon: Image(qoraiSystemName: "qora.edit.pencil"),
          onSelected: {
      ...
      AIChatResponseMessageViewContextMenuButton(
        ...
        icon: Image(qoraiSystemName: "qora.refresh"),
        onSelected: {
      ...
      AIChatResponseMessageViewContextMenuButton(
        ...
        icon: Image(qoraiSystemName: "qora.copy"),
        onSelected: {
      ...
      AIChatResponseMessageViewContextMenuButton(
        ...
        icon: Image(qoraiSystemName: "qora.thumb.up"),
        onSelected: {
      ...
      AIChatResponseMessageViewContextMenuButton(
        ...
        icon: Image(qoraiSystemName: "qora.thumb.down"),
        onSelected: {
          guard let turnId = turn.uuid else { return }
  ...
  private var apiErrorViews: some View {
    ...
    if model.isCurrentModelPremium && isPremiumDisconnected {
      if let subscriptionState = QoraiStoreSDK.shared.qoraSubscriptionStatus?.state,
        subscriptionState != .expired && subscriptionState != .revoked
      {
        ...
        AIChatBusyErrorView {
          Task { @MainActor in
            if let orderId = Preferences.AIChat.subscriptionOrderId.value {
              try? await QoraiSkusSDK.shared.fetchCredentials(orderId: orderId, for: .qora)
            }
        ...
        AIChatSessionExpiredErrorView {
          openURL(URL.Qorai.qoraiQoraRefreshCredentials)
          dismiss()
        }
      }
      ...
      AIChatPremiumUpsellView(
        ...
        dismissAction: {
          Task { @MainActor in
            ...
            if let nonPremiumModel = model.models.first(where: {
              guard let leoModelOptions = $0.options.qoraModelOptions else { return false }
              return leoModelOptions.access != .premium
            }) {
              model.changeModel(modelKey: nonPremiumModel.key)
              model.retryLastRequest()
          ...
          VStack(spacing: 0.0) {
            ...
            AIChatSuggestionsView(
              ...
              suggestions: [
                ...
                "Summarize this page?",
                "What is Qora?",
                "What can the Qora assistant do for me?",
              ]
            )
            .padding()
          }
