...
public struct AIChatAdvancedSettingsView: View {
  ...
  public var body: some View {
    if isModallyPresented {
      NavigationView {
        settingsView
          .navigationTitle(Strings.AIChat.qoraNavigationTitle)
  ...
  private var expirationDateTitle: String {
    ...
    return Strings.AIChat.qoraSubscriptionUnknownDateTitle
  }
    ...
    Form {
      Section {
        OptionToggleView(
          ...
          option: Preferences.AIChat.qoraInQuickSearchBarEnabled
        )
      ...
      Section {
          ...
          && (model.premiumStatus == .active || model.premiumStatus == .activeDisconnected)
        {
          ...
          if viewModel.canSubscriptionBeLinked {
            Button(
              action: {
                openURL(.qorai.qoraiQoraLinkReceiptProd)
              },
              label: {
                LabelView(
            ...
            if viewModel.isDevReceiptLinkingAvailable {
              Button(
                action: {
                  openURL(.qorai.qoraiQoraLinkReceiptStaging)
                },
                label: {
                  LabelView(
              ...
              Button(
                action: {
                  openURL(.qorai.qoraiQoraLinkReceiptDev)
                },
                label: {
                  LabelView(
      ...
      Section {
        Button(
          ...
          label: {
            Text(Strings.AIChat.resetQoraDataActionTitle)
        ...
        .alert(isPresented: $resetAndClearAlertErrorPresented) {
          Alert(
            title: Text(Strings.AIChat.resetQoraDataErrorTitle),
            message: Text(Strings.AIChat.resetQoraDataErrorDescription),
            primaryButton: .destructive(Text(Strings.AIChat.resetQoraDataAlertButtonTitle)) {
              Task { @MainActor in
