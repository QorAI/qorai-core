...
enum AIChatMenuOptionTypes {
  ...
  var imageName: String {
    ...
    case .goPremium, .managePremium:
      return "qora.lock.open"
    case .advancedSettings:
      return "qora.settings"
    }
  }
}
...
struct AIChatMenuView: View {
  ...
  var body: some View {
    VStack(spacing: 0.0) {
      ...
      AIChatMenuHeaderView(
        icon: "qora.message.bubble-comments",
      ...
      // Check if qora in-app purchase is activated before or not
      if let state = QoraiStoreSDK.shared.qoraSubscriptionStatus?.state {
    ...
    Button(
      ...
      label: {
        AIChatMenuItemView(
          ...
          case .qoraModelOptions:
            if model.options.qoraModelOptions?.access == .premium && premiumStatus != .active
  ...
  private func menuActionItems(for menuOption: AIChatMenuOptionTypes) -> some View {
    Button {
      if menuOption == .goPremium, !QoraiStoreSDK.shared.isQoraProductsLoaded {
      ...
      Image(qoraiSystemName: menuOption.imageName)
        .foregroundStyle(Color(qoraiSystemName: .iconDefault))
        .padding(.horizontal, 16.0)
        .padding(.vertical, 8.0)
    }
