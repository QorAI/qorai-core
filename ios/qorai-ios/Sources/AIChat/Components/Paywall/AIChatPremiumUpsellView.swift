...
struct AIChatPremiumUpsellView: View {
  ...
  var body: some View {
    ...
    .background(Color(qoraiSystemName: .primary10))
...
struct PremiumUpsellTitleView: View {
  ...
  var foregroundTextColor: Color {
    isPaywallPresented ? Color.white : Color(qoraiSystemName: .textPrimary)
  }
...
struct PremiumUpsellDetailView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      Color(qoraiSystemName: isPaywallPresented ? .primitivePrimary25 : .primary20)
        .frame(height: 1.0)
...
private struct PremiumUpsellTopicView: View {
  ...
  var body: some View {
    HStack {
      Image(qoraiSystemName: topicType.icon)
        .padding(8.0)
        .background(Color(qoraiSystemName: isPaywallPresented ? .primitivePrimary80 : .primary20))
        .foregroundColor(
          Color(qoraiSystemName: isPaywallPresented ? .primitivePrimary20 : .primary60)
        )
        .clipShape(RoundedRectangle(cornerRadius: 8.0, style: .continuous))
      ...
      VStack(alignment: .leading, spacing: 6.0) {
        ...
        Text(topicType.subTitle)
          .font(.footnote)
          .foregroundStyle(
            Color(qoraiSystemName: isPaywallPresented ? .primitivePrimary80 : .textSecondary)
          )
      }
      .lineLimit(2)
      .truncationMode(.tail)
  ...
  fileprivate enum UpsellTopicType {
    ...
    var icon: String {
      switch self {
      case .modelType:
        return "qora.widget.generic"
      case .creativity:
        return "qora.idea"
      case .accuracy:
        return "qora.edit.pencil"
      case .chatLength:
        return "qora.message.bubble-comments"
      }
    }
