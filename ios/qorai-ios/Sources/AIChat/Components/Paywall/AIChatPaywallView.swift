...
struct AIChatPaywallView: View {
  ...
  @ObservedObject
  private(set) var storeSDK = QoraiStoreSDK.shared
    ...
    NavigationView {
        ...
        .introspectViewController(customize: { vc in
          vc.navigationItem.do {
            let appearance = UINavigationBarAppearance().then {
              ...
              $0.backgroundColor = UIColor(qoraiSystemName: .primitivePrimary10)
      ...
      .background(
        Color(qoraiSystemName: .primitivePrimary10)
          .ignoresSafeArea()
          .overlay(
            Image("qora-product", bundle: .module),
    ...
    VStack {
      if availableTierTypes.contains(.yearly) {
        AIChatPremiumTierSelectionView(
          ...
          product: storeSDK.qoraYearlyProduct,
          type: .yearly,
      ...
      if availableTierTypes.contains(.monthly) {
        AIChatPremiumTierSelectionView(
          ...
          description: nil,
          product: storeSDK.qoraMonthlyProduct,
          type: .monthly,
          selectedTierType: $selectedTierType
        )
      }
      ...
      Text(Strings.AIChat.paywallPurchaseDeepNote)
        ...
        .foregroundStyle(Color(qoraiSystemName: .primitivePrimary90))
  ...
  private func purchaseSubscription() async {
    ...
    do {
      ...
      case .monthly:
        try await storeSDK.purchase(product: QoraiStoreProduct.qoraMonthly)
      case .yearly:
        try await storeSDK.purchase(product: QoraiStoreProduct.qoraYearly)
      }
  ...
  private func fetchIntroOfferStatus() async {
    ...
    isMonthlyIntroOfferAvailable = await storeSDK.isIntroOfferAvailable(
      for: QoraiStoreProduct.qoraMonthly
    )

    isYearlyIntroOfferAvailable = await storeSDK.isIntroOfferAvailable(
      for: QoraiStoreProduct.qoraYearly
    )
        ...
        HStack {
          VStack(alignment: .leading, spacing: 8.0) {
            ...
            if let description = description {
              Text(description)
                ...
                .foregroundColor(Color(qoraiSystemName: .green50))
                .padding(4.0)
                .background(Color(qoraiSystemName: .green20))
          ...
          if let product = product {
            HStack(alignment: .center, spacing: 2) {
              ...
              .font(.subheadline)
              .foregroundColor(Color(qoraiSystemName: .primitivePrimary80))
                + Text(
...
private struct AIChatActionButton: View {
  ...
  var body: some View {
    VStack(spacing: 8) {
      Text(title)
        .font(.callout.weight(.semibold))
        .foregroundColor(Color(qoraiSystemName: .primitivePrimary90))
        .multilineTextAlignment(.center)
      ...
      .overlay(
        ContainerRelativeShape()
          .strokeBorder(
            Color(qoraiSystemName: .dividerInteractive),
            lineWidth: 1.0
          )
      )
      .containerShape(RoundedRectangle(cornerRadius: 8.0, style: .continuous))
