...
public actor AdblockResourceDownloader: Sendable {
  ...
  static let handledResources: [QoraiS3Resource] = {
    return [.slimList]
  }()

  /// A list of old resources that need to be deleted so as not to take up the user's disk space
  private static let deprecatedResources: [QoraiS3Resource] = {
  ...
  private let resourceDownloader: ResourceDownloader<QoraiS3Resource>
  ...
  private func startFetching(resource: QoraiS3Resource, every fetchInterval: TimeInterval) {
    Task { @MainActor in
  ...
  private func handle(
    downloadResult: ResourceDownloader<QoraiS3Resource>.DownloadResult,
    for resource: QoraiS3Resource,
