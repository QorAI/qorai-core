...
public class QoraiSkusManager {
  ...
  public func refreshVPNCredentials() async {
    if let domain = Preferences.VPN.skusCredentialDomain.value {
      // Always refresh credentials and trust the credentials from Qorai-Core rather than cached credentials
  ...
  func prepareCredentialsPresentation(for domain: String, path: String) async -> String? {
    ...
    if !credential.isEmpty {
      ...
      case .vpn:
        if let vpnCredential = QoraiSkusWebHelper.fetchVPNCredential(credential, domain: domain) {
          Preferences.VPN.skusCredential.value = credential
          QoraiVPN.setCustomVPNCredential(vpnCredential)
        }
      case .qora:
        break
      case .unknown:
    ...
    do {
      ...
      case .valid:
        ...
        case .qora:
          ...
          if Preferences.AIChat.subscriptionOrderId.value != nil {
            Logger.module.debug("[SkusManager] - Preparing Qora Credentials")
            _ = await prepareCredentialsPresentation(for: domain, path: "*")
...
private enum CredentialType {
  ...
  static func from(domain: String) -> CredentialType {
    switch domain {
    case "vpn.qorai.software", "vpn.qoraisoftware.com", "vpn.qorai.com": return .vpn
    case "qora.qorai.software", "qora.qoraisoftware.com", "qora.qorai.com": return .qora
    default:
      return .unknown
    }
  }
...
private struct CredentialSummary {
  ...
  static func from(data: Data) throws -> CredentialSummary {
    ...
    let product = summary.order.items.compactMap({ QoraiStoreProduct(rawValue: $0.sku) }).first
