...
enum QoraiTranslateError: String, Error {
  case invalidURL
...
class QoraiTranslateTabHelper: NSObject, TabObserver {
  private weak var tab: (any TabState)?
  private weak var delegate: QoraiTranslateScriptHandlerDelegate?
  ...
  private var translationController: UIHostingController<QoraiTranslateContainerView>!
  private var translationSession: QoraiTranslateSession?
  private var translationTask: (() async throws -> Void)?
  private var canShowToast = false

  var currentLanguageInfo = QoraiTranslateLanguageInfo()
  ...
  // WebState in Qorai, is the same as `Tab`.
  init(tab: some TabState, delegate: QoraiTranslateScriptHandlerDelegate) {
    self.tab = tab
    self.delegate = delegate
    super.init()

    translationController = UIHostingController(
      rootView: QoraiTranslateContainerView(
  ...
  func startTranslation(canShowToast: Bool) {
    ...
    self.translationTask = { @MainActor [weak self] in
      ...
      let currentLanguage = currentLanguageInfo.currentLanguage.qoraiTranslateLanguageIdentifier

      guard
        let pageLanguage = currentLanguageInfo.pageLanguage?.qoraiTranslateLanguageIdentifier,
        currentLanguage != pageLanguage
      else {
        throw QoraiTranslateError.invalidLanguage
      }
  ...
  func finishSetup() async throws {
    ...
    let isTranslationSupported = await QoraiTranslateSession.isTranslationSupported(
      from: pageLanguage,
  ...
  func processTranslationRequest(
    _ request: QoraiTranslateSession.RequestMessage
  ) async throws -> (Data, HTTPURLResponse) {
    let translationSession = self.translationSession ?? QoraiTranslateSession()
    let isTranslationRequest = QoraiTranslateSession.isPhraseTranslationRequest(request)
  ...
  func tabDidFinishNavigation(_ tab: some TabState) {
    Task { @MainActor [weak self, weak tab] in
      ...
      if let delegate = self.delegate {
        delegate.updateTranslateURLBar(tab: tab, state: .unavailable)
        QoraiTranslateScriptHandler.checkTranslate(tab: tab)
      }
    }
  }
  ...
  private func loadTranslateScript() async throws {
    ...
    _ = try? await tab.callAsyncJavaScript(
      """
      await window.__firefox__.\(QoraiTranslateScriptHandler.namespace).loadTranslateScript();
      """,
    ...
    do {
      let result = try await tab.evaluateJavaScript(
        functionName: """
          ...
          window.__firefox__.\(QoraiTranslateScriptHandler.namespace).isPageTranslated()
          """,
        contentWorld: QoraiTranslateScriptHandler.scriptSandbox,
        asFunction: false
      )
      ...
      if await hasTranslationFinished() {
        ...
        if actualSourceLanguage.isEmpty
          || actualSourceLanguage
            == currentLanguageInfo.currentLanguage.qoraiTranslateLanguageIdentifier
        {
          delegate.updateTranslateURLBar(tab: tab, state: .available)
          delegate.presentTranslateError(tab: tab)
          return
    ...
    do {
      let result = try await tab.evaluateJavaScript(
        functionName: """
          typeof cr != 'undefined' && typeof cr.googleTranslate != 'undefined' && 
          typeof cr.googleTranslate.translate == 'function' &&
          window.__firefox__.\(QoraiTranslateScriptHandler.namespace).isTranslateScriptLoaded()
          """,
        contentWorld: QoraiTranslateScriptHandler.scriptSandbox,
        asFunction: false
      )
...
class QoraiTranslateLanguageInfo: ObservableObject {
  @Published
