...
window.__firefox__.includeOnce("QoraiSkusScript", function($) {
  ...
  // Request Qora Receipt
  sendMessage('$<setLocalStorageReceipt>', { "message": "qora" })
  .then(function(jsonData) {
    if (jsonData) {
      window.localStorage.setItem("qoraiQora.orderId", jsonData["qoraiQora.orderId"])
      window.localStorage.setItem(jsonData["key"], jsonData["data"]);
    } else {
      console.error("No Qora Subscription Data");
    }
  }).catch(function(err) {
    console.error(`An error occurred setting the local storage receipt: ${err}`);
  });

  Object.defineProperty(window.chrome, 'qoraiSkus', {
    enumerable: false,
    configurable: false,
    writable: false,
      value: {
