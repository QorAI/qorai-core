  ...
  const sendMessage = $((resourceURL) => {
    ...
    if (blockingCache.has(resourceURL.href)) {
      return Promise.resolve(blockingCache.get(resourceURL.href)).then(blocked => {
        if (blocked) {
          console.info(`Qorai prevented frame displaying ${window.location.href}${originDisplay} from loading a resource from ${resourceURL.href} (cached)`)
        }
        return blocked
      })
    }
    ...
    return $.postNativeMessage(messageHandler, {
      ...
      if (blocked) {
        console.info(`Qorai prevented frame displaying ${window.location.href}${originDisplay} from loading a resource from ${resourceURL.href}`)
      }
