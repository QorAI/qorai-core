...
extension ContentBlockerHelper: TabContentScript {
  ...
  func tab(
    ...
    do {
      ...
      dtos.forEach { dto in
        Task { @MainActor in
          guard let qoraiShieldsHelper = tab.qoraiShieldsHelper else { return }
          if !qoraiShieldsHelper.isQoraiShieldsEnabled(for: currentTabURL) {
            // if shields are disabled, can return early.
            return
          }

          if dto.resourceType == .script
            && qoraiShieldsHelper.isShieldExpected(
              for: currentTabURL,
              shield: .noScript,
            ...
            )
          {
            ...
            QoraiGlobalShieldStats.shared.scripts += 1
            return
          }
          ...
          let shieldLevel = qoraiShieldsHelper.shieldLevel(
            for: currentTabURL,
            considerAllShieldsOption: true
          )
          let genericTypes = AdBlockGroupsManager.shared.contentBlockerManager.validGenericTypes(
            isShieldsEnabled: qoraiShieldsHelper.isQoraiShieldsEnabled(for: currentTabURL),
          ...
          // Increase global stats (here due to BlocklistName being in Client and QoraiGlobalShieldStats being
          // in QoraiShared)
          let stats = QoraiGlobalShieldStats.shared
          switch blockedType {
          case .ad:
            stats.adblock += 1
