...
protocol QoraiTranslateScriptHandlerDelegate: NSObject {
  ...
  func presentTranslateToast(tab: some TabState, languageInfo: QoraiTranslateLanguageInfo)
  func presentTranslateError(tab: some TabState)
}

class QoraiTranslateScriptHandler: NSObject, TabContentScript {
  private static var elementScript: String?
  private var tasks = [UUID: Task<Void, Error>]()

  static let namespace = "translate_\(uniqueID)"
  static let scriptName = "QoraiTranslateScript"
  ...
  static let userScript: WKUserScript? = {
    ...
    return WKUserScript(
      source:
        script
        .replacingOccurrences(of: "$<qorai_translate_script>", with: namespace)
        .replacingOccurrences(
          of: "$<qorai_translate_api_key>",
          with: kQoraiServicesKey
        )
        .replacingOccurrences(of: "$<message_handler>", with: messageHandlerName)
        .replacingOccurrences(
          of: "$<qorai_core_translate_translate_js>",
  ...
  static func checkTranslate(tab: some TabState) {
    tab.evaluateJavaScript(
      functionName:
        ...
        """,
      contentWorld: QoraiTranslateScriptHandler.scriptSandbox,
      asFunction: false
    )
  }
  ...
  func tab(
    ...
    if tab.visibleSecureContentState != .secure && !isReaderMode {
      ...
      replyHandler(nil, QoraiTranslateError.translateDisabled.rawValue)
      return
    }
    ...
    guard let body = message.body as? [String: Any],
      ...
      Logger.module.error("Invalid Qorai Translate Message")
      return
    }
    ...
    let task = Task { [weak self, weak tab] in
      guard let self = self, let tab = tab else {
        replyHandler(nil, QoraiTranslateError.otherError.rawValue)
        return
      }
  ...
  private func processScriptMessage(
    ...
    if command == "load_qorai_translate_script" {
      if Preferences.Translate.translateEnabled.value {
      ...
      return (nil, QoraiTranslateError.translateDisabled.rawValue)
    }
    ...
    if command == "request" {
      ...
      } catch let error as QoraiTranslateError {
        Logger.module.error("Qorai Translate Error: \(error)")
    ...
    if command == "status" {
      guard let translateHelper = tab.translateHelper else {
        Logger.module.debug("[Qorai Translate] - Status: \(body)")
        return (nil, nil)
      }

      if let errorCode = body["errorCode"] as? Int,
        let status = QoraiTranslateTabHelper.TranslateError(rawValue: errorCode)
      {
        Logger.module.debug("[Qorai Translate] - Status: \(String(describing: status))")
  ...
  private func downloadElementScript() async throws -> String {
    var urlRequest = URLRequest(
      url: URL(string: "https://translate.qorai.com/static/v1/element.js")!
    )
    ...
    guard let response = response as? HTTPURLResponse, response.statusCode == 200 else {
      throw QoraiTranslateError.invalidTranslationResponse
    }
...
class QoraiTranslateScriptLanguageDetectionHandler: NSObject, TabContentScript {
  private static let namespace = "translate_\(uniqueID)"

  static let scriptName = "QoraiTranslateLanguageDetectionScript"
  static let scriptId = UUID().uuidString
  ...
  static let scriptSandbox = QoraiTranslateScriptHandler.scriptSandbox
  ...
  func tab(
    ...
    guard let body = message.body as? [String: Any] else {
      Logger.module.error("Invalid Qorai Translate Language Detection Message")
      return
    }
    ...
    do {
      ...
      Logger.module.error("Qorai Translate Language Detection Error: \(error)")
