...
class QoraiQoraScriptTabHelper: AIChatWebDelegate {
  ...
  func getPageContentType() async -> String? {
    ...
    return try? await tab.evaluateJavaScript(
      ...
      contentWorld: QoraiQoraScriptHandler.scriptSandbox,
      escapeArgs: false,
      asFunction: false
    ) as? String
  }
    ...
    do {
      let articleText =
        try await tab.evaluateJavaScript(
          functionName: "window.__firefox__.\(QoraiQoraScriptHandler.getMainArticle)",
          args: [QoraiQoraScriptHandler.scriptId],
          contentWorld: QoraiQoraScriptHandler.scriptSandbox,
          asFunction: true
        ) as? String
      return articleText
