...
window.__firefox__.includeOnce("QoraiTranslateScript", function($) {
  var translateScriptLoaded = false;
  
  Object.defineProperty(window.__firefox__, "$<qorai_translate_script>", {
    enumerable: false,
    configurable: false,
    writable: false,
    value: {
      ...
      "detectLanguage": (function() {
        window.webkit.messageHandlers["LanguageDetectionTextCaptured"].postMessage({
          "hasNoTranslate": window.__firefox__.$<qorai_translate_script>.hasNoTranslate(),
          "htmlLang": window.__firefox__.$<qorai_translate_script>.getPageLanguage(),
          "httpContentLanguage": window.__firefox__.$<qorai_translate_script>.getMetaContentByHttpEquiv(),
      ...
      "loadTranslateScript": (function() {
        return new Promise((resolve, reject) => {
          ...
          window.webkit.messageHandlers["$<message_handler>"].postMessage({
            "command": "load_qorai_translate_script"
          }).then((script) => {
...
var translateApiKey = '$<qorai_translate_api_key>';
var gtTimeInfo = {
    'fetchStart': Date.now(),
    'fetchEnd': Date.now() + 1
};
var serverParams = '';
var securityOrigin = 'https://translate.qorai.com/';


try {
  // components/translate/core/browser/resources/translate.js
  $<qorai_core_translate_translate_js>
...
// qorai-core/components/translate/core/browser/resources/qorai_translate.js

try {
  const useGoogleTranslateEndpoint = false;
  const qoraiTranslateStaticPath = '/static/v1/';
  // securityOrigin is predefined by translate_script.cc.
  const securityOriginHost = new URL(securityOrigin).host;

  // A method to rewrite URL in the scripts:
  // 1. change the domain to translate.qorai.com;
  // 2. adjust static paths to use qoraiTranslateStaticPath.
  const rewriteUrl = (url)=>{
      try {
          ...
          if (newURL.pathname === '/translate_a/t') {
              ...
              // qoraiTranslateStaticPath is predefined by translate_script.cc.
              newURL.pathname = newURL.pathname.replace('/translate_static/', qoraiTranslateStaticPath);
          }
  ...
  if (window.__firefox__.$<qorai_translate_script>.useNativeNetworking) {
  ...
  // 3. Add qoraiExtraStyles in the end.
  cr.googleTranslate.onLoadCSS = function(url) {
      ...
      // in future versions.
      const qoraiExtraStyles = `.goog-te-spinner-pos, #goog-gt-tt {display: none;}`
      ...
      xhr.onreadystatechange = function() {
          ...
          if ('adoptedStyleSheets' in Document.prototype && 'replaceSync' in CSSStyleSheet.prototype) {
              ...
              sheet.replaceSync(processCSS(this.responseText) + qoraiExtraStyles);
              document.adoptedStyleSheets = [...document.adoptedStyleSheets, sheet];
          } else {
              const element = document.createElement('style');
              element.type = 'text/css';
              element.charset = 'UTF-8';
              element.innerText = processCSS(this.responseText) + qoraiExtraStyles;
...
// Qorai Translate
window.__firefox__.$<qorai_translate_script>.loadTranslateScript().catch((error) => {
...
window.addEventListener("pageshow", (event) => {
  if (event.persisted) {
    setTimeout(() => {
      window.__firefox__.$<qorai_translate_script>.detectLanguage();
    }, 1000);
  }
});
