...
class SolanaProviderScriptHandler: TabContentScript {
  ...
  @MainActor func signAllTransactions(tab: some TabState, args: String?) async -> (Any?, String?) {
    ...
    let params: [QoraiWallet.SolanaSignTransactionParam] = transactions.compactMap { tx in
    ...
    let serializedTransactionDicts: [BaseValue] = zip(serializedTxs, versions).compactMap {
      serializedTx,
      versionInt in
      guard let version = QoraiWallet.SolanaMessageVersion(rawValue: versionInt.intValue) else {
        return nil
      }
  ...
  private func createSignTransactionParam(
  ...
  ) -> QoraiWallet.SolanaSignTransactionParam {
    ...
    let signatures: [QoraiWallet.SignaturePubkeyPair] = signaturesValues.map {
      QoraiWallet.SignaturePubkeyPair(
  ...
  private func buildSerializedTxJson(
    ...
    version: QoraiWallet.SolanaMessageVersion
  ) -> BaseValue {
  ...
  private func buildErrorJson(
    status: QoraiWallet.SolanaProviderError,
    errorMessage: String
  ) -> String? {
