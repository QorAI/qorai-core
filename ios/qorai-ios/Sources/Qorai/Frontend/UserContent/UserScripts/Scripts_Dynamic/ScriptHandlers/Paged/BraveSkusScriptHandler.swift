...
class QoraiSkusScriptHandler: TabContentScript {
  static let scriptName = "QoraiSkusScript"
  ...
  func tab(
    ...
    guard let response = message.body as? [String: Any] else {
      Logger.module.error("Qorai skus request with no message")
      return
    }
    ...
    guard let requestedMethod = response["method_id"] as? String,
      ...
      Logger.module.error("Qorai skus request with invalid method-id")
      return
    }
    ...
    Task { @MainActor in
      do {
        ...
        Logger.module.error("Qorai skus error processing request: \(error)")
  ...
  private func processRequest(
    ...
    guard let skusManager = QoraiSkusManager(isPrivateMode: tab.isPrivate) else {
      return nil
    }
    ...
    case .setLocalStorageReceipt:
      ...
      if storeMessage.message == "qora" {
          ...
          let product = QoraiStoreProduct(rawValue: aiChatSubscriptionProductId)
        {
          let storageKey = product.localStorageKey
          let receipt = try QoraiSkusSDK.receipt(for: product)
          return ["key": storageKey, "data": receipt, "qoraiQora.orderId": orderId]
        }
      }
...
extension QoraiSkusScriptHandler {
...
struct RequestHost {
  static func from(message: WKScriptMessage) throws -> String {
    guard message.frameInfo.isMainFrame else {
      Logger.module.error(
        "Qorai skus request with error: \(SkusWebMessageError.invalidRequestFrame)"
      )
    ...
    guard let requestHost = message.frameInfo.request.url?.host else {
      Logger.module.error(
        "Qorai skus request with error: \(SkusWebMessageError.invalidRequestHost)"
      )
      throw SkusWebMessageError.invalidRequestHost
    }

    guard DomainUserScript.qoraiSkus.associatedDomains.contains(requestHost) else {
      Logger.module.error(
        "Qorai skus request with error: \(SkusWebMessageError.invalidRequestHost)"
      )
      throw SkusWebMessageError.invalidRequestHost
