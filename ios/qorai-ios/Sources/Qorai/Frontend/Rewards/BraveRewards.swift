...
public class QoraiRewards: PreferencesObserver {

  /// Whether or not Qorai Rewards is available/can be enabled
  public static func isSupported(prefService: any PrefService) -> Bool {
    let isSupportedFromPrefs = QoraiRewardsAPI.isSupported(prefService)
    #if DEBUG
  ...
  private(set) var ads: QoraiAds
  private(set) var rewardsAPI: QoraiRewardsAPI?
  var rewardsServiceDidStart: ((QoraiRewardsAPI) -> Void)?
  ...
  public init(configuration: Configuration) {
    ...
    ads = QoraiAds(stateStoragePath: configuration.storageURL.appendingPathComponent("ads").path)

    if Preferences.Rewards.adsEnabledTimestamp.value == nil, ads.isEnabled {
      Preferences.Rewards.adsEnabledTimestamp.value = Date()
    }

    ads.notifyQoraiNewsIsEnabledPreferenceDidChange(Preferences.QoraiNews.isEnabled.value)
  ...
  func startRewardsService(_ completion: (() -> Void)?) {
    ...
    rewardsAPI = QoraiRewardsAPI(stateStoragePath: storagePath)
  ...
  private func fetchWalletAndInitializeAds(toggleAds: Bool? = nil) {
    ...
    rewardsAPI.currentWalletInfo { wallet in
      var walletInfo: QoraiAds.WalletInfo?
      if let wallet {
  ...
  func reset() async {
    ...
    if ads.isServiceRunning(), !Preferences.QoraiNews.isEnabled.value {
  ...
  /// Notifies Qorai Ads that the given tab did change
  ...
  /// Notifies Qorai Ads that the given tab did load
  ...
  func reportLoadedPage(
    ...
    if ads.isServiceRunning(), let tabData = tab.browserData {
        ...
        && responseStatusCodeClass != kHttpServerErrorResponseStatusCodeClass
      {
        ...
        // for Qorai Rewards users. However, we must notify that the tab content has
...
extension QoraiRewards {
  ...
  public struct Configuration {
    ...
    public var adsBuildChannel: QoraiAds.BuildChannelInfo = .init()
    ...
    public static func current(
      ...
      var configuration: QoraiRewards.Configuration = .production
