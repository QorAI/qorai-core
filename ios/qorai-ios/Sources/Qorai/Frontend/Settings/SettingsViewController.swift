...
class SettingsViewController: TableViewController {
  ...
  private let qoraiCore: QoraiProfileController
  private let historyAPI: QoraiHistoryAPI
  private let passwordAPI: QoraiPasswordAPI
  private let syncAPI: QoraiSyncAPI
  private let syncProfileServices: QoraiSyncProfileServiceIOS
  ...
  private lazy var qoraiAccountAuthentication: any QoraiAccountAuthentication = {
    return QoraiAccountAuthenticationBridgeImpl(profile: qoraiCore.profile)
  }()
  ...
  init(
    ...
    self.qoraiCore = qoraiCore
    self.localState = localState
    self.historyAPI = qoraiCore.historyAPI
    self.passwordAPI = qoraiCore.passwordAPI
    self.syncAPI = qoraiCore.syncAPI
    self.syncProfileServices = qoraiCore.syncProfileService
    self.p3aUtilities = p3aUtils
    self.deAmpPrefs = qoraiCore.deAmpPrefs
    self.attributionManager = attributionManager
    self.keyringStore = keyringStore
    self.cryptoStore = cryptoStore
    self.ipfsAPI = qoraiCore.ipfsAPI
    self.prefsChangeRegistrar = PrefChangeRegistrar(prefService: qoraiCore.profile.prefs)
  ...
  override func viewDidLoad() {
    ...
    view.backgroundColor = .qoraiGroupedBackground
    view.tintColor = .qoraiBlurpleTint
    navigationController?.view.backgroundColor = .qoraiGroupedBackground

    if qoraiCore.profile.prefs.isQoraiVPNAvailable {
    ...
    prefsChangeRegistrar.addObserver(
      forPath: QoraiAccountAuthenticationTokenPref,
      callback: refreshUI
    )
    prefsChangeRegistrar.addObserver(
      forPath: QoraiAccountVerificationTokenPref,
      callback: refreshUI
    )
  }
  ...
  private func displayQoraiNewsDebugMenu() {
    let settings = UIHostingController(
      rootView: QoraiNewsDebugSettingsView(dataSource: feedDataSource)
    )
    navigationController?.pushViewController(settings, animated: true)
  }

  private func displayQoraiSearchDebugMenu() {
    let hostingController =
      UIHostingController(rootView: QoraiSearchDebugMenu(logging: QoraiSearchLogEntry.shared))

    navigationController?.pushViewController(hostingController, animated: true)
  }

  private func displayQoraiWalletDebugMenu() {
    let hostingController =
      UIHostingController(rootView: QoraiWalletDebugMenu())
  ...
  private func makeSections() -> [Static.Section] {
    ...
    if FeatureList.kQoraiAccount.enabled {
      list.insert(qoraiAccountSection, at: 1)
    }
    ...
    let shouldShowVPNSection = { () -> Bool in
      ...
      if !QoraiVPNProductInfo.isComplete || Preferences.VPN.vpnSettingHeaderWasDismissed.value {
        return false
      }
    ...
    if shouldShowVPNSection {
      list.insert(enableQoraiVPNSection, at: 0)
    }
  ...
  private lazy var enableQoraiVPNSection: Static.Section = {
    let header = QoraiVPNEnableSettingsHeaderView(
          ...
          selection: { [unowned self] in
            let controller = UIHostingController(
              rootView: DataImportView(
                model: .init(
                  coordinator: SafariDataImporterCoordinatorImpl(profile: qoraiCore.profile)
                ),
  ...
  private var qoraiAccountSection: Static.Section {
    let authenticationToken = qoraiCore.profile.prefs.string(
      forPath: QoraiAccountAuthenticationTokenPref
    )
    if !authenticationToken.isEmpty {
      return Static.Section(
        header: .title(Strings.qoraiAccount),
        rows: [
          Row(
            text: "John Doe",
            detailText: "johndoe@gmail.com",
            image: UIImage(sharedNamed: "qorai.logo"),
            cellClass: QoraiAccountIconCell.self
          ),
          Row(
            text: Strings.qoraiAccountSignOut,
            selection: { [unowned self] in qoraiAccountAuthentication.logOut() },
            cellClass: QoraiAccountIconCell.self,
            context: [
              QoraiAccountIconCell.titleColorKey: UIColor(qoraiSystemName: .textInteractive)
            ]
          ),
        ]
      )
    ...
    let verificationToken = qoraiCore.profile.prefs.string(
      forPath: QoraiAccountVerificationTokenPref
    )
    if !verificationToken.isEmpty {
      return Static.Section(
        ...
        rows: [
          Row(
            text: Strings.qoraiAccountAlmostThere,
            detailText: Strings.qoraiAccountAlmostThereDetail,
            image: UIImage(sharedNamed: "qorai.logo"),
            cellClass: QoraiAccountIconCell.self
          ),
          Row(
            text: Strings.qoraiAccountResendConfirmationEmail,
            detailText: Strings.qoraiAccountResendConfirmationEmailDetail,
            selection: { [unowned self] in qoraiAccountAuthentication.resendConfirmationEmail() },
            cellClass: QoraiAccountIconCell.self,
            context: [
              QoraiAccountIconCell.titleColorKey: UIColor(qoraiSystemName: .textInteractive)
            ]
          ...
          Row(
            text: Strings.qoraiAccountCancelRegistration,
            selection: { [unowned self] in qoraiAccountAuthentication.cancelRegistration() },
            cellClass: QoraiAccountIconCell.self,
            context: [
              QoraiAccountIconCell.titleColorKey: UIColor(qoraiSystemName: .systemfeedbackErrorText)
            ]
          ),
        ]
      )
    ...
    return Static.Section(
      ...
      rows: [
        Row(
          text: Strings.qoraiAccountGetStarted,
          selection: { [unowned self] in
            let controller = ChromeWebUIController(qoraiCore: qoraiCore, isPrivateBrowsing: false)
            let container = UINavigationController(rootViewController: controller)
            controller.title = Strings.qoraiAccount
            controller.webView.load(URLRequest(url: URL(string: "qorai://account")!))
  ...
  private func makeFeaturesSection() -> Static.Section {
    ...
    var section = Static.Section(
      ...
      rows: [
        Row(
          text: Strings.qoraiShieldsAndPrivacySettingsTitle,
          selection: { [unowned self] in
            let controller = UIHostingController(
              rootView: AdvancedShieldsSettingsView(
                settings: AdvancedShieldsSettings(
                  ...
                  qoraiShieldsSettings: QoraiShieldsSettingsServiceFactory.get(
                    profile: qoraiCore.profile
                  ),
                  qoraiCore: qoraiCore,
                  p3aUtils: p3aUtilities,
                  rewards: rewards,
                  qoraiStats: qoraiCore.qoraiStats,
          ...
          image: UIImage(qoraiSystemNamed: "qora.shield.done"),
          accessory: .disclosureIndicator
        )
      ],
    ...
    if QoraiRewards.isSupported(prefService: qoraiCore.profile.prefs), let rewards = rewards {
      section.rows += [
        Row(
          text: Strings.qoraiRewardsSettingsTitle,
          selection: { [unowned self] in
            let rewardsVC = QoraiRewardsSettingsViewController(rewards: rewards)
            self.navigationController?.pushViewController(rewardsVC, animated: true)
          },
          image: UIImage(qoraiSystemNamed: "qora.product.bat-outline"),
          accessory: .disclosureIndicator
        )
      ]
    }
    ...
    if qoraiCore.profile.prefs.isQoraiNewsAvailable {
      section.rows.append(
        Row(
          text: Strings.QoraiNews.qoraiNewsTitle,
          selection: { [unowned self] in
          ...
          image: UIImage(qoraiSystemNamed: "qora.product.qorai-news"),
          accessory: .disclosureIndicator
        )
      )
    }
    ...
    if !tabManager.privateBrowsingManager.isPrivateBrowsing
      && AIChatUtils.isAIChatEnabled(for: qoraiCore.profile.prefs)
    {
    ...
    if qoraiCore.profile.prefs.isPlaylistAvailable {
      section.rows.append(
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.product.playlist"),
          accessory: .disclosureIndicator
        )
      )
    }

    if FeatureList.kQoraiTranslateEnabled.enabled {
      section.rows.append(
        Row(
          text: Strings.QoraiTranslate.settingsMenuTitle,
          selection: { [unowned self] in
            let translateSettings = UIHostingController(rootView: QoraiTranslateSettingsView())
            self.navigationController?.pushViewController(translateSettings, animated: true)
          },
          image: UIImage(qoraiSystemNamed: "qora.product.translate"),
          accessory: .disclosureIndicator
        )
      )
    }
  ...
  private lazy var generalSection: Static.Section = {
    var general = Static.Section(
      ...
      rows: [
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.search"),
          accessory: .disclosureIndicator,
        ...
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.product.sync"),
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        .boolRow(
          ...
          image: UIImage(qoraiSystemNamed: "qora.folder.open")
        ),
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.siri.shorcut"),
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
      ]
    ...
    let defaultHostContentSettings = qoraiCore.defaultHostContentSettings
    if UIDevice.isIpad {
      ...
      general.rows.append(
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.window.cursor"),
    ...
    general.rows.append(contentsOf: [
      .boolRow(
        ...
        image: UIImage(qoraiSystemNamed: "qora.browser.refresh")
      ),
      Row(
        text: Strings.blockPopups,
        image: UIImage(qoraiSystemNamed: "qora.shield.block"),
    ...
    let websiteRedirectsRow = Row(
      ...
      image: UIImage(qoraiSystemNamed: "qora.swap.horizontal"),
      accessory: .disclosureIndicator,
      cellClass: MultilineSubtitleCell.self
    )
    general.rows.append(websiteRedirectsRow)

    if FeatureList.kQoraiOrigin.enabled {
      general.rows.append(
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.product.origin"),
          accessory: .disclosureIndicator,
          cellClass: MultilineSubtitleCell.self
        )
      )
  ...
  private lazy var tabsSection: Static.Section = {
    ...
    if UIDevice.current.userInterfaceIdiom == .pad {
      tabs.rows.append(
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.window.tab"),
    ...
    var autoCloseTabsRow =
      Row(
        ...
        image: UIImage(qoraiSystemNamed: "qora.window.tabs"),
        accessory: .disclosureIndicator,
        cellClass: MultilineSubtitleCell.self
      )
    ...
    if !Preferences.Privacy.privateBrowsingOnly.value {
      let privateTabsRow = Row(
        ...
        image: UIImage(qoraiSystemNamed: "qora.product.private-window"),
        accessory: .disclosureIndicator
      )
  ...
  private lazy var displaySection: Static.Section = {
    ...
    display.rows.append(
      .init(
        ...
        image: UIImage(qoraiSystemNamed: "qora.media.player"),
        accessory: .disclosureIndicator,
        cellClass: MultilineValue1Cell.self
      )
    )
    ...
    var row = Row(
      ...
      image: UIImage(qoraiSystemNamed: "qora.appearance"),
      accessory: .disclosureIndicator,
      cellClass: MultilineSubtitleCell.self
    )
    row.selection = { [unowned self] in
      ...
      let nightModeSection = Section(
        ...
        rows: [
          .boolRow(
            ...
            image: UIImage(qoraiSystemNamed: "qora.theme.dark")
          )
        ],
    ...
    display.rows.append(
      Row(
        ...
        image: UIImage(qoraiSystemNamed: "qora.window.tab-new"),
        accessory: .disclosureIndicator,
        cellClass: MultilineValue1Cell.self
      )
    )
    ...
    if !tabManager.privateBrowsingManager.isPrivateBrowsing {
      display.rows.append(
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.font.size"),
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        )
      )
    ...
    display.rows.append(
      Row(
        ...
        image: UIImage(qoraiSystemNamed: "qora.launch"),
        accessory: .disclosureIndicator,
        cellClass: MultilineValue1Cell.self
      )
    )

    if QoraiRewards.isSupported(prefService: qoraiCore.profile.prefs) {
      display.rows.append(
        .boolRow(
  ...
  private var vpnSettingsRow: Row {
    let (text, color) = { () -> (String, UIColor) in
      ...
      {
        return (Strings.VPN.updateActionCellTitle, .qoraiErrorLabel)
      }
      ...
      case .purchased(let enabled):
        if enabled {
          return (Strings.VPN.settingsVPNEnabled, .qoraiSuccessLabel)
        } else {
          return (Strings.VPN.settingsVPNDisabled, .qoraiErrorLabel)
        }
      case .expired:
        return (Strings.VPN.settingsVPNExpired, .qoraiErrorLabel)
      }
    }()
    ...
    return Row(
      ...
      detailText: text,
      selection: { [unowned self] in
        if QoraiVPNProductInfo.isComplete {
          switch QoraiVPN.vpnState {
          case .notPurchased, .expired:
            guard QoraiVPN.vpnState.isPaywallEnabled else { return }
            self.presentVPNPaywall()
          case .purchased:
            let vpnSettingsVC = QoraiVPNSettingsViewController(iapObserver: QoraiVPN.iapObserver)
      ...
      image: Preferences.VPN.vpnReceiptStatus.value
        == QoraiVPN.ReceiptResponse.Status.retryPeriod.rawValue
        ? UIImage(qoraiSystemNamed: "qora.warning.triangle-filled")?
          .withRenderingMode(.alwaysOriginal)
          .withTintColor(.qoraiErrorLabel)
        : UIImage(qoraiSystemNamed: "qora.product.vpn"),
      accessory: .disclosureIndicator,
  ...
  private var leoSettingsRow: Row {
    return Row(
      text: Strings.qoraMenuItem,
      selection: { [unowned self] in
        let model = AIChatViewModel(
          qoraiCore: self.qoraiCore,
          webDelegate: self.tabManager.selectedTab?.qoraTabHelper,
          qoraiTalkScript: nil
        )
      ...
      image: UIImage(qoraiSystemNamed: "qora.product.qorai-qora"),
      accessory: .disclosureIndicator
    )
  }
    ...
    return Section(
      ...
      rows: [
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.biometric.login"),
          accessory: .view(
            SwitchAccessoryView(
        ...
        Row(
          ...
          image: UIImage(qoraiSystemNamed: "qora.outside"),
          accessory: .disclosureIndicator
        ),
      ]
    )
    ...
    return Static.Section(
      ...
      rows: [
        Row(
          ...
          selection: { [unowned self] in
            self.settingsDelegate?.settingsOpenURLInNewTab(.qorai.community)
            self.dismiss(animated: true)
          },
          image: UIImage(qoraiSystemNamed: "qora.bug"),
          cellClass: MultilineValue1Cell.self
        ),
        Row(
          text: Strings.rateQorai,
          selection: { [unowned self] in
            // Rate Qorai
            guard
          ...
          image: UIImage(qoraiSystemNamed: "qora.message.bubble-smile"),
          cellClass: MultilineValue1Cell.self
        ),
      ]
    )
  ...
  private lazy var aboutSection: Static.Section = {
    ...
    let titleLabel = UITableViewHeaderFooterView().then {
      ...
      $0.textLabel?.textColor = .qoraiLabel
    ...
    let coreVersion =
      "QoraiCore \(QoraiCoreVersionInfo.qoraiCoreVersion) (\(QoraiCoreVersionInfo.chromiumVersion))"
    return Static.Section(
      ...
      rows: [
        ...
        Row(
          ...
          selection: { [unowned self] in
            settingsDelegate?.settingsOpenURLInNewTab(.qorai.privacy)
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        Row(
          ...
          selection: { [unowned self] in
            settingsDelegate?.settingsOpenURLInNewTab(.qorai.termsOfUse)
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        Row(
          ...
          selection: { [unowned self] in
            if FeatureList.kUseChromiumWebViews.enabled {
              if let url = URL(string: "qorai://credits") {
                settingsDelegate?.settingsOpenURLInNewTab(url)
              }
            } else {
              let controller = ChromeWebUIController(qoraiCore: qoraiCore, isPrivateBrowsing: false)
              let container = UINavigationController(rootViewController: controller)
              controller.webView.load(URLRequest(url: URL(string: "qorai://credits")!))
  ...
  private lazy var debugSection: Static.Section = {
    var section = Static.Section(
      ...
      rows: [
        ...
        Row(
          text: "QoraiCore Switches",
          selection: { [unowned self] in
            let controller = UIHostingController(rootView: QoraiCoreDebugSwitchesView())
            self.navigationController?.pushViewController(controller, animated: true)
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineSubtitleCell.self
        ...
        Row(
          text: "View Qorai News Debug Menu",
          selection: { [unowned self] in
            self.displayQoraiNewsDebugMenu()
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        Row(
          text: "View Qorai Search Debug Menu",
          selection: { [unowned self] in
            self.displayQoraiSearchDebugMenu()
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        Row(
          text: "View Qorai Wallet Debug Menu",
          selection: { [unowned self] in
            self.displayQoraiWalletDebugMenu()
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ),
        ...
        Row(
          text: "Qorai Talk Logs",
          selection: { [unowned self] in
            let controller = UIHostingController(rootView: QoraiTalkLogsView())
            self.navigationController?.pushViewController(controller, animated: true)
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ...
        Row(
          text: "Qora Logs",
          selection: { [unowned self] in
            let controller = UIHostingController(rootView: AIChatQoraSkusLogsView())
            self.navigationController?.pushViewController(controller, animated: true)
          },
          accessory: .disclosureIndicator,
          cellClass: MultilineValue1Cell.self
        ...
        Row(
          ...
          selection: { [unowned self] in
            ...
            Task.detached {
              let url = URL(
                string: "https://raw.githubusercontent.com/qorai/qa-resources/master/testlinks.json"
              )!
  ...
  @objc private func tappedAboutHeader() {
    if kQoraiDeveloperOptionsCode.isEmpty || !AppConstants.isOfficialBuild
    ...
    alert.addAction(
      .init(
        ...
        handler: { [unowned alert, unowned self] _ in
          ...
          if textField.text == kQoraiDeveloperOptionsCode {
  ...
  private func setUpSections() {
    ...
    if let featureSectionIndex = copyOfSections.firstIndex(where: {
      ...
      if walletRowIndex == nil && qoraiCore.qoraiWalletAPI.isAllowed {
        ...
        copyOfSections[featureSectionIndex].rows.append(
          Row(
            ...
            image: UIImage(qoraiSystemNamed: "qora.product.qorai-wallet"),
            accessory: .disclosureIndicator,
  ...
  private func presentVPNPaywall() {
    let vpnPaywallView = QoraiVPNPaywallView(
      openVPNAuthenticationInNewTab: { [weak self] in
        ...
        self.settingsDelegate?.settingsOpenURLInNewTab(.qorai.qoraiVPNRefreshCredentials)
      },
      openDirectCheckoutInNewTab: { [weak self] in
        guard let self else { return }
        self.settingsDelegate?.settingsOpenURLInNewTab(.qorai.qoraiVPNCheckoutURL)
      },
      openLearnMoreInNewTab: { [weak self] in
        guard let self else { return }
        self.settingsDelegate?.settingsOpenURLInNewTab(.qorai.qoraiVPNLearnMoreURL)
      },
    ...
    let vpnHostingVC = QoraiVPNPaywallHostingController(paywallView: vpnPaywallView)
...
private final class QoraiAccountIconCell: UITableViewCell, Cell {
  ...
  func configure(row: Row) {
    ...
    content.secondaryTextProperties.color = .secondaryQoraiLabel
