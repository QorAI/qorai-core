...
class LocationViewPositionPickerCell: UITableViewCell, Cell {
  ...
  private let topBarButton = BarButton(
    image: UIImage(qoraiSystemNamed: "qora.browser.mobile-tabs-top")!,
    text: Strings.tabsOptionTopBar
  )

  private let bottomBarButton = BarButton(
    image: UIImage(qoraiSystemNamed: "qora.browser.mobile-tabs-bottom")!,
    ...
    override var isSelected: Bool {
      didSet {
        checkmarkView.image = UIImage(
          qoraiSystemNamed: isSelected ? "qora.check.circle-outline" : "qora.radio.unchecked"
        )
        checkmarkView.tintColor = isSelected ? .qoraiBlurpleTint : .qoraiDisabled
      }
    }
