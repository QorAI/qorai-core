...
class CookiesAndCacheClearable: Clearable {
  ...
  func clear() async throws {
    ...
    UserDefaults.standard.synchronize()
    for coin in [QoraiWallet.CoinType.eth, QoraiWallet.CoinType.sol] {
...
class HistoryClearable: Clearable {
  let historyAPI: QoraiHistoryAPI
  let httpsUpgradeService: HttpsUpgradeService?

  init(historyAPI: QoraiHistoryAPI, httpsUpgradeService: HttpsUpgradeService?) {
...
class QoraiNewsClearable: Clearable {
  ...
  var label: String {
    return Strings.QoraiNews.qoraiNews
  }
...
class QoraiAdsDataClearable: Clearable {
  ...
  var label: String {
    return Strings.Ads.qoraiAdsDataToggleOption
  }
