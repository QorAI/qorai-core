...
struct FilterListsView: View {
  ...
  enum FilterListUpdateStatus: Equatable {
    ...
    var qoraiImageName: String {
      switch self {
      case .unknown, .updating: "qora.refresh"
      case .updated: "qora.check.circle-outline"
      }
    }

    var forgroundColor: Color {
      switch self {
      case .unknown: Color(.qoraiBlurpleTint)
      case .updated: Color(.qoraiSuccessLabel)
      case .updating: Color(.qoraiDisabled)
      }
    }
  }
    ...
    VStack(alignment: .leading) {
      Button(
        ...
        label: {
          Label(
            status.label,
            qoraiSystemImage: status.qoraiImageName
          )
        }
      )
      ...
      if let error {
        Text(error.localizedDescription)
          .foregroundStyle(Color(.qoraiErrorLabel))
          .font(.caption)
      }
    }
  }
  ...
  @ViewBuilder private var defaultFilterListRows: some View {
    ...
    Toggle(isOn: allEnabled) {
      VStack(alignment: .leading) {
        Text("All").foregroundColor(Color(.qoraiPrimary))
      }
    }
    #endif
