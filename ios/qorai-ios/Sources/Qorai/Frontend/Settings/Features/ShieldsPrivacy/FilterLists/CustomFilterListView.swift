    ...
    ToolbarItem(placement: .confirmationAction) {
      if isSaving {
        ...
        Button(
          ...
          label: {
            Label(Strings.saveButtonTitle, qoraiSystemImage: "qora.check.normal")
...
#Preview {
  CustomFilterListView(
    customRules: .constant(
      ...
      ! Hide the qorai logo on qorai.com (Network test)
      ||qorai.com/static-assets/images/qorai-logo-sans-text.svg
      """
    )
  )
}
...
struct FilterListEditor: UIViewRepresentable {
  ...
  class Coordinator: NSObject, UITextViewDelegate {
    lazy var textView: UITextView = {
      ...
      textView.textColor = UIColor.qoraiLabel
      textView.autocorrectionType = .no
      textView.autocapitalizationType = .none
      textView.keyboardType = .alphabet
      textView.backgroundColor = .secondaryQoraiGroupedBackground
