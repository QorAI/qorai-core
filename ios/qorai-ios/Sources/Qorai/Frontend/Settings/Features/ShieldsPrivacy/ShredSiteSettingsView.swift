        ...
        Button(
          ...
          label: {
            HStack(alignment: .center) {
              ...
              Image(qoraiSystemName: "qora.shred.data")
            }
          }
        ).alert(
...
@MainActor class DomainSettings: ObservableObject {
  ...
  @Published var shredLevel: SiteShredLevel {
    didSet {
      tab.qoraiShieldsHelper?.setShredLevel(shredLevel, for: url)
    }
  }
  ...
  init(url: URL, isPrivate: Bool, tab: some TabState) {
    ...
    self.tab = tab
    self.shredLevel = tab.qoraiShieldsHelper?.shredLevel(for: url) ?? .never
  }
}
