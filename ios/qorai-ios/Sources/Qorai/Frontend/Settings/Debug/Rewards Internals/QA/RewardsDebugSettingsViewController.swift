...
class RewardsDebugSettingsViewController: TableViewController {
  ...
  private func reloadSections() {
    ...
    dataSource.sections = [
      Section(
        ...
        footer: .title(
          "Changing the environment automatically resets Qorai Rewards.\n\nThe app must be force-quit after rewards is reset"
        )
      ),
      Section(
        header: .title("Wallet"),
      ...
      Section(
        ...
        rows: [
          ...
          Row(text: "Estimated Payout", detailText: adsInfo.map { "\($0.amount) QOR" } ?? "—"),
          Row(
  ...
  private func showResetRewardsAlert() {
    let alert = UIAlertController(
      title: "Rewards Reset",
      message: "Qorai must be restarted to ensure expected Rewards behavior",
      preferredStyle: .alert
    )
    alert.addAction(
      UIAlertAction(
    ...
    Task { @MainActor in
      ...
      do {
        ...
        let alert = UIAlertController(
          ...
          message:
            "Qorai must be restarted after importing a database for data to be read from it correctly.",
          preferredStyle: .alert
        )
        alert.addAction(
          UIAlertAction(
