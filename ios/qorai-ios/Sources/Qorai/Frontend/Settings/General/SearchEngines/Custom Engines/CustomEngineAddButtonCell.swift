...
class CustomEngineAddButtonCell: UITableViewCell, TableViewReusable {
  ...
  private func setup() {
    ...
    spinner.do {
      $0.color = UIColor(qoraiSystemName: .dividerInteractive)
      $0.isHidden = true
  ...
  func updateButtonStatus(_ status: CustomEngineViewController.DoneButtonStatus) {
    ...
    case .enabled:
      ...
      doneButton.backgroundColor = UIColor(qoraiSystemName: .buttonBackground)
      doneButton.isEnabled = true
      spinner.stopAnimating()
    case .disabled:
      doneButton.setTitle(Strings.done, for: .normal)
      doneButton.backgroundColor = UIColor(qoraiSystemName: .buttonDisabled)
