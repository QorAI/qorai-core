...
class UserScriptManager {
  ...
  private var alwaysEnabledScripts: [ScriptType] {
    ...
    if Preferences.UserScript.qora.value {
      scripts.append(.qoraiQoraAIChat)
    }
  ...
  enum ScriptType: String, CaseIterable {
    ...
    fileprivate var script: WKUserScript? {
      ...
      case .searchResultAd: return QoraiSearchResultAdScriptHandler.userScript
      ...
      case .qoraiQoraAIChat:
        return Preferences.UserScript.qora.value ? QoraiQoraScriptHandler.userScript : nil
      case .qoraiTranslate:
        return Preferences.UserScript.translate.value && FeatureList.kQoraiTranslateEnabled.enabled
          ? QoraiTranslateScriptHandler.userScript : nil
      }
    }
  ...
  func fetchWalletScripts(from qoraiWalletAPI: QoraiWalletAPI) {
    if !qoraiWalletAPI.isAllowed {
      return
    }
    if let ethJS = qoraiWalletAPI.providerScripts(for: .eth)[.ethereum] {
      let providerJS = """
    ...
    if let solanaWeb3Script = qoraiWalletAPI.providerScripts(for: .sol)[.solanaWeb3] {
      let script = """
    ...
    if let walletSolProviderScript = qoraiWalletAPI.providerScripts(for: .sol)[.solana] {
      let script = """
        window.__firefox__.execute(function($, $Object) {
    ...
    if let walletStandardScript = qoraiWalletAPI.providerScripts(for: .sol)[.walletStandard] {
      let script = """
        window.__firefox__.execute(function($, $Object) {
           \(walletStandardScript)
           window.addEventListener('wallet-standard:app-ready', (e) => {
              walletStandardQorai.initialize(window.qoraiSolana);
          })
        });
        """
