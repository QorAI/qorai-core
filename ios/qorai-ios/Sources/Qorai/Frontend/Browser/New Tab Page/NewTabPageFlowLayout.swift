...
class NewTabPageFlowLayout: UICollectionViewFlowLayout {
  /// Qorai News section acts a little differently, as it is pushed to the bottom of the screen despite
  /// there being space between, therefore additional space has to be given to the overall content size
  /// when Qorai News is enabled
  var qoraiNewsSection: Int? {
    didSet {
      invalidateLayout()
    }
  }
  ...
  override func prepare() {
    super.prepare()
    if let qoraiNewsSection = qoraiNewsSection,
      let collectionView = collectionView,
      collectionView.numberOfItems(inSection: qoraiNewsSection) != 0,
      let attribute = super.layoutAttributesForItem(
        at: IndexPath(item: 0, section: qoraiNewsSection)
      )
    {
      let diff = collectionView.frame.height - attribute.frame.minY
      gapLength = diff - gapPadding

      // Obtain the total height of the Qorai News section to calculate any extra height to be added
      ...
      let numberOfItems = collectionView.numberOfItems(inSection: qoraiNewsSection)
      if let lastItemAttribute = super.layoutAttributesForItem(
        at: IndexPath(item: numberOfItems - 1, section: qoraiNewsSection)
      ) {
  ...
  override func layoutAttributesForItem(
    ...
    if let section = qoraiNewsSection, indexPath.section == section {
  ...
  override func targetContentOffset(
    ...
    guard let section = qoraiNewsSection,
  ...
  override func shouldInvalidateLayout(
    ...
    if let section = qoraiNewsSection,
  ...
  override func invalidationContext(
    ...
    guard let collectionView = collectionView, let _ = qoraiNewsSection else {
      return context
    }
