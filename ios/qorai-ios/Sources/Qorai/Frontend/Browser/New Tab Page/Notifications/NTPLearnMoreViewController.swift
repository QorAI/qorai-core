...
class NTPLearnMoreViewController: BottomSheetViewController {
  ...
  private let termsOfServiceUrl = "https://www.qorai.com/terms_of_use"
  private let learnMoreAboutQoraiRewardsUrl = "https://qorai.com/qorai-rewards/"

  init(state: BrandedImageCalloutState, rewards: QoraiRewards) {
    self.state = state
...
extension NTPLearnMoreViewController: NTPLearnMoreViewDelegate {
  func learnMoreTapped() {
    guard let url = URL(string: learnMoreAboutQoraiRewardsUrl) else { return }
