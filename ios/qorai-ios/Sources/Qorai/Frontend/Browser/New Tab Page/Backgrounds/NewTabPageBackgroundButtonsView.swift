...
class NewTabPageBackgroundButtonsView: UIView, PreferencesObserver {
  ...
  override func layoutSubviews() {
    ...
    let qoraiNewsVisible =
      profilePrefs.isQoraiNewsAvailable && !privateBrowsingManager.isPrivateBrowsing
...
extension NewTabPageBackgroundButtonsView {
  ...
  private class PlayButton: SpringButton {
    let imageView = UIImageView(
      image: UIImage(qoraiSystemNamed: "qora.play.circle")!.withAlignmentRectInsets(.zero)
    ).then {
      $0.tintColor = .white
      $0.contentMode = .scaleAspectFit
