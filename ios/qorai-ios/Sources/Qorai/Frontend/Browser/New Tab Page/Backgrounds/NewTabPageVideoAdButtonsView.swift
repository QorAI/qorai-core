...
class NewTabPageVideoAdButtonsView: UIView {
  ...
  @objc private func backgroundVideoTapped(sender: UITapGestureRecognizer) {
    ...
    if playStarted {
      playPauseButtonImage.imageView.image = UIImage(qoraiSystemNamed: "qora.play.circle")!
        .withAlignmentRectInsets(.zero)
    } else {
      playPauseButtonImage.imageView.image = UIImage(qoraiSystemNamed: "qora.pause.circle")!
        .withAlignmentRectInsets(.zero)
    }
