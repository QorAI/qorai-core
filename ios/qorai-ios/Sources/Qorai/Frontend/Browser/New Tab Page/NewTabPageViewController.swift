...
class NewTabPageViewController: UIViewController {
  ...
  var ntpStatsOnboardingFrame: CGRect? {
    ...
    if let cell = collectionView.cellForItem(at: IndexPath(item: 0, section: section))
      as? NewTabCenteredCollectionViewCell<QoraiShieldStatsView>
    {
  ...
  private var preventReloadOnQoraiNewsEnabledChange = false
  ...
  init(
        ...
        openPrivacyHubPressed: { [weak self] in
          ...
          host.rootView.openPrivacyReportsUrl = { [weak self] in
            self?.delegate?.navigateToInput(
              URL.qorai.privacyFeatures.absoluteString,
              inNewTab: false,
    ...
    if !privateBrowsingManager.isPrivateBrowsing, profilePrefs.isQoraiNewsAvailable {
      sections.append(
        QoraiNewsSectionProvider(
          dataSource: feedDataSource,
          rewards: rewards,
          actionHandler: { [weak self] in
            self?.handleQoraiNewsAction($0)
          }
        )
      )
      layout.qoraiNewsSection = sections.firstIndex(where: { $0 is QoraiNewsSectionProvider })
    }
    ...
    Preferences.QoraiNews.isEnabled.observe(from: self)
    ...
    let qoraiNewsFeatureUsage = P3AFeatureUsage.qoraiNewsFeatureUsage
    if isQoraiNewsVisible && Preferences.QoraiNews.isEnabled.value {
      qoraiNewsFeatureUsage.recordHistogram()
      recordQoraiNewsDaysUsedP3A()
    }

    recordNewTabCreatedP3A()
    recordQoraiNewsWeeklyUsageCountP3A()
  }
  ...
  override func viewDidLoad() {
    ...
    feedOverlayView.headerView.settingsButton.addTarget(
      self,
      action: #selector(tappedQoraiNewsSettings),
      for: .touchUpInside
    )
    if !AppConstants.isOfficialBuild {
      ...
      feedOverlayView.headerView.settingsButton.addGestureRecognizer(
        UILongPressGestureRecognizer(
          target: self,
          action: #selector(longPressedQoraiNewsSettingsButton)
        )
      )
    }
  ...
  private func reportSponsoredBackgroundEvent(
    _ event: QoraiAds.NewTabPageAdEventType,
  ...
  private func handleQoraiNewsAction(_ action: QoraiNewsSectionProvider.Action) {
    switch action {
    case .optInCardAction(.closedButtonTapped):
      Preferences.QoraiNews.isShowingOptIn.value = false
      if let section = layout.qoraiNewsSection,
    ...
    case .optInCardAction(.learnMoreButtonTapped):
      delegate?.navigateToInput(
        URL.qorai.qoraiNewsPrivacy.absoluteString,
        inNewTab: false,
        switchingToPrivateMode: false
      )
    case .optInCardAction(.turnOnQoraiNewsButtonTapped):
      preventReloadOnQoraiNewsEnabledChange = true
      Preferences.QoraiNews.userOptedIn.value = true
      Preferences.QoraiNews.isEnabled.value = true
    ...
    case .emptyCardTappedSourcesAndSettings:
      tappedQoraiNewsSettings()
    case .errorCardTappedRefresh:
      loadFeedContents()
    case .moreQoraiOffersTapped:
      delegate?.navigateToInput(
        URL.qorai.qoraiOffers.absoluteString,
        inNewTab: false,
        switchingToPrivateMode: false
      )
    case .qoraiPartnerLearnMoreTapped:
      delegate?.navigateToInput(
        URL.qorai.qoraiNews.absoluteString,
        inNewTab: false,
        switchingToPrivateMode: false
      )
    case .itemAction(.opened(let inNewTab, let switchingToPrivateMode), let context):
      ...
      // Donate Open Qorai News Activity for Custom Suggestions
      let openQoraiNewsActivity = ActivityShortcutManager.shared.createShortcutActivity(
        type: .openQoraiNews
      )
      self.userActivity = openQoraiNewsActivity
      openQoraiNewsActivity.becomeCurrent()
      // Record P3A
      newsArticlesOpened.insert(item.id)
      recordQoraiNewsArticlesVisitedP3A()
  ...
  private func handleFeedStateChange(
    ...
    guard let section = layout.qoraiNewsSection, parent != nil else { return }

    func _completeLoading() {
      if Preferences.QoraiNews.isShowingOptIn.value {
        Preferences.QoraiNews.isShowingOptIn.value = false
      }
      UIView.animate(
        withDuration: 0.2,
    ...
    case (
      ...
      if let cell = collectionView.cellForItem(at: IndexPath(item: 0, section: section))
        as? FeedCardCell<QoraiNewsErrorView>
      {
  ...
  @objc private func checkForUpdatedFeed() {
    if !isQoraiNewsVisible || Preferences.QoraiNews.isShowingOptIn.value { return }
    if collectionView.contentOffset.y == collectionView.contentInset.top {
  ...
  @objc private func tappedQoraiNewsSettings() {
  ...
  @objc private func longPressedQoraiNewsSettingsButton() {
    assert(
      !AppConstants.isOfficialBuild,
      "Debug settings are not accessible on public builds"
    )
    let settings = QoraiNewsDebugSettingsView(dataSource: feedDataSource) { [weak self] in
      self?.dismiss(animated: true)
    }
...
extension NewTabPageViewController: PreferencesObserver {
  func preferencesDidChange(for key: String) {
    ...
    if !preventReloadOnQoraiNewsEnabledChange {
      collectionView.reloadData()
    }
    if !isQoraiNewsVisible {
      collectionView.verticalScrollIndicatorInsets = .zero
      feedOverlayView.headerView.alpha = 0.0
      backgroundButtonsView.alpha = 1.0
    }
    preventReloadOnQoraiNewsEnabledChange = false
  }
}

// MARK: - UIScrollViewDelegate
extension NewTabPageViewController {
  var isQoraiNewsVisible: Bool {
    return profilePrefs.isQoraiNewsAvailable && !privateBrowsingManager.isPrivateBrowsing
      && (Preferences.QoraiNews.isEnabled.value || Preferences.QoraiNews.isShowingOptIn.value)
  }
  ...
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    guard isQoraiNewsVisible, let newsSection = layout.qoraiNewsSection else { return }
    if collectionView.numberOfItems(inSection: newsSection) > 0 {
      // Hide the buttons as Qorai News feeds appear
      backgroundButtonsView.alpha =
        1.0 - max(0.0, min(1.0, (scrollView.contentOffset.y - scrollView.contentInset.top) / 16))
      // Show the header as Qorai News feeds appear
      // Offset of where Qorai News starts
      let todayStart =
        collectionView.frame.height - feedOverlayView.headerView.bounds.height - 32 - 16
      ...
      if scrollView.contentOffset.y >= todayStart {
        recordQoraiNewsUsageP3A()
      }
    }
  }

  /// Moves New Tab Page Scroll to start of Qorai News - Used for shortcut
  func scrollToQoraiNews() {
    if !profilePrefs.isQoraiNewsAvailable {
      return
    }
    // Offset of where Qorai News starts
    let todayStart =
  ...
  private func recordQoraiNewsUsageP3A() {
    let qoraiNewsFeatureUsage = P3AFeatureUsage.qoraiNewsFeatureUsage
    if !isQoraiNewsVisible || !Preferences.QoraiNews.isEnabled.value
      || Calendar.current.startOfDay(for: Date()) == qoraiNewsFeatureUsage.lastUsageOption.value
    {
      // Don't have Qorai News enabled, or already recorded todays usage, no need to do it again
      return
    }

    // Usage
    qoraiNewsFeatureUsage.recordUsage()
    var qoraiNewsWeeklyCount = P3ATimedStorage<Int>.qoraiNewsWeeklyCount
    qoraiNewsWeeklyCount.add(value: 1, to: Date())

    // Usage over the past month
    var qoraiNewsDaysUsedStorage = P3ATimedStorage<Int>.qoraiNewsDaysUsedStorage
    qoraiNewsDaysUsedStorage.replaceTodaysRecordsIfLargest(value: 1)
    recordQoraiNewsDaysUsedP3A()
    ...
    // General Usage
    UmaHistogramBoolean("Qorai.Today.UsageDaily", true)
    UmaHistogramBoolean("Qorai.Today.UsageMonthly", true)
  }

  private func recordQoraiNewsWeeklyUsageCountP3A() {
    let storage = P3ATimedStorage<Int>.qoraiNewsWeeklyCount
  ...
  private func recordQoraiNewsDaysUsedP3A() {
    let storage = P3ATimedStorage<Int>.qoraiNewsDaysUsedStorage
    UmaHistogramRecordValueToBucket(
      "Qorai.Today.DaysInMonthUsedCount",
      buckets: [
        0,
        1,
        2,
  ...
  private func recordQoraiNewsArticlesVisitedP3A() {
    // Count is per NTP session, sends max value of the week
    var storage = P3ATimedStorage<Int>.qoraiNewsVisitedArticlesCount
    storage.replaceTodaysRecordsIfLargest(value: newsArticlesOpened.count)
    UmaHistogramRecordValueToBucket(
      "Qorai.Today.WeeklyMaxCardVisitsCount",
      buckets: [
  ...
  private func recordNewTabCreatedP3A() {
    ...
    if newTabsCreatedAnswer > 0 {
      ...
      UmaHistogramRecordValueToBucket(
        "Qorai.NTP.SponsoredNewTabsCreated",
        buckets: [
          0,
          .r(0..<10),
          .r(10..<20),
...
extension P3AFeatureUsage {
  fileprivate static var qoraiNewsFeatureUsage: Self = .init(
    name: "qorai-news-usage",
    histogram: "Qorai.Today.LastUsageTime",
    returningUserHistogram: "Qorai.Today.NewUserReturning"
  )
}

extension P3ATimedStorage where Value == Int {
  fileprivate static var qoraiNewsDaysUsedStorage: Self {
    .init(name: "qorai-news-days-used", lifetimeInDays: 30)
  }
  fileprivate static var qoraiNewsWeeklyCount: Self {
    .init(name: "qorai-news-weekly-usage", lifetimeInDays: 7)
  }
  fileprivate static var qoraiNewsVisitedArticlesCount: Self {
    .init(name: "qorai-news-weekly-clicked", lifetimeInDays: 7)
  }
