...
class StatsSectionProvider: NSObject, NTPSectionProvider {
  ...
  @objc private func tappedButton(_ gestureRecognizer: UIGestureRecognizer) {
    guard let cell = gestureRecognizer.view as? QoraiShieldStatsView else {
      return
    }
  ...
  func registerCells(to collectionView: UICollectionView) {
    collectionView.register(NewTabCenteredCollectionViewCell<QoraiShieldStatsView>.self)
  }
...
class QoraiShieldStatsView: SpringButton {
  var openPrivacyHubPressed: (() -> Void)?
  var hidePrivacyHubPressed: (() -> Void)?
  ...
  override init(frame: CGRect) {
    ...
    NotificationCenter.default.addObserver(
      ...
      name: NSNotification.Name(rawValue: QoraiGlobalShieldStats.didUpdateNotification),
      object: nil
    )
  }
  ...
  var isPrivateBrowsing: Bool = false {
    didSet {
      ...
      if !isPrivateBrowsing && Preferences.NewTabPage.showNewTabPrivacyHub.value {
        ...
        let settingsButton = QoraiButton(type: .system).then {
          $0.setImage(
  ...
  @objc private func update() {
    adsStatView.stat =
      ...
      .kFormattedNumber
    dataSavedStatView.stat = QoraiGlobalShieldStats.shared.dataSaved
    timeStatView.stat = QoraiGlobalShieldStats.shared.timeSaved
  }
}
