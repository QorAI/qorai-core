...
/// The section provider for Qorai News.
class QoraiNewsSectionProvider: NSObject, NTPObservableSectionProvider {
  /// Set of actions that can occur from the Qorai News section
  enum Action {
    ...
    /// The user tapped to show more qorai offers on one of the deal cards
    case moreQoraiOffersTapped
    /// The user tapped to learn more about qorai partners
    ...
    /// The user performed an action on an Rate qorai card
  ...
  func registerCells(to collectionView: UICollectionView) {
    collectionView.register(FeedCardCell<QoraiNewsOptInView>.self)
    collectionView.register(FeedCardCell<QoraiNewsErrorView>.self)
    collectionView.register(FeedCardCell<QoraiNewsEmptyFeedView>.self)
  ...
  private var isShowingOptInCard: Bool {
    Preferences.QoraiNews.isShowingOptIn.value
  }
  ...
  func collectionView(
    ...
    if indexPath.item == 0, let cell = cell as? FeedCardCell<QoraiNewsOptInView> {
  ...
  func collectionView(
    ...
    if let error = dataSource.state.error {
      let cell =
        collectionView.dequeueReusableCell(for: indexPath) as FeedCardCell<QoraiNewsErrorView>
      if let urlError = error as? URLError, urlError.code == .notConnectedToInternet {
        cell.content.titleLabel.text = Strings.QoraiNews.errorNoInternetTitle
        cell.content.errorMessageLabel.text = Strings.QoraiNews.errorNoInternetBody
      } else {
        cell.content.titleLabel.text = Strings.QoraiNews.errorGeneralTitle
        cell.content.errorMessageLabel.text = Strings.QoraiNews.errorGeneralBody
      }
    ...
    if isShowingOptInCard && indexPath.item == 0 {
      let cell =
        collectionView.dequeueReusableCell(for: indexPath) as FeedCardCell<QoraiNewsOptInView>
      cell.content.optInCardActionHandler = { [weak self] action in
        if action == .turnOnQoraiNewsButtonTapped && !cell.content.turnOnQoraiNewsButton.isLoading {
          cell.content.turnOnQoraiNewsButton.isLoading = true
        }
    ...
    if let cards = dataSource.state.cards, cards.isEmpty {
      let cell =
        collectionView.dequeueReusableCell(for: indexPath) as FeedCardCell<QoraiNewsEmptyFeedView>
    ...
    guard let card = dataSource.state.cards?[safe: indexPath.item] else {
      ...
      return collectionView.dequeueReusableCell(for: indexPath)
        as FeedCardCell<QoraiNewsEmptyFeedView>
    }
    ...
    case .deals(let items, let title):
      ...
      let title = title ?? Strings.QoraiNews.deals
      ...
      cell.content.moreOffersButtonTapped = { [weak self] in
        self?.actionHandler(.moreQoraiOffersTapped)
      }
    ...
    case .partner(let item):
      ...
      cell.content.promotedButtonTapped = { [weak self] in
        self?.actionHandler(.qoraiPartnerLearnMoreTapped)
      }
      return cell
    case .ad(let ad):
    ...
    return .init { index -> UIMenu? in
      ...
      var openInNewTab: UIAction {
        .init(
          ...
          image: UIImage(qoraiSystemNamed: "qora.browser.mobile-tab-new"),
  ...
  private func contextMenu(
    ...
    return .init { [weak self] index -> UIMenu? in
      ...
      var disableSource: UIAction {
        .init(
          title: String(format: Strings.QoraiNews.disablePublisherContent, item.source.name),
      ...
      var enableSource: UIAction {
        .init(
          title: String(format: Strings.QoraiNews.enablePublisherContent, item.source.name),
          image: UIImage(qoraiSystemNamed: "qora.eye.on"),
          handler: mapDeferredHandler(toggleSourceHandler)
        )
      }
  ...
  private func recordWeeklyAdsViewedP3A(adViewed: Bool) {
    ...
    UmaHistogramRecordValueToBucket(
      "Qorai.Today.WeeklyDisplayAdsViewedCount",
      buckets: [
        0,
        1,
        .r(2...4),
  ...
  private func recordWeeklyMaxRowsViewedP3A() {
    ...
    UmaHistogramRecordValueToBucket(
      "Qorai.Today.WeeklyMaxCardViewsCount",
      buckets: [
