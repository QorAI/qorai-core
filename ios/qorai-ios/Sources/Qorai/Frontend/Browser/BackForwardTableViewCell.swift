...
class BackForwardTableViewCell: UITableViewCell {
  ...
  private struct BackForwardViewCellUX {
    ...
    static let textColor: UIColor = .qoraiLabel
  }

  lazy var faviconView: UIImageView = {
    let faviconView = UIImageView(image: UIImage(sharedNamed: "qorai.logo"))
    faviconView.backgroundColor = .qoraiBackground
  ...
  let line = UIView().then {
    $0.backgroundColor = .qoraiSeparator
  }
  ...
  var isCurrentTab = false {
    didSet {
      if isCurrentTab {
        ...
        contentView.backgroundColor = .secondaryQoraiBackground
      }
    }
  }
    ...
    didSet {
      if let s = site {
        if InternalURL.isValid(url: s.tileURL) {
          ...
          faviconView.image = UIImage(sharedNamed: "qorai.logo")
        } else {
