  ...
  var body: some View {
    Section(
      ...
      ForEach(values, id: \.self) {
        $0.listRowBackground(Color(.secondaryQoraiGroupedBackground))
      }
    }
  }
}
...
private struct CertificateView: View {
  let model: QoraiCertificateModel
  let evaluationError: String?
    ...
    CertificateSectionView(
      ...
      values: [
        ...
        CertificateKeyValueView(
          ...
          value:
            "\(signatureAlgorithmDescription) (\(model.signature.absoluteObjectIdentifier.isEmpty ? QoraiCertificateUtils.oidToAbsoluteOID(oid: model.signature.objectIdentifier) : model.signature.absoluteObjectIdentifier))"
        ),
    ...
    CertificateSectionView(
      ...
      values: [
        ...
        CertificateKeyValueView(
          ...
          value: QoraiCertificateUtils.formatDate(model.notValidBefore)
        ),

        // Not Valid After
        CertificateKeyValueView(
          title: Strings.CertificateViewer.notValidAfterTitle,
          value: QoraiCertificateUtils.formatDate(model.notValidAfter)
        ),
      ]
    )
...
extension CertificateView {
  ...
  private var formattedSerialNumber: String {
    ...
    return QoraiCertificateUtils.formatHex(model.serialNumber)
  }

  private func rdnsSequenceViews(rdns: QoraiCertificateRDNSequence) -> [CertificateKeyValueView] {
    // Ordered mapping
    var mapping = [
  ...
  private func signatureParametersView() -> CertificateKeyValueView {
    ...
    let parameters =
      ...
      ? Strings.CertificateViewer.noneTitle : QoraiCertificateUtils.formatHex(signature.parameters)
  ...
  private func publicKeyInfoViews() -> [CertificateKeyValueView] {
    ...
    if !algorithm.isEmpty {
      ...
      if publicKeyInfo.absoluteObjectIdentifier.isEmpty {
        algorithm +=
          " (\(QoraiCertificateUtils.oidToAbsoluteOID(oid: publicKeyInfo.objectIdentifier)))"
      } else {
    ...
    let parameters =
      ...
      : "\(publicKeyInfo.parameters.count / 2) \(Strings.CertificateViewer.bytesUnitTitle) : \(QoraiCertificateUtils.formatHex(publicKeyInfo.parameters))"

    let publicKey =
      "\(publicKeyInfo.keyBytesSize) \(Strings.CertificateViewer.bytesUnitTitle) : \(QoraiCertificateUtils.formatHex(publicKeyInfo.keyHexEncoded))"
  ...
  private func formattedSignature() -> String {
    let signature = model.signature
    return
      "\(signature.bytesSize) \(Strings.CertificateViewer.bytesUnitTitle) : \(QoraiCertificateUtils.formatHex(signature.signatureHexEncoded))"
  }
  ...
  private func fingerprintViews() -> [CertificateKeyValueView] {
    ...
    return [
      CertificateKeyValueView(
        ...
        value: QoraiCertificateUtils.formatHex(sha256Fingerprint.fingerprintHexEncoded)
      ),
      CertificateKeyValueView(
        title: Strings.CertificateViewer.sha1Title,
        value: QoraiCertificateUtils.formatHex(sha1Fingerprint.fingerprintHexEncoded)
      ),
    ]
  }
...
class CertificateViewController: UIViewController, PopoverContentComponent {
  ...
  init(certificate: QoraiCertificateModel, evaluationError: String?) {
