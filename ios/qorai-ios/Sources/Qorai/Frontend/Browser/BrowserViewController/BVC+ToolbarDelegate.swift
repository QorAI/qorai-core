...
extension BrowserViewController: TopToolbarDelegate {
  ...
  func processAddressBar(
    text: String,
    isQoraiSearchPromotion: Bool = false,
    ...
    processAddressBarTask = Task { @MainActor in
        ...
        await submitValidURL(text, isUserDefinedURLNavigation: isUserDefinedURLNavigation)
      {
        ...
        submitSearchText(text, isQoraiSearchPromotion: isQoraiSearchPromotion)
  ...
  func topToolbarIsShieldsEnabled(_ topToolbar: TopToolbarView, for url: URL?) -> Bool {
    ...
    return currentTab.qoraiShieldsHelper?.isQoraiShieldsEnabled(for: url) ?? false
  }
  ...
  @MainActor private func submitValidURL(
    ...
    if let url = URL(string: text), url.scheme == "qorai" || url.scheme == "chrome" {
  ...
  /// Handles displaying a Chromium web view for qorai:// url that would display WebUI
  func handleChromiumWebUIURL(_ url: URL) -> Bool {
    ...
    let controller = ChromeWebUIController(qoraiCore: profileController, isPrivateBrowsing: false)
  ...
  func topToolbarDidTapQoraiShieldsButton(_ topToolbar: TopToolbarView) {
    presentQoraiShieldsView()
  }

  func presentQoraiShieldsView() {
  ...
  private func changedShieldSettings() {
    ...
    browsers.forEach { browser in
      ...
      // Reload the tabs. This will also trigger an update of the qorai icon in `TabLocationView` if
    ...
    let controller = UIHostingController(
      rootView: AdvancedShieldsSettingsView(
        settings: AdvancedShieldsSettings(
          ...
          qoraiCore: profileController,
          p3aUtils: qoraiCore.p3aUtils,
          rewards: rewards,
          qoraiStats: profileController.qoraiStats,
  ...
  // See: https://github.com/qorai/qorai-ios/issues/1452
  ...
  func topToolbarDidTapQoraiRewardsButton(_ topToolbar: TopToolbarView) {
    showQoraiRewardsPanel()
  }
  ...
  private func showSearchController() {
    ...
    let searchDataSource = SearchSuggestionDataSource(
      isPrivate: isPrivate,
      isAIChatAvailable: !isPrivate && Preferences.AIChat.qoraInQuickSearchBarEnabled.value
...
extension BrowserViewController: UIContextMenuInteractionDelegate {
  ...
  private func makeReloadMenu() -> UIMenu? {
    ...
    let reloadTitle =
      tab.currentUserAgentType == .desktop
      ? Strings.appMenuViewMobileSiteTitleString : Strings.appMenuViewDesktopSiteTitleString
    let reloadIcon = tab.currentUserAgentType == .desktop ? "qora.smartphone" : "qora.monitor"
    let reloadAction = UIAction(
      title: reloadTitle,
      image: UIImage(qoraiSystemNamed: reloadIcon),
  ...
  private func makeCopyMenu() -> UIMenu? {
    ...
    let children: [UIAction] = [
      UIAction(
        ...
        image: UIImage(qoraiSystemNamed: "qora.copy"),
        handler: UIAction.deferredActionHandler { _ in
      ...
      UIAction(
        ...
        image: UIImage(qoraiSystemNamed: "qora.copy.clean"),
        handler: UIAction.deferredActionHandler { _ in
