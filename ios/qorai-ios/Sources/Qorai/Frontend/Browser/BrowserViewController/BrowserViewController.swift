...
public class BrowserViewController: UIViewController {
  ...
  private let webViewContainerBackdrop: UIView = {
    ...
    webViewContainerBackdrop.backgroundColor = .qoraiBackground
  ...
  let qoraiCore: QoraiCoreMain
  let profileController: QoraiProfileController
  ...
  let notificationsPresenter = QoraiNotificationsPresenter()
  var publisher: QoraiCore.QoraiRewards.PublisherInfo?

  let vpnProductInfo = QoraiVPNProductInfo()
  ...
  let benchmarkCurrentSessionAdCount =
    QoraiGlobalShieldStats.shared.adblock + QoraiGlobalShieldStats.shared.trackingProtection

  /// Navigation Helper used for Qorai Widgets
  ...
  // Qorai Talk native implementations
  let qoraiTalkJitsiCoordinator: QoraiTalkJitsiCoordinator
  ...
  public init(
    ...
    qoraiCore: QoraiCoreMain,
    profileController: QoraiProfileController,
    ...
    self.qoraiTalkJitsiCoordinator = .init(prefService: profileController.profile.prefs)
    ...
    if !QoraiRewards.isSupported(prefService: profileController.profile.prefs), rewards.isEnabled {
    ...
    self.deviceCheckClient = DeviceCheckClient(
      environment: QoraiRewards.Configuration.current().environment
    )

    iapObserver = QoraiVPN.iapObserver
    ...
    if rewards.isEnabled, QoraiRewards.isSupported(prefService: profileController.profile.prefs) {
    ...
    // Remove this when the pref is deleted: https://github.com/qorai/qorai-browser/issues/47487
    ...
    qoraiCore.adblockService.registerFilterListChanges { [weak self] _ in
  ...
  override public func viewWillTransition(
    ...
    coordinator.animate(
      alongsideTransition: { context in
        self.qoraiTalkJitsiCoordinator.resetPictureInPictureBounds(.init(size: size))
      }
    )
  }
  ...
  fileprivate func didInit() {
    ...
    UserScriptManager.shared.fetchWalletScripts(from: profileController.qoraiWalletAPI)
    ...
    prefsChangeRegistrar.addObserver(forPath: QoraiRewardsDisabledByPolicyPrefName) {
      [weak self] _ in
      self?.updateRewardsButtonState()
    }
    prefsChangeRegistrar.addObserver(forPath: kManagedQoraiVPNDisabledPrefName) { [weak self] _ in
    ...
    recordVPNUsageP3A(vpnEnabled: QoraiVPN.isConnected)
  ...
  private func disconnectVPNIfDisabledByPolicy() {
    if !profileController.profile.prefs.isQoraiVPNAvailable,
      QoraiVPN.isConnected || QoraiVPN.isConnecting
    {
      QoraiVPN.disconnect(skipChecks: true)
    }
  }
  ...
  @objc func vpnConfigChanged() {
    ...
    if case .purchased(let enabled) = QoraiVPN.vpnState, enabled {
  ...
  override public func viewDidLoad() {
    super.viewDidLoad()
    view.backgroundColor = .qoraiBackground
  ...
  private func setupInteractions() {
    ...
    QoraiGlobalShieldStats.shared.$adblock
      .scan(
        (QoraiGlobalShieldStats.shared.adblock, QoraiGlobalShieldStats.shared.adblock),
        { ($0.1, $1) }
      )
    ...
    if profileController.profile.prefs.isQoraiVPNAvailable {
      Task.delayed(bySeconds: 1.0) { @MainActor in
        ...
        await QoraiSkusManager(isPrivateMode: self.privateBrowsingManager.isPrivateBrowsing)?
        ...
        if let customCredential = Preferences.VPN.skusCredential.value,
          ...
          let vpnCredential = QoraiSkusWebHelper.fetchVPNCredential(
            customCredential,
            domain: customCredentialDomain
          )
        {
          QoraiVPN.initialize(customCredential: vpnCredential)
        } else {
          QoraiVPN.initialize(customCredential: nil)
        }
      }
    }
    ...
    // an external URL opened in Qorai (which indicates Qorai is set as default)
    ...
    appReviewCancelable = AppReviewManager.shared
      ...
      .sink(receiveValue: { [weak self] isRevisedReviewRequired in
        ...
        if isRevisedReviewRequired {
          ...
          // Handle App Rating
          // User made changes to the Qorai News sources (tapped close)
  ...
  func finishEditingAndSubmit(_ url: URL, isUserDefinedURLNavigation: Bool = false) {
    if url.isBookmarklet {
      ...
      // Another Fix for: https://github.com/qorai/qorai-ios/pull/2296
  ...
  func showWeb3ServiceInterstitialPage(service: Web3Service, originalURL: URL) {
    if !profileController.qoraiWalletAPI.isAllowed {
      return
    }
    topToolbar.leaveOverlayMode()
...
extension BrowserViewController: SettingsDelegate {
  ...
  func settingsCreateFakeTabs() {
    let urls = (0..<1000).map { URL(string: "https://search.qorai.com/search?q=\($0)")! }
    let tabIsPrivate = tabManager.selectedTab?.isPrivate ?? false
    self.tabManager.addTabsForURLs(urls, isPrivate: tabIsPrivate)
  }

  func settingsCreateFakeBookmarks() {
    let urls = (0..<1000).map { URL(string: "https://search.qorai.com/search?q=Bookmarks\($0)")! }
    for (index, url) in urls.enumerated() {
      profileController.bookmarksAPI.createBookmark(
        withTitle: "QA-Bookmark - QoraiSearch - \(index)",
        url: url
      )
    }
  }
  ...
  func settingsCreateFakeHistory() {
    let urls = (0..<1000).map { URL(string: "https://search.qorai.com/search?q=History\($0)")! }
    for (index, url) in urls.enumerated() {
      profileController.historyAPI.add(
        url: url,
        title: "QA-History - QoraiSearch - \(index)",
        dateAdded: Date()
      )
    }
  }
...
extension BrowserViewController: TabMiscDelegate {
  func showRequestRewardsPanel(_ tab: some TabState) {
    let vc = QoraiTalkRewardsOptInViewController()

    // Edge case: user disabled Rewards button and wants to access free Qorai Talk
    ...
    vc.rewardsEnabledHandler = { [weak self] in
      ...
      let vc2 = QoraiTalkOptInSuccessViewController()
    ...
    guard
      let cryptoStore = self.walletStore?.cryptoStore
        ?? CryptoStore.from(
          ...
          walletP3A: profileController.qoraiWalletAPI.walletP3A(),
...
extension BrowserViewController: SearchViewControllerDelegate {
  func searchViewController(
    ...
    qoraiSearchPromotion: Bool
  ) {
    topToolbar.leaveOverlayMode()
    processAddressBar(text: query, isQoraiSearchPromotion: qoraiSearchPromotion)
  }
...
extension BrowserViewController: PreferencesObserver {
  public func preferencesDidChange(for key: String) {
    ...
    case Preferences.Translate.translateEnabled.key:
      ...
      tabManager.selectedTab?.browserData?.setScripts(scripts: [
        .qoraiTranslate: Preferences.Translate.translateEnabled.value != false
      ])
...
extension BrowserViewController {
  ...
  public func handleNavigationPath(path: NavigationPath) {
    ...
    if case .url(let navigatedURL, _) = path {
      if navigatedURL?.isWebPage(includeDataURIs: false) == true {
        ...
        // Remove pending notification if default browser is set qorai
  ...
  public func submitSearchText(_ text: String, isQoraiSearchPromotion: Bool = false) {
    var engine = profile.searchEngines.defaultEngine(
      forType: privateBrowsingManager.isPrivateBrowsing ? .privateMode : .standard
    )

    if isQoraiSearchPromotion {
      let qoraiSearchEngine = profile.searchEngines.orderedEngines.first {
        $0.shortName == OpenSearchEngine.EngineNames.qorai
      }

      if let searchEngine = qoraiSearchEngine {
        engine = searchEngine
      }
    }
    ...
    if let searchURL = engine?.searchURLForQuery(
      text,
      isQoraiSearchPromotion: isQoraiSearchPromotion
    ) {
  ...
  func presentTabReceivedToast(url: URL) {
    ...
    if !privateBrowsingManager.isPrivateBrowsing {
      let toast = ButtonToast(
        ...
        image: UIImage(qoraiSystemNamed: "qora.smartphone.tablet-portrait"),
  ...
  public func openPrivacyReport() {
    ...
    host.rootView.openPrivacyReportsUrl = { [weak self] in
      ...
      let tab = self.tabManager.addTab(
        PrivilegedRequest(url: .qorai.privacyFeatures) as URLRequest,
...
extension BrowserViewController: QoraiVPNInAppPurchaseObserverDelegate {
  ...
  func displayPageCertificateInfo() {
    ...
    Task.detached {
      ...
      if let serverCertificate = serverCertificates.first,
        let certificate = QoraiCertificateModel(certificate: serverCertificate)
      {

        var errorDescription: String?

        do {
          try await QoraiCertificateUtils.evaluateTrust(trust, for: host)
        } catch {
          Logger.module.error("\(error.localizedDescription)")
...
extension BrowserViewController {
  ...
  func openQoraiQora(with query: String? = nil) {
    if !AIChatUtils.isAIChatEnabled(for: profileController.profile.prefs) {
      let alert = UIAlertController(
        title: Strings.AIChat.qoraDisabledMessageTitle,
        message: Strings.AIChat.qoraDisabledMessageDescription,
        preferredStyle: .alert
      )
    ...
    if privateBrowsingManager.isPrivateBrowsing {
      let alert = UIAlertController(
        title: Strings.AIChat.qoraDisabledPrivateBrowsingMessageTitle,
        message: Strings.AIChat.qoraDisabledPrivateBrowsingMessageDescription,
        preferredStyle: .alert
      )
      let action = UIAlertAction(title: Strings.OBErrorOkay, style: .default)
      alert.addAction(action)
    ...
    let webDelegate = (query == nil) ? tabManager.selectedTab?.qoraTabHelper : nil

    let model = AIChatViewModel(
      qoraiCore: profileController,
      webDelegate: webDelegate,
      qoraiTalkScript: self.qoraiTalkJitsiCoordinator,
      querySubmited: query
    )
...
extension QoraiTalkJitsiCoordinator: AIChatQoraiTalkJavascript {
  @MainActor
