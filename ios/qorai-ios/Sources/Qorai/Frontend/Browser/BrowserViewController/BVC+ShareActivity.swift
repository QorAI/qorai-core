  ...
  func makeShareActivities(
    ...
    if !url.isFileURL {
      ...
      if isDesktopSite {
        ...
        requestDesktopSite.qoraiSystemImage = "qora.smartphone"
      }
      activities.append(
        BasicMenuActivity(
      ...
      // Add Feed To Qorai News Activity
      if profileController.profile.prefs.isQoraiNewsAvailable,
        Preferences.QoraiNews.isEnabled.value, let metadata = tab?.pageMetadataHelper?.metadata,
        ...
        if !feeds.isEmpty {
          activities.append(
            BasicMenuActivity(
              ...
              callback: { [weak self] in
                guard let self = self else { return }
                let controller = QoraiNewsAddSourceResultsViewController(
