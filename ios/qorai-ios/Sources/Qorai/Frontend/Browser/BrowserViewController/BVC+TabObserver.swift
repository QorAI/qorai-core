...
extension BrowserViewController: TabObserver {
  public func tabDidCreateWebView(_ tab: some TabState) {
    ...
    var injectedScripts: [TabContentScript] = [
      ...
      FocusScriptHandler(),
      QoraiGetUA(),
      QoraiSearchScriptHandler(profile: profile, rewards: rewards),
      ...
      QoraiQoraScriptHandler(),
      QoraiSkusScriptHandler(),
    ...
    if profileController.profile.prefs.isQoraiTalkAvailable {
      injectedScripts.append(
        QoraiTalkScriptHandler(
          rewards: rewards,
          launchNativeQoraiTalk: { [weak self] tab, room, token in
            self?.launchNativeQoraiTalk(tab: tab, room: room, token: token)
          }
        )
      )
    }
    ...
    if !tab.isPrivate {
      injectedScripts += [
        ...
        QoraiSearchResultAdScriptHandler(),
      ]
      if profileController.qoraiWalletAPI.isAllowed {
        injectedScripts += [
    ...
    if FeatureList.kQoraiTranslateEnabled.enabled {
      injectedScripts.append(contentsOf: [
        QoraiTranslateScriptLanguageDetectionHandler(),
        QoraiTranslateScriptHandler(),
      ])
    }
  ...
  public func tabDidCommitNavigation(_ tab: some TabState) {
    ...
    // https://github.com/qorai/qorai-core/blob/1.52.x/browser/qorai_content_browser_client.cc#L608
    if profileController.qoraiWalletAPI.isAllowed {
      tab.browserData?.clearSolanaConnectedAccounts()

      if let browserData = tab.browserData {
        if let provider = profileController.qoraiWalletAPI.ethereumProvider(
          with: browserData,
        ...
        if let provider = profileController.qoraiWalletAPI.solanaProvider(
          with: browserData,
          isPrivateBrowsing: tab.isPrivate
        ) {
  ...
  public func tabDidFinishNavigation(_ tab: some TabState) {
    ...
    // Inject app's IAP receipt for Qorai SKUs if necessary
    if !tab.isPrivate {
      Task { @MainActor in
        await QoraiSkusAccountLink.injectLocalStorage(tab: tab)
      }
    }

    // Second attempt to inject results to the QoraiSearch.
    ...
    if let qoraiSearchManager = tab.qoraiSearchManager {
      ...
      if !qoraiSearchManager.fallbackQueryResultsPending {
        tab.injectResults()
      }
    } else {
    ...
    if let lastCommittedURL = tab.lastCommittedURL {
      maybeRecordQoraiSearchDailyUsage(url: lastCommittedURL)
    }
  ...
  public func tabDidUpdateURL(_ tab: some TabState) {
    ...
    if let url = tab.visibleURL, !url.isLocal {
      // Notify Qorai Rewards library of the same document navigation.
