...
extension BrowserViewController: TabDelegate {
      ...
      if let currentTab = self.tabManager.selectedTab {
        ...
        if UIApplication.shared.supportsMultipleScenes {
          ...
          let openNewPrivateWindowAction = UIAction(
            ...
            image: UIImage(qoraiSystemNamed: "qora.window.tab-private")
          ) { _ in
            if !isPrivate, Preferences.Privacy.privateBrowsingLock.value {
              self.askForLocalAuthentication { [weak self] success, error in
                if success {
        ...
        let shareAction = UIAction(
          ...
          image: UIImage(qoraiSystemNamed: "qora.share.macos")
        ) { _ in
          // TODO: Find a way to add fixes qorai-ios#3323 and qorai-ios#2961 here:
        ...
        let linkPreviewAction = UIAction(
          ...
          image: UIImage(qoraiSystemNamed: linkPreview ? "qora.eye.off" : "qora.eye.on")
        ) { _ in
  ...
  public func tab(_ tab: some TabState, shouldBlockJavaScriptForRequest request: URLRequest) -> Bool
  {
    ...
    return tab.qoraiShieldsHelper?.isShieldExpected(
  ...
  public func tab(
    ...
    if tab.isPrivate || !Preferences.General.followUniversalLinks.value
      || (Preferences.General.keepYouTubeInQorai.value && isYouTubeLoad())
    {
      return true
    }
    return false
  ...
  public func tab(_ tab: some TabState, buildEditMenuWithBuilder builder: any UIMenuBuilder) {
    ...
    let searchWithQorai = UIAction(title: Strings.searchWithQorai) { [weak tab, weak self] _ in
      tab?.evaluateJavaScript(
        ...
        self?.didSelectSearchWithQorai(selectedText, tab: tab)
      }
    }
    if let lookupMenu = builder.menu(for: .lookup) {
      builder.replace(
        ...
        with: LookupMenuReplacement(
          ...
          searchWebAction: searchWithQorai
        )
      )
    }
    if #available(iOS 26, *) {
...
extension BrowserViewController {
  private func didSelectSearchWithQorai(_ selectedText: String, tab: some TabState) {
  ...
  public func tab(
    ...
    let isQoraiAllowedInUA =
      request.mainDocumentURL.flatMap {
        tab.qoraiUserAgentExceptions?.canShowQorai($0)
      } ?? true
    let mobile = isQoraiAllowedInUA ? UserAgent.mobile : UserAgent.mobileMasked
    let desktop = isQoraiAllowedInUA ? UserAgent.desktop : UserAgent.desktopMasked
    switch type {
    case .none, .automatic:
