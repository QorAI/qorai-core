...
extension BrowserViewController: TabPolicyDecider {
      ...
      ) == true
    {
      let scriptTypes =
        await tab.currentPageData?.makeUserScriptTypes(
          ...
          isAdBlockEnabled: tab.qoraiShieldsHelper?.shieldLevel(
            for: pageData.mainFrameURL,
            considerAllShieldsOption: true
          ).isEnabled ?? true,
          isBlockFingerprintingEnabled: tab.qoraiShieldsHelper?.isShieldExpected(
            for: pageData.mainFrameURL,
            shield: .fpProtection,
            considerAllShieldsOption: true
          ) ?? true
  ...
  public func tab(
    ...
    await LaunchHelper.shared.prepareAdBlockServices(
      adBlockService: self.qoraiCore.adblockService
    )
    ...
    if let mainDocumentURL = request.mainDocumentURL {
      ...
      let isAdBlockEnabled =
        tab.qoraiShieldsHelper?.shieldLevel(
          for: mainDocumentURL,
          considerAllShieldsOption: true
        ).isEnabled ?? true
      let isBlockFingerprintingEnabled =
        tab.qoraiShieldsHelper?.isShieldExpected(
          for: mainDocumentURL,
          shield: .fpProtection,
          considerAllShieldsOption: true
        ) ?? true
      ...
      if requestInfo.isMainFrame {
        tab.browserData?.setScripts(scripts: [
          ...
          // Add Qorai search result ads processing script
          // This script will process search result ads on the Qorai search page.
          .searchResultAd: QoraiSearchManager.isValidURL(requestURL) && !isPrivateBrowsing,
        ])
      }
      ...
      if requestInfo.isMainFrame,
        QoraiSearchManager.isValidURL(requestURL)
      {
        ...
        tab.qoraiSearchManager = QoraiSearchManager(
          url: requestURL,
          cookies: cookies
        )
        ...
        if QoraiSearchResultAdManager.shouldTriggerSearchResultAdClickedEvent(
          requestURL,
          ...
          if self.presentedViewController == nil {
            ...
            QoraiSearchResultAdManager.maybeTriggerSearchResultAdClickedEvent(
              requestURL,
              rewards: rewards,
          ...
          // The Qorai-Search-Ads header should be added with a negative value when all
          // of the following conditions are met:
          //   - The current tab is not a Private tab
          //   - Qorai Rewards is enabled.
          //   - The "Search Ads" is opted-out.
          //   - The requested URL host is one of the Qorai Search domains.
          if !isPrivateBrowsing && rewards.isEnabled
            && !rewards.ads.isOptedInToSearchResultAds()
            && request.allHTTPHeaderFields?["Qorai-Search-Ads"] == nil
          {
            var modifiedRequest = URLRequest(url: requestURL)
            modifiedRequest.setValue("?0", forHTTPHeaderField: "Qorai-Search-Ads")
            tab.loadRequest(modifiedRequest)
            return .cancel
          }

          tab.qoraiSearchResultAdManager = QoraiSearchResultAdManager(
            url: requestURL,
            rewards: rewards,
            isPrivateBrowsing: isPrivateBrowsing,
            isAggressiveAdsBlocking: isAdBlockModeAggressive
        ...
        if let qoraiSearchManager = tab.qoraiSearchManager {
          qoraiSearchManager.fallbackQueryResultsPending = true
          qoraiSearchManager.shouldUseFallback { backupQuery in
            guard let query = backupQuery else {
              qoraiSearchManager.fallbackQueryResultsPending = false
              return
            }
            ...
            if query.found {
              ...
              qoraiSearchManager.backupSearch(with: query) { completion in
                qoraiSearchManager.fallbackQueryResultsPending = false
                tab.injectResults()
              }
            }
        ...
        tab.qoraiSearchManager = nil
        tab.qoraiSearchResultAdManager = nil
      }
    }
    ...
    if requestURL.scheme?.contains("qorai") == true || requestURL.scheme?.contains("chrome") == true
    {
      // qorai://account should not be treated as a regular WebUI page.
    ...
    // Our own 'qorai' scheme does not require the switch-app prompt.
...
extension BrowserViewController {
  ...
  fileprivate func shouldUpgradeToHttps(url: URL, isPrivate: Bool) -> Bool {
    guard FeatureList.kQoraiHttpsByDefault.enabled,
    ...
    case .standard:
      ...
      shouldUpgrade =
        qoraiCore.httpsUpgradeExceptionsService.canUpgradeToHTTPS(for: url)
        && !isInUserAllowList
    case .disabled:
      shouldUpgrade = false
    }
