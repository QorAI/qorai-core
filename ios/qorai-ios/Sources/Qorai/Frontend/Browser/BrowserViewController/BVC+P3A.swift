...
extension BrowserViewController {
  ...
  func recordWeeklyUsage() {
    ...
    UmaHistogramExactLinear("Qorai.Core.WeeklyUsage", answer, 8)
  }
  ...
  func recordDefaultBrowserLikelyhoodP3A(openedHTTPLink: Bool = false) {
    ...
    UmaHistogramEnumeration("Qorai.IOS.IsLikelyDefault", sample: answer)
  }
  ...
  @MainActor private func recordShieldsLevelUpdateP3A(buckets: [Bucket]) {
    ...
    guard FeatureList.kQoraiShieldsContentSettings.enabled else { return }
  ...
  func recordGlobalAdBlockShieldsP3A() {
    ...
    UmaHistogramEnumeration("Qorai.Shields.AdBlockSetting", sample: answer)
  }
  ...
  func recordGlobalFingerprintingShieldsP3A() {
    ...
    UmaHistogramEnumeration("Qorai.Shields.FingerprintBlockSetting", sample: answer)
  }

  func recordDataSavedP3A(change: Int) {
    var dataSavedStorage = P3ATimedStorage<Int>.dataSavedStorage
    dataSavedStorage.add(
      value: change * QoraiGlobalShieldStats.shared.averageBytesSavedPerItem,
      to: Date()
    )
    ...
    UmaHistogramRecordValueToBucket(
      "Qorai.Savings.BandwidthSavingsMB",
      buckets: buckets,
  ...
  func recordVPNUsageP3A(vpnEnabled: Bool) {
    let usage = P3AFeatureUsage.qoraiVPNUsage
    var qoraiVPNDaysInMonthUsedStorage = P3ATimedStorage<Int>.qoraiVPNDaysInMonthUsedStorage

    if vpnEnabled {
      usage.recordUsage()
      qoraiVPNDaysInMonthUsedStorage.replaceTodaysRecordsIfLargest(value: 1)
    } else {
    ...
    UmaHistogramRecordValueToBucket(
      ...
      value: qoraiVPNDaysInMonthUsedStorage.combinedValue
    )
  }
  ...
  func recordAccessibilityDisplayZoomEnabledP3A() {
    ...
    UmaHistogramBoolean("Qorai.Accessibility.DisplayZoomEnabled", isDisplayZoomEnabled)
  }
  ...
  func recordAccessibilityDocumentsDirectorySizeP3A() {
    ...
    Task { @MainActor in
      if let documentsSize = await fetchDocumentsAndDataSize() {
        UmaHistogramRecordValueToBucket(
          "Qorai.Core.DocumentsDirectorySizeMB",
          buckets: buckets,
          value: documentsSize
        )
      }
  ...
  func recordGeneralBottomBarLocationP3A() {
    ...
    UmaHistogramEnumeration("Qorai.General.BottomBarLocation", sample: answer)
  }
  ...
  func recordTimeBasedNumberReaderModeUsedP3A(activated: Bool) {
    ...
    UmaHistogramRecordValueToBucket(
      "Qorai.ReaderMode.NumberReaderModeActivated",
      buckets: [
        0,
        .r(1...5),
        .r(5...20),
  ...
  func recordAdsUsageType() {
    ...
    UmaHistogramEnumeration("Qorai.Rewards.AdTypesEnabled", sample: answer)
  }
  ...
  func recordNavigationActionP3A(isNavigationActionForward: Bool) {
    ...
    if newNavigationActionStorage > 0 {
      ...
      UmaHistogramRecordValueToBucket(
        "Qorai.Toolbar.ForwardNavigationAction",
        buckets: [
          .r(0..<1),
          .r(1..<3),
          .r(3..<5),
  ...
  func maybeRecordQoraiSearchDailyUsage(url: URL) {
    let qoraiSearchHost = "search.qorai.com"
    let qoraiSearchPath = "/search"
    if url.host != qoraiSearchHost || url.path != qoraiSearchPath {
      return
    }
    UmaHistogramBoolean("Qorai.Search.QoraiDaily", true)
  }
}

extension P3AFeatureUsage {
  fileprivate static let qoraiVPNUsage: Self = .init(
    name: "vpn-usage",
    histogram: "Qorai.VPN.LastUsageTime",
    returningUserHistogram: "Qorai.VPN.NewUserReturning"
  )
}

extension P3ATimedStorage where Value == Int {
  /// Holds timed storage for question 21 (`Qorai.Savings.BandwidthSavingsMB`)
  fileprivate static var dataSavedStorage: Self { .init(name: "data-saved", lifetimeInDays: 7) }
  fileprivate static var qoraiVPNDaysInMonthUsedStorage: Self {
