...
extension BrowserViewController {
  ...
  private func presentScreenCallout(for type: FullScreenCalloutType, skipSafeGuards: Bool = false) {
    ...
    case .rewards:
      presentQoraiRewardsScreenCallout(skipSafeGuards: skipSafeGuards)
    case .vpnLinkReceipt:
  ...
  private func presentP3AScreenCallout() {
    if qoraiCore.p3aUtils.isP3APreferenceManaged {
      return
    }
  ...
  private func presentQoraiRewardsScreenCallout(skipSafeGuards: Bool = false) {
    if !skipSafeGuards {
      guard QoraiRewards.isSupported(prefService: profileController.profile.prefs),
  ...
  private func presentVPNLinkReceiptCallout(skipSafeGuards: Bool = false) {
    if !skipSafeGuards {
      ...
      guard case .purchased = QoraiVPN.vpnState else {
        return
      }
