...
extension BrowserViewController {
  func updateRewardsButtonState() {
    ...
    if !QoraiRewards.isSupported(prefService: profileController.profile.prefs) {
  ...
  func showQoraiRewardsPanel() {
    ...
    let qoraiRewardsPanel = QoraiRewardsViewController(
      tab: tab,
      rewards: rewards
    )
    qoraiRewardsPanel.actionHandler = { [weak self] action in
      switch action {
      case .unverifiedPublisherLearnMoreTapped:
        self?.loadNewTabWithRewardsURL(.qorai.rewardsUnverifiedPublisherLearnMoreURL)
      }
    }

    let popover = PopoverController(contentController: qoraiRewardsPanel)
...
extension TabBrowserData {
  func reportPageLoad(to rewards: QoraiRewards, redirectChain: [URL]) {
    ...
    if rewards.isEnabled {
      ...
      // joined Qorai Rewards. Desktop requires the user to have opted into
...
extension BrowserViewController: QoraiAdsCaptchaHandler {
