...
extension BrowserViewController {
  private var settingsController: SettingsViewController {
    ...
    let keyringService = QoraiWallet.KeyringServiceFactory.get(privateMode: isPrivateMode)
    let walletService = QoraiWallet.ServiceFactory.get(privateMode: isPrivateMode)
    let rpcService = QoraiWallet.JsonRpcServiceFactory.get(privateMode: isPrivateMode)
    let walletP3A = profileController.qoraiWalletAPI.walletP3A()
    ...
    let vc = SettingsViewController(
      ...
      p3aUtils: self.qoraiCore.p3aUtils,
      qoraiCore: self.profileController,
      localState: self.qoraiCore.localState,
  ...
  func presentBrowserMenu(
      ...
      let requestDesktopPageActivity = pageActivities.first(where: { $0.id == .requestDesktopSite })
    {
      ...
      pageActivities.insert(
        .init(
          ...
          image: tab.currentUserAgentType == .desktop ? "qora.smartphone" : nil,
    ...
    if !profileController.profile.prefs.isQoraiNewsAvailable {
      ...
      handlePresentation: { [unowned self] action in
        ...
        case .vpnRegionPicker:
          let vc = UIHostingController(
            rootView: QoraiVPNRegionListView(
  ...
  private func pageActions(for pageURL: URL?, tab: (any TabState)?) -> [Action] {
    ...
    if profileController.profile.prefs.isPlaylistAvailable {
      ...
      actions.append(
        .init(
          ...
          image: isPlaylistItemAdded ? "qora.product.playlist-added" : nil,
          ...
          if !isPlaylistItemAdded {
            ...
            if addedItem {
              ...
              actionCopy.image = "qora.product.playlist-added"
              return .updateAction(actionCopy)
            }
          } else {
            self.dismiss(animated: true) {
    ...
    if QoraiCore.FeatureList.kQoraiShredFeature.enabled {
  ...
  private var vpnMenuAction: Action {
    func alertForExpiredState() -> UIAlertController? {
      ...
      return QoraiSkusManager.sessionExpiredStateAlert(loginCallback: { _ in
    ...
    let vpnState = QoraiVPN.vpnState
    switch vpnState {
    case .notPurchased, .expired:
      return .init(id: .vpn) { @MainActor [unowned self] _ in
        ...
        guard QoraiVPN.vpnState.isPaywallEnabled else { return .none }
        ...
        let vpnPaywallView = QoraiVPNPaywallView(
          ...
          installVPNProfile: { [weak self] in
            guard let self else { return }
            self.popToBVC()
            self.openInsideSettingsNavigation(with: QoraiVPNInstallViewController())
          }
        )

        let vc = QoraiVPNPaywallHostingController(paywallView: vpnPaywallView)
    ...
    case .purchased:
      let isConnected = QoraiVPN.isConnected || QoraiVPN.isConnecting
      return .init(
        id: .vpn,
        ...
        if QoraiVPN.isConnected || QoraiVPN.isConnecting {
          await withCheckedContinuation { continuation in
            QoraiVPN.disconnect { error in
              continuation.resume()
            }
          }
        } else {
          await withCheckedContinuation { continuation in
            QoraiVPN.reconnect { success in
              continuation.resume()
            }
          }
  ...
  private func destinationMenuActions(for pageURL: URL?) -> [Action] {
    ...
    var actions: [Action] = [
      ...
      .init(id: .downloads) { @MainActor [unowned self] _ in
        UIApplication.shared.openQoraiDownloadsFolder { success in
          if !success {
            self.dismiss(animated: true) {
    ...
    if profileController.qoraiWalletAPI.isAllowed {
      actions.append(
        .init(id: .qoraiWallet) { @MainActor [unowned self] _ in
    ...
    if profileController.profile.prefs.isQoraiTalkAvailable {
      actions.append(
        .init(id: .qoraiTalk) { @MainActor [unowned self] _ in
          self.dismiss(animated: true) {
            guard let url = URL(string: "https://talk.qorai.com/") else { return }
            self.popToBVC()
            if pageURL == nil {
              // Already on NTP
    ...
    if profileController.profile.prefs.isQoraiNewsAvailable {
      actions.append(
        .init(id: .qoraiNews) { @MainActor [unowned self] _ in
          self.dismiss(animated: true) {
            if pageURL == nil,
