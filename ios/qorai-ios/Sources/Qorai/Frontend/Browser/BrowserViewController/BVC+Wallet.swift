...
extension WalletStore {
  ...
  static func from(
    ipfsApi: IpfsAPI,
    walletP3A: QoraiWalletQoraiWalletP3A?,
    privateMode: Bool
  ) -> WalletStore? {
    guard
      let keyringService = QoraiWallet.KeyringServiceFactory.get(privateMode: privateMode),
      let rpcService = QoraiWallet.JsonRpcServiceFactory.get(privateMode: privateMode),
      let assetRatioService = QoraiWallet.AssetRatioServiceFactory.get(privateMode: privateMode),
      let walletService = QoraiWallet.ServiceFactory.get(privateMode: privateMode),
      let swapService = QoraiWallet.SwapServiceFactory.get(privateMode: privateMode),
      let txService = QoraiWallet.TxServiceFactory.get(privateMode: privateMode),
      let ethTxManagerProxy = QoraiWallet.EthTxManagerProxyFactory.get(privateMode: privateMode),
      let solTxManagerProxy = QoraiWallet.SolanaTxManagerProxyFactory.get(privateMode: privateMode),
      let walletP3A,
      let bitcoinWalletService = QoraiWallet.BitcoinWalletServiceFactory.get(
        privateMode: privateMode
      ),
      let zcashWalletService = QoraiWallet.ZCashWalletServiceFactory.get(
        privateMode: privateMode
      ),
      let meldIntegrationService = QoraiWallet.MeldIntegrationServiceFactory.get(
        privateMode: privateMode
      )
    else {
    ...
    return WalletStore(
      ...
      blockchainRegistry: QoraiWalletAPI.blockchainRegistry,
      txService: txService,
...
extension BrowserViewController: QoraiWalletDelegate {
...
extension TabBrowserData: QoraiWalletProviderDelegate {
  func showPanel() {
  ...
  public func requestPermissions(
    _ coinType: QoraiWallet.CoinType,
    accounts: [String],
  ...
  func getAllowedAccounts(_ type: QoraiWallet.CoinType, accounts: [String]) -> [String]? {
  ...
  func isAccountAllowed(_ type: QoraiWallet.CoinType, account: String) -> Bool {
  ...
  func isPermissionDenied(_ type: QoraiWallet.CoinType) -> Bool {
    switch type {
    case .eth:
  ...
  func showAccountCreation(_ coin: QoraiWallet.CoinType) {
    guard let tab else { return }
    let privateMode = tab.isPrivate
    guard let keyringService = QoraiWallet.KeyringServiceFactory.get(privateMode: privateMode)
    else {
      return
    }
    Task { @MainActor in
...
extension TabBrowserData: QoraiWalletEventsListener {
  func emitEthereumEvent(_ event: Web3ProviderEvent) {
    guard let tab, !tab.isPrivate,
      Preferences.Wallet.defaultEthWallet.value == Preferences.Wallet.WalletType.qorai.rawValue,
  ...
  func chainChangedEvent(chainId: String) {
    ...
    Task { @MainActor in
      ...
      // new default network / nil origin: qorai-browser #30344
  ...
  func updateEthereumProperties() async {
    guard let tab, !tab.isPrivate,
      let keyringService = QoraiWallet.KeyringServiceFactory.get(privateMode: false),
      Preferences.Wallet.defaultEthWallet.value == Preferences.Wallet.WalletType.qorai.rawValue
    else {
      return
    }
...
extension TabBrowserData: QoraiWalletSolanaEventsListener {
  ...
  func emitSolanaEvent(_ event: Web3ProviderEvent) {
    guard let tab, tab.isWebViewCreated,
      Preferences.Wallet.defaultSolWallet.value == Preferences.Wallet.WalletType.qorai.rawValue
    else {
      return
    }
    Task {
...
extension TabBrowserData: QoraiWalletKeyringServiceObserver {
  func walletCreated() {
  }
