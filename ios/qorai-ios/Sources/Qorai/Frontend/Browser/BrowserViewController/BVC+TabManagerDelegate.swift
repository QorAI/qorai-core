...
extension BrowserViewController: TabManagerDelegate {
  func attachTabHelpers(to tab: some TabState) {
    ...
    tab.qoraiUserAgentExceptions = qoraiCore.qoraiUserAgentExceptions
    ...
    let qoraiShieldsHelper: QoraiShieldsTabHelper = .init(
      tab: tab,
      qoraiShieldsSettings: QoraiShieldsSettingsServiceFactory.get(profile: profile)
    )
    tab.qoraiShieldsHelper = qoraiShieldsHelper
    // When `QoraiShieldsTabHelper+TabPolicyDecider` is moved to `QoraiShields` target,
    // we should add it as a policy decider at initialization.
    tab.addPolicyDecider(qoraiShieldsHelper)
  }
  ...
  func tabManager(
    ...
    if FeatureList.kQoraiTranslateEnabled.enabled, let selectedTab = selected,
  ...
  func tabManager(_ tabManager: TabManager, didAddTab tab: some TabState) {
    ...
    tab.walletKeyringService = QoraiWallet.KeyringServiceFactory.get(privateMode: tab.isPrivate)
    updateTabsBarVisibility()
  }
  ...
  func hideToastsOnNavigationStartIfNeeded(_ tabManager: TabManager) {
    if tabManager.selectedTab?.qoraiSearchResultAdManager == nil {
  ...
  func updateToolbarUsingTabManager(_ tabManager: TabManager) {
    ...
    let openNewTab = UIAction(
      ...
      image: privateBrowsingManager.isPrivateBrowsing
        ? UIImage(qoraiSystemNamed: "qora.product.private-window")
        : UIImage(qoraiSystemNamed: "qora.browser.mobile-tab-new"),
      handler: UIAction.deferredActionHandler { [unowned self] _ in
    ...
    if containsWebPage {
      let bookmarkActiveTab = UIAction(
        ...
        image: UIImage(qoraiSystemNamed: "qora.browser.bookmark-add"),
        handler: UIAction.deferredActionHandler { [unowned self] _ in
    ...
    if !privateBrowsingManager.isPrivateBrowsing {
      let viewRecentlyClosedTabs = UIAction(
        ...
        image: UIImage(qoraiSystemNamed: "qora.browser.mobile-recent-tabs"),
      ...
      if let recentlyClosedTab = RecentlyClosed.first() {
        ...
        let reopenLastClosedTab = UIAction(
          ...
          image: UIImage(qoraiSystemNamed: "qora.browser.mobile-tab-ntp"),
          handler: UIAction.deferredActionHandler { [weak self] _ in
            guard let self = self else { return }
