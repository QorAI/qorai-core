...
public class AboutHomeHandler: InternalSchemeResponse {
  ...
  public func response(forRequest request: URLRequest) async -> (URLResponse, Data)? {
    ...
    let lightModeColour = UIColor(qoraiSystemName: .containerBackground).resolvedColor(
      with: .init(userInterfaceStyle: .light)
    ).toHexString()
    let darkModeColour = UIColor(qoraiSystemName: .containerBackground).resolvedColor(
