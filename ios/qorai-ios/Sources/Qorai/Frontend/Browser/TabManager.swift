...
class TabManager: NSObject {
  ...
  private var qoraiCore: QoraiProfileController?
  private weak var tabGeneratorAPI: QoraiTabGeneratorAPI?
  ...
  init(
    ...
    self.tabGeneratorAPI = qoraiCore?.tabGeneratorAPI
  ...
  @MainActor func forgetDataOnAppExitDomains() {
    guard QoraiCore.FeatureList.kQoraiShredFeature.enabled else { return }
  ...
  @MainActor func forgetDataIfNeeded(
    ...
    guard FeatureList.kQoraiShredFeature.enabled else { return }
    guard let url = url.urlToShred,
      let baseDomain = url.baseDomain
    else { return }
    forgetTasks[tab.isPrivate]?[baseDomain]?.cancel()
    let shredLevel =
      tab.qoraiShieldsHelper?.shredLevel(for: url) ?? .never
  ...
  @MainActor private func forgetData(for urls: [URL], dataStore: WKWebsiteDataStore? = nil) async {
    ...
    if QoraiCore.FeatureList.kQoraiShredCacheData.enabled {
      // Delete all cache data (otherwise 3P cache entries left behind
      // are visible in Manage Website Data view qorai-browser #41095)
      let cacheTypes = Set([
  ...
  @MainActor fileprivate var restoreTabsInternal: (any TabState)? {
    ...
    let shouldShredDomain: (_ tabURL: URL, _ isPrivate: Bool) -> Bool = { url, isPrivate in
      ...
      if let shouldShredDomain = shouldShredDomainCache[cacheKey] {
        ...
        // to access via `QoraiShieldsTabHelper`. Temporarily access here until
        // we switch to using `QoraiShieldsSettings` with qorai-browser#47350
