...
class SendTabToSelfController: SendTabTransitioningController {
  ...
  init(sendTabAPI: QoraiSendTabAPI, dataSource: SendableTabInfoDataSource) {
...
class SendTabToSelfContentController: UITableViewController {
  ...
  private var sendTabAPI: QoraiSendTabAPI?

  var sendWebSiteHandler: ((SendableTabInfoDataSource) -> Void)?

  // MARK: Lifecycle

  convenience init(sendTabAPI: QoraiSendTabAPI, dataSource: SendableTabInfoDataSource) {
...
extension SendTabToSelfContentController {
  ...
  override func tableView(
    ...
    if let device = dataSource?.deviceInformation(for: indexPath) {
      ...
      case .mobile:
        deviceTypeImage = UIImage(qoraiSystemNamed: "qora.smartphone.tablet-portrait")
      case .PC:
        deviceTypeImage = UIImage(qoraiSystemNamed: "qora.laptop")
      default:
        deviceTypeImage = UIImage(qoraiSystemNamed: "qora.smartphone.laptop")
      }
      ...
      cell.do {
        ...
        $0.imageView?.tintColor = .qoraiLabel
