  ...
  private var syncSettingsController: UIViewController {
    ...
    if Preferences.Chromium.syncEnabled.value {
      controller = SyncSettingsTableViewController(
        ...
        qoraiCoreMain: profileController,
  ...
  var body: some View {
    ...
    .background(alignment: .top) {
      if viewModel.isPrivateBrowsing, horizontalSizeClass == .compact {
        // Decorative blur
        Color(qoraiSystemName: .primitivePrivateWindow20)
      ...
      HStack(spacing: 20) {
        if viewModel.isShredMenuVisible {
          Menu {
            ...
            Label(Strings.TabGrid.shredTabsAccessibilityLabel, qoraiSystemImage: "qora.shred.data")
          }
        }
        Button {
          destinationSheet = .history
        } label: {
          Label(Strings.TabGrid.viewHistoryAccessibilityLabel, qoraiSystemImage: "qora.history")
        }
        Button {
  ...
  var footerBar: some View {
    HStack {
        ...
        if viewModel.isPrivateBrowsing && !privateBrowsingOnly.value {
          Section {
            Button {
              ...
              Label(Strings.TabGrid.privateTabsSettingsTitle, qoraiSystemImage: "qora.settings")
            }
          }
        }
        Section {
          if !viewModel.tabs.isEmpty {
            Button {
              ...
              Label(
                ...
                qoraiSystemImage: "qora.check.circle-outline"
              )
            }
            Button(role: .destructive) {
              viewModel.closeAllTabs()
              ...
              Label(Strings.TabGrid.closeAllTabsButtonTitle, qoraiSystemImage: "qora.close")
            }
          }
        }
      } label: {
      ...
      Button {
        ...
        Label(Strings.TabGrid.newTabAccessibilityLabel, qoraiSystemImage: "qora.plus.add")
          .labelStyle(.iconOnly)
          .font(.callout)
          .imageScale(.large)
  ...
  var editModeFooterBar: some View {
    HStack {
      Button {
        ...
        Label(Strings.TabGrid.shredSelectedTabsButtonTitle, qoraiSystemImage: "qora.shred.data")
      ...
      Button {
        ...
        Label(Strings.close, qoraiSystemImage: "qora.close")
          .frame(maxWidth: .infinity)
      }
      .buttonStyle(QoraiFilledButtonStyle(size: .normal))
    ...
    let selectAndShred = UIMenu(
      ...
      children: [
        UIAction(
          ...
          image: UIImage(qoraiSystemNamed: "qora.check.circle-outline"),
          handler: { _ in
            withAnimation {
        ...
        UIAction(
          ...
          image: UIImage(qoraiSystemNamed: "qora.shred.data"),
    ...
    let closeTabs = UIMenu(
      ...
      children: [
        UIAction(
          ...
          image: UIImage(qoraiSystemNamed: "qora.close"),
...
private struct PrivateBrowsingPicker: UIViewRepresentable {
    ...
    let uiView = UISegmentedControl(
      ...
      actions: [
        .init(
          ...
          image: UIImage(qoraiSystemNamed: "qora.browser.mobile-tabs"),
          handler: { _ in }
        ),
        .init(
  ...
  func updateUIView(_ uiView: UIViewType, context: Context) {
    ...
    uiView.backgroundColor = UIColor(
      qoraiSystemName: isPrivateBrowsing ? .privateWindow10 : .neutral10
    )
    uiView.selectedSegmentTintColor = UIColor(
      qoraiSystemName: isPrivateBrowsing ? .privateWindow30 : .containerBackground
    )
    uiView.setTitleTextAttributes(
      [
        .foregroundColor: UIColor(
          qoraiSystemName: isPrivateBrowsing ? .iconInteractive : .iconDefault
        )
      ],
      for: .selected
    )
    uiView.setTitleTextAttributes(
      [.foregroundColor: UIColor(qoraiSystemName: .iconDefault)],
      for: .normal
    )
  }
  func sizeThatFits(
  ...
  var body: some View {
    VStack(spacing: 48) {
      Label {
        ...
        Image(qoraiSystemName: "qora.product.private-window")
          .foregroundStyle(Color(qoraiSystemName: .iconInteractive))
      }
      .font(.title2.weight(.semibold))
      VStack(spacing: 24) {
        BulletPointView(
          icon: "qora.product.private-window",
  ...
  private struct BulletPointView: View {
    ...
    var body: some View {
      HStack(alignment: .top, spacing: 16) {
        Image(qoraiSystemName: icon)
          .foregroundStyle(Color(qoraiSystemName: .primitivePrivateWindow60))
          .frame(width: iconSize, height: iconSize)
          .background(Color(qoraiSystemName: .primitivePrivateWindow20), in: .circle)
        VStack(alignment: .leading, spacing: 2) {
          Text(title)
            .foregroundStyle(Color(qoraiSystemName: .primitivePrivateWindow90))
            .font(.callout.weight(.semibold))
          Text(description)
            .foregroundStyle(Color(qoraiSystemName: .primitivePrivateWindow80))
            .font(.footnote)
        }
