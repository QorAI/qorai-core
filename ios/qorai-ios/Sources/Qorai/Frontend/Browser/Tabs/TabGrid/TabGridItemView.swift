      ...
      HStack {
        ...
        Group {
          if editMode?.wrappedValue == .active {
            ...
            Image(qoraiSystemName: isSelected ? "qora.checkbox.checked" : "qora.checkbox.unchecked")
              .foregroundStyle(Color(qoraiSystemName: isSelected ? .iconInteractive : .iconDefault))
          } else {
            Button {
              actionHandler(.closedTab)
            } label: {
              Label(Strings.TabGrid.closeTab, qoraiSystemImage: "qora.close")
                .labelStyle(.iconOnly)
            }
          }
        }
            ...
            .shadow(
              .drop(
                color: viewModel.swipeToDeleteGestureState != nil
                  ? Color.black.opacity(0.25) : Color(qoraiSystemName: .elevationSecondary),
                radius: viewModel.swipeToDeleteGestureState != nil ? 15 : 4,
                x: 0,
                y: 1
              )
            )
            .shadow(.drop(color: Color(qoraiSystemName: .elevationPrimary), radius: 0, x: 0, y: 1)),
