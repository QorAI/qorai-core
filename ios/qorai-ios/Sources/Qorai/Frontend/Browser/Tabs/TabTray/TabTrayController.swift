...
class TabTrayController: AuthenticationController {
  ...
  let qoraiCore: QoraiProfileController
  ...
  private(set) var privateMode: Bool = false {
    didSet {
      ...
      tabTypeSelectorContainerView.isHidden =
        privateMode && !QoraiCore.FeatureList.kQoraiShredFeature.enabled
    }
  }
  ...
  let newTabButton = UIButton(type: .system).then {
    $0.setImage(UIImage(qoraiSystemNamed: "qora.plus.add"), for: .normal)
  ...
  let privateModeButton = SelectedInsetButton().then {
    ...
    $0.selectedBackgroundColor = UIColor(qoraiSystemName: .primitivePrivateWindow25)
  ...
  lazy var shredButton: UIButton = {
    ...
    button.setImage(UIImage(qoraiSystemNamed: "qora.shred.data"), for: .normal)
    button.tintColor = .qoraiPrimary
  ...
  init(
    ...
    openTabsSessionServiceListener = qoraiCore.openTabsAPI.add(
    ...
    syncServicStateListener = qoraiCore.syncAPI.addServiceStateObserver { [weak self] in
      guard let self = self else { return }

      DispatchQueue.main.async {
        if self.qoraiCore.syncAPI.shouldLeaveSyncGroup {
          self.tabSyncView.do {
  ...
  deinit {
    ...
    if let observer = openTabsSessionServiceListener {
      qoraiCore.openTabsAPI.removeObserver(observer)
    }
  }
  ...
  private func createTypeSelectorItems() {
    tabTypeSelectorItems = [
      UIImage(qoraiSystemNamed: "qora.browser.mobile-tabs")!.template,
      UIImage(qoraiSystemNamed: "qora.smartphone.laptop")!.template,
    ]
  }
  ...
  private func fetchSyncedSessions(for query: String? = nil) -> [OpenDistantSession] {
    let allSessions = qoraiCore.openTabsAPI.getSyncedSessions()
