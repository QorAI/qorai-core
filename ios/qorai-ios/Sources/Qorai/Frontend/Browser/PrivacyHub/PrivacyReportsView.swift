...
struct PrivacyReportsView: View {
  ...
  @ObservedObject private var showNotificationPermissionCallout = Preferences.PrivacyReports
    .shouldShowNotificationPermissionCallout
  ...
  private var clearAllDataButton: some View {
    Button(
      ...
      label: {
        Image(uiImage: .init(qoraiSystemNamed: "qora.trash")!.template)
      }
    )
    ...
    NavigationView {
      ScrollView(.vertical) {
        VStack(alignment: .leading, spacing: 16) {
          ...
          if Preferences.PrivacyReports.captureVPNAlerts.value, let lastVPNAlerts = lastVPNAlerts,
            !lastVPNAlerts.isEmpty
          {
      ...
      .background(Color(.secondaryQoraiBackground).ignoresSafeArea())
    }
