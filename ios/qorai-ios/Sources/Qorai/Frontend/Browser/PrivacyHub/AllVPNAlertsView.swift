...
extension PrivacyReportsView {
  struct AllVPNAlertsView: View {
    ...
    @FetchRequest(
      entity: QoraiVPNAlert.entity(),
      sortDescriptors: [NSSortDescriptor(keyPath: \QoraiVPNAlert.timestamp, ascending: false)],
    ...
    ) private var vpnAlerts: FetchedResults<QoraiVPNAlert>
    ...
    private func cell(for alert: QoraiVPNAlert) -> some View {
    ...
    var body: some View {
      ...
      .onAppear {
        QoraiVPNAlert.alertTotals { result in
          alerts = result
          alertsLoading = false
        }
      }
