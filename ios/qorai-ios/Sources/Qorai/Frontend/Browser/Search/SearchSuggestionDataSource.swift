...
class SearchSuggestionDataSource {
  ...
  private let maxPeriodQoraiSearchPromotion = 15
  ...
  var qoraiSearchPromotionAvailable: Bool {
    guard Preferences.Review.launchCount.value > 1,
      searchEngines?.defaultEngine(forType: isPrivate ? .privateMode : .standard)?
        .shortName != OpenSearchEngine.EngineNames.qorai,
      let qoraiSearchPromotionLaunchDate = Preferences.QoraiSearch.qoraiSearchPromotionLaunchDate
        .value,
      Preferences.QoraiSearch.qoraiSearchPromotionCompletionState.value
        != QoraiSearchPromotionState.dismissed.rawValue,
      Preferences.QoraiSearch.qoraiSearchPromotionCompletionState.value
    ...
    let rightNow = Date()
    let nextShowDate = qoraiSearchPromotionLaunchDate.addingTimeInterval(
      AppConstants.isOfficialBuild
        ? maxPeriodQoraiSearchPromotion.days : maxPeriodQoraiSearchPromotion.minutes
    )
