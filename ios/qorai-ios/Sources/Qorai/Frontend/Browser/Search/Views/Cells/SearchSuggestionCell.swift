...
class SearchSuggestionCell: UICollectionViewCell, CollectionViewReusable {
  ...
  private let searchImageView = UIImageView().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.search")!.template
  ...
  private let openButton = QoraiButton().then {
  ...
  private func setTheme() {
    searchImageView.tintColor = isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .iconDefault)
    ...
    openButton.do {
      $0.setImage(
        UIImage(qoraiSystemNamed: "qora.arrow.diagonal-up-left")?.template,
        for: .normal
      )
      $0.tintColor = isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .iconInteractive)
    }
  }
