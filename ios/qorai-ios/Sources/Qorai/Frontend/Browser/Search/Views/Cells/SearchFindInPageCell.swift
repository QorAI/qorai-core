...
class SearchFindInPageCell: UICollectionViewCell, CollectionViewReusable {
  ...
  private let searchImageView = UIImageView().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.window.search")!.template
  ...
  private func setTheme() {
    ...
    searchImageView.do {
      $0.tintColor = isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .iconDefault)
    }
  }

  private func updateTitleTheme() {
    titleLabel.textColor = isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .textPrimary)
    ...
    let attString = NSMutableAttributedString(
      ...
      attributes: [
        .font: boldFont,
        .foregroundColor: isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .textTertiary),
      ]
    )
    attString.append(
      NSAttributedString(
        ...
        attributes: [
          .font: regularFont,
          .foregroundColor: isPrivateBrowsing ? .white : UIColor(qoraiSystemName: .textPrimary),
        ]
      )
    )
