...
// A helper class to handle Qorai Search fallback needs.
class QoraiSearchManager: NSObject {
  private let fallbackProviderURLString = "https://www.google.com/search"

  /// Qorai Search query details which are passed to the fallback provider.
  ...
  /// URL of the Qorai Search request we performed.
  /// Contains a query and endpoint(prod or staging)
  private let url: URL
  /// What did we search for using Qorai Search
  private let query: String
  /// QoraiSearch cookies are used to pass search settings we saved on the website
  ...
  var fallbackQueryResultsPending = false {
    didSet {
      ...
      if let callbackLog = callbackLog, !fallbackQueryResultsPending {
        QoraiSearchLogEntry.shared.logs.append(callbackLog)
      }
    }
  }
  ...
  static let validDomains = [
    ...
    "safesearch.qorai.software",
    "safesearch.qoraisoftware.com",
  ...
  private var callbackLog: QoraiSearchLogEntry.FallbackLogEntry?
  ...
  init?(url: URL, cookies: [HTTPCookie]) {
    ...
    if QoraiSearchLogEntry.shared.isEnabled {
      callbackLog = .init(date: Date(), url: url, query: queryItem, cookies: domainCookies)
    }
  }

  /// A call is made to the Qorai Search api with query we performed.
  ...
  func shouldUseFallback(completion: @escaping (BackupQuery?) -> Void) {
    ...
    if callbackLog != nil {
      timer = PerformanceTimer(label: "Qorai Search Debug can answer api call")
    }
  ...
  func backupSearch(
    ...
    var timer: PerformanceTimer?
    if callbackLog != nil {
      timer = PerformanceTimer(label: "Qorai Search Debug fallback results call")
    }
