...
public class SearchViewController: UIViewController, LoaderListener {
  ...
  private lazy var compositionalLayout = SearchCompositionalLayout(
    ...
    sectionProvider: { [weak self] sectionIndex, _ in
      ...
      case .qoraiSearchPromotion:
        return self.qoraiSearchPromotionLayoutSection()
      }
    },
  ...
  private var availableSections: [SearchSuggestionDataSource.SearchListSection] {
    ...
    if dataSource.qoraiSearchPromotionAvailable {
      result.append(.qoraiSearchPromotion)
    }
  ...
  private func reloadSearchEngines() {
    ...
    if dataSource.isAIChatAvailable {
      ...
      let leoButtonBackgroundView = GradientView(qoraiSystemName: .iconsActive)

      let leoButton = UIButton()
      leoButton.setImage(
        UIImage(qoraiSystemNamed: "qora.product.qorai-qora")!.template,
        for: []
      )
      leoButton.imageView?.contentMode = .center
      leoButton.addTarget(self, action: #selector(didClickQoraButton), for: .touchUpInside)
      ...
      let separator = UIView().then {
        $0.backgroundColor = UIColor(qoraiSystemName: .materialSeparator)
      }
    ...
    settingsButton.setImage(
      UIImage(qoraiSystemNamed: "qora.settings")!.template,
      for: []
    )
    ...
    settingsButton.tintColor = UIColor(qoraiSystemName: .iconDefault)
  ...
  private func changeQoraiSearchPromotionState() {
    // The promotion state will changed "maybeLaterUpcomingSession" next launch
    if Preferences.QoraiSearch.qoraiSearchPromotionCompletionState.value
      != QoraiSearchPromotionState.maybeLaterSameSession.rawValue
    {
      Preferences.QoraiSearch.qoraiSearchPromotionCompletionState.value += 1
    }
  }

  private func submitSeachTemplateQuery(isQoraiSearchPromotion: Bool) {
    if !dataSource.isPrivate {
    ...
    searchDelegate?.searchViewController(
      ...
      qoraiSearchPromotion: isQoraiSearchPromotion
    )
  }
  ...
  private func qoraiSearchPromotionLayoutSection() -> NSCollectionLayoutSection {
    let itemSize = NSCollectionLayoutSize(
      widthDimension: .fractionalWidth(1),
  ...
  @objc func didSelectEngine(_ sender: UIButton) {
    ...
    let offset = dataSource.isAIChatAvailable ? 1 : 0  // offset for the Qora button
  ...
  @objc func onQoraiSearchPromotionOptinButton() {
    submitSeachTemplateQuery(isQoraiSearchPromotion: true)
    collectionView.reloadData()
  }

  @objc func onQoraiSearchPromtionIgnoreButton() {
    changeQoraiSearchPromotionState()
    collectionView.reloadData()
  }
...
extension SearchViewController: UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  public func collectionView(
    ...
    case .searchSuggestionsOptIn:
      if indexPath.row == 0 {
        ...
        cell.backgroundColor = UIColor(qoraiSystemName: .dividerSubtle)
    ...
    case .qoraiSearchPromotion:
      let cell = collectionView.dequeueReusableCell(for: indexPath) as SearchActionsCell
      cell.do {
        $0.backgroundColor = UIColor(qoraiSystemName: .containerInteractive)
        $0.layer.cornerRadius = 12
        $0.titleLabel.text = Strings.QoraiSearchPromotion.qoraiSearchPromotionBannerTitle
        $0.subtitleLabel.text = Strings.QoraiSearchPromotion.qoraiSearchPromotionBannerDescription
        $0.imageView.image = UIImage(named: "qorai-search-promotion", in: .module, with: nil)
        $0.primaryButton.setTitle(
          Strings.QoraiSearchPromotion.qoraiSearchPromotionBannerTryButtonTitle,
          for: .normal
        )
        $0.primaryButton.addTarget(
          self,
          action: #selector(onQoraiSearchPromotionOptinButton),
          for: .touchUpInside
        )
        $0.secondaryButton.setTitle(
          Preferences.QoraiSearch.qoraiSearchPromotionCompletionState.value
            != QoraiSearchPromotionState.maybeLaterUpcomingSession.rawValue
            ? Strings.QoraiSearchPromotion.qoraiSearchPromotionBannerMaybeLaterButtonTitle
            : Strings.QoraiSearchPromotion.qoraiSearchPromotionBannerDismissButtonTitle,
          for: .normal
        )
        $0.secondaryButton.addTarget(
          self,
          action: #selector(onQoraiSearchPromtionIgnoreButton),
          for: .touchUpInside
        )
      }
  ...
  public func collectionView(
    ...
    if kind == UICollectionView.elementKindSectionHeader {
        ...
        ) as? SearchSectionHeaderView
      {
        ...
        switch section {
        case .searchSuggestionsOptIn, .searchSuggestions, .qoraiSearchPromotion:
  ...
  public func collectionView(
    ...
    case .searchSuggestionsOptIn:
      if indexPath.row == 0 {
        // quick bar
        submitSeachTemplateQuery(isQoraiSearchPromotion: false)
      }
    case .searchSuggestions:
      if indexPath.row == 0 {
        // quick bar
