  ...
  override func viewDidLoad() {
    ...
    let tab = TabStateFactory.create(
      with: .init(
        ...
        qoraiCore: browserController.profileController
      )
    )
    ...
    let qoraiShieldsTabHelper: QoraiShieldsTabHelper = .init(
      tab: tab,
      qoraiShieldsSettings: QoraiShieldsSettingsServiceFactory.get(profile: profile)
    )
    tab.qoraiShieldsHelper = qoraiShieldsTabHelper
    tab.addPolicyDecider(qoraiShieldsTabHelper)
    ...
    Task(priority: .userInitiated) {
      let isQoraiShieldsEnabled = qoraiShieldsTabHelper.isQoraiShieldsEnabled(for: url)
      let shieldLevel = qoraiShieldsTabHelper.shieldLevel(for: url, considerAllShieldsOption: true)
