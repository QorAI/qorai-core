...
class AddEditBookmarkTableViewController: UITableViewController {
  ...
  private var rootLevelFolderCell: IndentedImageTableViewCell {
    let cell = IndentedImageTableViewCell(
      image: UIImage(qoraiSystemNamed: "qora.product.bookmarks")!
    ).then {
  ...
  private var favoritesCell: IndentedImageTableViewCell {
    let cell = IndentedImageTableViewCell(image: UIImage(qoraiSystemNamed: "qora.heart.outline")!)
      .then {
  ...
  private var addNewFolderCell: IndentedImageTableViewCell {
    let cell = IndentedImageTableViewCell(image: UIImage(qoraiSystemNamed: "qora.folder.new")!)
  ...
  private func sortFolders() -> [IndentedFolder] {
    ...
    return objects.map({
      if let folder = $0 as? QoraiBookmarkFolder {
  ...
  override func tableView(
    ...
    case .currentSelection:
      ...
      case .folder(let folder):
        let cell = IndentedImageTableViewCell(image: UIImage(qoraiSystemNamed: "qora.folder")!)
    ...
    case .folderHierarchy:
      ...
      if indentedFolder.folder.parent == nil {
        cell.customImage.image = UIImage(qoraiSystemNamed: "qora.product.bookmarks")
      } else {
        cell.customImage.image = UIImage(
          qoraiSystemNamed: hasChildrenFolders == true ? "qora.folder.open" : "qora.folder"
        )
      }
