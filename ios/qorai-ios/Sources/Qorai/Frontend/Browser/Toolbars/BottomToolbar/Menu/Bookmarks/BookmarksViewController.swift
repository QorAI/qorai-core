...
class BookmarksViewController: SiteTableViewController, ToolbarUrlActionsProtocol {
  ...
  private lazy var editBookmarksButton = UIBarButtonItem().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.edit.pencil")
    $0.style = .plain
    $0.target = self
  ...
  private lazy var addFolderButton = UIBarButtonItem().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.folder.new")
    $0.style = .plain
    $0.target = self
  ...
  private lazy var importExportButton = UIBarButtonItem().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.share.macos")
    $0.style = .plain
    $0.target = self
  ...
  fileprivate func configureCell(_ cell: BookmarkTableViewCell, atIndexPath indexPath: IndexPath) {
    ...
    func configCell(image: UIImage? = nil) {
      ...
      cell.imageView?.tintColor = .qoraiLabel
      ...
      if let image = image {
        ...
        // Sets the favIcon of a cell's imageView from Qorai-Core
        ...
        // Qorai-Core favIcons are async and notify an observer when changed..
    ...
    if !item.isFolder {
      ...
      configCell(image: UIImage(qoraiSystemNamed: "qora.folder"))
...
extension BookmarksViewController: BookmarksV2FetchResultsDelegate {
  ...
  func controllerDidReloadContents(_ controller: BookmarksV2FetchResultsController) {
    ...
    // See qorai-ios/issues/3011 && qorai-browser/issues/12530
    // - Brandon T.
