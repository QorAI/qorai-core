...
class TopToolbarView: UIView, ToolbarProtocol {
  ...
  private lazy var cancelButton = InsetButton().then {
    ...
    $0.setTitleColor(UIColor.secondaryQoraiLabel, for: .normal)
  ...
  private lazy var qrCodeButton = ToolbarButton().then {
    ...
    $0.setImage(UIImage(qoraiSystemNamed: "qora.qr.code", compatibleWith: nil), for: .normal)
    $0.tintColor = UIColor(qoraiSystemName: .iconDefault)
  ...
  private lazy var voiceSearchButton = ToolbarButton().then {
    ...
    $0.setImage(UIImage(qoraiSystemNamed: "qora.microphone", compatibleWith: nil), for: .normal)
    $0.tintColor = UIColor(qoraiSystemName: .iconDefault)
    $0.contentEdgeInsets = UIEdgeInsets(top: 0, left: 5, bottom: 0, right: 5)
    $0.setContentCompressionResistancePriority(.required, for: .horizontal)
    $0.setContentCompressionResistancePriority(.defaultHigh, for: .vertical)
  ...
  private(set) lazy var shieldsButton: ToolbarButton = {
    ...
    button.setImage(UIImage(sharedNamed: "qorai.logo"), for: .normal)
    button.addTarget(self, action: #selector(didTapQoraiShieldsButton), for: .touchUpInside)
    button.imageView?.contentMode = .scaleAspectFit
    button.accessibilityLabel = Strings.qoraiPanel
  ...
  private(set) lazy var rewardsButton: RewardsButton = {
    ...
    button.addTarget(self, action: #selector(didTapQoraiRewardsButton), for: .touchUpInside)
    return button
  }()
  ...
  private lazy var pasteAndGoButton = ToolbarButton().then {
    ...
    $0.setImage(UIImage(qoraiSystemNamed: "qora.clipboard", compatibleWith: nil), for: .normal)
    $0.tintColor = UIColor(qoraiSystemName: .iconDefault)
  ...
  private lazy var searchImageView = UIImageView().then {
    $0.image = UIImage(qoraiSystemNamed: "qora.search", compatibleWith: nil)
  ...
  func refreshShieldsStatus() {
    // Default on
    var shieldIcon = "qorai.logo"
    let shieldsOffIcon = "qorai.logo.greyscale"
  ...
  @objc private func didTapQoraiShieldsButton() {
    delegate?.topToolbarDidTapQoraiShieldsButton(self)
  }

  @objc private func didTapQoraiRewardsButton() {
    delegate?.topToolbarDidTapQoraiRewardsButton(self)
  }
}
