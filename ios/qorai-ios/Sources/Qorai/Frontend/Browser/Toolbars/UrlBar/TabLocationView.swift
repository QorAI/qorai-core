...
class TabLocationView: UIView {
  ...
  var loading: Bool = false {
    didSet {
      if loading {
        reloadButton.setImage(UIImage(qoraiSystemNamed: "qora.close"), for: .normal)
        reloadButton.accessibilityLabel = Strings.tabToolbarStopButtonAccessibilityLabel
      } else {
        reloadButton.setImage(UIImage(qoraiSystemNamed: "qora.browser.refresh"), for: .normal)
  ...
  private var secureContentStateButtonConfiguration: UIButton.Configuration {
    ...
    case .invalidCertificate:
      configuration.baseForegroundColor = UIColor(qoraiSystemName: .systemfeedbackErrorIcon)
      if isTitleVisible {
        configuration.attributedTitle = title
      }
      configuration.image = UIImage(qoraiSystemNamed: "qora.warning.triangle-filled")
    case .missingSSL, .mixedContent:
      configuration.baseForegroundColor = UIColor(qoraiSystemName: .textTertiary)
      if isTitleVisible {
        configuration.attributedTitle = title
      }
      configuration.image = UIImage(qoraiSystemNamed: "qora.warning.triangle-filled")
    case .unknown:
      configuration.baseForegroundColor = UIColor(qoraiSystemName: .iconDefault)
      configuration.image = UIImage(qoraiSystemNamed: "qora.info.filled")
    }
        ...
        if wasHidden != (state == TranslateURLBarButton.TranslateState.unavailable) {
          ...
          if !translateButton.isHidden {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
              UIAccessibility.post(
                notification: .announcement,
                argument: Strings.QoraiTranslate.availableVoiceOverAnnouncement
              )
            }
          }
        }
  ...
  lazy var reloadButton = ToolbarButton().then {
    ...
    $0.setImage(UIImage(qoraiSystemNamed: "qora.browser.refresh", compatibleWith: nil), for: .normal)
    $0.tintColor = .qoraiLabel
  ...
  private(set) lazy var progressBar = GradientProgressBar().then {
    ...
    $0.setGradientColors(startColor: .qoraiBlurpleTint, endColor: .qoraiBlurpleTint)
  }
...
private class DisplayURLLabel: UILabel {
  ...
  let clippingFade = GradientView(
    colors: [.qoraiBackground, .qoraiBackground.withAlphaComponent(0.0)],
    positions: [0, 1],
