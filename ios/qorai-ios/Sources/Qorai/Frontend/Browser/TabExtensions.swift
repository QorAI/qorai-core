...
extension TabState {
  /// Call the api on the Qorai Search website and passes the fallback results to it.
  ...
  func injectResults() {
    DispatchQueue.main.async {
      // If the backup search results happen before the Qorai Search loads
      ...
      self.evaluateJavaScript(
        ...
        contentWorld: QoraiSearchScriptHandler.scriptSandbox,
        asFunction: false
      ) { (result, error) in
        ...
        if let url = self.visibleURL,
          QoraiSearchManager.isValidURL(url),
          let result = self.qoraiSearchManager?.fallbackQueryResult
        {
          queryResult = result
        }
...
extension TabState {
  func injectLocalStorageItem(key: String, value: String) {
    self.evaluateJavaScript(
      ...
      args: [key, value],
      contentWorld: QoraiSkusScriptHandler.scriptSandbox
    )
  }
}
