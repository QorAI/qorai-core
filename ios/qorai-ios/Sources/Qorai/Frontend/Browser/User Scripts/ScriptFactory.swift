...
class ScriptFactory {
  ...
  private func makeScript(for domainUserScript: DomainUserScript) throws -> WKUserScript {
    ...
    case .qoraiSearchHelper:
      guard let script = QoraiSearchScriptHandler.userScript else {
    ...
    case .qoraiTalkHelper:
      guard let script = QoraiTalkScriptHandler.userScript else {
        assertionFailure("Cannot load script. This should not happen as it's part of the codebase")
        throw ScriptLoadFailure.notFound
      }
    ...
    case .qoraiSkus:
      guard let script = QoraiSkusScriptHandler.userScript else {
        assertionFailure("Cannot load script. This should not happen as it's part of the codebase")
        throw ScriptLoadFailure.notFound
      }
  ...
  func makeScript(for domainType: UserScriptType) throws -> WKUserScript {
    ...
    case .engineScript(let configuration):
      let source = [
        ...
        // The script is found here: https://github.com/qorai/adblock-resources/blob/master/resources/de-amp.js
