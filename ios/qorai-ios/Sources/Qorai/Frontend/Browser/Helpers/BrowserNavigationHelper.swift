...
class BrowserNavigationHelper {
  ...
  func openDownloads(_ completion: @escaping (Bool) -> Void) {
    UIApplication.shared.openQoraiDownloadsFolder(completion)
  }
  ...
  func openVPNBuyScreen(iapObserver: QoraiVPNInAppPurchaseObserver) {
    ...
    let vpnPaywallView = QoraiVPNPaywallView(
      ...
      installVPNProfile: { [weak bvc] in
        guard let bvc = bvc else { return }
        bvc.popToBVC()
        bvc.openInsideSettingsNavigation(with: QoraiVPNInstallViewController())
      }
    )
