...
class TabBrowserData: NSObject, TabObserver {
  ...
  init(
    tab: some TabState,
    tabGeneratorAPI: QoraiTabGeneratorAPI? = nil
  ) {
    self.tab = tab
    _syncTab = tabGeneratorAPI?.createQoraiSyncTab(isOffTheRecord: tab.isPrivate)
  ...
  private var _syncTab: QoraiSyncTab?
  private var _faviconDriver: FaviconDriver?
  private var _walletEthProvider: QoraiWalletEthereumProvider?
  private var _walletSolProvider: QoraiWalletSolanaProvider?
  private var _walletKeyringService: QoraiWalletKeyringService? {
    didSet {
      _walletKeyringService?.addObserver(self)
    }
  }

  weak var syncTab: QoraiSyncTab? {
    _syncTab
  }
  ...
  weak var walletEthProvider: QoraiWalletEthereumProvider? {
    get { _walletEthProvider }
    set { _walletEthProvider = newValue }
  }

  weak var walletSolProvider: QoraiWalletSolanaProvider? {
    get { _walletSolProvider }
    set { _walletSolProvider = newValue }
  }

  weak var walletKeyringService: QoraiWalletKeyringService? {
  ...
  // and report same document navigations to Qorai Rewards library.
  ...
  var translateHelper: QoraiTranslateTabHelper?
  private(set) lazy var leoTabHelper = QoraiQoraScriptTabHelper(tab: tab)
  ...
  /// A helper property that handles native to Qorai Search communication.
  var qoraiSearchManager: QoraiSearchManager?

  /// A helper property that handles Qorai Search Result Ads.
  var qoraiSearchResultAdManager: QoraiSearchResultAdManager?
