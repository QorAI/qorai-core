...
class SyncAddDeviceViewController: SyncViewController {
  ...
  private var modeControl = UISegmentedControl(items: [Strings.Sync.QRCode, Strings.Sync.codeWords])
    .then {
      ...
      $0.selectedSegmentTintColor = UIColor(qoraiSystemName: .containerBackground)
      $0.setTitleTextAttributes(
        [.foregroundColor: UIColor(qoraiSystemName: .textPrimary)],
        for: .selected
      )
    }
  ...
  init(title: String, type: SyncDeviceType, syncAPI: QoraiSyncAPI) {
  ...
  private func fetchExpirationDate(for type: SyncDeviceType, using syncAPI: QoraiSyncAPI) -> Date? {
