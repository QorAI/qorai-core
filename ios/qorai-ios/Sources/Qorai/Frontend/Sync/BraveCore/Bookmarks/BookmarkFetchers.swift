...
class Bookmarkv2Fetcher: NSObject, BookmarksV2FetchResultsController {
  ...
  private weak var bookmarksAPI: QoraiBookmarksAPI?

  private let parentNode: BookmarkNode?
  private var children = [Bookmarkv2]()

  init(_ parentNode: BookmarkNode?, api: QoraiBookmarksAPI) {
...
class Bookmarkv2ExclusiveFetcher: NSObject, BookmarksV2FetchResultsController {
  ...
  init(_ excludedFolder: BookmarkNode?, api: QoraiBookmarksAPI) {
  ...
  private func getNestedFolders(_ node: BookmarkNode, guid: String?) -> [Bookmarkv2] {
    if let guid = guid {
      return node.nestedChildFolders.filter({ $0.bookmarkNode.guid != guid }).map({
        QoraiBookmarkFolder($0)
      })
    }
    return node.nestedChildFolders.map({ QoraiBookmarkFolder($0) })
  }
}
