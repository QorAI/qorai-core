...
class BookmarkManager {
  ...
  init(bookmarksAPI: QoraiBookmarksAPI?) {
  ...
  public func byFrequency(
    ...
    // Invalid query.. QoraiCore doesn't store bookmarks based on last visited.
    // Any last visited bookmarks would show up in `History` anyway.
    // QoraiCore automatically sorts them by date as well.
  ...
  public func reorderBookmarks(
      ...
      let parent = node.parent ?? bookmarksAPI.mobileNode
    {
      ...
      // However, the Qorai-Core delegate is being called before the move is actually complete OR too quickly
  ...
  private let bookmarksAPI: QoraiBookmarksAPI?
  ...
  private func recordTotalBookmarkCountP3A() {
    ...
    UmaHistogramRecordValueToBucket(
      "Qorai.Core.BookmarksCountOnProfileLoad.2",
      buckets: [
