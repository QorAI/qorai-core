...
public struct QoraiSyncDevice: Codable {
...
extension QoraiSyncAPI {
  ...
  var isSendTabToSelfVisible: Bool {
    ...
    do {
      let devices = try JSONDecoder().decode([QoraiSyncDevice].self, from: data)
  ...
  @discardableResult
  func joinSyncGroup(codeWords: String, syncProfileService: QoraiSyncProfileServiceIOS) -> Bool {
    if setSyncCode(codeWords) {
      ...
      Preferences.Chromium.syncPasswordsEnabled.value =
        FeatureList.kQoraiSyncDefaultPasswords.enabled
  ...
  /// Removing Observers, clearing local preferences and calling reset chain on qorai-core side
  ...
  func resetSyncChain() {
    ...
    Preferences.Chromium.syncPasswordsEnabled.value = FeatureList.kQoraiSyncDefaultPasswords.enabled
    Preferences.Chromium.syncOpenTabsEnabled.value = false

    resetSync()
  }

  public func enableSyncTypes(syncProfileService: QoraiSyncProfileServiceIOS) {
  ...
  func addServiceStateObserver(
    ...
    let serviceStateListener = QoraiSyncServiceListener(onRemoved: { [weak self] observer in
  ...
  func addDeviceStateObserver(_ observer: @escaping () -> Void) -> AnyObject {
    let deviceStateListener = QoraiSyncDeviceListener(
      observer,
  ...
  public func removeAllObservers() {
    serviceObservers.objectEnumerator().forEach({
      ($0 as? QoraiSyncServiceListener)?.observer = nil
    })

    deviceObservers.objectEnumerator().forEach({
      ($0 as? QoraiSyncDeviceListener)?.observer = nil
    })
  ...
  private var serviceObservers: NSHashTable<QoraiSyncServiceListener> {
    if let observers = objc_getAssociatedObject(self, &AssociatedObjectKeys.serviceObservers)
      as? NSHashTable<QoraiSyncServiceListener>
    {
      return observers
    }

    let defaultValue = NSHashTable<QoraiSyncServiceListener>.weakObjects()
  ...
  private var deviceObservers: NSHashTable<QoraiSyncDeviceListener> {
    if let observers = objc_getAssociatedObject(self, &AssociatedObjectKeys.deviceObservers)
      as? NSHashTable<QoraiSyncDeviceListener>
    {
      return observers
    }

    let defaultValue = NSHashTable<QoraiSyncDeviceListener>.weakObjects()
    objc_setAssociatedObject(
      self,
...
extension QoraiSyncAPI {
  private class QoraiSyncServiceListener: NSObject {

    // MARK: Internal

    var observer: Any?
    private var onRemoved: (QoraiSyncServiceListener) -> Void

    // MARK: Lifecycle

    fileprivate init(onRemoved: @escaping (QoraiSyncServiceListener) -> Void) {
  ...
  private class QoraiSyncDeviceListener: NSObject {

    // MARK: Internal

    var observer: Any?
    private var onRemoved: (QoraiSyncDeviceListener) -> Void

    // MARK: Lifecycle

    fileprivate init(
      _ onDeviceInfoChanged: @escaping () -> Void,
      onRemoved: @escaping (QoraiSyncDeviceListener) -> Void
    ) {
      self.onRemoved = onRemoved
      super.init()
    }
...
extension QoraiSyncAPI.QrCodeDataValidationResult {
...
extension QoraiSyncAPI.WordsValidationStatus {
  var errorDescription: String {
    switch self {
    case .valid:
      return ""
