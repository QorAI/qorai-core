  ...
  UITableViewDataSource
{
  ...
  private let qoraiCoreMain: QoraiProfileController
  private let syncAPI: QoraiSyncAPI
  private let syncProfileService: QoraiSyncProfileServiceIOS

  private var syncDeviceObserver: AnyObject?
  private var syncServiceObserver: AnyObject?
  private var devices = [QoraiSyncDevice]()
  ...
  private lazy var emptyStateOverlayView = EmptyStateOverlayView(
    overlayDetails: EmptyOverlayStateDetails(
      ...
      icon: UIImage(qoraiSystemNamed: "qora.product.sync")
    )
  )
  ...
  init(
    ...
    qoraiCoreMain: QoraiProfileController,
    windowProtection: WindowProtection?
  ) {
    self.isModallyPresented = isModallyPresented
    self.qoraiCoreMain = qoraiCoreMain
    self.syncAPI = qoraiCoreMain.syncAPI
    self.syncProfileService = qoraiCoreMain.syncProfileService
  ...
  private func presentAlertPopup(for type: AlertActionType, device: QoraiSyncDevice? = nil) {
    var title: String?
    var message: String?
    ...
    popup.addButton(title: popupButtonName, type: .destructive) {
      ...
      case .syncChainDeleteConfirmation:
        self.doIfConnected {
          ...
          // Permanently Delete action is called on qorai-core side
          self.syncAPI.permanentlyDeleteAccount { status in
            ...
            default:
              // Clearing local preferences and calling reset chain on qorai-core side
...
extension SyncSettingsTableViewController {
    ...
    if section == .otherActions {
      if indexPath.row == 0 {
        ...
        let syncInternalsController = ChromeWebUIController(
          ...
          $0.webView.load(URLRequest(url: URL(string: "qorai://sync-internals")!))
        }
  ...
  private func configureCell(_ cell: UITableViewCell, atIndexPath indexPath: IndexPath) {
    ...
    case Sections.deviceList.rawValue:
      ...
      cell.textLabel?.textColor = .qoraiLabel
    case Sections.otherActions.rawValue:
      configureButtonCell(cell, atIndexPath: indexPath)
    case Sections.syncTypes.rawValue:
  ...
  private func configureButtonCell(_ cell: UITableViewCell, atIndexPath indexPath: IndexPath) {
    ...
    case Sections.deviceList.rawValue:
      ...
      cell.textLabel?.textColor = .qoraiBlurpleTint
    ...
    case Sections.otherActions.rawValue:
      if indexPath.row == 0 {
        ...
        cell.textLabel?.textColor = .qoraiErrorLabel
      }
    default:
      return
    }
  ...
  private func makeInformationHeaderTextView(with info: String) -> UITextView {
    return UITextView().then {
      ...
      $0.textColor = UIColor(qoraiSystemName: .textSecondary)
