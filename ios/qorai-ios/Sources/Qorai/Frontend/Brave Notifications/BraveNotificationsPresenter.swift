...
private class QoraiNotificationGesture: UIGestureRecognizer {
  var onBegan: () -> Void
  var onEnded: () -> Void
...
class QoraiNotificationsPresenter: UIViewController {
  private var notificationsQueue: [QoraiNotification] = []
  private var widthAnchor: NSLayoutConstraint?
  private var visibleNotification: QoraiNotification?
  ...
  func display(notification: QoraiNotification, from presentingController: UIViewController) {
    ...
    let customGesture = QoraiNotificationGesture(
  ...
  func hide(_ notification: QoraiNotification) {
  ...
  private func enqueueNotification(_ notification: QoraiNotification) {
    ...
    if !notification.id.isEmpty,
      notificationsQueue.contains(where: { $0.id == notification.id })
        || visibleNotification?.id == notification.id
    {
      return
    }
  ...
  private func setupTimeoutTimer(for notification: QoraiNotification, interval: TimeInterval) {
    if let timer = dismissTimers[notification.id] {
      // Invalidate and reschedule
      timer.invalidate()
    }
    dismissTimers[notification.id] = Timer.scheduledTimer(
  ...
  @objc private func dismissPannedAdView(_ pan: UIPanGestureRecognizer) {
    ...
    guard let notification = visibleNotification, let notificationView = pan.view,
  ...
  private func animateIn(notification: QoraiNotification) {
  ...
  private func animateOut(
    notification: QoraiNotification,
    ...
    let springTiming = UISpringTimingParameters(
      dampingRatio: 0.9,
      initialVelocity: velocity.map { CGVector(dx: 0, dy: $0) } ?? .zero
    )
...
extension QoraiNotificationsPresenter {
  class PresenterView: UIView {
    weak var viewController: QoraiNotificationsPresenter?

    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
      guard let view = super.hitTest(point, with: event) else { return nil }
      // Only allow tapping the nitification part of this VC
      if let notificationView = viewController?.visibleNotification?.view,
        view.isDescendant(of: notificationView)
      {
        return view
      }
      return nil
...
extension QoraiNotificationsPresenter: UIGestureRecognizerDelegate {
