...
struct ShieldsPanelView: View {
    ...
    VStack(alignment: .center, spacing: 8) {
      HStack(alignment: .center, spacing: 8) {
        ...
        URLElidedText(text: displayHost)
          .font(.title2)
          .foregroundStyle(Color(.qoraiPrimary))
      }
  ...
  @ViewBuilder private var shieldsReportView: some View {
    HStack(spacing: 2) {
      ...
      .background(Color(.secondaryQoraiBackground).cornerRadius(8))

      NavigationLink {
        AboutQoraiShieldsView()
      } label: {
        HStack {
          Image(qoraiSystemName: "qora.help.outline")
            .font(.title2)
        }
        .padding()
      ...
      Button {
        ...
        HStack {
          Image(qoraiSystemName: "qora.share")
            .font(.title2)
        }
        .padding()
        .contentShape(RoundedRectangle(cornerRadius: 8))
  ...
  @ViewBuilder private var shieldSettingsSectionView: some View {
    ...
    ShieldSettingRow {
      HStack {
        ...
        .tint(Color(.secondaryQoraiLabel))
        .buttonStyle(.plain)
    ...
    if FeatureList.kQoraiIOSDebugAdblock.enabled, let contentBlocker = tab.contentBlocker {
      ShieldSettingRow {
        NavigationLink {
      ...
      Button {
        ...
        ShieldSettingsNavigationWrapper {
          Label(
            ...
            qoraiSystemImage: "qora.globe.block"
          )
          .frame(maxWidth: .infinity, alignment: .leading)
          .multilineTextAlignment(.leading)
          .foregroundStyle(Color(.qoraiPrimary))
...
private struct ShieldsPanelDisclosureStyle: DisclosureGroupStyle {
  func makeBody(configuration: Configuration) -> some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      .buttonStyle(.plain)
      .frame(maxWidth: .infinity, alignment: .center)
      .background(Color(.secondaryQoraiBackground))
      .hoverEffect()
