    ...
    NavigationStack {
      ...
      .overlay {
        if isSubmittingReport {
          ProgressView()
            .progressViewStyle(.qoraiCircular(size: .normal, tint: .qoraiBlurpleTint))
            .frame(maxWidth: .infinity, maxHeight: .infinity)
            .background(Color(.qoraiBackground).opacity(0.5).ignoresSafeArea())
  ...
  @MainActor func createAndSubmitReport() async {
    ...
    webcompatReporterAPI.submitWebcompatReport(
      reportInfo: .init(
        ...
        qoraiVersion: version,
        reportUrl: url.absoluteString,
        shieldsEnabled: String(
          tab?.qoraiShieldsHelper?.isQoraiShieldsEnabled(for: tab?.visibleURL) ?? true
        ),
        adBlockSetting: (tab?.qoraiShieldsHelper?.shieldLevel(
          for: tab?.visibleURL,
          considerAllShieldsOption: false
        ...
        qoraiVpnConnected: String(QoraiVPN.isConnected),
...
#Preview {
  SubmitReportView(
    url: URL(string: "https://qorai.com/privacy-features")!,
