...
class ShieldsSettingsViewModel: ObservableObject {
  ...
  @Published var shieldsEnabled: Bool {
    didSet {
      ...
      tab.qoraiShieldsHelper?.setQoraiShieldsEnabled(shieldsEnabled, for: tab.visibleURL)
      updateState()
    }
  }
  @Published var blockAdsAndTrackingLevel: ShieldLevel {
    didSet {
      guard !isUpdatingState else { return }
      tab.qoraiShieldsHelper?.setShieldLevel(blockAdsAndTrackingLevel, for: tab.visibleURL)
    }
  }
  @Published var blockScripts: Bool {
    didSet {
      guard !isUpdatingState else { return }
      tab.qoraiShieldsHelper?.setBlockScriptsEnabled(blockScripts, for: tab.visibleURL)
    }
  }
  @Published var fingerprintProtection: Bool {
    didSet {
      guard !isUpdatingState else { return }
      tab.qoraiShieldsHelper?.setBlockFingerprintingEnabled(
  ...
  init(tab: some TabState) {
    self.tab = tab
    self.shieldsEnabled =
      tab.qoraiShieldsHelper?.isQoraiShieldsEnabled(for: tab.visibleURL)
      ?? true
  ...
  private func updateState() {
    ...
    shieldsEnabled = tab.qoraiShieldsHelper?.isQoraiShieldsEnabled(for: tab.visibleURL) ?? true
    // `considerAlwaysAggressiveETLDs` is `false` to mask that we may force
    // aggressive mode on some domains.
