...
class QoraiTalkRewardsOptInViewController: UIViewController, PopoverContentComponent {
  ...
  override func viewDidLoad() {
    ...
    qoraiTalkView.enableRewardsButton.addTarget(
      self,
      action: #selector(enableRewardsAction),
      for: .touchUpInside
    )
    qoraiTalkView.disclaimer.onLinkedTapped = { [unowned self] link in
      var request: URLRequest?

      self.dismiss(animated: true) {
        switch link.absoluteString {
        case "tos":
          request = URLRequest(url: .qorai.batTermsOfUse)
        case "privacy-policy":
          request = URLRequest(url: .qorai.privacy)
        default:
          assertionFailure()
        }
  ...
  private func updatePreferredContentSize() {
    let baseHeight: CGFloat = QoraiUX.baseDimensionValue
