...
class QoraiRewardsViewController: UIViewController, PopoverContentComponent {
  enum Action {
  ...
  private var publisher: QoraiCore.QoraiRewards.PublisherInfo? {
    didSet {
      ...
      rewardsView.publisherView.hostLabel.attributedText = publisher?.attributedDisplayName(
        fontSize: QoraiRewardsPublisherView.UX.hostLabelFontSize
      )
  ...
  init(tab: some TabState, rewards: QoraiRewards) {
    self.browserTab = tab
  ...
  private var rewardsView: QoraiRewardsView {
    view as! QoraiRewardsView
  }

  override func loadView() {
    view = QoraiRewardsView()
  }
