...
/// A structure used to register a device for Qorai's DeviceCheck enrollment
public struct DeviceCheckRegistration: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    guard let data = Data(base64Encoded: try container.decode(String.self, forKey: .enrollmentBlob))
      ...
      throw NSError(
        domain: "com.qorai.device.check.enrollment",
        code: -1,
        userInfo: [
...
public struct DeviceCheckEnrollment: Codable {
  // The payment Id from Qorai Rewards in UUIDv4 Format.
...
public class DeviceCheckClient {
  ...
  private static let privateKeyId = "com.qorai.device.check.private.key"

  // The current build environment
  private let environment: QoraiRewards.Configuration.Environment
  ...
  public init(
    environment: QoraiRewards.Configuration.Environment = QoraiRewards.Configuration.current()
      .environment
  ) {
...
extension DeviceCheckClient {
  ...
  private var baseURL: URL? {
    switch environment {
    case .development:
      return URL(string: "https://grant.rewards.qorai.software")
    case .staging:
      return URL(string: "https://grant.rewards.qoraisoftware.com")
    case .production:
      return URL(string: "https://grant.rewards.qorai.com")
    }
  }
