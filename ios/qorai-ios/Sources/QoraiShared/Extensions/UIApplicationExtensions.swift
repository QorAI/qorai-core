...
extension UIApplication {
  ...
  public func openQoraiDownloadsFolder(_ completion: @escaping (Bool) -> Void) {
    Task {
      do {
        guard
        ...
        guard let qoraiFolderURL = downloadsPathComponents.url else {
          completion(false)
          return
        }

        UIApplication.shared.open(qoraiFolderURL) { success in
          completion(success)
        }
      } catch {
        completion(false)
