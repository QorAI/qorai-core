...
public class TabStateFactory {
  public struct CreateTabParams {
    ...
    public var qoraiCore: QoraiProfileController?

    public init(
      id: UUID = .init(),
      initialConfiguration: WKWebViewConfiguration? = nil,
      lastActiveTime: Date? = nil,
      qoraiCore: QoraiProfileController? = nil
    ) {
      self.id = id
  ...
  public static func create(with params: CreateTabParams) -> any TabState {
    ...
    if let qoraiCore = params.qoraiCore, FeatureList.kUseChromiumWebViews.enabled {
      let cwvConfiuration =
        wkConfiguration.websiteDataStore.isPersistent
        ? qoraiCore.defaultWebViewConfiguration
        : qoraiCore.nonPersistentWebViewConfiguration
...
public protocol TabState: AnyObject {
  ...
  // TODO: Should be get only, make favicon fetch logic internal (qorai/qorai-browser#45095)
