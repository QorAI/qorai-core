...
class TabWKNavigationHandler: NSObject, WKNavigationDelegate {
  ...
  private func defaultAllowPolicy(
    ...
    {
      // Stop Qorai from opening universal links by using the private enum value
  ...
  func webView(
    ...
    if ["http", "https", "data", "blob", "file"].contains(requestURL.scheme) {
      ...
      if #unavailable(iOS 18.0) {
        ...
        // correctly in all cases: https://github.com/qorai/qorai-ios/issues/8585
  ...
  func webView(
    ...
    if challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodServerTrust {
      if let serverTrust = challenge.protectionSpace.serverTrust {
        ...
        let result = await QoraiCertificateUtils.verifyTrust(serverTrust, host: host, port: port)
        ...
        if result == Int32.min {
          ...
          // This is the same as calling `QoraiCertificateUtils.evaluateTrust` but with more error info provided by WebKit
          return (.performDefaultHandling, nil)
        }

        // Cert is invalid and cannot be pinned
        Logger.module.error("CERTIFICATE_INVALID")
        let errorCode = CFNetworkErrors.qoraiCertificatePinningFailed.rawValue
