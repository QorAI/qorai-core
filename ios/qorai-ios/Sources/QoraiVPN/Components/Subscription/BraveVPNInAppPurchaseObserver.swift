...
public class QoraiVPNIAPObserverManager: NSObject, ObservableObject {

  public enum QoraiVPNPaymentStatus: Equatable {
    case ongoing
    case success(receiptValidationRequired: Bool)
    case failure(_ error: QoraiVPNInAppPurchaseObserver.PurchaseError?)
    case unknown
  }

  @Published var paymentStatus: QoraiVPNPaymentStatus = .unknown

  private let iapObserver: QoraiVPNInAppPurchaseObserver
...
extension QoraiVPNIAPObserverManager: QoraiVPNInAppPurchaseObserverDelegate {
...
public protocol QoraiVPNInAppPurchaseObserverDelegate: AnyObject {
  func purchasedOrRestoredProduct(validateReceipt: Bool)
  func purchaseFailed(error: QoraiVPNInAppPurchaseObserver.PurchaseError)
  func handlePromotedInAppPurchase()
}

public class QoraiVPNInAppPurchaseObserver: NSObject, SKPaymentTransactionObserver {
  ...
  public weak var delegate: QoraiVPNInAppPurchaseObserverDelegate?
  ...
  private func processTransactions(_ transactions: [SKPaymentTransaction], on queue: SKPaymentQueue)
  {
    ...
    let vpnTransactions = transactions.filter({
      ($0.payment.productIdentifier == QoraiStoreProduct.vpnMonthly.rawValue)
        || ($0.payment.productIdentifier == QoraiStoreProduct.vpnYearly.rawValue)
    })
      ...
      .forEach { transaction in
        ...
        case .restored:
          ...
          if callPurchaseDelegateOnce {
            ...
            Task {
              do {
                let response = try await QoraiVPN.validateReceiptData()
                if response?.status == .expired {
                  ...
                  // If we purchased via Apple's IAP we reset the Qorai SKUs credential
                  // to avoid mixing two purchase types in the app.
                  //
                  // The user will be able to retrieve the shared credential
                  // after log in to account.qorai website.
                  QoraiVPN.clearSkusCredentials(includeExpirationDate: false)
                }
              } catch {
  ...
  public func paymentQueue(
    ...
    guard
      productIdentifier == QoraiStoreProduct.vpnMonthly.rawValue
        || productIdentifier == QoraiStoreProduct.vpnYearly.rawValue
    else {
      return false
    }
...
extension QoraiVPNInAppPurchaseObserver {
  @MainActor
