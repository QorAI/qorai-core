...
struct QoraiRegionDetailsView: View {
  ...
  var body: some View {
    ...
    .background {
      QoraiVPNRegionConfirmationContentView(
    ...
    .onReceive(NotificationCenter.default.publisher(for: .NEVPNStatusDidChange)) { _ in
      let isVPNEnabled = QoraiVPN.isConnected
  ...
  @ViewBuilder
  private func cityRegionItem(region: QoraiVPNCityRegion) -> some View {
    HStack {
      VStack(alignment: .leading) {
        Text(region.displayName.capitalizeFirstLetter)
          .foregroundStyle(
            cityRegionDetail.selectedRegion == region
              ? Color(qoraiSystemName: .textInteractive)
  ...
  private func generateServerCountDetails(for region: QoraiVPNCityRegion) -> String {
  ...
  private func selectDesignatedVPNCity(_ region: QoraiVPNCityRegion) {
    ...
    Task { @MainActor in
      ...
      var regionPrecision: QoraiVPN.RegionPrecision
      ...
      if region.regionName == QoraiVPNCityRegion.optimalCityRegionName {
      ...
      let success = await QoraiVPN.changeVPNRegionForPrecision(
        to: regionToChange,
...
#if DEBUG
struct QoraiRegionDetailsView_Previews: PreviewProvider {
  static var previews: some View {
    QoraiRegionDetailsView(countryRegion: GRDRegion(dictionary: [:]))
  }
}
#endif
