...
class QoraiVPNContactFormViewController: TableViewController {

  private let supportEmail = "qoraiios@guardianapp.com"
  ...
  override func viewDidLoad() {
    // MARK: Hostname
    let hostname = QoraiVPN.hostname
    let hostnameRow = Row(
    ...
    let subscriptionType = QoraiVPN.subscriptionName
      ...
      Row(
        ...
        accessory: .view(
          SwitchAccessoryView(
            ...
            valueChange: { [weak self] isOn in
              if isOn {
                self?.contactForm.logs = QoraiVPN.errorLog
              } else {
...
extension QoraiVPNContactFormViewController: MFMailComposeViewControllerDelegate {
