...
class QoraiVPNLinkSwitchView: UIView {
...
class QoraiVPNLinkSwitchCell: UITableViewCell, Cell {
  ...
  public func configure(row: Row) {
    guard case .view(let view) = row.accessory, let switchView = view as? QoraiVPNLinkSwitchView
    else {
      fatalError("row.accessory must be of type: QoraiVPNLinkSwitchView")
    }

    var titleAccessory: UIImage?
    if let imageName = row.context?[QoraiVPNLinkSwitchCell.textAccessoryKey] as? String {
