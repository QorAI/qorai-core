...
extension QoraiVPNInstallViewController {
  class View: UIView {
    ...
    private let imageView = UIView().then {
      $0.backgroundColor = QoraiVPNCommonUI.UX.purpleBackgroundColor
    ...
    private lazy var infoStackView = UIStackView().then {
      ...
      let textStackView = UIStackView().then { stackView in
        ...
        let titleLabel = QoraiVPNCommonUI.Views.ShrinkableLabel().then { label in
        ...
        let bodyLabel = QoraiVPNCommonUI.Views.ShrinkableLabel().then { label in
    ...
    let installVPNButton = QoraiButton().then {
      $0.setTitle(Strings.VPN.installProfileButtonText, for: .normal)
      $0.backgroundColor = .qoraiDarkerBlurple
    ...
    let contactSupportButton = QoraiButton(type: .system).then {
      $0.setTitle(Strings.VPN.settingsContactSupport, for: .normal)
      $0.titleLabel?.font = .systemFont(ofSize: 16)
      $0.setTitleColor(.secondaryQoraiLabel, for: .normal)
      $0.snp.makeConstraints { make in
        make.height.equalTo(44)
      }
      $0.contentEdgeInsets = UIEdgeInsets(top: 0, left: 25, bottom: 0, right: 25)
