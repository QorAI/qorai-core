...
struct QoraiVPNPremiumUpsellView: View {

  var body: some View {
    VStack(spacing: 8) {
      ForEach(QoraiVPNUpsellTopicView.UpsellTopicType.allCases) { type in
        QoraiVPNUpsellTopicView(topicType: type)
      }
    }
...
#Preview("UpsellTopicTypeView") {
  QoraiVPNUpsellTopicView(topicType: .speed)
    .padding([.horizontal, .top], 24)
    .padding(.bottom, 16)
    .background(
      Color(qoraiSystemName: .primitivePrimary10)
