...
struct QoraiVPNCityRegion: Identifiable, Equatable {
...
class VPNCityRegionDetail: ObservableObject {
  var cityRegions = [
    QoraiVPNCityRegion(
      ...
      regionName: QoraiVPNCityRegion.optimalCityRegionName,
      isAutomatic: true
    )
  ]
  ...
  @Published var selectedRegion: QoraiVPNCityRegion? = nil

  func assignSelectedRegion(
    countryName: String,
    cityRegions: [QoraiVPNCityRegion]
  ) {
    self.countryName = countryName
    self.cityRegions.append(contentsOf: cityRegions)

    let selectedCity = cityRegions.first(where: {
      $0.regionName == QoraiVPN.selectedRegion?.regionName
    })
