...
/// A static class to handle all things related to the Qorai VPN service.
public class QoraiVPN {
  ...
  public static let iapObserver = QoraiVPNInAppPurchaseObserver()

  private static let connectionName = "Qorai VPN"  // Non translatable
  ...
  public static func initialize(customCredential: QoraiVPNSkusCredential?) {
    ...
    helper.tunnelProviderBundleIdentifier = AppInfo.baseBundleIdentifier + ".QoraiWireGuard"
  ...
  public static var isSmartProxyRoutingEnabled: Bool {
    ...
    set {
      if newValue {
        ...
        QoraiVPN.smartProxyRoutingHosts = QoraiVPN.helper.smartProxyRoutingHosts ?? []
      } else {
        GRDVPNHelper.disableSmartProxyRouting()
        QoraiVPN.smartProxyRoutingHosts = []
      }
    }
  }
  ...
  /// Returns true if the user is connected to Qorai's vpn at the moment.
  ...
  public static func setCustomVPNCredential(_ credential: QoraiVPNSkusCredential) {
    GRDSubscriptionManager.setIsPayingUser(true)
    populateRegionDataIfNecessary()

    let dict: NSMutableDictionary =
      [
        "qorai-vpn-premium-monthly-pass": credential.guardianCredential,
        "qorai-payments-env": credential.environment,
        "validation-method": "qorai-premium",
      ]
  ...
  public static func processVPNAlerts() {
    ...
    Task {
      ...
      do {
        ...
        let decoded = try JSONDecoder().decode([QoraiVPNAlertJSONModel].self, from: dataAsJSON)

        QoraiVPNAlert.batchInsertIfNotExists(alerts: decoded)
      } catch {
  ...
  private static let errorLogQueue = DispatchQueue(label: "com.qorai.errorLogQueue")
