...
extension PrefService {
  /// Whether or not the Qorai VPN feature in general is available to use and the UI should display
  /// buttons/settings for it.
  public var isQoraiVPNAvailable: Bool {
    // Right now this feature is always available unless its managed/forced by policy
    let isDisabledByPolicy =
      isManagedPreference(forPath: kManagedQoraiVPNDisabledPrefName)
      && boolean(forPath: kManagedQoraiVPNDisabledPrefName)
