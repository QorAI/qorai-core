...
public struct BrowserMenu: View {
  ...
  public var body: some View {
    ScrollView {
      VStack(spacing: 16) {
        ...
        if case .connected(let region) = model.vpnStatus {
          Button {
            ...
            Label {
              ...
              Image(qoraiSystemName: "qora.product.vpn")
            }
          }
          .buttonStyle(MenuRowButtonStyle())
          .background(Color(qoraiSystemName: .schemesSurfaceBright))
        ...
        Button {
          ...
          Label(Strings.BrowserMenu.allSettingsButtonTitle, qoraiSystemImage: "qora.settings")
        }
        .buttonStyle(MenuRowButtonStyle())
        .background(Color(qoraiSystemName: .schemesSurfaceBright))
        .clipShape(.rect(cornerRadius: 14, style: .continuous))
    ...
    .background(Color(qoraiSystemName: .iosBrowserChromeBackgroundIos))
      ...
      ForEach($actions) { $action in
        Button {
          ...
          Label {
            ...
            Image(qoraiSystemName: action.image)
          }
        }
      ...
      func makeBody(configuration: Configuration) -> some View {
        VStack(spacing: 8) {
          configuration.icon
            ...
            .foregroundStyle(
              state == true
                ? Color(qoraiSystemName: .schemesOnPrimary)
                : isEnabled
                  ? Color(qoraiSystemName: .iconDefault)
                  : Color(qoraiSystemName: .iconDisabled)
            )
            .frame(maxWidth: .infinity)
            .background {
              Color(qoraiSystemName: state == true ? .schemesPrimary : .schemesSurfaceBright)
                .overlay(
                  Color(
                    qoraiSystemName: state == true
            ...
            .overlay(alignment: .topTrailing) {
              if let badgeColor = traits.badgeColor {
                Circle()
                  ...
                  .overlay {
                    Circle()
                      .stroke(Color(qoraiSystemName: .iosBrowserChromeBackgroundIos), lineWidth: 1)
                  }
          ...
          configuration.title
            ...
            .foregroundStyle(
              isEnabled
                ? Color(qoraiSystemName: .textPrimary)
                : Color(qoraiSystemName: .textDisabled)
            )
    ...
    VStack(alignment: .leading, spacing: 0) {
      ForEach($actions) { $action in
        ...
        if !additionalActions.isEmpty || action.id != actions.last?.id {
          Color(qoraiSystemName: .materialDivider)
      ...
      if !additionalActions.isEmpty {
        if !isAdditionalActionsVisible {
          Button {
            ...
            Label(Strings.BrowserMenu.showAllButtonTitle, qoraiSystemImage: "qora.more.horizontal")
          }
          .buttonStyle(MenuRowButtonStyle())
        }
        if isAdditionalActionsVisible {
...
private struct MenuRowButtonStyle: ButtonStyle {
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      ...
      .hoverEffect()
      .background(
        Color(qoraiSystemName: .iosBrowserContainerHighlightIos)
