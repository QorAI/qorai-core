...
/// Stores the alerts data we receive from Qorai VPN.
/// The alert is a resource blocked by the VPN service
public final class QoraiVPNAlert: NSManagedObject, CRUD, Identifiable {
  ...
  public static func batchInsertIfNotExists(alerts: [QoraiVPNAlertJSONModel]) {
    DataController.perform { context in
      ...
      alerts.forEach {
        let vpnAlert = QoraiVPNAlert(entity: entity, insertInto: context)
  ...
  public static func allByHostCount(completion: @escaping (Set<CountableEntity>) -> Void) {
    ...
    DataController.perform { context in
      let fetchRequest = qoraiVPNAlertFetchRequest(for: context)
  ...
  public static func last(_ count: Int) -> [QoraiVPNAlert]? {
    let timestampSort = NSSortDescriptor(keyPath: \QoraiVPNAlert.timestamp, ascending: false)
    let nonConsolidatedAlertsPredicate = NSPredicate(
      format: "\(#keyPath(QoraiVPNAlert.timestamp)) > 0"
    )
    ...
    DataController.perform { context in
      ...
      do {
        fetchRequest.predicate = .init(
          format: "category == %d",
          QoraiVPNAlert.TrackerType.app.rawValue
        )
        let trackerCount =
          try context.count(for: fetchRequest)
          + Preferences.QoraiVPNAlertTotals.consolidatedEmailTrackerCount.value

        fetchRequest.predicate = .init(
          format: "category == %d",
          QoraiVPNAlert.TrackerType.location.rawValue
        )
        let locationPingCount =
          try context.count(for: fetchRequest)
          + Preferences.QoraiVPNAlertTotals.consolidatedLocationPingCount.value

        fetchRequest.predicate = .init(
          format: "category == %d",
          QoraiVPNAlert.TrackerType.mail.rawValue
        )
  ...
  public static func consolidateData(olderThan days: Int) {
    ...
    DataController.perform { context in
      let timestampKeyPath = #keyPath(QoraiVPNAlert.timestamp)
      ...
      Preferences.QoraiVPNAlertTotals.consolidatedEmailTrackerCount.value += oldTrackerCount
      Preferences.QoraiVPNAlertTotals.consolidatedLocationPingCount.value += oldLocationPingCount
      Preferences.QoraiVPNAlertTotals.consolidatedEmailTrackerCount.value += oldEmailTrackerCount

      // 4. Consolidating vpn trackers, they are later used for all-time lists.
      uniqueTrackers.forEach {
        let consolidatedRecord = QoraiVPNAlert(entity: entity, insertInto: context)
  ...
  private static func qoraiVPNAlertFetchRequest(
    for context: NSManagedObjectContext
  ) -> NSFetchRequest<NSDictionary> {
    let _fetchRequest = NSFetchRequest<NSDictionary>(entityName: "QoraiVPNAlert")
    _fetchRequest.entity = QoraiVPNAlert.entity(in: context)
    return _fetchRequest
  }

  private class func entity(in context: NSManagedObjectContext) -> NSEntityDescription? {
    NSEntityDescription.entity(forEntityName: "QoraiVPNAlert", in: context)
  }
}
