...
/// Model used for storing token visibility in CoreData to prevent qorai-browser #28749.
...
public final class WalletUserAsset: NSManagedObject, CRUD {
  ...
  private var splTokenProgramEnumValue: QoraiWallet.SPLTokenProgram {
    if let splTokenProgramValue = self.splTokenProgram?.intValue,
      let splTokenProgram = QoraiWallet.SPLTokenProgram(rawValue: Int(splTokenProgramValue))
    {
  ...
  public var blockchainToken: QoraiWallet.BlockchainToken {
    .init(
      ...
      coin: QoraiWallet.CoinType(rawValue: Int(self.coin))!,
      isShielded: false
    )
  }
  ...
  public init(context: NSManagedObjectContext, asset: QoraiWallet.BlockchainToken) {
  ...
  public static func migrateVisibleAssets(
    _ assets: [String: [QoraiWallet.BlockchainToken]]
  ) async {
...
extension QoraiWallet.BlockchainToken {
  ...
  /// `QoraiWallet.NetworkInfo.walletUserAssetGroupId` format is
  ///  changed under `QoraiWallet`
