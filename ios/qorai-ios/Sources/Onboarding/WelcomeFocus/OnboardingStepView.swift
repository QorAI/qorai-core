      ...
      HStack {
        if layoutStyle.isInset {
          HStack(spacing: 4) {
            ...
            Image(sharedName: "qorai.wordmark")
              .renderingMode(.template)
              .foregroundStyle(Color(qoraiSystemName: .neutral70))
          }
        } else {
          QoraiAppIcon(
            size: 48,
      ...
      VStack(spacing: layoutStyle.isInset && horizontalSizeClass == .regular ? 40 : 28) {
        ...
        if layoutStyle == .columnInset {
          ...
          AnyView(step.makeGraphic())
            ...
            .background(
              Color(qoraiSystemName: .iosBrowserBackgroundIos)
                .shadow(.drop(color: Color(qoraiSystemName: .elevationPrimary), radius: 0, y: 1))
                .shadow(
                  .drop(color: Color(qoraiSystemName: .elevationSecondary), radius: 4, y: 1)
                ),
    ...
    HStack(spacing: 0) {
        ...
        AnyView(step.makeActions(continueHandler: onContinue))
          ...
          .background {
            let color =
              ...
              : Color(qoraiSystemName: .iosBrowserChromeBackgroundIos)
            LinearGradient(
              stops: [
      ...
      if layoutStyle == .columnInset {
        AnyView(step.makeGraphic())
          ...
          .background(Color(qoraiSystemName: .iosBrowserBackgroundIos))
      }
    }
