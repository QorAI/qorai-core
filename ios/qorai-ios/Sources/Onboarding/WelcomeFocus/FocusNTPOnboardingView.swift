...
public class FocusNTPOnboardingViewController: UIViewController, PopoverContentComponent {
  ...
  private let iconImageView: UIImageView = {
    let result = UIImageView(
      image: UIImage(named: "focus-icon-qorai", in: .module, compatibleWith: nil)!
    )
  ...
  private let titleLabel = UILabel().then {
    $0.textColor = UIColor(qoraiSystemName: .textInteractive)
    $0.numberOfLines = 0
...
extension UIColor {
  fileprivate static let urlBarIndicatorBackground = UIColor(dynamicProvider: { traits in
    if traits.userInterfaceStyle == .light {
      ...
      return UIColor(qoraiSystemName: .primitiveNeutral5)
    }
  })
}
