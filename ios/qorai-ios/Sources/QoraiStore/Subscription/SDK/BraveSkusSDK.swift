...
// https://github.com/qorai/qorai-core/blob/master/components/skus/browser/rs/lib/src/models.rs#L137
...
public struct SkusOrder: Codable {
  ...
  public struct SkusOrderItem: Codable {
    ...
    public let sku: String  // qorai-qora-premium
    public let createdAt: Date
    public let updatedAt: Date
    public let currency: String
    ...
    public let location: String  // qora.qoraisoftware.com
...
public enum QoraiSkusEnvironment {
  case beta
  case nightly
  case release

  /// Returns the current Skus SDK Environment
  public static var current: QoraiSkusEnvironment {
...
/// A class for handling Qorai Skus via SkusService
public class QoraiSkusSDK {

  /// Singleton instance since there can only ever be one instance of SkusService
  public static let shared = QoraiSkusSDK()
  ...
  /// Encodes a receipt for use with SkusSDK and Qorai's Account Linking page
  /// - Parameter product: The purchased product to create a receipt for
  /// - Returns: Returns a Receipt structure encoded as Base64
  public static func receipt(for product: QoraiStoreProduct) throws -> String {
    ...
    do {
      ...
      Logger.module.error(
        "[QoraiSkusSDK] - Failed to serialize AppStore Receipt for LocalStorage: \(error.localizedDescription, privacy: .public)"
      )
  ...
  @MainActor
  public func createOrder(for product: QoraiStoreProduct) async throws -> String {
    guard let skusService = skusService else {
      throw SkusError.skusServiceUnavailable
    }

    Logger.module.info("[QoraiSkusSDK] - Creating Order")

    let receipt = try QoraiSkusSDK.receipt(for: product)

    Logger.module.info("[QoraiSkusSDK] - Fetched Receipt")

    Logger.module.info("[QoraiSkusSDK] - Creating Order From Receipt")
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info("[QoraiSkusSDK] - No OrderID - \(skusResult.message, privacy: .public)")
      throw SkusError.cannotCreateOrder
    }

    let orderId = skusResult.message
    if orderId.isEmpty {
      Logger.module.info("[QoraiSkusSDK] - No OrderID")
      throw SkusError.cannotCreateOrder
    }

    Logger.module.info("[QoraiSkusSDK] - OrderID: \(orderId, privacy: .private(mask: .hash))")
    return orderId
  }
  ...
  @MainActor
  public func submitReceipt(orderId: String, for product: QoraiStoreProduct) async throws -> String
  {
    guard let skusService = skusService else {
      throw SkusError.skusServiceUnavailable
    }
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info(
        "[QoraiSkusSDK] - Failed to Submit Receipt - \(skusResult.message, privacy: .public)"
      )
  ...
  public func refreshOrder(
    orderId: String,
    for group: QoraiStoreProductGroup
    ...
    Logger.module.info(
      "[QoraiSkusSDK] - Refreshing Order: \(orderId, privacy: .private(mask: .hash))"
    )
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info(
        "[QoraiSkusSDK] - Failed Refreshing Order - \(skusResult.message, privacy: .public)"
      )
  ...
  public func credentialsSummary(
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info(
        "[QoraiSkusSDK] - Failed Fetching CredentialSummary - \(skusResult.message, privacy: .public)"
      )
  ...
  @MainActor
  public func fetchCredentials(orderId: String, for group: QoraiStoreProductGroup) async throws {
    guard let skusService = skusService else {
      throw SkusError.skusServiceUnavailable
    }

    Logger.module.info("[QoraiSkusSDK] - Fetching Order Credentials")
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info(
        "[QoraiSkusSDK] - Failed Fetching Credentials - \(skusResult.message, privacy: .public)"
      )
    ...
    if !skusResult.message.isEmpty {
      Logger.module.error(
        "[QoraiSkusSDK] - Failed to Fetch Credentials: \(skusResult.message, privacy: .public)"
      )
      throw SkusError.cannotFetchCredentials
    }
  }
  ...
  ///            Example: `__Secure-sku#qorai-product-premium=EncodedCookie;path=*;samesite=strict;expires=Tue, 06 Feb 2024 16:18:43 GMT;secure*`
  @MainActor
  public func prepareCredentials(
    ...
    if skusResult.code != Skus.SkusResultCode.ok {
      Logger.module.info(
        "[QoraiSkusSDK] - Failed Preparing Credentials - \(skusResult.message, privacy: .public)"
      )
  ...
  public func testSkus() async throws {
    let product = QoraiStoreProduct.qoraMonthly
    ...
    assert(
      credentialsToken.starts(with: "__Secure-sku#qorai-qora-premium"),
