...
extension QoraiStoreKitPurchase: Identifiable {
...
public struct StoreKitReceiptView: View {
  @State
  public var receipt: QoraiStoreKitReceipt?

  public init(receipt: QoraiStoreKitReceipt? = nil) {
        ...
        .task {
          if let receipt = try? AppStoreReceipt.receipt {
            if let data = Data(base64Encoded: receipt) {
              self.receipt = QoraiStoreKitReceipt(data: data)
            }
          }
        }
    }
  ...
  private func groupedProductsView(for purchases: [QoraiStoreKitPurchase]) -> some View {
  ...
  private func purchaseDateView(for purchase: QoraiStoreKitPurchase) -> some View {
    NavigationLink(
      destination: {
  ...
  private func purchaseView(for purchase: QoraiStoreKitPurchase) -> some View {
    return List {
  ...
  private func formatBundleId(_ bundleId: String) -> String {
    var components = bundleId.replacingOccurrences(of: "qorai", with: "qorai.").split(
      separator: "."
    ).map({ String($0) })
...
struct StoreKitReceiptView_Preview: PreviewProvider {
  static var previews: some View {
    ...
    return StoreKitReceiptView(
      receipt:
        QoraiStoreKitReceipt(data: Data(base64Encoded: receipt)!)
    )
  }
}
#endif
