...
public struct P3AFeatureUsage {
  ...
  ///
  /// Can be used in tests to override the calendar
  ...
  public init(name: String, histogram: String, returningUserHistogram: String? = nil) {
    ...
    self.returnedDayAfterOption = .init(
      key: "p3a.feature-usage.\(name).returned-day-after",
      default: false
    )
  }
  ...
  var returningUserState: ReturningUserState {
    guard let firstUsage = firstUsageOption.value,
      ...
      let firstUsageCutoff = calendar.date(byAdding: .day, value: 7, to: firstUsage)
    else {
      return .neverUsed
    }
    ...
    if today > firstUsageCutoff {
      ...
      if let yesterday = calendar.date(byAdding: .day, value: -1, to: today),
        (firstUsage == yesterday && lastUsage == today) || returnedDayAfterOption.value
      {
  ...
  private func recordReturningUsageMetric() {
    ...
    Logger.module.info(
      "Recorded P3A returning usage for \(name): \(String(describing: returningUserState))"
    )
  }
}
