...
public class DAU {
  ...
  private static var baseUrl: String {
    // TODO: Handle via qorai-stats-updater-server switch and get URL from qorai_stats_updater_url
    let domain =
      AppConstants.isOfficialBuild
      ? "https://usage-ping.qorai.com/"
      : "https://usage-ping.qoraisoftware.com/"
  ...
  private let qoraiCoreStats: QoraiStats?

  public init(
    qoraiCoreStats: QoraiStats?
  ) {
    self.qoraiCoreStats = qoraiCoreStats
    apiKey = kQoraiStatsAPIKey
  }
  ...
  @discardableResult public func sendPingToServer() -> Bool {
    defer {
      ...
      UmaHistogramBoolean("Qorai.Core.UsageMonthly", true)
      UmaHistogramBoolean("Qorai.Core.UsageDaily", true)
    }

    guard let stats = qoraiCoreStats, stats.isStatsReportingEnabled else {
  ...
  func paramsAndPrefsSetup(for date: Date) -> ParamsAndPrefs? {
    ...
    if let qoraiCoreStats = qoraiCoreStats {
      params += qoraiCoreParams(for: qoraiCoreStats)
    }
  ...
  func qoraiCoreParams(for qoraiStats: QoraiStats) -> [URLQueryItem] {
    var queryItems: [URLQueryItem] = []
    queryItems.append(
      contentsOf: qoraiStats.walletParams.map({ URLQueryItem(name: $0.key, value: $0.value) })
    )
    queryItems.append(
      .init(name: "ads_enabled", value: qoraiStats.isNotificationAdsEnabled ? "true" : "false")
    )
    return queryItems
  }
