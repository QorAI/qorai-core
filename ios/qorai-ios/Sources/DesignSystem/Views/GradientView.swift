...
/// A gradient view which uses a QoraiGradient adjusts its gradient properties based on a
/// trait collection.
///
/// If you want to resolve a `QoraiGradient` to a specific user interface style, use
/// `init(gradient:)` or set `overrideUserInterfaceStyle` if you have used one of the pre-set
/// providers
public class QoraiGradientView: GradientView {
  private var provider: (UITraitCollection) -> QoraiGradient

  public init(dynamicProvider provider: @escaping (UITraitCollection) -> QoraiGradient) {
    self.provider = provider
    super.init(colors: [], positions: [], startPoint: .zero, endPoint: .zero)
    updateGradient()
  }

  public convenience init(gradient: QoraiGradient) {
...
/// A gradient control which uses a QoraiGradient adjusts its gradient properties based on a
/// trait collection.
public class QoraiGradientButton: UIButton {
  private var provider: (UITraitCollection) -> QoraiGradient

  public init(
    dynamicProvider provider: @escaping (UITraitCollection) -> QoraiGradient,
    colors: [UIColor],
    positions: [CGFloat],
    startPoint: CGPoint,
    endPoint: CGPoint
