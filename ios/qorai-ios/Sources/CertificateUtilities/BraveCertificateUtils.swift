...
public struct QoraiCertificateUtils {
...
extension QoraiCertificateUtils {
...
public enum QoraiCertificateUtilError: LocalizedError {
...
extension QoraiCertificateUtils {
  private static let evaluationQueue = DispatchQueue(
    label: "com.qorai.cert-utils-evaluation-queue",
    qos: .userInitiated
  )
  ...
  public static func createServerTrust(
    ...
    if certificates.isEmpty {
      throw QoraiCertificateUtilError.noCertificatesProvided
    }
    ...
    guard status == errSecSuccess else {
      throw QoraiCertificateUtilError.cannotCreateServerTrust
    }
    return serverTrust!
  }
  ...
  public static func evaluateTrust(_ trust: SecTrust, for host: String?) async throws {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
      QoraiCertificateUtils.evaluationQueue.async {
        SecTrustEvaluateAsyncWithError(trust, QoraiCertificateUtils.evaluationQueue) {
          _,
          isTrusted,
          error in
          if !isTrusted {
            if let error = error {
              ...
              continuation.resume(throwing: QoraiCertificateUtilError.trustEvaluationFailed)
            }
          } else {
            continuation.resume()
          }
  ...
  /// Verifies ServerTrust using Qorai-Core which verifies only SSL Pinning Status
  public static func verifyTrust(_ trust: SecTrust, host: String, port: Int) async -> Int {
    return Int(QoraiCertificateUtility.verifyTrust(trust, host: host, port: port))
  }
}
