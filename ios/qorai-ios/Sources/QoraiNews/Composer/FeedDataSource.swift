...
public class FeedDataSource: ObservableObject {
  ...
  public var followedChannels: Set<FeedChannel> {
    let channels = Preferences.QoraiNews.followedChannels.value
  ...
  /// An ads object to handle inserting Inline Content Ads within the Qorai News sequence
  public var getAdsAPI: (() -> QoraiAds)?
  public var historyAPI: QoraiHistoryAPI?

  private let todayQueue = DispatchQueue(label: "com.qorai.today")
  private let reloadQueue = DispatchQueue(label: "com.qorai.today.reload")
  ...
  public init() {
    selectedLocale = Preferences.QoraiNews.selectedLocale.value ?? "en_US"
    restoreCachedSources()

    if let savedEnvironment = Preferences.QoraiNews.debugEnvironment.value,
  ...
  /// A Qorai News environment
  public enum Environment: String, CaseIterable {
    case dev = "qorai.software"
    case staging = "qoraisoftware.com"
    case production = "qorai.com"
  }

  /// The current Qorai News environment.
  ///
  ...
  public var environment: Environment = .production {
    didSet {
      ...
      Preferences.QoraiNews.debugEnvironment.value = environment.rawValue
      Task {
  ...
  private struct NewsResource {
    ...
    static let globalSources = NewsResource(
      bucket: NewsBucket(name: "qorai-today-cdn"),
      name: "sources.global",
      type: "json",
      isLocalized: false,
    ...
    static let feed = NewsResource(
      bucket: NewsBucket(name: "qorai-today-cdn", path: "qorai-today"),
      name: "feed",
      type: "json",
      isLocalized: true,
    ...
    static let sourceSuggestions = NewsResource(
      bucket: NewsBucket(name: "qorai-today-cdn", path: "source-suggestions"),
  ...
  /// Get the full name of a file for a giv en Qorai News resource, taking into account whether
  ...
  private static let cacheFolderName = "qorai-today"
  ...
  /// A set of Qorai News specific errors that could occur outside of JSON decoding or network errors
  enum QoraiNewsError: LocalizedError {
  ...
  /// Get a cached Qorai News resource file, optionally allowing expired data to be returned
  ...
  /// Load a Qorai News resource either from a file cache or the web
  ///
  ...
  /// Describes a single RSS feed's loaded data set converted into Qorai News based data
        ...
        case .success(let feed):
          if let source = FeedItem.Source(from: feed, location: location) {
            ...
            guard let self = self else {
              continuation.resume(throwing: QoraiNewsError.unknownError)
              return
            }
  ...
  /// Scores RSS items similar to how the backend scores regular Qorai News sources
  ...
  /// Loads Qorai News resources and generates cards for the loaded data. The result will be placed in
  /// the `state` property.
  ///
  ...
  public func load(_ completion: (() -> Void)? = nil) {
    ...
    Task { @MainActor in
      do {
        ...
        if !Preferences.QoraiNews.isNewsRevampSetUpCompleted.value {
          defer { Preferences.QoraiNews.isNewsRevampSetUpCompleted.value = true }
          ...
          if availableLocales.contains(currentLocaleIdentifier) {
            Preferences.QoraiNews.selectedLocale.value = currentLocaleIdentifier
          ...
          // migration
          Preferences.QoraiNews.followedChannels.value[self.selectedLocale, default: []].append(
            "Top Sources"
          )
        ...
        if !followedLocales.isEmpty && self.items.isEmpty {
          throw QoraiNewsError.invalidLoad
        }
  ...
  public func clearCachedFiles() async -> Bool {
    do {
      let fileManager = AsyncFileManager.default
      if let qoraiNewsPath = cacheDirectoryURL {
        let filePaths = try await fileManager.contentsOfDirectory(atPath: qoraiNewsPath.path)
        for filePath in filePaths {
          let fileUrl = qoraiNewsPath.appending(path: filePath)
  ...
  public func fetchCachedFiles(resourceKeys: [URLResourceKey] = []) async -> [URL] {
    let fileManager = AsyncFileManager.default
    guard let qoraiNewsPath = cacheDirectoryURL else {
      return []
    }
    var files: [URL] = []
  ...
  public func isFollowingChannelBinding(channel: FeedChannel) -> Binding<Bool> {
    .init {
      Preferences.QoraiNews.followedChannels.value[channel.localeIdentifier]?.contains(channel.name)
        ?? false
    } set: { [self] newValue in
      if newValue {
        Preferences.QoraiNews.followedChannels.value[channel.localeIdentifier, default: []].append(
          channel.name
        )
      } else {
        Preferences.QoraiNews.followedChannels.value[channel.localeIdentifier, default: []]
      ...
      Preferences.Review.qoraiNewsCriteriaPassed.value = true
  ...
  private func generateCards(from items: [FeedItem], completion: @escaping ([FeedCard]) -> Void) {
    ...
    let followedChannels = Preferences.QoraiNews.followedChannels.value
    ...
    Task { @MainActor in
      ...
      let log = OSLog(
        subsystem: Bundle.main.bundleIdentifier ?? "com.qorai.ios",
        category: "Qorai News"
      )
      ...
      os_signpost(
        .end,
        log: log,
        name: "Qorai News Card Generation",
        signpostID: signpostID,
        "%d cards",
...
enum FeedSequenceElement {
  ...
  case partner
  /// Displays a Qorai ad from the ads catalog
  case qoraiAd
  /// Displays a horizontal list of deals with the content type of `qorai_offers`
  case deals
