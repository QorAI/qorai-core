...
public enum OptInCardAction {
  ...
  case turnOnQoraiNewsButtonTapped
  case learnMoreButtonTapped
}

public class QoraiNewsOptInView: UIView, FeedCardContent {
  ...
  public let graphicAnimationView = LottieAnimationView(
    name: "qorai-today-welcome-graphic",
    bundle: .module
  ).then {
  ...
  public let turnOnQoraiNewsButton = ActionButton().then {
    $0.layer.borderWidth = 0
    $0.titleLabel?.font = .systemFont(ofSize: 16.0, weight: .semibold)
    $0.setTitleColor(.white, for: .normal)
    $0.setTitle(Strings.QoraiNews.turnOnQoraiNews, for: .normal)
    $0.contentEdgeInsets = UIEdgeInsets(top: 10, left: 20, bottom: 10, right: 20)
    $0.backgroundColor = .qoraiLighterBlurple
  ...
  private let learnMoreButton = UIButton(type: .system).then {
    $0.setTitle(Strings.QoraiNews.learnMoreTitle, for: .normal)
  ...
  public required init() {
    ...
    stackView.addStackViewItems(
      ...
      .view(
        UILabel().then {
          $0.text = Strings.QoraiNews.introCardTitle
      ...
      .view(
        UILabel().then {
          $0.text = Strings.QoraiNews.introCardBody
          $0.textAlignment = .center
          $0.textColor = .white
    ...
    turnOnQoraiNewsButton.addTarget(
      self,
      action: #selector(tappedTurnOnQoraiButton),
      for: .touchUpInside
    )
  }
  ...
  @objc private func tappedTurnOnQoraiButton() {
    optInCardActionHandler?(.turnOnQoraiNewsButtonTapped)
  }
