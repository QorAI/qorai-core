...
struct FollowToggle: View {
  ...
  var body: some View {
    Toggle(isOn: $isFollowing) {
      Text(
        isFollowing ? Strings.QoraiNews.unfollowToggleTitle : Strings.QoraiNews.followToggleTitle
      )
      .lineLimit(1)
  ...
  private struct FollowToggleStyle: ToggleStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
      ...
      Button {
        ...
        configuration.label
          .foregroundColor(isFollowing ? Color(.secondaryQoraiLabel) : Color.white)
          .font(.footnote.bold())
          .padding(.horizontal, 12)
          .padding(.vertical, 4)
          .background(
            (isFollowing ? Color(.qoraiBackground) : Color(.qoraiBlurpleTint))
              .clipShape(Capsule())
          )
          .overlay(
            Capsule()
