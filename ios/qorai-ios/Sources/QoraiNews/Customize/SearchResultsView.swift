    ...
    List {
      if !results.channels.isEmpty {
        Section {
          ...
          Text(Strings.QoraiNews.channelsHeaderTitle)
        }
      }
      if !results.sources.isEmpty {
        Section {
          ...
          Text(Strings.QoraiNews.sourcesHeaderTitle)
        }
      }
      if !results.rssFeeds.isEmpty {
        Section {
          ...
          Text(Strings.QoraiNews.userSourcesHeaderTitle)
        }
      }
      if let urlQuery {
        Section {
          Button {
            ...
            HStack {
              (Text("\(Strings.QoraiNews.getFeedsFromSiteButtonTitle) ").font(
          ...
          .sheet(item: $rssResults) { results in
            UIKitController(
              UINavigationController(
                rootViewController: QoraiNewsAddSourceResultsViewController(
          ...
          .alert(item: $rssSearchError) { error in
            Alert(
              title: Text(Strings.QoraiNews.addSourceFailureTitle),
              message: Text(error.localizedDescription),
              dismissButton: .default(Text(Strings.OKString))
            )
          }
        } header: {
          Text(Strings.QoraiNews.availableRSSFeedsHeaderTitle)
        }
      }
    }
    ...
    .overlay(
      Group {
        if results.isEmpty, urlQuery == nil {
          Text(Strings.QoraiNews.noResultsFound)
