...
/// Handles coordinating when to use the Jitsi SDK for better Qorai Talk integration
@MainActor public class QoraiTalkJitsiCoordinator: Sendable {
  ...
  static func isIntegrationEnabled(for prefs: PrefService) -> Bool {
    ...
    return prefs.isQoraiTalkAvailable
  }
  ...
  public init(prefService: PrefService) {
    ...
    if !AppConstants.isOfficialBuild || Preferences.Debug.developerOptionsEnabled.value {
      JitsiMeetLogger.add(QoraiTalkJitsiLogHandler())
    }
  }
  ...
  public private(set) var jitsiTranscriptProcessor: QoraiTalkJitsiTranscriptProcessor?
  private var delegate: JitsiDelegate?
  private var isMuted: Bool = false
  public private(set) var isQoraiTalkInPiPMode: Bool = false
  ...
  public func handleResponderPresses(presses: Set<UIPress>, phase: KeyboardPressPhase) {
    guard isIntegrationEnabled, isCallActive, !isQoraiTalkInPiPMode else {
      return
    }
  ...
  public func launchNativeQoraiTalk(
    for room: String,
    token: String,
    host: String,
    ...
    delegate = JitsiDelegate(
      conferenceWillJoin: { [weak self] in
        self?.jitsiTranscriptProcessor = QoraiTalkJitsiTranscriptProcessor()
      ...
      enterPictureInPicture: { [weak self] in
        self?.isQoraiTalkInPiPMode = true
        self?.pipViewCoordinator?.enterPictureInPicture()
      },
      exitedPictureInPicture: { [weak self] in
        self?.isQoraiTalkInPiPMode = false
      },
    ...
    jitsiMeetView?.join(.qoraiTalkOptions(room: room, token: token, host: host))
