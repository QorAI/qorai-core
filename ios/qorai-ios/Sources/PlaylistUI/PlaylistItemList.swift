        ...
        .contextMenu {
          if let cachedData = item.cachedData, !cachedData.isEmpty {
            Button {
              ...
              Label(Strings.Playlist.removeOfflineData, qoraiSystemImage: "qora.cloud.off")
            }
          } else {
            Button {
              PlaylistManager.shared.download(item: .init(item: item))
            } label: {
              Label(Strings.Playlist.saveOfflineData, qoraiSystemImage: "qora.cloud.download")
            }
          }
          Divider()
          if let url = URL(string: item.pageSrc) {
            Button {
              ...
              Label(Strings.Playlist.openInNewTab, qoraiSystemImage: "qora.browser.mobile-tab-new")
            }
            Button {
              ...
              Label(
                ...
                qoraiSystemImage: "qora.product.private-window"
              )
            }
            ShareLink(item: url) {
              Label(Strings.Playlist.share, qoraiSystemImage: "qora.share.macos")
            }
            Divider()
          }
          if folders.count > 1 {
            Picker(
              ...
              ForEach(folders, id: \.objectID) { folder in
                Label(folder.title ?? "", qoraiSystemImage: "qora.product.playlist")
                  .tag(folder.id)
              }
            } label: {
              Label(Strings.Playlist.moveMenuItemTitle, qoraiSystemImage: "qora.folder.exchange")
            }
            .pickerStyle(.menu)
          }
          Button(role: .destructive) {
            ...
            Label(Strings.Playlist.deleteItem, qoraiSystemImage: "qora.trash")
          }
        }
      }
    }
