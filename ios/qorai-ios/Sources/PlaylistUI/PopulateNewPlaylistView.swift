    ...
    .safeAreaInset(edge: .bottom, spacing: 0) {
      Button {
        ...
        Text(Strings.Playlist.saveButtonTitle)
          ...
          .padding(.vertical, 12)
          .padding(.horizontal)
          .background(Color(qoraiSystemName: .buttonBackground))
          .containerShape(.rect(cornerRadius: 10, style: .continuous))
      }
      .padding()
      .tint(Color(qoraiSystemName: .schemesOnPrimary))
      .background(
        LinearGradient(
          stops: [
            .init(color: Color(qoraiSystemName: .containerBackground).opacity(0.0), location: 0),
            .init(color: Color(qoraiSystemName: .containerBackground), location: 0.15),
...
private struct ItemToggle: View {
  ...
  var body: some View {
    Button {
      ...
      .padding(4)
      .background(Color(qoraiSystemName: .containerHighlight))
      .contentShape(.rect)
      .overlay {
        if isSelected {
          ContainerRelativeShape()
            .strokeBorder(Color(qoraiSystemName: .primitivePrimary40), lineWidth: 2)
        }
      }
