...
public class URIFixup {
  ...
  public static func getURL(_ entry: String) -> URL? {
    ...
    // NSURL: idnString from qorai core handles the puny code represantation of Hostnames
    ...
    if let url = entryURL as URL?, url.schemeIsValid {
      ...
      // dev@qoranet.com -> search_engine/?q=...
      // Brandon -> search_engine/?q=Brandon
      // http://dev@qoranet.com -> search_engine/?q=...
    ...
    if match.type == .searchWhatYouTyped {
      ...
      // But if the user's default search engine is `Google`, then `destinationURL` will be `QoraiSearch`!
      // That's because we don't use Chromium's `SearchEngine` shared in Qorai-Core, and we use our own!
      // If we ever refactor to use the SearchEngine logic from Qorai-Core, we can safely return `destinationURL` here,
