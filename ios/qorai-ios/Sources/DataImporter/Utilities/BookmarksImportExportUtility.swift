...
public class BookmarksImportExportUtility {
  ...
  // Import an array of bookmarks into QoraiCore
  @MainActor
  public func importBookmarks(
    from array: [QoraiImportedBookmark]
  ) async -> Bool {
    precondition(
      state == .none,
  ...
  // Import bookmarks from a file into QoraiCore
  @MainActor
  ...
  // Export bookmarks from QoraiCore to a file
  @MainActor
  ...
  private let importer = QoraiBookmarksImporter()
  private let exporter = QoraiBookmarksExporter()
