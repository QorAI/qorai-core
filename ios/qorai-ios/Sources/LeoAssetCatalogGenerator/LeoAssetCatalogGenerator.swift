...
/// Generates an asset catalog for the Qora design system resources
///
...
@main
struct QoraAssetCatalogGenerator {
  // Arguments: ./QoraAssetCatalogGenerator asset_catalog1[, asset_catalog2, ...] qora_symbols_directory output_directory
  static func main() throws {
    ...
    let generator = QoraAssetCatalogGenerator(
  ...
  init(
    ...
    self.qoraSymbolsDirectory = leoSymbolsDirectory
    ...
    for catalog in assetCatalogs {
      for symbol in symbolSets(in: catalog) {
        ...
        if try fileManager.contentsOfDirectory(atPath: symbol.path).contains(where: {
          ...
          print(
            "Skipped copying Qora SF Symbol \"\(symbolName)\" from qora-sf-symbols. Using local version"
          )
          continue
        }
        ...
        print("Qora symbol svg path: \(leoSymbolSVGPath)")
        if !fileManager.fileExists(atPath: leoSymbolSVGPath) {
          print("Couldn't find a Qora icon named \(symbolName).svg")
          exit(EXIT_FAILURE)
        }
