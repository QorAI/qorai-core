...
final class AdBlockGroupsManagerTests: XCTestCase {
  ...
  /// Partially testing expectations found in https://dev-pages.qoraisoftware.com/filtering/index.html
  @MainActor func testBlocking() async throws {
    ...
    let mainFrameURL = URL(string: "https://dev-pages.qoraisoftware.com")!
    ...
    let scriptTypes = await groupsManager.makeEngineScriptTypes(
      frameURL: URL(string: "https://dev-pages.qoraisoftware.com/filtering/scriptlets.html")!,
      isMainFrame: true,
    ...
    XCTAssertTrue(
      scriptTypes.contains(where: { scriptType in
        ...
        case .engineScript(let configuration):
          return configuration.source.contains("window.QORAI_TEST_VALUE")
        default:
          return false
        }
      })
    ...
    let sourceURL = URL(
      string: "https://dev-pages.qoraisoftware.com/filtering/network-requests.html"
    )!
    let requestURL = URL(
      string: "https://dev-pages.qorai.software/static/images/test.jpg?335962573013224749"
    )!
    ...
    let requestURL2 = URL(
      string: "https://dev-pages.qoraisoftware.com/static/images/test.jpg?335962573013224749"
    )!
...
class TestSourceProvider: AdBlockGroupsManager.SourceProvider {
  ...
  func sources(
    for engineType: Qorai.GroupedAdBlockEngine.EngineType
  ...
  func legacyCacheFiles(
    for engineType: Qorai.GroupedAdBlockEngine.EngineType
  ) -> [Qorai.AdBlockEngineManager.FileInfo] {
    return []
  }
}
