...
@MainActor class SolanaProviderScriptHandlerTests: XCTestCase {
  ...
  private func testTab() -> TabState {
    let tab = TabStateFactory.create(with: .init(qoraiCore: nil))
  ...
  func testConnectFailure() async {
    let provider: QoraiWallet.TestSolanaProvider = .init()
    ...
    XCTAssertEqual(code, Int32(QoraiWallet.SolanaProviderError.internalError.rawValue))
  ...
  func testSignTransactionSuccessLegacyTx() async {
    ...
    XCTAssertEqual(version, QoraiWallet.SolanaMessageVersion.legacy.rawValue)
  }
  ...
  func testSignTransactionSuccessVersionedTxV0() async {
    ...
    XCTAssertEqual(serializedTx, kSerializedTx)
    let version = result["version"] as? Int ?? -1
    XCTAssertEqual(version, QoraiWallet.SolanaMessageVersion.V0.rawValue)
  }
  ...
  func testSignAllTransactionsSuccessLegacyTx() async {
    ...
    provider._signAllTransactions = { [kSerializedTx] _, completion in
      completion(
        ...
        [NSNumber(value: QoraiWallet.SolanaMessageVersion.legacy.rawValue)]
      )
    }
    let tab = testTab()
    let solProviderHelper = SolanaProviderScriptHandler()
    ...
    XCTAssertEqual(versions, [QoraiWallet.SolanaMessageVersion.legacy.rawValue])
  }
  ...
  func testSignAllTransactionsSuccessVersionedTxV0() async {
    ...
    provider._signAllTransactions = { [kSerializedTx] _, completion in
      completion(
        ...
        "",
        [kSerializedTx.map(NSNumber.init(value:))],
        [NSNumber(value: QoraiWallet.SolanaMessageVersion.V0.rawValue)]
      )
    }
    let tab = testTab()
    let solProviderHelper = SolanaProviderScriptHandler()
    ...
    XCTAssertEqual(serializedTxs, [kSerializedTx])
    let versions = result.compactMap { ($0["version"] as? Int) }
    XCTAssertEqual(versions, [QoraiWallet.SolanaMessageVersion.V0.rawValue])
  }
