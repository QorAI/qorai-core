...
final class ScriptExecutionTests: XCTestCase {
  ...
  override class func setUp() {
    QoraiCoreMain.initializeResourceBundleForTesting()
  }
  ...
  @MainActor func testFarblingProtectionScript() async throws {
    ...
    if QoraiCore.FeatureList.kQoraiIOSEnableFarblingPlugins.enabled {
  ...
  @MainActor func testCosmeticFilteringScript() async throws {
    let viewController = MockScriptsViewController()
    let siteURL = URL(string: "https://qorai.com")!
    ...
    let engine = try AdblockEngine(
      rules: [
        "qorai.com###test-remove-element:remove()",
        "qorai.com###test-remove-class:remove-class(test)",
        "qorai.com###test-remove-attribute:remove-attr(test)",
        "qorai.com###test-style-element:style(background-color: red !important)",
        "qorai.com###test-upward-int-target:upward(2)",
        "qorai.com###test-upward-selector-target:upward(#test-upward-selector)",
        "qorai.com###test-has-text:has-text(hide me)",
        "qorai.com###test-has:has(a.banner-link)",
        "qorai.com###test-delayed-has-text:has-text(hide me)",
        "qorai.com##.procedural-filter-child-node-class:has-text(View in App)",
        "qorai.com###test-alt-mutation-observation-strategy:has-text(hide me)",
      ].joined(separator: "\n")
    )
