...
final class GroupedAdBlockEngineTests: XCTestCase {
  func test3rdPartyCheck() throws {
    let engine = try AdblockEngine(
      rules: [
        "||qorai.com/path",
        "||qorai.com/other-path",
        "@@||qorai.com/path$~third-party",
      ].joined(separator: "\n")
    )

    XCTAssertFalse(
      engine.shouldBlock(
        requestURL: URL(string: "https://subdomain.qorai.com/path")!,
        sourceURL: URL(string: "https://qorai.com")!,
    ...
    XCTAssertFalse(
      engine.shouldBlock(
        requestURL: URL(string: "https://subdomain.qorai.com/other-path")!,
        sourceURL: URL(string: "https://qorai.com")!,
        resourceType: .xmlhttprequest,
        isAggressive: false
      ),
