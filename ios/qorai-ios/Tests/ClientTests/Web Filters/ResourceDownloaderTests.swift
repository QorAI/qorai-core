...
class ResourceDownloaderTests: XCTestCase {
  func testSuccessfulResourceDownload() throws {
    ...
    let resource = QoraiS3Resource.slimList
    let firstDownloader = ResourceDownloader<QoraiS3Resource>(
    ...
    let secondDownloader = ResourceDownloader<QoraiS3Resource>(
      networkManager: NetworkManager.makeNetworkManager(
        for: [resource],
        statusCode: 304,
        etag: "123"
  ...
  func testFailedResourceDownload() throws {
    ...
    let resource = QoraiS3Resource.slimList
    let downloader = ResourceDownloader<QoraiS3Resource>(
      networkManager: NetworkManager.makeNetworkManager(
        for: [resource],
        statusCode: 404
      )
