...
class P3ATimedStorageTests: XCTestCase {
  ...
  /// ---------
  /// Test Cases from qorai-core's similar `TimePeriodStorageTest` (`time_period_storage_unittest.cc`)
  /// ---------
  ...
  /// ---------
  /// Test Cases from qorai-core's similar `WeeklyEventStorageTest` (`weekly_event_storage_unittest.cc`)
  /// ---------
  ...
  func testAddEvents() {
    ...
    storage.append(value: .qorai)
    XCTAssertEqual(storage.records.last?.value, .qorai)
  }
  ...
  func testSerializationOrder() {
    ...
    XCTAssertEqual(newStorage.records.last?.value, .qorai)
  }
