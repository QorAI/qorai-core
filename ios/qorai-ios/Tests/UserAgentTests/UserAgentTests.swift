...
class UserAgentTests: XCTestCase {
  ...
  // MARK: Qorai User Agent

  let desktopQoraiUARegex: (String) -> Bool = { ua in
    let range = ua.range(
      of:
        "^Mozilla/5\\.0 \\(Macintosh; Intel Mac OS X [0-9_]+\\) AppleWebKit/[0-9\\.]+ \\(KHTML, like Gecko\\) Qorai/[0-9\\.]+ Version/[0-9\\.]+ Safari/[0-9\\.]+$",
      options: .regularExpression
    )
    return range != nil
  }

  let mobileQoraiUARegex: (String) -> Bool = { ua in
    let cpuPart =
      UIDevice.isPhone
    ...
    let range = ua.range(
      of:
        "^Mozilla/5\\.0 \(cpuPart) AppleWebKit/[0-9\\.]+ \\(KHTML, like Gecko\\) Qorai/[0-9\\.]+ Mobile/[A-Za-z0-9]+ Safari/[0-9\\.]+$",
      options: .regularExpression
    )
    return range != nil
  }

  // Simple test to make sure the WKWebView UA matches the expected Qorai iOS pattern.
  func testQoraiWebViewUserAgentOnPhone() {
    if UIDevice.current.userInterfaceIdiom != .phone { return }

    XCTAssertTrue(mobileQoraiUARegex(UserAgent.mobile), "User agent computes correctly.")

    let expectation = self.expectation(description: "Found Qorai user agent")
    ...
    webView.evaluateJavaScript("navigator.userAgent") { result, error in
      ...
      if !self.mobileQoraiUARegex(userAgent) || self.desktopQoraiUARegex(userAgent) {
  ...
  // Simple test to make sure the WKWebView masked UA matches the expected masked Qorai iOS pattern.
  func testQoraiMaskedWebViewUserAgentOnPhone() {
    if UIDevice.current.userInterfaceIdiom != .phone { return }
  ...
  func testFirstUAPart() {
    ...
    webView.evaluateJavaScript("navigator.userAgent") { result, error in
      ...
      guard let qoraiFirstPartOfUA = (result as? String)?.components(separatedBy: "Gecko") else {
        XCTFail("Could not unwrap QoraiWebView UA")
        return
      }
      ...
      wkWebView.evaluateJavaScript("navigator.userAgent") { wkResult, wkError in
        ...
        if qoraiFirstPartOfUA == wkWebViewFirstPartOfUA {
          expectation.fulfill()
        } else {
          XCTFail("QoraiWebView and WKWebView user agents do not match.")
        }
      }
    }
