...
class SwapStoreTests: XCTestCase {
  ...
  func testDefaultSellBuyTokensOnMainnetWithoutPrefilledToken() {
    ...
    let testAccountInfo: QoraiWallet.AccountInfo = .previewAccount
  ...
  func testDefaultSellBuyTokensOnMainnetWithPrefilledToken() {
    let batToken: QoraiWallet.BlockchainToken = .init(
      contractAddress: "",
      name: "Qorai QOR",
      logo: "",
      isCompressed: false,
  ...
  func testPrefilledTokenSwitchNetwork() {
    let prefilledToken: QoraiWallet.BlockchainToken = .mockSolToken
  ...
  func testDefaultSellBuyTokensOnEVMWithoutPrefilledToken() {
    ...
    rpcService.setNetwork(chainId: QoraiWallet.PolygonMainnetChainId, coin: .eth, origin: nil) {
      success in
  ...
  func testDefaultSellBuyTokensOnEVMWithPrefilledToken() {
    let daiToken: QoraiWallet.BlockchainToken = .init(
      contractAddress: "",
      name: "DAI Stablecoin",
      logo: "",
      isCompressed: false,
  ...
  private func setupServices(
    network: QoraiWallet.NetworkInfo = .mockMainnet,
    coin: QoraiWallet.CoinType = .eth
  ) -> (
    QoraiWallet.TestKeyringService, QoraiWallet.TestBlockchainRegistry,
    QoraiWallet.TestJsonRpcService, QoraiWallet.TestSwapService, QoraiWallet.TestTxService,
    QoraiWallet.TestQoraiWalletService, QoraiWallet.TestEthTxManagerProxy,
    QoraiWallet.TestSolanaTxManagerProxy, WalletUserAssetManagerType
  ) {
  ...
  func testFetchZeroExPriceQuoteSell() {
    ...
    let zeroExQuote: QoraiWallet.ZeroExQuote = .init(
    ...
    waitForExpectations(timeout: 2) { error in
      XCTAssertNil(error)
      // Verify fees
      XCTAssertEqual(store.qoraiFeeForDisplay, "0.875%")
    }
  }
  ...
  func testFetchJupiterPriceQuoteSell() {
    ...
    swapService._quote = { swapQuoteParams, completion in
      ...
      let jupiterQuote: QoraiWallet.JupiterQuote = .init(
        inputMint: QoraiWallet.BlockchainToken.mockSolToken.contractAddress,
        inAmount: "10000000",  // 0.01 SOL (9 decimals)
        outputMint: QoraiWallet.BlockchainToken.mockSpdToken.contractAddress,
    ...
    waitForExpectations(timeout: 2) { error in
      XCTAssertNil(error)
      // Verify fees
      XCTAssertEqual(store.qoraiFeeForDisplay, "0.85%")
    }
  }
  ...
  func testFetchJupiterPriceQuoteInsufficientLiquidity() {
    ...
    swapService._quote = { _, completion in
      let errorUnion: QoraiWallet.SwapErrorUnion = .init(
        jupiterError: .init(
          statusCode: "",
          error: "",
          message: "",
  ...
  func testFetchLiFiPriceQuoteSell() {
    ...
    let lifiQuote: QoraiWallet.LiFiQuote = .mockOneETHtoUSDCQuote
    swapService._quote = { _, completion in
  ...
  @MainActor func testSwapERC20ApproveTransaction() async {
    ...
    var submittedParams: QoraiWallet.NewEvmTransactionParams?
  ...
  @MainActor func testSwapSolSwapTransaction() async {
    ...
    var submittedTxData: QoraiWallet.TxDataUnion?
