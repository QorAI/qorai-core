...
class SendTokenStoreTests: XCTestCase {
  ...
  private let batSymbol = "QOR"

  override func tearDown() {
    cancellables.removeAll()
  }

  private let mockERC721Metadata: QoraiWallet.NftMetadata = .init(
    name: "mock nft name",
  ...
  private let mockSolMetadata: QoraiWallet.NftMetadata = .init(
  ...
  private func setupServices(
    selectedAccount: QoraiWallet.AccountInfo = QoraiWallet.AccountInfo.previewAccount,
    userAssets: [QoraiWallet.NetworkInfo: [QoraiWallet.BlockchainToken]] = [
      .mockMainnet: [.previewToken]
    ],
    selectedCoin: QoraiWallet.CoinType = .eth,
    balance: String = "0",
    ...
    QoraiWallet.TestQoraiWalletService, QoraiWallet.TestEthTxManagerProxy,
    QoraiWallet.TestSolanaTxManagerProxy, QoraiWallet.TestBitcoinWalletService,
    QoraiWallet.TestZCashWalletService, WalletUserAssetManagerType
  ) {
  ...
  func testPrefilledTokenSwitchNetwork() {
    var selectedNetwork: QoraiWallet.NetworkInfo = .mockMainnet
    let (
      keyringService, rpcService, walletService, ethTxManagerProxy, solTxManagerProxy,
      bitcoinWalletService, zcashWalletService, mockAssetManager
    ) = setupServices()
    ...
    rpcService._setNetwork = { chainId, coin, origin, completion in
      ...
      XCTAssertEqual(chainId, QoraiWallet.SolanaMainnet)
  ...
  func testUpdate() {
    let mockUserAssets: [QoraiWallet.BlockchainToken] = [.previewToken, .previewDaiToken]
  ...
  func testMakeSendERC20EIP1559Transaction() {
    ...
    let token: QoraiWallet.BlockchainToken = .init(
  ...
  func testSendFullBalanceNoRounding() {
    ...
    rpcService._chainIdForOrigin = { $2(QoraiWallet.NetworkInfo.mockSepolia.chainId) }
    rpcService._network = { $2(QoraiWallet.NetworkInfo.mockSepolia) }
  ...
  func testSolSendSystemProgramTransfer() {
    ...
    let store = SendTokenStore(
      ...
      bitcoinWalletService: bitcoinWalletService,
      zcashWalletService: zcashWalletService,
      prefilledToken: QoraiWallet.NetworkInfo.mockSolana.nativeToken,
      ipfsApi: TestIpfsAPI(),
      userAssetManager: mockAssetManager
    )
    store.selectedSendToken = QoraiWallet.NetworkInfo.mockSolana.nativeToken
  ...
  private let account: QoraiWallet.AccountInfo = .previewAccount
  private let ethSepolia: QoraiWallet.BlockchainToken = QoraiWallet.NetworkInfo.mockSepolia
    .nativeToken
    .copy(asVisibleAsset: true)
  private let usdcSepolia: QoraiWallet.BlockchainToken = .mockUSDCToken
    .copy(asVisibleAsset: true).then {
      $0.chainId = QoraiWallet.SepoliaChainId
    }
  private let account2: QoraiWallet.AccountInfo = .init(
    accountId: .init(
      coin: .eth,
  ...
  @MainActor func testDidSelectNewAccountSameNetwork() async {
    let account: QoraiWallet.AccountInfo = .previewAccount
    let ethSepolia: QoraiWallet.BlockchainToken = QoraiWallet.NetworkInfo.mockSepolia.nativeToken
      .copy(asVisibleAsset: true)
    let (
      keyringService, rpcService, walletService, ethTxManagerProxy, solTxManagerProxy,
      bitcoinWalletService, zcashWalletService, mockAssetManager
  ...
  @MainActor func testDidSelectNewAccountNewNetwork() async {
    let ethMainnet: QoraiWallet.BlockchainToken = QoraiWallet.NetworkInfo.mockMainnet.nativeToken
      .copy(asVisibleAsset: true)
    var selectedNetwork: QoraiWallet.NetworkInfo = .mockMainnet
    let (
      keyringService, rpcService, walletService, ethTxManagerProxy, solTxManagerProxy,
  ...
  @MainActor func testDidSelectCoinTypeSwitch() async {
    let solMainnet: QoraiWallet.BlockchainToken = QoraiWallet.NetworkInfo.mockSolana.nativeToken
      .copy(asVisibleAsset: true)
    let filTokenMainnet: QoraiWallet.BlockchainToken = .mockFilToken.copy(asVisibleAsset: true)
    var selectedNetwork: QoraiWallet.NetworkInfo = .mockSepolia
    var selectedNetworkToBe: QoraiWallet.NetworkInfo = .mockSolana
    var selectedAccount: QoraiWallet.AccountInfo = account
    let (
      keyringService, rpcService, walletService, ethTxManagerProxy, solTxManagerProxy,
      bitcoinWalletService, zcashWalletService, mockAssetManager
    ) = setupServices(
