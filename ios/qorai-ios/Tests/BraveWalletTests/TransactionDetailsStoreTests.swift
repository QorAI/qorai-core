...
class TransactionDetailsStoreTests: XCTestCase {
  ...
  private func setupServices() -> (
    QoraiWalletKeyringService,
    QoraiWalletQoraiWalletService,
    QoraiWalletJsonRpcService,
    QoraiWalletAssetRatioService,
    QoraiWalletBlockchainRegistry,
    QoraiWalletTxService,
    QoraiWalletSolanaTxManagerProxy,
    IpfsAPI,
    WalletUserAssetManagerType
  ) {
    let keyringService = QoraiWallet.TestKeyringService()
    keyringService._allAccounts = { completion in
      let allAccounts = [QoraiWallet.AccountInfo.previewAccount]
      let allAccountsInfo: QoraiWallet.AllAccountsInfo = .init(
  ...
  func testUpdate() {
    let transaction: QoraiWallet.TransactionInfo = .previewConfirmedSend.then {
      $0.chainId = QoraiWallet.SepoliaChainId
    }
    let (
      keyringService, walletService, rpcService, assetRatioService, blockchainRegistry, txService,
  ...
  func testUpdateTransaction() {
    let submittedTx: QoraiWallet.TransactionInfo = .previewConfirmedSend.then {
      $0.chainId = QoraiWallet.SepoliaChainId
      $0.txStatus = .submitted
    }
    let confirmedTx: QoraiWallet.TransactionInfo = .previewConfirmedSend.then {
      $0.chainId = QoraiWallet.SepoliaChainId
