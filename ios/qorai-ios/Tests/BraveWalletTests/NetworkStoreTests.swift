...
@MainActor class NetworkStoreTests: XCTestCase {
  ...
  private func setupServices() -> (
    ...
    QoraiWallet.TestQoraiWalletService, QoraiWallet.TestSwapService
  ) {
    ...
    walletService._ensureSelectedAccountForChain = { coin, chainId, completion in
      completion(QoraiWallet.AccountInfo.previewAccount.accountId)
    }
  ...
  func testSetSelectedNetwork() async {
    ...
    XCTAssertNotEqual(store.defaultSelectedChainId, QoraiWallet.NetworkInfo.mockSepolia.chainId)
    ...
    XCTAssertEqual(store.defaultSelectedChainId, QoraiWallet.NetworkInfo.mockSepolia.chainId)
  }
  ...
  func testSetSelectedNetworkSameNetwork() async {
    ...
    XCTAssertEqual(store.defaultSelectedChainId, QoraiWallet.NetworkInfo.mockMainnet.chainId)
  ...
  func testSetSelectedNetworkWithOrigin() async {
    let origin: URLOrigin = .init(url: URL(string: "https://qorai.com")!)
    let (keyringService, rpcService, walletService, swapService) = setupServices()
    ...
    XCTAssertNotEqual(store.selectedChainIdForOrigin, QoraiWallet.NetworkInfo.mockSepolia.chainId)
    let error = await store.setSelectedChain(.mockSepolia, isForOrigin: true)
    XCTAssertNil(error, "Expected success")
    XCTAssertEqual(store.selectedChainIdForOrigin, QoraiWallet.NetworkInfo.mockSepolia.chainId)
  }
  ...
  func testSetSelectedNetworkNoAccounts() async {
    ...
    XCTAssertNotEqual(store.defaultSelectedChainId, QoraiWallet.NetworkInfo.mockSolana.chainId)
    ...
    XCTAssertNotEqual(
      ...
      QoraiWallet.NetworkInfo.mockFilecoinTestnet.chainId
    )
  }
  ...
  func testUpdateChainList() async {
    ...
    rpcService._hiddenNetworks = { coin, completion in
      if coin == .eth {
        completion(
          [
            QoraiWallet.NetworkInfo.mockSepolia.chainId,
            QoraiWallet.NetworkInfo.mockPolygon.chainId,
          ]
        )
      } else {
        completion([])
    ...
    let expectedAllChains: [QoraiWallet.NetworkInfo] = [
      .mockSolana,
      .mockSolanaTestnet,
      .mockMainnet,
      .mockSepolia,
    ...
    let expectedCustomChains: [QoraiWallet.NetworkInfo] = [
      .mockCustomNetwork
    ]

    let expectedHiddenChains: [QoraiWallet.NetworkInfo] = [
      .mockSepolia,
      .mockPolygon,
    ]

    let expectedDefaultNetworks: [QoraiWallet.CoinType: QoraiWallet.NetworkInfo] = [
      .eth: .mockMainnet,
      .sol: .mockSolana,
...
extension QoraiWallet.NetworkInfo {
  fileprivate static var mockCustomNetwork: QoraiWallet.NetworkInfo = .init(
    chainId: "0x987654321",
