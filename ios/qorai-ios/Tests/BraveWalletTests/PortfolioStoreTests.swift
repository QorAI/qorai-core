...
@MainActor class PortfolioStoreTests: XCTestCase {
  ...
  // Accounts
  let ethAccount1: QoraiWallet.AccountInfo = .mockEthAccount
  let ethAccount2 = (QoraiWallet.AccountInfo.mockEthAccount.copy() as! QoraiWallet.AccountInfo).then
  {
    $0.address = "mock_eth_id_2"
    $0.accountId.uniqueKey = $0.address
    $0.name = "Ethereum Account 2"
  }
  let solAccount1: QoraiWallet.AccountInfo = .mockSolAccount
  let solAccount2 = (QoraiWallet.AccountInfo.mockSolAccount.copy() as! QoraiWallet.AccountInfo).then
  {
    $0.address = "mock_sol_id_2"
    $0.accountId.uniqueKey = $0.address
    $0.name = "Solana Account 2"
  }
  let filAccount1: QoraiWallet.AccountInfo = .mockFilAccount
  let filAccount2 = (QoraiWallet.AccountInfo.mockFilAccount.copy() as! QoraiWallet.AccountInfo).then
  {
    $0.address = "mock_fil_id_2"
    $0.accountId.uniqueKey = $0.address
    $0.name = "Filecoin Account 2"
  }
  let filTestnetAccount: QoraiWallet.AccountInfo = .mockFilTestnetAccount
  let btcAccount1: QoraiWallet.AccountInfo = .mockBtcAccount
  let btcAccount2 = (QoraiWallet.AccountInfo.mockBtcAccount.copy() as! QoraiWallet.AccountInfo).then
  {
    $0.accountId.accountIndex = 1
    $0.accountId.uniqueKey = "4_0_0_1"
    $0.name = "Bitcoin Account 2"
  }
  let btcTestnetAccount: QoraiWallet.AccountInfo = .mockBtcTestnetAccount
  let zcashAccount1: QoraiWallet.AccountInfo = .mockZcashAccount
  let zcashTestnetAccount: QoraiWallet.AccountInfo = .mockZcashTestnetAccount
  ...
  lazy var mockETHAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .eth,
  ...
  lazy var mockETHPriceHistory: [QoraiWallet.AssetTimePrice] = [
    .init(date: Date(timeIntervalSinceNow: -1000), price: "3000.00"),
    .init(date: Date(), price: mockETHPrice),
  ]
  lazy var mockETHAssetPriceOnSepolia: QoraiWallet.AssetPrice = .init(
    coin: .eth,
  ...
  lazy var mockUSDCAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .eth,
    chainId: QoraiWallet.MainnetChainId,
  ...
  lazy var mockUSDCPriceHistory: [QoraiWallet.AssetTimePrice] = [
  ...
  lazy var mockSOLAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .sol,
  ...
  lazy var mockSOLPriceHistory: [QoraiWallet.AssetTimePrice] = [
  ...
  lazy var mockFILAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .fil,
  ...
  lazy var mockFILPriceHistory: [QoraiWallet.AssetTimePrice] = [
  ...
  lazy var mockFILAssetPriceOnTestnet: QoraiWallet.AssetPrice = .init(
    coin: .fil,
  ...
  lazy var mockBTCAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .btc,
  ...
  lazy var mockBTCPriceHistory: [QoraiWallet.AssetTimePrice] = [
    .init(date: Date(timeIntervalSinceNow: -1000), price: "65326.00.06"),
    .init(date: Date(), price: mockBTCPrice),
  ]
  lazy var mockBTCAssetPriceOnTestnet: QoraiWallet.AssetPrice = .init(
    coin: .btc,
  ...
  lazy var mockZECAssetPrice: QoraiWallet.AssetPrice = .init(
    coin: .zec,
  ...
  lazy var mockZECPriceHistory: [QoraiWallet.AssetTimePrice] = [
    .init(date: Date(timeIntervalSinceNow: -1000), price: "36.0"),
    .init(date: Date(), price: mockZECPrice),
  ]
  lazy var mockZECAssetPriceOnTestnet: QoraiWallet.AssetPrice = .init(
    coin: .zec,
  ...
  private func setupStore() -> PortfolioStore {
    ...
    let mockFilUserAssets: [QoraiWallet.BlockchainToken] = [
    ...
    let mockFilBalanceInWei =
      formatter.weiString(
        ...
        radix: .decimal,
        decimals: Int(QoraiWallet.BlockchainToken.mockFilToken.decimals)
      ) ?? ""
    // config filecoin on testnet
    let mockFilTestnetUserAssets: [QoraiWallet.BlockchainToken] = [
    ...
    // config Solana
    let mockSolUserAssets: [QoraiWallet.BlockchainToken] = [
    ...
    // config Ethereum
    let mockEthUserAssets: [QoraiWallet.BlockchainToken] = [
    ...
    let mockEthSepoliaUserAssets: [QoraiWallet.BlockchainToken] = [
      QoraiWallet.NetworkInfo.mockSepolia.nativeToken.copy(asVisibleAsset: true)
    ]

    // config bitcoin on mainnet
    let mockBtcUserAssets: [QoraiWallet.BlockchainToken] = [
      QoraiWallet.NetworkInfo.mockBitcoinMainnet.nativeToken.copy(asVisibleAsset: true)
    ]
    // config bitcoin on testnet
    let mockBtcTestnetUserAssets: [QoraiWallet.BlockchainToken] = [
      QoraiWallet.NetworkInfo.mockBitcoinTestnet.nativeToken.copy(asVisibleAsset: true)
    ]

    let mockZcashUserAssets: [QoraiWallet.BlockchainToken] = [
      QoraiWallet.NetworkInfo.mockZcashMainnet.nativeToken.copy(asVisibleAsset: true)
    ]
    let mockZcashTestnetUserAssets: [QoraiWallet.BlockchainToken] = [
      QoraiWallet.NetworkInfo.mockZcashTestnet.nativeToken.copy(asVisibleAsset: true)
    ]
    ...
    rpcService._balance = { accountAddress, coin, chainId, completion in
      // eth balance
      if coin == .eth {
        if chainId == QoraiWallet.MainnetChainId, accountAddress == self.ethAccount1.address {
          completion(ethBalanceWei, .success, "")
        } else {
          completion("", .success, "")
        }
      } else if coin == .fil {  // .fil
        if chainId == QoraiWallet.FilecoinMainnet {
    ...
    assetRatioService._priceHistory = { priceId, _, _, completion in
      ...
      case QoraiWallet.BlockchainToken.mockUSDCToken.assetRatioId:
    ...
    zcashWalletService._balance = { accountId, completion in
      if accountId.keyringId == QoraiWallet.KeyringId.zCashMainnet {
        completion(
          .init(
    ...
    return PortfolioStore(
      ...
      blockchainRegistry: QoraiWallet.TestBlockchainRegistry(),
      ipfsApi: TestIpfsAPI(),
    ...
    store.$assetGroups
      ...
      .sink { assetGroups in
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.previewToken.symbol
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.mockSolToken.symbol
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.mockFilToken.symbol
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.mockBTCToken.symbol
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.mockZecToken.symbol
        )
        XCTAssertEqual(
    ...
    store.saveFilters(
      .init(
        ...
        networks: MockJsonRpcService.allKnownNetworks.map {
          ...
          .init(isSelected: $0.chainId == QoraiWallet.SepoliaChainId, model: $0)
        }
      )
    )
    await fulfillment(
    ...
    store.$assetGroups
      ...
      .sink { assetGroups in
        ...
        XCTAssertEqual(
          ...
          QoraiWallet.BlockchainToken.mockZecToken.id
        )
      ...
      .sink { assetGroups in
        ...
        if bitcoinAndZcashTestnetEnabled {
          ...
          XCTAssertEqual(
            ...
            QoraiWallet.NetworkInfo.mockBitcoinTestnet.nativeToken.symbol
          )
          XCTAssertEqual(
        ...
        XCTAssertEqual(
          group.assets[safe: offset]?.token.symbol,
          QoraiWallet.NetworkInfo.mockSepolia.nativeToken.symbol
        )
        XCTAssertEqual(group.assets[safe: offset]?.quantity, String(format: "%.04f", 0))
      }.store(in: &cancellables)
    isLocked = false
