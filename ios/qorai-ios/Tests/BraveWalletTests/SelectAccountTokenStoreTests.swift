...
class SelectAccountTokenStoreTests: XCTestCase {
  ...
  private let allUserAssets: [QoraiWallet.BlockchainToken] = [
    .previewToken.copy(asVisibleAsset: true),
    .mockUSDCToken.copy(asVisibleAsset: true).then { $0.chainId = QoraiWallet.SepoliaChainId },
    .mockSolToken.copy(asVisibleAsset: true),
    .mockSpdToken.copy(asVisibleAsset: false),  // not visible
    .mockSolanaNFTToken.copy(asVisibleAsset: true).then { $0.chainId = QoraiWallet.SolanaTestnet },
    .mockFilToken.copy(asVisibleAsset: true),
    .mockFilToken.copy(asVisibleAsset: true).then { $0.chainId = QoraiWallet.FilecoinTestnet },
  ]
  ...
  private let mockEthAccount2: QoraiWallet.AccountInfo = .init(
    accountId: .init(
      coin: .eth,
  ...
  @MainActor func testUpdate() async {
    ...
    let mockNFTMetadata: QoraiWallet.NftMetadata = .init(
    ...
    let mockUSDCAssetPrice: QoraiWallet.AssetPrice = .init(
      coin: .eth,
    ...
    let mockFILTestnetAssetPrice: QoraiWallet.AssetPrice = .init(
      coin: .fil,
