...
class NFTStoreTests: XCTestCase {
  ...
  let unownedEthNFT =
    (QoraiWallet.BlockchainToken.mockERC721NFTToken.copy() as! QoraiWallet.BlockchainToken).then {
  ...
  let solAccount: QoraiWallet.AccountInfo = .mockSolAccount
  ...
  let mockInvisibleERC721Metadata: QoraiWallet.NftMetadata = .init(
  ...
  let mockUnownedERC721Metadata: QoraiWallet.NftMetadata = .init(
  ...
  let mockSpamMetadata: QoraiWallet.NftMetadata = .init(
  ...
  private func setupServices(
    mockEthUserAssets: [QoraiWallet.BlockchainToken],
    mockSolUserAssets: [QoraiWallet.BlockchainToken],
    mockNFTMetadata: [QoraiWallet.BlockchainToken: QoraiWallet.NftMetadata]
  ) -> (
    QoraiWallet.TestKeyringService, QoraiWallet.TestJsonRpcService,
    QoraiWallet.TestQoraiWalletService, QoraiWallet.TestAssetRatioService,
    TestableWalletUserAssetManager, QoraiWallet.TestTxService
  ) {
    ...
    rpcService._nftBalances = { accountAddress, nftIdentifiers, completion in
      ...
      for nft in nftIdentifiers {
        if nft.contractAddress.caseInsensitiveCompare(
          QoraiWallet.BlockchainToken.mockERC721NFTToken.contractAddress
        ) == .orderedSame,
    ...
    rpcService._nftMetadatas = { nftIdentifiers, completion in
      var allMetadata = [QoraiWallet.NftMetadata]()
    ...
    walletService._simpleHashSpamNfTs = { walletAddress, chainIds, _, completion in
      if walletAddress == self.ethAccount1.address,
        chainIds.contains(QoraiWallet.ChainId(coin: .eth, chainId: QoraiWallet.MainnetChainId))
      {
