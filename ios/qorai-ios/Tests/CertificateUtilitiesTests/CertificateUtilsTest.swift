...
class CertificateUtilsTest: XCTestCase {
  ...
  func testHexFormatting() {
    XCTAssertEqual(QoraiCertificateUtils.formatHex("AABBCCDDEEFF"), "AA BB CC DD EE FF")
    XCTAssertEqual(
      QoraiCertificateUtils.formatHex("AABBCCDDEEFF", separator: "-"),
      "AA-BB-CC-DD-EE-FF"
    )
    XCTAssertEqual(QoraiCertificateUtils.formatHex("AABBCCDDEEFF", separator: ""), "AABBCCDDEEFF")
  }
  ...
  func testAbsolute_OID_To_OID() {
    ...
    for test in testOIDs {
      do {
        let result: [UInt8] = try QoraiCertificateUtils.absoluteOIDToOID(oid: test.key)
  ...
  func testOID_To_Absolute_OID() {
    ...
    for test in testOIDs {
      do {
        let result: String = try QoraiCertificateUtils.oidToAbsoluteOID(oid: test.key)
  ...
  func testCertificateInfo() {
    guard let model = QoraiCertificateModel(certificate: certificate(named: "github.com")) else {
    ...
    XCTAssertEqual(
      model.publicKeyInfo.keyHexEncoded,
      "04ADF6F775B1D349540A5D1071BDDC25064B221CA2234E9FA1FEB9D08CBD39BC0C23C7CF91A6905AD845AB0313BEC1237AB9C4C89D47F696E0B9766B503666F70D"
    )
    ...
    XCTAssertEqual(
      model.sha1Fingerprint.fingerprintHexEncoded,
      "8463B3A92912CCFD1D314705989BEC139937D0D7"
    )
    XCTAssertEqual(
      model.sha256Fingerprint.fingerprintHexEncoded,
      "0AE384BFD4DDE9D13E50C5857C05A442C93F8E01445EE4B34540D22BD1E37F1B"
    )
  }

  func testCertificateInfo2() {
    guard let model = QoraiCertificateModel(certificate: certificate(named: "qorai.com")) else {
      XCTAssert(false, "Cannot load Certificate")
      return
    }
    ...
    XCTAssertEqual(model.subjectName.organizationalUnit, [])
    XCTAssertEqual(model.subjectName.commonName, "qorai.com")
    XCTAssertEqual(model.subjectName.userId, "")
    ...
    XCTAssertEqual(
      model.sha1Fingerprint.fingerprintHexEncoded,
      "BAA20BC0E5A5885974F2050CC6D1F7A206C0F945"
    )
    XCTAssertEqual(
      model.sha256Fingerprint.fingerprintHexEncoded,
      "D3C09366E951BF0F1D604D5DCD74B6F8FAB8B85ABDC96401D61816CE09740D07"
    )
  }
}
