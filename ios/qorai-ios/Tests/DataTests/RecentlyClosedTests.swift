...
class RecentlyClosedTests: CoreDataTestCase {
  ...
  func testInsert() {
    let url1 = URL(string: "https://qorai.com")!
    let title1 = "Qorai"
  ...
  func testGetExisting() {
    ...
    let savedUrl = URL(string: "https://qorai.com")!
  ...
  func testRemoveETLDPlusOnes() {
    let qoraiURL = URL(string: "https://qorai.com")!
    let qoraiURLTitle = "Qorai"
    ...
    let qoraiSearchURL = URL(string: "https://search.qorai.com")!
    let qoraiSearchTitle = "Qorai Search"

    let savedObject2 = SavedRecentlyClosed(
      url: qoraiSearchURL,
      title: qoraiSearchTitle,
      dateAdded: Date(),
      interactionState: Data(),
      order: 0
    )

    let googleQueryURL = URL(string: "https://www.google.com/search?q=qorai.com")!
    ...
    XCTAssertNotNil(RecentlyClosed.get(with: qoraiURL.absoluteString))
    XCTAssertNotNil(RecentlyClosed.get(with: qoraiSearchURL.absoluteString))
    XCTAssertNotNil(RecentlyClosed.get(with: googleQueryURL.absoluteString))

    backgroundSaveAndWaitForExpectation {
      RecentlyClosed.remove(baseDomains: .init(["qorai.com"]))
    }

    XCTAssertEqual(try! DataController.viewContext.count(for: fetchRequest), 1)
    // base domain matches get removed
    XCTAssertNil(RecentlyClosed.get(with: qoraiURL.absoluteString))
    XCTAssertNil(RecentlyClosed.get(with: qoraiSearchURL.absoluteString))
