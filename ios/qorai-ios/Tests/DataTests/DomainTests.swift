...
class DomainTests: CoreDataTestCase {
  ...
  let url2 = URL(string: "http://qorai.com")!
  let url2HTTPS = URL(string: "https://qorai.com")!
  ...
  func testGetOrCreateURLs() {
    ...
    let url3 = URL(string: "https://qorai.com")!
    let url4 = URL(string: "data://qorai.com")!
  ...
  @MainActor func testDefaultShieldSettings() {
    ...
    XCTAssertFalse(domain.isShieldExpected(QoraiShield.allOff, considerAllShieldsOption: true))
    XCTAssertFalse(domain.isShieldExpected(QoraiShield.noScript, considerAllShieldsOption: true))
    XCTAssertTrue(domain.isShieldExpected(QoraiShield.fpProtection, considerAllShieldsOption: true))
  ...
  @MainActor func testAllShieldsOff() {
    ...
    backgroundSaveAndWaitForExpectation {
      Domain.setQoraiShield(forUrl: url, shield: .allOff, isOn: true, isPrivateBrowsing: false)
    }
    ...
    XCTAssertFalse(domain.isShieldExpected(QoraiShield.noScript, considerAllShieldsOption: true))
    XCTAssertFalse(
      domain.isShieldExpected(QoraiShield.fpProtection, considerAllShieldsOption: true)
    )

    backgroundSaveAndWaitForExpectation {
      Domain.setQoraiShield(forUrl: url, shield: .allOff, isOn: false, isPrivateBrowsing: false)
    }
...
extension Domain {
  fileprivate class func setQoraiShield(
    forUrl url: URL,
    shield: QoraiShield,
    isOn: Bool?,
