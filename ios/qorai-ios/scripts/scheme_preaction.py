...
"""
Actions to run before every build in the qorai-ios Xcode project
"""
...
qorai_root_dir = os.path.join(
    os.path.join(os.path.join(this_dir, os.pardir), os.pardir), os.pardir)
src_dir = os.path.join(qorai_root_dir, os.pardir)
...
def BuildCore(config, target_arch, target_environment):
    """Generates and builds the QoraiCore.framework"""
    cmd_args = [
...
def CallNpm(cmd):
    retcode = subprocess.call(cmd, cwd=qorai_root_dir, stderr=subprocess.STDOUT)
    if retcode:
