...
#include "qorai/ios/web/webui/qorai_url_data_source_ios.h"
...
// static
QoraiWebUIIOSDataSource* QoraiWebUIIOSDataSource::Create(
    const std::string& source_name) {
  return new QoraiWebUIIOSDataSource(source_name);
}

// static
QoraiWebUIIOSDataSource* QoraiWebUIIOSDataSource::CreateAndAdd(
...
class QoraiWebUIIOSDataSource::QoraiInternalDataSource
    : public QoraiURLDataSourceIOS {
 public:
  QoraiInternalDataSource(QoraiWebUIIOSDataSource* parent) : parent_(parent) {}
  ~QoraiInternalDataSource() override {}
  ...
  std::string GetContentSecurityPolicy(
    ...
    return QoraiURLDataSourceIOS::GetContentSecurityPolicy(directive);
  }
  ...
  bool ShouldServiceRequest(const GURL& url) const override {
    ...
    return QoraiURLDataSourceIOS::ShouldServiceRequest(url);
  }

 private:
  raw_ptr<QoraiWebUIIOSDataSource> parent_;
};

// WebUIIOSDataSource implementation:

QoraiWebUIIOSDataSource::QoraiWebUIIOSDataSource(const std::string& source_name)
    : web::WebUIIOSDataSourceImpl(source_name,
                                  new QoraiInternalDataSource(this)) {
  CHECK(!source_name.ends_with("://"));
}

QoraiWebUIIOSDataSource::~QoraiWebUIIOSDataSource() = default;

std::string QoraiWebUIIOSDataSource::GetMimeType(
...
void QoraiWebUIIOSDataSource::SetSupportedScheme(std::string_view scheme) {
  CHECK(!supported_scheme_.has_value());
  supported_scheme_ = scheme;
}

void QoraiWebUIIOSDataSource::OverrideContentSecurityPolicy(
    network::mojom::CSPDirectiveName directive,
    const std::string& value) {
  csp_overrides_.insert_or_assign(directive, value);
}

void QoraiWebUIIOSDataSource::AddFrameAncestor(const GURL& frame_ancestor) {
...
void QoraiWebUIIOSDataSource::DisableTrustedTypesCSP() {
