...
#import "qorai/ios/browser/api/net/certificate_utility.h"
...
// AmazonRootCA1
// From:
// https://github.com/qorai/qorai-core/blob/master/chromium_src/net/tools/transport_security_state_generator/input_file_parsers.cc
static const char* qorai_test_cert = R"(
...
SecCertificateRef GetTestCertificate() {
  NSString* cert_string = [NSString stringWithFormat:@"%s", qorai_test_cert];
...
TEST_F(CertificateTest, AcceptableCertificates) {
  std::size_t acceptable_certs_count = sizeof(net::kQoraiAcceptableCerts) /
                                       sizeof(net::kQoraiAcceptableCerts[0]);

  NSArray<NSData*>* certs = [QoraiCertificateUtility acceptableSPKIHashes];

  EXPECT_TRUE([certs count] > 3);
  EXPECT_EQ([certs count], acceptable_certs_count);

  auto test_cert = base::as_byte_span(*net::kQoraiAcceptableCerts[0]);
  if (test_cert.size() > 0) {
...
TEST_F(CertificateTest, HashCertificateSPKI) {
  ...
  NSData* spki_hash =
      [QoraiCertificateUtility hashCertificateSPKI:GetTestCertificate()];
