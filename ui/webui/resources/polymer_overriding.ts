...
const debug = false || document.location.href.includes('qorai-overriding-debug')
...
function addQoraiBehaviorsLegacy(moduleName: string, component: { behaviors: Function[] }) {
...
function addQoraiProperties(moduleName: string, component: { properties: any }) {
...
const allQoraiTemplateModificationsMap = {}

function addQoraiTemplateModifications(moduleName: string, component: PolymerElement, modifyFn: (content: DocumentFragment) => unknown) {
...
const styleOverridePrefix = 'qorai-override-style-'

function addQoraiStyleOverride(moduleName: string, styleOverrideModuleNames: string[], component: PolymerElement, template = component._template || component.template) {
  ...
  if (debug) {
    console.log(`Qorai Style Override added of "${styleOverrideModuleNames.join(' ')}" for ${moduleName}`, styleElement)
...
export function RegisterPolymerComponentProperties(propertiesMap) {
  ...
  for (const componentName in propertiesMap) {
    if (componentPropertyModifications[componentName]) {
      addQoraiProperties(componentName, componentPropertyModifications[componentName])
    }
  }
}
...
export function RegisterPolymerTemplateModifications(modificationsMap: ModificationsMap) {
  ...
  Object.assign(allQoraiTemplateModificationsMap, awaitingComponentModifications)
}
...
export function OverrideIronIcons(iconSetName: string, overridingIconSetName: string, iconOverrides: IconOverridesMap) {
  ...
  for (const chromiumIconName in iconOverrides) {
    ...
    if (!chromiumIcon) {
      console.error(`[qorai overrides] Could not find chromium icon '${chromiumIconName}' in iconset '${iconSetName}' for replacement!`)
      continue
    }
    const qoraiIconName = iconOverrides[chromiumIconName]
    const qoraiIcon = overrideIconSet.querySelector(`#${qoraiIconName}`)
    if (!qoraiIcon) {
      console.error(`[qorai overrides] Could not find qorai icon '${qoraiIconName}' in iconset '${overridingIconSetName}' for replacement!`)
      continue
    }
    // replace
    ...
    while (qoraiIcon.firstChild) {
      chromiumIcon.appendChild(qoraiIcon.firstChild)
    }
  }
...
Polymer.Class = function (info, mixin) {
  ...
  addQoraiBehaviorsLegacy(name, info)
  return oldClass(info, mixin)
}
...
PolymerElement._prepareTemplate = function QoraiPolymer_PrepareTemplate() {
  ...
  addQoraiProperties(name, this.prototype)

  // This allows us to add new functions to the prototype and/or replace
  // existing ones.
  applyPrototypeModifications(name, this.prototype)

  const templateModifyFn = allQoraiTemplateModificationsMap[name]
  if (templateModifyFn) {
    addQoraiTemplateModifications(name, this.prototype, templateModifyFn)
    // TODO(petemill): delete allQoraiTemplateModificationsMap entry when done so that the
  ...
  if (styleOverrideModules && styleOverrideModules) {
    addQoraiStyleOverride(name, styleOverrideModules, this.prototype)
  }
}
 ...
 * @returns
 */
function QoraiDefineCustomElements(name, component, options, useIgnoreList = true) {
  if (component.polymerElementVersion) {
    if (debug) {
      console.log('QoraiDefineCustomElements PolymerElement defined', name, component, options)
    }
    // Global ignore (likely due to manual replacement)
    if (useIgnoreList && ignoredComponents.includes(name)) {
      if (debug) {
        console.log(`QoraiDefineCustomElements ignored ${name}`)
      }
      return
    }
    // Inject behaviors
    if (allBehaviorsMap[name]) {
      if (debug) {
        console.log('QoraiDefineCustomElements added behavior', allBehaviorsMap[name])
      }
...
window.customElements.define = QoraiDefineCustomElements

export function define(name, component, options?) {
  // We still want style and template overrides
  QoraiDefineCustomElements.call(window.customElements, name, component, options, false)
}
