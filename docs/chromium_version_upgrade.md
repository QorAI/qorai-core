# Upgrading `qorai-core` to a newer version of Chromium

This document describes the basics and the tooling available to have
`qorai-core` upstream tag selection chromium, also known as a *version bump*.

## Understanding the basics

In order to be able to depend on, and make changes to vanilla Chromium,
`qorai-core` has to pick a tag of Chromium to checkout and build against. This
  ...
  "config": {
    "projects": {
      "chrome": {
        ...
        "tag": "134.0.6998.95",
        "repository": {
          "url": "https://github.com/qorai/chromium"
        }
      },
```
...
There may be many other changes necessary to get Qorai to build with the tag
...
Depending what changed since the last time a tag was picked for `qorai-core`,
...
qorai/patches/chrome-browser-ui-views-frame-browser_view_layout.cc.patch
qorai/patches/chrome-browser-ui-webui-settings-site_settings_helper.cc.patch

| Commit name | Purpose |
| --- | --- |
| `chrome/browser/ui/views/frame/browser_view_layout.cc` | `qorai/patches/chrome-browser-ui-views-frame-browser_view_layout.cc.patch` |
| `chrome/browser/ui/webui/settings/site_settings_helper.cc` | `qorai/patches/chrome-browser-ui-webui-settings-site_settings_helper.cc.patch` |
