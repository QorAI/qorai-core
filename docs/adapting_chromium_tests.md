# Adapting Chromium tests for the Qorai codebase

To make Chromium tests work properly in the Qorai codebase, you may need to
...
running test vendor. It is set to `kQorai` for `qorai_*` test binaries and
...
The `QoraiChromeTestSetupHelper` class in
`//qorai/chromium_src/chrome/test/base/chrome_test_suite.cc` allows you to
...
The `QoraiChromeTestSetupHelper` class uses a list of `TestAdjustments` to
specify which features to enable or disable for specific test patterns.

```cpp
class QoraiChromeTestSetupHelper : public testing::EmptyTestEventListener {
