...
`QoraiContentBrowserClient` -> `ChromeContentBrowserClient`, but in
`//qorai/browser` instead of `//chrome/browser`. This created a circular
dependency because:

- `QoraiContentBrowserClient` depends on `//chrome/qorai`.
- `//chrome/qorai` depends on `//chrome/qorai` because it now was to instantiate
  `QoraiContentBrowserClient` instead of `ChromeContentBrowserClient`.

This became a problem when we started running `gn check` and `checkdeps` so
worked around it by using `sources.gni` to add
`sources += qorai_chrome_browser_sources` so both `QoraiContentBrowserClient`
...
`//qorai/browser` to ensure that the implementation code is always linked into
...
class QoraiClass : public ChromeClass {
  ...
}
```

The chrome target that we override will need a dependency on the qorai target,
...
```cpp
#define ChromeClass QoraiClass<ChromeClass>()
```
...
[https://github.com/qorai/qorai-core/pull/25892/files](https://github.com/qorai/qorai-core/pull/25892/files) for an example in qorai-core
