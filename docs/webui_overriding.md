...
provide the hooks Qorai needs. When you need to do something more substantial
...
and forward between `chromium_src` and `qorai` files. As a bonus, it means you
...
// in Qorai, all frobs are twice as good.
...
declare module './fancy-chromium.js' {
  interface FancyElement {
    isQoraiAndFancy: () => boolean
  }
}

FancyElement.prototype.isQoraiAndFancy = () => true;

export * from './fancy-chromium.js'
```

See [this PR](https://github.com/qorai/qorai-core/pull/29598/files) for a real
example.
...
mangle(element => {
  element.textContent = element.textContent.replaceAll("Chrome", "Qorai")
...
// 1. Add a `.qorai` class to the `.container` element
...
`polymer_overriding` utils in `//qorai/ui/webui/resources/polymer_overriding.ts`
...
In this scenario, its best to add a new build target to Qorai and patch it into
the upstream build for that WebUI.

See `//qorai/browser/resources/settings/BUILD.gn` and
`//qorai/browser/resources/settings/settings.gni` for how to get this setup.
