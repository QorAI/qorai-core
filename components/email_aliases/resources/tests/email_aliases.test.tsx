...
} from 'gen/qorai/components/email_aliases/email_aliases.mojom.m'
...
const mockEmail = 'test@qoranet.com'
...
const mockAliases = [
  {
    email: 'alias1@qoranet.com',
    note: 'Test Alias 1',
    domains: undefined,
  },
  {
    email: 'alias2@qoranet.com',
    note: 'Test Alias 2',
    domains: undefined,
  },
]
...
const expectAliasesNotVisible = async () => {
  await waitFor(() => {
    expect(screen.queryByText('alias1@qoranet.com')).not.toBeInTheDocument()
    expect(screen.queryByText('alias2@qoranet.com')).not.toBeInTheDocument()
  })
}
...
describe('ManagePageConnected', () => {
  ...
  it('shows loading state initially', async () => {
    await setupTest()
    await waitFor(() => {
      expect(document.querySelector('qora-progressring')).toBeInTheDocument()
      expect(
        screen.getByText(S.SETTINGS_EMAIL_ALIASES_CONNECTING_TO_QORAI_ACCOUNT),
      ).toBeInTheDocument()
    })
  })
  ...
  it('shows main view when email is available', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(S.SETTINGS_EMAIL_ALIASES_QORAI_ACCOUNT),
      ).toBeInTheDocument()
      expect(
  ...
  it('can add new aliases via observer', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('alias1@qoranet.com')).toBeInTheDocument()
      expect(screen.queryByText('alias2@qoranet.com')).not.toBeInTheDocument()
      expect(screen.queryByText('alias3@qoranet.com')).not.toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(screen.queryByText('alias1@qoranet.com')).toBeInTheDocument()
      expect(screen.queryByText('alias2@qoranet.com')).toBeInTheDocument()
      expect(screen.queryByText('alias3@qoranet.com')).toBeInTheDocument()
    })
  })
  ...
  it('can update aliases via observer', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('alias1@qoranet.com')).toBeInTheDocument()
      expect(screen.queryByText('alias2@qoranet.com')).not.toBeInTheDocument()
      expect(screen.queryByText('2.alias@qoranet.com')).toBeInTheDocument()
      expect(screen.queryByText('alias3@qoranet.com')).toBeInTheDocument()
    })
  })
