...
#include "qorai/components/email_aliases/email_aliases_service.h"
...
#include "qorai/components/email_aliases/email_aliases_api.h"
...
constexpr char kQoraiServicesKeyHeader[] = "Qorai-Key";
...
const net::NetworkTrafficAnnotationTag kTrafficAnnotation =
    net::DefineNetworkTrafficAnnotation("qorai_accounts_service", R"(
      semantics {
        sender: "Email Aliases service"
        description:
          "Call Qorai Accounts Service API"
        trigger:
          "When the user requests to authenticate with Email Aliases"
        destination: QORAI_OWNED_SERVICE
      }
      policy {
        cookies_allowed: YES
    })");
...
GURL EmailAliasesService::GetAccountsServiceVerifyInitURL() {
  return GURL(absl::StrFormat(kAccountServiceEndpoint,
                              qorai_domains::GetServicesDomain("accounts.bsg"),
...
GURL EmailAliasesService::GetEmailAliasesServiceURL() {
  return GURL(absl::StrFormat(kEmailAliasesServiceURL,
                              qorai_domains::GetServicesDomain("aliases")));
}
...
void EmailAliasesService::RequestAuthentication(
  ...
  resource_request->headers.SetHeader(kQoraiServicesKeyHeader,
...
void EmailAliasesService::UpdateAlias(
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/49229):
...
void EmailAliasesService::OnRefreshAliasesResponse(
    std::optional<std::string> response_body) {
  // TODO(https://github.com/qorai/qorai-browser/issues/48959):
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/49624):
