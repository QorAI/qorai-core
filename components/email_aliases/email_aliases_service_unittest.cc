...
// TODO(https://github.com/qorai/qorai-browser/issues/48696): Add tests for
    ...
    testing::Values(
        GenerateAliasTestCase{
            {.name = "Success",
             .body =
                 R"({ "message": "created",
                      "alias": "mock-1234@qoraialias.com" })",
             .expected_result = base::ok("mock-1234@qoraialias.com")}},
        GenerateAliasTestCase{
            {.name = "BackendError",
...
TEST_F(EmailAliasesAPITest, ApiFetch_AttachesAuthTokenAndAPIKeyHeaders) {
  ...
  const std::string alias_email = "mock-1234@qoraialias.com";
  url_loader_factory_.AddResponse(
      manage_url.spec(),
  ...
  EXPECT_EQ(seen_api_key, BUILDFLAG(QORAI_SERVICES_KEY));
}
