...
TEST_F(NTPBackgroundImagesServiceTest, InternalDataTest) {
  ...
  EXPECT_EQ("QorAI Software", background_images_data->backgrounds[0].author);
  EXPECT_EQ("https://qorai.com/", background_images_data->backgrounds[0].link);
  EXPECT_TRUE(observer_.on_background_images_updated);
  EXPECT_TRUE(
...
constexpr char kTestSuperReferral[] = R"(
    {
      ...
      "topSites": [
        {
          "name": "QorAI",
          "destinationUrl": "https://qorai.com/",
