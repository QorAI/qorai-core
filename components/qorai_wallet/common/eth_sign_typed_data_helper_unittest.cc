...
TEST(EthSignedTypedDataHelperUnitTest, EncodedData) {
  ...
  EXPECT_FALSE(helper->EncodeData("Qorai", data_dict));
...
TEST(EthSignedTypedDataHelperUnitTest, EncodeField) {
  ...
  // string
  {
    auto encoded_field = helper->EncodeField("string", base::Value("qorai"));
    ASSERT_TRUE(encoded_field);
    EXPECT_EQ(
        base::ToLowerASCII(base::HexEncode(*encoded_field)),
  ...
  {  // custom type but not dictionary
    EXPECT_FALSE(helper->EncodeField("Qorai123", base::Value(1)));
    EXPECT_FALSE(helper->EncodeField("Qorai123", base::Value("123")));
    EXPECT_FALSE(helper->EncodeField("Qorai123", base::Value(true)));
    EXPECT_FALSE(helper->EncodeField("Qorai123", list));
  }
}
