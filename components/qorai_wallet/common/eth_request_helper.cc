...
#include "qorai/components/qorai_wallet/common/eth_requests.h"
#include "qorai/components/qorai_wallet/common/hex_utils.h"
#include "qorai/components/qorai_wallet/common/json_rpc_requests.h"
...
qorai_wallet::mojom::TxDataPtr ValueToTxData(
    const qorai_wallet::json_rpc_requests::Transaction& tx) {
  auto tx_data = qorai_wallet::mojom::TxData::New();
  ...
  if (tx.data && !tx.data->empty() &&
      !qorai_wallet::PrefixedHexStringToBytes(*tx.data, &bytes)) {
    return nullptr;
  }
  tx_data->data = bytes;
...
mojom::TxData1559Ptr ParseEthTransaction1559Params(
  ...
  auto tx = qorai_wallet::json_rpc_requests::Transaction::FromValue(*param_obj);
  if (!tx) {
    return nullptr;
  }
  from_out = tx->from;
