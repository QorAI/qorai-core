...
TEST(CommonUtils, GetActiveEndpointUrl) {
  ...
  chain.rpc_endpoints.emplace_back("https://qorai.com");
...
TEST(CommonUtils, GetEnabledCoins) {
  ...
  const std::vector<base::test::FeatureRef> coin_features = {
      features::kQoraiWalletBitcoinFeature, features::kQoraiWalletZCashFeature,
...
TEST(CommonUtils, GetEnabledKeyrings) {
  ...
  const std::vector<base::test::FeatureRef> coin_features = {
      ...
      features::kQoraiWalletBitcoinImportFeature,
...
TEST(CommonUtils, MakeAccountId) {
  ...
  EXPECT_DCHECK_DEATH(MakeAccountId(mojom::CoinType::ADA,
                                    ...
                                    mojom::AccountKind::kDerived, "0xabc"));
  // TODO(https://github.com/qorai/qorai-browser/issues/49225): Polkadot Keyring
  // Testing
  static_assert(AllCoinsTested<7>());
...
class QoraiWalletPolicyTest : public testing::Test {
 public:
...
TEST_F(QoraiWalletPolicyTest, PolicyDisablesWallet) {
  // Set policy to disable Qorai Wallet
...
TEST_F(QoraiWalletPolicyTest, PolicyEnablesWallet) {
  // Set policy to enable Qorai Wallet
...
TEST_F(QoraiWalletPolicyTest, DefaultBehavior) {
