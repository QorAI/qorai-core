 ...
 protected:
  void InitWallet() {
    ...
    asset_ratio_service_ =
        qorai_wallet::AssetRatioServiceFactory::GetServiceForContext(
            GetProfile());
  ...
  base::span<const std::string_view> GetWebUISchemes() {
    static auto constexpr kWebUISchemes =
        std::to_array<std::string_view>({"chrome://", "qorai://"});
  ...
  raw_ptr<qorai_wallet::AssetRatioService> asset_ratio_service_;
  raw_ptr<qorai_wallet::KeyringService> keyring_service_;
  raw_ptr<qorai_wallet::JsonRpcService> json_rpc_service_;
  raw_ptr<qorai_wallet::QoraiWalletService> wallet_service_;
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestWalletPageRoute) {
  ...
  const GURL expected_virtual_url =
      GURL("qorai://wallet/crypto/portfolio/assets");
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest,
  ...
  const GURL expected_url = GURL("qorai://wallet/crypto/portfolio/assets");
  for (auto scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest,
  ...
  const GURL expected_url = GURL("qorai://wallet/swap");
  for (auto scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestSendPageAppearing) {
  const GURL expected_url = GURL("qorai://wallet/send");
  for (auto scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest,
  ...
  const GURL expected_url = GURL("qorai://wallet/crypto/deposit-funds");
  for (auto scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestMeldPageAppearing) {
  const GURL expected_url = GURL("qorai://wallet/crypto/fund-wallet");
  for (auto scheme : GetWebUISchemes()) {
