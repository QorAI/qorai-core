...
TEST(AssetRatioResponseParserUnitTest, ParseAssetPrices) {
  ...
  std::vector<qorai_wallet::mojom::AssetPricePtr> prices =
      ParseAssetPrices(ParseJson(json));
  ASSERT_EQ(prices.size(), 2UL);

  // Check QOR token (ETH chain with contract address)
...
TEST(AssetRatioResponseParserUnitTest, ParseAssetPriceHistory) {
  // https://ratios.bsg.qoraisoftware.com/v2/history/coingecko/basic-attention-token/usd/2021-06-03T15%3A00%3A00.000Z/2021-06-03T18%3A00%3A00.000Z
  std::string json(R"(
    {
      "payload": {
...
TEST(AssetRatioResponseParserUnitTest, ParseCoinMarkets) {
  // https://ratios.rewards.qorai.software/v2/market/provider/coingecko\?vsCurrency\=usd\&limit\=2
  std::string json(R"(
    {
      "payload": [
        {
  ...
  std::vector<qorai_wallet::mojom::CoinMarketPtr>& values = *parsed_values;
