...
#include "qorai/components/qorai_wallet/browser/swap_request_helper.h"
...
base::flat_map<std::string, std::string> GetHeaders() {
  return {{kQoraiServicesKeyHeader, BUILDFLAG(QORAI_SERVICES_KEY)}};
}
  ...
  if (auto quote = lifi::ParseQuoteResponse(api_request_result.value_body())) {
    if (quote->routes.empty()) {
      ...
      error_response->message =
          l10n_util::GetStringUTF8(IDS_QORAI_WALLET_NO_ROUTES_FOUND);
