...
qorai_wallet::mojom::MeldLogoImagesPtr ParseMeldLogos(
    const std::optional<qorai_wallet::meld_integration_responses::Logos>&
        logos) {
  if (!logos) {
    return nullptr;
  }

  return qorai_wallet::mojom::MeldLogoImages::New(
      logos->dark, logos->dark_short, logos->light, logos->light_short);
}

std::optional<std::vector<qorai_wallet::mojom::MeldRegionPtr>> ParseMeldRegions(
    const std::optional<
        std::vector<qorai_wallet::meld_integration_responses::Region>>&
        regions) {
  if (!regions) {
    return std::nullopt;
  }

  std::vector<qorai_wallet::mojom::MeldRegionPtr> result;
  for (const auto& region_value : *regions) {
    auto reg = qorai_wallet::mojom::MeldRegion::New(region_value.region_code,
                                                    region_value.name);
