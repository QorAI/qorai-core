...
#include "qorai/components/qorai_wallet/browser/tx_storage_delegate_impl.h"
...
TestQoraiWalletServiceDelegate::TestQoraiWalletServiceDelegate() {
  EXPECT_TRUE(temp_dir_.CreateUniqueTempDir());
}

base::FilePath TestQoraiWalletServiceDelegate::GetWalletBaseDirectory() {
  return temp_dir_.GetPath();
}

bool TestQoraiWalletServiceDelegate::IsPrivateWindow() {
  return false;
}

// static
std::unique_ptr<QoraiWalletServiceDelegate>
TestQoraiWalletServiceDelegate::Create() {
  return std::make_unique<TestQoraiWalletServiceDelegate>();
}
...
SignMessageRequestWaiter::SignMessageRequestWaiter(
  ...
  subscription_ =
      qorai_wallet_service_->RegisterSignMessageRequestAddedCallback(
          base::BindRepeating(
...
void SignMessageRequestWaiter::OnSignMessageRequestAdded() {
  future_.SetValue(
      qorai_wallet_service_->GetPendingSignMessageRequestsSync().back()->id);
}
...
SignCardanoTransactionRequestWaiter::SignCardanoTransactionRequestWaiter(
    ...
    : qorai_wallet_service_(qorai_wallet_service) {
  subscription_ =
      qorai_wallet_service_->RegisterSignTransactionRequestAddedCallback(
...
void SignCardanoTransactionRequestWaiter::OnSignTransactionRequestAdded() {
  future_.SetValue(
      qorai_wallet_service_->GetPendingSignCardanoTransactionRequestsSync()
          .back()
          .Clone());
}
...
SignCardanoTransactionRequestWaiter::WaitAndProcess(bool approved) {
  ...
  auto id = request->id;
  qorai_wallet_service_->NotifySignCardanoTransactionRequestProcessed(
