...
SolanaProviderImpl::GetDeserializedMessage(
  ...
  // implementation. See https://github.com/qorai/qorai-browser/issues/23542
  // for more details.
...
void SolanaProviderImpl::ContinueSignTransaction(
  ...
  qorai_wallet_service_->AddSignSolTransactionsRequest(
      std::move(request),
...
void SolanaProviderImpl::OnContentSettingChanged(
  ...
  if (content_type != ContentSettingsType::QORAI_SOLANA) {
    return;
  }
  auto account = keyring_service_->GetSelectedSolanaDappAccount();
  if (!account) {
