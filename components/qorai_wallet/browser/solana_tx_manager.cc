...
#include "qorai/components/qorai_wallet/browser/solana_tx_manager.h"
...
#include "qorai/components/qorai_wallet/browser/solana_block_tracker.h"
...
#include "qorai/components/qorai_wallet/browser/solana_message_header.h"
#include "qorai/components/qorai_wallet/browser/solana_tx_meta.h"
#include "qorai/components/qorai_wallet/browser/solana_tx_state_manager.h"
...
void SolanaTxManager::RetryTransaction(const std::string& tx_meta_id,
  ...
  if (!meta->IsRetriable()) {
    std::move(callback).Run(
        false, "",
        l10n_util::GetStringUTF8(IDS_QORAI_WALLET_TRANSACTION_NOT_RETRIABLE));
    return;
  }
