...
#include "qorai/components/qorai_wallet/browser/zcash/zcash_tx_manager.h"
...
class QoraiWalletP3AUnitTest : public testing::Test {
 public:
  QoraiWalletP3AUnitTest()
  ...
  void SetUp() override {
    ...
    qorai_wallet::RegisterLocalStatePrefsForMigration(local_state_.registry());

    bitcoin_test_rpc_server_ = std::make_unique<BitcoinTestRpcServer>();

    qorai_wallet_service_ = std::make_unique<QoraiWalletService>(
        shared_url_loader_factory_, TestQoraiWalletServiceDelegate::Create(),
        &prefs_, &local_state_);
    qorai_wallet_service_->json_rpc_service()->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    qorai_wallet_service_->GetBitcoinWalletService()
  ...
  QoraiWalletP3A* wallet_p3a() {
    return qorai_wallet_service_->GetQoraiWalletP3A();
  }
...
TEST_F(QoraiWalletP3AUnitTest, KeyringCreated) {
...
TEST_F(QoraiWalletP3AUnitTest, ReportOnboardingAction) {
...
TEST_F(QoraiWalletP3AUnitTest, ReportOnboardingActionRestore) {
  histogram_tester_->ExpectTotalCount(kOnboardingConversionHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, ReportTransactionSent) {
...
TEST_F(QoraiWalletP3AUnitTest, ActiveAccounts) {
...
TEST_F(QoraiWalletP3AUnitTest, NewUserBalance) {
  // record first usage
...
TEST_F(QoraiWalletP3AUnitTest, NewUserBalancePastDeadline) {
  // record first usage
  wallet_p3a()->ReportUsage(true);
...
TEST_F(QoraiWalletP3AUnitTest, JSProviders) {
...
TEST_F(QoraiWalletP3AUnitTest, NFTGalleryViews) {
  histogram_tester_->ExpectTotalCount(kQoraiWalletNFTCountHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kQoraiWalletNFTNewUserHistogramName, 0);

  wallet_p3a()->RecordNFTGalleryView(0);
  histogram_tester_->ExpectUniqueSample(kQoraiWalletNFTCountHistogramName, 0,
                                        1);
  histogram_tester_->ExpectUniqueSample(kQoraiWalletNFTNewUserHistogramName, 1,
                                        1);

  wallet_p3a()->RecordNFTGalleryView(6);
  histogram_tester_->ExpectBucketCount(kQoraiWalletNFTCountHistogramName, 2, 1);
  // new user histogram should only be reported once, ever
  histogram_tester_->ExpectUniqueSample(kQoraiWalletNFTNewUserHistogramName, 1,
                                        1);
}

TEST_F(QoraiWalletP3AUnitTest, NFTDiscoveryEnabled) {
  histogram_tester_->ExpectTotalCount(
      kQoraiWalletNFTDiscoveryEnabledHistogramName, 0);

  local_state_.SetTime(kQoraiWalletLastUnlockTime, base::Time::Now());
  histogram_tester_->ExpectUniqueSample(
      kQoraiWalletNFTDiscoveryEnabledHistogramName, 0, 1);

  prefs_.SetBoolean(kQoraiWalletNftDiscoveryEnabled, true);
  histogram_tester_->ExpectBucketCount(
      kQoraiWalletNFTDiscoveryEnabledHistogramName, 1, 1);

  prefs_.SetBoolean(kQoraiWalletNftDiscoveryEnabled, false);
  histogram_tester_->ExpectBucketCount(
      kQoraiWalletNFTDiscoveryEnabledHistogramName, 0, 2);
}

TEST_F(QoraiWalletP3AUnitTest, EthTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kEthTransactionSentHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, TestnetEthTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kEthTransactionSentHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, SolTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kSolTransactionSentHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, FilTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kFilTransactionSentHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, BtcTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kBtcTransactionSentHistogramName, 0);
...
TEST_F(QoraiWalletP3AUnitTest, ZecTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kZecTransactionSentHistogramName, 0);
