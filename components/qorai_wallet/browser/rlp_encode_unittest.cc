...
#include "qorai/components/qorai_wallet/browser/rlp_encode.h"
...
TEST(RLPEncodeTest, LongString2) {
  auto v = RLPEncode(RLPTestStringToValue(
      ...
      "suscipit nec lorem. Aenean quis qora mollis, vulputate elit varius, "
...
TEST(RLPEncodeTest, DictionaryValueNotSupported) {
  base::Value::Dict d;
  d.Set("test", true);
  ASSERT_TRUE(qorai_wallet::RLPEncode(base::Value(std::move(d))).empty());
}
