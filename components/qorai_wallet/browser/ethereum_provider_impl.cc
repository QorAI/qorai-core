...
#include "qorai/components/qorai_wallet/browser/account_resolver_delegate_impl.h"
...
#include "qorai/components/qorai_wallet/browser/siwe_message_parser.h"
...
#include "qorai/components/qorai_wallet/common/eth_sign_typed_data_helper.h"
...
EthereumProviderImpl::EthereumProviderImpl(
    ...
    : host_content_settings_map_(host_content_settings_map),
      ...
      json_rpc_service_(qorai_wallet_service->json_rpc_service()),
      tx_service_(qorai_wallet_service->tx_service()),
      keyring_service_(qorai_wallet_service->keyring_service()),
...
void EthereumProviderImpl::AddEthereumChain(base::Value::List params,
  ...
  if (qorai_wallet_service_->network_manager()
  ...
  // https://github.com/qorai/qorai-browser/issues/17637
...
void EthereumProviderImpl::SendOrSignTransactionInternal(
  ...
  const bool is_eip_1559_network =
      qorai_wallet_service_->network_manager()->IsEip1559Chain(chain->chain_id);
...
void EthereumProviderImpl::SignMessage(const std::string& address,
  ...
  if (siwe_message) {
    ...
    if (!base::HexStringToUInt64(chain_id_hex, &chain_id) ||
      ...
      qorai_wallet_service_->AddSignMessageError(mojom::SignMessageError::New(
          ...
          l10n_util::GetStringFUTF8(
              ...
              base::StrCat(
                  {l10n_util::GetStringUTF16(IDS_QORAI_WALLET_CHAIN_ID), u": ",
    ...
    if (EthAddress::FromHex(address) !=
      ...
      return RejectMismatchError(
          std::move(id),
          l10n_util::GetStringFUTF8(
              IDS_QORAI_WALLET_SIGN_MESSAGE_ACCOUNT_MISMATCH,
              base::ASCIIToUTF16(siwe_message->address)),
          std::move(callback));
    }
    if (delegate_->GetOrigin() != siwe_message->origin) {
      ...
      return RejectMismatchError(
          std::move(id),
          l10n_util::GetStringFUTF8(
              IDS_QORAI_WALLET_SIGN_MESSAGE_DOMAIN_MISMATCH,
              base::ASCIIToUTF16(err_domain)),
          std::move(callback));
    }
...
void EthereumProviderImpl::GetEncryptionPublicKey(const std::string& address,
  ...
  // Only show bubble when there is no immediate error
  qorai_wallet_service_->AddGetPublicKeyRequest(
...
void EthereumProviderImpl::ContinueDecryptWithSanitizedJson(
  ...
  qorai_wallet_service_->AddDecryptRequest(account_id, origin,
...
void EthereumProviderImpl::SignMessageInternal(
  ...
  qorai_wallet_service_->AddSignMessageRequest(
      std::move(request),
...
void EthereumProviderImpl::OnContentSettingChanged(
  ...
  if (content_type == ContentSettingsType::QORAI_ETHEREUM) {
    UpdateKnownAccounts();
  }
}
...
void EthereumProviderImpl::AddSuggestToken(mojom::BlockchainTokenPtr token,
  ...
  qorai_wallet_service_->AddSuggestTokenRequest(
