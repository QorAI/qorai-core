...
std::optional<std::string> ParseJsonFile(base::FilePath path,
  ...
  // See https://github.com/qorai/qorai-browser/issues/30940 for details.
...
TokenListMap BlockchainRegistry::GetEthTokenListMap(
  ...
  for (const auto& chain_id : chain_ids) {
    ...
    const auto& tokens = token_list_map_[key];
    std::vector<qorai_wallet::mojom::BlockchainTokenPtr> tokens_copy(
        tokens.size());
    std::transform(
        tokens.begin(), tokens.end(), tokens_copy.begin(),
        [](const qorai_wallet::mojom::BlockchainTokenPtr& current_token)
            -> qorai_wallet::mojom::BlockchainTokenPtr {
