...
#include "qorai/components/qorai_wallet/browser/swap_response_parser.h"
#include "qorai/components/qorai_wallet/common/qorai_wallet.mojom-shared.h"
...
mojom::SwapTransactionParamsUnionPtr GetCannedJupiterTransactionParams(
  ...
  auto quote = qorai_wallet::mojom::JupiterQuote::New();
  ...
  auto platform_fee = qorai_wallet::mojom::JupiterPlatformFee::New();
  platform_fee->amount = "93326";
  platform_fee->fee_bps = "85";
  quote->platform_fee = std::move(platform_fee);

  auto swap_info_1 = qorai_wallet::mojom::JupiterSwapInfo::New();
  ...
  auto step_1 = qorai_wallet::mojom::JupiterRouteStep::New();
  step_1->percent = "100";
  step_1->swap_info = std::move(swap_info_1);

  auto swap_info_2 = qorai_wallet::mojom::JupiterSwapInfo::New();
  ...
  auto step_2 = qorai_wallet::mojom::JupiterRouteStep::New();
  step_2->percent = "100";
...
TEST_F(SwapServiceUnitTest, GetZeroExQuoteError) {
  ...
  error_response->message =
      l10n_util::GetStringUTF8(IDS_QORAI_WALLET_SWAP_INSUFFICIENT_LIQUIDITY);
...
TEST_F(SwapServiceUnitTest, GetZeroExQuoteURL) {
  ...
  for (const auto& [chain_id, encoded_chain_id] : chain_ids) {
    ...
    EXPECT_EQ(url,
              absl::StrFormat(
                  "https://api.0x.wallet.qorai.com/swap/allowance-holder/price?"
                  "chainId=%s&"
...
TEST_F(SwapServiceUnitTest, GetZeroExTransactionURL) {
  ...
  for (const auto& [chain_id, encoded_chain_id] : chain_ids) {
    ...
    EXPECT_EQ(url,
              absl::StrFormat(
                  "https://api.0x.wallet.qorai.com/swap/allowance-holder/quote?"
                  "chainId=%s&"
                  "taker=0xa92D461a9a988A7f11ec285d39783A637Fdd6ba4&"
                  "sellAmount=1000000000000000000000&"
                  "buyToken=ETH&"
...
TEST_F(SwapServiceUnitTest, GetJupiterQuoteURL) {
  ...
  EXPECT_EQ(url,
            "https://jupiter-lite.wallet.qorai.com/swap/v1/quote?"
...
TEST_F(SwapServiceUnitTest, GetJupiterTransactionURL) {
  ...
  EXPECT_EQ(url, "https://jupiter-lite.wallet.qorai.com/swap/v1/swap");
}
...
TEST_F(SwapServiceUnitTest, GetLiFiQuoteURL) {
  ...
  EXPECT_EQ(url, "https://lifi.wallet.qorai.com/v1/advanced/routes");
}

TEST_F(SwapServiceUnitTest, GetLiFiTransactionURL) {
  auto url = swap_service_->GetLiFiTransactionURL();
  EXPECT_EQ(url, "https://lifi.wallet.qorai.com/v1/advanced/stepTransaction");
}
...
TEST_F(SwapServiceUnitTest, GetSquidURL) {
  ...
  EXPECT_EQ(url, "https://squid.wallet.qorai.com/v2/route");
}
