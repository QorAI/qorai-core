...
#include "qorai/components/qorai_wallet/browser/bip39.h"
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_hd_keyring.h"
...
#include "qorai/components/qorai_wallet/browser/ethereum_keyring.h"
#include "qorai/components/qorai_wallet/browser/fil_transaction.h"
#include "qorai/components/qorai_wallet/browser/filecoin_keyring.h"
...
#include "qorai/components/qorai_wallet/browser/keyring_service_prefs.h"
...
#include "qorai/components/qorai_wallet/common/switches.h"
#include "qorai/components/qorai_wallet/common/test_utils.h"  // IWYU pragma: keep
...
constexpr char kPasswordQorai[] = "qorai";
constexpr char kPasswordQorai123[] = "qorai123";
...
class KeyringServiceUnitTest : public testing::Test {
 ...
 protected:
  ...
  static std::optional<std::string> EncodePrivateKeyForExport(
      KeyringService* service,
      mojom::AccountIdPtr account_id,
      const std::string& password = kPasswordQorai) {
 ...
 private:
  base::test::ScopedFeatureList scoped_btc_ledger_feature_{
      features::kQoraiWalletBitcoinLedgerFeature};
...
TEST_F(KeyringServiceUnitTest, SetPrefForKeyring) {
  ...
  const auto& keyrings_pref = GetPrefs()->GetDict(kQoraiWalletKeyrings);
...
TEST_F(KeyringServiceUnitTest, UnlockResumesDefaultKeyring) {
  ...
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    ASSERT_TRUE(CreateWallet(&service, "qorai"));
    ASSERT_TRUE(AddAccount(&service, mojom::CoinType::ETH,
                           mojom::KeyringId::kDefault, "Account2"));

    salt = GetPrefs()->GetString(kQoraiWalletEncryptorSalt);
    mnemonic = GetPrefs()->GetDict(kQoraiWalletMnemonic).Clone();
  }
  {
    // KeyringService is now destructed, simulating relaunch
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    EXPECT_TRUE(Unlock(&service, "qorai"));
    ASSERT_FALSE(service.IsLockedSync());

    EXPECT_EQ(GetPrefs()->GetString(kQoraiWalletEncryptorSalt), salt);
    EXPECT_EQ(GetPrefs()->GetDict(kQoraiWalletMnemonic), mnemonic);
    EXPECT_EQ(
        2u,
  ...
  }
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    // wrong password
    EXPECT_FALSE(Unlock(&service, "qorai123"));
...
TEST_F(KeyringServiceUnitTest, UnlockResumesNewKeyring) {
  ...
  {
    ScopedDictPrefUpdate keyrings_update(GetPrefs(), kQoraiWalletKeyrings);
...
TEST_F(KeyringServiceUnitTest, GetWalletMnemonic) {
  ...
  GetPrefs()->SetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated, true);
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());

  // no pref exists yet
  EXPECT_EQ(GetWalletMnemonic(kPasswordQorai, &service), std::nullopt);

  AccountUtils(&service).CreateWallet(kMnemonicDivideCruise, kPasswordQorai);
  EXPECT_EQ(GetWalletMnemonic(kPasswordQorai, &service), kMnemonicDivideCruise);
  ...
  ASSERT_FALSE(Unlock(&service, kPasswordQorai123));
  ASSERT_TRUE(service.IsLockedSync());

  EXPECT_EQ(GetWalletMnemonic(kPasswordQorai, &service), std::nullopt);

  ASSERT_TRUE(Unlock(&service, kPasswordQorai));
  ASSERT_FALSE(service.IsLockedSync());

  // Can only get mnemonic when password is correct.
  EXPECT_EQ(GetWalletMnemonic(kPasswordQorai123, &service), std::nullopt);
  EXPECT_EQ(GetWalletMnemonic(kPasswordQorai, &service), kMnemonicDivideCruise);
}
...
TEST_F(KeyringServiceUnitTest, ValidatePassword) {
  ...
  EXPECT_TRUE(ValidatePassword(&service, "qorai"));
  EXPECT_FALSE(service.IsLockedSync());
  EXPECT_FALSE(ValidatePassword(&service, "qorai123"));
  EXPECT_FALSE(service.IsLockedSync());
...
TEST_F(KeyringServiceUnitTest, Reset) {
  ...
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kQoraiWalletKeyrings));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kQoraiWalletEncryptorSalt));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kQoraiWalletMnemonic));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kQoraiWalletLegacyEthSeedFormat));

  EXPECT_CALL(observer, WalletReset());
  service.Reset();
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kQoraiWalletKeyrings));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kQoraiWalletEncryptorSalt));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kQoraiWalletMnemonic));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kQoraiWalletLegacyEthSeedFormat));
...
TEST_F(KeyringServiceUnitTest, AccountMetasForKeyring_PolkadotDisabled) {
  ...
  feature_list.InitAndDisableFeature(features::kQoraiWalletPolkadotFeature);

  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  ASSERT_TRUE(RestoreWallet(&service, kMnemonicDivideCruise, "qorai", false));
...
TEST_F(KeyringServiceUnitTest, AccountMetasForKeyring_PolkadotEnabled) {
  base::test::ScopedFeatureList feature_list{
      features::kQoraiWalletPolkadotFeature};
...
TEST_F(KeyringServiceUnitTest, CreateAndRestoreWallet) {
  ...
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&mnemonic_to_be_restored, &service, &address0] {
        EXPECT_TRUE(
            RestoreWallet(&service, *mnemonic_to_be_restored, "qorai1", false));
        {
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::KeyringId::kDefault);
          EXPECT_EQ(account_infos.size(), 1u);
...
TEST_F(KeyringServiceUnitTest, DefaultSolanaAccountCreated) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  ASSERT_NE(CreateWallet(&service, "qorai"), std::nullopt);
...
TEST_F(KeyringServiceUnitTest, SetDefaultKeyringImportedAccountName) {
  ...
  ASSERT_TRUE(CreateWallet(&service, kPasswordQorai));
...
TEST_F(KeyringServiceUnitTest, RestoreLegacyQoraiWallet) {
  ...
  QoraiWalletService qorai_wallet_service(
      shared_url_loader_factory(), TestQoraiWalletServiceDelegate::Create(),
      GetPrefs(), GetLocalState());
  KeyringService& service = *qorai_wallet_service.keyring_service();
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&service](const char* mnemonic, const char* address, bool is_legacy,
                 bool expect_result) {
        service.Reset();
        EXPECT_EQ(expect_result,
                  RestoreWallet(&service, mnemonic, "qorai1", is_legacy));
        if (expect_result) {
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::KeyringId::kDefault);
          ...
          // legacy_qorai_wallet pref so it will use the right seed
          service.Lock();
          EXPECT_TRUE(Unlock(&service, "qorai1"));
          account_infos.clear();
          account_infos =
              service.GetAccountInfosForKeyring(mojom::KeyringId::kDefault);
          ASSERT_EQ(account_infos.size(), 1u);
  ...
  // qorai legacy mnemonic can only be 24 words
...
TEST_F(KeyringServiceUnitTest, HardwareAccounts) {
  ...
  for (const auto& account : accounts) {
    ...
    ASSERT_TRUE(
        GetPrefs()->GetDict(kQoraiWalletKeyrings).FindByDottedPath(path));
  }
  {
...
TEST_F(KeyringServiceUnitTest, AutoLock) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  std::optional<std::string> mnemonic = CreateWallet(&service, "qorai");
  ...
  service.Reset();
  ASSERT_TRUE(RestoreWallet(&service, *mnemonic, "qorai", false));
  ASSERT_FALSE(service.IsLockedSync());
  ...
  ASSERT_FALSE(service.IsLockedSync());
  task_environment_.FastForwardBy(base::Minutes(4));
  GetPrefs()->SetInteger(kQoraiWalletAutoLockMinutes, 3);
  ...
  task_environment_.FastForwardBy(base::Minutes(2));
  EXPECT_FALSE(service.IsLockedSync());
  GetPrefs()->SetInteger(kQoraiWalletAutoLockMinutes, 10);
  task_environment_.FastForwardBy(base::Minutes(9));
  EXPECT_FALSE(service.IsLockedSync());
  task_environment_.FastForwardBy(base::Minutes(1));
  EXPECT_TRUE(service.IsLockedSync());
...
TEST_F(KeyringServiceUnitTest, SetSelectedAccount_CardanoDisabled) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitWithFeatures({}, {features::kQoraiWalletCardanoFeature});
  ...
  EXPECT_CALL(observer, SelectedDappAccountChanged(mojom::CoinType::SOL, _))
      .Times(0);
  EXPECT_TRUE(
      RemoveAccount(&service, imported_account->account_id, kPasswordQorai));
  EXPECT_TRUE(Lock(&service));
  EXPECT_EQ(first_account, service.GetSelectedWalletAccount());
  EXPECT_EQ(first_account, service.GetSelectedEthereumDappAccount());
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, SignMessageByDefaultKeyring) {
  ...
  EXPECT_EQ(
      service.SignMessageByDefaultKeyring(invalid_account, message, false)
          .error(),
      l10n_util::GetStringFUTF8(IDS_QORAI_WALLET_SIGN_MESSAGE_INVALID_ADDRESS,
  ...
  EXPECT_EQ(
      ...
      l10n_util::GetStringUTF8(IDS_QORAI_WALLET_SIGN_MESSAGE_UNLOCK_FIRST));
}
...
TEST_F(KeyringServiceUnitTest, ImportBitcoinAccount) {
  ...
  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc1->account_id, kPasswordQorai));
  observer.WaitAndVerify();

  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc2->account_id, kPasswordQorai));
  observer.WaitAndVerify();

  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc3->account_id, kPasswordQorai));
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, SolanaKeyring) {
  {
    ...
    service.Lock();
    ASSERT_TRUE(Unlock(&service, "qorai"));
...
class KeyringServiceAccountDiscoveryUnitTest : public KeyringServiceUnitTest {
 public:
  ...
  void PrepareAccounts(mojom::CoinType coin_type, mojom::KeyringId keyring_id) {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    saved_mnemonic_ = CreateWallet(&service, "qorai").value_or("");
...
TEST_F(KeyringServiceAccountDiscoveryUnitTest, AccountDiscovery) {
  ...
  QoraiWalletService qorai_wallet_service(
      shared_url_loader_factory(), TestQoraiWalletServiceDelegate::Create(),
      GetPrefs(), GetLocalState());
  ASSERT_TRUE(qorai_wallet_service.GetBitcoinWalletService());
  BitcoinTestRpcServer bitcoin_test_rpc_server(
      *qorai_wallet_service.GetBitcoinWalletService());
  ...
  EXPECT_TRUE(RestoreWallet(&service, saved_mnemonic(), "qorai1", false));
  observer.WaitAndVerify();
  std::vector<mojom::AccountInfoPtr> account_infos =
      service.GetAccountInfosForKeyring(mojom::KeyringId::kDefault);
...
TEST_F(KeyringServiceUnitTest, AccountsAdded) {
  ...
  RestoreWallet(&service, kMnemonicDivideCruise, kPasswordQorai, false);
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, GetBitcoinAddresses) {
  ...
  ASSERT_TRUE(RestoreWallet(&service, kMnemonicAbandonAbandon, "qorai", false));
...
TEST_F(KeyringServiceUnitTest, SignCip30MessageByCardanoKeyring) {
  ...
  ASSERT_TRUE(RestoreWallet(&service, kMnemonicAbandonAbandon, "qorai", false));
  auto cardano_acc = GetAccountUtils(&service).EnsureAdaAccount(0);
  auto message = base::byte_span_from_cstring("qorai");
...
TEST_F(KeyringServiceUnitTest,
  ...
  ASSERT_TRUE(base::ReadFileToString(QoraiWalletComponentsTestDataFolder()
                                         .AppendASCII("cardano")
...
TEST_F(KeyringServiceUnitTest, GetOrchardRawBytes_ZCashDisabled) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kQoraiWalletZCashFeature);
