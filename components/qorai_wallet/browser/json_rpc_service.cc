...
#include "qorai/components/qorai_wallet/browser/eth_data_builder.h"
#include "qorai/components/qorai_wallet/browser/eth_requests.h"
#include "qorai/components/qorai_wallet/browser/eth_response_parser.h"
#include "qorai/components/qorai_wallet/browser/fil_requests.h"
#include "qorai/components/qorai_wallet/browser/fil_response_parser.h"
#include "qorai/components/qorai_wallet/browser/json_rpc_requests_helper.h"
...
#include "qorai/components/qorai_wallet/browser/nft_metadata_fetcher.h"
#include "qorai/components/qorai_wallet/browser/pref_names.h"
#include "qorai/components/qorai_wallet/browser/solana_keyring.h"
#include "qorai/components/qorai_wallet/browser/solana_requests.h"
#include "qorai/components/qorai_wallet/browser/solana_response_parser.h"
#include "qorai/components/qorai_wallet/browser/unstoppable_domains_dns_resolve.h"
#include "qorai/components/qorai_wallet/browser/unstoppable_domains_multichain_calls.h"
...
#include "qorai/components/json/json_helper.h"
#include "qorai/net/base/url_util.h"
...
using qorai_wallet::mojom::ResolveMethod;
...
constexpr char kUDPattern[] =
    ...
    "binanceus|bitcoin|bitget|bitscrunch|blockchain|boomer|qorai|calicoin|caw|"
  ...
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      semantics {
        ...
        description:
          ...
          "on behalf of the user interacting with the native Qorai wallet."
        trigger:
...
net::NetworkTrafficAnnotationTag GetENSOffchainNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable ENS on qorai://settings/extensions page."
        policy_exception_justification:
          "Not implemented."
      }
    )");
...
qorai_wallet::mojom::ResolveMethod ToMojomResolveMethod(
...
decentralized_dns::ResolveMethodTypes FromMojomResolveMethod(
    qorai_wallet::mojom::ResolveMethod method) {
  switch (method) {
...
qorai_wallet::mojom::ResolveMethod ToMojomEnsOffchainResolveMethod(
...
T* GetCustomEVMNetworkFromPrefsDict(const std::string& chain_id,
  ...
  auto* custom_evm_networks =
      custom_networks.FindList(qorai_wallet::kEthereumPrefKey);
...
void JsonRpcService::MaybeUpdateIsEip1559(const std::string& chain_id) {
  ...
  if (chain_id != qorai_wallet::mojom::kLocalhostChainId &&
      !network_manager_->CustomChainExists(chain_id, mojom::CoinType::ETH)) {
    return;
  }
...
void JsonRpcService::AnkrGetAccountBalances(
  ...
  auto headers = IsEndpointUsingQoraiWalletProxy(GURL(kAnkrAdvancedAPIBaseURL))
                     ? MakeQoraiServicesKeyHeaders()
