...
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_hardware_keyring.h"
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_hd_keyring.h"
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_import_keyring.h"
...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_cip30_serializer.h"
...
#include "qorai/components/qorai_wallet/browser/json_keystore_parser.h"
...
#include "qorai/components/qorai_wallet/browser/password_encryptor.h"
#include "qorai/components/qorai_wallet/browser/polkadot/polkadot_keyring.h"
#include "qorai/components/qorai_wallet/browser/pref_names.h"
#include "qorai/components/qorai_wallet/browser/solana_keyring.h"
...
#include "qorai/components/qorai_wallet/common/bitcoin_utils.h"
...
constexpr char kLegacyQoraiWalletDeprecated[] = "legacy_qorai_wallet";
...
std::string GetAccountName(size_t number) {
  return l10n_util::GetStringFUTF8(IDS_QORAI_WALLET_NUMBERED_ACCOUNT_NAME,
...
std::optional<std::string> DecryptWalletMnemonicFromPrefs(
  ...
  if (auto decrypted = encryptor.DecryptFromDict(
          profile_prefs->GetDict(kQoraiWalletMnemonic))) {
...
std::vector<uint8_t> GetSaltFromPrefs(const PrefService* profile_prefs) {
  ...
  auto salt_encoded = profile_prefs->GetString(kQoraiWalletEncryptorSalt);
...
bool IsLegacyEthSeedFormat(const PrefService* profile_prefs) {
  CHECK(profile_prefs);
  return profile_prefs->GetBoolean(kQoraiWalletLegacyEthSeedFormat);
}
...
void SetImportedAccountsForKeyring(
  ...
  ScopedDictPrefUpdate update(profile_prefs, kQoraiWalletKeyrings);
...
std::string GetSelectedWalletAccountFromPrefs(PrefService* profile_prefs) {
  return profile_prefs->GetString(kQoraiWalletSelectedWalletAccount);
}
...
void MaybeMigratePBKDF2Iterations(PrefService* profile_prefs,
  ...
  if (profile_prefs->GetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated)) {
    return;
  }

  // Pref is supposed to be set only as true.
  DCHECK(
      !profile_prefs->HasPrefPath(kQoraiWalletKeyringEncryptionKeysMigrated));
  ...
  for (auto keyring_id :
    ...
    if (keyring_id == mojom::KeyringId::kDefault) {
      profile_prefs->SetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated,
                                true);
    }
...
void MaybeMigrateToWalletMnemonic(PrefService* profile_prefs,
  ...
  if (auto* value =
          GetPrefForKeyring(profile_prefs, kLegacyQoraiWalletDeprecated,
                            mojom::KeyringId::kDefault)) {
    if (value->GetIfBool().value_or(false)) {
      profile_prefs->SetBoolean(kQoraiWalletLegacyEthSeedFormat, true);
    }
  }

  if (auto* value = GetPrefForKeyring(profile_prefs, kBackupCompleteDeprecated,
                                      mojom::KeyringId::kDefault)) {
    profile_prefs->SetBoolean(kQoraiWalletMnemonicBackedUp,
                              value->GetIfBool().value_or(false));
  }

  profile_prefs->SetString(kQoraiWalletEncryptorSalt,
                           base::Base64Encode(wallet_salt));
  profile_prefs->SetDict(
      kQoraiWalletMnemonic,
  ...
  for (auto keyring_id :
    ...
    SetPrefForKeyring(profile_prefs, kPasswordEncryptorSaltDeprecated,
                      base::Value(), keyring_id);
    SetPrefForKeyring(profile_prefs, kLegacyQoraiWalletDeprecated,
                      base::Value(), keyring_id);
...
bool KeyringService::IsWalletCreatedSync() {
  return profile_prefs_->GetDict(kQoraiWalletKeyrings).size() > 0;
}
...
bool KeyringService::CreateWalletInternal(const std::string& mnemonic,
  ...
  profile_prefs_->SetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated, true);
  profile_prefs_->SetBoolean(kQoraiWalletLegacyEthSeedFormat,
                             is_legacy_eth_seed_format);
  profile_prefs_->SetDict(
      kQoraiWalletMnemonic,
      encryptor_->EncryptToDict(base::as_byte_span(mnemonic),
                                CreateNonceArray()));
  profile_prefs_->SetString(kQoraiWalletEncryptorSalt,
...
bool KeyringService::IsWalletBackedUpSync() {
  return profile_prefs_->GetBoolean(kQoraiWalletMnemonicBackedUp);
}

void KeyringService::NotifyWalletBackupComplete() {
  profile_prefs_->SetBoolean(kQoraiWalletMnemonicBackedUp, true);
  for (const auto& observer : observers_) {
    observer->BackedUp();
  }
}
...
std::vector<mojom::AccountInfoPtr> KeyringService::AddHardwareAccountsSync(
  ...
  ScopedDictPrefUpdate keyrings_update(profile_prefs_, kQoraiWalletKeyrings);
...
void KeyringService::ResetAutoLockTimer() {
  if (auto_lock_timer_->IsRunning()) {
    ...
    size_t auto_lock_minutes =
        (size_t)profile_prefs_->GetInteger(kQoraiWalletAutoLockMinutes);
...
void KeyringService::GetAutoLockMinutes(GetAutoLockMinutesCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetInteger(kQoraiWalletAutoLockMinutes));
}
...
void KeyringService::SetAutoLockMinutes(int32_t minutes,
  ...
  if (minutes != old_auto_lock_minutes) {
    profile_prefs_->SetInteger(kQoraiWalletAutoLockMinutes, minutes);
  }
  std::move(callback).Run(true);
}
...
mojom::AccountInfoPtr KeyringService::GetSelectedWalletAccount() {
  ...
  auto unique_key =
      profile_prefs_->GetString(kQoraiWalletSelectedWalletAccount);
