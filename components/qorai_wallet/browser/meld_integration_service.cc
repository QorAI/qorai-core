...
#include "qorai/components/qorai_wallet/browser/meld_integration_response_parser.h"
#include "qorai/components/qorai_wallet/browser/meld_integration_responses.h"
#include "qorai/components/qorai_wallet/common/buildflags.h"
#include "qorai/components/qorai_wallet/common/meld_integration.mojom-forward.h"
  ...
  return net::DefineNetworkTrafficAnnotation("meld_integration_service", R"(
      semantics {
        ...
        description:
          ...
          "the external Meld API  for the Qorai wallet."
        trigger:
...
base::flat_map<std::string, std::string> MakeMeldApiHeaders() {
  ...
  request_headers[qorai_wallet::kMeldRpcVersionHeader] =
      qorai_wallet::kMeldRpcVersion;
...
GURL AppendFilterParams(
    GURL url,
    const qorai_wallet::mojom::MeldFilterPtr& filter,
...
void FillCustomerData(
    const qorai_wallet::mojom::CryptoWidgetCustomerDataPtr& customer_data,
...
std::optional<base::Value::Dict> GetCryptoBuyWidgetPayload(
    qorai_wallet::mojom::CryptoBuySessionDataPtr session_data,
    qorai_wallet::mojom::CryptoWidgetCustomerDataPtr customer_data) {
  if (!session_data) {
    return std::nullopt;
  }
...
std::optional<base::Value::Dict> GetCryptoSellWidgetPayload(
    qorai_wallet::mojom::CryptoSellSessionDataPtr session_data,
    qorai_wallet::mojom::CryptoWidgetCustomerDataPtr customer_data) {
  if (!session_data) {
    return std::nullopt;
  }
...
std::optional<base::Value::Dict> GetCryptoTransferWidgetPayload(
    qorai_wallet::mojom::CryptoTransferSessionDataPtr session_data,
    qorai_wallet::mojom::CryptoWidgetCustomerDataPtr customer_data) {
  if (!session_data) {
    return std::nullopt;
  }
