...
const base::Value* GetPrefForKeyring(PrefService* profile_prefs,
  ...
  const auto& keyrings_pref = profile_prefs->GetDict(kQoraiWalletKeyrings);
...
bool SetSelectedWalletAccountInPrefs(PrefService* profile_prefs,
  ...
  if (unique_key ==
      profile_prefs->GetString(kQoraiWalletSelectedWalletAccount)) {
    return false;
  }

  profile_prefs->SetString(kQoraiWalletSelectedWalletAccount, unique_key);
  return true;
}
...
std::string GetSelectedDappAccountFromPrefs(PrefService* profile_prefs,
  ...
  const char* pref_name =
      dapp_coin == mojom::CoinType::ETH   ? kQoraiWalletSelectedEthDappAccount
      : dapp_coin == mojom::CoinType::SOL ? kQoraiWalletSelectedSolDappAccount
                                          : kQoraiWalletSelectedAdaDappAccount;
