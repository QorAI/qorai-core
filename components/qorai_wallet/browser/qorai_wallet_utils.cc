...
std::optional<std::string> GetUserAssetAddress(const std::string& address,
  ...
  if (coin == mojom::CoinType::SOL) {
    ...
    if (!::qorai_wallet::IsBase58EncodedSolanaPubkey(address)) {
      return std::nullopt;
    }
    return address;
  }
...
bool ShouldCheckTokenId(const qorai_wallet::mojom::BlockchainTokenPtr& token) {
  return token->is_erc721 || token->is_erc1155;
}

bool TokenMatchesDict(const qorai_wallet::mojom::BlockchainTokenPtr& token,
...
bool IsEndpointUsingQoraiWalletProxy(const GURL& url) {
  return url.DomainIs("wallet.qorai.com") ||
         url.DomainIs("wallet.qoraisoftware.com") ||
         url.DomainIs("wallet.s.qorai.io");
}

base::flat_map<std::string, std::string> MakeQoraiServicesKeyHeaders() {
  return {
...
void UpdateLastUnlockPref(PrefService* prefs) {
  prefs->SetTime(kQoraiWalletLastUnlockTime, base::Time::Now());
}

bool HasCreatedWallets(PrefService* prefs) {
  return !prefs->GetTime(kQoraiWalletLastUnlockTime).is_null();
}
...
mojom::DefaultWallet GetDefaultEthereumWallet(PrefService* prefs) {
  return static_cast<qorai_wallet::mojom::DefaultWallet>(
...
void SetDefaultCardanoWallet(PrefService* prefs,
                             mojom::DefaultWallet default_wallet) {
  DCHECK(default_wallet == mojom::DefaultWallet::QoraiWallet ||
...
std::vector<mojom::BlockchainTokenPtr> GetAllUserAssets(PrefService* prefs) {
  ...
  const auto& user_assets_list = prefs->GetList(kQoraiWalletUserAssetsList);
...
mojom::BlockchainTokenPtr AddUserAsset(PrefService* prefs,
  ...
  ScopedListPrefUpdate update(prefs, kQoraiWalletUserAssetsList);
...
std::vector<mojom::BlockchainTokenPtr> GetDefaultEthereumAssets() {
  ...
  for (const auto& chain :
    ...
    // ETH Mainnet token is followed by QOR token.
    if (chain->chain_id == mojom::kMainnetChainId) {
      mojom::BlockchainTokenPtr qor_token = NetworkToNativeToken(*chain);
      qor_token->contract_address = mojom::kQorTokenContractAddress;
      qor_token->name = "Basic Attention Token";
      qor_token->symbol = "QOR";
      qor_token->is_erc20 = true;
      qor_token->logo = "qor.png";
      user_assets_list.push_back(std::move(qor_token));
    }
  }
...
std::string GetWeb3ClientVersion() {
  return base::StrCat(
      {"QoraiWallet/v", version_info::GetQoraiChromiumVersionNumber()});
}
...
std::string WalletInsufficientBalanceErrorMessage() {
  return l10n_util::GetStringUTF8(IDS_QORAI_WALLET_INSUFFICIENT_BALANCE);
}
...
mojom::BlowfishOptInStatus GetTransactionSimulationOptInStatus(
  ...
  return static_cast<mojom::BlowfishOptInStatus>(
      prefs->GetInteger(kQoraiWalletTransactionSimulationOptInStatus));
}

void SetTransactionSimulationOptInStatus(
    PrefService* prefs,
    const mojom::BlowfishOptInStatus& status) {
  prefs->SetInteger(kQoraiWalletTransactionSimulationOptInStatus,
...
bool IsQoraiWalletOrigin(const url::Origin& origin) {
  return origin == url::Origin::Create(GURL(kQoraiUIWalletPanelURL)) ||
         origin == url::Origin::Create(GURL(kQoraiUIWalletPageURL));
}
