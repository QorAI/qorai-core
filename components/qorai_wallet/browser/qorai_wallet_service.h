...
#ifndef QORAI_COMPONENTS_QORAI_WALLET_BROWSER_QORAI_WALLET_SERVICE_H_
#define QORAI_COMPONENTS_QORAI_WALLET_BROWSER_QORAI_WALLET_SERVICE_H_
...
#include "qorai/components/qorai_wallet/browser/asset_discovery_manager.h"
...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_wallet_service.h"
#include "qorai/components/qorai_wallet/browser/polkadot/polkadot_wallet_service.h"
...
class QoraiWalletService : public KeyedService,
                           public mojom::QoraiWalletService,
                           public KeyringServiceObserverBase,
                           public QoraiWalletServiceDelegate::Observer {
 public:
  ...
  ~QoraiWalletService() override;

  QoraiWalletService(const QoraiWalletService&) = delete;
  QoraiWalletService& operator=(const QoraiWalletService&) = delete;
  ...
  // mojom::QoraiWalletService:
  void AddObserver(::mojo::PendingRemote<mojom::QoraiWalletServiceObserver>
                       observer) override;
  ...
  // QoraiWalletServiceDelegate::Observer:
  ...
  // Resets things back to the original state of QoraiWalletService.
  ...
  QoraiWalletP3A* GetQoraiWalletP3A();
  ...
  QoraiWalletServiceDelegate* GetDelegateForTesting() {
 ...
 private:
  ...
  friend class QoraiWalletServiceUnitTest;

  FRIEND_TEST_ALL_PREFIXES(QoraiWalletServiceUnitTest, AddSuggestToken);
  FRIEND_TEST_ALL_PREFIXES(QoraiWalletServiceUnitTest, ImportFromMetaMask);
  FRIEND_TEST_ALL_PREFIXES(QoraiWalletServiceUnitTest, Reset);
  ...
  void OnQoraiWalletNftDiscoveryEnabled();
  ...
  mojo::RemoteSet<mojom::QoraiWalletServiceObserver> observers_;
  mojo::RemoteSet<mojom::QoraiWalletServiceTokenObserver> token_observers_;
  std::unique_ptr<QoraiWalletServiceDelegate> delegate_;
  ...
  std::unique_ptr<QoraiWalletP3A> qorai_wallet_p3a_;
  ...
  mojo::ReceiverSet<mojom::QoraiWalletService> receivers_;
  mojo::Receiver<qorai_wallet::mojom::KeyringServiceObserver>
      keyring_observer_receiver_{this};
  PrefChangeRegistrar pref_change_registrar_;
  base::WeakPtrFactory<QoraiWalletService> weak_ptr_factory_;
};

}  // namespace qorai_wallet

#endif  // QORAI_COMPONENTS_QORAI_WALLET_BROWSER_QORAI_WALLET_SERVICE_H_
