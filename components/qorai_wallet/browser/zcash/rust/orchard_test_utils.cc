...
#include "qorai/components/qorai_wallet/browser/zcash/rust/orchard_test_utils.h"
...
#include "qorai/components/qorai_wallet/browser/zcash/rust/orchard_decoded_blocks_bundle_impl.h"
...
class TestingDecodedBundleBuilderImpl : public TestingDecodedBundleBuilder {
 public:
  ...
  void SetPriorTreeState(::qorai_wallet::OrchardTreeState tree_state) override {
    prior_tree_state_ = std::move(tree_state);
  }

  void AddCommitment(::qorai_wallet::OrchardCommitment commitment) override {
 ...
 private:
  std::optional<::qorai_wallet::OrchardTreeState> prior_tree_state_;
