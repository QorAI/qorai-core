...
#include "qorai/components/qorai_wallet/browser/zcash/rust/cxx_orchard_shard_tree_delegate.h"
...
::qorai_wallet::OrchardShardAddress FromRust(
    const CxxOrchardShardAddress& addr) {
  return ::qorai_wallet::OrchardShardAddress{addr.level, addr.index};
}

CxxOrchardShardAddress ToRust(const ::qorai_wallet::OrchardShardAddress& addr) {
  return CxxOrchardShardAddress{addr.level, addr.index};
}

CxxOrchardShardTreeCap ToRust(
    ::qorai_wallet::OrchardShardTreeCap& shard_store_cap) {
...
::qorai_wallet::OrchardShardTreeCap FromRust(
...
::qorai_wallet::OrchardShard FromRust(const CxxOrchardShard& tree) {
  ...
  return ::qorai_wallet::OrchardShard(FromRust(tree.address), shard_root_hash,
                                      std::move(data));
}

CxxOrchardShard ToRust(const ::qorai_wallet::OrchardShard& tree) {
  ::rust::Vec<uint8_t> data;
...
CxxOrchardCheckpoint ToRust(
    const ::qorai_wallet::OrchardCheckpoint& checkpoint) {
...
::qorai_wallet::OrchardCheckpoint FromRust(
  ...
  return ::qorai_wallet::OrchardCheckpoint{
