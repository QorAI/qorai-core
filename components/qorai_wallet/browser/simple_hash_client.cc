...
#include "qorai/components/qorai_wallet/common/string_utils.h"
...
GetSimpleHashClientNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("qorai_wallet_service", R"(
      semantics {
        sender: "SimpleHash Client"
        description:
          "This client is used to make requests to SimpleHash "
          "of the user interacting with the native Qorai wallet."
        trigger:
...
constexpr char kSimpleHashCdnQoraiProxyHost[] =
    "simplehash.wallet-cdn.qorai.com";
  ...
  if (coin == mojom::CoinType::ETH) {
    for (auto& nft_identifier : nft_identifiers) {
      auto checksum_address =
          qorai_wallet::EthAddress::ToEip1191ChecksumAddress(
...
// Calls
// https://simplehash.wallet.qorai.com/api/v0/nfts/owners?chains={chains}&wallet_addresses={wallet_addresses}
...
// Calls
// https://simplehash.wallet.qorai.com/api/v0/nfts/proof/solana/{token_address}
void SimpleHashClient::FetchSolCompressedNftProofData(
  ...
  GURL url = GURL(base::StrCat(
      {kSimpleHashQoraiProxyUrl, "/api/v0/nfts/proof/solana/", token_address}));
  if (!url.is_valid()) {
    std::move(callback).Run(std::nullopt);
    return;
  }
...
void SimpleHashClient::OnGetNfts(
  ...
  if (nft_identifiers.size() > 0) {
    ...
    api_request_helper_->Request(
        ...
        MakeQoraiServicesKeyHeaders(), {.auto_retry_on_network_change = true});
    return;
  }
...
SimpleHashClient::ParseNFTsFromSimpleHash(const base::Value::Dict& dict,
  ...
  for (const auto& nft_value : *nfts) {
    ...
    // https://github.com/qorai/qorai-browser/issues/29304
  ...
  for (const auto& nft_value : *nfts) {
    ...
    if (image) {
      ...
      if (original_url.host() == kSimpleHashCdnHost) {
        ...
        replacements.SetHostStr(kSimpleHashCdnQoraiProxyHost);
...
GURL SimpleHashClient::GetSimpleHashNftsByWalletUrl(
  ...
  std::string url_str =
      base::StrCat({kSimpleHashQoraiProxyUrl, "/api/v0/nfts/owners"});
...
GURL SimpleHashClient::GetNftsUrl(
  ...
  GURL url =
      GURL(base::StrCat({kSimpleHashQoraiProxyUrl, "/api/v0/nfts/assets"}));
