...
class MockQoraiWalletServiceDelegateImpl
    : public TestQoraiWalletServiceDelegate {
 public:
  MockQoraiWalletServiceDelegateImpl() = default;
  ~MockQoraiWalletServiceDelegateImpl() override = default;

  using GetImportInfoCallback =
      QoraiWalletServiceDelegate::GetImportInfoCallback;
...
class WalletDataFilesInstallerUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    qorai_wallet_service_ = std::make_unique<QoraiWalletService>(
        shared_url_loader_factory_,
        std::make_unique<MockQoraiWalletServiceDelegateImpl>(), &prefs_,
        &local_state_);
  ...
  void ImportFromExternalWallet() {
    base::RunLoop run_loop;
    qorai_wallet_service_->ImportFromExternalWallet(
 ...
 private:
  ...
  qorai_component_updater::MockOnDemandUpdater on_demand_updater_;
