...
#ifndef QORAI_COMPONENTS_QORAI_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
#define QORAI_COMPONENTS_QORAI_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
...
class BlockchainRegistry : public mojom::BlockchainRegistry {
 ...
 private:
  ...
  std::vector<mojom::BlockchainTokenPtr>* GetTokenListFromChainId(
      const std::string& chain_id);
  std::vector<qorai_wallet::mojom::BlockchainTokenPtr> GetBuyTokens(
      const std::vector<mojom::OnRampProvider>& providers,
      const std::string& chain_id);
...
#endif  // QORAI_COMPONENTS_QORAI_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
