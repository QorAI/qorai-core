...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_api_impl.h"
...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_test_utils.h"
#include "qorai/components/qorai_wallet/browser/cardano/cardano_transaction_serializer.h"
...
class MockQoraiWalletProviderDelegate : public QoraiWalletProviderDelegate {
 public:
  MockQoraiWalletProviderDelegate() = default;
  ~MockQoraiWalletProviderDelegate() override {}
 ...
 public:
  ...
  void SetUp() override {
    ...
    qorai_wallet_service_ = std::make_unique<QoraiWalletService>(
        ...
        &prefs_, &local_state_);
    auto delegate =
        std::make_unique<testing::NiceMock<MockQoraiWalletProviderDelegate>>();
    ON_CALL(*delegate, GetOrigin).WillByDefault([&]() {
      return url::Origin::Create(GURL("https://qorai.com"));
    });
    provider_ = std::make_unique<CardanoApiImpl>(
        *qorai_wallet_service_, std::move(delegate),
    ...
    cardano_test_rpc_server_ = std::make_unique<CardanoTestRpcServer>(
        *(qorai_wallet_service_->GetCardanoWalletService()));
  }
  ...
  MockQoraiWalletProviderDelegate* delegate() {
    return static_cast<MockQoraiWalletProviderDelegate*>(
        provider()->delegate());
  }

  QoraiWalletService* qorai_wallet_service() {
    return qorai_wallet_service_.get();
  }
...
TEST_F(CardanoApiImplTest, SignData_Approved) {
  ...
  SignMessageRequestWaiter waiter(qorai_wallet_service());
...
TEST_F(CardanoApiImplTest, GetBalance) {
  ...
  test_rpc_service()->AddUtxo(qorai_wallet_service()
...
TEST_F(CardanoApiImplTest, SignTx_Declined) {
  ...
  SignCardanoTransactionRequestWaiter waiter(qorai_wallet_service());
...
TEST_F(CardanoApiImplTest, SignTx_DeclinedByPartialSignError) {
  ...
  {
    auto sign_result =
        qorai_wallet_service()->keyring_service()->SignMessageByCardanoKeyring(
            added_account->account_id,
            mojom::CardanoKeyId::New(mojom::CardanoKeyRole::kExternal, 0),
            tx_bytes);
...
TEST_F(CardanoApiImplTest, SignTx) {
  ...
  EXPECT_EQ(request->origin_info->origin_spec, "https://qorai.com");
