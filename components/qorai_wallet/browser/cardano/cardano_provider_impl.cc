...
CardanoProviderImpl::CardanoProviderImpl(
  ...
  qorai_wallet_service_->keyring_service()->AddObserver(
...
void CardanoProviderImpl::RequestCardanoPermissions(
  ...
  switch (state) {
    ...
    case PermissionCheckResult::kWalletLocked:
      ...
      qorai_wallet_service_->keyring_service()->RequestUnlock();
      delegate_->ShowPanel();
      return;
    ...
    case PermissionCheckResult::kNeedsPermissionRequest:
      auto cardano_account_ids = GetCardanoAccountPermissionIdentifiers(
          qorai_wallet_service_->keyring_service());
...
CardanoProviderImpl::EvaluatePermissionsState(
  ...
  if (!qorai_wallet_service_->keyring_service()->IsWalletCreatedSync()) {
  ...
  if (qorai_wallet_service_->keyring_service()->IsLockedSync()) {
...
QoraiWalletProviderDelegate* CardanoProviderImpl::delegate() {
