 ...
 public:
  ...
  void SetUp() override {
    ...
    qorai_wallet_service_ = std::make_unique<QoraiWalletService>(
        ...
        TestQoraiWalletServiceDelegate::Create(), &prefs_, &local_state_);
    provider_ = std::make_unique<CardanoProviderImpl>(
        *qorai_wallet_service_, base::BindLambdaForTesting([]() {
          std::unique_ptr<QoraiWalletProviderDelegate> result =
              std::make_unique<
                  testing::NiceMock<MockQoraiWalletProviderDelegate>>();
          return result;
        }));
  }
  ...
  CardanoWalletService* cardano_wallet_service() {
    return qorai_wallet_service_->GetCardanoWalletService();
  }
...
TEST_F(CardanoProviderImplUnitTest, Enable_PermissionApproved) {
  ...
  ON_CALL(*delegate(), RequestPermissions(_, _, _))
      .WillByDefault(
          [&](mojom::CoinType coin, const std::vector<std::string>& accounts,
              MockQoraiWalletProviderDelegate::RequestPermissionsCallback
                  callback) {
            EXPECT_EQ(coin, mojom::CoinType::ADA);
            EXPECT_EQ(accounts.size(), 1u);
            EXPECT_EQ(accounts[0], added_account->account_id->unique_key);
