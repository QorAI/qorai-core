...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_dapp_utils.h"
...
QoraiWalletProviderDelegate* CardanoApiImpl::delegate() {
...
void CardanoApiImpl::GetUsedAddresses(GetUsedAddressesCallback callback) {
  ...
  for (auto& address :
       qorai_wallet_service_->GetCardanoWalletService()->GetUsedAddresses(
           selected_account_)) {
...
void CardanoApiImpl::GetUnusedAddresses(GetUnusedAddressesCallback callback) {
  ...
  std::vector<std::string> result;
  for (auto& address :
       qorai_wallet_service_->GetCardanoWalletService()->GetUnusedAddresses(
           selected_account_)) {
    auto cardano_address = CardanoAddress::FromString(address->address_string);
    CHECK(cardano_address);
...
void CardanoApiImpl::GetChangeAddress(GetChangeAddressCallback callback) {
  ...
  auto address =
      qorai_wallet_service_->GetCardanoWalletService()->GetChangeAddress(
          selected_account_);
  if (!address) {
    std::move(callback).Run(
        std::nullopt,
...
void CardanoApiImpl::GetBalance(GetBalanceCallback callback) {
  ...
  qorai_wallet_service_->GetCardanoWalletService()->GetBalance(
...
void CardanoApiImpl::GetUtxos(const std::optional<std::string>& amount_cbor,
  ...
  qorai_wallet_service_->GetCardanoWalletService()->GetUtxos(
      selected_account_.Clone(),
...
void CardanoApiImpl::OnGetUtxosForSignTx(
  ...
  qorai_wallet_service_->AddSignCardanoTransactionRequest(
      std::move(request),
...
mojom::SignCardanoTransactionRequestPtr CardanoApiImpl::FromRestoredTransaction(
  ...
  auto addresses =
      qorai_wallet_service_->keyring_service()->GetCardanoAddresses(
          selected_account_);
  if (!addresses) {
    return nullptr;
  }
...
void CardanoApiImpl::OnSignTransactionRequestProcessed(
  ...
  for (const auto& input : tx.tx_body.inputs) {
    ...
    auto sign_result =
        qorai_wallet_service_->keyring_service()->SignMessageByCardanoKeyring(
...
void CardanoApiImpl::SignData(const std::string& address,
  ...
  auto supported_signing_address =
      qorai_wallet_service_->keyring_service()->GetCardanoAddress(
          selected_account_,
  ...
  auto request = mojom::SignMessageRequest::New(
      ...
      mojom::CoinType::ADA,
      qorai_wallet_service_->network_manager()->GetCurrentChainId(
...
void CardanoApiImpl::OnSignMessageRequestProcessed(
  ...
  auto sig_data = qorai_wallet_service_->keyring_service()
...
void CardanoApiImpl::SubmitTx(const std::string& signed_tx_cbor,
  ...
  auto* cardano_rpc =
      qorai_wallet_service_->GetCardanoWalletService()->GetCardanoRpc(
          GetNetworkForCardanoAccount(selected_account_));
...
CardanoApiImpl::CheckSelectedAccountValid() {
  auto account_id = GetCardanoPreferredDappAccount(
      delegate(), qorai_wallet_service_->keyring_service());
  if (!account_id) {
