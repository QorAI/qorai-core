...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_rpc.h"
...
#include "qorai/components/qorai_wallet/browser/cardano/cardano_rpc_blockfrost_api.h"
...
base::flat_map<std::string, std::string> MakeCardanoRpcHeaders(
  ...
  base::flat_map<std::string, std::string> request_headers =
      qorai_wallet::IsEndpointUsingQoraiWalletProxy(request_url)
          ? qorai_wallet::MakeQoraiServicesKeyHeaders()
          : base::flat_map<std::string, std::string>();

  std::string cardano_project_id =
      base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
          chain_id == qorai_wallet::mojom::kCardanoMainnet
              ? qorai_wallet::switches::kCardanoMainnetProjectId
              : qorai_wallet::switches::kCardanoTestnetProjectId);
...
bool ShouldThrottleEndpoint(const GURL& request_url) {
  // Don't throttle requests if host matches qorai proxy.
  return !qorai_wallet::IsEndpointUsingQoraiWalletProxy(request_url);
}

template <class TCallback>
void ReplyWithInvalidJsonError(TCallback callback) {
  std::move(callback).Run(
      base::unexpected(qorai_wallet::WalletParsingErrorMessage()));
}

template <class TCallback>
void ReplyWithInternalError(TCallback callback) {
  std::move(callback).Run(
      base::unexpected(qorai_wallet::WalletInternalErrorMessage()));
}
...
std::optional<std::string> HandleGetUtxoListRawResponse(
  ...
  return qorai_wallet::ConvertAllNumbersToString("", raw_response);
}

}  // namespace

namespace qorai_wallet::cardano_rpc {
