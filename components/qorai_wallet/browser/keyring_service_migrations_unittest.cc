...
namespace {
constexpr char kPassword[] = "qorai123!";

// "qorai.wallet.keyrings" pref for 1.43.93 before encryptor's pbkdf2 iterations
...
// "qorai.wallet.keyrings" pref for 1.66.95 before encrypted mnemonic was
...
class KeyringServiceMigrationsLegacyIterationsUnitTest
 ...
 public:
  void SetupLegacyKeyringPrefs() {
    GetPrefs()->SetDict(kQoraiWalletKeyrings,
                        ParseJsonDict(kLegacyKeyringsDict143));

    qorai_wallet::MigrateObsoleteProfilePrefs(GetPrefs());
  }

  void ValidatePrefs() {
    EXPECT_EQ(GetPrefs()->GetDict(kQoraiWalletKeyrings),
              ParseJsonDict(kUpgradedKeyringsDict143));
    EXPECT_TRUE(GetPrefs()->GetBoolean(kQoraiWalletMnemonicBackedUp));
    EXPECT_EQ(GetPrefs()->GetString(kQoraiWalletEncryptorSalt),
              "AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE=");
    EXPECT_EQ(GetPrefs()->GetDict(kQoraiWalletMnemonic), ParseJsonDict(R"(
    {
    ...
    EXPECT_TRUE(
        GetPrefs()->GetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated));
  }
...
TEST_F(KeyringServiceMigrationsLegacyIterationsUnitTest, NoMigration) {
  ...
  EXPECT_FALSE(GetPrefs()->GetBoolean(kQoraiWalletMnemonicBackedUp));
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kQoraiWalletKeyringEncryptionKeysMigrated));
...
TEST_F(KeyringServiceMigrationsLegacyIterationsUnitTest,
  ...
  EXPECT_EQ(GetPrefs()->GetDict(kQoraiWalletMnemonic), base::Value::Dict());
