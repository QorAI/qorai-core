...
#include "qorai/components/qorai_wallet/browser/eip1559_transaction.h"
#include "qorai/components/qorai_wallet/browser/eth_transaction.h"
#include "qorai/components/qorai_wallet/browser/eth_tx_meta.h"
...
#include "qorai/components/qorai_wallet/browser/solana_tx_meta.h"
...
class SimulationServiceUnitTest : public testing::Test {
 public:
  SimulationServiceUnitTest()
    ...
    qorai_wallet_service_ = std::make_unique<QoraiWalletService>(
        ...
        &prefs_, &local_state_);
    network_manager_ = qorai_wallet_service_->network_manager();
    ...
    simulation_service_ = std::make_unique<SimulationService>(
        shared_url_loader_factory_, qorai_wallet_service_.get());
  ...
  AccountUtils GetAccountUtils() {
    return AccountUtils(qorai_wallet_service_->keyring_service());
  }
 ...
 protected:
  ...
  base::test::ScopedFeatureList feature_list_{
      features::kQoraiWalletTransactionSimulationsFeature};
...
TEST_F(SimulationServiceUnitTest, GetScanTransactionURL) {
  ...
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/ethereum/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSepoliaChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/ethereum/v0/sepolia/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kPolygonMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/polygon/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kBnbSmartChainMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/bnb/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kArbitrumMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/arbitrum/v0/one/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kBaseMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/base/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaMainnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/solana/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaDevnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/solana/v0/devnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaTestnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.qorai.com/solana/v0/testnet/scan/"
            "transactions?language=en-US");
}
...
TEST_F(SimulationServiceUnitTest, ScanEVMTransactionUnsupportedNetwork) {
  ...
  EXPECT_CALL(
      ...
      Run(EqualsMojo(mojom::EVMSimulationResponsePtr()), "",
          l10n_util::GetStringUTF8(IDS_QORAI_WALLET_UNSUPPORTED_NETWORK)));
