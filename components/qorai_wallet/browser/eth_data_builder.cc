...
bool Transfer(std::string_view to_address,
  ...
  if (!qorai_wallet::PadHexEncodedParameter(to_address, &padded_address)) {
    return false;
  }
...
bool BalanceOf(std::string_view address, std::string* data) {
  ...
  std::string params;
  if (!qorai_wallet::PadHexEncodedParameter(address, &params)) {
    return false;
  }
  return qorai_wallet::ConcatHexStrings(function_hash, params, data);
}
...
bool Approve(std::string_view spender_address,
  ...
  std::string padded_address;
  if (!qorai_wallet::PadHexEncodedParameter(spender_address, &padded_address)) {
    return false;
  }
  std::string padded_amount;
  if (!PadHexEncodedParameter(Uint256ValueToHex(amount), &padded_amount)) {
...
bool Allowance(std::string_view owner_address,
  ...
  if (!qorai_wallet::PadHexEncodedParameter(owner_address,
                                            &padded_owner_address)) {
    return false;
  }
  std::string padded_spender_address;
  if (!qorai_wallet::PadHexEncodedParameter(spender_address,
...
bool TransferFromOrSafeTransferFrom(bool is_safe_transfer_from,
  ...
  if (!qorai_wallet::PadHexEncodedParameter(from, &padded_from)) {
    return false;
  }

  std::string padded_to;
  if (!qorai_wallet::PadHexEncodedParameter(to, &padded_to)) {
    return false;
  }
...
bool OwnerOf(uint256_t token_id, std::string* data) {
  ...
  return qorai_wallet::ConcatHexStrings(function_hash, padded_token_id, data);
}
...
bool BalanceOf(std::string_view owner_address,
  ...
  std::string padded_address;
  if (!qorai_wallet::PadHexEncodedParameter(owner_address, &padded_address)) {
    return false;
  }
  std::string padded_token_id;
  if (!PadHexEncodedParameter(Uint256ValueToHex(token_id), &padded_token_id)) {
...
std::optional<std::string> TokensBalance(
  ...
  for (const auto& contract_address : contract_addresses) {
    ...
    if (!qorai_wallet::PadHexEncodedParameter(contract_address,
