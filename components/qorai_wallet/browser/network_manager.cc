...
// TODO(https://github.com/qorai/qorai-browser/issues/46940): Adding these
...
std::optional<GURL> GetURLForKnownChainId(std::string_view chain_id) {
  // TODO(https://github.com/qorai/qorai-browser/issues/46940): kChainSubdomains
  ...
  return GURL(absl::StrFormat("https://%s.wallet.qorai.com", *subdomain));
}
...
GURL ZCashMainnetRpcUrl() {
  ...
  return GURL("https://zcash.wallet.qorai.com/");
}
...
const base::Value::List* GetCustomNetworksList(PrefService* prefs,
                                               mojom::CoinType coin) {
  const auto& custom_networks = prefs->GetDict(kQoraiWalletCustomNetworks);
...
std::string GetCurrentChainIdFromPrefs(PrefService* prefs,
                                       mojom::CoinType coin) {
  const auto& selected_networks = prefs->GetDict(kQoraiWalletSelectedNetworks);
...
mojom::NetworkInfoPtr NetworkManager::GetCustomChain(std::string_view chain_id,
  ...
  for (const auto& it : *custom_list) {
    if (auto opt_chain_id =
            qorai_wallet::ExtractChainIdFromValue(it.GetIfDict())) {
      if (base::CompareCaseInsensitiveASCII(chain_id, *opt_chain_id) == 0) {
        return qorai_wallet::ValueToNetworkInfo(it);
      }
    }
  }
  return nullptr;
...
bool NetworkManager::IsEip1559Chain(std::string_view chain_id) {
  ...
  if (auto is_eip_1559 = prefs_->GetDict(kQoraiWalletEip1559CustomChains)
...
void NetworkManager::SetEip1559ForCustomChain(std::string_view chain_id,
  ...
  ScopedDictPrefUpdate update(prefs_, kQoraiWalletEip1559CustomChains);
...
void NetworkManager::AddCustomNetwork(const mojom::NetworkInfo& chain) {
  ScopedDictPrefUpdate update(prefs_, kQoraiWalletCustomNetworks);
...
std::vector<std::string> NetworkManager::GetHiddenNetworks(
  ...
  const auto& hidden_networks = prefs_->GetDict(kQoraiWalletHiddenNetworks);
...
void NetworkManager::AddHiddenNetwork(mojom::CoinType coin,
  ...
  ScopedDictPrefUpdate update(prefs_, kQoraiWalletHiddenNetworks);
...
bool NetworkManager::SetCurrentChainId(mojom::CoinType coin,
  ...
  if (!origin) {
    ScopedDictPrefUpdate update(prefs_, kQoraiWalletSelectedNetworks);
    ...
    if (origin->scheme() == url::kHttpScheme ||
      ...
      ScopedDictPrefUpdate update(prefs_,
                                  kQoraiWalletSelectedNetworksPerOrigin);
