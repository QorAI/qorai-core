...
// TODO(https://github.com/qorai/qorai-browser/issues/47669) this file should be
// in content/browser subfolder of a layered qorai_wallet component.
...
void ExtractAddresses(permissions::RequestType type,
  ...
  std::string match;
  re2::RE2* regex;
  if (type == permissions::RequestType::kQoraiEthereum) {
    regex = kEthAddrRegex.get();
  } else if (type == permissions::RequestType::kQoraiCardano) {
...
bool ParseRequestingOriginInternal(permissions::RequestType type,
                                   ...
                                   std::string* account,
                                   std::queue<std::string>* address_queue) {
  if (origin.opaque() || (type != permissions::RequestType::kQoraiEthereum &&
                          type != permissions::RequestType::kQoraiSolana &&
                          type != permissions::RequestType::kQoraiCardano)) {
    return false;
  }
...
std::optional<blink::PermissionType> CoinTypeToPermissionType(
  ...
  switch (coin_type) {
    case mojom::CoinType::ETH:
      return blink::PermissionType::QORAI_ETHEREUM;
    case mojom::CoinType::SOL:
      return blink::PermissionType::QORAI_SOLANA;
    case mojom::CoinType::ADA:
      return blink::PermissionType::QORAI_CARDANO;
    default:
      return std::nullopt;
  }
}
...
std::optional<permissions::RequestType> CoinTypeToPermissionRequestType(
  ...
  switch (coin_type) {
    case mojom::CoinType::ETH:
      return permissions::RequestType::kQoraiEthereum;
    case mojom::CoinType::SOL:
      return permissions::RequestType::kQoraiSolana;
    case mojom::CoinType::ADA:
      return permissions::RequestType::kQoraiCardano;
    default:
      return std::nullopt;
  }
}
