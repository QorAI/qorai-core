...
#include "qorai/components/qorai_wallet/renderer/js_cardano_wallet_api.h"
...
void JSCardanoProvider::Install(content::RenderFrame* render_frame) {
  // TODO(https://github.com/qorai/qorai-browser/issues/46369): Add proxy object
  // handler script
  CHECK(render_frame);
  v8::Isolate* isolate =
  ...
  JSCardanoProvider* cardano_qorai_provider =
      cppgc::MakeGarbageCollected<JSCardanoProvider>(
          isolate->GetCppHeap()->GetAllocationHandle(), render_frame);

  v8::Local<v8::Object> cardano_qorai_provider_object =
      cardano_qorai_provider->GetWrapper(isolate).ToLocalChecked();
