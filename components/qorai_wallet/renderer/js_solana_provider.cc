...
constexpr char kQoraiSolana[] = "qoraiSolana";
...
constexpr char kWalletStandardOnDemandScript[] = R"((function () {
  window.addEventListener('wallet-standard:app-ready', (e) => {
    window.qoraiSolana.walletStandardInit()
  })
})())";
...
void JSSolanaProvider::Install(bool allow_overwrite_window_solana,
  ...
  // check window.qoraiSolana existence
  v8::Local<v8::Object> global = context->Global();
  v8::Local<v8::Value> qorai_solana_value =
      global->Get(context, gin::StringToV8(isolate, kQoraiSolana))
          .ToLocalChecked();
  if (!qorai_solana_value->IsUndefined()) {
    return;
  }
  ...
  // exposed via window.qoraiSolana and window.solana.
  ...
  SetProviderNonWritable(context, global, solana_proxy,
                         gin::StringToV8(isolate, kQoraiSolana), true);

  // window.solana will be removed in the future, we use window.qoraiSolana
...
gin::ObjectTemplateBuilder JSSolanaProvider::GetObjectTemplateBuilder(
  ...
  return gin::Wrappable<JSSolanaProvider>::GetObjectTemplateBuilder(isolate)
      ...
      .SetProperty("isQoraiWallet", &JSSolanaProvider::GetIsQoraiWallet)
...
bool JSSolanaProvider::GetIsQoraiWallet(gin::Arguments* arguments) {
  return true;
}
...
void JSSolanaProvider::WalletStandardInit(gin::Arguments* arguments) {
  ...
  std::string wallet_standard_module_str = base::StrCat(
      {"(function() {", LoadDataResource(IDR_QORAI_WALLET_STANDARD_JS),
       "return walletStandardQorai; })()"});
  ...
  v8::Isolate* isolate = arguments->isolate();
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  if (!GetProperty(context, context->Global(), kQoraiSolana).ToLocal(&object)) {
    return;
  }
...
void JSSolanaProvider::FireEvent(
  ...
  CallMethodOfObject(render_frame()->GetWebFrame(), kQoraiSolana, kEmit,
                     std::move(args));
}
...
bool JSSolanaProvider::LoadSolanaWeb3ModuleIfNeeded(v8::Isolate* isolate) {
  ...
  std::string solana_web3_module_str = base::StrCat(
      {"(function() {", LoadDataResource(IDR_QORAI_WALLET_SOLANA_WEB3_JS),
       "return solanaWeb3; })()"});
