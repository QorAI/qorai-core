...
#include "qorai/components/qorai_wallet/common/web3_provider_constants.h"
#include "qorai/components/qorai_wallet/renderer/resource_helper.h"
#include "qorai/components/qorai_wallet/renderer/v8_helper.h"
...
constexpr char kQoraiEthereum[] = "qoraiEthereum";
constexpr char kEthereum[] = "ethereum";
constexpr char kEmit[] = "emit";
constexpr char kIsQoraiWallet[] = "isQoraiWallet";
...
void JSEthereumProvider::Install(bool install_ethereum_provider,
  ...
  // Check window.qoraiEthereum existence.
  v8::Local<v8::Object> global = context->Global();
  v8::Local<v8::Value> qorai_ethereum_value =
      global->Get(context, gin::StringToV8(isolate, kQoraiEthereum))
          .ToLocalChecked();
  if (!qorai_ethereum_value->IsUndefined()) {
    return;
  }
  ...
  // Set window.qoraiEthereum
  SetProviderNonWritable(context, global, ethereum_proxy,
                         gin::StringToV8(isolate, kQoraiEthereum), true);
  ...
  // https://github.com/qorai/qorai-browser/issues/22213;
  SetOwnPropertyWritable(context, provider_object,
...
bool JSEthereumProvider::GetIsQoraiWallet() {
  return true;
}
...
gin::ObjectTemplateBuilder JSEthereumProvider::GetObjectTemplateBuilder(
  ...
  return gin::Wrappable<JSEthereumProvider>::GetObjectTemplateBuilder(isolate)
      .SetProperty(kIsQoraiWallet, &JSEthereumProvider::GetIsQoraiWallet)
...
void JSEthereumProvider::FireEvent(const std::string& event,
  ...
  CallMethodOfObject(render_frame()->GetWebFrame(), kQoraiEthereum, kEmit,
                     std::move(args));
}
...
void JSEthereumProvider::AnnounceProvider() {
  ...
  auto provider =
      GetProperty(context, context->Global(), kQoraiEthereum).ToLocalChecked();
...
const std::string& JSEthereumProvider::GetQoraiWalletImage() {
  if (!qorai_wallet_image_) {
    qorai_wallet_image_ =
        "data:image/png;base64," +
        base::Base64Encode(LoadDataResource(IDR_QORAI_WALLET_PROVIDER_ICON));
  }
  return qorai_wallet_image_.value();
}
