...
void PageMetrics::ReportPagesLoaded() {
  ...
  if (profile_prefs_->GetBoolean(qorai_rewards::prefs::kEnabled)) {
    if (profile_prefs_->GetString(qorai_rewards::prefs::kExternalWalletType)
            .empty()) {
...
void PageMetrics::ReportFailedHTTPSUpgrades() {
  qorai_shields::ControlType https_upgrade_settings =
      qorai_shields::GetHttpsUpgradeControlType(host_content_settings_map_,
                                                GURL());
  if (https_upgrade_settings == qorai_shields::ControlType::ALLOW) {
    if (local_state_->HasPrefPath(
            kMiscMetricsFailedHTTPSUpgradeMetricAddedTime)) {
...
void PageMetrics::OnQoraiQuery() {
  UMA_HISTOGRAM_BOOLEAN(kSearchQoraiDailyHistogramName, true);
}
