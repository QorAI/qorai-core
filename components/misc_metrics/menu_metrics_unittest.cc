...
TEST_F(MenuMetricsUnitTest, MostFrequent) {
  ...
  for (size_t i = 0; i < 2; i++) {
    menu_metrics_->RecordMenuGroupAction(MenuGroup::kQoraiFeatures);
  }
  ...
  // "qorai features" is the most frequent with a max count of 4
  ...
  // "qorai features" is the most frequent with a max count of 6
  histogram_tester_.ExpectBucketCount(kFrequentMenuGroupHistogramName, 2, 1);
