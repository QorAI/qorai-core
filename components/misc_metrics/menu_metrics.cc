...
constexpr char kQoraiFeaturesPrefKey[] = "qorai_features";
...
const char* GetMenuGroupPrefKey(MenuGroup group) {
  switch (group) {
    ...
    case MenuGroup::kQoraiFeatures:
      return kQoraiFeaturesPrefKey;
...
void MenuMetrics::RecordMenuGroupAction(MenuGroup group) {
  ...
  if (current_max_group_.has_value() && current_max_group_->first == group) {
    ...
    } else if (result->first == kQoraiFeaturesPrefKey) {
      histogram_value = 1;
      current_max_group_ = std::make_pair(MenuGroup::kQoraiFeatures, 1);
