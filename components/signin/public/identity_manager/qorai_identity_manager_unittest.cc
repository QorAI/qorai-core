...
class QoraiIdentityManagerTest : public testing::Test {
 protected:
  QoraiIdentityManagerTest()
  ...
  ~QoraiIdentityManagerTest() override {
    gaia_cookie_manager_service_ = nullptr;
    identity_manager_->Shutdown();
    signin_client_.Shutdown();
  }

  QoraiIdentityManagerTest(const QoraiIdentityManagerTest&) = delete;
  QoraiIdentityManagerTest& operator=(const QoraiIdentityManagerTest&) = delete;
...
TEST_F(QoraiIdentityManagerTest, GetAccountsInCookieJarWithNoAccounts) {
...
TEST_F(QoraiIdentityManagerTest, GetAccountsInCookieJarWithOneAccount) {
  base::RunLoop run_loop;
  identity_manager_observer()->SetOnAccountsInCookieUpdatedCallback(
      run_loop.QuitClosure());
...
TEST_F(QoraiIdentityManagerTest, GetAccountsInCookieJarWithTwoAccounts) {
  base::RunLoop run_loop;
  identity_manager_observer()->SetOnAccountsInCookieUpdatedCallback(
      run_loop.QuitClosure());
