...
#include "qorai/components/qorai_search/browser/backup_results_allowed_urls.h"
#include "qorai/components/qorai_search/browser/backup_results_service.h"
#include "qorai/components/web_discovery/browser/pref_names.h"
#include "qorai/components/web_discovery/browser/request_queue.h"
#include "qorai/components/web_discovery/browser/util.h"
...
constexpr net::NetworkTrafficAnnotationTag kFetchNetworkTrafficAnnotation =
    ...
    semantics {
      sender: "QorAI Web Discovery Double Fetch"
      description:
    ...
    policy {
      cookies_allowed: NO
      setting:
        "Users can opt-in or out via qorai://settings/search"
    })");
...
void DoubleFetcher::OnFetchTimer(const base::Value& request_data) {
  ...
  if (qorai_search::IsBackupResultURLAllowed(url) &&
...
void DoubleFetcher::OnRenderedResponse(
    const GURL& url,
    std::optional<qorai_search::BackupResultsService::BackupResults> results) {
  if (results) {
