...
class QoraiSyncPrefsTest : public testing::Test {
 protected:
  QoraiSyncPrefsTest() {
    qorai_sync::Prefs::RegisterProfilePrefs(pref_service_.registry());
    qorai_sync_prefs_ = std::make_unique<qorai_sync::Prefs>(&pref_service_);
#if BUILDFLAG(IS_ANDROID)
  ...
  qorai_sync::Prefs* qorai_sync_prefs() { return qorai_sync_prefs_.get(); }

  PrefService* pref_service() { return &pref_service_; }

  base::test::SingleThreadTaskEnvironment task_environment_;
  TestingPrefServiceSimple pref_service_;
  std::unique_ptr<qorai_sync::Prefs> qorai_sync_prefs_;
...
// locked keyring
TEST_F(QoraiSyncPrefsTest, ValidPassphraseKeyringLocked) {
  OSCryptMocker::SetUp();

  qorai_sync_prefs()->SetSeed(kValidSyncCode);
...
TEST_F(QoraiSyncPrefsTest, FailedToDecryptQoraiSeedValue) {
  OSCryptMocker::SetUp();
...
using QoraiSyncPrefsDeathTest = QoraiSyncPrefsTest;
...
#endif
TEST_F(QoraiSyncPrefsDeathTest, MAYBE_GetSeedOutNullptrCHECK) {
  EXPECT_CHECK_DEATH(qorai_sync_prefs()->GetSeed(nullptr));
}

TEST_F(QoraiSyncPrefsTest, LeaveChainDetailsMaxLenIOS) {
  ...
  qorai_sync_prefs()->AddLeaveChainDetail("", 0, details.c_str());
  details = qorai_sync_prefs()->GetLeaveChainDetails();
  EXPECT_LE(details.size(), max_len);
  EXPECT_GE(details.size(), 1u);

  details.assign(max_len + 1, 'a');
  qorai_sync_prefs()->AddLeaveChainDetail(__FILE__, __LINE__, details.c_str());
  details = qorai_sync_prefs()->GetLeaveChainDetails();
...
TEST_F(QoraiSyncPrefsTest, PasswordSyncAllowedExplicitValue) {
