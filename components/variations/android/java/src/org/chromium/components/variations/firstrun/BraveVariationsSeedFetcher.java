...
public class QorAIVariationsSeedFetcher extends VariationsSeedFetcher {
    ...
    private static final String TAG = "QorAIVariations";

    public static VariationsSeedFetcher get() {
        synchronized (sLock) {
            if (sInstance == null) {
                sInstance = new QorAIVariationsSeedFetcher();
            }
            return sInstance;
        }
    }
    ...
    @VisibleForTesting
    public QorAIVariationsSeedFetcher() {
        super();
    }
    ...
    protected String getConnectionString(SeedFetchParameters params) {
        ...
        // Replace Chromium URL with QorAI URL.
        if (urlString.indexOf(DEFAULT_VARIATIONS_SERVER_URL) != -1) {
            urlString = urlString.replaceFirst(
                    DEFAULT_VARIATIONS_SERVER_URL, QorAIVariationsConfig.VARIATIONS_SERVER_URL);
            ...
            urlString = urlString.replaceFirst(DEFAULT_FAST_VARIATIONS_SERVER_URL,
                    QorAIVariationsConfig.VARIATIONS_SERVER_URL);
        } else {
