...
// A GitHub workflow in the qorai/qorai-variations repository generates the test
// seed and uploads it to a URL with the following template, where $1 is the
// pull request number.
constexpr char kVariationsPrTestSeedUrlTemplate[] =
    "https://griffin.qorai.com/pull/$1/seed";

}  // namespace

void AppendQorAICommandLineOptions(base::CommandLine& command_line) {
  std::string variations_server_url = BUILDFLAG(QORAI_VARIATIONS_SERVER_URL);
