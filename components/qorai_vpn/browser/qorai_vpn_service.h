...
#ifndef QORAI_COMPONENTS_QORAI_VPN_BROWSER_QORAI_VPN_SERVICE_H_
#define QORAI_COMPONENTS_QORAI_VPN_BROWSER_QORAI_VPN_SERVICE_H_
...
class QoraiVpnService :
#if !BUILDFLAG(IS_ANDROID)
    public QoraiVPNConnectionManager::Observer,
#endif
    public mojom::ServiceHandler,
    public KeyedService,
    public QoraiVpnMetrics::Delegate {
 public:
  QoraiVpnService(
      ...
      base::RepeatingCallback<mojo::PendingRemote<skus::mojom::SkusService>()>
          skus_service_getter);
  ~QoraiVpnService() override;

  QoraiVpnService(const QoraiVpnService&) = delete;
  QoraiVpnService& operator=(const QoraiVpnService&) = delete;

#if BUILDFLAG(IS_ANDROID)
  mojo::PendingRemote<qorai_vpn::mojom::ServiceHandler> MakeRemote();
#endif  // BUILDFLAG(IS_ANDROID)
  ...
  void set_delegate(std::unique_ptr<QoraiVPNServiceDelegate> delegate) {
  ...
  QoraiVpnMetrics* qorai_vpn_metrics() { return &qorai_vpn_metrics_; }
 ...
 private:
  ...
  // QoraiVPNConnectionManager::Observer overrides:
...
#if !BUILDFLAG(IS_ANDROID)
  base::ScopedObservation<QoraiVPNConnectionManager,
                          QoraiVPNConnectionManager::Observer>
      observed_{this};
  bool wait_region_data_ready_ = false;
  raw_ptr<QoraiVPNConnectionManager, DanglingUntriaged> connection_manager_ =
      nullptr;
  ...
  std::unique_ptr<QoraiVpnAPIRequest> api_request_;
  std::unique_ptr<QoraiVPNServiceDelegate> delegate_;
  base::OneShotTimer subs_cred_refresh_timer_;
  QoraiVpnMetrics qorai_vpn_metrics_;
  base::WeakPtrFactory<QoraiVpnService> weak_ptr_factory_{this};
};

}  // namespace qorai_vpn

#endif  // QORAI_COMPONENTS_QORAI_VPN_BROWSER_QORAI_VPN_SERVICE_H_
