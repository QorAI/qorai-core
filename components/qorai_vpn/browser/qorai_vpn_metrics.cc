...
QoraiVpnMetrics::QoraiVpnMetrics(
    ...
    : local_state_(local_state),
      ...
      widget_usage_storage_(local_state_,
                            prefs::kQoraiVPNWidgetUsageWeeklyStorage),
      connected_minutes_storage_(
          local_state_,
          prefs::kQoraiVPNConnectedMinutesWeeklyStorage) {
  pref_change_registrar_.Init(profile_prefs);
  pref_change_registrar_.Add(
      kNewTabPageShowQoraiVPN,
      base::BindRepeating(&QoraiVpnMetrics::HandleShowWidgetChange,
...
QoraiVpnMetrics::~QoraiVpnMetrics() = default;

void QoraiVpnMetrics::RecordAllMetrics(bool new_usage) {
  if (new_usage) {
    p3a_utils::RecordFeatureUsage(local_state_, prefs::kQoraiVPNFirstUseTime,
                                  prefs::kQoraiVPNLastUseTime);
  }
  p3a_utils::RecordFeatureNewUserReturning(
      local_state_, prefs::kQoraiVPNFirstUseTime, prefs::kQoraiVPNLastUseTime,
      prefs::kQoraiVPNUsedSecondDay, kNewUserReturningHistogramName);
  p3a_utils::RecordFeatureDaysInMonthUsed(
      local_state_, new_usage, prefs::kQoraiVPNLastUseTime,
      prefs::kQoraiVPNDaysInMonthUsed, kDaysInMonthUsedHistogramName);
  p3a_utils::RecordFeatureLastUsageTimeMetric(
      local_state_, prefs::kQoraiVPNLastUseTime, kLastUsageTimeHistogramName);
  RecordWidgetUsage(false);
  report_timer_.Start(FROM_HERE,
                      base::Time::Now() + base::Hours(kP3AIntervalHours),
                      base::BindOnce(&QoraiVpnMetrics::RecordAllMetrics,
                                     base::Unretained(this), false));
}

#if BUILDFLAG(IS_ANDROID)
void QoraiVpnMetrics::RecordAndroidBackgroundP3A(int64_t session_start_time_ms,
  ...
  for (base::Time day = session_start_time; day <= session_end_time;
    ...
    p3a_utils::RecordFeatureUsage(local_state_, prefs::kQoraiVPNFirstUseTime,
                                  prefs::kQoraiVPNLastUseTime, day);
    p3a_utils::RecordFeatureNewUserReturning(
        local_state_, prefs::kQoraiVPNFirstUseTime, prefs::kQoraiVPNLastUseTime,
        prefs::kQoraiVPNUsedSecondDay, kNewUserReturningHistogramName,
        is_last_day);
    p3a_utils::RecordFeatureDaysInMonthUsed(
        local_state_, day, prefs::kQoraiVPNLastUseTime,
        prefs::kQoraiVPNDaysInMonthUsed, kDaysInMonthUsedHistogramName,
        is_last_day);
  }
  p3a_utils::RecordFeatureLastUsageTimeMetric(
...
void QoraiVpnMetrics::RecordWidgetUsage(bool new_usage) {
  if (new_usage) {
...
void QoraiVpnMetrics::HandleShowWidgetChange() {
  if (profile_prefs_->GetBoolean(kNewTabPageShowQoraiVPN)) {
    return;
  }
  UMA_HISTOGRAM_BOOLEAN(kHideWidgetHistogramName, true);
}

void QoraiVpnMetrics::RecordVPNConnectedInterval() {
  connected_minutes_storage_.AddDelta(kConnectionReportInterval.InMinutes());
}

void QoraiVpnMetrics::ReportVPNConnectedDuration() {
  connection_report_timer_.Start(
      FROM_HERE, base::Time::Now() + kConnectionReportInterval,
      base::BindOnce(&QoraiVpnMetrics::ReportVPNConnectedDuration,
                     base::Unretained(this)));
