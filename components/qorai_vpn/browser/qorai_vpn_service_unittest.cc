...
#include "qorai/components/qorai_vpn/browser/api/qorai_vpn_api_request.h"
...
#include "qorai/components/skus/browser/pref_names.h"
#include "qorai/components/skus/browser/skus_context_impl.h"
#include "qorai/components/skus/browser/skus_service_impl.h"
...
#include "qorai/components/qorai_vpn/browser/connection/ikev2/connection_api_impl_sim.h"
#endif
...
class TestQoraiVPNServiceObserver : public QoraiVPNServiceObserver {
 public:
  TestQoraiVPNServiceObserver() = default;
...
class QoraiVPNServiceTest : public testing::Test {
 public:
  QoraiVPNServiceTest()
      : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME) {
    scoped_feature_list_.InitWithFeatures(
        {skus::features::kSkusFeature, features::kQoraiVPN}, {});
  }
  ...
  void SetUp() override {
    ...
    qorai_vpn::RegisterProfilePrefs(profile_pref_service_.registry());
    shared_url_loader_factory_ =
        base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(
            &url_loader_factory_);
    url_loader_factory_.SetInterceptor(base::BindRepeating(
        &QoraiVPNServiceTest::Interceptor, base::Unretained(this)));
  ...
  void ResetVpnService() {
    ...
    service_ = std::make_unique<QoraiVpnService>(
#if !BUILDFLAG(IS_ANDROID)
        ...
        base::BindRepeating(&QoraiVPNServiceTest::GetSkusService,
  ...
  void BlockVPNByPolicy(bool value) {
    profile_pref_service_.SetManagedPref(prefs::kManagedQoraiVPNDisabled,
                                         base::Value(value));
    EXPECT_EQ(qorai_vpn::IsQoraiVPNDisabledByPolicy(&profile_pref_service_),
              value);
  }
  ...
  const std::vector<mojom::RegionPtr>& regions() const {
    return GetQoraiVPNConnectionManager()->GetRegionDataManager().GetRegions();
  }

  void SetSelectedRegion(const std::string& region) {
    GetQoraiVPNConnectionManager()->SetSelectedRegion(region);
  }

  std::string GetSelectedRegion() {
    return GetQoraiVPNConnectionManager()
  ...
  void OnFetchRegionList(const std::string& region_list, bool success) {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().OnFetchRegionList(
        region_list, success);
  }

  void OnFetchTimezones(const std::string& timezones_list, bool success) {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().OnFetchTimezones(
        timezones_list, success);
  }

  QoraiVPNConnectionManager* GetQoraiVPNConnectionManager() const {
  ...
  void SetDeviceRegion(const std::string& name) {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().SetDeviceRegion(
        name);
  }

  void SetFallbackDeviceRegion() {
    GetQoraiVPNConnectionManager()
        ->GetRegionDataManager()
        .SetFallbackDeviceRegion();
  }

  void SetTestTimezone(const std::string& timezone) {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().test_timezone_ =
        timezone;
  }
  ...
  void SetAndExpectPurchasedStateChange(TestQoraiVPNServiceObserver* observer,
  ...
  void GetAllRegions(QoraiVpnService::ResponseCallback callback) {
    service_->api_request_->GetServerRegions(
        std::move(callback), qorai_vpn::mojom::kRegionPrecisionCityByCountry);
  }
  ...
  std::unique_ptr<QoraiVpnService> service_;
  data_decoder::test::InProcessDataDecoder in_process_data_decoder_;
  network::TestURLLoaderFactory url_loader_factory_;
  scoped_refptr<network::SharedURLLoaderFactory> shared_url_loader_factory_;
};

TEST_F(QoraiVPNServiceTest, ResponseSanitizingTest) {
...
#if !BUILDFLAG(IS_ANDROID)
TEST_F(QoraiVPNServiceTest, SkusCredentialCacheTest) {
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateSessionExpiredTest) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  ...
  auto session_expired_time =
      local_pref_service_.GetTime(prefs::kQoraiVPNSessionExpiredDate);
  ...
  // starts.
  SetPurchasedState(env, PurchasedState::LOADING);
  local_pref_service_.SetTime(prefs::kQoraiVPNSessionExpiredDate,
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateOutOfCredentialsTest) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);

  // Set an expiry in the future - this would be when the last redeemed
  // credential (whether it was successful  or not) expires.
  local_pref_service_.SetTime(prefs::kQoraiVPNLastCredentialExpiry,
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateTest) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  // Service try loading
  SetPurchasedState(env, PurchasedState::LOADING);
...
TEST_F(QoraiVPNServiceTest, ConnectionStateUpdateWithPurchasedStateTest) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
  std::string env = skus::GetDefaultEnvironment();
  SetPurchasedState(env, PurchasedState::PURCHASED);
...
TEST_F(QoraiVPNServiceTest, IsConnectedWithPurchasedStateTest) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
  std::string env = skus::GetDefaultEnvironment();
  SetPurchasedState(env, PurchasedState::PURCHASED);
  ...
  // Check QoraiVpnService gives false for IsConnected() when
...
TEST_F(QoraiVPNServiceTest, DisconnectedIfDisabledByPolicy) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
  std::string env = skus::GetDefaultEnvironment();
  SetPurchasedState(env, PurchasedState::PURCHASED);
...
TEST_F(QoraiVPNServiceTest, SelectedRegionChangedUpdateTest) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(QoraiVPNServiceTest, ClearSelectedRegionTest) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(QoraiVPNServiceTest, SelectedRegionChangedUpdateWithDeviceRegionTest) {
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(QoraiVPNServiceTest, GetProperRegionFromTimeZoneTest) {
  ...
  // data is fetched.
  local_pref_service_.ClearPref(prefs::kQoraiVPNSelectedRegionV2);
  local_pref_service_.SetInteger(prefs::kQoraiVPNRegionListVersion, 2);
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateForAnotherEnvFailed) {
...
TEST_F(QoraiVPNServiceTest, CheckInitialPurchasedStateTest) {
...
TEST_F(QoraiVPNServiceTest, SubscribedCredentialsWithTokenNoLongerValid) {
  std::string env = skus::GetDefaultEnvironment();
...
TEST_F(QoraiVPNServiceTest, CheckConnectionStateAfterPurchased) {
  std::string env = skus::GetDefaultEnvironment();
...
TEST_F(QoraiVPNServiceTest, GetPurchasedInfoSync) {
  std::string env = skus::GetDefaultEnvironment();
...
TEST_F(QoraiVPNServiceTest, SetPurchasedState) {
  std::string env = skus::GetDefaultEnvironment();
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
  EXPECT_EQ(PurchasedState::NOT_PURCHASED, GetPurchasedInfoSync());
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateNotifications) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  TestQoraiVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(QoraiVPNServiceTest, LoadPurchasedStateForAnotherEnv) {
  auto development = SetupTestingStoreForEnv(skus::GetDefaultEnvironment());
  EXPECT_EQ(skus::GetEnvironmentForDomain(development),
            skus::GetDefaultEnvironment());
  TestQoraiVPNServiceObserver observer;
