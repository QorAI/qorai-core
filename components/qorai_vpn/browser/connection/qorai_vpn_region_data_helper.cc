...
mojom::RegionPtr GetRegionFromValueWithoutCity(const base::Value::Dict& value) {
  ...
  if (auto* name = value.FindString(qorai_vpn::kRegionNameKey)) {
    region->name = *name;
  }
  if (auto* name_pretty = value.FindString(qorai_vpn::kRegionNamePrettyKey)) {
    region->name_pretty = *name_pretty;
  }
  if (auto* country = value.FindString(qorai_vpn::kRegionCountryKey)) {
    region->country = *country;
  }
  if (auto* continent = value.FindString(qorai_vpn::kRegionContinentKey)) {
    region->continent = *continent;
  }
  if (auto* country_iso_code =
          value.FindString(qorai_vpn::kRegionCountryIsoCodeKey)) {
    region->country_iso_code = *country_iso_code;
  }
  if (auto* region_precision =
          value.FindString(qorai_vpn::kRegionPrecisionKey)) {
    region->region_precision = *region_precision;
  }
  if (auto latitude = value.FindDouble(qorai_vpn::kRegionLatitudeKey)) {
    region->latitude = *latitude;
  }
  if (auto longitude = value.FindDouble(qorai_vpn::kRegionLongitudeKey)) {
    region->longitude = *longitude;
  }
  if (auto server_count = value.FindInt(qorai_vpn::kRegionServerCountKey)) {
    region->server_count = *server_count;
  }

  if (auto* smart_routing_proxy_state =
          value.FindString(qorai_vpn::kRegionSmartRoutingProxyStateKey)) {
