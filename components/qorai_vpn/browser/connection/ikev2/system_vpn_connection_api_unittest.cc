...
constexpr char kProfileCredentialData[] = R"(
        {
          "eap-username": "qorai-user",
          "eap-password": "qorai-pwd"
        }
      )";
constexpr char kHostNamesTestData[] = R"([
        {
          "hostname": "host-1.qorai.com",
          "display-name": "host-1",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-2.qorai.com",
          "display-name": "host-2",
          "offline": false,
          "capacity-score": 1
        },
        {
          "hostname": "host-3.qorai.com",
          "display-name": "Singapore",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-4.qorai.com",
          "display-name": "host-4",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-5.qorai.com",
          "display-name": "host-5",
          "offline": false,
          "capacity-score": 1
        }
...
class SystemVPNConnectionAPIUnitTest : public testing::Test {
 public:
  ...
  void LoadCachedRegionData() {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().
        LoadCachedRegionData();
  }

  void ClearRegions() {
    GetQoraiVPNConnectionManager()->GetRegionDataManager().regions_.clear();
  }
  ...
  mojom::RegionPtr device_region() {
    const auto device_region_name = GetQoraiVPNConnectionManager()
                                        ->GetRegionDataManager()
                                        .GetDeviceRegion();
  ...
  QoraiVPNConnectionInfo connection_info() {
...
TEST_F(SystemVPNConnectionAPIUnitTest, CreateOSVPNEntryWithInvalidInfoTest) {
  ...
  auto* test_api = GetConnectionAPI();
  test_api->CheckConnection();
  local_state()->SetString(prefs::kQoraiVPNSelectedRegionV2, "eu-be");
  // Prepare valid connection info.
  test_api->OnFetchHostnames("eu-be", kHostNamesTestData, true);
  test_api->SetPreventCreationForTesting(true);
  test_api->OnGetProfileCredentials(kProfileCredentialData, true);
...
TEST_F(SystemVPNConnectionAPIUnitTest, HostnamesTest) {
  ...
  EXPECT_EQ("host-2.qorai.com", test_api->hostname_->hostname);
