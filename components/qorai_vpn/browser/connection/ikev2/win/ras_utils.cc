...
#include "qorai/base/process/process_launcher.h"
...
RasOperationResult DisconnectEntry(const std::wstring& entry_name) {
  ...
  if (connection_result == CheckConnectionResult::DISCONNECTING) {
    VLOG(2) << __func__
            << " Don't try to disconnect while qorai vpn entry is already in "
               "disconnecting state";
...
RasOperationResult SetConnectionParamsUsingPowerShell(
  ...
  auto result = qorai::ProcessLauncher::ReadAppOutput(power_shell, options, 10);
  if (!result.has_value()) {
...
RasOperationResult CreateEntry(const QoraiVPNConnectionInfo& info) {
  ...
  if (connection_result == CheckConnectionResult::CONNECTING ||
      connection_result == CheckConnectionResult::CONNECTED) {
    VLOG(2) << __func__
            << " Don't try to create entry when qorai vpn entry is in "
  ...
  // `eventvwr`:
  // >> The user DESKTOP - DRCJVG6\brian dialed a connection named QORAIVPN
