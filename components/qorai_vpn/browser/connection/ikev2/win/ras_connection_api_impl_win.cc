...
using qorai_vpn::ras::RasOperationResult;
...
RasOperationResult ConnectEntry(const std::wstring& name) {
  return qorai_vpn::ras::ConnectEntry(name);
}

RasOperationResult DisconnectEntry(const std::wstring& name) {
  return qorai_vpn::ras::DisconnectEntry(name);
}
...
void RasConnectionAPIImplWin::OnRasConnectionStateChanged() {
  ...
  // Check connection state for QoraiVPN entry again when connected or
