...
const NSString* kQoraiVPNKey = @"QoraiVPNKey";
...
NSData* GetPasswordRefForAccount() {
  ...
  NSDictionary* query = @{
    ...
    (__bridge id)kSecAttrAccount : kQoraiVPNKey,
...
NEVPNProtocolIKEv2* CreateProtocolConfig(const QoraiVPNConnectionInfo& info) {
...
void IKEv2ConnectionAPIImplMac::CreateVPNConnectionImpl(
    const QoraiVPNConnectionInfo& info) {
  ...
  [vpn_manager loadFromPreferencesWithCompletionHandler:^(NSError* error) {
    ...
    switch (current_status) {
      case NEVPNStatusReasserting:
        ...
        // https://github.com/qorai/qorai-browser/issues/29500#issuecomment-1989762108
...
bool IKEv2ConnectionAPIImplMac::IsOnDemandEnabled() const {
  return manager_->local_prefs()->GetBoolean(prefs::kQoraiVPNOnDemandEnabled);
}
