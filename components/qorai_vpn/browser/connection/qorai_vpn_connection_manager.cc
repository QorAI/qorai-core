...
QoraiVPNConnectionManager::QoraiVPNConnectionManager(
  ...
  wireguard_enabled_.Init(
      prefs::kQoraiVPNWireguardEnabled, local_prefs_,
      base::BindRepeating(&QoraiVPNConnectionManager::UpdateConnectionAPIImpl,
  ...
  region_data_manager_.set_selected_region_changed_callback(base::BindRepeating(
      &QoraiVPNConnectionManager::NotifySelectedRegionChanged,
      base::Unretained(this)));
  region_data_manager_.set_region_data_ready_callback(
      base::BindRepeating(&QoraiVPNConnectionManager::NotifyRegionDataReady,
                          base::Unretained(this)));
}

QoraiVPNConnectionManager::~QoraiVPNConnectionManager() = default;

QoraiVPNRegionDataManager& QoraiVPNConnectionManager::GetRegionDataManager() {
  return region_data_manager_;
}

void QoraiVPNConnectionManager::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void QoraiVPNConnectionManager::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void QoraiVPNConnectionManager::NotifyRegionDataReady(bool ready) const {
  for (auto& obs : observers_) {
    obs.OnRegionDataReady(ready);
  }
}

void QoraiVPNConnectionManager::NotifySelectedRegionChanged(
...
void QoraiVPNConnectionManager::UpdateConnectionAPIImpl() {
...
mojom::ConnectionState QoraiVPNConnectionManager::GetConnectionState() const {
  if (connection_api_impl_) {
...
void QoraiVPNConnectionManager::ResetConnectionState() {
  if (connection_api_impl_) {
    connection_api_impl_->ResetConnectionState();
  }
}

void QoraiVPNConnectionManager::Connect() {
...
void QoraiVPNConnectionManager::Disconnect() {
  if (connection_api_impl_) {
    connection_api_impl_->Disconnect();
  }
}

void QoraiVPNConnectionManager::CheckConnection() {
  if (connection_api_impl_) {
    connection_api_impl_->CheckConnection();
  }
}

void QoraiVPNConnectionManager::SetSelectedRegion(const std::string& name) {
...
std::string QoraiVPNConnectionManager::GetHostname() const {
  if (connection_api_impl_) {
...
std::string QoraiVPNConnectionManager::GetLastConnectionError() const {
  if (connection_api_impl_) {
...
void QoraiVPNConnectionManager::ToggleConnection() {
  if (connection_api_impl_) {
    connection_api_impl_->ToggleConnection();
  }
}

std::string QoraiVPNConnectionManager::GetCurrentEnvironment() const {
  return local_prefs_->GetString(prefs::kQoraiVPNEnvironment);
}

void QoraiVPNConnectionManager::MaybeInstallSystemServices() {
  ...
  // This API could be called more than once because QoraiVpnService is a
  ...
  base::ThreadPool::CreateCOMSTATaskRunner({base::MayBlock()})
      ->PostTaskAndReplyWithResult(
          ...
          base::BindOnce(
              &QoraiVPNConnectionManager::OnInstallSystemServicesCompleted,
              weak_factory_.GetWeakPtr()));
#endif
}

void QoraiVPNConnectionManager::OnInstallSystemServicesCompleted(bool success) {
...
bool QoraiVPNConnectionManager::ScheduleConnectRequestIfNeeded() {
  if (!install_in_progress_) {
    return false;
  }

  system_service_installed_event_.Post(
      FROM_HERE, base::BindOnce(&QoraiVPNConnectionManager::Connect,
                                weak_factory_.GetWeakPtr()));
  return true;
}

void QoraiVPNConnectionManager::NotifyConnectionStateChanged(
