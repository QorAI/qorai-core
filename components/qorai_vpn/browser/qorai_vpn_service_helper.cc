...
void SetSubscriberCredential(PrefService* local_prefs,
  ...
  local_prefs->SetDict(prefs::kQoraiVPNSubscriberCredential,
                       std::move(cred_dict));
}

void ClearSubscriberCredential(PrefService* local_prefs) {
  local_prefs->ClearPref(prefs::kQoraiVPNSubscriberCredential);
}
