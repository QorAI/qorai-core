...
#include "qorai/components/qorai_vpn/browser/api/vpn_response_parser.h"
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("qorai_vpn_service", R"(
      semantics {
        sender: "Qorai VPN Service"
        description:
          "This service is used to communicate with Guardian VPN apis"
          "on behalf of the user interacting with the Qorai VPN."
        trigger:
          "Triggered by user connecting the Qorai VPN."
        data:
          "Servers, hosts and credentials for Qorai VPN"
        destination: WEBSITE
      }
      policy {
        cookies_allowed: NO
...
QoraiVpnAPIRequest::QoraiVpnAPIRequest(
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory)
    : api_request_helper_(GetNetworkTrafficAnnotationTag(),
                          url_loader_factory) {}

QoraiVpnAPIRequest::~QoraiVpnAPIRequest() = default;

void QoraiVpnAPIRequest::GetServerRegions(ResponseCallback callback,
                                          const std::string& region_precision) {
  auto internal_callback =
      base::BindOnce(&QoraiVpnAPIRequest::OnGetResponse,
...
void QoraiVpnAPIRequest::GetTimezonesForRegions(ResponseCallback callback) {
  auto internal_callback =
      base::BindOnce(&QoraiVpnAPIRequest::OnGetResponse,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback));
...
void QoraiVpnAPIRequest::GetHostnamesForRegion(
...
void QoraiVpnAPIRequest::GetProfileCredentials(
    ResponseCallback callback,
...
void QoraiVpnAPIRequest::GetWireguardProfileCredentials(
    ResponseCallback callback,
    const std::string& subscriber_credential,
...
void QoraiVpnAPIRequest::VerifyCredentials(
    ResponseCallback callback,
    const std::string& hostname,
...
void QoraiVpnAPIRequest::InvalidateCredentials(
    ResponseCallback callback,
    const std::string& hostname,
    const std::string& client_id,
    const std::string& subscriber_credential,
...
void QoraiVpnAPIRequest::VerifyPurchaseToken(ResponseCallback callback,
...
void QoraiVpnAPIRequest::GetSubscriberCredential(
    ResponseCallback callback,
    const std::string& product_type,
    const std::string& product_id,
    ...
    const std::string& bundle_id) {
  auto internal_callback =
      base::BindOnce(&QoraiVpnAPIRequest::OnGetSubscriberCredential,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback));
...
void QoraiVpnAPIRequest::GetSubscriberCredentialV12(
    ResponseCallback callback,
  ...
  base::Value::Dict dict;
  dict.Set("validation-method", "qorai-premium");
  dict.Set("qorai-vpn-premium-monthly-pass", skus_credential);
  std::string request_body = CreateJSONRequestBody(dict);
  OAuthRequest(base_url, "POST", request_body, std::move(internal_callback),
               {{"Qorai-Payments-Environment", environment}});
}

void QoraiVpnAPIRequest::CreateSupportTicket(
    ResponseCallback callback,
    ...
    const std::string& subscriber_credential) {
  auto internal_callback =
      base::BindOnce(&QoraiVpnAPIRequest::OnCreateSupportTicket,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback));
...
void QoraiVpnAPIRequest::OAuthRequest(
    const GURL& url,
...
void QoraiVpnAPIRequest::OnGetResponse(
    ResponseCallback callback,
...
void QoraiVpnAPIRequest::OnGetSubscriberCredential(
    ResponseCallback callback,
...
void QoraiVpnAPIRequest::OnCreateSupportTicket(
    ResponseCallback callback,
    APIRequestResult api_request_result) {
  bool success = api_request_result.response_code() == 200;
