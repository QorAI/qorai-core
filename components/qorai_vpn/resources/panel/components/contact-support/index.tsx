...
import getPanelBrowserAPI, * as QoraiVPN from '../../api/panel_browser_api'
...
const SUBJECT_OPTIONS = new Map([
  ['otherConnectionProblems',
    getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_OTHER_CONNECTION_PROBLEM)],
  ['noInternet',
    getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_NO_INTERNET)],
  ['slowConnection',
    getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_SLOW_CONNECTION)],
  ['websiteProblems',
    getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_WEBSITE_DOESNT_WORK)],
  ['other',
    getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_OTHER)]
])
...
function ContactSupport(props: Props) {
  const [supportData, setSupportData] = React.useState<QoraiVPN.SupportData>()
  ...
  const handlePrivacyPolicyClick = () => {
    getPanelBrowserAPI().panelHandler.openVpnUI(QoraiVPN.ManageURLType.PRIVACY)
  }
  ...
  if (showErrors) {
    if (formData.contactEmail && !emailAddressIsValid) {
      emailAddressErrorMessage = getLocale(S.QORAI_VPN_SUPPORT_EMAIL_NOT_VALID)
    } else {
      emailAddressErrorMessage = getLocale(S.QORAI_VPN_SUPPORT_FIELD_IS_REQUIRED)
    }
  }
  ...
  return (
    <Styles.Box>
      <Styles.PanelContent>
        <PanelHeader
          title={getLocale(S.QORAI_VPN_CONTACT_SUPPORT)}
          buttonAriaLabel={getLocale(S.QORAI_VPN_SUPPORT_PANEL_BACK_BUTTON_ARIA_LABEL)}
          onClick={() => props.onCloseContactSupport()}
        />
        <Styles.TopContent>
          <Styles.Form onSubmit={(e) => e.preventDefault()}>
            <Input
              placeholder={getLocale(S.QORAI_VPN_SUPPORT_EMAIL_PLACEHOLDER)}
              showErrors={emailAddressErrorMessage.length !== 0}
              value={formData.contactEmail ?? ''}
              onChange={handleInputOnChange}
            >
              <Styles.StyledLabel>{getLocale(S.QORAI_VPN_SUPPORT_EMAIL)}</Styles.StyledLabel>
            ...
            >
              <Styles.StyledDropdownPlaceholder slot="placeholder">
                {getLocale(S.QORAI_VPN_SUPPORT_SUBJECT_NOTSET)}
              </Styles.StyledDropdownPlaceholder>
              <Styles.StyledLabel slot="label">
                {getLocale(S.QORAI_VPN_SUPPORT_SUBJECT)}
              </Styles.StyledLabel>
              {[...SUBJECT_OPTIONS.keys()].map((key) => {
                return (
                  <qora-option class='option' key={key} value={key}>
            ...
            >
              <Styles.StyledLabel>{getLocale(S.QORAI_VPN_SUPPORT_BODY)}</Styles.StyledLabel>
              <textarea
                placeholder={getLocale(S.QORAI_VPN_SUPPORT_BODY_PLACEHOLDER)}
                value={formData.problemBody}
                onChange={getOnChangeField('problemBody')}
              />
              {(showErrors && formData.problemBody?.length === 0) && (
                <Styles.ErrorLabel>
                  {getLocale(S.QORAI_VPN_SUPPORT_FIELD_IS_REQUIRED)}
                </Styles.ErrorLabel>
              )}
            </Styles.TextareaWrapper>
            <Styles.OptionalValues>
              <Styles.SectionDescription>
                {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_HEADER)}
              </Styles.SectionDescription>
              <Styles.Notes>
                <p>
                  {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_NOTES)}{' '}
                  <a
                    href='#'
                    onClick={handlePrivacyPolicyClick}
                  >
                    {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_NOTES_PRIVACY_POLICY)}
                  </a>
                  .
                </p>
              </Styles.Notes>
              <Styles.OptionalValuesFields>
                ...
                >
                  <div className={'optionalValueTitle'}>
                    <span className={'optionalValueTitleKey'}>
                      {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_VPN_HOSTNAME)}
                    </span>{' '}
                ...
                >
                  <div className={'optionalValueTitle'}>
                    <span className={'optionalValueTitleKey'}>
                      {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_APP_VERSION)}
                    </span>{' '}
                ...
                >
                  <div className={'optionalValueTitle'}>
                    <span className={'optionalValueTitleKey'}>
                      {getLocale(S.QORAI_VPN_SUPPORT_OPTIONAL_OS_VERSION)}
                    </span>{' '}
                ...
                <Styles.OptionalValueLabel>
                  <div className={'optionalValueTitle'}>
                    <span className={'optionalValueTitleKey'}>
                      {getLocale(S.QORAI_VPN_SUPPORT_TIMEZONE)}
                    </span>{' '}
            ...
            <Styles.SupportNotes>
              {getLocale(S.QORAI_VPN_SUPPORT_NOTES)}
            </Styles.SupportNotes>
            {isRemoteSubmissionError && (
              <Styles.ErrorLabel>
                {getLocale(S.QORAI_VPN_SUPPORT_TICKET_FAILED)}
              </Styles.ErrorLabel>
            )}
            ...
            >
              {getLocale(S.QORAI_VPN_SUPPORT_SUBMIT)}
            </Styles.StyledSubmitButton>
          </Styles.Form>
        </Styles.TopContent>
      </Styles.PanelContent>
