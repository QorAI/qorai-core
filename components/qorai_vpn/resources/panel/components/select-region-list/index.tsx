...
function getCountryInfo(numCity: number, numServer: number) {
  const city =
    numCity === 1
      ? getLocale(S.QORAI_VPN_SERVER_SELECTION_SINGLE_CITY)
      : getLocale(S.QORAI_VPN_SERVER_SELECTION_MULTIPLE_CITIES).replace(
          '$1',
          `${numCity}`
        )
...
function getCityInfo(numServer: number) {
  return numServer === 1
    ? getLocale(S.QORAI_VPN_SERVER_SELECTION_SINGLE_SERVER)
    : getLocale(S.QORAI_VPN_SERVER_SELECTION_MULTIPLE_SERVERS).replace(
        '$1',
        `${numServer}`
      )
}
  ...
  return (
    ...
    >
          ...
          <RegionLabelBox>
            ...
            {smartProxyRoutingEnabled && props.region.smartRoutingProxyState === 'all' && (
              <Tooltip mode='mini'>
                ...
                <div slot='content'>
                  {getLocale(S.QORAI_VPN_SMART_PROXY_ROUTING_ICON_TOOLTIP)}
                </div>
              </Tooltip>)
            }
          </RegionLabelBox>
      ...
      {props.open && (
        <>
          <RegionCity
            cityLabel={getLocale(S.QORAI_VPN_SERVER_SELECTION_OPTIMAL_LABEL)}
            serverInfo={getLocale(S.QORAI_VPN_SERVER_SELECTION_OPTIMAL_DESC)}
            selected={
      ...
      <Styles.RegionCountry>
        ...
        <Styles.CountryInfo>
          <Styles.RegionCountryLabel selected={currentRegion.isAutomatic}>
            {getLocale(S.QORAI_VPN_SERVER_SELECTION_AUTOMATIC_LABEL)}
          </Styles.RegionCountryLabel>
          <Styles.CountryServerInfo selected={currentRegion.isAutomatic}>
            {getLocale(S.QORAI_VPN_SERVER_SELECTION_OPTIMAL_DESC)}
          </Styles.CountryServerInfo>
        </Styles.CountryInfo>
...
function SelectRegion() {
  ...
  return (
    <Styles.Box>
      <Styles.PanelContent>
        <PanelHeader
          title={getLocale(S.QORAI_VPN_SELECT_YOUR_SERVER)}
          buttonAriaLabel={getLocale(S.QORAI_VPN_SETTINGS_PANEL_BACK_BUTTON_ARIA_LABEL)}
