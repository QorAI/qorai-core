...
import ToggleButton from '@qorai/qora/react/toggle'
...
const getStatusText = (status: ConnectionState) => {
  if (status === ConnectionState.CONNECTED) {
    return getLocale(S.QORAI_VPN_CONNECTED)
  }

  if (status === ConnectionState.CONNECTING) {
    return getLocale(S.QORAI_VPN_CONNECTING)
  }

  if (status === ConnectionState.DISCONNECTING) {
    return getLocale(S.QORAI_VPN_DISCONNECTING)
  }

  if (status === ConnectionState.DISCONNECTED) {
    return getLocale(S.QORAI_VPN_DISCONNECTED)
  }
  ...
  if (status === ConnectionState.CONNECT_FAILED) {
    return getLocale(S.QORAI_VPN_CONNECTION_FAILED)
  }
