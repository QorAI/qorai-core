...
function SessionExpiredContent() {
  ...
  return (
    <span>
      {formatLocale('qoraiVpnSessionExpiredContent', {
        $1: (content) => (
          <a
            href='#'
...
function MainPanel() {
  ...
  const regionServerLabel =
    currentRegion.regionPrecision === REGION_PRECISION_COUNTRY
      ? getLocale(S.QORAI_VPN_SERVER_SELECTION_OPTIMAL_LABEL)
      : currentRegion.namePretty
  return (
    <PanelBox>
      <PanelHeader
        title={getLocale(S.QORAI_VPN)}
        settingsTooltip={getLocale(S.QORAI_VPN_MAIN_PANEL_VPN_SETTINGS_TITLE)}
      ...
      <Styles.PanelContent>
        ...
        {connectionStatus === ConnectionState.CONNECT_NOT_ALLOWED && (
          <Styles.StyledAlert
            ...
            hideIcon
          >
            {getLocale(S.QORAI_VPN_CONNECT_NOT_ALLOWED)}
          </Styles.StyledAlert>
        )}
        {expired && (
          <Styles.StyledAlert
            ...
            hideIcon
          >
            <div slot='title'>{getLocale(S.QORAI_VPN_MAIN_PANEL_SESSION_EXPIRED_PART_TITLE)}</div>
        ...
        {outOfCredentials && (
          <Styles.StyledAlert
            ...
            hideIcon
          >
            <div slot='title'>{getLocale(S.QORAI_VPN_MAIN_PANEL_OUT_OF_CREDENTIALS_TITLE)}</div>
