...
const qoraiExtensionId = 'mnojpmjdmbbfmejpflffifhffcmidifd'
...
export function send<U= void, T= void> (messageType: string, payload?: T): Promise<U> {
  ...
  console.debug(`Sending data to qorai extension for ${messageType}`, { messageType, payload })
  return new Promise(function (resolve) {
    chrome.runtime.sendMessage(qoraiExtensionId, { messageType, payload }, function (responseData: U) {
      console.debug(`got response from qorai extension for "${messageType}"`, responseData)
