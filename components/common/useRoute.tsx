...
(['pushState', 'replaceState'] as const).forEach(method => {
  ...
  window.history[method] = (state, title, url) => {
    ...
    window.dispatchEvent(new CustomEvent('qorai-history-changed', {
      detail: {
        state,
        title,
        url
...
export const useLocation = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('qorai-history-changed', handler)
    window.addEventListener('popstate', handler)
    return () => {
      window.removeEventListener('qorai-history-changed', handler)
