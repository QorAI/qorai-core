...
#if BUILDFLAG(IS_WIN)
constexpr const char kComponentName[] = "QorAI Pluggable Transports (Windows)";
...
constexpr const char kComponentName[] = "QorAI Pluggable Transports (Mac)";
constexpr const char kTorPluggableTransportComponentId[] =
...
constexpr const char kComponentName[] = "QorAI Pluggable Transports (Linux)";
constexpr const char kTorPluggableTransportComponentId[] =
...
constexpr const char kSnowflakeExecutableName[] = "tor-snowflake-qorai";
constexpr const char kObfs4ExecutableName[] = "tor-obfs4-qorai";
...
QorAITorPluggableTransportUpdater::QorAITorPluggableTransportUpdater(
...
void QorAITorPluggableTransportUpdater::Register() {
  if (registered_)
    return;

  QorAIComponent::Register(kComponentName, kTorPluggableTransportComponentId,
                           kComponentBase64PublicKey);
  registered_ = true;
  is_ready_ = false;
}

void QorAITorPluggableTransportUpdater::Unregister() {
  registered_ = false;
  is_ready_ = false;
}

void QorAITorPluggableTransportUpdater::Cleanup() {
...
bool QorAITorPluggableTransportUpdater::IsReady() const {
  return is_ready_;
}

const base::FilePath&
QorAITorPluggableTransportUpdater::GetSnowflakeExecutable() const {
  return snowflake_path_;
}

const base::FilePath& QorAITorPluggableTransportUpdater::GetObfs4Executable()
    const {
  return obfs4_path_;
}

void QorAITorPluggableTransportUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void QorAITorPluggableTransportUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void QorAITorPluggableTransportUpdater::OnComponentReady(
    const std::string& component_id,
    const base::FilePath& install_dir,
    const std::string& manifest) {
  GetTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&Initialize, install_dir),
      base::BindOnce(&QorAITorPluggableTransportUpdater::OnInitialized,
                     weak_ptr_factory_.GetWeakPtr(), install_dir));
}

void QorAITorPluggableTransportUpdater::OnInitialized(
    const base::FilePath& install_dir,
    bool success) {
  if (success) {
