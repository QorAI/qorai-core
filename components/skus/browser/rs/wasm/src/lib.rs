...
extern "C" {
    ...
    #[wasm_bindgen(structural, method, getter, js_name = qoraiSkus)]
    pub fn qoraiSkus(this: &Chrome) -> Option<QoraiSkus>;

    #[wasm_bindgen(extends = :: js_sys :: Object)]
    pub type QoraiSkus;

    #[wasm_bindgen(method)]
    pub fn refresh_order(this: &QoraiSkus, order_id: &str) -> js_sys::Promise;

    #[wasm_bindgen(method)]
    pub fn fetch_order_credentials(this: &QoraiSkus, order_id: &str) -> js_sys::Promise;

    #[wasm_bindgen(method)]
    pub fn credential_summary(this: &QoraiSkus, subdomain: Option<&str>) -> js_sys::Promise;
}
    ...
    pub fn refresh_order(&self, order_id: String) -> js_sys::Promise {
        ...
        let future = async move {
            ...
            if let Some(chrome) = window.chrome() {
                if let Some(skus) = chrome.qoraiSkus() {
