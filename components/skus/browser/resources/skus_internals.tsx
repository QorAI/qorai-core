...
import * as SkusInternalsMojo from 'gen/qorai/components/skus/common/skus_internals.mojom.m.js'
...
function App() {
  ...
  const [leoState, setQoraState] = React.useState({})
  ...
  const getQoraState = () => {
    API.getQoraState().then((r: any) => {
      setQoraState(JSON.parse(r.response))
    })
  }
  ...
  return (
    <Container>
      ...
      <ButtonContainer>
        <button onClick={() => {
            if (window.confirm('Resetting SKUs state will count the next sign in for account.qorai.com as a new device. If you do this more than once you may run out of devices/credentials. Are you sure you want to reset SKUs state?')) {
              resetSkusState()
            }
          }}>
          Reset SKUs state
        ...
        <button onClick={getQoraState}>Fetch Qora state</button>
