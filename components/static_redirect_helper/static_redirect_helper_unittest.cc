...
TEST_F(StaticRedirectHelperUnitTest, FaviconServiceMatch) {
  GURL old_url = GURL(
      ...
      "size=256&fallback_opts=TYPE,SIZE,URL&url=https://search.qorai.com/");
  GURL new_url;

  qorai::StaticRedirectHelper(old_url, &new_url);

  EXPECT_EQ(
      new_url.spec(),
      "https://favicons.proxy.qorai.com/"
      "faviconV2?client=chrome&nfrp=2&check_seen=true&size=32&min_size=16&max_"
      "size=256&fallback_opts=TYPE,SIZE,URL&url=https://search.qorai.com/");
}
...
TEST_F(StaticRedirectHelperUnitTest, FaviconServiceMatchNoParams) {
  ...
  EXPECT_EQ(new_url.spec(), "https://favicons.proxy.qorai.com/faviconV2");
}
