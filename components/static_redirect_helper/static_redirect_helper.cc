...
#include "qorai/components/safebrowsing/buildflags.h"
...
void StaticRedirectHelper(const GURL& request_url, GURL* new_url) {
  ...
  // https://github.com/qorai/qorai-browser/issues/10314
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kQorAISafeBrowsingSslProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kQorAISafeBrowsing2Proxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
  ...
  if (crxDownload_pattern->MatchesURL(request_url)) {
    ...
    replacements.SetHostStr("crxdownload.qorai.com");
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
  ...
  if (autofill_pattern->MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kQorAIStaticProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }

  if (favicon_pattern->MatchesURL(request_url)) {
    replacements.SetHostStr("favicons.proxy.qorai.com");
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
  ...
  if (crlSet_pattern1->MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr("redirector.qorai.com");
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
  ...
  if (gvt1_pattern->MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kQorAIRedirectorProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return;
  }
