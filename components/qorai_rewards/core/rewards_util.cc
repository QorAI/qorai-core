...
#if BUILDFLAG(IS_ANDROID)
  if (!base::FeatureList::IsEnabled(features::kQoraiRewards)) {
    return true;
  }
#endif  // BUILDFLAG(IS_ANDROID)
  return false;
...
bool IsOFACSanctionedRegion(const std::string& country_code) {
  return qorai_l10n::IsISOCountryCodeOFACSanctioned(country_code);
}

const std::string GetCountryCode() {
  return qorai_l10n::GetDefaultISOCountryCodeString();
}
