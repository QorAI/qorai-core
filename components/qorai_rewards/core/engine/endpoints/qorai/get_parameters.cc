...
Result ParseBody(RewardsEngine& engine, const std::string& body) {
  ...
  const auto rate = dict.FindDouble("qorRate");
  if (!rate) {
    engine.LogError(FROM_HERE) << "Failed to parse body";
    return base::unexpected(Error::kFailedToParseBody);
  }
  ...
  const auto* vqor_deadline = dict.FindString("vqorDeadline");
  if (vqor_deadline) {
    if (base::Time time;
        base::Time::FromUTCString(vqor_deadline->c_str(), &time)) {
      parameters->vqor_deadline = time;
    }
  }

  const auto vqor_expired = dict.FindBool("vqorExpired");
  if (vqor_expired) {
