...
namespace qorai_rewards::internal::wallet {
...
mojom::RewardsWalletPtr Wallet::GetWallet(bool* corrupted) {
  ...
  *corrupted = false;
  ...
  const auto& json =
      engine_->Get<RewardsPrefs>().GetString(prefs::kWalletQorai);
  if (json.empty()) {
    return nullptr;
  }
  ...
  if (!value) {
    engine_->LogError(FROM_HERE) << "Parsing of qorai wallet failed";
    *corrupted = true;
    return nullptr;
  }
...
bool Wallet::SetWallet(mojom::RewardsWalletPtr wallet) {
  ...
  engine_->Get<RewardsPrefs>().SetString(prefs::kWalletQorai, json);
