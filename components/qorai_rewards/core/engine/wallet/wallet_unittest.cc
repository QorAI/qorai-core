...
TEST_F(RewardsWalletTest, GetWallet) {
  ...
  engine().Get<RewardsPrefs>().SetString(prefs::kWalletQorai, "");
  corrupted = true;
  ...
  engine().Get<RewardsPrefs>().SetString(prefs::kWalletQorai, "BAD-DATA");
  wallet = engine().wallet()->GetWallet(&corrupted);
  EXPECT_FALSE(wallet);
  EXPECT_TRUE(corrupted);
  EXPECT_EQ(engine().Get<RewardsPrefs>().GetString(prefs::kWalletQorai),
            "BAD-DATA");
}
