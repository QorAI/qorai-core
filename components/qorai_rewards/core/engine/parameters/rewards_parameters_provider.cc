...
#include "qorai/components/qorai_rewards/core/engine/util/callback_helpers.h"
...
constexpr char kVQorDeadlineKey[] = "vqor_deadline";
constexpr char kVQorExpiredKey[] = "vqor_expired";
...
mojom::RewardsParametersPtr RewardsParametersProvider::DictToParameters(
  ...
  if (auto* deadline_value = dict.Find(kVQorDeadlineKey)) {
    if (auto time = base::ValueToTime(*deadline_value)) {
      parameters->vqor_deadline = *time;
    }
  }

  if (auto vqor_expired = dict.FindBool(kVQorExpiredKey)) {
    parameters->vqor_expired = *vqor_expired;
  }
...
void RewardsParametersProvider::StoreParameters(
  ...
  dict.Set(kVQorDeadlineKey, base::TimeToValue(parameters.vqor_deadline));
  dict.Set(kVQorExpiredKey, parameters.vqor_expired);
