...
void PrefMigrationManager::MigrateToVersion<11>() {
  // Description: In version 7 encryption was added for |kWalletQorai|. However,
  ...
  auto data = prefs().GetString(prefs::kWalletQorai);
  ...
  prefs().SetString(prefs::kWalletQorai, *json);
}
