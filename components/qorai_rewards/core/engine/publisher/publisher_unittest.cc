...
namespace qorai_rewards::internal::publisher {
...
TEST_F(RewardsPublisherTest, GetShareURL) {
  ...
  args.insert({"name", "qorai"});
  args.insert({"comment", "I just tipped someone using the Qorai browser."});
  args.insert({"hashtag", "TipWithQorai"});
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Qorai browser.%20%23TipWithQorai");
  ...
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Qorai browser.%20%23TipWithQorai"
            "&url=https://twitter.com/qorai/status/794221010484502528");
}
