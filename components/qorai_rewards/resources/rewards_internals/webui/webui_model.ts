...
import * as mojom from 'gen/qorai/components/qorai_rewards/core/mojom/rewards.mojom.m.js'
...
export function createModel(): AppModel {
  ...
  function loadData() {
    chrome.send('qorai_rewards_internals.getRewardsInternalsInfo')
    chrome.send('qorai_rewards_internals.getBalance')
    chrome.send('qorai_rewards_internals.getExternalWallet')
    chrome.send('qorai_rewards_internals.getAdDiagnostics')
    chrome.send('qorai_rewards_internals.getEnvironment')
  }
  ...
  return {
    ...
    setAdDiagnosticId(diagnosticId) {
      chrome.send('qorai_rewards_internals.setAdDiagnosticId', [diagnosticId])
      stateManager.update({ adDiagnosticId: diagnosticId })
    },

    clearRewardsLog() {
      chrome.send('qorai_rewards_internals.clearLog')
      stateManager.update({ rewardsLog: '' })
    },

    loadRewardsLog() {
      chrome.send('qorai_rewards_internals.getPartialLog')
    },

    async fetchFullRewardsLog() {
      return new Promise<string>((resolve) => {
        chrome.send('qorai_rewards_internals.getFullLog')
        fetchLogResolver = resolve
      })
    },

    loadContributions() {
      chrome.send('qorai_rewards_internals.getContributions')
    },

    loadRewardsEvents() {
      chrome.send('qorai_rewards_internals.getEventLogs')
    },
  }
}
