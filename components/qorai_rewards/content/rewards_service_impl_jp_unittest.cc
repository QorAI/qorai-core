...
#include "qorai/components/qorai_rewards/content/test_util.h"
...
// npm run test -- qorai_unit_tests --filter=RewardsServiceJPTest.*
 ...
 protected:
  void SetUp() override {
    ...
    scoped_default_locale_ =
        std::make_unique<qorai_l10n::test::ScopedDefaultLocale>("ja_JP");
    profile_ = CreateQoraiRewardsProfile(temp_dir_.GetPath());
