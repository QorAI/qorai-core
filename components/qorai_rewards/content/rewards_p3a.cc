...
void RecordAdTypesEnabled(PrefService* prefs) {
  ...
  int notification_enabled =
      prefs->GetBoolean(qorai_ads::prefs::kOptedInToNotificationAds);
  int search_result_enabled =
      prefs->GetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds);
  int answer =
...
void RecordSearchResultAdsOptinChange(PrefService* prefs) {
  if (prefs->GetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds)) {
