...
#include "qorai/components/qorai_rewards/content/diagnostic_log.h"
#include "qorai/components/qorai_rewards/content/logging.h"
#include "qorai/components/qorai_rewards/content/rewards_notification_service.h"
#include "qorai/components/qorai_rewards/content/rewards_notification_service_impl.h"
...
#include "qorai/components/qorai_rewards/content/service_sandbox_type.h"
...
#include "qorai/components/qorai_rewards/core/engine/parameters/rewards_parameters_provider.h"
...
#include "qorai/components/qorai_rewards/core/publisher_utils.h"
...
constexpr char pref_prefix[] = "qorai.rewards";
...
GetNetworkTrafficAnnotationTagForFaviconFetch() {
  return net::DefineNetworkTrafficAnnotation("qorai_rewards_favicon_fetcher",
                                             R"(
      semantics {
        sender:
          "Qorai Rewards Media Fetcher"
        description:
  ...
  return net::DefineNetworkTrafficAnnotation("rewards_service_impl", R"(
      semantics {
        ...
        description:
          ...
          "monthly QOR contribution."
        trigger:
          "User-initiated for direct tipping or on a set interval while Qorai "
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature via the QOR icon in the URL "
          "bar or by visiting qorai://rewards/."
        policy_exception_justification:
          "Not implemented."
      })");
}
...
RewardsServiceImpl::RewardsServiceImpl(
    ...
    qorai_wallet::QoraiWalletService* qorai_wallet_service)
    : prefs_(prefs),
...
void RewardsServiceImpl::OnPreferenceChanged(const std::string& key) {
  if (key == ntp_background_images::prefs::
                 kNewTabPageShowSponsoredImagesBackgroundImage ||
      key == qorai_ads::prefs::kOptedInToNotificationAds ||
      key == qorai_ads::prefs::kOptedInToSearchResultAds) {
    p3a::RecordAdTypesEnabled(prefs_);
  }

  if (key == qorai_ads::prefs::kOptedInToSearchResultAds) {
    GetExternalWallet(
...
void RewardsServiceImpl::CheckPreferences() {
  if (prefs_->GetBoolean(qorai_ads::prefs::kOptedInToNotificationAds)) {
  ...
  if (prefs_->GetBoolean(prefs::kEnabled) &&
      ...
      !prefs_->HasPrefPath(qorai_ads::prefs::kOptedInToSearchResultAds)) {
    prefs_->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, false);
  }
}
  ...
  auto on_start = [](base::WeakPtr<RewardsServiceImpl> self,
    ...
    auto on_created = [](base::WeakPtr<RewardsServiceImpl> self,
      ...
      if (!self->prefs_->GetBoolean(prefs::kEnabled)) {
        ...
        self->prefs_->SetBoolean(qorai_ads::prefs::kOptedInToNotificationAds,
                                 true);
...
void RewardsServiceImpl::GetSPLTokenAccountBalance(
  ...
  qorai_wallet_service_->json_rpc_service()->GetSPLTokenAccountBalance(
      solana_address, token_mint_address, qorai_wallet::mojom::kSolanaMainnet,
...
void RewardsServiceImpl::OnGetSPLTokenAccountBalance(
  ...
  if (error != qorai_wallet::mojom::SolanaProviderError::kSuccess) {
...
mojom::RewardsEngineOptionsPtr RewardsServiceImpl::HandleFlags(
  ...
  // in a future version. Use --enable-features=QoraiRewardsVerboseLogging
  // instead.
...
void RewardsServiceImpl::ConnectExternalWallet(
  ...
  const auto url = GURL("qorai:/" + path + query);
...
void RewardsServiceImpl::ExternalWalletConnected() {
  ...
  // users cannot earn QOR for them yet. The user can turn them on manually.
  prefs_->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, false);
  for (auto& observer : observers_) {
