 ...
 public:
  DeAmpBrowserTest() {
    ...
    feature_list_.InitWithFeatures(
        /*enabled_features*/ {de_amp::features::kQoraiDeAMP},
        disabled_features);
  ...
  void SetRequestHandler(
    ...
    auto handler = [](const std::string& page_path, const std::string& body,
      ...
      [&request] {
        ...
        ASSERT_EQ(request.headers.find("X-Qorai-De-AMP"),
                  request.headers.end());
      }();
...
IN_PROC_BROWSER_TEST_F(DeAmpBrowserTest, NonHttpScheme) {
  TogglePref(true);
  SetRequestHandler(kTestAmpPage, Amp(GURL("qorai://settings")));
  StartServer();
...
class DeAmpBrowserTestBaseFeatureDisabled : public DeAmpBrowserTest {
 public:
  DeAmpBrowserTestBaseFeatureDisabled() {
    feature_list_.Reset();
    feature_list_.InitAndDisableFeature(de_amp::features::kQoraiDeAMP);
  }
};
