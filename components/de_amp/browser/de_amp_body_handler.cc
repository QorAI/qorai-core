...
constexpr const char kDeAmpHeaderName[] = "X-Qorai-De-AMP";
...
std::unique_ptr<DeAmpBodyHandler> DeAmpBodyHandler::Create(
  ...
  if (!base::FeatureList::IsEnabled(de_amp::features::kQoraiDeAMP) ||
...
bool DeAmpBodyHandler::OpenCanonicalURL(const GURL& new_url) {
  ...
  // https://github.com/qorai/qorai-browser/issues/22610
