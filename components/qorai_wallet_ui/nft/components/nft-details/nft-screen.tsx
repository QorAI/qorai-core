...
// types
import { QoraiWallet, AccountPageTabs } from '../../../constants/types'
...
interface Props {
  ...
  tokenNetwork?: QoraiWallet.NetworkInfo
}
...
export const NftScreen = ({ selectedAsset, tokenNetwork }: Props) => {
  ...
  const onClickViewOnBlockExplorer = useExplorer(
    tokenNetwork || new QoraiWallet.NetworkInfo(),
  )
  ...
  const onClickContractAddress = React.useCallback(() => {
    onClickViewOnBlockExplorer(
      selectedAsset.coin === QoraiWallet.CoinType.ETH ? 'nft' : 'token',
  ...
  if (nftMetadataError)
    return (
      <StyledWrapper>
        ...
        >
          <ErrorMessage>
            {getLocale('qoraiWalletNftFetchingError')}
          </ErrorMessage>
        </Row>
      </StyledWrapper>
    )
  ...
  return (
    <StyledWrapper>
      {isTokenWatchOnly(
        ...
        <>
          <Alert type='info'>{getLocale('qoraiWalletUnownedNftAlert')}</Alert>
      ...
      <SectionTitle>{getLocale('qoraiWalletNFTDetailsOverview')}</SectionTitle>
      {ownerAddress && (
        <SectionWrapper>
          <InfoBox>
            <InfoTitle>{getLocale('qoraiWalletNFTDetailsOwnedBy')}</InfoTitle>
            <Row
            ...
            {ownerAccount ? (
              <ViewAccount onClick={() => onClickViewAccount(ownerAccount)}>
                {getLocale('qoraiWalletNFTDetailsViewAccount')}
              </ViewAccount>
            ) : (
              <ViewAccount
                onClick={onClickViewOnBlockExplorer('address', ownerAddress)}
              >
                {getLocale('qoraiWalletPortfolioViewOnExplorerMenuLabel')}
              </ViewAccount>
            )}
          </InfoBox>
        </SectionWrapper>
      ...
      <SectionWrapper>
        {selectedAsset.tokenId !== '' && (
          <InfoBox>
            <InfoTitle>{getLocale('qoraiWalletNFTDetailTokenID')}</InfoTitle>
            <Row
              justifyContent='flex-start'
              gap='4px'
            >
        ...
        {tokenNetwork && (
          <InfoBox>
            <InfoTitle>{getLocale('qoraiWalletNFTDetailBlockchain')}</InfoTitle>
      ...
      <SectionWrapper>
        ...
        <InfoBox>
          <InfoTitle>
            {getLocale('qoraiWalletNFTDetailTokenStandard')}
          </InfoTitle>
      ...
      <SectionWrapper>
        {isFetchingNFTMetadata ? (
          ...
          <InfoBox>
            <InfoTitle>
              {getLocale('qoraiWalletNFTDetailDescription')}
            </InfoTitle>
            <Description>
      ...
      {isFetchingNFTMetadata ? (
        ...
        <>
          {nftMetadata?.attributes?.length !== 0 && (
            <SectionTitle>
              {getLocale('qoraiWalletNFTDetailsProperties')}
            </SectionTitle>
          )}
        </>
      )}
