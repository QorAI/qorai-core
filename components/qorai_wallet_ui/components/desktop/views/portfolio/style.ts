...
export const PercentBubble = styled.div<{ isDown?: boolean }>`
  ...
  border-radius: 4px;
  background-color: ${(p) =>
    p.isDown ? qora.color.red[20] : qora.color.green[20]};
  font-family: Poppins;
  font-size: 11px;
  line-height: 16px;
  letter-spacing: 0.02em;
  font-weight: 500;
  color: ${(p) => (p.isDown ? qora.color.red[50] : qora.color.green[50])};
`
...
export const ControlsRow = styled(Row)`
  box-shadow: 0px -1px 1px ${qora.color.elevation.primary};
...
export const PortfolioActionButton = styled(WalletButton)`
  --button-border-color: ${qora.color.divider.interactive};
  display: flex;
  align-items: center;
  justify-content: center;
  cursor: pointer;
...
export const ContentWrapper = styled(Column)<{
  ...
  background-color: ${(p) =>
    p.isMobileOrPanel ? qora.color.container.background : 'transparent'};
`
