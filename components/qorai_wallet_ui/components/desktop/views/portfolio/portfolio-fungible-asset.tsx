...
export const PortfolioFungibleAsset = () => {
  ...
  const fullAssetBalance = React.useMemo(() => {
    const amounts = candidateAccounts.map((account: QoraiWallet.AccountInfo) =>
      getBalance(
        account.accountId,
        selectedAssetFromParams,
        ...
        <Row padding='0px 20px'>
          <ButtonRow>
            {foundMeldBuyToken && !isRewardsToken && (
              <PortfolioAssetActionButton
                text={getLocale('qoraiWalletBuy')}
                icon='coins-alt1'
                onClick={onClickBuy}
              />
            )}
            <PortfolioAssetActionButton
              text={getLocale('qoraiWalletSend')}
              icon='send'
              onClick={onClickSend}
            />
            {isSwapOrBridgeSupported && (
              <>
                <PortfolioAssetActionButton
                  text={getLocale('qoraiWalletSwap')}
                  icon='currency-exchange'
                  onClick={() => onClickSwapOrBridge('swap')}
                />
                <PortfolioAssetActionButton
                  text={getLocale('qoraiWalletBridge')}
                  icon='web3-bridge'
            ...
            {isSelectedAssetDepositSupported && (
              <PortfolioAssetActionButton
                text={getLocale('qoraiWalletAccountsDeposit')}
