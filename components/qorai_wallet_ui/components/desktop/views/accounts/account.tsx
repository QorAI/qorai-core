...
const INDIVIDUAL_TESTNET_ACCOUNT_KEYRING_IDS = [
  ...BitcoinTestnetKeyringIds,
  QoraiWallet.KeyringId.kFilecoinTestnet,
  ...ZCashTestnetKeyringIds,
  ...CardanoTestnetKeyringIds,
]
...
const coinSupportsNFTs = (coin: QoraiWallet.CoinType) => {
  return [QoraiWallet.CoinType.ETH, QoraiWallet.CoinType.SOL].includes(coin)
}

const coinSupportsAssets = (coin: QoraiWallet.CoinType) => {
  return [QoraiWallet.CoinType.ETH, QoraiWallet.CoinType.SOL].includes(coin)
}
...
export const Account = () => {
  ...
  const zcashAccountIds = accounts
    .filter((account) => account.accountId.coin === QoraiWallet.CoinType.ZEC)
  ...
  const accountsTokensList = React.useMemo(() => {
    ...
    const hasLocalHostNetwork = networkList.some(
      (network) =>
        network.chainId === QoraiWallet.LOCALHOST_CHAIN_ID
    ...
    const localHostCoins = userVisibleTokensInfo.filter(
      (token) => token.chainId === QoraiWallet.LOCALHOST_CHAIN_ID,
    )
    ...
    if (
      accountsLocalHost
      && hasLocalHostNetwork
      && selectedAccount.accountId.keyringId !== QoraiWallet.KeyringId.kFilecoin
    ) {
  ...
  const onClickMenuOption = React.useCallback(
    (option: AccountModalTypes) => {
      ...
      if (
        ...
        && (selectedAccount?.accountId.coin === QoraiWallet.CoinType.ETH
          || selectedAccount?.accountId.coin === QoraiWallet.CoinType.SOL)
      ) {
      ...
      {showSyncWarning && (
        ...
        >
          ...
          >
            <div slot='title'>
              {!chainTipStatus
                ? getLocale('qoraiWalletOutOfSyncTitle')
                : getLocale(
                    blocksBehind < 1000
                      ? 'qoraiWalletBlocksBehind'
            ...
            <div>
              {accountsTokensList
                ...
                && getLocale('qoraiWalletZCashPendingBalanceTitle').replace(
                  '$1',
                  formatTokenBalanceWithSymbol(
            ...
            {getLocale('qoraiWalletOutOfSyncDescription')}
            <Row
              slot='actions'
              width='unset'
              gap='8px'
            >
              ...
              >
                ...
                />
                {isAccountSyncing
                  ? getLocale('qoraiWalletSyncAccountButtonInProgress')
                  : getLocale('qoraiWalletSyncAccountButton')}
              </Button>
              <Button
                size='small'
                kind='plain-faint'
              ...
              >
                {getLocale('qoraiWalletDismissButton')}
              </Button>
            </Row>
          </SyncAlert>
        </SyncAlertWrapper>
      ...
      {selectedTab === AccountPageTabs.AccountNFTsSub
        && (nonFungibleTokens?.length !== 0 ? (
          <NFTsWrapper fullWidth={true}>
            <NftGrid>
              {nonFungibleTokens?.map((nft: QoraiWallet.BlockchainToken) => (
                <NFTGridViewItem
          ...
          {transactionList.length !== 0 ? (
            ...
            >
              ...
              <Text
                ...
                isBold={true}
              >
                {getLocale('qoraiWalletNoTransactionsYet')}
              </Text>
              ...
              <Text
                ...
                isBold={false}
              >
                {getLocale('qoraiWalletNoTransactionsYetDescription')}
              </Text>
            </Column>
          )}
        </>
