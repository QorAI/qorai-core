...
export const Accounts = () => {
  ...
  const onSelectAccount = React.useCallback(
    (account: QoraiWallet.AccountInfo | undefined) => {
      if (account) {
        history.push(
  ...
  const derivedAccounts = React.useMemo(() => {
    return accounts.filter(
      (account) => account.accountId.kind === QoraiWallet.AccountKind.kDerived,
    )
  }, [accounts])

  const importedAccounts = React.useMemo(() => {
    return accounts.filter(
      (account) => account.accountId.kind === QoraiWallet.AccountKind.kImported,
    )
  }, [accounts])
  ...
  const trezorAccounts = React.useMemo(() => {
    const foundTrezorAccounts = accounts.filter((account) => {
      return (
        account.accountId.kind === QoraiWallet.AccountKind.kHardware
        && account.hardware?.vendor === QoraiWallet.HardwareVendor.kTrezor
      )
    })
  ...
  const ledgerAccounts = React.useMemo(() => {
    const foundLedgerAccounts = accounts.filter((account) => {
      return (
        account.accountId.kind === QoraiWallet.AccountKind.kHardware
        && account.hardware?.vendor === QoraiWallet.HardwareVendor.kLedger
      )
    })
  ...
  return (
    ...
    >
      ...
      >
        <SectionTitle>{getLocale('qoraiWalletAccounts')}</SectionTitle>
      </Row>
      <AccountsListWrapper
        fullWidth={true}
        alignItems='flex-start'
      ...
      {importedAccounts.length !== 0 && (
        <>
          ...
          >
            <SectionTitle>
              {getLocale('qoraiWalletAccountsSecondary')}
            </SectionTitle>
          </Row>
          <AccountsListWrapper
            fullWidth={true}
      ...
      {showHardwareWallets && (
        <>
          ...
          >
            <SectionTitle>
              {getLocale('qoraiWalletConnectedHardwareWallets')}
            </SectionTitle>
          </Row>
          <Column
            fullWidth={true}
      ...
      {externalRewardsAccount && (
        <>
          ...
          >
            <SectionTitle>
              {getLocale('qoraiWalletConnectedAccounts')}
            </SectionTitle>
          </Row>
          <AccountsListWrapper
            fullWidth={true}
