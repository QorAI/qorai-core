...
export const ExploreWeb3View = () => {
  ...
  const [dappCategories, categoryDappsMap, visibleCategories] =
    React.useMemo(() => {
      if (!topDappsForChains) {
        return [[], new Map<string, QoraiWallet.Dapp[]>(), []]
      }

      const categoriesSet = new Set<string>()
      const categoriesMap = new Map<string, QoraiWallet.Dapp[]>()
  ...
  const visibleDappsMap = React.useMemo(() => {
    ...
    const filterResultsMap = new Map<string, QoraiWallet.Dapp[]>()
    ...
    <>
      ...
      >
        ...
        {showFiltersRow && (
          ...
          >
            ...
            <PlainButton onClick={onClearFilters}>
              {getLocale('qoraiWalletClearFilters')}
            </PlainButton>
          </Row>
        )}
        ...
        || selectedCategoryDapps?.length === 0 ? (
          ...
          <Row>
            <h2>{getLocale('qoraiWalletNoDappsFound')}</h2>
          </Row>
        )}
      </Column>
