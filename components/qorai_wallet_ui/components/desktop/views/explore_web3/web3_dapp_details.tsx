...
import { DialogProps } from '@qorai/qora/react/dialog'
...
import type { QoraiWallet } from '../../../../constants/types'
...
export const DappDetails = ({ dapp, ...rest }: DappDetailsProps) => {
  ...
  return (
    <DappDetailDialog
      ...
      showClose={true}
    >
      <Title slot='title'>{getLocale('qoraiWalletDetails')}</Title>
      <Column
        fullHeight
        fullWidth
      ...
      >
        ...
        >
          <Row gap={'8px'}>
            <DappMetric
              metricName={getLocale('qoraiWalletActiveWallets')}
              meticValue={dapp.uaw}
              isUSDValue={false}
            />
            <DappMetric
              metricName={getLocale('qoraiWalletTransactions')}
          ...
          <Row gap={'8px'}>
            <DappMetric
              metricName={getLocale('qoraiWalletMarketDataVolumeColumn')}
              meticValue={dapp.volume}
              isUSDValue={true}
            />
            <DappMetric
              metricName={getLocale('qoraiWalletBalance')}
        ...
        >
          <Text textAlign='left'>{getLocale('qoraiWalletNetworks')}</Text>
          <Row
            justifyContent={'flex-start'}
            alignItems={'flex-start'}
          >
        ...
        {isHttpsUrl(dapp.website) ? (
          <ExternalLink
            href={dapp.website}
            text={getLocale('qoraiWalletVisitDapp').replace('$1', dapp.name)}
            width='100%'
          />
        ) : null}
      </Column>
