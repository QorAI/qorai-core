...
export const AssetItemMenu = (props: Props) => {
  ...
  const { data: availableShieldedAccount } =
    useGetAvailableShieldedAccountQuery(
      asset.coin === QoraiWallet.CoinType.ZEC
        && !asset.isShielded
    ...
    <StyledWrapper yPosition={42}>
      {foundMeldBuyToken && (
        <PopupButton onClick={onClickBuy}>
          ...
          <PopupButtonText>{getLocale('qoraiWalletBuy')}</PopupButtonText>
        </PopupButton>
      )}
      {!isAssetsBalanceZero && (
        <PopupButton onClick={onClickSend}>
          <ButtonIcon name='send' />
          <PopupButtonText>{getLocale('qoraiWalletSend')}</PopupButtonText>
        </PopupButton>
      )}
      {isSwapOrBridgeSupported && (
        <>
          <PopupButton onClick={() => onClickSwapOrBridge('swap')}>
            ...
            <PopupButtonText>{getLocale('qoraiWalletSwap')}</PopupButtonText>
          </PopupButton>
          <PopupButton onClick={() => onClickSwapOrBridge('bridge')}>
            <ButtonIcon name='web3-bridge' />
            <PopupButtonText>{getLocale('qoraiWalletBridge')}</PopupButtonText>
          </PopupButton>
        </>
      )}
      <PopupButton onClick={onClickDeposit}>
        ...
        <PopupButtonText>
          {getLocale('qoraiWalletAccountsDeposit')}
        </PopupButtonText>
      </PopupButton>
      {isSellSupported && (
        <PopupButton onClick={onClickSell}>
          <ButtonIcon name='usd-circle' />
          <PopupButtonText>{getLocale('qoraiWalletSell')}</PopupButtonText>
        </PopupButton>
      )}
      {onClickEditToken && (
      ...
      {availableShieldedAccount && (
        <>
          ...
          <PopupButton onClick={onClickShieldFunds}>
            ...
            <PopupButtonText>
              {getLocale('qoraiWalletShieldFunds')}
            </PopupButtonText>
          </PopupButton>
        </>
      )}
