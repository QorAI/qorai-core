...
export const WalletSettingsMenu = (props: Props) => {
  ...
  const onClickConnectedSites = React.useCallback(() => {
    ...
    chrome.tabs.create({ url: `qorai://settings/content/${route}` }, () => {
  ...
  const onClickHelpCenter = React.useCallback(() => {
    chrome.tabs.create(
      {
        url: 'https://support.qorai.app/hc/categories/360001062531-Wallet',
      },
      () => {
        if (chrome.runtime.lastError) {
          console.error(
  ...
  const accountSettingsOptions = React.useMemo(() => {
    if (isMobile) {
      return CreateAccountOptions.filter(
        (option) => option.name !== 'qoraiWalletConnectHardwareWallet',
      )
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <PopupButtonText>
            {getLocale('qoraiWalletWalletPopupLock')}
          </PopupButtonText>
        </PopupButton>
        ...
        <PopupButton onClick={onClickBackup}>
          ...
          <PopupButtonText>
            {getLocale('qoraiWalletBackupButton')}
          </PopupButtonText>
        </PopupButton>

        {(selectedNetwork?.coin === QoraiWallet.CoinType.ETH
          || selectedNetwork?.coin === QoraiWallet.CoinType.SOL)
          && !isMobile && (
            <PopupButton onClick={onClickConnectedSites}>
              <ButtonIcon name='link-normal' />
              <PopupButtonText>
                {getLocale('qoraiWalletWalletPopupConnectedSites')}
              </PopupButtonText>
            </PopupButton>
          )}
        ...
        {!isMobile && (
          <PopupButton onClick={onClickSettings}>
            ...
            <PopupButtonText>
              {getLocale('qoraiWalletWalletPopupSettings')}
            </PopupButtonText>
          </PopupButton>
        )}
      </Column>
      ...
      {(walletLocation === WalletRoutes.PortfolioNFTs
        ...
        <>
          <SectionLabel justifyContent='flex-start'>
            {getLocale('qoraiWalletPortfolioSettings')}
          </SectionLabel>
          <Column
            fullWidth={true}
            padding='8px 8px 0px 8px'
          >
            <ToggleRow onClick={onToggleHideBalances}>
              <Row>
                ...
                <PopupButtonText>
                  {getLocale('qoraiWalletWalletPopupHideBalances')}
                </PopupButtonText>
                <Toggle
            ...
            <ToggleRow onClick={onToggleHideGraph}>
              <Row>
                ...
                <PopupButtonText>
                  {getLocale('qoraiWalletWalletPopupShowGraph')}
                </PopupButtonText>
              </Row>
              <Toggle
            ...
            <ToggleRow onClick={onToggleHideNFTsTab}>
              <Row>
                <ButtonIcon name='nft' />
                <PopupButtonText>
                  {getLocale('qoraiWalletWalletNFTsTab')}
                </PopupButtonText>
              </Row>
              <Toggle
      ...
      {walletLocation === WalletRoutes.Accounts && isMobileOrPanel && (
        <>
          <SectionLabel justifyContent='flex-start'>
            {getLocale('qoraiWalletAccountSettings')}
          </SectionLabel>
          <Column
            fullWidth={true}
            padding='8px 8px 0px 8px'
