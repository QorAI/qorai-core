...
export const SellAssetModal = (props: Props) => {
  ...
  return (
    <PopupModal
      title={`${getLocale('qoraiWalletSell')} ${selectedAsset.name}`}
    ...
    >
      <StyledWrapper>
          ...
          {insufficientBalance && !isInvalidAmount && (
            <ErrorBox>
              ...
              <Text
                ...
                textAlign='left'
              >
                {getLocale('qoraiWalletNotEnoughBalance').replace(
                  '$1',
                  selectedAsset.symbol,
                )}
              </Text>
        ...
        >
          {getLocale('qoraiWalletSellWithProvider').replace('$1', 'Ramp')}
        </QoraSquaredButton>
      </StyledWrapper>
    </PopupModal>
  )
