...
interface VirtualizedTokensListProps {
  ...
  onRemoveAsset: (token: QoraiWallet.BlockchainToken) => void
  isAssetSelected: (token: QoraiWallet.BlockchainToken) => boolean
  onClickAddCustomAsset: () => void
  onCheckWatchlistItem: (token: QoraiWallet.BlockchainToken) => void
}

interface ListItemProps extends Omit<VirtualizedTokensListProps, 'tokenList'> {
  index: number
  data: QoraiWallet.BlockchainToken
...
const isRemovable = (token: QoraiWallet.BlockchainToken): boolean => {
  return (
      ...
      {isLastIndex && (
        <PaddedColumn margin='8px 0px'>
          ...
          <Text
            ...
            textColor='text02'
          >
            {getLocale('qoraiWalletDidntFindAssetEndOfList')}
          </Text>
          <VerticalSpace space='14px' />
          <Button
