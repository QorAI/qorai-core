...
const onlyInLeft = (
  left: QoraiWallet.BlockchainToken[],
  right: QoraiWallet.BlockchainToken[],
) =>
  left.filter(
    (leftValue) =>
      !right.some(
...
export const EditVisibleAssetsModal = ({ onClose }: Props) => {
  ...
  const [selectedNetworkFilter, setSelectedNetworkFilter] =
    React.useState<QoraiWallet.NetworkInfo>(AllNetworksOption)
  ...
  const tokenListForSelectedNetworks: QoraiWallet.BlockchainToken[] =
    React.useMemo(() => {
  ...
  const sortedTokenListForSelectedNetworks: QoraiWallet.BlockchainToken[] =
    React.useMemo(() => {
    ...
    <PopupModal
      title={
        showAddCustomToken
          ...
          : getLocale('qoraiWalletAccountsEditVisibleAssets')
      }
      onClose={onClose}
      width='500px'
    ...
    >
          ...
          >
            ...
            >
              {hash === WalletRoutes.AvailableAssetsHash
                ? getLocale('qoraiWalletAvailableAssets')
                : getLocale('qoraiWalletMyAssets')}
            </Text>
            {hash === WalletRoutes.AvailableAssetsHash && (
                ...
                <EmptyStateWrapper>
                  <Column>
                    ...
                    <Text
                      ...
                      isBold={true}
                    >
                      {getLocale('qoraiWalletAssetNotFound')}
                    </Text>
                    ...
                    <Text
                      ...
                      isBold={false}
                    >
                      {getLocale('qoraiWalletDidntFindAssetInList')}
                    </Text>
      ...
      {!showAddCustomToken && (
        <ButtonRow>
          ...
          <HorizontalSpace space='16px' />
          <QoraSquaredButton onClick={onClickDone}>
