...
import { type InputEventDetail } from '@qorai/qora/react/input'
...
// style
import { Column, QoraSquaredButton, Row, Text } from '../../../shared/style'
...
export const RemoveAccountModal = () => {
  ...
  const onConfirmRemoveAccount = React.useCallback(
    async (password: string) => {
      ...
      if (
        accountId.kind === QoraiWallet.AccountKind.kHardware
        || accountId.kind === QoraiWallet.AccountKind.kImported
      ) {
  ...
  return (
    <PopupModal
      title={getLocale('qoraiWalletAccountSettingsRemove')}
      onClose={() => dispatch(AccountsTabActions.setAccountToRemove(undefined))}
      width={modalWidth}
      headerPaddingHorizontal={qora.spacing['3Xl']}
    >
      <StyledWrapper>
        <Column
          fullWidth
        ...
        >
          {accountToRemove && (
            <Column
              ...
              margin={`0px 0px ${qora.spacing['2Xl']} 0px`}
              gap={qora.spacing.m}
            >
              <Text
                textSize='16px'
                textAlign='left'
              >
                {getLocale('qoraiWalletRemoveAccountModalTitle').replace(
                  '$1',
              ...
              <Text
                ...
                textColor={'tertiary'}
              >
                {getLocale('qoraiWalletPasswordIsRequiredToTakeThisAction')}
              </Text>
            </Column>
          )}
        ...
        {isMobile || isPanel ? (
          <Row gap='4px'>
            ...
            >
              {getLocale('qoraiWalletAccountsRemove')}
            </QoraSquaredButton>
          </Row>
        ) : (
          <Row
