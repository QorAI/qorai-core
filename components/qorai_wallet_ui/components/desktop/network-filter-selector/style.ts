...
export const DropDownButton = styled(WalletButton)<{
  ...
  flex-direction: row;
  background-color: ${(p) =>
    p.isV2 ? qora.color.container.highlight : 'transparent'};
  cursor: pointer;
  outline: none;
  border: ${(p) =>
  ...
  color: ${(p) => (p.isV2 ? qora.color.text.secondary : p.theme.color.text01)};
`
...
export const DropDownIcon = styled(CaratStrongDownIcon)<{
  ...
  height: 18px;
  color: ${(p) =>
    p.isV2 ? qora.color.text.secondary : p.theme.color.interactive07};
`
