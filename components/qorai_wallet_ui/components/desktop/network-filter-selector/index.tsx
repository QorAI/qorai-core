...
interface Props {
  networkListSubset?: QoraiWallet.NetworkInfo[]
...
export const NetworkFilterSelector = ({
  ...
  // memos
  const filteredNetworks: QoraiWallet.NetworkInfo[] = React.useMemo(() => {
  ...
  const { primaryNetworks, secondaryNetworks, testNetworks } =
    React.useMemo(() => {
      const primaryNetworks: QoraiWallet.NetworkInfo[] =
        disableAllAccountsOption ? [] : [AllNetworksOption]
      const secondaryNetworks: QoraiWallet.NetworkInfo[] = []
      const testNetworks: QoraiWallet.NetworkInfo[] = []
