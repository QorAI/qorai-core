...
export const AccountDetailsHeader = (props: Props) => {
  ...
  const menuOptions = React.useMemo((): AccountButtonOptionsObjectType[] => {
    ...
    if (account.accountId.kind === QoraiWallet.AccountKind.kDerived) {
    ...
    if (
      ...
      || account.accountId.coin === QoraiWallet.CoinType.DOT
    ...
    // Link to issue https://github.com/qorai/qorai-browser/issues/39699
    if (
      account.accountId.coin === QoraiWallet.CoinType.BTC
      || account.accountId.coin === QoraiWallet.CoinType.ZEC
      || account.accountId.coin === QoraiWallet.CoinType.ADA
      ...
      <Row width='unset'>
        {!isMobileOrPanel && (
          <>
            ...
            >
              <AccountsNetworkText>
                {getLocale('qoraiWalletAccountBalance')}
              </AccountsNetworkText>
