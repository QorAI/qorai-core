...
export const AssetTicker = styled.span<{
  ...
  margin-right: 4px;
  color: ${(p) =>
    p.role === 'currency' ? qora.color.text.tertiary : qora.color.text.primary};
  cursor: pointer;
`

export const CaratDownIcon = styled(Icon).attrs({ name: 'carat-down' })`
  --qora-icon-size: 22px;
  color: ${(p) => qora.color.icon.default};
`
...
export const Input = styled.input<{ hasError?: boolean }>`
  ...
  margin: 4px 0px;
  color: ${(p) =>
    p.hasError ? p.theme.color.errorText : qora.color.text.primary};
