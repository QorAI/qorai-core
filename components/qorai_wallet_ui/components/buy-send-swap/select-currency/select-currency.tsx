...
export interface Props {
  onSelectCurrency: (currency: QoraiWallet.OnRampCurrency) => void
  onBack: () => void
}
...
export const SelectOnRampFiatCurrency = (props: Props) => {
  ...
  return (
    <SelectCurrencyWrapper>
      <Header
        title={getLocale('qoraiWalletSelectCurrency')}
        onBack={onBack}
        hasAddButton={false}
      />
      <SearchBar
      ...
      <SelectScrollSearchContainer>
        {isLoading ? (
          ...
          filteredCurrencies.map((currency: QoraiWallet.OnRampCurrency) => (
            <SelectCurrencyItem
