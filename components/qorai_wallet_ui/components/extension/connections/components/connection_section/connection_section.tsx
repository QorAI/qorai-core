...
interface Props {
  ...
  selectedAccountId: QoraiWallet.AccountId | undefined
}
...
export const ConnectionSection = (props: Props) => {
  ...
  const connectionStatusText = React.useMemo((): string => {
    if (isPermissionDenied) {
      return getLocale('qoraiWalletPanelBlocked')
    }
    return isConnected
      ? getLocale('qoraiWalletPanelConnected')
      : getLocale('qoraiWalletNotConnected')
  ...
  const onClickUnblock = React.useCallback(() => {
    chrome.tabs.create(
      {
        url: 'qorai://settings/content/all',
      },
      () => {
  ...
  const canChangeNetwork = props.coin !== QoraiWallet.CoinType.ADA
  ...
  React.useEffect(() => {
    ...
    if (selectedAccount && coin === QoraiWallet.CoinType.SOL) {
      ;(async () => {
    ...
    if (coin) {
      ;(async () => {
        await getWalletPanelApiProxy()
          .qoraiWalletService.isPermissionDenied(coin)
          .then((result) => {
            if (subscribed) {
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <SelectButton
              ...
              data-test-id={
                coin === QoraiWallet.CoinType.ETH ? 'select-network-button' : ''
              }
            >
              <Row width='unset'>
                <CreateNetworkIcon
          ...
          >
            ...
            >
              {isPermissionDenied
                ? getLocale('qoraiWalletUnblock')
                : isConnected
                  ? getLocale('qoraiWalletSitePermissionsDisconnect')
                  : getLocale('qoraiWalletAddAccountConnect')}
            </Button>
          </Row>
        </ControlsWrapper>
      </ConnectionCard>
      <BottomSheet
        ...
        title={getLocale('qoraiWalletChangeAccount')}
      >
        <DAppConnectionAccounts
          coin={coin}
      ...
      <BottomSheet
        ...
        title={getLocale('qoraiWalletChangeNetwork')}
      >
        <DAppConnectionNetworks
          accountId={selectedAccountId}
