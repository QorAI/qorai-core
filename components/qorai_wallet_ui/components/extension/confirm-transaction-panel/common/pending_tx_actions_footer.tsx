...
// Styled components
import { QoraSquaredButton, Row } from '../../../shared/style'
...
interface Props {
  blowfishWarnings?: QoraiWallet.BlowfishWarning[]
...
type Warning = Pick<QoraiWallet.BlowfishWarning, 'message' | 'severity'>
  ...
  const { confirmButton, rejectButton } = React.useMemo(() => {
    return {
      confirmButton: (
        ...
        >
          {isAccountSyncing
            ? getLocale('qoraiWalletSyncing')
            : isShieldingFunds
              ? getLocale('qoraiWalletShieldZEC')
              : getLocale('qoraiWalletAllowSpendConfirmButton')}
        </QoraSquaredButton>
      ),
      rejectButton: (
        <QoraSquaredButton
        ...
        >
          {getLocale('qoraiWalletAllowSpendRejectButton')}
        </QoraSquaredButton>
      ),
    }
  }, [
      ...
      {rejectAllTransactions && transactionsQueueLength > 1 && (
        <Row padding={rejectAllButtonRowPadding}>
          <QueueStepButton onClick={rejectAllTransactions}>
            {getLocale('qoraiWalletQueueRejectAll').replace(
              '$1',
