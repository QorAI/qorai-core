    ...
    <FullWidth>
      <Alert type={retrySimulation ? 'info' : isCritical ? 'error' : 'warning'}>
        ...
        <Row justifyContent='space-between'>
          {retrySimulation ? (
            <AlertTitle isInfo>
              {getLocale('qoraiWalletTransactionPreviewFailed')}{' '}
              <Button
                kind='plain'
                onClick={retrySimulation}
              >
                <u>{getLocale('qoraiWalletButtonRetry')}</u>
              </Button>
            </AlertTitle>
          ) : (
...
export function TransactionWarnings({
  ...
  warnings: Array<Pick<QoraiWallet.BlowfishWarning, 'message' | 'severity'>>
  ...
  const hasCriticalWarnings = warnings.some(
    (warning) =>
      warning.severity === QoraiWallet.BlowfishWarningSeverity.kCritical,
  )
    ...
    <FullWidth>
      ...
      >
        ...
        >
          {getLocale(
            classifyAs === 'risks'
              ? 'qoraiWalletFoundRisks'
              : 'qoraiWalletFoundIssues',
