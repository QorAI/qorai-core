...
type EVMApprovalData =
  | QoraiWallet.BlowfishERC20ApprovalData
  | QoraiWallet.BlowfishERC721ApprovalData
  | QoraiWallet.BlowfishERC721ApprovalForAllData
  | QoraiWallet.BlowfishERC1155ApprovalForAllData
...
export const EvmNativeAssetOrErc20TokenTransfer = ({
  ...
  transfer:
    | QoraiWallet.BlowfishERC20TransferData
    | QoraiWallet.BlowfishNativeAssetTransferData
  network: ChainInfo
}): JSX.Element => {
  // queries
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {getLocale(isReceive ? 'qoraiWalletReceive' : 'qoraiWalletSend')}
        </Text>
...
export const NonFungibleErcTokenTransfer = ({
  ...
  network: ChainInfo
  transfer: Pick<
    | QoraiWallet.BlowfishERC721TransferData
    | QoraiWallet.BlowfishERC1155TransferData,
...
export const ErcTokenApproval = ({
  ...
  | {
      isERC20: true
      approval: QoraiWallet.BlowfishERC20ApprovalData
      isApprovalForAll?: false
    }
)): JSX.Element => {
  /**
   * Not available for `QoraiWallet.BlowfishERC1155ApprovalForAllData`
   */
  ...
  const beforeAmount = React.useMemo(() => {
    ...
    if (
      ...
      return getLocale('qoraiWalletUnlimitedAssetAmount').replace(
        '$1',
        assetSymbol || '',
      )
    }
  ...
  return (
    ...
    >
      ...
      >
        <StateChangeText>
          <span>{getLocale('qoraiWalletFrom')}</span>
          <strong>{beforeAmount}</strong>
          <ArrowRightIcon />
          <span>{getLocale('qoraiWalletSwapTo')}</span>
      ...
      >
        ...
        >
          <Text textSize='11px'>
            {getLocale('qoraiWalletSpenderAddress').replace(
              '$1',
...
function getTransferTokenVerificationStatusText(
  ...
  if (!transfer.asset.verified) {
    return getLocale('qoraiWalletTokenIsUnverified')
  }
  return transfer.asset.lists.length > 1
    ? getLocale('qoraiWalletTokenIsVerifiedByLists').replace(
        '$1',
        transfer.asset.lists.length.toString(),
      )
    : getLocale('qoraiWalletTokenIsVerified')
}

export function getComponentForEvmApproval(
  approval: QoraiWallet.BlowfishEVMStateChange,
  network: ChainInfo,
) {
...
export function getComponentForEvmTransfer(
  transfer: QoraiWallet.BlowfishEVMStateChange,
  network: ChainInfo,
) {
