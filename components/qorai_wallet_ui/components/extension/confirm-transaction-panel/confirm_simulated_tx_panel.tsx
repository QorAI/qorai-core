...
type Props =
  | {
      simulationType: 'EVM'
      txSimulation: QoraiWallet.EVMSimulationResponse
    }
  | {
      simulationType: 'SVM'
      txSimulation: QoraiWallet.SolanaSimulationResponse
    }
...
export const ConfirmSimulatedTransactionPanel = ({
  ...
  const [isCriticalWarningPopupOpen, setIsCriticalWarningPopupOpen] =
    React.useState<boolean>(
      txSimulation.action === QoraiWallet.BlowfishSuggestedAction.kBlock,
    )
  ...
  const simulationHasOutgoingSPLandSOLTransfers =
    ...
    && txSimulation.expectedStateChanges.some(
      (sim) =>
        sim.rawInfo.kind === QoraiWallet.BlowfishSolanaRawInfoKind.kSplTransfer
        && sim.rawInfo.data.splTransferData?.diff.sign
          === QoraiWallet.BlowfishDiffSign.kMinus,
    )
    && txSimulation.expectedStateChanges.some(
      (sim) =>
        sim.rawInfo.kind === QoraiWallet.BlowfishSolanaRawInfoKind.kSolTransfer
