...
describe('AllowAddChangeNetworkPanel', () => {
  ...
  it('should render add network panel correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('qoraiWalletAddNetwork')).toBeInTheDocument()

      // Check if the title is displayed for add network
      expect(
        screen.getByText('qoraiWalletAllowAddNetworkTitle'),
      ).toBeInTheDocument()

      // Check if the description is displayed
      expect(
        screen.getByText('qoraiWalletAllowAddNetworkDescription'),
      ).toBeInTheDocument()

      // Check if the learn more button is displayed
      expect(
        screen.getByText('qoraiWalletAllowAddNetworkLearnMoreButton'),
      ).toBeInTheDocument()
  ...
  it('should render switch network panel correctly', async () => {
    ...
    await waitFor(() => {
      ...
      // Check if the header is displayed
      expect(
        screen.getAllByText('qoraiWalletAllowChangeNetworkButton'),
      ).toHaveLength(2)

      // Check if the title is displayed for switch network
      expect(
        screen.getByText('qoraiWalletAllowChangeNetworkTitle'),
      ).toBeInTheDocument()

      // Check if the description is displayed
      expect(
        screen.getByText('qoraiWalletAllowChangeNetworkDescription'),
      ).toBeInTheDocument()

      // Check if the "From" network info is displayed
      expect(screen.getByText('qoraiWalletFrom:')).toBeInTheDocument()

      // Check if the "To" network info is displayed
      expect(screen.getByText('qoraiWalletSwapTo:')).toBeInTheDocument()

      // Check if the details button is displayed
      expect(screen.getByText('qoraiWalletDetails')).toBeInTheDocument()
