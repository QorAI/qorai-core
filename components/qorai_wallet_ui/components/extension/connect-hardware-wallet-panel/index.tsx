...
const onClickInstructions = () => {
  const url = 'https://support.qorai.app/hc/en-us/articles/4409309138701'
...
function getAppName(coinType: QoraiWallet.CoinType): string {
  switch (coinType) {
...
export const ConnectHardwareWalletPanel = ({ hardwareWalletCode }: Props) => {
  ...
  const title = React.useMemo(() => {
    ...
    if (hardwareWalletCode === 'deviceBusy') {
      return getLocale('qoraiWalletConnectHardwarePanelConfirmation')
    }
    ...
    if (
      ...
      return getLocale('qoraiWalletConnectHardwarePanelConnect').replace(
        '$1',
        account.name,
      )
    }
    ...
    return getLocale('qoraiWalletConnectHardwarePanelOpenApp')
      .replace('$1', network)
  ...
  React.useEffect(() => {
    ...
    // back to qorai://wallet-panel.top-chrome/ without any
  ...
  return (
    <StyledWrapper>
      <ConnectionRow>
        ...
        <Description>
          {isConnected
            ? getLocale('qoraiWalletConnectHardwarePanelConnected').replace(
                '$1',
                account.name,
              )
            : getLocale('qoraiWalletConnectHardwarePanelDisconnected').replace(
                '$1',
                account.name,
              )}
        </Description>
      ...
      <InstructionsButton onClick={onClickInstructions}>
        {getLocale('qoraiWalletConnectHardwarePanelInstructions')}
      </InstructionsButton>
      <PageIcon />
