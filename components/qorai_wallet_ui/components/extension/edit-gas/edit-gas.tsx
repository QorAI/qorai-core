...
export const EditGas = ({
  ...
  const gasLimitComponent = React.useMemo(
    () => (
      <>
        <InputLabel>{getLocale('qoraiWalletEditGasLimit')}</InputLabel>
        <Input
          placeholder='0'
          type='text'
        ...
        />
        {gasLimit === '0' && (
          <ErrorText>{getLocale('qoraiWalletEditGasLimitError')}</ErrorText>
        )}
      </>
    ),
  ...
  return (
    <Panel
      ...
      title={
        isEIP1559Transaction
          ? getLocale('qoraiWalletEditGasTitle1')
          : getLocale('qoraiWalletEditGasTitle2')
      }
    >
      <StyledWrapper>
        {isEIP1559Transaction && (
          <Description>
            {getLocale('qoraiWalletEditGasDescription')}
          </Description>
        )}
        {showCustomMaxPriorityPanel && (
          <FormColumn>
            <CurrentBaseRow>
              <CurrentBaseText>
                {getLocale('qoraiWalletEditGasBaseFee')}
              </CurrentBaseText>
              <CurrentBaseText>
                {`${new Amount(baseFeePerGas).divideByDecimals(9).format()}
                  ${getLocale('qoraiWalletEditGasGwei')}`}
              </CurrentBaseText>
            </CurrentBaseRow>
            ...
            <InputLabel>
              {getLocale('qoraiWalletEditGasPerTipLimit')}
            </InputLabel>
            <Input
              placeholder='0'
              type='number'
            ...
            {/* Gas-Per-Price Limit */}
            <InputLabel>
              {getLocale('qoraiWalletEditGasPerPriceLimit')}
            </InputLabel>
            <Input
              placeholder='0'
              min={0}
            ...
            <MaximumFeeRow>
              <MaximumFeeText>
                {getLocale('qoraiWalletEditGasMaximumFee')}
              </MaximumFeeText>
              <MaximumFeeText>
        ...
        {showSuggestedMaxPriorityPanel && (
          <SliderWrapper>
            <SliderLabel>{getLocale('qoraiWalletEditGasMaxFee')}:</SliderLabel>
            <SliderValue>
            ...
            />
            <SliderLabelRow>
              <SliderLabel>{getLocale('qoraiWalletEditGasLow')}</SliderLabel>
              <SliderLabel>
                {getLocale('qoraiWalletEditGasOptimal')}
              </SliderLabel>
              <SliderLabel>{getLocale('qoraiWalletEditGasHigh')}</SliderLabel>
            </SliderLabelRow>
          </SliderWrapper>
        )}
        ...
        {!isEIP1559Transaction && (
          <FormColumn>
            ...
            <InputLabel>
              {getLocale('qoraiWalletEditGasPerGasPrice')}
            </InputLabel>
            <Input
              placeholder='0'
              type='number'
            ...
            {isZeroGasPrice && (
              <WarningText>
                {getLocale('qoraiWalletEditGasZeroGasPriceWarning')}
              </WarningText>
            )}
          </FormColumn>
        )}
        ...
        <ButtonRow>
            ...
            text={
              !isEIP1559Transaction
                ...
                : maxPriorityPanel === MaxPriorityPanels.setCustom
                  ? getLocale('qoraiWalletEditGasSetSuggested')
                  : getLocale('qoraiWalletEditGasSetCustom')
            }
            onSubmit={
              !isEIP1559Transaction
                ? onCancel
          ...
          <NavButton
            buttonType='primary'
            text={getLocale('qoraiWalletAccountSettingsSave')}
            onSubmit={onSave}
