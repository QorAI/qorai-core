...
describe('PendingTransactionDetails', () => {
  ...
  describe('BTC/ZEC/Cardano Transactions', () => {
    it('should render BTC transaction inputs and outputs', () => {
      ...
      // Check locale exists
      expect(screen.getByText('qoraiWalletInput:')).toBeInTheDocument()
      expect(screen.getAllByText('qoraiWalletOutput:')).toHaveLength(2)
      expect(screen.getAllByText('qoraiWalletValue:')).toHaveLength(3)
      expect(screen.getAllByText('qoraiWalletAddress:')).toHaveLength(3)
  ...
  describe('Solana Transactions', () => {
    it('should render Solana transaction with send options', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('qoraiWalletSolanaMaxRetries'),
        ).toBeInTheDocument()
      })
  ...
  describe('EVM & FIL Transactions', () => {
    it('should render EVM transaction with function type', () => {
      ...
      expect(
        screen.getByText('qoraiWalletTransactionDetailBoxFunction:'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('qoraiWalletTransactionTypeNameTokenTransfer'),
      ).toBeInTheDocument()
