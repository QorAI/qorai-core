      ...
      <StyledWrapper>
        ...
        {btcZecOrAdaData.outputs?.map((output, index) => {
          return (
            ...
            >
              <DetailColumn>
                <LabelText>{getLocale('qoraiWalletOutput')}:</LabelText>
                <DetailText>{`${index}`}</DetailText>
              </DetailColumn>
              <DetailColumn>
                <LabelText>{getLocale('qoraiWalletValue')}:</LabelText>
  ...
  if (dataArray.length === 0 && !solData) {
    return (
      <StyledWrapper>
        ...
        >
          <NoDataText>
            {getLocale('qoraiWalletConfirmTransactionNoData')}
          </NoDataText>
        </Column>
      </StyledWrapper>
    )
      ...
      >
          ...
          <>
            {!!Number(sendOptions?.maxRetries?.maxRetries) && (
              <DetailColumn key={'maxRetries'}>
                <LabelText>
                  {getLocale('qoraiWalletSolanaMaxRetries')}
                </LabelText>
            ...
            {sendOptions?.preflightCommitment && (
              <DetailColumn key={'preflightCommitment'}>
                <LabelText>
                  {getLocale('qoraiWalletSolanaPreflightCommitment')}
                </LabelText>
            ...
            {sendOptions?.skipPreflight && (
              <DetailColumn key={'skipPreflight'}>
                <LabelText>
                  {getLocale('qoraiWalletSolanaSkipPreflight')}
                </LabelText>
                <DetailText>
        ...
        {(solData || dataArray) && (
          <DetailColumn gap='8px'>
            <DetailColumn>
              <LabelText>
                {getLocale('qoraiWalletTransactionDetailBoxFunction')}:
              </LabelText>
  ...
  if (!type) {
    ...
    return (
      <DetailColumn gap='8px'>
        {programId && (
          <DetailColumn>
            <LabelText>{getLocale('qoraiWalletSolanaProgramID')}</LabelText>
        ...
        {accountMetas.length > 0 && (
          <>
            <DetailColumn>
              <LabelText>{getLocale('qoraiWalletSolanaAccounts')}</LabelText>
        ...
        {data && (
          <>
            <DetailColumn>
              <LabelText>{getLocale('qoraiWalletSolanaData')}</LabelText>
    ...
    <DetailColumn gap='8px'>
      <DetailColumn>
        {lookupTableIndex !== undefined && (
          <LabelText>
            {getLocale('qoraiWalletSolanaAddressLookupTableAccount')}
          </LabelText>
        )}
        <DetailText>{formattedValue}</DetailText>
      </DetailColumn>
      {lookupTableIndex !== undefined && (
        <DetailColumn>
          <LabelText>
            {getLocale('qoraiWalletSolanaAddressLookupTableIndex')}
          </LabelText>
