...
export const TransactionIntent = (props: Props) => {
  ...
  const isSOLSwapOrBridge =
    txCoinType === QoraiWallet.CoinType.SOL && isSwapOrBridge
  ...
  const {
    ...
    txCoinType === QoraiWallet.CoinType.ZEC
      && transactionNetwork
      && transactionsToken
      && txAccount
      && txToAddress
  ...
  const isShieldingFunds =
    getZCashTransactionTypeResult.txType === QoraiWallet.ZCashTxType.kShielding
  ...
  const transactionFailed =
    transaction.txStatus === QoraiWallet.TransactionStatus.Dropped
    || transaction.txStatus === QoraiWallet.TransactionStatus.Error

  const transactionConfirmed =
    transaction.txStatus === QoraiWallet.TransactionStatus.Confirmed
  ...
  const sendSwapOrBridgeLocale = React.useMemo(() => {
    if (isBridge) {
      return getLocale('qoraiWalletBridge').toLocaleLowerCase()
    }
    if (isSwap) {
      return getLocale('qoraiWalletSwap').toLocaleLowerCase()
    }
    return getLocale('qoraiWalletSend').toLocaleLowerCase()
  }, [isBridge, isSwap])

  const swappingOrBridgingLocale = React.useMemo(() => {
    if (isBridge) {
      return getLocale('qoraiWalletBridging')
    }
    return getLocale('qoraiWalletSwapping')
  }, [isBridge])
  ...
  const descriptionLocale = React.useMemo(() => {
    if (isSolanaATACreation && transactionFailed) {
      return 'qoraiWalletFailedToCreateAssociatedTokenAccount'
    }
    if (isSolanaATACreation && transactionConfirmed) {
      return 'qoraiWalletAssociatedTokenAccountCreated'
    }
    if (isSolanaATACreation) {
      return 'qoraiWalletCreatingAssociatedTokenAccount'
    }
    if (transactionFailed && isSOLSwapOrBridge) {
      return 'qoraiWalletErrorAttemptingToTransactOnNetwork'
    }
    if (transactionFailed) {
      return 'qoraiWalletErrorAttemptingToTransact'
    }
    if (isSOLSwapOrBridge) {
      return 'qoraiWalletSwappingOrBridgingOnNetwork'
    }
    if (isSwapOrBridge && transactionConfirmed) {
      return 'qoraiWalletAmountAddedToAccount'
    }
    if (isBridge) {
      return 'qoraiWalletBridgingAmountToNetwork'
    }
    if (isSwap) {
      return 'qoraiWalletSwappingAmountToAmountOnNetwork'
    }
    if (isERC20Approval) {
      return 'qoraiWalletApprovingAmountOnExchange'
    }
    if (transactionConfirmed && isShieldingFunds) {
      return 'qoraiWalletAmountHasBeenShielded'
    }
    if (isShieldingFunds) {
      return 'qoraiWalletShieldingAmount'
    }
    if (transactionConfirmed) {
      return 'qoraiWalletAmountSentToAccount'
    }
    return 'qoraiWalletSendingAmountToAccount'
  }, [
    transactionFailed,
    isSwapOrBridge,
    transactionConfirmed,
