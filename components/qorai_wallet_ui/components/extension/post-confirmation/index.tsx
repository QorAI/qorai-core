...
// Constants
import { QoraiWallet, TransactionInfoLookup } from '../../../constants/types'
...
export function TransactionStatus({ transactionLookup }: Props) {
  ...
  if (
    tx.txStatus === QoraiWallet.TransactionStatus.Submitted
    || tx.txStatus === QoraiWallet.TransactionStatus.Signed
  ) {
    return (
      <TransactionSubmittedOrSigned
        transaction={tx}
  ...
  if (tx.txStatus === QoraiWallet.TransactionStatus.Confirmed) {
    return (
      <TransactionComplete
        transaction={tx}
        onClose={onClose}
  ...
  if (
    tx.txStatus === QoraiWallet.TransactionStatus.Error
    || tx.txStatus === QoraiWallet.TransactionStatus.Dropped
  ) {
    return (
      <TransactionFailedOrCanceled
        transaction={tx}
