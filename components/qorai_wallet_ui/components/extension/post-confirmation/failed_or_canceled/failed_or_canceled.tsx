...
export const TransactionFailedOrCanceled = (props: Props) => {
  ...
  const errorCode =
    ...
    ?? QoraiWallet.ProviderError.kUnknown
  ...
  const sendSwapOrBridgeLocale = React.useMemo(() => {
    if (isBridge) {
      return 'qoraiWalletBridge'
    }
    if (isSwap) {
      return 'qoraiWalletSwap'
    }
    return 'qoraiWalletSend'
  }, [isBridge, isSwap])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Title>
          {isSolanaATACreation
            ? getLocale('qoraiWalletTransactionFailedTitle')
            : getLocale('qoraiWalletUnableToSendSwapOrBridge').replace(
                '$1',
      ...
      >
        ...
        <QoraButton onClick={onClickRetryTransaction}>
          {getLocale('qoraiWalletButtonRetry')}
        </QoraButton>
      </Row>
    </Wrapper>
  )
