...
export const ConnectPanelButton = styled(WalletButton)<{
  ...
  padding: 8px 0px;
  border-top: ${(p) =>
    p.border === 'top' ? `1px solid ${qora.color.divider.subtle}` : 'none'};
  border-bottom: ${(p) =>
    p.border === 'bottom' ? `1px solid ${qora.color.divider.subtle}` : 'none'};
`
...
export const SelectedIcon = styled(Icon)<{ isSelected: boolean }>`
  --qora-icon-size: 20px;
  color: ${(p) =>
    p.isSelected ? qora.color.primary[40] : qora.color.icon.default};
`
