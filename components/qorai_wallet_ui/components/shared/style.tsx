...
// types
import { QoraiWallet, StringWithAutocomplete } from '../../constants/types'
import IThemeProps from 'qorai-ui/theme/theme-interface'
...
// graphics
import QoraiWalletWithCoins from '../../assets/svg-icons/onboarding/qorai-wallet-with-coins.svg'
...
export const StatusBubble = styled.div<{
  ...
  background-color: ${(p) =>
    ...
    || p.status === QoraiWallet.TransactionStatus.Approved
      ...
      : p.status === QoraiWallet.TransactionStatus.Rejected
...
export const WarningTriangleFilledIcon = styled.div<{
  ...
  mask-size: 100%;
  background-color: ${(p) =>
    p?.color ? p.theme.color[p.color] : qora.color.systemfeedback.warningIcon};
...
export const WarningCircleFilledIcon = styled.div<{
  ...
  mask-repeat: no-repeat;
  background-color: ${(p) =>
    p?.color ? p.theme.color[p.color] : qora.color.systemfeedback.errorIcon};
...
export const WalletWelcomeGraphic = styled.div<{
  ...
  width: 350px;
  height: 264px;
  background: url(${QoraiWalletWithCoins});
...
export const QoraiRewardsIndicator = styled.div`
  display: flex;
  align-items: center;
  justify-content: center;
  font-family: Poppins;
...
export const QoraSquaredButton = styled(Button)`
...
export const LoadingRing = styled(ProgressRing)<{ size?: string }>`
  --qora-progressring-size: ${(p) => p?.size || '40px'};
`
