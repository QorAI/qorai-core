...
import { Column, QoraSquaredButton, Row } from '../style'
...
export const AddNftForm = (props: Props) => {
  ...
  const {
    ...
    customAssetsNetwork
      ...
      && (customAssetsNetwork.coin === QoraiWallet.CoinType.ETH
        ? !!customTokenID
        : true)
      ? {
          contractAddress: tokenContractAddress,
  ...
  const metadataLookupArg: GetBlockchainTokenIdArg | undefined =
    React.useMemo(() => {
      ...
      return {
        ...
        contractAddress: tokenContractAddress,
        tokenId:
          customAssetsNetwork.coin !== QoraiWallet.CoinType.SOL && customTokenID
            ? new Amount(customTokenID).toHex()
            : '',
        isErc721:
          !!customTokenID
          && customAssetsNetwork.coin !== QoraiWallet.CoinType.SOL,
        isNft: true,
        isShielded: false,
      }
  ...
  const {
    ...
    metadataLookupArg
      && (metadataLookupArg.coin === QoraiWallet.CoinType.SOL
  ...
  const tokenIdError =
    selectedAssetNetwork?.coin === QoraiWallet.CoinType.ETH && !customTokenID
  ...
  const formErrors = React.useMemo(() => {
    return [
      tokenContractAddressError
        && getLocale('qoraiWalletInvalidTokenContractAddressError'),
      customAssetsNetworkError
        && getLocale('qoraiWalletNetworkIsRequiredError'),
      tokenNameError && getLocale('qoraiWalletTokenNameIsRequiredError'),
      tokenSymbolError && getLocale('qoraiWalletTokenSymbolIsRequiredError'),
      tokenIdError && getLocale('qoraiWalletWatchListTokenIdError'),
    ]
  }, [
    tokenContractAddressError,
    customAssetsNetworkError,
  ...
  const metadataAssetInfo:
    | (GetBlockchainTokenIdArg
        & Pick<QoraiWallet.BlockchainToken, 'logo' | 'name' | 'symbol'>)
    | undefined = React.useMemo(() => {
    return metadataLookupArg
      && !isFetchingNftMetadata
      && (metadataLookupArg?.coin === QoraiWallet.CoinType.SOL || customTokenID)
      ? {
          ...metadataLookupArg,
          logo:
  ...
  const addOrUpdateToken = React.useCallback(async () => {
    ...
    const updatedToken: QoraiWallet.BlockchainToken = {
      name: customTokenName,
      symbol: customTokenSymbol,
      decimals: 0,
      coingeckoId: '',
      ...
      isErc20: false,
      isErc1155: false,
      splTokenProgram: QoraiWallet.SPLTokenProgram.kUnknown,
      isSpam: false,
      visible: true,
      chainId: customAssetsNetwork.chainId,
      coin: customAssetsNetwork.coin,
    ...
    <>
      <FormWrapper onClick={onHideNetworkDropDown}>
        {!selectedAsset && (
          <FullWidthFormColumn>
            <DescriptionRow>
              {getLocale('qoraiWalletAddNftModalDescription')}
            </DescriptionRow>
          </FullWidthFormColumn>
        )}
        ...
        <FullWidthFormColumn>
          <Input
            ...
            placeholder={getLocale('qoraiWalletExempliGratia').replace(
              '$1',
          ...
          >
            ...
            >
              <InputLabel>
                {customAssetsNetwork?.coin === QoraiWallet.CoinType.SOL
                  ? getLocale('qoraiWalletTokenMintAddress')
                  : getLocale('qoraiWalletNFTDetailContractAddress')}
              </InputLabel>
              <InfoIconTooltip
                placement='bottom'
                text={getLocale('qoraiWalletWhatIsAnNftContractAddress')}
              />
            </Row>
          </Input>
        </FullWidthFormColumn>
          ...
          && customAssetsNetwork?.coin !== QoraiWallet.CoinType.SOL && (
            <FullWidthFormColumn>
              ...
              >
                ...
                >
                  <InputLabel>
                    {getLocale('qoraiWalletNFTDetailTokenID')}
                  </InputLabel>
                  <InfoIconTooltip
                    placement='bottom'
                    text={getLocale('qoraiWalletWhatIsAnNftTokenId')}
                  />
                </Row>
              </Input>
            </FullWidthFormColumn>
        ...
        <FullWidthFormColumn>
          ...
          >
            ...
            >
              <InputLabel>
                {getLocale('qoraiWalletWatchListTokenName')}
              </InputLabel>
              <InfoIconTooltip
                placement='bottom'
                text={getLocale('qoraiWalletNftNameFieldExplanation')}
              />
            </Row>
        ...
        <FullWidthFormColumn>
          ...
          >
            ...
            >
              <InputLabel>
                {getLocale('qoraiWalletWatchListTokenSymbol')}
              </InputLabel>
              <InfoIconTooltip
                placement='bottom'
                text={getLocale('qoraiWalletNftSymbolFieldExplanation')}
              />
            </Row>
            <div slot='left-icon'>
              {(isFetchingNftMetadata || isTokenInfoLoading) && (
        ...
        {metadataLookupArg?.coin === QoraiWallet.CoinType.ETH
          && !metadataLookupArg?.tokenId && (
            <ErrorText>
              {getLocale('qoraiWalletWatchListTokenIdError')}
            </ErrorText>
          )}

        {hasError ? (
          <ErrorText>{getLocale('qoraiWalletWatchListError')}</ErrorText>
        ) : (
          <Column
            fullWidth
            alignItems='center'
            ...
            {isFetchingNftMetadata ? (
              ...
              <>
                {hasNftMetadataError ? (
                  <Column>
                    <ErrorText>
                      {getLocale('qoraiWalletFetchNftMetadataError')}
                    </ErrorText>
                  </Column>
                ) : (
                  <>
                    {metadataAssetInfo && (
                      ...
                      >
                        ...
                        {!userOwnsNft && (
                          <Alert type='info'>
                            {getLocale('qoraiWalletUnownedNftAlert')}
                          </Alert>
                        )}
                      </Column>
                    )}
      ...
      <ButtonRow gap='16px'>
        ...
        >
          <Row>
            ...
            >
              {!userOwnsNft
                ? getLocale('qoraiWalletWatchThisNft')
                : selectedAsset
                  ? getLocale('qoraiWalletButtonSaveChanges')
                  : getLocale('qoraiWalletWatchListAdd')}
            </QoraSquaredButton>
          </Row>
        </Tooltip>
      </ButtonRow>
