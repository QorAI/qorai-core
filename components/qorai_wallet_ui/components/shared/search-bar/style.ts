...
export const StyledWrapper = styled.div<{
  ...
  --background-color: ${(p) =>
    p.isV2 ? qora.color.container.highlight : p.theme.color.background02};
  --font-size: ${(p) =>
    p.isV2 ? qora.color.container.highlight : p.theme.color.background02};
  display: flex;
  align-items: center;
...
export const SearchInput = styled.input<{
  ...
  ::placeholder {
    ...
    letter-spacing: 0.01em;
    color: ${(p) =>
      p.isV2 ? qora.color.text.tertiary : qora.color.text.secondary};
  }
  :focus {
    outline: none;
  }
...
export const SearchIcon = styled(Icon)<{
  ...
  --qora-icon-size: ${(p) => (p.isV2 ? 20 : 15)}px;
