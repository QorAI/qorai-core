...
export const Input = styled.input<{ hasError: boolean }>`
  ...
  border: ${(p) =>
    p.hasError
      ? `2px solid ${qora.color.systemfeedback.errorText}`
  ...
  :focus-visible {
    ...
    border: ${(p) =>
      p.hasError
        ? `2px solid ${qora.color.systemfeedback.errorText}`
        : `2px solid ${qora.color.text.interactive}`};
  }
...
export const ErrorIcon = styled(QoraIcon).attrs({
...
export const ToggleVisibilityIcon = styled(QoraIcon)`
  --qora-icon-size: 20px;
