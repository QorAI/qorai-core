...
export type MarketGridProps = {
  ...
  coinMarketData: QoraiWallet.CoinMarket[]
  visibleRows?: number
  rowHeight?: number
  ...
  onSelectCoinMarket: (coinMarket: QoraiWallet.CoinMarket) => void
  isBuySupported: (coinMarket: QoraiWallet.CoinMarket) => boolean
  isDepositSupported: (coinMarket: QoraiWallet.CoinMarket) => boolean
  onClickBuy: (coinMarket: QoraiWallet.CoinMarket) => void
  onClickDeposit: (coinMarket: QoraiWallet.CoinMarket) => void
...
export const MarketGrid = ({
  ...
  const rows: MarketGridRow[] = React.useMemo(() => {
    return coinMarketData.map((coinMarketItem: QoraiWallet.CoinMarket) => {
  ...
  return (
    ...
    >
        ...
        {showEmptyState ? (
          <Row margin='30px 0px'>
            <EmptyStateText
              ...
              textSize='14px'
            >
              {getLocale('qoraiWalletMarketDataNoAssetsFound')}
            </EmptyStateText>
          </Row>
        ) : (
          <FixedSizeList
      ...
      <CoinGeckoText>
        {getLocale('qoraiWalletPoweredByCoinGecko')}
      </CoinGeckoText>
    </StyledWrapper>
  )
}
