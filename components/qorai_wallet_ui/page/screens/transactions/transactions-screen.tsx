...
// utils
import { getLocale } from 'qorai-ui'
...
export const TransactionsScreen = (props: Props) => {
  ...
  const { address, chainId, chainCoinType } = React.useMemo(() => {
    ...
    return {
      ...
      chainCoinType:
        Number(searchParams.get('chainCoinType')) || QoraiWallet.CoinType.ETH,
    }
  ...
  const onClickTransaction = React.useCallback(
    (
      tx: Pick<QoraiWallet.TransactionInfo | SerializableTransactionInfo, 'id'>,
    ): void => {
      history.push(
