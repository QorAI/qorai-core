      ...
      >
        <WalletPageWrapper
          ...
          cardHeader={
            isMobileOrPanel ? (
              <PanelActionHeader
                title={getLocale('qoraiWalletDepositCryptoButton')}
...
function AssetSelection() {
  ...
  const { nftCollectionAssets, nftCollectionAssetsIds } = React.useMemo(() => {
    const nftCollectionAssets: QoraiWallet.BlockchainToken[] = []
  ...
  const fullAssetsList: QoraiWallet.BlockchainToken[] = React.useMemo(() => {
    // separate QOR from other tokens in the list so they can be placed higher
    // in the list
    const sortedFungibleAssets = sortNativeAndAndQorAssetsToTop(
      tokensList,
  ...
  const renderToken = React.useCallback<
    RenderTokenFunc<QoraiWallet.BlockchainToken>
  >(
    ({ item: asset, ref }) => {
...
function DepositAccount() {
  ...
  const depositTitleText: string = React.useMemo(() => {
    const isSolOrSpl = selectedAsset?.coin === QoraiWallet.CoinType.SOL
    const isErc = selectedAsset?.isErc20 || selectedAsset?.isErc721

    // ERC-based tokens
    if (isErc) {
      return getLocale('qoraiWalletDepositErc')
    }

    // Solana assets
    if (isSolOrSpl) {
      return getLocale('qoraiWalletDepositSolSplTokens')
    }

    return getLocale('qoraiWalletDepositX').replace(
      '$1',
    ...
    >
      <Column alignItems='flex-start'>
        ...
        {selectedAssetNetwork && (
          <Description>
            {getLocale('qoraiWalletDepositOnlySendOnXNetwork').replace(
              '$1',
