...
export const CurrencyListItem = ({
  ...
  return (
    <ContainerButton onClick={() => onSelect(currency)}>
      ...
      >
        {isSelected && (
          <SelectedLabel>{getLocale('qoraiWalletSelected')}</SelectedLabel>
        )}
    ...
    return (
      <>
        <DialogTitle slot='title'>
          {getLocale('qoraiWalletSelectCurrency')}
        </DialogTitle>
        <Row
          padding='24px 0 0 0'
          slot='subtitle'
        >
          <SearchInput
            placeholder={getLocale('qoraiWalletSearchCurrency')}
        ...
        >
          {searchResults.length === 0 ? (
            <Row
              ...
              alignItems='center'
            >
              {getLocale('qoraiWalletNoAvailableCurrencies')}
            </Row>
          ) : (
            <ScrollableColumn>
