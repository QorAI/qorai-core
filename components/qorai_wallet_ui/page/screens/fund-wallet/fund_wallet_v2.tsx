...
export const FundWalletScreen = () => {
  ...
  const pageTitle = getLocale('qoraiWalletBuyAsset').replace(
    '$1',
      ...
      >
        ...
        >
          <ControlPanel width='100%'>
            <SelectAssetButton
              labelText={getLocale('qoraiWalletAsset')}
              selectedAsset={selectedMeldAsset}
              onClick={() => setIsAssetDialogOpen(true)}
            />
            <SelectAccountButton
              labelText={getLocale('qoraiWalletSubviewAccount')}
              selectedAccount={selectedAccount}
              onClick={() => setIsAccountDialogOpen(true)}
            />
            <AmountButton
              labelText={getLocale('qoraiWalletSwapFrom')}
              currencyCode={
          ...
          >
            ...
            {isFetchingFirstTimeQuotes ? (
              ...
              >
                <Loader />
                <LoaderText>
                  {getLocale('qoraiWalletGettingBestPrices')}
                </LoaderText>
              </Column>
            ) : (
              <>
                {hasQuoteError ? (
                  ...
                  >
                    ...
                    <Text
                      ...
                      isBold={true}
                    >
                      {getLocale('qoraiWalletNoProviderFound').replace(
                        '$1',
                        getAssetSymbol(selectedMeldAsset),
                      )}
                    </Text>
                    <Text
                      ...
                      isBold={false}
                    >
                      {getLocale('qoraiWalletTrySearchingForDifferentAsset')}
                    </Text>
                  </Column>
                ) : (
                  <>
                    {searchTerm !== '' && filteredQuotes.length === 0 ? (
                      ...
                      >
                        ...
                        <Text
                          ...
                          isBold={true}
                        >
                          {getLocale('qoraiWalletNoResultsFound').replace(
                            '$1',
                            searchTerm,
                          )}
                        </Text>
                        <Text
                          ...
                          isBold={false}
                        >
                          {getLocale('qoraiWalletTryDifferentKeywords')}
                        </Text>
                      </Column>
                    ) : (
                      <Column
