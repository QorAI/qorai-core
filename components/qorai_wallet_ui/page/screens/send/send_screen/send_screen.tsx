...
export const SendScreen = React.memo(() => {
  ...
  const {
    ...
    networkFromParams?.coin === QoraiWallet.CoinType.ZEC
      && accountFromParams
      && toAddressOrUrl
      ? {
  ...
  const isShieldingFunds =
    tokenFromParams
    && tokenFromParams.coin === QoraiWallet.CoinType.ZEC
    && getZCashTransactionTypeResult.txType
      === QoraiWallet.ZCashTxType.kShielding
  ...
  const selectSendAsset = React.useCallback(
    (asset: QoraiWallet.BlockchainToken, account?: QoraiWallet.AccountInfo) => {
      ...
      case QoraiWallet.CoinType.ETH: {
        ...
        if (
          (tokenFromParams.chainId
            === QoraiWallet.FILECOIN_ETHEREUM_MAINNET_CHAIN_ID
            || tokenFromParams.chainId
    ...
    <>
      <WalletPageWrapper
        ...
        cardHeader={
          isMobileOrPanel ? (
            <PanelActionHeader
              title={getLocale('qoraiWalletSend')}
          ...
          >
            ...
            >
              ...
              >
                ...
                {tokenFromParams?.coin === QoraiWallet.CoinType.BTC && (
                  <OrdinalsWarningMessage
                ...
                {isZCashShieldedTransactionsEnabled
                  ...
                  && toAddressOrUrl
                  && (getZCashTransactionTypeResult.txType
                    === QoraiWallet.ZCashTxType.kTransparentToOrchard
                    || getZCashTransactionTypeResult.txType
                      === QoraiWallet.ZCashTxType.kOrchardToOrchard) && (
                    <AddMemo
                      memoText={memoText}
                ...
                {isShieldingFunds && (
                  ...
                  >
                    <ShieldingFundsAlert type='info'>
                      {getLocale('qoraiWalletShieldingFundsAlertDescription')}
                    </ShieldingFundsAlert>
                  </Row>
                )}
              </Column>
...
function ethToWeiAmount(
  sendAmount: string,
  selectedSendAsset: QoraiWallet.BlockchainToken,
): Amount {
...
function getReviewButtonText(
  ...
  if (sendAmountValidationError) {
    return 'qoraiWalletDecimalPlacesError'
  }
  if (insufficientFundsError) {
    return 'qoraiWalletNotEnoughFunds'
  }
  if (isAccountSyncing) {
    return 'qoraiWalletAccountIsSyncing'
  }
  if (isShieldingFunds) {
    return 'qoraiWalletReviewShield'
  }

  return 'qoraiWalletReviewSend'
}
