...
import Checkbox from '@qorai/qora/react/checkbox'
...
export const OnboardingNetworkSelection = () => {
  ...
  const { featuredNetworks, popularNetworks } = React.useMemo(() => {
    ...
    const featuredNetworks = featuredChainIds
      ...
      .filter((net): net is QoraiWallet.NetworkInfo => Boolean(net))
  ...
  const onSubmit = React.useCallback(async () => {
    ...
    > = []
    const hiddenNets: Array<Pick<QoraiWallet.NetworkInfo, 'chainId' | 'coin'>> =
      []
  ...
  return (
    <OnboardingContentLayout
      title={getLocale('qoraiWalletSupportedNetworks')}
      subTitle={getLocale('qoraiWalletChooseChainsToUse')}
      padding='44px 0 0'
    >
      <Column
        fullWidth
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {getLocale('qoraiWalletShowTestnets')}
            </Text>
          </Checkbox>
        </Row>
          ...
          <ChainSelectionContextProvider value={selectedChainsContext}>
            {isLoadingNetworks ? (
              ...
              <>
                ...
                >
                  <GroupingText>{getLocale('qoraiWalletPopular')}</GroupingText>
                  {networks && (
                    <SelectAllText
                      onClick={() => {
                    ...
                    >
                      {getLocale(
                        areAllChainsSelected
                          ? 'qoraiWalletDeselectAll'
                          : 'qoraiWalletSelectAll',
                      )}
                    </SelectAllText>
                  )}
                </Row>
      ...
      <NextButtonRow>
        ...
        >
          {visibleSelectedChainIds.length
            ? getLocale('qoraiWalletContinueWithXItems')
                .replace(
                  '$1', // Number of items
                ...
                .replace(
                  ...
                  visibleSelectedChainIds.length > 1
                    ? getLocale('qoraiWalletNetworks')
                    : getLocale('qoraiWalletAllowAddNetworkNetworkPanelTitle'),
                )
            : getLocale('qoraiWalletButtonContinue')}
        </ContinueButton>
      </NextButtonRow>
      <MutedLinkText onClick={openNetworkSettings}>
        {getLocale('qoraiWalletAddNetworksAnytimeInSettings')}
      </MutedLinkText>
    </OnboardingContentLayout>
  )
}
