...
export const ToSectionWrapper = styled(Column)<{
  ...
  --default-background: ${qora.color.container.interactive};
  @media (prefers-color-scheme: dark) {
    --default-background: ${qora.color.container.highlight};
  }
...
export const AmountInput = styled(Input)<{
  ...
  color: ${(p) =>
    p.hasError ? qora.color.systemfeedback.errorText : qora.color.text.primary};
  font-weight: 500;
  font-size: 32px;
  line-height: 42px;
  text-align: right;
...
export const PresetButton = styled(WalletButton)`
  outline: none;
  border: none;
  background-color: ${qora.color.neutral[10]};
  border-radius: 4px;
  font-family: Poppins;
  font-size: 10px;
  font-style: normal;
...
export const Button = styled(WalletButton)<{
  ...
  padding: 10px 0px;
  color: ${(p) =>
    p.isPlaceholder ? qora.color.text.tertiary : qora.color.text.primary};
  white-space: nowrap;
  :disabled {
    cursor: not-allowed;
  }
...
export const AlertMessageButton = styled(QoraButton)`
  --qora-button-padding: 0px;
`
