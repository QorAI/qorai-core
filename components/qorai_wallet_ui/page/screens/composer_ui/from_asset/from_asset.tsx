...
interface Props {
  ...
  network: QoraiWallet.NetworkInfo | undefined
...
export const FromAsset = (props: Props) => {
  ...
  const { data: bitcoinBalances, isLoading: isLoadingBitcoinBalances } =
    useGetBitcoinBalancesQuery(
      token && token?.coin === QoraiWallet.CoinType.BTC && account?.accountId
        ? account.accountId
        : skipToken,
    )
  ...
  const tokenBalance = React.useMemo(() => {
    ...
    if (token.coin === QoraiWallet.CoinType.BTC && bitcoinBalances) {
        ...
        >
          {isLoadingBalances || isLoadingBitcoinBalances ? (
            ...
            <>
              ...
              {token.coin === QoraiWallet.CoinType.BTC && hasPendingBalance && (
                <>
                  <BalanceText
                    textSize='14px'
                    isBold={true}
                    textColor='primary'
                  >
                    {`(${getLocale('qoraiWalletAvailable')})`}
                  </BalanceText>
                  <div>
                    <Button
                      kind='plain'
  ...
  }, [spotPrices, token, inputValue, defaultFiatCurrency])
          ...
          {token && !token.isNft && account !== undefined && (
            <Row width='unset'>
              <PresetButton onClick={() => setPresetAmountValue(0.5)}>
                {getLocale('qoraiWalletSendHalf')}
              </PresetButton>
              <HorizontalSpace space='8px' />
              <PresetButton onClick={() => setPresetAmountValue(1)}>
                {getLocale('qoraiWalletSendMax')}
              </PresetButton>
            </Row>
          )}
        </AccountNameAndPresetsRow>
        ...
        >
          <Row width='unset'>
            <SelectButton
              ...
              placeholderText={getLocale('qoraiWalletChooseAsset')}
            />
          </Row>
          {!token?.isNft && (
            <AmountInput
        ...
        >
          {network && token && (
            <NetworkText
              ...
              textColor='secondary'
            >
              {getLocale('qoraiWalletPortfolioAssetNetworkDescription')
                .replace('$1', '')
