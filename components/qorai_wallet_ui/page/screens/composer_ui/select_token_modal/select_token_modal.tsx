...
interface Props {
  ...
  selectedFromToken?: QoraiWallet.BlockchainToken
  selectedToToken?: QoraiWallet.BlockchainToken
...
export const SelectTokenModal = React.forwardRef<HTMLDivElement, Props>(
  (props: Props, forwardedRef) => {
    ...
    const [selectedNetworkFilter, setSelectedNetworkFilter] =
      React.useState<QoraiWallet.NetworkInfo>(
        selectedNetwork || AllNetworksOption,
      )
    const [selectedAccountFilter, setSelectedAccountFilter] =
      React.useState<QoraiWallet.AccountInfo>(AllAccountsOption)
    const [pendingSelectedAssetState, setPendingSelectedAssetState] =
      React.useState<QoraiWallet.BlockchainToken | undefined>(undefined)
    ...
    const checkIsAccountOptionDisabled = React.useCallback(
      (account: QoraiWallet.AccountInfo) => {
        ...
        if (
          ...
          return (
            account.accountId.coin === QoraiWallet.CoinType.SOL
            && selectedFromToken.coin === QoraiWallet.CoinType.SOL
          )
        }
        if (
          modalType === 'bridge'
          ...
          return (
            account.accountId.coin === QoraiWallet.CoinType.SOL
            && selectedToToken.coin === QoraiWallet.CoinType.SOL
          )
        }
        return false
      },
      ...
      if (emptyTokensList) {
        return (
          <NoAssetsText
            ...
            isBold={false}
          >
            {getLocale('qoraiWalletNoAvailableTokens')}
          </NoAssetsText>
        )
      }
    ...
    const swapSupportedAccounts = React.useMemo(() => {
      return accounts.filter(
        (account) =>
          account.accountId.coin === QoraiWallet.CoinType.ETH
          || account.accountId.coin === QoraiWallet.CoinType.SOL,
      )
    }, [accounts])
    ...
    return (
      <>
        <PopupModal
          ...
          title={getLocale(
            modalType === 'swap'
              ? 'qoraiWalletChooseAssetToSwap'
              : modalType === 'bridge'
                ? 'qoraiWalletChooseAssetToBridge'
                : 'qoraiWalletChooseAssetToSend',
          )}
          width='560px'
          height='90vh'
            ...
            <SearchInput
              ...
              placeholder={
                selectedSendOption === SendPageTabHashes.nft
                  ? getLocale('qoraiWalletSearchNFTs')
                  : getLocale('qoraiWalletSearchTokens')
              }
              type='text'
            >
              <div slot='left-icon'>
