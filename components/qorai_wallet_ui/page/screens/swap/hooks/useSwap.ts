...
export const useSwap = () => {
  ...
  const [quoteUnion, setQuoteUnion] = useState<
    QoraiWallet.SwapQuoteUnion | undefined
  >(undefined)
  const [quoteErrorUnion, setQuoteErrorUnion] = useState<
    QoraiWallet.SwapErrorUnion | undefined
  >(undefined)
  ...
  const [swapFees, setSwapFees] = useState<QoraiWallet.SwapFees | undefined>(
    undefined,
  )
  ...
  const [selectedProvider, setSelectedProvider] =
    useState<QoraiWallet.SwapProvider>(QoraiWallet.SwapProvider.kAuto)
    ...
    async (overrides: SwapParamsOverrides) => {
      ...
      try {
        quoteResponse = await generateSwapQuote({
          ...
          routePriority: QoraiWallet.RoutePriority.kCheapest,
  ...
  const availableProvidersForSwap = useMemo(() => {
    ...
    if (!fromToken && !toToken) {
      return [QoraiWallet.SwapProvider.kAuto]
    }

    const hasSolInFillPath =
      fromToken?.coin === QoraiWallet.CoinType.SOL
      || toToken?.coin === QoraiWallet.CoinType.SOL

    const hasEthInFillPath =
      fromToken?.coin === QoraiWallet.CoinType.ETH
      || toToken?.coin === QoraiWallet.CoinType.ETH

    if (!isBridge && hasSolInFillPath) {
      return [QoraiWallet.SwapProvider.kAuto, QoraiWallet.SwapProvider.kJupiter]
    }
    ...
    if (isBridge && hasSolInFillPath) {
      return [QoraiWallet.SwapProvider.kAuto, QoraiWallet.SwapProvider.kLiFi]
    }
  ...
  const swapValidationError: SwapValidationErrorType | undefined =
    useMemo(() => {
      ...
      if (
        ...
        && quoteErrorUnion?.lifiError?.code
          !== QoraiWallet.LiFiErrorCode.kSuccess
      ) {
        return quoteErrorUnion?.lifiError.code
          === QoraiWallet.LiFiErrorCode.kNotFoundError
  ...
  const onChangeSwapProvider = useCallback(
    async (provider: QoraiWallet.SwapProvider) => {
  ...
  const submitButtonText = useMemo(() => {
    if (isFetchingQuote) {
      return getLocale('qoraiWalletFetchingQuote')
    }

    const defaultText = isBridge
      ? getLocale('qoraiWalletReviewBridge')
      : getLocale('qoraiWalletReviewSwap')
    ...
    if (swapValidationError === 'insufficientBalance') {
      return getLocale('qoraiSwapInsufficientBalance').replace(
        '$1',
        fromToken.symbol,
      )
    }
    ...
    if (
      fromNetwork.coin === QoraiWallet.CoinType.ETH
      && swapValidationError === 'insufficientAllowance'
    ) {
      return getLocale('qoraiSwapApproveToken').replace('$1', fromToken.symbol)
    }

    if (swapValidationError === 'insufficientLiquidity') {
      return getLocale('qoraiSwapInsufficientLiquidity')
    }

    if (swapValidationError === 'unknownError') {
      return getLocale('qoraiWalletSwapUnknownError')
    }
  ...
  const isSubmitButtonDisabled = useMemo(() => {
    return (
      ...
      || (swapValidationError
        && fromNetwork.coin === QoraiWallet.CoinType.ETH
        && swapValidationError !== 'insufficientAllowance')
      || (swapValidationError && fromNetwork.coin === QoraiWallet.CoinType.SOL)
    )
  }, [
    fromNetwork,
    fromAccount,
  ...
  useEffect(() => {
    ...
    if (!fromToken && !toToken) {
      ...
      setToAmount('')
      reset()
      setSelectedProvider(QoraiWallet.SwapProvider.kAuto)
    }
