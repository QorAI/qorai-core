    ...
    >
      ...
      >
        <Text
          ...
          textAlign='left'
        >
          {step.type === QoraiWallet.LiFiStepType.kSwap
            ? getLocale('qoraiWalletSwapOnNetworkViaExchange')
                .replace('$1', fromNetwork?.chainName ?? '')
                .replace('$2', toolDetails.name)
            : getLocale('qoraiWalletBridgeFromNetworkToNetwork')
                .replace('$1', fromNetwork?.chainName ?? '')
