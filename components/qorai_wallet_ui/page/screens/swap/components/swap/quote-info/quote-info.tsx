...
interface Props {
  fromToken: QoraiWallet.BlockchainToken | undefined
  toToken: QoraiWallet.BlockchainToken | undefined
  isBridge: boolean
  ...
  toAccount?: QoraiWallet.AccountInfo
  swapFees?: QoraiWallet.SwapFees
}
...
export const QuoteInfo = (props: Props) => {
  ...
  const swapRate: string = React.useMemo(() => {
    ...
    return getLocale('qoraiWalletExchangeFor')
  ...
  const coinGeckoDeltaText: string = React.useMemo(() => {
    if (coinGeckoDelta.gte(0)) {
      return getLocale('qoraiSwapCoinGeckoCheaper').replace(
        '$1',
    ...
    if (coinGeckoDelta.gte(-1)) {
      return getLocale('qoraiSwapCoinGeckoWithin').replace(
        '$1',
        coinGeckoDelta.times(-1).format(2),
      )
    }

    return getLocale('qoraiSwapCoinGeckoExpensive').replace(
      '$1',
      coinGeckoDelta.times(-1).format(2),
    )
  }, [coinGeckoDelta])
  ...
  const qoraiFee: QoraiWallet.SwapFees | undefined = React.useMemo(() => {
    if (!swapFees) {
      return undefined
    }
  ...
  const effectiveFeeAmount = qoraiFee && new Amount(qoraiFee.effectiveFeePct)
  const firstStep =
    selectedQuoteOption?.sources.find((source) =>
      source.includedSteps?.some(
        (step) => step.type === QoraiWallet.LiFiStepType.kCross,
      ),
      ...
      >
        {minimumReceived !== '' && (
          <Row justifyContent='space-between'>
            <Text
              ...
              textAlign='left'
            >
              {getLocale('qoraiSwapMinimumReceivedAfterSlippage')}
            </Text>
            <Text
              textSize='12px'
              isBold={true}
        ...
        <Row justifyContent='space-between'>
          <Text
            ...
            textAlign='left'
          >
            {getLocale('qoraiWalletMaxSlippage')}
          </Text>
          <Row width='unset'>
            <Text
              textSize='12px'
            ...
            {!isBridge && (
              <Row justifyContent='space-between'>
                <Text
                  ...
                  textColor='secondary'
                >
                  {getLocale('qoraiWalletExchangeRate')}
                </Text>
                <Text
                  textSize='12px'
                  isBold={true}
            ...
            <Row justifyContent='space-between'>
              ...
              >
                <Text
                  ...
                  textAlign='left'
                >
                  {getLocale('qoraiSwapPriceImpact')}
                </Text>
                <InfoIconTooltip
                  placement='right'
                  text={getLocale('qoraiWalletPriceImpactDescription')}
            ...
            {isBridge && (
              <Row justifyContent='space-between'>
                <Text
                  ...
                  textAlign='left'
                >
                  {getLocale('qoraiWalletRecipient')}
                </Text>
                  ...
                  {effectiveFeeAmount && effectiveFeeAmount.isZero() ? (
                    <Bubble padding='5px 6px'>
                      <FreeText
                        ...
                        isBold={true}
                      >
                        {getLocale('qoraiSwapFree')}
                      </FreeText>
                    </Bubble>
                  ) : (
                    <Text textSize='14px'>{qoraiFee.effectiveFeePct}%</Text>
                  )}

                  {qoraiFee.discountCode
                    !== QoraiWallet.SwapDiscountCode.kNone && (
                    <>
                      <QoraiFeeDiscounted
                        textSize='12px'
                        isBold={true}
