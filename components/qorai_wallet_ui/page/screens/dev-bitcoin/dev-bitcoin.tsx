...
const GetBalanceSection = (props: GetBalanceSectionProps) => {
  ...
  const [balance, setBalance] = useState<
    QoraiWallet.BitcoinBalance | undefined
  >()
...
> = ({ accountId }) => {
  ...
  const [bitcoinAccountInfo, setBitcoinAccountInfo] = useState<
    QoraiWallet.BitcoinAccountInfo | undefined
  >()
  ...
  const keyId = (keyId: QoraiWallet.BitcoinKeyId | undefined) => {
    if (!keyId) {
      return '-'
    }
...
interface AccountSectionProps {
  accountInfo: QoraiWallet.AccountInfo
}
...
export const DevBitcoin = () => {
  const [accounts, setAccounts] = useState<QoraiWallet.AccountInfo[]>([])
    ...
    const fetchBitcoinAccount = async () => {
      ...
      setAccounts(
        allAccounts.accounts.filter(
          (acc) => acc.accountId.coin === QoraiWallet.CoinType.BTC,
        ),
      )
    }
