...
export function useHasAccount() {
  ...
  const hasSolAccount = React.useMemo((): boolean => {
    return accounts.some(
      (account) => account.accountId.coin === QoraiWallet.CoinType.SOL,
    )
  }, [accounts])
  const hasFilAccount = React.useMemo((): boolean => {
    const keyringForCurrentNetwork =
      selectedNetwork?.chainId === QoraiWallet.FILECOIN_MAINNET
        ? QoraiWallet.KeyringId.kFilecoin
        : QoraiWallet.KeyringId.kFilecoinTestnet
    return accounts.some(
      (account) =>
        account.accountId.coin === QoraiWallet.CoinType.FIL
