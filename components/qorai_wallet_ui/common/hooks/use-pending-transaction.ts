...
export const usePendingTransactions = () => {
  ...
  const {
    ...
    transactionsNetwork
      && transactionsNetwork.coin === QoraiWallet.CoinType.ETH
      && transactionInfo
  ...
  const gasFee = React.useMemo(() => {
    ...
    return txCoinType === QoraiWallet.CoinType.SOL
      ? (solFeeEstimate ?? '')
  ...
  const isERC20Approve =
    transactionInfo?.txType === QoraiWallet.TransactionType.ERC20Approve
  ...
  const isERC721TransferFrom =
    transactionInfo?.txType === QoraiWallet.TransactionType.ERC721TransferFrom
      ...
      dispatch(
        UIActions.setTransactionProviderError({
          providerError: {
            code: {
              ...emptyProviderErrorCodeUnion,
              providerError: QoraiWallet.ProviderError.kUnknown,
            },
  ...
  const {
    ...
    transactionsNetwork?.coin === QoraiWallet.CoinType.ZEC
      && txToken
      && txAccount
  ...
  const transactionTitle = React.useMemo(
    (): string =>
      isShieldingFunds
        ? getLocale('qoraiWalletShielding')
        : isSolanaDappTransaction
          ? getLocale('qoraiWalletApproveTransaction')
          : transactionDetails?.isSwap
            ? getLocale('qoraiWalletSwap')
            : getLocale('qoraiWalletSend'),
  ...
  const isLoadingGasFee = React.useMemo(() => {
    ...
    if (
      txCoinType === QoraiWallet.CoinType.BTC
      || txCoinType === QoraiWallet.CoinType.ZEC
      || txCoinType === QoraiWallet.CoinType.ADA
    ) {
      return false
    }

    // SOL
    if (txCoinType === QoraiWallet.CoinType.SOL) {
      return isLoadingSolFeeEstimates
    }

    // FIL has gas info provided by txDataUnion
    if (txCoinType === QoraiWallet.CoinType.FIL) {
      return gasFee === ''
    }

    // EVM
    if (txCoinType === QoraiWallet.CoinType.ETH) {
