...
export class MockedWalletApiProxy {
  ...
  selectedAccountId: QoraiWallet.AccountId = mockAccount.accountId
  ...
  selectedNetwork: QoraiWallet.NetworkInfo = mockEthMainnet

  chainIdsForCoins: Record<QoraiWallet.CoinType, string> = {
    [QoraiWallet.CoinType.ETH]: QoraiWallet.MAINNET_CHAIN_ID,
    [QoraiWallet.CoinType.SOL]: QoraiWallet.SOLANA_MAINNET,
    [QoraiWallet.CoinType.FIL]: QoraiWallet.FILECOIN_MAINNET,
    [QoraiWallet.CoinType.ADA]: QoraiWallet.CARDANO_MAINNET,
  }

  chainsForCoins: Record<QoraiWallet.CoinType, QoraiWallet.NetworkInfo> = {
    [QoraiWallet.CoinType.ETH]: mockEthMainnet,
    [QoraiWallet.CoinType.SOL]: mockSolanaMainnetNetwork,
    [QoraiWallet.CoinType.FIL]: mockFilecoinMainnetNetwork,
    [QoraiWallet.CoinType.ADA]: mockCardanoMainnetNetwork,
  }

  networks: QoraiWallet.NetworkInfo[] = mockNetworks
  ...
  userAssets: QoraiWallet.BlockchainToken[] = mockAccountAssetOptions

  evmSimulationResponse: QoraiWallet.EVMSimulationResponse | null = null

  svmSimulationResponse: QoraiWallet.SolanaSimulationResponse | null = null

  txSimulationOptInStatus: QoraiWallet.BlowfishOptInStatus =
  ...
  nativeBalanceRegistry: NativeAssetBalanceRegistry = {
    [mockAccount.address]: {
      [QoraiWallet.MAINNET_CHAIN_ID]: '0', // 0 ETH
    },
  }
  ...
  transactionInfos: QoraiWallet.TransactionInfo[] = [
    ...
    mockFilSendTransaction as QoraiWallet.TransactionInfo,
  ...
  requireOffchainConsent: number = QoraiWallet.ResolveMethod.kAsk

  private pendingAddChainRequests = [mockAddChainRequest]
  private pendingSwitchChainRequests: QoraiWallet.SwitchChainRequest[] = [
    mockSwitchChainRequest,
  ]

  private pendingDecryptRequests: QoraiWallet.DecryptRequest[] = [
    mockDecryptRequest,
  ]
  ...
  private signSolTransactionsRequests =
    [] as QoraiWallet.SignSolTransactionsRequest[]

  private signCardanoTransactionRequests =
    [] as QoraiWallet.SignCardanoTransactionRequest[]
  ...
  assetsRatioService: Partial<
    InstanceType<typeof QoraiWallet.AssetRatioServiceInterface>
  > = {
    getBuyUrlV1: async (
      provider,
      chainId,
  ...
  blockchainRegistry: Partial<
    InstanceType<typeof QoraiWallet.BlockchainRegistryInterface>
  > = {
    ...
    getTopDapps: async (chainId, coin) => {
      switch (chainId) {
        ...
        case QoraiWallet.MAINNET_CHAIN_ID: {
        ...
        case QoraiWallet.SOLANA_MAINNET: {
  ...
  qoraiWalletService: Partial<
    InstanceType<typeof QoraiWallet.QoraiWalletServiceInterface>
  > = {
    getUserAssets: async (chainId: string, coin: QoraiWallet.CoinType) => {
      return {
    ...
    getAnkrSupportedChainIds: async () => {
      return {
        chainIds: [
          ...
          QoraiWallet.BASE_MAINNET_CHAIN_ID,
          QoraiWallet.BNB_SMART_CHAIN_MAINNET_CHAIN_ID,
          QoraiWallet.MAINNET_CHAIN_ID,
          QoraiWallet.FANTOM_MAINNET_CHAIN_ID,
          QoraiWallet.FLARE_MAINNET_CHAIN_ID,
          QoraiWallet.GNOSIS_CHAIN_ID,
          QoraiWallet.OPTIMISM_MAINNET_CHAIN_ID,
          QoraiWallet.POLYGON_MAINNET_CHAIN_ID,
          QoraiWallet.POLYGON_ZKEVM_CHAIN_ID,
          QoraiWallet.ROLLUX_MAINNET_CHAIN_ID,
          QoraiWallet.SYSCOIN_MAINNET_CHAIN_ID,
          QoraiWallet.ZK_SYNC_ERA_CHAIN_ID,
        ],
      }
    },
    getNetworkForAccountOnActiveOrigin: async (
      ...
      if (account.coin === QoraiWallet.CoinType.SOL) {
  ...
  swapService: Partial<InstanceType<typeof QoraiWallet.SwapServiceInterface>> =
    {
      ...
      getQuote: async (
        ...
        fees: {
          ...
          discountCode: QoraiWallet.SwapDiscountCode.kNone,
        },
        error: null,
        errorString: '',
      }),
  ...
  meldIntegrationService: Partial<
    InstanceType<typeof QoraiWallet.MeldIntegrationServiceInterface>
  > = {
  ...
  keyringService: Partial<
    InstanceType<typeof QoraiWallet.KeyringServiceInterface>
  > = {
    getAllAccounts: async (): Promise<{
      ...
      const allAccounts: QoraiWallet.AllAccountsInfo = {
  ...
  ethTxManagerProxy: Partial<
    InstanceType<typeof QoraiWallet.EthTxManagerProxyInterface>
  > = {
    getGasEstimation1559: async () => {
      return {
        estimation: {
          ...
          fastMaxFeePerGas: '0',
          baseFeePerGas: '0',
        } as QoraiWallet.GasEstimation1559 | null,
      }
    },
  }

  qoraiWalletP3A: InstanceType<typeof QoraiWallet.QoraiWalletP3AInterface> = {
    ...
    recordActiveWalletCount(count, coinType) {
      // Follow up issue to fix test via https://github.com/qorai/qorai-browser/issues/43583
  ...
  jsonRpcService: Partial<
    InstanceType<typeof QoraiWallet.JsonRpcServiceInterface>
  > = {
    ...
    notifySwitchChainRequestProcessed: (requestId, approved) => {
      ...
      if (request) {
        ...
        this.qoraiWalletService.setNetworkForSelectedAccountOnActiveOrigin?.(
          request.chainId,
        )
      }
    },
    ...
    getERC20TokenAllowance: async (
      ...
      return {
        ...
        error: QoraiWallet.ProviderError.kSuccess,
        errorMessage: '',
      }
    },
    // NFT Metadata
    ...
    getNftMetadatas: async (nftIdentifiers) => {
      const metadatas: QoraiWallet.NftMetadata[] = nftIdentifiers.map((id) => {
    ...
    ensGetEthAddr: async (domain) => {
      return {
        ...
        requireOffchainConsent:
          this.requireOffchainConsent !== QoraiWallet.ResolveMethod.kEnabled,
      }
    },
    ...
    getEthTokenInfo: async (contractAddress, chainId) => {
      ...
      if (contractAddress === '0xInvalidToken') {
        return {
          token: null,
          error: QoraiWallet.ProviderError.kResourceNotFound,
    ...
    ankrGetAccountBalances: async (accountAddress, chainIds) => {
      ...
      const balances: QoraiWallet.AnkrAssetBalance[] = tokens.map((token) => {
  ...
  solanaTxManagerProxy: Partial<
    InstanceType<typeof QoraiWallet.SolanaTxManagerProxyInterface>
  > = {
  ...
  walletHandler: Partial<
    InstanceType<typeof QoraiWallet.WalletHandlerInterface>
  > = {
  ...
  txService: Partial<InstanceType<typeof QoraiWallet.TxServiceInterface>> = {
  ...
  simulationService: InstanceType<
    typeof QoraiWallet.SimulationServiceInterface
  > = {
  ...
  qoraiWalletIpfsService: Partial<
    InstanceType<typeof QoraiWallet.IpfsServiceInterface>
  > = {
  ...
  zcashWalletService: Partial<
    InstanceType<typeof QoraiWallet.ZCashWalletServiceInterface>
  > = {
...
function createDappParserForRange(
  range: string,
): (value: DappListResult) => QoraiWallet.Dapp {
  return (d) => ({
