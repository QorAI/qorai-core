...
export async function signTrezorTransaction(
  ...
  if (!nonce || !nonce.nonce) {
    return {
      success: false,
      error: getLocale('qoraiWalletApproveTransactionError'),
      code: undefined,
    }
  }
  ...
  if (!signed.success) {
    const error = signed.error
      ? signed.error
      : getLocale('qoraiWalletSignOnDeviceError')
  ...
  if (!result.status) {
    return {
      success: false,
      error: getLocale('qoraiWalletProcessTransactionError'),
      code: undefined,
    }
  }
  return { success: true }
...
export async function signLedgerEthereumTransaction(
  ...
  if (!hexMessage) {
    return {
      success: false,
      error: getLocale('qoraiWalletNoMessageToSignError'),
      code: undefined,
    }
  }
  ...
  if (!signed.success) {
    return {
      success: false,
      error: signed.error ?? getLocale('qoraiWalletSignOnDeviceError'),
      code: signed.code ?? '',
    }
  }
  const result = await apiProxy.ethTxManagerProxy.processEthHardwareSignature(
...
export async function signEthMessageWithHardwareKeyring(
  ...
  path: string,
  messageData: Omit<QoraiWallet.SignMessageRequest, 'originInfo'>,
  ...
  if (deviceKeyring instanceof EthereumLedgerBridgeKeyring) {
    if (signTypedData) {
      if (!signTypedData.domainHash || !signTypedData.primaryHash) {
        return {
          success: false,
          error: getLocale('qoraiWalletUnknownInternalError'),
          code: undefined,
        }
      }
    ...
    return {
      success: false,
      error: getLocale('qoraiWalletHardwareOperationUnsupportedError'),
      code: undefined,
    }
  }
  return {
    success: false,
    error: getLocale('qoraiWalletUnknownKeyringError'),
    code: undefined,
  }
}
