...
export default class TrezorBridgeKeyring implements TrezorKeyring {
  ...
  unlock = async (): Promise<HardwareOperationResult> => {
    ...
    if (!data.payload.success) {
      ...
      const error =
        response.payload?.error ?? getLocale('qoraiWalletUnlockError')
  ...
  signEip712Message = async (
    ...
    if (!response.success) {
      ...
      if (unsuccess.code && unsuccess.code === 'Method_InvalidParameter') {
        return {
          success: false,
          error: getLocale('qoraiWalletTrezorSignTypedDataError'),
          code: undefined,
        }
      }
      return { success: false, error: unsuccess.error, code: unsuccess.code }
  ...
  private readonly createErrorFromCode = (
    ...
    const deviceName = getLocale('qoraiWalletConnectHardwareTrezor')

    switch (code) {
      case TrezorErrorsCodes.BridgeNotReady:
        return {
          success: false,
          error: getLocale('qoraiWalletBridgeNotReady').replace(
            '$1',
            deviceName,
          ),
          code: code,
      ...
      case TrezorErrorsCodes.CommandInProgress:
        return {
          success: false,
          error: getLocale('qoraiWalletBridgeCommandInProgress').replace(
            '$1',
            deviceName,
          ),
          code: code,
