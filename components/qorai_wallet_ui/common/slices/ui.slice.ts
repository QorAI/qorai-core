...
import { QoraiWallet, UIState } from '../../constants/types'
        ...
        (state, { payload }) => {
          ...
          if (!state.selectedPendingTransactionId) {
            const firstPendingTx = payload.find(
              (tx) => tx.txStatus === QoraiWallet.TransactionStatus.Unapproved,
            )
            if (firstPendingTx) {
      ...
      builder.addMatcher(
        ...
        (state, { payload }) => {
          ...
          if (
            ...
            && payload.status === QoraiWallet.TransactionStatus.Unapproved
          ) {
            state.selectedPendingTransactionId = payload.txId
          }
        },
