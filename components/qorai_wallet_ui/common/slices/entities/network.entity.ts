...
export const getNetworkId = ({
  ...
  chainId === QoraiWallet.LOCALHOST_CHAIN_ID ? `${chainId}-${coin}` : chainId

export type NetworkEntityAdaptor = EntityAdapter<QoraiWallet.NetworkInfo> & {
  selectId: (network: {
    chainId: string
    coin: QoraiWallet.CoinType
  }) => EntityId
...
export const networkEntityAdapter: NetworkEntityAdaptor =
  createEntityAdapter<QoraiWallet.NetworkInfo>({
...
> & {
  hiddenIds: string[]
  hiddenIdsByCoinType: Record<QoraiWallet.CoinType, EntityId[]>
  visibleIdsByCoinType: Record<QoraiWallet.CoinType, EntityId[]>
  mainnetIds: string[]
  testnetIds: string[]
  onRampIds: string[]
  offRampIds: string[]
