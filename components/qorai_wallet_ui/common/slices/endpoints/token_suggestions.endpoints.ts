...
export const tokenSuggestionsEndpoints = ({
  ...
  return {
    getPendingTokenSuggestionRequests: query<
      QoraiWallet.AddSuggestTokenRequest[],
      void
    >({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.qoraiWalletService.getPendingAddSuggestTokenRequests()
          return {
            data: requests,
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        ...
        try {
          apiProxy.qoraiWalletService.notifyAddSuggestTokenRequestsProcessed(
            arg.approved,
            [arg.contractAddress],
          )
