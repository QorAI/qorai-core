  ...
  return {
    ...
    getTransactions: query<
      SerializableTransactionInfo[],
      {
         ...
         */
        coinType: QoraiWallet.CoinType | null
        /**
         * will fetch for all coin-type addresses if null
         */
        accountId: QoraiWallet.AccountId | null
        /**
      ...
      queryFn: async (
        ...
        try {
            ...
            fromAccountId && coinType !== null
              ...
              : (
                  await mapLimit(
                    ...
                    10,
                    async (account: QoraiWallet.AccountInfo) => {
          ...
          const nonRejectedTransactionInfos = txInfos
            // hide rejected txs
            .filter(
              (tx) => tx.txStatus !== QoraiWallet.TransactionStatus.Rejected,
            )
    ...
    sendSolanaSerializedTransaction: mutation<
      boolean, // success,
      {
        ...
        sendOptions?: QoraiWallet.SolanaSendTransactionOptions
      }
    >({
      queryFn: async (payload, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          if (result.error !== QoraiWallet.ProviderError.kSuccess) {
            throw new Error(
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.qoraiWalletService.getPendingSignSolTransactionsRequests()
    ...
    processSignSolTransactionsRequestHardware: mutation<
      ...
      {
        request: QoraiWallet.SignSolTransactionsRequest
      }
    >({
      queryFn: async (arg, { endpoint, ...store }, extraOptions, baseQuery) => {
        try {
          const { data: api, cache } = baseQuery(undefined)
          const { qoraiWalletService, panelHandler } = api

          if (!isHardwareAccount(arg.request.fromAccountId)) {
            const errorString = getLocale('qoraiWalletHardwareAccountNotFound')

            qoraiWalletService.notifySignSolTransactionsRequestProcessed(
              false,
              arg.request.id,
              [],
              errorString,
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.qoraiWalletService.getPendingSignCardanoTransactionRequests()
    ...
    >({
      queryFn: async (payload, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const params: QoraiWallet.NewBitcoinTransactionParams = {
    ...
    >({
      queryFn: async (payload, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const params: QoraiWallet.NewZCashTransactionParams = {
            chainId: payload.network.chainId,
            from: payload.fromAccount.accountId,
            to: payload.to,
            amount: BigInt(payload.value),
    ...
    >({
      queryFn: async (payload, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const params: QoraiWallet.NewCardanoTransactionParams = {
            chainId: payload.network.chainId,
            from: payload.fromAccount.accountId,
            to: payload.to,
            amount: BigInt(payload.value),
      ...
      invalidatesTags: (res, err, arg) =>
        err
          ...
          : [
              ...
              ...(arg.txStatus === QoraiWallet.TransactionStatus.Confirmed
                ? ([
    ...
    approveTransaction: mutation<
      {
        success: boolean
        errorUnion: QoraiWallet.ProviderErrorUnion
        errorMessage: string
      },
    ...
    >({
      queryFn: async (txInfo, store, extraOptions, baseQuery) => {
        try {
          ...
          const hardwareAccount: QoraiWallet.HardwareInfo =
          ...
          if (hardwareAccount.vendor === QoraiWallet.HardwareVendor.kLedger) {
            let result
            ...
            hardwareAccount.vendor === QoraiWallet.HardwareVendor.kTrezor
          ) {
    ...
    rejectTransactions: mutation<
      { success: boolean },
      Array<
        Pick<QoraiWallet.TransactionInfo, 'id' | 'chainId'> & {
          coinType: QoraiWallet.CoinType
        }
      >
    >({
      queryFn: async (txs, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          await mapLimit(
            ...
            10,
            async (
              tx: Pick<QoraiWallet.TransactionInfo, 'id' | 'chainId'> & {
                coinType: QoraiWallet.CoinType
              },
            ) => {
      ...
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const priorityFee =
            ...
            / BigInt(QoraiWallet.MICRO_LAMPORTS_PER_LAMPORT)
...
async function sendEvmTransaction({
  ...
  txService: QoraiWallet.TxServiceRemote
}): Promise<{ data: { success: boolean } }> {
  const params: QoraiWallet.NewEvmTransactionParams = {
    chainId: payload.network.chainId,
    from: payload.fromAccount.accountId,
    gasLimit: payload.gasLimit,
    to: payload.to,
...
async function processSignSolTransactionsRequest(
  qoraiWalletService: QoraiWallet.QoraiWalletServiceRemote,
  arg: ProcessSignSolTransactionsRequestPayload,
  panelHandler: QoraiWallet.PanelHandlerRemote | undefined,
) {
  qoraiWalletService.notifySignSolTransactionsRequestProcessed(
    arg.approved,
    arg.id,
...
async function processSignCardanoTransactionRequest(
  ...
  qoraiWalletService.notifySignCardanoTransactionRequestProcessed(
    arg.approved,
    arg.id,
    arg.error || null,
  )
