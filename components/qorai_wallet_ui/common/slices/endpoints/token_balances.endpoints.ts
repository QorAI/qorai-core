...
export type GetTokenBalancesRegistryArg = {
  accountIds: QoraiWallet.AccountId[]
...
export const tokenBalancesEndpoints = ({
  ...
  return {
      ...
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          if (includeRewardsBalance) {
            const rewardsData =
              cache.rewardsInfo || (await cache.getQoraiRewardsInfo())
          ...
          function onAnkrBalances(
            accountId: QoraiWallet.AccountId,
            ankrAssetBalances: QoraiWallet.AnkrAssetBalance[],
          ) {
          ...
          if (arg.useAnkrBalancesFeature) {
            const { chainIds: ankrSupportedChainIds } =
              await qoraiWalletService.getAnkrSupportedChainIds()
          ...
          await eachLimit(
            ...
            async (accountId: QoraiWallet.AccountId) => {
              ...
              const ankrAssetBalances: QoraiWallet.AnkrAssetBalance[] =
              ...
              if (nonAnkrSupportedAccountNetworks.length) {
                await eachLimit(
                  ...
                  3,
                  async (network: QoraiWallet.NetworkInfo) => {
    ...
    getBitcoinBalances: query<BitcoinBalances, QoraiWallet.AccountId>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { balance, errorMessage } =
...
async function fetchAnkrAccountBalances(
  jsonRpcService: QoraiWallet.JsonRpcServiceRemote,
  accountId: QoraiWallet.AccountId,
  ...
  if (error !== QoraiWallet.ProviderError.kSuccess || errorMessage) {
    console.log(
...
async function fetchAccountCurrentNativeBalance({
  ...
  jsonRpcService: QoraiWallet.JsonRpcServiceRemote
  bitcoinWalletService: QoraiWallet.BitcoinWalletServiceRemote
  zcashWalletService: QoraiWallet.ZCashWalletServiceRemote
  cardanoWalletService: QoraiWallet.CardanoWalletServiceRemote
  polkadotWalletService: QoraiWallet.PolkadotWalletServiceRemote
}): Promise<string> {
  // LOCALHOST
  if (
    token.chainId === QoraiWallet.LOCALHOST_CHAIN_ID
    && accountId.coin !== QoraiWallet.CoinType.SOL
  ) {
  ...
  switch (accountId.coin) {
    case QoraiWallet.CoinType.SOL: {
      ...
      if (token.chainId === QoraiWallet.LOCALHOST_CHAIN_ID && error !== 0) {
        return Amount.zero().format()
      }
    ...
    case QoraiWallet.CoinType.ETH: {
      ...
      if (error && errorMessage) {
        throw new Error(
          `getBalance (${
            accountId.coin === QoraiWallet.CoinType.FIL ? 'FIL' : 'ETH'
    ...
    if (supportedChainIds.includes(arg.chainId)) {
      ...
      const erc20Tokens = arg.tokens.filter(
        (token) =>
          ...
          && !isNativeAsset(token)
          && !(
            token.chainId === QoraiWallet.POLYGON_MAINNET_CHAIN_ID
...
async function fetchNftBalancesForAccount({
  ...
  if (nfts.length) {
    ...
    // https://github.com/qorai/qorai-browser/issues/39954
    const nftBatches = []
