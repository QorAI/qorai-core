...
export const networkEndpoints = ({
  ...
  return {
    ...
    getAllKnownNetworks: query<QoraiWallet.NetworkInfo[], void>({
    ...
    getSwapSupportedNetworks: query<QoraiWallet.NetworkInfo[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const {
            data: { swapService },
    ...
    getSelectedChain: query<QoraiWallet.NetworkInfo | null, void>({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { network } = await api.qoraiWalletService //
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.qoraiWalletService.setNetworkForAccountOnActiveOrigin(
            arg.accountId,
            arg.chainId,
          )
    ...
    getPendingAddChainRequest: query<QoraiWallet.AddChainRequest | null, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
    ...
    getPendingSwitchChainRequest: query<
      QoraiWallet.SwitchChainRequest | null,
      void
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    hideNetworks: mutation<
      boolean,
      Array<Pick<QoraiWallet.NetworkInfo, 'chainId' | 'coin'>>
    >({
      queryFn: async (
        chains,
        { endpoint, dispatch },
        ...
        try {
          ...
          await mapLimit(
            ...
            10,
            async (
              chain: Pick<QoraiWallet.NetworkInfo, 'chainId' | 'coin'>,
            ) => {
    ...
    setNetwork: mutation<
      ...
      Pick<QoraiWallet.NetworkInfo, 'chainId' | 'coin'>
    >({
      queryFn: async (
        { chainId, coin },
        { endpoint },
        ...
        try {
          ...
          const { accountId: selectedAccountId } =
            await qoraiWalletService.ensureSelectedAccountForChain(
              coin,
              chainId,
            )
