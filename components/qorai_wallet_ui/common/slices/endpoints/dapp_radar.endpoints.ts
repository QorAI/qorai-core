...
export const dappRadarEndpoints = ({
  ...
  return {
    getTopDapps: query<QoraiWallet.Dapp[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const topDapps: QoraiWallet.Dapp[] = await flatMapLimit(
            networks,
            10,
            async (net: QoraiWallet.NetworkInfo) => {
          ...
          const uniqueTopDapps: QoraiWallet.Dapp[] = []
          ...
          const parsedUniqueTopDapps: QoraiWallet.Dapp[] = uniqueTopDapps.map(
            (dapp) => {
              return {
                ...dapp,
                chains: dapp.chains
