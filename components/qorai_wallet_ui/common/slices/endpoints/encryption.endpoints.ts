...
export const encryptionEndpoints = ({
  ...
  return {
    getPendingDecryptRequest: query<QoraiWallet.DecryptRequest | null, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)

          const { requests } =
            await api.qoraiWalletService.getPendingDecryptRequests()
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.qoraiWalletService.notifyDecryptRequestProcessed(
            arg.requestId,
            arg.approved,
          )
    ...
    getPendingGetEncryptionPublicKeyRequest: query<
      QoraiWallet.GetEncryptionPublicKeyRequest | null,
      void
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          qoraiWalletService.notifyGetPublicKeyRequestProcessed(
            arg.requestId,
            arg.approved,
          )
