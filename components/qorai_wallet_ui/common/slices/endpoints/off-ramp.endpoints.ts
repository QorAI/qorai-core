...
// Types
import { QoraiWallet, SupportedOffRampNetworks } from '../../../constants/types'
...
export const offRampEndpoints = ({ query }: WalletApiEndpointBuilderParams) => {
  return {
    ...
    >({
      queryFn: async (_arg, _store, _extraOptions, baseQuery) => {
        try {
          ...
          const { kRamp } = QoraiWallet.OffRampProvider
          ...
          // moves Gas coins and QOR to front of list
    ...
    getSellAssetUrl: query<
      ...
      {
        assetSymbol: string
        offRampProvider: QoraiWallet.OffRampProvider
        chainId: string
        amount: string
