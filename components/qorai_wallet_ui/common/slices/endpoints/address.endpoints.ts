...
interface GetFVMAddressArg {
  coin: QoraiWallet.CoinType | undefined
  isMainNet: boolean
  addresses: string[]
}
...
type GetZCashTransactionTypeResult = {
  txType: QoraiWallet.ZCashTxType
  error: QoraiWallet.ZCashAddressError
}
  ...
  return {
      ...
      queryFn: async (_arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.jsonRpcService.setEnsOffchainLookupResolveMethod(
            QoraiWallet.ResolveMethod.kEnabled,
          )
          return {
            data: true,
          }
    ...
    getIsBase58EncodedSolPubkey: query<boolean, string>({
      queryFn: async (pubKeyArg, { endpoint }, _extra, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { result } =
            await api.qoraiWalletService.isBase58EncodedSolanaPubkey(pubKeyArg)
    ...
    >({
      queryFn: async (arg, { endpoint }, _extra, baseQuery) => {
        try {
          ...
          // https://github.com/qorai/qorai-browser/issues/34796
    ...
    getFVMAddress: query<GetFVMAddressResult, GetFVMAddressArg>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        if (arg.coin !== QoraiWallet.CoinType.FIL) {
          // invalid coin type
          return { data: {} }
        }
        try {
          const { qoraiWalletService } = baseQuery(undefined).data
          const convertResult = (
            await qoraiWalletService.convertFEVMToFVMAddress(
              arg.isMainNet,
              arg.addresses,
            )
          ).result
    ...
    generateReceiveAddress: mutation<string, QoraiWallet.AccountId>({
      queryFn: async (accountId, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { qoraiWalletService } = baseQuery(undefined).data
          const { address, errorMessage } =
            await qoraiWalletService.generateReceiveAddress(accountId)
