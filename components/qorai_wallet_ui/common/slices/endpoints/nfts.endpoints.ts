  ...
  return {
    getNftDiscoveryEnabledStatus: query<boolean, void>({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const result = await qoraiWalletService.getNftDiscoveryEnabled()
          return {
            data: result.enabled,
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { qoraiWalletService } = baseQuery(undefined).data
          await qoraiWalletService.setNftDiscoveryEnabled(arg)
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          try {
            ...
            const uniqueTokens: QoraiWallet.BlockchainToken[] = arg.filter(
              (t) => {
            ...
            for (const token of uniqueTokens) {
              ...
              if (token.coin === QoraiWallet.CoinType.ETH) {
                const {
                  token: tokenInfo,
                  error,
                  errorMessage,
    ...
    getSimpleHashSpamNfts: query<
      QoraiWallet.BlockchainToken[],
      void | undefined | { accounts: QoraiWallet.AccountInfo[] }
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { cache } = baseQuery(undefined)
