...
export const sitePermissionEndpoints = ({
  ...
  return {
    getActiveOriginConnectedAccountIds: query<QoraiWallet.AccountId[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { accountsWithPermission } =
            await qoraiWalletService.hasPermission(
    ...
    getIsPrivateWindow: query<boolean, void>({
      queryFn: async (_, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { isPrivateWindow } = await qoraiWalletService.isPrivateWindow()
    ...
    connectToSite: mutation<
      true,
      {
        ...
        duration: QoraiWallet.PermissionLifetimeOption
      }
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    requestSitePermission: mutation<true, QoraiWallet.AccountId>({
    ...
    removeSitePermission: mutation<true, QoraiWallet.AccountId>({
      queryFn: async (accountId, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { qoraiWalletService } = api

          await qoraiWalletService.resetPermission(accountId)
