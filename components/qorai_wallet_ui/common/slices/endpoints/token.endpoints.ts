...
export const tokenEndpoints = ({
  ...
  return {
    ...
    addUserToken: mutation<{ id: EntityId }, QoraiWallet.BlockchainToken>({
      queryFn: async (
        tokenArg,
        { dispatch, endpoint },
        extraOptions,
    ...
    >({
      queryFn: async (tokenId, { endpoint }, extraOptions, baseQuery) => {
        ...
        try {
          ...
          if (token) {
            const deleteResult = await qoraiWalletService.removeUserAsset(token)
            if (!deleteResult.success) {
              throw new Error(
                'qoraiWalletService.removeUserAsset was unsuccessful',
              )
            }
          }
    ...
    updateUserToken: mutation<
      ...
      string,
      {
        existingToken: QoraiWallet.BlockchainToken
        updatedToken: QoraiWallet.BlockchainToken
      }
    >({
      queryFn: async (
        ...
        try {
          ...
          // update core user assets list
          const deleteResult =
            await qoraiWalletService.removeUserAsset(existingToken)
    ...
    >({
      queryFn: async (
        ...
        try {
          ...
          const { success } = await qoraiWalletService.setUserAssetVisible(
            token,
            isVisible,
          )
    ...
    getTokenInfo: query<
      QoraiWallet.BlockchainToken | null,
      Pick<QoraiWallet.BlockchainToken, 'chainId' | 'coin' | 'contractAddress'>
    >({
      queryFn: async (
        ...
        try {
          ...
          if (error !== QoraiWallet.ProviderError.kSuccess) {
    ...
    discoverAssets: mutation<true, void>({
      queryFn: async (
        ...
        try {
          const { data: api } = baseQuery(undefined)
          api.qoraiWalletService.discoverAssetsOnAllSupportedChains(true)
          return {
            data: true,
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const { success } = await qoraiWalletService.setAssetSpamStatus(
            arg.token,
            arg.isSpam,
          )
...
async function addUserToken({
  ...
  tokenArg: QoraiWallet.BlockchainToken
}) {
  ...
  return await qoraiWalletService.addUserAsset({
    ...tokenArg,
