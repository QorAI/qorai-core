  ...
  return {
    ...
    getDefaultEthereumWallet: query<number, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { qoraiWalletService } = api
          const { defaultWallet } =
            await qoraiWalletService.getDefaultEthereumWallet()
          return {
            data: defaultWallet,
          }
        } catch (error) {
    ...
    getDefaultSolanaWallet: query<number, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { qoraiWalletService } = api
          const { defaultWallet } =
            await qoraiWalletService.getDefaultSolanaWallet()
          return {
            data: defaultWallet,
          }
        } catch (error) {
    ...
    getIsMetaMaskInstalled: query<boolean, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { qoraiWalletService } = api
          const { installed } =
            await qoraiWalletService.isExternalWalletInstalled(
              QoraiWallet.ExternalWalletType.MetaMask,
            )
          return {
            data: installed,
          }
    ...
    getActiveOrigin: query<QoraiWallet.OriginInfo, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { qoraiWalletService } = api

          const { originInfo } = await qoraiWalletService.getActiveOrigin()
          return {
            data: originInfo,
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const mmResult = await qoraiWalletService.isExternalWalletInitialized(
            QoraiWallet.ExternalWalletType.MetaMask,
          )
    ...
    checkExternalWalletPassword: mutation<
      ...
      { walletType: QoraiWallet.ExternalWalletType; password: string }
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          ...
          const { errorMessage } =
            await qoraiWalletService.importFromExternalWallet(
              arg.walletType,
              arg.password,
              '',
            )
          ...
          const checkExistingPasswordError =
            errorMessage === getLocale('qoraiWalletImportPasswordError')
    ...
    >({
      queryFn: async (arg, { endpoint, getState }, extraOptions, baseQuery) => {
        try {
          ...
          const { data: api, cache } = baseQuery(undefined)
          const { keyringService, qoraiWalletService } = api

          const { errorMessage }: ImportWalletResults =
            await importFromExternalWallet(
              QoraiWallet.ExternalWalletType.MetaMask,
              arg,
              { qoraiWalletService, keyringService },
            )
    ...
    copyToClipboardConfidentially: mutation<boolean, { text: string }>({
      queryFn: async ({ text }, { endpoint }, extraOptions, baseQuery) => {
        const { data: api } = baseQuery(undefined)
        api.qoraiWalletService.writeToClipboard(text, true)
        return {
          data: true,
        }
      },
...
async function importFromExternalWallet(
  ...
  const { qoraiWalletService, keyringService } = services

  const { errorMessage } = await qoraiWalletService.importFromExternalWallet(
    walletType,
    payload.password,
    payload.newPassword,
  )
...
async function createDefaultAccounts({
  ...
  await mapLimit(
    ...
    3,
    async function (net: QoraiWallet.NetworkInfo) {
      ...
      if (
        ...
        || net.coin === QoraiWallet.CoinType.ETH
        || net.coin === QoraiWallet.CoinType.SOL
      ) {
        return
      }
