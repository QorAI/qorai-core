...
interface ProcessSignMessageRequestArgs {
  approved: boolean
  id: number
  hwSignature?: QoraiWallet.EthereumSignatureBytes | undefined
...
export const signingEndpoints = ({
  ...
  return {
    getPendingSignMessageRequests: query<
      QoraiWallet.SignMessageRequest[],
      void
    >({
      queryFn: async (_arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.qoraiWalletService.getPendingSignMessageRequests()
    ...
    getPendingSignMessageErrors: query<QoraiWallet.SignMessageError[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)

          const { errors } =
            await api.qoraiWalletService.getPendingSignMessageErrors()
    ...
    >({
      queryFn: async (errorIdArg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.qoraiWalletService.notifySignMessageErrorProcessed(errorIdArg)
    ...
    signMessageHardware: mutation<
      ...
      {
        request: QoraiWallet.SignMessageRequest
    ...
    >({
      queryFn: async (arg, { endpoint, ...store }, extraOptions, baseQuery) => {
        try {
          ...
          if (arg.account.accountId.coin !== QoraiWallet.CoinType.ETH) {
            throw new Error('Not an ETH account')
          }

          if (!isHardwareAccount(arg.account.accountId)) {
            api.qoraiWalletService.notifySignMessageRequestProcessed(
              false,
              arg.request.id,
              null,
              getLocale('qoraiWalletHardwareAccountNotFound'),
            )
          ...
          if (!signed.success && signed.code) {
            ...
            const deviceError =
              info.vendor === QoraiWallet.HardwareVendor.kTrezor
...
async function processSignMessageRequestInternal(
  api: {
    ...
    panelHandler?: QoraiWallet.PanelHandlerRemote
  },
