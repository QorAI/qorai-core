...
export const mockATAInstruction = {
  programId: QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID,
  accountMetas: [
    {
      pubkey: mockSolanaAccount.address,
      isSigner: true,
...
export const mockSolanaTransactionInfoAccount: QoraiWallet.AccountInfo = {
  ...mockSolanaAccount,
  accountId: mockSolanaTransactionInfo.fromAccountId,
}

export const mockSvmTxInfos: QoraiWallet.TransactionInfo[] = [
...
export const mockFilSendTransaction: FileCoinTransactionInfo = {
  ...
  txHash: 'fil-send-tx',
  txParams: [],
  txStatus: QoraiWallet.TransactionStatus.Confirmed,
...
export const createMockTransactionInfo = (arg: {
  ...
  const txBase =
    coinType === QoraiWallet.CoinType.FIL
...
const getMockTransactionType = (
  ...
  if (isSwapOrBridge && coinType === QoraiWallet.CoinType.SOL) {
    return QoraiWallet.TransactionType.SolanaSwap
  }
  if (isSwapOrBridge) {
    return QoraiWallet.TransactionType.ETHSwap
  }
  if (transactionType === 'Approve') {
    return QoraiWallet.TransactionType.ERC20Approve
  }
  if (coinType === QoraiWallet.CoinType.SOL) {
    return QoraiWallet.TransactionType.SolanaSystemTransfer
  }
  return QoraiWallet.TransactionType.ETHSend
}

export const getPostConfirmationStatusMockTransaction = (
  transactionType: StorybookTransactionTypes,
  transactionStatus: QoraiWallet.TransactionStatus,
  ...
  return {
    ...
    swapInfo: isSwapOrBridge
      ? ({
          fromCoin: QoraiWallet.CoinType.ETH,
          fromChainId: QoraiWallet.MAINNET_CHAIN_ID,
...
export const mockETHSwapTransaction: QoraiWallet.TransactionInfo = {
  ...
  swapInfo: {
    ...
    toCoin: QoraiWallet.CoinType.ETH,
    toChainId: QoraiWallet.MAINNET_CHAIN_ID,
