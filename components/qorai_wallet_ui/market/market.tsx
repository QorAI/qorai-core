...
const App = () => {
  ...
  // Constants
  const origin =
    window.location.ancestorOrigins[0] === qoraiWalletPanelOrigin
      ? qoraiWalletPanelOrigin
      : qoraiWalletOrigin
  ...
  const isBuySupported = React.useCallback(
    (coinMarket: QoraiWallet.CoinMarket) => {
      return (
        buyableAssets?.some(
          (asset) =>
  ...
  const onMessageEventListener = React.useCallback(
    (event: MessageEvent<MarketCommandMessage>) => {
      ...
      if (
        event.origin === qoraiWalletOrigin
        || event.origin === qoraiWalletPanelOrigin
      ) {
