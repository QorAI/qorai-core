...
import { QoraiWallet, SerializableTransactionInfo } from '../constants/types'
    ...
    const { buyAmountWei, sellAmountWei, buyToken, sellToken } =
      getETHSwapTransactionBuyAndSellTokens({
        ...
        tx: {
          ...
          txType: QoraiWallet.TransactionType.ETHSwap,
          isRetriable: false,
          swapInfo: {
...
describe('check for insufficient funds errors', () => {
  describe.each([
    ['ERC20Transfer', QoraiWallet.TransactionType.ERC20Transfer],
    ['ERC20Approve', QoraiWallet.TransactionType.ERC20Approve],
    ['ERC721TransferFrom', QoraiWallet.TransactionType.ERC721TransferFrom],
    [
      'ERC721SafeTransferFrom',
      QoraiWallet.TransactionType.ERC721SafeTransferFrom,
    ],
    ['ETHSend', QoraiWallet.TransactionType.ETHSend],
    ['Other', QoraiWallet.TransactionType.Other],
  ])('%s', (_, txType) => {
    ...
    it('should be false when funds are sufficient for gas', () => {
      ...
      const mockTxData: QoraiWallet.TxData1559 =
      ...
      const mockTransactionInfo = {
        ...
        txArgs: [
          ...
          QoraiWallet.TransactionType.ERC20Transfer,
        ].includes(txType)
          ? ['mockRecipient', '0x0']
  ...
  describe('ERC20Transfer', () => {
    it('should be true when funds are insufficient for send amount', () => {
      ...
      const transactionInfo: SerializableTransactionInfo = {
        ...
        txType: QoraiWallet.TransactionType.ERC20Transfer,
        txDataUnion: {
          ethTxData: {} as any,
          filTxData: undefined,
          solanaTxData: undefined,
...
describe('toTxDataUnion', () => {
  test('works', () => {
    const filTxData: QoraiWallet.FilTxData = {
      nonce: '',
      gasPremium: '',
      gasFeeCap: '',
      gasLimit: '',
...
describe('getIsRevokeApprovalTx', () => {
  test('correctly detects revocations', () => {
    expect(
      getIsRevokeApprovalTx({
        ...mockTransactionInfo,
        txType: QoraiWallet.TransactionType.ERC20Approve,
        txArgs: [
    ...
    it('should detect sent ERC20 tokens', () => {
      expect(
        findTransactionToken(
          createMockTransactionInfo({
            chainId: QoraiWallet.MAINNET_CHAIN_ID,
            coinType: QoraiWallet.CoinType.ETH,
      ...
      expect(
        findTransactionToken(
          createMockTransactionInfo({
            ...
            coinType: QoraiWallet.CoinType.SOL,
            fromAccount: mockSolanaAccount,
            sendApproveOrSellAmount: '1000',
            sendApproveOrSellAssetContractAddress: mockSplQor.contractAddress,
            toAddress: mockSolanaAccount.address,
          }),
          [mockSolToken, mockSplQor],
        )?.symbol,
      ).toBe('QOR')
    })
  })
...
describe('getTransactionTypeName', () => {
  test.each([
    {
      txType: QoraiWallet.TransactionType.ERC1155SafeTransferFrom,
    ...
    {
      txType: QoraiWallet.TransactionType.ERC20Approve,
      expectedString: getLocale('qoraiWalletTransactionTypeNameErc20Approve'),
    },

    {
      txType: QoraiWallet.TransactionType.ERC20Transfer,
      expectedString: getLocale('qoraiWalletTransactionTypeNameTokenTransfer'),
    },
...

    {
      ...
      expectedString: getLocale('qoraiWalletTransactionTypeNameNftTransfer'),
    },

    {
      txType: QoraiWallet.TransactionType.ETHFilForwarderTransfer,
      expectedString: getLocale('qoraiWalletTransactionTypeNameForwardFil'),
    },

    {
      txType: QoraiWallet.TransactionType.ETHSend,
      expectedString: getLocale('qoraiWalletTransactionIntentSend').replace(
        '$1',
        'ETH',
      ),
    },
    ...
    {
      txType: QoraiWallet.TransactionType.ETHSwap,
      expectedString: getLocale('qoraiWalletSwap'),
    },

    {
      txType: QoraiWallet.TransactionType.Other,
      expectedString: getLocale('qoraiWalletTransactionTypeNameOther'),
    },

    {
      txType: QoraiWallet.TransactionType.SolanaCompressedNftTransfer,
      expectedString: getLocale(
    ...
    {
      txType: QoraiWallet.TransactionType.SolanaDappSignAndSendTransaction,
      expectedString: getLocale(
    ...
    {
      txType: QoraiWallet.TransactionType.SolanaDappSignTransaction,
      expectedString: getLocale(
    ...
    {
      txType: QoraiWallet.TransactionType.SolanaSPLTokenTransfer,
      expectedString: getLocale('qoraiWalletTransactionTypeNameTokenTransfer'),
    },

    {
      ...
      expectedString: getLocale(
        'qoraiWalletTransactionTypeNameSplTokenTransferWithAssociatedTokenAccountCreation',
      ),
    },

    {
      txType: QoraiWallet.TransactionType.SolanaSwap,
      expectedString: getLocale('qoraiWalletSwap'),
    },

    {
      txType: QoraiWallet.TransactionType.SolanaSystemTransfer,
      expectedString: getLocale('qoraiWalletTransactionIntentSend').replace(
        '$1',
        'SOL',
      ),
  ...
  test('should return "Other" for unknown tx type', () => {
    expect(getTransactionTypeName(999)).toBe(
      getLocale('qoraiWalletTransactionTypeNameOther'),
    )
  })
})
  ...
  it('should return true for ATACreation transaction', () => {
    expect(
      getIsSolanaAssociatedTokenAccountCreation({
        ...
        txDataUnion: {
          solanaTxData: {
            staticAccountKeys: [QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID],
          ...
          solanaTxData: {
            ...
            instructions: [
              ...
              },
              {
                ...
                accountMetas: [
                  {
                    pubkey: QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID,
                    isSigner: true,
                    isWritable: true,
                    addrTableLookupIndex: undefined,
                  },
                ],
                programId: QoraiWallet.SOLANA_STAKE_PROGRAM_ID,
              },
            ],
          },
        },
...
describe('isCancelTransaction', () => {
  ...
  const mockEthSendTestTransaction = {
    ...mockEthSendTransaction,
    txType: QoraiWallet.TransactionType.ETHSend,
    fromAccountId: { address: mockFromAddress } as QoraiWallet.AccountId,
  }
  ...
  test(
    ...
    () => {
      ...
      const submittedTx = {
        ...
        txStatus: QoraiWallet.TransactionStatus.Submitted,
        id: 'submitted-tx',
      }
