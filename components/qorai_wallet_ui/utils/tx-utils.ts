...
export type SolanaTransactionInfo = TransactionInfo & {
  txDataUnion: {
    solanaTxData: QoraiWallet.SolanaTxData
    ethTxData1559: undefined
    ethTxData: undefined
    filTxData: undefined
  }
...
export interface ParsedTransaction
  extends ParsedTransactionFees,
    Pick<QoraiWallet.TransactionInfo, 'txType'> {
  // Common fields
  id: string
  hash: string
  nonce: string
  ...
  intent: string
  chainId: string
  originInfo?: QoraiWallet.OriginInfo | undefined
  ...
  erc721BlockchainToken?: QoraiWallet.BlockchainToken
  ...
  // Swap
  sellToken?: QoraiWallet.BlockchainToken
  sellAmount?: Amount
  sellAmountWei?: Amount
  buyToken?: QoraiWallet.BlockchainToken
  minBuyAmount?: Amount
...
export const getLocaleKeyForTxStatus = (
  ...
  switch (status) {
    case QoraiWallet.TransactionStatus.Unapproved:
      return 'qoraiWalletTransactionStatusUnapproved'
    case QoraiWallet.TransactionStatus.Approved:
      return 'qoraiWalletTransactionStatusApproved'
    case QoraiWallet.TransactionStatus.Rejected:
      return 'qoraiWalletTransactionStatusRejected'
    case QoraiWallet.TransactionStatus.Submitted:
      return 'qoraiWalletTransactionStatusSubmitted'
    case QoraiWallet.TransactionStatus.Confirmed:
      return 'qoraiWalletTransactionStatusConfirmed'
    case QoraiWallet.TransactionStatus.Error:
      return 'qoraiWalletTransactionStatusError'
    case QoraiWallet.TransactionStatus.Dropped:
      return 'qoraiWalletTransactionStatusDropped'
    case QoraiWallet.TransactionStatus.Signed:
      return 'qoraiWalletTransactionStatusSigned'
    default:
      return ''
  }
}
...
export function isSolanaDappTransaction(
  ...
  return (
    ...
    && [
      ...
      QoraiWallet.TransactionType.Other,
    ].includes(tx.txType)
  )
}
...
export function isSolanaSplTransaction(
  ...
  return (
    tx.txType === QoraiWallet.TransactionType.SolanaSPLTokenTransfer
    || tx.txType
...
>(
  ...
  // Native Asset Send
  if (
    tx.txType === QoraiWallet.TransactionType.SolanaSystemTransfer
    || tx.txType
      === QoraiWallet.TransactionType.SolanaDappSignAndSendTransaction
    || tx.txType === QoraiWallet.TransactionType.SolanaDappSignTransaction
    || tx.txType === QoraiWallet.TransactionType.ETHSend
    || tx.txType === QoraiWallet.TransactionType.Other
...
export const getETHSwapTransactionBuyAndSellTokens = ({
  ...
  tx: TransactionInfo | undefined
  nativeAsset?: QoraiWallet.BlockchainToken
  ...
  if (
    !tx
    || !tx.swapInfo
    || tx.txType !== QoraiWallet.TransactionType.ETHSwap
  ) {
    return {
      buyToken: undefined,
      sellToken: undefined,
    ...
    tx.swapInfo.fromAsset === NATIVE_EVM_ASSET_CONTRACT_ADDRESS
      ...
      : findTokenByContractAddress(tx.swapInfo.fromAsset, tokensList)
        ...
        || ({
          ...
          isSpam: false,
          visible: true,
        } as QoraiWallet.BlockchainToken)
...
interface GetTransactionTransferredValueArgs {
  ...
  token?: QoraiWallet.BlockchainToken
  sellToken?: QoraiWallet.BlockchainToken
  txAccount: QoraiWallet.AccountInfo | undefined
  txNetwork: QoraiWallet.NetworkInfo | undefined
}
...
export function getTransactionTransferredValue(
  ...
  // ERC20 Transfers
  if (
    tx.txType === QoraiWallet.TransactionType.ERC20Approve
    || tx.txType === QoraiWallet.TransactionType.ERC20Transfer
  ) {
  ...
  // ERC721
  if (
    tx.txType === QoraiWallet.TransactionType.ERC721TransferFrom
    || tx.txType === QoraiWallet.TransactionType.ERC721SafeTransferFrom
  ) {
  ...
  // ETH Swap
  if (tx.txType === QoraiWallet.TransactionType.ETHSwap) {
...
export const getTransactionApprovalTargetAddress = (
  ...
  if (tx.txType === QoraiWallet.TransactionType.ERC20Approve) {
...
export function getTransactionDecimals({
  ...
  sellToken?: QoraiWallet.BlockchainToken
  erc721Token?: QoraiWallet.BlockchainToken
  token?: QoraiWallet.BlockchainToken
}) {
  if (sellToken) {
...
export const getTransactionErc721TokenId = (
  ...
  if (
    [
      QoraiWallet.TransactionType.ERC721TransferFrom,
      QoraiWallet.TransactionType.ERC721SafeTransferFrom,
    ].includes(tx.txType)
  ) {
...
export const isSendingToKnownTokenContractAddress = (
  ...
  // ERC20Transfer
  if (tx.txType === QoraiWallet.TransactionType.ERC20Transfer) {
...
export const transactionHasSameAddressError = (
  tx: TransactionInfo,
  txAccount: QoraiWallet.AccountInfo,
): boolean => {
  const { txArgs, txType } = tx

  // transfer(address recipient, uint256 amount) → bool
  if (txType === QoraiWallet.TransactionType.ERC20Transfer) {
  ...
  if (
    txType === QoraiWallet.TransactionType.ERC721TransferFrom
    || txType === QoraiWallet.TransactionType.ERC721SafeTransferFrom
  ) {
    // The owner of the ERC721 must not be confused with the caller
    // (fromAddress).
    // (address owner, address to, uint256 tokenId)
  ...
  if (txType === QoraiWallet.TransactionType.ERC20Approve) {
  ...
  if (
    QoraiWallet.TransactionType.ETHSend
    || QoraiWallet.TransactionType.ETHSwap
    || QoraiWallet.TransactionType.Other
  ) {
    return false
  }
...
export function getGasFeeFiatValue({
  ...
  txNetwork?: Pick<QoraiWallet.NetworkInfo, 'decimals'>
}) {
...
export const accountHasInsufficientFundsForTransaction = ({
  ...
  // Eth Swap
  if (txType === QoraiWallet.TransactionType.ETHSwap) {
    return sellTokenBalance !== '' && sellAmountWei.gt(sellTokenBalance)
  }

  if (
    tx.chainId === QoraiWallet.Z_CASH_MAINNET
    || tx.chainId === QoraiWallet.Z_CASH_TESTNET
  ) {
    return (
      accountTokenBalance !== ''
...
export function getTransactionTransferredToken({
  ...
  tx: TransactionInfo
  txNetwork?: QoraiWallet.NetworkInfo
  token?: QoraiWallet.BlockchainToken
  sellToken?: QoraiWallet.BlockchainToken
...
export function getTransactionTokenSymbol({
  ...
  tx: TransactionInfo
  txNetwork?: Pick<QoraiWallet.NetworkInfo, 'symbol'>
  token?: QoraiWallet.BlockchainToken
  sellToken?: QoraiWallet.BlockchainToken
}): string {
  if (isSolanaDappTransaction(tx)) {
...
export const getTransactionIntent = ({
  ...
  sellToken?: QoraiWallet.BlockchainToken
  token?: QoraiWallet.BlockchainToken
  transactionNetwork?: QoraiWallet.NetworkInfo
  tx: TransactionInfo
}): string => {
  // ERC20 Approve
  if (tx.txType === QoraiWallet.TransactionType.ERC20Approve) {
    return (
      toProperCase(getLocale('qoraiWalletApprovalTransactionIntent'))
      + ' '
      + (token?.symbol ?? '')
    )
  }
  ...
  // ERC20 Transfer
  if (tx.txType === QoraiWallet.TransactionType.ERC20Transfer) {
    return getLocale('qoraiWalletTransactionIntentSend').replace(
      '$1',
  ...
  // Solana Dapps
  if (isSolanaDappTransaction(tx)) {
    return tx.txType === QoraiWallet.TransactionType.SolanaSwap
      ? getLocale('qoraiWalletSwap')
      : getLocale('qoraiWalletTransactionIntentDappInteraction')
  }
  ...
  // ETHSwap
  if (tx.txType === QoraiWallet.TransactionType.ETHSwap) {
    return getLocale('qoraiWalletTransactionIntentSwap')
...
export const parseTransactionWithoutPrices = ({
  ...
  tx: TransactionInfo
  transactionAccount: QoraiWallet.AccountInfo
  transactionNetwork: QoraiWallet.NetworkInfo
  tokensList: QoraiWallet.BlockchainToken[]
  ...
  const missingGasLimitError = isMissingGasLimit
    ? getLocale('qoraiWalletMissingGasLimitError')
    : undefined
  ...
  const contractAddressError = isSendingToKnownTokenContractAddress(
    ...
    ? getLocale('qoraiWalletContractAddressError')
    : undefined
  ...
  const sameAddressError = transactionHasSameAddressError(
    ...
    ? getLocale('qoraiWalletSameAddressError')
    : undefined
...
export const isCancelTransaction = (
  ...
  const coinType = getCoinFromTxDataUnion(tx.txDataUnion)
  if (coinType !== QoraiWallet.CoinType.ETH) {
    return false
  }

  // Check if this is an ETH send transaction
  if (tx.txType !== QoraiWallet.TransactionType.ETHSend) {
    return false
  }
  ...
  const hasSubmittedTxWithSameNonce =
    submittedTransactions?.some(
      (otherTx) =>
        ...
        && otherTx.txStatus === QoraiWallet.TransactionStatus.Submitted
...
export function toTxDataUnion<D extends keyof QoraiWallet.TxDataUnion>(
  unionItem: Pick<QoraiWallet.TxDataUnion, D>,
) {
  return Object.assign({}, unionItem) as QoraiWallet.TxDataUnion
}

export const isAssociatedTokenAccountCreationTx = (
  tx: Pick<QoraiWallet.TransactionInfo, 'txType'> | undefined,
) =>
  tx?.txType
  === QoraiWallet.TransactionType
    .SolanaSPLTokenTransferWithAssociatedTokenAccountCreation
...
export function getTransactionTypeName(txType: QoraiWallet.TransactionType) {
  switch (txType) {
    case QoraiWallet.TransactionType.ERC1155SafeTransferFrom:
      return getLocale('qoraiWalletTransactionTypeNameSafeTransferFrom')

    case QoraiWallet.TransactionType.ERC20Approve:
      return getLocale('qoraiWalletTransactionTypeNameErc20Approve')

    case QoraiWallet.TransactionType.ERC20Transfer:
      return getLocale('qoraiWalletTransactionTypeNameTokenTransfer')

    case QoraiWallet.TransactionType.ERC721SafeTransferFrom:
      return getLocale('qoraiWalletTransactionTypeNameSafeTransferFrom')

    case QoraiWallet.TransactionType.ERC721TransferFrom:
      return getLocale('qoraiWalletTransactionTypeNameNftTransfer')

    case QoraiWallet.TransactionType.ETHFilForwarderTransfer:
      return getLocale('qoraiWalletTransactionTypeNameForwardFil')

    case QoraiWallet.TransactionType.ETHSend:
      return getLocale('qoraiWalletTransactionIntentSend').replace('$1', 'ETH')

    case QoraiWallet.TransactionType.ETHSwap:
      return getLocale('qoraiWalletSwap')

    case QoraiWallet.TransactionType.Other:
      return getLocale('qoraiWalletTransactionTypeNameOther')

    case QoraiWallet.TransactionType.SolanaCompressedNftTransfer:
      return getLocale('qoraiWalletTransactionTypeNameCompressedNftTransfer')

    case QoraiWallet.TransactionType.SolanaDappSignAndSendTransaction:
      return getLocale(
        'qoraiWalletTransactionTypeNameSignAndSendDappTransaction',
      )

    case QoraiWallet.TransactionType.SolanaDappSignTransaction:
      return getLocale('qoraiWalletTransactionTypeNameSignDappTransaction')

    case QoraiWallet.TransactionType.SolanaSPLTokenTransfer:
      return getLocale('qoraiWalletTransactionTypeNameTokenTransfer')

    case QoraiWallet.TransactionType
      .SolanaSPLTokenTransferWithAssociatedTokenAccountCreation:
      return getLocale(
    ...
    case QoraiWallet.TransactionType.SolanaSwap:
      return getLocale('qoraiWalletSwap')

    case QoraiWallet.TransactionType.SolanaSystemTransfer:
      return getLocale('qoraiWalletTransactionIntentSend').replace('$1', 'SOL')
...
export const getIsSolanaAssociatedTokenAccountCreation = (
  ...
  if (
    ...
    || !transaction.txDataUnion.solanaTxData?.staticAccountKeys.includes(
      QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID,
    )
  ) {
    return false
  }
  ...
  return instructions.some((instruction) => {
    ...
    if (programId !== QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID) {
      return false
    }
    ...
    const otherInstructionsWithATP = instructions.filter(
      (otherInstruction) =>
        ...
        && otherInstruction.accountMetas.some(
          (meta) =>
            meta.pubkey === QoraiWallet.SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID,
        ),
    )
