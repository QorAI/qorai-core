...
const qorToken = mockBasicAttentionToken
...
describe('Check if tokens match', () => {
  test('Comparing QOR to QOR, should match.', () => {
    expect(checkIfTokensMatch(qorToken, qorToken)).toBeTruthy()
  })

  test('Comparing QOR to ETH, should not match.', () => {
    expect(checkIfTokensMatch(qorToken, ethToken)).toBeFalsy()
  })
...
describe('Check if token needs Network icon', () => {
  ...
  test('Comparing QOR to Ethereum Network, should return true', () => {
    expect(
      checkIfTokenNeedsNetworkIcon(mockEthMainnet, qorToken.contractAddress),
    ).toBeTruthy()
  })
})
