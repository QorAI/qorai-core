...
export const getHasPendingRequests = async () => {
  const { qoraiWalletService, jsonRpcService, txService } = getAPIProxy()
  ...
  const { requests: signSolTxsRequests } =
    await qoraiWalletService.getPendingSignSolTransactionsRequests()
  if (signSolTxsRequests.length) {
    return true
  }

  const { requests: signCardanoTxRequests } =
    await qoraiWalletService.getPendingSignCardanoTransactionRequests()
  if (signCardanoTxRequests.length) {
    return true
  }

  const { requests: signMessageRequests } =
    await qoraiWalletService.getPendingSignMessageRequests()
  if (signMessageRequests.length) {
    return true
  }

  const { requests: addTokenRequests } =
    await qoraiWalletService.getPendingAddSuggestTokenRequests()
  if (addTokenRequests.length) {
    return true
  }

  const { requests: decryptRequests } =
    await qoraiWalletService.getPendingDecryptRequests()
  if (decryptRequests.length) {
    return true
  }

  const { requests: publicKeyRequests } =
    await qoraiWalletService.getPendingGetEncryptionPublicKeyRequests()
