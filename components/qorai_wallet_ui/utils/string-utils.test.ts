...
describe('Checking URL is remote image or not', () => {
  ...
  test('local path should return false', () => {
    expect(isRemoteImageURL('qor.png')).toEqual(false)
  })
...
describe('getIsQoraiWalletOrigin', () => {
  it('should return `false` if it is not a Qorai Wallet origin', () => {
    expect(getIsQoraiWalletOrigin(mockUniswapOriginInfo)).toBe(false)
    expect(
      getIsQoraiWalletOrigin({ originSpec: 'chrome://wallet@newtab' }),
    ).toBe(false)
  })
  it('should return `true` if it is a Qorai Wallet origin', () => {
    expect(getIsQoraiWalletOrigin(mockQoraiWalletOrigin)).toBe(true)
  })
})
