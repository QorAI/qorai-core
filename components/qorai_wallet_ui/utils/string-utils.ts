...
import { QoraiWallet, WalletRoutes, TokenStandards } from '../constants/types'
...
export function getIsQoraiWalletOrigin({
  originSpec,
}: Pick<QoraiWallet.OriginInfo, 'originSpec'>) {
  try {
    const url = new URL(originSpec)
    return (
      (url.protocol === 'chrome:' || url.protocol === 'qorai:')
      && url.host === 'wallet'
    )
  } catch (error) {
    console.log(error)
...
export const getRampNetworkPrefix = (chainId: string, isOfframp?: boolean) => {
  switch (chainId) {
    ...
    case QoraiWallet.AVALANCHE_MAINNET_CHAIN_ID:
      return 'AVAXC'
    case QoraiWallet.BNB_SMART_CHAIN_MAINNET_CHAIN_ID:
      return 'BSC'
    case QoraiWallet.POLYGON_MAINNET_CHAIN_ID:
      return 'MATIC'
    case QoraiWallet.SOLANA_MAINNET:
      return 'SOLANA'
    case QoraiWallet.OPTIMISM_MAINNET_CHAIN_ID:
      return 'OPTIMISM'
    // Offramp uses CELO prefix
    case QoraiWallet.CELO_MAINNET_CHAIN_ID:
      return isOfframp ? 'CELO' : ''
    case QoraiWallet.FANTOM_MAINNET_CHAIN_ID:
      return 'FANTOM'
    case QoraiWallet.FILECOIN_MAINNET:
      return 'FILECOIN'
    case QoraiWallet.BITCOIN_MAINNET:
...
export const getWalletLocationTitle = (location: string) => {
  ...
  if (location.includes(WalletRoutes.FundWalletPageStart)) {
    return getLocale('qoraiWalletBuyCryptoButton')
  }
  /** Deposit crypto */
  if (location.includes(WalletRoutes.DepositFundsPageStart)) {
    return getLocale('qoraiWalletDepositFundsTitle')
  }
  /** Swap */
  ...
  if (location === WalletRoutes.Send) {
    return getLocale('qoraiWalletSend')
  }
  /** Wallet */
  return getLocale('qoraiWalletTitle')
}
...
export const getNFTTokenStandard = (token: QoraiWallet.BlockchainToken) => {
  if (token.isNft && token.coin === QoraiWallet.CoinType.SOL) {
    return TokenStandards.SPL
  }
  if (token.isErc721 && token.coin === QoraiWallet.CoinType.ETH) {
...
export const isComponentInStorybook = () => {
  const nftDisplayOrigin =
    loadTimeData.getString('qoraiWalletNftBridgeUrl') || ''
  return (
    nftDisplayOrigin === 'qoraiWalletNftBridgeUrl' || nftDisplayOrigin === ''
  )
}
