...
export const TX_CACHE_TAGS = {
  ID: (txId: QoraiWallet.TransactionInfo['id']) => ({
    type: 'Transactions' as const,
    id: txId,
  }),
  IDS: (txIds: Array<QoraiWallet.TransactionInfo['id']>) =>
    txIds.map((id) => TX_CACHE_TAGS.ID(id)),
  FOR_COIN_TYPE: (coinType: QoraiWallet.CoinType) =>
    ({
      type: 'Transactions',
  ...
  FROM_ACCOUNT_ID: (fromAccountId: QoraiWallet.AccountId) =>
    ({
      type: 'Transactions',
  ...
  LISTS: ({
    ...
    chainId: string | null
    coin: QoraiWallet.CoinType | null
    fromAccountId: QoraiWallet.AccountId | null
  }) =>
    [
