...
export const makeInitialFilteredOutNetworkKeys = () => {
  ...
  const testNetworkKeys = SupportedTestNetworks.filter(
    (chainId) => chainId !== QoraiWallet.LOCALHOST_CHAIN_ID,
  ).map((chainId) => {
    if (
...
export function getStoredPortfolioTimeframe(): QoraiWallet.AssetPriceTimeframe {
  ...
  return QoraiWallet.AssetPriceTimeframe.OneDay
}
...
export const getPersistedTokenBalancesSubset = (arg: {
  ...
  networks: Array<Pick<QoraiWallet.NetworkInfo, 'chainId' | 'coin'>>
