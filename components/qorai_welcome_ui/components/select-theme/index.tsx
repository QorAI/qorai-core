...
interface ThemeModeItemProps {
  themeType: chrome.qoraiTheme.ThemeType
  title: string
  isActive: boolean
  onChange?: (themeType: chrome.qoraiTheme.ThemeType) => void
}

const themeList = [
  { themeType: 'System', title: getLocale('qoraiWelcomeSelectThemeSystemLabel') },
  { themeType: 'Light', title: getLocale('qoraiWelcomeSelectThemeLightLabel') },
  { themeType: 'Dark', title: getLocale('qoraiWelcomeSelectThemeDarkLabel') }
]
...
function SelectTheme () {
  ...
  const [currentSelectedTheme, setCurrentTheme] = React.useState<chrome.qoraiTheme.ThemeType>('System')

  const handleSelectionChange = (themeType: chrome.qoraiTheme.ThemeType) => {
  ...
  const handleNext = () => {
    chrome.qoraiTheme.setQoraiThemeType(currentSelectedTheme)
    goForward()
  }
    ...
    <S.Box>
      <div className="view-header-box">
        ...
        <div className="view-details">
          <h1 className="view-title">{getLocale('qoraiWelcomeSelectThemeLabel')}</h1>
        </div>
      </div>
      <S.ThemeListBox>
        <div className={classnames({
          ...
          {themeList.map((entry, id) => {
            return (
              <ThemeModeItem
                key={id}
                themeType={entry.themeType as chrome.qoraiTheme.ThemeType}
                title={entry.title}
      ...
      <div className="view-note">{getLocale('qoraiWelcomeSelectThemeNote')}</div>
      <S.ActionBox>
        <Button
          kind="plain-faint"
          onClick={handleSkip}
          size="large"
        >
          {getLocale('qoraiWelcomeSkipButtonLabel')}
        </Button>
        <Button
          kind="filled"
          onClick={handleNext}
          size="large"
        >
          {getLocale('qoraiWelcomeNextButtonLabel')}
        </Button>
      </S.ActionBox>
    </S.Box>
  )
