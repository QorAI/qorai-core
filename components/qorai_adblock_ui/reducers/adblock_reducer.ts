...
const updateCustomFilters = debounce((customFilters: string) => {
  chrome.send('qorai_adblock.updateCustomFilters', [customFilters])
}, 1500)
...
const adblockReducer: Reducer<AdBlock.State | undefined> = (state: AdBlock.State | undefined, action) => {
  ...
  switch (action.type) {
    case types.ADBLOCK_ENABLE_FILTER_LIST:
      chrome.send('qorai_adblock.enableFilterList', [action.payload.uuid, action.payload.enabled])
      state = {
        ...state,
        settings: {
          ...state.settings,
    ...
    case types.ADBLOCK_GET_CUSTOM_FILTERS:
      chrome.send('qorai_adblock.getCustomFilters')
      break
    case types.ADBLOCK_GET_REGIONAL_LISTS:
      chrome.send('qorai_adblock.getRegionalLists')
      break
    case types.ADBLOCK_GET_LIST_SUBSCRIPTIONS:
      chrome.send('qorai_adblock.getListSubscriptions')
      break
    case types.ADBLOCK_SUBMIT_NEW_SUBSCRIPTION:
      chrome.send('qorai_adblock.submitNewSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_SET_SUBSCRIPTION_ENABLED:
      chrome.send('qorai_adblock.setSubscriptionEnabled', [action.payload.listUrl, action.payload.enabled])
      break
    case types.ADBLOCK_DELETE_SUBSCRIPTION:
      chrome.send('qorai_adblock.deleteSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_REFRESH_SUBSCRIPTION:
      chrome.send('qorai_adblock.refreshSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_VIEW_SUBSCRIPTION_SOURCE:
      chrome.send('qorai_adblock.viewSubscriptionSource', [action.payload.listUrl])
      break
