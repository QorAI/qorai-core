...
class QorAIDataTypeWorkerTest : public ::testing::Test {
 protected:
  explicit QorAIDataTypeWorkerTest(DataType data_type = PREFERENCES)
      : data_type_(data_type) {}

  ~QorAIDataTypeWorkerTest() override = default;
  ...
  void InitializeWithState(const DataType type, const DataTypeState& state) {
    ...
    processor->SetDisconnectCallback(base::BindOnce(
        &QorAIDataTypeWorkerTest::DisconnectProcessor, base::Unretained(this)));

    worker_ = std::make_unique<QorAIDataTypeWorker>(
  ...
  QorAIDataTypeWorker* worker() { return worker_.get(); }
 ...
 private:
  ...
  std::unique_ptr<QorAIDataTypeWorker> worker_;
...
TEST_F(QorAIDataTypeWorkerTest, ResetProgressMarker) {
  g_minimal_time_between_reset_marker =
      QorAIDataTypeWorker::MinimalTimeBetweenResetForTests();
  ...
  for (const auto& err : err_codes) {
    ...
    for (size_t i = 0;
         i < QorAIDataTypeWorker::GetFailuresToResetMarkerForTests() - 1; ++i) {
      auto local_time_override = AdvanceTimeToAllowResetMarker();
      worker()->OnCommitResponse(CommitResponseDataList(),
...
TEST_F(QorAIDataTypeWorkerTest, ResetProgressMarkerMaxPeriod) {
  NormalInitialize();
...
TEST_F(QorAIDataTypeWorkerTest, ResetProgressMarkerDisabledFeature) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kQorAISyncResetProgressMarker);
