...
constexpr char kAccountEmail[] =
    ...
    "9886900AB36F2FFF655635DBE516765E @qoranet.com";
...
class QorAISyncAuthManagerTest : public testing::Test {
 protected:
  QorAISyncAuthManagerTest() : identity_env_(&test_url_loader_factory_) {}

  ~QorAISyncAuthManagerTest() override = default;

  std::unique_ptr<QorAISyncAuthManager> CreateAuthManager() {
    return std::make_unique<QorAISyncAuthManager>(
        identity_env_.identity_manager(), &delegate_);
  }

  void SetNetworkTime() {
    qorai_sync::NetworkTimeHelper::GetInstance()->SetNetworkTimeForTest(
...
TEST_F(QorAISyncAuthManagerTest, IgnoresEventsIfNotRegistered) {
...
TEST_F(QorAISyncAuthManagerTest, GetAccessToken) {
...
TEST_F(QorAISyncAuthManagerTest, Reset) {
...
TEST_F(QorAISyncAuthManagerTest, MalformedSyncCode) {
  EXPECT_CALL(delegate(), SyncAuthAccountStateChanged).Times(0);
  EXPECT_CALL(delegate(), SyncAuthCredentialsChanged).Times(0);
  auto auth_manager = CreateAuthManager();
  ...
  auth_manager->DeriveSigningKeys("qorai5566");
  EXPECT_TRUE(
      auth_manager->GetActiveAccountInfo().account_info.account_id.empty());
}
