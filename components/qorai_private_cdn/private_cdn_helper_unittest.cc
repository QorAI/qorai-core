...
TEST(QoraiPrivateCdnHelperTest, RemovePadding) {
  ...
  for (auto invalid_input : invalid_inputs) {
    EXPECT_FALSE(qorai::private_cdn::RemovePadding(&invalid_input));
  }
  ...
  for (auto [input, output] : base::zip(inputs, outputs)) {
    ...
    EXPECT_TRUE(qorai::private_cdn::RemovePadding(&padded_string));
