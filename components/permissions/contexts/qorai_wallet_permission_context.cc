...
#include "qorai/components/permissions/permission_lifetime_utils.h"
...
QorAIWalletPermissionContext::QorAIWalletPermissionContext(
...
QorAIWalletPermissionContext::~QorAIWalletPermissionContext() = default;

bool QorAIWalletPermissionContext::IsRestrictedToSecureOrigins() const {
...
void QorAIWalletPermissionContext::RequestPermission(
  ...
  if (!qorai_wallet::ParseRequestingOrigin(
  ...
  auto sub_request_origin =
      qorai_wallet::GetSubRequestOrigin(type, origin, addr_queue.front());
...
// static
void QorAIWalletPermissionContext::AcceptOrCancel(
    const std::vector<std::string>& accounts,
...
// static
void QorAIWalletPermissionContext::Cancel(content::WebContents* web_contents) {
  PermissionRequestManager* manager =
      PermissionRequestManager::FromWebContents(web_contents);
  if (!manager) {
    return;
...
// static
bool QorAIWalletPermissionContext::HasRequestsInProgress(
...
// static
void QorAIWalletPermissionContext::RequestPermissions(
  ...
  auto origin = qorai_wallet::GetConcatOriginFromWalletAddresses(
...
QorAIWalletPermissionContext::GetAllowedAccounts(
    blink::PermissionType permission,
    content::RenderFrameHost* rfh,
    const std::vector<std::string>& addresses) {
  if (!rfh) {
...
// static
bool QorAIWalletPermissionContext::IsPermissionDenied(
    blink::PermissionType permission,
...
// static
bool QorAIWalletPermissionContext::AddPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
...
// static
bool QorAIWalletPermissionContext::HasPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
    const std::string& account,
...
// static
bool QorAIWalletPermissionContext::ResetPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
    const std::string& account) {
...
QorAIWalletPermissionContext::GetWebSitesWithPermission(
    blink::PermissionType permission,
    content::BrowserContext* context) {
  const ContentSettingsType content_settings_type =
      PermissionUtil::PermissionTypeToContentSettingsTypeSafe(permission);
...
// static
bool QorAIWalletPermissionContext::ResetWebSitePermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
...
void QorAIWalletPermissionContext::ResetAllPermissions(
    content::BrowserContext* context) {
  HostContentSettingsMap* map =
      PermissionsClient::Get()->GetSettingsMap(context);
  map->ClearSettingsForOneType(ContentSettingsType::QORAI_ETHEREUM);
  map->ClearSettingsForOneType(ContentSettingsType::QORAI_SOLANA);
  map->ClearSettingsForOneType(ContentSettingsType::QORAI_CARDANO);
}
