...
TEST_F(PermissionsClientUnitTest, QorAICanBypassEmbeddingOriginCheck) {
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    EXPECT_TRUE(client->QorAICanBypassEmbeddingOriginCheck(
    ...
    EXPECT_FALSE(client->QorAICanBypassEmbeddingOriginCheck(
        UNSAFE_TODO(cases[i]).requesting_origin, embedding_origin,
