...
async function startWDP() {
  if (window.WDP === undefined) {
    const wdp = await import('gen/qorai/web-discovery-project')
...
function onCommitted (details: chrome.webNavigation.WebNavigationTransitionCallbackDetails) {
  ...
  // devtools://, qorai://, etc.). We also make sure to only inject if we
...
if (!chrome.extension.inIncognitoContext) {
  const WEB_DISCOVERY_PREF_KEY = 'qorai.web_discovery_enabled'
  const WEB_DISCOVERY_DISABLED_BY_POLICY_KEY = 'qorai.web_discovery_disabled_by_policy'
