...
ShieldsSettingCounts GetAdsSettingCount(HostContentSettingsMap* map) {
  ContentSettingsForOneType cosmetic_rules =
      map->GetSettingsForOneType(ContentSettingsType::QORAI_COSMETIC_FILTERING);
...
void RecordShieldsDomainSettingCounts(PrefService* profile_prefs,
  ...
  VLOG(1) << "QoraiShieldsP3A: Recording counts: is_fp=" << is_fingerprint
...
void RecordShieldsDomainSettingCountsWithChange(PrefService* profile_prefs,
  ...
  if (prev_setting != nullptr) {
    ...
    VLOG(1) << "QoraiShieldsP3A: Decreasing prev setting count: prev_setting="
  ...
  VLOG(1) << "QoraiShieldsP3A: Increasing new setting count: new_setting="
...
void MaybeRecordInitialShieldsSettings(
  ...
  VLOG(1) << "QoraiShieldsP3A: Starting initial report for profile";
  ...
  VLOG(1) << "QoraiShieldsP3A: Domain FP counts: allow=" << fp_counts.allow
          << " standard=" << fp_counts.standard
          << " agg=" << fp_counts.aggressive;
  VLOG(1) << "QoraiShieldsP3A: Domain Ad counts: allow=" << ads_counts.allow
