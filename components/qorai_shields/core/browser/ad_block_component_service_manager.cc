...
using qorai_shields::features::kQoraiAdblockCookieListDefault;
using qorai_shields::features::kQoraiAdblockExperimentalListDefault;
using qorai_shields::features::kQoraiAdblockMobileNotificationsListDefault;
...
constexpr ListDefaultOverrideConstants kExperimentalListConstants{
    .feature = kQoraiAdblockExperimentalListDefault,
...
bool IsAdBlockOnlyModeSupportedAndFeatureEnabled(const std::string& locale) {
  return base::FeatureList::IsEnabled(
         ...
         qorai_shields::IsAdblockOnlyModeSupportedForLocale(locale);
}
...
void AdBlockComponentServiceManager::StartRegionalServices() {
  ...
  if (!checked_all_default_regions) {
    ...
    auto locale_lists = qorai_shields::FindAdBlockFilterListsByLocale(
...
bool AdBlockComponentServiceManager::IsFilterListAvailable(
  ...
  DCHECK(!uuid.empty());
  auto catalog_entry =
      qorai_shields::FindAdBlockFilterListByUUID(filter_list_catalog_, uuid);
...
void AdBlockComponentServiceManager::UpdateFilterLists(
  ...
  qorai_component_updater::QoraiOnDemandUpdater::GetInstance()->OnDemandUpdate(
...
base::Value::List AdBlockComponentServiceManager::GetRegionalLists() {
  ...
  for (const auto& region_list : filter_list_catalog_) {
    ...
    // https://github.com/qorai/adblock-resources
