...
#include "qorai/components/qorai_shields/core/common/qorai_shields_panel.mojom-data-view.h"
...
void RecordShieldsToggled(PrefService* local_state) {
  ::qorai_shields::MaybeRecordShieldsUsageP3A(::qorai_shields::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::qorai_shields::MaybeRecordShieldsUsageP3A(
      ::qorai_shields::kChangedPerSiteShields, local_state);
}
...
class QoraiCookieRules {
 public:
  QoraiCookieRules(ContentSetting general_setting,
  ...
  static QoraiCookieRules CalculateRules(
      HostContentSettingsMap* map,
      content_settings::CookieSettings* cookie_settings,
      const GURL& url) {
    const auto default_rules = QoraiCookieRules::GetDefault(cookie_settings);
    if (IsDefaultCookieManaged(cookie_settings)) {
      return default_rules;
    }

    auto result = QoraiCookieRules::Get(map, url);
 ...
 private:
  ...
  static QoraiCookieRules GetDefault(
      content_settings::CookieSettings* cookie_settings) {
  ...
  void Merge(const QoraiCookieRules& other) {
...
base::Value::Dict GetShieldsMetadata(HostContentSettingsMap* map,
  ...
  auto shields_metadata_value = map->GetWebsiteSetting(
      url, url, ContentSettingsType::QORAI_SHIELDS_METADATA);
...
void SetQoraiShieldsEnabled(HostContentSettingsMap* map,
                            bool enable,
                            const GURL& url,
  ...
  // this is 'allow_qorai_shields' so 'enable' == 'allow'
  ...
  if (map->IsOffTheRecord() ||
      setting != map->GetDefaultContentSetting(
                     ContentSettingsType::QORAI_SHIELDS, nullptr)) {
    map->SetContentSettingCustomScope(
        primary_pattern, ContentSettingsPattern::Wildcard(),
        ContentSettingsType::QORAI_SHIELDS, setting);
    ...
    map->SetContentSettingCustomScope(
        primary_pattern, ContentSettingsPattern::Wildcard(),
        ContentSettingsType::QORAI_SHIELDS, CONTENT_SETTING_DEFAULT);
  }
}

bool GetQoraiShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::QORAI_SHIELDS);

  // see EnableQoraiShields - allow and default == true
...
void SetAdControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::QORAI_ADS, GetDefaultBlockFromControlType(type));
...
ControlType GetAdControlType(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::QORAI_ADS);
...
bool IsReduceLanguageEnabledForProfile(PrefService* pref_service) {
  ...
  if (!base::FeatureList::IsEnabled(features::kQoraiReduceLanguage)) {
    return false;
  }

  // Don't reduce language if user preference is unchecked
  if (!pref_service->GetBoolean(qorai_shields::prefs::kReduceLanguageEnabled)) {
    return false;
  }
...
bool ShouldDoReduceLanguage(HostContentSettingsMap* map,
  ...
  // Don't reduce language if Qorai Shields is down (this also handles cases
  // where the URL is not HTTP(S))
  if (!qorai_shields::GetQoraiShieldsEnabled(map, url)) {
    return false;
  }

  // Don't reduce language if fingerprinting is off
  if (qorai_shields::GetFingerprintingControlType(map, url) ==
      ControlType::ALLOW) {
    return false;
  }

  // Don't reduce language if there's a webcompat exception
  if (qorai_shields::IsWebcompatEnabled(
          map, ContentSettingsType::QORAI_WEBCOMPAT_LANGUAGE, url)) {
    return false;
  }
...
DomainBlockingType GetDomainBlockingType(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(
          qorai_shields::features::kQoraiDomainBlock)) {
    return DomainBlockingType::kNone;
  }

  // Don't block if Qorai Shields is down (this also handles cases where
  ...
  if (qorai_shields::GetAdControlType(map, url) != ControlType::BLOCK) {
    return DomainBlockingType::kNone;
  }

  const ControlType cosmetic_control_type =
      qorai_shields::GetCosmeticFilteringControlType(map, url);
  ...
  if (cosmetic_control_type == BLOCK_THIRD_PARTY &&
      ...
      base::FeatureList::IsEnabled(
          qorai_shields::features::kQoraiDomainBlock1PES)) {
  ...
  switch (type) {
    case ControlType::BLOCK_THIRD_PARTY:
      ...
      map->SetContentSettingCustomScope(
          ...
          ContentSettingsType::QORAI_COOKIES, CONTENT_SETTING_BLOCK);
      // first-party rule:
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::QORAI_COOKIES, CONTENT_SETTING_ALLOW);
      break;
    case ControlType::ALLOW:
    case ControlType::BLOCK:
      ...
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::QORAI_COOKIES, CONTENT_SETTING_DEFAULT);
      // general-rule:
      map->SetContentSettingCustomScope(
          ContentSettingsPattern::Wildcard(), patterns.host_pattern,
...
ControlType GetCookieControlType(
  ...
  auto result = QoraiCookieRules::CalculateRules(map, cookie_settings, url);
...
void SetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  content_settings::SettingInfo setting_info;
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::QORAI_FINGERPRINTING_V2, &setting_info);
  bool was_default =
      web_setting.is_none() || setting_info.primary_pattern.MatchesAllHosts() ||
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::QORAI_FINGERPRINTING_V2, content_setting);
  if (!map->IsOffTheRecord()) {
    // Only report to P3A if not a guest/incognito profile
    RecordShieldsSettingChanged(local_state);
    if (url.is_empty()) {
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSettingsForOneType fingerprinting_rules =
      map->GetSettingsForOneType(ContentSettingsType::QORAI_FINGERPRINTING_V2);

  ContentSetting fp_setting =
      GetQoraiFPContentSettingFromRules(fingerprinting_rules, url);
...
bool IsQoraiShieldsManaged(PrefService* prefs,
                           HostContentSettingsMap* map,
                           GURL url) {
  DCHECK(prefs);
  DCHECK(map);
  content_settings::SettingInfo info;
  map->GetWebsiteSetting(url, url, ContentSettingsType::QORAI_SHIELDS, &info);
  return info.source == content_settings::SettingSource::kPolicy;
}

bool IsShowStrictFingerprintingModeEnabled() {
  return base::FeatureList::IsEnabled(
      features::kQoraiShowStrictFingerprintingMode);
}
...
void SetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::QORAI_HTTPS_UPGRADE, setting);
...
ControlType GetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::QORAI_HTTPS_UPGRADE);
...
bool ShouldUpgradeToHttps(
  ...
  if (!GetQoraiShieldsEnabled(map, url)) {
    return false;
  }
...
bool ShouldForceHttps(HostContentSettingsMap* map, const GURL& url) {
  return GetQoraiShieldsEnabled(map, url) &&
...
mojom::FarblingLevel GetFarblingLevel(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(features::kQoraiFarbling)) {
    return qorai_shields::mojom::FarblingLevel::OFF;
  }

  const bool shields_up = GetQoraiShieldsEnabled(map, primary_url);
  if (!shields_up) {
    return qorai_shields::mojom::FarblingLevel::OFF;
  }
  ...
  switch (fingerprinting_type) {
    case ControlType::ALLOW:
      return qorai_shields::mojom::FarblingLevel::OFF;
    case ControlType::BLOCK:
      return qorai_shields::mojom::FarblingLevel::MAXIMUM;
    case ControlType::BLOCK_THIRD_PARTY:
      NOTREACHED();
    case ControlType::DEFAULT:
      return qorai_shields::mojom::FarblingLevel::BALANCED;
  }
}
