...
const handleLearnMoreClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
  e.preventDefault()
  chrome.tabs.create({ url: 'https://qorai.com/privacy-features/', active: true })
}
...
function MainPanel () {
  ...
  const qoraiShieldsStatus = formatLocale(siteBlockInfo?.isQoraiShieldsEnabled
    ? 'qoraiShieldsUp'
    : 'qoraiShieldsDown', {
        $1: (content) => <span>{content}</span>,
    })

  const qoraiShieldsBrokenText = getLocale('qoraiShieldsBroken')
  const qoraiShieldsNoteKey = siteBlockInfo?.isQoraiShieldsAdBlockOnlyModeEnabled
    ? 'qoraiShieldsAdsBlockedNote'
    : siteBlockInfo?.isQoraiShieldsEnabled
      ? 'qoraiShieldsBlockedNote'
      : 'qoraiShieldsNOTBlockedNote';
  const qoraiShieldsNote = formatLocale(qoraiShieldsNoteKey, {
    $1: content => <a href='https://qorai.com/privacy-features/'
  ...
  const handleToggleChange = async (detail: { checked: boolean }) => {
    await getPanelBrowserAPI().dataHandler.setQoraiShieldsEnabled(detail.checked)
    if (detail.checked) {
  ...
  let reportSiteOrFootnoteElement = (
    <S.Footnote>
      {qoraiShieldsBrokenText}
      {!siteBlockInfo?.isQoraiShieldsAdBlockOnlyModeEnabled && <>
          {' '}
          {formatLocale('qoraiShieldsBrokenNote', {
            $1: content => <span>{content}</span>
          })}
        </>
      }
  ...
  let managedFootnoteElement = (
    <S.Footnote>
      <S.ControlBox>
        <S.ManagedIcon />
        <S.ManagedText>{getLocale('qoraiShieldsManaged')}</S.ManagedText>
      </S.ControlBox>
    </S.Footnote>
  )
  let advancedControlButtonElement = (
    isExpanded != null && !siteBlockInfo?.isQoraiShieldsAdBlockOnlyModeEnabled
  ) && (
    ...
    >
      ...
      <span>{getLocale('qoraiShieldsAdvancedCtrls')}</span>
  ...
  if (!siteBlockInfo?.isQoraiShieldsEnabled) {
    ...
    advancedControlButtonElement = (
      ...
      >
        ...
        <span>{getLocale('qoraiShieldsChangeDefaults')}</span>
      </S.GlobalDefaultsButton>
    )
    ...
    reportSiteOrFootnoteElement = (
      <S.ReportSiteBox>
        <S.ReportSiteAction>
          <span>{getLocale('qoraiShieldsReportSiteDesc')}</span>
          {siteBlockInfo?.showShieldsDisabledAdBlockOnlyModePrompt ? (
              ...
              >
                {getLocale('qoraiShieldsReportSite')}
              </Button>
            ) : (
              <Button
                kind="filled"
  ...
  return (
    <S.Box>
      ...
      <S.StatusBox>
        <S.ControlBox>
          <S.ShieldsIcon isActive={siteBlockInfo?.isQoraiShieldsEnabled ?? false}>
          ...
          <S.StatusToggle>
            <Toggle
              checked={siteBlockInfo?.isQoraiShieldsEnabled}
              onChange={handleToggleChange}
              disabled={siteBlockInfo?.isQoraiShieldsManaged}
            />
          </S.StatusToggle>
        </S.ControlBox>
        {!siteBlockInfo?.isQoraiShieldsManaged &&
          !(siteBlockInfo?.isQoraiShieldsAdBlockOnlyModeEnabled &&
            !siteBlockInfo?.isQoraiShieldsEnabled) &&
        <S.StatusFootnoteBox>
          {reportSiteOrFootnoteElement}
      ...
      {siteBlockInfo?.isQoraiShieldsAdBlockOnlyModeEnabled && (
        <AdBlockOnlyModeControlsContent
          showGlobalSettings={siteBlockInfo?.isQoraiShieldsEnabled}
        />
      )}
      {advancedControlButtonElement}
      { isExpanded &&
      ...
      }
      {
        ...
        >
          <span>{getLocale('qoraiShieldsShowAllBlockedElems')}</span>
        </S.GlobalDefaultsButton>
      }
    </S.Box>
  )
