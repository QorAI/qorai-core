...
const kLearnMoreLink = 'https://support.qorai.app/hc/en-us/articles/360022806212-How-do-I-use-Shields-while-browsing#h_01HXSZ8JPHR8YMBEZCT5M0VZTR'
...
function generateWebcompatEntries (invokedWebcompatList: Number[] | undefined) : Array<[string, Number]> {
  ...
  for (const name of names) {
    ...
    if (value > ContentSettingsType.QORAI_WEBCOMPAT_NONE &&
        value < ContentSettingsType.QORAI_WEBCOMPAT_ALL &&
        invokedWebcompatList.includes(value)) {
      const displayName = name.replace("QORAI_WEBCOMPAT_", "").replaceAll("_", " ").toLowerCase();
    ...
    <S.Box>
      ...
      <Footer>
        ...
        >
          ...
          {getLocale('qoraiShieldsStandalone')}
        </Button>
      </Footer>
    </S.Box>
  )
        ...
        <>
          <ScriptsInfo>
            ...
            <span>{<Link href="#" onClick={handleBlockAllScripts}>
              {getLocale('qoraiShieldsBlockScriptsAll')}
            </Link>
            }</span>
          </ScriptsInfo>
          <ScriptsList>
            {[...allowedScriptsByOrigin.keys()].map((origin, idx) => {
              return (<TreeNode
                ...
                permissionButtonTitle={getLocale('qoraiShieldsBlockScript')}
              />)
            })}
          </ScriptsList>
        </>
      ...
      <ScriptsInfo>
        ...
        {props.allowedList && (<span>
          {<Link href="#" onClick={handleAllowAllScripts}>
            {getLocale('qoraiShieldsAllowScriptsAll')}
          </Link>
          }</span>
        )}
      </ScriptsInfo>
      <ScriptsList>
        {[...blockedScriptsByOrigin.keys()].map((origin, idx) => {
          return (<TreeNode
            ...
            permissionButtonTitle={getLocale('qoraiShieldsAllowScriptOnce')}
          />)
        })}
      </ScriptsList>
    </SidePanel>
