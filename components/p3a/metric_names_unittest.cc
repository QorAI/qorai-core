...
// npm run test -- qorai_unit_tests --filter=P3AMetrics*
...
TEST(P3AMetrics, Searchable) {
  ...
  EXPECT_TRUE(kCollectedTypicalHistograms.contains("Qorai.Core.IsDefault"));
  EXPECT_TRUE(
      kCollectedTypicalHistograms.contains("Qorai.P3A.SentAnswersCount"));
  // Verify set membership testing isn't returning true unconditionally.
  EXPECT_FALSE(kCollectedTypicalHistograms.contains("Not.A.Qorai.Metric"));
