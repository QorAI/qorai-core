...
#include "qorai/components/p3a/message_manager.h"
...
#include "qorai/components/p3a/metric_config_utils.h"
#include "qorai/components/p3a/metric_log_type.h"
#include "qorai/components/p3a/metric_names.h"
 ...
 protected:
  ...
  std::vector<std::string> GetTestHistogramNames(MetricLogType log_type,
    ...
    for (auto histogram_i = histograms_begin; histogram_i != histograms_end;
         histogram_i++) {
      if (p3a_i < p3a_count && (histogram_i->first.starts_with("Qorai.Core") ||
...
TEST_F(P3AMessageManagerTest, ActivationDate) {
  ...
  for (const auto& [histogram_name, config] : kCollectedExpressHistograms) {
    ...
    } else if (base::StartsWith(histogram_name, "Qorai")) {
...
TEST_F(P3AMessageManagerTest, ActivationDateCleanup) {
  ...
  const char metric_name[] = "Qorai.NonExistentMetric";
  auto ref_time = base::Time::Now();
  {
...
TEST_F(P3AMessageManagerTest, EphemeralMetricOnlySentOnce) {
  ...
  message_manager_->UpdateMetricValue("Qorai.Today.UsageDaily", 1);
