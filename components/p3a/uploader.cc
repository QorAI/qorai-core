...
#include "qorai/components/p3a/constellation_helper.h"
#include "qorai/components/p3a/metric_log_type.h"
#include "qorai/components/p3a/network_annotations.h"
...
constexpr char kQoraiP3AHeader[] = "X-Qorai-P3A";
constexpr char kQoraiP3AVersionHeader[] = "Qorai-P3A-Version";
constexpr char kQoraiP3AConstellationThresholdHeader[] =
    "Qorai-P3A-Constellation-Threshold";
...
void Uploader::UploadLog(const std::string& compressed_log_data,
  ...
  resource_request->headers.SetHeader(
      kQoraiP3AVersionHeader, base::NumberToString(kCurrentP3AVersionValue));

  const size_t threshold =
      is_nebula ? kNebulaThreshold : kConstellationDefaultThreshold;
  resource_request->headers.SetHeader(kQoraiP3AConstellationThresholdHeader,
                                      base::NumberToString(threshold));
  resource_request->headers.SetHeader(kQoraiP3AHeader, "?1");
