...
#include "qorai/components/p3a/p3a_message.h"
...
#include "qorai/components/p3a/region.h"
#include "qorai/components/p3a/uploader.h"
...
std::string InferActivationDate(const MessageMetainfo& meta,
  ...
  if (truncate_to_week) {
    activation_date = qorai_stats::GetLastMondayTime(*activation_date);
  }
...
base::Value::Dict GenerateP3AMessageDict(std::string_view metric_name,
  ...
  base::Time date_of_install_monday =
      qorai_stats::GetLastMondayTime(meta.date_of_install());
  ...
  if (log_type != MetricLogType::kSlow) {
    ...
    date_of_survey = qorai_stats::GetLastMondayTime(date_of_survey);
  }
  ...
  if (log_type == MetricLogType::kSlow) {
    ...
    result.Set(kWosAttributeName,
               qorai_stats::GetIsoWeekNumber(date_of_survey));
  }
...
void MessageMetainfo::Init(PrefService* local_state,
                           std::string qorai_channel,
                           base::Time first_run_time) {
  local_state_ = local_state;
  platform_ = qorai_stats::GetPlatformIdentifier();
  general_platform_ = qorai_stats::GetGeneralPlatformIdentifier();
  channel_ = qorai_channel;
  InitVersion();
  InitRef();

  date_of_install_ = first_run_time;
  woi_ = qorai_stats::GetIsoWeekNumber(date_of_install_);

  country_code_from_timezone_raw_ =
      base::ToUpperASCII(base::CountryCodeForCurrentTimezone());
  country_code_from_locale_raw_ = qorai_l10n::GetDefaultISOCountryCodeString();
