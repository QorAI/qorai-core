...
using QoraiWaybackMachineTest = InProcessBrowserTest;

IN_PROC_BROWSER_TEST_F(QoraiWaybackMachineTest, BubbleLaunchTest) {
  BrowserView* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  auto* button_provider = browser_view->toolbar_button_provider();

  auto* model = browser()->tab_strip_model();
  auto* contents = model->GetActiveWebContents();
  QoraiWaybackMachineTabHelper* tab_helper =
      QoraiWaybackMachineTabHelper::FromWebContents(contents);
