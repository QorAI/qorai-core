...
void InitializeAllShieldSettingsInDictionary(
  ...
  base::Value::Dict* per_resource_dict =
      InitializeCommonSettingsAndGetPerResourceDictionary(dict,
                                                          last_modified_time);
  per_resource_dict->Set(qorai_shields::kAds, value);
  per_resource_dict->Set(qorai_shields::kCookies, value);
  // The old name for such an old profile.
  per_resource_dict->Set(qorai_shields::kObsoleteCosmeticFiltering, value);
  per_resource_dict->Set(qorai_shields::kFingerprintingV2, value);
  per_resource_dict->Set(qorai_shields::kHTTPUpgradableResources, value);
  per_resource_dict->Set(qorai_shields::kReferrers, value);
  per_resource_dict->Set(qorai_shields::kTrackers, value);
}

void InitializeQoraiShieldsSettingInDictionary(
    base::Value::Dict* dict,
    const base::Time& last_modified_time,
    int value) {
  base::Value::Dict* per_resource_dict =
      InitializeCommonSettingsAndGetPerResourceDictionary(dict,
                                                          last_modified_time);
  per_resource_dict->Set(qorai_shields::kQoraiShields, value);
}
...
class ShieldsSetting {
 public:
  ShieldsSetting(QoraiPrefProvider* provider,
 ...
 protected:
  ...
  raw_ptr<QoraiPrefProvider> provider_ = nullptr;
  const std::vector<GURLSourcePair> urls_;
};

class ShieldsCookieSetting : public ShieldsSetting {
 public:
  ShieldsCookieSetting(QoraiPrefProvider* provider, PrefService* prefs)
      : ShieldsSetting(
            provider,
            {{GURL(), ContentSettingsType::QORAI_COOKIES},
             {GURL("https://firstParty/*"), ContentSettingsType::QORAI_COOKIES},
             {GURL(), ContentSettingsType::QORAI_REFERRERS}}),
        prefs_(prefs) {}
 ...
 private:
  void CheckSettings(const GURL& url, ContentSetting setting) const override {
    if (prefs_->GetInteger(kQoraiShieldsSettingsVersion) < 3) {
      return ShieldsSetting::CheckSettings(url, setting);
    }
    // We need this because if version below than 3 qorai cookies patterns
    // are reversed.
    for (const auto& url_source : urls_) {
      if (url_source.second == ContentSettingsType::QORAI_COOKIES) {
        EXPECT_EQ(setting,
...
class CookieSettings : public ShieldsSetting {
 public:
  explicit CookieSettings(QoraiPrefProvider* provider)
      : ShieldsSetting(provider, {}) {}
};

class ShieldsFingerprintingSetting : public ShieldsSetting {
 public:
  explicit ShieldsFingerprintingSetting(QoraiPrefProvider* provider)
      : ShieldsSetting(provider, {}) {}
...
class ShieldsHTTPSESetting : public ShieldsSetting {
 public:
  explicit ShieldsHTTPSESetting(QoraiPrefProvider* provider)
      : ShieldsSetting(
            provider,
            {{GURL(), ContentSettingsType::QORAI_HTTP_UPGRADABLE_RESOURCES}}) {}
};

class ShieldsAdsSetting : public ShieldsSetting {
 public:
  explicit ShieldsAdsSetting(QoraiPrefProvider* provider)
      : ShieldsSetting(provider,
                       {{GURL(), ContentSettingsType::QORAI_ADS},
                        {GURL(), ContentSettingsType::QORAI_TRACKERS}}) {}
};

class ShieldsEnabledSetting : public ShieldsSetting {
 public:
  explicit ShieldsEnabledSetting(QoraiPrefProvider* provider)
      : ShieldsSetting(provider,
                       {{GURL(), ContentSettingsType::QORAI_SHIELDS}}) {}
};

class ShieldsScriptSetting : public ShieldsSetting {
 public:
  explicit ShieldsScriptSetting(QoraiPrefProvider* provider)
      : ShieldsSetting(provider, {}) {}
...
class QoraiPrefProviderTest : public testing::Test {
 public:
  QoraiPrefProviderTest() {
...
TEST_F(QoraiPrefProviderTest, TestShieldsSettingsMigration) {
  QoraiPrefProvider provider(
  ...
  GURL url("http://qorai.com:8080/");
  GURL url2("http://allowed.qorai.com:3030");
  ...
  // ------------------------------------------------------
  testing_profile()->GetPrefs()->SetInteger(kQoraiShieldsSettingsVersion, 1);
  ...
  cookie_settings.SetPreMigrationSettings(
      ContentSettingsPattern::FromString("*://help.qorai.com/*"),
      CONTENT_SETTING_BLOCK);
  // Check that settings would block qorai.com:8080, but not qorai.com:5555.
  cookie_settings.CheckSettingsWouldBlock(url);
  cookie_settings.CheckSettingsWouldAllow(url2);
  cookie_settings.CheckSettingsAreDefault(GURL("http://qorai.com:5555"));

  // Finterprinting.
  fp_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_ALLOW);
  // Check that settings would allow qorai.com:8080, but not qorai.com:5555.
  fp_settings.CheckSettingsWouldAllow(url);
  fp_settings.CheckSettingsAreDefault(GURL("http://qorai.com:5555"));

  // HTTPSE.
  httpse_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_BLOCK);
  // Check that settings would block qorai.com:8080, but not qorai.com:5555.
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsAreDefault(GURL("http://qorai.com:5555"));

  // Ads.
  ads_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_ALLOW);
  // Check that settings would allow qorai.com:8080, but not qorai.com:5555.
  ads_settings.CheckSettingsWouldAllow(url);
  ads_settings.CheckSettingsAreDefault(GURL("http://qorai.com:5555"));
  ...
  script_settings.CheckSettingsAreDefault(GURL("http://qorai.com:5555"));
  ...
  // ------------------------------------------------------
  // Cookies.
  // Check that settings would block qorai.com with any protocol and port.
  cookie_settings.CheckSettingsWouldBlock(url);
  cookie_settings.CheckSettingsWouldBlock(GURL("http://qorai.com:5555"));
  cookie_settings.CheckSettingsWouldBlock(GURL("https://qorai.com"));
  // Check that settings would allow allow.qorai.com with any protocol and port.
  cookie_settings.CheckSettingsWouldAllow(url2);
  cookie_settings.CheckSettingsWouldAllow(GURL("https://allowed.qorai.com"));
  // Check the pattern that didn't need to be migrated.
  cookie_settings.CheckSettingsWouldBlock(
      GURL("https://help.qorai.com/article1.html"));
  // Would not block a different domain.
  cookie_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));

  // Fingerprinting.
  // Check that settings would allow qorai.com with any protocol and port.
  fp_settings.CheckSettingsWouldAllow(url);
  fp_settings.CheckSettingsWouldAllow(GURL("http://qorai.com:5555"));
  fp_settings.CheckSettingsWouldAllow(GURL("https://qorai.com"));
  // Would not allow a different domain.
  fp_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));

  // HTTPSE.
  // Check that settings would block qorai.com with any protocol and port.
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsWouldBlock(GURL("http://qorai.com:5555"));
  // Would not block a different domain.
  httpse_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));

  // Ads.
  // Check that settings would allow qorai.com with any protocol and port.
  ads_settings.CheckSettingsWouldAllow(url);
  ads_settings.CheckSettingsWouldAllow(GURL("http://qorai.com:5555"));
  ads_settings.CheckSettingsWouldAllow(GURL("https://qorai.com"));
  // Would not allow a different domain.
  ads_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));

  // Enabled.
  // Check that settings would block qorai.com with any protocol and port.
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsWouldBlock(GURL("http://qorai.com:5555"));
  httpse_settings.CheckSettingsWouldBlock(GURL("https://qorai.com"));
  // Would not block a different domain.
  httpse_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));
  ...
  // Check that settings would block qorai.com with any protocol and port.
  script_settings.CheckSettingsWouldBlock(url);
  script_settings.CheckSettingsWouldBlock(GURL("http://qorai.com:5555"));
  script_settings.CheckSettingsWouldBlock(GURL("https://qorai.com"));
  // Would not block a different domain.
  script_settings.CheckSettingsAreDefault(GURL("http://qorai2.com"));

  provider.ShutdownOnUIThread();
}

TEST_F(QoraiPrefProviderTest, TestShieldsSettingsMigrationVersion) {
  PrefService* prefs = testing_profile()->GetPrefs();
  QoraiPrefProvider provider(prefs, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);

  // Should have migrated when constructed (with profile).
  EXPECT_EQ(4, prefs->GetInteger(kQoraiShieldsSettingsVersion));

  // Reset and check that migration runs.
  prefs->SetInteger(kQoraiShieldsSettingsVersion, 1);
  provider.MigrateShieldsSettings(/*incognito*/ false);
  EXPECT_EQ(4, prefs->GetInteger(kQoraiShieldsSettingsVersion));

  // Test that migration doesn't run for another version.
  prefs->SetInteger(kQoraiShieldsSettingsVersion, 5);
  provider.MigrateShieldsSettings(/*incognito*/ false);
  EXPECT_EQ(5, prefs->GetInteger(kQoraiShieldsSettingsVersion));

  provider.ShutdownOnUIThread();
}

TEST_F(QoraiPrefProviderTest, MigrateFPShieldsSettings) {
  QoraiPrefProvider provider(
      testing_profile()->GetPrefs(), false /* incognito */,
      true /* store_last_modified */, false /* restore_session */);
  ...
  auto rule_iterator = provider.GetRuleIterator(
      ContentSettingsType::QORAI_FINGERPRINTING_V2, false,
...
TEST_F(QoraiPrefProviderTest, TestShieldsSettingsMigrationFromResourceIDs) {
  PrefService* pref_service = testing_profile()->GetPrefs();
  QoraiPrefProvider provider(pref_service, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);
  ...
  // Disable Qorai Shields for www.qorai.com.
  base::Value::Dict* qorai_settings =
      plugins.value()->EnsureDict("www.qorai.com,*");

  const int expected_qorai_com_settings_value = 1;
  InitializeQoraiShieldsSettingInDictionary(qorai_settings,
                                            expected_last_modified,
                                            expected_qorai_com_settings_value);
  ...
  for (auto content_type : GetShieldsContentSettingsTypes()) {
    ...
    if (content_type_name == qorai_shields::kCosmeticFiltering) {
      // CosmeticFiltering migrated to the obsolete pref for the futher
      // migration.
      content_type_name = qorai_shields::kObsoleteCosmeticFiltering;
    }
    const auto& qorai_shields_dict = pref_service->GetDict(
        GetShieldsSettingUserPrefsPath(content_type_name));

    if (content_type == ContentSettingsType::QORAI_SHIELDS) {
      // We only changed the value of QORAI_SHIELDS in www.qorai.com.
      CheckMigrationFromResourceIdentifierForDictionary(
          qorai_shields_dict, "www.qorai.com,*", expected_last_modified,
          expected_qorai_com_settings_value);
    } else if (content_type == ContentSettingsType::QORAI_AUTO_SHRED) {
      // QORAI_AUTO_SHRED was added after the ResourceIdentifier migration
      // and never existed in that old format, so we can skip validating it.
      EXPECT_TRUE(qorai_shields_dict.empty());
    } else {
      // All the other settings we changed them globally and in www.example.com.
      CheckMigrationFromResourceIdentifierForDictionary(
          qorai_shields_dict, "*,*", expected_last_modified,
          expected_global_settings_value);
      CheckMigrationFromResourceIdentifierForDictionary(
          qorai_shields_dict, "www.example.com,*", expected_last_modified,
          expected_example_com_settings_value);
    }
  }
...
TEST_F(QoraiPrefProviderTest, TestShieldsSettingsMigrationFromUnknownSettings) {
  PrefService* pref_service = testing_profile()->GetPrefs();
  QoraiPrefProvider provider(pref_service, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);
  ...
  // names, so that we can test that Qorai doesn't crash while attempting the
  // migration and simply ignore those unsupported names instead.
  //
  // For a list of supported names, see |kQoraiContentSettingstypes| inside the
...
TEST_F(QoraiPrefProviderTest, TestShieldsSettingsMigrationV2toV4) {
  QoraiPrefProvider provider(
      testing_profile()->GetPrefs(), false /* incognito */,
      true /* store_last_modified */, false /* restore_session */);
  ...
  GURL blocked("http://qorai.com:8080/");
  GURL allowed("http://allowed.qorai.com:3030");
  ...
  testing_profile()->GetPrefs()->SetInteger(kQoraiShieldsSettingsVersion, 2);
  provider.MigrateShieldsSettings(/*incognito*/ false);

  shields_cookie_settings.CheckSettingsWouldAllow(allowed);

  // QORAI_COOKIES blocked but COOKIES allowed.
  ...
  // V3 to V4
  testing_profile()->GetPrefs()->SetInteger(kQoraiShieldsSettingsVersion, 3);
  provider.MigrateShieldsSettings(/*incognito*/ false);
...
TEST_F(QoraiPrefProviderTest, EnsureNoWildcardEntries) {
  QoraiPrefProvider provider(
      testing_profile()->GetPrefs(), false /* incognito */,
      true /* store_last_modified */, false /* restore_session */);
  ShieldsEnabledSetting shields_enabled_settings(&provider);
  ...
  // Remove wildcards
  provider.EnsureNoWildcardEntries(ContentSettingsType::QORAI_SHIELDS);
...
TEST_F(QoraiPrefProviderTest, CosmeticFilteringMigration) {
  ...
  // BLOCK_THIRD_PARTY
  cosmetic_filtering_v1.AddRule("qorai.b3p", "*", CONTENT_SETTING_BLOCK);
  cosmetic_filtering_v1.AddRule("qorai.b3p", kFirstParty,
                                CONTENT_SETTING_ALLOW);

  // ALLOW
  cosmetic_filtering_v1.AddRule("qorai.allow", "*", CONTENT_SETTING_ALLOW);
  cosmetic_filtering_v1.AddRule("qorai.allow", kFirstParty,
                                CONTENT_SETTING_ALLOW);

  // BLOCK
  cosmetic_filtering_v1.AddRule("qorai.block", "*", CONTENT_SETTING_BLOCK);
  cosmetic_filtering_v1.AddRule("qorai.block", kFirstParty,
                                CONTENT_SETTING_BLOCK);

  // Missing setting value https://github.com/qorai/qorai-browser/issues/49861
  cosmetic_filtering_v1.AddRuleWithoutSettingValue("qorai.missing", "*");
  cosmetic_filtering_v1.AddRuleWithoutSettingValue("qorai.missing",
                                                   kFirstParty);
  ...
  testing_profile()->GetPrefs()->ClearPref(
      "qorai.cosmetic_filtering_migration");
  QoraiPrefProvider provider(
      testing_profile()->GetPrefs(), false /* incognito */,
      true /* store_last_modified */, false /* restore_session */);
  ...
  const auto block3p = qorai_shields::CosmeticFilteringSetting::ToValue(
      qorai_shields::ControlType::BLOCK_THIRD_PARTY);
  const auto allow = qorai_shields::CosmeticFilteringSetting::ToValue(
      qorai_shields::ControlType::ALLOW);
  const auto block = qorai_shields::CosmeticFilteringSetting::ToValue(
      qorai_shields::ControlType::BLOCK);

  // Check there is no first-party rule anymore.
  EXPECT_EQ(base::Value(),
            cosmetic_filtering_v2.GetSettingDirectly("qorai.b3p", kFirstParty));
  EXPECT_EQ(block3p,
            cosmetic_filtering_v2.GetSettingDirectly("qorai.b3p", "*"));
  EXPECT_EQ(block3p, cosmetic_filtering_v2.GetContentSetting(
                         &provider, GURL("https://qorai.b3p")));

  // Check there is no first-party rule anymore.
  EXPECT_EQ(base::Value(), cosmetic_filtering_v2.GetSettingDirectly(
                               "qorai.allow", kFirstParty));
  EXPECT_EQ(allow,
            cosmetic_filtering_v2.GetSettingDirectly("qorai.allow", "*"));
  EXPECT_EQ(allow, cosmetic_filtering_v2.GetContentSetting(
                       &provider, GURL("https://qorai.allow")));

  // Check there is no first-party rule anymore.
  EXPECT_EQ(base::Value(), cosmetic_filtering_v2.GetSettingDirectly(
                               "qorai.block", kFirstParty));
  EXPECT_EQ(block,
            cosmetic_filtering_v2.GetSettingDirectly("qorai.block", "*"));
  EXPECT_EQ(block, cosmetic_filtering_v2.GetContentSetting(
                       &provider, GURL("https://qorai.block")));
