...
constexpr char kQoraiShieldsFPSettingsMigration[] =
    "qorai.shields_fp_settings_migration";
...
// static
void QoraiPrefProvider::CopyPluginSettingsForMigration(PrefService* prefs) {
  ...
  prefs->SetDict("qorai.migrate.content_settings.exceptions.plugins",
                 plugins.Clone());
...
QoraiPrefProvider::QoraiPrefProvider(PrefService* prefs,
                                     bool off_the_record,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&QoraiPrefProvider::OnCookiePrefsChanged,
  ...
  OnCookieSettingsChanged(ContentSettingsType::QORAI_COOKIES);
...
QoraiPrefProvider::~QoraiPrefProvider() = default;

void QoraiPrefProvider::ShutdownOnUIThread() {
  RemoveObserver(this);
...
// static
void QoraiPrefProvider::RegisterProfilePrefs(
    user_prefs::PrefRegistrySyncable* registry) {
  PrefProvider::RegisterProfilePrefs(registry);
  // Register shields settings migration pref.
  registry->RegisterIntegerPref(kQoraiShieldsSettingsVersion, 1);

  // migration of obsolete plugin prefs
  registry->RegisterDictionaryPref(
      "qorai.migrate.content_settings.exceptions.plugins");
...
void QoraiPrefProvider::MigrateShieldsSettings(bool incognito) {
  ...
  const int version = prefs_->GetInteger(kQoraiShieldsSettingsVersion);
  ...
  // https://github.com/qorai/qorai-browser/issues/23113
    ...
    if (!pref_change_registrar_.IsObserved(
      ...
      pref_change_registrar_.Add(
          ...
          base::BindPostTask(
              ...
              base::BindRepeating(&QoraiPrefProvider::EnsureNoWildcardEntries,
...
void QoraiPrefProvider::EnsureNoWildcardEntries(
    ContentSettingsType content_type) {
  // ContentSettingsType::QORAI_SHIELDS should not have wildcard entries, i.e.
...
void QoraiPrefProvider::MigrateShieldsSettingsFromResourceIds() {
  QoraiPrefProvider::CopyPluginSettingsForMigration(prefs_);

  const auto& plugins_dict =
      prefs_->GetDict("qorai.migrate.content_settings.exceptions.plugins");
  ...
  for (const auto [key, value] : plugins_dict) {
    ...
    if (resource_dict) {
      ...
      for (const auto&& [resource_key, resource_value] : *resource_dict) {
        ...
        if (resource_identifier == qorai_shields::kObsoleteAds) {
          shields_preference_name = qorai_shields::kAds;
        } else if (resource_identifier == qorai_shields::kObsoleteCookies) {
          shields_preference_name = qorai_shields::kObsoleteShieldsCookies;
        }
        if (resource_identifier == qorai_shields::kObsoleteCosmeticFiltering) {
        ...
        // Drop a "global" value of qorai shields, that actually shouldn't exist
        // at all since we don't have any global toggle for this.
        if (shields_preference_name == qorai_shields::kQoraiShields &&
            patterns_string == "*,*") {
          continue;
        }
  ...
  prefs_->ClearPref("qorai.migrate.content_settings.exceptions.plugins");
}

void QoraiPrefProvider::MigrateShieldsSettingsFromResourceIdsForOneType(
...
void QoraiPrefProvider::MigrateShieldsSettingsV1ToV2() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kQoraiShieldsSettingsVersion) != 1) {
    return;
  }

  // All sources in Qorai-specific ContentSettingsType(s) we want to migrate.
  ...
  prefs_->SetInteger(kQoraiShieldsSettingsVersion, 2);
}

void QoraiPrefProvider::MigrateShieldsSettingsV2ToV3() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kQoraiShieldsSettingsVersion) != 2) {
    return;
  }
  ...
  while (rule_iterator && rule_iterator->HasNext()) {
    ...
    if (old_rule->primary_pattern == wildcard &&
      ...
      // Remove default rules from QORAI_COKIES because it's already mapped to
      // the Chromium prefs.
      continue;
    }
  ...
  ClearAllContentSettingsRules(ContentSettingsType::QORAI_COOKIES,
                               content_settings::PartitionKey::WipGetDefault());
  ...
  // Mark migration as done.
  prefs_->SetInteger(kQoraiShieldsSettingsVersion, 3);
}

void QoraiPrefProvider::MigrateShieldsSettingsV3ToV4(int start_version) {
  ...
  if (start_version == 3) {
    // Because of
    // https://github.com/qorai/qorai-browser/issues/24119 the cookies
    // overwritten by the obsolete cookies. Repeat v2 to v3 migration.
    prefs_->SetInteger(kQoraiShieldsSettingsVersion, 2);
    MigrateShieldsSettingsV2ToV3();
  }
  prefs_->SetInteger(kQoraiShieldsSettingsVersion, 4);
}

void QoraiPrefProvider::MigrateShieldsSettingsV1ToV2ForOneType(
    ContentSettingsType content_type) {
  using OldRule = std::pair<ContentSettingsPattern, ContentSettingsPattern>;
  // Find rules that can be migrated and create replacement rules for them.
  std::vector<OldRule> old_rules;
...
void QoraiPrefProvider::MigrateFingerprintingSettings() {
  if (prefs_->GetBoolean(kQoraiShieldsFPSettingsMigration) || off_the_record_) {
    return;
  }
  ...
  prefs_->SetBoolean(kQoraiShieldsFPSettingsMigration, true);
}

void QoraiPrefProvider::MigrateFingerprintingSetingsToOriginScoped() {
  if (off_the_record_) {
    return;
  }
...
void QoraiPrefProvider::MigrateCosmeticFilteringSettings() {
  ...
  {
    auto rule_iterator = PrefProvider::GetRuleIterator(
        ContentSettingsType::QORAI_COSMETIC_FILTERING, false,
        content_settings::PartitionKey::WipGetDefault());
    while (rule_iterator && rule_iterator->HasNext()) {
  ...
  auto merge_values = [](const Rule* fp_rule, const Rule* general_rule) {
    ...
    if (content_settings::ValueToContentSetting(*setting) ==
      ...
      return qorai_shields::CosmeticFilteringSetting::ToValue(
  ...
  // QORAI_COSMETIC_FILTERING rules are set in pairs:
...
bool QoraiPrefProvider::SetWebsiteSetting(
  ...
  if (content_type == ContentSettingsType::COOKIES) {
    if (cookie_is_found_in(qorai_shield_down_rules_[off_the_record_])) {
      // Don't do anything with the generated Shiels-down rules. Unremovable
      // rule.
      return true;
    }
    if (cookie_is_found_in(qorai_cookie_rules_[off_the_record_])) {
      // change to type ContentSettingsType::QORAI_COOKIES
...
bool QoraiPrefProvider::SetWebsiteSettingForTest(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    base::Value&& value,
...
bool QoraiPrefProvider::SetWebsiteSettingInternal(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    base::Value&& in_value,
  ...
  if (content_settings::IsShieldsContentSettingsType(content_type) &&
    ...
    if (content_type == ContentSettingsType::QORAI_COOKIES) {
      // Default value for QORAI_COOKIES handled in chromium code. This value
      // based on default COOKIES value (which provided by DefaultPrefProvider)
      // and kCookieControlsMode pref (default value in
      // qorai::SetDefaultThirdPartyCookieBlockValue).
      return false;
    }
  ...
  if (content_type == ContentSettingsType::QORAI_FINGERPRINTING_V2 &&
...
std::unique_ptr<RuleIterator> QoraiPrefProvider::GetRuleIterator(
    ContentSettingsType content_type,
    bool incognito,
...
std::unique_ptr<Rule> QoraiPrefProvider::GetRule(
...
QoraiPrefProvider::CookieType QoraiPrefProvider::GetCookieType(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
  ...
  if (find_cookie(qorai_shield_down_rules_.at(incognito))) {
    return CookieType::kShieldsDownCookie;
  }

  if (find_cookie(qorai_cookie_rules_.at(incognito))) {
...
void QoraiPrefProvider::UpdateCookieRules(ContentSettingsType content_type,
                                          bool incognito) {
  std::vector<std::unique_ptr<Rule>> rules;
  auto old_rules = std::move(qorai_cookie_rules_[incognito]);
  auto old_shields_down_rules = std::move(qorai_shield_down_rules_[incognito]);
  qorai_cookie_rules_[incognito].clear();
  ...
  if (!google_sign_in_flag_enabled &&
    ...
    qorai_cookie_rules_[incognito].emplace_back(CloneRule(*google_auth_rule));
    ...
    qorai_cookie_rules_[incognito].emplace_back(CloneRule(*firebase_rule));
  } else if (google_sign_in_flag_enabled) {
    // Google Sign-In feature:
    // Add per-site cookie exception for Google/Firebase auth domains.
    // Get all sites that have QORAI_GOOGLE_SIGN_IN turned on, and add exception
    // for them
    auto google_sign_in_content_setting_it = PrefProvider::GetRuleIterator(
        ContentSettingsType::QORAI_GOOGLE_SIGN_IN, incognito,
        content_settings::PartitionKey::WipGetDefault());
    while (google_sign_in_content_setting_it &&
      ...
      // for the QORAI_GOOGLE_SIGN_IN permission.
  ...
  {
    auto qorai_shields_iterator = PrefProvider::GetRuleIterator(
        ContentSettingsType::QORAI_SHIELDS, incognito,
        content_settings::PartitionKey::WipGetDefault());
    while (qorai_shields_iterator && qorai_shields_iterator->HasNext()) {
      shield_rules.emplace_back(
          CloneRule(CHECK_DEREF(qorai_shields_iterator->Next().get())));
    }
  }

  // Add qorai cookies after checking shield status.
  {
    auto qorai_cookies_iterator = PrefProvider::GetRuleIterator(
        ContentSettingsType::QORAI_COOKIES, incognito,
        content_settings::PartitionKey::WipGetDefault());
    // Matching cookie rules against shield rules.
    while (qorai_cookies_iterator && qorai_cookies_iterator->HasNext()) {
      auto rule = qorai_cookies_iterator->Next();
      if (IsActive(rule.get(), shield_rules)) {
        rules.emplace_back(CloneRule(CHECK_DEREF(rule.get())));
        qorai_cookie_rules_[incognito].emplace_back(
  ...
  for (const auto& shield_rule : shield_rules) {
    ...
    if (ValueToContentSetting(shield_rule->value) == CONTENT_SETTING_BLOCK) {
      ...
      rules.emplace_back(std::make_unique<Rule>(
          ...
          ContentSettingToValue(CONTENT_SETTING_ALLOW), metadata.Clone()));
      qorai_shield_down_rules_[incognito].emplace_back(std::make_unique<Rule>(
          ContentSettingsPattern::Wildcard(), shield_rule->primary_pattern,
          ContentSettingToValue(CONTENT_SETTING_ALLOW), metadata.Clone()));
      qorai_cookie_rules_[incognito].emplace_back(std::make_unique<Rule>(
          ContentSettingsPattern::Wildcard(), shield_rule->primary_pattern,
          ContentSettingToValue(CONTENT_SETTING_ALLOW), std::move(metadata)));
    }
  }
  ...
  std::vector<std::unique_ptr<Rule>> qorai_cookie_updates;
  for (const auto& new_rule : qorai_cookie_rules_[incognito]) {
    auto match =
    ...
    if (match == old_rules.end()) {
      qorai_cookie_updates.emplace_back(CloneRule(CHECK_DEREF(new_rule.get())));
    }
  }
  ...
  for (const auto& old_rule : old_rules) {
    auto match = std::ranges::find_if(
        qorai_cookie_rules_[incognito], [&old_rule](const auto& new_rule) {
          // We only care about the patterns here because we're looking for
          // deleted rules, not changed rules.
          return new_rule->primary_pattern == old_rule->primary_pattern &&
                 new_rule->secondary_pattern == old_rule->secondary_pattern;
        });
    if (match == qorai_cookie_rules_[incognito].end()) {
      qorai_cookie_updates.emplace_back(std::make_unique<Rule>(
  ...
  // Notify qorai cookie changes as ContentSettingsType::COOKIES.
  if (initialized_ &&
      (content_type == ContentSettingsType::QORAI_COOKIES ||
       content_type == ContentSettingsType::QORAI_SHIELDS ||
       content_type == ContentSettingsType::QORAI_GOOGLE_SIGN_IN)) {
    NotifyChanges(qorai_cookie_updates, incognito);
  }
}

void QoraiPrefProvider::NotifyChanges(
    const std::vector<std::unique_ptr<Rule>>& rules,
    bool incognito) {
  for (const auto& rule : rules) {
...
void QoraiPrefProvider::OnCookiePrefsChanged(const std::string& pref) {
  OnCookieSettingsChanged(ContentSettingsType::QORAI_COOKIES);
}

void QoraiPrefProvider::OnCookieSettingsChanged(
    ContentSettingsType content_type) {
  UpdateCookieRules(content_type, true);
  UpdateCookieRules(content_type, false);
}

void QoraiPrefProvider::OnContentSettingChanged(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type) {
  if (content_type == ContentSettingsType::COOKIES ||
