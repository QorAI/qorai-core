...
SidebarItem::BuiltInItemType GetBuiltInItemTypeForLegacyURL(
  ...
  // need to be updated.
  if (url == "https://together.qorai.com/" ||
      url == "https://talk.qorai.com/") {
    return SidebarItem::BuiltInItemType::kQoraiTalk;
  }
...
void SidebarService::RegisterProfilePrefs(
  ...
  registry->RegisterBooleanPref(kQoraPanelOneShotOpen, false);
...
void SidebarService::MigratePrefSidebarBuiltInItemsToHidden() {
  ...
  built_in_items_to_hide.push_back(
      GetBuiltInItemForType(SidebarItem::BuiltInItemType::kQoraiTalk));
  built_in_items_to_hide.push_back(
...
SidebarItem SidebarService::GetBuiltInItemForType(
  ...
  switch (type) {
    case SidebarItem::BuiltInItemType::kQoraiTalk:
      if (!prefs_->GetBoolean(kQoraiTalkDisabledByPolicy)) {
        return SidebarItem::Create(
            GURL(kQoraiTalkURL),
            l10n_util::GetStringUTF16(IDS_SIDEBAR_QORAI_TALK_ITEM_TITLE),
    ...
    case SidebarItem::BuiltInItemType::kWallet: {
      if (qorai_wallet::IsAllowed(prefs_)) {
        return SidebarItem::Create(
            GURL("chrome://wallet/"),
    ...
    case SidebarItem::BuiltInItemType::kReadingList:
      return SidebarItem::Create(
          // TODO(petemill): Have these items created under qorai/browser
