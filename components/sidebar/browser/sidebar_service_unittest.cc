...
#include "qorai/components/sidebar/browser/sidebar_p3a.h"
...
constexpr sidebar::SidebarItem::BuiltInItemType
    kDefaultBuiltInItemTypesForTest[] = {
        sidebar::SidebarItem::BuiltInItemType::kQoraiTalk,
...
class SidebarServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    // Register the Qorai Talk policy preference that SidebarService now checks
    prefs_.registry()->RegisterBooleanPref(kQoraiTalkDisabledByPolicy, false);
  }
...
TEST_F(SidebarServiceTest, AddRemoveItems) {
  ...
  const SidebarItem item2 = SidebarItem::Create(
      GURL("https://www.qorai.com/"), u"qorai software",
...
TEST_F(SidebarServiceTest, UpdateItem) {
  ...
  SidebarItem qorai_item;
  const GURL qorai_url("https://qorai.com/");
  const std::u16string qorai_title(u"Qorai software");
  qorai_item.url = qorai_url;
  qorai_item.title = qorai_title;
  qorai_item.type = SidebarItem::Type::kTypeWeb;
  qorai_item.built_in_item_type = SidebarItem::BuiltInItemType::kNone;
  service_->AddItem(qorai_item);
  last_item_index++;
  EXPECT_EQ(qorai_url, service_->items()[last_item_index].url);
  ...
  service_->UpdateItem(qorai_url, qorai_url, qorai_title, u"Qorai");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  EXPECT_EQ(u"Qorai", service_->items()[last_item_index].title);

  // Update title & url.
  update.title_updated = true;
  update.url_updated = true;
  EXPECT_CALL(observer_, OnItemUpdated(testing::_, update)).Times(1);
  service_->UpdateItem(qorai_url, GURL("https://a.com/"), u"Qorai", u"a");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  ...
  // check item is not updated and it still has qorai url.
  EXPECT_CALL(observer_, OnItemUpdated(testing::_, testing::_)).Times(0);
  service_->UpdateItem(qorai_url, GURL("https://a.com/"), u"a", u"ab");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  EXPECT_EQ(qorai_url, service_->items()[last_item_index].url);
}
...
TEST_F(SidebarServiceTest, HideBuiltInItem) {
  ...
  }
  {
    ...
    dict.Set(sidebar::kSidebarItemURLKey, "https://custom1.qorai.com/");
...
TEST_F(SidebarServiceTest, MigratePrefSidebarBuiltInItemsSomeHidden) {
  ...
  {
    base::Value::Dict dict;
    dict.Set(sidebar::kSidebarItemURLKey, "https://anything.qorai.com/");
    ...
    dict.Set(sidebar::kSidebarItemBuiltInItemTypeKey,
             static_cast<int>(SidebarItem::BuiltInItemType::kQoraiTalk));
  ...
  // Verify service has migrated the "include" of kQoraiTalk to be the exclude
  ...
  auto items = service_->items();
  auto talk_iter =
      std::ranges::find(items, SidebarItem::BuiltInItemType::kQoraiTalk,
                        &SidebarItem::built_in_item_type);
  ...
  EXPECT_EQ(talk_item.url, kQoraiTalkURL);
}
...
TEST_F(SidebarServiceTest, MigratePrefSidebarBuiltInItemsNoneHidden) {
  ...
  // Also add a custom item so that the main items pref is not default value.
  {
    std::vector<SidebarItem::BuiltInItemType> hideable_types{
        SidebarItem::BuiltInItemType::kQoraiTalk,
...
TEST_F(SidebarServiceTest, BuiltInItemUpdateTestWithBuiltInItemTypeKey) {
  ...
  {
    base::Value::Dict dict;
    dict.Set(sidebar::kSidebarItemURLKey, "https://deprecated.qorai.com/");
    dict.Set(sidebar::kSidebarItemTitleKey, "Qorai together");
    dict.Set(sidebar::kSidebarItemTypeKey,
             static_cast<int>(SidebarItem::Type::kTypeBuiltIn));
    dict.Set(sidebar::kSidebarItemBuiltInItemTypeKey,
             static_cast<int>(SidebarItem::BuiltInItemType::kQoraiTalk));
  ...
  EXPECT_EQ(GURL(kQoraiTalkURL), service_->items()[0].url);
...
TEST_F(SidebarServiceOrderingTest, BuiltInItemsDefaultOrder) {
  ...
  EXPECT_TRUE(
      ValidateBuiltInTypesOrdering({SidebarItem::BuiltInItemType::kQoraiTalk,
                                    SidebarItem::BuiltInItemType::kWallet,
