...
#include "qorai/components/qorai_news/browser/peeking_card.h"
#include "qorai/components/qorai_news/browser/publishers_controller.h"
#include "qorai/components/qorai_news/browser/signal_calculator.h"
...
std::vector<mojom::FeedItemV2Ptr> GenerateBlock(FeedGenerationInfo& info,
  ...
  const int block_min_inline = features::kQoraiNewsMinBlockCards.Get();
  const int block_max_inline = features::kQoraiNewsMaxBlockCards.Get();
...
std::vector<mojom::FeedItemV2Ptr> GenerateBlockFromContentGroups(
  ...
  const double inline_discovery_ratio =
      features::kQoraiNewsInlineDiscoveryRatio.Get();
...
// (more or less) what Qorai Search does.
std::vector<mojom::FeedItemV2Ptr> GenerateTopTopicsBlock(
  ...
  const auto max_block_size =
      static_cast<size_t>(features::kQoraiNewsMaxBlockCards.Get());
...
std::vector<mojom::FeedItemV2Ptr> GenerateTopicBlock(
  ...
  uint64_t max_articles = features::kQoraiNewsMaxBlockCards.Get();
...
// configured through the |kQoraiNewsCategoryTopicRatio| FeatureParam.
...
std::vector<mojom::FeedItemV2Ptr> GenerateClusterBlock(
  ...
  auto generate_channel =
      (!channels.empty() &&
       base::RandDouble() < features::kQoraiNewsCategoryTopicRatio.Get()) ||
