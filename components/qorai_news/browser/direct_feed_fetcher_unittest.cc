...
TEST(QoraiNewsDirectFeed, ParseFeed) {
  FeedData data;
  // If this errors, probably our xml was not valid
  auto json = GetFeedJson();
  bool parse_success = qorai_news::parse_feed_bytes(
...
TEST(QoraiNewsDirectFeed, ParseWindows1251Feed) {
  FeedData data;
...
TEST(QoraiNewsDirectFeed, ParseEUCJPFeed) {
  FeedData data;
...
TEST(QoraiNewsDirectFeed, ParseFeedRegression) {
  FeedData data;
...
TEST(QoraiNewsDirectFeed, ParseToArticle) {
  // Create a feed item which should be valid as a Qorai News Article
  FeedItem item;
  item.id = "1";
...
TEST(QoraiNewsDirectFeed, ParseOnlyAllowsHTTPLinks) {
  // Create a feed item which should be invalid as a Qorai News Article
  FeedItem item;
  item.id = "1";
  item.published_timestamp = 1672793966;
  item.title = "Title";
