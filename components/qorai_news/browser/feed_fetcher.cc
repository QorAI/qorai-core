...
GURL GetFeedUrl(const std::string& locale) {
  GURL feed_url("https://" + qorai_news::GetHostname() + "/qorai-today/feed." +
                locale + "json");
  return feed_url;
}
...
void FeedFetcher::OnFetchFeedFetchedPublishers(
  ...
  if (publishers.empty()) {
    LOG(ERROR) << "Qorai News Publisher list was empty";
...
void FeedFetcher::OnFetchFeedFetchedFeed(
  ...
  if (result.response_code() != 200 || result.value_body().is_none()) {
    LOG(ERROR) << "Bad response from qorai news feed.json. Status: "
...
void FeedFetcher::OnIsUpdateAvailableFetchedPublishers(
  ...
  for (const auto& locale : locales) {
    ...
    api_request_helper_.Request(
        ...
        qorai::private_cdn_headers, {.auto_retry_on_network_change = true});
  }
}
...
void FeedFetcher::OnIsUpdateAvailableFetchedHead(
  ...
  if (etag.empty()) {
    LOG(ERROR) << "Qorai News did not get correct etag, "
