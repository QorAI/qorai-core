...
ArticleMetadata GetArticleMetadata(const mojom::FeedItemMetadataPtr& article,
  ...
  const double source_visits_min = features::kQoraiNewsSourceVisitsMin.Get();
...
const ArticleInfos& FeedGenerationInfo::GetArticleInfos() {
  if (!article_infos_) {
    article_infos_ = qorai_news::GetArticleInfos(locale_, feed_items_,
