...
class QoraiNewsInitializationPromiseTest : public testing::Test {
 public:
  QoraiNewsInitializationPromiseTest()
  ...
  ~QoraiNewsInitializationPromiseTest() override = default;
 ...
 private:
  const qorai_l10n::test::ScopedDefaultLocale locale_{"en_NZ"};
};

TEST_F(QoraiNewsInitializationPromiseTest,
...
TEST_F(QoraiNewsInitializationPromiseTest, InitializedLocaleIsCorrect) {
  SucceedAfter(0);
...
TEST_F(QoraiNewsInitializationPromiseTest, InitializationRetries) {
  SucceedAfter(1);
  auto locale = InitializeAndGetLocale();
  EXPECT_EQ("en_NZ", locale);
