...
const LoadNewContentButton = styled(NewsButton)`
  ...
  @media ${isSmallQuery} {
    top: calc(var(--bn-top-bar-height) + var(--qora-spacing-m));
  }
`
...
export default function FeedV2() {
  ...
  const { feedV2, setCustomizePage, refreshFeedV2, feedV2UpdatesAvailable, reportViewCount, reportSessionStart } = useQoraiNews()
  ...
  return <Root ref={ref as any} data-theme="dark">
    <SidebarContainer className='qorai-news-sidebar'>
    ...
    <Flex align='center' direction='column' gap={spacing.l}>
      {feedV2UpdatesAvailable &&
        <LoadNewContentButton
          className='qorai-news-load-new-content-button'
          onClick={refreshFeedV2}
        >
          {getLocale(S.QORAI_NEWS_NEW_CONTENT_AVAILABLE)}
        </LoadNewContentButton>}
      <Feed feed={feedV2} onViewCountChange={reportViewCount} onSessionStart={reportSessionStart} />
    </Flex>

    <ButtonsContainer className='qorai-news-feed-controls'>
      <ButtonSpacer>
        {isSmall && <React.Suspense fallback={null}><SidebarMenu /></React.Suspense>}
        <SettingsButton onClick={() => setCustomizePage('news')} title={getLocale(S.QORAI_NEWS_CUSTOMIZE_FEED)}>
          <Icon name="tune" />
        </SettingsButton>
        <SettingsButton isLoading={!feedV2} title={getLocale(S.QORAI_NEWS_REFRESH_FEED)} onClick={() => {
          refreshFeedV2()
