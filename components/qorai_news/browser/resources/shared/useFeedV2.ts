...
import getQoraiNewsController, { FeedV2, FeedV2Type } from "./api";
...
const fetchFeed = (feedView: FeedView) => {
  if (!loadTimeData.getBoolean('featureFlagQoraiNewsFeedV2Enabled')) return Promise.resolve(undefined)
  let promise: Promise<{ feed: FeedV2 }> | undefined
  if (feedView.startsWith('publishers/')) {
    promise = getQoraiNewsController().getPublisherFeed(feedView.split('/')[1]);
  } else if (feedView.startsWith('channels/')) {
    promise = getQoraiNewsController().getChannelFeed(feedView.split('/')[1])
  } else if (feedView === 'following') {
    promise = getQoraiNewsController().getFollowingFeed()
  } else {
    promise = getQoraiNewsController().getFeedV2()
  }
...
export const useFeedV2 = (enabled: boolean) => {
  ...
  // Add a listener for the latest hash if Qorai News is enabled. Note: We need
  ...
  const refresh = useCallback(() => {
    ...
    setFeedV2(undefined)
    getQoraiNewsController().ensureFeedV2IsUpdating()
