...
import getQoraiNewsController, { Channels, Configuration, FeedV2, Publisher, PublisherType, Publishers, isPublisherEnabled } from './api'
...
interface QoraiNewsContext {
  ...
  toggleQoraiNewsOnNTP: (enabled: boolean) => void
...
export const QoraiNewsContext = React.createContext<QoraiNewsContext>({
  locale: '',
  feedView: 'all',
  feedV2: undefined,
  ...
  toggleQoraiNewsOnNTP: (enabled: boolean) => { },
...
export function QoraiNewsContextProvider(props: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    getQoraiNewsController().getLocale().then(({ locale }) => setLocale(locale))
  ...
  const updateSuggestedPublisherIds = useCallback(async () => {
    ...
    const { suggestedPublisherIds } = await getQoraiNewsController().getSuggestedPublisherIds()
  ...
  const toggleQoraiNewsOnNTP = (shouldEnable: boolean) => {
    if (shouldEnable) {
  ...
  const reportViewCount = useCallback((newViews: number) => {
    getQoraiNewsController().onNewCardsViewed(newViews)
  }, [])

  const reportVisit = useCallback((depth: number) => {
    getQoraiNewsController().onCardVisited(depth + 1)
  }, [])

  const reportSessionStart = useCallback(() => {
    getQoraiNewsController().onInteractionSessionStarted()
  }, [])
  ...
  const reportSidebarFilterUsage = useCallback(() => {
    ...
    getQoraiNewsController().onSidebarFilterUsage()
  }, [isSidebarFilterUsed.current])

  const context = useMemo<QoraiNewsContext>(() => ({
    locale,
    feedView,
    setFeedView,
    feedV2,
    ...
    reportSessionStart,
    shouldRenderImages,
  }), [customizePage, setFeedView, feedV2, feedV2UpdatesAvailable, channels, publishers, suggestedPublisherIds, filteredPublisherIds, updateSuggestedPublisherIds, configuration, toggleQoraiNewsOnNTP, reportSidebarFilterUsage, reportViewCount, reportVisit, reportSessionStart, shouldRenderImages])

  return <QoraiNewsContext.Provider value={context}>
    {props.children}
  </QoraiNewsContext.Provider>
}

export const useQoraiNews = () => {
  return React.useContext(QoraiNewsContext)
}

export const useChannels = (options: { subscribedOnly: boolean } = { subscribedOnly: false }) => {
  const { channels } = useQoraiNews()
...
export const useChannelSubscribed = (channelName: string) => {
  const { channels, locale } = useQoraiNews()
