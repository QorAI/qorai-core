...
import { effect, font, radius, spacing } from '@qorai/qora/tokens/css/variables';
...
const AddButton = styled(SettingsButton)`
  --qora-button-padding: ${spacing.xs};
`
...
export function Item(props: { id: FeedView, name: string }) {
  const { feedView, setFeedView, reportSidebarFilterUsage } = useQoraiNews()
...
export default function Sidebar() {
  const { channels, publishers, setCustomizePage } = useQoraiNews()
  ...
  return <Container>
    <Item id='all' name={getLocale(S.QORAI_NEWS_FOR_YOU_FEED)} />
    <Item id='following' name={getLocale(S.QORAI_NEWS_FOLLOWING_FEED)} />
    <Section open>
      <summary>
        {subscribedChannels.length ? Marker : PlaceholderMarker}
        {getLocale(S.QORAI_NEWS_BROWSE_CHANNELS_HEADER)}
      ...
      {subscribedChannels.length > DEFAULT_SHOW_COUNT
        && <CustomButton faint onClick={() => setShowingMoreChannels(s => !s)}>
          {showingMoreChannels
            ? getLocale(S.QORAI_NEWS_SHOW_LESS)
            : getLocale(S.QORAI_NEWS_SHOW_ALL)}
        </CustomButton>}
    </Section>
    <Section open>
      <summary>
        {subscribedPublisherIds.length ? Marker : PlaceholderMarker}
        {getLocale(S.QORAI_NEWS_PUBLISHERS_HEADING)}
        <AddButton size="tiny" onClick={(event) => {
          event.preventDefault()
