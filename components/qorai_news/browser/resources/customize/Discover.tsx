...
import { useQoraiNews, useChannels } from '../shared/Context'
...
export default function Discover () {
  ...
  return <Flex direction='column'>
    ...
    <SearchInput type="search" placeholder={getLocale(S.QORAI_NEWS_SEARCH_PLACEHOLDER_LABEL)} value={query} onChange={e => setQuery(e.currentTarget.value)} />
    { query.length
...
function Home () {
  ...
  const { updateSuggestedPublisherIds } = useQoraiNews()
  ...
  return (
    <>
      <PopularCarousel />
      <SuggestionsCarousel />
      <DiscoverSection name={getLocale(S.QORAI_NEWS_BROWSE_CHANNELS_HEADER)}>
...
function SearchResults (props: SearchResultsProps) {
  ...
  return (
    <>
      ...
      <DiscoverSection name={getLocale(S.QORAI_NEWS_ALL_SOURCES_HEADER)}>
        ...
        {showFetchPermissionButton &&
          <div>
            <Button size='tiny' kind='plain-faint' onClick={() => search.setCanFetchUrl(true)} isLoading={search.loading}>
              {getLocale(S.QORAI_NEWS_DIRECT_SEARCH_BUTTON).replace('$1', search.feedUrlQuery ?? '')}
            </Button>
          </div>
        }
        ...
        {!search.canQueryFilterSources &&
          getLocale(S.QORAI_NEWS_SEARCH_QUERY_TOO_SHORT)
        }
        {isFetchable && !hasAnySources && !showFetchPermissionButton &&
          getLocale(S.QORAI_NEWS_DIRECT_SEARCH_NO_RESULTS).replace('$1', search.feedUrlQuery ?? '')
        }
      </DiscoverSection>
    </>
  )
