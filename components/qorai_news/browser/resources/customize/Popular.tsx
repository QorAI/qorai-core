...
const usePopularPublisherIds = () => {
  const { filteredPublisherIds, publishers, locale } = useQoraiNews()
...
export function PopularCarousel () {
  ...
  return (
    <Carousel title={<Flex justify='space-between'>
      {getLocale(S.QORAI_NEWS_POPULAR_TITLE)}
...
export function PopularPage () {
  const popularPublisherIds = usePopularPublisherIds()
  return <CustomizePage title={getLocale(S.QORAI_NEWS_POPULAR_TITLE)}>
    <DiscoverSection>
