...
const BackButtonContainer = styled.div`
  ...
  padding: 12px;
  padding-left: 34px;
  & > qora-button { max-width: max-content; }
`
...
export default function Configure() {
  ...
  const feedV2Enabled =
    loadTimeData.getBoolean('featureFlagQoraiNewsFeedV2Enabled')

  // TODO(petemill): We'll probably need to have 2 toggles, or some other
  // way to know if qorai news is "enabled" when Qorai News is exposed
  // in places other than just the NTP. For now this is pretty tied to NTP.
  const isQoraiNewsFullyEnabled = isOptInPrefEnabled && isShowOnNTPPrefEnabled

  let content: JSX.Element
  if (!isQoraiNewsFullyEnabled) {
    content = <DisabledPlaceholder enableQoraiNews={() => toggleQoraiNewsOnNTP(true)} />
  ...
  return (
    <Grid id='qorai-news-configure'>
      ...
      <Header direction="row-reverse" gap={12} align="center" justify="space-between">
        ...
        {isQoraiNewsFullyEnabled && <Flex direction="row" align="center" gap={8}>
          <HeaderText>{getLocale(S.QORAI_NEWS_SETTINGS_TITLE)}</HeaderText>
          <Toggle checked={isShowOnNTPPrefEnabled} onChange={e => toggleQoraiNewsOnNTP(e.checked)} />
          {feedV2Enabled && <OpenArticlesDropdown size='small' value={openArticlesInNewTab ? 'true' : 'false'} onChange={e => setOpenArticlesInNewTab(e.value === 'true')}>
            <span slot="label">{getLocale(S.QORAI_NEWS_OPEN_ARTICLES_IN)}</span>
            <span slot='value'>
              {openArticlesInNewTab ? getLocale(S.QORAI_NEWS_OPEN_ARTICLES_IN_NEW_TAB) : getLocale(S.QORAI_NEWS_OPEN_ARTICLES_IN_CURRENT_TAB)}
            </span>
            <qora-option value={'true'}>{getLocale(S.QORAI_NEWS_OPEN_ARTICLES_IN_NEW_TAB)}</qora-option>
            <qora-option value={'false'}>{getLocale(S.QORAI_NEWS_OPEN_ARTICLES_IN_CURRENT_TAB)}</qora-option>
      ...
      <Sidebar>
        <SourcesList />
        {!isQoraiNewsFullyEnabled && <SidebarOverlay />}
      </Sidebar>
      <Content>
        {content}
      </Content>
