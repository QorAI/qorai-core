...
const ToggleButton = styled.button`
  ...
  flex: 0 0 auto;
  cursor: pointer;
  color: var(--qorai-color-text02);
  &:hover {
    text-decoration: underline;
  }
  &:active {
    color: var(--qorai-color-interactive08);
  }
  &:focus-visible {
    outline: 1px solid var(--qorai-color-focusBorder);
    outline-offset: 4px;
  }
`
...
function FavIcon (props: { publisherId: string }) {
  ...
  return (
    <FavIconContainer>
      {faviconUrl && !error && <img loading='lazy' src={`chrome://qorai-image?url=${encodeURIComponent(faviconUrl)}`} onError={() => setError(true)} />}
    </FavIconContainer>
  )
}
  ...
  return (
    <Container direction="row" justify="space-between" align='center' gap={8}>
      ...
      <ToggleButton onClick={() => setFollowed(false)}>
        {getLocale(S.QORAI_NEWS_FOLLOW_BUTTON_FOLLOWING)}
      </ToggleButton>
    </Container>
  )
}
