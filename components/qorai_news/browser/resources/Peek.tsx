...
const NewsButton = styled.button`
  ...
  & > qora-icon[name="carat-down"] {
    --qora-icon-color: rgba(255, 255, 255, 0.25);
  }
...
export default function Peek() {
  const { feedV2, isShowOnNTPPrefEnabled, isOptInPrefEnabled } = useQoraiNews()
  ...
  // For some reason |createGlobalStyle| doesn't seem to work in Qorai Core
  ...
  return isShowOnNTPPrefEnabled
    ? <Container>
      {showNewsButton && <NewsButton onClick={scrollToNews}>
        <Icon name='product-qorai-news' />
        {getLocale(S.QORAI_NEWS_NEWS_PEEK)}
