...
import { color, effect, font, radius, spacing } from '@qorai/qora/tokens/css/variables';
import styled from "styled-components";
import SecureLink, { SecureLinkProps, defaultAllowedSchemes, validateScheme } from '$web-common/SecureLink';
import { configurationCache, useQoraiNews } from '../shared/Context';
...
const HidableImage = ({
  ...
  const { shouldRenderImages } = useQoraiNews()
  ...
  if (src) {
    src = 'chrome://qorai-image?url=' + encodeURIComponent(src);
  }
...
export const qoraiNewsCardClickHandler = (href: string | undefined, allowedSchemes: string[] = defaultAllowedSchemes) => (e: React.MouseEvent) => {
...
interface QoraiNewsLinkProps extends SecureLinkProps {
  feedDepth?: number
}

export function QoraiNewsLink({ feedDepth, ...rest }: QoraiNewsLinkProps) {
  const props = { feeddepth: feedDepth, ...rest }
  const { openArticlesInNewTab, reportVisit } = useQoraiNews()
  return <SecureLink
    {...props}
    onClick={e => {
      e.stopPropagation()
