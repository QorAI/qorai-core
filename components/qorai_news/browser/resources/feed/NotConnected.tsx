...
export default function NotConnected() {
  const {  refreshFeedV2 } = useQoraiNews()
  return <Container align='center' direction='column' justify='center' gap={spacing.m}>
    ...
    <Title>{getLocale(S.QORAI_NEWS_OFFLINE_TITLE)}</Title>
    <div>
      {getLocale(S.QORAI_NEWS_OFFLINE_MESSAGE)}
    </div>
    <NewsButton onClick={refreshFeedV2}>
      {getLocale(S.QORAI_NEWS_REFRESH_FEED)}
    </NewsButton>
  </Container>
}
