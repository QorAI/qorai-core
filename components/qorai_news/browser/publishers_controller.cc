...
#include "qorai/components/qorai_news/common/qorai_news.mojom-shared.h"
...
#include "qorai/components/qorai_private_cdn/headers.h"
...
void PublishersController::GetLocale(
    const SubscriptionsSnapshot& subscriptions,
    mojom::QoraiNewsController::GetLocaleCallback callback) {
  GetOrFetchPublishers(
      subscriptions,
      base::BindOnce(
          [](base::WeakPtr<PublishersController> controller,
...
void PublishersController::EnsurePublishersIsUpdating(
  ...
  GURL sources_url(
      base::StrCat({"https://", qorai_news::GetHostname(), "/sources.",
...
void PublishersController::UpdateDefaultLocale() {
  ...
  // Locale can be "language_Script_COUNTRY.charset@variant" but Qorai News
  // wants the format to be "language_COUNTRY".
  const std::string qorai_news_locale =
  ...
  default_locale_ = base::Contains(available_locales, qorai_news_locale)
                        ? qorai_news_locale
                        : "en_US";
}
