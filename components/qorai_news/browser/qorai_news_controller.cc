...
#include "qorai/components/qorai_news/browser/publishers_parsing.h"
...
#include "qorai/components/qorai_private_cdn/private_cdn_helper.h"
...
// Invokes a method on the QoraiNewsEngine in a background thread and invokes
// |CB| on the current thread.
#define IN_ENGINE(Method, CB, ...)                                     \
  task_runner_->PostTask(                                              \
      FROM_HERE,                                                       \
      base::BindOnce(                                                  \
          &QoraiNewsEngine::Method, engine_->AsWeakPtr(),              \
          pref_manager_.GetSubscriptions() __VA_OPT__(, ) __VA_ARGS__, \
          base::BindPostTaskToCurrentDefault(CB)))

// Invokes a method on the QoraiNewsEngine in a background thread. Unlike
// |IN_ENGINE| it doesn't take a reply callback (it's Fire and Forget).
#define IN_ENGINE_FF(Method)                                         \
  task_runner_->PostTask(                                            \
      FROM_HERE,                                                     \
      base::BindOnce(&QoraiNewsEngine::Method, engine_->AsWeakPtr(), \
                     pref_manager_.GetSubscriptions()))

QoraiNewsController::QoraiNewsController(
    PrefService* prefs,
  ...
  // Monitor kQoraiNewsSources and update feed / publisher cache
...
void QoraiNewsController::Bind(
    mojo::PendingReceiver<mojom::QoraiNewsController> receiver) {
  DVLOG(1) << __FUNCTION__;
  receivers_.Add(this, std::move(receiver));
}

void QoraiNewsController::Bind(
    mojo::PendingReceiver<mojom::QoraiNewsInternals> receiver) {
  DVLOG(1) << __FUNCTION__;
  internals_receivers_.Add(this, std::move(receiver));
}

void QoraiNewsController::ClearHistory() {
  DVLOG(1) << __FUNCTION__;
...
QoraiNewsController::MakeRemote() {
  mojo::PendingRemote<mojom::QoraiNewsController> remote;
  DVLOG(1) << __FUNCTION__;
  receivers_.Add(this, remote.InitWithNewPipeAndPassReceiver());
  return remote;
}

void QoraiNewsController::GetLocale(GetLocaleCallback callback) {
  DVLOG(1) << __FUNCTION__;
...
void QoraiNewsController::GetFeed(GetFeedCallback callback) {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    std::move(callback).Run(qorai_news::mojom::Feed::New());
    return;
  }
  ...
  if (!initialization_promise_.complete()) {
    initialization_promise_.OnceInitialized(
        base::BindOnce(&QoraiNewsController::GetFeed,
...
void QoraiNewsController::GetFollowingFeed(GetFollowingFeedCallback callback) {
  DVLOG(1) << __FUNCTION__;
  IN_ENGINE(GetFollowingFeed, std::move(callback));
}

void QoraiNewsController::GetChannelFeed(const std::string& channel,
                                         GetChannelFeedCallback callback) {
  DVLOG(1) << __FUNCTION__;
  IN_ENGINE(GetChannelFeed, std::move(callback), channel);
}

void QoraiNewsController::GetPublisherFeed(const std::string& publisher_id,
                                           GetPublisherFeedCallback callback) {
  DVLOG(1) << __FUNCTION__;
  IN_ENGINE(GetPublisherFeed, std::move(callback), publisher_id);
}

void QoraiNewsController::GetPublisherForSite(const GURL& site_url,
...
void QoraiNewsController::GetPublisherForFeed(const GURL& feed_url,
                                              GetPublisherCallback callback) {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    std::move(callback).Run(nullptr);
...
void QoraiNewsController::EnsureFeedV2IsUpdating() {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    return;
  }
...
void QoraiNewsController::GetFeedV2(GetFeedV2Callback callback) {
  DVLOG(1) << __FUNCTION__;
  // If we're only recently opted-in but we haven't yet finished adding the
  // top sources subscription (via the async functions in MaybeInitPrefs), we
  // need to wait for that to complete before we can fetch the feed.
  if (!initialization_promise_.complete()) {
    initialization_promise_.OnceInitialized(
        base::BindOnce(&QoraiNewsController::GetFeedV2,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    return;
  }
...
void QoraiNewsController::GetSignals(GetSignalsCallback callback) {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    std::move(callback).Run({});
    return;
...
void QoraiNewsController::GetPublishers(GetPublishersCallback callback) {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    std::move(callback).Run({});
    return;
...
void QoraiNewsController::AddPublishersListener(
  ...
  GetPublishers(base::BindOnce(
      [](QoraiNewsController* controller,
         mojo::PendingRemote<mojom::PublishersListener> listener,
...
void QoraiNewsController::GetSuggestedPublisherIds(
...
void QoraiNewsController::FindFeeds(const GURL& possible_feed_or_site_url,
...
void QoraiNewsController::GetChannels(GetChannelsCallback callback) {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    std::move(callback).Run({});
    return;
  ...
  if (!initialization_promise_.complete()) {
    initialization_promise_.OnceInitialized(
        base::BindOnce(&QoraiNewsController::GetChannels,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    return;
  }
...
void QoraiNewsController::AddChannelsListener(
  ...
  GetChannels(base::BindOnce(
      [](mojo::RemoteSetElementId id,
         base::WeakPtr<QoraiNewsController> controller, Channels channels) {
        if (!controller) {
          return;
        }
        ...
        auto event = qorai_news::mojom::ChannelsEvent::New();
...
void QoraiNewsController::SetChannelSubscribed(
...
void QoraiNewsController::SubscribeToNewDirectFeed(
    const GURL& feed_url,
  ...
  direct_feed_controller_.VerifyFeedUrl(
      feed_url,
      base::BindOnce(&QoraiNewsController::OnVerifiedDirectFeedUrl,
                     base::Unretained(this), feed_url, std::move(callback)));
}

void QoraiNewsController::OnVerifiedDirectFeedUrl(
    const GURL& feed_url,
    SubscribeToNewDirectFeedCallback callback,
    const bool is_valid,
...
void QoraiNewsController::RemoveDirectFeed(const std::string& publisher_id) {
  DVLOG(1) << __FUNCTION__;
...
void QoraiNewsController::GetImageData(const GURL& padded_image_url,
  ...
  // will contain (Qorai's PrivateCDN) padding or
  // be a direct image
      ...
      base::BindOnce(
          [](GetImageDataCallback callback, const bool is_padded,
            ...
            if (response_code < 200 || response_code >= 300 ||
                (is_padded &&
                 !qorai::private_cdn::RemovePadding(&body_payload))) {
...
void QoraiNewsController::SetPublisherPref(const std::string& publisher_id,
  ...
  GetPublishers(base::BindOnce(
      [](const std::string& publisher_id, mojom::UserEnabled new_status,
         QoraiNewsController* controller, Publishers publishers) {
...
void QoraiNewsController::ClearPrefs() {
  DVLOG(1) << __FUNCTION__;
  pref_manager_.ClearPrefs();
}

void QoraiNewsController::IsFeedUpdateAvailable(
...
void QoraiNewsController::AddConfigurationListener(
...
void QoraiNewsController::GetDisplayAd(GetDisplayAdCallback callback) {
  DVLOG(1) << __FUNCTION__;
  ...
  auto on_ad_received = base::BindOnce(
      [](GetDisplayAdCallback callback, const std::string& dimensions,
         base::optional_ref<const qorai_ads::InlineContentAdInfo>
             inline_content_ad) {
        ...
        // TODO(petemill): qorai_ads seems to use mojom, perhaps we can
...
void QoraiNewsController::OnInteractionSessionStarted() {
  DVLOG(1) << __FUNCTION__;
  news_metrics_.RecordAtSessionStart();
}

void QoraiNewsController::OnPromotedItemView(
  ...
  if (ads_service_ && !item_id.empty() && !creative_instance_id.empty()) {
    ads_service_->TriggerPromotedContentAdEvent(
        ...
        qorai_ads::mojom::PromotedContentAdEventType::kViewedImpression,
        /*intentional*/ base::DoNothing());
  }
}

void QoraiNewsController::OnPromotedItemVisit(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  DVLOG(1) << __FUNCTION__;
  if (ads_service_ && !item_id.empty() && !creative_instance_id.empty()) {
    ads_service_->TriggerPromotedContentAdEvent(
        item_id, creative_instance_id,
        qorai_ads::mojom::PromotedContentAdEventType::kClicked,
        /*intentional*/ base::DoNothing());
  }
}

void QoraiNewsController::OnNewCardsViewed(uint16_t card_views) {
  DVLOG(1) << __FUNCTION__;
  news_metrics_.RecordTotalActionCount(p3a::ActionType::kCardView, card_views);
}

void QoraiNewsController::OnCardVisited(uint32_t depth) {
  DVLOG(1) << __FUNCTION__;
  news_metrics_.RecordTotalActionCount(p3a::ActionType::kCardVisit, 1);
  news_metrics_.RecordVisitCardDepth(depth);
}

void QoraiNewsController::OnSidebarFilterUsage() {
  DVLOG(1) << __FUNCTION__;
  news_metrics_.RecordTotalActionCount(p3a::ActionType::kSidebarFilterUsage, 1);
}

void QoraiNewsController::OnDisplayAdVisit(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  DVLOG(1) << __FUNCTION__;
  // Validate
  ...
  ads_service_->TriggerInlineContentAdEvent(
      item_id, creative_instance_id,
      qorai_ads::mojom::InlineContentAdEventType::kClicked,
      /*intentional*/ base::DoNothing());
}

void QoraiNewsController::OnDisplayAdView(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  DVLOG(1) << __FUNCTION__;
  // Validate
  ...
  ads_service_->TriggerInlineContentAdEvent(
      item_id, creative_instance_id,
      qorai_ads::mojom::InlineContentAdEventType::kViewedImpression,
      /*intentional*/ base::DoNothing());

  news_metrics_.RecordWeeklyDisplayAdsViewedCount(true);
}

void QoraiNewsController::GetVisitedSites(GetVisitedSitesCallback callback) {
...
void QoraiNewsController::CheckForPublishersUpdate() {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    return;
  }
...
void QoraiNewsController::CheckForFeedsUpdate() {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
    return;
  }
  ...
  IN_ENGINE(CheckForFeedsUpdate,
            base::BindOnce(&QoraiNewsController::NotifyFeedChanged,
                           weak_ptr_factory_.GetWeakPtr()),
            /*refetch_data=*/true);
}

void QoraiNewsController::Prefetch() {
  VLOG(1) << __FUNCTION__;

  IN_ENGINE_FF(PrefetchFeed);
}

void QoraiNewsController::ResetEngine() {
  engine_.reset(
      new QoraiNewsEngine(url_loader_factory_->Clone(), MakeHistoryQuerier(),
                          direct_feed_fetcher_delegate_->AsWeakPtr()));
}

void QoraiNewsController::ConditionallyStartOrStopTimer() {
  DVLOG(1) << __FUNCTION__;
  // If the user has just enabled the feature for the first time,
  // make sure we're setup or migrated.
  MaybeInitPrefs();
  // Refresh data on an interval only if Qorai News is enabled
  if (pref_manager_.IsEnabled()) {
    VLOG(1) << "STARTING TIMERS";
    if (!timer_feed_update_.IsRunning()) {
      timer_feed_update_.Start(FROM_HERE, base::Hours(3), this,
                               &QoraiNewsController::CheckForFeedsUpdate);
    }
    if (!timer_publishers_update_.IsRunning()) {
      timer_publishers_update_.Start(
          FROM_HERE, base::Days(1), this,
          &QoraiNewsController::CheckForPublishersUpdate);
    }
    if (!timer_prefetch_.IsRunning()) {
      timer_prefetch_.Start(FROM_HERE, base::Minutes(1), this,
                            &QoraiNewsController::Prefetch);
    }

    // Notify listeners of the current publishers when QoraiNews is enabled.
    GetPublishers(base::BindOnce(
        [](base::WeakPtr<QoraiNewsController> controller,
           Publishers publishers) {
          if (!controller) {
            return;
          }

          auto event = qorai_news::mojom::PublishersEvent::New();
          event->addedOrUpdated = std::move(publishers);
    ...
    // Notify listeners of the current channels when QoraiNews is enabled.
    GetChannels(base::BindOnce(
        [](base::WeakPtr<QoraiNewsController> controller, Channels channels) {
          if (!controller) {
            return;
...
void QoraiNewsController::MaybeInitPrefs() {
  DVLOG(1) << __FUNCTION__;
  ...
  initialization_promise_.OnceInitialized(
      base::BindOnce(&QoraiNewsController::OnInitializingPrefsComplete,
                     weak_ptr_factory_.GetWeakPtr()));
}

void QoraiNewsController::OnInitializingPrefsComplete() {
  DVLOG(1) << __FUNCTION__;
  ...
  GetChannels(
      base::BindOnce([](Channels channels) {
        ...
        return event;
          ...
          .Then(base::BindOnce(&QoraiNewsController::NotifyChannelsChanged,
                               weak_ptr_factory_.GetWeakPtr())));
}

void QoraiNewsController::OnNetworkChanged(
  ...
  // handles the case where Qorai News is started with no network.
  IN_ENGINE_FF(EnsurePublishersIsUpdating);
}

void QoraiNewsController::OnConfigChanged() {
  DVLOG(1) << __FUNCTION__;
  ConditionallyStartOrStopTimer();
...
void QoraiNewsController::OnPublishersChanged() {
  DVLOG(1) << __FUNCTION__;
  if (!pref_manager_.IsEnabled()) {
  ...
  GetPublishers(
      base::BindOnce(
          ...
          std::move(diff))
          .Then(base::BindOnce(&QoraiNewsController::NotifyPublishersChanged,
                               weak_ptr_factory_.GetWeakPtr())));
...
void QoraiNewsController::NotifyPublishersChanged(
...
void QoraiNewsController::OnChannelsChanged() {
  DVLOG(1) << __FUNCTION__;
  if (pref_manager_.IsEnabled()) {
...
void QoraiNewsController::NotifyChannelsChanged(mojom::ChannelsEventPtr event) {
  DVLOG(1) << __FUNCTION__;
...
void QoraiNewsController::NotifyFeedChanged(const std::string& hash) {
  DVLOG(1) << __FUNCTION__;
...
BackgroundHistoryQuerier QoraiNewsController::MakeHistoryQuerier() {
  return qorai_news::MakeHistoryQuerier(
