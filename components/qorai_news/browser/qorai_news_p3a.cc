...
NewsMetrics::NewsMetrics(PrefService* prefs, QoraiNewsPrefManager& pref_manager)
    : prefs_(prefs),
...
void NewsMetrics::RecordAtSessionStart() {
  ...
  p3a_utils::RecordFeatureUsage(prefs_, prefs::kQoraiNewsFirstSessionTime,
                                prefs::kQoraiNewsLastSessionTime);
...
void NewsMetrics::RecordWeeklyDisplayAdsViewedCount(bool is_add) {
  ...
  uint64_t total = AddToWeeklyStorageAndGetSum(
      prefs::kQoraiNewsWeeklyDisplayAdViewedCount, is_add);
  ...
  if (prefs_->GetBoolean(qorai_rewards::prefs::kEnabled)) {
...
void NewsMetrics::RecordWeeklyAddedDirectFeedsCount(int change) {
  ...
  uint64_t weekly_total = AddToWeeklyStorageAndGetSum(
      prefs::kQoraiNewsWeeklyAddedDirectFeedsCount, change);
...
void NewsMetrics::RecordTotalActionCount(ActionType action,
  ...
  switch (action) {
    case ActionType::kCardView:
      pref_name = prefs::kQoraiNewsTotalCardViews;
      break;
    case ActionType::kCardVisit:
      pref_name = prefs::kQoraiNewsTotalCardVisits;
      break;
    case ActionType::kSidebarFilterUsage:
      pref_name = prefs::kQoraiNewsTotalSidebarFilterUsages;
      break;
  }
  CHECK(pref_name);
...
void NewsMetrics::RecordVisitCardDepth(uint32_t new_visit_card_depth) {
  WeeklyStorage total_visits_storage(prefs_, prefs::kQoraiNewsTotalCardVisits);
  auto total_visits = total_visits_storage.GetWeeklySum();

  WeeklyStorage visit_depth_sum_storage(prefs_, prefs::kQoraiNewsVisitDepthSum);
...
void NewsMetrics::RecordFeatureEnabledChange() {
  ...
  bool was_ever_enabled = prefs_->GetBoolean(prefs::kQoraiNewsWasEverEnabled);
  ...
  prefs_->SetBoolean(prefs::kQoraiNewsWasEverEnabled, true);
...
bool NewsMetrics::IsMonthlyActiveUser() {
  base::Time last_usage = prefs_->GetTime(prefs::kQoraiNewsLastSessionTime);
...
void NewsMetrics::RecordLastUsageTime() {
  ...
  p3a_utils::RecordFeatureLastUsageTimeMetric(
      prefs_, prefs::kQoraiNewsLastSessionTime, kLastUsageTimeHistogramName);
}

void NewsMetrics::RecordNewUserReturning() {
  DVLOG(1) << __FUNCTION__;
  p3a_utils::RecordFeatureNewUserReturning(
      prefs_, prefs::kQoraiNewsFirstSessionTime,
      prefs::kQoraiNewsLastSessionTime, prefs::kQoraiNewsUsedSecondDay,
...
void NewsMetrics::RecordWeeklySessionCount(bool is_add) {
  ...
  // user has scrolled to Qorai News.
  constexpr int kBuckets[] = {0, 1, 3, 7, 12, 18, 25, 1000};
  uint64_t total_session_count =
      AddToWeeklyStorageAndGetSum(prefs::kQoraiNewsWeeklySessionCount, is_add);
