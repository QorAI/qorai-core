...
#include "qorai/components/qorai_news/browser/test/wait_for_callback.h"
...
class QoraiNewsPublishersControllerTest : public testing::Test {
 public:
  QoraiNewsPublishersControllerTest()
  ...
  std::string GetSourcesUrl() {
    return "https://" + qorai_news::GetHostname() + "/sources." +
           qorai_news::kRegionUrlPart + "json";
  }

  void SetSubscribedSources(const std::vector<std::string>& publisher_ids) {
    ScopedDictPrefUpdate update(&pref_service_, prefs::kQoraiNewsSources);
  ...
  bool CombinedSourceExists(const std::string& publisher_id) {
    const auto& value = pref_service_.GetDict(prefs::kQoraiNewsSources);
  ...
  Publishers GetPublishers() {
    auto [publishers] = WaitForCallback(base::BindOnce(
        [](QoraiNewsPublishersControllerTest* test,
  ...
  mojom::PublisherPtr GetPublisherForSite(const GURL& url) {
    auto [publisher] = WaitForCallback(base::BindOnce(
        [](QoraiNewsPublishersControllerTest* test, const GURL& url,
...
TEST_F(QoraiNewsPublishersControllerTest, CanReceiveFeeds) {
...
TEST_F(QoraiNewsPublishersControllerTest, CanGetPublisherBySiteUrl) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), kPublishersResponse,
                                       net::HTTP_OK);
  GetPublishers();
...
TEST_F(QoraiNewsPublishersControllerTest,
...
TEST_F(QoraiNewsPublishersControllerTest, CanGetPublisherByFeedUrl) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), kPublishersResponse,
                                       net::HTTP_OK);
  GetPublishers();
...
TEST_F(QoraiNewsPublishersControllerTest, NoPreferredLocale_ReturnsFirstMatch) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), R"([
    {
        "publisher_id": "111",
        "publisher_name": "Test Publisher 1 JA",
...
TEST_F(QoraiNewsPublishersControllerTest, CacheCanBeCleared) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), kPublishersResponse,
                                       net::HTTP_OK);
  GetPublishers();
...
TEST_F(QoraiNewsPublishersControllerTest, LocaleDefaultsToENUS) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), R"([
    {
        "publisher_id": "111",
        "publisher_name": "Test Publisher 1",
...
TEST_F(QoraiNewsPublishersControllerTest, CanGetPublishers) {
  test_url_loader_factory_.AddResponse(GetSourcesUrl(), kPublishersResponse,
                                       net::HTTP_OK);
