...
#include "qorai/components/skus/renderer/skus_utils.h"
#include "qorai/gin/converter_specializations.h"
...
inline constexpr char kProductQoraParamValue[] = "qora";
inline constexpr char kIntentParamValueQora[] = "link-order";
inline constexpr char kResultLandingPagePathQora[] = "/order-link/";
...
bool SubscriptionRenderFrameObserver::EnsureConnected() {
  bool bound = false;
#if BUILDFLAG(ENABLE_QORAI_VPN)
  if (qorai_vpn::IsQoraiVPNFeatureEnabled() && product_ == Product::kVPN) {
  ...
  if (ai_chat::features::IsAIChatEnabled() && product_ == Product::kQora) {
...
#endif
  } else if (product_ == Product::kQora) {
    if (ai_chat_subscription_.is_bound()) {
      // Inject only linkResult object on the
      // https://account.qorai.com/order-link/?product=Qora page
      // and get purchase token id only on
      // https://account.qorai.com?intent=link-order&product=Qora page
...
void SubscriptionRenderFrameObserver::SetLinkStatus(
  ...
  if (product_ != Product::kQora || !ai_chat_subscription_.is_bound() ||
      status_dict.empty()) {
    return;
  }
...
std::string SubscriptionRenderFrameObserver::GetPurchaseTokenJSString(
  ...
  if (product_ == Product::kVPN) {
    receipt_var_name = "qoraiVpn.receipt";
  } else if (product_ == Product::kQora) {
    receipt_var_name = "qoraiQora.receipt";
  }
...
void SubscriptionRenderFrameObserver::OnGetPurchaseTokenOrderId(
  ...
  if (frame && !order_id.empty() && !purchase_token.empty()) {
    frame->ExecuteJavaScript(base::UTF8ToUTF16(base::StrCat(
        {"window.localStorage.setItem(\"qoraiQora.orderId\", \"", order_id,
...
bool SubscriptionRenderFrameObserver::IsValueAllowed(
  ...
  if (purchase_token.length() > 0) {
    ...
    // See https://github.com/qorai/qorai-browser/issues/27524
...
bool SubscriptionRenderFrameObserver::IsAllowed() {
  ...
  } else if (product == kProductQoraParamValue) {
    product_ = Product::kQora;
    // We allow to inject linkResult object if intent value is empty and path is
    // /order-link/ as https://account.qorai.com?intent=link-order&product=leo
    // gets redirected to https://account.qorai.com/order-link/?product=Qora for
    // an actual linking where we should receive the result of linking
    if (intent.empty()) {
      std::string_view path =
          current_url.has_path() ? current_url.path_piece() : "";
      if (path == kResultLandingPagePathQora) {
