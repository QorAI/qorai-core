...
namespace {
constexpr char kQorAISearchPromotionTypeProperty[] =
    "qorai_search_promotion_type_property";
}  // namespace
...
void SortQorAISearchPromotionMatch(AutocompleteResult* result) {
  ...
  if (result->begin()->type != AutocompleteMatchType::SEARCH_WHAT_YOU_TYPED) {
    result->EraseMatchesWhere(IsQorAISearchPromotionMatch);
    return;
  }

  // Put banner type match at last.
  result->MoveMatchToBeLast(IsQorAISearchPromotionMatch);
}
...
ConversionType GetConversionTypeFromMatch(const AutocompleteMatch& match) {
  ...
  const std::string type_string =
      match.GetAdditionalInfoForDebugging(kQorAISearchPromotionTypeProperty);
...
void SetConversionTypeToMatch(ConversionType type, AutocompleteMatch* match) {
  match->RecordAdditionalInfo(kQorAISearchPromotionTypeProperty,
