...
class QorAISearchProviderTest : public testing::Test {
 public:
  QorAISearchProviderTest() {
  ...
  QorAISearchProviderTest(const QorAISearchProviderTest&) = delete;
  QorAISearchProviderTest& operator=(const QorAISearchProviderTest&) = delete;
  ...
  void SetUp() override {
    ...
    provider_ =
        base::MakeRefCounted<QorAISearchProvider>(client_.get(), nullptr);
 ...
 protected:
  ...
  scoped_refptr<QorAISearchProvider> provider_;
...
TEST_F(QorAISearchProviderTest, SearchIncludesHistoryWhenHistoryEnabled) {
...
TEST_F(QorAISearchProviderTest,
...
TEST_F(QorAISearchProviderTest, DontSendClipboardTextToSuggest) {
  // Not blocked when clipboard cont is different from URL bar.
  test_clipboard_->WriteText("best browser");
  ASSERT_NO_FATAL_FAILURE(QueryForInput(u"qorai"));
  EXPECT_TRUE(test_url_loader_factory_.IsPending(
      base::StrCat({kSuggestionUrlHost, "qorai"})));

  // Blocked when clipboard content is the same.
  test_clipboard_->WriteText("qorai_private");
  ASSERT_NO_FATAL_FAILURE(QueryForInput(u"qorai_private"));
  EXPECT_FALSE(test_url_loader_factory_.IsPending(
      base::StrCat({kSuggestionUrlHost, "qorai_private"})));
...
#if BUILDFLAG(ENABLE_STRICT_QUERY_CHECK_FOR_SEARCH_SUGGESTIONS)
TEST_F(QorAISearchProviderTest, SearchSuggestionsSendTest) {
  struct {
    std::string input;
    const bool expect_to_send_to_default_provider;
  } cases[] = {
      // QorAI specific constraints.
