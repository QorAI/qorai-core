...
using qorai_search_conversion::GetPromoURL;
using ::testing::Eq;
 ...
 public:
  ...
  void SetUp() override {
    scoped_default_locale_ =
        std::make_unique<qorai_l10n::test::ScopedDefaultLocale>("en_US");
  }

  void RegisterAndInitializePrefs(TestingPrefServiceSimple* pref_service) {
    qorai_search_conversion::RegisterPrefs(pref_service->registry());
    omnibox::RegisterProfilePrefs(pref_service->registry());
    omnibox::RegisterQorAIProfilePrefs(pref_service->registry());
  ...
  std::unique_ptr<AutocompleteController> CreateController(
      ...
      bool incognito) {
    // Set non qorai search provider to get promotion match.
  ...
  std::unique_ptr<qorai_l10n::test::ScopedDefaultLocale> scoped_default_locale_;
};
...
TEST_F(OmniboxPromotionTest, ProfileTest) {
  ...
  AutocompleteInput input(u"qorai", metrics::OmniboxEventProto::OTHER,
                          classifier_);
...
TEST_F(OmniboxPromotionTest, PromotionEntrySortTest) {
  ...
  auto qorai_search_conversion_match =
      std::ranges::find_if(controller->result(), IsQorAISearchPromotionMatch);
  EXPECT_NE(qorai_search_conversion_match, controller->result().end());

  // Located as last entry for banner type.
  EXPECT_EQ(kTotalMatchCount - 1, std::distance(controller->result().begin(),
                                                qorai_search_conversion_match));
...
TEST_F(OmniboxPromotionTest, AutocompleteResultTest) {
  ...
  SortQorAISearchPromotionMatch(&result);
  EXPECT_TRUE(IsQorAISearchPromotionMatch(*result.match_at(3)));
  ...
  for (const auto& match : result) {
    EXPECT_FALSE(IsQorAISearchPromotionMatch(match));
  }
}
