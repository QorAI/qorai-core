...
class QorAIHistoryQuickProviderTest : public testing::Test {
 public:
  QorAIHistoryQuickProviderTest() = default;
  QorAIHistoryQuickProviderTest(const QorAIHistoryQuickProviderTest&) = delete;
  QorAIHistoryQuickProviderTest& operator=(
      const QorAIHistoryQuickProviderTest&) = delete;
 ...
 protected:
  ...
  void SetUp() override {
    ...
    provider_ = base::MakeRefCounted<QorAIHistoryQuickProvider>(client_.get());
  }
 ...
 private:
  ...
  scoped_refptr<QorAIHistoryQuickProvider> provider_;
};

QorAIHistoryQuickProviderTest::SetShouldContain::SetShouldContain(
    const ACMatches& matched_urls) {
  for (const auto& iter : matched_urls)
    matches_.insert(iter.destination_url.spec());
}

void QorAIHistoryQuickProviderTest::SetShouldContain::operator()(
    const std::string& expected) {
  EXPECT_EQ(1U, matches_.erase(expected))
      << "Results did not contain '" << expected << "' but should have.";
}

TEST_F(QorAIHistoryQuickProviderTest, HasResultsWhenHistoryEnabled) {
...
TEST_F(QorAIHistoryQuickProviderTest, HasNoResultsWhenHistoryDisabled) {
