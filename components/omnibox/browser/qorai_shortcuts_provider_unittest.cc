...
class QorAIShortcutsProviderTest : public testing::Test {
 public:
  QorAIShortcutsProviderTest() = default;
  ...
  void SetUp() override {
    ...
    provider_ = base::MakeRefCounted<QorAIShortcutsProvider>(&client_);
  }
 ...
 protected:
  ...
  scoped_refptr<QorAIShortcutsProvider> provider_;
  scoped_refptr<ShortcutsBackend> backend_;
};

TEST_F(QorAIShortcutsProviderTest, SuggestionsDisabledNoResults) {
  prefs()->SetBoolean(omnibox::kHistorySuggestionsEnabled, false);
  provider_->Start(CreateAutocompleteInput("hel"), true);
  EXPECT_TRUE(provider_->matches().empty());
}

TEST_F(QorAIShortcutsProviderTest, SuggestionsEnabledHasResults) {
