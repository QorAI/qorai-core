...
#include "qorai/components/time_period_storage/weekly_event_storage.h"
...
namespace {
constexpr char kPrefName[] = "qorai.weekly_event_test";
}
...
TEST_F(WeeklyEventStorageTest, AddEvents) {
  ...
  state_->Add(TestValues::kQorAI);
  EXPECT_EQ(state_->GetLatest(), std::optional<TestValues>(TestValues::kQorAI));
}
