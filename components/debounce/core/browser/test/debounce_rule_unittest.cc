...
#include "qorai/components/debounce/core/browser/debounce_rule.h"
...
TEST(DebounceRuleUnitTest, CheckBaseCase) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(),
                     GURL("https://test.com/https://qorai.com/test/abc.jpg"),
                     "https://qorai.com/test/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, MalformedParam) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/https://qorai.com"), "",
                     true);
  }
}
...
TEST(DebounceRuleUnitTest, ParamCapturesNonURLNoPrependScheme) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/qorai.com"), "", true);
  }
}
...
TEST(DebounceRuleUnitTest, ParamCapturesNonURLWithPrependScheme) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/qorai.com"),
                     "http://qorai.com/", false);
  }
}
...
TEST(DebounceRuleUnitTest, TwoCaptureGroups) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/qorai.com/xyz/abc.jpg"),
                     "https://qorai.com/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, ThreeCaptureGroups) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(
        rule.get(),
        GURL("https://test.com/turbo/qorai.com/xyz/2022/xyzzy/abc.jpg"),
        "https://qorai.com/2022/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, PrefToggle) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterBooleanPref("qorai.de_amp.enabled", false);
  const std::string contents = R"json(
      [{
          "include": [
              "*://test.com/*"
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/qorai.com"), "", &prefs,
                     true);
  }
  prefs.SetBoolean("qorai.de_amp.enabled", true);

  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/http://qorai.com"),
                     "http://qorai.com/", &prefs, false);
  }
}
...
TEST(DebounceRuleUnitTest, ArbitrarySubdomainInInclude) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterBooleanPref("qorai.de_amp.enabled", true);
...
TEST(DebounceRuleUnitTest, UrlEncodedDestinationUrl) {
  ...
  // From https://github.com/qorai/qorai-browser/issues/22429
  auto* tracker_url =
