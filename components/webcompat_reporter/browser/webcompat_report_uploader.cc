...
#include "qorai/components/webcompat_reporter/browser/fields.h"
...
void WebcompatReportUploader::SubmitReport(mojom::ReportInfoPtr report_info) {
  ...
  std::string api_key = qorai_stats::GetAPIKey();
  ...
  if (report_info->qorai_version) {
    report_details_dict.Set(kVersionField, report_info->qorai_version.value());
  }
  ...
  if (report_info->qorai_vpn_connected) {
    report_details_dict.Set(
        kQorAIVPNEnabledField,
        report_info->qorai_vpn_connected.value() == kStringTrue);
  }
