...
TEST_F(WebcompatReportUploaderUnitTest, GenerateReport) {
  ...
  EXPECT_EQ(TestReportGeneration(std::move(report_empty)),
            base::Value::Dict().Set("api_key", qorai_stats::GetAPIKey()));
  ...
  EXPECT_EQ(
      ...
      base::Value::Dict()
          ...
          .Set("api_key", qorai_stats::GetAPIKey())
          .Set("block_scripts", *report_copy->block_scripts == "true")
          .Set("qoraiVPNEnabled", report_copy->qorai_vpn_connected.has_value())
          ...
          .Set("version", *report_copy->qorai_version)
