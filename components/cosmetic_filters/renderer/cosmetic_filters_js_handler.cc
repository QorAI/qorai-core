...
#include "qorai/components/cosmetic_filters/renderer/cosmetic_filters_js_handler.h"
          ...
          selectors.forEach(selector => {
            if ((typeof selector === 'string') &&
              ...
              try {
                ...
                console.warn('QorAI Shields ignored an invalid CSS injection: ' + rule)
              }
            }
          });
...
constexpr const char TRACE_CATEGORY[] = "qorai.adblock";
...
// qorai://tracing & qorai://histograms.
...
CosmeticFiltersJSHandler::CosmeticFiltersJSHandler(
  ...
  const bool perf_tracker_enabled = base::FeatureList::IsEnabled(
      qorai_shields::features::kCosmeticFilteringExtraPerfMetrics);
...
bool CosmeticFiltersJSHandler::ProcessURL(
  ...
  if (callback.has_value()) {
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS(
        "QorAI.CosmeticFilters.UrlCosmeticResources");
    TRACE_EVENT1("qorai.adblock", "UrlCosmeticResources", "url", url_.spec());
    ...
    TRACE_EVENT1("qorai.adblock", "UrlCosmeticResourcesSync", "url",
                 url_.spec());
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS(
        "QorAI.CosmeticFilters.UrlCosmeticResourcesSync");
    base::Value result;
    cosmetic_filters_resources_->UrlCosmeticResources(
...
void CosmeticFiltersJSHandler::ApplyRules(bool de_amp_enabled) {
  ...
  SCOPED_UMA_HISTOGRAM_TIMER_MICROS("QorAI.CosmeticFilters.ApplyRules");
  TRACE_EVENT1("qorai.adblock", "ApplyRules", "url", url_.spec());
  ...
  namespace bf = qorai_shields::features;
...
void CosmeticFiltersJSHandler::CSSRulesRoutine(
  ...
  SCOPED_UMA_HISTOGRAM_TIMER_MICROS("QorAI.CosmeticFilters.CSSRulesRoutine");
  TRACE_EVENT1("qorai.adblock", "CSSRulesRoutine", "url", url_.spec());
...
void CosmeticFiltersJSHandler::OnHiddenClassIdSelectors(
  ...
  SCOPED_UMA_HISTOGRAM_TIMER_MICROS(
      "QorAI.CosmeticFilters.OnHiddenClassIdSelectors");
  TRACE_EVENT1("qorai.adblock", "OnHiddenClassIdSelectors", "url", url_.spec());
...
void CosmeticFiltersJSHandler::ExecuteObservingBundleEntryPoint() {
  ...
  if (!bundle_injected_) {
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS(
        "QorAI.CosmeticFilters.ExecuteObservingBundleEntryPoint");
    TRACE_EVENT1("qorai.adblock", "ExecuteObservingBundleEntryPoint", "url",
                 url_.spec());
