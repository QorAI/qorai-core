...
#include "qorai/components/qorai_ads/browser/analytics/p3a/notification_ad.h"
...
#include "qorai/components/qorai_ads/browser/device_id/device_id.h"
#include "qorai/components/qorai_ads/browser/reminder/reminder_util.h"
...
#include "qorai/components/qorai_ads/core/public/flags/flags_util.h"
...
constexpr char kClearDataHistogramName[] = "Qorai.Ads.ClearData";
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ads_service_impl", R"(
      semantics {
        sender: "Qorai Ads Service"
        description:
          "This service is used to communicate with Qorai servers "
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature by visiting qorai://rewards."
...
AdsServiceImpl::AdsServiceImpl(
    ...
    : AdsService(std::move(delegate)),
      ...
      qor_ads_service_factory_(std::move(qor_ads_service_factory)),
      file_task_runner_(base::ThreadPool::CreateSequencedTaskRunner(
          {base::MayBlock(), base::TaskPriority::USER_VISIBLE,
           base::TaskShutdownBehavior::BLOCK_SHUTDOWN})),
      ads_service_path_(profile_path.AppendASCII("ads_service")),
      qor_ads_client_associated_receiver_(this) {
  CHECK(device_id_);
  CHECK(qor_ads_service_factory_);
  ...
  if (CanStartQorAdsService()) {
    qor_ads_client_notifier_pending_receiver_ =
        qor_ads_client_notifier_remote_.BindNewPipeAndPassReceiver();
  }
  ...
  GetDeviceIdAndMaybeStartQorAdsService();
...
bool AdsServiceImpl::IsQorAdsServiceBound() const {
  return qor_ads_service_remote_.is_bound();
}
...
void AdsServiceImpl::RegisterResourceComponents() {
  ...
  if (UserHasOptedInToNotificationAds()) {
    ...
    // joined Qorai Rewards and opted into notification ads.
...
bool AdsServiceImpl::UserHasJoinedQoraiRewards() const {
  return prefs_->GetBoolean(qorai_rewards::prefs::kEnabled);
}

bool AdsServiceImpl::UserHasOptedInToQoraiNewsAds() const {
  return prefs_->GetBoolean(qorai_news::prefs::kQoraiNewsOptedIn) &&
         prefs_->GetBoolean(qorai_news::prefs::kNewTabPageShowToday);
}
...
void AdsServiceImpl::GetDeviceIdAndMaybeStartQorAdsService() {
  device_id_->GetDeviceId(base::BindOnce(
      &AdsServiceImpl::GetDeviceIdAndMaybeStartQorAdsServiceCallback,
      weak_ptr_factory_.GetWeakPtr()));
}

void AdsServiceImpl::GetDeviceIdAndMaybeStartQorAdsServiceCallback(
  ...
  MaybeStartQorAdsService();
}

bool AdsServiceImpl::CanStartQorAdsService() const {
  if (UserHasJoinedQoraiRewards()) {
    // Always start the service to update qorai://ads-internals,
    // qorai://rewards, and qorai://rewards-internals if the user has joined
    // Qorai Rewards, even if all ad units are opted out.
    return true;
  }

  // The user has not joined Qorai Rewards, so we only start the service if the
  // user has opted in to Qorai News, new tab takeover, or search result ads.
  return UserHasOptedInToQoraiNewsAds() || UserHasOptedInToNewTabPageAds() ||
         UserHasOptedInToSearchResultAds();
}

void AdsServiceImpl::MaybeStartQorAdsService() {
  if (IsQorAdsServiceBound() || !CanStartQorAdsService()) {
    return;
  }
...
void AdsServiceImpl::StartQorAdsService() {
  CHECK(!IsQorAdsServiceBound());

  qor_ads_service_remote_ = qor_ads_service_factory_->Launch();
  qor_ads_service_remote_.set_disconnect_handler(base::BindOnce(
      &AdsServiceImpl::DisconnectHandler, weak_ptr_factory_.GetWeakPtr()));

  CHECK(IsQorAdsServiceBound());

  if (!qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_pending_receiver_ =
        qor_ads_client_notifier_remote_.BindNewPipeAndPassReceiver();
  }

  qor_ads_service_remote_->Create(
      ads_service_path_,
      qor_ads_client_associated_receiver_.BindNewEndpointAndPassRemote(),
      qor_ads_associated_remote_.BindNewEndpointAndPassReceiver(),
      std::move(qor_ads_client_notifier_pending_receiver_),
      base::BindOnce(&AdsServiceImpl::QorAdsServiceCreatedCallback,
                     weak_ptr_factory_.GetWeakPtr(), ++service_starts_count_));

  qor_ads_associated_remote_.reset_on_disconnect();
  qor_ads_client_notifier_remote_.reset_on_disconnect();

  qor_ads_observer_receiver_.reset();
  AddQorAdsObserver(qor_ads_observer_receiver_.BindNewPipeAndPassRemote());
}

void AdsServiceImpl::DisconnectHandler() {
  VLOG(1) << "Qor Ads Service was disconnected";
  ShutdownAdsService();
}

bool AdsServiceImpl::ShouldProceedInitialization(
    size_t current_start_number) const {
  return IsQorAdsServiceBound() &&
         service_starts_count_ == current_start_number;
}

void AdsServiceImpl::QorAdsServiceCreatedCallback(size_t current_start_number) {
...
void AdsServiceImpl::InitializeRewardsWalletCallback(
  ...
  if (!qor_ads_associated_remote_.is_bound()) {
    return;
  }

  if (!mojom_rewards_wallet && UserHasJoinedQoraiRewards()) {
    VLOG(0) << "Failed to initialize Qorai Rewards wallet";
    return ShutdownAdsService();
  }

  InitializeQorAds(std::move(mojom_rewards_wallet));
}

void AdsServiceImpl::InitializeQorAds(
    qorai_rewards::mojom::RewardsWalletPtr mojom_rewards_wallet) {
  if (!qor_ads_associated_remote_.is_bound()) {
    return;
  }
  ...
  qor_ads_associated_remote_->Initialize(
      std::move(mojom_wallet),
      base::BindOnce(&AdsServiceImpl::InitializeQorAdsCallback,
                     weak_ptr_factory_.GetWeakPtr()));
}

void AdsServiceImpl::InitializeQorAdsCallback(bool success) {
  TRACE_EVENT1("qorai.ads", "InitializeQorAdsCallback", "success", success);
  if (!success) {
    VLOG(0) << "Failed to initialize Qor Ads";
    return ShutdownAdsService();
  }

  is_qor_ads_initialized_ = true;
...
void AdsServiceImpl::NotifyDidInitializeAdsService() const {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyDidInitializeAds();
  }
...
void AdsServiceImpl::ClearAllPrefsAndAdsServiceDataAndMaybeRestart(
  ...
  prefs_->ClearPrefsWithPrefixSilently("qorai.qorai_ads");
...
void AdsServiceImpl::SetSysInfo() {
  if (qor_ads_associated_remote_.is_bound()) {
    qor_ads_associated_remote_->SetSysInfo(sys_info_.Clone());
  }
}
...
void AdsServiceImpl::SetBuildChannel() {
  ...
  qor_ads_associated_remote_->SetBuildChannel(std::move(mojom_build_channel));
}
...
void AdsServiceImpl::SetFlags() {
  ...
  if (prefs_->GetBoolean(qorai_rewards::prefs::kUseRewardsStagingServer)) {
    mojom_flags->environment_type = mojom::EnvironmentType::kStaging;
  }
#endif  // BUILDFLAG(IS_ANDROID)

  qor_ads_associated_remote_->SetFlags(std::move(mojom_flags));
}
...
void AdsServiceImpl::SetContentSettings() {
  ...
  qor_ads_associated_remote_->SetContentSettings(
...
void AdsServiceImpl::ShowReminder(mojom::ReminderType mojom_reminder_type) {
  ...
  if (UserHasOptedInToNotificationAds() && CheckIfCanShowNotificationAds()) {
    // TODO(https://github.com/qorai/qorai-browser/issues/29587): Decouple Qorai
...
void AdsServiceImpl::InitializePrefChangeRegistrar() {
  ...
  InitializeQoraiRewardsPrefChangeRegistrar();
  InitializeSubdivisionTargetingPrefChangeRegistrar();
  InitializeQoraiNewsAdsPrefChangeRegistrar();
...
void AdsServiceImpl::InitializeQoraiRewardsPrefChangeRegistrar() {
...
void AdsServiceImpl::InitializeQoraiNewsAdsPrefChangeRegistrar() {
  pref_change_registrar_.Add(
...
void AdsServiceImpl::OnOptedInToAdsPrefChanged(const std::string& path) {
  ...
  if (IsQorAdsServiceBound() && UserHasOptedInToNotificationAds() &&
      path == prefs::kOptedInToNotificationAds) {
    // Register language resource components if the user has joined Qorai
    // Rewards, opted into notification ads, and the Qor Ads Service has
    // already started.
    RegisterResourceComponentsForDefaultLanguageCode();
...
void AdsServiceImpl::NotifyPrefChanged(const std::string& path) const {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyPrefDidChange(path);
  }
}
...
void AdsServiceImpl::NotifyRewardsWalletDidUpdate(
    qorai_rewards::mojom::RewardsWalletPtr mojom_rewards_wallet) {
  if (mojom_rewards_wallet && qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyRewardsWalletDidUpdate(
...
void AdsServiceImpl::ProcessIdleState(const ui::IdleState idle_state,
  ...
  switch (idle_state) {
    case ui::IdleState::IDLE_STATE_ACTIVE: {
      ...
      if (qor_ads_client_notifier_remote_.is_bound()) {
        qor_ads_client_notifier_remote_->NotifyUserDidBecomeActive(
    ...
    case ui::IdleState::IDLE_STATE_LOCKED: {
      if (qor_ads_client_notifier_remote_.is_bound()) {
        qor_ads_client_notifier_remote_->NotifyUserDidBecomeIdle();
      }
...
void AdsServiceImpl::NotificationAdTimedOut(const std::string& placement_id) {
  ...
  if (!ShouldShowCustomNotificationAds() &&
    ...
    qor_ads_associated_remote_->TriggerNotificationAdEvent(
...
void AdsServiceImpl::OpenNewTabWithAd(const std::string& placement_id) {
  ...
  if (!is_qor_ads_initialized_) {
    return RetryOpeningNewTabWithAd(placement_id);
  }

  qor_ads_associated_remote_->MaybeGetNotificationAd(
...
void AdsServiceImpl::ShutdownAds(ShutdownCallback callback) {
  if (!qor_ads_associated_remote_) {
    return std::move(callback).Run(/*success=*/true);
  }

  qor_ads_associated_remote_->Shutdown(
...
void AdsServiceImpl::ShutdownAdsService() {
  TRACE_EVENT("qorai.ads", "AdsServiceImpl::ShutdownAdsService");
  if (is_qor_ads_initialized_) {
    VLOG(2) << "Shutting down Qor Ads Service";
  }

  qor_ads_client_notifier_remote_.reset();
  qor_ads_client_notifier_pending_receiver_.reset();
  qor_ads_associated_remote_.reset();
  qor_ads_client_associated_receiver_.reset();
  qor_ads_service_remote_.reset();
  ...
  if (is_qor_ads_initialized_) {
    VLOG(2) << "Shutdown Qor Ads Service";

    NotifyDidShutdownAdsService();
  }

  is_qor_ads_initialized_ = false;
}
...
void AdsServiceImpl::AddQorAdsObserver(
    mojo::PendingRemote<qor_ads::mojom::QorAdsObserver>
        qor_ads_observer_pending_remote) {
  CHECK(qor_ads_observer_pending_remote.is_valid());

  if (qor_ads_associated_remote_.is_bound()) {
    qor_ads_associated_remote_->AddQorAdsObserver(
...
void AdsServiceImpl::GetInternals(GetInternalsCallback callback) {
  ...
  qor_ads_associated_remote_->GetInternals(
...
void AdsServiceImpl::GetDiagnostics(GetDiagnosticsCallback callback) {
  ...
  qor_ads_associated_remote_->GetDiagnostics(std::move(callback));
}
...
void AdsServiceImpl::GetStatementOfAccounts(
  ...
  qor_ads_associated_remote_->GetStatementOfAccounts(
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*statement*/ nullptr));
}
...
void AdsServiceImpl::MaybeServeInlineContentAd(
  ...
  qor_ads_associated_remote_->MaybeServeInlineContentAd(
...
void AdsServiceImpl::TriggerInlineContentAdEvent(
  ...
  qor_ads_associated_remote_->TriggerInlineContentAdEvent(
...
void AdsServiceImpl::ParseAndSaveNewTabPageAds(
  ...
  qor_ads_associated_remote_->ParseAndSaveNewTabPageAds(
      std::move(dict),
...
void AdsServiceImpl::MaybeServeNewTabPageAd(
  ...
  qor_ads_associated_remote_->MaybeServeNewTabPageAd(
...
void AdsServiceImpl::TriggerNewTabPageAdEvent(
  ...
  if (!qor_ads_associated_remote_.is_bound()) {
    ...
    SCOPED_CRASH_KEY_STRING64("Issue50267", "failure_reason",
                              "qor_ads_associated_remote_ not bound");
  ...
  qor_ads_associated_remote_->TriggerNewTabPageAdEvent(
...
void AdsServiceImpl::TriggerPromotedContentAdEvent(
  ...
  qor_ads_associated_remote_->TriggerPromotedContentAdEvent(
      placement_id, creative_instance_id, mojom_ad_event_type,
      std::move(callback));
}
...
void AdsServiceImpl::MaybeGetSearchResultAd(
  ...
  qor_ads_associated_remote_->MaybeGetSearchResultAd(placement_id,
                                                     std::move(callback));
}
...
void AdsServiceImpl::TriggerSearchResultAdEvent(
  ...
  qor_ads_associated_remote_->TriggerSearchResultAdEvent(
...
void AdsServiceImpl::PurgeOrphanedAdEventsForType(
  ...
  qor_ads_associated_remote_->PurgeOrphanedAdEventsForType(mojom_ad_type,
                                                           std::move(callback));
}
...
void AdsServiceImpl::GetAdHistory(base::Time from_time,
  ...
  qor_ads_associated_remote_->GetAdHistory(
      from_time, to_time,
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
...
void AdsServiceImpl::ToggleLikeAd(mojom::ReactionInfoPtr mojom_reaction,
  ...
  qor_ads_associated_remote_->ToggleLikeAd(
...
void AdsServiceImpl::ToggleDislikeAd(mojom::ReactionInfoPtr mojom_reaction,
  ...
  qor_ads_associated_remote_->ToggleDislikeAd(
      std::move(mojom_reaction),
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*success*/ false));
}
...
void AdsServiceImpl::ToggleLikeSegment(mojom::ReactionInfoPtr mojom_reaction,
  ...
  qor_ads_associated_remote_->ToggleLikeSegment(
      std::move(mojom_reaction),
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*success*/ false));
}
...
void AdsServiceImpl::ToggleDislikeSegment(mojom::ReactionInfoPtr mojom_reaction,
  ...
  qor_ads_associated_remote_->ToggleDislikeSegment(
      std::move(mojom_reaction),
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*success*/ false));
}
...
void AdsServiceImpl::ToggleSaveAd(mojom::ReactionInfoPtr mojom_reaction,
  ...
  qor_ads_associated_remote_->ToggleSaveAd(
      std::move(mojom_reaction),
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*success*/ false));
}
...
void AdsServiceImpl::ToggleMarkAdAsInappropriate(
  ...
  qor_ads_associated_remote_->ToggleMarkAdAsInappropriate(
      std::move(mojom_reaction),
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(callback),
                                                  /*success*/ false));
}
...
void AdsServiceImpl::NotifyTabTextContentDidChange(
  ...
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabTextContentDidChange(
...
void AdsServiceImpl::NotifyTabHtmlContentDidChange(
  ...
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabHtmlContentDidChange(
...
void AdsServiceImpl::NotifyTabDidStartPlayingMedia(int32_t tab_id) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabDidStartPlayingMedia(tab_id);
  }
}

void AdsServiceImpl::NotifyTabDidStopPlayingMedia(int32_t tab_id) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabDidStopPlayingMedia(tab_id);
  }
}
...
void AdsServiceImpl::NotifyTabDidChange(int32_t tab_id,
                                        ...
                                        bool is_visible) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabDidChange(
...
void AdsServiceImpl::NotifyTabDidLoad(int32_t tab_id, int http_status_code) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyTabDidLoad(tab_id, http_status_code);
  }
}

void AdsServiceImpl::NotifyDidCloseTab(int32_t tab_id) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyDidCloseTab(tab_id);
  }
}
...
void AdsServiceImpl::NotifyUserGestureEventTriggered(
  ...
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyUserGestureEventTriggered(
...
void AdsServiceImpl::NotifyBrowserDidBecomeActive() {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyBrowserDidBecomeActive();
  }
}

void AdsServiceImpl::NotifyBrowserDidResignActive() {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyBrowserDidResignActive();
  }
}

void AdsServiceImpl::NotifyDidSolveAdaptiveCaptcha() {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyDidSolveAdaptiveCaptcha();
  }
}
...
void AdsServiceImpl::ShowScheduledCaptcha(const std::string& payment_id,
  ...
  if (prefs_->GetBoolean(
          qorai_adaptive_captcha::prefs::kScheduledCaptchaPaused)) {
...
void AdsServiceImpl::OnBrowserDidEnterForeground() {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyBrowserDidEnterForeground();
#if BUILDFLAG(IS_ANDROID)
    qor_ads_client_notifier_remote_->NotifyBrowserDidBecomeActive();
#endif  // BUILDFLAG(IS_ANDROID)
  }
...
void AdsServiceImpl::OnBrowserDidEnterBackground() {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    ...
    qor_ads_client_notifier_remote_->NotifyBrowserDidResignActive();
#endif  // BUILDFLAG(IS_ANDROID)
    qor_ads_client_notifier_remote_->NotifyBrowserDidEnterBackground();
  }
}
...
void AdsServiceImpl::OnResourceComponentDidChange(
    ...
    const std::string& id) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyResourceComponentDidChange(
        manifest_version, id);
  }
}
...
void AdsServiceImpl::OnDidUnregisterResourceComponent(const std::string& id) {
  if (qor_ads_client_notifier_remote_.is_bound()) {
    qor_ads_client_notifier_remote_->NotifyDidUnregisterResourceComponent(id);
  }
}
