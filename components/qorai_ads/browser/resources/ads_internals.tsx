...
import * as AdsInternalsMojo from 'gen/qorai/components/services/qor_ads/public/interfaces/qor_ads.mojom.m.js'
...
const App: React.FC = () => {
  ...
  pageCallbackRouter.updateQoraiRewardsEnabled.addListener((enabled: boolean) => {
  ...
  return (
    <Container>
      ...
      <StateContainer>
        <b>Active Qorai Ads conversion URL patterns:</b><br /><br />
        <ConversionUrlPatternTable data={conversionUrlPatterns} /><br /><br />
      </StateContainer>

      <StateContainer>
        <b>Active Qorai Ads events:</b><br /><br />
...
const ConversionUrlPatternTable: React.FC<{ data: ConversionUrlPattern[] }> = React.memo(({ data }) => {
  if (data.length === 0) {
    return (
      <p>No Qorai Ads conversion URL patterns are currently being matched.</p>
    );
  }
