...
#include "qorai/components/qorai_ads/core/browser/service/virtual_pref_provider_util.h"
...
class QoraiAdsVirtualPrefProviderTest : public ::testing::Test {
 public:
  QoraiAdsVirtualPrefProviderTest() {
  ...
  ~QoraiAdsVirtualPrefProviderTest() override {
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserBuildChannel) {
  // Arrange
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserVersion) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserMajorVersion) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserMinorVersion) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserBuildVersion) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, BrowserPatchVersion) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, OperatingSystemLocaleLanguage) {
  // Arrange
...
TEST_F(QoraiAdsVirtualPrefProviderTest, OperatingSystemLocaleRegion) {
  // Arrange
...
TEST_F(QoraiAdsVirtualPrefProviderTest, OperatingSystemName) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, IsSurveyPanelist) {
  // Act
  const base::Value::Dict virtual_prefs = provider().GetPrefs();
...
TEST_F(QoraiAdsVirtualPrefProviderTest, SearchEngineDefaultName) {
  // Arrange
  EXPECT_CALL(delegate(), GetDefaultSearchEngineName)
      .WillOnce(testing::Return("Qorai"));
  ...
  // Assert
  EXPECT_EQ(*default_search_engine_name, "Qorai");
}

TEST_F(QoraiAdsVirtualPrefProviderTest, PrefsWithSkus) {
  // Arrange
