...
#include "qorai/components/qorai_ads/core/internal/studies/studies_util.h"
...
constexpr char kActiveFieldTrialStudyPrefix[] = "QoraiAds.";
...
std::optional<base::FieldTrial::ActiveGroup> GetActiveFieldTrialStudyGroup() {
  ...
  if (active_field_trial_groups.size() == 1) {
    // Only one `QoraiAds.` study is allowed to be active at any given time.
