...
#include "qorai/components/qorai_ads/core/internal/account/confirmations/confirmations.h"
#include "qorai/components/qorai_ads/core/internal/account/deposits/deposit_interface.h"
#include "qorai/components/qorai_ads/core/internal/account/deposits/deposits_factory.h"
#include "qorai/components/qorai_ads/core/internal/account/statement/statement.h"
#include "qorai/components/qorai_ads/core/internal/account/transactions/transaction_info.h"
#include "qorai/components/qorai_ads/core/internal/account/transactions/transactions.h"
...
#include "qorai/components/qorai_ads/core/internal/ads_notifier_manager.h"
...
void Account::GetStatement(GetStatementOfAccountsCallback callback) {
  if (!UserHasJoinedQoraiRewards()) {
    // No-op if the user has not joined Qorai Rewards.
...
void Account::ProcessDeposit(const std::string& creative_instance_id,
                             ...
                             base::Value::Dict user_data) const {
  if (!UserHasJoinedQoraiRewards()) {
    // If the user has not joined Qorai Rewards, there's no need to record
    // transactions.
...
void Account::MaybeInitializeUserRewards() {
  ...
  if (!UserHasJoinedQoraiRewardsAndConnectedWallet()) {
    // No-op if the user has not joined Qorai Rewards and connected a wallet,
  ...
  // Qorai Rewards because the associated data and the `Ads` instance will be
  // destroyed.
...
void Account::OnNotifyPrefDidChange(const std::string& path) {
  if (DoesMatchUserHasJoinedQoraiRewardsPrefPath(path)) {
    Initialize();
  }
}
