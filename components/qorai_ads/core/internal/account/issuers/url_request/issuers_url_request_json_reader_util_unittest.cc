...
namespace qorai_ads {
class QoraiAdsIssuersUrlRequestJsonReaderUtilTest : public test::TestBase {};

TEST_F(QoraiAdsIssuersUrlRequestJsonReaderUtilTest, ParsePing) {
  // Arrange
...
TEST_F(QoraiAdsIssuersUrlRequestJsonReaderUtilTest, DoNotParseMissingPing) {
  // Act & Assert
  EXPECT_FALSE(json::reader::ParsePing({}));
}

TEST_F(QoraiAdsIssuersUrlRequestJsonReaderUtilTest, ParseTokenIssuers) {
  // Arrange
  const base::Value::Dict dict = base::test::ParseJsonDict(R"JSON(
    {
      "issuers": [
...
TEST_F(QoraiAdsIssuersUrlRequestJsonReaderUtilTest,
