...
#include "qorai/components/qorai_ads/core/internal/account/issuers/issuers_delegate_mock.h"
...
class QoraiAdsIssuersTest : public test::TestBase {
 protected:
  void SetUp() override {
    test::TestBase::SetUp();
...
TEST_F(QoraiAdsIssuersTest, FetchIssuers) {
  // Arrange
...
TEST_F(QoraiAdsIssuersTest, DoNotFetchIfInvalidResponseBody) {
  // Arrange
  const test::URLResponseMap url_responses = {
      {BuildIssuersUrlPath(),
...
TEST_F(QoraiAdsIssuersTest, DoNotRetryAfterHttpForbiddenResponseStatusCode) {
  // Arrange
  const test::URLResponseMap url_responses = {
      {BuildIssuersUrlPath(),
       {{net::HTTP_FORBIDDEN,
