...
## Non-Qorai Rewards User

### User Data Submitted when Redeeming Anonymous Confirmations

Included as part of the "confirmation" payload. See [non-Qorai Rewards user confirmation payload](../confirmations/non_reward/README.md).

| key  | example  | description  |
|---|---|---|
| conversion.action[^2]  | {"conversion":[{"action":"click"}]}  | Conversion attribution to determine which advertising efforts led a user to view or click on an ad and subsequently complete a desired action. Also, see [reporting metrics](https://ads-help.qorai.com/campaign-performance/reporting/#available-reporting-metrics-in-qorai-ads-manager).<br><br> Supported types:<br><br>Qorai Search ads:<br>- click-through  |

## Qorai Rewards User

### User Data Submitted when Redeeming Anonymous Confirmation Tokens

Included as part of the "confirmation token" payload. See [Qorai Rewards user confirmation token payload](../confirmations/reward/README.md).
...
| diagnosticId[^1][^2]  | {"diagnosticId":"c1298fde-7fdb-401f-a3ce-0b58fe86e6e2"}  | Diagnostic id from [qorai://rewards-internals](qorai://rewards-internals) to assist in troubleshooting issues. This is only included if it has been set manually by the user, for example, at the request of Qorai Support in order to troubleshoot a problem.  |
...
| segment[^2]  | {"segment":"sports"}  | Advertising taxonomy for the chosen ad. Also see, https://ads-help.qorai.com/campaign-performance/targeting.  |
| studies  | {"studies":[{"group":"GroupA","name":"QoraiAds.FooStudy"},{"group":"GroupB","name":"QoraiAds.BarStudy"}]}  | User studies, utilized for A/B testing, should not exceed one active study at any time. Configuration of studies is achievable through Griffin by adding "QoraiAds." as a prefix to the experiment name. See [Griffin](https://github.com/qorai/qorai-browser/wiki/Qorai-Variations-(Griffin)).  |
...
Included as part of the "payment token" payload. See [Qorai Rewards user payment token redemption](../utility/redeem_payment_tokens/README.md).
