# Qorai Rewards User Payment Token Redemption

Return to [Qorai Rewards user confirmation token redemption](../../../account/utility/redeem_confirmation/reward/README.md).

Request payment for all user-collected Qorai Rewards, by calling https://mywallet.ads.qorai.com/v{version}/confirmation/payment/{paymentId}.

Here, `{version}` represents the version number of the targeted API, and `{paymentId}` is the Rewards payment id, which can be found at [qorai://rewards-internals](qorai://rewards-internals).

The request body includes the following:

| key  | example  | description  |
|---|---|---|
| payload   | {"paymentId":"27a39b2f-9b2e-4eb0-bbb2-2f84447496e7"}  | Rewards payment id, which can be found at [qorai://rewards-internals](qorai://rewards-internals).  |
