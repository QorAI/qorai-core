...
#include "qorai/components/qorai_ads/core/internal/common/strings/string_strip_util.h"
...
TEST(QoraiAdsStringStripUtilTest, StripNonAlphaCharactersFromEmptyContent) {
  // Act & Assert
  EXPECT_THAT(StripNonAlphaCharacters(""), ::testing::IsEmpty());
}

TEST(QoraiAdsStringStripUtilTest, StripNonAlphaCharactersFromWhitespace) {
  // Act & Assert
  EXPECT_THAT(StripNonAlphaCharacters("   "), ::testing::IsEmpty());
}

TEST(QoraiAdsStringStripUtilTest, StripNonAlphaCharacters) {
  // Arrange
...
TEST(QoraiAdsStringStripUtilTest,
...
TEST(QoraiAdsStringStripUtilTest, StripNonAlphaNumericCharacters) {
  // Arrange
  const std::string content =
      "  The quick brown fox jumps over the lazy dog. "
      "$123,000.0 !\"#$%&'()*+,-./:<=>?@\\[]^_`{|}~ 0123456789 \t\n\v\f\r "
