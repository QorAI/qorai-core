...
TEST(QoraiAdsCryptoUtilTest, Sha256) {
  // Act
...
TEST(QoraiAdsCryptoUtilTest, Sha256WithEmptyString) {
  // Act
...
TEST(QoraiAdsCryptoUtilTest, GenerateSignKeyPairFromSeed) {
  // Arrange
...
TEST(QoraiAdsCryptoUtilTest, GenerateBoxKeyPair) {
  // Act
...
TEST(QoraiAdsCryptoUtilTest, GenerateRandomNonce) {
  // Act & Assert
  EXPECT_THAT(GenerateRandomNonce(), ::testing::SizeIs(crypto_box_NONCEBYTES));
}

TEST(QoraiAdsCryptoUtilTest, Sign) {
  // Act
...
TEST(QoraiAdsCryptoUtilTest, Verify) {
  // Act
  std::optional<std::string> signature = Sign(kMessage, kSecretKeyBase64);
...
TEST(QoraiAdsCryptoUtilTest, EncryptAndDecrypt) {
  // Arrange
  const KeyPairInfo key_pair = GenerateBoxKeyPair();
