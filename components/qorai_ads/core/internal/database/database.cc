...
#include "qorai/components/qorai_ads/core/internal/common/database/database_row_util.h"
...
void Database::ErrorCallback(int extended_error,
  ...
  if (!sql::Database::IsExpectedSqliteError(extended_error)) {
    ...
    if (result_code != sql::SqliteResultCode::kFullDisk &&
      ...
      SCOPED_CRASH_KEY_NUMBER("QoraiAds", "sqlite_schema_version",
                              database::kVersionNumber);
      SCOPED_CRASH_KEY_STRING1024(
          "QoraiAds", "sqlite_diagnostic_info",
          db_.GetDiagnosticInfo(extended_error, statement));
      SCOPED_CRASH_KEY_STRING1024("QoraiAds", "sqlite_error_message",
                                  db_.GetErrorMessage());
      SCOPED_CRASH_KEY_NUMBER("QoraiAds", "sqlite_result_code",
