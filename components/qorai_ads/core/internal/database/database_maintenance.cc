...
#include "qorai/components/qorai_ads/core/internal/database/database_maintenance.h"
...
#include "qorai/components/qorai_ads/core/internal/database/database_manager.h"
#include "qorai/components/qorai_ads/core/internal/history/ad_history_database_table_util.h"
...
#include "qorai/components/qorai_ads/core/internal/user_engagement/ad_events/ad_events_database_table_util.h"
...
// Notification and search result ad events are not purged since only Qorai
// Rewards users can opt out of them.

void MaybePurgeNewTabPageAdEvents() {
  if (UserHasJoinedQoraiRewards()) {
    // Do not purge ad events if the user has joined Qorai Rewards.
    return;
  }
...
void MaybePurgeQoraiNewsAdEvents() {
  if (UserHasJoinedQoraiRewards()) {
    // Do not purge ad events if the user has joined Qorai Rewards.
    return;
  }

  if (!UserHasOptedInToQoraiNewsAds()) {
...
void Maintenance::OnNotifyPrefDidChange(const std::string& path) {
  ...
  } else if (DoesMatchUserHasOptedInToQoraiNewsAdsPrefPath(path)) {
    MaybePurgeQoraiNewsAdEvents();
  }
}
