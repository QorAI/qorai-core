...
bool DoesRequireResourceForNewTabPageAds() {
  ...
  // - The user has opted into new tab page ads and joined Qorai Rewards.
  return UserHasJoinedQoraiRewards() && UserHasOptedInToNewTabPageAds();
}

bool DoesRequireResource() {
  // Require resource only if:
  // - User has opted into Qorai News ads.
  // - The user has opted into new tab page ads and and joined Qorai Rewards.
  // - User has joined Qorai Rewards and opted into notification ads.
  return UserHasOptedInToQoraiNewsAds() ||
...
void SubdivisionTargeting::OnNotifyPrefDidChange(const std::string& path) {
  ...
  } else if (DoesMatchUserHasJoinedQoraiRewardsPrefPath(path) ||
