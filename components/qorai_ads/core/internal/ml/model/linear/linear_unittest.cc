...
#include "qorai/components/qorai_ads/core/internal/common/resources/flat/text_classification_linear_model_generated.h"
...
class QoraiAdsLinearTest : public test::TestBase {
 public:
...
TEST_F(QoraiAdsLinearTest, ThreeClassesPredictionTest) {
  // Arrange
...
TEST_F(QoraiAdsLinearTest, BiasesPredictionTest) {
  // Arrange
  const std::map<std::string, VectorData> weights = {
      {"class_1", VectorData({1.0, 0.0, 0.0})},
      {"class_2", VectorData({0.0, 1.0, 0.0})},
...
TEST_F(QoraiAdsLinearTest, BinaryClassifierPredictionTest) {
  // Arrange
...
TEST_F(QoraiAdsLinearTest, TopPredictionsTest) {
  // Arrange
