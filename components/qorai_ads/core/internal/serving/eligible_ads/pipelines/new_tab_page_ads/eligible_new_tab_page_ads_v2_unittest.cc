...
class QoraiAdsEligibleNewTabPageAdsV2Test : public test::TestBase {
 protected:
  void SetUp() override {
    test::TestBase::SetUp();
...
TEST_F(QoraiAdsEligibleNewTabPageAdsV2Test, GetAds) {
  // Arrange
...
TEST_F(QoraiAdsEligibleNewTabPageAdsV2Test, GetAdsForNoMatchingSegments) {
  // Arrange
  CreativeNewTabPageAdList creative_ads;
...
TEST_F(QoraiAdsEligibleNewTabPageAdsV2Test, DoNotGetAdsIfNoEligibleAds) {
  // Act & Assert
  base::RunLoop run_loop;
  base::MockCallback<EligibleAdsCallback<CreativeNewTabPageAdList>> callback;
  EXPECT_CALL(callback, Run(/*creative_ads=*/::testing::IsEmpty()))
