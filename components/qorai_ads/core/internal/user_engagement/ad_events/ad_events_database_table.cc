...
void MigrateToV50(const mojom::DBTransactionInfoPtr& mojom_db_transaction) {
  ...
  Execute(mojom_db_transaction, R"(
      CREATE TABLE ad_events_temp (
        ...
        advertiser_id TEXT,
        segment TEXT,
        target_url TEXT NOT NULL DEFAULT 'https://qorai.com/qorai-ads/',
...
void AdEvents::PurgeExpired(ResultCallback callback) const {
  ...
  days = UserHasJoinedQoraiRewards() ? 90 : 30;
  Execute(mojom_db_transaction, R"(
            DELETE FROM
              $1
            WHERE
  ...
  // New tab page ads.
  days =
      UserHasJoinedQoraiRewards() || UserHasOptedInToSurveyPanelist() ? 90 : 2;
  Execute(mojom_db_transaction, R"(
            DELETE FROM
              $1
            WHERE
