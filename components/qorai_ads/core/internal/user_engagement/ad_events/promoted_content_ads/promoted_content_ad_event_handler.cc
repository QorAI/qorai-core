...
#include "qorai/components/qorai_ads/core/internal/serving/permission_rules/promoted_content_ads/promoted_content_ad_permission_rules.h"
...
void PromotedContentAdEventHandler::GetForTypeCallback(
    ...
    const AdEventList& ad_events) {
  if (!success) {
    BLOG(0, "Promoted content ad: Failed to get ad events");
...
void PromotedContentAdEventHandler::FailedToFireEvent(
    ...
    mojom::PromotedContentAdEventType mojom_ad_event_type,
    FirePromotedContentAdEventHandlerCallback callback) const {
  BLOG(0, "Failed to fire promoted content ad "
              << mojom_ad_event_type << " event for placement id "
