...
#include "qorai/components/qorai_ads/core/internal/common/resources/country_components.h"
#include "qorai/components/qorai_ads/core/internal/common/resources/resource_util_impl.h"
...
#include "qorai/components/qorai_ads/core/internal/user_engagement/conversions/resource/conversion_resource_info.h"
...
bool DoesRequireResource() {
  ...
  // - The user has joined Qorai Rewards and opted into Qorai News ads, new tab
  //   page ads, notification ads, or search result ads.
  return UserHasJoinedQoraiRewards() &&
         (UserHasOptedInToQoraiNewsAds() || UserHasOptedInToNewTabPageAds() ||
...
void ConversionResource::OnNotifyPrefDidChange(const std::string& path) {
  if (DoesMatchUserHasJoinedQoraiRewardsPrefPath(path) ||
      DoesMatchUserHasOptedInToQoraiNewsAdsPrefPath(path) ||
