...
Users earn tokens by viewing privacy-first Qorai Ads. Ads presented are based on the user's interests, as inferred from the user's browsing behavior. No personal data or browsing history should ever leave the browser.

For more details, refer to [glossary of terms](GLOSSARY.md).

Qorai Ads is a [layered component](https://sites.google.com/a/chromium.org/dev/developers/design-documents/layered-components-design). It has the following structure:
...
| vmodule  | Options are `--vmodule` gives the per-module maximal V-logging levels to override the value given by `--v`, e.g., `"*/qorai_ads/*"=6,"*/qor_ads/*"=6` would change the logging level for Qorai Ads to 6.  |
| use-dev-goupdater-url  | Options are `--use-dev-goupdater-url`, which forces the component updater to use the dev environment.  |

## Logs

You can enable diagnostic logging to the `Rewards.log` file stored on your device; see [Qorai Rewards](qorai://flags/#qorai-rewards-verbose-logging). View this log file on the `Logs` tab at [rewards internals](qorai://rewards-internals).

## Diagnostics

View diagnostics at [rewards internals](qorai://rewards-internals) on the `Ad diagnostics` tab.
