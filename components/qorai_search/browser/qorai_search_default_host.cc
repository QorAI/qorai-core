...
#include "qorai/components/time_period_storage/daily_storage.h"
...
using qorai_search_conversion::IsNTPPromotionEnabled;
...
// static
void QoraiSearchDefaultHost::RegisterProfilePrefs(
    PrefRegistrySimple* registry) {
  registry->RegisterListPref(qorai_search::prefs::kDailyAsked);
  registry->RegisterIntegerPref(qorai_search::prefs::kTotalAsked, 0);
#if BUILDFLAG(IS_ANDROID)
  registry->RegisterBooleanPref(qorai_search::prefs::kFetchFromNative, false);
#endif
}

QoraiSearchDefaultHost::QoraiSearchDefaultHost(
    const std::string& host,
    TemplateURLService* template_url_service,
    PrefService* prefs)
    : host_(host), template_url_service_(template_url_service), prefs_(prefs) {}

QoraiSearchDefaultHost::~QoraiSearchDefaultHost() = default;

bool QoraiSearchDefaultHost::CanSetDefaultSearchProvider(TemplateURL* provider,
                                                         bool is_historic) {
  // Return false if:
...
uint64_t QoraiSearchDefaultHost::GetMaxDailyCanAskCount() {
  return static_cast<uint64_t>(
      features::kQoraiSearchDefaultAPIDailyLimit.Get());
}

uint64_t QoraiSearchDefaultHost::GetMaxTotalCanAskCount() {
  return static_cast<uint64_t>(
      features::kQoraiSearchDefaultAPITotalLimit.Get());
}

void QoraiSearchDefaultHost::SetCanAlwaysSetDefault() {
...
void QoraiSearchDefaultHost::GetCanSetDefaultSearchProvider(
...
void QoraiSearchDefaultHost::SetIsDefaultSearchProvider() {
