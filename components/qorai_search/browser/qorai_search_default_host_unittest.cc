...
class QoraiSearchDefaultHostTest : public ::testing::Test {
 protected:
  using MockGetCanSetCallback = base::MockCallback<
      QoraiSearchDefaultHost::GetCanSetDefaultSearchProviderCallback>;

  QoraiSearchDefaultHostTest() {
  ...
  std::unique_ptr<QoraiSearchDefaultHost> GetAPIHost(const std::string& host) {
    return std::make_unique<QoraiSearchDefaultHost>(
...
TEST_F(QoraiSearchDefaultHostTest, AllowsIfPresent) {
...
TEST_F(QoraiSearchDefaultHostTest, DisallowsIfNotPresent) {
  auto host = GetAPIHost("search.test.com");
...
TEST_F(QoraiSearchDefaultHostTest, DisallowsIfDefault) {
  auto host = GetAPIHost("search.test.com");
...
TEST_F(QoraiSearchDefaultHostTest, AllowsIfNotDefault) {
  auto host = GetAPIHost("search.test.com");
  // Add a search provider for the host
  AddSearchProviderForHost("search.test.com");
...
TEST_F(QoraiSearchDefaultHostTest, DisallowsAfterMaxTimesAsked) {
  auto host = GetAPIHost("search.test.com");
  // Add a search provider for the host
  AddSearchProviderForHost("search.test.com");
...
TEST_F(QoraiSearchDefaultHostTest, CanSetDefaultAlwaysTestWithSearchPromotion) {
  base::test::ScopedFeatureList feature_list;

  qorai_l10n::test::ScopedDefaultLocale scoped_default_locale{"en_US"};
  ...
  feature_list.InitAndEnableFeatureWithParameters(
      ...
      {{qorai_search_conversion::features::kBannerTypeParamName, "type_C"}});
  ...
  feature_list.Reset();
  feature_list.InitAndEnableFeature(qorai_search_conversion::features::kNTP);
  host->SetCanAlwaysSetDefault();
