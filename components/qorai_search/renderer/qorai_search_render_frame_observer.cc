...
QoraiSearchRenderFrameObserver::QoraiSearchRenderFrameObserver(
    content::RenderFrame* render_frame,
    int32_t world_id)
    : RenderFrameObserver(render_frame), world_id_(world_id) {}

QoraiSearchRenderFrameObserver::~QoraiSearchRenderFrameObserver() = default;

void QoraiSearchRenderFrameObserver::DidCreateScriptContext(
  ...
  if (!native_javascript_handle_) {
    native_javascript_handle_ = std::make_unique<QoraiSearchDefaultJSHandler>(
...
void QoraiSearchRenderFrameObserver::DidStartNavigation(
    const GURL& url,
    std::optional<blink::WebNavigationType> navigation_type) {
  url_ = url;
}

void QoraiSearchRenderFrameObserver::OnDestruct() {
  delete this;
}
