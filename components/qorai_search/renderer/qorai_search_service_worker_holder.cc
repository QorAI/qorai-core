...
// QoraiSearchFallbackJSHandler instance in local thread storage, and providing
...
class [[maybe_unused, nodiscard]] JsHandlersForCurrentThread
 ...
 public:
  ...
  void AddJsHandler(std::unique_ptr<QoraiSearchFallbackJSHandler> js_handler);
 ...
 private:
  ...
  std::vector<std::unique_ptr<QoraiSearchFallbackJSHandler>> js_handlers_;
...
void JsHandlersForCurrentThread::AddJsHandler(
    std::unique_ptr<QoraiSearchFallbackJSHandler> js_handler) {
...
void JsHandlersForCurrentThread::RemoveContext(
  ...
  auto to_remove = std::ranges::remove_if(
      js_handlers_,
      [&v8_context](
          const std::unique_ptr<QoraiSearchFallbackJSHandler>& js_handler) {
...
QoraiSearchServiceWorkerHolder::QoraiSearchServiceWorkerHolder()
    : broker_(nullptr) {}

QoraiSearchServiceWorkerHolder::~QoraiSearchServiceWorkerHolder() = default;

void QoraiSearchServiceWorkerHolder::SetBrowserInterfaceBrokerProxy(
    blink::ThreadSafeBrowserInterfaceBrokerProxy* broker) {
  broker_ = broker;
}

void QoraiSearchServiceWorkerHolder::WillEvaluateServiceWorkerOnWorkerThread(
