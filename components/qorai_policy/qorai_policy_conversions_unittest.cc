...
class QoraiPolicyConversionsTest : public ::testing::Test {
 protected:
  void SetUp() override {}
};

// Test that Qorai policy source appears in policy conversions
// This tests the override in
// chromium_src/components/policy/core/browser/policy_conversions.cc The test
// ensures that "policySourceQorai" appears in the policy source mapping array
TEST_F(QoraiPolicyConversionsTest, QoraiInPolicyConversions) {
  // Add a policy with POLICY_SOURCE_QORAI
  PolicyMap policy_map;
  const std::string test_policy_name = "TestQoraiPolicy";

  PolicyMap::Entry qorai_entry(POLICY_LEVEL_MANDATORY, POLICY_SCOPE_USER,
                               POLICY_SOURCE_QORAI, base::Value("test_value"),
                               nullptr);

  policy_map.Set(test_policy_name, qorai_entry.DeepCopy());
  ...
  // Check that the source is properly labeled as "policySourceQorai"
  ...
  EXPECT_EQ(*source, "policySourceQorai");
}

// Test that the string resource is correctly defined
TEST_F(QoraiPolicyConversionsTest, QoraiStringResourceExists) {
  // Verify that IDS_POLICY_SOURCE_QORAI is defined and has expected
  // content This is a compile-time check that our string resource is available
  EXPECT_NE(IDS_POLICY_SOURCE_QORAI, 0);
}

// Test that kPolicySources array has the correct entry for Qorai
TEST_F(QoraiPolicyConversionsTest, QoraiInPolicySourcesArray) {
  // Verify that the kPolicySources array has "policySourceQorai" at the correct
  // index
  EXPECT_LT(static_cast<int>(POLICY_SOURCE_QORAI), POLICY_SOURCE_COUNT);
  EXPECT_STREQ(kPolicySources[POLICY_SOURCE_QORAI].name, "policySourceQorai");
  EXPECT_EQ(kPolicySources[POLICY_SOURCE_QORAI].id, IDS_POLICY_SOURCE_QORAI);
}

// Test that GetSource function handles POLICY_SOURCE_QORAI without
// hitting NOTREACHED()
TEST_F(QoraiPolicyConversionsTest, GetSourceHandlesQorai) {
  // Create a policy dictionary with POLICY_SOURCE_QORAI
  ...
  test_policy.Set("source", static_cast<int>(POLICY_SOURCE_QORAI));
  test_policy.Set("value", base::Value("test_value"));

  base::Value::Dict chrome_policies;
  chrome_policies.Set("TestQoraiPolicy", std::move(test_policy));

  base::Value::Dict policies;
  policies.Set("chromePolicies", std::move(chrome_policies));

  // Test that AppendChromePolicyInfoIntoProfileReport handles Qorai
  ...
  EXPECT_EQ(policy_proto.name(), "TestQoraiPolicy");
