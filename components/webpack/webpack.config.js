...
module.exports = async function (env, argv) {
  ...
  const entry = {}
  if (!env.qorai_entries) {
    throw new Error(
      "Entry point(s) must be provided via env.qorai_entries param."
    )
  }
  const entryInput = env.qorai_entries.split(',').sort()
  for (const entryInputItem of entryInput) {
    const entryInputItemParts = entryInputItem.split('=')
    if (entryInputItemParts.length !== 2) {
      throw new Error(
        'QorAI Webpack config could not parse entry env param item: ' +
        entryInputItem
      )
    }
  ...
  return {
    ...
    plugins: [
      ...
      // to the src/qorai working directory then the paths could include the
    ...
    module: {
      rules: [
        ...
        // Unfortunately, qorai-ui is compiled as a "module" so Webpack5 expects
        // it to provide file extensions (which it does not), so we need to
        // special case it here.
        {
          test: p => p.includes(path.join('@qorai', 'qorai-ui')) && p.endsWith('.js'),
          resolve: {
