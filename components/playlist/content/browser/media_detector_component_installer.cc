...
#include "qorai/components/playlist/content/browser/media_detector_component_installer.h"
...
bool MediaDetectorComponentInstallerPolicy::IsQorAIComponent() const {
  return true;
}

void OnRegisteredToComponentUpdateService() {
  QorAIOnDemandUpdater::GetInstance()->EnsureInstalled(kComponentID);
}
