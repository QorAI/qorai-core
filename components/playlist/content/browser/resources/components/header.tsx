...
import {
  ...
  icon,
  elevation
} from '@qorai/qora/tokens/css/variables'
...
const GradientIcon = styled(Icon)`
  --qora-icon-color: linear-gradient(
    314.42deg,
    #fa7250 8.49%,
    #ff1893 43.72%,
    #a78aff 99.51%
...
const StyledButton = styled(QoraButton)`
  flex: 0 0 auto;
`

const MediumSizedButton = styled(StyledButton)`
  width: var(--qora-icon-m);
...
function PlaylistHeader ({ playlistId }: { playlistId: string }) {
  ...
  if (playlist?.items.length) {
    contextualMenuItems.push({
      name: getLocalizedString('qoraiPlaylistContextMenuEdit'),
  ...
  if (contextualMenuItems && !isDefaultPlaylist) {
    contextualMenuItems.unshift({
      name: getLocalizedString('qoraiPlaylistContextMenuRenamePlaylist'),
      iconName: 'edit-box',
      onClick: () => {
    ...
    contextualMenuItems.push({
      name: getLocalizedString('qoraiPlaylistContextMenuDeletePlaylist'),
      iconName: 'trash',
      onClick: () => {
...
function NewPlaylistButton () {
  return (
    <MediumSizedButton
      ...
      kind='plain'
      fab
      title={getLocalizedString('qoraiPlaylistTooltipCreatePlaylistFolder')}
      onClick={() => {
...
function SettingButton () {
  return (
    <MediumSizedButton
      ...
      kind='plain'
      fab
      title={getLocalizedString('qoraiPlaylistTooltipOpenPlaylistSettings')}
...
function CloseButton () {
  return (
    <MediumSizedButton
      ...
      kind='plain'
      fab
      title={getLocalizedString('qoraiPlaylistTooltipClosePanel')}
