...
function handlePlayerEvents(payload: PlayerEventsPayload) {
  switch (payload.type) {
    ...
    case types.PLAYLIST_PLAYER_FAILED_TO_PLAY_ITEM: {
      showAlert(
        {
          title: getLocalizedString('qoraiPlaylistFailedToPlayTitle'),
          ...
          actions: [
            {
              text: getLocalizedString('qoraiPlaylistFailedToPlayRecover'),
              kind: 'filled',
              action: (a) => {
            ...
            {
              text: getLocalizedString('qoraiPlaylistAlertDismiss'),
              kind: 'plain-faint',
              action: (a) => a.dismiss()
            }
          ]
