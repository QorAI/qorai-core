...
TEST(UserAgentUtilsTest, UserAgentMetadata) {
  ...
  const blink::UserAgentBrandVersion product_brand_version = {"QorAI",
                                                              major_version};
  EXPECT_TRUE(
...
TEST(UserAgentUtilsTest, UserAgentFromCommandLine) {
  ...
  const auto qorai_metadata = GetUserAgentMetadata(nullptr);
  const blink::UserAgentMetadata empty_metadata;

  EXPECT_EQ(GetUserAgent(), kCmdUserAgentValue);
  EXPECT_EQ(qorai_metadata, empty_metadata);
}
