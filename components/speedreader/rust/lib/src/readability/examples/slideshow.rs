...
static SPEEDREADER_CSS: &str = "https://raw.githubusercontent.com/qorai-experiments/SpeedReader/master/data/content-stylesheet.css";
...
fn main() {
    ...
    let sheet = format!(
        "<style id=\"qorai_speedreader_style\">{}</style>",
    ...
    Command::new("qorai").arg(slideshow_path).spawn().unwrap();
}
