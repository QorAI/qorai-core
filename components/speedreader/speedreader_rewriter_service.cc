...
#include "qorai/components/speedreader/common/url_readable_hints.h"
#include "qorai/components/speedreader/rust/ffi/speedreader.h"
...
std::string WrapStylesheetWithCSP(const std::string& stylesheet,
  ...
  const auto make_style_data = [](std::string_view id, std::string_view data) {
    ...
    return base::StrCat({"<script type=\"qorai-style-data\" id=\"", id,
                         "\" integrity=\"", sha256, "\">", data, "</script>"});
  };

  return base::StrCat({kCSP,
                       make_style_data("qorai_speedreader_style", stylesheet),
