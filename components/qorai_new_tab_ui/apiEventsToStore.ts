...
export function wireApiEventsToStore () {
  ...
  .then((initialData) => {
    ...
    backgroundData.updateImages(initialData.qoraiBackgrounds)
    ...
    if (loadTimeData.getBoolean('vpnWidgetSupported')) {
      getActions().qoraiVPN.initialize(initialData.purchasedState)
    }
  })
  .catch(e => {
...
export function rewardsInitData () {
  getRewardsPreInitialData().then((preInitialRewardsData) => {
    ...
    chrome.qoraiRewards.isInitialized((isInitialized) => {
      if (isInitialized) {
...
chrome.qoraiRewards.initialized.addListener(fetchRewardsData)

chrome.qoraiRewards.onRewardsWalletCreated.addListener(fetchRewardsData)

chrome.qoraiRewards.onCompleteReset.addListener((properties: { success: boolean }) => {
  getActions().onCompleteReset(properties.success)
})

chrome.qoraiRewards.onSelfCustodyInviteDismissed.addListener(fetchRewardsData)

chrome.qoraiRewards.onTermsOfServiceUpdateAccepted.addListener(fetchRewardsData)

chrome.qoraiRewards.onExternalWalletLoggedOut.addListener(fetchRewardsData)

chrome.qoraiRewards.onExternalWalletDisconnected.addListener(fetchRewardsData)
