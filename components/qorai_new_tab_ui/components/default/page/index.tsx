...
const StyledPage = styled('div') <PageProps>`
  ...
  .${CLASSNAME_PAGE_STUCK} & {
    /* Fix the main NTP content so, when Qorai News is in-view,
...
export const Page: React.FunctionComponent<React.PropsWithChildren<PageProps>> = (props) => {
  ...
  // extra scroll area (Qorai News) then we "sticky" the Page at
...
function getPageBackground(p: HasImageProps) {
  ...
  // and Page is the element with blur (when Qorai News is active)
...
export const App = styled('div') <AppProps & HasImageProps>`
  ...
  ${defaultState.featureFlagQoraiNewsFeedV2Enabled && css`
  &::before {
...
export const IconButton = styled('button') <IconButtonProps>`
  ...
  &:focus-visible {
    outline: 2px solid ${p => p.theme.color.brandQoraiInteracting};
  }
`
