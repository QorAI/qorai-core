...
const isDarkTheme = (p: any) => {
  return p.theme.name === 'Qorai Dark'
}
...
export const SettingsRow = styled('div') <SettingsRowProps>`
  ...
  ${p => p.isInteractive && css`
    cursor: pointer;
    &:focus-visible {
      outline: solid 1px ${p => p.theme.color.brandQorai};
    }
    &:hover {
      color: ${p => p.theme.color.brandQoraiInteracting}
    }
    &:active {
      color: ${p => p.theme.color.brandQoraiActive}
    }
  `}
...
export const StyledCustomBackgroundOption = styled('button')`
  ...
  &:focus-visible {
    outline-style: solid;
    outline-color: ${p => p.theme.color.brandQorai};
    outline-width: 1px;
  }
`
