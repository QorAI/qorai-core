...
import CardOptIn from '../../../../qorai_news/browser/resources/OptIn'
import * as QoraiNewsElement from './default'

const Content = React.lazy(() => import('./content'))
const FeedV2 = React.lazy(() => import('../../../../qorai_news/browser/resources/Page'))

// When FeedV2 is enabled, immediately start loading the Qorai News chunk,
// rather than waiting for the parent component to render.
if (defaultState.featureFlagQoraiNewsFeedV2Enabled) {
  import('../../../../qorai_news/browser/resources/Page')
}
...
export type GetDisplayAdContent = QoraiNews.QoraiNewsControllerRemote['getDisplayAd']
...
export type Props = {
  ...
  isPrompting: boolean
  feed?: QoraiNews.Feed
  publishers?: QoraiNews.Publishers
  articleToScrollTo?: QoraiNews.FeedItemMetadata
...
export default function QoraiNewsSection(props: Props) {
  const dispatch = useDispatch()
  const [optedIn] = useNewTabPref('isQoraiNewsOptedIn')
  ...
  const loadDataObserver = React.useMemo<PossibleInteractionObserver>(() => {
    ...
    const handleHits: IntersectionObserverCallback = (entries) => {
      ...
      if (isIntersecting && !hasRequestedLoad.current) {
        ...
        console.debug('Qorai News: section is in position that requires requesting data load')
  ...
  return (
    <QoraiNewsElement.Section>
      <div
        ref={loadDataTrigger}
        style={{ position: 'sticky', top: '100px' }}
      />
      {!optedIn && <CardOptIn />}
      {shouldDisplayContent && <React.Suspense fallback={defaultState.featureFlagQoraiNewsFeedV2Enabled ? null : <CardLoading />}>
        {defaultState.featureFlagQoraiNewsPromptEnabled
