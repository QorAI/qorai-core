...
import { Feed } from '../../../../qorai_news/browser/resources/shared/api'
...
export default function QoraiNewsContent(props: Props) {
  const { feed, publishers } = props

  const dispatch = useDispatch()
  const { setCustomizePage } = useQoraiNews()
  ...
  const setScrollTriggerRef = React.useCallback((element: HTMLElement | null) => {
    ...
    const endOfCurrentArticlesListObserver = new IntersectionObserver((entries) => {
      console.debug('Qorai News content Intersection Observer triggered', entries)
      const hasReachedPaginationPoint = entries.some(
        entry => entry.intersectionRatio > 0 ||
          entry.boundingClientRect.top < 0)
      if (hasReachedPaginationPoint) {
        console.debug('Qorai News content Intersection Observer determined need new page.')
  ...
  const interactionObserver = React.useMemo<IntersectionObserver>(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      console.debug('Qorai News: Intersection Observer trigger show options, changing', isInteracting)
  ...
  const intersectionObserver = React.useRef(new IntersectionObserver(entries => {
    ...
    console.debug(`Qorai News: viewed ${count} cards.`)
  ...
  if (!feed || !publishers) {
    console.error('Qorai News: should have shown error or loading state, but ran in to an unintended code path.')
    return null
  }
  ...
  return (
    <>
      ...
      {showNoContentMessage &&
        <CardNoContent onCustomize={props.onCustomizeQoraiNews} />}
      {/* featured item */}
      ...
      {!isOnlyDisplayingPeekingCard &&
        <>
          ...
          <Customize onCustomizeQoraiNews={() => setCustomizePage('news')} show={showOptions} />
