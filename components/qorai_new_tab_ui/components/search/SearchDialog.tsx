...
export default function Component(props: Props) {
  ...
  React.useEffect(() => {
    setTimeout(() => {
      (ref.current?.querySelector('qora-input') as HTMLElement)?.focus()
    })
  }, [])
  return createPortal(<Dialog ref={ref} offsetY={props.offsetY} onClick={e => {
    ...
    // bounds, so check if the qora-input contained the click.
    const input = e.nativeEvent.composedPath().find(t => (t as HTMLElement).tagName === 'QORA-INPUT')
    if (input) return
