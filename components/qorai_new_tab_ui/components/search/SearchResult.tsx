...
import { color, font, gradient, icon, radius, spacing } from '@qorai/qora/tokens/css/variables';
...
const QoraIcon = styled(Icon)`
  --qora-icon-size: ${icon.l};
...
function Image({ match, isAskQora }: { match: AutocompleteMatch, isAskQora: boolean }) {
  // AskQora is a case we treat specially. It's included on most queries.
  if (isAskQora) return <QoraIcon name='product-qorai-qora' />
...
export default function SearchResult({ match, selected, onClick }: Props) {
  ...
  const isAskQora = description === getLocale('searchAskQora')
  ...
  const result = <Container href={match.destinationUrl.url} aria-selected={selected} onClick={e => {
    ...
    <IconContainer>
      <Image key={match.imageUrl ?? match.iconUrl} match={match} isAskQora={isAskQora} />
    </IconContainer>
