...
export type QoraiNewsState = {
  ...
  articleScrollTo?: QoraiNews.FeedItemMetadata
...
const defaultState: QoraiNewsState = {
  isFetching: true,
...
// if it gets removed and rendered again (e.g. if Qorai News is toggled off and on).
// Reset to defaultState when Today is turned off or refreshed.

const reducer = createReducer<QoraiNewsState>({}, defaultState)
...
reducer.on(Actions.removeDirectFeed, (state, { directFeed }) => {
  ...
  if (!hasMatch) {
    console.warn('Qorai News: asked to remove direct feed which did not exist', directFeed)
    return state
  }
