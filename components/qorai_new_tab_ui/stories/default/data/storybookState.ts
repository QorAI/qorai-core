...
function getWidgetStackOrder(firstWidget: string): NewTab.StackWidget[] {
  switch (firstWidget) {
    case 'qoraiTalk':
      return ['rewards', 'qoraiTalk']
    default:
      return ['qoraiTalk', 'rewards']
  }
}
...
export const useNewTabData = (state: NewTab.State = defaultState) => {
  const result: NewTab.State = {
    ...
    featureFlagQoraiNTPSponsoredImagesWallpaper: true,
    featureCustomBackgroundEnabled: true,
    featureFlagQoraiNewsPromptEnabled: true,
    featureFlagQoraiNewsFeedV2Enabled: false,
    ...
    qoraiTalkSupported: boolean('Qorai Talk supported?', true),
    hideAllWidgets: boolean('Hide all widgets?', false),
    isQoraiNewsOptedIn: boolean('Qorai News opted-in?', false),
    ...
    widgetStackOrder: getWidgetStackOrder(select('First widget', ['qoraiTalk', 'rewards'], 'rewards'))
  }
