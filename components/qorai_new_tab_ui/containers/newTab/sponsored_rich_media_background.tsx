...
export interface SponsoredRichMediaBackgroundInfo {
  ...
  metricType: QoraiAds.NewTabPageAdMetricType
  targetUrl: string
}
...
interface Props extends StatusProps {
  ...
  onEventReported: (name: QoraiAds.NewTabPageAdEventType) => void
  onLoaded: () => void
}
...
const SponsoredRichMediaBackgroundIframe = styled('iframe') <StatusProps>`
  ...
  /* Blur out the content when Qorai News is interacted
...
///   value: 'click'
/// }
function getEventType(event: MessageEvent): QoraiAds.NewTabPageAdEventType | undefined {
  if (!event.data || event.data.type !== 'richMediaEvent') {
    return undefined
  }

  const eventMap: { [key: string]: QoraiAds.NewTabPageAdEventType } = {
    'click': QoraiAds.NewTabPageAdEventType.kClicked,
    'interaction': QoraiAds.NewTabPageAdEventType.kInteraction,
    'mediaPlay': QoraiAds.NewTabPageAdEventType.kMediaPlay,
    'media25': QoraiAds.NewTabPageAdEventType.kMedia25,
    'media100': QoraiAds.NewTabPageAdEventType.kMedia100
  }
