...
import getVPNServiceHandler, { ConnectionState, PurchasedState, ManageURLType, Region, ServiceObserverReceiver } from '../api/qoraiVpn'
...
handler.on(Actions.toggleConnection.getType(), async (store) => {
  ...
  const connectionState = state.qoraiVPN.connectionState
  if (
