...
export const defaultState: NewTab.State = {
  ...
  featureFlagQoraiNTPSponsoredImagesWallpaper: loadTimeData.getBoolean('featureFlagQoraiNTPSponsoredImagesWallpaper'),
  featureFlagQoraiNewsPromptEnabled: loadTimeData.getBoolean('featureFlagQoraiNewsPromptEnabled'),
  featureFlagQoraiNewsFeedV2Enabled: loadTimeData.getBoolean('featureFlagQoraiNewsFeedV2Enabled'),
  ...
  isQoraiNewsOptedIn: false,
  isQoraiNewsDisabledByPolicy: false,
  isQoraiTalkDisabledByPolicy: false,
  showEmptyPage: false,
  qoraiRewardsSupported: false,
  qoraiTalkSupported: false,
  ...
  widgetStackOrder: ['rewards', 'qoraiVPN'],
...
// as a result of https://github.com/qorai/qorai-browser/issues/10067
...
export const debouncedSave = debounce((data: NewTab.State) => {
  if (data) {
    ...
    // (or are obsolete).
    const dataToSave = {
      qoraiRewardsSupported: data.qoraiRewardsSupported,
      qoraiTalkSupported: data.qoraiTalkSupported,
