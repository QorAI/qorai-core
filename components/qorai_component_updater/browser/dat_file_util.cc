...
void GetDATFileData(const base::FilePath& file_path,
                    qorai_component_updater::DATFileDataBuffer* buffer) {
...
DATFileDataBuffer ReadDATFileData(const base::FilePath& dat_file_path) {
  TRACE_EVENT_BEGIN("qorai.adblock", "ReadDATFileData", "path",
                    dat_file_path.MaybeAsASCII());
  DATFileDataBuffer buffer;
  GetDATFileData(dat_file_path, &buffer);
  TRACE_EVENT_END("qorai.adblock", "size", buffer.size());
  return buffer;
}

std::string GetDATFileAsString(const base::FilePath& file_path) {
  TRACE_EVENT_BEGIN("qorai.adblock", "GetDATFileAsString", "path",
  ...
  TRACE_EVENT_END("qorai.adblock", "size", contents.size());
  return contents;
}
