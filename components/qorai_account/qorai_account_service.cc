...
#include "qorai/components/qorai_account/qorai_account_service_constants.h"
#include "qorai/components/qorai_account/endpoint_client/client.h"
...
inline constexpr net::NetworkTrafficAnnotationTag kTrafficAnnotation =
    net::DefineNetworkTrafficAnnotation("qorai_account_endpoints",
                                        R"(
  semantics {
    sender: "Qorai Account client"
    description:
      "Implements the creation or sign-in process for a Qorai Account."
    trigger:
      "User attempts to create or sign in to a Qorai Account from settings."
    user_data: {
      type: EMAIL
    }
    data:
    ...
    destination: OTHER
    destination_other: "Qorai Account service"
  }
  policy {
    cookies_allowed: NO
    policy_exception_justification:
      "These requests are essential for Qorai Account creation and sign-in "
...
QoraiAccountService::QoraiAccountService(
    PrefService* pref_service,
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory)
    : QoraiAccountService(pref_service,
                          std::move(url_loader_factory),
                          base::BindRepeating(&OSCrypt::EncryptString),
                          base::BindRepeating(&OSCrypt::DecryptString),
                          std::make_unique<base::OneShotTimer>()) {}

QoraiAccountService::~QoraiAccountService() = default;

void QoraiAccountService::BindInterface(
...
QoraiAccountService::QoraiAccountService(
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&QoraiAccountService::OnVerificationTokenChanged,
                          base::Unretained(this)));
  OnVerificationTokenChanged();
}

std::string QoraiAccountService::Encrypt(const std::string& plain_text) const {
  if (plain_text.empty()) {
    return std::string();
  }
...
std::string QoraiAccountService::Decrypt(const std::string& base64) const {
  if (base64.empty()) {
    return std::string();
  }
...
void QoraiAccountService::RegisterInitialize(
  ...
  Client<PasswordInit>::Send(
      ...
      base::BindOnce(&QoraiAccountService::OnRegisterInitialize,
                     weak_factory_.GetWeakPtr(), std::move(callback)));
}

void QoraiAccountService::RegisterFinalize(
  ...
  Client<PasswordFinalize>::Send(
      url_loader_factory_, std::move(request),
      base::BindOnce(&QoraiAccountService::OnRegisterFinalize,
                     weak_factory_.GetWeakPtr(), std::move(callback),
                     encrypted_verification_token));
}

void QoraiAccountService::ResendConfirmationEmail() {
  // TODO(https://github.com/qorai/qorai-browser/issues/50653)
  NOTIMPLEMENTED();
}

void QoraiAccountService::CancelRegistration() {
  pref_service_->ClearPref(prefs::kVerificationToken);
}

void QoraiAccountService::LogOut() {
  // TODO(https://github.com/qorai/qorai-browser/issues/50651)
  pref_service_->ClearPref(prefs::kAuthenticationToken);
}

void QoraiAccountService::OnRegisterInitialize(
    RegisterInitializeCallback callback,
    int response_code,
...
void QoraiAccountService::OnRegisterFinalize(
    RegisterFinalizeCallback callback,
    const std::string& encrypted_verification_token,
    int response_code,
...
std::optional<mojom::RegisterErrorCode> QoraiAccountService::TransformError(
...
void QoraiAccountService::OnVerificationTokenChanged() {
...
void QoraiAccountService::VerifyResult(
  ...
  current_verify_result_request =
      Client<endpoints::VerifyResult>::Send<RequestCancelability::kCancelable>(
          url_loader_factory_, std::move(request),
          base::BindOnce(&QoraiAccountService::OnVerifyResult,
...
void QoraiAccountService::OnVerifyResult(
    int response_code,
  ...
  if (!authentication_token.empty()) {
    ...
    // TODO(https://github.com/qorai/qorai-browser/issues/50307)
    pref_service_->ClearPref(prefs::kVerificationToken);
