...
import { getCss } from './qorai_account_create_dialog.css.js'
import { getHtml } from './qorai_account_create_dialog.html.js'
import { Error, isEmailValid } from './qorai_account_common.js'
import {
  RegisterError,
  RegisterErrorCode,
} from './qorai_account.mojom-webui.js'

// @ts-expect-error
import { Registration } from 'chrome://resources/qorai/opaque_ke.bundle.js'
...
class PasswordStrengthMeter extends CrLitElement {
  ...
  static override get styles() {
    return css`
      ...
      :host([category='Weak']) {
        --primary-color: var(--qora-color-systemfeedback-error-icon);
        --secondary-color: var(--qora-color-systemfeedback-error-background);
      }

      :host([category='Medium']) {
        --primary-color: var(--qora-color-systemfeedback-warning-icon);
        --secondary-color: var(--qora-color-systemfeedback-warning-background);
      }

      :host([category='Strong']) {
        --primary-color: var(--qora-color-systemfeedback-success-icon);
        --secondary-color: var(--qora-color-systemfeedback-success-background);
      }
    ...
    return html`
      ...
      <div class="text">
        ${loadTimeData.getString(
          `qoraiAccountPasswordStrengthMeter${this.category}`,
        )}
      </div>
    `
  }
...
export class QoraiAccountCreateDialogElement extends CrLitElement {
  static get is() {
    return 'qorai-account-create-dialog'
  }
  ...
  static override get properties() {
    return {
      ...
      isEmailQoraiAlias: { type: Boolean },
  ...
  protected onEmailInput(detail: { value: string }) {
    ...
    this.isEmailQoraiAlias = /@qoraialias\.com$/i.test(this.email)
  }
  ...
  // The reason this happens here (rather than in QoraiAccountService) is that
  ...
  protected accessor isEmailQoraiAlias: boolean = false
...
declare global {
  interface HTMLElementTagNameMap {
    'qorai-account-create-dialog': QoraiAccountCreateDialogElement
  }
}

customElements.define(
  QoraiAccountCreateDialogElement.is,
  QoraiAccountCreateDialogElement,
)
