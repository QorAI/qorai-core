...
#ifndef QORAI_COMPONENTS_QORAI_ACCOUNT_QORAI_ACCOUNT_SERVICE_TEST_H_
#define QORAI_COMPONENTS_QORAI_ACCOUNT_QORAI_ACCOUNT_SERVICE_TEST_H_
...
class QoraiAccountServiceTest : public testing::TestWithParam<const TestCase*> {
 public:
 ...
 protected:
  void SetUp() override {
    ...
    qorai_account_service_ = base::WrapUnique(new QoraiAccountService(
        &pref_service_, test_url_loader_factory_.GetSafeWeakWrapper(),
        base::BindRepeating(&QoraiAccountServiceTest::Encrypt,
                            base::Unretained(this)),
        base::BindRepeating(&QoraiAccountServiceTest::Decrypt,
                            base::Unretained(this)),
  ...
  void RunTestCase() {
    ...
    if constexpr (std::is_same_v<TestCase, RegisterInitializeTestCase> ||
      ...
      TestCase::Run(test_case, CHECK_DEREF(qorai_account_service_.get()),
                    future.GetCallback());
      ...
      TestCase::Run(test_case, pref_service_,
                    CHECK_DEREF(qorai_account_service_.get()));
    } else {
      static_assert(false);
    }
  }
  ...
  base::test::ScopedFeatureList scoped_feature_list_{features::kQoraiAccount};
  TestingPrefServiceSimple pref_service_;
  network::TestURLLoaderFactory test_url_loader_factory_;
  std::unique_ptr<QoraiAccountService> qorai_account_service_;
  raw_ptr<base::OneShotTimer> verify_result_timer_;
};

}  // namespace qorai_account

#endif  // QORAI_COMPONENTS_QORAI_ACCOUNT_QORAI_ACCOUNT_SERVICE_TEST_H_
