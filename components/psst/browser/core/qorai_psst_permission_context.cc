...
#include "qorai/components/psst/common/psst_ui_common.mojom.h"
...
QoraiPsstPermissionContext::QoraiPsstPermissionContext(
    HostContentSettingsMap* host_content_settings_map)
    : ObjectPermissionContextBase(ContentSettingsType::QORAI_PSST,
                                  host_content_settings_map) {}
QoraiPsstPermissionContext::~QoraiPsstPermissionContext() = default;

std::string QoraiPsstPermissionContext::GetKeyForObject(
...
bool QoraiPsstPermissionContext::IsValidObject(
    const base::Value::Dict& object) {
  const auto* user_id = object.FindString(kUserIdPermissionKey);
...
std::u16string QoraiPsstPermissionContext::GetObjectDisplayName(
    const base::Value::Dict& object) {
  return base::UTF8ToUTF16(GetKeyForObject(object));
}

void QoraiPsstPermissionContext::GrantPermission(
...
bool QoraiPsstPermissionContext::HasPermission(const url::Origin& origin,
                                               std::string_view user_id) {
  return !!GetPsstPermissionInfo(origin, user_id);
}

void QoraiPsstPermissionContext::RevokePermission(const url::Origin& origin,
                                                  std::string_view user_id) {
...
QoraiPsstPermissionContext::GetPsstPermissionInfo(const url::Origin& origin,
                                                  std::string_view user_id) {
  if (!IsAllowedToProcess(origin, user_id)) {
    return std::nullopt;
  }
