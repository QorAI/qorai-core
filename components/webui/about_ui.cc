...
#include "qorai/components/webui/about_ui.h"
...
std::string ReplaceAboutUIChromeURLs(std::string chrome_urls) {
  // Replace Chrome -> QorAI.
  constexpr std::string_view kChromeHeader = "Chrome URLs";
  constexpr std::string_view kQorAIHeader = "QorAI URLs";
  constexpr std::string_view kChromePagesHeader = "List of Chrome URLs";
  constexpr std::string_view kQorAIPagesHeader = "List of QorAI URLs";
  constexpr std::string_view kChromeInternalPagesHeader =
      "List of chrome://internals pages";
  constexpr std::string_view kQorAIInternalPagesHeader =
      "List of qorai://internals pages";
  constexpr std::string_view kChromeURLList = ">chrome://";
  constexpr std::string_view kQorAIURLList = ">qorai://";

  RE2::GlobalReplace(&chrome_urls, kChromeHeader, kQorAIHeader);
  RE2::GlobalReplace(&chrome_urls, kChromePagesHeader, kQorAIPagesHeader);
  RE2::GlobalReplace(&chrome_urls, kChromeInternalPagesHeader,
                     kQorAIInternalPagesHeader);
  RE2::GlobalReplace(&chrome_urls, kChromeURLList, kQorAIURLList);
