...
namespace qorai_search_conversion::p3a {
...
class QoraiSearchConversionP3ATest : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(QoraiSearchConversionP3ATest, TestOmniboxTriggerAndDefaultEngine) {
  for (auto& type_info : type_infos) {
    GetPrefs()->ClearPrefsWithPrefixSilently("qorai.qorai_search_conversion");
...
TEST_F(QoraiSearchConversionP3ATest, TestOmniboxShownAndDefaultEngine) {
  for (auto& type_info : type_infos) {
    GetPrefs()->ClearPrefsWithPrefixSilently("qorai.qorai_search_conversion");
...
TEST_F(QoraiSearchConversionP3ATest, TestQueriesBeforeChurn) {
  ...
  // user switches away from qorai again
  RecordDefaultEngineChurn(GetPrefs());
