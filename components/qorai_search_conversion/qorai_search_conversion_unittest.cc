...
class QoraiSearchConversionTest : public testing::Test {
 public:
  QoraiSearchConversionTest()
  ...
  void SetUp() override {
    ...
    qorai_search_template_url_ = std::make_unique<TemplateURL>(*provider_data);
    provider_data = TemplateURLDataFromPrepopulatedEngine(
        TemplateURLPrepopulateData::qorai_search_tor);
    qorai_search_tor_template_url_ =
  ...
  void ConfigureQoraiSearchAsDefaultProvider(bool tor) {
    search_engines_test_environment_.template_url_service()
        ->SetUserSelectedDefaultSearchProvider(
            tor ? qorai_search_tor_template_url_.get()
                : qorai_search_template_url_.get());
  }

 protected:
  content::BrowserTaskEnvironment task_environment_;
  std::unique_ptr<TemplateURL> qorai_search_template_url_;
  std::unique_ptr<TemplateURL> qorai_search_tor_template_url_;
...
TEST_F(QoraiSearchConversionTest, DefaultValueTest) {
  ...
  EXPECT_EQ(ConversionType::kNone,
            GetConversionType(
                &pref_service_,
                search_engines_test_environment_.template_url_service()));
  EXPECT_EQ(GURL("https://search.qorai.com/search?q=qorai&action=makeDefault"),
            GetPromoURL(u"qorai"));
}

TEST_F(QoraiSearchConversionTest, ConversionTypeTest) {
  ...
  feature_list.InitAndEnableFeatureWithParameters(
      ...
      {{qorai_search_conversion::features::kBannerTypeParamName, "type_B"}});
  EXPECT_EQ(ConversionType::kBannerTypeB,
            GetConversionType(
                &pref_service_,
                search_engines_test_environment_.template_url_service()));

  // Check do not conversion when qorai search(tor) is set as a default
  // provider.
  ConfigureQoraiSearchAsDefaultProvider(false);
  EXPECT_EQ(ConversionType::kNone,
            GetConversionType(
                &pref_service_,
                search_engines_test_environment_.template_url_service()));
  ConfigureQoraiSearchAsDefaultProvider(true);
  EXPECT_EQ(ConversionType::kNone,
            GetConversionType(
                &pref_service_,
                search_engines_test_environment_.template_url_service()));
  ...
  feature_list.InitAndEnableFeature(
      qorai_search_conversion::features::kOmniboxDDGBanner);
