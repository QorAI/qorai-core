...
#include "qorai/components/qorai_search_conversion/constants.h"
...
bool IsNTPPromotionEnabled(PrefService* prefs, TemplateURLService* service) {
  ...
  // Don't need to prompt for conversion if user uses qorai as a default
  // provider.
  ...
  const auto id = template_url->prepopulate_id();
  if (id == TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_QORAI ||
      id == TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_QORAI_TOR) {
    return false;
  }
...
ConversionType GetConversionType(PrefService* prefs,
  ...
  // Don't need to ask conversion if user uses qorai as a default provider.
...
GURL GetPromoURL(const std::string& search_term) {
  std::string promo_url(kQoraiSearchConversionPromotionURL);
