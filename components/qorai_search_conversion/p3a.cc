...
constexpr char kSwitchSearchEngineMetric[] = "Qorai.Search.SwitchEngine";
...
void UpdateHistograms(PrefService* prefs) {
  ...
  for (const auto type : types) {
    ...
    // 0 = have not triggered promo, have not made Qorai default via SERP
    // 1 = have triggered promo, have not made Qorai default via SERP
    int answer = promo_triggered;

    if (default_engine_triggered) {
      // 2 = have not triggered promo, have made Qorai default via SERP
      // 3 = have triggered promo, have made Qorai default via SERP
      answer += 2;
    }
