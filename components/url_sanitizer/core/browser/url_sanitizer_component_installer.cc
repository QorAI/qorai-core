...
using qorai_component_updater::LocalDataFilesObserver;
...
qorai::URLSanitizerComponentInstaller::RawConfig LoadRawConfig(
    const base::FilePath& resource_dir) {
  qorai::URLSanitizerComponentInstaller::RawConfig config;
  config.matchers = qorai_component_updater::GetDATFileAsString(
      resource_dir.AppendASCII(kCleanURLsConfigFile));
  config.permissions = qorai_component_updater::GetDATFileAsString(
