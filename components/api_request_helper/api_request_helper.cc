...
void APIRequestHelper::URLLoaderHandler::OnDataReceived(
  ...
  if (is_sse_) {
    ...
    TRACE_EVENT0("qorai", "APIRequestHelper_OnDataReceivedNoSSE");
    ScopedPerfTracker tracker("Qorai.APIRequestHelper.OnDataReceivedNoSSE");
...
void APIRequestHelper::URLLoaderHandler::OnParseJsonResponse(
  ...
  TRACE_EVENT1("qorai", "APIRequestHelper_ProcessResultOnUI", "url",
               result.final_url().spec());
  ScopedPerfTracker tracker("Qorai.APIRequestHelper.ProcessResultOnUI");
  ...
  for (const auto& data : stream_data) {
    ...
    auto on_json_parsed =
        [](base::WeakPtr<APIRequestHelper::URLLoaderHandler> handler,
          ...
          TRACE_EVENT0("qorai", "APIRequestHelper_ParseSSECallback");
          ScopedPerfTracker tracker("Qorai.APIRequestHelper.ParseSSECallback");
