...
namespace {
constexpr char kXattrName[] = "com.qorai.refcode";
constexpr char kXattrValue[] = "0xDEADFACE";
}  // namespace

class QoraiFileExtendedAttributeTest : public testing::Test {
 public:
  QoraiFileExtendedAttributeTest() = default;
  ~QoraiFileExtendedAttributeTest() override = default;
...
TEST_F(QoraiFileExtendedAttributeTest, GetPromoCodeAttribute) {
  std::vector<char> value;
  // Test file has this extended attribute
  int result_errno =
      qorai::GetFileExtendedAttribute(test_file_path(), kXattrName, &value);
  EXPECT_EQ(0, result_errno);
  std::string xattr_value(value.begin(), value.end());
  EXPECT_EQ(kXattrValue, xattr_value);
}

TEST_F(QoraiFileExtendedAttributeTest, GetNonexistentAttribute) {
  std::vector<char> value;
  // Test file does NOT have this extended attribute
  int result_errno = qorai::GetFileExtendedAttribute(
      test_file_path(), "com.qorai.MadSheep", &value);
