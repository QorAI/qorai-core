...
class QoraiReferralsServiceTest : public testing::Test {
 public:
  QoraiReferralsServiceTest()
  ...
  void SetUp() override {
    ...
    RegisterPrefsForQoraiReferralsService(pref_service_.registry());
    ...
    QoraiReferralsService::SetPromoFilePathForTesting(promo_code_file_path);
    ...
    QoraiReferralsService::SetReferralInitializedCallbackForTesting(
        new QoraiReferralsService::ReferralInitializedCallback(
  ...
  void TearDown() override {
    ...
    QoraiReferralsService::SetPromoFilePathForTesting(base::FilePath());
    QoraiReferralsService::SetReferralInitializedCallbackForTesting(nullptr);
  }

  class TestQoraiReferralsServiceDelegate
      : public QoraiReferralsService::Delegate {
   public:
    TestQoraiReferralsServiceDelegate() = default;
    ~TestQoraiReferralsServiceDelegate() override = default;
  ...
  void CreateReferralsService() {
    auto test_delegate = std::make_unique<TestQoraiReferralsServiceDelegate>();
    ...
    referrals_service_ = std::make_unique<QoraiReferralsService>(
 ...
 protected:
  ...
  std::unique_ptr<QoraiReferralsService> referrals_service_;
  size_t request_count_ = 0;
  bool init_callback_called_ = false;
  std::string received_download_id_;
};

TEST_F(QoraiReferralsServiceTest, InitAndActivity) {
...
TEST_F(QoraiReferralsServiceTest, StatsDisabledAtInit) {
...
TEST_F(QoraiReferralsServiceTest, StatsDisabledAfterInit) {
  CreateReferralsService();
  referrals_service_->Start();
