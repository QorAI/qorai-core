...
#include "qorai/vendor/qorai_base/random.h"
...
#include "qorai/components/qorai_referrals/browser/file_extended_attribute.h"
...
QoraiReferralsService::ReferralInitializedCallback*
...
std::string ReadPromoCodeFromXattr() {
  static constexpr char kRefCodeAttr[] = "com.qorai.refcode";
  base::FilePath bundle_path = base::apple::OuterBundlePath();
  std::vector<char> value;
  int result_errno =
      qorai::GetFileExtendedAttribute(bundle_path, kRefCodeAttr, &value);
...
std::string BuildReferralEndpoint(const std::string& path) {
  ...
  std::string referral_server =
      env->GetVar("QORAI_REFERRALS_SERVER").value_or(std::string());
  if (referral_server.empty()) {
    auto referral_domain = qorai_domains::GetServicesDomain("usage-ping");
...
QoraiReferralsService::QoraiReferralsService(PrefService* pref_service,
...
QoraiReferralsService::~QoraiReferralsService() = default;

void QoraiReferralsService::Start() {
  if (initialized_)
    return;
  ...
  finalization_checks_timer_->Start(
      FROM_HERE,
      base::Seconds(
          qorai_base::random::Geometric(kFinalizationChecksFrequency)),
      this, &QoraiReferralsService::OnFinalizationChecksTimerFired);
  ...
  if (!checked_for_promo_code_file && !has_initialized && download_id.empty()) {
    ...
    task_runner_->PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&QoraiReferralsService::OnReadPromoCodeComplete,
...
void QoraiReferralsService::Stop() {
...
// static
void QoraiReferralsService::SetReferralInitializedCallbackForTesting(
...
// static
bool QoraiReferralsService::IsDefaultReferralCode(const std::string& code) {
  return code == kDefaultPromoCode;
}

void QoraiReferralsService::OnFinalizationChecksTimerFired() {
  PerformFinalizationChecks();
}

void QoraiReferralsService::OnReferralInitLoadComplete(
  ...
  if (referral_init_loader_->NetError() != net::OK || response_code < 200 ||
    ...
    initialization_timer_->Start(FROM_HERE,
                                 base::Seconds(qorai_base::random::Geometric(
                                     kReportInitializationFrequency)),
                                 this, &QoraiReferralsService::InitReferral);
...
void QoraiReferralsService::OnReferralFinalizationCheckLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
...
void QoraiReferralsService::OnReadPromoCodeComplete(
...
void QoraiReferralsService::GetFirstRunTime() {
#if BUILDFLAG(IS_ANDROID)
  ...
  task_runner_->PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&QoraiReferralsService::SetFirstRunTime,
                     weak_factory_.GetWeakPtr()));
#endif
}

void QoraiReferralsService::SetFirstRunTime(
...
void QoraiReferralsService::PerformFinalizationChecks() {
  // Delete the promo code preference, if appropriate.
  content::GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&QoraiReferralsService::MaybeDeletePromoCodePref,
                     weak_factory_.GetWeakPtr()));

  // Check for referral finalization, if appropriate.
  content::GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&QoraiReferralsService::MaybeCheckForReferralFinalization,
                     weak_factory_.GetWeakPtr()));
}

// static
void QoraiReferralsService::SetPromoFilePathForTesting(
    const base::FilePath& path) {
  g_promo_file_path = path;
}

base::FilePath QoraiReferralsService::GetPromoCodeFileName() const {
...
void QoraiReferralsService::MaybeCheckForReferralFinalization() {
  ...
  std::string check_time_str =
      env->GetVar("QORAI_REFERRALS_CHECK_TIME").value_or(std::string());
...
void QoraiReferralsService::MaybeDeletePromoCodePref() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  uint64_t delete_time = 90 * 24 * 60 * 60;
  auto env = base::Environment::Create();
  std::string delete_time_str =
      env->GetVar("QORAI_REFERRALS_DELETE_TIME").value_or(std::string());
...
std::string QoraiReferralsService::BuildReferralInitPayload() const {
...
std::string QoraiReferralsService::BuildReferralFinalizationCheckPayload()
    const {
  base::Value::Dict root;
  root.Set("api_key", api_key_);
...
void QoraiReferralsService::InitReferral() {
  ...
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("qorai_referral_initializer", R"(
        semantics {
          sender:
            "Qorai Referrals Service"
          description:
            "Validates the current referral offer with Qorai, potentially "
            "unlocking special features and/or services."
          trigger:
            "On startup, sends the current referral code to Qorai."
          data: "Qorai referral metadata."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url = GURL(BuildReferralEndpoint(kQoraiReferralsInitPath));
  ...
  referral_init_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&QoraiReferralsService::OnReferralInitLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

void QoraiReferralsService::CheckForReferralFinalization() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("qorai_referral_finalization_checker",
                                          R"(
        semantics {
          sender:
            "Qorai Referrals Service"
          description:
            "Fetches referral finalization data from Qorai."
          trigger:
            ""
          data: "Qorai referral finalization status."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url =
      GURL(BuildReferralEndpoint(kQoraiReferralsActivityPath));
  resource_request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  resource_request->load_flags = net::LOAD_DO_NOT_SAVE_COOKIES |
                                 net::LOAD_BYPASS_CACHE |
                                 net::LOAD_DISABLE_CACHE;
  ...
  referral_finalization_check_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(
          &QoraiReferralsService::OnReferralFinalizationCheckLoadComplete,
          base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

#if BUILDFLAG(IS_ANDROID)
void QoraiReferralsService::InitAndroidReferrer() {
  android_qorai_referrer::InitReferrerCallback init_referrer_callback =
      base::BindOnce(&QoraiReferralsService::OnAndroidQoraiReferrerReady,
                     weak_factory_.GetWeakPtr());
  android_qorai_referrer_.InitReferrer(std::move(init_referrer_callback));
}

void QoraiReferralsService::OnAndroidQoraiReferrerReady() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  task_runner_->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&ReadPromoCode, GetPromoCodeFileName()),
      base::BindOnce(&QoraiReferralsService::OnReadPromoCodeComplete,
