...
std::string formatSkusStateValue(const base::Time start_time,
  ...
  const std::string skus_domain = "qora.qorai.com";
#else
  const std::string skus_domain = "qora.qoraisoftware.com";
#endif
...
class AIChatCredentialManagerUnitTest : public testing::Test {
 public:
  ...
  void SetSkusState(const std::string& skusStateValue) {
    ...
    // Qora SKUs, which is staging. Official builds will use
...
TEST_F(AIChatCredentialManagerUnitTest, CacheDefault) {
  ...
  EXPECT_EQ(prefs_service_.GetDict(prefs::kQoraiChatPremiumCredentialCache),
            base::Value::Dict());
}
