...
TEST_F(HistoryUIHandlerTest, SearchWithQuery) {
  ...
  AddTestHistoryEntry("Qorai Browser", GURL("https://qorai.com"));
  AddTestHistoryEntry("Google Search", GURL("https://google.com"));
  AddTestHistoryEntry("Qorai Search", GURL("https://search.qorai.com"));

  // Search for "Qorai"
  auto history = GetHistory("Qorai");

  // Should return only entries matching "Qorai"
  ...
  ExpectResults(FROM_HERE, {"Qorai Browser", "Qorai Search"},
                {GURL("https://qorai.com"), GURL("https://search.qorai.com")},
                history);
}
