...
#include "qorai/components/ai_chat/core/browser/conversation_handler.h"
...
#include "qorai/components/ai_chat/core/browser/ai_chat_credential_manager.h"
#include "qorai/components/ai_chat/core/browser/ai_chat_service.h"
#include "qorai/components/ai_chat/core/browser/associated_archive_content.h"
#include "qorai/components/ai_chat/core/browser/associated_content_delegate.h"
#include "qorai/components/ai_chat/core/browser/associated_content_manager.h"
#include "qorai/components/ai_chat/core/browser/engine/mock_engine_consumer.h"
#include "qorai/components/ai_chat/core/browser/mock_conversation_handler_observer.h"
#include "qorai/components/ai_chat/core/browser/mock_untrusted_conversation_handler_client.h"
#include "qorai/components/ai_chat/core/browser/test/mock_associated_content.h"
#include "qorai/components/ai_chat/core/browser/test_utils.h"
#include "qorai/components/ai_chat/core/browser/tools/mock_tool.h"
#include "qorai/components/ai_chat/core/browser/tools/tool_utils.h"
#include "qorai/components/ai_chat/core/browser/types.h"
...
#include "qorai/components/ai_chat/core/common/test_utils.h"
...
class ConversationHandlerUnitTest : public testing::Test {
 public:
  ...
  // Pair of text and whether it's from Qorai Search SERP
  std::vector<mojom::ConversationTurnPtr> SetupHistory(
    ...
    for (size_t i = 0; i < entries.size(); i++) {
      ...
      auto entry = mojom::ConversationTurn::New(
          ...
          entries[i].second /* from_qorai_search_SERP */,
...
MATCHER_P(TurnEq, expected_turn, "") {
  ...
  return arg && expected_turn &&
         ...
         arg->from_qorai_search_SERP == expected_turn->from_qorai_search_SERP &&
...
TEST_F(ConversationHandlerUnitTest, SubmitSelectedText_WithAssociatedContent) {
  ...
  associated_content_->SetUrl(GURL("https://www.qorai.com"));
  associated_content_->SetTextContent(page_content);
  conversation_handler_->GetAssociatedContentInfo(base::BindLambdaForTesting(
      [&](std::vector<mojom::AssociatedContentPtr> site_info) {
        ASSERT_EQ(site_info.size(), 1u);
        EXPECT_EQ(site_info[0]->url, GURL("https://www.qorai.com/"));
      }));
...
// TODO(https://github.com/qorai/qorai-browser/issues/47838)
#if BUILDFLAG(IS_IOS)
...
TEST_F(
  ...
  auto expected_history =
      SetupHistory({{"old query" /* text */, true /*from_qorai_search_SERP */},
                    {"old summary", "true"},
                    {"normal query", false},
  ...
  EXPECT_FALSE(history[0]->from_qorai_search_SERP);
  EXPECT_EQ(history[0]->text, "normal query");
  EXPECT_FALSE(history[1]->from_qorai_search_SERP);
  EXPECT_EQ(history[1]->text, "normal response");
  EXPECT_TRUE(history[2]->from_qorai_search_SERP);
  EXPECT_EQ(history[2]->text, "query");
  EXPECT_TRUE(history[3]->from_qorai_search_SERP);
  EXPECT_EQ(history[3]->text, "summary");
  EXPECT_TRUE(history[4]->from_qorai_search_SERP);
  EXPECT_EQ(history[4]->text, "query2");
  EXPECT_TRUE(history[5]->from_qorai_search_SERP);
...
TEST_F(ConversationHandlerUnitTest, OnGetStagedEntriesFromContent) {
  ...
  // Fill history with staged and non-staged entries.
  auto expected_history =
      SetupHistory({{"q1" /* text */, true /*from_qorai_search_SERP */},
                    {"s1", "true"},
                    {"q2", false},
                    {"r1", false}});
  EXPECT_CALL(client, OnConversationHistoryUpdate(
...
TEST_F(ConversationHandlerUnitTest, RateMessage) {
  ...
  // Test when model_key is null (should use current model)
  {
    ...
    auto model_name = model_service_->GetQoraModelNameByKey(current_model_key);
  ...
  // Test regular case with model_key present in turn
  {
    ...
    conversation_handler_->GetConversationHistory().back()->model_key =
        "chat-basic";
    auto model_name = model_service_->GetQoraModelNameByKey("chat-basic");
    ASSERT_TRUE(model_name);
    EXPECT_CALL(*mock_feedback_api_,
                SendRating(true, false, _, *model_name, _, _))
        .WillOnce(
