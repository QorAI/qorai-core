...
#include "qorai/components/ai_chat/core/browser/ollama/ollama_service.h"
...
constexpr net::NetworkTrafficAnnotationTag kOllamaConnectionAnnotation =
    net::DefineNetworkTrafficAnnotation("qorai_qora_assistant_ollama_connection",
                                        R"(
        semantics {
          sender: "Qorai Qora Assistant"
          description:
            "Check if Ollama is running on localhost to enable fetching."
          trigger:
            "User accesses Qora Assistant settings with Ollama fetching."
          data:
        ...
        policy {
          cookies_allowed: NO
          setting: "This feature can be controlled in Qora Assistant settings."
        })");
