...
TEST_F(ModelServiceTest, MigrateOldClaudeDefaultModelKey) {
  ...
  pref_service_.SetString("qorai.ai_chat.default_model_key",
                          "chat-claude-instant");
...
TEST_F(ModelServiceTest, AddAndModifyCustomModel) {
  ...
  const GURL endpoint = GURL("http://qorai.com");
...
TEST_F(ModelServiceTest, CalcuateMaxAssociatedContentLengthForModel_QoraModel) {
  // Setup a qora model with predefined page content length
  static constexpr size_t expected_content_length = 10'000;

  mojom::QoraModelOptionsPtr qora_options = mojom::QoraModelOptions::New();
  qora_options->max_associated_content_length = expected_content_length;

  mojom::Model qora_model;
  qora_model.options =
      mojom::ModelOptions::NewQoraModelOptions(std::move(qora_options));

  // Calculate max associated content length
  size_t max_content_length =
      GetService()->CalcuateMaxAssociatedContentLengthForModel(qora_model);

  // Validate that the predefined value is returned
  EXPECT_EQ(max_content_length, expected_content_length);
}

TEST_F(ModelServiceTest, GetQoraModelKeyByName_And_GetQoraModelNameByKey) {
  auto& models = GetService()->GetModels();
  for (const auto& model : models) {
    ASSERT_TRUE(model->options->is_qora_model_options());
    EXPECT_EQ(GetService()->GetQoraModelKeyByName(
                  model->options->get_qora_model_options()->name),
              model->key);
    EXPECT_EQ(GetService()->GetQoraModelNameByKey(model->key),
              model->options->get_qora_model_options()->name);
  }

  // Test with an invalid model name or key
  auto key = GetService()->GetQoraModelKeyByName("nonexistent-model");
  EXPECT_FALSE(key.has_value());
  auto name = GetService()->GetQoraModelNameByKey("nonexistent-key");
