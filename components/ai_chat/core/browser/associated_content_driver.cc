...
#include "qorai/components/ai_chat/core/browser/qorai_search_responses.h"
...
GetSearchQuerySummaryNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation(
      ...
      R"(
      semantics {
        sender: "Qorai Qora AI Chat"
        description:
          "This sender is used to get search query summary from Qorai search."
        trigger:
          "Triggered by uses of Qorai Qora AI Chat on Qorai Search SERP."
        data:
...
void AssociatedContentDriver::GetStagedEntriesFromContent(
  ...
  // - Qorai Search results page
  if (!IsQoraiSearchSERP(url())) {
...
void AssociatedContentDriver::OnSearchSummarizerKeyFetched(
  ...
  // https://search.qorai.com/api/chatllm/raw_data?key=<key>
  GURL base_url(
...
AssociatedContentDriver::ParseSearchQuerySummaryResponse(
  ...
  auto search_query_response =
      qorai_search_responses::QuerySummaryResponse::FromValue(value);
