...
#include "qorai/components/ai_chat/core/common/mojom/ai_chat.mojom-shared.h"
...
constexpr char kQoraiSearchEntryPointKey[] = "qorai_search";
...
constexpr auto kEntryPointKeys =
    base::MakeFixedFlatMap<EntryPoint, const char*>(
        {{EntryPoint::kOmniboxItem, kOmniboxItemEntryPointKey},
         ...
         {EntryPoint::kQoraiSearch, kQoraiSearchEntryPointKey}});
...
AIChatMetrics::AIChatMetrics(PrefService* local_state,
    ...
    : is_premium_(
          local_state->GetBoolean(prefs::kQoraiChatP3ALastPremiumStatus)),
      chat_count_storage_(local_state,
                          prefs::kQoraiChatP3AChatCountWeeklyStorage),
      chat_with_history_count_storage_(
          local_state,
          prefs::kQoraiChatP3AChatWithHistoryCountWeeklyStorage),
      chat_durations_storage_(local_state,
                              prefs::kQoraiChatP3AChatDurationsWeeklyStorage),
      prompt_count_storage_(local_state,
                            prefs::kQoraiChatP3APromptCountWeeklyStorage),
      rate_limit_storage_(local_state, prefs::kQoraiChatP3ARateLimitStops),
      context_limit_storage_(local_state, prefs::kQoraiChatP3AContextLimits),
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
      omnibox_open_storage_(local_state,
                            prefs::kQoraiChatP3AOmniboxOpenWeeklyStorage,
                            14),
      omnibox_autocomplete_storage_(
          local_state,
          prefs::kQoraiChatP3AOmniboxAutocompleteWeeklyStorage,
          14),
      sidebar_usage_storage_(local_state, prefs::kQoraiChatP3ASidebarUsages),
      full_page_switch_storage_(local_state,
                                prefs::kQoraiChatP3AFullPageSwitches),
#endif  // !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
  ...
  for (int i = 0; i <= static_cast<int>(ContextMenuAction::kMaxValue); i++) {
    ...
    context_menu_usage_storages_[action] = std::make_unique<WeeklyStorage>(
        local_state_, prefs::kQoraiChatP3AContextMenuUsages,
  ...
  for (int i = 0; i <= static_cast<int>(EntryPoint::kMaxValue); i++) {
    ...
    entry_point_storages_[entry_point] = std::make_unique<WeeklyStorage>(
        local_state_, prefs::kQoraiChatP3AEntryPointUsages,
  ...
  for (int i = 0; i <= static_cast<int>(ContextSource::kMaxValue); i++) {
    ...
    context_source_storages_[context_source] = std::make_unique<WeeklyStorage>(
        local_state_, prefs::kQoraiChatP3AContextSourceUsages,
...
void AIChatMetrics::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterListPref(prefs::kQoraiChatP3AChatCountWeeklyStorage);
  registry->RegisterListPref(
      prefs::kQoraiChatP3AChatWithHistoryCountWeeklyStorage);
  registry->RegisterListPref(prefs::kQoraiChatP3AChatDurationsWeeklyStorage);
  registry->RegisterListPref(prefs::kQoraiChatP3APromptCountWeeklyStorage);
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
  registry->RegisterListPref(prefs::kQoraiChatP3AOmniboxOpenWeeklyStorage);
  registry->RegisterListPref(
      prefs::kQoraiChatP3AOmniboxAutocompleteWeeklyStorage);
  registry->RegisterDictionaryPref(prefs::kQoraiChatP3AContextMenuUsages);
  registry->RegisterTimePref(prefs::kQoraiChatP3ALastContextMenuUsageTime, {});
#endif  // !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
  registry->RegisterTimePref(prefs::kQoraiChatP3ALastPremiumCheck, {});
  registry->RegisterBooleanPref(prefs::kQoraiChatP3ALastPremiumStatus, false);
  registry->RegisterTimePref(prefs::kQoraiChatP3AFirstUsageTime, {});
  registry->RegisterTimePref(prefs::kQoraiChatP3ALastUsageTime, {});
  registry->RegisterBooleanPref(prefs::kQoraiChatP3AUsedSecondDay, false);
  registry->RegisterBooleanPref(prefs::kQoraiChatP3AFirstChatPromptsReported,
                                false);
  registry->RegisterDictionaryPref(prefs::kQoraiChatP3AContextSourceUsages);
  registry->RegisterDictionaryPref(prefs::kQoraiChatP3AEntryPointUsages);
  registry->RegisterListPref(prefs::kQoraiChatP3ASidebarUsages);
  registry->RegisterListPref(prefs::kQoraiChatP3AFullPageSwitches);
  registry->RegisterListPref(prefs::kQoraiChatP3ARateLimitStops);
  registry->RegisterListPref(prefs::kQoraiChatP3AContextLimits);
...
void AIChatMetrics::RecordEnabled(
  ...
  if (is_enabled && !is_new_user &&
      local_state_->GetTime(prefs::kQoraiChatP3AFirstUsageTime).is_null()) {
    ...
    local_state_->SetTime(prefs::kQoraiChatP3AFirstUsageTime, three_months_ago);
    local_state_->SetTime(prefs::kQoraiChatP3ALastUsageTime, three_months_ago);
  }
  ...
  if (retrieve_premium_status_callback) {
    base::Time last_premium_check =
        local_state_->GetTime(prefs::kQoraiChatP3ALastPremiumCheck);
...
void AIChatMetrics::OnPremiumStatusUpdated(bool is_enabled,
  ...
  local_state_->SetBoolean(prefs::kQoraiChatP3ALastPremiumStatus, is_premium_);
  local_state_->SetTime(prefs::kQoraiChatP3ALastPremiumCheck,
                        base::Time::Now());
...
void AIChatMetrics::RecordNewPrompt(ConversationHandlerForMetrics* handler,
  ...
  p3a_utils::RecordFeatureUsage(local_state_,
                                prefs::kQoraiChatP3AFirstUsageTime,
                                prefs::kQoraiChatP3ALastUsageTime);
  ReportFeatureUsageMetrics();
...
void AIChatMetrics::MaybeRecordLastError(
  ...
  if (handler->current_error() == mojom::APIError::RateLimitReached &&
      (base::Time::Now() -
       local_state_->GetTime(prefs::kQoraiChatP3AFirstUsageTime)) <=
          base::Days(7)) {
...
void AIChatMetrics::MaybeReportFirstChatPrompts(bool new_prompt_made) {
  if (local_state_->GetBoolean(prefs::kQoraiChatP3AFirstChatPromptsReported)) {
    return;
  }
  if (new_prompt_made) {
  ...
  local_state_->SetBoolean(prefs::kQoraiChatP3AFirstChatPromptsReported, true);
}
...
void AIChatMetrics::RecordContextMenuUsage(ContextMenuAction action) {
  ...
  local_state_->SetTime(prefs::kQoraiChatP3ALastContextMenuUsageTime,
                        base::Time::Now());
...
void AIChatMetrics::ReportFeatureUsageMetrics() {
  p3a_utils::RecordFeatureNewUserReturning(
      local_state_, prefs::kQoraiChatP3AFirstUsageTime,
      prefs::kQoraiChatP3ALastUsageTime, prefs::kQoraiChatP3AUsedSecondDay,
      kNewUserReturningHistogramName);
  p3a_utils::RecordFeatureLastUsageTimeMetric(
      local_state_, prefs::kQoraiChatP3ALastUsageTime,
...
void AIChatMetrics::ReportContextMenuMetrics() {
  ...
  p3a_utils::RecordFeatureLastUsageTimeMetric(
      local_state_, prefs::kQoraiChatP3ALastContextMenuUsageTime,
