...
TEST_F(AIChatUtilsUnitTest, IsQoraiSearchSERP) {
  EXPECT_TRUE(IsQoraiSearchSERP(GURL("https://search.qorai.com/search?q=foo")));
  // Missing or wrong path.
  EXPECT_FALSE(IsQoraiSearchSERP(GURL("https://search.qorai.com?q=foo")));
  EXPECT_FALSE(
      IsQoraiSearchSERP(GURL("https://search.qorai.com/test.html?q=foo")));
  // Missing or wrong query parameter.
  EXPECT_FALSE(IsQoraiSearchSERP(GURL("https://search.qorai.com/search")));
  EXPECT_FALSE(IsQoraiSearchSERP(GURL("https://search.qorai.com/search?t=t")));
  // HTTP scheme.
  EXPECT_FALSE(IsQoraiSearchSERP(GURL("http://search.qorai.com/search?q=foo")));
  // Wrong host.
  EXPECT_FALSE(IsQoraiSearchSERP(GURL("https://qorai.com/search?q=foo")));
}
