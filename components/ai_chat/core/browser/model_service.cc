...
#include "qorai/components/ai_chat/core/browser/constants.h"
#include "qorai/components/ai_chat/core/browser/engine/engine_consumer.h"
#include "qorai/components/ai_chat/core/browser/engine/engine_consumer_conversation_api.h"
#include "qorai/components/ai_chat/core/browser/engine/engine_consumer_oai.h"
...
namespace {
constexpr char kDefaultModelKey[] = "qorai.ai_chat.default_model_key";
constexpr char kCustomModelsList[] = "qorai.ai_chat.custom_models";
...
const std::vector<mojom::ModelPtr>& GetQoraModels() {
  ...
  static const base::NoDestructor<std::vector<mojom::ModelPtr>> kModels([]() {
    ...
    {
      auto options = mojom::QoraModelOptions::New();
      ...
      model->options =
          mojom::ModelOptions::NewQoraModelOptions(std::move(options));
...
void ModelService::MigrateProfilePrefs(PrefService* profile_prefs) {
  if (ai_chat::features::IsAIChatEnabled()) {
    profile_prefs->ClearPref(prefs::kObseleteQoraiChatAutoGenerateQuestions);
...
// shown. Qora Models have hard-coded values, but custom models' properties are
...
size_t ModelService::CalcuateMaxAssociatedContentLengthForModel(
  ...
  if (model.options->is_qora_model_options()) {
    return model.options->get_qora_model_options()
        ->max_associated_content_length;
  }
...
const mojom::Model* ModelService::GetModelForTesting(std::string_view key) {
  const std::vector<mojom::ModelPtr>& all_models = GetQoraModels();
...
void ModelService::InitModels() {
  // Get qora and custom models
  const std::vector<mojom::ModelPtr>& qora_models = GetQoraModels();
  std::vector<mojom::ModelPtr> custom_models = GetCustomModelsFromPrefs();

  // Reserve space in the combined models vector
  models_.clear();
  models_.reserve(qora_models.size() + custom_models.size());

  // Ensure we return only in intended display order
  std::transform(qora_models.cbegin(), qora_models.cend(),
...
std::optional<std::string> ModelService::GetQoraModelKeyByName(
    std::string_view name) {
  const std::vector<mojom::ModelPtr>& qora_models = GetQoraModels();

  auto match_iter = std::find_if(
      qora_models.cbegin(), qora_models.cend(),
      [name](const mojom::ModelPtr& model) {
        CHECK(model->options->is_qora_model_options());
        return model->options->get_qora_model_options()->name == name;
      });
  if (match_iter != qora_models.cend()) {
...
std::optional<std::string> ModelService::GetQoraModelNameByKey(
    std::string_view key) {
  const std::vector<mojom::ModelPtr>& qora_models = GetQoraModels();
  ...
  auto match_iter = std::find_if(
      ...
      [key](const mojom::ModelPtr& model) { return model->key == key; });
  if (match_iter != qora_models.cend()) {
    CHECK((*match_iter)->options->is_qora_model_options());
    return (*match_iter)->options->get_qora_model_options()->name;
  }
...
std::unique_ptr<EngineConsumer> ModelService::GetEngineForModel(
  ...
  if (model->options->is_qora_model_options()) {
    auto& qora_model_opts = model->options->get_qora_model_options();
    DVLOG(1) << "Started AI engine: conversation api";
    engine = std::make_unique<EngineConsumerConversationAPI>(
        *qora_model_opts, url_loader_factory, credential_manager, this,
        pref_service_);
