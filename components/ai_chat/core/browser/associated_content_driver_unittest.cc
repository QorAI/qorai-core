...
TEST_F(AssociatedContentDriverUnitTest, GetStagedEntriesFromContent) {
  ...
  associated_content_driver_->SetUrl(
      GURL("https://search.qorai.com/search?q=test"));
...
TEST_F(AssociatedContentDriverUnitTest,
       GetStagedEntriesFromContent_NotQoraiSearchSERP) {
  SetSearchQuerySummaryInterceptor(true);
  // Fetch should not be called if page URL is not Qorai Search SERP, staged
  // query and summary will be cleared.
  associated_content_driver_->SetUrl(GURL("https://search.qorai.com"));
