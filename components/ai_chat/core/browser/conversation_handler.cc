...
#include "qorai/components/ai_chat/core/browser/ai_chat_feedback_api.h"
...
#include "qorai/components/ai_chat/core/browser/tools/tool.h"
...
ConversationHandler::ConversationHandler(
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/49261): This is
...
bool ConversationHandler::HasAnyHistory() {
  ...
  return std::ranges::any_of(chat_history_,
                             [](const mojom::ConversationTurnPtr& turn) {
                               return !turn->from_qorai_search_SERP;
                             });
}
...
void ConversationHandler::InitEngine() {
  ...
  if (!model) {
    ...
    SCOPED_CRASH_KEY_STRING1024("QoraiAIChatModel", "key", model_key_);
    base::debug::DumpWithoutCrashing();
    // Use default
    model = model_service_->GetModel(features::kAIModelsDefaultKey.Get());
    if (!model) {
      SCOPED_CRASH_KEY_STRING1024("QoraiAIChatModel", "key",
...
void ConversationHandler::RateMessage(bool is_liked,
  ...
  auto* model = (*entry_it)->model_key
                    ...
                    : &GetCurrentModel();
  // Only Qora models are allowed to be rated.
  if (!model || !model->options->is_qora_model_options()) {
    std::move(callback).Run(std::nullopt);
    return;
  }
  ...
  feedback_api_->SendRating(
      ...
      model->options->get_qora_model_options()->name, selected_language_,
      base::BindOnce(
...
void ConversationHandler::ChangeModel(const std::string& model_key) {
  ...
  if (new_model) {
    ...
    // another. For example, if --allow-qora-private-ips is enabled, the endpoint
...
void ConversationHandler::OnConversationEntryAdded(
  ...
  if (entry->from_qorai_search_SERP) {
    OnHistoryUpdate(nullptr);
    return;
  }
      ...
      std::ranges::all_of(chat_history_,
                          [&entry](mojom::ConversationTurnPtr& history_entry) {
                            return history_entry == entry ||
                                   history_entry->from_qorai_search_SERP;
                          })) {
...
ConversationHandler::GetStateForConversationEntries() {
  ...
  bool is_qora_model = model.options->is_qora_model_options();
  ...
  entries_state->is_qora_model = is_qora_model;
  ...
  entries_state->can_submit_user_entries =
      !IsRequestInProgress() &&
      (ai_chat_service_->IsPremiumStatus() || !is_qora_model ||
       model.options->get_qora_model_options()->access !=
