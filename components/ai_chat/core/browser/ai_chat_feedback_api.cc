...
const GURL GetEndpointBaseUrl() {
  auto domain = qorai_domains::GetServicesDomain(kFeedbackHostnamePart);
...
void AIChatFeedbackAPI::SendRating(
  ...
  payload.Set("ymd", qorai_stats::GetDateAsYMD(base::Time::Now()));
  ...
  payload.Set("platform", qorai_stats::GetPlatformIdentifier());
...
void AIChatFeedbackAPI::SendFeedback(
  ...
  dict.Set("ymd", qorai_stats::GetDateAsYMD(base::Time::Now()));
