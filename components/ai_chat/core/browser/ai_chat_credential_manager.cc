...
constexpr char kQoraSkuHostnamePart[] = "qora";
...
void AIChatCredentialManager::GetPremiumStatus(
  ...
  const auto& cached_creds_dict =
      prefs_service_->GetDict(prefs::kQoraiChatPremiumCredentialCache);
  ...
  const std::string qora_sku_domain = qorai_domains::GetServicesDomain(
      kQoraSkuHostnamePart, qorai_domains::ServicesEnvironment::STAGING);
  ...
  skus_service_->CredentialSummary(
      ...
      base::BindOnce(&AIChatCredentialManager::OnCredentialSummary,
                     ...
                     qora_sku_domain, credential_in_cache));
}
...
void AIChatCredentialManager::FetchPremiumCredential(
  ...
  ScopedDictPrefUpdate update(prefs_service_,
                              prefs::kQoraiChatPremiumCredentialCache);
...
void AIChatCredentialManager::CreateOrderFromReceipt(
  ...
  skus_service_->CreateOrderFromReceipt(qora_sku_domain, encoded_request_json,
                                        std::move(callback));
}
...
void AIChatCredentialManager::FetchOrderCredentials(
  ...
  skus_service_->FetchOrderCredentials(qora_sku_domain, order_id,
                                       std::move(callback));
}
...
void AIChatCredentialManager::RefreshOrder(
  ...
  skus_service_->RefreshOrder(qora_sku_domain, order_id, std::move(callback));
}
#endif
