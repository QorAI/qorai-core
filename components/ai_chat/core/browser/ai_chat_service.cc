...
#include "qorai/components/ai_chat/core/browser/ai_chat_database.h"
...
#include "qorai/components/ai_chat/core/browser/conversation_tools.h"
#include "qorai/components/ai_chat/core/browser/model_service.h"
#include "qorai/components/ai_chat/core/browser/tab_tracker_service.h"
#include "qorai/components/ai_chat/core/browser/tools/memory_storage_tool.h"
...
#include "qorai/components/ai_chat/core/common/mojom/ai_chat.mojom-forward.h"
...
AIChatService::AIChatService(
  ...
  pref_change_registrar_.Add(
      prefs::kQoraiChatStorageEnabled,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&AIChatService::OnStateChanged,
                          weak_ptr_factory_.GetWeakPtr()));
  pref_change_registrar_.Add(
      prefs::kQoraiAIChatUserMemoryEnabled,
      base::BindRepeating(&AIChatService::OnMemoryEnabledChanged,
                          weak_ptr_factory_.GetWeakPtr()));
  pref_change_registrar_.Add(
      prefs::kQoraiAIChatSkills,
...
void AIChatService::OnMemoryEnabledChanged() {
  bool memory_enabled =
      profile_prefs_->GetBoolean(prefs::kQoraiAIChatUserMemoryEnabled);
...
void AIChatService::InitializeTools() {
  ...
  if (profile_prefs_->GetBoolean(prefs::kQoraiAIChatUserMemoryEnabled)) {
    memory_tool_ = std::make_unique<MemoryStorageTool>(profile_prefs_);
  }
}
...
void AIChatService::OnOsCryptAsyncReady(os_crypt_async::Encryptor encryptor) {
  ...
  if (!profile_prefs_->GetBoolean(prefs::kQoraiChatStorageEnabled)) {
    return;
  }
...
void AIChatService::EnableStoragePref() {
  profile_prefs_->SetBoolean(prefs::kQoraiChatStorageEnabled, true);
}
...
void AIChatService::OnPremiumStatusReceived(GetPremiumStatusCallback callback,
  ...
  if (status == mojom::PremiumStatus::ActiveDisconnected &&
      profile_prefs_->GetBoolean(prefs::kQoraiChatSubscriptionActiveAndroid)) {
...
mojom::ServiceStatePtr AIChatService::BuildState() {
  ...
  bool is_storage_enabled =
      profile_prefs_->GetBoolean(prefs::kQoraiChatStorageEnabled);
...
void AIChatService::OpenConversationWithStagedEntries(
  ...
  if (ai_chat_metrics_ != nullptr) {
    ai_chat_metrics_->HandleOpenViaEntryPoint(EntryPoint::kQoraiSearch);
  }
#endif
  // Open AI Chat and trigger a fetch of staged conversations from Qorai Search.
