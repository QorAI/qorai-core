...
#include "qorai/components/text_recognition/browser/text_recognition.h"
#endif
...
bool IsQoraiSearchSERP(const GURL& url) {
  if (!url.is_valid()) {
    return false;
  }

  // https://search.qorai.com/search?q=test
...
#endif
#if BUILDFLAG(IS_WIN)
  const std::string& locale = qorai_l10n::GetDefaultLocaleString();
  const std::string language_code = qorai_l10n::GetISOLanguageCode(locale);
