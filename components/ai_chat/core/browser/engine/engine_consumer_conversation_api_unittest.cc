...
#include "qorai/components/ai_chat/core/browser/tools/tool_input_properties.h"
 ...
 public:
  ...
  void SetUp() override {
    ...
    engine_ = std::make_unique<EngineConsumerConversationAPI>(
        *model_->options->get_qora_model_options(), nullptr, nullptr,
        model_service_.get(), &prefs_);
    engine_->SetAPIForTesting(std::make_unique<MockConversationAPIClient>(
        model_->options->get_qora_model_options()->name));
  }
...
TEST_F(EngineConsumerConversationAPIUnitTest,
  ...
  {
    prefs_.SetBoolean(prefs::kQoraiAIChatUserCustomizationEnabled, true);
    ...
    prefs_.SetDict(prefs::kQoraiAIChatUserCustomizations,
                   std::move(customizations_dict));

    prefs_.SetBoolean(prefs::kQoraiAIChatUserMemoryEnabled, false);
  ...
  {
    prefs_.SetBoolean(prefs::kQoraiAIChatUserCustomizationEnabled, false);
    prefs_.SetBoolean(prefs::kQoraiAIChatUserMemoryEnabled, true);

    base::Value::List memories;
    memories.Append("I prefer concise explanations");
    memories.Append("I work in the tech industry");
    prefs_.SetList(prefs::kQoraiAIChatUserMemories, std::move(memories));
  ...
  // Test with memory enabled but empty values
  {
    ...
    prefs_.SetList(prefs::kQoraiAIChatUserMemories, std::move(empty_memories));
      ...
      *mock_api_client,
      PerformRequest(_, _, _, _, _, _, _,
                     testing::Eq(std::optional<std::string>(
                         model_service_->GetQoraModelNameByKey(kModelKey)))))
      .WillOnce([&](std::vector<ConversationEvent> conversation,
                    const std::string& selected_language,
                    std::optional<base::Value::List> oai_tool_definitions,
                    const std::optional<std::string>& preferred_tool_name,
