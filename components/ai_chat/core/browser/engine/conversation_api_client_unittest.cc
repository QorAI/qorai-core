...
#include "qorai/components/api_request_helper/mock_api_request_helper.h"
...
TEST_F(ConversationAPIUnitTest, PerformRequest_PremiumHeaders) {
  ...
  const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale(
  ...
  EXPECT_CALL(*mock_request_helper, RequestSSE(_, _, _, _, _, _, _, _))
      .WillOnce([&](const std::string& method, const GURL& url,
        ...
        EXPECT_EQ(cookie_header->second,
                  "__Secure-sku#qorai-qora-premium=" + expected_crediential);
        EXPECT_NE(headers.find("x-qorai-key"), headers.end());
        ...
        }
        {
          ...
          }
          {
            ...
            query.GetDict().Set("url", "https://starwars.com");
            query.GetDict().Set(
                "favicon", "http://imgs.search.qorai.com/starwars.com/favicon");
            sources.GetList().Append(std::move(query));
          }
          {
            // Valid
            ...
            query.GetDict().Set(
                "favicon",
                "https://imgs.search.qorai.com/starwars.com/favicon");
            sources.GetList().Append(std::move(query));
          }
          {
            // Valid
            ...
            query.GetDict().Set(
                "favicon",
                "https://imgs.search.qorai.com/startrek.com/favicon");
            sources.GetList().Append(std::move(query));
          }
...
TEST_F(ConversationAPIUnitTest, ParseResponseEvent_ParsesWebSourcesEvent) {
  // Case 1: Valid favicon from allowed qorai host
  ...
  source1.Set("favicon", "https://imgs.search.qorai.com/favicon.ico");
  ...
  EXPECT_EQ(result1->event->get_sources_event()->sources[0]->favicon_url.spec(),
            "https://imgs.search.qorai.com/favicon.ico");
  ...
  EXPECT_EQ(result2->event->get_sources_event()->sources[0]->favicon_url.spec(),
            "chrome-untrusted://resources/qorai-icons/globe.svg");
