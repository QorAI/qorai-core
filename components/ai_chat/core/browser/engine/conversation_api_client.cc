...
#include "qorai/components/ai_chat/core/browser/engine/conversation_api_parsing.h"
#include "qorai/components/ai_chat/core/browser/engine/oai_parsing.h"
...
constexpr char kAllowedWebSourceFaviconHost[] = "imgs.search.qorai.com";
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ai_chat", R"(
      semantics {
        sender: "AI Chat"
        description:
          "This is used to communicate with Qorai's AI Conversation API"
...
GURL GetEndpointUrl(bool premium, const std::string& path) {
  ...
  auto hostname = qorai_domains::GetServicesDomain(
      prefix, qorai_domains::ServicesEnvironment::DEV);
...
std::string ConversationAPIClient::CreateJSONRequestBody(
  ...
  dict.Set("system_language",
           base::StrCat({qorai_l10n::GetDefaultISOLanguageCodeString(), "_",
                         qorai_l10n::GetDefaultISOCountryCodeString()}));
...
void ConversationAPIClient::PerformRequestWithCredentials(
  ...
  const auto digest_header = qorai_service_keys::GetDigestHeader(request_body);
  headers.emplace(digest_header.first, digest_header.second);
  auto result = qorai_service_keys::GetAuthorizationHeader(
  ...
  if (premium_enabled) {
    // Add Qora premium SKU credential as a Cookie header.
    std::string cookie_header_value =
        "__Secure-sku#qorai-qora-premium=" + credential->credential;
    headers.emplace("Cookie", cookie_header_value);
  }
  headers.emplace("x-qorai-key", BUILDFLAG(QORAI_SERVICES_KEY));
...
void ConversationAPIClient::OnQueryCompleted(
  ...
  if (success) {
    ...
    if (result.value_body().is_dict()) {
      ...
      if (model_value) {
        model_key = model_service_->GetQoraModelKeyByName(*model_value);
      }
    }
...
ConversationAPIClient::ParseResponseEvent(base::Value::Dict& response_event,
  ...
  if (*type == "completion") {
    ...
    for (auto& item : *sources) {
      ...
      GURL item_favicon_url =
          favicon_url
              ? GURL(*favicon_url)
              : GURL("chrome-untrusted://resources/qorai-icons/globe.svg");
  ...
  return GenerationResultData(std::move(event),
                              model_service->GetQoraModelKeyByName(*model));
}
