...
import { isQoraModel } from '../model_utils'
...
export function ConversationContextProvider(props: React.PropsWithChildren) {
  ...
  const shouldShowLongConversationInfo = React.useMemo(() => {
    ...
    const options =
      context.currentModel?.options.qoraModelOptions
  ...
  const shouldDisableUserInput = !!(
    ...
    || (!aiChatContext.isPremiumUser
      && context.currentModel?.options.qoraModelOptions?.access
  ...
  const isCurrentModelQora =
    context.currentModel !== undefined && isQoraModel(context.currentModel)
  ...
  const switchToBasicModel = () => {
    ...
    const nonPremium = context.allModels.find(
      (m) => m.options.qoraModelOptions?.access !== Mojom.ModelAccess.PREMIUM,
    )
    if (!nonPremium) {
