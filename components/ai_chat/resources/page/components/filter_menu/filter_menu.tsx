...
export default function FilterMenu<T>(props: Props<T>) {
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        ...
        const menuItem =
          focused ?? ref.current?.querySelector<HTMLElement>('qora-menu-item')
        if (!menuItem) return
      ...
      if (direction !== 0 && !focused) {
        ...
        const items: HTMLElement[] = Array.from(
          ref.current?.querySelectorAll('qora-menu-item') ?? [],
        )
  ...
  return (
    <ButtonMenu
      ...
      // https://github.com/qorai/qorai-browser/issues/48262#issuecomment-3166296480
      // This should be removed once the underlying issue in Nala is fixed.
      // https://github.com/qorai/qora/issues/1170
