...
import {
  AssociatedContent,
  ContentType,
} from 'gen/qorai/components/ai_chat/core/common/mojom/common.mojom.m.js'
import {
  Bookmark,
  HistoryEntry,
...
const mockTabs = [
  ...
  {
    contentId: 2,
    title: 'GitHub - Qorai Browser',
    url: { url: 'https://github.com/qorai/qorai-browser' },
    id: 2,
  },
  {
    contentId: 3,
...
describe('Attachments Component', () => {
  ...
  it('renders the close button', () => {
    ...
    const closeButton = container.querySelector('qora-button')
  ...
  it('displays all unassociated tabs', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('GitHub - Qorai Browser')).toBeInTheDocument()
  ...
  it('displays tab URLs', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('https://github.com/qorai/qorai-browser'),
      ).toBeInTheDocument()
  ...
  it('shows checkboxes in unchecked state for unassociated tabs', async () => {
    ...
    await waitFor(() => {
      const checkboxes = container.querySelectorAll('qora-checkbox')
  ...
  it('properly renders associated content info', async () => {
    ...
    await waitFor(() => {
      ...
      const firstCheckbox = container.querySelectorAll('qora-checkbox')[0]
      expect(firstCheckbox).toContainHTML('Google Search')
  ...
  describe('Search functionality', () => {
    it('renders search input', () => {
      ...
      const searchInput = container.querySelector('qora-input')
    ...
    it('shows search icon in input', () => {
      ...
      const searchIcon = container.querySelector(
        'qora-input qora-icon[slot="left-icon"]',
      )
  ...
  describe('Empty states', () => {
    ...
    it('renders properly with empty tabs list', () => {
      ...
      expect(container.querySelector('qora-input')).toBeInTheDocument()

      // But no tab items should be present
      expect(container.querySelectorAll('qora-checkbox')).toHaveLength(0)
    })
  })
  ...
  describe('Bookmarks display', () => {
    ...
    it('displays bookmarks when attachmentsDialog is set to bookmarks', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Qorai Browser')).toBeInTheDocument()
        expect(screen.getByText('MDN Web Docs')).toBeInTheDocument()
        expect(screen.getByText('https://qorai.com')).toBeInTheDocument()
        expect(
    ...
    it('displays favicons for bookmarks', async () => {
      ...
      await waitFor(() => {
        ...
        expect(favicons[0]).toHaveAttribute(
          'src',
          expect.stringContaining('qorai.com'),
        )
        expect(favicons[1]).toHaveAttribute(
          'src',
  ...
  describe('History display', () => {
    const mockHistory: HistoryEntry[] = [
      {
        id: BigInt(1),
        title: 'Qorai Privacy Browser',
        url: { url: 'https://qorai.com/privacy' },
      },
      {
        id: BigInt(2),
    ...
    it('displays history when attachmentsDialog is set to history', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Qorai Privacy Browser')).toBeInTheDocument()
        expect(screen.getByText('Web3 Standards')).toBeInTheDocument()
        expect(
          screen.getByText('https://qorai.com/privacy'),
        ).toBeInTheDocument()
        expect(
