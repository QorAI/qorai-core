...
function SearchSummary(props: { searchQueries: string[] }) {
  ...
  const handleLearnMore = () => {
    context.uiHandler?.openLearnMoreAboutQoraiSearchWithQora()
  }
  ...
  return (
    <div className={styles.searchSummary}>
      <Icon name='qorai-icon-search-color' />
      <span>
        {message}{' '}
        <a
...
function AssistantEvent(
  ...
  const { allowedLinks, event, isEntryInProgress, isQoraModel } = props
  ...
  if (event.completionEvent) {
    ...
    return (
      <MarkdownRenderer
        ...
        disableLinkRestrictions={!isQoraModel}
      />
    )
  }
  if (
  ...
  //   return (<>
  //     {event.searchQueriesEvent.searchQueries.map(query => <div className={styles.searchQuery}>Searching for <span className={styles.searchLink}><Icon name="qorai-icon-search-color" /><Link href='#'>{query}</Link></span></div>)}
  //   </>)
  // }
