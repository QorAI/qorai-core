...
import { QORA_DISTILLATION_LEVEL } from '../distillation'
...
export function getUserSignature(user: any) {
  ...
  if (
    ...
    || getDistillationLevel() === QORA_DISTILLATION_LEVEL.LOW
  ) {
...
export function distillSeenUsers(level = QORA_DISTILLATION_LEVEL.MEDIUM) {
  switch (level) {
    case QORA_DISTILLATION_LEVEL.LOW:
      return distillLowSeenUsers()
    case QORA_DISTILLATION_LEVEL.MEDIUM:
    case QORA_DISTILLATION_LEVEL.HIGH:
    case QORA_DISTILLATION_LEVEL.FULL:
