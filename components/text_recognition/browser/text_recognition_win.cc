...
#include "qorai/components/text_recognition/browser/text_recognizer_win.h"
...
std::vector<std::string> GetAvailableRecognizerLanguages() {
  ...
  for (uint32_t i = 0; i < lang_count; ++i) {
    ...
    const auto code =
        qorai_l10n::GetISOLanguageCode(ScopedHString(text).GetAsUTF8());
  ...
  if (GetUserPreferredUILanguages(MUI_LANGUAGE_NAME, &langs,
    ...
    std::string default_language_code =
        qorai_l10n::GetISOLanguageCode(base::WideToUTF8(name));
