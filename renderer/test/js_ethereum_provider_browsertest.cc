      ...
      try {
        let promise = new Promise((resolve) => {
          const listener = (event) => {
            ...
            let is_qorai_wallet = event.detail.info.name === "Qorai Wallet" &&
                                  event.detail.provider.isQoraiWallet === true;
            resolve(is_qorai_wallet);
          }
...
std::string NonWriteableScriptProperty(const std::string& property) {
  return absl::StrFormat(
      R"(window.ethereum.%s = "qorai";
         !(window.ethereum.%s === "qorai");)",
      property, property);
}
...
// TODO(darkdh): Move this browser test to //qorai/browser/qorai_wallet/ because
// it has layer violation (//chrome/browser,
// //qorai/components/qorai_wallet/browser and //qorai/browser)
class JSEthereumProviderBrowserTest : public InProcessBrowserTest {
 public:
  ...
  qorai_wallet::JsonRpcService* GetJsonRpcService() {
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, AttachOnReload) {
  constexpr auto* kHistogramName = "Qorai.Wallet.EthProvider.4";
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  {
    std::string command = "window.qoraiEthereum.isMetaMask";
    EXPECT_THAT(content::EvalJs(primary_main_frame(), command,
                                content::EXECUTE_SCRIPT_DEFAULT_OPTIONS,
                                ISOLATED_WORLD_ID_TRANSLATE),
                content::EvalJsResult::ErrorIs(
  ...
  histogram_tester_->ExpectTotalCount("Qorai.Wallet.EthProvider.3", 0);
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  {
    std::string command = "window.ethereum.isQoraiWallet";
    EXPECT_THAT(content::EvalJs(primary_main_frame(), command),
                content::EvalJsResult::ErrorIs(
                    testing::HasSubstr("Cannot read properties of undefined")));
  }
  ...
  {
    std::string command = "window.qoraiEthereum.isQoraiWallet";
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, AttachIfWalletCreated) {
  ...
  {
    constexpr char kEvalIsQoraiWallet[] = "window.ethereum.isQoraiWallet";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), kEvalIsQoraiWallet)
                    .ExtractBool());
  }

  {
    constexpr char kEvalIsQoraiWallet[] = "window.qoraiEthereum.isQoraiWallet";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), kEvalIsQoraiWallet)
                    .ExtractBool());
  }
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  {
    constexpr char kEvalIsQoraiWallet[] = "window.ethereum.isQoraiWallet";
    EXPECT_THAT(content::EvalJs(primary_main_frame(), kEvalIsQoraiWallet),
                content::EvalJsResult::ErrorIs(
                    testing::HasSubstr("Cannot read properties of undefined")));
  }
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  scoped_refptr<const extensions::Extension> extension(
      extensions::ExtensionBuilder("MetaMask")
          .SetID(qorai_wallet::kMetamaskExtensionId)
          .Build());
  ...
  constexpr char kEvalIsQoraiWallet[] = "window.qoraiEthereum.isQoraiWallet";
  EXPECT_TRUE(
      content::EvalJs(primary_main_frame(), kEvalIsQoraiWallet).ExtractBool());
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  // Check whether window.qoraiEthereum is installed
  {
    std::string command = "window.qoraiEthereum.isQoraiWallet";
    EXPECT_EQ(base::Value(true),
              content::EvalJs(primary_main_frame(), command));
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, NonWritable) {
  ...
  for (const std::string& property : {"isQoraiWallet", "_metamask", "chainId",
  ...
  // https://github.com/qorai/qorai-browser/issues/25078
  for (const std::string& method :
...

    {
      auto result = EvalJs(web_contents(),
                           NonWriteableScriptMethod("qoraiEthereum", method));
      EXPECT_EQ(base::Value(true), result) << result;
    }
  }
  {
  ...
  {
    auto result = EvalJs(web_contents(),
                         NonWriteableScriptMethod("qoraiEthereum", "send"));
    EXPECT_EQ(base::Value(false), result) << result;
  }
...
// See https://github.com/qorai/qorai-browser/issues/22213 for details
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
                       QoraiEthereum_NonConfigurable) {
  qorai_wallet::mojom::DefaultWallet non_configurable_states[] = {
      qorai_wallet::mojom::DefaultWallet::QoraiWallet,
      qorai_wallet::mojom::DefaultWallet::QoraiWalletPreferExtension};
  for (const auto& default_wallet : non_configurable_states) {
    qorai_wallet::SetDefaultEthereumWallet(browser()->profile()->GetPrefs(),
                                           default_wallet);
    const GURL url = https_server_.GetURL("/simple.html");
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
    std::string overwrite =
        R"(try {
           Object.defineProperty(window, 'qoraiEthereum', {
             writable: true,
           });
         } catch (e) {}
         window.qoraiEthereum = 42;
         typeof window.qoraiEthereum === 'object'
      )";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), overwrite).ExtractBool());
  }
}
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, OnlyWriteOwnProperty) {
  ...
  GetJsonRpcService()->SetNetwork(
      "0xaa36a7", qorai_wallet::mojom::CoinType::ETH, std::nullopt);
  ...
  GetJsonRpcService()->SetNetwork("0x4", qorai_wallet::mojom::CoinType::ETH,
                                  std::nullopt);
  // Needed so ChainChangedEvent observers run
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(content::EvalJs(primary_main_frame(), get_chain_id).ExtractString(),
