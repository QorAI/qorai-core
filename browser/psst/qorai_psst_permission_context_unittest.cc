...
namespace psst {
class QoraiPsstPermissionContextUnitTest : public testing::Test {
 public:
  void SetUp() override {
    psst_permission_context_ =
        QoraiPsstPermissionContextFactory::GetForProfile(&profile_);
    ASSERT_TRUE(psst_permission_context_);
  }

  void TearDown() override {
    psst_permission_context_ = nullptr;
    QoraiPsstPermissionContextFactory::GetInstance()->SetTestingFactory(
        &profile_, {});
  }

  TestingProfile* profile() { return &profile_; }

  QoraiPsstPermissionContext* psst_permission_context() {
 ...
 private:
  ...
  raw_ptr<QoraiPsstPermissionContext> psst_permission_context_;
};

TEST_F(QoraiPsstPermissionContextUnitTest,
  ...
  const url::Origin qorai_scheme_origin =
      url::Origin::Create(GURL("qorai://a.test"));
  ASSERT_TRUE(psst_permission_context()
                  ->GetGrantedObjects(qorai_scheme_origin)
                  .empty());
  psst_permission_context()->GrantPermission(qorai_scheme_origin,
                                             first_permission_info->Clone());
  ASSERT_TRUE(psst_permission_context()
                  ->GetGrantedObjects(qorai_scheme_origin)
                  .empty());
...
TEST_F(QoraiPsstPermissionContextUnitTest, CreateUpdateRevokePermissionInfo) {
