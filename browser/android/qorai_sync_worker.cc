...
#include "qorai/browser/android/qorai_sync_worker.h"
...
#include "chrome/android/chrome_jni_headers/QoraiSyncWorker_jni.h"
...
QoraiSyncWorker::QoraiSyncWorker(JNIEnv* env,
                                 const base::android::JavaRef<jobject>& obj)
    : weak_java_qorai_sync_worker_(env, obj) {
  Java_QoraiSyncWorker_setNativePtr(env, obj, reinterpret_cast<intptr_t>(this));
...
void QoraiSyncWorker::Destroy(JNIEnv* env) {
  delete this;
}

base::android::ScopedJavaLocalRef<jstring> QoraiSyncWorker::GetSyncCodeWords(
    JNIEnv* env) {
...
void QoraiSyncWorker::SaveCodeWords(
    JNIEnv* env,
...
void QoraiSyncWorker::RequestSync(JNIEnv* env) {
...
void QoraiSyncWorker::FinalizeSyncSetup(JNIEnv* env) {
  MarkFirstSetupComplete();
}

bool QoraiSyncWorker::IsInitialSyncFeatureSetupComplete(JNIEnv* env) {
  syncer::SyncService* sync_service = GetSyncService();
  return sync_service &&
         sync_service->GetUserSettings()->IsInitialSyncFeatureSetupComplete();
}

void QoraiSyncWorker::ResetSync(JNIEnv* env) {
  auto* sync_service = GetSyncService();
...
void QoraiSyncWorker::SetDecryptionPassphrase(syncer::SyncService* service) {
  ...
  VLOG_IF(3, !set_decryption_result)
      << "[QoraiSync] " << __func__ << " SetDecryptionPassphrase failed";
}
...
void NativePermanentlyDeleteAccountCallback(
  ...
  Java_QoraiSyncWorker_onPermanentlyDeleteAccountResult(
...
void NativeJoinSyncChainCallback(
    ...
    const base::android::ScopedJavaGlobalRef<jobject>& callback,
    bool result) {
  Java_QoraiSyncWorker_onJoinSyncChainResult(env, callback,
...
void QoraiSyncWorker::ClearAccountDeletedNoticePending(JNIEnv* env) {
  Profile* profile =
...
bool QoraiSyncWorker::IsAccountDeletedNoticePending(JNIEnv* env) {
  Profile* profile =
      ProfileManager::GetActiveUserProfile()->GetOriginalProfile();
  qorai_sync::Prefs qorai_sync_prefs(profile->GetPrefs());
...
static void JNI_QoraiSyncWorker_Init(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  new QoraiSyncWorker(env, jcaller);
}

static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetSeedHexFromWords(
    JNIEnv* env,
  ...
  if (qorai_sync::crypto::PassphraseToBytes32(str_seed_words, &bytes)) {
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetWordsFromSeedHex(
    JNIEnv* env,
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetQrDataJson(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& seed_hex) {
  std::string str_seed_hex = base::android::ConvertJavaStringToUTF8(seed_hex);
  DCHECK(!str_seed_hex.empty());
  ...
  const std::string qr_code_string =
      qorai_sync::QrCodeData::CreateWithActualDate(str_seed_hex)->ToJson();

  return ConvertUTF8ToJavaString(env, qr_code_string);
}

int JNI_QoraiSyncWorker_GetQrCodeValidationResult(
    JNIEnv* env,
  ...
  return static_cast<int>(
      qorai_sync::QrCodeDataValidator::ValidateQrDataJson(str_json_qr));
}

int JNI_QoraiSyncWorker_GetWordsValidationResult(
    JNIEnv* env,
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetPureWordsFromTimeLimited(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& time_limited_words) {
  std::string str_time_limited_words =
      base::android::ConvertJavaStringToUTF8(time_limited_words);
...
static int64_t JNI_QoraiSyncWorker_GetNotAfterFromFromTimeLimitedWords(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& time_limited_words) {
  std::string str_time_limited_words =
      base::android::ConvertJavaStringToUTF8(time_limited_words);
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetFormattedTimeDelta(JNIEnv* env, jlong seconds) {
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetTimeLimitedWordsFromPure(
    JNIEnv* env,
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_QoraiSyncWorker_GetSeedHexFromQrJson(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& json_qr) {
  std::string str_json_qr = base::android::ConvertJavaStringToUTF8(json_qr);
  DCHECK(!str_json_qr.empty());

  auto qr_data = qorai_sync::QrCodeData::FromJson(str_json_qr);
...
static int JNI_QoraiSyncWorker_GetWordsCount(
    JNIEnv* env,
