...
#include "qorai/browser/notifications/qorai_notification_platform_bridge_helper_android.h"
...
namespace {
constexpr char kNotificationAdUrlPrefix[] = "https://www.qorai.com/ads/?";
}  // namespace
...
void AdsServiceDelegate::ShowNotificationAd(const std::string& id,
  ...
  if (is_custom) {
    ...
    GetNotificationDisplayService()->Display(
        NotificationHandler::Type::QORAI_ADS, *notification, nullptr);
  }
}
...
void AdsServiceDelegate::CloseNotificationAd(const std::string& id,
  ...
  if (is_custom) {
...
#if BUILDFLAG(IS_ANDROID)
    const std::string qorai_ads_url_prefix = kNotificationAdUrlPrefix;
    const GURL url =
        GURL(qorai_ads_url_prefix.substr(0, qorai_ads_url_prefix.size() - 1));
    QorAINotificationPlatformBridgeHelperAndroid::MaybeRegenerateNotification(
        id, url);
#endif
    ...
    // browser tests changes NotificationDisplayService instance during test run
    // which leads to dangling pointer errors.
    GetNotificationDisplayService()->Close(NotificationHandler::Type::QORAI_ADS,
                                           id);
  }
}
