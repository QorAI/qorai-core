...
#include "qorai/browser/qorai_ads/application_state/notification_helper/notification_helper_impl_android.h"

#include "base/android/jni_string.h"
#include "base/system/sys_info.h"
#include "qorai/browser/qorai_ads/android/jni_headers/QoraiAdsSignupDialog_jni.h"
#include "qorai/browser/qorai_ads/android/jni_headers/QoraiAds_jni.h"
#include "qorai/build/android/jni_headers/QoraiSiteChannelsManagerBridge_jni.h"
...
bool IsQoraiAdsNotificationChannelEnabled(bool is_foreground) {
  ...
  const auto j_channel_id =
      (is_foreground) ? Java_QoraiAds_getQoraiAdsChannelId(env)
                      : Java_QoraiAds_getQoraiAdsBackgroundChannelId(env);

  const auto status = static_cast<NotificationChannelStatus>(
      Java_QoraiSiteChannelsManagerBridge_getChannelStatus(env, j_channel_id));
...
bool NotificationHelperImplAndroid::CanShowNotifications() {
  ...
  const bool is_notification_channel_enabled =
      IsQoraiAdsNotificationChannelEnabled(is_foreground);
...
bool NotificationHelperImplAndroid::
  ...
  JNIEnv* env = jni_zero::AttachCurrentThread();
  return Java_QoraiAdsSignupDialog_showAdsInBackground(env);
}
...
bool NotificationHelperImplAndroid::ShowOnboardingNotification() {
  ...
  JNIEnv* env = jni_zero::AttachCurrentThread();
  Java_QoraiAdsSignupDialog_enqueueOnboardingNotificationNative(
