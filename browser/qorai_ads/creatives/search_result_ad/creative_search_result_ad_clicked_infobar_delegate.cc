...
#include "qorai/browser/qorai_ads/creatives/search_result_ad/creative_search_result_ad_clicked_infobar_delegate.h"
...
constexpr char kLearnMoreUrl[] =
    "https://search.qorai.com/help/conversion-reporting";
...
#else   // BUILDFLAG(IS_ANDROID)
  return CreateQoraiConfirmInfoBar(
      std::make_unique<CreativeSearchResultAdClickedInfoBarDelegate>());
#endif  // BUILDFLAG(IS_ANDROID)
}
...
std::u16string CreativeSearchResultAdClickedInfoBarDelegate::GetMessageText()
  ...
  return l10n_util::GetStringUTF16(
      IDS_QORAI_ADS_SEARCH_RESULT_AD_CLICKED_INFOBAR_MESSAGE);
}
...
std::u16string CreativeSearchResultAdClickedInfoBarDelegate::GetLinkText()
    const {
  return l10n_util::GetStringUTF16(
      IDS_QORAI_ADS_SEARCH_RESULT_AD_LEARN_MORE_OPT_OUT_CHOICES_LABEL);
}
