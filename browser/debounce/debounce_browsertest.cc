...
using qorai_shields::SetCosmeticFilteringControlType;
...
class SpyContentBrowserClient : public QoraiContentBrowserClient {
 public:
...
class DebounceBrowserTest : public BaseLocalDataFilesBrowserTest {
 public:
  ...
  LocalDataFilesObserver* service() override {
    return g_qorai_browser_process->debounce_component_installer();
  }
  ...
  void WaitForService() override {
    ...
    debounce::DebounceComponentInstaller* component_installer =
        g_qorai_browser_process->debounce_component_installer();
  ...
  void InitAdBlockForDebounce() {
    auto source_provider =
        std::make_unique<qorai_shields::TestFiltersProvider>("||blocked.com^");
    g_qorai_browser_process->ad_block_service()->UseSourceProviderForTest(
        source_provider.get());
    source_providers_.push_back(std::move(source_provider));
    auto* engine =
        g_qorai_browser_process->ad_block_service()->default_engine_.get();
 ...
 private:
  ...
  std::vector<std::unique_ptr<qorai_shields::TestFiltersProvider>>
      source_providers_;
};
...
IN_PROC_BROWSER_TEST_F(DebounceBrowserTest, IgnoreHostnameMismatch) {
  ...
  // If you paste that in Chrome or Qorai, the backslashes are changed
