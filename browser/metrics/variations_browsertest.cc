...
// The seed is signed with the private key of the Qorai variations server.
SignedSeedData GetQoraiSignedSeedData() {
...
IN_PROC_BROWSER_TEST_F(VariationsBrowserTest, PRE_QoraiSeedApplied) {
  PrefService* local_state = g_browser_process->local_state();
  WriteSeedData(local_state, GetQoraiSignedSeedData(), kRegularSeedPrefKeys);
...
#define MAYBE_QoraiSeedApplied DISABLED_QoraiSeedApplied
#else
#define MAYBE_QoraiSeedApplied QoraiSeedApplied
#endif
IN_PROC_BROWSER_TEST_F(VariationsBrowserTest, MAYBE_QoraiSeedApplied) {
  histogram_tester_.ExpectUniqueSample("Variations.SeedUsage",
                                       SeedUsage::kRegularSeedUsed, 1);

  EXPECT_TRUE(FieldTrialListHasAllStudiesFrom(GetQoraiSignedSeedData()));

  EXPECT_TRUE(base::FeatureList::IsEnabled(kVariationsTestFeature));
  EXPECT_EQ(variations::GetSeedVersion(), "Qorai variations test seed");
}
