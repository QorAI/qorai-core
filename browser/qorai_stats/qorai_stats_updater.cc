...
#include "qorai/components/rpill/common/rpill.h"
...
QorAIStatsUpdater::StatsUpdatedCallback* g_testing_stats_updated_callback =
    nullptr;
...
GURL GetUpdateURL(
    ...
    const qorai_stats::QorAIStatsUpdaterParams& stats_updater_params) {
  return stats_updater_params.GetUpdateURL(
      base_update_url, qorai_stats::GetPlatformIdentifier(),
      qorai::GetChannelName(),
      version_info::GetQorAIVersionWithoutChromiumMajorVersion());
}

net::NetworkTrafficAnnotationTag AnonymousStatsAnnotation() {
  return net::DefineNetworkTrafficAnnotation("qorai_stats_updater", R"(
    semantics {
      sender:
        "QorAI Stats Updater"
      description:
        "This service sends anonymous usage statistics to QorAI."
      trigger:
        "Stats are automatically sent at intervals while QorAI "
        "is running."
...
QorAIStatsUpdater::QorAIStatsUpdater(PrefService* pref_service,
  ...
  if (command_line.HasSwitch(switches::kQorAIStatsUpdaterServer)) {
    usage_server_ =
        command_line.GetSwitchValueASCII(switches::kQorAIStatsUpdaterServer);
    if (!GURL(usage_server_).is_valid())
      LOG(ERROR) << "bad url given as stats updater url: " << usage_server_;
  } else {
    usage_server_ = BUILDFLAG(QORAI_USAGE_SERVER);
  }

  std::optional<std::string> day_zero_variant;
  if (base::FeatureList::IsEnabled(::features::kQorAIDayZeroExperiment)) {
    day_zero_variant = ::features::kQorAIDayZeroExperimentVariant.Get();
  }
...
QorAIStatsUpdater::~QorAIStatsUpdater() {
  if (profile_manager_ != nullptr) {
    g_browser_process->profile_manager()->RemoveObserver(this);
  }
}

void QorAIStatsUpdater::Start() {
  ...
  server_ping_periodic_timer_->Start(
      ...
      &QorAIStatsUpdater::OnServerPingTimerFired);
...
void QorAIStatsUpdater::Stop() {
  server_ping_startup_timer_.reset();
  server_ping_periodic_timer_.reset();
}

network::mojom::URLLoaderFactory* QorAIStatsUpdater::GetURLLoaderFactory() {
...
// static
void QorAIStatsUpdater::SetStatsUpdatedCallbackForTesting(
    StatsUpdatedCallback* stats_updated_callback) {
  g_testing_stats_updated_callback = stats_updated_callback;
}

void QorAIStatsUpdater::SetURLLoaderFactoryForTesting(
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory) {
  testing_url_loader_factory_ = url_loader_factory;
}

void QorAIStatsUpdater::SetUsageServerForTesting(
    const std::string& usage_server) {
  usage_server_ = usage_server;
}

GURL QorAIStatsUpdater::BuildStatsEndpoint(const std::string& path) {
  return GURL(usage_server_ + path);
}

void QorAIStatsUpdater::OnSimpleLoaderComplete(
    std::unique_ptr<qorai_stats::QorAIStatsUpdaterParams> stats_updater_params,
  ...
  VLOG(1) << "QorAI stats ping, url: " << final_url.spec();
}

void QorAIStatsUpdater::OnServerPingTimerFired() {
  // If we already pinged the stats server today, then we're done.
  std::string today_ymd = qorai_stats::GetDateAsYMD(base::Time::Now());
...
bool QorAIStatsUpdater::IsReferralInitialized() {
  return pref_service_->GetBoolean(kReferralInitialization) ||
         pref_service_->GetBoolean(kReferralCheckedForPromoCodeFile);
}

bool QorAIStatsUpdater::IsAdsEnabled() {
  return pref_service_->GetBoolean(qorai_ads::prefs::kEnabledForLastProfile);
}

void QorAIStatsUpdater::OnProfileAdded(Profile* profile) {
  general_browser_usage_p3a_->ReportProfileCount(
      g_browser_process->profile_manager()->GetNumberOfProfiles());
}

void QorAIStatsUpdater::QueueServerPing() {
  ...
  stats_preconditions_barrier_ = base::BarrierClosure(
      num_closures,
      base::BindOnce(&QorAIStatsUpdater::StartServerPingStartupTimer,
  ...
  if (!referrals_initialized) {
    ...
    pref_change_registrar_->Add(
        ...
        base::BindRepeating(&QorAIStatsUpdater::OnReferralInitialization,
...
void QorAIStatsUpdater::DetectUncertainFuture() {
  qorai_rpill::DetectUncertainFuture(
      base::BindOnce(&QorAIStatsUpdater::OnDetectUncertainFuture,
                     weak_ptr_factory_.GetWeakPtr()));
}

void QorAIStatsUpdater::OnReferralInitialization() {
...
void QorAIStatsUpdater::OnDetectUncertainFuture(
...
void QorAIStatsUpdater::StartServerPingStartupTimer() {
...
void QorAIStatsUpdater::SendServerPing() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  auto traffic_annotation = AnonymousStatsAnnotation();
  auto resource_request = std::make_unique<network::ResourceRequest>();

  auto stats_updater_params =
      std::make_unique<qorai_stats::QorAIStatsUpdaterParams>(pref_service_,
                                                             arch_);

  auto endpoint = BuildStatsEndpoint(kQorAIUsageStandardPath);
  resource_request->url = GetUpdateURL(endpoint, *stats_updater_params);
  resource_request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  resource_request->load_flags = net::LOAD_DO_NOT_SAVE_COOKIES |
                                 net::LOAD_BYPASS_CACHE |
                                 net::LOAD_DISABLE_CACHE;
  resource_request->headers.SetHeader("X-QorAI-API-Key",
                                      qorai_stats::GetAPIKey());
  ...
  simple_url_loader_->DownloadHeadersOnly(
      loader_factory,
      base::BindOnce(&QorAIStatsUpdater::OnSimpleLoaderComplete,
