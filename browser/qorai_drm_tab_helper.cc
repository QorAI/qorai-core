...
#include "qorai/browser/widevine/widevine_permission_request.h"
#include "qorai/browser/widevine/widevine_utils.h"
#include "qorai/components/constants/pref_names.h"
#include "qorai/components/widevine/constants.h"
...
QorAIDrmTabHelper::QorAIDrmTabHelper(content::WebContents* contents)
    : WebContentsObserver(contents),
      content::WebContentsUserData<QorAIDrmTabHelper>(*contents),
      qorai_drm_receivers_(contents, this) {
#if !BUILDFLAG(IS_ANDROID)
...
QorAIDrmTabHelper::~QorAIDrmTabHelper() = default;

// static
void QorAIDrmTabHelper::BindQorAIDRM(
  ...
  auto* tab_helper = QorAIDrmTabHelper::FromWebContents(web_contents);
  if (!tab_helper)
    return;
  tab_helper->qorai_drm_receivers_.Bind(rfh, std::move(receiver));
}

bool QorAIDrmTabHelper::ShouldShowWidevineOptIn() const {
...
void QorAIDrmTabHelper::DidStartNavigation(
...
void QorAIDrmTabHelper::OnWidevineKeySystemAccessRequest() {
...
void QorAIDrmTabHelper::OnEvent(const update_client::CrxUpdateItem& item) {
#if !BUILDFLAG(IS_ANDROID)
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(QorAIDrmTabHelper);
