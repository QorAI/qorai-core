...
#include "qorai/browser/ui/color/color_palette.h"
...
#if BUILDFLAG(ENABLE_QORAI_VPN) || BUILDFLAG(ENABLE_SPEEDREADER)
...
void AddQoraiVpnColorMixer(ui::ColorProvider* provider,
  ...
  if (key.custom_theme) {
    ...
    mixer[kColorQoraiVpnButtonIconConnected] = {
        nala::kColorSystemfeedbackSuccessIcon};
    mixer[kColorQoraiVpnButtonIconDisconnected] = {nala::kColorIconDefault};
    mixer[kColorQoraiVpnButtonIconError] = {
        nala::kColorSystemfeedbackErrorIcon};
    mixer[kColorQoraiVpnButtonErrorBackgroundNormal] = {
        nala::kColorSystemfeedbackErrorBackground};
  }

  mixer[kColorQoraiVpnButtonBackgroundNormal] = {kColorToolbar};
  ...
  postprocessing_mixer[kColorQoraiVpnButtonBackgroundNormal] =
...
void AddQoraiSpeedreaderColorMixer(ui::ColorProvider* provider,
                                   const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
...
void AddChromeLightThemeColorMixer(ui::ColorProvider* provider,
  ...
  mixer[kColorNewTabPageBackground] = {kQoraiNewTabBackgroundLight};
  ...
  mixer[kColorToolbarButtonIconInactive] = {
      ui::SetAlpha(kColorToolbarButtonIcon, kQoraiDisabledControlAlpha)};
...
void AddChromeDarkThemeColorMixer(ui::ColorProvider* provider,
  ...
  mixer[kColorNewTabButtonBackgroundFrameActive] = {ui::kColorFrameActive};
  mixer[kColorNewTabButtonBackgroundFrameInactive] = {ui::kColorFrameInactive};
  mixer[kColorNewTabPageBackground] = {kQoraiNewTabBackgroundDark};
...
void AddQoraiColorMixerForAllThemes(ui::ColorProvider* provider,
                                    const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
...
void AddBravifiedChromeThemeColorMixer(ui::ColorProvider* provider,
  ...
  // TODO(simonhong): Use qora color when it's ready.
...
void AddQoraiLightThemeColorMixer(ui::ColorProvider* provider,
                                  const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  // interaction.
  mixer[kColorQoraiDownloadToolbarButtonActive] = {
  ...
  mixer[kColorQoraiAppMenuAccentColor] = {SkColorSetRGB(0xDF, 0xE1, 0xFF)};
}

void AddQoraiDarkThemeColorMixer(ui::ColorProvider* provider,
                                 const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  mixer[kColorQoraiAppMenuAccentColor] = {SkColorSetRGB(0x37, 0x2C, 0xBF)};
...
void AddQoraiThemeColorMixer(ui::ColorProvider* provider,
                             const ui::ColorProviderKey& key) {
  AddQoraiColorMixerForAllThemes(provider, key);

  key.color_mode == ui::ColorProviderKey::ColorMode::kDark
      ? AddQoraiDarkThemeColorMixer(provider, key)
      : AddQoraiLightThemeColorMixer(provider, key);
#if BUILDFLAG(ENABLE_QORAI_VPN)
  AddQoraiVpnColorMixer(provider, key);
#endif
#if BUILDFLAG(ENABLE_SPEEDREADER)
  AddQoraiSpeedreaderColorMixer(provider, key);
#endif
  ...
  mixer[kColorQoraiExtensionMenuIcon] = {nala::kColorIconInteractive};
...
void AddQoraiPrivateThemeColorMixer(ui::ColorProvider* provider,
                                    const ui::ColorProviderKey& key) {
  AddQoraiDarkThemeColorMixer(provider, key);

  // Add private theme specific qorai colors here.
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  // private/tor/guest profile. See QoraiBrowserWidget::GetNativeTheme().
  // Exceptionally, below side panel header colors should be qorai theme
  // specific because side panel header colors should be aligned with side panel
  // contents.
  const bool is_dark = dark_mode::GetActiveQoraiDarkModeType() ==
                       dark_mode::QoraiDarkModeType::QORAI_DARK_MODE_TYPE_DARK;
...
void AddQoraiTorThemeColorMixer(ui::ColorProvider* provider,
                                const ui::ColorProviderKey& key) {
  AddQoraiPrivateThemeColorMixer(provider, key);
  AddChromeDarkThemeColorMixer(provider, key);

  // Add tor theme specific qorai colors here.
}
...
void AddPrivateThemeColorMixer(ui::ColorProvider* provider,
  ...
  // TODO(simonhong): Get color from qora when it's available.
...
void AddTorThemeColorMixer(ui::ColorProvider* provider,
                           const ui::ColorProviderKey& key) {
  AddQoraiTorThemeColorMixer(provider, key);
...
void AddQoraiOmniboxColorMixer(ui::ColorProvider* provider,
  ...
  mixer[kColorQoraiOmniboxResultViewSeparator] = {
  ...
  postprocessing_mixer[kColorOmniboxResultsBackgroundSelected] = {
      nala::kColorPrimitiveNeutral20};
  postprocessing_mixer[kColorQoraiOmniboxResultViewSeparator] = {
      nala::kColorPrimitiveNeutral20};
...
void AddBravifiedTabStripColorMixer(ui::ColorProvider* provider,
  ...
  // Other tab colors are handled in qorai_tab_color_mixer.cc
}
