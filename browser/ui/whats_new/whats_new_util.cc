...
// Qorai's major version is second and third component like 1.51.
// Ignored fourth number as it's build number.
std::optional<double> GetQoraiMajorVersionAsDouble(
    const base::Version& version) {
  double qorai_major_version;
  CHECK(base::StringToDouble(absl::StrFormat("%d.%d", version.components()[1],
                                             version.components()[2]),
                             &qorai_major_version));
  return qorai_major_version;
}
...
std::optional<double> GetCurrentBrowserVersion() {
  ...
  return GetQoraiMajorVersionAsDouble(version);
}
...
bool DoesUserGetMajorUpdateSinceInstall() {
  ...
  const auto profile_created_version = GetQoraiMajorVersionAsDouble(
...
bool ShouldShowQoraiWhatsNewForState(PrefService* local_state) {
  ...
  const std::string default_lang_code =
      qorai_l10n::GetDefaultISOLanguageCodeString();
...
void StartQoraiWhatsNew(Browser* browser) {
  constexpr char kQoraiWhatsNewURL[] = "https://qorai.com/whats-new/";
  // Load whats-new url in the first foreground tab.
  chrome::AddTabAt(browser, GURL(kQoraiWhatsNewURL), 0, true);
