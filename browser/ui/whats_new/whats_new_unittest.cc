...
#include "qorai/browser/ui/whats_new/pref_names.h"
...
class QoraiWhatsNewTest : public testing::Test {
 public:
  QoraiWhatsNewTest()
      : testing_profile_manager_(TestingBrowserProcess::GetGlobal()) {}
  ~QoraiWhatsNewTest() override = default;
...
TEST_F(QoraiWhatsNewTest, SupportedLangTest) {
  // Prepare all other factors to show qorai whats-new except lang.
  ...
  {
    const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("it_IT");
    EXPECT_FALSE(ShouldShowQoraiWhatsNewForState(&local_state_));
  }

  // South korea is supported.
  {
    const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("ko_KR");
    EXPECT_TRUE(ShouldShowQoraiWhatsNewForState(&local_state_));
  }
}
...
// called.
TEST_F(QoraiWhatsNewTest, FieldTrialNotAvailableTest) {
  // Set supported lang.
  const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(QoraiWhatsNewTest, NotUpdatedUserTest) {
...
TEST_F(QoraiWhatsNewTest, MatchedCurrentVersionTest) {
  // Set supported lang.
  const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(QoraiWhatsNewTest, NotMatchedCurrentVersionTest) {
  // Set supported lang.
  const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(QoraiWhatsNewTest, NotWhatsNewIsAlreadyShown) {
  // Set supported lang.
  const qorai_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
