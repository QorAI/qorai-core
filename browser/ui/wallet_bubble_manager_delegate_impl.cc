...
#include "qorai/browser/ui/wallet_bubble_manager_delegate_impl.h"
...
#include "qorai/browser/ui/views/wallet_bubble_focus_observer.h"
...
class WalletWebUIBubbleManager : public WebUIBubbleManagerImpl<WalletPanelUI>,
 ...
 public:
  ...
  void CloseOpenedPopups() {
    ...
    qorai_observer_.reset();
    for (auto tab_id : contents_wrapper->popup_ids()) {
      Browser* popup_browser = nullptr;
      content::WebContents* popup_contents =
          qorai_wallet::GetWebContentsFromTabId(&popup_browser, tab_id);
  ...
  void SetCloseOnDeactivate(bool close) {
    if (bubble_view_) {
      ...
      if (qorai_observer_) {
        qorai_observer_->UpdateBubbleDeactivationState(close);
      }
    }
  }
 ...
 private:
  ...
  std::unique_ptr<WalletBubbleFocusObserver> qorai_observer_;
...
WalletBubbleManagerDelegateImpl::WalletBubbleManagerDelegateImpl(
  ...
  if (browser->is_type_normal()) {
    anchor_view = static_cast<QoraiBrowserView*>(browser->window())
  ...
  webui_bubble_manager_ = std::make_unique<WalletWebUIBubbleManager>(
      anchor_view, browser, webui_url_, IDS_ACCNAME_QORAI_WALLET_BUTTON,
