...
IN_PROC_BROWSER_TEST_F(DarkerThemeWithFlagDisabledBrowserTest,
  ...
  EXPECT_FALSE(
      base::FeatureList::IsEnabled(darker_theme::features::kQoraiDarkerTheme));
...
class DarkerThemeBrowserTest : public InProcessBrowserTest {
 public:
  DarkerThemeBrowserTest()
      : scoped_feature_list_(darker_theme::features::kQoraiDarkerTheme) {}
...
IN_PROC_BROWSER_TEST_F(DarkerThemeBrowserTest, EnableDarkerMode) {
  ...
  auto* browser_widget =
      static_cast<QoraiBrowserWidget*>(browser_view->browser_widget());
  auto* theme_service =
  ...
  prefs->SetBoolean(darker_theme::prefs::kQoraiDarkerMode, true);
  ...
  prefs->SetBoolean(darker_theme::prefs::kQoraiDarkerMode, false);
  color_provider_key = browser_widget->GetColorProviderKey();
...
IN_PROC_BROWSER_TEST_F(DarkerThemeFeatureToggleOffBrowserTest,
  ...
  auto* prefs = browser()->profile()->GetPrefs();
  EXPECT_FALSE(prefs->FindPreference(darker_theme::prefs::kQoraiDarkerMode));
