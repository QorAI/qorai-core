...
class WindowClosingConfirmBrowserTest : public InProcessBrowserTest,
 ...
 public:
  void SetUpOnMainThread() override {
    QoraiBrowser::SuppressBrowserWindowClosingDialogForTesting(false);
  ...
  void SetDownloadConfirmReturn(bool allow) {
    QoraiBrowserView::SetDownloadConfirmReturnForTesting(allow);
  }
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest, TestWithTwoNTPTabs) {
  QoraiBrowser* qorai_browser = static_cast<QoraiBrowser*>(browser());
  // One tab. Doesn't need to ask.
  EXPECT_FALSE(qorai_browser->ShouldAskForBrowserClosingBeforeHandlers());

  // Two tabs. Need to ask browser closing.
  ui_test_utils::NavigateToURLWithDisposition(
      qorai_browser, GURL(url::kAboutBlankURL),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP);
  EXPECT_TRUE(qorai_browser->ShouldAskForBrowserClosingBeforeHandlers());
  ...
  chrome::CloseWindow(qorai_browser);
  chrome::CloseWindow(qorai_browser);
  ui_test_utils::WaitForBrowserToClose(qorai_browser);
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest,
  ...
  EXPECT_TRUE(static_cast<QoraiBrowser*>(browser())
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest,
  ...
  ASSERT_NO_FATAL_FAILURE(ASSERT_TRUE(ui_test_utils::NavigateToURL(
      qorai_browser, embedded_test_server()->GetURL("/beforeunload.html"))));
  ui_test_utils::NavigateToURLWithDisposition(
      qorai_browser, GURL(url::kAboutBlankURL),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP);

  PrepareForBeforeUnloadDialog(qorai_browser);
...
// https://github.com/qorai/qorai-browser/issues/41936
...
// So, test with private profile window on macOS.
#if BUILDFLAG(IS_MAC)
  auto* qorai_browser = static_cast<QoraiBrowser*>(CreateIncognitoBrowser());
#else
  auto* qorai_browser = static_cast<QoraiBrowser*>(browser());
#endif
  qorai_browser->profile()->GetPrefs()->SetBoolean(prefs::kPromptForDownload,
                                                   false);
  ...
  // Ensure that we have enough disk space to download the large file.
  {
    ...
    int64_t free_space = base::SysInfo::AmountOfFreeDiskSpace(
        GetDownloadDirectory(qorai_browser));
  ...
  std::unique_ptr<content::DownloadTestObserver> progress_waiter(
      CreateInProgressWaiter(qorai_browser, 1));

  // Start downloading a file, wait for it to be created.
  ui_test_utils::NavigateToURLWithDisposition(
      qorai_browser, url, WindowOpenDisposition::CURRENT_TAB,
