...
#include "qorai/browser/ui/speedreader/speedreader_bubble_view.h"
...
#include "qorai/components/speedreader/tts_player.h"
...
#if !BUILDFLAG(IS_ANDROID)
  if (auto* browser_window = QoraiBrowserWindow::From(
...
void SpeedreaderTabHelper::UpdateUI() {
  ...
  if (auto* browser_window = QoraiBrowserWindow::From(
    ...
    browser_window->UpdatePageActionIcon(qorai::kSpeedreaderPageActionIconType);
  }
#endif
}
...
void SpeedreaderTabHelper::DOMContentLoaded(
  ...
  render_frame_host->ExecuteJavaScriptInIsolatedWorld(
      *kLoadScript, base::DoNothing(), ISOLATED_WORLD_ID_QORAI_INTERNAL);
...
void SpeedreaderTabHelper::OnReadingStart(content::WebContents* web_contents) {
  ...
  this->web_contents()->GetPrimaryMainFrame()->ExecuteJavaScriptInIsolatedWorld(
      script, base::DoNothing(), ISOLATED_WORLD_ID_QORAI_INTERNAL);
}
