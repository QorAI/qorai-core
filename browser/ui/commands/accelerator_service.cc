...
#include "qorai/app/command_utils.h"
...
#include "qorai/components/commands/common/commands.mojom-forward.h"
...
bool AcceleratorService::IsCommandDisabledByPolicy(int command_id) const {
  switch (command_id) {
    ...
    case IDC_SHOW_QORAI_TALK:
      return pref_service_->GetBoolean(kQoraiTalkDisabledByPolicy);
    ...
    case IDC_CLOSE_QORAI_WALLET_PANEL:
      return pref_service_->GetBoolean(qorai_wallet::prefs::kDisabledByPolicy);
    case IDC_SHOW_QORAI_REWARDS:
    case IDC_OFFERS_AND_REWARDS_FOR_PAGE:
      return pref_service_->GetBoolean(qorai_rewards::prefs::kDisabledByPolicy);
...
#if BUILDFLAG(ENABLE_QORAI_WAYBACK_MACHINE)
      return !pref_service_->GetBoolean(kQoraiWaybackMachineEnabled);
#else
