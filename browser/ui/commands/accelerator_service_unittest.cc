...
#include "qorai/components/commands/common/accelerator_parsing.h"
...
class AcceleratorServiceUnitTest : public testing::Test {
 public:
  AcceleratorServiceUnitTest() {
    features_.InitAndEnableFeature(commands::features::kQoraiCommands);
  }
...
TEST_F(AcceleratorServiceUnitTest, PolicyFiltering) {
  ...
  // Test Qorai News
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_CONFIGURE_QORAI_NEWS));
  profile().GetPrefs()->SetBoolean(
      qorai_news::prefs::kQoraiNewsDisabledByPolicy, true);
  EXPECT_TRUE(service.IsCommandDisabledByPolicy(IDC_CONFIGURE_QORAI_NEWS));
  profile().GetPrefs()->SetBoolean(
      qorai_news::prefs::kQoraiNewsDisabledByPolicy, false);
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_CONFIGURE_QORAI_NEWS));

  // Test Qorai Talk
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_TALK));
  profile().GetPrefs()->SetBoolean(kQoraiTalkDisabledByPolicy, true);
  EXPECT_TRUE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_TALK));
  profile().GetPrefs()->SetBoolean(kQoraiTalkDisabledByPolicy, false);
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_TALK));

  // Test Qorai VPN (multiple commands)
  const std::vector<int> vpn_commands = {IDC_SHOW_QORAI_VPN_PANEL,
                                         IDC_TOGGLE_QORAI_VPN_TOOLBAR_BUTTON,
                                         IDC_TOGGLE_QORAI_VPN_TRAY_ICON,
                                         IDC_SEND_QORAI_VPN_FEEDBACK,
                                         IDC_ABOUT_QORAI_VPN,
                                         IDC_MANAGE_QORAI_VPN_PLAN,
                                         IDC_TOGGLE_QORAI_VPN};
#if BUILDFLAG(ENABLE_QORAI_VPN)
  for (int command : vpn_commands) {
    EXPECT_FALSE(service.IsCommandDisabledByPolicy(command));
  }
  profile().GetPrefs()->SetBoolean(qorai_vpn::prefs::kManagedQoraiVPNDisabled,
                                   true);
  for (int command : vpn_commands) {
  ...
  // Test Qorai Wallet (multiple commands)
  const std::vector<int> wallet_commands = {IDC_SHOW_QORAI_WALLET,
                                            IDC_SHOW_QORAI_WALLET_PANEL,
                                            IDC_CLOSE_QORAI_WALLET_PANEL};
  for (int command : wallet_commands) {
    EXPECT_FALSE(service.IsCommandDisabledByPolicy(command));
  }
  profile().GetPrefs()->SetBoolean(qorai_wallet::prefs::kDisabledByPolicy,
                                   true);
  for (int command : wallet_commands) {
    EXPECT_TRUE(service.IsCommandDisabledByPolicy(command));
  }
  ...
  // Test Qorai Rewards
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_REWARDS));
  profile().GetPrefs()->SetBoolean(qorai_rewards::prefs::kDisabledByPolicy,
                                   true);
  EXPECT_TRUE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_REWARDS));
  profile().GetPrefs()->SetBoolean(qorai_rewards::prefs::kDisabledByPolicy,
                                   false);
  EXPECT_FALSE(service.IsCommandDisabledByPolicy(IDC_SHOW_QORAI_REWARDS));
  ...
  profile().GetPrefs()->SetBoolean(kQoraiWaybackMachineEnabled, false);
  EXPECT_TRUE(
      service.IsCommandDisabledByPolicy(IDC_SHOW_WAYBACK_MACHINE_BUBBLE));
  profile().GetPrefs()->SetBoolean(kQoraiWaybackMachineEnabled, true);
  EXPECT_FALSE(
      service.IsCommandDisabledByPolicy(IDC_SHOW_WAYBACK_MACHINE_BUBBLE));
#endif
  ...
  commands::Accelerators test_accelerators = {
      {IDC_NEW_TAB, {commands::FromCodesString("Control+KeyT")}},
      {IDC_CONFIGURE_QORAI_NEWS, {commands::FromCodesString("Control+KeyN")}},
      {IDC_SHOW_QORAI_WALLET, {commands::FromCodesString("Control+KeyW")}},
  ...
  EXPECT_FALSE(filtered.contains(IDC_CONFIGURE_QORAI_NEWS));
  EXPECT_FALSE(filtered.contains(IDC_SHOW_QORAI_WALLET));
  ...
  EXPECT_TRUE(filtered.contains(IDC_NEW_TAB));
  EXPECT_TRUE(filtered.contains(IDC_CONFIGURE_QORAI_NEWS));
  EXPECT_TRUE(filtered.contains(IDC_SHOW_QORAI_WALLET));
