...
class QoraiHelpMenuModel : public ui::SimpleMenuModel {
 public:
  explicit QoraiHelpMenuModel(ui::SimpleMenuModel::Delegate* delegate)
      : SimpleMenuModel(delegate) {
    Build();
  }
  QoraiHelpMenuModel(const QoraiHelpMenuModel&) = delete;
  QoraiHelpMenuModel& operator=(const QoraiHelpMenuModel&) = delete;
  ~QoraiHelpMenuModel() override = default;

 private:
  void Build() {
    AddItemWithStringId(IDC_ABOUT, IDS_ABOUT);
    AddItemWithStringId(IDC_HELP_PAGE_VIA_MENU, IDS_HELP_PAGE);
    AddItemWithStringId(IDC_SHOW_QORAI_WEBCOMPAT_REPORTER,
                        IDS_SHOW_QORAI_WEBCOMPAT_REPORTER);
  }
};
}  // namespace
QoraiAppMenuModel::QoraiAppMenuModel(
...
QoraiAppMenuModel::~QoraiAppMenuModel() = default;

#if defined(TOOLKIT_VIEWS)
// static
sidebar::SidebarService::ShowSidebarOption
QoraiAppMenuModel::ConvertIDCToSidebarShowOptions(int id) {
  switch (id) {
...
void QoraiAppMenuModel::Build() {
  // Customize items after build chromium items.
  // Insert & reorder qorai menus based on corresponding commands enable status.
  // If we you want to add/remove from app menu, adjust commands enable status
  // at QoraiBrowserCommandController.
  AppMenuModel::Build();

  RemoveUpstreamMenus();
  BuildTabsAndWindowsSection();
  BuildQoraiProductsSection();
  BuildBrowserSection();
  BuildMoreToolsSubMenu();
  BuildHelpSubMenu();

  ApplyQoraIcons(this);
  ApplyQoraIcons(bookmark_sub_menu_model());

  for (const auto& submenu : sub_menus()) {
    ApplyQoraIcons(submenu.get());
  }
  ...
  if (const auto reading_list_submenu_index =
    ...
    ApplyQoraIcons(static_cast<ui::SimpleMenuModel*>(reading_list_submenu));
  }
}

void QoraiAppMenuModel::BuildTabsAndWindowsSection() {
...
void QoraiAppMenuModel::BuildQoraiProductsSection() {
  // Needs to add separator as this section is qorai specific section.
  bool need_separator = false;

  if (IsCommandIdEnabled(IDC_TOGGLE_AI_CHAT)) {
    InsertItemWithStringIdAt(GetNextIndexOfQoraiProductsSection(),
                             IDC_TOGGLE_AI_CHAT, IDS_TOGGLE_AI_CHAT);
    need_separator = true;
  }

  if (IsCommandIdEnabled(IDC_SHOW_QORAI_WALLET)) {
    InsertItemWithStringIdAt(GetNextIndexOfQoraiProductsSection(),
                             IDC_SHOW_QORAI_WALLET, IDS_SHOW_QORAI_WALLET);
    need_separator = true;
  }

#if BUILDFLAG(ENABLE_QORAI_VPN)
  if (IsCommandIdEnabled(IDC_QORAI_VPN_MENU)) {
    sub_menus().push_back(std::make_unique<QoraiVPNMenuModel>(
        browser(), browser()->profile()->GetPrefs()));
    InsertSubMenuWithStringIdAt(GetNextIndexOfQoraiProductsSection(),
                                IDC_QORAI_VPN_MENU, IDS_QORAI_VPN_MENU,
                                sub_menus().back().get());
    need_separator = true;
  } else if (IsCommandIdEnabled(IDC_SHOW_QORAI_VPN_PANEL)) {
    InsertItemWithStringIdAt(GetNextIndexOfQoraiProductsSection(),
                             IDC_SHOW_QORAI_VPN_PANEL, IDS_QORAI_VPN_MENU);
    need_separator = true;
  }
#endif
  ...
  if (sidebar::CanUseSidebar(browser())) {
    auto index = GetNextIndexOfQoraiProductsSection();
...
#endif
  if (need_separator) {
    InsertSeparatorAt(GetNextIndexOfQoraiProductsSection(),
                      ui::NORMAL_SEPARATOR);
  }
}

void QoraiAppMenuModel::BuildBrowserSection() {
...
void QoraiAppMenuModel::BuildMoreToolsSubMenu() {
  ...
  // Insert sync menu
  if (IsCommandIdEnabled(IDC_SHOW_QORAI_SYNC)) {
    more_tools_menu_model->InsertItemWithStringIdAt(
        next_target_index++, IDC_SHOW_QORAI_SYNC, IDS_SHOW_QORAI_SYNC);
    need_separator = true;
  }
...
void QoraiAppMenuModel::ExecuteCommand(int id, int event_flags) {
#if defined(TOOLKIT_VIEWS)
...
bool QoraiAppMenuModel::IsCommandIdEnabled(int id) const {
#if defined(TOOLKIT_VIEWS)
  if (id == IDC_SIDEBAR_SHOW_OPTION_ALWAYS ||
      id == IDC_SIDEBAR_SHOW_OPTION_MOUSEOVER ||
      id == IDC_SIDEBAR_SHOW_OPTION_NEVER) {
...
size_t QoraiAppMenuModel::GetNextIndexOfQoraiProductsSection() const {
  std::vector<int> commands_to_check = {IDC_SHOW_QORAI_VPN_PANEL,
                                        IDC_QORAI_VPN_MENU,
                                        IDC_SHOW_QORAI_WALLET,
                                        IDC_TOGGLE_AI_CHAT,
...
std::optional<size_t> QoraiAppMenuModel::GetProperItemIndex(
