...
class QoraiBookmarkSubMenuModelUnitTest : public testing::Test {
 public:
  QoraiBookmarkSubMenuModelUnitTest() {}
...
TEST_F(QoraiBookmarkSubMenuModelUnitTest, Build) {
  QoraiBookmarkSubMenuModel model(delegate(), GetBrowser());
  EXPECT_GT(model.GetItemCount(), 0u);
  EXPECT_FALSE(model.GetIndexOfCommandId(IDC_SHOW_BOOKMARK_BAR).has_value());
  EXPECT_FALSE(model.GetIndexOfCommandId(IDC_SHOW_BOOKMARK_BAR).has_value());
  EXPECT_FALSE(
      model.GetIndexOfCommandId(IDC_QORAI_BOOKMARK_BAR_ALWAYS).has_value());
  EXPECT_FALSE(
      model.GetIndexOfCommandId(IDC_QORAI_BOOKMARK_BAR_NEVER).has_value());
  EXPECT_FALSE(
      model.GetIndexOfCommandId(IDC_QORAI_BOOKMARK_BAR_NTP).has_value());

  auto submenu_index =
      model.GetIndexOfCommandId(IDC_QORAI_BOOKMARK_BAR_SUBMENU);
