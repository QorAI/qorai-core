...
class QoraiAppMenuModelBrowserTest : public InProcessBrowserTest {
 public:
  QoraiAppMenuModelBrowserTest() {
#if BUILDFLAG(ENABLE_QORAI_VPN)
  ...
  void RunCommandFromAppMenuModel(Browser* browser, int command_id) {
    ...
    QoraiAppMenuModel model(browser_view->toolbar(), browser);
    model.Init();
...
IN_PROC_BROWSER_TEST_F(QoraiAppMenuModelBrowserTest, CommandsExecutionTest) {
  RunCommandFromAppMenuModel(CreateIncognitoBrowser(),
                             IDC_EXTENSIONS_SUBMENU_MANAGE_EXTENSIONS);
}

// Test qorai menu order test.
// Qorai menu is inserted based on corresponding commands enable status.
// So, this doesn't test for each profiles(normal, private, tor and guest).
// Instead, QoraiBrowserCommandControllerTest will do that.
IN_PROC_BROWSER_TEST_F(QoraiAppMenuModelBrowserTest, MenuOrderTest) {
...
IN_PROC_BROWSER_TEST_F(QoraiAppMenuModelBrowserTest, QoraiVPNMenuTest) {
...
IN_PROC_BROWSER_TEST_F(QoraiAppMenuModelBrowserTest, MenuItemsHaveIcons) {
  auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  QoraiAppMenuModel model(browser_view->toolbar(), browser());
  model.Init();
