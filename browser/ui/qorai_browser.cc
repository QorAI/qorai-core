...
#include "qorai/browser/ui/qorai_file_select_utils.h"
#include "qorai/browser/ui/sidebar/sidebar.h"
...
#include "qorai/browser/ui/tabs/public/constants.h"
...
// static
void QoraiBrowser::SuppressBrowserWindowClosingDialogForTesting(bool suppress) {
  g_suppress_dialog_for_testing = suppress;
}

QoraiBrowser::QoraiBrowser(const CreateParams& params) : Browser(params) {
  if (auto* sidebar_controller = GetFeatures().sidebar_controller()) {
    // TODO(https://github.com/qorai/qorai-browser/issues/45633): Cleanup this.
    // Below call order is important.
    // When reaches here, Sidebar UI is setup in QoraiBrowserView but
    // not initialized. It's just empty because sidebar controller/model is not
    // ready yet. QoraiBrowserView is instantiated by the ctor of Browser.
    ...
    sidebar_controller->SetSidebar(qorai_window()->InitSidebar());
  }

  if (webui_browser::IsWebUIBrowserEnabled() && is_type_normal()) {
    // WebUIBrowserWindow was created in Browser's c'tor (in
    // BrowserWindow::CreateBrowserWindow), not a QoraiBrowserWindow.
    return;
  }
  ...
  qorai_window()->ReadyToListenFullscreenChanges();
}

QoraiBrowser::~QoraiBrowser() = default;

void QoraiBrowser::ScheduleUIUpdate(content::WebContents* source,
...
void QoraiBrowser::OnTabClosing(content::WebContents* contents) {
  ...
  if (chrome::FindAllTabbedBrowsersWithProfile(profile()).size() > 1) {
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(
                       [](base::WeakPtr<QoraiBrowser> browser) {
                         if (browser) {
...
void QoraiBrowser::TabStripEmpty() {
...
void QoraiBrowser::RunFileChooser(
  ...
  if (new_params->title.empty()) {
    ...
    new_params->title = qorai::GetFileSelectTitle(
        content::WebContents::FromRenderFrameHost(render_frame_host), origin,
        origin,
        params.mode == blink::mojom::FileChooserParams::Mode::kSave
            ? qorai::FileSelectTitleType::kSave
            : qorai::FileSelectTitleType::kOpen);
  }
  Browser::RunFileChooser(render_frame_host, listener, *new_params);
#endif
}

void QoraiBrowser::TabCustomTitleChanged(
  ...
  if (session_service) {
    ...
    session_service->AddTabExtraData(session_id(),
                                     ...
                                     tabs::kQoraiTabCustomTitleExtraDataKey,
                                     custom_title.value_or(std::string()));
  }
}

void QoraiBrowser::OnTabStripModelChanged(
...
void QoraiBrowser::FinishWarnBeforeClosing(WarnBeforeClosingResult result) {
...
void QoraiBrowser::BeforeUnloadFired(content::WebContents* source,
                                     bool proceed,
...
bool QoraiBrowser::TryToCloseWindow(
...
void QoraiBrowser::ResetTryToCloseWindow() {
  confirmed_to_close_ = false;
  Browser::ResetTryToCloseWindow();
}

bool QoraiBrowser::NormalBrowserSupportsWindowFeature(
...
bool QoraiBrowser::IsWebContentsVisible(content::WebContents* web_contents) {
...
void QoraiBrowser::UpdateTargetURL(content::WebContents* source,
                                   const GURL& url) {
  GURL target_url = url;
...
bool QoraiBrowser::ShouldAskForBrowserClosingBeforeHandlers() {
...
bool QoraiBrowser::AreAllTabsSharedPinnedTabs() {
  ...
  if (!profile()->GetPrefs()->GetBoolean(qorai_tabs::kSharedPinnedTab)) {
    return false;
  }
...
QoraiBrowserWindow* QoraiBrowser::qorai_window() {
  return static_cast<QoraiBrowserWindow*>(window_.get());
}
