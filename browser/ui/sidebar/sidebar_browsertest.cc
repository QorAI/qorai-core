...
#include "qorai/browser/ui/views/sidebar/sidebar_control_view.h"
#include "qorai/browser/ui/views/sidebar/sidebar_items_contents_view.h"
#include "qorai/browser/ui/views/sidebar/sidebar_items_scroll_view.h"
...
class SidebarBrowserTest : public InProcessBrowserTest {
 public:
  ...
  SidePanelButton* GetSidePanelToolbarButton() const {
    return static_cast<QoraiToolbarView*>(
  ...
  views::View* GetVerticalTabsContainer() const {
    ...
    return static_cast<QoraiBrowserView*>(view)->vertical_tab_strip_host_view_;
  }
  ...
  QoraiSidePanel* GetSidePanel() const {
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTest, BasicTest) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("qorai://newtab/")));
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTest, WebTypePanelTest) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURLWithDisposition(
      browser(), GURL("qorai://newtab/"),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP));
  current_tab_index = tab_model()->active_index();
  EXPECT_EQ(1, current_tab_index);

  // Activate sidebar item(qorai://settings) and check existing first tab is
  // activated.
...
class SidebarBrowserWithWebPanelTest
 ...
 public:
  ...
  QoraiMultiContentsView* GetQoraiMultiContentsView() {
    return browser_view()->GetQoraiMultiContentsView();
  }
...
IN_PROC_BROWSER_TEST_P(SidebarBrowserWithWebPanelTest, WebPanelTest) {
  auto contents_container_view_for_web_panel =
      GetQoraiMultiContentsView()->contents_container_view_for_web_panel_;
  if (!IsWebPanelEnabled()) {
  ...
  EXPECT_GT(GetQoraiMultiContentsView()->web_panel_width_, 0);
  EXPECT_EQ(GetQoraiMultiContentsView()->GetWebPanelWidth(), 0);
  EXPECT_EQ(contents_container_view_for_web_panel->width(), 0);
  EXPECT_FALSE(GetQoraiMultiContentsView()->web_panel_on_left_);
  EXPECT_EQ(
      GetQoraiMultiContentsView()->width(),
      GetQoraiMultiContentsView()->GetActiveContentsContainerView()->width());

  contents_container_view_for_web_panel->SetVisible(true);
  EXPECT_GT(GetQoraiMultiContentsView()->GetWebPanelWidth(), 0);
  GetQoraiMultiContentsView()->InvalidateLayout();
  ASSERT_TRUE(base::test::RunUntil([&]() {
    return contents_container_view_for_web_panel->width() ==
           GetQoraiMultiContentsView()->web_panel_width_;
  }));
  ...
  GetQoraiMultiContentsView()->SetWebPanelOnLeft(true);
  ASSERT_TRUE(base::test::RunUntil([&]() {
  ...
  EXPECT_TRUE(CanAddCurrentActiveTabToSidebar(browser()));
  controller()->AddItemWithCurrentTab();
  EXPECT_FALSE(GetQoraiMultiContentsView()->IsWebPanelVisible());
  ...
  EXPECT_TRUE(GetQoraiMultiContentsView()->IsWebPanelVisible());
...
IN_PROC_BROWSER_TEST_P(SidebarBrowserTestWithkSidebarShowAlwaysOnStable,
  ...
  // Check one shot Qora panel is opened or not based on test parameter.
  if (GetParam()) {
    // If Qora panel is opened, panel active index is changed.
  ...
  // qora panel open. As we don't want to show one-shot qora panel open for
  ...
  // does early return w/o opening qora panel.
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURLWithDisposition(
      browser(), GURL("https://www.qorai.com/"),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP));
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTest, SidebarRightSideTest) {
  ...
  if (QoraiContentsViewUtil::GetRoundedCornersWebViewMargin(browser())) {
