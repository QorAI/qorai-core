...
TEST_F(SidebarModelTest, ItemsChangedTest) {
  ...
  service()->UpdateItem(GURL("https://www.qorai.com/"),
                        GURL("https://qorai.com/"), u"qorai software",
                        u"qorai software");
  ...
  EXPECT_CALL(observer_, OnItemUpdated(testing::_, expected_update)).Times(1);
  service()->UpdateItem(GURL("https://qorai.com/"), GURL("https://qorai.com/"),
                        u"qorai software", u"qorai");
  testing::Mock::VerifyAndClearExpectations(&observer_);
...
TEST_F(SidebarModelTest, CanUseNotAddedBuiltInItemInsteadOfTest) {
  GURL talk("https://talk.qorai.com/1Ar1vHfLBWX2sAdi");
...
TEST_F(SidebarModelTest, ActiveIndexChangedAfterItemAdded) {
  ...
  SidebarItem item_2 = SidebarItem::Create(
      GURL("https://www.qorais.com/"), u"qorai software",
      SidebarItem::Type::kTypeWeb, SidebarItem::BuiltInItemType::kNone, false);
...
// Check Qora item is top-most item.
...
TEST(SidebarUtilTest, ConvertURLToBuiltInItemURLTest) {
  EXPECT_EQ(GURL(kQoraiTalkURL),
            ConvertURLToBuiltInItemURL(GURL("https://talk.qorai.com")));
  EXPECT_EQ(GURL(kQoraiTalkURL),
            ConvertURLToBuiltInItemURL(
                GURL("https://talk.qorai.com/1Ar1vHfLBWX2sAdi")));
  EXPECT_EQ(
      GURL(kQoraiUIWalletPageURL),
      ConvertURLToBuiltInItemURL(GURL("chrome://wallet/crypto/onboarding")));

  // Not converted for url that doesn't relavant builtin item.
  GURL qorai_com("https://www.qorai.com/");
  EXPECT_EQ(qorai_com, ConvertURLToBuiltInItemURL(qorai_com));
}
