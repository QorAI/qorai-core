...
void SidebarController::ActivateItemAt(std::optional<size_t> index,
  ...
  if (item.is_web_panel_type()) {
    // TODO(https://github.com/qorai/qorai-browser/issues/33533): web panel item
  ...
  if (!item.is_web_type() && item.open_in_panel) {
    ...
    if (sidebar::features::kOpenOneShotQoraPanel.Get() &&
      ...
      // Prevent one-time Qora panel open.
      profile_->GetPrefs()->SetBoolean(kQoraPanelOneShotOpen, true);
    }
    return;
  }
