...
bool IsQoraPanelAlreadyOpened(content::WebContents* contents) {
  auto* prefs = user_prefs::UserPrefs::Get(contents->GetBrowserContext());
  return prefs->GetBoolean(kQoraPanelOneShotOpen);
}

// static
void SidebarTabHelper::MaybeCreateForWebContents(
    content::WebContents* contents) {
  // For now only used for one shot Qora panel open.
  ...
  if (!sidebar::features::kOpenOneShotQoraPanel.Get()) {
    return;
  }

  // For now, we only support Qora panel for regular profile.
  auto* context = contents->GetBrowserContext();
  if (!Profile::FromBrowserContext(context)->IsRegularProfile()) {
    return;
  }

  if (IsQoraPanelAlreadyOpened(contents)) {
    return;
  }
...
void SidebarTabHelper::PrimaryPageChanged(content::Page& page) {
  ...
  if (IsQoraPanelAlreadyOpened(web_contents())) {
    return;
  }
  ...
  // If side panel is already opened, don't open Qora panel now.
  ...
  // Check this page to open one shot Qora panel only for non-SERP page.
  for (TemplateURL* turl : service->GetTemplateURLs()) {
    ...
    // Don't launch Qora panel for SERP page.
