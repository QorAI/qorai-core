...
QoraiAutoplayContentSettingBubbleModel::QoraiAutoplayContentSettingBubbleModel(
    Delegate* delegate,
...
QoraiAutoplayContentSettingBubbleModel::
    ~QoraiAutoplayContentSettingBubbleModel() = default;

void QoraiAutoplayContentSettingBubbleModel::CommitChanges() {
  if (settings_changed()) {
...
bool QoraiAutoplayContentSettingBubbleModel::settings_changed() const {
  return selected_item() != bubble_content().radio_group.default_item;
}

void QoraiAutoplayContentSettingBubbleModel::SetTitle() {
  set_title(l10n_util::GetStringUTF16(IDS_BLOCKED_AUTOPLAY_TITLE));
}

void QoraiAutoplayContentSettingBubbleModel::SetRadioGroup() {
...
void QoraiAutoplayContentSettingBubbleModel::SetNarrowestContentSetting(
    ContentSetting setting) {
  if (!GetProfile())
    return;
