...
QoraiBrowserTabStripController::QoraiBrowserTabStripController(
    TabStripModel* model,
...
const std::optional<int> QoraiBrowserTabStripController::GetModelIndexOf(
    Tab* tab) {
  return tabstrip_->GetModelIndexOf(tab);
}

void QoraiBrowserTabStripController::EnterTabRenameModeAt(int index) {
  CHECK(base::FeatureList::IsEnabled(tabs::features::kQoraiRenamingTabs));
  return static_cast<QoraiTabStrip*>(tabstrip_)->EnterTabRenameModeAt(index);
}

void QoraiBrowserTabStripController::SetCustomTitleForTab(
    int index,
    const std::optional<std::u16string>& title) {
  static_cast<QoraiTabStripModel*>(model_.get())
      ->SetCustomTitleForTab(index, title);
}

void QoraiBrowserTabStripController::ShowContextMenuForTab(
    Tab* tab,
    const gfx::Point& p,
  ...
  context_menu_contents_ =
      std::make_unique<QoraiTabContextMenuContents>(tab, this, *tab_index);
  context_menu_contents_->RunMenuAt(p, source_type);
}

void QoraiBrowserTabStripController::ExecuteCommandForTab(
