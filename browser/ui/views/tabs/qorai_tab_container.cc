...
#include "qorai/browser/ui/views/tabs/qorai_tab_container.h"
...
QoraiTabContainer::QoraiTabContainer(
  ...
  show_vertical_tabs_.Init(
      ...
      base::BindRepeating(&QoraiTabContainer::UpdateLayoutOrientation,
...
base::OnceClosure QoraiTabContainer::LockLayout() {
  DCHECK(!layout_locked_) << "LockLayout() doesn't allow reentrance";
  layout_locked_ = true;
  return base::BindOnce(&QoraiTabContainer::OnUnlockLayout,
                        base::Unretained(this));
}

gfx::Size QoraiTabContainer::CalculatePreferredSize(
  ...
  if (tab_count) {
    if (Tab* last_tab = tabs_view_model_.view_at(tab_count - 1);
      ...
      height += QoraiTabGroupHeader::kPaddingForGroup;
    }
...
void QoraiTabContainer::UpdateClosingModeOnRemovedTab(int model_index,
                                                      bool was_active) {
...
gfx::Rect QoraiTabContainer::GetTargetBoundsForClosingTab(
    Tab* tab,
...
void QoraiTabContainer::EnterTabClosingMode(std::optional<int> override_width,
...
bool QoraiTabContainer::ShouldTabBeVisible(const Tab* tab) const {
...
void QoraiTabContainer::StartInsertTabAnimation(int model_index) {
  // Note that we check this before checking currently we're in vertical tab
  // strip mode. We might be in the middle of changing orientation.
  if (layout_locked_) {
    return;
...
void QoraiTabContainer::RemoveTab(int index, bool was_active) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void QoraiTabContainer::OnTabCloseAnimationCompleted(Tab* tab) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void QoraiTabContainer::UpdateLayoutOrientation() {
...
void QoraiTabContainer::PaintBoundingBoxForSplitTabs(gfx::Canvas& canvas) {
...
void QoraiTabContainer::PaintBoundingBoxForSplitTab(
    gfx::Canvas& canvas,
  ...
  if (!is_vertical_tab) {
    ...
    bounding_rects.Inset(gfx::Insets::TLBR(
        vertical_margin, qorai_tabs::kHorizontalTabInset,
        vertical_margin + GetLayoutConstant(TABSTRIP_TOOLBAR_OVERLAP),
        qorai_tabs::kHorizontalTabInset));
  }

  constexpr auto kRadius = 12.f;  // same value with --qora-radius-l
  ...
  flags.setColor(cp->GetColor(
      is_vertical_tab ? kColorQoraiSplitViewTileBackgroundVertical
  ...
  if (!is_vertical_tab && active_tab_index != indices[0] &&
    ...
    canvas.DrawLine(separator_top, separator_bottom,
                    cp->GetColor(kColorQoraiSplitViewTileDivider));
  }

  bounding_rects.Outset(1);
  flags.setStyle(cc::PaintFlags::kStroke_Style);
  flags.setColor(cp->GetColor(kColorQoraiSplitViewTileBackgroundBorder));
  canvas.DrawRoundRect(bounding_rects, kRadius, flags);
}

void QoraiTabContainer::OnUnlockLayout() {
...
void QoraiTabContainer::CompleteAnimationAndLayout() {
  // Note that we check this before checking currently we're in vertical tab
  // strip mode. We might be in the middle of changing orientation.
  if (layout_locked_) {
    return;
...
void QoraiTabContainer::PaintChildren(const views::PaintInfo& paint_info) {
...
void QoraiTabContainer::SetTabSlotVisibility() {
  ...
  // in that case.
  // https://github.com/qorai/qorai-browser/issues/39298
...
void QoraiTabContainer::InvalidateIdealBounds() {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void QoraiTabContainer::Layout(PassKey) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser()) &&
      last_layout_size_ == size()) {
    return;
  }
...
void QoraiTabContainer::OnSplitCreated(const std::vector<int>& indices) {
  UpdateTabsBorderInSplitTab(indices);
}

void QoraiTabContainer::OnSplitRemoved(const std::vector<int>& indices) {
  UpdateTabsBorderInSplitTab(indices);
}

void QoraiTabContainer::OnSplitContentsChanged(
    const std::vector<int>& indices) {
  UpdateTabsBorderInSplitTab(indices);
}

std::optional<BrowserRootView::DropIndex> QoraiTabContainer::GetDropIndex(
...
// QoraiTabContainer::DropArrow:
// ----------------------------------------------------------
QoraiTabContainer::DropArrow::DropArrow(const BrowserRootView::DropIndex& index,
                                        Position position,
                                        bool beneath,
...
QoraiTabContainer::DropArrow::~DropArrow() = default;

void QoraiTabContainer::DropArrow::SetBeneath(bool beneath) {
  if (beneath_ == beneath) {
    return;
  }
...
void QoraiTabContainer::DropArrow::SetWindowBounds(const gfx::Rect& bounds) {
  arrow_window_->SetBounds(bounds);
}

void QoraiTabContainer::HandleDragUpdate(
...
void QoraiTabContainer::HandleDragExited() {
  if (!tabs::utils::ShouldShowVerticalTabs(
          tab_slot_controller_->GetBrowser())) {
...
gfx::Rect QoraiTabContainer::GetDropBounds(int drop_index,
                                           bool drop_before,
                                           bool drop_in_group,
                                           bool* is_beneath) {
  ...
  const gfx::ImageSkia* drop_image = GetDropArrowImage(
      QoraiTabContainer::DropArrow::Position::Horizontal, false);
...
gfx::ImageSkia* QoraiTabContainer::GetDropArrowImage(
    QoraiTabContainer::DropArrow::Position pos,
    bool beneath) {
  using Position = QoraiTabContainer::DropArrow::Position;
...
void QoraiTabContainer::SetDropArrow(
    const std::optional<BrowserRootView::DropIndex>& index) {
  if (!index) {
...
bool QoraiTabContainer::IsPinnedTabContainer() const {
  return tabs_view_model_.view_size() > 0 &&
         tabs_view_model_.view_at(0)->data().pinned;
}

void QoraiTabContainer::UpdateTabsBorderInSplitTab(
    const std::vector<int>& indices) {
  for (auto& index : indices) {
    ...
    // See QoraiVerticalTabStyle::GetContentsInsets().
...
BEGIN_METADATA(QoraiTabContainer)
END_METADATA
