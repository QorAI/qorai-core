...
#include "qorai/browser/ui/views/tabs/qorai_tab_group_underline.h"
...
QoraiTabGroupUnderline::QoraiTabGroupUnderline(
    TabGroupViews* tab_group_views,
    const tab_groups::TabGroupId& group,
    const TabGroupStyle& style)
    : TabGroupUnderline(tab_group_views, group, style) {}

QoraiTabGroupUnderline::~QoraiTabGroupUnderline() = default;

void QoraiTabGroupUnderline::UpdateBounds(const views::View* leading_view,
...
gfx::Insets QoraiTabGroupUnderline::GetInsetsForUnderline(
  ...
  int horizontal_inset =
      TabGroupUnderline::kStrokeThickness + qorai_tabs::kHorizontalTabInset;

  return gfx::Insets::VH(0, horizontal_inset);
}

gfx::Rect QoraiTabGroupUnderline::CalculateTabGroupUnderlineBounds(
  ...
  if (trailing_bounds.height()) {
    trailing_bounds.set_height(trailing_bounds.height() +
                               QoraiTabGroupHeader::kPaddingForGroup);
  }
...
void QoraiTabGroupUnderline::OnPaint(gfx::Canvas* canvas) {
  if (!tabs::features::HorizontalTabsUpdateEnabled()) {
...
bool QoraiTabGroupUnderline::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(tab_group_views_->GetBrowser());
}

// static
int QoraiTabGroupUnderline::GetStrokeInset() {
  if (!tabs::features::HorizontalTabsUpdateEnabled()) {
    return TabGroupUnderline::GetStrokeInset();
  }
  return qorai_tabs::kHorizontalTabInset;
}

BEGIN_METADATA(QoraiTabGroupUnderline)
END_METADATA
