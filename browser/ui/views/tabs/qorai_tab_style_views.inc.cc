...
////////////////////////////////////////////////////////////////////////////////
// QoraiVerticalTabStyle
//
// This class deals with tab styling when vertical tab strip feature flag is
// enabled.
//
class QoraiVerticalTabStyle : public TabStyleViewsImpl {
 public:
  explicit QoraiVerticalTabStyle(Tab* tab);
  QoraiVerticalTabStyle(const QoraiVerticalTabStyle&) = delete;
  QoraiVerticalTabStyle& operator=(const QoraiVerticalTabStyle&) = delete;
  ~QoraiVerticalTabStyle() override = default;
 ...
 private:
  ...
  // true when |tab| is shown in the split view(with SideBySide or qorai split
  // view)
...
QoraiVerticalTabStyle::QoraiVerticalTabStyle(Tab* tab)
    : TabStyleViewsImpl(tab) {}

SkPath QoraiVerticalTabStyle::GetPath(
  ...
  if (!ShouldShowVerticalTabs()) {
    ...
    aligned_bounds.Inset(
        gfx::InsetsF::VH(qorai_tabs::kHorizontalTabVerticalSpacing * scale,
                         qorai_tabs::kHorizontalTabInset * scale));
    ...
    if (path_type == TabStyle::PathType::kHitTest) {
      auto hit_test_outsets =
          gfx::OutsetsF::VH(qorai_tabs::kHorizontalTabVerticalSpacing * scale,
                            qorai_tabs::kHorizontalTabGap / 2 * scale);
      ...
      if (frame_condensed && tab()->controller()->IsTabFirst(tab())) {
        if (tab()->GetMirrored()) {
          hit_test_outsets.set_right(qorai_tabs::kHorizontalTabInset * scale);
        } else {
          hit_test_outsets.set_left(qorai_tabs::kHorizontalTabInset * scale);
        }
      }
  ...
  if (!is_pinned && IsSplitTab(tab()) &&
    ...
    if (ShouldShowVerticalTabs()) {
      ...
      constexpr int kAdditionalVerticalPadding =
          qorai_tabs::kHorizontalSplitViewTabVerticalSpacing -
          qorai_tabs::kHorizontalTabGap;
...
gfx::Insets QoraiVerticalTabStyle::GetContentsInsets() const {
  const bool is_pinned = tab()->data().pinned;
...
TabStyle::SeparatorBounds QoraiVerticalTabStyle::GetSeparatorBounds(
    float scale) const {
  if (!HorizontalTabsUpdateEnabled()) {
...
float QoraiVerticalTabStyle::GetSeparatorOpacity(bool for_layout,
                                                 bool leading) const {
  if (ShouldShowVerticalTabs()) {
    return 0;
  }
...
int QoraiVerticalTabStyle::GetStrokeThickness(
...
void QoraiVerticalTabStyle::PaintTab(gfx::Canvas* canvas) const {
  if (ShouldShowVerticalTabs()) {
  ...
  if (tab()->data().pinned) {
    ...
    if (ShouldShowVerticalTabs()) {
      color_id = kColorQoraiVerticalTabSeparator;
    }
...
SkColor QoraiVerticalTabStyle::GetTargetTabBackgroundColor(
    TabStyle::TabSelectionState selection_state,
    bool hovered) const {
  ...
  if (tab()->IsActive()) {
    return cp->GetColor(kColorQoraiVerticalTabActiveBackground);
  }

  if (hovered) {
    return cp->GetColor(kColorQoraiVerticalTabHoveredBackground);
  }
  ...
  return cp->GetColor(kColorQoraiVerticalTabInactiveBackground);
}

bool QoraiVerticalTabStyle::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(tab()->controller()->GetBrowser());
}

bool QoraiVerticalTabStyle::IsSplitTab(const Tab* tab) const {
  if (!tab) {
    return false;
  }
...
bool QoraiVerticalTabStyle::IsStartSplitTab(const Tab* tab) const {
  if (!tab) {
    return false;
  }
...
std::unique_ptr<TabStyleViews> TabStyleViews::CreateForTab(Tab* tab) {
  return std::make_unique<QoraiVerticalTabStyle>(tab);
}
