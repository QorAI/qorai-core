...
class QoraiTabTest : public ChromeViewsTestBase {
 public:
  QoraiTabTest() = default;
  ~QoraiTabTest() override = default;

  void LayoutAndCheckBorder(QoraiTab* tab, const gfx::Rect& bounds) {
    ...
    left_inset += QoraiTab::kExtraLeftPadding;
    EXPECT_EQ(left_inset, tab->GetInsets().left());
  }
};

TEST_F(QoraiTabTest, ExtraPaddingLayoutTest) {
  FakeTabSlotController tab_slot_controller;
  QoraiTab tab(&tab_slot_controller);

  // Our tab should have extra padding always.
  // See the comment at QoraiTab::GetInsets().
...
TEST_F(QoraiTabTest, TabHeightTest) {
  FakeTabSlotController tab_slot_controller;
  QoraiTab tab(&tab_slot_controller);
...
TEST_F(QoraiTabTest, TabStyleTest) {
  FakeTabSlotController tab_slot_controller;
  QoraiTab tab(&tab_slot_controller);
...
class QoraiTabRenamingUnitTest : public QoraiTabTest {
 public:
  ...
  QoraiTabRenamingUnitTest() = default;
  ~QoraiTabRenamingUnitTest() override = default;

 protected:
  QoraiTab* tab() { return tab_.get(); }
  ...
  void SetUp() override {
    QoraiTabTest::SetUp();
    tab_ = std::make_unique<QoraiTab>(&tab_slot_controller_);
    LayoutAndCheckBorder(tab_.get(), {0, 0, 100, 50});
  }

  void TearDown() override {
    tab_.reset();
    QoraiTabTest::TearDown();
  }

 private:
  base::test::ScopedFeatureList feature_list_{
      tabs::features::kQoraiRenamingTabs};
  std::unique_ptr<QoraiTab> tab_;
  testing::NiceMock<MockTabSlotController> tab_slot_controller_;
};

TEST_F(QoraiTabRenamingUnitTest, EnterRenameMode) {
...
TEST_F(QoraiTabRenamingUnitTest, CommitRename) {
...
TEST_F(QoraiTabRenamingUnitTest, ExitRenameMode) {
...
TEST_F(QoraiTabRenamingUnitTest, EnterKeyCommitsRename) {
  constexpr char16_t kNewTitle[] = u"New Title";
  tab()->EnterRenameMode();
  rename_textfield().SetText(kNewTitle);
...
TEST_F(QoraiTabRenamingUnitTest, EscapeKeyExitsRenameMode) {
  constexpr char16_t kOriginalTitle[] = u"Original Title";
  title()->SetText(kOriginalTitle);
...
TEST_F(QoraiTabRenamingUnitTest, ClickingOutsideRenamingTabCommitsRename) {
  tab()->EnterRenameMode();
  ...
  static_cast<QoraiTab::RenameTextfield*>(&rename_textfield())
      ->MouseMovedOutOfHost();
