...
QoraiTabGroupHeader::~QoraiTabGroupHeader() = default;

void QoraiTabGroupHeader::AddedToWidget() {
...
void QoraiTabGroupHeader::VisualsChanged() {
  ...
  if (!ShouldShowVerticalTabs()) {
    title_->SetLineHeight(qorai_tabs::kTabGroupLineHeight);
  }
  ...
  if (ShouldShowHeaderIcon()) {
    sync_icon_->SetImage(ui::ImageModel::FromVectorIcon(
        kQoraProductSyncIcon, SkColorSetA(GetGroupColor(), 0.6 * 255),
        group_style_->GetSyncIconWidth()));
  }
}

int QoraiTabGroupHeader::GetDesiredWidth() const {
  if (!tabs::features::HorizontalTabsUpdateEnabled() ||
      ShouldShowVerticalTabs()) {
    return TabGroupHeader::GetDesiredWidth();
  }
  return qorai_tabs::kHorizontalTabInset * 2 + title_chip_->width();
}

void QoraiTabGroupHeader::Layout(PassKey) {
...
bool QoraiTabGroupHeader::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(
      tab_slot_controller_->GetBrowser());
}

void QoraiTabGroupHeader::LayoutTitleChipForVerticalTabs() {
...
SkColor QoraiTabGroupHeader::GetGroupColor() const {
...
std::optional<SkColor> QoraiTabGroupHeader::GetChipBackgroundColor() const {
  if (ShouldShowVerticalTabs()) {
    return {};
  }
...
BEGIN_METADATA(QoraiTabGroupHeader)
END_METADATA
