...
class ContentsView : public views::View {
 public:
  explicit ContentsView(QoraiCompoundTabContainer* container)
  ...
  void ChildPreferredSizeChanged(views::View* child) override {
    // Bypass ScrollView and notify the QoraiCompoundTabContainer directly.
    container_->ChildPreferredSizeChanged(child);
  }

  base::raw_ptr<QoraiCompoundTabContainer> container_;
};
 ...
 public:
  explicit CustomScrollView(PrefService* prefs)
    ...
    should_show_scroll_bar_.Init(
        qorai_tabs::kVerticalTabsShowScrollbar, prefs,
...
QoraiCompoundTabContainer::QoraiCompoundTabContainer(
...
void QoraiCompoundTabContainer::SetAvailableWidthCallback(
  ...
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser()) &&
    ...
    if (base::FeatureList::IsEnabled(
      ...
      unpinned_tab_container_->SetAvailableWidthCallback(base::BindRepeating(
          &QoraiCompoundTabContainer::GetAvailableWidthConsideringScrollBar,
          base::Unretained(this)));
    } else {
...
base::OnceClosure QoraiCompoundTabContainer::LockLayout() {
  std::vector<base::OnceClosure> closures;
  for (const auto& tab_container :
       {unpinned_tab_container_, pinned_tab_container_}) {
    closures.push_back(
        static_cast<QoraiTabContainer*>(base::to_address(tab_container))
            ->LockLayout());
  }
...
void QoraiCompoundTabContainer::SetScrollEnabled(bool enabled) {
...
void QoraiCompoundTabContainer::TransferTabBetweenContainers(
...
void QoraiCompoundTabContainer::Layout(PassKey) {
  if (!ShouldShowVerticalTabs()) {
...
gfx::Size QoraiCompoundTabContainer::CalculatePreferredSize(
  ...
  // |QoraiVerticalTabStripRegionView|
  for (auto* parent_view = parent(); parent_view;
       parent_view = parent_view->parent()) {
    auto* region_view =
        views::AsViewClass<QoraiVerticalTabStripRegionView>(parent_view);
    if (!region_view) {
      continue;
    }
...
gfx::Size QoraiCompoundTabContainer::GetMinimumSize() const {
  if (!ShouldShowVerticalTabs()) {
...
views::SizeBounds QoraiCompoundTabContainer::GetAvailableSize(
...
std::vector<Tab*> QoraiCompoundTabContainer::AddTabs(
...
void QoraiCompoundTabContainer::MoveTab(int from_model_index,
                                        int to_model_index) {
  CompoundTabContainer::MoveTab(from_model_index, to_model_index);
  UpdatePinnedTabContainerBorder();
}

void QoraiCompoundTabContainer::RemoveTab(int index, bool was_active) {
  CompoundTabContainer::RemoveTab(index, was_active);
  UpdatePinnedTabContainerBorder();
}

void QoraiCompoundTabContainer::SetTabPinned(int model_index,
                                             TabPinned pinned) {
  CompoundTabContainer::SetTabPinned(model_index, pinned);
  UpdatePinnedTabContainerBorder();
}

int QoraiCompoundTabContainer::GetUnpinnedContainerIdealLeadingX() const {
  if (!ShouldShowVerticalTabs()) {
...
QoraiCompoundTabContainer::GetDropIndex(const ui::DropTargetEvent& event) {
  if (!ShouldShowVerticalTabs()) {
...
BrowserRootView::DropTarget* QoraiCompoundTabContainer::GetDropTarget(
...
void QoraiCompoundTabContainer::PaintChildren(const views::PaintInfo& info) {
  if (ShouldShowVerticalTabs()) {
...
void QoraiCompoundTabContainer::ChildPreferredSizeChanged(views::View* child) {
...
void QoraiCompoundTabContainer::SetActiveTab(
...
views::View* QoraiCompoundTabContainer::TargetForRect(views::View* root,
                                                      const gfx::Rect& rect) {
  if (base::FeatureList::IsEnabled(
          tabs::features::kQoraiVerticalTabScrollBar) &&
      scroll_view_) {
...
TabContainer* QoraiCompoundTabContainer::GetTabContainerAt(
...
gfx::Rect QoraiCompoundTabContainer::ConvertUnpinnedContainerIdealBoundsToLocal(
...
bool QoraiCompoundTabContainer::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(
      tab_slot_controller_->GetBrowser());
}

void QoraiCompoundTabContainer::UpdatePinnedTabContainerBorder() {
  ...
  if (should_have_separator_between_pinned_and_unpinned) {
    pinned_tab_container_->SetBorder(views::CreateSolidSidedBorder(
        gfx::Insets().set_bottom(1), kColorQoraiVerticalTabSeparator));
  } else {
    pinned_tab_container_->SetBorder(nullptr);
  }
}

void QoraiCompoundTabContainer::UpdateUnpinnedContainerSize() {
  DCHECK(scroll_view_);
...
void QoraiCompoundTabContainer::ScrollTabToBeVisible(Tab* tab) {
  CHECK(tab);
  if (!scroll_view_) {
    return;
  }
...
int QoraiCompoundTabContainer::GetAvailableWidthConsideringScrollBar() {
  CHECK(
      base::FeatureList::IsEnabled(tabs::features::kQoraiVerticalTabScrollBar));
  if (scroll_view_) {
    auto* scroll_bar = scroll_view_->vertical_scroll_bar();
...
BEGIN_METADATA(QoraiCompoundTabContainer)
END_METADATA
