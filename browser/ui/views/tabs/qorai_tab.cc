...
QoraiTab::RenameTextfield::RenameTextfield(
...
QoraiTab::RenameTextfield::~RenameTextfield() = default;

// views::Textfield:
void QoraiTab::RenameTextfield::VisibilityChanged(views::View* starting_from,
                                                  bool is_visible) {
...
void QoraiTab::RenameTextfield::MouseMovedOutOfHost() {
  CHECK(on_click_outside_callback_);
  on_click_outside_callback_.Run();
}

BEGIN_METADATA(QoraiTab, RenameTextfield)
END_METADATA

QoraiTab::QoraiTab(TabSlotController* controller) : Tab(controller) {
  ...
  rename_textfield_ =
      AddChildView(std::make_unique<RenameTextfield>(base::BindRepeating(
          // This is safe to pass base::Unretained(this), as QoraiTab is the
          // owner of RenameTextfield and outlives it.
          &QoraiTab::CommitRename, base::Unretained(this))));
...
QoraiTab::~QoraiTab() = default;

void QoraiTab::EnterRenameMode() {
...
std::u16string QoraiTab::GetRenderedTooltipText(const gfx::Point& p) const {
  auto* browser = controller_->GetBrowser();
  if (browser &&
      qorai_tabs::AreTooltipsEnabled(browser->profile()->GetPrefs())) {
...
int QoraiTab::GetWidthOfLargestSelectableRegion() const {
...
void QoraiTab::ActiveStateChanged() {
  Tab::ActiveStateChanged();

  // This should be called whenever the active state changes
  // see comment on UpdateEnabledForMuteToggle();
  // https://github.com/qorai/qorai-browser/issues/23476/
  alert_indicator_button_->UpdateEnabledForMuteToggle();
}

std::optional<SkColor> QoraiTab::GetGroupColor() const {
...
void QoraiTab::UpdateIconVisibility() {
...
gfx::Insets QoraiTab::GetInsets() const {
  ...
  // See https://www.github.com/qorai/qorai-browser/issues/30469.
  auto insets = Tab::GetInsets();
  insets.set_left(insets.left() + kExtraLeftPadding);
  return insets;
}

void QoraiTab::MaybeAdjustLeftForPinnedTab(gfx::Rect* bounds,
...
bool QoraiTab::ShouldRenderAsNormalTab() const {
  if (IsAtMinWidthForVerticalTabStrip()) {
...
bool QoraiTab::IsAtMinWidthForVerticalTabStrip() const {
  return tabs::utils::ShouldShowVerticalTabs(controller()->GetBrowser()) &&
         width() <= tabs::kVerticalTabMinWidth;
}

void QoraiTab::SetData(TabRendererData data) {
...
bool QoraiTab::HandleKeyEvent(views::Textfield* sender,
...
void QoraiTab::UpdateRenameTextfieldBounds() {
