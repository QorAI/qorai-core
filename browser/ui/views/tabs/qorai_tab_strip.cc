...
#include "qorai/browser/ui/views/tabs/qorai_tab.h"
...
QoraiTabStrip::QoraiTabStrip(std::unique_ptr<TabStripController> controller)
    : TabStrip(std::move(controller)) {}

QoraiTabStrip::~QoraiTabStrip() = default;

bool QoraiTabStrip::IsVerticalTabsFloating() const {
  ...
  return vertical_region_view->state() ==
             QoraiVerticalTabStripRegionView::State::kFloating ||
         (vertical_region_view->is_animating() &&
          vertical_region_view->last_state() ==
              QoraiVerticalTabStripRegionView::State::kFloating &&
          vertical_region_view->state() ==
              QoraiVerticalTabStripRegionView::State::kCollapsed);
}

bool QoraiTabStrip::ShouldDrawStrokes() const {
  if (ShouldShowVerticalTabs()) {
  ...
  return contrast_ratio < kQoraiMinimumContrastRatioForOutlines;
}

void QoraiTabStrip::ShowHover(Tab* tab, TabStyle::ShowHoverStyle style) {
  // Chromium asks hover style to all split tabs but we only set hover style
  // to hovered tab.
  tab->ShowHover(style);
}

void QoraiTabStrip::HideHover(Tab* tab, TabStyle::HideHoverStyle style) {
  // See the comment of ShowHover().
  tab->HideHover(style);
}

void QoraiTabStrip::UpdateHoverCard(Tab* tab, HoverCardUpdateType update_type) {
  if (qorai_tabs::AreTooltipsEnabled(controller_->GetProfile()->GetPrefs())) {
    return;
  }
  TabStrip::UpdateHoverCard(tab, update_type);
}

void QoraiTabStrip::MaybeStartDrag(
    TabSlotView* source,
...
void QoraiTabStrip::AddedToWidget() {
  ...
  if (BrowserView::GetBrowserViewForBrowser(GetBrowser())) {
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&QoraiTabStrip::UpdateTabContainer,
                                  weak_factory_.GetWeakPtr()));
  }
}

std::optional<int> QoraiTabStrip::GetCustomBackgroundId(
...
void QoraiTabStrip::SetCustomTitleForTab(
    Tab* tab,
    const std::optional<std::u16string>& title) {
  auto index = GetModelIndexOf(tab);
  CHECK(index);

  static_cast<QoraiBrowserTabStripController*>(controller_.get())
      ->SetCustomTitleForTab(*index, title);
}

void QoraiTabStrip::EnterTabRenameModeAt(int index) {
  auto* tab = tab_at(index);
  CHECK(tab);
  static_cast<QoraiTab*>(tab)->EnterRenameMode();
}

void QoraiTabStrip::UpdateTabContainer() {
  ...
  const bool is_using_compound_tab_container =
      views::IsViewClass<QoraiCompoundTabContainer>(
  ...
  if (should_use_compound_tab_container != is_using_compound_tab_container) {
    ...
    if (should_use_compound_tab_container) {
      ...
      auto* qorai_tab_container = AddChildViewAt(
          std::make_unique<QoraiCompoundTabContainer>(
              *this, hover_card_controller_.get(), drag_context, *this, this),
          0);
      tab_container_ = *qorai_tab_container;
      layout_lock =
          base::ScopedClosureRunner(qorai_tab_container->LockLayout());

      qorai_tab_container->SetScrollEnabled(using_vertical_tabs);
      ...
      auto* qorai_tab_container =
          AddChildViewAt(std::make_unique<QoraiTabContainer>(
...
bool QoraiTabStrip::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(GetBrowser());
}

TabContainer* QoraiTabStrip::GetTabContainerForTesting() {
  return &tab_container_.get();  // IN-TEST
}

void QoraiTabStrip::Layout(PassKey) {
  if (ShouldShowVerticalTabs()) {
