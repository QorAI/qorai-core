...
class QoraiTabContextMenuContentsTest : public InProcessBrowserTest {
 public:
  QoraiTabContextMenuContentsTest() = default;
  ~QoraiTabContextMenuContentsTest() override = default;

 protected:
  std::unique_ptr<QoraiTabContextMenuContents> CreateMenuAt(int tab_index) {
    TabStrip* tabstrip =
...
IN_PROC_BROWSER_TEST_F(QoraiTabContextMenuContentsTest, Basics) {
  auto menu = CreateMenuAt(0);

  // All items are disable state when there is only one tab.
  EXPECT_FALSE(menu->IsCommandIdEnabled(QoraiTabMenuModel::CommandRestoreTab));
  EXPECT_FALSE(
      menu->IsCommandIdEnabled(QoraiTabMenuModel::CommandBookmarkAllTabs));
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("qorai://version/")));
  chrome::CloseTab(browser());
  EXPECT_TRUE(menu->IsCommandIdEnabled(QoraiTabMenuModel::CommandRestoreTab));
  EXPECT_FALSE(
      menu->IsCommandIdEnabled(QoraiTabMenuModel::CommandBookmarkAllTabs));
}

IN_PROC_BROWSER_TEST_F(QoraiTabContextMenuContentsTest,
                       BringAllTabsToThisWindow_VisibleWhenOtherBrowserExists) {
  auto menu = CreateMenuAt(0);
  auto is_command_visible = [&]() {
    return menu->IsCommandIdVisible(
        QoraiTabMenuModel::CommandBringAllTabsToThisWindow);
  };
...
IN_PROC_BROWSER_TEST_F(QoraiTabContextMenuContentsTest,
  ...
  ASSERT_TRUE(menu->IsCommandIdVisible(
      QoraiTabMenuModel::CommandBringAllTabsToThisWindow));
  menu->ExecuteCommand(QoraiTabMenuModel::CommandBringAllTabsToThisWindow,
                       /*event_flags=*/0);
