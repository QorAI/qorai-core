...
namespace {
void QoraiUpdateContextMenu(ui::SimpleMenuModel* menu_contents, GURL url) {
...
QoraiOmniboxViewViews::~QoraiOmniboxViewViews() = default;

std::optional<GURL> QoraiOmniboxViewViews::GetURLToCopy() {
  GURL url;
...
bool QoraiOmniboxViewViews::SelectedTextIsURL() {
  return GetURLToCopy().has_value();
}

void QoraiOmniboxViewViews::CleanAndCopySelectedURL() {
...
void QoraiOmniboxViewViews::CopySanitizedURL(const GURL& url) {
  OnBeforePossibleChange();
  qorai::CopySanitizedURL(chrome::FindLastActive(), url);
  OnAfterPossibleChange(true);
}

#if BUILDFLAG(IS_WIN)
bool QoraiOmniboxViewViews::AcceleratorPressed(
    const ui::Accelerator& accelerator) {
  if (!base::FeatureList::IsEnabled(features::kQoraiCopyCleanLinkByDefault)) {
...
bool QoraiOmniboxViewViews::GetAcceleratorForCommandId(
    int command_id,
    ui::Accelerator* accelerator) const {
  if (base::FeatureList::IsEnabled(features::kQoraiCopyCleanLinkByDefault)) {
    bool is_url = const_cast<QoraiOmniboxViewViews*>(this)->SelectedTextIsURL();
    if (is_url) {
      if (command_id == kCopy) {
        return false;
      }
...
void QoraiOmniboxViewViews::ExecuteTextEditCommand(
...
void QoraiOmniboxViewViews::ExecuteCommand(int command_id, int event_flags) {
...
QoraiOmniboxViewViews::GetClipboardTextForPasteAndSearch() {
...
void QoraiOmniboxViewViews::UpdateContextMenu(
    ui::SimpleMenuModel* menu_contents) {
  OmniboxViewViews::UpdateContextMenu(menu_contents);
  auto url_to_copy = GetURLToCopy();
  if (url_to_copy.has_value()) {
    QoraiUpdateContextMenu(menu_contents, url_to_copy.value());
  }
}

BEGIN_METADATA(QoraiOmniboxViewViews)
END_METADATA
