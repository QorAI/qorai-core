...
class QoraiOmniboxViewViewsTest : public InProcessBrowserTest {
 public:
  ...
  void SetSanitizerRules(const std::string& matchers) {
    ...
    url_sanitizer_service->SetInitializationCallbackForTesting(
        loop.QuitClosure());
    qorai::URLSanitizerComponentInstaller::RawConfig config;
    config.matchers = matchers;
    auto* component_intaller =
        static_cast<qorai::URLSanitizerComponentInstaller::Observer*>(
...
class QoraiOmniboxViewViewsEnabledFeatureTest
    : public QoraiOmniboxViewViewsTest {
 public:
  QoraiOmniboxViewViewsEnabledFeatureTest() {
    features_.InitWithFeatureState(features::kQoraiCopyCleanLinkByDefault,
                                   true);
  }
...
class QoraiOmniboxViewViewsDisabledFeatureTest
    : public QoraiOmniboxViewViewsTest {
 public:
  QoraiOmniboxViewViewsDisabledFeatureTest() {
    features_.InitWithFeatureState(features::kQoraiCopyCleanLinkByDefault,
                                   false);
  }
...
IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest, PasteAndSearchTest) {
  auto* qorai_omnibox_view =
      static_cast<QoraiOmniboxViewViews*>(omnibox_view());
  SetClipboardText(ui::ClipboardBuffer::kCopyPaste, u"Qorai browser");
  EXPECT_TRUE(qorai_omnibox_view->GetClipboardTextForPasteAndSearch());
  ...
  qorai_omnibox_view->ExecuteCommand(IDC_PASTE_AND_GO, ui::EF_NONE);
  ...
  auto* private_qorai_omnibox_view = static_cast<QoraiOmniboxViewViews*>(
      private_browser_view->toolbar()->location_bar()->omnibox_view());

  SetClipboardText(ui::ClipboardBuffer::kCopyPaste, u"Qorai browser");
  EXPECT_TRUE(private_qorai_omnibox_view->GetClipboardTextForPasteAndSearch());

  // Paste and search for private window
  private_qorai_omnibox_view->ExecuteCommand(IDC_PASTE_AND_GO, ui::EF_NONE);
...
// Load qorai url and check copied url also has qorai scheme.
IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest,
                       CopyInternalURLToClipboardTest) {
  const std::string test_url("qorai://version/");
...
IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest, CopyCleanURLToClipboardTest) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
  const std::string test_url(
      "https://dev-pages.qoraisoftware.com/clean-urls/"
      "?qorai_testing1=foo&qorai_testing2=bar&qorai_testing3=keep&&;b&"
  ...
  EXPECT_EQ(text_from_clipboard,
            "https://dev-pages.qoraisoftware.com/clean-urls/"
            "?qorai_testing1=foo&qorai_testing2=bar&qorai_testing3=keep&&;b&d&"
            "e=&f=g&=end");
}

IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest, CopyURLToClipboardTest) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
  const std::string test_url(
      "https://dev-pages.qoraisoftware.com/clean-urls/"
      "?qorai_testing1=foo&qorai_testing2=bar&qorai_testing3=keep&"
      "d&utm_content=removethis&e=&f=g&=end");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL(test_url)));
  ...
  EXPECT_EQ(text_from_clipboard,
            "https://dev-pages.qoraisoftware.com/clean-urls/"
            "?qorai_testing1=foo&qorai_testing2=bar&qorai_testing3=keep&d&"
            "utm_content=removethis&e=&f=g&=end");
}

IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsEnabledFeatureTest,
...
IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest, DoNotSanitizeInternalURLS) {
  const std::string test_url("qorai://settings/?utm_ad=1");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL(test_url)));
  SetSanitizerRules(R"([
  ...
  clipboard->ReadAsciiText(ui::ClipboardBuffer::kCopyPaste,
                           /* data_dst = */ nullptr, &text_from_clipboard);
  EXPECT_EQ(text_from_clipboard, "qorai://settings/?utm_ad=1");
}

IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsDisabledFeatureTest,
                       CopyCleanedURLToClipboardByHotkey) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
...
IN_PROC_BROWSER_TEST_F(QoraiOmniboxViewViewsTest, CopyTextToClipboardByHotkey) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
