...
using qorai_search_conversion::SetDismissed;
using qorai_search_conversion::SetMaybeLater;
 ...
 public:
  explicit CloseButton(PressedCallback callback = PressedCallback())
    ...
    GetViewAccessibility().SetRoleDescription(l10n_util::GetStringUTF16(
        IDS_ACC_QORAI_SEARCH_CONVERSION_DISMISS_BUTTON));
  }
...
class QoraiOmniboxResultSelectionIndicator : public views::View {
  METADATA_HEADER(QoraiOmniboxResultSelectionIndicator, views::View)
 public:

  static constexpr int kStrokeThickness = 3;

  explicit QoraiOmniboxResultSelectionIndicator(
      QoraiSearchConversionPromotionView* parent_view)
 ...
 private:
  ...
  const raw_ptr<QoraiSearchConversionPromotionView> parent_view_;
...
BEGIN_METADATA(QoraiOmniboxResultSelectionIndicator)
END_METADATA

}  // namespace

QoraiSearchConversionPromotionView::QoraiSearchConversionPromotionView(
    QoraiOmniboxResultView* result_view,
    PrefService* local_state,
    PrefService* profile_prefs,
    TemplateURLService* template_url_service)
    : result_view_(result_view),
      mouse_enter_exit_handler_(base::BindRepeating(
          &QoraiSearchConversionPromotionView::UpdateHoverState,
...
void QoraiSearchConversionPromotionView::SetTypeAndInput(
    qorai_search_conversion::ConversionType type,
  ...
  qorai_search_conversion::p3a::RecordPromoShown(local_state_, type);
}

void QoraiSearchConversionPromotionView::OnSelectionStateChanged(
    bool selected) {
  selected_ = selected;
  UpdateState();
}

void QoraiSearchConversionPromotionView::OpenMatch() {
  qorai_search_conversion::p3a::RecordPromoTrigger(local_state_, type_);
  result_view_->OpenMatch();
}

void QoraiSearchConversionPromotionView::Dismiss() {
  SetDismissed(profile_prefs_);
  result_view_->RefreshOmniboxResult();
}

void QoraiSearchConversionPromotionView::MaybeLater() {
  SetMaybeLater(profile_prefs_);
  result_view_->RefreshOmniboxResult();
}

void QoraiSearchConversionPromotionView::UpdateState() {
...
void QoraiSearchConversionPromotionView::ConfigureForBannerType() {
  if (banner_type_container_) {
    return;
  }
  ...
  auto* close_button = banner_type_container_->AddChildView(
      std::make_unique<CloseButton>(views::Button::PressedCallback(
          base::BindRepeating(&QoraiSearchConversionPromotionView::Dismiss,
                              base::Unretained(this)))));
  views::SetImageFromVectorIconWithColor(
      close_button, kQoraCloseIcon,
  ...
  close_button->SetTooltipText(l10n_util::GetStringUTF16(
      IDS_QORAI_SEARCH_CONVERSION_CLOSE_BUTTON_TOOLTIP));
}

std::unique_ptr<views::View>
QoraiSearchConversionPromotionView::GetPrimaryButton() {
  auto primary_button = std::make_unique<CustomMdTextButton>(
      views::Button::PressedCallback(base::BindRepeating(
          &QoraiSearchConversionPromotionView::OnPrimaryButtonPressed,
          base::Unretained(this))));
  ...
  primary_button->SetText(l10n_util::GetStringUTF16(
      UseDDG() ? IDS_QORAI_SEARCH_CONVERSION_SET_AS_DEFAULT_BUTTON_LABEL
               : IDS_QORAI_SEARCH_CONVERSION_TRY_BUTTON_LABEL));
...
std::unique_ptr<views::View>
QoraiSearchConversionPromotionView::GetSecondaryButton() {
  auto secondary_button = std::make_unique<CustomMdTextButton>(
      views::Button::PressedCallback(base::BindRepeating(
          &QoraiSearchConversionPromotionView::OnSecondaryButtonPressed,
          base::Unretained(this))));
  ...
  secondary_button->SetText(l10n_util::GetStringUTF16(
      UseDDG() ? IDS_QORAI_SEARCH_CONVERSION_TRY_BUTTON_LABEL
               : IDS_QORAI_SEARCH_CONVERSION_MAYBE_LATER_BUTTON_LABEL));
  ...
  secondary_button->SetTooltipText(l10n_util::GetStringUTF16(
      UseDDG() ? IDS_QORAI_SEARCH_CONVERSION_TRY_BUTTON_LABEL
               : IDS_QORAI_SEARCH_CONVERSION_MAYBE_LATER_BUTTON_TOOLTIP));
  return secondary_button;
}

void QoraiSearchConversionPromotionView::OnPrimaryButtonPressed() {
  if (UseDDG()) {
    SetQoraiAsDefault();
  }
...
void QoraiSearchConversionPromotionView::OnSecondaryButtonPressed() {
  if (UseDDG()) {
    OpenMatch();
    return;
  }
...
void QoraiSearchConversionPromotionView::SetQoraiAsDefault() {
...
SkColor QoraiSearchConversionPromotionView::GetCloseButtonColor() const {
...
gfx::Size QoraiSearchConversionPromotionView::CalculatePreferredSize(
...
void QoraiSearchConversionPromotionView::OnThemeChanged() {
  View::OnThemeChanged();

  UpdateState();
}

int QoraiSearchConversionPromotionView::
...
void QoraiSearchConversionPromotionView::UpdateHoverState() {
  UpdateState();
}

int QoraiSearchConversionPromotionView::GetBannerTypeTitleStringResourceId() {
  switch (type_) {
    case ConversionType::kBannerTypeA:
      NOTREACHED();
    case ConversionType::kBannerTypeB:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_B_TITLE;
    case ConversionType::kBannerTypeC:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_TITLE;
    case ConversionType::kBannerTypeD:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_TITLE;
    case ConversionType::kDDGBannerTypeC:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_DDG_TITLE;
    case ConversionType::kDDGBannerTypeD:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_DDG_TITLE;
...
int QoraiSearchConversionPromotionView::GetBannerTypeDescStringResourceId() {
  switch (type_) {
    case ConversionType::kBannerTypeA:
      NOTREACHED();
    case ConversionType::kBannerTypeB:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_B_DESC;
    case ConversionType::kBannerTypeC:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_DESC;
    case ConversionType::kBannerTypeD:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_DESC;
    case ConversionType::kDDGBannerTypeC:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_DDG_DESC;
    case ConversionType::kDDGBannerTypeD:
      return IDS_QORAI_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_DDG_DESC;
    case ConversionType::kNone:
    case ConversionType::kButton:
    case ConversionType::kNTP:
      break;
...
bool QoraiSearchConversionPromotionView::UseDDG() const {
  return (type_ == ConversionType::kDDGBannerTypeC ||
          type_ == ConversionType::kDDGBannerTypeD);
}

bool QoraiSearchConversionPromotionView::ShouldDrawGraphic() const {
  if (!UseDDG()) {
    return true;
  }
...
std::optional<int> QoraiSearchConversionPromotionView::GetBackgroundGraphic()
    const {
  if (!ShouldDrawGraphic()) {
    return std::nullopt;
  }
  ...
  if (!UseDDG()) {
    return use_dark ? IDR_QORAI_SEARCH_CONVERSION_BANNER_GRAPHIC_DARK
                    : IDR_QORAI_SEARCH_CONVERSION_BANNER_GRAPHIC;
  }

  return use_dark ? IDR_QORAI_SEARCH_CONVERSION_BANNER_GRAPHIC_DDG_DARK
                  : IDR_QORAI_SEARCH_CONVERSION_BANNER_GRAPHIC_DDG;
}

BEGIN_METADATA(QoraiSearchConversionPromotionView)
END_METADATA
