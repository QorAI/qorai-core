...
class QoraiBookmarkContextMenuTest : public testing::Test {
 public:
  QoraiBookmarkContextMenuTest() : model_(nullptr) {}
  ...
  void SetUp() override {
    ...
    qorai::bookmarks::prefs::RegisterProfilePrefs(prefs_->registry());
 ...
 protected:
  std::unique_ptr<QoraiBookmarkContextMenu> CreateBookmarkContextMenu() {
    std::vector<raw_ptr<const BookmarkNode, VectorExperimental>> nodes = {
        model_->bookmark_bar_node()};
    auto menu = std::make_unique<QoraiBookmarkContextMenu>(
...
TEST_F(QoraiBookmarkContextMenuTest, OverrideBookmarkAlwaysShowMenu) {
  ...
  EXPECT_EQ(submenu->GetMenuItemAt(2)->GetCommand(),
            IDC_QORAI_BOOKMARK_BAR_NTP);
  EXPECT_EQ(submenu->GetMenuItems().size(), 3u);
}

TEST_F(QoraiBookmarkContextMenuTest, AddShowAllBookmarksButtonMenu) {
  auto bookmark_context_menu = CreateBookmarkContextMenu();
  auto* menu = bookmark_context_menu->menu()->GetMenuItemByID(
...
TEST_F(QoraiBookmarkContextMenuTest, ShowAllBookmarksButtonMenuCheckedState) {
  auto bookmark_context_menu = CreateBookmarkContextMenu();
  ASSERT_TRUE(bookmark_context_menu->IsItemChecked(
      IDC_TOGGLE_ALL_BOOKMARKS_BUTTON_VISIBILITY));

  prefs_->SetBoolean(qorai::bookmarks::prefs::kShowAllBookmarksButton, false);
  EXPECT_FALSE(bookmark_context_menu->IsItemChecked(
      IDC_TOGGLE_ALL_BOOKMARKS_BUTTON_VISIBILITY));

  prefs_->SetBoolean(qorai::bookmarks::prefs::kShowAllBookmarksButton, true);
  EXPECT_TRUE(bookmark_context_menu->IsItemChecked(
      IDC_TOGGLE_ALL_BOOKMARKS_BUTTON_VISIBILITY));
}

TEST_F(QoraiBookmarkContextMenuTest,
