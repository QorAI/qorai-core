...
#include "qorai/browser/ui/views/qorai_javascript_tab_modal_dialog_view_views.h"
...
class SideBySideEnabledBrowserTest : public InProcessBrowserTest {
 public:
  ...
  QoraiBrowserView* qorai_browser_view() const {
    return QoraiBrowserView::From(
        BrowserView::GetBrowserViewForBrowser(browser()));
  }

  views::View* split_view_separator() const {
    return qorai_multi_contents_view()->resize_area_for_testing();
  }

  QoraiMultiContentsView* qorai_multi_contents_view() const {
    return static_cast<QoraiMultiContentsView*>(
        qorai_browser_view()->multi_contents_view());
  }

  BrowserNonClientFrameView* browser_non_client_frame_view() {
    return qorai_browser_view()->browser_widget()->GetFrameView();
  }
...
IN_PROC_BROWSER_TEST_F(SideBySideEnabledBrowserTest,
  ...
  auto* browser_view = qorai_browser_view();
  ...
  // Check corner radius.
  auto* multi_contents_view = qorai_multi_contents_view();
  ASSERT_TRUE(multi_contents_view);
  auto* start_contents_container_view =
      static_cast<QoraiContentsContainerView*>(
          multi_contents_view->contents_container_views_for_testing()[0]);
  auto* end_contents_container_view = static_cast<QoraiContentsContainerView*>(
  ...
  // Check borders.
  EXPECT_EQ(gfx::Insets(QoraiContentsContainerView::kBorderThickness),
...
IN_PROC_BROWSER_TEST_F(SideBySideEnabledBrowserTest, SelectTabTest) {
  ...
  EXPECT_TRUE(
      qorai_multi_contents_view()->mini_toolbar_for_testing(0)->GetVisible());
  EXPECT_TRUE(
      qorai_multi_contents_view()->mini_toolbar_for_testing(1)->GetVisible());

  // Check mini toolbar uses our menu model.
  qorai_multi_contents_view()->mini_toolbar_for_testing(0)->OpenSplitViewMenu();
  auto* menu_model =
      static_cast<SplitTabMenuModel*>(qorai_multi_contents_view()
...
IN_PROC_BROWSER_TEST_F(SideBySideWithRoundedCornersTest, ContentsShadowTest) {
  ...
  EXPECT_TRUE(qorai_browser_view()->contents_shadow_);
  ...
  EXPECT_FALSE(qorai_browser_view()->contents_shadow_);
...
IN_PROC_BROWSER_TEST_F(SideBySideWithRoundedCornersTest,
  ...
  auto* contents_container = qorai_browser_view()->contents_container();
  auto* contents_view = qorai_browser_view()->GetContentsView();

  // Check it has rounded corners.
  EXPECT_EQ(contents_container->layer()->rounded_corner_radii(),
            gfx::RoundedCornersF(QoraiContentsViewUtil::kBorderRadius));
  ...
  fullscreen_controller->set_is_tab_fullscreen_for_testing(true);
  qorai_browser_view()->UpdateWebViewRoundedCorners();
  EXPECT_EQ(contents_container->layer()->rounded_corner_radii(),
            gfx::RoundedCornersF());
  EXPECT_EQ(contents_view->layer()->rounded_corner_radii(),
            gfx::RoundedCornersF());
...
// Use for testing qorai split view and SideBySide together.
...
IN_PROC_BROWSER_TEST_F(SplitViewCommonBrowserTest, BookmarksBarVisibilityTest) {
  ...
  qorai::SetBookmarkState(qorai::BookmarkBarState::kNtp, prefs);
...
// https://github.com/qorai/qorai-browser/issues/48804
#if BUILDFLAG(IS_MAC)
...
IN_PROC_BROWSER_TEST_F(
  ...
  auto* dialog = new QoraiJavaScriptTabModalDialogViewViews(
