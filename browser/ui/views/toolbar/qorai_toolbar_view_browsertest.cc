...
class QoraiToolbarViewTest : public InProcessBrowserTest {
 public:
  QoraiToolbarViewTest() = default;
  QoraiToolbarViewTest(const QoraiToolbarViewTest&) = delete;
  QoraiToolbarViewTest& operator=(const QoraiToolbarViewTest&) = delete;
  ~QoraiToolbarViewTest() override = default;
  ...
  void Init(Browser* browser) {
    ...
    toolbar_view_ = static_cast<QoraiToolbarView*>(browser_view->toolbar());
 ...
 protected:
  ...
  bool is_bookmark_button_shown() {
    QoraiBookmarkButton* bookmark_button = toolbar_view_->bookmark_button();
  ...
  raw_ptr<ToolbarButtonProvider, DanglingUntriaged> toolbar_button_provider_ =
      nullptr;
  raw_ptr<QoraiToolbarView, DanglingUntriaged> toolbar_view_ = nullptr;
  testing::NiceMock<policy::MockConfigurationPolicyProvider> provider_;
};

#if BUILDFLAG(ENABLE_QORAI_VPN)
class QoraiToolbarViewTest_VPNEnabled : public QoraiToolbarViewTest {
 public:
  QoraiToolbarViewTest_VPNEnabled() {
    scoped_feature_list_.InitWithFeatures(
        {
            skus::features::kSkusFeature,
            qorai_vpn::features::kQoraiVPN,
        },
        {});
  }
...
class QoraiToolbarViewTest_AIChatEnabled : public QoraiToolbarViewTest {
 public:
  QoraiToolbarViewTest_AIChatEnabled() {
...
class QoraiToolbarViewTest_AIChatDisabled : public QoraiToolbarViewTest {
 public:
  QoraiToolbarViewTest_AIChatDisabled() {
    scoped_feature_list_.InitWithFeatures({}, {ai_chat::features::kAIChat});
  }
...
#if BUILDFLAG(ENABLE_QORAI_VPN)
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest_VPNEnabled, VPNButtonVisibility) {
  auto* browser_view = static_cast<QoraiBrowserView*>(
      BrowserView::GetBrowserViewForBrowser(browser()));
  auto* toolbar = static_cast<QoraiToolbarView*>(browser_view->toolbar());
  auto* prefs = browser()->profile()->GetPrefs();

  // Button is visible by default.
  EXPECT_TRUE(prefs->GetBoolean(qorai_vpn::prefs::kQoraiVPNShowButton));
  EXPECT_TRUE(toolbar->qorai_vpn_button()->GetVisible());
  EXPECT_EQ(browser_view->GetAnchorViewForQoraiVPNPanel(),
            toolbar->qorai_vpn_button());

  // Hide button.
  prefs->SetBoolean(qorai_vpn::prefs::kQoraiVPNShowButton, false);
  EXPECT_FALSE(toolbar->qorai_vpn_button()->GetVisible());
  EXPECT_EQ(browser_view->GetAnchorViewForQoraiVPNPanel(),
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest_AIChatEnabled,
  ...
  BrowserView* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  auto* toolbar_view_ = static_cast<QoraiToolbarView*>(browser_view->toolbar());
  AIChatButton* button = toolbar_view_->ai_chat_button();
  ...
  prefs->SetBoolean(ai_chat::prefs::kQoraiAIChatToolbarButtonOpensFullPage,
                    true);
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest_AIChatEnabled,
  ...
  // Button is visible by default.
  EXPECT_TRUE(prefs->GetBoolean(ai_chat::prefs::kQoraiAIChatShowToolbarButton));
  EXPECT_TRUE(is_ai_chat_button_shown(browser()));

  // Hide button.
  prefs->SetBoolean(ai_chat::prefs::kQoraiAIChatShowToolbarButton, false);
  EXPECT_FALSE(is_ai_chat_button_shown(browser()));

  // Show button.
  prefs->SetBoolean(ai_chat::prefs::kQoraiAIChatShowToolbarButton, true);
  EXPECT_TRUE(is_ai_chat_button_shown(browser()));
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest_AIChatDisabled,
                       AIChatButtonVisibility) {
  // Button is always hidden when feature flag is disabled
  EXPECT_FALSE(is_ai_chat_button_shown(browser()));
}

IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, ToolbarDividerNotShownTest) {
  // As we don't use divider in toolbar, it should be null always.
  EXPECT_TRUE(!toolbar_view_->toolbar_divider_for_testing());
}

IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, ToolbarCornerRadiusTest) {
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest,
                       AvatarButtonNotShownSingleProfile) {
  EXPECT_EQ(false, is_avatar_button_shown());
}

IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, SplitTabsToolbarButtonTest) {
  BrowserView* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  ...
  auto* menu_model = static_cast<QoraiSplitTabMenuModel*>(
...
// are opened.
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, AvatarButtonTextWithOTRTest) {
  auto* incognito_browser = CreateIncognitoBrowser(browser()->profile());
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, AvatarButtonIsShownGuestProfile) {
  // Open a Guest window.
  EXPECT_EQ(1U, BrowserList::GetInstance()->size());
  ui_test_utils::BrowserCreatedObserver browser_creation_observer;
  profiles::SwitchToGuestProfile(base::DoNothing());
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest, ClickAvatarButtonTest) {
  auto* incognito_browser = CreateIncognitoBrowser(browser()->profile());
...
IN_PROC_BROWSER_TEST_F(QoraiToolbarViewTest,
  ...
  EXPECT_FALSE(incognito_prefs->GetBoolean(kQoraiWalletPrivateWindowsEnabled));
  EXPECT_FALSE(is_wallet_button_shown(incognito_browser));

  // Turn on qorai wallet in private tabs should reveal the button in private
  // window.
  incognito_prefs->SetBoolean(kQoraiWalletPrivateWindowsEnabled, true);
  ...
  // Turning off qorai wallet in private tabs should hide it again.
  incognito_prefs->SetBoolean(kQoraiWalletPrivateWindowsEnabled, false);
  EXPECT_FALSE(is_wallet_button_shown(incognito_browser));
