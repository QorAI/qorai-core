...
class WalletButtonNotificationSourceTest : public InProcessBrowserTest {
 public:
  ...
  NetworkManager* network_manager() {
    return qorai_wallet_service()->network_manager();
  }
  ...
  JsonRpcService* json_rpc_service() {
    return qorai_wallet_service()->json_rpc_service();
  }

  TxService* tx_service() { return qorai_wallet_service()->tx_service(); }

  qorai_wallet::AccountUtils GetAccountUtils() {
    return qorai_wallet::AccountUtils(keyring_service());
  }

  void CreateWallet() {
    GetAccountUtils().CreateWallet(qorai_wallet::kMnemonicDripCaution,
                                   qorai_wallet::kTestWalletPassword);
  }
  ...
  void StartRPCServer(
    ...
    json_rpc_service()->SetNetwork(qorai_wallet::mojom::kLocalhostChainId,
                                   qorai_wallet::mojom::CoinType::SOL,
                                   std::nullopt);
    base::RunLoop run_loop;
    json_rpc_service()->GetNetwork(
        qorai_wallet::mojom::CoinType::SOL, std::nullopt,
        base::BindLambdaForTesting(
            [&](qorai_wallet::mojom::NetworkInfoPtr info) {
              chain = info.Clone();
              run_loop.Quit();
            }));
    run_loop.Run();
    ...
    json_rpc_service()->AddChain(
        ...
        base::BindLambdaForTesting([&](const std::string& chain_id,
          ...
          ASSERT_EQ(chain_id, qorai_wallet::mojom::kLocalhostChainId);
          ASSERT_EQ(error, qorai_wallet::mojom::ProviderError::kSuccess);
...
IN_PROC_BROWSER_TEST_F(WalletButtonNotificationSourceTest,
  ...
  std::string first_tx_meta_id;
  {
    ...
    auto tx_data = qorai_wallet::mojom::TxDataUnion::NewFilTxData(
        qorai_wallet::mojom::FilTxData::New(
            "" /* nonce */, "10" /* gas_premium */, "10" /* gas_fee_cap */,
            "100" /* gas_limit */, "" /* max_fee */, to_account, "11"));
    auto chain_id = network_manager()->GetCurrentChainId(
        qorai_wallet::mojom::CoinType::FIL, std::nullopt);
  ...
  std::string second_tx_meta_id;
  {
    ...
    auto params = qorai_wallet::mojom::NewEvmTransactionParams::New(
  ...
  std::string third_tx_meta_id;
  {
    ...
    std::vector<qorai_wallet::mojom::SolanaAccountMetaPtr> account_metas;
    auto account_meta1 = qorai_wallet::mojom::SolanaAccountMeta::New(
        from_account->address, nullptr, true, true);
    auto account_meta2 = qorai_wallet::mojom::SolanaAccountMeta::New(
    ...
    auto instruction = qorai_wallet::mojom::SolanaInstruction::New(
        qorai_wallet::mojom::kSolanaSystemProgramId, std::move(account_metas),
        data, nullptr);
    std::vector<qorai_wallet::mojom::SolanaInstructionPtr> instructions;
    instructions.push_back(std::move(instruction));
    auto tx_data = qorai_wallet::mojom::SolanaTxData::New(
        "", 0, from_account->address, to_account, "", 10000000, 0,
        qorai_wallet::mojom::TransactionType::SolanaSystemTransfer,
        std::move(instructions),
        qorai_wallet::mojom::SolanaMessageVersion::kLegacy,
        qorai_wallet::mojom::SolanaMessageHeader::New(1, 0, 1),
        std::vector<std::string>({from_account->address, to_account,
                                  qorai_wallet::mojom::kSolanaSystemProgramId}),
        std::vector<qorai_wallet::mojom::SolanaMessageAddressTableLookupPtr>(),
        nullptr, nullptr, nullptr);

    tx_service()->AddUnapprovedTransaction(
        qorai_wallet::mojom::TxDataUnion::NewSolanaTxData(std::move(tx_data)),
        network_manager()->GetCurrentChainId(qorai_wallet::mojom::CoinType::SOL,
                                             std::nullopt),
  ...
  // Reject first transaction
  {
    ...
    tx_service()->RejectTransaction(
        qorai_wallet::mojom::CoinType::FIL,
        network_manager()->GetCurrentChainId(qorai_wallet::mojom::CoinType::FIL,
                                             std::nullopt),
  ...
  // Reject second transaction
  {
    ...
    tx_service()->RejectTransaction(
        ...
        network_manager()->GetCurrentChainId(qorai_wallet::mojom::CoinType::ETH,
                                             std::nullopt),
