...
QoraiVPNPanelController::QoraiVPNPanelController(QoraiBrowserView* browser_view)
    : browser_view_(browser_view) {
  DCHECK(browser_view_);
}

QoraiVPNPanelController::~QoraiVPNPanelController() = default;

void QoraiVPNPanelController::ShowQoraiVPNPanel(bool show_select) {
  auto* anchor_view = browser_view_->GetAnchorViewForQoraiVPNPanel();
  if (!anchor_view)
    return;
...
void QoraiVPNPanelController::ResetBubbleManager() {
