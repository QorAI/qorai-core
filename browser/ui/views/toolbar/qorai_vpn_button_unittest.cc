...
class QoraiVpnButtonUnitTest : public testing::Test {
 public:
  QoraiVpnButtonUnitTest() = default;

  QoraiVpnButtonUnitTest(const QoraiVpnButtonUnitTest&) = delete;
  QoraiVpnButtonUnitTest& operator=(const QoraiVpnButtonUnitTest&) = delete;
  ...
  void SetUp() override {
    ...
    builder.AddTestingFactory(QoraiVpnServiceFactory::GetInstance(),
    ...
    manager->SetConnectionAPIImplForTesting(
        std::make_unique<ConnectionAPIImplSim>(manager.get(),
                                               shared_url_loader_factory_));
    TestingQoraiBrowserProcess::GetGlobal()
        ->SetQoraiVPNConnectionManagerForTesting(std::move(manager));
    ASSERT_TRUE(qorai_vpn::QoraiVpnServiceFactory::GetForProfile(
        GetBrowser()->profile()));
    ASSERT_TRUE(ThemeServiceFactory::GetForProfile(profile()));
    button_ = std::make_unique<QoraiVPNButton>(GetBrowser());
  }
  ...
  void TearDown() override {
    ...
    // QoraiVPNConnectionManager should be reset after profile is destoryed
    // and before local_state is gone as it uses local_state.
    TestingQoraiBrowserProcess::GetGlobal()
        ->SetQoraiVPNConnectionManagerForTesting(nullptr);
  ...
  std::unique_ptr<QoraiVPNButton> button_;
...
TEST_F(QoraiVpnButtonUnitTest, SkipAttemptsToConnectInFailedState) {
  EXPECT_FALSE(IsErrorState());
  FireVpnState(qorai_vpn::mojom::ConnectionState::CONNECT_FAILED);
  EXPECT_TRUE(IsErrorState());
  FireVpnState(qorai_vpn::mojom::ConnectionState::CONNECTING);
  EXPECT_TRUE(IsErrorState());
  FireVpnState(qorai_vpn::mojom::ConnectionState::DISCONNECTING);
  EXPECT_TRUE(IsErrorState());
  FireVpnState(qorai_vpn::mojom::ConnectionState::CONNECTED);
  EXPECT_FALSE(IsErrorState());

  FireVpnState(qorai_vpn::mojom::ConnectionState::CONNECT_FAILED);
  EXPECT_TRUE(IsErrorState());
  FireVpnState(qorai_vpn::mojom::ConnectionState::DISCONNECTED);
  EXPECT_FALSE(IsErrorState());
}

TEST_F(QoraiVpnButtonUnitTest, ButtonStateTestWithPurchasedState) {
