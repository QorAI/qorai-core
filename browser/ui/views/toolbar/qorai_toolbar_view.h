...
#ifndef QORAI_BROWSER_UI_VIEWS_TOOLBAR_QORAI_TOOLBAR_VIEW_H_
#define QORAI_BROWSER_UI_VIEWS_TOOLBAR_QORAI_TOOLBAR_VIEW_H_
...
class QoraiVPNButton;
#endif

class QoraiBookmarkButton;
class SidePanelButton;
class WalletButton;

class QoraiToolbarView : public ToolbarView,
                         public ProfileAttributesStorage::Observer {
  METADATA_HEADER(QoraiToolbarView, ToolbarView)
 public:
  class LayoutGuard;

  explicit QoraiToolbarView(Browser* browser, BrowserView* browser_view);
  ~QoraiToolbarView() override;

  QoraiBookmarkButton* bookmark_button() const { return bookmark_; }
...
#if BUILDFLAG(ENABLE_QORAI_VPN)
  QoraiVPNButton* qorai_vpn_button() const { return qorai_vpn_; }
 ...
 private:
  FRIEND_TEST_ALL_PREFIXES(QoraiToolbarViewTest, ToolbarDividerNotShownTest);
  ...
  raw_ptr<QoraiBookmarkButton> bookmark_ = nullptr;
...
#if BUILDFLAG(ENABLE_QORAI_VPN)
  raw_ptr<QoraiVPNButton> qorai_vpn_ = nullptr;
  BooleanPrefMember show_qorai_vpn_button_;
  BooleanPrefMember hide_qorai_vpn_button_by_policy_;
#endif
...
#endif  // QORAI_BROWSER_UI_VIEWS_TOOLBAR_QORAI_TOOLBAR_VIEW_H_
