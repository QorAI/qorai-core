...
#include "qorai/browser/ui/views/toolbar/qorai_app_menu.h"
...
#include "qorai/components/misc_metrics/menu_metrics.h"
...
class SidebarShowOptionInMenuButton : public views::LabelButton,
 ...
 public:
  SidebarShowOptionInMenuButton(QoraiAppMenu* app_menu,
...
SidebarShowOptionInMenuButton::SidebarShowOptionInMenuButton(
    ...
    : LabelButton(
      ...
      show_option_(QoraiAppMenuModel::ConvertIDCToSidebarShowOptions(
...
void SidebarShowOptionInMenuButton::PaintButtonContents(gfx::Canvas* canvas) {
  if (is_active_option_) {
    ...
    bounds.Inset(2);
    cc::PaintFlags flags;
    flags.setColor(cp->GetColor(kColorQoraiAppMenuAccentColor));
...
class SidebarShowOptionMenu : public views::BoxLayoutView {
 ...
 public:
  SidebarShowOptionMenu(QoraiAppMenu* app_menu, ui::ButtonMenuItemModel* model);
...
SidebarShowOptionMenu::SidebarShowOptionMenu(QoraiAppMenu* app_menu,
                                             ui::ButtonMenuItemModel* model)
    : model_(model) {
...
QoraiAppMenu::QoraiAppMenu(Browser* browser,
                           ui::MenuModel* model,
                           int run_types)
    : AppMenu(browser, model, run_types),
      menu_metrics_(
          g_qorai_browser_process->process_misc_metrics()->menu_metrics()) {
  DCHECK(menu_metrics_);
  UpdateMenuItemView();
}

QoraiAppMenu::~QoraiAppMenu() = default;

void QoraiAppMenu::RunMenu(views::MenuButtonController* host) {
  AppMenu::RunMenu(host);
  menu_metrics_->RecordMenuShown();
}

void QoraiAppMenu::ExecuteCommand(int command_id, int mouse_event_flags) {
  // Suspect that the entry is null but can't imagine which command causes it.
  // See
  // https://github.com/qorai/qorai-browser/issues/37862#issuecomment-2078553575
  if (!IsBookmarkCommand(command_id) && !IsTabGroupsCommand(command_id) &&
    ...
    SCOPED_CRASH_KEY_NUMBER("QoraiAppMenu", "command_id", command_id);
...
void QoraiAppMenu::OnMenuClosed(views::MenuItemView* menu) {
...
void QoraiAppMenu::RecordMenuUsage(int command_id) {
  ...
  switch (command_id) {
    ...
    case IDC_SHOW_QORAI_REWARDS:
      group = misc_metrics::MenuGroup::kQoraiFeatures;
      break;
...
void QoraiAppMenu::UpdateMenuItemView() {
#if BUILDFLAG(ENABLE_QORAI_VPN)
  CHECK(root_menu_item());
  if (auto* menu_item =
          root_menu_item()->GetMenuItemByID(IDC_TOGGLE_QORAI_VPN)) {
    menu_item->AddChildView(std::make_unique<QoraiVPNStatusLabel>(browser_));
    menu_item->AddChildView(std::make_unique<QoraiVPNToggleButton>(browser_));
  }
#endif
