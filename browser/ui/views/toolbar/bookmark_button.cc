...
QoraiBookmarkButton::QoraiBookmarkButton(PressedCallback callback)
    : ToolbarButton(std::move(callback)) {
  SetID(VIEW_ID_STAR_BUTTON);
  set_tag(IDC_BOOKMARK_THIS_TAB);
}

QoraiBookmarkButton::~QoraiBookmarkButton() = default;

void QoraiBookmarkButton::SetToggled(bool on) {
  active_ = on;
  UpdateImageAndText();
}

void QoraiBookmarkButton::UpdateImageAndText() {
...
BEGIN_METADATA(QoraiBookmarkButton)
END_METADATA
