...
#include "qorai/browser/ui/views/qorai_first_run_dialog.h"
...
void ShowQoraiFirstRunDialogViews() {
  base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
  QoraiFirstRunDialog::Show(run_loop.QuitClosure());
  run_loop.Run();
}
...
// static
void QoraiFirstRunDialog::Show(base::RepeatingClosure quit_runloop) {
  QoraiFirstRunDialog* dialog =
      new QoraiFirstRunDialog(std::move(quit_runloop));
  views::DialogDelegate::CreateDialogWidget(dialog, gfx::NativeWindow(),
                                            gfx::NativeView())
      ->Show();
}

QoraiFirstRunDialog::QoraiFirstRunDialog(base::RepeatingClosure quit_runloop)
...
QoraiFirstRunDialog::~QoraiFirstRunDialog() = default;

void QoraiFirstRunDialog::Done() {
  CHECK(!quit_runloop_.is_null());
  quit_runloop_.Run();
}

bool QoraiFirstRunDialog::Accept() {
  GetWidget()->Hide();

#if BUILDFLAG(ENABLE_PIN_SHORTCUT)
  base::MakeRefCounted<shell_integration::QoraiDefaultBrowserWorker>()
      ->StartSetAsDefault(base::BindOnce(
          [](bool pin_to_shortcut,
             shell_integration::DefaultWebClientState state) {
            if (pin_to_shortcut &&
                state == shell_integration::DefaultWebClientState::IS_DEFAULT) {
              // Try to pin to taskbar when Qorai is set as a default browser.
...
void QoraiFirstRunDialog::WindowClosing() {
  Done();
}

BEGIN_METADATA(QoraiFirstRunDialog)
END_METADATA
