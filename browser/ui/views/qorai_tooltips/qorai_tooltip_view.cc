...
QoraiTooltipView::QoraiTooltipView(
    QoraiTooltipPopup* tooltip_popup,
...
QoraiTooltipView::~QoraiTooltipView() = default;

bool QoraiTooltipView::OnMousePressed(const ui::MouseEvent& event) {
  initial_mouse_pressed_location_ = event.location();

  return true;
}

bool QoraiTooltipView::OnMouseDragged(const ui::MouseEvent& event) {
...
void QoraiTooltipView::OnMouseReleased(const ui::MouseEvent& event) {
  if (is_dragging_) {
    is_dragging_ = false;
    return;
  }
...
void QoraiTooltipView::OnDeviceScaleFactorChanged(
...
void QoraiTooltipView::OnThemeChanged() {
...
void QoraiTooltipView::CreateView() {
...
views::View* QoraiTooltipView::CreateHeaderView() {
...
views::ImageView* QoraiTooltipView::CreateIconView() {
  views::ImageView* view = new views::ImageView();

  view->SetImage(ui::ImageModel::FromImageSkia(
      gfx::CreateVectorIcon(kQoraiTooltipsStopwatchIcon, kIconColor)));

  return view;
}

views::Label* QoraiTooltipView::CreateTitleLabel() {
...
void QoraiTooltipView::UpdateTitleLabelColors() {
  if (!title_label_) {
    return;
  }
...
views::View* QoraiTooltipView::CreateButtonView() {
  views::View* view = new views::View();
...
views::View* QoraiTooltipView::CreateBodyView() {
  views::View* view = new views::View();
...
views::Label* QoraiTooltipView::CreateBodyLabel() {
...
void QoraiTooltipView::UpdateBodyLabelColors() {
  if (!body_label_) {
    return;
  }
...
QoraiTooltipLabelButton* QoraiTooltipView::CreateOkButton() {
  auto* button = new QoraiTooltipLabelButton(
      base::BindRepeating(&QoraiTooltipView::OnOkButtonPressed,
...
void QoraiTooltipView::UpdateOkButtonColors() {
  if (!ok_button_) {
    return;
  }
...
void QoraiTooltipView::OnOkButtonPressed() {
  if (tooltip_popup_) {
    tooltip_popup_->OnOkButtonPressed();
  }
}

QoraiTooltipLabelButton* QoraiTooltipView::CreateCancelButton() {
  auto* button = new QoraiTooltipLabelButton(
      base::BindRepeating(&QoraiTooltipView::OnCancelButtonPressed,
                          base::Unretained(this)),
...
void QoraiTooltipView::UpdateCancelButtonColors() {
  if (!cancel_button_) {
    return;
  }
...
void QoraiTooltipView::OnCancelButtonPressed() {
  if (tooltip_popup_) {
    tooltip_popup_->OnCancelButtonPressed();
  }
}

BEGIN_METADATA(QoraiTooltipView)
END_METADATA
