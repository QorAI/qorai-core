...
#include "qorai/browser/ui/qorai_tooltips/bounds_util.h"
...
QoraiTooltipPopup::QoraiTooltipPopup(std::unique_ptr<QoraiTooltip> tooltip)
  ...
  GetViewAccessibility().SetRoleDescription(l10n_util::GetStringUTF8(
      IDS_QORAI_TOOLTIPS_QORAI_TOOLTIP_ACCESSIBLE_NAME));
...
QoraiTooltipPopup::~QoraiTooltipPopup() {
...
void QoraiTooltipPopup::Show() {
  GetWidget()->ShowInactive();

  QoraiTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
    delegate->OnTooltipShow(tooltip_->id());
  }
}

void QoraiTooltipPopup::Close() {
  QoraiTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
...
void QoraiTooltipPopup::CloseWidget() {
  CloseWidgetView();
}

void QoraiTooltipPopup::OnOkButtonPressed() {
...
void QoraiTooltipPopup::OnCancelButtonPressed() {
...
void QoraiTooltipPopup::set_normalized_display_coordinates(double x, double y) {
  normalized_display_coordinate_x_ = x;
  normalized_display_coordinate_y_ = y;
}

void QoraiTooltipPopup::set_display_work_area_insets(int x, int y) {
  display_work_area_inset_x_ = x;
  display_work_area_inset_y_ = y;
}

void QoraiTooltipPopup::OnDisplaysRemoved(
    const display::Displays& old_displays) {
  // Called when |old_display| has been removed
  RecomputeAlignment();
}

void QoraiTooltipPopup::OnDisplayMetricsChanged(const display::Display& display,
                                                uint32_t changed_metrics) {
  // Called when the metrics of a display change
  RecomputeAlignment();
}

void QoraiTooltipPopup::OnDisplayChanged() {
  // Called with the display changes (color depth or resolution)
  RecomputeAlignment();
}

void QoraiTooltipPopup::OnWorkAreaChanged() {
  // Called when the work area (the desktop area minus task bars, menu bars,
  // etc.) changes in size
  RecomputeAlignment();
}

void QoraiTooltipPopup::OnPaintBackground(gfx::Canvas* canvas) {
  DCHECK(canvas);
...
void QoraiTooltipPopup::OnThemeChanged() {
  views::View::OnThemeChanged();

  SchedulePaint();
}

void QoraiTooltipPopup::OnWidgetCreated(views::Widget* widget) {
  DCHECK(widget);
...
void QoraiTooltipPopup::OnWidgetDestroyed(views::Widget* widget) {
  DCHECK(widget);
...
void QoraiTooltipPopup::OnWidgetBoundsChanged(views::Widget* widget,
...
void QoraiTooltipPopup::CreatePopup() {
  ...
  tooltip_view_ =
      AddChildView(new QoraiTooltipView(this, tooltip_->attributes()));

  CreateWidgetView();
}

gfx::Point QoraiTooltipPopup::GetDefaultOriginForSize(const gfx::Size& size) {
...
gfx::Rect QoraiTooltipPopup::CalculateBounds(bool use_default_origin) {
  DCHECK(tooltip_view_);
...
void QoraiTooltipPopup::RecomputeAlignment() {
  if (!IsWidgetValid()) {
    return;
  }
...
const gfx::ShadowDetails& QoraiTooltipPopup::GetShadowDetails() const {
  return gfx::ShadowDetails::Get(kShadowElevation, kCornerRadius);
}

gfx::Insets QoraiTooltipPopup::GetShadowMargin() const {
  const gfx::ShadowDetails& shadow_details = GetShadowDetails();
...
void QoraiTooltipPopup::CreateWidgetView() {
...
void QoraiTooltipPopup::CloseWidgetView() {
  if (!GetWidget()) {
    DeleteDelegate();
    return;
  }
...
bool QoraiTooltipPopup::IsWidgetValid() const {
  return GetWidget() && !GetWidget()->IsClosed();
}

BEGIN_METADATA(QoraiTooltipPopup)
END_METADATA
