 ...
 public:
  ...
  bool ContainsDeleteLocalFileCommand() {
    ...
    return std::ranges::find(
               quick_actions, QoraiDownloadCommands::DELETE_LOCAL_FILE,
...
TEST_F(DownloadBubbleTest, ContextMenuCompletedItemTest) {
  ...
  EXPECT_TRUE(
      menu_model->GetIndexOfCommandId(QoraiDownloadCommands::REMOVE_FROM_LIST));
}
...
TEST_F(DownloadBubbleTest, QoraiDownloadCommands_DeleteLocalFileEnabled) {
  SetupDownloadItemDefaults();
  SetupCompletedDownloadItem();
  ...
  QoraiDownloadCommands commands(model_.GetWeakPtr());
  EXPECT_TRUE(
      commands.IsCommandEnabled(QoraiDownloadCommands::DELETE_LOCAL_FILE));
}

TEST_F(DownloadBubbleTest,
       QoraiDownloadCommands_DeleteLocalFileDisabledWhenFullPathEmpty) {
  SetupDownloadItemDefaults();
  SetupCompletedDownloadItem();
...
TEST_F(DownloadBubbleTest,
       QoraiDownloadCommands_DeleteLocalFileDisabledWhenExternallyRemoved) {
  SetupDownloadItemDefaults();
  SetupCompletedDownloadItem();
