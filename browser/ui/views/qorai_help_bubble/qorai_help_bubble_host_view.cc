...
[[clang::no_destroy]] sk_sp<cc::PaintShader> kQoraiGradient =
...
QoraiHelpBubbleHostView::QoraiHelpBubbleHostView() {
...
QoraiHelpBubbleHostView::~QoraiHelpBubbleHostView() = default;

bool QoraiHelpBubbleHostView::Show() {
  if (help_bubble_) {
    return false;
  }
  ...
  auto* qorai_help_bubble_delegate_view =
      new QoraiHelpBubbleDelegateView(this, text_);
  help_bubble_ = views::BubbleDialogDelegateView::CreateBubble(
      qorai_help_bubble_delegate_view);
  auto* frame_view = qorai_help_bubble_delegate_view->GetBubbleFrameView();
  ...
  activated_subscription_ =
      ui::ElementTracker::GetElementTracker()->AddElementActivatedCallback(
          ...
          base::BindRepeating(
              &QoraiHelpBubbleHostView::OnTrackedElementActivated,
  ...
  bubble_auto_closing_timer_.Start(
      ...
      base::BindOnce(&QoraiHelpBubbleHostView::Hide,
                     weak_factory_.GetWeakPtr()));
...
void QoraiHelpBubbleHostView::Hide() {
  if (!help_bubble_) {
    return;
  }
...
void QoraiHelpBubbleHostView::UpdatePosition() {
...
void QoraiHelpBubbleHostView::OnPaint(gfx::Canvas* canvas) {
  cc::PaintFlags flags;
  flags.setAntiAlias(true);
  flags.setStyle(cc::PaintFlags::kStroke_Style);
  flags.setShader(kQoraiGradient);
...
void QoraiHelpBubbleHostView::OnViewBoundsChanged(views::View* observed_view) {
  // Update host view position when |tracked_element_| bounds is changed.
  CHECK(tracked_element_ == observed_view);
  UpdatePosition();
}

void QoraiHelpBubbleHostView::OnViewIsDeleting(views::View* observed_view) {
  CHECK(tracked_element_ == observed_view);
  tracked_element_ = nullptr;
  tracked_view_observation_.Reset();
}

void QoraiHelpBubbleHostView::OnViewVisibilityChanged(
...
void QoraiHelpBubbleHostView::OnWidgetBoundsChanged(views::Widget* widget,
                                                    const gfx::Rect&) {
...
void QoraiHelpBubbleHostView::OnWidgetDestroying(views::Widget* widget) {
  if (help_bubble_ == widget) {
...
void QoraiHelpBubbleHostView::OnWidgetActivationChanged(views::Widget* widget,
                                                        bool active) {
  if (help_bubble_ != widget) {
    return;
  }
...
void QoraiHelpBubbleHostView::OnTrackedElementActivated(
    ui::TrackedElement* element) {
  Hide();
}

BEGIN_METADATA(QoraiHelpBubbleHostView)
END_METADATA
