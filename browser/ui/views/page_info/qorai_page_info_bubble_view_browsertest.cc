...
QoraiPageInfoBubbleView* GetBubbleView() {
  return views::AsViewClass<QoraiPageInfoBubbleView>(
      PageInfoBubbleView::GetPageInfoBubbleForTesting());
}
...
class QoraiPageInfoBubbleViewBrowserTestBase : public InProcessBrowserTest {
 public:
  QoraiPageInfoBubbleViewBrowserTestBase() = default;
  ~QoraiPageInfoBubbleViewBrowserTestBase() override = default;
 ...
 protected:
  ...
  bool IsShieldsViewDrawn(views::View* bubble_view) {
    auto* shields_view = bubble_view->GetViewByID(
        static_cast<int>(QoraiPageInfoViewID::kShieldsWebView));
    return shields_view && shields_view->IsDrawn();
  }
};

class QoraiPageInfoBubbleViewBrowserTest
    : public QoraiPageInfoBubbleViewBrowserTestBase {
 public:
  QoraiPageInfoBubbleViewBrowserTest() {
    feature_list_.InitAndEnableFeature(
        page_info::features::kShowQoraiShieldsInPageInfo);
  }

  ~QoraiPageInfoBubbleViewBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(QoraiPageInfoBubbleViewBrowserTest, CloseButtonHidden) {
...
IN_PROC_BROWSER_TEST_F(QoraiPageInfoBubbleViewBrowserTest,
...
IN_PROC_BROWSER_TEST_F(QoraiPageInfoBubbleViewBrowserTest, TabSwitching) {
  NavigateAndOpenBubble();
  auto* bubble_view = GetBubbleView();
  ASSERT_TRUE(bubble_view);
  ...
  EXPECT_TRUE(bubble_view->GetViewByID(
      static_cast<int>(QoraiPageInfoViewID::kTabSwitcherShieldsButton)));
  EXPECT_TRUE(bubble_view->GetViewByID(
      static_cast<int>(QoraiPageInfoViewID::kTabSwitcherSiteSettingsButton)));
...
class QoraiPageInfoBubbleViewFlagDisabledBrowserTest
    : public QoraiPageInfoBubbleViewBrowserTestBase {
 public:
  QoraiPageInfoBubbleViewFlagDisabledBrowserTest() {
    feature_list_.InitAndDisableFeature(
        page_info::features::kShowQoraiShieldsInPageInfo);
  }

  ~QoraiPageInfoBubbleViewFlagDisabledBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(QoraiPageInfoBubbleViewFlagDisabledBrowserTest,
                       CloseButtonVisible) {
  NavigateAndOpenBubble();
  auto* bubble_view = GetBubbleView();
  ASSERT_TRUE(bubble_view);
