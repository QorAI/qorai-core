...
QoraiTabStripRegionView::~QoraiTabStripRegionView() = default;

void QoraiTabStripRegionView::Layout(PassKey) {
...
void QoraiTabStripRegionView::UpdateTabStripMargin() {
  ...
  if (tabs::features::HorizontalTabsUpdateEnabled()) {
    if (!tab_strip_->controller()->IsFrameCondensed() && !vertical_tabs) {
      margins.set_left(qorai_tabs::kHorizontalTabStripLeftMargin);
    } else {
      margins.set_left(0);
    }
  }
...
void QoraiTabStripRegionView::Initialize() {
  // Use our own icon for the new tab button.
  if (auto* ntb = views::AsViewClass<TabStripControlButton>(new_tab_button_)) {
    ntb->SetVectorIcon(kQoraPlusAddIcon);
  }
}

BEGIN_METADATA(QoraiTabStripRegionView)
END_METADATA
