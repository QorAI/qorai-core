...
using QoraiNonClientHitTestHelperBrowserTest = InProcessBrowserTest;

// TODO(sko) It might be good to have resizable area tests. But testing it
// is pretty flaky depending on platforms.
IN_PROC_BROWSER_TEST_F(QoraiNonClientHitTestHelperBrowserTest, Toolbar) {
  ...
  for (views::View* view : toolbar_container->GetChildrenInZOrder()) {
    ...
    // QoraiToolbarView::ViewHierarchyChanged where we set children that way).
