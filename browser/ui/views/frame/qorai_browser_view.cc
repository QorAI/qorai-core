...
#include "qorai/browser/translate/qorai_translate_utils.h"
...
#include "qorai/browser/ui/views/qorai_help_bubble/qorai_help_bubble_host_view.h"
#include "qorai/browser/ui/views/frame/qorai_contents_layout_manager.h"
#include "qorai/browser/ui/views/frame/qorai_contents_view_util.h"
#include "qorai/browser/ui/views/frame/split_view/qorai_contents_container_view.h"
#include "qorai/browser/ui/views/frame/split_view/qorai_multi_contents_view.h"
#include "qorai/browser/ui/views/frame/vertical_tabs/vertical_tab_strip_region_view.h"
#include "qorai/browser/ui/views/frame/vertical_tabs/vertical_tab_strip_widget_delegate_view.h"
#include "qorai/browser/ui/views/location_bar/qorai_location_bar_view.h"
#include "qorai/browser/ui/views/omnibox/qorai_omnibox_view_views.h"
...
#include "qorai/browser/ui/views/window_closing_confirm_dialog_view.h"
...
#include "qorai/browser/ui/views/update_recommended_message_box_mac.h"
#endif

#if BUILDFLAG(ENABLE_SPEEDREADER)
#include "qorai/browser/ui/speedreader/speedreader_tab_helper.h"
#include "qorai/browser/ui/views/speedreader/reader_mode_bubble.h"
#include "qorai/browser/ui/views/speedreader/reader_mode_toolbar_view.h"
#endif

#if BUILDFLAG(ENABLE_QORAI_WAYBACK_MACHINE)
#include "qorai/browser/ui/views/wayback_machine_bubble_view.h"
#endif
...
class SidebarSeparator : public views::View {
 ...
 public:
  SidebarSeparator() {
    SetBackground(
        views::CreateSolidBackground(kColorQoraiVerticalTabSeparator));
  }
};
...
// static
void QoraiBrowserView::SetDownloadConfirmReturnForTesting(bool allow) {
  g_download_confirm_return_allow_for_testing = allow;
}

class QoraiBrowserView::SidebarOnMouseOverEventHandler
    : public ui::EventObserver {
 public:
  explicit SidebarOnMouseOverEventHandler(QoraiBrowserView* browser_view)
...
class QoraiBrowserView::TabCyclingEventHandler : public ui::EventObserver,
                                                 public views::WidgetObserver {
 public:
  explicit TabCyclingEventHandler(QoraiBrowserView* browser_view)
      : browser_view_(browser_view) {
    Start();
  }
...
// static
QoraiBrowserView* QoraiBrowserView::From(BrowserView* view) {
  return static_cast<QoraiBrowserView*>(view);
}

bool QoraiBrowserView::ShouldUseQoraiWebViewRoundedCornersForContents(
    Browser* browser) {
...
QoraiBrowserView::QoraiBrowserView(Browser* browser) : BrowserView(browser) {
#if BUILDFLAG(ENABLE_SPEEDREADER)
  ...
  if (!base::FeatureList::IsEnabled(features::kSideBySide)) {
    ...
    contents_container_->SetLayoutManager(
        std::make_unique<QoraiContentsLayoutManager>(
  ...
  pref_change_registrar_.Add(
      kTabsSearchShow,
      base::BindRepeating(&QoraiBrowserView::OnPreferenceChanged,
  ...
  if (can_have_sidebar) {
    ...
    if (IsQoraiWebViewRoundedCornersEnabled()) {
...
void QoraiBrowserView::OnPreferenceChanged(const std::string& pref_name) {
...
#if BUILDFLAG(ENABLE_QORAI_VPN)
  if (pref_name == qorai_vpn::prefs::kQoraiVPNShowButton) {
...
void QoraiBrowserView::UpdateSideBarHorizontalAlignment() {
  ...
  if (multi_contents_view_ &&
    ...
    GetQoraiMultiContentsView()->SetWebPanelOnLeft(on_left);
  }

  DeprecatedLayoutImmediately();
}

void QoraiBrowserView::UpdateSearchTabsButtonState() {
...
QoraiBrowserView::~QoraiBrowserView() {
  tab_cycling_event_handler_.reset();
}

sidebar::Sidebar* QoraiBrowserView::InitSidebar() {
  // Start Sidebar UI initialization.
  DCHECK(sidebar_container_view_);
  sidebar_container_view_->Init();

  // Ask QoraiMultiContentsView for preparing web panel feature.
  if (multi_contents_view_ &&
      base::FeatureList::IsEnabled(sidebar::features::kSidebarWebPanel)) {
    GetQoraiMultiContentsView()->SetWebPanelWidth(
        sidebar_container_view_->side_panel()->GetPreferredSize().width());
    GetQoraiMultiContentsView()->UseContentsContainerViewForWebPanel();
  }
...
void QoraiBrowserView::ToggleSidebar() {
  browser_->GetFeatures().side_panel_ui()->Toggle();
}

void QoraiBrowserView::ShowQoraiVPNBubble(bool show_select) {
#if BUILDFLAG(ENABLE_QORAI_VPN)
  vpn_panel_controller_.ShowQoraiVPNPanel(show_select);
#endif
}

views::View* QoraiBrowserView::GetAnchorViewForQoraiVPNPanel() {
#if BUILDFLAG(ENABLE_QORAI_VPN)
  auto* vpn_button =
      static_cast<QoraiToolbarView*>(toolbar())->qorai_vpn_button();
...
gfx::Rect QoraiBrowserView::GetShieldsBubbleRect() {
  auto* qorai_location_bar_view =
      static_cast<QoraiLocationBarView*>(GetLocationBarView());
  if (!qorai_location_bar_view) {
    return gfx::Rect();
  }

  auto* shields_action_view =
      qorai_location_bar_view->qorai_actions_contatiner_view()
...
bool QoraiBrowserView::GetTabStripVisible() const {
...
void QoraiBrowserView::SetStarredState(bool is_starred) {
  QoraiBookmarkButton* button =
      static_cast<QoraiToolbarView*>(toolbar())->bookmark_button();
  if (button) {
...
#if BUILDFLAG(ENABLE_SPEEDREADER)
ReaderModeToolbarView* QoraiBrowserView::reader_mode_toolbar() {
  if (base::FeatureList::IsEnabled(features::kSideBySide)) {
    return GetQoraiMultiContentsView()
...
speedreader::SpeedreaderBubbleView* QoraiBrowserView::ShowSpeedreaderBubble(
...
void QoraiBrowserView::UpdateReaderModeToolbar() {
  ...
  if (base::FeatureList::IsEnabled(features::kSideBySide)) {
    ...
    auto* contents_container =
        GetQoraiMultiContentsView()->GetInactiveContentsContainerView();
...
void QoraiBrowserView::ShowUpdateChromeDialog() {
#if BUILDFLAG(ENABLE_SPARKLE)
...
bool QoraiBrowserView::HasSelectedURL() const {
  if (!GetLocationBarView() || !GetLocationBarView()->HasFocus()) {
    return false;
  }
  auto* qorai_omnibox_view =
      static_cast<QoraiOmniboxViewViews*>(GetLocationBarView()->omnibox_view());
  return qorai_omnibox_view && qorai_omnibox_view->SelectedTextIsURL();
}

void QoraiBrowserView::CleanAndCopySelectedURL() {
  if (!GetLocationBarView()) {
    return;
  }
  auto* qorai_omnibox_view =
      static_cast<QoraiOmniboxViewViews*>(GetLocationBarView()->omnibox_view());
  if (!qorai_omnibox_view) {
    return;
  }
  qorai_omnibox_view->CleanAndCopySelectedURL();
}

#if BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
void QoraiBrowserView::ShowPlaylistBubble() {
  static_cast<QoraiLocationBarView*>(GetLocationBarView())
      ->ShowPlaylistBubble();
}
#endif
...
#if BUILDFLAG(ENABLE_QORAI_WAYBACK_MACHINE)
void QoraiBrowserView::ShowWaybackMachineBubble() {
...
WalletButton* QoraiBrowserView::GetWalletButton() {
  return static_cast<QoraiToolbarView*>(toolbar())->wallet_button();
}

void QoraiBrowserView::NotifyDialogPositionRequiresUpdate() {
  GetBrowserViewLayout()->NotifyDialogPositionRequiresUpdate();
}

views::View* QoraiBrowserView::GetWalletButtonAnchorView() {
  return static_cast<QoraiToolbarView*>(toolbar())
      ->wallet_button()
      ->GetAsAnchorView();
}

void QoraiBrowserView::OnAcceleratorsChanged(
    const commands::Accelerators& changed) {
  DCHECK(base::FeatureList::IsEnabled(commands::features::kQoraiCommands));
...
bool QoraiBrowserView::ShowQoraiHelpBubbleView(const std::string& text) {
  auto* shields_action_view =
      static_cast<QoraiLocationBarView*>(GetLocationBarView())
          ->qorai_actions_contatiner_view()
          ->GetShieldsActionView();
  ...
  if (!qorai_help_bubble_host_view_) {
    qorai_help_bubble_host_view_ =
        AddChildView(std::make_unique<QoraiHelpBubbleHostView>());
  }
  qorai_help_bubble_host_view_->set_text(text);
  qorai_help_bubble_host_view_->set_tracked_element(shields_action_view);
  return qorai_help_bubble_host_view_->Show();
}

void QoraiBrowserView::LoadAccelerators() {
...
void QoraiBrowserView::OnTabStripModelChanged(
  ...
  if (selection.active_tab_changed() && qorai_help_bubble_host_view_ &&
      qorai_help_bubble_host_view_->GetVisible()) {
    qorai_help_bubble_host_view_->Hide();
  }
}

views::CloseRequestResult QoraiBrowserView::OnWindowCloseRequested() {
  if (GetQoraiBrowser()->ShouldAskForBrowserClosingBeforeHandlers()) {
    if (!closing_confirm_dialog_activated_) {
      WindowClosingConfirmDialogView::Show(
          browser(),
          base::BindOnce(&QoraiBrowserView::OnWindowClosingConfirmResponse,
...
void QoraiBrowserView::OnWindowClosingConfirmResponse(bool allowed_to_close) {
  DCHECK(closing_confirm_dialog_activated_);
  closing_confirm_dialog_activated_ = false;

  auto* browser = GetQoraiBrowser();
  ...
  if (allowed_to_close) {
    ...
    // as we set QoraiBrowser::confirmed_to_closed_window_ to true.
...
void QoraiBrowserView::ConfirmBrowserCloseWithPendingDownloads(
    int download_count,
...
void QoraiBrowserView::MaybeShowReadingListInSidePanelIPH() {
  // Do nothing.
}

bool QoraiBrowserView::MaybeUpdateDevtools(content::WebContents* web_contents) {
...
void QoraiBrowserView::OnWidgetActivationChanged(views::Widget* widget,
                                                 bool active) {
...
void QoraiBrowserView::OnWidgetWindowModalVisibilityChanged(
    views::Widget* widget,
    bool visible) {
...
bool QoraiBrowserView::IsQoraiWebViewRoundedCornersEnabled() {
  return browser_->profile()->GetPrefs()->GetBoolean(kWebViewRoundedCorners) &&
         browser_->is_type_normal();
}

void QoraiBrowserView::UpdateContentsShadowVisibility() {
  bool show_contents_shadow = IsQoraiWebViewRoundedCornersEnabled();
  ...
  if (show_contents_shadow && !contents_shadow_) {
    contents_shadow_ = QoraiContentsViewUtil::CreateShadow(contents_container_);
    return;
  }
...
void QoraiBrowserView::ShowSplitView(bool focus_active_view) {
  BrowserView::ShowSplitView(focus_active_view);

  UpdateRoundedCornersUI();
}

void QoraiBrowserView::HideSplitView() {
  BrowserView::HideSplitView();

  UpdateRoundedCornersUI();
}

QoraiMultiContentsView* QoraiBrowserView::GetQoraiMultiContentsView() const {
  return QoraiMultiContentsView::From(multi_contents_view_);
}

bool QoraiBrowserView::ShouldShowWindowTitle() const {
...
void QoraiBrowserView::OnThemeChanged() {
...
void QoraiBrowserView::UpdateRoundedCornersUI() {
...
void QoraiBrowserView::UpdateVerticalTabStripBorder() {
...
void QoraiBrowserView::UpdateSidebarBorder() {
...
void QoraiBrowserView::OnActiveTabChanged(content::WebContents* old_contents,
...
void QoraiBrowserView::UpdateContentsSeparatorVisibility() {
  // It's not shown with rounded corners mode always.
  if (ShouldUseQoraiWebViewRoundedCornersForContents(browser_.get())) {
...
bool QoraiBrowserView::AcceleratorPressed(const ui::Accelerator& accelerator) {
...
bool QoraiBrowserView::IsInTabDragging() const {
  return browser_widget()->tab_drag_kind() == TabDragKind::kAllTabs;
}

void QoraiBrowserView::ReadyToListenFullscreenChanges() {
...
void QoraiBrowserView::StopListeningFullscreenChanges() {
  CHECK(browser_->GetFeatures().exclusive_access_manager());
...
void QoraiBrowserView::HandleSidebarOnMouseOverMouseEvent(
...
bool QoraiBrowserView::IsSidebarVisible() const {
  return sidebar_container_view_ && sidebar_container_view_->IsSidebarVisible();
}

QoraiBrowser* QoraiBrowserView::GetQoraiBrowser() const {
  return static_cast<QoraiBrowser*>(browser_.get());
}

void QoraiBrowserView::UpdateWebViewRoundedCorners() {
  gfx::RoundedCornersF corners;

  if (ShouldUseQoraiWebViewRoundedCornersForContents(browser_.get())) {
    corners = gfx::RoundedCornersF(QoraiContentsViewUtil::kBorderRadius);
  }
  ...
  if (multi_contents_view_) {
    GetQoraiMultiContentsView()->UpdateCornerRadius();
    return;
  }
...
void QoraiBrowserView::Layout(PassKey) {
  LayoutSuperclass<BrowserView>(this);
  UpdateWebViewRoundedCorners();
}

void QoraiBrowserView::StartTabCycling() {
  tab_cycling_event_handler_ = std::make_unique<TabCyclingEventHandler>(this);
}

void QoraiBrowserView::StopTabCycling() {
  tab_cycling_event_handler_.reset();
  static_cast<QoraiTabStripModel*>(browser()->tab_strip_model())
      ->StopMRUCycling();
}

void QoraiBrowserView::SetSidePanelOperationByActiveTabChange(bool tab_change) {
  if (!sidebar_container_view_) {
    return;
  }
...
BEGIN_METADATA(QoraiBrowserView)
END_METADATA
