...
// static
QoraiMultiContentsView* QoraiMultiContentsView::From(MultiContentsView* view) {
  CHECK(view);
  return static_cast<QoraiMultiContentsView*>(view);
}

QoraiMultiContentsView::QoraiMultiContentsView(
...
QoraiMultiContentsView::~QoraiMultiContentsView() = default;

void QoraiMultiContentsView::Layout(PassKey) {
  LayoutSuperclass<MultiContentsView>(this);

  QoraiBrowserView::From(browser_view_)->NotifyDialogPositionRequiresUpdate();
}

void QoraiMultiContentsView::UseContentsContainerViewForWebPanel() {
  if (!contents_container_view_for_web_panel_) {
    contents_container_view_for_web_panel_ = AddChildView(
        std::make_unique<QoraiContentsContainerView>(browser_view_));
    contents_container_view_for_web_panel_->SetVisible(false);
  }
}

void QoraiMultiContentsView::SetWebPanelVisible(bool visible) {
...
bool QoraiMultiContentsView::IsWebPanelVisible() const {
  CHECK(contents_container_view_for_web_panel_);
  return contents_container_view_for_web_panel_->GetVisible();
}

void QoraiMultiContentsView::SetWebPanelWidth(int width) {
  web_panel_width_ = width;
  InvalidateLayout();
}

void QoraiMultiContentsView::SetWebPanelOnLeft(bool left) {
  web_panel_on_left_ = left;
  InvalidateLayout();
}

views::ProposedLayout QoraiMultiContentsView::CalculateProposedLayout(
  ...
  // our custom border drawn by QoraiContentsContainerView is not visible
  // because QoraiContentsContainerView doesn't have layer and its parent
  // layer is behind the |background_view_|.
  // We can handle this by having QoraiContentsContainerView's own layer but
...
void QoraiMultiContentsView::ResetResizeArea() {
...
int QoraiMultiContentsView::GetWebPanelWidth() const {
  if (!contents_container_view_for_web_panel_ ||
...
void QoraiMultiContentsView::UpdateCornerRadius() {
  UpdateContentsBorderAndOverlay();
}

QoraiContentsContainerView*
QoraiMultiContentsView::GetActiveContentsContainerView() {
  return QoraiContentsContainerView::From(
      contents_container_views_[active_index_]);
}

QoraiContentsContainerView*
QoraiMultiContentsView::GetInactiveContentsContainerView() {
  return QoraiContentsContainerView::From(
      contents_container_views_[GetInactiveIndex()]);
}

BEGIN_METADATA(QoraiMultiContentsView)
END_METADATA
