...
#include "qorai/browser/ui/views/frame/split_view/qorai_multi_contents_view_mini_toolbar.h"
...
// QoraiMultiContentsViewMiniToolbar.
class QoraiContentsContainerOutline : public ContentsContainerOutline {
  METADATA_HEADER(QoraiContentsContainerOutline, ContentsContainerOutline)
 public:
  using ContentsContainerOutline::ContentsContainerOutline;
  ~QoraiContentsContainerOutline() override = default;
...
BEGIN_METADATA(QoraiContentsContainerOutline)
END_METADATA

}  // namespace

// static
QoraiContentsContainerView* QoraiContentsContainerView::From(
    ContentsContainerView* view) {
  CHECK(view);
  return static_cast<QoraiContentsContainerView*>(view);
}

QoraiContentsContainerView::QoraiContentsContainerView(
  ...
  if (base::FeatureList::IsEnabled(features::kSideBySide)) {
    ...
    mini_toolbar_ =
        AddChildView(std::make_unique<QoraiMultiContentsViewMiniToolbar>(
            browser_view, contents_view_));
    container_outline_ = AddChildView(
        std::make_unique<QoraiContentsContainerOutline>(mini_toolbar_));
  }
}

QoraiContentsContainerView::~QoraiContentsContainerView() = default;

void QoraiContentsContainerView::UpdateBorderAndOverlay(bool is_in_split,
                                                        bool is_active,
                                                        bool is_highlighted) {
  ...
  if (is_active) {
    SetBorder(views::CreateRoundedRectBorder(
        ...
        kColorQoraiSplitViewActiveWebViewBorder));
  } else {
    SetBorder(views::CreateBorderPainter(
        views::Painter::CreateRoundRectWith1PxBorderPainter(
            GetColorProvider()->GetColor(
                kColorQoraiSplitViewInactiveWebViewBorder),
...
void QoraiContentsContainerView::UpdateBorderRoundedCorners() {
...
views::ProposedLayout QoraiContentsContainerView::CalculateProposedLayout(
...
void QoraiContentsContainerView::ChildVisibilityChanged(views::View* child) {
  ContentsContainerView::ChildVisibilityChanged(child);
  InvalidateLayout();
}

#if BUILDFLAG(ENABLE_SPEEDREADER)
void QoraiContentsContainerView::OnReaderModeToolbarActivate(
...
float QoraiContentsContainerView::GetCornerRadius(bool for_border) const {
  ...
  return QoraiBrowserView::ShouldUseQoraiWebViewRoundedCornersForContents(
             browser_view_->browser())
             ? QoraiContentsViewUtil::kBorderRadius +
                   (for_border ? kBorderThickness : 0)
             : 0;
}

BEGIN_METADATA(QoraiContentsContainerView)
END_METADATA
