...
QoraiBrowserViewLayout::QoraiBrowserViewLayout(
...
QoraiBrowserViewLayout::~QoraiBrowserViewLayout() = default;

int QoraiBrowserViewLayout::GetIdealSideBarWidth() const {
...
int QoraiBrowserViewLayout::GetIdealSideBarWidth(int available_width) const {
  if (!sidebar_container_) {
    return 0;
  }
...
void QoraiBrowserViewLayout::Layout(views::View* host) {
  BrowserViewLayout::Layout(host);
  LayoutVerticalTabs();
}

void QoraiBrowserViewLayout::LayoutVerticalTabs() {
...
void QoraiBrowserViewLayout::LayoutTabStripRegion(gfx::Rect& available_bounds) {
  if (tabs::utils::ShouldShowVerticalTabs(browser_view_->browser())) {
...
void QoraiBrowserViewLayout::LayoutBookmarkBar(gfx::Rect& available_bounds) {
...
void QoraiBrowserViewLayout::LayoutInfoBar(gfx::Rect& available_bounds) {
...
void QoraiBrowserViewLayout::LayoutContentsContainerView(
  ...
  if (vertical_tab_strip_host_) {
    ...
    // https://github.com/qorai/qorai-browser/issues/48373
...
bool QoraiBrowserViewLayout::IsImmersiveModeEnabledWithoutToolbar() const {
  ...
  // is not applied by controlling its bounds from QoraiBrowserView.
  return true;
}

void QoraiBrowserViewLayout::LayoutSideBar(gfx::Rect& contents_bounds) {
  if (!sidebar_container_) {
    return;
  }
...
void QoraiBrowserViewLayout::UpdateContentsContainerInsets(
  ...
  int contents_margin_for_rounded_corners =
      QoraiContentsViewUtil::GetRoundedCornersWebViewMargin(
          browser_view_->browser());
...
gfx::Insets QoraiBrowserViewLayout::GetContentsMargins() const {
  if (!QoraiBrowserView::ShouldUseQoraiWebViewRoundedCornersForContents(
          browser_view_->browser())) {
    return {};
  }
  ...
  gfx::Insets margins(QoraiContentsViewUtil::kMarginThickness);
...
bool QoraiBrowserViewLayout::IsFullscreenForBrowser() const {
...
bool QoraiBrowserViewLayout::IsFullscreenForTab() const {
  auto* exclusive_access_manager = browser_view_->GetExclusiveAccessManager();
  if (!exclusive_access_manager) {
    return false;
  }
...
bool QoraiBrowserViewLayout::ShouldPushBookmarkBarForVerticalTabs() {
...
gfx::Insets QoraiBrowserViewLayout::GetInsetsConsideringVerticalTabHost() {
  CHECK(vertical_tab_strip_host_)
      << "This method is used only when vertical tab strip host is set";
  gfx::Insets insets;
  if (tabs::utils::IsVerticalTabOnRight(browser_view_->browser())) {
...
#if BUILDFLAG(IS_MAC)
gfx::Insets QoraiBrowserViewLayout::AdjustInsetsConsideringFrameBorder(
