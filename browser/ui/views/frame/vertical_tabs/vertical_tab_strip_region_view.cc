...
class ToggleButton : public ToolbarButton {
 ...
 public:
  ToggleButton(PressedCallback callback,
               QoraiVerticalTabStripRegionView* region_view)
  ...
  void OnThemeChanged() override {
    ...
    SetHighlighted(region_view_->state() ==
                   QoraiVerticalTabStripRegionView::State::kExpanded);
  }
  ...
  void StateChanged(ButtonState old_state) override {
    ...
    if (GetState() == views::Button::STATE_NORMAL) {
      ...
      // https://github.com/qorai/qorai-browser/issues/31421
      SetHighlighted(region_view_->state() ==
                     QoraiVerticalTabStripRegionView::State::kExpanded);
    }
  }
 ...
 private:
  raw_ref<QoraiVerticalTabStripRegionView> region_view_;
};
...
class ShortcutBox : public views::View {
 ...
 private:
  void AddShortcutPart(const std::u16string& text) {
    ...
    shortcut_part->SetEnabledColor(kColorQoraiVerticalTabNTBShortcutTextColor);
    shortcut_part->SetBorder(views::CreateRoundedRectBorder(
        /*thickness*/ 1, /*radius*/ 4, kColorQoraiVerticalTabSeparator));
...
class VerticalTabNewTabButton : public QoraiNewTabButton {
  METADATA_HEADER(VerticalTabNewTabButton, QoraiNewTabButton)
 public:
  VerticalTabNewTabButton(TabStripController* tab_strip_controller,
                          PressedCallback callback,
                          const std::u16string& shortcut_text)
      : QoraiNewTabButton(tab_strip_controller,
                          std::move(callback),
                          kQoraPlusAddIcon) {
    ...
    plus_icon_->SetImage(ui::ImageModel::FromVectorIcon(
        kQoraPlusAddIcon, kColorQoraiVerticalTabNTBIconColor,
        /* icon_size= */ 16));
    ...
    text_->SetEnabledColor(kColorQoraiVerticalTabNTBTextColor);
  ...
  void StateChanged(ButtonState old_state) override {
    QoraiNewTabButton::StateChanged(old_state);
    UpdateColors();
  }
 ...
 private:
  void UpdateColors() override {
    ...
    QoraiNewTabButton::UpdateColors();

    int bg_color_id = kColorToolbar;
    if (GetState() == views::Button::STATE_PRESSED) {
      bg_color_id = kColorQoraiVerticalTabActiveBackground;
    } else if (GetState() == views::Button::STATE_HOVERED) {
      bg_color_id = kColorQoraiVerticalTabHoveredBackground;
    }
...
class ResettableResizeArea : public views::ResizeArea {
 ...
 public:
  explicit ResettableResizeArea(QoraiVerticalTabStripRegionView* region_view)
 ...
 private:
  raw_ptr<QoraiVerticalTabStripRegionView> region_view_;
};
...
class QoraiVerticalTabStripRegionView::HeaderView : public views::View {
 ...
 private:
  ...
  raw_ptr<QoraiVerticalTabStripRegionView> region_view_ = nullptr;
...
using HeaderView = QoraiVerticalTabStripRegionView::HeaderView;
...
class QoraiVerticalTabStripRegionView::MouseWatcher : public ui::EventObserver {
 public:
  explicit MouseWatcher(QoraiVerticalTabStripRegionView* region_view)
...
QoraiVerticalTabStripRegionView::QoraiVerticalTabStripRegionView(
  ...
  header_view_ = AddChildView(std::make_unique<HeaderView>(
      base::BindRepeating(&QoraiVerticalTabStripRegionView::ToggleState,
                          base::Unretained(this)),
      this, browser_));
  contents_view_ =
  ...
  sidebar_side_.Init(prefs::kSidePanelHorizontalAlignment, prefs,
                     base::BindRepeating(
                         &QoraiVerticalTabStripRegionView::OnBrowserPanelsMoved,
                         base::Unretained(this)));

  expanded_width_pref_.Init(
      qorai_tabs::kVerticalTabsExpandedWidth, prefs,
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnExpandedWidthPrefChanged,
          base::Unretained(this)));
  ...
  show_vertical_tabs_.Init(
      ...
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnShowVerticalTabsPrefChanged,
          base::Unretained(this)));
  UpdateLayout();

  collapsed_pref_.Init(
      qorai_tabs::kVerticalTabsCollapsed, prefs,
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnCollapsedPrefChanged,
          base::Unretained(this)));
  OnCollapsedPrefChanged();

  expanded_state_per_window_pref_.Init(
      qorai_tabs::kVerticalTabsExpandedStatePerWindow, prefs,
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnExpandedStatePerWindowPrefChanged,
          base::Unretained(this)));

  floating_mode_pref_.Init(
      qorai_tabs::kVerticalTabsFloatingEnabled, prefs,
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnFloatingModePrefChanged,
          base::Unretained(this)));
  ...
  show_toolbar_on_fullscreen_pref_.Init(
      ...
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnFullscreenStateChanged,
          base::Unretained(this)));
#endif

  vertical_tab_on_right_.Init(
      qorai_tabs::kVerticalTabsOnRight, browser()->profile()->GetPrefs(),
      base::BindRepeating(
          &QoraiVerticalTabStripRegionView::OnBrowserPanelsMoved,
          base::Unretained(this)));
  ...
  if (base::FeatureList::IsEnabled(
    ...
    hide_completely_when_collapsed_pref_.Init(
        qorai_tabs::kVerticalTabsHideCompletelyWhenCollapsed, prefs,
        base::BindRepeating(&QoraiVerticalTabStripRegionView::
...
void QoraiVerticalTabStripRegionView::ToggleState() {
...
void QoraiVerticalTabStripRegionView::OnWidgetActivationChanged(
    views::Widget* widget,
    bool active) {
  if (active) {
...
void QoraiVerticalTabStripRegionView::OnWidgetDestroying(
    views::Widget* widget) {
  widget_observation_.Reset();
}

void QoraiVerticalTabStripRegionView::OnFullscreenStateChanged() {
...
void QoraiVerticalTabStripRegionView::ListenFullscreenChanges() {
...
void QoraiVerticalTabStripRegionView::StopListeningFullscreenChanges() {
  fullscreen_observation_.Reset();
}

FullscreenController* QoraiVerticalTabStripRegionView::GetFullscreenController()
    const {
...
bool QoraiVerticalTabStripRegionView::IsTabFullscreen() const {
...
bool QoraiVerticalTabStripRegionView::IsBrowserFullscren() const {
  const auto* fullscreen_controller = GetFullscreenController();
  return fullscreen_controller &&
         fullscreen_controller->IsFullscreenForBrowser();
}

bool QoraiVerticalTabStripRegionView::
...
void QoraiVerticalTabStripRegionView::SetState(State state) {
  if (state_ == state) {
    return;
  }
  ...
  tab_strip->SetAvailableWidthCallback(base::BindRepeating(
      &QoraiVerticalTabStripRegionView::GetAvailableWidthForTabContainer,
      base::Unretained(this)));
  ...
  if (!GetVisible() && state_ != State::kCollapsed) {
    ...
    // tabs::features::kQoraiVerticalTabHideCompletely on.
    SetVisible(true);
  }
...
void QoraiVerticalTabStripRegionView::SetExpandedWidth(int dest_width) {
...
void QoraiVerticalTabStripRegionView::UpdateStateAfterDragAndDropFinished(
...
QoraiVerticalTabStripRegionView::ScopedStateResetter
QoraiVerticalTabStripRegionView::ExpandTabStripForDragging() {
  if (state_ == State::kExpanded) {
    return {};
  }

  auto resetter = std::make_unique<base::ScopedClosureRunner>(base::BindOnce(
      &QoraiVerticalTabStripRegionView::UpdateStateAfterDragAndDropFinished,
...
gfx::Vector2d QoraiVerticalTabStripRegionView::GetOffsetForDraggedTab() const {
  return {0, header_view_->GetPreferredSize().height()};
}

int QoraiVerticalTabStripRegionView::GetAvailableWidthForTabContainer() {
  DCHECK(tabs::utils::ShouldShowVerticalTabs(browser_));
  return GetPreferredWidthForState(state_, /*include_border=*/false,
                                   /*ignore_animation=*/false);
}

gfx::Size QoraiVerticalTabStripRegionView::CalculatePreferredSize(
    const views::SizeBounds& available_size) const {
  return GetPreferredSizeForState(state_, /*include_border=*/true,
                                  /*ignore_animation=*/false);
}

gfx::Size QoraiVerticalTabStripRegionView::GetMinimumSize() const {
  if (IsFloatingEnabledForBrowserFullscreen()) {
...
void QoraiVerticalTabStripRegionView::Layout(PassKey) {
...
void QoraiVerticalTabStripRegionView::OnShowVerticalTabsPrefChanged() {
...
void QoraiVerticalTabStripRegionView::OnBrowserPanelsMoved() {
  UpdateBorder();
  PreferredSizeChanged();
}

void QoraiVerticalTabStripRegionView::UpdateLayout(bool in_destruction) {
...
void QoraiVerticalTabStripRegionView::OnThemeChanged() {
  View::OnThemeChanged();

  UpdateBorder();
}

void QoraiVerticalTabStripRegionView::OnMouseExited(
...
void QoraiVerticalTabStripRegionView::OnMouseEntered(
    const ui::MouseEvent& event) {
  OnMouseEntered();
}
...
void QoraiVerticalTabStripRegionView::OnMousePressedInTree() {
  if (IsFloatingVerticalTabsEnabled()) {
    return;
  }
...
void QoraiVerticalTabStripRegionView::OnBoundsChanged(
  ...
  if (previous_bounds.size() != size()) {
    if (GetAvailableWidthForTabContainer() != tab_strip()->width()) {
      ...
      // force it to layout.
      // https://github.com/qorai/qorai-browser/issues/29941
      tab_strip()->tab_container_->InvalidateIdealBounds();
      tab_strip()->tab_container_->CompleteAnimationAndLayout();
    }
  }
  ...
  if (auto width = GetContentsBounds().width();
    ...
    CHECK_GE(
        ...
        tabs::kVerticalTabMinWidth + tabs::kMarginForVerticalTabContainers * 2 -
            QoraiContentsViewUtil::GetRoundedCornersWebViewMargin(browser_));
  }
#endif
}

void QoraiVerticalTabStripRegionView::AddedToWidget() {
  View::AddedToWidget();
  mouse_watcher_ = std::make_unique<MouseWatcher>(this);
}

void QoraiVerticalTabStripRegionView::OnResize(int resize_amount,
                                               bool done_resizing) {
...
void QoraiVerticalTabStripRegionView::AnimationProgressed(
    const gfx::Animation* animation) {
  PreferredSizeChanged();
}

void QoraiVerticalTabStripRegionView::AnimationEnded(
    const gfx::Animation* animation) {
  PreferredSizeChanged();
...
void QoraiVerticalTabStripRegionView::UpdateNewTabButtonVisibility() {
...
int QoraiVerticalTabStripRegionView::GetTabStripViewportMaxHeight() const {
...
void QoraiVerticalTabStripRegionView::ResetExpandedWidth() {
  auto* prefs = browser_->profile()->GetPrefs();
  prefs->ClearPref(qorai_tabs::kVerticalTabsExpandedWidth);

  PreferredSizeChanged();
}

void QoraiVerticalTabStripRegionView::UpdateBorder() {
  ...
  if (show_visible_border()) {
    SetBorder(views::CreateSolidSidedBorder(
        border_insets,
        GetColorProvider()->GetColor(kColorQoraiVerticalTabSeparator)));
  } else {
...
void QoraiVerticalTabStripRegionView::OnCollapsedPrefChanged() {
...
void QoraiVerticalTabStripRegionView::OnFloatingModePrefChanged() {
  if (!IsFloatingVerticalTabsEnabled()) {
    if (state_ == State::kFloating) {
      SetState(State::kCollapsed);
    }
...
void QoraiVerticalTabStripRegionView::OnExpandedStatePerWindowPrefChanged() {
  OnCollapsedPrefChanged();
  OnExpandedWidthPrefChanged();
}

void QoraiVerticalTabStripRegionView::
    OnHideComopletelyWhenCollapsedPrefChanged() {
  OnFloatingModePrefChanged();
  PreferredSizeChanged();
  if (state_ == State::kCollapsed) {
...
void QoraiVerticalTabStripRegionView::OnExpandedWidthPrefChanged() {
  if (!expanded_state_per_window_pref_.GetPrefName().empty() &&
      *expanded_state_per_window_pref_) {
    // On creation(when expanded_state_per_window_pref_ is empty), we set the
...
gfx::Size QoraiVerticalTabStripRegionView::GetPreferredSizeForState(
    State state,
    bool include_border,
...
int QoraiVerticalTabStripRegionView::GetPreferredWidthForState(
    State state,
    bool include_border,
    bool ignore_animation) const {
...
QoraiVerticalTabStripRegionView::GetTabStripScrollContainer() {
...
bool QoraiVerticalTabStripRegionView::IsFloatingVerticalTabsEnabled() const {
  return IsFloatingEnabledForBrowserFullscreen() ||
         tabs::utils::IsFloatingVerticalTabsEnabled(browser_) ||
         tabs::utils::ShouldHideVerticalTabsCompletelyWhenCollapsed(browser_);
}

bool QoraiVerticalTabStripRegionView::IsFloatingEnabledForBrowserFullscreen()
    const {
  return IsBrowserFullscren() && !ShouldShowVerticalTabsInBrowserFullscreen();
}

void QoraiVerticalTabStripRegionView::ScheduleFloatingModeTimer() {
  ...
  if (state_ == State::kCollapsed) {
    ...
    mouse_enter_timer_.Start(
        ...
        base::BindOnce(&QoraiVerticalTabStripRegionView::SetState,
                       base::Unretained(this), State::kFloating));
  }
}

void QoraiVerticalTabStripRegionView::ScheduleCollapseTimer() {
  if (state_ != State::kFloating) {
    return;
  }
...
#if !BUILDFLAG(IS_MAC)
std::u16string QoraiVerticalTabStripRegionView::GetShortcutTextForNewTabButton(
    BrowserView* browser_view) {
...
views::LabelButton&
QoraiVerticalTabStripRegionView::GetToggleButtonForTesting() {
  return *header_view_->toggle_button();
}

void QoraiVerticalTabStripRegionView::OnCollapseAnimationEnded() {
...
bool QoraiVerticalTabStripRegionView::IsMenuShowing() const {
  return menu_runner_ && menu_runner_->IsRunning();
}

// Show context menu in unobscured area.
void QoraiVerticalTabStripRegionView::ShowContextMenuForViewImpl(
    views::View* source,
    const gfx::Point& p,
  ...
  menu_runner_ = std::make_unique<views::MenuRunner>(
      ...
      base::BindRepeating(&QoraiVerticalTabStripRegionView::OnMenuClosed,
                          base::Unretained(this)));
...
void QoraiVerticalTabStripRegionView::OnMenuClosed() {
  menu_runner_.reset();
}

BEGIN_METADATA(QoraiVerticalTabStripRegionView)
END_METADATA
