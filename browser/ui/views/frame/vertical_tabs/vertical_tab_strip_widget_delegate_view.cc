...
#include "qorai/browser/ui/views/frame/vertical_tabs/vertical_tab_strip_root_view.h"
...
VerticalTabStripWidgetDelegateView::VerticalTabStripWidgetDelegateView(
    ...
    : browser_view_(browser_view),
      host_(host),
      region_view_(
          AddChildView(std::make_unique<QoraiVerticalTabStripRegionView>(
              browser_view_,
...
void VerticalTabStripWidgetDelegateView::OnWidgetVisibilityChanged(
  ...
  if (widget == GetWidget()) {
    if (!tabs::utils::ShouldShowVerticalTabs(browser_view_->browser()) &&
      ...
      // https://github.com/qorai/qorai-browser/issues/29917
      widget->Hide();
    }
  }
}
