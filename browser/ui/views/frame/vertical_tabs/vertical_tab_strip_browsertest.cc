...
#include "qorai/browser/ui/tabs/qorai_tab_menu_model.h"
...
#include "qorai/browser/ui/views/tabs/qorai_compound_tab_container.h"
#include "qorai/browser/ui/views/tabs/qorai_new_tab_button.h"
#include "qorai/browser/ui/views/tabs/qorai_tab_context_menu_contents.h"
...
class VerticalTabStripBrowserTest : public InProcessBrowserTest {
 public:
  ...
  const QoraiBrowserView* browser_view() const {
    return static_cast<QoraiBrowserView*>(browser()->window());
  }
  QoraiBrowserView* browser_view() {
    return static_cast<QoraiBrowserView*>(browser()->window());
  }
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, WindowTitle) {
  ...
  auto check_if_window_title_gets_visible = [&]() {
    ...
    qorai::ToggleWindowTitleVisibilityForVerticalTabs(browser());
    browser_non_client_frame_view()->DeprecatedLayoutImmediately();
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, VisualState) {
  ...
  using State = QoraiVerticalTabStripRegionView::State;
  ...
  // See QoraiVerticalTabStripRegionView::UpdateBorder() for
  ...
  prefs->SetBoolean(qorai_tabs::kVerticalTabsCollapsed, true);
  EXPECT_EQ(State::kCollapsed, region_view->state());
  prefs->SetBoolean(qorai_tabs::kVerticalTabsCollapsed, false);
  ...
  prefs->SetBoolean(qorai_tabs::kVerticalTabsFloatingEnabled, false);
  {
    base::AutoReset resetter(&region_view->mouse_events_for_test_, true);
    ui::MouseEvent event(ui::EventType::kMouseEntered, gfx::PointF(),
                         gfx::PointF(), {}, {}, {});
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, LayoutSanity) {
  ...
  auto* region_view = widget_delegate_view->vertical_tab_strip_region_view();
  ASSERT_TRUE(region_view);
  ASSERT_EQ(QoraiVerticalTabStripRegionView::State::kExpanded,
            region_view->state());
  ...
  // https://github.com/qorai/qorai-browser/issues/28084
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, ScrollBarVisibility) {
  ...
  auto* pref = prefs->FindPreference(qorai_tabs::kVerticalTabsShowScrollbar);

  // Check if the default value is false
  EXPECT_TRUE(pref && pref->IsDefaultValue());
  EXPECT_FALSE(prefs->GetBoolean(qorai_tabs::kVerticalTabsShowScrollbar));

  auto get_tab_container = [&]() {
    return views::AsViewClass<QoraiTabStrip>(browser_view()->tabstrip())
        ->GetTabContainerForTesting();
  };

  auto* qorai_tab_container =
      views::AsViewClass<QoraiCompoundTabContainer>(get_tab_container());
  EXPECT_TRUE(qorai_tab_container);
  EXPECT_EQ(views::ScrollView::ScrollBarMode::kHiddenButEnabled,
            qorai_tab_container->scroll_view_->GetVerticalScrollBarMode());

  // Turn on the prefs and checks if scrollbar becomes visible
  prefs->SetBoolean(qorai_tabs::kVerticalTabsShowScrollbar, true);
  ...
  prefs->SetBoolean(qorai_tabs::kVerticalTabsShowScrollbar, false);
  EXPECT_EQ(views::ScrollView::ScrollBarMode::kHiddenButEnabled,
            qorai_tab_container->scroll_view_->GetVerticalScrollBarMode());
}
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, ExpandedState) {
  ...
  auto* prefs = browser()->profile()->GetPrefs();
  ASSERT_FALSE(
      prefs->GetBoolean(qorai_tabs::kVerticalTabsExpandedStatePerWindow));
  ...
  EXPECT_TRUE(prefs->GetBoolean(qorai_tabs::kVerticalTabsCollapsed));

  // it affects all browsers.
  auto* region_view_2 =
      static_cast<QoraiBrowserView*>(
  ...
  prefs->SetBoolean(qorai_tabs::kVerticalTabsExpandedStatePerWindow, true);
  ...
  EXPECT_FALSE(prefs->GetBoolean(qorai_tabs::kVerticalTabsCollapsed));
  EXPECT_EQ(State::kCollapsed, region_view_2->state());
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, ExpandedWidth) {
  ...
  EXPECT_EQ(100, prefs->GetValue(qorai_tabs::kVerticalTabsExpandedWidth));
  ...
  EXPECT_EQ(200, prefs->GetValue(qorai_tabs::kVerticalTabsExpandedWidth));
  EXPECT_EQ(100, region_view_2->expanded_width_);
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripStringBrowserTest, ContextMenuString) {
  ...
  auto create_tab_context_menu_contents = [&]() {
    return std::make_unique<QoraiTabContextMenuContents>(
        GetTabAt(browser(), 0),
        static_cast<QoraiBrowserTabStripController*>(
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripBrowserTest, PinningGroupedTab) {
  ...
  // Regression check for https://github.com/qorai/qorai-browser/issues/40201
  ToggleVerticalTabStrip();
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripDragAndDropBrowserTest,
  ...
  GetTabStrip(browser())->StopAnimating();  // Drag-and-drop doesn't start when
                                            // animation is running.
  {
    // Regression test for https://github.com/qorai/qorai-browser/issues/28488
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripDragAndDropBrowserTest, MAYBE_DragURL) {
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("https://qorai.com/")));
  ...
  // https://github.com/qorai/qorai-browser/issues/28592
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripWithScrollableTabBrowserTest, Sanity) {
  ...
  // https://github.com/qorai/qorai-browser/issues/28877
  ToggleVerticalTabStrip();
  Browser::Create(Browser::CreateParams(browser()->profile(), true));
}
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripWithScrollableTabBrowserTest,
  ...
  // Make sure browser works with both vertical tab and scrollable tab strip
  // even with groups.
  // https://github.com/qorai/qorai-browser/issues/46615
...
class VerticalTabStripScrollBarFlagTest : public VerticalTabStripBrowserTest {
 public:
  VerticalTabStripScrollBarFlagTest()
      : feature_list_(tabs::features::kQoraiVerticalTabScrollBar) {}
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripHideCompletelyTest, GetMinimumWidth) {
  ...
  ASSERT_EQ(QoraiVerticalTabStripRegionView::State::kCollapsed,
            region_view->state());
  ...
  // view's difference. See QoraiVerticalTabStripRegionView::UpdateBorder().
