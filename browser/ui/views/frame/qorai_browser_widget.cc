...
QoraiBrowserWidget::QoraiBrowserWidget(BrowserView* browser_view)
  ...
  if (view_->browser()->profile()->IsIncognitoProfile() ||
    ...
    theme_supplier_ = base::MakeRefCounted<QoraiPrivateWindowThemeSupplier>(
        !view_->browser()->profile()->IsTor());
  }
}

QoraiBrowserWidget::~QoraiBrowserWidget() {
  ...
  QoraiBrowserView::From(view_)->StopListeningFullscreenChanges();
}

ui::ColorProviderKey::ThemeInitializerSupplier*
QoraiBrowserWidget::GetCustomTheme() const {
...
views::internal::RootView* QoraiBrowserWidget::CreateRootView() {
  root_view_ = new QoraiBrowserRootView(browser_view_, this);
  return root_view_;
}

void QoraiBrowserWidget::SetTabDragKind(TabDragKind kind) {
...
ui::ColorProviderKey QoraiBrowserWidget::GetColorProviderKey() const {
  ...
  if (base::FeatureList::IsEnabled(darker_theme::features::kQoraiDarkerTheme)) {
    ...
    if (!theme_supplier_ &&
        ...
        browser_view_->browser()->profile()->GetPrefs()->GetBoolean(
            darker_theme::prefs::kQoraiDarkerMode)) {
