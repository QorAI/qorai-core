...
sidebar::SidebarService* GetSidebarService(QoraiBrowser* browser) {
...
SidebarContainerView::SidebarContainerView(
    ...
    std::unique_ptr<QoraiSidePanel> side_panel)
...
void SidebarContainerView::Init() {
  ...
  SetSidebarShowOption(
      GetSidebarService(GetQoraiBrowser())->GetSidebarShowOption());
}
...
void SidebarContainerView::SetSidebarOnLeft(bool sidebar_on_left) {
  ...
  side_panel_->SetHorizontalAlignment(
      sidebar_on_left ? QoraiSidePanel::HorizontalAlignment::kLeft
                      : QoraiSidePanel::HorizontalAlignment::kRight);
}
...
void SidebarContainerView::AddChildViews() {
  sidebar_control_view_ = AddChildView(
      std::make_unique<SidebarControlView>(this, GetQoraiBrowser()));
...
void SidebarContainerView::ShowSidebar(bool show_side_panel) {
  ...
  if (ShouldUseAnimation()) {
    ...
    if (show_side_panel) {
      ...
      // width and `QoraiBrowserViewLayout` only knows it because side panel's
      ...
      // preferred width, `QoraiBrowserViewLayout` allocates more smaller width
      // to it.
      auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser_);
      const int target_sidebar_width =
          static_cast<QoraiBrowserViewLayout*>(browser_view->GetLayoutManager())
...
void SidebarContainerView::UpdateToolbarButtonVisibility() {
  ...
  auto has_panel_item =
      GetSidebarService(GetQoraiBrowser())->GetDefaultPanelItem().has_value();
  auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser_);
  auto* qorai_toolbar = static_cast<QoraiToolbarView*>(browser_view->toolbar());
  if (qorai_toolbar && qorai_toolbar->side_panel_button()) {
    qorai_toolbar->side_panel_button()->SetVisible(
...
void SidebarContainerView::OnTabStripModelChanged(
  ...
  if ((change.type() == TabStripModelChange::kReplaced)) {
    ...
    // to fix https://github.com/qorai/qorai-browser/issues/40681.
...
QoraiBrowser* SidebarContainerView::GetQoraiBrowser() const {
