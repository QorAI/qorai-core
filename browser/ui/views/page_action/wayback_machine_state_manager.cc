...
#include "qorai/browser/ui/views/page_action/wayback_machine_state_manager.h"
...
WaybackState WaybackMachineStateManager::GetActiveTabWaybackState() const {
  ...
  auto* tab_helper =
      QoraiWaybackMachineTabHelper::FromWebContents(active_contents);
  CHECK(tab_helper);
...
void WaybackMachineStateManager::OnTabStripModelChanged(
  ...
  if (selection.old_contents) {
    auto* tab_helper =
        QoraiWaybackMachineTabHelper::FromWebContents(selection.old_contents);
    CHECK(tab_helper);
  ...
  if (selection.new_contents) {
    auto* tab_helper =
        QoraiWaybackMachineTabHelper::FromWebContents(selection.new_contents);
    CHECK(tab_helper);
