...
class QoraiAvatarButtonHighlightPathGenerator
    : public views::HighlightPathGenerator {
 public:
  QoraiAvatarButtonHighlightPathGenerator(
      const base::WeakPtr<QoraiAvatarToolbarButton>& avatar_button,
      int radius)
      : avatar_button_(avatar_button), radius_(radius) {}
  QoraiAvatarButtonHighlightPathGenerator(
      const QoraiAvatarButtonHighlightPathGenerator&) = delete;
  QoraiAvatarButtonHighlightPathGenerator& operator=(
      const QoraiAvatarButtonHighlightPathGenerator&) = delete;
  ~QoraiAvatarButtonHighlightPathGenerator() override = default;
 ...
 private:
  base::WeakPtr<QoraiAvatarToolbarButton> avatar_button_;
  int radius_ = 0;
};

}  // namespace

QoraiAvatarToolbarButton::QoraiAvatarToolbarButton(BrowserView* browser_view)
...
QoraiAvatarToolbarButton::~QoraiAvatarToolbarButton() = default;

void QoraiAvatarToolbarButton::SetHighlight(
...
void QoraiAvatarToolbarButton::OnThemeChanged() {
  ...
  views::HighlightPathGenerator::Install(
      this, std::make_unique<QoraiAvatarButtonHighlightPathGenerator>(
                weak_ptr_factory_.GetWeakPtr(), radius));
}

int QoraiAvatarToolbarButton::GetIconSize() const {
  return GetLayoutConstant(LOCATION_BAR_ICON_SIZE);
}

void QoraiAvatarToolbarButton::UpdateColorsAndInsets() {
  ...
  if (browser_->profile()->IsOffTheRecord()) {
    ...
    constexpr int kQoraiAvatarButtonHorizontalSpacing = 8;
    gfx::Insets target_insets = ::GetLayoutInsets(TOOLBAR_BUTTON);
    target_insets.set_left_right(kQoraiAvatarButtonHorizontalSpacing,
                                 kQoraiAvatarButtonHorizontalSpacing);
    if (!is_tor) {
      // Use smaller vertical margins as we user more larger icon.
      constexpr int kQoraiAvatarButtonVerticalSpacing = 3;
      target_insets.set_top_bottom(kQoraiAvatarButtonVerticalSpacing,
                                   kQoraiAvatarButtonVerticalSpacing);
    }
    ...
    constexpr int kQoraiAvatarImageLabelSpacing = 8;
    SetImageLabelSpacing(kQoraiAvatarImageLabelSpacing);
    return;
  }
...
BEGIN_METADATA(QoraiAvatarToolbarButton)
END_METADATA
