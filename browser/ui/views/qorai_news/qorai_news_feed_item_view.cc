...
QoraiNewsFeedItemView::QoraiNewsFeedItemView(const GURL& feed_url,
                                             content::WebContents* contents)
    : feed_url_(feed_url), contents_(contents) {
  DCHECK(contents_);
  tab_helper_ = QoraiNewsTabHelper::FromWebContents(contents);
  ...
  subscribe_button_ = AddChildView(std::make_unique<views::MdTextButton>(
      base::BindRepeating(&QoraiNewsFeedItemView::OnPressed,
...
QoraiNewsFeedItemView::~QoraiNewsFeedItemView() = default;

void QoraiNewsFeedItemView::Update() {
  ...
  subscribe_button_->SetText(l10n_util::GetStringUTF16(
      is_subscribed ? IDS_QORAI_NEWS_BUBBLE_FEED_ITEM_UNSUBSCRIBE
                    : IDS_QORAI_NEWS_BUBBLE_FEED_ITEM_SUBSCRIBE));
  ...
  subscribe_button_->SetIcon(
      is_subscribed ? &kQoraHeartFilledIcon : &kQoraHeartOutlineIcon,
      kFollowButtonIconSize);
}

void QoraiNewsFeedItemView::OnPressed() {
...
void QoraiNewsFeedItemView::OnAvailableFeedsChanged(
    const std::vector<GURL>& feed_urls) {
  loading_ = false;
  Update();
}

BEGIN_METADATA(QoraiNewsFeedItemView)
END_METADATA
