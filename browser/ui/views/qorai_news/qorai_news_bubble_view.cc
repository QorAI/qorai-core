...
#include "qorai/browser/ui/views/qorai_news/qorai_news_feed_item_view.h"
#include "qorai/browser/ui/views/qorai_news/qorai_news_feeds_container_view.h"
...
// static
base::WeakPtr<views::Widget> QoraiNewsBubbleView::Show(
    views::View* anchor,
    content::WebContents* contents) {
  auto* widget = views::BubbleDialogDelegateView::CreateBubble(
      std::make_unique<QoraiNewsBubbleView>(anchor, contents));
  widget->Show();
  return widget->GetWeakPtr();
}

QoraiNewsBubbleView::QoraiNewsBubbleView(views::View* action_view,
                                         content::WebContents* contents)
  ...
  auto* controller =
      qorai_news::QoraiNewsBubbleController::FromWebContents(contents);
  CHECK(controller);
  ...
  title_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_QORAI_NEWS_BUBBLE_TITLE), custom_font));
  ...
  subtitle_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_QORAI_NEWS_BUBBLE_SUBTITLE),
  ...
  feeds_container_ =
      AddChildView(std::make_unique<QoraiNewsFeedsContainerView>(contents));
  ...
  auto* manage_feeds_button =
      AddChildView(std::make_unique<views::MdTextButton>(
          base::BindRepeating(&QoraiNewsBubbleView::OpenManageFeeds,
                              base::Unretained(this)),
          l10n_util::GetStringUTF16(IDS_QORAI_NEWS_BUBBLE_MANAGE_FEEDS)));
  ...
  manage_feeds_button->SetIcon(&kQoraArrowRightIcon);
  manage_feeds_button->SetHorizontalAlignment(
      gfx::HorizontalAlignment::ALIGN_RIGHT);
}

QoraiNewsBubbleView::~QoraiNewsBubbleView() = default;

void QoraiNewsBubbleView::OpenManageFeeds() {
  auto* browser = chrome::FindBrowserWithTab(contents_);
  browser->OpenURL(
      {GURL("qorai://newtab/?openSettings=QoraiNews"), content::Referrer(),
       WindowOpenDisposition::NEW_FOREGROUND_TAB, ui::PAGE_TRANSITION_LINK,
       false},
      /*navigation_handle_callback=*/{});
}

void QoraiNewsBubbleView::OnWidgetDestroyed(views::Widget*) {
  if (!controller_) {
    return;
  }
...
void QoraiNewsBubbleView::OnThemeChanged() {
  views::BubbleDialogDelegateView::OnThemeChanged();

  auto is_dark = dark_mode::GetActiveQoraiDarkModeType() ==
...
BEGIN_METADATA(QoraiNewsBubbleView)
END_METADATA
