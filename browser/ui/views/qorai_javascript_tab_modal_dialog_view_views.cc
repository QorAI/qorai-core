...
QoraiJavaScriptTabModalDialogViewViews::QoraiJavaScriptTabModalDialogViewViews(
  ...
  widget->widget_delegate()->set_desired_position_delegate(base::BindRepeating(
      [](base::WeakPtr<QoraiJavaScriptTabModalDialogViewViews> dialog_view) {
        if (!dialog_view) {
          return gfx::Point();
        }
...
QoraiJavaScriptTabModalDialogViewViews::
    ~QoraiJavaScriptTabModalDialogViewViews() = default;
...
JavaScriptTabModalDialogManagerDelegateDesktop::CreateNewDialog(
  ...
  if (base::FeatureList::IsEnabled(features::kSideBySide)) {
    return (new QoraiJavaScriptTabModalDialogViewViews(
...
QoraiJavaScriptTabModalDialogViewViews::GetModalDialogHost() {
...
void QoraiJavaScriptTabModalDialogViewViews::UpdateWidgetBounds() {
  auto* widget = GetWidget();
  CHECK(widget);
...
gfx::Point QoraiJavaScriptTabModalDialogViewViews::
  ...
  ContentsWebView* target_web_view = nullptr;
    ...
    auto* multi_contents_view = browser_view->GetQoraiMultiContentsView();
...
BEGIN_METADATA(QoraiJavaScriptTabModalDialogViewViews)
END_METADATA
