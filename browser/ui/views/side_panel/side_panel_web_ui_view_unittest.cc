...
#include "qorai/browser/ui/views/side_panel/side_panel_web_ui_view_utils.h"
...
TEST(SidePanelWebUIViewUtilsTest,
  ...
  bool result = qorai::ShouldEnableContextMenu(ai_chat_url);
  EXPECT_TRUE(result);
}
...
TEST(SidePanelWebUIViewUtilsTest,
  ...
  bool result = qorai::ShouldEnableContextMenu(bookmarks_url);
  EXPECT_FALSE(result);
}
...
TEST(SidePanelWebUIViewUtilsTest,
  ...
  GURL invalid_url;
  bool result = qorai::ShouldEnableContextMenu(invalid_url);
  EXPECT_FALSE(result);
}
...
TEST(SidePanelWebUIViewUtilsTest,
  ...
  GURL empty_url("");
  bool result = qorai::ShouldEnableContextMenu(empty_url);
  EXPECT_FALSE(result);
}
