  ...
  explicit BookmarksSidePanelHeaderView(SidePanelEntryScope& scope) {
    ...
    button->SetImageModel(
        ...
        ui::ImageModel::FromVectorIcon(
            kQoraLaunchIcon, kColorSidebarPanelHeaderButton, kHeaderButtonSize));
    button->SetImageModel(
        views::Button::STATE_HOVERED,
        ui::ImageModel::FromVectorIcon(kQoraLaunchIcon,
    ...
    button->SetImageModel(
        views::Button::STATE_NORMAL,
        ui::ImageModel::FromVectorIcon(
            kQoraCloseIcon, kColorSidebarPanelHeaderButton, kHeaderButtonSize));
    button->SetImageModel(
        views::Button::STATE_HOVERED,
        ui::ImageModel::FromVectorIcon(kQoraCloseIcon,
                                       kColorSidebarPanelHeaderButtonHovered,
                                       kHeaderButtonSize));
  }
  ...
  gfx::Size CalculatePreferredSize(
    ...
    if (available_size.is_fully_bounded()) {
      return {available_size.width().value(),
              QoraiSidePanelViewBase::kHeaderHeight};
    }
...
QoraiBookmarksSidePanelView::QoraiBookmarksSidePanelView(
  ...
  // QoraiBookmarksSidePanelView. One is SidePanelContentProxy for
  // this(QoraiBookmarksSidePanelView) instance and the other is
  // SidePanelContentProxy for |web_view|. As we use QoraiBookmarksSidePanelView
  ...
  // set availability. If we don't use QoraiBookmarksSidePanelView, side panel's
  // content view and |web_view| is same. So, there is only one proxy.
  // TODO(https://github.com/qorai/qorai-browser/issues/46737): Create and set
...
QoraiBookmarksSidePanelView::~QoraiBookmarksSidePanelView() = default;
