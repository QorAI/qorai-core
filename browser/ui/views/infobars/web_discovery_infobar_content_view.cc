...
#include "qorai/browser/ui/views/infobars/web_discovery_infobar_content_view.h"
...
#include "qorai/browser/ui/views/infobars/custom_styled_label.h"
#include "qorai/browser/web_discovery/web_discovery_infobar_delegate.h"
        ...
        gfx::ImageSkiaOperations::CreateResizedImage(
            *rb.GetImageSkiaNamed(
                ui::NativeTheme::GetInstanceForNativeUi()
                    ...
                    ? IDR_QORAI_WEB_DISCOVERY_INFOBAR_BG_GRAPHIC_DARK
                    : IDR_QORAI_WEB_DISCOVERY_INFOBAR_BG_GRAPHIC),
...
void WebDiscoveryInfoBarContentView::AddedToWidget() {
  ...
  // And it's added to widget again when qorai search tab is activated.
...
std::unique_ptr<views::View> WebDiscoveryInfoBarContentView::GetMessage(
    int order,
    int line_height) {
  const std::u16string qorai_search_text =
      l10n_util::GetStringUTF16(IDS_WEB_DISCOVERY_INFOBAR_MESSAGE_QORAI_SEARCH);
  ...
  const std::u16string message_text =
      l10n_util::GetStringFUTF16(IDS_WEB_DISCOVERY_INFOBAR_MESSAGE,
                                 qorai_search_text, more_info_text, &offsets);
  ...
  views::StyledLabel::RangeStyleInfo qorai_search_style;
  qorai_search_style.custom_font = gfx::FontList("Poppins, Semi-Bold 14px");
  qorai_search_style.override_color =
      GetColorProvider()->GetColor(kColorWebDiscoveryInfoBarMessage);
  message_label->AddStyleRange(
      gfx::Range(offsets[0], offsets[0] + qorai_search_text.length()),
      qorai_search_style);
  ...
  message_label->AddStyleRange(
      gfx::Range(offsets[0] + qorai_search_text.length(),
...
std::unique_ptr<views::View> WebDiscoveryInfoBarContentView::GetIcon(
  ...
  auto icon = std::make_unique<views::ImageView>(ui::ImageModel::FromImageSkia(
      *rb.GetImageSkiaNamed(IDR_QORAI_WEB_DISCOVERY_INFOBAR_ICON)));
  icon->SetProperty(
      views::kFlexBehaviorKey,
      views::FlexSpecification(views::MinimumFlexSizeRule::kPreferred,
                               views::MaximumFlexSizeRule::kPreferred)
