...
std::unique_ptr<infobars::InfoBar> CreateQoraiConfirmInfoBar(
  ...
  return std::make_unique<QoraiConfirmInfoBar>(std::move(delegate));
}

QoraiConfirmInfoBar::QoraiConfirmInfoBar(
    std::unique_ptr<QoraiConfirmInfoBarDelegate> delegate)
  ...
  const auto create_button =
      [this](ConfirmInfoBarDelegate::InfoBarButton type,
             void (QoraiConfirmInfoBar::*click_function)()) {
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_OK) {
    ok_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_OK,
                               &QoraiConfirmInfoBar::OkButtonPressed);
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_CANCEL) {
    cancel_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_CANCEL,
                                   &QoraiConfirmInfoBar::CancelButtonPressed);
    if (buttons == ConfirmInfoBarDelegate::BUTTON_CANCEL ||
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_EXTRA) {
    extra_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_EXTRA,
                                  &QoraiConfirmInfoBar::ExtraButtonPressed);
    if (buttons == ConfirmInfoBarDelegate::BUTTON_EXTRA ||
  ...
  if (delegate_ptr->HasCheckbox()) {
    checkbox_ = AddChildView(std::make_unique<views::Checkbox>(
        ...
        base::BindRepeating(&QoraiConfirmInfoBar::CheckboxPressed,
  ...
  // that container.
  // TODO(https://github.com/qorai/qorai-browser/issues/48822): Add our children
...
QoraiConfirmInfoBar::~QoraiConfirmInfoBar() = default;

views::MdTextButton* QoraiConfirmInfoBar::GetButtonById(int id) {
  switch (id) {
...
void QoraiConfirmInfoBar::Layout(PassKey) {
...
void QoraiConfirmInfoBar::MaybeLayoutMultiLineLabelAndLink() {
  ...
  // height.
  QoraiSetTargetHeight(target_height);
}

void QoraiConfirmInfoBar::CheckboxPressed() {
  GetDelegate()->SetCheckboxChecked(checkbox_->GetChecked());
}

void QoraiConfirmInfoBar::CloseButtonPressed() {
...
void QoraiConfirmInfoBar::OkButtonPressed() {
  if (!owner()) {
...
void QoraiConfirmInfoBar::CancelButtonPressed() {
  if (!owner()) {
    return;  // We're closing; don't call anything, it might access the owner.
  }
...
void QoraiConfirmInfoBar::ExtraButtonPressed() {
  if (!owner()) {
    return;  // We're closing; don't call anything, it might access the owner.
  }
...
QoraiConfirmInfoBarDelegate* QoraiConfirmInfoBar::GetDelegate() {
  return reinterpret_cast<QoraiConfirmInfoBarDelegate*>(delegate());
}

const QoraiConfirmInfoBarDelegate* QoraiConfirmInfoBar::GetDelegate() const {
  return reinterpret_cast<const QoraiConfirmInfoBarDelegate*>(delegate());
}

int QoraiConfirmInfoBar::GetContentMinimumWidth() const {
  return label_->GetMinimumSize().width() + link_->GetMinimumSize().width() +
         NonLabelWidth();
}

int QoraiConfirmInfoBar::NonLabelWidth() const {
  ChromeLayoutProvider* layout_provider = ChromeLayoutProvider::Get();
...
BEGIN_METADATA(QoraiConfirmInfoBar)
END_METADATA
