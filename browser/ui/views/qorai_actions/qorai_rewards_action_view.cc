...
using qorai_rewards::RewardsNotificationService;
...
class ButtonHighlightPathGenerator : public views::HighlightPathGenerator {
 public:
  ...
  SkPath GetHighlightPath(const views::View* view) override {
    ...
    rect.Inset(gfx::Insets::TLBR(0, 0, 0, -1 * kQoraiActionLeftMarginExtra));
...
class RewardsBadgeImageSource : public qorai::QoraiIconWithBadgeImageSource {
 public:
 ...
 private:
  ...
  void PaintBadgeWithoutText(const gfx::Rect& badge_rect,
    ...
    if (!verified_icon_) {
      QoraiIconWithBadgeImageSource::PaintBadgeWithoutText(badge_rect, canvas);
      return;
    }
    ...
    auto image = gfx::CreateVectorIcon(kQoraVerificationFilledIcon,
...
class RewardsActionMenuModel : public ui::SimpleMenuModel,
 ...
 private:
  enum ContextMenuCommand { kHideQoraiRewardsIcon };

  // ui::SimpleMenuModel::Delegate override:
  void ExecuteCommand(int command_id, int event_flags) override {
    if (command_id == kHideQoraiRewardsIcon) {
      prefs_->SetBoolean(qorai_rewards::prefs::kShowLocationBarButton, false);
    }
  }

  void Build() {
    AddItemWithStringId(kHideQoraiRewardsIcon,
                        IDS_HIDE_QORAI_REWARDS_ACTION_ICON);
  }
...
std::unique_ptr<WebUIBubbleManager> CreateBubbleManager(
  ...
  return WebUIBubbleManager::Create<qorai_rewards::RewardsPageTopUI>(
      anchor_view, browser_window_interface, GURL(kRewardsPageTopURL),
      IDS_QORAI_UI_QORAI_REWARDS);
}

}  // namespace

QoraiRewardsActionView::QoraiRewardsActionView(
    BrowserWindowInterface* browser_window_interface)
    : ToolbarButton(
          base::BindRepeating(&QoraiRewardsActionView::OnButtonPressed,
  ...
  SetAccessibleName(l10n_util::GetStringUTF16(IDS_QORAI_UI_QORAI_REWARDS));

  auto* profile = browser_window_interface_->GetProfile();

  pref_change_registrar_.Init(profile->GetPrefs());
  pref_change_registrar_.Add(
      qorai_rewards::prefs::kBadgeText,
      base::BindRepeating(&QoraiRewardsActionView::OnPreferencesChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      qorai_rewards::prefs::kDeclaredGeo,
      base::BindRepeating(&QoraiRewardsActionView::OnPreferencesChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      qorai_rewards::prefs::kTosVersion,
      base::BindRepeating(&QoraiRewardsActionView::OnPreferencesChanged,
                          base::Unretained(this)));
...
QoraiRewardsActionView::~QoraiRewardsActionView() = default;

void QoraiRewardsActionView::Update() {
  ...
  image_source->SetBadge(std::make_unique<IconWithBadgeImageSource::Badge>(
      text, qorai::kBadgeTextColor, background_color));
...
void QoraiRewardsActionView::ClosePanelForTesting() {
  if (IsPanelOpen()) {
    ToggleRewardsPanel();
  }
}

gfx::Rect QoraiRewardsActionView::GetAnchorBoundsInScreen() const {
  if (!GetVisible()) {
...
QoraiRewardsActionView::CreateDefaultBorder() const {
  auto border = ToolbarButton::CreateDefaultBorder();
  border->set_insets(gfx::Insets::TLBR(0, 0, 0, 0));
  return border;
}

void QoraiRewardsActionView::OnThemeChanged() {
...
void QoraiRewardsActionView::OnWidgetDestroying(views::Widget* widget) {
  DCHECK(bubble_observation_.IsObservingSource(widget));
  bubble_observation_.Reset();
}

void QoraiRewardsActionView::OnTabStripModelChanged(
...
void QoraiRewardsActionView::OnPublisherForTabUpdated(
...
void QoraiRewardsActionView::OnRewardsPanelRequested() {
  if (!IsPanelOpen()) {
    ToggleRewardsPanel();
  }
}

void QoraiRewardsActionView::OnPublisherRegistryUpdated() {
  UpdatePublisherStatus();
}

void QoraiRewardsActionView::OnPublisherUpdated(
    const std::string& publisher_id) {
...
void QoraiRewardsActionView::OnNotificationAdded(
    RewardsNotificationService* service,
    const RewardsNotificationService::RewardsNotification& notification) {
  Update();
}

void QoraiRewardsActionView::OnNotificationDeleted(
    RewardsNotificationService* service,
    const RewardsNotificationService::RewardsNotification& notification) {
  Update();
}

void QoraiRewardsActionView::OnButtonPressed() {
  qorai_rewards::RewardsService* rewards_service = GetRewardsService();
...
void QoraiRewardsActionView::OnPreferencesChanged(const std::string& key) {
  Update();
}

content::WebContents* QoraiRewardsActionView::GetActiveWebContents() {
  return browser_window_interface_->GetTabStripModel()->GetActiveWebContents();
}

qorai_rewards::RewardsService* QoraiRewardsActionView::GetRewardsService() {
  return RewardsServiceFactory::GetForProfile(
      browser_window_interface_->GetProfile());
}

qorai_rewards::RewardsNotificationService*
QoraiRewardsActionView::GetNotificationService() {
  if (auto* rewards_service = GetRewardsService()) {
...
bool QoraiRewardsActionView::IsPanelOpen() {
  return bubble_observation_.IsObserving();
}

void QoraiRewardsActionView::ToggleRewardsPanel() {
  if (IsPanelOpen()) {
  ...
  prefs->SetString(qorai_rewards::prefs::kBadgeText, "");
...
gfx::ImageSkia QoraiRewardsActionView::GetRewardsIcon() {
  // Since the QOR icon has color the actual color value here is not relevant,
...
QoraiRewardsActionView::GetBadgeTextAndBackground() {
  ...
  std::string text_pref =
      browser_window_interface_->GetProfile()->GetPrefs()->GetString(
          qorai_rewards::prefs::kBadgeText);
  if (!text_pref.empty()) {
    return {text_pref, qorai::kBadgeNotificationBG};
  }
  ...
  if (notifications > 0) {
    ...
    return {text, qorai::kBadgeNotificationBG};
  }
  ...
  return {"", qorai::kBadgeNotificationBG};
}

size_t QoraiRewardsActionView::GetRewardsNotificationCount() {
  size_t count = 0;
  ...
  if (prefs->GetBoolean(qorai_rewards::prefs::kEnabled) &&
      prefs->GetString(qorai_rewards::prefs::kDeclaredGeo).empty()) {
    ++count;
  }
...
bool QoraiRewardsActionView::UpdatePublisherStatus() {
  ...
  rewards_service->IsPublisherRegistered(
      publisher_id,
      base::BindOnce(&QoraiRewardsActionView::IsPublisherRegisteredCallback,
                     weak_factory_.GetWeakPtr(), publisher_id));

  return true;
}

void QoraiRewardsActionView::IsPublisherRegisteredCallback(
    const std::string& publisher_id,
    bool is_registered) {
  if (publisher_id == std::get<std::string>(publisher_registered_)) {
...
void QoraiRewardsActionView::UpdateTabHelper(
...
BEGIN_METADATA(QoraiRewardsActionView)
END_METADATA
