...
QoraiVideoOverlayWindowViews::QoraiVideoOverlayWindowViews(
    content::VideoPictureInPictureWindowController* controller)
    : VideoOverlayWindowViews(controller) {
  CHECK(!base::FeatureList::IsEnabled(
      media::kVideoPictureInPictureControlsUpdate2024))
      << "QoraiVideoOverlayWindowViews class was implemented with the "
...
QoraiVideoOverlayWindowViews::~QoraiVideoOverlayWindowViews() = default;

void QoraiVideoOverlayWindowViews::SetUpViews() {
  ...
  fullscreen_button_ =
      controls_container_view_->AddChildView(std::make_unique<CloseImageButton>(
          base::BindRepeating(&QoraiVideoOverlayWindowViews::RequestFullscreen,
...
void QoraiVideoOverlayWindowViews::OnUpdateControlsBounds() {
  VideoOverlayWindowViews::OnUpdateControlsBounds();

  // Copies size in order to avoid ASAN failure
  // https://github.com/qorai/internal/issues/1108
...
void QoraiVideoOverlayWindowViews::UpdateControlIcons() {
  close_controls_view_->SetImageModel(
      views::Button::STATE_NORMAL,
      ui::ImageModel::FromVectorIcon(kQoraCloseIcon, kColorPipWindowForeground,
                                     kTopControlIconSize));
  fullscreen_button_->SetImageModel(
      views::Button::STATE_NORMAL,
      ui::ImageModel::FromVectorIcon(kQoraFullscreenOnIcon,
  ...
  if (back_to_tab_label_button_) {
    back_to_tab_label_button_->SetImageModel(
        views::Button::STATE_NORMAL,
        ui::ImageModel::FromVectorIcon(kQoraPictureInPictureReturnIcon,
                                       kColorPipWindowForeground,
                                       kTopControlIconSize));
  ...
  previous_track_controls_view_->override_icon(kQoraPreviousOutlineIcon);
  next_track_controls_view_->override_icon(kQoraNextOutlineIcon);
}

void QoraiVideoOverlayWindowViews::SetMediaPosition(
...
void QoraiVideoOverlayWindowViews::SetPlaybackState(
...
bool QoraiVideoOverlayWindowViews::ControlsHitTestContainsPoint(
...
void QoraiVideoOverlayWindowViews::SetSeekerEnabled(bool enabled) {
  seeker_->SetEnabled(enabled);
}

void QoraiVideoOverlayWindowViews::ShowInactive() {
  VideoOverlayWindowViews::ShowInactive();
  UpdateTimestampPeriodically();
}

void QoraiVideoOverlayWindowViews::Close() {
  timestamp_update_timer_.Stop();
  VideoOverlayWindowViews::Close();
}

void QoraiVideoOverlayWindowViews::Hide() {
  timestamp_update_timer_.Stop();
  VideoOverlayWindowViews::Hide();
}

int QoraiVideoOverlayWindowViews::GetNonClientComponent(
    const gfx::Point& point) {
...
void QoraiVideoOverlayWindowViews::OnKeyEvent(ui::KeyEvent* event) {
...
void QoraiVideoOverlayWindowViews::SliderValueChanged(
    views::Slider* sender,
    float value,
    float old_value,
...
void QoraiVideoOverlayWindowViews::SliderDragStarted(views::Slider* sender) {
  timestamp_update_timer_.Stop();
  is_seeking_ = true;
...
void QoraiVideoOverlayWindowViews::SliderDragEnded(views::Slider* sender) {
  is_seeking_ = false;
  UpdateTimestampPeriodically();
...
void QoraiVideoOverlayWindowViews::UpdateTimestampPosition() {
  CHECK(timestamp_);
...
void QoraiVideoOverlayWindowViews::UpdateTimestampPeriodically() {
  ...
  if (should_update_timestamp_periodically) {
    ...
    timestamp_update_timer_.Start(
        ...
        base::BindRepeating(
            &QoraiVideoOverlayWindowViews::UpdateTimestampPeriodically,
            base::Unretained(this)));
  } else {
    timestamp_update_timer_.Stop();
  }
...
void QoraiVideoOverlayWindowViews::RequestFullscreen() {
