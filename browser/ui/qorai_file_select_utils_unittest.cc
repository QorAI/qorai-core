...
TEST(QoraiFileSelectUtilsUnitTest, GetSiteFrameTitle_InSyncWithUpstream) {
  ...
  for (const auto& test_case : kCases) {
    ...
    EXPECT_EQ(javascript_dialogs::AppModalDialogManager::GetSiteFrameTitle(
              ...
              qorai::GetSiteFrameTitleForFileSelect(
                  qorai::GetSiteFrameTitleType(main_frame_origin,
                                               alerting_frame_origin),
                  alerting_frame_origin,
                  qorai::FileSelectTitleType::kChromiumDefault));
  }
}

TEST(QoraiFileSelectUtilsUnitTest, GetSiteFrameTitleForFileSelect_Open) {
  constexpr struct Case {
    // The name of the test case.
    const char* case_name;
  ...
  qorai_l10n::test::ScopedDefaultLocale scoped_locale("en-US");
  ...
  for (const auto& test_case : kCases) {
    ...
    EXPECT_EQ(base::UTF8ToUTF16(test_case.expected),
              qorai::GetSiteFrameTitleForFileSelect(
                  qorai::GetSiteFrameTitleType(main_frame_origin,
                                               alerting_frame_origin),
                  alerting_frame_origin, qorai::FileSelectTitleType::kOpen));
  }
}

TEST(QoraiFileSelectUtilsUnitTest, GetSiteFrameTitleForFileSelect_Save) {
  constexpr struct Case {
    // The name of the test case.
    const char* case_name;
  ...
  for (const auto& test_case : kCases) {
    ...
    EXPECT_EQ(base::UTF8ToUTF16(test_case.expected),
              qorai::GetSiteFrameTitleForFileSelect(
                  qorai::GetSiteFrameTitleType(main_frame_origin,
                                               alerting_frame_origin),
                  alerting_frame_origin, qorai::FileSelectTitleType::kSave));
  }
}
