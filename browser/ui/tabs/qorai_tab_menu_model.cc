...
QoraiTabMenuModel::QoraiTabMenuModel(
...
#endif  // BUILDFLAG(ENABLE_CONTAINERS)
{
  ...
  auto indices = static_cast<QoraiTabStripModel*>(tab_strip_model)
...
QoraiTabMenuModel::~QoraiTabMenuModel() = default;

int QoraiTabMenuModel::GetRestoreTabCommandStringId() const {
...
std::u16string QoraiTabMenuModel::GetLabelAt(size_t index) const {
  if (!is_vertical_tab_) {
...
bool QoraiTabMenuModel::IsNewFeatureAt(size_t index) const {
...
void QoraiTabMenuModel::Build(Browser* browser,
                              TabStripModel* tab_strip_model,
                              int selected_index,
  ...
  // Replace SplitTabMenuModel with QoraiSplitTabMenuModel.
  if (arrange_split_view_submenu_) {
    ...
    arrange_split_view_submenu_ = std::make_unique<QoraiSplitTabMenuModel>(
...
#if BUILDFLAG(ENABLE_CONTAINERS)
void QoraiTabMenuModel::BuildItemForContainers(
...
void QoraiTabMenuModel::BuildItemForCustomization(
    TabStripModel* tab_strip_model,
    int tab_index) {
