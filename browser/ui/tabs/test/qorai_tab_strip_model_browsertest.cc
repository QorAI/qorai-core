...
using QoraiTabStripModelTest = InProcessBrowserTest;

IN_PROC_BROWSER_TEST_F(QoraiTabStripModelTest, MRUCyclingBasic) {
...
IN_PROC_BROWSER_TEST_F(QoraiTabStripModelTest, TabClosingWhileMRUCycling) {
  // Activate MRU cycling
  browser()->profile()->GetPrefs()->SetBoolean(kMRUCyclingEnabled, true);
...
class QoraiTabStripModelRenamingTabBrowserTest : public QoraiTabStripModelTest {
 public:
  ...
  QoraiTabStripModelRenamingTabBrowserTest() {
    scoped_feature_list_.InitAndEnableFeature(
        tabs::features::kQoraiRenamingTabs);
  }
  ~QoraiTabStripModelRenamingTabBrowserTest() override = default;

  void SetUpOnMainThread() override {
    host_resolver()->AddRule("*", "127.0.0.1");
    auto request_handler = base::BindRepeating(
        &QoraiTabStripModelRenamingTabBrowserTest::ServeTestPage);
...
IN_PROC_BROWSER_TEST_F(QoraiTabStripModelRenamingTabBrowserTest,
                       SettingCustomTabTitle_TabRendererDataUpdated) {
  QoraiTabStripModel* tab_strip_model =
...
IN_PROC_BROWSER_TEST_F(QoraiTabStripModelRenamingTabBrowserTest,
  ...
  auto custom_tab_it = extra_data.find(tabs::kQoraiTabCustomTitleExtraDataKey);
