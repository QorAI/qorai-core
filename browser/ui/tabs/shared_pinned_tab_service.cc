...
SharedPinnedTabService::SharedPinnedTabService(Profile* profile)
    : profile_(profile) {
  DCHECK(base::FeatureList::IsEnabled(tabs::features::kQoraiSharedPinnedTabs));
  profile_observation_.Observe(profile_);

  shared_pinned_tab_enabled_.Init(
      qorai_tabs::kSharedPinnedTab, profile_->GetPrefs(),
...
bool SharedPinnedTabService::IsBrowserInTabDragging(Browser* browser) const {
  CHECK(browser);
  return static_cast<QoraiBrowserWindow*>(browser->window())->IsInTabDragging();
}
