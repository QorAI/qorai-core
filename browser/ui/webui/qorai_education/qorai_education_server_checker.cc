...
#include "qorai/browser/ui/webui/qorai_education/qorai_education_server_checker.h"
...
constexpr auto kTrafficAnnotation =
    net::DefineNetworkTrafficAnnotation("qorai_education_server_checker",
                                        R"(
      semantics {
        sender: "Qorai Education"
        description: "Attempts to fetch the content for a Qorai Education
          server page to ensure that it loads successfully."
        trigger:
          "Completing the Qorai Welcome UX flow."
        data:
...
QoraiEducationServerChecker::QoraiEducationServerChecker(
    PrefService& pref_service,
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory)
    : pref_service_(pref_service), url_loader_factory_(url_loader_factory) {}

QoraiEducationServerChecker::~QoraiEducationServerChecker() = default;

void QoraiEducationServerChecker::IsServerPageAvailable(
  ...
  auto download_callback = base::BindOnce(
      &QoraiEducationServerChecker::OnURLResponse, weak_factory_.GetWeakPtr(),
...
void QoraiEducationServerChecker::OnURLResponse(
    EducationPageType page_type,
