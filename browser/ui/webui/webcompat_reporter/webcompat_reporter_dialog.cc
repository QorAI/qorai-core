...
#include "qorai/components/qorai_shields/core/common/qorai_shields_panel.mojom-shared.h"
...
GURL WebcompatReporterDialogDelegate::GetDialogContentURL() const {
  return GURL(kQoraiUIWebcompatReporterURL);
}
...
void OpenReporterDialog(content::WebContents* initiator, UISource source) {
  ...
  qorai_shields::mojom::FingerprintMode fp_block_mode =
      qorai_shields::mojom::FingerprintMode::STANDARD_MODE;
  qorai_shields::mojom::AdBlockMode ad_block_mode =
      qorai_shields::mojom::AdBlockMode::STANDARD;
  qorai_shields::QoraiShieldsTabHelper* shields_data_controller =
      qorai_shields::QoraiShieldsTabHelper::FromWebContents(initiator);
  if (shields_data_controller != nullptr) {
    shields_enabled = shields_data_controller->GetQoraiShieldsEnabled();
