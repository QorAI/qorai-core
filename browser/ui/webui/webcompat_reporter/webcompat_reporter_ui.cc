...
#include "qorai/components/webcompat_reporter/resources/grit/webcompat_reporter_generated_map.h"
...
constexpr char kUISourceHistogramName[] = "Qorai.Webcompat.UISource";
...
void WebcompatReporterDOMHandler::InitAdditionalParameters(Profile* profile) {
  ...
  if (vpn_service != nullptr) {
    pending_report_->qorai_vpn_connected =
  ...
  pending_report_->language_farbling = BoolToString(
      profile_prefs->GetBoolean(qorai_shields::prefs::kReduceLanguageEnabled));
  pending_report_->channel = qorai::GetChannelName();
}
