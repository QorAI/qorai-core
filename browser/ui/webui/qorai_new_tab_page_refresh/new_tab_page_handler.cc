...
void NewTabPageHandler::GetQoraiBackgrounds(
    GetQoraiBackgroundsCallback callback) {
  std::move(callback).Run(background_facade_->GetQoraiBackgrounds());
}
...
void NewTabPageHandler::SetShowSearchBox(bool show_search_box,
  ...
  pref_service_->SetBoolean(qorai_search_conversion::prefs::kShowNTPSearchBox,
                            show_search_box);
  std::move(callback).Run();
}
...
void NewTabPageHandler::GetSearchSuggestionsPromptDismissed(
  ...
  std::move(callback).Run(
      pref_service_->GetBoolean(qorai_search_conversion::prefs::kDismissed));
}

void NewTabPageHandler::SetSearchSuggestionsPromptDismissed(
    bool dismissed,
    SetSearchSuggestionsPromptDismissedCallback callback) {
  pref_service_->SetBoolean(qorai_search_conversion::prefs::kDismissed,
                            dismissed);
  std::move(callback).Run();
}
...
void NewTabPageHandler::SetLastUsedSearchEngine(
  ...
  pref_service_->SetString(
      qorai_search_conversion::prefs::kLastUsedNTPSearchEngine, engine_host);
  std::move(callback).Run();
}
...
void NewTabPageHandler::GetShowTalkWidget(GetShowTalkWidgetCallback callback) {
  std::move(callback).Run(pref_service_->GetBoolean(kNewTabPageShowQoraiTalk));
}

void NewTabPageHandler::SetShowTalkWidget(bool show_talk_widget,
                                          SetShowTalkWidgetCallback callback) {
  pref_service_->SetBoolean(kNewTabPageShowQoraiTalk, show_talk_widget);
  std::move(callback).Run();
}
...
void NewTabPageHandler::OpenVPNAccountPage(
    qorai_vpn::mojom::ManageURLType url_type,
