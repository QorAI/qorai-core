...
#include "qorai/components/qorai_rewards/resources/grit/rewards_internals_generated_map.h"
...
class RewardsInternalsDOMHandler : public content::WebUIMessageHandler {
 ...
 private:
  ...
  void OnGetRewardsInternalsInfo(
      qorai_rewards::mojom::RewardsInternalsInfoPtr info);
  void GetBalance(const base::Value::List& args);
  void OnGetBalance(qorai_rewards::mojom::BalancePtr balance);
  void GetContributions(const base::Value::List& args);
  void OnGetContributions(
      std::vector<qorai_rewards::mojom::ContributionInfoPtr> contributions);
  ...
  void OnGetEventLogs(std::vector<qorai_rewards::mojom::EventLogPtr> logs);
  ...
  void OnGetEnvironment(qorai_rewards::mojom::Environment environment);

  raw_ptr<qorai_rewards::RewardsService> rewards_service_ =
      nullptr;                                            // NOT OWNED
  raw_ptr<qorai_ads::AdsService> ads_service_ = nullptr;  // NOT OWNED
...
void RewardsInternalsDOMHandler::Init() {
  ...
  rewards_service_ =
      qorai_rewards::RewardsServiceFactory::GetForProfile(profile_);
...
void RewardsInternalsDOMHandler::OnGetRewardsInternalsInfo(
  ...
  if (info) {
    ...
    const std::string declared_geo =
        prefs->GetString(::qorai_rewards::prefs::kDeclaredGeo);
    const int wallet_creation_environment =
        prefs->GetInteger(::qorai_rewards::prefs::kWalletCreationEnvironment);
  ...
  CallJavascriptFunction("qorai_rewards_internals.onGetRewardsInternalsInfo",
                         info_dict);
}
...
void RewardsInternalsDOMHandler::OnGetBalance(
  ...
  CallJavascriptFunction("qorai_rewards_internals.balance", std::move(data));
}
...
void RewardsInternalsDOMHandler::OnGetContributions(
  ...
  CallJavascriptFunction("qorai_rewards_internals.contributions", list);
}
...
void RewardsInternalsDOMHandler::OnGetPartialLog(const std::string& log) {
  ...
  CallJavascriptFunction("qorai_rewards_internals.partialLog",
                         base::Value(log));
}
...
void RewardsInternalsDOMHandler::OnGetFulllLog(const std::string& log) {
  ...
  CallJavascriptFunction("qorai_rewards_internals.fullLog", base::Value(log));
}
...
void RewardsInternalsDOMHandler::OnClearLog(const bool success) {
  ...
  CallJavascriptFunction("qorai_rewards_internals.partialLog", base::Value(""));
}
...
void RewardsInternalsDOMHandler::OnGetExternalWallet(
  ...
  if (!wallet) {
    CallJavascriptFunction("qorai_rewards_internals.onGetExternalWallet");
    return;
  }
  ...
  CallJavascriptFunction("qorai_rewards_internals.onGetExternalWallet", data);
}
...
void RewardsInternalsDOMHandler::OnGetEventLogs(
    std::vector<qorai_rewards::mojom::EventLogPtr> logs) {
  if (!IsJavascriptAllowed()) {
    return;
  }
  ...
  CallJavascriptFunction("qorai_rewards_internals.eventLogs", data);
}
...
void RewardsInternalsDOMHandler::OnGetAdDiagnostics(
  ...
  const std::string& diagnostic_id =
      prefs->GetString(qorai_ads::prefs::kDiagnosticId);
  ...
  CallJavascriptFunction("qorai_rewards_internals.adDiagnostics", diagnostics);
}
...
void RewardsInternalsDOMHandler::SetAdDiagnosticId(
  ...
  prefs->SetString(qorai_ads::prefs::kDiagnosticId, args[0].GetString());
}
...
void RewardsInternalsDOMHandler::OnGetEnvironment(
    qorai_rewards::mojom::Environment environment) {
  if (!IsJavascriptAllowed()) {
    return;
  }

  CallJavascriptFunction("qorai_rewards_internals.environment",
                         base::Value(static_cast<int>(environment)));
}

}  // namespace

QoraiRewardsInternalsUI::QoraiRewardsInternalsUI(content::WebUI* web_ui,
                                                 const std::string& name)
    : WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, name, kRewardsInternalsGenerated,
                              IDR_QORAI_REWARDS_INTERNALS_HTML);
...
QoraiRewardsInternalsUI::~QoraiRewardsInternalsUI() = default;
