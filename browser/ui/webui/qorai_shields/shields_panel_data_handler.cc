...
#include "qorai/browser/ui/webui/qorai_shields/shields_panel_data_handler.h"
...
using qorai_shields::mojom::SiteSettings;
...
void ShieldsPanelDataHandler::RegisterUIHandler(
    mojo::PendingRemote<qorai_shields::mojom::UIHandler> ui_handler_receiver) {
...
void ShieldsPanelDataHandler::SetQoraiShieldsEnabled(bool is_enabled) {
  if (!active_shields_data_controller_) {
    return;
  }

  active_shields_data_controller_->SetQoraiShieldsEnabled(is_enabled);
}

void ShieldsPanelDataHandler::SetQoraiShieldsAdBlockOnlyModeEnabled(
    bool is_enabled) {
  if (!active_shields_data_controller_) {
    return;
  }

  active_shields_data_controller_->SetQoraiShieldsAdBlockOnlyModeEnabled(
      is_enabled);
}

void ShieldsPanelDataHandler::SetQoraiShieldsAdBlockOnlyModePromptDismissed() {
  if (!active_shields_data_controller_) {
    return;
  }
  active_shields_data_controller_
      ->SetQoraiShieldsAdBlockOnlyModePromptDismissed();
...
void ShieldsPanelDataHandler::AreAnyBlockedElementsPresent(
  ...
  std::move(callback).Run(
      g_qorai_browser_process->ad_block_service()->AreAnyBlockedElementsPresent(
...
void ShieldsPanelDataHandler::UpdateSiteBlockInfo() {
  ...
  site_block_info_.is_qorai_shields_enabled =
      active_shields_data_controller_->GetQoraiShieldsEnabled();
  site_block_info_.is_qorai_shields_ad_block_only_mode_enabled =
      active_shields_data_controller_->IsQoraiShieldsAdBlockOnlyModeEnabled();
  site_block_info_.show_shields_disabled_ad_block_only_mode_prompt =
      active_shields_data_controller_
          ->ShouldShowShieldsDisabledAdBlockOnlyModePrompt();
  site_block_info_.is_qorai_shields_managed =
      active_shields_data_controller_->IsQoraiShieldsManaged();
...
void ShieldsPanelDataHandler::OnTabStripModelChanged(
  ...
  if (selection.active_tab_changed()) {
    ...
    if (selection.new_contents) {
      active_shields_data_controller_ =
          QoraiShieldsTabHelper::FromWebContents(selection.new_contents);
      active_shields_data_controller_->AddObserver(this);
