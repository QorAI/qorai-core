...
#include "qorai/components/qorai_shields/core/common/qorai_shield_localized_strings.h"
#include "qorai/components/qorai_shields/core/common/features.h"
#include "qorai/components/qorai_shields/resources/panel/grit/qorai_shields_panel_generated_map.h"
...
ShieldsPanelUI::ShieldsPanelUI(content::WebUI* web_ui)
  ...
  for (const auto& str : qorai_shields::kLocalizedStrings) {
  ...
  source->AddBoolean("isQoraiForgetFirstPartyStorageFeatureEnabled",
                     base::FeatureList::IsEnabled(
  ...
  webui::SetupWebUIDataSource(source, kQoraiShieldsPanelGenerated,
...
void ShieldsPanelUI::BindInterface(
    mojo::PendingReceiver<qorai_shields::mojom::PanelHandlerFactory> receiver) {
  panel_factory_receiver_.reset();
  panel_factory_receiver_.Bind(std::move(receiver));
}

void ShieldsPanelUI::CreatePanelHandler(
    mojo::PendingReceiver<qorai_shields::mojom::PanelHandler> panel_receiver,
