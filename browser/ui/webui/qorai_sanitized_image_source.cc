...
constexpr char kChromeUIQoraiImageURL[] = "chrome://qorai-image/";
constexpr char kChromeUIQoraiImageHost[] = "qorai-image";
...
QoraiSanitizedImageSource::~QoraiSanitizedImageSource() = default;

void QoraiSanitizedImageSource::OnImageLoaded(
  ...
  if (pcdn_domain_.empty()) {
    pcdn_domain_ = qorai_domains::GetServicesDomain("pcdn");
  }
  ...
  if (loader->NetError() == net::OK && body &&
    ...
    if (!qorai::private_cdn::RemovePadding(&body_payload)) {
      std::move(callback).Run(nullptr);
      return;
    }
  ...
  auto cb =
      base::BindOnce(&QoraiSanitizedImageSource::EncodeAndReplyStaticImage,
...
void QoraiSanitizedImageSource::StartDataRequest(
    const GURL& url,
  ...
  if (url !=
      GURL(base::StrCat({kChromeUIQoraiImageURL, "?", image_url_or_params}))) {
    std::move(callback).Run(nullptr);
    return;
  }
...
QoraiSanitizedImageSource::QoraiSanitizedImageSource(Profile* profile)
    : QoraiSanitizedImageSource(profile,
                                profile->GetDefaultStoragePartition()
                                    ->GetURLLoaderFactoryForBrowserProcess()) {}

QoraiSanitizedImageSource::QoraiSanitizedImageSource(
    Profile* profile,
...
bool QoraiSanitizedImageSource::AllowCaching() {
  return false;
}
std::string QoraiSanitizedImageSource::GetSource() {
  return kChromeUIQoraiImageHost;
}

void QoraiSanitizedImageSource::EncodeAndReplyStaticImage(
    RequestAttributes request_attributes,
    content::URLDataSource::GotDataCallback callback,
...
void QoraiSanitizedImageSource::StartImageDownload(
    RequestAttributes request_attributes,
    content::URLDataSource::GotDataCallback callback) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  const net::NetworkTrafficAnnotationTag traffic_annotation =
      ...
      semantics {
        sender: "Qorai WebUI Sanitized Image Source"
        description:
          "This data source fetches an arbitrary image to be displayed in a "
          "Qorai WebUI."
        trigger:
          "When a WebUI triggers the download of chrome://qorai-image?<URL> or "
          "chrome://qorai-image?url=<URL> by e.g. setting "
  ...
  loader_ptr->DownloadToString(
      ...
      base::BindOnce(&QoraiSanitizedImageSource::OnImageLoaded,
                     weak_ptr_factory_.GetWeakPtr(), std::move(loader),
                     std::move(request_attributes), std::move(callback)),
      network::SimpleURLLoader::kMaxBoundedStringDownloadSize);
}

std::string QoraiSanitizedImageSource::GetMimeType(const GURL&) {
  return "image/png";
}
