...
#include "qorai/browser/ntp_background/new_tab_takeover_infobar_delegate.h"
...
#include "qorai/components/p3a/utils.h"
...
base::Value::Dict GetPreferencesDictionary(PrefService* prefs) {
  ...
  pref_data.Set("showToday",
                prefs->GetBoolean(qorai_news::prefs::kNewTabPageShowToday));
  ...
  pref_data.Set("isQoraiNewsOptedIn",
                prefs->GetBoolean(qorai_news::prefs::kQoraiNewsOptedIn));
  pref_data.Set(
      "isQoraiNewsDisabledByPolicy",
      prefs->GetBoolean(qorai_news::prefs::kQoraiNewsDisabledByPolicy));
  pref_data.Set("hideAllWidgets", prefs->GetBoolean(kNewTabPageHideAllWidgets));
  pref_data.Set("showQoraiTalk", prefs->GetBoolean(kNewTabPageShowQoraiTalk));
  pref_data.Set("isQoraiTalkDisabledByPolicy",
                prefs->GetBoolean(kQoraiTalkDisabledByPolicy));
#if BUILDFLAG(ENABLE_QORAI_VPN)
  pref_data.Set("showQoraiVPN", prefs->GetBoolean(kNewTabPageShowQoraiVPN));
#endif
  pref_data.Set(
      "showSearchBox",
      prefs->GetBoolean(qorai_search_conversion::prefs::kShowNTPSearchBox));
  pref_data.Set("lastUsedNtpSearchEngine",
                prefs->GetString(
                    qorai_search_conversion::prefs::kLastUsedNTPSearchEngine));
  pref_data.Set("promptEnableSearchSuggestions",
                prefs->GetBoolean(
                    qorai_search_conversion::prefs::kPromptEnableSuggestions));
...
constexpr char kNTPCustomizeUsageStatus[] =
    "qorai.new_tab_page.customize_p3a_usage";
...
// static
void QoraiNewTabMessageHandler::RegisterLocalStatePrefs(
    PrefRegistrySimple* local_state) {
  local_state->RegisterIntegerPref(kNTPCustomizeUsageStatus, -1);
}

void QoraiNewTabMessageHandler::RecordInitialP3AValues(
...
// static
QoraiNewTabMessageHandler* QoraiNewTabMessageHandler::Create(
  ...
  if (!ads_service) {
    ...
    is_ads_supported_locale = qorai_ads::IsSupportedRegion();
  }

  source->AddBoolean("featureFlagQoraiNTPSponsoredImagesWallpaper",
  ...
  return new QoraiNewTabMessageHandler(profile, was_restored);
}

QoraiNewTabMessageHandler::QoraiNewTabMessageHandler(Profile* profile,
                                                     bool was_restored)
    : profile_(profile), was_restored_(was_restored), weak_ptr_factory_(this) {
  ads_service_ = qorai_ads::AdsServiceFactory::GetForProfile(profile_);
}

QoraiNewTabMessageHandler::~QoraiNewTabMessageHandler() = default;

void QoraiNewTabMessageHandler::RegisterMessages() {
  ...
  plural_string_handler->AddLocalizedString("QORAI_NEWS_SOURCE_COUNT",
                                            IDS_QORAI_NEWS_SOURCE_COUNT);
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleGetPreferences,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getNewTabPageStats",
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleGetStats,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getNewTabAdsData",
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleGetNewTabAdsData,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "saveNewTabPagePref",
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleSaveNewTabPagePref,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "registerNewTabPageView",
      base::BindRepeating(
          &QoraiNewTabMessageHandler::HandleRegisterNewTabPageView,
          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "recordClickedAdEvent",
      base::BindRepeating(
          &QoraiNewTabMessageHandler::HandleBrandedWallpaperLogoClicked,
          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getWallpaperData",
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleGetWallpaperData,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "customizeClicked",
      base::BindRepeating(&QoraiNewTabMessageHandler::HandleCustomizeClicked,
                          base::Unretained(this)));
}

void QoraiNewTabMessageHandler::OnJavascriptAllowed() {
  ...
  pref_change_registrar_.Add(
      kAdsBlocked,
      base::BindRepeating(&QoraiNewTabMessageHandler::OnStatsChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kTrackersBlocked,
      base::BindRepeating(&QoraiNewTabMessageHandler::OnStatsChanged,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&QoraiNewTabMessageHandler::OnPreferencesChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&QoraiNewTabMessageHandler::OnPreferencesChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      qorai_search_conversion::prefs::kShowNTPSearchBox,
      base::BindRepeating(&QoraiNewTabMessageHandler::OnPreferencesChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      qorai_search_conversion::prefs::kLastUsedNTPSearchEngine,
...
void QoraiNewTabMessageHandler::OnJavascriptDisallowed() {
  pref_change_registrar_.RemoveAll();
  bat_ads_observer_receiver_.reset();
  weak_ptr_factory_.InvalidateWeakPtrs();
}

void QoraiNewTabMessageHandler::HandleGetPreferences(
...
void QoraiNewTabMessageHandler::HandleGetStats(const base::Value::List& args) {
  AllowJavascript();
  PrefService* prefs = profile_->GetPrefs();
...
void QoraiNewTabMessageHandler::HandleGetNewTabAdsData(
    const base::Value::List& args) {
  AllowJavascript();
...
void QoraiNewTabMessageHandler::HandleSaveNewTabPagePref(
    const base::Value::List& args) {
  if (args.size() != 2) {
  ...
  if (settings_key_input != "showToday" &&
      settings_key_input != "isQoraiNewsOptedIn" &&
  ...
  if (settings_value.is_string()) {
    ...
    if (settings_key_input == "clockFormat") {
      ...
      settings_key = qorai_search_conversion::prefs::kLastUsedNTPSearchEngine;
    } else {
  ...
  if (settings_key_input == "showBackgroundImage") {
    ...
    settings_key = qorai_news::prefs::kNewTabPageShowToday;
  } else if (settings_key_input == "isQoraiNewsOptedIn") {
    settings_key = qorai_news::prefs::kQoraiNewsOptedIn;
  ...
  } else if (settings_key_input == "showQoraiTalk") {
    settings_key = kNewTabPageShowQoraiTalk;
#if BUILDFLAG(ENABLE_QORAI_VPN)
  } else if (settings_key_input == "showQoraiVPN") {
    settings_key = kNewTabPageShowQoraiVPN;
#endif
  } else if (settings_key_input == "showSearchBox") {
    settings_key = qorai_search_conversion::prefs::kShowNTPSearchBox;
  } else if (settings_key_input == "promptEnableSearchSuggestions") {
    settings_key = qorai_search_conversion::prefs::kPromptEnableSuggestions;
...
void QoraiNewTabMessageHandler::HandleRegisterNewTabPageView(
    const base::Value::List& args) {
  AllowJavascript();
...
void QoraiNewTabMessageHandler::HandleBrandedWallpaperLogoClicked(
    const base::Value::List& args) {
  AllowJavascript();
  if (args.size() != 1) {
    LOG(ERROR) << "Invalid input";
  ...
  qorai_ads::mojom::NewTabPageAdMetricType metric_type =
      qorai_ads::mojom::NewTabPageAdMetricType::kConfirmation;
  if (std::optional<int> value =
          dict->FindInt(ntp_background_images::kWallpaperMetricTypeKey)) {
    metric_type = static_cast<qorai_ads::mojom::NewTabPageAdMetricType>(*value);
  }
...
void QoraiNewTabMessageHandler::HandleGetWallpaperData(
    const base::Value::List& args) {
  AllowJavascript();
...
void QoraiNewTabMessageHandler::HandleCustomizeClicked(
    const base::Value::List& args) {
  AllowJavascript();
  p3a::RecordValueIfGreater<NTPCustomizeUsage>(
...
void QoraiNewTabMessageHandler::OnStatsChanged() {
  PrefService* prefs = profile_->GetPrefs();
  auto data = GetStatsDictionary(prefs);
  FireWebUIListener("stats-updated", data);
}

void QoraiNewTabMessageHandler::OnPreferencesChanged() {
  PrefService* prefs = profile_->GetPrefs();
  auto data = GetPreferencesDictionary(prefs);
  FireWebUIListener("preferences-changed", data);
}

base::Value::Dict QoraiNewTabMessageHandler::GetAdsDataDictionary() const {
  if (!ads_service_) {
    return {};
  }
...
void QoraiNewTabMessageHandler::OnBrowserUpgradeRequiredToServeAds() {
