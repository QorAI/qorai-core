...
bool IsNTPPromotionEnabled(Profile* profile) {
  if (!qorai_search_conversion::IsNTPPromotionEnabled(
          profile->GetPrefs(),
...
QoraiNewTabPageHandler::QoraiNewTabPageHandler(
...
QoraiNewTabPageHandler::~QoraiNewTabPageHandler() = default;

void QoraiNewTabPageHandler::InitForSearchPromotion() {
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&QoraiNewTabPageHandler::OnSearchPromotionDismissed,
                          base::Unretained(this)));
  template_url_service_observation_.Observe(
      TemplateURLServiceFactory::GetForProfile(profile_));

  qorai_search_conversion::p3a::RecordPromoShown(
      g_browser_process->local_state(),
      qorai_search_conversion::ConversionType::kNTP);
}

void QoraiNewTabPageHandler::ChooseLocalCustomBackground() {
...
void QoraiNewTabPageHandler::UseCustomImageBackground(
...
void QoraiNewTabPageHandler::GetCustomImageBackgrounds(
    GetCustomImageBackgroundsCallback callback) {
  std::vector<qorai_new_tab_page::mojom::CustomBackgroundPtr> backgrounds;
  for (const auto& name :
       NTPBackgroundPrefs(profile_->GetPrefs()).GetCustomImageList()) {
    auto value = qorai_new_tab_page::mojom::CustomBackground::New();
...
void QoraiNewTabPageHandler::RemoveCustomImageBackground(
  ...
  file_manager_->RemoveImage(
      file_path,
      base::BindOnce(&QoraiNewTabPageHandler::OnRemoveCustomImageBackground,
                     weak_factory_.GetWeakPtr(), file_path));
}

void QoraiNewTabPageHandler::UseQoraiBackground(
    const std::string& selected_background) {
  // Call ntp custom background images service.
  auto pref = NTPBackgroundPrefs(profile_->GetPrefs());
  pref.SetType(NTPBackgroundPrefs::Type::kQorai);
...
void QoraiNewTabPageHandler::TryQoraiSearchPromotion(const std::string& input,
                                                     bool open_new_tab) {
  const GURL promo_url = qorai_search_conversion::GetPromoURL(input);
...
void QoraiNewTabPageHandler::DismissQoraiSearchPromotion() {
  qorai_search_conversion::SetDismissed(profile_->GetPrefs());
}

void QoraiNewTabPageHandler::IsSearchPromotionEnabled(
    IsSearchPromotionEnabledCallback callback) {
  std::move(callback).Run(IsNTPPromotionEnabled(profile_));
}

void QoraiNewTabPageHandler::NotifySearchPromotionDisabledIfNeeded() const {
...
void QoraiNewTabPageHandler::OnSearchPromotionDismissed() {
  NotifySearchPromotionDisabledIfNeeded();
}

void QoraiNewTabPageHandler::UseColorBackground(const std::string& color,
                                                bool use_random_color) {
  if (use_random_color) {
    DCHECK(color == qorai_new_tab_page::mojom::kRandomSolidColorValue ||
           color == qorai_new_tab_page::mojom::kRandomGradientColorValue)
...
void QoraiNewTabPageHandler::GetSearchEngines(
  ...
  std::vector<qorai_new_tab_page::mojom::SearchEngineInfoPtr> search_engines;
  for (TemplateURL* template_url : urls) {
    if (template_url->GetBuiltinEngineType() !=
        BuiltinEngineType::KEYWORD_MODE_PREPOPULATED_ENGINE) {
      continue;
    }
    auto search_engine = qorai_new_tab_page::mojom::SearchEngineInfo::New();
...
void QoraiNewTabPageHandler::SearchWhatYouTyped(const std::string& host,
...
void QoraiNewTabPageHandler::ReportVPNWidgetUsage() {
  auto* vpn_service =
      qorai_vpn::QoraiVpnServiceFactory::GetForProfile(profile_);
  vpn_service->qorai_vpn_metrics()->RecordWidgetUsage(true);
}
#endif

bool QoraiNewTabPageHandler::IsCustomBackgroundImageEnabled() const {
...
bool QoraiNewTabPageHandler::IsColorBackgroundEnabled() const {
  return NTPBackgroundPrefs(profile_->GetPrefs()).IsColorType();
}

void QoraiNewTabPageHandler::OnSavedCustomImage(const base::FilePath& path) {
  if (path.empty()) {
    LOG(ERROR) << "Failed to save custom image";
    return;
  }

  if (qorai_new_tab_page::mojom::kMaxCustomImageBackgrounds -
...
void QoraiNewTabPageHandler::OnRemoveCustomImageBackground(
  ...
  if (background_pref.GetType() == NTPBackgroundPrefs::Type::kCustomImage) {
    if (auto custom_images = background_pref.GetCustomImageList();
      ...
      background_pref.SetType(NTPBackgroundPrefs::Type::kQorai);
...
void QoraiNewTabPageHandler::OnBackgroundUpdated() {
  if (IsCustomBackgroundImageEnabled()) {
    ...
    page_->OnBackgroundUpdated(
        qorai_new_tab_page::mojom::Background::NewCustom(std::move(value)));
    return;
  }
  ...
  DCHECK(ntp_background_prefs.IsQoraiType());
  if (ntp_background_prefs.ShouldUseRandomValue()) {
    // Pass empty value for random Qorai background.
    page_->OnBackgroundUpdated(nullptr);
    return;
  }

  auto* service = g_qorai_browser_process->ntp_background_images_service();
  if (!service) {
  ...
  auto value = qorai_new_tab_page::mojom::QoraiBackground::New();
  value->image_url = GURL(image_url);
  value->author = iter->author;
  value->link = GURL(iter->link);
  page_->OnBackgroundUpdated(
      qorai_new_tab_page::mojom::Background::NewQorai(std::move(value)));
}

void QoraiNewTabPageHandler::OnCustomImageBackgroundsUpdated() {
  std::vector<qorai_new_tab_page::mojom::CustomBackgroundPtr> backgrounds;
  for (const auto& name :
       NTPBackgroundPrefs(profile_->GetPrefs()).GetCustomImageList()) {
    auto value = qorai_new_tab_page::mojom::CustomBackground::New();
...
void QoraiNewTabPageHandler::FileSelected(const ui::SelectedFileInfo& file,
                                          int index) {
  profile_->set_last_selected_directory(file.path().DirName());

  file_manager_->SaveImage(
      file.path(), base::BindOnce(&QoraiNewTabPageHandler::OnSavedCustomImage,
                                  weak_factory_.GetWeakPtr()));

  select_file_dialog_ = nullptr;
}

void QoraiNewTabPageHandler::MultiFilesSelected(
    const std::vector<ui::SelectedFileInfo>& files) {
  NTPBackgroundPrefs prefs(profile_->GetPrefs());
  auto available_image_count =
      qorai_new_tab_page::mojom::kMaxCustomImageBackgrounds -
...
void QoraiNewTabPageHandler::FileSelectionCanceled() {
  select_file_dialog_ = nullptr;
}

void QoraiNewTabPageHandler::OnTemplateURLServiceChanged() {
  NotifySearchPromotionDisabledIfNeeded();
}

void QoraiNewTabPageHandler::OnTemplateURLServiceShuttingDown() {
  template_url_service_observation_.Reset();
}

void QoraiNewTabPageHandler::GetQoraiBackgrounds(
    GetQoraiBackgroundsCallback callback) {
  auto* service = g_qorai_browser_process->ntp_background_images_service();
  if (!service) {
    LOG(ERROR) << "No NTP background images service";
  ...
  std::vector<qorai_new_tab_page::mojom::QoraiBackgroundPtr> backgrounds;
