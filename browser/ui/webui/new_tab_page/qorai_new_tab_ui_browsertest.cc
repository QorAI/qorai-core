...
#include "qorai/browser/extensions/qorai_extension_functional_test.h"
...
class QoraiNewTabUIBrowserTest : public extensions::ExtensionFunctionalTest {
 protected:
...
IN_PROC_BROWSER_TEST_F(QoraiNewTabUIBrowserTest, StartupURLTest) {
...
// This test simply checks that by default the Qorai new tab page is used.
// It does this by loading the newtab page and then checking if
// window.qorai_new_tab exists.
IN_PROC_BROWSER_TEST_F(QoraiNewTabUIBrowserTest, QoraiNewTabIsDefault) {
  content::WebContents* web_contents = GetActiveWebContents();
  SimulateOpenNewTabAndWaitForLoad(web_contents);
  VerifyNewTabPageLoadedExpectation(web_contents);
}
...
IN_PROC_BROWSER_TEST_F(QoraiNewTabUIBrowserTest, NewTabPageLocationOverride) {
