...
class QoraiCustomizeColorSchemeModeHandlerUnitTest : public testing::Test {
 protected:
  // testing::Test:
  void SetUp() override {
    handler_ = std::make_unique<QoraiCustomizeColorSchemeModeHandler>(
  ...
  std::unique_ptr<QoraiCustomizeColorSchemeModeHandler> handler_;
};

TEST_F(QoraiCustomizeColorSchemeModeHandlerUnitTest,
       ClientSetColorSchemeModeShouldBeCalledWhenQoraiDarkModeTypeChanges) {
...
TEST_F(
    QoraiCustomizeColorSchemeModeHandlerUnitTest,
  ...
  ASSERT_EQ(dark_mode::GetQoraiDarkModeType(),
            dark_mode::QoraiDarkModeType::QORAI_DARK_MODE_TYPE_DEFAULT);
...
TEST_F(QoraiCustomizeColorSchemeModeHandlerUnitTest,
  ...
  auto* theme_service = ThemeServiceFactory::GetForProfile(&testing_profile_);
  ASSERT_TRUE(theme_service);
  ASSERT_EQ(dark_mode::QoraiDarkModeType::QORAI_DARK_MODE_TYPE_DEFAULT,
            dark_mode::GetQoraiDarkModeType());
  ...
  EXPECT_EQ(dark_mode::QoraiDarkModeType::QORAI_DARK_MODE_TYPE_DARK,
            dark_mode::GetQoraiDarkModeType())
      << "Calling SetColorSchemeMode should set the Qorai dark mode type "
         "to dark";
  ...
  handler_->SetColorSchemeMode(
      customize_color_scheme_mode::mojom::ColorSchemeMode::kSystem);
  EXPECT_EQ(dark_mode::QoraiDarkModeType::QORAI_DARK_MODE_TYPE_DEFAULT,
            dark_mode::GetQoraiDarkModeType())
      << "Calling SetColorSchemeMode should set the Qorai dark mode type "
         "to default";
  EXPECT_EQ(customize_color_scheme_mode::mojom::ColorSchemeMode::kSystem,
