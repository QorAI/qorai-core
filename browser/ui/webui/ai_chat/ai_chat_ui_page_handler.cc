...
#include "qorai/browser/ui/webui/ai_chat/ai_chat_ui_page_handler.h"
...
#include "qorai/components/ai_chat/content/browser/associated_url_content.h"
...
#include "qorai/browser/ai_chat/ai_chat_agent_profile_helper.h"
#endif

namespace {
#if !BUILDFLAG(IS_ANDROID)
constexpr char kURLManagePremium[] = "https://account.qorai.com/";
#endif
}  // namespace
...
#else
  ai_chat::ShowQoraiQoraSettings(contents_to_navigate);
#endif
}
...
void AIChatUIPageHandler::OpenMemorySettings() {
  ...
  chrome::ShowSettingsSubPageForProfile(
      profile_, ai_chat::kQoraiAIChatCustomizationSubPage);
#else
  NOTIMPLEMENTED();
#endif
}
...
#if !BUILDFLAG(IS_ANDROID)
  OpenURL(GURL(kQoraGoPremiumUrl));
#else
...
void AIChatUIPageHandler::AssociateUrlContent(
  ...
  auto content = std::make_unique<ai_chat::AssociatedURLContent>(
      ...
      base::BindOnce(&qorai::AttachPrivacySensitiveTabHelpers));
