...
TEST_F(CustomizeToolbarHandlerUnitTest,
       OnQoraiActionPinnedChanged_ShouldBeCalledWhenPrefsChanged) {
  for (const auto& [id, qorai_action] : kQoraiActions) {
    const bool pinned =
        GetTestingPrefService()->GetBoolean(qorai_action->pref_name);
    EXPECT_CALL(mock_page_, SetActionPinned(id, !pinned));
    GetTestingPrefService()->SetBoolean(qorai_action->pref_name, !pinned);
  }
}

TEST_F(CustomizeToolbarHandlerUnitTest, YourChromeLabelShouldBeQoraiMenu) {
