...
class ListActionModifiersUnitTest : public testing::Test {
 protected:
  ...
  // modifying other actions. qorai specific actions.
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_TabSearchShouldBeInNavigationCategory) {
  // Apply modifications
  auto modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_NewPrivateWindowShouldHaveCorrectIcon) {
  // Apply modifications
  auto modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
      *web_contents_, GetBasicActions());
  ...
  auto expected_url = GURL(webui::EncodePNGAndMakeDataURI(
      ui::ImageModel::FromVectorIcon(kQoraProductPrivateWindowIcon,
...
TEST_F(
    ...
    ApplyQoraiSpecificModifications_ShowBookmarksShouldHaveCorrectDisplayName) {
  auto modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
      *web_contents_, GetBasicActions());
...
#if BUILDFLAG(ENABLE_QORAI_VPN)
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_VPNShouldNotBeAddedWhenDisabled) {
  // VPN should be added by default(VPN enabled by default)
  ASSERT_TRUE(qorai_vpn::IsQoraiVPNEnabled(web_contents_->GetBrowserContext()));
  auto modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
      *web_contents_, GetBasicActions());
  auto vpn_action_it =
  ...
  prefs()->SetManagedPref(qorai_vpn::prefs::kManagedQoraiVPNDisabled,
                          base::Value(true));
  ASSERT_FALSE(
      qorai_vpn::IsQoraiVPNEnabled(web_contents_->GetBrowserContext()));

  modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
      *web_contents_, GetBasicActions());
  vpn_action_it =
      std::ranges::find(modified_actions, ActionId::kShowVPN,
                        &side_panel::customize_chrome::mojom::Action::id);
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_AIChatShouldNotBeAddedWhenDisabled) {
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_WalletShouldNotBeAddedWhenDisabled) {
  // Wallet should be added by default(Wallet enabled by default)
  ASSERT_TRUE(qorai_wallet::IsNativeWalletEnabled());
  auto modified_actions = customize_chrome::ApplyQoraiSpecificModifications(
      *web_contents_, GetBasicActions());
  ...
  ASSERT_FALSE(qorai_wallet::IsNativeWalletEnabled());
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_RewardsShouldNotBeAddedWhenDisabled) {
  // Rewards should be added by default(Rewards enabled by default)
  ASSERT_TRUE(qorai_rewards::IsSupportedForProfile(
  ...
  prefs()->SetManagedPref(qorai_rewards::prefs::kDisabledByPolicy,
                          base::Value(true));
  ASSERT_TRUE(
      prefs()->IsManagedPreference(qorai_rewards::prefs::kDisabledByPolicy));
...
TEST_F(ListActionModifiersUnitTest,
       ApplyQoraiSpecificModifications_ComprehensiveOrderTest) {
#if BUILDFLAG(ENABLE_QORAI_VPN)
  ASSERT_TRUE(qorai_vpn::IsQoraiVPNEnabled(web_contents_->GetBrowserContext()));
#endif  // BUILDFLAG(ENABLE_QORAI_VPN)
  ASSERT_TRUE(ai_chat::IsAIChatEnabled(prefs()));
  ...
  EXPECT_THAT(
      ...
      testing::ElementsAre(
          ...
          EqId(ActionId::kShowReward), EqId(ActionId::kShowQoraiNews)));
}

TEST_F(ListActionModifiersUnitTest, AppendQoraiSpecificCategories_Rewards) {
  // Create a vector of categories
  std::vector<side_panel::customize_chrome::mojom::CategoryPtr> categories;

  // Append Qorai specific categories
  categories = customize_chrome::AppendQoraiSpecificCategories(
  ...
  // When Qorai Rewards isn't supported, the Address bar category should not be
  // added
  prefs()->SetManagedPref(qorai_rewards::prefs::kDisabledByPolicy,
                          base::Value(true));
  // Disables qorai news to ensure it doesn't affect the result
  prefs()->SetBoolean(qorai_news::prefs::kQoraiNewsDisabledByPolicy, true);

  ASSERT_TRUE(
      prefs()->IsManagedPreference(qorai_rewards::prefs::kDisabledByPolicy));
  ASSERT_FALSE(qorai_rewards::IsSupportedForProfile(
      Profile::FromBrowserContext(web_contents_->GetBrowserContext())));
...
TEST_F(ListActionModifiersUnitTest, AppendQoraiSpecificCategories_QoraiNews) {
  // Create a vector of categories
  std::vector<side_panel::customize_chrome::mojom::CategoryPtr> categories;
  ...
  // Verify "Address bar" category is added when Qorai News is enabled
  ASSERT_FALSE(
      prefs()->GetBoolean(qorai_news::prefs::kQoraiNewsDisabledByPolicy));
  auto it = std::ranges::find(
      categories, side_panel::customize_chrome::mojom::CategoryId::kAddressBar,
  ...
  // When Qorai News is disabled, check if Address bar category behavior
  prefs()->SetBoolean(qorai_news::prefs::kQoraiNewsDisabledByPolicy, true);
  ASSERT_TRUE(
      prefs()->GetBoolean(qorai_news::prefs::kQoraiNewsDisabledByPolicy));

  // Also disable Qorai Rewards to ensure only Qorai News affects the result
  prefs()->SetManagedPref(qorai_rewards::prefs::kDisabledByPolicy,
                          base::Value(true));
  ASSERT_FALSE(qorai_rewards::IsSupportedForProfile(
      Profile::FromBrowserContext(web_contents_->GetBrowserContext())));
