...
#include "qorai/components/qorai_ads/core/public/history/ad_history_item_value_util.h"
...
class RewardsPageHandler::UpdateObserver
 ...
 public:
  UpdateObserver(RewardsService* rewards_service,
    ...
    AddPrefListener(qorai_ads::prefs::kOptedInToNotificationAds,
                    UpdateSource::kAds);
    AddPrefListener(qorai_ads::prefs::kMaximumNotificationAdsPerHour,
                    UpdateSource::kAds);
    AddPrefListener(
        qorai_ads::prefs::kSubdivisionTargetingUserSelectedSubdivision,
        UpdateSource::kAds);
...
RewardsPageHandler::RewardsPageHandler(
    ...
    RewardsService* rewards_service,
    qorai_ads::AdsService* ads_service,
    qorai_adaptive_captcha::QoraiAdaptiveCaptchaService* captcha_service,
    PrefService* prefs)
...
void RewardsPageHandler::GetAdsSettings(GetAdsSettingsCallback callback) {
  ...
  settings->is_supported_region = qorai_ads::IsSupportedRegion();
  settings->new_tab_page_ads_enabled =
      prefs_->GetBoolean(ntp_background_images::prefs::
                             kNewTabPageShowSponsoredImagesBackgroundImage);
  settings->notification_ads_enabled =
      prefs_->GetBoolean(qorai_ads::prefs::kOptedInToNotificationAds);
  ...
  settings->auto_detected_subdivision = prefs_->GetString(
      qorai_ads::prefs::kSubdivisionTargetingAutoDetectedSubdivision);
  settings->should_allow_subdivision_targeting =
      prefs_->GetBoolean(qorai_ads::prefs::kShouldAllowSubdivisionTargeting);

  auto& subdivisions = qorai_ads::GetSupportedSubdivisions();
  auto iter = subdivisions.find(qorai_l10n::GetDefaultISOCountryCodeString());
...
void RewardsPageHandler::GetAdsStatement(GetAdsStatementCallback callback) {
  auto on_statement = [](decltype(callback) callback,
                         qorai_ads::mojom::StatementInfoPtr info) {
    if (!info) {
      std::move(callback).Run(nullptr);
      return;
    }
    ...
    using AdType = qorai_ads::mojom::AdType;
...
void RewardsPageHandler::GetAdsHistory(GetAdsHistoryCallback callback) {
  ...
  base::Time from_time =
      now - qorai_ads::kAdHistoryRetentionPeriod.Get() - base::Days(1);
  ...
  qorai_rewards::p3a::RecordAdsHistoryView();
...
void RewardsPageHandler::SetAdTypeEnabled(qorai_ads::mojom::AdType ad_type,
                                          bool enabled,
  ...
  switch (ad_type) {
    ...
    case AdType::kNotificationAd:
      prefs_->SetBoolean(qorai_ads::prefs::kOptedInToNotificationAds, enabled);
      break;
...
void RewardsPageHandler::SetNotificationAdsPerHour(
  ...
  prefs_->SetInt64(qorai_ads::prefs::kMaximumNotificationAdsPerHour,
                   ads_per_hour);
  std::move(callback).Run();
}
...
void RewardsPageHandler::ToggleAdLike(const std::string& history_item,
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/40852): Refactor UI
  // reactions to use `mojom::ReactionInfo` instead of `AdHistoryItemInfo`.
  const qorai_ads::AdHistoryItemInfo ad_history_item =
      qorai_ads::AdHistoryItemFromValue(*dict);

  ads_service_->ToggleLikeAd(qorai_ads::CreateReaction(ad_history_item),
...
void RewardsPageHandler::ToggleAdDislike(const std::string& history_item,
  ...
  ads_service_->ToggleDislikeAd(qorai_ads::CreateReaction(ad_history_item),
                                base::IgnoreArgs<bool>(std::move(callback)));
}
...
void RewardsPageHandler::ToggleAdInappropriate(
  ...
  ads_service_->ToggleMarkAdAsInappropriate(
      qorai_ads::CreateReaction(ad_history_item),
      base::IgnoreArgs<bool>(std::move(callback)));
}
