...
class WalletPanelUIBrowserTest : public InProcessBrowserTest {
 public:
  void SetUpOnMainThread() override {
    ...
    qorai_wallet_service()->json_rpc_service()->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);

    AssetRatioServiceFactory::GetServiceForContext(browser()->profile())
        ->EnableDummyPricesForTesting();

    qorai_wallet_service()->keyring_service()->CreateWallet("password_123",
                                                            base::DoNothing());

    SetEthChainIdInterceptor(
        {GURL(kSomeEndpoint), qorai_wallet_service()
                                  ->network_manager()
  ...
  void CreateWalletTab() {
    ui_test_utils::NavigateToURLWithDisposition(
        browser(),
        GURL(std::string(kQoraiUIWalletPanelURL) + "crypto/connections"),
  ...
  void WaitForNeonEVMNetworkUrl(const GURL& url) {
    ...
    if (qorai_wallet_service()->network_manager()->GetNetworkURL(
    ...
    pref_change_registrar.Add(
        kQoraiWalletCustomNetworks, base::BindLambdaForTesting([&] {
          if (qorai_wallet_service()->network_manager()->GetNetworkURL(
                  mojom::kNeonEVMMainnetChainId, mojom::CoinType::ETH) == url) {
            run_loop.Quit();
          }
