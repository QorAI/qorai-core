...
#include "qorai/components/qorai_adblock/adblock_internals/resources/grit/qorai_adblock_internals_generated_map.h"
...
// Class acting as a controller of the qorai://adblock-internals WebUI.
class QoraiAdblockInternalsMessageHandler
    : public content::WebUIMessageHandler {
 public:
  QoraiAdblockInternalsMessageHandler() = default;
  ~QoraiAdblockInternalsMessageHandler() override = default;

 private:
  // WebUIMessageHandler implementation.
  void RegisterMessages() override {
    web_ui()->RegisterMessageCallback(
        "qorai_adblock_internals.getDebugInfo",
        base::BindRepeating(&QoraiAdblockInternalsMessageHandler::GetDebugInfo,
                            base::Unretained(this)));
    web_ui()->RegisterMessageCallback(
        "qorai_adblock_internals.discardRegex",
        base::BindRepeating(&QoraiAdblockInternalsMessageHandler::DiscardRegex,
                            base::Unretained(this)));
  }
  ...
  void GetDebugInfo(const base::Value::List& args) {
    ...
    instrumentation->RequestGlobalDumpForPid(
        ...
        base::BindOnce(&QoraiAdblockInternalsMessageHandler::OnGetMemoryDump,
  ...
  void OnGetMemoryDump(
    ...
    g_qorai_browser_process->ad_block_service()->GetDebugInfoAsync(
        base::BindOnce(&QoraiAdblockInternalsMessageHandler::OnGetDebugInfo,
                       weak_ptr_factory_.GetWeakPtr(), callback_id,
                       std::move(mem_info)));
  }
  ...
  void DiscardRegex(const base::Value::List& args) {
    ...
    g_qorai_browser_process->ad_block_service()->DiscardRegex(regex_id);
  }
  ...
  base::WeakPtrFactory<QoraiAdblockInternalsMessageHandler> weak_ptr_factory_{
      this};
};

}  // namespace

QoraiAdblockInternalsUI::QoraiAdblockInternalsUI(content::WebUI* web_ui)
    : content::WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, kAdblockInternalsHost,
                              kQoraiAdblockInternalsGenerated,
                              IDR_QORAI_ADBLOCK_INTERNALS_HTML);

  web_ui->AddMessageHandler(
      std::make_unique<QoraiAdblockInternalsMessageHandler>());
}

QoraiAdblockInternalsUI::~QoraiAdblockInternalsUI() = default;
