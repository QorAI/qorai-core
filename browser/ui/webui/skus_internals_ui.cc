...
#include "qorai/components/skus/browser/resources/grit/skus_internals_generated_map.h"
...
void SkusInternalsUI::GetVpnState(GetVpnStateCallback callback) {
  ...
  if (!qorai_vpn::IsQoraiVPNEnabled(profile->GetPrefs())) {
    dict.Set("Order", base::Value::Dict());
  } else {
    auto order_info = GetOrderInfo("vpn.");
    order_info.Set(
        "env", local_state_->GetString(qorai_vpn::prefs::kQoraiVPNEnvironment));
...
void SkusInternalsUI::GetQoraState(GetQoraStateCallback callback) {
  base::Value::Dict dict;
  dict.Set("Order", GetOrderInfo("qora."));
  std::string result;
  base::JSONWriter::Write(dict, &result);
  std::move(callback).Run(result);
}
...
void SkusInternalsUI::ResetSkusState() {
  ...
  auto* profile = Profile::FromWebUI(web_ui());
  if (qorai_vpn::IsQoraiVPNEnabled(profile->GetPrefs())) {
    qorai_vpn::ClearSubscriberCredential(local_state_);
    if (auto* service =
            qorai_vpn::QoraiVpnServiceFactory::GetForProfile(profile)) {
...
std::string SkusInternalsUI::GetLastVPNConnectionError() const {
  std::string error;
#if BUILDFLAG(ENABLE_QORAI_VPN)
  auto* manager = g_qorai_browser_process->qorai_vpn_connection_manager();
  CHECK(manager);
...
std::string SkusInternalsUI::GetSkusStateAsString() const {
  ...
  if (qorai_vpn::IsQoraiVPNEnabled(profile->GetPrefs())) {
    dict.Set("env",
             local_state_->GetString(qorai_vpn::prefs::kQoraiVPNEnvironment));
  }
#endif
