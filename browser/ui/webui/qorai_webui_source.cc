...
void CustomizeWebUIHTMLSource(content::WebUI* web_ui,
  ...
  source->AddResourcePaths(qorai::GetWebUIResources(name));
  source->AddLocalizedStrings(qorai::GetWebUILocalizedStrings(name));
}
...
content::WebUIDataSource* CreateWebUIDataSource(
  ...
  // Some parts of Qorai's UI pages are not yet migrated to work without doing
  // assignments of strings directly into |innerHTML| elements (i.e. see usage
  // of |dangerouslySetInnerHTML| in .tsx files). This will break Qorai due to
...
void AddBackgroundColorToSource(content::WebUIDataSource* source,
  ...
  // here because it does not include QoraiPrivateWindowThemeSupplier. This
