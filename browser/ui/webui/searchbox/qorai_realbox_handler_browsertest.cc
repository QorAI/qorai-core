...
class QoraiRealboxHandlerTest : public InProcessBrowserTest {
 public:
  QoraiRealboxHandlerTest() {}
  QoraiRealboxHandlerTest(const QoraiRealboxHandlerTest&) = delete;
  QoraiRealboxHandlerTest& operator=(const QoraiRealboxHandlerTest&) = delete;
  ~QoraiRealboxHandlerTest() override = default;
...
IN_PROC_BROWSER_TEST_F(QoraiRealboxHandlerTest, QoraiSearchUsesNewTabSource) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());

  OnAutocompleteAccept(
      GURL("https://search.qorai.com/search?q=hello+world&source=desktop"),
      u":br");
  EXPECT_EQ(GURL("https://search.qorai.com/search?q=hello+world&source=newtab"),
            contents()->GetLastCommittedURL());
}

IN_PROC_BROWSER_TEST_F(QoraiRealboxHandlerTest,
                       QoraiSearchNoKeywordIsUnaffected) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());

  GURL match_url(
      "https://search.qorai.com/search?q=hello+world&source=desktop");
  OnAutocompleteAccept(match_url, u"");
  EXPECT_EQ(match_url, contents()->GetLastCommittedURL());
}

IN_PROC_BROWSER_TEST_F(QoraiRealboxHandlerTest, NonQoraiSearchIsUnaffected) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());
