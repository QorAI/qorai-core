...
class QoraiWelcomeUIBrowserTest : public InProcessBrowserTest {
 public:
  QoraiWelcomeUIBrowserTest() = default;
  ~QoraiWelcomeUIBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(QoraiWelcomeUIBrowserTest, PRE_StartupURLTest) {
...
IN_PROC_BROWSER_TEST_F(QoraiWelcomeUIBrowserTest, StartupURLTest) {
  Browser* new_browser = OpenNewBrowser(browser()->profile());
  ASSERT_TRUE(new_browser);
  TabStripModel* tab_strip = new_browser->tab_strip_model();
  ASSERT_EQ(1, tab_strip->count());
