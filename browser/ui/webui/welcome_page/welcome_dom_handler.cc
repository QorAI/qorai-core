...
constexpr char kP3AOnboardingHistogramName[] =
    "Qorai.Welcome.InteractionStatus.2";
...
bool ShouldRedirectToGettingStartedPage() {
  if (base::FeatureList::IsEnabled(features::kQoraiDayZeroExperiment) &&
      features::kQoraiDayZeroExperimentVariant.Get() == "c") {
    return true;
  }
  return base::FeatureList::IsEnabled(
      qorai_education::features::kShowGettingStartedPage);
}
...
WelcomeDOMHandler::WelcomeDOMHandler(Profile* profile)
    : profile_(profile),
      qorai_education_server_checker_(*profile->GetPrefs(),
                                      profile->GetURLLoaderFactory()) {
  base::MakeRefCounted<shell_integration::DefaultSchemeClientWorker>(
      GURL("https://browser-education.qorai.com"))
...
void WelcomeDOMHandler::HandleOpenSettingsPage(const base::Value::List& args) {
  ...
  if (browser) {
    content::OpenURLParams open_params(
        GURL("qorai://settings/privacy"), content::Referrer(),
...
void WelcomeDOMHandler::HandleGetWelcomeCompleteURL(
  ...
  qorai_education_server_checker_.IsServerPageAvailable(
      qorai_education::EducationPageType::kGettingStarted,
...
void WelcomeDOMHandler::OnGettingStartedServerCheck(
  ...
  GURL url;
  if (available) {
    url = qorai_education::GetEducationPageBrowserURL(
        qorai_education::EducationPageType::kGettingStarted);
  } else {
