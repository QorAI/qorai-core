...
std::string LoadTestFile(const std::string& name) {
  base::FilePath path = base::PathService::CheckedGet(qorai::DIR_TEST_DATA);
...
class QoraiSanitizedImageSourceTest : public testing::Test {
 public:
  QoraiSanitizedImageSourceTest()
      : source_(
            &profile_,
            base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(
                &test_url_loader_factory_)) {
    source_.set_pcdn_domain_for_testing("pcdn.qorai.com");
  }
  ~QoraiSanitizedImageSourceTest() override = default;
  ...
  scoped_refptr<base::RefCountedMemory> Decode(
    ...
    GURL image_url = GURL(
        "chrome://qorai-image/?url=" + EncodeQuery(url.spec()) +
        (target_size.empty() ? ""
 ...
 private:
  ...
  QoraiSanitizedImageSource source_;
};

TEST_F(QoraiSanitizedImageSourceTest, ImageIsDecoded) {
  auto url = GURL("https://example.com/image.png");
  EXPECT_TRUE(Decode(url, kValidImage));
}

TEST_F(QoraiSanitizedImageSourceTest, ImageIsDecodedAndResized) {
...
TEST_F(QoraiSanitizedImageSourceTest, PaddedImageIsDecoded) {
  auto url = GURL("https://pcdn.qorai.com/image.png.pad");
  EXPECT_TRUE(Decode(url, kPaddedImage));
}

TEST_F(QoraiSanitizedImageSourceTest, InvalidPaddedImageIsNotDecoded) {
  auto url = GURL("https://example.com/image.png.pad");
  EXPECT_FALSE(Decode(url, kInvalidImage));
}

TEST_F(QoraiSanitizedImageSourceTest, PaddedImageWithoutDotPadIsNotDecoded) {
  auto url = GURL("https://pcdn.qorai.com/image.png");
  EXPECT_FALSE(Decode(url, kPaddedImage));
}

TEST_F(QoraiSanitizedImageSourceTest, PaddedImageOnNonQoraiCDNIsNotDecoded) {
  auto url = GURL("https://example.com/image.png.pad");
  EXPECT_FALSE(Decode(url, kPaddedImage));
}

TEST_F(QoraiSanitizedImageSourceTest, DotPadOnNonQoraiCDNButValidIsDecoded) {
  auto url = GURL("https://example.com/image.png.pad");
  EXPECT_TRUE(Decode(url, kValidImage));
}
