...
class AdblockDOMHandler
 ...
 private:
  ...
  base::ScopedObservation<
      qorai_shields::AdBlockSubscriptionServiceManager,
      qorai_shields::AdBlockSubscriptionServiceManagerObserver>
...
void AdblockDOMHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&AdblockDOMHandler::HandleUpdateCustomFilters,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_adblock.submitNewSubscription",
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&AdblockDOMHandler::HandleDeleteSubscription,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_adblock.refreshSubscription",
      base::BindRepeating(&AdblockDOMHandler::HandleRefreshSubscription,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_adblock.viewSubscriptionSource",
...
void AdblockDOMHandler::HandleGetCustomFilters(const base::Value::List& args) {
  ...
  const std::string custom_filters = g_qorai_browser_process->ad_block_service()
                                         ...
                                         ->GetCustomFilters();
  CallJavascriptFunction("qorai_adblock.onGetCustomFilters",
...
void AdblockDOMHandler::HandleGetRegionalLists(const base::Value::List& args) {
  ...
  auto regional_lists = g_qorai_browser_process->ad_block_service()
                            ->component_service_manager()
                            ->GetRegionalLists();
  CallJavascriptFunction("qorai_adblock.onGetRegionalLists", regional_lists);
}
...
void AdblockDOMHandler::RefreshSubscriptionsList() {
  ...
  CallJavascriptFunction("qorai_adblock.onGetListSubscriptions", list_value);
}

}  // namespace

QoraiAdblockUI::QoraiAdblockUI(content::WebUI* web_ui)
    : WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, kAdblockHost, kQoraiAdblockGenerated,
                              IDR_QORAI_ADBLOCK_HTML);
  web_ui->AddMessageHandler(std::make_unique<AdblockDOMHandler>());
}

QoraiAdblockUI::~QoraiAdblockUI() = default;
