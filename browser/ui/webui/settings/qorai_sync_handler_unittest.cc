...
class QoraiSyncHandlerUnittest : public testing::Test {
 public:
  QoraiSyncHandlerUnittest() {
    ...
    handler_ = std::make_unique<QoraiSyncHandler>();
    handler_->set_web_ui(&test_web_ui_);
    handler_->RegisterMessages();
  }
  ~QoraiSyncHandlerUnittest() override {
 ...
 private:
  std::unique_ptr<QoraiSyncHandler> handler_;
...
void QoraiSyncHandlerUnittest::CallHandleCopySyncCodeToClipboard(
    const base::Value::List& args) {
  handler_->HandleCopySyncCodeToClipboard(args);
}

TEST_F(QoraiSyncHandlerUnittest, CopySyncCodeToClipboard) {
