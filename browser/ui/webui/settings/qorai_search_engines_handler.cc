...
namespace {
constexpr char kQoraiSearchForTorKeyword[] =
...
void SortDefaultSearchEnginesListInJP(
  ...
  const GURL yahoo_jp_url =
      GURL(TemplateURLPrepopulateData::qorai_yahoo_jp.search_url);
...
QoraiSearchEnginesHandler::QoraiSearchEnginesHandler(
    Profile* profile,
    regional_capabilities::RegionalCapabilitiesService* regional_capabilities)
    : SearchEnginesHandler(profile),
      regional_capabilities_(regional_capabilities) {}
QoraiSearchEnginesHandler::~QoraiSearchEnginesHandler() = default;

void QoraiSearchEnginesHandler::RegisterMessages() {
  SearchEnginesHandler::RegisterMessages();

  web_ui()->RegisterMessageCallback(
      "getPrivateSearchEnginesList",
      base::BindRepeating(
          &QoraiSearchEnginesHandler::HandleGetPrivateSearchEnginesList,
          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setDefaultPrivateSearchEngine",
      base::BindRepeating(
          &QoraiSearchEnginesHandler::HandleSetDefaultPrivateSearchEngine,
          base::Unretained(this)));
}

void QoraiSearchEnginesHandler::OnModelChanged() {
  SearchEnginesHandler::OnModelChanged();

  qorai::UpdateDefaultPrivateSearchProviderData(*profile_);
...
void QoraiSearchEnginesHandler::HandleGetPrivateSearchEnginesList(
...
base::Value::List QoraiSearchEnginesHandler::GetPrivateSearchEnginesList() {
  ...
  for (int i = 0; i < last_default_engine_index; ++i) {
    ...
    // Don't show two qorai search entries from settings to prevent confusion.
    // Hide qorai search for tor entry from settings UI. User doesn't need to
    // select qorai search tor entry for private profile.
    if (base::UTF16ToUTF8(template_url->keyword()) ==
        kQoraiSearchForTorKeyword) {
      continue;
    }
...
void QoraiSearchEnginesHandler::HandleSetDefaultPrivateSearchEngine(
    const base::Value::List& args) {
  CHECK_EQ(1U, args.size());
...
base::Value::Dict QoraiSearchEnginesHandler::GetSearchEnginesList() {
  auto search_engines_info = SearchEnginesHandler::GetSearchEnginesList();
  // Don't show two qorai search entries from settings to prevent confusion.
  // Hide qorai search for tor entry from settings UI. User doesn't need to
  // select qorai search tor entry for normal profile.
  ...
  defaults->EraseIf([](const auto& val) {
    ...
    DCHECK(keyword);
    return *keyword == kQoraiSearchForTorKeyword;
  });
