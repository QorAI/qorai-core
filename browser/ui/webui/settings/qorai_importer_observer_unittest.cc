...
class QoraiImporterObserverUnitTest : public testing::Test {
 public:
  QoraiImporterObserverUnitTest() {}
 ...
 private:
  ...
  raw_ptr<QoraiExternalProcessImporterHost> _ = nullptr;
};

TEST_F(QoraiImporterObserverUnitTest, ImportEvents) {
  ...
  std::unique_ptr<QoraiImporterObserver> observer =
      std::make_unique<QoraiImporterObserver>(
          importer_host, source_profile, imported_items,
          base::BindRepeating(
              &QoraiImporterObserverUnitTest::NotifyImportProgress,
...
TEST_F(QoraiImporterObserverUnitTest, DestroyObserverEarly) {
  auto* importer_host = new QoraiExternalProcessImporterHost();
  user_data_importer::SourceProfile source_profile;
  source_profile.importer_name = u"importer_name";
  source_profile.importer_type = user_data_importer::TYPE_CHROME;
