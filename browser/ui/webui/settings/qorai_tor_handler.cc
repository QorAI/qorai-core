...
constexpr net::NetworkTrafficAnnotationTag kTorBridgesMoatAnnotation =
    net::DefineNetworkTrafficAnnotation("qorai_tor_bridges", R"(
    semantics {
      sender:
        "Qorai Tor Handler"
      description:
...
QoraiTorHandler::QoraiTorHandler() = default;
QoraiTorHandler::~QoraiTorHandler() = default;

void QoraiTorHandler::RegisterMessages() {
  web_ui()->RegisterMessageCallback(
      "qorai_tor.getBridgesConfig",
      base::BindRepeating(&QoraiTorHandler::GetBridgesConfig,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.setBridgesConfig",
      base::BindRepeating(&QoraiTorHandler::SetBridgesConfig,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.requestBridgesCaptcha",
      base::BindRepeating(&QoraiTorHandler::RequestBridgesCaptcha,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.resolveBridgesCaptcha",
      base::BindRepeating(&QoraiTorHandler::ResolveBridgesCaptcha,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.setTorEnabled",
      base::BindRepeating(&QoraiTorHandler::SetTorEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.isTorEnabled",
      base::BindRepeating(&QoraiTorHandler::IsTorEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "qorai_tor.isTorManaged",
      base::BindRepeating(&QoraiTorHandler::IsTorManaged,
                          base::Unretained(this)));

  local_state_change_registrar_.Init(g_browser_process->local_state());
  local_state_change_registrar_.Add(
      tor::prefs::kTorDisabled,
      base::BindRepeating(&QoraiTorHandler::OnTorEnabledChanged,
                          base::Unretained(this)));
}

void QoraiTorHandler::GetBridgesConfig(const base::Value::List& args) {
  AllowJavascript();
...
void QoraiTorHandler::SetBridgesConfig(const base::Value::List& args) {
  CHECK_EQ(1u, args.size());
...
void QoraiTorHandler::RequestBridgesCaptcha(const base::Value::List& args) {
  CHECK_EQ(1u, args.size());

  auto captcha_callback = base::BindOnce(
      &QoraiTorHandler::SendResultToJavascript, base::Unretained(this),
...
void QoraiTorHandler::ResolveBridgesCaptcha(const base::Value::List& args) {
...
void QoraiTorHandler::SendResultToJavascript(bool reset_request,
...
void QoraiTorHandler::SetTorEnabled(const base::Value::List& args) {
...
void QoraiTorHandler::IsTorEnabled(const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
  AllowJavascript();
...
void QoraiTorHandler::OnTorEnabledChanged() {
...
void QoraiTorHandler::IsTorManaged(const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
