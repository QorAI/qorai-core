...
#include "qorai/browser/ui/webui/settings/qorai_full_disk_access_confirm_dialog_delegate.h"
...
QoraiImportDataHandler::QoraiImportDataHandler() = default;
QoraiImportDataHandler::~QoraiImportDataHandler() = default;

void QoraiImportDataHandler::StartImport(
  ...
  CheckDiskAccess(imported_items, source_profile.source_path,
                  ...
                  base::BindOnce(&QoraiImportDataHandler::StartImportImpl,
...
void QoraiImportDataHandler::StartImportImpl(
    const user_data_importer::SourceProfile& source_profile,
    uint16_t imported_items,
    Profile* profile) {
  ...
  auto* importer_host = new QoraiExternalProcessImporterHost();
  import_observers_[source_profile.source_path] =
      std::make_unique<QoraiImporterObserver>(
          importer_host, source_profile, imported_items,
          base::BindRepeating(&QoraiImportDataHandler::NotifyImportProgress,
...
void QoraiImportDataHandler::NotifyImportProgress(
    const user_data_importer::SourceProfile& source_profile,
  ...
  if (*event == "ImportItemEnded") {
    ...
    content::GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&QoraiImportDataHandler::OnImportEnded,
                                  weak_factory_.GetWeakPtr(), source_profile));
  }
}

void QoraiImportDataHandler::HandleImportData(const base::Value::List& args) {
  ImportDataHandler::HandleImportData(args);
}

void QoraiImportDataHandler::OnImportEnded(
    const user_data_importer::SourceProfile& source_profile) {
  import_observers_.erase(source_profile.source_path);
...
void QoraiImportDataHandler::OnJavascriptDisallowed() {
...
QoraiImportDataHandler::GetSourceProfileAt(int browser_index) {
  return importer_list_->GetSourceProfileAt(browser_index);
}

#if BUILDFLAG(IS_MAC)
void QoraiImportDataHandler::CheckDiskAccess(
    uint16_t imported_items,
  ...
  if (importer_type == user_data_importer::TYPE_SAFARI) {
    // Start import if Qorai has full disk access permission.
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&QoraiImportDataHandler::OnGetDiskAccessPermission,
...
void QoraiImportDataHandler::OnGetDiskAccessPermission(
    ContinueImportCallback callback,
    base::FilePath source_path,
    bool allowed) {
  if (!allowed) {
...
void QoraiImportDataHandler::DidStopLoading() {
  Observe(nullptr);
