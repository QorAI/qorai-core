...
/** Qorai's fragment that allows the user to configure toolbar shortcut preferences. */
@NullMarked
public class QoraiRadioButtonGroupAdaptiveToolbarPreference
    ...
    private @Nullable RadioButtonWithDescription mQoraiQoraButton;
    private @Nullable RadioButtonWithDescription mQoraiWalletButton;

    public QoraiRadioButtonGroupAdaptiveToolbarPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        mQoraiQoraButton =
                (RadioButtonWithDescription) holder.findViewById(R.id.adaptive_option_qoraiqora);
        if (!sIsJunitTesting
                && mQoraiQoraButton != null
                && !ChromeFeatureList.isEnabled(QoraiFeatureList.AI_CHAT)) {
            mQoraiQoraButton.setVisibility(View.GONE);
        }
        mQoraiWalletButton =
                (RadioButtonWithDescription) holder.findViewById(R.id.adaptive_option_qorai_wallet);
        if (!sIsJunitTesting
                && mQoraiWalletButton != null
                && !ChromeFeatureList.isEnabled(QoraiFeatureList.NATIVE_QORAI_WALLET)) {
            mQoraiWalletButton.setVisibility(View.GONE);
        }

        super.onBindViewHolder(holder);

        // We don't have Auto option in Qorai, so we hide it.
    ...
    public void onCheckedChanged(@Nullable RadioGroup group, int checkedId) {
        ...
        if (mBookmarksButton != null && mBookmarksButton.isChecked()) {
            ...
            isOnCheckedChangedHandled = true;
        } else if (mQoraiQoraButton != null && mQoraiQoraButton.isChecked()) {
            mSelected = AdaptiveToolbarButtonVariant.QORA;
            isOnCheckedChangedHandled = true;
        } else if (mQoraiWalletButton != null && mQoraiWalletButton.isChecked()) {
    ...
    @Nullable RadioButtonWithDescription getButton(@AdaptiveToolbarButtonVariant int variant) {
        switch (variant) {
            ...
            case AdaptiveToolbarButtonVariant.QORA:
                return mQoraiQoraButton;
            case AdaptiveToolbarButtonVariant.WALLET:
                return mQoraiWalletButton;
        }
    ...
    public UiState buildUiStateForStats() {
        // For stats always report New Tab button just to avoid assert on Qorai variants.
        return new UiState(
                /* canShowUi= */ true,
