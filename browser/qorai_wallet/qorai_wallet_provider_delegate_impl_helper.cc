...
constexpr auto kAccountCreationCoinName =
    base::MakeFixedFlatMap<qorai_wallet::mojom::CoinType, std::string_view>(
        {{qorai_wallet::mojom::CoinType::SOL, "Solana"},
         {qorai_wallet::mojom::CoinType::ADA, "Cardano"}});
}  // namespace
...
void ShowWalletOnboarding(content::WebContents* web_contents) {
  ...
  if (browser) {
    qorai::ShowQorAIWalletOnboarding(browser);
...
void ShowAccountCreation(content::WebContents* web_contents,
  ...
  if (browser) {
    qorai::ShowQorAIWalletAccountCreation(browser, it->second);
