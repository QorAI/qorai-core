...
class TestJsonRpcServiceObserver
 ...
 public:
  TestJsonRpcServiceObserver(base::OnceClosure callback,
                             ...
                             qorai_wallet::mojom::CoinType expected_coin,
  ...
  qorai_wallet::mojom::CoinType expected_coin_;
...
class QoraiWalletEthereumChainTest : public InProcessBrowserTest {
 public:
  QoraiWalletEthereumChainTest() {
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_->ServeFilesFromDirectory(
        qorai_wallet::QoraiWalletTestDataFolder());
    https_server_->RegisterRequestHandler(
        base::BindRepeating(&QoraiWalletEthereumChainTest::HandleChainRequest,
  ...
  GURL GetWalletEthereumChainPageURL(const std::string& host = "a.com",
    ...
    auto url =
        https_server()->GetURL(host, "/qorai_wallet_ethereum_chain.html");
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddEthereumChainApproved) {
  ...
  auto* tab_helper =
      qorai_wallet::QoraiWalletTabHelper::FromWebContents(contents);
  tab_helper->SetShowBubbleCallbackForTesting(loop.QuitClosure());
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  WaitForLoadStop(contents);
  loop.Run();

  ASSERT_TRUE(qorai_wallet::QoraiWalletTabHelper::FromWebContents(contents)
                  ->IsShowingBubble());
  CallAndWaitForEthereumChainRequestCompleted(
      kSomeChainId, true, qorai_wallet::mojom::CoinType::ETH, "");
  ...
  EXPECT_EQ(GetJsonRpcService()->GetChainIdSync(
                qorai_wallet::mojom::CoinType::ETH, origin),
            kSomeChainId);
}

IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddEthereumChainRejected) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddChainSameOrigin) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest,
  ...
  base::RunLoop loop;
  auto* tab_helperA =
      qorai_wallet::QoraiWalletTabHelper::FromWebContents(contentsA);
  ...
  auto* tab_helperB =
      qorai_wallet::QoraiWalletTabHelper::FromWebContents(web_contentsB);
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest,
  ...
  CallAndWaitForEthereumChainRequestCompleted(
      "0x11", true, qorai_wallet::mojom::CoinType::ETH, "");
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddDifferentChainsSwitch) {
  ASSERT_TRUE(GetAllEthCustomChains().empty());
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddChainAndCloseTab) {
  GURL urlA = GetWalletEthereumChainPageURL();
  content::WebContents* contentsA =
      browser()->tab_strip_model()->GetActiveWebContents();
  auto* tab_helperA =
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, AddBrokenChain) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(QoraiWalletEthereumChainTest, CheckIncognitoTab) {
  GURL url = GetWalletEthereumChainPageURL();
