...
#include "qorai/browser/qorai_wallet/notifications/wallet_notification_service.h"
...
namespace {
int GetStatusTitle(qorai_wallet::mojom::TransactionStatus status) {
  switch (status) {
    case qorai_wallet::mojom::TransactionStatus::Confirmed:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_CONFIRMED;
    case qorai_wallet::mojom::TransactionStatus::Error:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_ERROR;
    case qorai_wallet::mojom::TransactionStatus::Dropped:
...
void PushNotification(content::BrowserContext* context,
  ...
  auto notification = CreateMessageCenterNotification(
      title, body, uuid,
      GURL("qorai://wallet/crypto/accounts/" + from + "#" + uuid));
...
WalletNotificationService::WalletNotificationService(
    ...
    : qorai_wallet_service_(qorai_wallet_service), context_(context) {
  if (qorai_wallet_service_) {
    qorai_wallet_service_->tx_service()->AddObserver(
...
void WalletNotificationService::OnTransactionStatusChanged(
  ...
  if (ShouldDisplayUserNotification(tx_info->tx_status)) {
    auto account = qorai_wallet_service_->keyring_service()->FindAccount(
