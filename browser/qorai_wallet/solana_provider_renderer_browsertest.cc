...
// IDR_QORAI_WALLET_SOLANA_WEB3_JS_FOR_TEST is excluded from Android build to
// save space. Ensure this test is not build on Android. If it will be required
// to run these tests on Android, include again
// IDR_QORAI_WALLET_SOLANA_WEB3_JS_FOR_TEST
static_assert(!BUILDFLAG(IS_ANDROID));

using qorai_wallet::mojom::SolanaProviderError;
...
constexpr char OnAccountChangedScript[] =
    R"(async function disconnect() {await window.qoraiSolana.disconnect()}
       new Promise(resolve => {
        window.qoraiSolana.on('accountChanged', (result) => {
...
constexpr char CheckSolanaProviderScript[] = "!!window.qoraiSolana";
...
std::string ConnectScript(std::string_view args) {
  return absl::StrFormat(
      R"(async function connect() {
          try {
            const result = await window.qoraiSolana.connect(%s);
...
std::string SignTransactionScript(std::string_view args) {
  ...
  return absl::StrFormat(
      R"(async function signTransaction() {
          try {
            const result = await window.qoraiSolana.signTransaction%s
...
std::string SignAllTransactionsScript(std::string_view args) {
  ...
  return absl::StrFormat(
      R"(async function signAllTransactions() {
          try {
            const result = await window.qoraiSolana.signAllTransactions%s
            const isSameTx =
...
std::string SignAndSendTransactionScript(std::string_view args) {
  ...
  return absl::StrFormat(
      R"(async function signAndSendTransaction() {
          try {
            const result = await window.qoraiSolana.signAndSendTransaction%s
...
std::string SignMessageScript(std::string_view args) {
  std::vector<uint8_t> signature(qorai_wallet::kSolanaSignatureSize);
  EXPECT_TRUE(
      qorai_wallet::Base58Decode(kTestSignature, &signature, signature.size()));
  ...
  return absl::StrFormat(
      R"(async function signMessage() {
          try {
            const result = await window.qoraiSolana.signMessage%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
            else
              return false;
...
std ::string RequestScript(std::string_view args) {
  ...
  return absl::StrFormat(
      R"(async function request() {
          try {
            const result = await window.qoraiSolana.request%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
...
class TestSolanaProvider final : public qorai_wallet::mojom::SolanaProvider {
 public:
  ...
  void Init(mojo::PendingRemote<qorai_wallet::mojom::SolanaEventsListener>
  ...
  void SignTransaction(qorai_wallet::mojom::SolanaSignTransactionParamPtr param,
                       SignTransactionCallback callback) override {
    EXPECT_EQ(param->encoded_serialized_msg,
              qorai_wallet::Base58Encode(kSerializedMessage));
    if (error_ == SolanaProviderError::kSuccess) {
      std::move(callback).Run(
          SolanaProviderError::kSuccess, "", base::ToVector(kSignedTx),
          qorai_wallet::mojom::SolanaMessageVersion::kLegacy);
    } else {
      std::move(callback).Run(
  ...
  void SignAllTransactions(
      std::vector<qorai_wallet::mojom::SolanaSignTransactionParamPtr> params,
    ...
    if (error_ == SolanaProviderError::kSuccess) {
      ...
      std::move(callback).Run(
          ...
          std::vector<qorai_wallet::mojom::SolanaMessageVersion>());
      ClearError();
    }
  }
  void SignAndSendTransaction(
      qorai_wallet::mojom::SolanaSignTransactionParamPtr param,
 ...
 private:
  ...
  mojo::Remote<qorai_wallet::mojom::SolanaEventsListener> events_listener_;
};

class TestQorAIContentBrowserClient : public QorAIContentBrowserClient {
 public:
  TestQorAIContentBrowserClient() = default;
  ...
  void RegisterBrowserInterfaceBindersForFrame(
    ...
    map->Add<qorai_wallet::mojom::SolanaProvider>(
        base::BindRepeating(&TestQorAIContentBrowserClient::BindSolanaProvider,
 ...
 private:
  void BindSolanaProvider(
      ...
      mojo::PendingReceiver<qorai_wallet::mojom::SolanaProvider> receiver) {
    ...
    provider->set_connection_error_handler(base::BindOnce(
        &TestQorAIContentBrowserClient::OnDisconnect,
  ...
  base::flat_map<
      ...
      mojo::SelfOwnedReceiverRef<qorai_wallet::mojom::SolanaProvider>>
      provider_map_;
...
class SolanaProviderRendererTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    qorai_wallet::SetDefaultSolanaWallet(
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script = qorai_wallet::LoadDataResource(
          IDR_QORAI_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest,
  ...
  constexpr char kEvalIsQorAIWallet[] = "window.solana.isQorAIWallet";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonWritable) {
  for (const std::string& provider : {"qoraiSolana", "solana"}) {
    // window.qoraiSolana.* and window.solana.* (methods)
    ...
    // window.qoraiSolana.* and window.solana.* (properties)
    for (const std::string& property :
         {"isPhantom", "isQorAIWallet", "isConnected", "publicKey"}) {
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsPhantomAndIsQorAIWallet) {
  ASSERT_TRUE(ExecJs(web_contents(browser()),
                     "window.qoraiSolana.isPhantom = 123; "
                     "window.qoraiSolana.isQorAIWallet = 456"));
  // Both are non-writable
  auto result1 =
      EvalJs(web_contents(browser()), "window.qoraiSolana.isPhantom");
  EXPECT_EQ(base::Value(true), result1);
  auto result2 =
      EvalJs(web_contents(browser()), "window.qoraiSolana.isQorAIWallet");
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, OnConnect) {
  auto result =
      EvalJs(web_contents(browser()),
             R"(async function connect() {await window.qoraiSolana.connect()}
                new Promise(resolve => {
                  window.qoraiSolana.on(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsConnected) {
  auto result =
      EvalJs(web_contents(browser()), "window.qoraiSolana.isConnected");
  EXPECT_EQ(base::Value(true), result);
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, GetPublicKey) {
  auto result = EvalJs(web_contents(browser()),
                       "window.qoraiSolana.publicKey.toString()");
  EXPECT_EQ(base::Value(kTestPublicKey), result);
}
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, Disconnect) {
  auto result = EvalJs(web_contents(browser()),
                  ...
                  const result = await window.qoraiSolana.disconnect()
                  if (result == undefined)
                    return true;
                  else
                    return false;
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonConfigurable) {
  ...
  EXPECT_TRUE(content::EvalJs(web_contents(browser()),
                              NonConfigurableScript("qoraiSolana"))
                  .ExtractBool());
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, Iframe3P) {
  constexpr char kEvalSolanaUndefined[] =
      R"(typeof window.qoraiSolana === 'undefined')";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, SecureContextOnly) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  constexpr char kEvalSolana[] = "typeof window.qoraiSolana !== 'undefined'";
  content::RenderFrameHost* main_frame =
      web_contents(browser())->GetPrimaryMainFrame();
