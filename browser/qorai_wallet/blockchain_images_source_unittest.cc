...
class BlockchainImagesSourceTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    // qorai/test/data/qorai_wallet/1.0.1
    qorai_wallet::SetLastInstalledWalletVersionForTest(base::Version("1.0.1"));
    source_ =
        std::make_unique<BlockchainImagesSource>(QorAIWalletTestDataFolder());
  }
...
TEST_F(BlockchainImagesSourceTest, GetMimeType) {
  EXPECT_EQ(source()->GetMimeType(GURL("qorai://test/img1.png")), "image/png");
  EXPECT_EQ(source()->GetMimeType(GURL("qorai://test/img1.gif")), "image/gif");
  EXPECT_EQ(source()->GetMimeType(GURL("qorai://test/img1.jpg")), "image/jpg");
  EXPECT_EQ(source()->GetMimeType(GURL("qorai://test/img1.svg")),
            "image/svg+xml");
}
