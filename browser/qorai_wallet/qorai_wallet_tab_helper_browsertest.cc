...
int32_t OpenNonPanelPopup(const GURL& url,
  ...
  auto* child_popup = qorai_wallet::GetWebContentsFromTabId(nullptr, popup_id);
...
int32_t OpenPanelPopup(const GURL& url,
                       ...
                       qorai_wallet::QorAIWalletTabHelper* tab_helper) {
...
class QorAIWalletTabHelperBrowserTest : public InProcessBrowserTest {
 public:
  QorAIWalletTabHelperBrowserTest() {
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_->RegisterRequestHandler(base::BindRepeating(
        &QorAIWalletTabHelperBrowserTest::HandleChainRequest,
  ...
  GURL GetWalletEthereumChainPageURL() {
    ...
    auto url =
        https_server()->GetURL("a.com", "/qorai_wallet_ethereum_chain.html");
...
IN_PROC_BROWSER_TEST_F(QorAIWalletTabHelperBrowserTest,
...
IN_PROC_BROWSER_TEST_F(QorAIWalletTabHelperBrowserTest,
  ...
  auto* tab_helper =
      qorai_wallet::QorAIWalletTabHelper::FromWebContents(active_contents);
  ASSERT_TRUE(
      qorai_wallet::GetWebContentsFromTabId(nullptr, non_panel_popup_id));
  ...
  ASSERT_FALSE(
      qorai_wallet::GetWebContentsFromTabId(&target_browser, popup1_id));
  EXPECT_EQ(target_browser, nullptr);
  target_browser = nullptr;
  ASSERT_FALSE(
      qorai_wallet::GetWebContentsFromTabId(&target_browser, popup2_id));
  EXPECT_EQ(target_browser, nullptr);
  EXPECT_EQ(tab_helper->GetPopupIdsForTesting().size(), 0u);
  target_browser = nullptr;
  ASSERT_TRUE(qorai_wallet::GetWebContentsFromTabId(&target_browser,
                                                    non_panel_popup_id));
  EXPECT_EQ(target_browser, browser());
}

IN_PROC_BROWSER_TEST_F(QorAIWalletTabHelperBrowserTest, ClosePopupsWithBubble) {
  auto blank_url = https_server()->GetURL("c.com", "/popup.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), blank_url));
  content::WebContents* active_contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  ...
  base::RunLoop().RunUntilIdle();
  ASSERT_FALSE(tab_helper->IsShowingBubble());
  ASSERT_FALSE(qorai_wallet::GetWebContentsFromTabId(nullptr, popup1_id));
  ASSERT_FALSE(qorai_wallet::GetWebContentsFromTabId(nullptr, popup2_id));
  EXPECT_EQ(tab_helper->GetPopupIdsForTesting().size(), 0u);
  ASSERT_TRUE(
      qorai_wallet::GetWebContentsFromTabId(nullptr, non_panel_popup_id));
}
