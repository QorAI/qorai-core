...
#include "qorai/components/qorai_wallet/browser/eth_tx_manager.h"
...
class TestEventsListener : public qorai_wallet::mojom::EventsListener {
 public:
  ...
  mojo::PendingRemote<qorai_wallet::mojom::EventsListener> GetReceiver() {
 ...
 private:
  mojo::Receiver<qorai_wallet::mojom::EventsListener> observer_receiver_{this};
};
...
class EthereumProviderImplUnitTest : public testing::Test {
 public:
  ...
  void TearDown() override {
    ...
    QorAIWalletServiceDelegateImpl::SetActiveWebContentsForTesting(nullptr);
  }
  ...
  void SetUp() override {
    ...
    QorAIWalletServiceDelegateImpl::SetActiveWebContentsForTesting(
        web_contents_.get());
    ...
    asset_ratio_service_->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    qorai_wallet_service_ = std::make_unique<QorAIWalletService>(
        shared_url_loader_factory_,
        QorAIWalletServiceDelegate::Create(browser_context()), prefs(),
        &local_state_);
    ASSERT_TRUE(qorai_wallet_service_.get());
    ...
    provider_ = std::make_unique<EthereumProviderImpl>(
        host_content_settings_map(), qorai_wallet_service_.get(),
        std::make_unique<qorai_wallet::QorAIWalletProviderDelegateImpl>(
            web_contents(),
  ...
  void Unlock() {
    ...
    keyring_service()->Unlock(
        "qorai", base::BindLambdaForTesting([&run_loop](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
    run_loop.Run();
  ...
  TxService* tx_service() { return qorai_wallet_service_->tx_service(); }
  JsonRpcService* json_rpc_service() {
    return qorai_wallet_service_->json_rpc_service();
  }
  KeyringService* keyring_service() {
    return qorai_wallet_service_->keyring_service();
  }
  ...
  void CreateQorAIWalletTabHelper() {
    qorai_wallet::QorAIWalletTabHelper::CreateForWebContents(
        web_contents_.get());
  }
  qorai_wallet::QorAIWalletTabHelper* qorai_wallet_tab_helper() {
    return qorai_wallet::QorAIWalletTabHelper::FromWebContents(
        web_contents_.get());
  }

  void AddEthereumPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
        blink::PermissionType::QORAI_ETHEREUM, browser_context(), GetOrigin(),
        account_id->address));
  }

  void ResetEthereumPermission(const mojom::AccountIdPtr& account_id) {
    base::RunLoop run_loop;
    qorai_wallet_service_->ResetPermission(
        account_id.Clone(), base::BindLambdaForTesting([&](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
  ...
  void SignMessageHardware(
    ...
    browser_task_environment_.RunUntilIdle();
    qorai_wallet_service_->NotifySignMessageRequestProcessed(
        user_approved, qorai_wallet_service_->sign_message_id_ - 1,
  ...
  void SignMessage(std::optional<bool> user_approved,
    ...
    if (user_approved) {
      qorai_wallet_service_->NotifySignMessageRequestProcessed(
          *user_approved, qorai_wallet_service_->sign_message_id_ - 1, nullptr,
          std::nullopt);
    }
    run_loop.Run();
  }
  ...
  int SignMessageRequest(const std::string& address,
    ...
    browser_task_environment_.RunUntilIdle();
    return qorai_wallet_service_->sign_message_id_ - 1;
  }

  size_t GetSignMessageQueueSize() const {
    return qorai_wallet_service_->sign_message_requests_.size();
  }

  mojom::SignMessageRequestPtr GetSignMessageQueueFront() const {
    return qorai_wallet_service_->GetPendingSignMessageRequestsSync()
        .front()
        .Clone();
  }
  const mojom::SignMessageErrorPtr& GetSignMessageErrorQueueFront() const {
    return qorai_wallet_service_->sign_message_errors_.front();
  }
  ...
  GetPendingGetEncryptionPublicKeyRequests() const {
    ...
    qorai_wallet_service_->GetPendingGetEncryptionPublicKeyRequests(
        base::BindLambdaForTesting(
  ...
  std::vector<mojom::DecryptRequestPtr> GetPendingDecryptRequests() const {
    ...
    qorai_wallet_service_->GetPendingDecryptRequests(base::BindLambdaForTesting(
  ...
  void GetEncryptionPublicKey(const std::string& address,
    ...
    // QorAIWalletProviderDelegateImpl::GetAllowedAccounts
    browser_task_environment_.RunUntilIdle();
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_TRUE(qorai_wallet_tab_helper()->IsShowingBubble());
      qorai_wallet_service_->NotifyGetPublicKeyRequestProcessed(
    ...
    *unsafe_message = "";
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_EQ(requests[0]->account_id,
                GetAccountUtils().FindAccountIdByAddress(address));
      EXPECT_TRUE(qorai_wallet_tab_helper()->IsShowingBubble());
      qorai_wallet_service_->NotifyDecryptRequestProcessed(
          requests[0]->request_id, approved);
    }
    run_loop.Run();
  }
  ...
  void AddSuggestToken(mojom::BlockchainTokenPtr token,
    ...
    if (!token) {
      ...
      EXPECT_TRUE(qorai_wallet_tab_helper()->IsShowingBubble());
      qorai_wallet_service_->NotifyAddSuggestTokenRequestsProcessed(
  ...
  GetPendingAddSuggestTokenRequests() const {
    ...
    qorai_wallet_service_->GetPendingAddSuggestTokenRequests(
        base::BindLambdaForTesting(
 ...
 protected:
  std::unique_ptr<QorAIWalletService> qorai_wallet_service_;
};
...
TEST_F(EthereumProviderImplUnitTest, EmptyDelegate) {
  EthereumProviderImpl provider_impl(host_content_settings_map(),
                                     qorai_wallet_service_.get(), nullptr,
                                     prefs());
...
TEST_F(EthereumProviderImplUnitTest, RequestEthereumPermissionsWithAccounts) {
  ...
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::QORAI_ETHEREUM, CONTENT_SETTING_BLOCK);
  base::RunLoop run_loop;
  provider()->RequestEthereumPermissions(
      base::BindLambdaForTesting(
          [&](mojom::EthereumProviderResponsePtr response) {
  ...
  // again.
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::QORAI_ETHEREUM, CONTENT_SETTING_DEFAULT);
  EXPECT_THAT(RequestEthereumPermissions(), ElementsAre(address_0, address_1));
}
...
TEST_F(EthereumProviderImplUnitTest, SignMessage) {
  CreateWallet();
  auto account_0 = GetAccountUtils().EnsureEthAccount(0);
  Navigate(GURL("https://qorai.com"));
  for (const auto& message : {
           std::string("0x1234"),
           ToHex(GetSIWEMessage("https://qorai.com", account_0->address,
                                "https://qorai.com/login", "1")),
       }) {
...
TEST_F(EthereumProviderImplUnitTest, SigninWithEthereumError) {
  ...
  mojom::ProviderError error = mojom::ProviderError::kUnknown;
  std::string error_message;
  CreateQorAIWalletTabHelper();
  qorai_wallet_tab_helper()->SetSkipDelegateForTesting(true);
  Navigate(GURL("https://qorai.com"));
  AddEthereumPermission(account_0->account_id);
  ...
  struct {
    ...
    std::string provider_err_msg;
      ...
      {"https://qorai.com", account_0->address, "https://qorai.com/login",
       "5566", mojom::SignMessageErrorType::kChainIdMismatched,
       l10n_util::GetStringFUTF8(
           IDS_QORAI_WALLET_SIGN_MESSAGE_MISMATCH_ERR,
           l10n_util::GetStringUTF16(IDS_QORAI_WALLET_NETWORK),
           base::StrCat({l10n_util::GetStringUTF16(IDS_QORAI_WALLET_CHAIN_ID),
                         u": 5566"})),
       std::string("5566"), mojom::ProviderError::kInternalError,
       l10n_util::GetStringFUTF8(
           IDS_QORAI_WALLET_SIGN_MESSAGE_CHAIN_ID_MISMATCH, u"5566")},
      {"https://qorai.com", account_1->address, "https://qorai.com/login", "1",
       mojom::SignMessageErrorType::kAccountMismatched,
       l10n_util::GetStringFUTF8(
           IDS_QORAI_WALLET_SIGN_MESSAGE_MISMATCH_ERR,
           l10n_util::GetStringUTF16(IDS_QORAI_WALLET_ACCOUNT),
           base::ASCIIToUTF16(account_1->address)),
       std::nullopt, mojom::ProviderError::kInternalError,
       l10n_util::GetStringFUTF8(IDS_QORAI_WALLET_SIGN_MESSAGE_ACCOUNT_MISMATCH,
                                 base::ASCIIToUTF16(account_1->address))},
      {"https://example.com", account_0->address, "https://qorai.com/login",
       "1", mojom::SignMessageErrorType::kDomainMismatched,
       l10n_util::GetStringFUTF8(
           IDS_QORAI_WALLET_SIGN_MESSAGE_MISMATCH_ERR,
           l10n_util::GetStringUTF16(IDS_QORAI_WALLET_DOMAIN),
           u"https://example.com"),
       std::nullopt, mojom::ProviderError::kInternalError,
       l10n_util::GetStringFUTF8(IDS_QORAI_WALLET_SIGN_MESSAGE_DOMAIN_MISMATCH,
                                 u"https://example.com")},
  };
  for (const auto& invalid_case : cases) {
    ...
    qorai_wallet_service_->NotifySignMessageErrorProcessed(
...
TEST_F(EthereumProviderImplUnitTest, SignTypedMessage) {
  ...
  EXPECT_EQ(error_message, l10n_util::GetStringFUTF8(
                               IDS_QORAI_WALLET_SIGN_MESSAGE_CHAIN_ID_MISMATCH,
...
TEST_F(EthereumProviderImplUnitTest, SignMessageRequestQueue) {
  ...
  // wrong order
  qorai_wallet_service_->NotifySignMessageRequestProcessed(true, id2, nullptr,
                                                           std::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()
  ...
  qorai_wallet_service_->NotifySignMessageRequestProcessed(true, id3, nullptr,
                                                           std::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()
  ...
  qorai_wallet_service_->NotifySignMessageRequestProcessed(true, id1, nullptr,
                                                           std::nullopt);
...
TEST_F(EthereumProviderImplUnitTest, AccountsChangedEvent) {
  ...
  Navigate(GURL("https://qoraisoftware.com"));
  AddEthereumPermission(account_1->account_id);
  SetSelectedAccount(account_0->account_id);
  EXPECT_FALSE(observer_->AccountsChangedFired());
}
...
TEST_F(EthereumProviderImplUnitTest, Web3ClientVersion) {
  std::string expected_version = absl::StrFormat(
      "QorAIWallet/v%s", version_info::GetQorAIChromiumVersionNumber());
  std::string version;
  mojom::ProviderError error = mojom::ProviderError::kUnknown;
  std::string error_message;
...
TEST_F(EthereumProviderImplUnitTest, SwitchEthereumChain) {
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringFUTF8(IDS_WALLET_UNKNOWN_CHAIN,
                                      base::ASCIIToUTF16(chain_id)));
  EXPECT_FALSE(qorai_wallet_tab_helper()->IsShowingBubble());
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
  EXPECT_TRUE(qorai_wallet_tab_helper()->IsShowingBubble());
  qorai_wallet_tab_helper()->CloseBubble();
  EXPECT_FALSE(qorai_wallet_tab_helper()->IsShowingBubble());
...
TEST_F(EthereumProviderImplUnitTest, AddEthereumChainSwitchesForInactive) {
  ...
  std::string params = R"({"params": [{
        "chainId": "0xaa36a7",
        "chainName": "Sepolia",
        "rpcUrls": ["https://ethereum-sepolia.wallet.qorai.com"]
      }]})";
  base::RunLoop run_loop;
  provider()->AddEthereumChain(
...
TEST_F(EthereumProviderImplUnitTest, AddSuggestToken) {
  ...
  mojom::BlockchainTokenPtr token = mojom::BlockchainToken::New(
      "0x0D8775F648430679A709E98d2b0Cb6250d2887EF", "QOR", "", false, true,
      false, false, mojom::SPLTokenProgram::kUnsupported, false, false, "QOR",
