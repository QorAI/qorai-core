 ...
 public:
  ...
  void AddEthereumPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
        blink::PermissionType::QORAI_ETHEREUM, browser()->profile(),
  ...
  void SetNetworkForTesting(const std::string& chain_id,
    ...
    if (chain && !skip_rpc_url_override) {
      ...
      browser()->profile()->GetPrefs()->ClearPref(kQorAIWalletCustomNetworks);
      chain->rpc_endpoints =
          std::vector<GURL>({https_server_for_rpc()->base_url()});
...
IN_PROC_BROWSER_TEST_F(SendOrSignTransactionBrowserTest, CallViaProxy) {
  ...
  EXPECT_TRUE(
      EvalJs(web_contents(), "getIsQorAIWalletViaProxy()").ExtractBool());
}
