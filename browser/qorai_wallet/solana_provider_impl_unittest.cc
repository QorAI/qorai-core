...
#include "qorai/components/qorai_wallet/browser/solana_account_meta.h"
#include "qorai/components/qorai_wallet/browser/solana_instruction.h"
#include "qorai/components/qorai_wallet/browser/solana_message.h"
 ...
 public:
  ...
  void SetUp() override {
    ...
    qorai_wallet_service_ = std::make_unique<QorAIWalletService>(
        shared_url_loader_factory_,
        QorAIWalletServiceDelegate::Create(browser_context()),
        profile_.GetPrefs(), &local_state_);
    json_rpc_service_ = qorai_wallet_service_->json_rpc_service();
    json_rpc_service_->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    keyring_service_ = qorai_wallet_service_->keyring_service();
  ...
  std::vector<mojom::SignMessageRequestPtr> GetPendingSignMessageRequests() {
    ...
    qorai_wallet_service_->GetPendingSignMessageRequests(
  ...
  void AddSolanaPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
        blink::PermissionType::QORAI_SOLANA, browser_context(), GetOrigin(),
        account_id->address));
  }
  ...
  std::string SignMessage(
    ...
    if (run_notify) {
      qorai_wallet_service_->NotifySignMessageRequestProcessed(
          approve, qorai_wallet_service_->sign_message_id_ - 1,
              ...
              base::BindLambdaForTesting([&]() {
                ...
                qorai_wallet_service_
                    ->NotifySignSolTransactionsRequestProcessed(
                        approve,
                        qorai_wallet_service_->sign_sol_transactions_id_ - 1,
...
TEST_F(SolanaProviderImplUnitTest, Connect) {
  ...
  // permissions::QorAIWalletPermissionContext::RequestPermissions failed
  ...
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::QORAI_SOLANA, CONTENT_SETTING_BLOCK);
  account = Connect(std::nullopt, &error, &error_message);
  EXPECT_TRUE(account.empty());
  EXPECT_EQ(error, mojom::SolanaProviderError::kUserRejectedRequest);
  EXPECT_EQ(error_message,
  ...
  // again.
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::QORAI_SOLANA, CONTENT_SETTING_DEFAULT);
  account = Connect(std::nullopt, &error, &error_message);
  EXPECT_EQ(account, added_account->address);
  EXPECT_EQ(error, mojom::SolanaProviderError::kSuccess);
  EXPECT_TRUE(error_message.empty());
...
TEST_F(SolanaProviderImplUnitTest, SignMessage) {
  ...
  EXPECT_EQ(requests[0]->sign_data->get_solana_sign_data()->message, "QORAI");
  EXPECT_EQ(requests[1]->sign_data->get_solana_sign_data()->message, "QORAI");
  EXPECT_EQ(requests[2]->sign_data->get_solana_sign_data()->message,
            "0x4252415645");
  EXPECT_EQ(requests[3]->sign_data->get_solana_sign_data()->message, "QORAI");
}
