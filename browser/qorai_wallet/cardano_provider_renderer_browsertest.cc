...
constexpr char kCheckCardanoProviderScript[] =
    "!!window.cardano && !!window.cardano.qorai";
constexpr char kOverwriteCardanoScript[] =
    "window.cardano = ['test']; window.cardano[0]";
constexpr char kOverwriteCardanoQorAIScript[] =
    "window.cardano.qorai = ['test']; window.cardano.qorai[0]";
...
std::string NonWriteableScriptApiMethod(std::string_view method) {
  return absl::StrFormat(
      R"(async function check() {
          let x = await window.cardano.qorai.enable()
          x.%s = "qorai"
...
std::string EnableScript() {
  return
      R"(async function connect() {
          try {
            const result = await window.cardano.qorai.enable();
            return true;
          } catch (err) {
            return false;
          }
...
std::string NonWriteableScriptProperty(const std::string& property) {
  return absl::StrFormat(
      R"(window.cardano.qorai.%s = "qorai";
         !(window.cardano.qorai.%s === "qorai");)",
      property, property);
}

class TestCardanoProvider : public qorai_wallet::mojom::CardanoProvider {
 public:
  ...
  void BindReceiver(
      mojo::PendingReceiver<qorai_wallet::mojom::CardanoProvider> receiver) {
    receivers_.Add(this, std::move(receiver));
  }

 private:
  mojo::ReceiverSet<qorai_wallet::mojom::CardanoProvider> receivers_;
};

class TestCardanoApi : public qorai_wallet::mojom::CardanoApi {
 public:
  ...
  void BindReceiver(
      mojo::PendingReceiver<qorai_wallet::mojom::CardanoApi> receiver) {
    receivers_.Add(this, std::move(receiver));
  }

 private:
  mojo::ReceiverSet<qorai_wallet::mojom::CardanoApi> receivers_;
};

class TestQorAIContentBrowserClient : public QorAIContentBrowserClient {
 public:
  TestQorAIContentBrowserClient() {
    provider_ = std::make_unique<TestCardanoProvider>();
  }

  ~TestQorAIContentBrowserClient() override = default;
  TestQorAIContentBrowserClient(const TestQorAIContentBrowserClient&) = delete;
  TestQorAIContentBrowserClient& operator=(
      const TestQorAIContentBrowserClient&) = delete;

  void RegisterBrowserInterfaceBindersForFrame(
      content::RenderFrameHost* render_frame_host,
      mojo::BinderMapWithContext<content::RenderFrameHost*>* map) override {
    QorAIContentBrowserClient::RegisterBrowserInterfaceBindersForFrame(
        render_frame_host, map);
    // override binding for CardanoProvider
    map->Add<qorai_wallet::mojom::CardanoProvider>(
        base::BindRepeating(&TestQorAIContentBrowserClient::BindCardanoProvider,
 ...
 private:
  ...
  base::WeakPtrFactory<TestQorAIContentBrowserClient> weak_ptr_factory_{this};
};
...
class CardanoProviderRendererTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    // TestQorAIContentBrowserClient::RegisterBrowserInterfaceBindersForFrame
    ASSERT_TRUE(
        ui_test_utils::NavigateToURL(browser(), GURL("qorai://settings")));
  ...
  qorai_wallet::KeyringService* GetKeyringService() {
 ...
 protected:
  ...
  TestQorAIContentBrowserClient test_content_browser_client_;
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, ExtensionWallet) {
  ReloadAndWaitForLoadStop(browser());
  // Add a new wallet along with the qorai one
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest,
                       ExtensionOverwriteCardanoQorAI) {
  ReloadAndWaitForLoadStop(browser());
  // can't be overwritten
  EXPECT_TRUE(
      content::EvalJs(web_contents(browser()), kOverwriteCardanoQorAIScript)
          .is_ok());
  {
    auto result = EvalJs(web_contents(browser()), kCheckCardanoProviderScript);
    EXPECT_EQ(base::Value(true), result);
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, Properties) {
  ...
  {
    auto result = EvalJs(web_contents(browser()), "window.cardano.qorai.name");
    EXPECT_EQ(base::Value("QorAI"), result);
  }

  {
    auto result = EvalJs(web_contents(browser()),
                         "window.cardano.qorai.supportedExtensions");
    EXPECT_EQ(base::Value::List(), result);
  }

  {
    auto result = EvalJs(web_contents(browser()), "window.cardano.qorai.icon");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, NonWritableCardanoQorAI) {
  {
    auto result = EvalJs(web_contents(browser()),
                         NonWriteableScriptMethod("cardano.qorai", "enable"));
...

  {
    auto result =
        EvalJs(web_contents(browser()),
               NonWriteableScriptMethod("cardano.qorai", "isEnabled"));
    EXPECT_EQ(base::Value(true), result) << result;
  }

  // window.cardano.qorai.* (properties)
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, EnableFail) {
  ...
  auto result =
      content::EvalJs(web_contents(browser()), R"(async function connect() {
          try {
            return await window.cardano.qorai.enable();
          } catch (err) {
            return err;
          }
        }
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, IsEnabled) {
  ...
  auto result = EvalJs(
      ...
      "(async () => { return await window.cardano.qorai.isEnabled()})()");
  EXPECT_EQ(base::Value(true), result);
}
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetNetworkId) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       ...
                       "window.cardano.qorai.enable()).getNetworkId() })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetUsedAddresses) {
  ...
  auto result =
      EvalJs(web_contents(browser()),
             "(async () => { return await (await "
             "window.cardano.qorai.enable()).getUsedAddresses() })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetUnusedAddresses) {
  ...
  auto result =
      EvalJs(web_contents(browser()),
             "(async () => { return await (await "
             "window.cardano.qorai.enable()).getUnusedAddresses() })();");
  base::Value::List list_value;
  list_value.Append(base::Value("1"));
  list_value.Append(base::Value("2"));
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetBalance) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { return await (await "
                       "window.cardano.qorai.enable()).getBalance() })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetChangeAddress) {
  ...
  auto result =
      EvalJs(web_contents(browser()),
             "(async () => { return await (await "
             "window.cardano.qorai.enable()).getChangeAddress() })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetRewardAddresses) {
  ...
  auto result =
      EvalJs(web_contents(browser()),
             "(async () => { return await (await "
             "window.cardano.qorai.enable()).getRewardAddresses() })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetUtxos) {
  // Amount and pagination are defined.
  {
    ...
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).getUtxos(\"1\", {page: "
               "2, limit:3}) })();");
  ...
  // Amount undefined, pagination not defined.
  {
    ...
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).getUtxos(undefined) })();");
  ...
  // Amount undefined with defined pagination.
  {
    ...
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).getUtxos(undefined, {page: "
               "2, limit:3}) })();");
  ...
  // Both undefined.
  {
    ...
    auto result = EvalJs(
        web_contents(browser()),
        "(async () => { return await (await "
        "window.cardano.qorai.enable()).getUtxos(undefined, undefined) })();");
  ...
  // With amount, pagination not defined.
  {
    ...
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).getUtxos(\"1\") })();");
  ...
  // With amount, pagination is undefined.
  {
    ...
    auto result = EvalJs(
        web_contents(browser()),
        "(async () => { return await (await "
        "window.cardano.qorai.enable()).getUtxos(\"1\", undefined) })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetUtxos_NoArgs) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { return await (await "
                       "window.cardano.qorai.enable()).getUtxos() })();");
  ...
  // Wrong argument types.
  {
...

    {
      auto result =
          EvalJs(web_contents(browser()),
                 "(async () => { return await (await "
                 "window.cardano.qorai.enable()).getUtxos(1, 2) })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SignTx) {
  ...
  auto result =
      EvalJs(web_contents(browser()),
             "(async () => { return await (await "
             "window.cardano.qorai.enable()).signTx(\"tx\", true) })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SignTx_PartialUndefined) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { return await (await "
                       "window.cardano.qorai.enable()).signTx(\"tx\") })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SignTx_WrongArguments) {
...

  {
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).signTx(\"tx\", 1) })();");
    EXPECT_FALSE(result.is_ok());
  }
...

  {
    auto result = EvalJs(web_contents(browser()),
                         "(async () => { return await (await "
                         "window.cardano.qorai.enable()).signTx(1) })();");
    EXPECT_FALSE(result.is_ok());
  }

  {
    auto result = EvalJs(web_contents(browser()),
                         "(async () => { return await (await "
                         "window.cardano.qorai.enable()).signTx() })();");
    EXPECT_FALSE(result.is_ok());
  }
}
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SignData) {
  ...
  auto result = EvalJs(
      web_contents(browser()),
      "(async () => { return await (await "
      "window.cardano.qorai.enable()).signData(\"addr\", \"data\") })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SignData_WrongArguments) {
...

  {
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).signData(\"addr\") })();");
    EXPECT_FALSE(result.is_ok());
  }

  {
    auto result = EvalJs(web_contents(browser()),
                         "(async () => { return await (await "
                         "window.cardano.qorai.enable()).signData() })();");
    EXPECT_FALSE(result.is_ok());
  }

  {
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).signData(\"\", 1) })();");
    EXPECT_FALSE(result.is_ok());
  }
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SubmitTx_Error) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { try { return await (await "
                       "window.cardano.qorai.enable()).submitTx(\"1\") } "
                       "catch(err) {return err}})();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, SubmitTx_WrongArguments) {
...

  {
    auto result = EvalJs(web_contents(browser()),
                         "(async () => { return await (await "
                         "window.cardano.qorai.enable()).submitTx() })();");
...

  {
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).submitTx(\"1\", \"2\") })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetExtensions) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { return await (await "
                       "window.cardano.qorai.enable()).getExtensions()})();");
  EXPECT_EQ(base::Value::List(), result);
}
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, GetCollateral) {
  ...
  auto result = EvalJs(web_contents(browser()),
                       "(async () => { return await (await "
                       "window.cardano.qorai.enable()).getCollateral({amount: "
                       "\"amount\"}) })();");
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest,
...

  {
    auto result =
        EvalJs(web_contents(browser()),
               "(async () => { return await (await "
               "window.cardano.qorai.enable()).getCollateral(1) })();");
    EXPECT_FALSE(result.is_ok());
  }

  {
    auto result = EvalJs(
        web_contents(browser()),
        "(async () => { return await (await "
        "window.cardano.qorai.enable()).getCollateral({amount: 1}) })();");
    EXPECT_FALSE(result.is_ok());
  }
}
...
IN_PROC_BROWSER_TEST_F(CardanoProviderRendererTest, NotInstalled) {
  qorai_wallet::SetDefaultCardanoWallet(
