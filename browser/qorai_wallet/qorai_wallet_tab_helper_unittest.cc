...
class QorAIWalletTabHelperUnitTest : public BrowserWithTestWindowTest {
 public:
  QorAIWalletTabHelperUnitTest() = default;
  ~QorAIWalletTabHelperUnitTest() override = default;
...
TEST_F(QorAIWalletTabHelperUnitTest, GetApproveBubbleURL) {
  auto* helper = qorai_wallet_tab_helper();
  ASSERT_TRUE(helper);
  ASSERT_EQ(helper->GetApproveBubbleURL(),
            GURL("chrome://wallet-panel.top-chrome/#approveTransaction"));
}

TEST_F(QorAIWalletTabHelperUnitTest, ChooserTitle) {
  auto wallet_label = QorAICreateTitleLabel();
  EXPECT_EQ(GetHIDTitle(web_contents(), GURL(kQorAIUIWalletPanelURL)),
            wallet_label);
  EXPECT_EQ(GetHIDTitle(web_contents(), GURL(kQorAIUIWalletPageURL)),
            wallet_label);
  EXPECT_NE(GetHIDTitle(web_contents(), GURL("a.com")), wallet_label);
}

TEST_F(QorAIWalletTabHelperUnitTest, SolanaConnectedAccount) {
  auto* helper = qorai_wallet_tab_helper();
  ASSERT_TRUE(helper);
