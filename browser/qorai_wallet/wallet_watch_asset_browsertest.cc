...
class WalletWatchAssetBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void RestoreWallet() {
    ASSERT_TRUE(qorai_wallet_service()->keyring_service()->RestoreWalletSync(
  ...
  std::vector<mojom::BlockchainTokenPtr> GetUserAssets() {
    ...
    qorai_wallet_service()->GetUserAssets(
        qorai_wallet_service()->network_manager()->GetCurrentChainId(
  ...
  for (size_t i = 0; i < methods_.size(); i++) {
    ...
    EXPECT_TRUE(
        qorai_wallet::QorAIWalletTabHelper::FromWebContents(web_contents())
            ->IsShowingBubble());
    qorai_wallet_service()->NotifyAddSuggestTokenRequestsProcessed(
        true, {addresses_[i]});
  }
