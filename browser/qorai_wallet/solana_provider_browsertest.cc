...
// signMessage
constexpr char kMessage[] = "qoraiy baby!";
...
class TestTxServiceObserver : public mojom::TxServiceObserver {
 public:
  ...
  mojo::PendingRemote<qorai_wallet::mojom::TxServiceObserver> GetReceiver() {
    return observer_receiver_.BindNewPipeAndPassRemote();
  }

 private:
  mojo::Receiver<qorai_wallet::mojom::TxServiceObserver> observer_receiver_{
      this};
...
class TestContentBrowserClient : public QorAIContentBrowserClient {
 public:
 ...
 private:
  void BindSolanaProvider(
    ...
    QorAIWalletTabHelper::BindSolanaProvider(frame_host, std::move(receiver));
 ...
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script =
          LoadDataResource(IDR_QORAI_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
  ...
  void UserGrantPermission(bool granted,
    ...
    if (granted) {
      ...
      permissions::QorAIWalletPermissionContext::Cancel(web_contents());
    }
  ...
  bool GetIsQorAIWalletViaProxy() {
    return EvalJs(web_contents(), "getIsQorAIWalletViaProxy()").ExtractBool();
  }
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectRequestInProgress) {
  ...
  (async function() {
    try {
      return await window.qoraiSolana.connect()
    } catch (err) {
      return err
    }
  })()
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectedStatusAndPermission) {
  ...
  host_content_settings_map()->ClearSettingsForOneType(
      ContentSettingsType::QORAI_SOLANA);
  ...
  host_content_settings_map()->ClearSettingsForOneTypeWithPredicate(
      ContentSettingsType::QORAI_SOLANA, base::Time(), base::Time::Max(),
      base::BindLambdaForTesting(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectedStatusInIframes) {
  ...
  // 2. QorAIWalletProviderDelegateImpl::RenderFrameHostChanged doesn't get
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, SignMessage) {
  ...
  qorai_wallet_service()->NotifySignMessageRequestProcessed(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, GetPublicKey) {
  ...
  constexpr char get_public_key_script[] =
      "window.qoraiSolana.publicKey ? window.qoraiSolana.publicKey.toString() "
      ": ''";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, SignTransaction) {
  ...
  EXPECT_TRUE(WaitForWalletBubble(web_contents()));
  // user rejected request
  qorai_wallet_service()->NotifySignSolTransactionsRequestProcessed(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, Request) {
  ...
  qorai_wallet_service()->NotifySignMessageRequestProcessed(true, 0, nullptr,
                                                            std::nullopt);
  WaitForResultReady();
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, CallViaProxy) {
  ...
  EXPECT_TRUE(GetIsQorAIWalletViaProxy());
