...
class TestQoraiWalletServiceObserverForAssetDiscoveryManager
    : public qorai_wallet::QoraiWalletServiceObserverBase {
 public:
  TestQoraiWalletServiceObserverForAssetDiscoveryManager() = default;
 ...
 protected:
  void SetUp() override {
    scoped_feature_list_.InitAndEnableFeature(
        features::kNativeQoraiWalletFeature);
    ...
    wallet_service_ = std::make_unique<QoraiWalletService>(
        shared_url_loader_factory_,
        QoraiWalletServiceDelegate::Create(profile_.get()), GetPrefs(),
        GetLocalState());
    ...
    wallet_service_observer_ = std::make_unique<
        TestQoraiWalletServiceObserverForAssetDiscoveryManager>();
  ...
  std::unique_ptr<TestQoraiWalletServiceObserverForAssetDiscoveryManager>
      wallet_service_observer_;
  content::BrowserTaskEnvironment task_environment_;
  std::unique_ptr<api_request_helper::APIRequestHelper> api_request_helper_;
  TestingPrefServiceSimple local_state_;
  std::unique_ptr<TestingProfile> profile_;
  std::unique_ptr<QoraiWalletService> wallet_service_;
...
TEST_F(AssetDiscoveryManagerUnitTest, DiscoverAssetsOnAllSupportedChains) {
  ...
  // kQoraiWalletLastDiscoveredAssetsAt does not update if accounts_added set
  base::Time current_assets_last_discovered_at =
      GetPrefs()->GetTime(kQoraiWalletLastDiscoveredAssetsAt);
  ...
  // Verify OnDiscoverAssetsStarted and OnDiscoverAssetsCompleted both fire and
  // kQoraiWalletLastDiscoveredAssetsAt updates if accounts_added not set
