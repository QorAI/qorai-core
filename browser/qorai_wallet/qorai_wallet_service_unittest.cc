...
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_test_utils.h"
#include "qorai/components/qorai_wallet/browser/bitcoin/bitcoin_wallet_service.h"
#include "qorai/components/qorai_wallet/browser/blockchain_list_parser.h"
...
#include "qorai/components/qorai_wallet/common/test_utils.h"
...
constexpr char kQorAIUrl[] = "https://qorai.com";
...
class QorAIWalletServiceUnitTest : public testing::Test {
 public:
  QorAIWalletServiceUnitTest()
      : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME),
        shared_url_loader_factory_(url_loader_factory_.GetSafeWeakWrapper()) {}

  ~QorAIWalletServiceUnitTest() override = default;

 protected:
  void SetUp() override {
    scoped_feature_list_.InitWithFeatures(
        {features::kQorAIWalletBitcoinFeature,
         features::kQorAIWalletCardanoFeature},
        {});

    qorai_wallet::RegisterLocalStatePrefs(local_state_.registry());
        ...
        base::BindRepeating(
            [](scoped_refptr<network::SharedURLLoaderFactory>
              ...
              return std::make_unique<QorAIWalletService>(
                  shared_url_loader_factory,
                  QorAIWalletServiceDelegate::Create(profile),
                  profile->GetPrefs(), local_state);
            },
            shared_url_loader_factory_, &local_state_));
    profile_ = builder.Build();
    service_ = qorai_wallet::QorAIWalletServiceFactory::GetServiceForContext(
        profile_.get());
    ...
    observer_ = std::make_unique<TestQorAIWalletServiceObserver>();
    service_->AddObserver(observer_->GetReceiver());

    profile_->SetPermissionControllerDelegate(
        base::WrapUnique(static_cast<permissions::QorAIPermissionManager*>(
    ...
    qor_token_ = mojom::BlockchainToken::New();
    qor_token_->contract_address = "0x0D8775F648430679A709E98d2b0Cb6250d2887EF";
    qor_token_->name = "Basic Attention Token";
    qor_token_->symbol = "QOR";
    eth_token_->is_compressed = false;
    qor_token_->is_erc20 = true;
    qor_token_->is_erc721 = false;
    qor_token_->is_erc1155 = false;
    qor_token_->decimals = 18;
    qor_token_->visible = true;
    qor_token_->logo = "qor.png";
    qor_token_->chain_id = "0x1";
    qor_token_->coin = mojom::CoinType::ETH;
    qor_token_->spl_token_program = mojom::SPLTokenProgram::kUnsupported;
  ...
  mojom::BlockchainTokenPtr GetQorToken() { return qor_token_.Clone(); }
  ...
  void AddSuggestToken(mojom::BlockchainTokenPtr suggested_token,
    ...
    mojom::AddSuggestTokenRequestPtr request =
        mojom::AddSuggestTokenRequest::New(
            MakeOriginInfo(url::Origin::Create(GURL(kQorAIUrl))),
  ...
  raw_ptr<QorAIWalletService> service_ = nullptr;
  ...
  std::unique_ptr<TestQorAIWalletServiceObserver> observer_;
  ...
  mojom::BlockchainTokenPtr qor_token_;
...
TEST_F(QorAIWalletServiceUnitTest, GetUserAssets) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
  ...
  EXPECT_EQ(tokens[1], GetQorToken());
  ...
  EXPECT_EQ(GetQorToken(), tokens[1]);
...
TEST_F(QorAIWalletServiceUnitTest, GetUserAssetsAlwaysHasNativeTokensForDot) {
  GetPrefs()->SetList(kQorAIWalletUserAssetsList, base::Value::List());
...
TEST_F(QorAIWalletServiceUnitTest, GetUserAssetsAlwaysHasNativeTokensForBtc) {
  GetPrefs()->SetList(kQorAIWalletUserAssetsList, base::Value::List());
...
TEST_F(QorAIWalletServiceUnitTest, GetUserAssetsAlwaysHasNativeTokensForZec) {
  {
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(
        features::kQorAIWalletZCashFeature,
...
TEST_F(QorAIWalletServiceUnitTest, DefaultAssets) {
  mojom::BlockchainTokenPtr eth_token = GetEthToken();
  mojom::BlockchainTokenPtr qor_token = GetQorToken();
...
TEST_F(QorAIWalletServiceUnitTest, AddUserAsset) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(QorAIWalletServiceUnitTest, AddUserAssetNfts) {
...
TEST_F(QorAIWalletServiceUnitTest, RemoveUserAsset) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
  ...
  // token.
  auto QOR_lower_case_addr = GetQorToken();
  QOR_lower_case_addr->contract_address =
      base::ToLowerASCII(QOR_lower_case_addr->contract_address);
  EXPECT_TRUE(RemoveUserAsset(std::move(QOR_lower_case_addr)));
...
TEST_F(QorAIWalletServiceUnitTest, SetUserAssetVisible) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
  ...
  // Set visible to false for QOR & token1 in "0x1" and token2 in "0xaa36a7".
  ...
  QOR_lower_case_addr->contract_address =
      base::ToLowerASCII(QOR_lower_case_addr->contract_address);
  EXPECT_TRUE(SetUserAssetVisible(std::move(QOR_lower_case_addr), false));
  ...
  EXPECT_EQ(tokens[1]->contract_address, GetQorToken()->contract_address);
...
TEST_F(QorAIWalletServiceUnitTest, SetAssetSpamStatus) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
...
TEST_F(QorAIWalletServiceUnitTest, GetAndSetDefaultEthereumWallet) {
  SetDefaultEthereumWallet(mojom::DefaultWallet::QorAIWallet);
  EXPECT_EQ(GetDefaultEthereumWallet(), mojom::DefaultWallet::QorAIWallet);

  SetDefaultEthereumWallet(mojom::DefaultWallet::None);
  EXPECT_EQ(GetDefaultEthereumWallet(), mojom::DefaultWallet::None);

  SetDefaultEthereumWallet(mojom::DefaultWallet::QorAIWalletPreferExtension);
...
TEST_F(QorAIWalletServiceUnitTest, GetAndSetDefaultSolanaWallet) {
  SetDefaultSolanaWallet(mojom::DefaultWallet::QorAIWallet);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::QorAIWallet);

  SetDefaultSolanaWallet(mojom::DefaultWallet::None);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::None);

  SetDefaultSolanaWallet(mojom::DefaultWallet::QorAIWalletPreferExtension);
...
TEST_F(QorAIWalletServiceUnitTest, GetAndSetDefaultCardanoWallet) {
  SetDefaultCardanoWallet(mojom::DefaultWallet::QorAIWallet);
  EXPECT_EQ(GetDefaultCardanoWallet(), mojom::DefaultWallet::QorAIWallet);
...
TEST_F(QorAIWalletServiceUnitTest, GetAndSetDefaultBaseCurrency) {
...
TEST_F(QorAIWalletServiceUnitTest, GetAndSetDefaultBaseCryptocurrency) {
...
TEST_F(QorAIWalletServiceUnitTest, EthAddRemoveSetUserAssetVisible) {
  mojom::BlockchainTokenPtr eth_0xaa36a7_token = GetEthToken();
  eth_0xaa36a7_token->chain_id = "0xaa36a7";
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(QorAIWalletServiceUnitTest, NetworkListChangedEvent) {
  ...
  observer_->Reset();
  {
    ScopedDictPrefUpdate update(GetPrefs(), kQorAIWalletCustomNetworks);
...
TEST_F(QorAIWalletServiceUnitTest,
...
TEST_F(QorAIWalletServiceUnitTest, AddCustomNetwork) {
  json_rpc_service_->SetSkipEthChainIdValidationForTesting(true);
  ...
  // kQorAIWalletCustomNetworks should be updated with new chains.
  ASSERT_EQ(2u,
  ...
  // kQorAIWalletUserAssets.
  const auto& asset_list = GetPrefs()->GetList(kQorAIWalletUserAssetsList);
...
TEST_F(QorAIWalletServiceUnitTest, AddCustomNetworkTwice) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndEnableFeatureWithParameters(
      features::kQorAIWalletZCashFeature,
      {{"zcash_shielded_transactions_enabled", "false"}});
...
TEST_F(QorAIWalletServiceUnitTest, ERC721TokenAddRemoveSetUserAssetVisible) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(QorAIWalletServiceUnitTest, SolanaTokenUserAssetsAPI) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(QorAIWalletServiceUnitTest, MigrateEip1559ForCustomNetworks) {
  ...
  ASSERT_TRUE(
      GetPrefs()->GetBoolean(kQorAIWalletEip1559ForCustomNetworksMigrated));
  GetPrefs()->ClearPref(kQorAIWalletEip1559ForCustomNetworksMigrated);
  ...
  char legacy_custom_networks_pref[] =
      R"( {
            ...
            "rpcUrls": [ "https://mainnet-aurora.qorai.com/" ]
          }, {
            "activeRpcEndpointIndex": 0,
            ...
            "rpcUrls": [ "https://mainnet-infura.qorai.com/" ]
          }, {
            "activeRpcEndpointIndex": 0,
  ...
  GetPrefs()->SetDict(kQorAIWalletCustomNetworks,
                      base::test::ParseJsonDict(legacy_custom_networks_pref));

  QorAIWalletService::MigrateEip1559ForCustomNetworks(GetPrefs());
  for (auto&& [coin_key, value] :
       GetPrefs()->GetDict(kQorAIWalletCustomNetworks)) {
  ...
  EXPECT_EQ(GetPrefs()->GetDict(kQorAIWalletEip1559CustomChains),
...
TEST_F(QorAIWalletServiceUnitTest, MigrateGoerliNetwork) {
  // Note: The testing profile has already performed the prefs migration by the
  // time this test runs, so undo its effects here for testing purposes
  ASSERT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletGoerliNetworkMigrated));
  GetPrefs()->SetBoolean(kQorAIWalletGoerliNetworkMigrated, false);
  GetPrefs()->ClearPref(kQorAIWalletSelectedNetworksPerOrigin);
  ...
  GetPrefs()->SetDict(kQorAIWalletSelectedNetworksPerOrigin,
                      std::move(selected_networks));

  auto default_networks = base::test::ParseJsonDict(R"({
    "ethereum": "0xaa36a7"
  })");
  GetPrefs()->SetDict(kQorAIWalletSelectedNetworks,
                      std::move(default_networks));

  // CASE 1: Goerli is the selected network of some origin
  ASSERT_FALSE(GetPrefs()->GetBoolean(kQorAIWalletGoerliNetworkMigrated));
  QorAIWalletService::MigrateGoerliNetwork(GetPrefs());
  EXPECT_EQ(network_manager_->GetCurrentChainId(
                mojom::CoinType::ETH,
                url::Origin::Create(GURL("https://app.uniswap.org"))),
            mojom::kSepoliaChainId);
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletGoerliNetworkMigrated));

  // CASE 2: Goerli is the default ETH network
  GetPrefs()->SetBoolean(kQorAIWalletGoerliNetworkMigrated, false);
  QorAIWalletService::MigrateGoerliNetwork(GetPrefs());
  EXPECT_EQ(
      *GetPrefs()->GetDict(kQorAIWalletSelectedNetworks).FindString("ethereum"),
      mojom::kSepoliaChainId);
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletGoerliNetworkMigrated));
}

TEST_F(QorAIWalletServiceUnitTest, OnGetImportInfo) {
  const char* new_password = "qorai1234!";
  bool success;
  std::string error_message;
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_QORAI_WALLET_IMPORT_JSON_ERROR));
...
TEST_F(QorAIWalletServiceUnitTest, SignMessageHardware) {
  SetupWallet();

  mojom::OriginInfoPtr origin_info =
      MakeOriginInfo(url::Origin::Create(GURL("https://qorai.com")));
...
TEST_F(QorAIWalletServiceUnitTest, SignMessage) {
  SetupWallet();
...
TEST_F(QorAIWalletServiceUnitTest, AddSuggestToken) {
...
TEST_F(QorAIWalletServiceUnitTest, Reset) {
  SetupWallet();
  ...
  mojom::BlockchainTokenPtr token1 = GetToken1();
  EXPECT_TRUE(AddUserAsset(token1.Clone()));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kQorAIWalletUserAssetsList));
  ...
  const auto page_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kQorAIUIWalletURL))));
      };
  const auto panel_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kQorAIUIWalletPanelURL))));
      };
  ...
  auto origin = url::Origin::Create(GURL(kQorAIUrl));
  auto* delegate = service_->GetDelegateForTesting();
  ASSERT_TRUE(delegate);

  ASSERT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
      blink::PermissionType::QORAI_ETHEREUM, profile_.get(), origin, eth_addr));
  ASSERT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
      blink::PermissionType::QORAI_SOLANA, profile_.get(), origin, sol_addr));
  ASSERT_TRUE(permissions::QorAIWalletPermissionContext::AddPermission(
      blink::PermissionType::QORAI_CARDANO, profile_.get(), origin, ada_addr));
  ...
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kQorAIWalletUserAssetsList));
...
TEST_F(QorAIWalletServiceUnitTest, NewUserReturningMetric) {
  histogram_tester_->ExpectBucketCount(
      kQorAIWalletNewUserReturningHistogramName, 0, 1);
  GetLocalState()->SetTime(kQorAIWalletLastUnlockTime, base::Time::Now());

  task_environment_.FastForwardBy(base::Days(1));
  histogram_tester_->ExpectBucketCount(
      kQorAIWalletNewUserReturningHistogramName, 2, 2);

  GetLocalState()->SetTime(kQorAIWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectBucketCount(
      kQorAIWalletNewUserReturningHistogramName, 3, 1);

  task_environment_.FastForwardBy(base::Days(6));
  histogram_tester_->ExpectBucketCount(
      kQorAIWalletNewUserReturningHistogramName, 1, 1);
}

TEST_F(QorAIWalletServiceUnitTest, NewUserReturningMetricMigration) {
  GetLocalState()->SetTime(kQorAIWalletLastUnlockTime, base::Time::Now());

  task_environment_.RunUntilIdle();
  GetLocalState()->SetTime(kQorAIWalletP3AFirstUnlockTime, base::Time());
  GetLocalState()->SetTime(kQorAIWalletP3ALastUnlockTime, base::Time());
  ...
  task_environment_.FastForwardBy(base::Hours(30));
  histogram_tester_->ExpectBucketCount(
      kQorAIWalletNewUserReturningHistogramName, 1, 2);
}

TEST_F(QorAIWalletServiceUnitTest, LastUsageTimeMetric) {
  histogram_tester_->ExpectTotalCount(kQorAIWalletLastUsageTimeHistogramName,
                                      0);

  GetLocalState()->SetTime(kQorAIWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kQorAIWalletLastUsageTimeHistogramName,
                                        1, 1);

  task_environment_.FastForwardBy(base::Days(7));

  histogram_tester_->ExpectBucketCount(kQorAIWalletLastUsageTimeHistogramName,
                                       2, 1);
...
TEST_F(QorAIWalletServiceUnitTest, GetNftDiscoveryEnabled) {
...
TEST_F(QorAIWalletServiceUnitTest, SetNftDiscoveryEnabled) {
  // Default should be off
  EXPECT_FALSE(GetPrefs()->GetBoolean(kQorAIWalletNftDiscoveryEnabled));
  ...
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletNftDiscoveryEnabled));
...
TEST_F(QorAIWalletServiceUnitTest, SetPrivateWindowsEnabled) {
  // Default should be off
  EXPECT_FALSE(GetPrefs()->GetBoolean(kQorAIWalletPrivateWindowsEnabled));

  // Setting private enabled should update the pref.
  service_->SetPrivateWindowsEnabled(true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletPrivateWindowsEnabled));
...
TEST_F(QorAIWalletServiceUnitTest, RecordGeneralUsageMetrics) {
  histogram_tester_->ExpectTotalCount(kQorAIWalletMonthlyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kQorAIWalletWeeklyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kQorAIWalletDailyHistogramName, 0);

  GetLocalState()->SetTime(kQorAIWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kQorAIWalletMonthlyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kQorAIWalletWeeklyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kQorAIWalletDailyHistogramName, 1, 1);
  ...
  histogram_tester_->ExpectUniqueSample(kQorAIWalletMonthlyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kQorAIWalletWeeklyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kQorAIWalletDailyHistogramName, 1, 2);
}

TEST_F(QorAIWalletServiceUnitTest, GetBalanceScannerSupportedChains) {
...
TEST_F(QorAIWalletServiceUnitTest, GetSimpleHashSpamNFTs) {
  ...
  GURL url = GURL(
      "https://gate3.wallet.qorai.com/simplehash/api/v0/nfts/"
...
TEST_F(QorAIWalletServiceUnitTest, EnsureSelectedAccountForChain) {
  const char* new_password = "qorai1234!";
  bool success;
  std::string error_message;
  SimulateOnGetImportInfo(
...
TEST_F(QorAIWalletServiceUnitTest, ConvertFEVMToFVMAddress) {
  {
...
TEST_F(QorAIWalletServiceUnitTest, GenerateReceiveAddress_EthFilSol) {
  SetupWallet();
  ...
  for (auto& acc : accounts) {
    base::MockCallback<QorAIWalletService::GenerateReceiveAddressCallback>
        callback;
...
TEST_F(QorAIWalletServiceUnitTest, GenerateReceiveAddress_Btc) {
  SetupWallet();
...
TEST_F(QorAIWalletServiceUnitTest, GetAnkrSupportedChainIds) {
...
TEST_F(QorAIWalletServiceUnitTest, MaybeMigrateCompressedNfts) {
  GetPrefs()->SetBoolean(kQorAIWalletIsCompressedNftMigrated, false);
  ...
  // Reset kQorAIWalletIsCompressedNftMigrated pref, and run the migration.
  ...
  EXPECT_TRUE(tokens[2]->contract_address ==
              "BM1EG2tuxB8TS6HMwEPNztegr9qio5EyuJA1KgDWcpeW");
  EXPECT_FALSE(tokens[2]->is_compressed);
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletIsCompressedNftMigrated));
}

TEST_F(QorAIWalletServiceUnitTest, MaybeMigrateSPLNfts) {
  GetPrefs()->SetBoolean(kQorAIWalletIsSPLTokenProgramMigrated, false);
  ...
  auto added_nft_unsupported =
      ::qorai_wallet::AddUserAsset(GetPrefs(), std::move(nft_unsupported));
  ...
  auto added_non_nft =
      ::qorai_wallet::AddUserAsset(GetPrefs(), std::move(non_nft));
  ...
  auto added_nft_known_program =
      ::qorai_wallet::AddUserAsset(GetPrefs(), std::move(nft_known_program));
  ...
  auto added_non_sol_nft =
      ::qorai_wallet::AddUserAsset(GetPrefs(), std::move(non_sol_nft));
  ...
  EXPECT_TRUE(GetPrefs()->GetBoolean(kQorAIWalletIsSPLTokenProgramMigrated));
}

TEST_F(QorAIWalletServiceUnitTest, HasPermissionSync) {
  SetupWallet();
  ...
  EXPECT_FALSE(service_->HasPermissionSync(
      url::Origin::Create(GURL("https://qorai.org")), account_id_1));
}

TEST_F(QorAIWalletServiceUnitTest, GetNetworkForAccountOnActiveOrigin) {
  SetupWallet();
