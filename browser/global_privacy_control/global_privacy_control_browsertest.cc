...
using blink::features::kQoraiGlobalPrivacyControl;
...
// `navigator.globalPrivacyControl` JS property, which is read-only. In Qorai
...
class GlobalPrivacyControlFlagDisabledTest
 ...
 public:
  GlobalPrivacyControlFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQoraiGlobalPrivacyControl);
  }
...
class GlobalPrivacyControlPolicyTest : public GlobalPrivacyControlBrowserTest {
 public:
  ...
  void SetGlobalPrivacyControlPolicy(bool enabled) {
    ...
    policies.Set(policy::key::kQoraiGlobalPrivacyControlEnabled,
