...
class QoraiWebGLFarblingBrowserTest : public InProcessBrowserTest {
 public:
  QoraiWebGLFarblingBrowserTest() {
...
IN_PROC_BROWSER_TEST_F(QoraiWebGLFarblingBrowserTest, FarbleGetParameterWebGL) {
...
IN_PROC_BROWSER_TEST_F(QoraiWebGLFarblingBrowserTest,
  ...
  for (const auto& pair : tests) {
    ...
    qorai_shields::SetWebcompatEnabled(
        content_settings(), ContentSettingsType::QORAI_WEBCOMPAT_WEBGL, true,
        url, nullptr);
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
...
IN_PROC_BROWSER_TEST_F(QoraiWebGLFarblingBrowserTest, GetSupportedExtensions) {
  std::string domain = "a.com";
  GURL url =
...
IN_PROC_BROWSER_TEST_F(QoraiWebGLFarblingBrowserTest, GetExtension) {
  std::string domain = "a.com";
...
IN_PROC_BROWSER_TEST_F(QoraiWebGLFarblingBrowserTest, GetAttachedShaders) {
  std::string domain = "a.com";
  ...
  // https://github.com/qorai/qorai-browser/issues/37044
  EXPECT_EQ(EvalJs(contents(), kTitleScript).ExtractString(),
            "[object WebGLShader]");
}
