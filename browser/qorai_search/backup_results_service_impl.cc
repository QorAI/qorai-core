...
#include "qorai/browser/qorai_search/backup_results_service_impl.h"
...
constexpr net::NetworkTrafficAnnotationTag kNetworkTrafficAnnotationTag =
    net::DefineNetworkTrafficAnnotation("qorai_search_backup", R"(
      semantics {
        sender: "Qorai Search Backup Results Service"
        description:
          "Requests results from a backup search "
          "provider for users that have opted into this feature."
        trigger:
          "Triggered by Qorai Search or Web Discovery Project if a user has opted in."
        data:
      ...
      policy {
        cookies_allowed: YES
        setting:
          "You can enable or disable these features on qorai://settings/search "
          "and https://search.qorai.com/settings"
...
void BackupResultsServiceImpl::FetchBackupResults(
  ...
  if (should_render) {
    ...
    if (host_content_settings_map &&
        qorai_shields::GetNoScriptControlType(host_content_settings_map, url) ==
