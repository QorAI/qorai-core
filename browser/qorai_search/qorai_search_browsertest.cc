...
constexpr char kEmbeddedTestServerDirectory[] = "qorai-search";
constexpr char kAllowedDomain[] = "search.qorai.com";
constexpr char kAllowedDomainDev[] = "search.qorai.software";
constexpr char kNotAllowedDomain[] = "qorai.com";
constexpr char kQoraiSearchPath[] = "/qoraisearch.html";
...
constexpr char kSearchAdsHeader[] = "Qorai-Search-Ads";
...
constexpr char kScriptDefaultAPIExists[] =
    "!!(window.qorai && window.qorai.getCanSetDefaultSearchProvider)";
...
constexpr char kScriptDefaultAPIGetValue[] = R"(
  new Promise(resolve => {
    setTimeout(function () {
    qorai.getCanSetDefaultSearchProvider()
    .then((canSet) => {
      resolve(canSet)
    })
    }, 1200)
...
class QoraiSearchTest : public InProcessBrowserTest {
 public:
  QoraiSearchTest() = default;
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_->RegisterRequestHandler(base::BindRepeating(
        &QoraiSearchTest::HandleRequest, base::Unretained(this)));
    ...
    qorai_search::QoraiSearchFallbackHost::SetBackupProviderForTest(url);

    // Force default search engine to Google
    // Some tests will fail if Qorai is default
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (path == "/" || path == "/sw.js" || path == kQoraiSearchPath ||
...
class QoraiSearchTestDisabled : public QoraiSearchTest {
 public:
  QoraiSearchTestDisabled() {
    feature_list_.InitAndDisableFeature(
        qorai_search::features::kQoraiSearchDefaultAPIFeature);
  }
};

class QoraiSearchTestEnabled : public QoraiSearchTest {
 public:
  QoraiSearchTestEnabled() {
    feature_list_.InitAndEnableFeatureWithParameters(
        qorai_search::features::kQoraiSearchDefaultAPIFeature,
        {{qorai_search::features::kQoraiSearchDefaultAPIDailyLimitName, "3"},
         {qorai_search::features::kQoraiSearchDefaultAPITotalLimitName, "10"}});
  }
};

IN_PROC_BROWSER_TEST_F(QoraiSearchTest, CheckForAFunction) {
  GURL url = https_server()->GetURL(kAllowedDomain, kQoraiSearchPath);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, CheckForAFunctionDev) {
  GURL url = https_server()->GetURL(kAllowedDomainDev, kQoraiSearchPath);
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  WaitForLoadStop(contents);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, CheckForAnUndefinedFunction) {
  GURL url = https_server()->GetURL(kNotAllowedDomain, kQoraiSearchPath);
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  WaitForLoadStop(contents);
...
// TODO(https://github.com/qorai/qorai-browser/issues/29631): Test flaky on
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTestEnabled,
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTestEnabled, DefaultAPIHiddenUnknownHost) {
  // Opensearch providers are only allowed in the root of a site,
  // See SearchEngineTabHelper::GenerateKeywordFromNavigationEntry.
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTestEnabled, DefaultAPIFalsePrivateWindow) {
  // Opensearch providers are only allowed in the root of a site,
  // See SearchEngineTabHelper::GenerateKeywordFromNavigationEntry.
  GURL url = https_server()->GetURL(kAllowedDomain, "/");
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTestDisabled, DefaultAPIInvisibleKnownHost) {
  // Opensearch providers are only allowed in the root of a site,
  // See SearchEngineTabHelper::GenerateKeywordFromNavigationEntry.
  GURL url = https_server()->GetURL(kAllowedDomain, "/");
  search_test_utils::WaitForTemplateURLServiceToLoad(
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsHeader) {
  ...
  SetRequestExpectationsCallback(base::BindRepeating(
      [](base::OnceClosure loop_closure,
        ...
        if (request.GetURL().path_piece() == kQoraiSearchPath) {
  ...
  prefs->SetBoolean(qorai_rewards::prefs::kEnabled, true);
  prefs->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, false);

  const GURL url = https_server()->GetURL(kAllowedDomain, kQoraiSearchPath);
  EXPECT_TRUE(ui_test_utils::NavigateToURL(browser(), url));

  run_loop.Run();
}

IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsHeaderRewardsDisabled) {
  base::RunLoop run_loop;
  SetRequestExpectationsCallback(base::BindRepeating(
      [](base::OnceClosure loop_closure,
         const net::test_server::HttpRequest& request) {
  ...
  PrefService* prefs = browser()->profile()->GetPrefs();
  prefs->SetBoolean(qorai_rewards::prefs::kEnabled, false);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsSearchResultAdsEnabled) {
  base::RunLoop run_loop;
  SetRequestExpectationsCallback(base::BindRepeating(
      [](base::OnceClosure loop_closure,
         const net::test_server::HttpRequest& request) {
  ...
  PrefService* prefs = browser()->profile()->GetPrefs();
  prefs->SetBoolean(qorai_rewards::prefs::kEnabled, true);
  prefs->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, true);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsHeaderNotAllowedDomain) {
  base::RunLoop run_loop;
  SetRequestExpectationsCallback(base::BindRepeating(
      [](base::OnceClosure loop_closure,
         const net::test_server::HttpRequest& request) {
  ...
  const GURL url = https_server()->GetURL(kNotAllowedDomain, kQoraiSearchPath);
  EXPECT_TRUE(ui_test_utils::NavigateToURL(browser(), url));

  run_loop.Run();
}

IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsHeaderForFetchRequest) {
  PrefService* prefs = browser()->profile()->GetPrefs();
  prefs->SetBoolean(qorai_rewards::prefs::kEnabled, true);
  prefs->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, false);
  ...
  url = https_server()->GetURL(kAllowedDomain, kQoraiSearchPath);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, FetchRequestForNonQoraiSearchTab) {
  PrefService* prefs = browser()->profile()->GetPrefs();
  prefs->SetBoolean(qorai_rewards::prefs::kEnabled, true);
  prefs->SetBoolean(qorai_ads::prefs::kOptedInToSearchResultAds, false);
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, SearchAdsHeaderIncognitoBrowser) {
  base::RunLoop run_loop;
  SetRequestExpectationsCallback(base::BindRepeating(
      [](base::OnceClosure loop_closure,
         const net::test_server::HttpRequest& request) {
...
IN_PROC_BROWSER_TEST_F(QoraiSearchTest, CheckNoCookieForFallback) {
