...
#include "qorai/browser/infobars/sync_cannot_run_infobar_delegate.h"
...
bool SeedDecryptionFailed(raw_ptr<qorai_sync::Prefs> qorai_sync_prefs) {
  CHECK_NE(qorai_sync_prefs, nullptr);
  bool failed_to_decrypt = false;
  std::string seed = qorai_sync_prefs->GetSeed(&failed_to_decrypt);
...
void SyncCannotRunInfoBarDelegate::Create(
  ...
  if (qorai_sync_prefs.IsFailedDecryptSeedNoticeDismissed()) {
    return;
  }

  if (!SeedDecryptionFailed(&qorai_sync_prefs)) {
    return;
  }
...
std::u16string SyncCannotRunInfoBarDelegate::GetMessageText() const {
  return l10n_util::GetStringUTF16(IDS_QORAI_SYNC_CANNOT_RUN_INFOBAR_MESSAGE);
}
...
std::u16string SyncCannotRunInfoBarDelegate::GetButtonLabel(
  ...
  if (button == BUTTON_CANCEL) {
    return l10n_util::GetStringUTF16(
        IDS_QORAI_SYNC_CANNOT_RUN_INFOBAR_DONT_SHOW_BUTTON);
  }

  DCHECK(button == BUTTON_OK);

  return l10n_util::GetStringUTF16(
      IDS_QORAI_SYNC_CANNOT_RUN_INFOBAR_CHECK_DETAILS_BUTTON);
}
