...
#include "qorai/browser/speedreader/page_distiller.h"
...
#include "qorai/browser/ui/webui/speedreader/speedreader_toolbar_data_handler_impl.h"
...
#include "qorai/components/speedreader/common/speedreader.mojom.h"
 ...
 public:
  ...
  PageActionIconView* GetReaderButton() {
    return BrowserView::GetBrowserViewForBrowser(browser())
        ...
        ->GetPageActionIconView(qorai::kSpeedreaderPageActionIconType);
  }
...
IN_PROC_BROWSER_TEST_F(SpeedReaderBrowserTest, DISABLED_SmokeTest) {
  ...
  const std::string kGetStyleLength =
      "document.getElementById('qorai_speedreader_style').innerHTML.length";
...
IN_PROC_BROWSER_TEST_F(SpeedReaderBrowserTest, Redirect) {
  ...
  const std::string kCheckNoStyle =
      "!document.getElementById('qorai_speedreader_style')";
...
IN_PROC_BROWSER_TEST_F(SpeedReaderBrowserTest, Toolbar) {
  ...
  auto* toolbar_view = static_cast<QoraiBrowserView*>(browser()->window())
                           ->reader_mode_toolbar();
...
IN_PROC_BROWSER_TEST_F(SpeedReaderBrowserTest, RSS) {
  ...
  const std::string kNoStyleInjected =
      R"js(document.getElementById('qorai_speedreader_style'))js";
...
class SpeedReaderWithSplitViewBrowserTest : public SpeedReaderBrowserTest {
 public:
  ...
  ReaderModeToolbarView* GetPrimaryToolbar() {
    return qorai_browser_view()->reader_mode_toolbar();
  }

  // Don't cache as it changes whenever active tab changes.
  ReaderModeToolbarView* GetSecondaryToolbar() {
      return qorai_browser_view()
