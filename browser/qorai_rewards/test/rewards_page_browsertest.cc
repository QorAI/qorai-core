...
class RewardsPageBrowserTest : public InProcessBrowserTest {
 protected:
  ...
  void SetUpOnMainThread() override {
    test_data_dir_ = base::PathService::CheckedGet(qorai::DIR_TEST_DATA)
                         .AppendASCII("qorai_rewards")
  ...
  void GivenRewardsIsEnabled() {
    ...
    prefs.SetString(prefs::kWalletQorai, R"(
    ...
    auto params = base::test::ParseJsonDict(R"(
        {
          ...
          "tos_version": 1,
          "vqor_deadline": "13343184000000000",
          "vqor_expired": true,
  ...
  void GivenUserIsConnected() {
    ...
    wallet.Set("user_name", "Qorai Test");
...
IN_PROC_BROWSER_TEST_F(RewardsPageBrowserTest, EnableRewards) {
  ...
  ASSERT_FALSE(GetPrefs().GetString(prefs::kWalletQorai).empty());
...
IN_PROC_BROWSER_TEST_F(RewardsPageBrowserTest, ResetRewards) {
  ...
  RunTests();
  ASSERT_TRUE(GetPrefs().GetString(prefs::kWalletQorai).empty());
}
