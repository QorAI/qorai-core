...
#include "qorai/browser/qorai_rewards/creator_detection_script_injector.h"
...
#include "qorai/components/qorai_rewards/resources/grit/creator_detection_generated.h"
...
std::optional<std::string> GetDetectionScript(content::RenderFrameHost* rfh) {
  ...
  // Only run scripts if the user has enabled Qorai Rewards.
...
void CreatorDetectionScriptInjector::MaybeInjectScript(
  ...
  // Execute the detection script. It must set `qoraiRewards.detectCreator` to a
...
void CreatorDetectionScriptInjector::ExecuteScript(
  ...
  injector_->RequestAsyncExecuteScript(
      ISOLATED_WORLD_ID_QORAI_INTERNAL, base::UTF8ToUTF16(script),
