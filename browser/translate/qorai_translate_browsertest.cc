...
class QoraiTranslateBrowserTest : public InProcessBrowserTest {
 public:
  QoraiTranslateBrowserTest()
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestHandler(base::BindRepeating(
        &QoraiTranslateBrowserTest::HandleRequest, base::Unretained(this)));
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // Remap translate.qorai.com requests to the https test server.
    const std::string host_port = https_server_.host_port_pair().ToString();
    command_line->AppendSwitchASCII(network::switches::kHostResolverRules,
                                    "MAP translate.qorai.com:443 " + host_port +
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (request.GetURL().path() == "/translate") {
      ...
      EXPECT_NE(
          query.find(absl::StrFormat("&key=%s", BUILDFLAG(QORAI_SERVICES_KEY))),
          std::string::npos)
          << "bad qorai api key for request " << request.GetURL();
    }
...
IN_PROC_BROWSER_TEST_F(QoraiTranslateBrowserTest, InternalTranslation) {
  ResetObserver();
...
IN_PROC_BROWSER_TEST_F(QoraiTranslateBrowserTest, NoAutoTranslate) {
...
class QoraiTranslateBrowserGoogleRedirectTest
    : public QoraiTranslateBrowserTest {
 public:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    QoraiTranslateBrowserTest::SetUpCommandLine(command_line);
    const std::string host_port = https_server_.host_port_pair().ToString();
...
IN_PROC_BROWSER_TEST_F(QoraiTranslateBrowserGoogleRedirectTest,
  ...
  EXPECT_EQ("https://translate.qorai.com/something.svg",
