...
class QoraiTorBrowserTest : public InProcessBrowserTest {
 public:
  struct TorInfo {
  ...
  QoraiTorBrowserTest() {
    QoraiSettingsUI::ShouldExposeElementsForTesting() = true;
  }

  ~QoraiTorBrowserTest() override {
    QoraiSettingsUI::ShouldExposeElementsForTesting() = false;
  }
  ...
  void WaitForUpdaterThread(qorai_component_updater::QoraiComponent* updater) {
...
IN_PROC_BROWSER_TEST_F(QoraiTorBrowserTest, OpenCloseDisableTorWindow) {
  ...
  // Disable tor, expect executables are removed.
  {
    ...
    WaitForUpdaterThread(g_qorai_browser_process->tor_client_updater());
...
class QoraiTorWithCustomProfileBrowserTest : public QoraiTorBrowserTest {
 private:
...
IN_PROC_BROWSER_TEST_F(QoraiTorWithCustomProfileBrowserTest, PRE_SetupBridges) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
  ...
  EXPECT_TRUE(
      g_qorai_browser_process->tor_pluggable_transport_updater()->IsReady());
  ...
  WaitForUpdaterThread(
      g_qorai_browser_process->tor_pluggable_transport_updater());
}

IN_PROC_BROWSER_TEST_F(QoraiTorWithCustomProfileBrowserTest, SetupBridges) {
...
IN_PROC_BROWSER_TEST_F(QoraiTorWithCustomProfileBrowserTest, Incognito) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
...
IN_PROC_BROWSER_TEST_F(QoraiTorWithCustomProfileBrowserTest, Autofill) {
  GURL fake_url("http://qorai.com/");
  // Disable autofill in private windows.
  browser()->profile()->GetPrefs()->SetBoolean(kQoraiAutofillPrivateWindows,
                                               false);
  auto* tor_profile = OpenTorWindow();
  EXPECT_NE(nullptr, tor_profile);
  EXPECT_TRUE(tor_profile->IsTor());
...
IN_PROC_BROWSER_TEST_F(QoraiTorBrowserTest, PRE_ResetBridges) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
  DownloadTorPluggableTransports();
...
IN_PROC_BROWSER_TEST_F(QoraiTorBrowserTest, ResetBridges) {
...
IN_PROC_BROWSER_TEST_F(QoraiTorBrowserTest, HttpAllowlistIsolation) {
  // Normal window
...
class QoraiTorBrowserTest_EnableTorHttpsOnlyFlag
    : public QoraiTorBrowserTest,
      public ::testing::WithParamInterface<bool> {
 public:
  QoraiTorBrowserTest_EnableTorHttpsOnlyFlag() {
    if (IsQoraiHttpsByDefaultEnabled()) {
      std::vector<base::test::FeatureRef> enabled_features{
          net::features::kQoraiTorWindowsHttpsOnly};
      std::vector<base::test::FeatureRef> disabled_features;
      if (IsQoraiHttpsByDefaultEnabled()) {
        enabled_features.push_back(net::features::kQoraiHttpsByDefault);
      } else {
        disabled_features.push_back(net::features::kQoraiHttpsByDefault);
      }
  ...
  ~QoraiTorBrowserTest_EnableTorHttpsOnlyFlag() override = default;

  bool IsQoraiHttpsByDefaultEnabled() { return GetParam(); }

 protected:
  base::test::ScopedFeatureList scoped_feature_list_;
};

IN_PROC_BROWSER_TEST_P(QoraiTorBrowserTest_EnableTorHttpsOnlyFlag,
                       TorWindowHttpsOnly) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
...
INSTANTIATE_TEST_SUITE_P(QoraiTorBrowserTest_EnableTorHttpsOnlyFlag,
                         QoraiTorBrowserTest_EnableTorHttpsOnlyFlag,
                         ::testing::Bool());

// Test that Tor settings are visible when Tor is disabled by user preference
// but not by admin policy.
IN_PROC_BROWSER_TEST_F(QoraiTorBrowserTest,
  ...
  // Check that qoraiTorDisabledByPolicy is false when disabled by user (not
  // policy)
  std::string script = R"(
    loadTimeData.getBoolean('qoraiTorDisabledByPolicy');
  )";
