...
#include "qorai/components/tor/mock_tor_launcher_factory.h"
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  EXPECT_EQ(qorai_rewards::RewardsServiceFactory::GetForProfile(tor_profile),
            nullptr);
  EXPECT_EQ(qorai_ads::AdsServiceFactory::GetForProfile(tor_profile), nullptr);
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  const GURL qorai_url("https://www.qorai.com");
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
  ...
  ContentSetting setting = GetScriptSetting(tor_content_settings, qorai_url);
  ...
  setting = GetScriptSetting(tor_content_settings, qorai_url);
  ...
  SetScriptSetting(tor_content_settings,
                   ContentSettingsPattern::FromURL(qorai_url),
                   CONTENT_SETTING_BLOCK);
  setting = GetScriptSetting(parent_content_settings, qorai_url);
  EXPECT_EQ(setting, CONTENT_SETTING_ALLOW);
  setting = GetScriptSetting(tor_content_settings, qorai_url);
  EXPECT_EQ(setting, CONTENT_SETTING_BLOCK);
}
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest, NavigateToURL) {
  ...
  for (bool connected : {false, true}) {
    EXPECT_CALL(*GetTorLauncherFactory(), IsTorConnected)
        .WillRepeatedly(testing::Return(connected));
    const GURL url("https://qorai.com");
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest, CanShare) {
  ...
  Browser* tor_browser = SwitchToTorProfile(
      browser()->profile(), GetTorLauncherFactory(), 1, GURL("qorai://newtab"));
  Profile* tor_profile = tor_browser->profile();
  ...
  auto* regular_contents =
      ui_test_utils::NavigateToURL(browser(), GURL("qorai://newtab"));
...
class TorProfileManagerExtensionTest : public extensions::ExtensionBrowserTest {
 public:
  void SetUpOnMainThread() override {
    ...
    base::PathService::Get(qorai::DIR_TEST_DATA, &test_data_dir_);
