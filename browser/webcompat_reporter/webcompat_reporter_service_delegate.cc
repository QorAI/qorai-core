...
#include "qorai/browser/webcompat_reporter/webcompat_reporter_service_delegate.h"
...
WebcompatReporterServiceDelegateImpl::WebcompatReporterServiceDelegateImpl(
    ...
    qorai_shields::AdBlockService* adblock_service,
...
WebcompatReporterServiceDelegateImpl::GetAdblockFilterListNames() const {
  ...
  qorai_shields::AdBlockComponentServiceManager* service_manager =
      adblock_service_->component_service_manager();
  CHECK(service_manager);
  for (const qorai_shields::FilterListCatalogEntry& entry :
...
WebcompatReporterServiceDelegateImpl::GetCookiePolicy(
  ...
  return qorai_shields::ControlTypeToString(qorai_shields::GetCookieControlType(
...
WebcompatReporterServiceDelegateImpl::GetScriptBlockingFlag(
  ...
  return BoolToString(
      qorai_shields::GetNoScriptControlType(host_content_settings_map_,
                                            GURL(current_url.value())) ==
