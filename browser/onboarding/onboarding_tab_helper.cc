...
#include "qorai/browser/onboarding/pref_names.h"
...
void OnboardingTabHelper::DidStopLoading() {
  ...
  PerformQoraiShieldsChecksAndShowHelpBubble();
}

void OnboardingTabHelper::PerformQoraiShieldsChecksAndShowHelpBubble() {
  ...
  if (shields_data_controller->GetQoraiShieldsEnabled() &&
      shields_data_controller->GetTotalBlockedCount() > 0 &&
      CanHighlightQoraiShields()) {
    ShowQoraiHelpBubbleView();
  }
}

bool OnboardingTabHelper::CanHighlightQoraiShields() {
  base::Time last_shields_icon_highlight_time =
      g_browser_process->local_state()->GetTime(
          onboarding::prefs::kLastShieldsIconHighlightTime);
...
void OnboardingTabHelper::ShowQoraiHelpBubbleView() {
  Browser* browser = chrome::FindBrowserWithTab(web_contents());
  DCHECK(browser);
  if (!browser) {
    return;
  ...
  if (!QoraiBrowserWindow::From(browser->window())
           ->ShowQoraiHelpBubbleView(GetTextForOnboardingShieldsBubble())) {
    return;
  }
...
std::string OnboardingTabHelper::GetTextForOnboardingShieldsBubble() {
  ...
  std::string label_text = l10n_util::GetPluralStringFUTF8(
      IDS_QORAI_SHIELDS_ONBOARDING_LABEL_WITHOUT_COMPANIES,
