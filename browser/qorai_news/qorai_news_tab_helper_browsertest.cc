...
namespace {
class WaitForFeedsChanged : public QorAINewsTabHelper::PageFeedsObserver {
 public:
  WaitForFeedsChanged(QorAINewsTabHelper* tab_helper,
...
class WaitForFeedTitle {
 public:
  explicit WaitForFeedTitle(QorAINewsTabHelper* tab_helper)
...
class QorAINewsTabHelperTest : public InProcessBrowserTest {
 public:
  QorAINewsTabHelperTest()
...
IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest, TabHelperIsCreated) {
  EXPECT_NE(nullptr, QorAINewsTabHelper::FromWebContents(contents()));
}

IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest,
                       TabHelperNotifiesObserversWhenFoundFeeds) {
  auto* tab_helper = QorAINewsTabHelper::FromWebContents(contents());
...
IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest, FeedsAreDeduplicated) {
  auto* tab_helper = QorAINewsTabHelper::FromWebContents(contents());
...
IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest, NonExistingFeedsAreRemoved) {
  OptIn();
...
IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest, FeedsAreFoundWhenTheyExist) {
  OptIn();
...
IN_PROC_BROWSER_TEST_F(QorAINewsTabHelperTest, FeedsAreNotFoundWhenNotOptedIn) {
  ASSERT_TRUE(https_server()->Start());
  GURL rss_page_url = https_server()->GetURL("/page_with_rss.html");
