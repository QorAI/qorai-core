...
// accesses resources.
class QoraiSpellingOptionsSubMenuObserverTest : public InProcessBrowserTest {
 public:
  QoraiSpellingOptionsSubMenuObserverTest() {}
  QoraiSpellingOptionsSubMenuObserverTest(
      const QoraiSpellingOptionsSubMenuObserverTest&) = delete;
  QoraiSpellingOptionsSubMenuObserverTest& operator=(
      const QoraiSpellingOptionsSubMenuObserverTest&) = delete;
  ~QoraiSpellingOptionsSubMenuObserverTest() override {}
  ...
  void Reset(bool incognito = false,
             QoraiSpellingOptionsSubMenuObserver::GtestMode gtest_mode =
                 QoraiSpellingOptionsSubMenuObserver::GTEST_MODE_NORMAL) {
    Clear();
    menu_ = std::make_unique<QoraiMockRenderViewContextMenu>(
        incognito ? browser()->profile()->GetPrimaryOTRProfile(
                        /*create_if_needed=*/true)
                  : browser()->profile());
    std::unique_ptr<QoraiSpellingOptionsSubMenuObserver> observer =
        std::make_unique<QoraiSpellingOptionsSubMenuObserver>(menu_.get(),
                                                              menu_.get(), 1);
  ...
  void CheckExpected() {
    for (size_t i = 0; i < menu()->GetMenuSize(); ++i) {
      QoraiMockRenderViewContextMenu::MockMenuItem item;
  ...
  QoraiMockRenderViewContextMenu* menu() { return menu_.get(); }
  SpellingOptionsSubMenuObserver* observer() { return observer_.get(); }

 private:
  std::unique_ptr<QoraiMockRenderViewContextMenu> menu_;
  std::unique_ptr<SpellingOptionsSubMenuObserver> observer_;
};

// Tests that "Ask Qorai for suggestions" isn't shown in the menu and the menu
// doesn't end with a separator.

IN_PROC_BROWSER_TEST_F(QoraiSpellingOptionsSubMenuObserverTest,
                       CheckAskQoraiNotShown) {
  ...
  // Test empty submenu.
  Reset(false, QoraiSpellingOptionsSubMenuObserver::GTEST_MODE_EMPTY_SUBMENU);
  InitMenu(false, "en-US", std::vector<std::string>());
...
IN_PROC_BROWSER_TEST_F(QoraiSpellingOptionsSubMenuObserverTest,
                       CheckAskQoraiNotShownIncognito) {
  // Test with spellcheck enabled.
  Reset(true);
  InitMenu(true, "en-US", std::vector<std::string>(1, "en-US"));
