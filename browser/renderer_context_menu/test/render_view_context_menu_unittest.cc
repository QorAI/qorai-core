...
class QoraiRenderViewContextMenuMock : public QoraiRenderViewContextMenu {
 public:
  using QoraiRenderViewContextMenu::QoraiRenderViewContextMenu;
  ...
  Browser* GetBrowser() const override {
    ...
    return QoraiRenderViewContextMenu::GetBrowser();
  }
...
class QoraiRenderViewContextMenuTest : public testing::Test {
 protected:
  QoraiRenderViewContextMenuTest() = default;
  content::WebContents* GetWebContents() { return web_contents_.get(); }

  // Returns a test context menu.
  std::unique_ptr<QoraiRenderViewContextMenuMock> CreateContextMenu(
      content::WebContents* web_contents,
      content::ContextMenuParams params,
      bool is_pwa_browser = false) {
    ResetBrowser();
    auto menu = std::make_unique<QoraiRenderViewContextMenuMock>(
...
TEST_F(QoraiRenderViewContextMenuTest, MenuForPlainText) {
...
TEST_F(QoraiRenderViewContextMenuTest, MenuForSelectedUrl) {
  content::ContextMenuParams params = CreateSelectedTextParams(u"qorai.com");
  auto context_menu = CreateContextMenu(GetWebContents(), params);
  EXPECT_TRUE(context_menu);
  std::optional<size_t> clean_link_index =
      context_menu->menu_model().GetIndexOfCommandId(IDC_COPY_CLEAN_LINK);
...
TEST_F(QoraiRenderViewContextMenuTest, MenuForLink) {
  content::ContextMenuParams params =
      CreateLinkParams(GURL("https://qorai.com"));
  auto context_menu = CreateContextMenu(GetWebContents(), params);
  EXPECT_TRUE(context_menu);
  std::optional<size_t> clean_link_index =
      context_menu->menu_model().GetIndexOfCommandId(IDC_COPY_CLEAN_LINK);
...
TEST_F(QoraiRenderViewContextMenuTest, MenuForAIChat) {
  content::ContextMenuParams params = CreateSelectedTextParams(u"hello");

  for (auto enabled : {true, false}) {
    GetPrefs()->SetBoolean(ai_chat::prefs::kQoraiAIChatContextMenuEnabled,
                           enabled);
    auto context_menu = CreateContextMenu(GetWebContents(), params);
    EXPECT_TRUE(context_menu);
    std::optional<size_t> ai_chat_index =
        context_menu->menu_model().GetIndexOfCommandId(
            IDC_AI_CHAT_CONTEXT_QORA_TOOLS);
    EXPECT_EQ(ai_chat_index.has_value(), enabled);
    EXPECT_EQ(context_menu->IsCommandIdEnabled(IDC_AI_CHAT_CONTEXT_QORA_TOOLS),
              enabled);
  }
}

TEST_F(QoraiRenderViewContextMenuTest, MenuForAIChat_PWA) {
  content::ContextMenuParams params = CreateSelectedTextParams(u"hello");

  GetPrefs()->SetBoolean(ai_chat::prefs::kQoraiAIChatContextMenuEnabled, true);
