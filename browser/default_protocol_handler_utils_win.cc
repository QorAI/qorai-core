...
std::wstring GetBrowserProgId() {
  base::FilePath qorai_exe;
  if (!base::PathService::Get(base::FILE_EXE, &qorai_exe)) {
    LOG(ERROR) << "Error getting app exe path";
    return std::wstring();
  }

  const auto suffix = ShellUtil::GetCurrentInstallationSuffix(qorai_exe);
  std::wstring qorai_html =
  ...
  if (ShellUtil::GetUserSpecificRegistrySuffix(&new_style_suffix) &&
      suffix == new_style_suffix && qorai_html.length() > 39) {
    NOTREACHED();
  }
  return qorai_html;
}
