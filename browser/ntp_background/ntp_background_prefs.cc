...
NTPBackgroundPrefs::Type StringToType(const std::string& type_string) {
  // See class description for details.
  if (type_string == "qorai") {
    return NTPBackgroundPrefs::Type::kQorai;
  }
...
void NTPBackgroundPrefs::RegisterPref(
  ...
  dict.Set(kTypeKey, TypeToString(Type::kQorai));
...
bool NTPBackgroundPrefs::IsQoraiType() const {
  return GetType() == Type::kQorai;
}
...
bool NTPBackgroundPrefs::ShouldUseRandomValue() const {
  ...
  return optional_bool.value_or(IsQoraiType());
}
