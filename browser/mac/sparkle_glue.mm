...
#import "qorai/browser/mac/su_updater.h"
...
std::string GetUpdateChannel() {
  std::string channel_name = qorai::GetChannelName();
...
- (BOOL)loadSparkleFramework {
  ...
  DUMP_WILL_BE_CHECK(!qorai_updater::ShouldUseOmaha4());
#endif
...
- (void)registerWithSparkle {
  // This can be called by QoraiBrowserMainPartsMac::PreMainMessageLoopStart()
  // again when browser is relaunched.
  if (_registered) {
    return;
  }

  DCHECK(qorai::UpdateEnabled());
  DCHECK(_su_updater);
  ...
  constexpr NSTimeInterval kQoraiUpdateCheckIntervalInSec = 3 * 60 * 60;
  [_su_updater setUpdateCheckInterval:kQoraiUpdateCheckIntervalInSec];
...
- (void)updater:(id)updater didFinishLoadingAppcast:(id)appcast {
  VLOG(0) << "qorai update: did finish loading appcast";

  [self updateStatus:kAutoupdateChecking version:nil error:nil];
}

- (void)updater:(id)updater didFindValidUpdate:(id)item {
  VLOG(0) << "qorai update: did find valid update with " +
...
- (void)updaterDidNotFindUpdate:(id)updater {
  VLOG(0) << "qorai update: did not find update";
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "qorai update: willDownloadUpdate with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater
    failedToDownloadUpdate:(id)item
                     error:(NSError *)error {
  VLOG(0) << "qorai update: failed to download update with " +
...
- (void)userDidCancelDownload:(id)updater {
  VLOG(0) << "qorai update: user did cancel download";
  [self updateStatus:kAutoupdateInstallFailed
               version:nil
                 error:nil];
}
...
- (void)updater:(id)updater willInstallUpdate:(id)item {
  VLOG(0) << "qorai update: will install update with " +
             GetDescriptionFromAppcastItem(item);
  [self updateStatus:kAutoupdateInstalling
             version:nil
               error:nil];
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "qorai update: will install update on quit with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater didAbortWithError:(NSError *)error {
  VLOG(0) << "qorai update: did abort with error: " +
...
- (NSString*)feedURLStringForUpdater:(id)__unused updater {
  ...
  return [NSString stringWithFormat:@"https://updates.qoraisoftware.com/"
                                    @"sparkle/Qorai-Browser/%s/appcast.xml",
