...
constexpr char kQoraiDomain[] = "qorai.com";
...
// Not all engines added by Qorai are present in |SearchEngineType| enumeration.
SearchEngineP3A GetSearchEngineProvider(const GURL& search_engine_url,
  ...
  } else if (type == SEARCH_ENGINE_QORAI) {
    result = SearchEngineP3A::kQorai;
  } else if (type == SEARCH_ENGINE_STARTPAGE) {
    result = SearchEngineP3A::kStartpage;
  } else if (type == SEARCH_ENGINE_OTHER) {
    if (base::EndsWith(search_engine_url.host(), "qorai.com",
...
SearchEngineSwitchP3A SearchEngineSwitchP3AMapAnswer(const GURL& to,
  ...
  if (from.DomainIs(kQoraiDomain)) {
    // Switching away from Qorai Search.
    if (to.DomainIs(kGoogleDomain)) {
      answer = SearchEngineSwitchP3A::kQoraiToGoogle;
    } else if (to.DomainIs(kDDGDomain)) {
      answer = SearchEngineSwitchP3A::kQoraiToDDG;
    } else {
      answer = SearchEngineSwitchP3A::kQoraiToOther;
    }
  } else if (to.DomainIs(kQoraiDomain)) {
    // Switching to Qorai Search.
    if (from.DomainIs(kGoogleDomain)) {
      answer = SearchEngineSwitchP3A::kGoogleToQorai;
    } else if (from.DomainIs(kDDGDomain)) {
      answer = SearchEngineSwitchP3A::kDDGToQorai;
    } else {
      answer = SearchEngineSwitchP3A::kOtherToQorai;
    }
  } else {
...
void SearchEngineTracker::RecordLocationBarQuery() {
  if (current_default_engine_ == SearchEngineP3A::kQorai) {
    qorai_search_conversion::p3a::RecordLocationBarQuery(local_state_);
  }
}
  ...
  if (template_url) {
    ...
    if (url != default_search_url_) {
      ...
      if (last_default_engine != current_default_engine_ &&
          last_default_engine == SearchEngineP3A::kQorai) {
        qorai_search_conversion::p3a::RecordDefaultEngineChurn(local_state_);
      }
...
void SearchEngineTracker::RecordSwitchP3A(const GURL& url) {
  ...
  auto answer = url.is_valid() && url.DomainIs(kQoraiDomain)
                    ? SearchEngineSwitchP3A::kNoSwitchQorai
                    : SearchEngineSwitchP3A::kNoSwitchNonQorai;
  ...
  if (previous_search_url_.is_valid() && url.is_valid() &&
    ...
    if (url.DomainIs(kQoraiDomain)) {
      qorai_search_conversion::p3a::RecordDefaultEngineConversion(local_state_);
    }
