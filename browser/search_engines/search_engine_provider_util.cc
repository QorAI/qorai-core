...
void SetQoraiAsDefaultPrivateSearchProvider(Profile& profile) {
...
void UpdateDefaultPrivateSearchProviderData(Profile& profile) {
  ...
  if (private_provider_guid.empty()) {
    ...
    // In this case, set qorai as a default search provider.
    SetQoraiAsDefaultPrivateSearchProvider(profile);
    return;
  }
...
void PrepareDefaultPrivateSearchProviderDataIfNeeded(Profile& profile) {
  ...
  // Set Qorai as a private window's initial search provider.
  if (private_provider_guid.empty()) {
    SetQoraiAsDefaultPrivateSearchProvider(profile);
    return;
  }
  ...
  if (preference->IsDefaultValue()) {
    if (auto* url = service->GetTemplateURLForGUID(private_provider_guid)) {
      ...
      // This could happen with update default provider list when qorai is not
      // updated for longtime. So it doesn't have any chance to cache url data.
      // Set Qorai as default private search provider.
      SetQoraiAsDefaultPrivateSearchProvider(profile);
    }
    return;
  }
