...
std::string GetQoraiSearchProviderSyncGUID(Profile* profile) {
  CHECK(profile);
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  // Check Qorai Search is default provider for private window.
  ...
  QoraiProfileResetter resetter(profile);
...
class SearchSuggestionsEnabledTest : public InProcessBrowserTest,
 ...
 public:
  ...
  const qorai_l10n::test::ScopedDefaultLocale default_locale;
};
...
IN_PROC_BROWSER_TEST_P(SearchSuggestionsEnabledTest,
  ...
  service->SetUserSelectedDefaultSearchProvider(&qorai_template_url);
  EXPECT_EQ(IsSearchSuggestionsEnabled(),
            prefs->GetBoolean(prefs::kSearchSuggestEnabled));
}
...
class MigrateSearchEnginePrefsInJPTest : public InProcessBrowserTest {
 ...
 private:
  const qorai_l10n::test::ScopedDefaultLocale default_locale{"ja_JP"};
};
...
IN_PROC_BROWSER_TEST_F(MigrateSearchEnginePrefsInJPTest,
  ...
  prefs()->SetInteger(prefs::kQoraiDefaultSearchVersion, 31);
...
IN_PROC_BROWSER_TEST_F(MigrateSearchEnginePrefsInJPTest,
  ...
  // version number.
  prefs()->SetInteger(prefs::kQoraiDefaultSearchVersion, 30);
...
IN_PROC_BROWSER_TEST_F(ExtensionBrowserTest,
  ...
  // Check Qorai Search is back to as a default provider for private window
