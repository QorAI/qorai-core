...
class TestingQoraiProfileManager : public QoraiProfileManagerWithoutInit {
 public:
  explicit TestingQoraiProfileManager(const base::FilePath& user_data_dir)
      : QoraiProfileManagerWithoutInit(user_data_dir) {}

  ~TestingQoraiProfileManager() override = default;
...
class QoraiProfileManagerTest : public testing::Test {
 public:
  QoraiProfileManagerTest() = default;
 ...
 protected:
  std::unique_ptr<QoraiProfileManager> CreateProfileManagerForTest() {
    return std::make_unique<TestingQoraiProfileManager>(temp_dir_.GetPath());
  }
...
TEST_F(QoraiProfileManagerTest, EnableMediaRouterOnRestartDefaultValue) {
...
TEST_F(QoraiProfileManagerTest, EnableMediaRouterOnRestartNonDefaultValue) {
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
...
class QoraiProfileManagerAIAgentProfileTest
    : public QoraiProfileManagerTest,
      public testing::WithParamInterface<bool> {
 protected:
  void SetUp() override {
    QoraiProfileManagerTest::SetUp();
  ...
  void TearDown() override {
    QoraiProfileManagerTest::TearDown();
  ...
  Profile* CreateAIChatAgentProfile() {
    ...
    base::FilePath path = GetTempPath().Append(qorai::kAIChatAgentProfileDir);
...
TEST_P(QoraiProfileManagerAIAgentProfileTest, SetProfileAsLastUsed) {
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
...
TEST_P(QoraiProfileManagerAIAgentProfileTest, GetNumberOfProfiles) {
  QoraiProfileManager* profile_manager =
      static_cast<QoraiProfileManager*>(g_browser_process->profile_manager());
  ASSERT_TRUE(profile_manager);
...
TEST_P(QoraiProfileManagerAIAgentProfileTest, GetLastOpenedProfiles) {
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
...
INSTANTIATE_TEST_SUITE_P(FeatureEnabledAndDisabled,
                         QoraiProfileManagerAIAgentProfileTest,
                         testing::Bool(),
...
#endif  // BUILDFLAG(ENABLE_QORAI_AI_CHAT_AGENT_PROFILE)
