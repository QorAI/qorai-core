...
class QoraiProfileManagerTest : public PlatformBrowserTest {
};
...
#if !BUILDFLAG(IS_ANDROID)
IN_PROC_BROWSER_TEST_F(QoraiProfileManagerTest,
  ...
  EXPECT_EQ(qorai_rewards::RewardsServiceFactory::GetForProfile(guest_profile),
            nullptr);
  EXPECT_EQ(qorai_ads::AdsServiceFactory::GetForProfile(guest_profile),
            nullptr);
  ...
  EXPECT_NE(qorai_rewards::RewardsServiceFactory::GetForProfile(profile),
            nullptr);
  EXPECT_EQ(qorai_rewards::RewardsServiceFactory::GetForProfile(otr_profile),
            nullptr);

  EXPECT_NE(qorai_ads::AdsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(qorai_ads::AdsServiceFactory::GetForProfile(otr_profile), nullptr);
}
#endif
...
IN_PROC_BROWSER_TEST_F(QoraiProfileManagerTest, MediaRouterEnabledRestartTest) {
  Profile* profile = g_browser_process->profile_manager()->GetLastUsedProfile();
  {
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(::prefs::kEnableMediaRouter));
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(kEnableMediaRouterOnRestart));
