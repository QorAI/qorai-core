...
IN_PROC_BROWSER_TEST_F(DeleteProfileHelperBrowserTest,
  ...
  EXPECT_TRUE(qorai_sync_service->SetSyncCode(kValidSyncCode));

  qorai_sync::Prefs qorai_sync_prefs(profile_to_delete.GetPrefs());
  ...
  // Ensure that we've invoked QoraiSyncServiceImpl::StopAndClear() from
  // DisableSyncForProfileDeletion at delete_profile_helper.cc, so
  // now seed should be empty
  seed = qorai_sync_prefs.GetSeed(&failed_to_decrypt);
  ASSERT_FALSE(failed_to_decrypt);
