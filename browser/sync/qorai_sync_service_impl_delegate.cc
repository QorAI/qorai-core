...
QoraiSyncServiceImplDelegate::QoraiSyncServiceImplDelegate(
...
QoraiSyncServiceImplDelegate::~QoraiSyncServiceImplDelegate() = default;

void QoraiSyncServiceImplDelegate::OnDeviceInfoChange() {
  ...
  if (!found_local_device) {
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE,
        base::BindOnce(&QoraiSyncServiceImplDelegate::OnSelfDeviceInfoDeleted,
                       weak_ptr_factory_.GetWeakPtr()));
  }
}

void QoraiSyncServiceImplDelegate::OnSelfDeviceInfoDeleted() {
  sync_service_impl_->OnSelfDeviceInfoDeleted(base::DoNothing());
}

void QoraiSyncServiceImplDelegate::SuspendDeviceObserverForOwnReset() {
  device_info_observer_.Reset();
}

void QoraiSyncServiceImplDelegate::ResumeDeviceObserver() {
  if (!device_info_observer_.IsObserving()) {
    device_info_observer_.Observe(device_info_tracker_);
  }
}

void QoraiSyncServiceImplDelegate::RecordP3ASyncStatus() {
  ...
  base::UmaHistogramExactLinear("Qorai.Sync.Status.2", p3a_value, 3);
}

void QoraiSyncServiceImplDelegate::SetLocalDeviceAppearedCallback(
...
void QoraiSyncServiceImplDelegate::GetKnownToSyncHistoryCount(
