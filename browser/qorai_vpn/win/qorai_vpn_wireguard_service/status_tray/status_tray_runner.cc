...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/status_tray_runner.h"
...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/resources/resource.h"
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/status_icon/icon_utils.h"
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/status_icon/status_icon.h"
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/status_icon/status_tray.h"
...
std::u16string GetVpnStatusLabel(bool active) {
  return l10n_util::GetStringUTF16(
      active ? IDS_QORAI_VPN_WIREGUARD_TRAY_STATUS_ITEM_ACTIVE
             : IDS_QORAI_VPN_WIREGUARD_TRAY_STATUS_ITEM_INACTIVE);
}

int GetStatusIconTooltip(qorai_vpn::mojom::ConnectionState state) {
  switch (state) {
    case qorai_vpn::mojom::ConnectionState::CONNECTING:
      return IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTING;
    case qorai_vpn::mojom::ConnectionState::CONNECTED:
      return IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTED;
    case qorai_vpn::mojom::ConnectionState::DISCONNECTING:
      return IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTING;
    case qorai_vpn::mojom::ConnectionState::DISCONNECTED:
      return IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTED;
    case qorai_vpn::mojom::ConnectionState::CONNECT_FAILED:
    case qorai_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED:
      return IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_ERROR;
  }

  NOTREACHED();
}

int GetStatusTrayIcon(qorai_vpn::mojom::ConnectionState state) {
  bool dark_theme =
      ui::NativeTheme::GetInstanceForNativeUi()->preferred_color_scheme() ==
      ui::NativeTheme::PreferredColorScheme::kDark;
  switch (state) {
    case qorai_vpn::mojom::ConnectionState::CONNECTED:
      return dark_theme ? IDR_QORAI_VPN_TRAY_LIGHT_CONNECTED
                        : IDR_QORAI_VPN_TRAY_DARK_CONNECTED;
    case qorai_vpn::mojom::ConnectionState::DISCONNECTING:
    case qorai_vpn::mojom::ConnectionState::DISCONNECTED:
      return dark_theme ? IDR_QORAI_VPN_TRAY_LIGHT : IDR_QORAI_VPN_TRAY_DARK;
    case qorai_vpn::mojom::ConnectionState::CONNECTING:
      return dark_theme ? IDR_QORAI_VPN_TRAY_LIGHT_CONNECTING
                        : IDR_QORAI_VPN_TRAY_DARK_CONNECTING;
    case qorai_vpn::mojom::ConnectionState::CONNECT_FAILED:
    case qorai_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED:
      return dark_theme ? IDR_QORAI_VPN_TRAY_LIGHT_ERROR
                        : IDR_QORAI_VPN_TRAY_DARK_ERROR;
  }
...
bool StatusTrayRunner::IsVPNConnected() const {
  ...
  return IsWireguardActive()
             ? wireguard::IsQorAIVPNWireguardTunnelServiceRunning()
             : ras::IsRasConnected();
}

void StatusTrayRunner::ConnectVPN() {
  if (IsWireguardActive()) {
    wireguard::EnableQorAIVpnWireguardService(
        // passing empty params will reconnect using last known good config.
        // TODO(https://github.com/qorai/qorai-browser/issues/47115): fetch
...
void StatusTrayRunner::DisconnectVPN() {
  if (IsWireguardActive()) {
    wireguard::DisableQorAIVpnWireguardService(base::BindOnce(
...
void StatusTrayRunner::SetupStatusIcon() {
  ...
  current_state_ = IsVPNConnected()
                       ...
                       : qorai_vpn::mojom::ConnectionState::DISCONNECTED;
...
void StatusTrayRunner::ExecuteCommand(int command_id, int event_flags) {
  switch (command_id) {
    case IDC_QORAI_VPN_TRAY_EXIT:
      SignalExit();
      break;
    case IDC_QORAI_VPN_TRAY_HIDE_ICON:
      EnableVPNTrayIcon(false);
      SignalExit();
      break;
    case IDC_QORAI_VPN_TRAY_CONNECT_VPN_ITEM:
      ConnectVPN();
      break;
    case IDC_QORAI_VPN_TRAY_DISCONNECT_VPN_ITEM:
      DisconnectVPN();
      break;
    case IDC_QORAI_VPN_TRAY_MANAGE_ACCOUNT_ITEM:
      OpenURLInBrowser(kManageUrlProd);
      break;
    case IDC_QORAI_VPN_TRAY_ABOUT_ITEM:
...
void StatusTrayRunner::OnMenuWillShow(ui::SimpleMenuModel* source) {
  ...
  source->Clear();
  source->AddItem(IDC_QORAI_VPN_TRAY_STATUS_ITEM, GetVpnStatusLabel(connected));
  source->SetEnabledAt(0, false);
  if (connected) {
    source->AddItem(IDC_QORAI_VPN_TRAY_DISCONNECT_VPN_ITEM,
                    l10n_util::GetStringUTF16(
                        IDS_QORAI_VPN_WIREGUARD_TRAY_DISCONNECT_ITEM));
  } else {
    source->AddItem(
        IDC_QORAI_VPN_TRAY_CONNECT_VPN_ITEM,
        l10n_util::GetStringUTF16(IDS_QORAI_VPN_WIREGUARD_TRAY_CONNECT_ITEM));
  }
  source->AddSeparator(ui::NORMAL_SEPARATOR);
  source->AddItem(IDC_QORAI_VPN_TRAY_MANAGE_ACCOUNT_ITEM,
                  l10n_util::GetStringUTF16(
                      IDS_QORAI_VPN_WIREGUARD_TRAY_MANAGE_ACCOUNT_ITEM));
  source->AddItem(
      IDC_QORAI_VPN_TRAY_ABOUT_ITEM,
      l10n_util::GetStringUTF16(IDS_QORAI_VPN_WIREGUARD_TRAY_ABOUT_ITEM));
  source->AddSeparator(ui::NORMAL_SEPARATOR);
  source->AddItem(
      IDC_QORAI_VPN_TRAY_HIDE_ICON,
      l10n_util::GetStringUTF16(IDS_QORAI_VPN_WIREGUARD_TRAY_REMOVE_ICON_ITEM));
}
...
qorai_vpn::mojom::ConnectionState StatusTrayRunner::GetConnectionState() {
  if (IsVPNConnected()) {
    return qorai_vpn::mojom::ConnectionState::CONNECTED;
  }

  auto state_from_storage = qorai_vpn::GetConnectionState();
  if (state_from_storage.has_value()) {
    return static_cast<qorai_vpn::mojom::ConnectionState>(
        state_from_storage.value());
  }

  return qorai_vpn::mojom::ConnectionState::DISCONNECTED;
}
...
void StatusTrayRunner::UpdateConnectionState() {
  ...
  bool should_skip_connection_attempt =
      (current_state_ == qorai_vpn::mojom::ConnectionState::CONNECT_FAILED &&
       (state == qorai_vpn::mojom::ConnectionState::CONNECTING ||
        state == qorai_vpn::mojom::ConnectionState::DISCONNECTING));
...
void StatusTrayRunner::OnWireguardServiceStateChanged(int mask) {
  UpdateConnectionState();
  auto service_name = IsVPNConnected() ? GetQorAIVpnWireguardTunnelServiceName()
                                       : GetQorAIVpnWireguardServiceName();
  if (!qorai_vpn::IsWindowsServiceRunning(service_name)) {
...
void StatusTrayRunner::OnStorageUpdated() {
  // Checking if tray icon enabled from QorAI-> App menu -> QorAI VPN -> Show
  // VPN tray icon.
...
void StatusTrayRunner::SetupConnectionObserver() {
  if (IsWireguardActive()) {
    ...
    SubscribeForWireguardNotifications(
        IsVPNConnected() ? GetQorAIVpnWireguardTunnelServiceName()
                         : GetQorAIVpnWireguardServiceName());
    return;
  }
...
HRESULT StatusTrayRunner::Run() {
  ...
  if (qorai_vpn::IsQorAIVpnTrayIconRunning()) {
  ...
  base::ThreadPoolInstance::CreateAndStartWithDefaultParams(
      "QorAI VPN Wireguard status tray process");
