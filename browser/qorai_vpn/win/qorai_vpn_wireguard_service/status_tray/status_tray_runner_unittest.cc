...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/status_tray/status_icon/tray_menu_model.h"
 ...
 public:
  void CheckConnectedMenuState(TrayMenuModel* menu_model) {
    ...
    EXPECT_EQ(menu_model->GetCommandIdAt(1),
              IDC_QORAI_VPN_TRAY_DISCONNECT_VPN_ITEM);
    EXPECT_FALSE(
        menu_model->GetIndexOfCommandId(IDC_QORAI_VPN_TRAY_CONNECT_VPN_ITEM));
  }
...
TEST_F(StatusTrayRunnerTest, UpdateConnectionState) {
  ...
  WaitIconStateChangedTo(
      IDR_QORAI_VPN_TRAY_LIGHT,
      IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTED);
  EXPECT_FALSE(qorai_vpn::GetConnectionState().has_value());

  // Tunnel service stopped, registry state as "connecting"
  WriteConnectionState(
      static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECTING));
  WaitIconStateChangedTo(IDR_QORAI_VPN_TRAY_LIGHT_CONNECTING,
                         IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTING);
  EXPECT_EQ(qorai_vpn::GetConnectionState().value(),
            static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECTING));

  // Tunnel service stopped, registry state as "connected"
  WriteConnectionState(
      static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECTED));
  EXPECT_EQ(qorai_vpn::GetConnectionState().value(),
            static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECTED));
  ...
  EXPECT_EQ(qorai_vpn::GetConnectionState().value(),
            static_cast<int>(qorai_vpn::mojom::ConnectionState::DISCONNECTED));

  // Tunnel service stopped, registry state as "disconnecting"
  WriteConnectionState(
      static_cast<int>(qorai_vpn::mojom::ConnectionState::DISCONNECTING));
  WaitIconStateChangedTo(
      IDR_QORAI_VPN_TRAY_LIGHT,
      IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTING);
  EXPECT_EQ(qorai_vpn::GetConnectionState().value(),
            static_cast<int>(qorai_vpn::mojom::ConnectionState::DISCONNECTING));

  // Tunnel service stopped, registry state as "CONNECT_NOT_ALLOWED"
  WriteConnectionState(
      static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED));
  WaitIconStateChangedTo(IDR_QORAI_VPN_TRAY_LIGHT_ERROR,
                         IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_ERROR);
  EXPECT_EQ(
      qorai_vpn::GetConnectionState().value(),
      static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED));

  // Tunnel service stopped, registry state as "CONNECT_FAILED"
  WriteConnectionState(
      static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECT_FAILED));
  WaitIconStateChangedTo(IDR_QORAI_VPN_TRAY_LIGHT_ERROR,
                         IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_ERROR);
  EXPECT_EQ(
      qorai_vpn::GetConnectionState().value(),
  ...
  StatusTrayRunner::GetInstance()->SetVPNConnectedForTesting(true);
  WaitIconStateChangedTo(IDR_QORAI_VPN_TRAY_LIGHT_CONNECTED,
                         IDS_QORAI_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTED);
  EXPECT_EQ(qorai_vpn::GetConnectionState().value(),
            static_cast<int>(qorai_vpn::mojom::ConnectionState::CONNECTED));
}
