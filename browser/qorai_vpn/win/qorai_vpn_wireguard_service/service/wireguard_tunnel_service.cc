...
#include "qorai/browser/qorai_vpn/win/service_commands.h"
...
constexpr wchar_t kQorAIWireguardConfig[] = L"wireguard";
constexpr wchar_t kQorAIWireguardConfigExtension[] = L".qorai.conf";
...
std::wstring GetWireguardConfigName(const std::wstring& prefix) {
  return prefix + std::wstring(kQorAIWireguardConfigExtension);
}
...
std::optional<base::FilePath> WriteConfigToFile(const std::string& config) {
  ...
  if (!scoped_temp_dir.Set(temp_dir_path.Append(base::FilePath(L"QorAIVpn")))) {
    return std::nullopt;
  }

  base::FilePath temp_file_path(scoped_temp_dir.GetPath().Append(
      GetWireguardConfigName(kQorAIWireguardConfig)));
...
bool LaunchWireguardService(const std::wstring& config) {
  ...
  if (!RemoveExistingWireguardService()) {
    VLOG(1) << "Failed to remove existing qorai wireguard service";
    return false;
  }
  return CreateAndRunQorAIWireguardService(config);
}
...
bool RemoveExistingWireguardService() {
  ...
  if (!scm.IsValid()) {
    VLOG(1) << "::OpenSCManager failed. service_name: "
            << GetQorAIVpnWireguardTunnelServiceName()
            << ", error: " << std::hex << HRESULTFromLastError();
    return false;
  }
  ScopedScHandle service(
      ::OpenService(scm.Get(), GetQorAIVpnWireguardTunnelServiceName().c_str(),
                    SERVICE_ALL_ACCESS));
  ...
  if (service.IsValid()) {
    if (IsServiceRunning(service.Get())) {
      ...
      qorai_vpn::RunWireGuardCommandForUsers(
          qorai_vpn::kQorAIVpnWireguardServiceNotifyDisconnectedSwitchName);
    }
...
bool CreateAndRunQorAIWireguardService(const std::wstring& encoded_config) {
  ...
  base::CommandLine service_cmd(
      directory.Append(qorai_vpn::kQorAIVpnWireguardServiceExecutable));
  service_cmd.AppendSwitchPath(
      qorai_vpn::kQorAIVpnWireguardServiceConnectSwitchName,
      config_file_path.value());
  ScopedScHandle service(::CreateService(
      scm.Get(), GetQorAIVpnWireguardTunnelServiceName().c_str(),
      GetQorAIVpnWireguardTunnelServiceName().c_str(), SERVICE_ALL_ACCESS,
      SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL,
      service_cmd.GetCommandLineString().c_str(), NULL, NULL, L"Nsi\0TcpIp\0",
      NULL, NULL));
  if (!service.IsValid()) {
    VLOG(1) << "::CreateService failed. service_name: "
            << GetQorAIVpnWireguardTunnelServiceName() << ", error: 0x"
            << ::GetLastError();
    return false;
  }
...
int RunWireguardTunnelService(const base::FilePath& config_file_path) {
...

  {
    ...
    qorai_vpn::RunWireGuardCommandForUsers(
        qorai_vpn::kQorAIVpnWireguardServiceNotifyConnectedSwitchName);
