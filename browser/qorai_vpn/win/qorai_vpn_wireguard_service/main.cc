...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/notifications/notification_utils.h"
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/resources/resource_loader.h"
#include "qorai/browser/qorai_vpn/win/qorai_vpn_wireguard_service/service/wireguard_service_runner.h"
...
std::optional<int> ProcessUserLevelCommands(
  ...
  if (command_line.HasSwitch(
    ...
    return qorai_vpn::StatusTrayRunner::GetInstance()->Run();
  }
  ...
  if (command_line.HasSwitch(
    ...
    qorai_vpn::ShowDesktopNotification(
        base::UTF16ToWide(l10n_util::GetStringUTF16(
            IDS_QORAI_VPN_WIREGUARD_TRAY_NOTIFICATION_CONNECTED)));
    return 0;
  }
  ...
  if (command_line.HasSwitch(
    ...
    qorai_vpn::ShowDesktopNotification(
        base::UTF16ToWide(l10n_util::GetStringUTF16(
            IDS_QORAI_VPN_WIREGUARD_TRAY_NOTIFICATION_DISCONNECTED)));
    return 0;
  }
  return std::nullopt;
}
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prev, wchar_t*, int) {
  ...
  if (!process_type.empty()) {
    QorAIWireguardCrashReporterClient::InitializeCrashReportingForProcess(
        process_type);
  }
  ...
  if (command_line->HasSwitch(
    ...
    return qorai_vpn::wireguard::RunWireguardTunnelService(
  ...
  // Runs QorAIVpnWireguardService, called by system SCM.
  return qorai_vpn::WireguardServiceRunner::GetInstance()->RunAsService();
}
