...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_helper/vpn_utils.h"
...
VpnDnsHandler::VpnDnsHandler(QorAIVpnDnsDelegate* delegate)
    : delegate_(delegate) {
  DCHECK(delegate_);
}
...
ras::CheckConnectionResult VpnDnsHandler::GetVpnEntryStatus() {
  ...
  return ras::CheckConnection(GetQorAIVPNConnectionName());
}
...
void VpnDnsHandler::DisconnectVPN() {
  ...
  auto result = ras::DisconnectEntry(GetQorAIVPNConnectionName());
  if (!result.success) {
...
void VpnDnsHandler::UpdateFiltersState() {
  ...
  switch (GetVpnEntryStatus()) {
    case ras::CheckConnectionResult::CONNECTED:
      VLOG(1) << "QorAIVPN connected, set filters";
      if (IsActive()) {
        VLOG(1) << "Filters are already installed";
        return;
      }
      if (!SetFilters(GetQorAIVPNConnectionName())) {
    ...
    case ras::CheckConnectionResult::DISCONNECTED:
      VLOG(1) << "QorAIVPN Disconnected, remove filters";
      if (!RemoveFilters(GetQorAIVPNConnectionName())) {
        VLOG(1) << "Failed to remove DNS filters";
        Exit();
        break;
      }
      ...
      break;
    default:
      VLOG(1) << "QorAIVPN is connecting, try later after "
