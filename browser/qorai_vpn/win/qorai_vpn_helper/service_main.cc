...
#include "qorai/browser/qorai_vpn/win/qorai_vpn_helper/service_main.h"
...
int ServiceMain::RunAsService() {
  const std::wstring& service_name(qorai_vpn::GetQorAIVpnHelperServiceName());
...
void ServiceMain::ServiceMainImpl() {
  VLOG(1) << __func__ << " QorAIVPN Service started";
  service_status_handle_ = ::RegisterServiceCtrlHandler(
      qorai_vpn::GetQorAIVpnHelperServiceName().c_str(),
...
HRESULT ServiceMain::Run() {
  ...
  if (command_line && command_line->HasSwitch(kQorAIVpnHelperCrashMe)) {
    CHECK(!command_line->HasSwitch(kQorAIVpnHelperCrashMe))
        << "--crash-me was used.";
  }
