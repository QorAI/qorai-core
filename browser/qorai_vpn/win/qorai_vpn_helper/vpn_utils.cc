...
std::wstring GetQorAIVpnServiceFilterName() {
  switch (chrome::GetChannel()) {
    case version_info::Channel::CANARY:
      return L"QorAI VPN Nightly Service DNS Filter";
    case version_info::Channel::DEV:
      return L"QorAI VPN Dev Service DNS Filter";
    case version_info::Channel::BETA:
      return L"QorAI VPN Beta Service DNS Filter";
    case version_info::Channel::STABLE:
      return L"QorAI VPN Service DNS Filter";
    case version_info::Channel::UNKNOWN:
      return L"QorAI VPN Development Service DNS Filter";
  }
...
DWORD AddSublayer(GUID uuid) {
  ...
  if (result == ERROR_SUCCESS) {
    std::wstring name(GetQorAIVpnServiceFilterName());
...
DWORD RegisterSublayer(HANDLE engine_handle, GUID uuid) {
  ...
  if (result != (DWORD)FWP_E_ALREADY_EXISTS && result != ERROR_SUCCESS) {
    VLOG(1) << "Failed to add a persistent sublayer with "
               "QORAIVPN_DNS_SUBLAYER UUID";
    return result;
  }
  VLOG(1) << "Added a persistent sublayer with QORAIVPN_DNS_SUBLAYER UUID";
  return ERROR_SUCCESS;
}
...
void SetFiltersInstalledFlag() {
  base::win::RegKey key(HKEY_LOCAL_MACHINE,
                        GetQorAIVpnHelperRegistryStoragePath().c_str(),
                        KEY_ALL_ACCESS);
  if (!key.Valid()) {
  ...
  DWORD launch = 1;
  key.WriteValue(kQorAIVpnHelperFiltersInstalledValue, launch);
}
...
void ResetFiltersInstalledFlag() {
  ...
  key.DeleteValue(kQorAIVpnHelperFiltersInstalledValue);
}
