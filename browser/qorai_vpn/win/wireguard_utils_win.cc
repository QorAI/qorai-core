...
// TODO(https://github.com/qorai/qorai-browser/issues/47115): remove this value
  ...
  if (rv == ERROR_SUCCESS && key.Valid()) {
    ...
    if (key.ReadValue(kAutoConfigURL, &current_proxy_url) == ERROR_SUCCESS) {
      ...
      VLOG(1) << L"User has `" << kAutoConfigURL << "` value already set: \""
              << current_proxy_url
              << "\".\nQorAI VPN smart proxy routing can't be used.";
    } else {
      ...
      VLOG(1) << L"Set QorAI VPN smart proxy routing to = \""
              << current_proxy_url << "\"";
    }
  }
}
...
bool IsWireguardServiceInstalled() {
  ...
  return qorai_vpn::GetWindowsServiceStatus(
...
bool IsQorAIVPNWireguardTunnelServiceRunning() {
  auto status =
      GetWindowsServiceStatus(GetQorAIVpnWireguardTunnelServiceName());
  if (!status.has_value()) {
    return false;
  }
...
bool EnableQorAIVpnWireguardServiceImpl(
  ...
  Microsoft::WRL::ComPtr<IQorAIVpnWireguardManager> service;
  if (FAILED(CoCreateInstance(qorai_vpn::GetQorAIVpnWireguardServiceClsid(),
                              nullptr, CLSCTX_LOCAL_SERVER,
                              qorai_vpn::GetQorAIVpnWireguardServiceIid(),
                              IID_PPV_ARGS_Helper(&service)))) {
    VLOG(1) << "Unable to create IQorAIVpnWireguardManager instance";
    return false;
  }
...
void EnableQorAIVpnWireguardService(const std::string& server_public_key,
  ...
  base::ThreadPool::CreateCOMSTATaskRunner(
      ...
      ->PostTaskAndReplyWithResult(
          FROM_HERE,
          base::BindOnce(&EnableQorAIVpnWireguardServiceImpl, server_public_key,
                         client_private_key, mapped_ip4_address,
                         vpn_server_hostname),
          std::move(callback));
}

bool DisableQorAIVpnWireguardServiceImpl() {
  base::win::AssertComInitialized();
  ...
  if (FAILED(CoCreateInstance(qorai_vpn::GetQorAIVpnWireguardServiceClsid(),
                              ...
                              qorai_vpn::GetQorAIVpnWireguardServiceIid(),
                              IID_PPV_ARGS_Helper(&service)))) {
    VLOG(1) << "Unable to create IQorAIVpnService instance";
    return false;
  }
...
void DisableQorAIVpnWireguardService(wireguard::BooleanCallback callback) {
  base::ThreadPool::CreateCOMSTATaskRunner(
      {base::MayBlock(), base::WithBaseSyncPrimitives(),
       base::TaskPriority::BEST_EFFORT,
       base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN},
      base::SingleThreadTaskRunnerThreadMode::DEDICATED)
      ->PostTaskAndReplyWithResult(
          FROM_HERE, base::BindOnce(&DisableQorAIVpnWireguardServiceImpl),
          std::move(callback));
}

void ShowQorAIVpnStatusTrayIcon() {
  auto executable_path = qorai_vpn::GetQorAIVPNWireguardServiceExecutablePath(
