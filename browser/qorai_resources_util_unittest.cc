...
TEST(QoraiResourcesUtil, CheckIds) {
  const struct {
    const char* name;
    int id;
  } kCases[] = {
    // IDRs from qorai/app/theme/qorai_theme_resources.grd should be valid.
...
#if BUILDFLAG(IS_WIN)
TEST(QoraiResourcesWinTest, CheckStringsForInstaller) {
  // Test whether strings for installer are filled.
  // This test is added because these strings are initially empty and filled
  // with qorai's one.
  // qorai::InitializeResourceBundle can only be called once per unittest
