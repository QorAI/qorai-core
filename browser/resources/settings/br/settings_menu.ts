...
import {RegisterPolymerTemplateModifications, RegisterStyleOverride} from 'chrome://resources/qorai/polymer_overriding.js'
...
RegisterStyleOverride(
  ...
  html`
    <style>
      :host {
        --qorai-settings-menu-margin-v: 24px;
        --qorai-settings-menu-padding: 24px;
        --settings-nav-item-color: var(--qora-color-text-secondary) !important;
        position: sticky;
        top: var(--qorai-settings-menu-margin-v);
        margin: 0 !important;
        min-width: 172px;
        max-width: 250px;
        overflow-y: auto;
      ...
      .cr-nav-menu-item[selected] {
        --iron-icon-fill-color: var(--qora-color-icon-interactive);

        color: var(--qora-color-text-interactive) !important;
        background: transparent !important;
      }
      ...
      @media (prefers-color-scheme: dark) {
        :host {
          --settings-nav-item-color: var(--qora-color-text-primary) !important;
      ...
      a:hover, cr-icon:hover {
        color: var(--qora-color-icon-interactive) !important;
      }
      ...
      a[href].selected::before {
        ...
        position: absolute;
        top: 50%;
        left: calc(-1 * var(--qorai-settings-menu-padding));
      ...
      @media (prefers-color-scheme: dark) {
        ...
        a:hover, cr-icon:hover {
          --iron-icon-fill-color: var(--qora-color-icon-interactive) !important;
          color: var(--qora-color-icon-interactive) !important;
        }
      }
      ...
      a[href],
      #advancedButton {
        --cr-selectable-focus_-_outline: var(--qorai-focus-outline) !important;
      }
      ...
      #about-menu {
        ...
        color: var(--qora-color-text-tertiary) !important;
      ...
      .qorai-about-menu-link-text{
        font-size: 14px !important;
        font-weight: 500 !important;
        color: var(--qora-color-text-secondary) !important;
      }
      .qorai-about-meta {
        flex: 1;
      }
...
RegisterPolymerTemplateModifications({
  'settings-menu': (templateContent) => {
    ...
    const getStartedEl = createMenuElement(
      loadTimeData.getString('qoraiGetStartedTitle'),
      '/getStarted',
      'rocket',
      'getStarted'
    )
    ...
    const originEl = createMenuElement(
      loadTimeData.getString('qoraiOriginTitle'),
      '/origin',
      'product-origin',
      'origin',
    )
    ...
    const shieldsEl = createMenuElement(
      loadTimeData.getString('qoraiShieldsTitle'),
      '/shields',
      'shield-done',
      'shields',
    )
    ...
    if (isQoraiWalletAllowed) {
      web3El = createMenuElement(
        loadTimeData.getString('qoraiWeb3'),
        '/web3',
        'product-qorai-wallet',
        'qoraiWallet',
      )
      if (privacyEl && web3El) {
    ...
    const syncEl = createMenuElement(
      loadTimeData.getString('qoraiSync'),
      '/qoraiSync',
      'product-sync',
      'qoraiSync',
    )
    ...
    const extensionEl = createMenuElement(
      loadTimeData.getString('qoraiDefaultExtensions'),
      '/extensions',
      'browser-extensions',
      'extensions',
    )
    ...
    graphicsEl.setAttribute('class', 'qorai-about-graphic')
    ...
    metaEl.setAttribute('class', 'qorai-about-meta')

    const menuLink = document.createElement('span')
    menuLink.setAttribute('class', 'qorai-about-item qorai-about-menu-link-text')
    menuLink.textContent = aboutEl.textContent

    const versionEl = document.createElement('span')
    versionEl.setAttribute('class', 'qorai-about-item qorai-about-menu-version')
    versionEl.textContent = `v ${loadTimeData.getString('qoraiProductVersion')}`
