...
RegisterPolymerTemplateModifications({
  'settings-cookies-page': (templateContent) => {
    ...
    if (!isNot3pcdRedesignEnabledTemplate) {
      ...
      if (!generalControls) {
        console.error(
          '[Qorai Settings Overrides] Could not find generalControls id ' +
          'on cookies page.')
      } else {
    ...
    if (!additionalProtections) {
      console.error(
        '[Qorai Settings Overrides] Could not find additionalProtections ' +
        'id on cookies page.')
    } else {
    ...
    if (!siteDataTrigger) {
      console.error(
        '[Qorai Settings Overrides] Could not find site-data-trigger id ' +
        'on cookies page')
    } else {
    ...
    if (!doNotTrackToggle) {
      console.error(
        '[Qorai Settings Overrides] Could not find toggle id on cookies page')
    } else {
...
RegisterPolymerComponentBehaviors({
  'settings-cookies-page': [QoraiSettingsCookiePageBehavior]
})
