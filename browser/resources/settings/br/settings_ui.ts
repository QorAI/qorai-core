...
import * as QoraiOriginMojom from '../qorai_origin_settings.mojom-webui.js';

// Expose QoraiOriginHandler to window object for devtools console access
function initializeQoraiOriginHandler() {
  const handler = QoraiOriginMojom.QoraiOriginSettingsHandler.getRemote();
  (window as any).QoraiOriginHandler = handler;
}

// Initialize when DOM is ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeQoraiOriginHandler);
} else {
  initializeQoraiOriginHandler();
}
  ...
  html`
    <style>
      :host {
        ...
        --qorai-settings-content-max-width: 708px;
        --qorai-settings-menu-width: var(--settings-menu-width);
        --qorai-settings-menu-margin: 12px;
        background-color: var(--qora-color-container-background);
      }

      /** Styling tweaks for the settings-menu when we display it inside the
        * drawer. */
      cr-drawer settings-menu {
        --qorai-settings-menu-margin-v: 0;
        --qorai-settings-menu-padding: 24px;
        --qorai-settings-menu-margin: 0;
      }
      ...
      #main {
        margin: var(--qora-spacing-m) var(--qora-spacing-m) var(--qora-spacing-m) 0;
        height: calc(100% - 40px);
        background: var(--qora-color-page-background);
        padding-bottom: var(--qora-spacing-2xl);
        border-radius: var(--qora-radius-xl) var(--qora-radius-m) var(--qora-radius-m) var(--qora-radius-xl);
        overflow: auto;
      }
      #right {
...
const QoraiClearSettingsMenuHighlightBehavior = {
...
// In Qorai we scroll the settings-main element, not the container.
const QoraiSetGlobalScrollTargetBehavior = {
  ready: function(this: SettingsUiElement) {
...
RegisterPolymerComponentBehaviors({
  'settings-ui': [
    QoraiClearSettingsMenuHighlightBehavior,
    QoraiSetGlobalScrollTargetBehavior
  ]
})
