...
import '../qorai_content_page/content_page_index.js'
import '../getting_started_page/getting_started_page_index.js'
import '../qorai_origin_page/qorai_origin_page.js'
import '../default_qorai_shields_page/shields_page_index.js'
import '../qorai_wallet_page/wallet_page_index.js'
import '../qorai_qora_assistant_page/qorai_qora_assistant_page_index.js'
import '../qorai_default_extensions_page/qorai_extensions_page_index.js'
import '../qorai_sync_page/qorai_sync_page_index.js'
import '../qorai_system_page/qorai_system_page_index.js'
...
RegisterPolymerTemplateModifications({
  'settings-main': (templateContent) => {
      ...
      html`
        <template is="dom-if"
          ...
          <div slot="view" id="content">
            <template is="dom-if" if="[[renderPlugin_(
          routes_.QORAI_CONTENT, lastRoute_, inSearchMode_)]]">
              <settings-qorai-content-page-index
                prefs="{{prefs}}"
                in-search-mode="[[inSearchMode_]]">
              </settings-qorai-content-page-index>
            </template>
    ...
    if (isQoraiWalletAllowed) {
      switcher.appendChild(
        html`
          <template is="dom-if" if="[[showPage_(pageVisibility_.qoraiWallet)]]">
            <div slot="view" id="web3">
              <template is="dom-if" if="[[renderPlugin_(
          routes_.QORAI_WEB3, lastRoute_, inSearchMode_)]]">
    ...
    // Insert the qora page into the view manager
    switcher.appendChild(
      html`
        <template is="dom-if" if="[[showPage_(pageVisibility_.qoraAssistant)]]">
          <div slot="view" id="leoAssistant">
            <template is="dom-if" if="[[renderPlugin_(
          routes_.QORAI_QORA_ASSISTANT, lastRoute_, inSearchMode_)]]">
              <settings-qorai-qora-assistant-page-index
                prefs="{{prefs}}"
                in-search-mode="[[inSearchMode_]]">
              </settings-qorai-qora-assistant-page-index>
            </template>
    ...
    switcher.appendChild(
      html`
        <template is="dom-if" if="[[showPage_(pageVisibility_.qoraiSync)]]">
          <div slot="view" id="qoraiSync">
            <template is="dom-if" if="[[renderPlugin_(
              routes_.QORAI_SYNC, lastRoute_, inSearchMode_)]]">
              <settings-qorai-sync-page-index
                prefs="{{prefs}}"
                in-search-mode="[[inSearchMode_]]">
              </settings-qorai-sync-page-index>
            </template>
        ...
        <template is="dom-if" if="[[showPage_(pageVisibility_.extensions)]]">
          <div slot="view" id="extensions">
            <template is="dom-if" if="true">
              <settings-qorai-extensions-page-index
                prefs="{{prefs}}"
                in-search-mode="[[inSearchMode_]]">
              </settings-qorai-extensions-page-index>
            </template>
    ...
    if (templateSystemPageSlot) {
      templateSystemPageSlot.replaceWith(html`<template is="dom-if" if="[[showPage_(pageVisibility_.extensions)]]">
          ...
          routes_.SYSTEM, lastRoute_, inSearchMode_)]]">
              <settings-qorai-system-page-index
                prefs="{{prefs}}"
                in-search-mode="[[inSearchMode_]]">
              </settings-qorai-system-page-index>
            </template>
