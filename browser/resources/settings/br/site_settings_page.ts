...
RegisterPolymerTemplateModifications({
  'settings-site-settings-page': (templateContent) => {
    ...
    const siteSettingsShieldsTitle =
      templateContent.getElementById('qoraiShields')
...
RegisterPolymerComponentReplacement(
  ...
  class QoraiSiteSettingsComponent extends SettingsSiteSettingsPageElement {
        ...
        if (!lists.permissionsAdvanced) {
          ...
          if (indexForAutoplay === -1) {
            ...
            if (isOpenAIChatFromQoraiSearchEnabled) {
              currentIndex++
              const AIChatItem = {
                route: routes.SITE_SETTINGS_QORAI_OPEN_AI_CHAT,
                id: ContentSettingsTypes.QORAI_OPEN_AI_CHAT,
                label: 'siteSettingsQoraiOpenAIChat',
                icon: 'product-qorai-qora',
                enabledLabel: 'siteSettingsQoraiOpenAIChatAsk',
                disabledLabel: 'siteSettingsQoraiOpenAIChatBlock'
              }
              lists.permissionsAdvanced.splice(currentIndex, 0,
                AIChatItem)
            }
