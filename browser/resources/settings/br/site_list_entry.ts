...
RegisterPolymerComponentReplacement(
  'site-list-entry',
  class QoraiSiteListEntryElement extends SiteListEntryElement {
    private shouldDisableReset_() {
      return this.model.qoraiCookieType !== undefined
    }
    ...
    private onResetSiteShowTooltip_() {
      ...
      if (
        this.model.qoraiCookieType === QoraiCookieTypes.SHIELDS_SETTINGS ||
        this.model.qoraiCookieType === QoraiCookieTypes.SHIELDS_DOWN
      ) {
        const text = this.i18n('cookieControlledByShieldsTooltip')
        this.fire('show-tooltip', { target: button, text })
      } else if (
        this.model.qoraiCookieType === QoraiCookieTypes.GOOGLE_SIGN_IN
      ) {
