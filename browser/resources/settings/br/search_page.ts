...
import '../qorai_search_engines_page/normal_search_engine_list_dialog.js'
import '../qorai_search_engines_page/private_search_engine_list_dialog.js'
  ...
  'settings-search-page': (templateContent) => {
    ...
    if (!enginesSubpageTrigger) {
      ...
      enginesSubpageTrigger.insertAdjacentHTML(
        'beforebegin',
        getTrustedHTML`
          <settings-qorai-search-page prefs="{{prefs}}">
          </settings-qorai-search-page>
        `)
    }
...
RegisterPolymerComponentReplacement(
  ...
  class QoraiSettingsSearchPageElement
