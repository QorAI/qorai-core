...
import './qorai_sync_configure.js'
import './qorai_sync_setup.js'
...
import {
  QoraiSyncBrowserProxy,
  QoraiSyncStatus
} from './qorai_sync_browser_proxy.js'
import {SyncPrefs} from '/shared/settings/people_page/sync_browser_proxy.js';

import {getTemplate} from './qorai_sync_subpage.html.js'

type PageStatus = 'configure' | 'setup' | 'spinner'

const SettingQoraiSyncSubpageBase =
  I18nMixin(RouteObserverMixin(PolymerElement))

class SettingQoraiSyncSubpage extends SettingQoraiSyncSubpageBase {
  static get is() {
    return 'settings-qorai-sync-subpage'
  }
  ...
  private declare syncStatus: QoraiSyncStatus
  ...
  private qoraiSyncBrowserProxy_: QoraiSyncBrowserProxy =
    QoraiSyncBrowserProxy.getInstance()
  ...
  override currentRouteChanged() {
    ...
    if (router.getCurrentRoute() === router.getRoutes().QORAI_SYNC_SETUP) {
      this.onNavigateToPage_()
      return
    }

    if (router.getRoutes().QORAI_SYNC_SETUP.
  ...
  private onNavigateToPage_() {
    const router = Router.getInstance()
    assert(router.getCurrentRoute() === router.getRoutes().QORAI_SYNC_SETUP)
    if (this.beforeunloadCallback_) {
      return
    }
  ...
  private onSetupSuccess_() {
    ...
    const router = Router.getInstance()
    if (router.getCurrentRoute() === router.getRoutes().QORAI_SYNC_SETUP) {
      router.navigateTo(router.getRoutes().QORAI_SYNC)
    }
  }
  ...
  async onRejoin_(_e: Event) {
    ...
    await this.qoraiSyncBrowserProxy_.resetSyncChain()
...
customElements.define(
  SettingQoraiSyncSubpage.is, SettingQoraiSyncSubpage)
