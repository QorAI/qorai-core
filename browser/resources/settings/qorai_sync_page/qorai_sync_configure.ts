...
import './qorai_sync_code_dialog.js';
import './qorai_sync_delete_account_dialog.js';
...
import {QoraiDeviceInfo, QoraiSyncBrowserProxy} from './qorai_sync_browser_proxy.js';
import {getTemplate} from './qorai_sync_configure.html.js'

/**
 * @fileoverview
 * 'settings-qorai-sync-configure' is the set of controls which fetches, displays
 * and updates the sync configuration.
 */

const SettingsQoraiSyncConfigureElementBase =
...
export class SettingsQoraiSyncConfigureElement extends SettingsQoraiSyncConfigureElementBase {
  static get is() {
    return 'settings-qorai-sync-configure'
  }
  ...
  private declare deviceList_: QoraiDeviceInfo[];
  private declare syncCodeDialogType_: string | null;
  private declare syncDoingDeleteAccount_: Boolean | false;

  browserProxy_: QoraiSyncBrowserProxy = QoraiSyncBrowserProxy.getInstance();
  ...
  handleDeviceInfo_(deviceList: QoraiDeviceInfo[]) {
    this.deviceList_ = deviceList
  }

  getDeviceDisplayDate(device: QoraiDeviceInfo): string {
  ...
  async onResetSyncChain_() {
    const messageText = this.i18n('qoraiSyncResetConfirmation')
    ...
    router.navigateTo((router.getRoutes() as {QORAI_SYNC: Route}).QORAI_SYNC);
  }
  ...
  async onDeleteDevice_(e: DomRepeatEvent<QoraiDeviceInfo>) {
    const messageText = this.i18n('qoraiSyncDeleteDeviceConfirmation')
    const shouldDeleteDevice = confirm(messageText)
    if (!shouldDeleteDevice) {
      return
    }
    const device: QoraiDeviceInfo = e.model.item
    await this.browserProxy_.deleteDevice(device.guid);
  }
}

customElements.define(
  SettingsQoraiSyncConfigureElement.is, SettingsQoraiSyncConfigureElement)
