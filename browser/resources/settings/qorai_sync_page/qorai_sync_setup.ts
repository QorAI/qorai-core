...
import {QoraiSyncBrowserProxy} from './qorai_sync_browser_proxy.js';
import {getTemplate} from './qorai_sync_setup.html.js'

/**
 * @fileoverview
 * 'qorai-sync-setup' is the UI for starting or joining a sync chain
 * settings.
 */

const SettingsQoraiSyncSetupElementBase =
  I18nMixin(BaseMixin(PolymerElement)) as {
    new(): PolymerElement & I18nMixinInterface
  }

export class SettingsQoraiSyncSetupElement extends SettingsQoraiSyncSetupElementBase {
  static get is() {
    return 'settings-qorai-sync-setup'
  }
  ...
  syncBrowserProxy_: QoraiSyncBrowserProxy = QoraiSyncBrowserProxy.getInstance();
  ...
  handleSyncCodeDialogDone_() {
    if (this.syncCodeDialogType === 'input') {
      const messageText = this.i18n('qoraiSyncFinalSecurityWarning')
...
customElements.define(
  SettingsQoraiSyncSetupElement.is, SettingsQoraiSyncSetupElement)
