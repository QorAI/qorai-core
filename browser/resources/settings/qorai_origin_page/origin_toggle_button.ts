...
export class OriginToggleButtonElement extends PolymerElement {
  ...
  private qoraiOriginHandler_:
      QoraiOriginMojom.QoraiOriginSettingsHandlerRemote;
  ...
  async loadPolicyValue_() {
    ...
    try {
      const {value} =
          await this.qoraiOriginHandler_.getPolicyValue(this.policyKey);
  ...
  private async handleToggleChange_() {
    ...
    try {
      const {success} = await this.qoraiOriginHandler_.setPolicyValue(
