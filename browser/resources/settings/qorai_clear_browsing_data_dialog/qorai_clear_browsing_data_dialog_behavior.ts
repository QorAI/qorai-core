...
import './qorai_clear_browsing_data_on_exit_page.js'

import {CrButtonElement} from 'chrome://resources/cr_elements/cr_button/cr_button.js'
import {
  SettingsQoraiClearBrowsingDataOnExitPageElement
} from './qorai_clear_browsing_data_on_exit_page.js'
...
import {
  QoraiClearBrowsingDataDialogBrowserProxy,
  QoraiClearBrowsingDataDialogBrowserProxyImpl
} from './qorai_clear_browsing_data_dialog_proxy.js'

export class QoraiSettingsClearBrowsingDataDialogElement
extends SettingsClearBrowsingDataDialogElement {
  declare qoraiRewardsEnabled_: boolean
  declare onClearQoraiAdsDataClickHandler_: ((e: Event) => void)

  private clearDataBrowserProxy_: QoraiClearBrowsingDataDialogBrowserProxy =
    QoraiClearBrowsingDataDialogBrowserProxyImpl.getInstance()
  ...
  static override get properties() {
    return {
      ...
      qoraiRewardsEnabled_: {
        type: Boolean,
        value: false,
      },
      onClearQoraiAdsDataClickHandler_: {
        type: Function,
        value: () => {},
      },
    }
  ...
  override ready() {
    ...
    this.addWebUiListener(
      'qorai-rewards-enabled-changed', (enabled: boolean) => {
      this.qoraiRewardsEnabled_ = enabled
    })

    this.clearDataBrowserProxy_.getQoraiRewardsEnabled().then((enabled) => {
      this.qoraiRewardsEnabled_ = enabled
    })
  }
  ...
  override connectedCallback() {
    ...
    this.onClearQoraiAdsDataClickHandler_ = this.clearQoraiAdsData_.bind(this)
  }
  ...
  override disconnectedCallback() {
    ...
    this.onClearQoraiAdsDataClickHandler_ = () => {}
  }
...
/**
  ...
  */
  private updateOnExitCountersText_(prefName: string, text: string) {
    ...
    if (matches) {
      this.shadowRoot!.
        querySelector<SettingsQoraiClearBrowsingDataOnExitPageElement>(
  ...
  /**
   * Clears Qorai Ads data.
   */
  private clearQoraiAdsData_(e: Event) {
    e.preventDefault()
    this.clearDataBrowserProxy_.clearQoraiAdsData()
    this.$.clearBrowsingDataDialog.close()
  }
}
