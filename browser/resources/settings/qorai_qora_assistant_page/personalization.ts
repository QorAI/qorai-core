...
import {
  QoraiQoraAssistantBrowserProxy,
  QoraiQoraAssistantBrowserProxyImpl,
  PremiumStatus,
  ModelWithSubtitle,
  PremiumInfo,
  ModelAccess,
...
const QoraiQoraPersonalizationBase =
  PrefsMixin(I18nMixin(BaseMixin(SettingsViewMixin(PolymerElement))))

class QoraiQoraPersonalization extends QoraiQoraPersonalizationBase {
  static get is() {
    return 'qorai-qora-personalization'
  }
  ...
  private isQoraModel_(model: Model) {
    return model.options.qoraModelOptions !== undefined
  }
  ...
  private onStorageEnabledChange_(event: Event) {
    ...
    // deleted.
    if (!target?.checked) {
      if (!confirm(this.i18n('qoraiQoraAssistantHistoryPreferenceConfirm'))) {
...
customElements.define(QoraiQoraPersonalization.is, QoraiQoraPersonalization)
