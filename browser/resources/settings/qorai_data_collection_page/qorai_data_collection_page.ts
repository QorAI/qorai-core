...
import {QoraiDataCollectionBrowserProxyImpl} from './qorai_data_collection_browser_proxy.js'
import {getTemplate} from './qorai_data_collection_page.html.js'
...
const SettingQoraiDataCollectionPageElementBase =
  WebUiListenerMixin(PrefsMixin(SettingsViewMixin(PolymerElement)))

interface SettingsQoraiDataCollectionPageElement {
  $: {
...
/**
 * 'settings-qorai-data-collection-page' is the settings page containing
 * Qorai's data collection settings.
 */
class SettingsQoraiDataCollectionPageElement
extends SettingQoraiDataCollectionPageElementBase
{
  static get is() {
    return 'settings-qorai-data-collection-subpage'
  }
  ...
  browserProxy_ = QoraiDataCollectionBrowserProxyImpl.getInstance()
  ...
  onSurveyPanelistLinkClicked_() {
    ...
    router.navigateTo(router.getRoutes().QORAI_SURVEY_PANELIST)
  }
}

customElements.define(
  SettingsQoraiDataCollectionPageElement.is,
  SettingsQoraiDataCollectionPageElement)
