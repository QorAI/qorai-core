...
import './components/qorai_adblock_editor.js'
import './components/qorai_adblock_scriptlet_list.js'
import './components/qorai_adblock_subscribe_dropdown.js'
...
import {
  QoraiAdblockBrowserProxy,
  QoraiAdblockBrowserProxyImpl,
  FilterList,
  Scriptlet,
  SubscriptionInfo
} from './qorai_adblock_browser_proxy.js'

import {SettingsViewMixin} from '../settings_page/settings_view_mixin.js';

import {getTemplate} from './qorai_adblock_subpage.html.js'
...
class AdBlockSubpage extends AdBlockSubpageBase {
  ...
  static get observers() {
    return [
      'updateState_(prefs.qorai.ad_block.developer_mode.value, customFilters_, customScriptlets_)'
    ]
  }
  ...
  private browserProxy_: QoraiAdblockBrowserProxy =
    QoraiAdblockBrowserProxyImpl.getInstance()
  ...
  override ready() {
    ...
    this.browserProxy_.addWebUiListener(
      'qorai_adblock.onGetListSubscriptions', (value: SubscriptionInfo[]) => {
        this.subscriptionList_ = value
    })
    this.browserProxy_.addWebUiListener(
      'qorai_adblock.onCustomFiltersChanged',
      (value: string) => {
        this.customFilters_ = value
      }
    )
    this.browserProxy_.addWebUiListener(
      'qorai_adblock.onCustomResourcesChanged',
      () => {
        this.browserProxy_.getCustomScriptlets().then((value: Scriptlet[]) => {
          this.customScriptlets_ = value
        })
