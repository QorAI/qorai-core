...
import {
  QoraiWalletBrowserProxy,
  QoraiWalletBrowserProxyImpl,
  type Option,
  type SolanaProvider,
  type CardanoProvider
} from './qorai_wallet_browser_proxy.js'

import {assert} from 'chrome://resources/js/assert.js'

import {getTemplate} from './qorai_wallet_page.html.js'

interface SettingsQoraiWalletPage {
  $: {
...
const SettingsQoraiWalletPageBase =
  WebUiListenerMixin(PrefsMixin(I18nMixin(RouteObserverMixin(SettingsViewMixin(PolymerElement)))))

class SettingsQoraiWalletPage extends SettingsQoraiWalletPageBase {
  static get is() {
    return 'settings-qorai-wallet-page'
  }
  ...
  private browserProxy_: QoraiWalletBrowserProxy =
    QoraiWalletBrowserProxyImpl.getInstance()
  ...
  private onInputAutoLockMinutes_() {
    ...
    this.setPrefValue('qorai.wallet.auto_lock_minutes', value)
  }

  private onWalletNetworksEditorClick_() {
    const router = Router.getInstance()
    router.navigateTo(router.getRoutes().QORAI_WALLET_NETWORKS)
  }
...
customElements.define(
  SettingsQoraiWalletPage.is, SettingsQoraiWalletPage)
