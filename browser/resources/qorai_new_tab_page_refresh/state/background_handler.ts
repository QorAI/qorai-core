...
import { SponsoredRichMediaAdEventHandler } from 'gen/qorai/components/ntp_background_images/browser/mojom/ntp_background_images.mojom.m.js'
import { NewTabPageProxy } from './new_tab_page_proxy'
import { Store } from '../lib/store'
import { debounce } from '$web-common/debounce'
import { BackgroundState, BackgroundActions, QoraiBackground } from './background_state'
...
// the current collection of Qorai backgrounds.
const defaultQoraiBackground: QoraiBackground = {
...
export function createBackgroundHandler(
  ...
  store.update({
    qoraiBackgrounds: [defaultQoraiBackground],
  ...
  async function updateQoraiBackgrounds() {
    const { backgrounds } = await handler.getQoraiBackgrounds()
    store.update({
      qoraiBackgrounds:
        backgrounds.length ? backgrounds : [defaultQoraiBackground]
    })
  }
