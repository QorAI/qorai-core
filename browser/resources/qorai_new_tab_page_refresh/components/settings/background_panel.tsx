...
import { settingsURL } from '../../../../../components/qorai_rewards/resources/shared/lib/rewards_urls'
...
export function BackgroundPanel() {
  ...
  const qoraiBackgrounds = useBackgroundState((s) => s.qoraiBackgrounds)
  ...
  function getTypePreviewValue(type: SelectedBackgroundType) {
    ...
    switch (type) {
      case SelectedBackgroundType.kQorai:
        return qoraiBackgrounds[0]?.imageUrl ?? ''
        ...
        backgroundsEnabled && backgroundsCustomizable && <>
          <div className='background-options'>
            ...
            <div className='background-option'>
              <button
                onClick={() => {
                  actions.selectBackground(SelectedBackgroundType.kQorai, '')
                }}
              >
                {renderTypePreview(SelectedBackgroundType.kQorai)}
                {getString(S.NEW_TAB_QORAI_BACKGROUND_LABEL)}
              </button>
            </div>
            <div className='background-option'>
