...
import { qoraiSearchHost } from '../../state/search_state'
import {
  useSearchMatches,
  useSearchActions,
...
export function useRichMediaMessageHandler(
  ...
  const queryAutocomplete = useQoraiSearchAutocomplete(frameHandle)
  ...
  return React.useCallback(
    (data: unknown) => {
      handleIncomingMessage(data, {
        ...
        openQoraiSearch(pathAndQuery) {
          openLink(`https://${qoraiSearchHost}/${pathAndQuery}`)
        },
        queryQoraiSearchAutocomplete(query) {
...
function useQoraiSearchAutocomplete(frameHandle?: RichMediaFrameHandle) {
  ...
  return (query: string) => {
    ...
    searchActions.queryAutocomplete(query, qoraiSearchHost)
  }
}
