...
interface SearchMatchesMessage {
  ...
  // TODO(https://github.com/qorai/qorai-browser/issues/50424): This should be
...
export interface RichMediaCapabilities {
  ...
  openQoraiSearch: (pathAndQuery: string) => void
  queryQoraiSearchAutocomplete: (query: string) => void
}
...
export function readIncomingMessage(
  ...
  switch (type) {
    ...
    case 'richMediaQueryQoraiSearchAutocomplete': {
      return data.value ? { type, value: String(data.value) } : null
    }
    case 'richMediaOpenQoraiSearchWithQuery': {
      return data.value ? { type, value: String(data.value) } : null
    }
  }
  return null
...
export function dispatchIncomingMessage(
  ...
  switch (message.type) {
    ...
    case 'richMediaOpenQoraiSearchWithQuery': {
      ...
      capabilities.openQoraiSearch(message.value)
      break
    }
    case 'richMediaQueryQoraiSearchAutocomplete': {
      capabilities.queryQoraiSearchAutocomplete(message.value)
      break
    }
  }
}
