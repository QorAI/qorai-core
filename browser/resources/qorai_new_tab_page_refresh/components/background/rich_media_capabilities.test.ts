...
describe('dispatchIncomingMessage', () => {
  ...
  beforeEach(() => {
    mockCapabilities = {
      ...
      openQoraiSearch: jest.fn(),
      queryQoraiSearchAutocomplete: jest.fn(),
    }
  })
  ...
  describe('richMediaQueryQoraiSearchAutocomplete message', () => {
    it('should call queryQoraiSearchAutocomplete with the query value', () => {
      dispatch({
        type: 'richMediaQueryQoraiSearchAutocomplete',
        value: 'test query',
      })

      expect(
        mockCapabilities.queryQoraiSearchAutocomplete,
      ).toHaveBeenCalledWith('test query')

      expect(mockCapabilities.notifyAdEvent).not.toHaveBeenCalled()
      expect(mockCapabilities.openDestinationUrl).not.toHaveBeenCalled()
      expect(mockCapabilities.openQoraiSearch).not.toHaveBeenCalled()
    })
  })

  describe('richMediaOpenQoraiSearchWithQuery message', () => {
    it('should call notifyAdEvent with kClicked and openQoraiSearch', () => {
      dispatch({
        type: 'richMediaOpenQoraiSearchWithQuery',
        value: 'search query',
      })
      ...
      expect(mockCapabilities.openQoraiSearch).toHaveBeenCalledWith(
        'search query',
      )
    })
  })
