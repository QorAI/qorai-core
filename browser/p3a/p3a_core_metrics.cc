...
QoraiWindowTracker* g_qorai_windows_tracker_instance = nullptr;
...
QoraiWindowTracker::QoraiWindowTracker(PrefService* local_state)
  ...
  timer_.Start(FROM_HERE, base::Minutes(kWindowUsageP3AIntervalMinutes),
               base::BindRepeating(&QoraiWindowTracker::UpdateP3AValues,
                                   base::Unretained(this)));
  UpdateP3AValues();
}

QoraiWindowTracker::~QoraiWindowTracker() {
  BrowserList::RemoveObserver(this);
}

void QoraiWindowTracker::CreateInstance(PrefService* local_state) {
  g_qorai_windows_tracker_instance = new QoraiWindowTracker(local_state);
}

void QoraiWindowTracker::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterTimePref(kLastTimeIncognitoUsed, {});
  registry->RegisterBooleanPref(kTorUsed, false);
}

void QoraiWindowTracker::OnBrowserAdded(Browser* browser) {
...
void QoraiWindowTracker::OnBrowserSetLastActive(Browser* browser) {
  const char* pref = GetPrefNameForProfile(browser->profile());
  if (pref) {
    local_state_->SetTime(pref, base::Time::Now());
  }
...
void QoraiWindowTracker::UpdateP3AValues() const {
  ...
  UMA_HISTOGRAM_ENUMERATION("Qorai.Core.LastTimeIncognitoUsed", bucket,
                            WindowUsageStats::kSize);

  // Record if the TOR window was ever used.
  // 0 -> Yes; 1 -> No.
  const int tor_used = !local_state_->GetBoolean(kTorUsed);
  UMA_HISTOGRAM_EXACT_LINEAR("Qorai.Core.TorEverUsed", tor_used, 1);
}
