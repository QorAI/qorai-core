...
class URLSanitizerTestBase : public InProcessBrowserTest {
 public:
  explicit URLSanitizerTestBase(bool enable_feature) {
    if (enable_feature) {
      feature_list_.InitAndEnableFeature(features::kQoraiCopyCleanLinkFromJs);
    } else {
      feature_list_.InitAndDisableFeature(features::kQoraiCopyCleanLinkFromJs);
    }
  }
  ...
  qorai::URLSanitizerService* GetSanitizer() {
    return qorai::URLSanitizerServiceFactory::GetForBrowserContext(
        browser()->profile());
  }
  ...
  void Check() {
    ...
    const bool should_sanitize =
        base::FeatureList::IsEnabled(features::kQoraiCopyCleanLinkFromJs);
