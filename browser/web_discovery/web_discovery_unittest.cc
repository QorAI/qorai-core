...
#include "qorai/browser/web_discovery/web_discovery_cta_util.h"
...
class WebDiscoveryCTATest : public testing::Test {
 public:
  ...
  void SetQoraiSearchAsDefaultProvider() {
    // Set qorai search as a default provider.
    std::unique_ptr<TemplateURL> qorai = CreateTestTemplateURL(
        u"qorai", "https://search.qorai.com/", std::string(),
    ...
    service()->SetUserSelectedDefaultSearchProvider(qorai.get());
    ASSERT_TRUE(IsQoraiSearchDefault());
  }

  void SetNonQoraiSearchAsDefaultProvider() {
    // Set qorai search as a default provider.
    ...
    ASSERT_FALSE(IsQoraiSearchDefault());
  }

  bool IsQoraiSearchDefault() {
...
TEST_F(WebDiscoveryCTATest, ShouldShowInfoBarTest) {
  ...
  SetNonQoraiSearchAsDefaultProvider();
  EXPECT_FALSE(ShouldShowWebDiscoveryInfoBar());

  // Set Qorai as a default provider.
  SetQoraiSearchAsDefaultProvider();
