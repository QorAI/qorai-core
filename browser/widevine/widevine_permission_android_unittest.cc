...
class WidevinePermissionAndroidTest : public ChromeRenderViewHostTestHarness {
 public:
  ...
  void SimulateNavigation() {
    ...
    qorai_drm_tab_helper()->DidStartNavigation(&test_handle);
  }
 ...
 protected:
  void SetUp() override {
    ...
    QoraiDrmTabHelper::CreateForWebContents(web_contents_.get());
    tab_helper_ = QoraiDrmTabHelper::FromWebContents(web_contents_.get());
  ...
  QoraiDrmTabHelper* qorai_drm_tab_helper() const { return tab_helper_; }

 private:
  std::unique_ptr<TestingProfileManager> profile_manager_;
  raw_ptr<TestingProfile> profile_ = nullptr;
  std::unique_ptr<content::WebContents> web_contents_;
  raw_ptr<QoraiDrmTabHelper> tab_helper_ = nullptr;
  raw_ptr<permissions::PermissionRequestManager> permission_request_manager_ =
      nullptr;
};

TEST_F(WidevinePermissionAndroidTest, QoraiDrmTabHelperTest) {
  SanityCheck();

  permissions::PermissionRequestManager* manager = permission_request_manager();
  EXPECT_FALSE(qorai_drm_tab_helper()->ShouldShowWidevineOptIn());

  qorai_drm_tab_helper()->OnWidevineKeySystemAccessRequest();
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(qorai_drm_tab_helper()->ShouldShowWidevineOptIn());
