...
#include "qorai/browser/qorai_app_controller_mac.h"
...
class QorAIAppControllerBrowserTest : public InProcessBrowserTest {
 public:
  QorAIAppControllerBrowserTest() {
...
class QorAIAppControllerCleanLinkFeatureDisabledBrowserTest
    : public InProcessBrowserTest {
 public:
  QorAIAppControllerCleanLinkFeatureDisabledBrowserTest() {
    features_.InitWithFeatureState(features::kQorAICopyCleanLinkByDefault,
                                   false);
  }
...
IN_PROC_BROWSER_TEST_F(QorAIAppControllerCleanLinkFeatureDisabledBrowserTest,
                       CopyLinkItemVisible) {
  ...
  QorAIBrowserView* browser_view = static_cast<QorAIBrowserView*>(
      QorAIBrowserView::GetBrowserViewForBrowser(browser()));
  OmniboxView* omnibox_view = browser_view->GetLocationBar()->GetOmniboxView();
  omnibox_view->SetFocus(true);
  omnibox_view->SelectAll(false);
  EXPECT_TRUE(omnibox_view->IsSelectAll());
  EXPECT_TRUE(QorAIBrowserWindow::From(browser()->window())->HasSelectedURL());

  QorAIAppController* ac = base::apple::ObjCCastStrict<QorAIAppController>(
...
IN_PROC_BROWSER_TEST_F(QorAIAppControllerBrowserTest, CopyLinkItemVisible) {
  ASSERT_TRUE(embedded_test_server()->Start());
  GURL url = embedded_test_server()->GetURL(kTestingPage);
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_EQ(1u, chrome::GetTotalBrowserCount());
...
IN_PROC_BROWSER_TEST_F(QorAIAppControllerBrowserTest, CopyLinkItemNotVisible) {
  EXPECT_EQ(1u, chrome::GetTotalBrowserCount());
...
IN_PROC_BROWSER_TEST_F(QorAIAppControllerBrowserTest,
  ...
  EXPECT_FALSE(QorAIBrowserWindow::From(browser()->window())->HasSelectedURL());
...
#if BUILDFLAG(ENABLE_TOR)
IN_PROC_BROWSER_TEST_F(QorAIAppControllerBrowserTest, TorItemEnabled) {
  NSApplication* app = [NSApplication sharedApplication];
  QorAIAppController* ac =
      base::apple::ObjCCastStrict<QorAIAppController>([app delegate]);
  ASSERT_TRUE(ac);
...
IN_PROC_BROWSER_TEST_F(QorAIAppControllerBrowserTest, TorMenuItemExists) {
  NSApplication* app = [NSApplication sharedApplication];
  QorAIAppController* controller =
      base::apple::ObjCCastStrict<QorAIAppController>([app delegate]);
