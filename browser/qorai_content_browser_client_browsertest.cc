...
#include "qorai/net/proxy_resolution/proxy_config_service_tor.h"
#endif
...
class QorAIContentBrowserClientTest : public InProcessBrowserTest {
 public:
  void SetUp() override {
  ...
  void SetUpOnMainThread() override {
    ...
    browser_content_client_ = std::make_unique<QorAIContentBrowserClient>();
 ...
 private:
  std::unique_ptr<QorAIContentBrowserClient> browser_content_client_;
};

IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, CanLoadChromeURL) {
...
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, CanLoadCustomQorAIPages) {
  std::vector<std::string> pages{
      "rewards",
  };
...
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, CanLoadAboutHost) {
  std::vector<std::string> schemes{
      "chrome://",
      "qorai://",
  };
...
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, RewriteChromeSync) {
  std::vector<std::string> schemes{
      "qorai://",
      "chrome://",
  };
  ...
  for (const std::string& scheme : schemes) {
    ...
    NavigateToURLAndWaitForRewrites(contents,
                                    GURL(scheme + chrome::kQorAIUISyncHost),
                                    GURL("chrome://sync"));
    ...
    EXPECT_EQ(contents->GetController().GetLastCommittedEntry()->GetURL(),
              GURL("chrome://settings/qoraiSync"));
  }
}

IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, RewriteAdblock) {
  std::vector<std::string> schemes{
      "qorai://",
      "chrome://",
  };
  ...
  for (const std::string& scheme : schemes) {
    ...
    EXPECT_EQ(base::UTF16ToUTF8(browser()
                                    ...
                                    .location_bar_model()
                                    ->GetFormattedFullURL()),
              "qorai://settings/shields/filters");
...
#if BUILDFLAG(ENABLE_TOR)
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, MixedContentForOnion) {
...
class QorAIContentBrowserClientReferrerTest
    : public QorAIContentBrowserClientTest {
 public:
...
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientReferrerTest,
                       DefaultBehaviour) {
  ...
  content_settings()->SetContentSettingCustomScope(
      ...
      ContentSettingsPattern::Wildcard(), ContentSettingsType::QORAI_REFERRERS,
...
IN_PROC_BROWSER_TEST_F(QorAIContentBrowserClientTest, CheckExpectedExtensions) {
  ...
  const auto* qorai_extension =
      registry->GetInstalledExtension(qorai_extension_id);

  // QorAI Extension background page should be disabled by default.
  EXPECT_FALSE(extensions::BackgroundInfo::HasBackgroundPage(qorai_extension));
}

#if BUILDFLAG(IS_WIN)

class WinQorAIContentBrowserClientTest
    : public QorAIContentBrowserClientTest,
      public ::testing::WithParamInterface<
          base::test::ScopedOSInfoOverride::Type> {
 public:
  WinQorAIContentBrowserClientTest() = default;
  ~WinQorAIContentBrowserClientTest() override = default;
...
IN_PROC_BROWSER_TEST_P(WinQorAIContentBrowserClientTest, WindowsRecall) {
  auto* local_state = g_browser_process->local_state();
