...
class AIChatPolicyTest : public InProcessBrowserTest,
 ...
 public:
  ...
  void SetUpInProcessBrowserTestFixture() override {
    ...
    PolicyMap policies;
    policies.Set(key::kQoraiAIChatEnabled, POLICY_LEVEL_MANDATORY,
...
// Testing if we insert QoraProvider
IN_PROC_BROWSER_TEST_P(AIChatPolicyTest, Autocomplete) {
  ...
  bool is_in_providers =
      std::ranges::any_of(providers, [](const auto& provider) {
        return provider->type() == AutocompleteProvider::TYPE_QORAI_QORA;
      });
  if (IsAIChatEnabledTest()) {
...
IN_PROC_BROWSER_TEST_P(AIChatPolicyTest, ContextMenu) {
  ...
  params.selection_text = u"qorai";
  TestRenderViewContextMenu menu(*web_contents()->GetPrimaryMainFrame(),
                                 params);
  menu.Init();
  std::optional<size_t> ai_chat_index =
      menu.menu_model().GetIndexOfCommandId(IDC_AI_CHAT_CONTEXT_QORA_TOOLS);
  if (IsAIChatEnabledTest()) {
