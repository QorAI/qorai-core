...
[[nodiscard]] std::string SanitizeContentText(const std::string& input) {
  ...
  base::ReplaceSubstringsAfterOffset(&output, 0, kQoraiUntrustedContentTagName,
                                     "");
  return output;
}
...
std::vector<mojom::ContentBlockPtr> ConvertAnnotatedPageContentToBlocks(
  ...
  if (tree_string.length() > kMaxTreeStringLength) {
    // TODO(https://github.com/qorai/qorai-browser/issues/49262): prioritize
  ...
  base::StrAppend(&result, {"\n", kQoraiUntrustedContentCloseTag, "\n"});
