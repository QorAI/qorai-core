...
#include "chrome/android/chrome_jni_headers/QoraiQoraMojomHelper_jni.h"
...
void AIChatSettingsHelper::GetManageUrl(GetManageUrlCallback callback) {
  ...
  std::string domain = qorai_domains::GetServicesDomain(kAccountHostnamePart);
#else
  std::string domain = qorai_domains::GetServicesDomain(kAccountHostnamePart,
                                                        qorai_domains::STAGING);
#endif
...
#if BUILDFLAG(IS_ANDROID)
static jlong JNI_QoraiQoraMojomHelper_Init(
    JNIEnv* env,
...
void AIChatSettingsHelper::CreateOrderId(CreateOrderIdCallback callback) {
  ...
  auto* purchase_token =
      pref_service_->FindPreference(prefs::kQoraiChatPurchaseTokenAndroid);
  if (purchase_token && !purchase_token->IsDefaultValue()) {
    purchase_token_string =
        pref_service_->GetString(prefs::kQoraiChatPurchaseTokenAndroid);
  }
  std::string package_string;
  auto* package =
      pref_service_->FindPreference(prefs::kQoraiChatPackageNameAndroid);
  if (package && !package->IsDefaultValue()) {
    package_string =
        pref_service_->GetString(prefs::kQoraiChatPackageNameAndroid);
  }
  std::string subscription_id_string;
  auto* subscription_id =
      pref_service_->FindPreference(prefs::kQoraiChatProductIdAndroid);
  if (subscription_id && !subscription_id->IsDefaultValue()) {
    subscription_id_string =
        pref_service_->GetString(prefs::kQoraiChatProductIdAndroid);
  }
