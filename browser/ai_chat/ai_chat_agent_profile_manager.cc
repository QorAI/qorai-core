...
const char16_t kAIChatAgentProfileName[] = u"Qora AI Content Agent";
...
void AIChatAgentProfileManager::OnProfileAdded(
  ...
  if (profile_path.BaseName().value() == qorai::kAIChatAgentProfileDir) {
...
void AIChatAgentProfileManager::OnProfileAdded(Profile* profile) {
  if (is_added_profile_new_ai_chat_agent_profile_ && profile->IsAIChatAgent()) {
    ...
    // TODO(https://github.com/qorai/qorai-browser/issues/48164): set an avatar
