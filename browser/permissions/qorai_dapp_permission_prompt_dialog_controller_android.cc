...
#include "chrome/android/chrome_jni_headers/QoraiDappPermissionPromptDialog_jni.h"
...
void QoraiDappPermissionPromptDialogController::ShowDialog() {
  if (!GetOrCreateJavaObject())
    return;

  JNIEnv* env = base::android::AttachCurrentThread();
  Java_QoraiDappPermissionPromptDialog_show(env, GetOrCreateJavaObject());
}

void QoraiDappPermissionPromptDialogController::OnPrimaryButtonClicked(
    JNIEnv* env,
...
void QoraiDappPermissionPromptDialogController::OnNegativeButtonClicked(
    JNIEnv* env) {
  delegate_->CancelConnectToSite();
}

void QoraiDappPermissionPromptDialogController::OnDialogDismissed(JNIEnv* env) {
  java_object_.Reset();
  delegate_->OnDialogDismissed();
}

void QoraiDappPermissionPromptDialogController::DismissDialog() {
  if (java_object_) {
    Java_QoraiDappPermissionPromptDialog_dismissDialog(
        base::android::AttachCurrentThread(), java_object_);
  }
}

base::android::ScopedJavaGlobalRef<jobject>
QoraiDappPermissionPromptDialogController::GetOrCreateJavaObject() {
  if (java_object_)
    return java_object_;
  ...
  return java_object_ = Java_QoraiDappPermissionPromptDialog_create(
