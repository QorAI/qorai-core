...
static void JNI_ConnectAccountFragment_ConnectAccount(
  ...
  qorai_wallet::mojom::CoinType coin =
      static_cast<qorai_wallet::mojom::CoinType>(account_id_coin);
  CHECK(qorai_wallet::mojom::IsKnownEnumValue(coin));

  auto request_type = qorai_wallet::CoinTypeToPermissionRequestType(coin);
  auto permission = qorai_wallet::CoinTypeToPermissionType(coin);
