...
class PermissionManagerBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetPermissionManagerForProfile(Profile* profile) {
    permission_manager_ = static_cast<permissions::QoraiPermissionManager*>(
  ...
  QoraiPermissionManager* permission_manager() { return permission_manager_; }
  ...
  void TestRequestPermissionsDoNotLeak(Profile* profile1, Profile* profile2) {
    ...
    ContentSettingsType type = ContentSettingsType::QORAI_ETHEREUM;
    blink::PermissionType permission = blink::PermissionType::QORAI_ETHEREUM;
    ...
    permissions::QoraiWalletPermissionContext::AcceptOrCancel(
        {address}, qorai_wallet::mojom::PermissionLifetimeOption::kForever,
        web_contents());
 ...
 protected:
  ...
  raw_ptr<QoraiPermissionManager, DanglingUntriaged> permission_manager_ =
      nullptr;
...
IN_PROC_BROWSER_TEST_F(PermissionManagerBrowserTest, RequestPermissions) {
  ...
  for (auto& test_case : cases) {
    ...
    permissions::QoraiWalletPermissionContext::AcceptOrCancel(
        ...
        qorai_wallet::mojom::PermissionLifetimeOption::kForever,
        web_contents());
    testing::Mock::VerifyAndClearExpectations(&callback);
