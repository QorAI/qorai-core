...
constexpr char kQoraiSearchPath[] = "/search";
}  // namespace
...
void PageMetricsTabHelper::DidFinishNavigation(
  ...
  if (navigation_handle->GetURL().host_piece() == kQoraiSearchHost &&
      navigation_handle->GetURL().path_piece() == kQoraiSearchPath) {
    page_metrics_->OnQoraiQuery();
  }
}
