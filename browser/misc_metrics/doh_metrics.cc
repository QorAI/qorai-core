...
using net::features::kQoraiFallbackDoHProvider;
using net::features::kQoraiFallbackDoHProviderEndpoint;
...
const char* GetAutoSecureRequestsHistogramName() {
  ...
  if (base::FeatureList::IsEnabled(net::features::kQoraiFallbackDoHProvider)) {
    auto endpoint_type = kQoraiFallbackDoHProviderEndpoint.Get();
...
DohMetrics::DohMetrics(PrefService* local_state) : local_state_(local_state) {
  ...
  if (base::FeatureList::IsEnabled(net::features::kQoraiFallbackDoHProvider)) {
    current_fallback_int =
        static_cast<int>(kQoraiFallbackDoHProviderEndpoint.Get());
  }
