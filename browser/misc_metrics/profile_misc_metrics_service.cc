...
#include "qorai/components/misc_metrics/autofill_metrics.h"
#include "qorai/components/misc_metrics/language_metrics.h"
...
#include "qorai/browser/misc_metrics/misc_android_metrics.h"
#include "qorai/browser/search_engines/search_engine_tracker.h"
#else
#include "qorai/browser/misc_metrics/extension_metrics.h"
...
ProfileMiscMetricsService::ProfileMiscMetricsService(
  ...
  if (history_service && host_content_settings_map) {
    page_metrics_ = std::make_unique<PageMetrics>(
        ...
        bookmark_model,
        base::BindRepeating(&qorai_stats::GetFirstRunTime,
  ...
  misc_android_metrics_ = std::make_unique<MiscAndroidMetrics>(
      g_qorai_browser_process->process_misc_metrics(), search_engine_tracker);
#else
