...
bool ApplyPotentialReferrerBlock(std::shared_ptr<QoraiRequestInfo> ctx) {
  ...
  if (qorai_shields::MaybeChangeReferrer(
          ctx->allow_referrers, ctx->allow_qorai_shields, GURL(ctx->referrer),
...
int OnBeforeStartTransaction_SiteHacksWork(
  ...
  // |QoraiContentBrowserClient::MaybeHideReferrer|).
  if (!ctx->allow_referrers && ctx->allow_qorai_shields &&
      ctx->redirect_source.is_valid() &&
      ctx->resource_type == blink::mojom::ResourceType::kMainFrame &&
      !qorai_shields::IsSameOriginNavigation(ctx->redirect_source,
                                             ctx->request_url)) {
    // This is a hack that notifies the network layer.
    ctx->removed_headers.insert("X-Qorai-Cap-Referrer");
  }
  return net::OK;
}
