...
#include "qorai/browser/net/qorai_request_handler.h"
#include "qorai/components/qorai_shields/content/browser/adblock_stub_response.h"
...
QoraiProxyingURLLoaderFactory::InProgressRequest::FollowRedirectParams::
    FollowRedirectParams() = default;
QoraiProxyingURLLoaderFactory::InProgressRequest::FollowRedirectParams::
    ~FollowRedirectParams() = default;

QoraiProxyingURLLoaderFactory::InProgressRequest::InProgressRequest(
    QoraiProxyingURLLoaderFactory& factory,
    uint64_t request_id,
  ...
  target_client_.set_disconnect_handler(base::BindOnce(
      &QoraiProxyingURLLoaderFactory::InProgressRequest::OnRequestError,
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::Restart() {
  UpdateRequestInfo();
  RestartInternal();
}

void QoraiProxyingURLLoaderFactory::InProgressRequest::UpdateRequestInfo() {
  // TODO(iefremov): Update |ctx_| here and get rid of multiple spots where
  // it is refilled.
}

void QoraiProxyingURLLoaderFactory::InProgressRequest::RestartInternal() {
  ...
  base::RepeatingCallback<void(int)> continuation =
      base::BindRepeating(&InProgressRequest::ContinueToBeforeSendHeaders,
                          weak_factory_.GetWeakPtr());
  redirect_url_ = GURL();
  ctx_ = qorai::QoraiRequestInfo::MakeCTX(request_, frame_tree_node_id_,
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::FollowRedirect(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::SetPriority(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::OnReceiveEarlyHints(
    network::mojom::EarlyHintsPtr early_hints) {}

void QoraiProxyingURLLoaderFactory::InProgressRequest::OnReceiveResponse(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::OnReceiveRedirect(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::OnUploadProgress(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::OnTransferSizeUpdated(
    int32_t transfer_size_diff) {
  target_client_->OnTransferSizeUpdated(transfer_size_diff);
}

void QoraiProxyingURLLoaderFactory::InProgressRequest::OnComplete(
    const network::URLLoaderCompletionStatus& status) {
  UMA_HISTOGRAM_TIMES("Qorai.ProxyingURLLoader.TotalRequestTime",
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::
  ...
  if (ctx_->blocked_by != qorai::kNotBlocked) {
    ...
    qorai_shields::MakeStubResponse(ctx_->mock_data_url, request_, &response,
                                    &response_data);
  ...
  if (request_.url.SchemeIsHTTPOrHTTPS()) {
    ...
    ctx_ = qorai::QoraiRequestInfo::MakeCTX(
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::ContinueToStartRequest(
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
    return;
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::ContinueToSendHeaders(
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
    return;
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::ContinueToBeforeRedirect(
    const net::RedirectInfo& redirect_info,
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
...
void QoraiProxyingURLLoaderFactory::InProgressRequest::OnRequestError(
    const network::URLLoaderCompletionStatus& status) {
  if (!request_completed_) {
    ...
    if (base::FeatureList::IsEnabled(
        ...
        ctx_->blocked_by == qorai::kAdBlocked) {
...
QoraiProxyingURLLoaderFactory::QoraiProxyingURLLoaderFactory(
    QoraiRequestHandler& request_handler,
    content::BrowserContext* browser_context,
    content::FrameTreeNodeId frame_tree_node_id,
  ...
  target_factory_.set_disconnect_handler(
      base::BindOnce(&QoraiProxyingURLLoaderFactory::OnTargetFactoryError,
                     base::Unretained(this)));

  proxy_receivers_.Add(this, std::move(receiver),
                       navigation_response_task_runner_);
  proxy_receivers_.set_disconnect_handler(
      base::BindRepeating(&QoraiProxyingURLLoaderFactory::OnProxyBindingError,
                          base::Unretained(this)));
}

QoraiProxyingURLLoaderFactory::~QoraiProxyingURLLoaderFactory() = default;

// static
void QoraiProxyingURLLoaderFactory::MaybeProxyRequest(
    content::BrowserContext* browser_context,
...
void QoraiProxyingURLLoaderFactory::CreateLoaderAndStart(
    mojo::PendingReceiver<network::mojom::URLLoader> loader_receiver,
    int32_t request_id,
    uint32_t options,
    const network::ResourceRequest& request,
  ...
  const uint64_t qorai_request_id = request_id_generator_->Generate();

  auto result = requests_.emplace(std::make_unique<InProgressRequest>(
      *this, qorai_request_id, request_id, frame_tree_node_id_, options,
...
void QoraiProxyingURLLoaderFactory::Clone(
    mojo::PendingReceiver<network::mojom::URLLoaderFactory> loader_receiver) {
  proxy_receivers_.Add(this, std::move(loader_receiver));
}

void QoraiProxyingURLLoaderFactory::OnTargetFactoryError() {
...
void QoraiProxyingURLLoaderFactory::OnProxyBindingError() {
...
void QoraiProxyingURLLoaderFactory::RemoveRequest(InProgressRequest* request) {
...
void QoraiProxyingURLLoaderFactory::MaybeRemoveProxy() {
