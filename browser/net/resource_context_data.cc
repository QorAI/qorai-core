...
void ResourceContextData::StartProxying(
  ...
  if (!self->request_handler_) {
    self->request_handler_ = std::make_unique<QoraiRequestHandler>();
  }

  auto proxy = std::make_unique<QoraiProxyingURLLoaderFactory>(
...
// static
QoraiProxyingWebSocket* ResourceContextData::CreateProxyingWebSocket(
  ...
  auto proxy = std::make_unique<QoraiProxyingWebSocket>(
...
void ResourceContextData::RemoveProxy(QoraiProxyingURLLoaderFactory* proxy) {
  auto it = proxies_.find(proxy);
  DCHECK(it != proxies_.end());
  proxies_.erase(it);
}

void ResourceContextData::RemoveProxyWebSocket(QoraiProxyingWebSocket* proxy) {
