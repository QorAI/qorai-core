...
#include "qorai/browser/net/qorai_network_audit_allowed_lists.h"
...
constexpr char kEmbeddedTestServerDirectory[] = "qorai_ads";
constexpr char kDomain[] = "search.qorai.com";
constexpr char kQoraiSearchPath[] = "/search_result_ad_click.html";
...
class QoraiNetworkAuditSearchAdTest : public InProcessBrowserTest {
 public:
  QoraiNetworkAuditSearchAdTest() = default;

  QoraiNetworkAuditSearchAdTest(const QoraiNetworkAuditSearchAdTest&) = delete;
  QoraiNetworkAuditSearchAdTest& operator=(
      const QoraiNetworkAuditSearchAdTest&) = delete;
  ...
  void SetUpOnMainThread() override {
    ...
    auto* qorai_template_url =
        template_url_service->GetTemplateURLForHost("search.qorai.com");
    ASSERT_TRUE(qorai_template_url);
  ...
  void TearDownInProcessBrowserTestFixture() override {
    ...
    // Qorai search
    // The test simulation has a pattern https://search.qorai.com:<port>
    // port is changed dynamically
    static constexpr char kAllowedQoraiSearchTemplate[] =
        "https://search.qorai.com:%s/";
    allowed_prefixes.push_back(
        absl::StrFormat(kAllowedQoraiSearchTemplate, port));
...
IN_PROC_BROWSER_TEST_F(QoraiNetworkAuditSearchAdTest, SearchAdTest) {
  WaitForTimeout(kMaxTimeoutForAdsServiceInit);
  GURL url = https_server()->GetURL(kDomain, kQoraiSearchPath);
