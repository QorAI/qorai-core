...
QoraiRequestInfo::QoraiRequestInfo() = default;

QoraiRequestInfo::QoraiRequestInfo(const GURL& url) : request_url(url) {}

QoraiRequestInfo::~QoraiRequestInfo() = default;

// static
std::shared_ptr<qorai::QoraiRequestInfo> QoraiRequestInfo::MakeCTX(
    const network::ResourceRequest& request,
    content::FrameTreeNodeId frame_tree_node_id,
    uint64_t request_identifier,
    content::BrowserContext* browser_context,
    std::shared_ptr<qorai::QoraiRequestInfo> old_ctx) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  auto ctx = std::make_shared<qorai::QoraiRequestInfo>();
  ...
  ctx->allow_qorai_shields =
      map ? qorai_shields::GetQoraiShieldsEnabled(map, ctx->tab_origin) : true;
  ctx->allow_ads =
      map && qorai_shields::GetAdControlType(map, ctx->tab_origin) ==
  ...
  ctx->aggressive_blocking =
      map && qorai_shields::GetCosmeticFilteringControlType(
                 map, ctx->tab_origin) == qorai_shields::ControlType::BLOCK;

  // HACK: after we fix multiple creations of QoraiRequestInfo we should
  // use only tab_origin. Since we recreate QoraiRequestInfo during consequent
  ...
  ctx->allow_referrers =
      map && qorai_shields::AreReferrersAllowed(
  ...
  // qorai_proxying_url_loader_factory.cc is refactored. See
  // QoraiProxyingURLLoaderFactory::InProgressRequest::UpdateRequestInfo().
  if (old_ctx) {
    ctx->internal_redirect = old_ctx->internal_redirect;
    ctx->redirect_source = old_ctx->redirect_source;
  }
