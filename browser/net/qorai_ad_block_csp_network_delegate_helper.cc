...
std::optional<std::string> GetCspDirectivesOnTaskRunner(
  ...
  std::optional<std::string> csp_directives =
      g_qorai_browser_process->ad_block_service()->GetCspDirectives(
          ctx->request_url, ctx->resource_type, source_host);

  qorai_shields::MergeCspDirectiveInto(original_csp, &csp_directives);
...
int OnHeadersReceived_AdBlockCspWork(
  ...
  if (!response_headers || !ctx->allow_qorai_shields || ctx->allow_ads) {
    return net::OK;
  }
