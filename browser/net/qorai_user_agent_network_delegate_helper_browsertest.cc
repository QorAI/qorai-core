...
class QoraiUserAgentNetworkDelegateBrowserTest
    : public InProcessBrowserTest,
      public ::testing::WithParamInterface<bool> {
 public:
  QoraiUserAgentNetworkDelegateBrowserTest() {
    auto* command_line = base::CommandLine::ForCurrentProcess();
    if (command_line->HasSwitch("enable-qorai-user-agent")) {
      feature_list_.InitAndEnableFeature(
          qorai_user_agent::features::kUseQoraiUserAgent);
    } else if (command_line->HasSwitch("disable-qorai-user-agent")) {
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    if (GetParam()) {
      command_line->AppendSwitch("enable-qorai-user-agent");
    } else {
      command_line->AppendSwitch("disable-qorai-user-agent");
    }
  }
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &QoraiUserAgentNetworkDelegateBrowserTest::HandleRequest,
  ...
  void ExpectHeaderBrands(const HeaderCapture& capture, bool feature_enabled) {
    ...
    if (feature_enabled) {
      ...
      EXPECT_EQ(!feature_enabled,
                base::Contains(*capture.sec_ch_ua, kQoraiBrand));
      EXPECT_EQ(
          !feature_enabled,
          base::Contains(*capture.sec_ch_ua_full_version_list, kQoraiBrand));
      EXPECT_EQ(feature_enabled,
...
void QoraiUserAgentNetworkDelegateBrowserTest::RegisterImagePageHandler() {
...
void QoraiUserAgentNetworkDelegateBrowserTest::HandleRequest(
    const net::test_server::HttpRequest& request) {
...
void QoraiUserAgentNetworkDelegateBrowserTest::StartTracking() {
  base::AutoLock auto_lock(header_lock_);
  start_tracking_ = true;
  header_capture_ = HeaderCapture{};
}

HeaderCapture QoraiUserAgentNetworkDelegateBrowserTest::header_capture() {
  base::AutoLock auto_lock(header_lock_);
  return header_capture_;
}

net::EmbeddedTestServer&
QoraiUserAgentNetworkDelegateBrowserTest::https_server() {
  return https_server_;
}

void QoraiUserAgentNetworkDelegateBrowserTest::NavigateAndWait(
    const GURL& url) {
...
void QoraiUserAgentNetworkDelegateBrowserTest::RunBrandHeaderTest(
...
IN_PROC_BROWSER_TEST_P(QoraiUserAgentNetworkDelegateBrowserTest,
