...
#include "qorai/components/qorai_shields/content/browser/ad_block_pref_service.h"
...
#include "qorai/content/public/browser/devtools/adblock_devtools_instumentation.h"
...
class AdblockCnameResolveHostClient : public network::mojom::ResolveHostClient {
 ...
 public:
  ...
  void OnComplete(
    ...
    UMA_HISTOGRAM_TIMES("Qorai.ShieldsCNAMEBlocking.TotalResolutionTime",
...
EngineFlags ShouldBlockRequestOnTaskRunner(
  ...
  SCOPED_UMA_HISTOGRAM_TIMER("Qorai.Adblock.ShouldBlockRequest");
  auto adblock_result =
      g_qorai_browser_process->ad_block_service()->ShouldStartRequest(
...
void OnShouldBlockRequestResult(
  ...
  if (ctx->blocked_by == kAdBlocked) {
    qorai_shields::QoraiShieldsWebContentsObserver::DispatchBlockedEvent(
        ctx->request_url, ctx->frame_tree_node_id, qorai_shields::kAds);
...
bool ProxySettingsAllowUncloaking(content::BrowserContext* browser_context,
  ...
  auto* ad_block_pref_service =
      qorai_shields::AdBlockPrefServiceFactory::GetForBrowserContext(
          browser_context);
...
int OnBeforeURLRequest_AdBlockTPPreWork(const ResponseCallback& next_callback,
  ...
  if (ctx->request_url.is_empty() ||
      ...
      !ctx->allow_qorai_shields || ctx->allow_ads ||
      ctx->resource_type == QoraiRequestInfo::kInvalidResourceType) {
    return net::OK;
  }
  ...
  // https://github.com/qorai/qorai-browser/issues/26302).
