...
QoraiProxyingWebSocket::QoraiProxyingWebSocket(
    WebSocketFactory factory,
    const network::ResourceRequest& request,
    content::FrameTreeNodeId frame_tree_node_id,
    content::BrowserContext* browser_context,
    scoped_refptr<RequestIDGenerator> request_id_generator,
    QoraiRequestHandler& handler,
...
// static
QoraiProxyingWebSocket* QoraiProxyingWebSocket::ProxyWebSocket(
...
void QoraiProxyingWebSocket::Start(
  ...
  forwarding_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&QoraiProxyingWebSocket::OnMojoConnectionError,
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &QoraiProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &QoraiProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }
...
QoraiProxyingWebSocket::CreateWebSocketFactory() {
  return base::BindOnce(&QoraiProxyingWebSocket::WebSocketFactoryRun,
                        weak_factory_.GetWeakPtr());
}

bool QoraiProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void QoraiProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void QoraiProxyingWebSocket::OnOpeningHandshakeStarted(
...
void QoraiProxyingWebSocket::ContinueToHeadersReceived() {
  auto continuation = base::BindRepeating(
      &QoraiProxyingWebSocket::OnHeadersReceivedComplete,
      weak_factory_.GetWeakPtr());
  ctx_ = qorai::QoraiRequestInfo::MakeCTX(request_, frame_tree_node_id_,
                                          request_id_, browser_context_, ctx_);
...
void QoraiProxyingWebSocket::OnFailure(const std::string& message,
                                       int32_t net_error,
                                       int32_t response_code) {}

void QoraiProxyingWebSocket::OnConnectionEstablished(
...
void QoraiProxyingWebSocket::OnAuthRequired(
...
void QoraiProxyingWebSocket::OnBeforeSendHeaders(
...
void QoraiProxyingWebSocket::OnHeadersReceived(
...
void QoraiProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &QoraiProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void QoraiProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
  ...
  auto continuation = base::BindRepeating(
      &QoraiProxyingWebSocket::OnBeforeSendHeadersComplete,
      weak_factory_.GetWeakPtr());
...
void QoraiProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void QoraiProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
...
void QoraiProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void QoraiProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &QoraiProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void QoraiProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void QoraiProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void QoraiProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void QoraiProxyingWebSocket::OnMojoConnectionError(
