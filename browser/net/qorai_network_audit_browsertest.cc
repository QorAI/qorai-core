...
class QoraiNetworkAuditTest : public InProcessBrowserTest {
 public:
  QoraiNetworkAuditTest() {
#if BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
    scoped_feature_list_.InitAndEnableFeature(playlist::features::kPlaylist);
#endif  // BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
  }

  QoraiNetworkAuditTest(const QoraiNetworkAuditTest&) = delete;
  QoraiNetworkAuditTest& operator=(const QoraiNetworkAuditTest&) = delete;
...
// Loads qorai://welcome first to simulate a first run and then loads another
// URL, waiting some time after each load to allow gathering network requests.
IN_PROC_BROWSER_TEST_F(QoraiNetworkAuditTest, BasicTests) {
  // Load the Welcome page.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("qorai://welcome")));
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("qorai://wallet")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
