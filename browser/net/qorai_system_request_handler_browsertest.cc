...
// Test to check if key is added for qorai apis
class QoraiSystemRequestHandlerBrowsertest : public PlatformBrowserTest {
 public:
  QoraiSystemRequestHandlerBrowsertest()
      : https_server_(net::test_server::EmbeddedTestServer::TYPE_HTTPS) {}

  ~QoraiSystemRequestHandlerBrowsertest() override = default;
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &QoraiSystemRequestHandlerBrowsertest::MonitorResourceRequest,
 ...
 private:
  void MonitorResourceRequest(const net::test_server::HttpRequest& request) {
    service_key_present_ = request.headers.count(kQoraiServicesKeyHeader) > 0;
  }
...
IN_PROC_BROWSER_TEST_F(QoraiSystemRequestHandlerBrowsertest,
                       CheckForQoraiServiceKey) {
  struct TestCase {
    std::string_view url;
    bool service_key_should_be_set;
  } test_cases[] = {
      {"demo.qorai.com", true},
      {"demo.qoraisoftware.com", true},
      {"qorai.demo.com", false},
