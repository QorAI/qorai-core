...
void CheckUrl(const std::string& test_url,
              ...
              int expected_error) {
  GURL new_url;
  int rc = qorai::OnBeforeURLRequest_BlockSafeBrowsingReportingURLs(
      GURL(test_url), &new_url);
  EXPECT_EQ(new_url, GURL(expected_url));
  EXPECT_EQ(rc, expected_error);
}

TEST(QoraiBlockReportingUrlsHelperTest, PreserveNormalUrls) {
...
TEST(QoraiBlockReportingUrlsHelperTest, CancelReportingUrl) {
