...
#ifndef QORAI_BROWSER_NET_URL_CONTEXT_H_
#define QORAI_BROWSER_NET_URL_CONTEXT_H_
...
int OnBeforeURLRequest(const qorai::ResponseCallback& next_callback,
                       std::shared_ptr<qorai::QoraiRequestInfo> ctx);
}  // namespace qorai_rewards

namespace qorai {

enum QoraiNetworkDelegateEventType {
  kOnBeforeRequest,
...
struct QoraiRequestInfo {
  QoraiRequestInfo();
  QoraiRequestInfo(const QoraiRequestInfo&) = delete;
  QoraiRequestInfo& operator=(const QoraiRequestInfo&) = delete;

  // For tests, should not be used directly.
  explicit QoraiRequestInfo(const GURL& url);
  ...
  QoraiNetworkDelegateEventType event_type = kUnknownEventType;
  ...
  static std::shared_ptr<qorai::QoraiRequestInfo> MakeCTX(
 ...
 private:
  ...
  friend class ::QoraiRequestHandler;
...
#endif  // QORAI_BROWSER_NET_URL_CONTEXT_H_
