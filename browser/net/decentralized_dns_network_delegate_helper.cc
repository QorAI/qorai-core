...
int OnBeforeURLRequest_DecentralizedDnsPreRedirectWork(
  ...
  // Check if Qorai Wallet is disabled by policy - if so, disable decentralized
  // DNS
  auto* prefs = user_prefs::UserPrefs::Get(ctx->browser_context);
  if (!qorai_wallet::IsAllowed(prefs)) {
    return net::OK;
  }
  ...
  auto* json_rpc_service = qorai_wallet_service->json_rpc_service();
...
void OnBeforeURLRequest_EnsRedirectWork(
  ...
  if (error != qorai_wallet::mojom::ProviderError::kSuccess) {
    next_callback.Run();
    return;
  }
...
void OnBeforeURLRequest_SnsRedirectWork(
    ...
    const std::string& error_message) {
  if (error == qorai_wallet::mojom::SolanaProviderError::kSuccess && url &&
      url->is_valid()) {
...
void OnBeforeURLRequest_UnstoppableDomainsRedirectWork(
    ...
    qorai_wallet::mojom::ProviderError error,
    const std::string& error_message) {
  if (error == qorai_wallet::mojom::ProviderError::kSuccess && url &&
      url->is_valid()) {
    ctx->new_url_spec = url->spec();
  }
