...
class QoraiSiteHacksNetworkDelegateHelperTest : public testing::Test {
 public:
  QoraiSiteHacksNetworkDelegateHelperTest() {
  ...
  std::shared_ptr<qorai::QoraiRequestInfo> MakeRequest(const GURL& url) {
    auto request = std::make_shared<qorai::QoraiRequestInfo>(url);
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, UANotAllowedTest) {
  const std::vector<GURL> urls({GURL("https://brianbondy.com"),
                                GURL("https://qoraicombo.com"),
                                GURL("https://qorai.example.com"),
  ...
  for (const auto& url : urls) {
    ...
    auto qorai_request_info = MakeRequest(url);
    int rc = qorai::OnBeforeStartTransaction_SiteHacksWork(
        &headers, ResponseCallback(), qorai_request_info);
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, ReferrerPreserved) {
  ...
  for (const auto& url : urls) {
    ...
    auto qorai_request_info = MakeRequest(url);
    qorai_request_info->referrer = original_referrer;
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
    // new_url should not be set.
    EXPECT_TRUE(qorai_request_info->new_url_spec.empty());
    EXPECT_EQ(qorai_request_info->referrer, original_referrer);
  }
}

TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, ReferrerTruncated) {
  ...
  for (const auto& url : urls) {
    ...
    // new_url should not be set.
    EXPECT_TRUE(qorai_request_info->new_url_spec.empty());
    EXPECT_TRUE(qorai_request_info->new_referrer.has_value());
    EXPECT_EQ(qorai_request_info->new_referrer.value(),
              url::Origin::Create(original_referrer).GetURL());
  }
}

TEST_F(QoraiSiteHacksNetworkDelegateHelperTest,
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, OnionReferrerStripped) {
  const GURL original_referrer(
      "https://"
      "qorai4u7jddbv7cyviptqjc7jusxh72uik7zt6adtckl5f4nwy2v72qd.onion/");
  const GURL destination("https://qorai.com");
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, QueryStringUntouched) {
  ...
  for (const auto& url : urls) {
    auto qorai_request_info = MakeRequest(GURL(url));
    qorai_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    qorai_request_info->method = "GET";
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
    // new_url should not be set
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, QueryStringExempted) {
  ...
  for (const auto& initiator : initiators) {
    auto qorai_request_info = MakeRequest(tracking_url);
    qorai_request_info->initiator_url = GURL(initiator);
    qorai_request_info->method = "GET";
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
  ...
  // Internal redirect
  {
    ...
    qorai_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    qorai_request_info->method = "GET";
    qorai_request_info->internal_redirect = true;
    qorai_request_info->redirect_source =
  ...
  // POST requests
  {
    ...
    qorai_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    qorai_request_info->method = "POST";
    qorai_request_info->redirect_source =
        GURL("https://example.org");  // cross-site
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest, QueryStringFiltered) {
  const std::vector<std::pair<std::string, std::string>> urls(
      {// { original url, expected url after filtering ("" means unchanged) }
       ...
       {"https://qorai.com/?ref_src=example.com", ""},
  ...
  for (const auto& pair : urls) {
    auto qorai_request_info = MakeRequest(GURL(pair.first));
    qorai_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    qorai_request_info->method = "GET";
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(qorai_request_info->new_url_spec, pair.second);
  }
  ...
  // Cross-site redirect
  {
    ...
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(qorai_request_info->new_url_spec, "https://example.com/");
  }
  ...
  // Direct navigation
  {
    ...
    qorai_request_info->initiator_url = GURL();
    qorai_request_info->method = "GET";
    int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     qorai_request_info);
    EXPECT_EQ(rc, net::OK);
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest,
  ...
  auto qorai_request_info = MakeRequest(GURL("https://example.com/?fbclid=1"));
  qorai_request_info->initiator_url = GURL("https://example.net");
  qorai_request_info->method = "GET";
  int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                   qorai_request_info);
  EXPECT_EQ(rc, net::OK);
...
TEST_F(QoraiSiteHacksNetworkDelegateHelperTest,
       QueryParametersFilteringEnabledByPolicyWhenQoraiShieldsDisabled) {
  GetPrefs()->SetManagedPref(
      query_filter::kTrackingQueryParametersFilteringEnabled,
      base::Value(true));
  ...
  qorai_request_info->initiator_url = GURL("https://example.net");
  qorai_request_info->method = "GET";
  qorai_request_info->allow_qorai_shields = false;
  int rc = qorai::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                   qorai_request_info);
  EXPECT_EQ(rc, net::OK);

  // new_url should not be set because Qorai Shields is disabled
  EXPECT_TRUE(qorai_request_info->new_url_spec.empty());
}
