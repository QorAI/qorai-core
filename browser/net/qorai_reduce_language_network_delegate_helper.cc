...
static constexpr auto kFarbleAcceptLanguageExceptions =
    base::MakeFixedFlatSet<std::string_view>(
        base::sorted_unique,
        {
            // https://github.com/qorai/qorai-browser/issues/26325
            "aeroplan.rewardops.com",
            // https://github.com/qorai/qorai-browser/issues/31196
            "login.live.com",
            // https://github.com/qorai/qorai-browser/issues/25309
            "ulta.com",
            "www.ulta.com",
        });
}  // namespace
...
std::string FarbleAcceptLanguageHeader(
  ...
  auto* qorai_farbling_service =
      QoraiFarblingServiceFactory::GetForProfile(profile);
  if (qorai_farbling_service &&
      qorai_farbling_service->MakePseudoRandomGeneratorForURL(origin_url,
                                                              &prng)) {
...
int OnBeforeStartTransaction_ReduceLanguageWork(
  ...
  if (!qorai_shields::ShouldDoReduceLanguage(content_settings, origin_url,
                                             profile->GetPrefs())) {
    return net::OK;
  }
  ...
  if (headers->HasHeader(net::HttpRequestHeaders::kAcceptLanguage)) {
    ...
    // https://github.com/qorai/qorai-browser/issues/28945
    return net::OK;
  }

  std::string accept_language_string;
  switch (qorai_shields::GetFingerprintingControlType(content_settings,
                                                      origin_url)) {
