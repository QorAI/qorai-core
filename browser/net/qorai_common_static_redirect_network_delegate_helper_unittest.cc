...
TEST(QoraiCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  EXPECT_EQ(redirect.host(), kQoraiRedirectorProxy);
...
TEST(QoraiCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  int rc = OnBeforeURLRequest_CommonStaticRedirectWork(ResponseCallback(),
                                                       request_info);
  const GURL redirect = GURL(request_info->new_url_spec);
  EXPECT_EQ(redirect.host(), kQoraiClients4Proxy);
  EXPECT_TRUE(redirect.SchemeIs(url::kHttpsScheme));
  EXPECT_EQ(redirect.path(), url.path());
  EXPECT_EQ(rc, net::OK);
}
...
TEST(QoraiCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  EXPECT_TRUE(redirect.SchemeIs(url::kHttpsScheme));
  EXPECT_EQ(redirect.path(), "/qorai/qorai-browser/issues/new");
  EXPECT_EQ(redirect.query(),
            "title=Crash%20Report&labels=crash&body=IMPORTANT%20Qorai");
  EXPECT_EQ(rc, net::OK);
  ...
  request_info = std::make_shared<qorai::QoraiRequestInfo>(url_fewer_keys);
  ...
  request_info = std::make_shared<qorai::QoraiRequestInfo>(url_wrong_keys);
  rc = OnBeforeURLRequest_CommonStaticRedirectWork(ResponseCallback(),
                                                   request_info);
  EXPECT_TRUE(request_info->new_url_spec.empty());
  EXPECT_EQ(rc, net::OK);
