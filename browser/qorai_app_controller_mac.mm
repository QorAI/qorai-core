...
#import "qorai/browser/qorai_app_controller_mac.h"
...
class TorPrefObserver : public BooleanPrefMember {
 public:
  TorPrefObserver(QorAIAppController* controller, NSMenu* menu)
    ...
    // https://github.com/qorai/qorai-core/pull/21892#issuecomment-1928944343
 ...
 private:
  ...
  QorAIAppController* controller_;  // Owner of this
  NSMenu* menu_;
...
@implementation QorAIAppController
...
- (BOOL)shouldShowCleanLinkItem {
  return qorai::HasSelectedURL([self getBrowser]);
}
...
- (void)executeCommand:(id)sender withProfile:(Profile*)profile {
  ...
  if (tag == IDC_COPY_CLEAN_LINK) {
    qorai::CleanAndCopySelectedURL([self getBrowser]);
    return;
  }

#if BUILDFLAG(ENABLE_TOR)
  if (tag == IDC_NEW_OFFTHERECORD_WINDOW_TOR) {
    qorai::NewOffTheRecordWindowTor(profile);
    return;
  }
#endif  // BUILDFLAG(ENABLE_TOR)
...
@end  // @implementation QorAIAppController
