...
QoraiBrowserMainPartsMac::QoraiBrowserMainPartsMac(bool is_integration_test,
                                                   StartupData* startup_data)
    : ChromeBrowserMainPartsMac(is_integration_test, startup_data) {}

QoraiBrowserMainPartsMac::~QoraiBrowserMainPartsMac() = default;

void QoraiBrowserMainPartsMac::PreCreateMainMessageLoop() {
  ChromeBrowserMainPartsMac::PreCreateMainMessageLoop();

  bool use_omaha4 = false;
#if BUILDFLAG(ENABLE_OMAHA4)
  use_omaha4 = qorai_updater::ShouldUseOmaha4();
#endif

  if (base::FeatureList::IsEnabled(qorai::kUpgradeWhenIdle)) {
    if (use_omaha4) {
      upgrade_when_idle_ = std::make_unique<qorai::UpgradeWhenIdle>(
...
void QoraiBrowserMainPartsMac::PostProfileInit(Profile* profile,
...
void QoraiBrowserMainPartsMac::PostMainMessageLoopRun() {
