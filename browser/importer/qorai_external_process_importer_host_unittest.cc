...
class QoraiExternalProcessImporterHostUnitTest : public testing::Test {
 public:
  QoraiExternalProcessImporterHostUnitTest() = default;

  void SetUp() override {
    TestingProfile::Builder profile_builder;
    EXPECT_TRUE(qorai_profile_dir_.CreateUniqueTempDir());
    profile_builder.SetPath(GetProductProfilePath("Qorai"));
  ...
  base::FilePath GetProductProfilePath(const std::string& product) {
    return qorai_profile_dir_.GetPath()
  ...
  void LaunchExtensionsImportAndWait(
    ...
    // QoraiExternalProcessImporterHost uses `delete this`.
    auto* external_process_host = new QoraiExternalProcessImporterHost();
  ...
  std::string ReadTargetStore(const std::string& id) {
    return ReadStore(GetExtensionLocalSettingsPath("Qorai", id), id);
  }

  std::string ReadTargetIndexedDB(const std::string& id,
                                  const std::string& type) {
    base::ScopedAllowBlockingForTesting allow_io;
    const auto base_path = GetProductProfilePath("Qorai")
...
TEST_F(QoraiExternalProcessImporterHostUnitTest, ImportEtensionsSettings) {
  user_data_importer::SourceProfile source_profile;
