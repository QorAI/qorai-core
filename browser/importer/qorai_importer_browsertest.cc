...
class QoraiImporterBrowserTest : public InProcessBrowserTest {
 public:
...
IN_PROC_BROWSER_TEST_F(QoraiImporterBrowserTest, ImportExtensions) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    for (bool reimport : {false, true}) {
      ...
      // Deletes itself.
      auto* host = new QoraiExternalProcessImporterHost;
...
IN_PROC_BROWSER_TEST_F(QoraiImporterBrowserTest, ReImportExtensions) {
  auto* source_profile = CreateProfile();
  AddTestExtension(source_profile, kExtensionId);
