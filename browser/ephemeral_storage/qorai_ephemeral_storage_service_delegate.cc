...
#include "qorai/browser/ephemeral_storage/qorai_ephemeral_storage_service_delegate.h"
...
QoraiEphemeralStorageServiceDelegate::QoraiEphemeralStorageServiceDelegate(
...
void QoraiEphemeralStorageServiceDelegate::CleanupTLDEphemeralArea(
  ...
  if (qorai_shields::GetCookieControlType(host_content_settings_map_,
...
void QoraiEphemeralStorageServiceDelegate::CleanupFirstPartyStorageArea(
    const TLDEphemeralAreaKey& key) {
  DVLOG(1) << __func__ << " " << key.first << " " << key.second;
...
void QoraiEphemeralStorageServiceDelegate::RegisterFirstWindowOpenedCallback(
...
#if !BUILDFLAG(IS_ANDROID)
void QoraiEphemeralStorageServiceDelegate::OnBrowserAdded(Browser* browser) {
