...
// design document at:
// https://github.com/qorai/qorai-browser/wiki/Ephemeral-Storage-Design
EphemeralStorageTabHelper::EphemeralStorageTabHelper(WebContents* web_contents)
  ...
  DCHECK(base::FeatureList::IsEnabled(net::features::kQoraiEphemeralStorage));
...
void EphemeralStorageTabHelper::UpdateShieldsState(const GURL& url) {
  ...
  const bool shields_enabled =
      qorai_shields::GetQoraiShieldsEnabled(host_content_settings_map_, url);
  const bool cookies_restricted =
      qorai_shields::GetCookieControlType(host_content_settings_map_,
