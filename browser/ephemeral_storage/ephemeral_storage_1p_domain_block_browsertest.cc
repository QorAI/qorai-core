...
class EphemeralStorage1pDomainBlockBrowserTest
 ...
 public:
  ...
  void UpdateAdBlockInstanceWithRules(const std::string& rules) {
    source_provider_ =
        std::make_unique<qorai_shields::TestFiltersProvider>(rules);
  ...
  void WaitForAdBlockServiceThreads() {
    scoped_refptr<base::ThreadTestHelper> tr_helper(new base::ThreadTestHelper(
        g_qorai_browser_process->local_data_files_service()->GetTaskRunner()));
  ...
  WebContents* BlockAndNavigateToBlockedDomain(const GURL& url,
    ...
    if (is_aggressive) {
      qorai_shields::SetCosmeticFilteringControlType(
          content_settings(), qorai_shields::ControlType::BLOCK, url);
    }
 ...
 protected:
  std::unique_ptr<qorai_shields::TestFiltersProvider> source_provider_;
