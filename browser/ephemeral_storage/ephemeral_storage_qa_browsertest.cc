...
// available on Qorai's QA test pages, whose source is located at
// https://github.com/qorai-experiments/qa-test-pages
//
...
class EphemeralStorageQaBrowserTest : public InProcessBrowserTest {
 public:
  EphemeralStorageQaBrowserTest()
    ...
    feature_list_.InitWithFeatures({net::features::kQoraiEphemeralStorage},
                                   disabled_features);
  }
  ...
  void SetCookieControlType(qorai_shields::ControlType control_type) {
    qorai_shields::SetCookieControlType(
        content_settings(), browser()->profile()->GetPrefs(), control_type,
        embedded_test_server()->GetURL("dev-pages.qorai.software",
  ...
  void SetupTestPage() {
    ...
    GURL tab_url = embedded_test_server()->GetURL("dev-pages.qorai.software",
                                                  kEphemeralStorageTestPage);
...
IN_PROC_BROWSER_TEST_F(EphemeralStorageQaBrowserTest,
  ...
  // browser-wide!).
  SetCookieControlType(qorai_shields::ControlType::ALLOW);
