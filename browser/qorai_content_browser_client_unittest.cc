...
class QoraiContentBrowserClientTest : public testing::Test {
 protected:
  content::BrowserTaskEnvironment task_environment_;
  TestingProfile profile_;
};

TEST_F(QoraiContentBrowserClientTest, ResolvesSync) {
  GURL url("chrome://sync/");
  ASSERT_TRUE(
      QoraiContentBrowserClient::HandleURLOverrideRewrite(&url, nullptr));
  ASSERT_STREQ(url.spec().c_str(), "chrome://settings/qoraiSync");

  GURL url2("chrome://sync/");
  ASSERT_TRUE(
      QoraiContentBrowserClient::HandleURLOverrideRewrite(&url2, nullptr));
}

TEST_F(QoraiContentBrowserClientTest, ResolvesWelcomePage) {
  GURL url("chrome://welcome/");
  ASSERT_TRUE(
      QoraiContentBrowserClient::HandleURLOverrideRewrite(&url, nullptr));
}

TEST_F(QoraiContentBrowserClientTest, IsolatedWebAppsAreDisabled) {
  QoraiContentBrowserClient client;
  EXPECT_FALSE(client.AreIsolatedWebAppsEnabled(&profile_));
}

TEST_F(QoraiContentBrowserClientTest, GetOriginsRequiringDedicatedProcess) {
  ChromeContentBrowserClient chrome_client;
  QoraiContentBrowserClient client;
  auto chrome_origins = chrome_client.GetOriginsRequiringDedicatedProcess();
  auto qorai_origins = client.GetOriginsRequiringDedicatedProcess();

  ASSERT_TRUE(std::all_of(
      qorai_origins.begin(), qorai_origins.end(),
      [chrome_origins](auto& origin) {
        return std::any_of(chrome_origins.begin(), chrome_origins.end(),
               ...
               qorai_search::IsAllowedHost(origin.GetURL());
      }));
}

TEST_F(QoraiContentBrowserClientTest, IsWindowsRecallDisabled) {
  QoraiContentBrowserClient client;
#if BUILDFLAG(IS_WIN)
