...
bool IsBackupAvailableFor(
  ...
  CHECK(extensions_mv2::IsKnownQoraiHostedExtension(qorai_hosted_extension_id));

  const auto webstore_extension_id =
      extensions_mv2::GetWebStoreHostedExtensionId(qorai_hosted_extension_id);
...
bool ClearExtensionSettingsOnFileThread(
  ...
  const auto local_settings =
      GetLocalSettings(qorai_hosted_extension_id, profile_dir);
  if (!base::DeletePathRecursively(local_settings)) {
    return false;
  }

  auto indexeddb_settings =
      GetIndexedSettings(qorai_hosted_extension_id, profile_dir);
  bool error = false;
...
void ImportExtensionSettingsOnFileThread(
    const extensions::ExtensionId& qorai_hosted_extension_id,
    const base::Version& qorai_hosted_extension_version,
    const base::FilePath& profile_dir) {
  if (!IsBackupAvailableFor(qorai_hosted_extension_id, profile_dir)) {
    return;
  }
  ...
  base::ScopedClosureRunner clear_settings_on_error(base::BindOnce(
      [](const extensions::ExtensionId& qorai_hosted_extension_id,
         const base::FilePath& profile_dir) {
        ClearExtensionSettingsOnFileThread(qorai_hosted_extension_id,
                                           profile_dir);
      },
      qorai_hosted_extension_id, profile_dir));

  const bool error = MoveExtensionSettings(webstore_extension_id, backup_path,
                                           qorai_hosted_extension_id,
...
void ExtensionsManifestV2Migrator::OnExtensionInstalled(
  ...
  if (is_updates ||
      !extensions_mv2::IsKnownQoraiHostedExtension(extension->id())) {
    return;
  }
