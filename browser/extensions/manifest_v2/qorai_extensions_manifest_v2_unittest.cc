...
struct QoraiExtensionsManifestV2Test
...
TEST_F(QoraiExtensionsManifestV2Test, CheckInstallVerifier) {
  struct {
...
class QoraiExtensionsManifestV2DeprecationTest
    : public QoraiExtensionsManifestV2Test,
      public ::testing::WithParamInterface<extensions::MV2ExperimentStage> {
 public:
  QoraiExtensionsManifestV2DeprecationTest() {
    switch (GetParam()) {
  ...
  ~QoraiExtensionsManifestV2DeprecationTest() = default;
...
TEST_P(QoraiExtensionsManifestV2DeprecationTest, CheckStages) {
...
TEST_P(QoraiExtensionsManifestV2DeprecationTest,
...
class QoraiExtensionsManifestV2SettingsBackupTest
    : public extensions::ExtensionServiceTestBase,
      public testing::WithParamInterface<TestCase> {
 public:
  QoraiExtensionsManifestV2SettingsBackupTest() {
  ...
  ~QoraiExtensionsManifestV2SettingsBackupTest() override = default;
...
TEST_P(QoraiExtensionsManifestV2SettingsBackupTest, BackupSettings) {
  if (!GetParam().feature_enabled) {
  ...
  {
    // Install uBlock from Qorai host.
    auto extension =
        extensions::ExtensionBuilder("test")
    ...
    if (GetParam().backup_enabled && GetParam().import_enabled) {
      auto check_settings_dir = [this](const base::FilePath& path) {
        ...
        cws_settings.ForEach([&checked, this](const base::FilePath& path) {
          checked = true;
          auto qorai_hosted_name = path.BaseName().value();
          ...
          EXPECT_TRUE(AreDirectoriesEqual(
              path, path.DirName().Append(qorai_hosted_name)));
        });
        EXPECT_TRUE(checked);
      };
