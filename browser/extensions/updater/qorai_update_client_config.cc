...
std::string QoraiUpdateClientConfig::GetChannel() const {
  return std::string("stable");
}

scoped_refptr<update_client::NetworkFetcherFactory>
QoraiUpdateClientConfig::GetNetworkFetcherFactory() {
...
QoraiUpdateClientConfig::~QoraiUpdateClientConfig() = default;

// static
scoped_refptr<ChromeUpdateClientConfig> QoraiUpdateClientConfig::Create(
    content::BrowserContext* context,
    std::optional<GURL> update_url_override) {
  FactoryCallback& factory = GetFactoryCallback();
  return factory.is_null() ? base::MakeRefCounted<QoraiUpdateClientConfig>(
