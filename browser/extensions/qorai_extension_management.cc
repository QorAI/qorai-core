...
#include "qorai/browser/extensions/qorai_extension_management.h"
...
QoraiExtensionManagement::QoraiExtensionManagement(Profile* profile)
  ...
  providers_.push_back(std::make_unique<QoraiExtensionProvider>());
  if (g_browser_process->local_state()) {
    local_state_pref_change_registrar_.Init(g_browser_process->local_state());
#if BUILDFLAG(ENABLE_TOR)
    local_state_pref_change_registrar_.Add(
        tor::prefs::kTorDisabled,
        base::BindRepeating(&QoraiExtensionManagement::OnTorDisabledChanged,
                            base::Unretained(this)));
    local_state_pref_change_registrar_.Add(
        tor::prefs::kBridgesConfig,
        base::BindRepeating(
            &QoraiExtensionManagement::OnTorPluggableTransportChanged,
            base::Unretained(this)));
#endif
  }
  Cleanup(profile);
...
QoraiExtensionManagement::~QoraiExtensionManagement() {
  local_state_pref_change_registrar_.RemoveAll();
}

void QoraiExtensionManagement::OnTorDisabledChanged() {
#if BUILDFLAG(ENABLE_TOR)
  if (TorProfileServiceFactory::IsTorDisabled(profile_)) {
    TorProfileManager::GetInstance().CloseAllTorWindows();
    if (g_qorai_browser_process->tor_client_updater()) {
      g_qorai_browser_process->tor_client_updater()->Cleanup();
    }
    if (g_qorai_browser_process->tor_pluggable_transport_updater()) {
      g_qorai_browser_process->tor_pluggable_transport_updater()->Cleanup();
    }
  }
#endif
}

void QoraiExtensionManagement::OnTorPluggableTransportChanged() {
#if BUILDFLAG(ENABLE_TOR)
...
void QoraiExtensionManagement::Cleanup(content::BrowserContext* context) {
