...
void RegisterGCMProfilePrefs(user_prefs::PrefRegistrySyncable* registry) {
  ...
  // Current Qorai equivalent of the deprecated pref.
  registry->RegisterBooleanPref(kQoraiGCMChannelStatus, false);
}
...
void MigrateGCMPrefs(PrefService* prefs) {
  ...
  if (pref && !pref->IsDefaultValue())
    prefs->SetBoolean(kQoraiGCMChannelStatus, pref->GetValue()->GetBool());
