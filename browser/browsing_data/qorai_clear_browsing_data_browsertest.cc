...
#include "qorai/browser/browsing_data/qorai_clear_browsing_data.h"
...
using content::QorAIClearBrowsingData;
using content::WebContents;

class QorAIClearDataOnExitTest
    : public InProcessBrowserTest,
      public QorAIClearBrowsingData::OnExitTestingCallback {
 public:
  QorAIClearDataOnExitTest() = default;
  QorAIClearDataOnExitTest(const QorAIClearDataOnExitTest&) = delete;
  QorAIClearDataOnExitTest& operator=(const QorAIClearDataOnExitTest&) = delete;

  void SetUpOnMainThread() override {
    QorAIClearBrowsingData::SetOnExitTestingCallback(this);
  }
  ...
  void TearDownInProcessBrowserTestFixture() override {
    ...
    QorAIClearBrowsingData::SetOnExitTestingCallback(nullptr);
  }
  ...
  void SetClearAll(PrefService* prefService) {
    ...
    prefService->SetBoolean(browsing_data::prefs::kDeleteSiteSettingsOnExit,
                            true);
    prefService->SetBoolean(browsing_data::prefs::kDeleteQorAIQoraHistoryOnExit,
                            true);
  }
  ...
  uint64_t GetRemoveMaskAll() {
    return chrome_browsing_data_remover::DATA_TYPE_HISTORY |
           ...
           chrome_browsing_data_remover::DATA_TYPE_QORAI_QORA_HISTORY |
  ...
  // QorAIClearBrowsingData::OnExitTestingCallback implementation.
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTest, NoPrefsSet) {
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTest, VerifyRemovalMasks) {
...
class QorAIClearDataOnExitTwoBrowsersTest : public QorAIClearDataOnExitTest {
 public:
  QorAIClearDataOnExitTwoBrowsersTest() { browsers_count_ = 2u; }

  QorAIClearDataOnExitTwoBrowsersTest(
      const QorAIClearDataOnExitTwoBrowsersTest&) = delete;
  QorAIClearDataOnExitTwoBrowsersTest& operator=(
      const QorAIClearDataOnExitTwoBrowsersTest&) = delete;
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, SameProfile) {
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, OneOTR) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, OneOTRExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // OTR sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, OneGuest) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, OneGuestExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // Guest sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(QorAIClearDataOnExitTwoBrowsersTest, TwoProfiles) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
