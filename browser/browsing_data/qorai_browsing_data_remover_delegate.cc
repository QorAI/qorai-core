...
// https://github.com/qorai/qorai-browser/issues/44327.
  ...
  explicit ContentSettingsDefaultsKeeper(Profile* profile) : profile_(profile) {
    ...
    for (const auto content_type : shields_content_types) {
      if (content_type == ContentSettingsType::QORAI_COOKIES) {
        // QORAI_COOKIES don't have own default values. They relies on the
 ...
 private:
  ...
  void RestoreSetting(HostContentSettingsMap* map,
    ...
    if (content_type == ContentSettingsType::QORAI_FINGERPRINTING_V2 &&
      ...
      // as outlined in
      // `QoraiPrefProvider::MigrateFingerprintingSettingsToOriginScoped`.
      ...
      map->SetWebsiteSettingCustomScope(
          ...
          ContentSettingsType::QORAI_FINGERPRINTING_V2, base::Value(), {});
    } else {
...
QoraiBrowsingDataRemoverDelegate::QoraiBrowsingDataRemoverDelegate(
    content::BrowserContext* browser_context)
    : ChromeBrowsingDataRemoverDelegate(browser_context),
      profile_(Profile::FromBrowserContext(browser_context)) {}

QoraiBrowsingDataRemoverDelegate::~QoraiBrowsingDataRemoverDelegate() = default;

void QoraiBrowsingDataRemoverDelegate::RemoveEmbedderData(
  ...
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_HISTORY) {
    // Qorai News feed cache
    if (auto* qorai_news_controller =
            qorai_news::QoraiNewsControllerFactory::GetForBrowserContext(
                profile_)) {
      qorai_news_controller->ClearHistory();
    }
  ...
  // QoraiClearBrowsingDataFragment::onClearBrowsingData(). It's done
  ...
  // functions
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_QORAI_QORA_HISTORY) {
    ai_chat::AIChatService* ai_chat_service =
        ai_chat::AIChatServiceFactory::GetForBrowserContext(profile_);
    if (ai_chat_service) {
  ...
  if ((remove_mask & content::BrowsingDataRemover::DATA_TYPE_COOKIES) ||
    ...
    host_content_settings_map->ClearSettingsForOneTypeWithPredicate(
        ContentSettingsType::QORAI_SHIELDS_METADATA, delete_begin, delete_end,
        website_settings_filter);
  }
}

void QoraiBrowsingDataRemoverDelegate::ClearShieldsSettings(
  ...
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile_);
  auto* provider =
      static_cast<content_settings::QoraiPrefProvider*>(map->GetPrefProvider());
