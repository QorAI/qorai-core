...
#include "qorai/browser/qorai_browser_main_parts.h"
...
#include "qorai/components/speedreader/speedreader_extended_info_handler.h"
...
#include "qorai/components/ipfs/ipfs_component_cleaner.h"
...
void ChromeBrowserMainParts::PostBrowserStart() {
  ...
  for (Profile* profile : profile_manager->GetLoadedProfiles()) {
    const base::FilePath tor_legacy_session_path =
        profile->GetPath()
            .Append(qorai::kSessionProfileDir)
  ...
  if (browser) {
    ...
    if (active_web_contents) {
      ...
      if (profile && infobar_manager) {
        QoraiConfirmP3AInfoBarDelegate::Create(
...
void ChromeBrowserMainParts::PreShutdown() {
  content::QoraiClearBrowsingData::ClearOnExit();
...
void ChromeBrowserMainParts::PreProfileInit() {
  ...
  if (!base::FeatureList::IsEnabled(qorai_sync::features::kQoraiSync)) {
