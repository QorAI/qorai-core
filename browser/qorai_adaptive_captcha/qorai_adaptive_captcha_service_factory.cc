...
class CaptchaDelegate
    : public qorai_adaptive_captcha::QorAIAdaptiveCaptchaDelegate {
 public:
...
QorAIAdaptiveCaptchaServiceFactory::GetInstance() {
  static base::NoDestructor<QorAIAdaptiveCaptchaServiceFactory> instance;
  return instance.get();
}

// static
QorAIAdaptiveCaptchaService* QorAIAdaptiveCaptchaServiceFactory::GetForProfile(
    Profile* profile) {
  if (!profile->IsRegularProfile()) {
    return nullptr;
  }

  return static_cast<QorAIAdaptiveCaptchaService*>(
      GetInstance()->GetServiceForBrowserContext(profile, true));
}

QorAIAdaptiveCaptchaServiceFactory::QorAIAdaptiveCaptchaServiceFactory()
...
QorAIAdaptiveCaptchaServiceFactory::BuildServiceInstanceForBrowserContext(
    content::BrowserContext* context) const {
  auto url_loader_factory = context->GetDefaultStoragePartition()
                                ->GetURLLoaderFactoryForBrowserProcess();
  return std::make_unique<QorAIAdaptiveCaptchaService>(
      user_prefs::UserPrefs::Get(context), std::move(url_loader_factory),
      qorai_rewards::RewardsServiceFactory::GetForProfile(
