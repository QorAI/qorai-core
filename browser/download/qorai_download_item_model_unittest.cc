...
#include "qorai/browser/download/qorai_download_item_model.h"
...
class QoraiDownloadItemModelTest : public testing::Test {
 public:
  QoraiDownloadItemModelTest() : model_(&item_), qorai_model_(&model_) {}
  ~QoraiDownloadItemModelTest() override = default;
 ...
 protected:
  ...
  QoraiDownloadItemModel& model() { return qorai_model_; }

 private:
  NiceMock<download::MockDownloadItem> item_;
  DownloadItemModel model_;
  QoraiDownloadItemModel qorai_model_;
};

}  // namespace

TEST_F(QoraiDownloadItemModelTest, GetOriginUrlText) {
...
TEST_F(QoraiDownloadItemModelTest, GetTooltipText) {
...
TEST_F(QoraiDownloadItemModelTest, NoDownloadItem) {
  ...
  QoraiDownloadItemModel qorai_model(&model);
  bool is_secure = false;
  EXPECT_EQ("", base::UTF16ToUTF8(qorai_model.GetOriginURLText(&is_secure)));
}
