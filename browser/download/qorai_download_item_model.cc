...
QoraiDownloadItemModel::QoraiDownloadItemModel(DownloadUIModel* model)
    : model_(model) {}

QoraiDownloadItemModel::~QoraiDownloadItemModel() = default;

// Adds origin url to the tooltip text and "Not secure", if needed.
std::u16string QoraiDownloadItemModel::GetTooltipText() {
...
std::u16string QoraiDownloadItemModel::GetOriginURLText(bool* is_secure) {
  *is_secure = false;
    ...
    *is_secure = network::IsUrlPotentiallyTrustworthy(gurl);
    if (gurl.SchemeIs(url::kAboutScheme)) {
      ...
      if (gurl.SchemeIsFile()) {
        ...
        // case, see the QoraiDownloadItemModelTest.GetOriginUrlText unit test.
        origin = "file:///";
      }
    }
  } else {
