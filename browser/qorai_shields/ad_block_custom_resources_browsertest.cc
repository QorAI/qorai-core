...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, Add) {
  ...
  NavigateToURL(GURL("qorai://settings/shields/filters"));
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, Edit) {
  ...
  base::test::TestFuture<
      qorai_shields::AdBlockCustomResourceProvider::ErrorCode>
      result;
  g_qorai_browser_process->ad_block_service()
      ->custom_resource_provider()
      ->AddResource(profile()->GetPrefs(),
                    ...
                    result.GetCallback());
  EXPECT_EQ(qorai_shields::AdBlockCustomResourceProvider::ErrorCode::kOk,
            result.Get());
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, NameConflicts) {
  ...
  constexpr const char kQoraiFix[] = "window.test = 'default-script'";
  constexpr const char kQoraiFixResource[] = R"json(
    [{
      "name": "user-Fix.js",
  ...
  UpdateAdBlockResources(base::ReplaceStringPlaceholders(
      kQoraiFixResource, {base::Base64Encode(kQoraiFix)}, nullptr));
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, TwoProfiles) {
  ...
  content::NavigateToURLBlockUntilNavigationsComplete(
      second_web_contents, GURL("qorai://settings/shields/filters"), 1, true);
  ...
  // is disabled.
  EXPECT_FALSE(
      qorai_shields::IsDeveloperModeEnabled(second_profile.GetPrefs()));
