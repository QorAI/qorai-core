...
#include "qorai/browser/qorai_shields/ad_block_service_browsertest.h"
...
#include "qorai/components/qorai_shields/core/browser/ad_block_default_resource_provider.h"
#include "qorai/components/qorai_shields/core/browser/qorai_shields_utils.h"
#include "qorai/components/qorai_shields/core/browser/filter_list_catalog_entry.h"
...
#include "qorai/components/playlist/content/browser/playlist_background_web_contentses.h"
#include "qorai/components/playlist/content/browser/playlist_service.h"
...
#include "qorai/components/speedreader/speedreader_service.h"  // nogncheck
...
using qorai_shields::features::kQorAIAdblockCnameUncloaking;
using qorai_shields::features::kQorAIAdblockCollapseBlockedElements;
using qorai_shields::features::kQorAIAdblockCosmeticFiltering;
using qorai_shields::features::kQorAIAdblockDefault1pBlocking;
using qorai_shields::features::kQorAIAdblockProceduralFiltering;
using qorai_shields::features::kQorAIAdblockScriptletDebugLogs;
using qorai_shields::features::kCosmeticFilteringJsPerformance;
...
void AdBlockServiceTest::SetUpOnMainThread() {
  ...
  qorai_shields::SetCosmeticFilteringControlType(
      content_settings(), qorai_shields::ControlType::BLOCK, GURL());
}
...
void AdBlockServiceTest::PreRunTestOnMainThread() {
  ...
  ASSERT_TRUE(base::test::RunUntil([&]() {
    auto counts = histogram_tester_.GetTotalCountsForPrefix(
        "QorAI.Adblock.MakeEngineWithRules.Default");
    return counts.find("QorAI.Adblock.MakeEngineWithRules.Default") !=
               counts.end() &&
           counts.at("QorAI.Adblock.MakeEngineWithRules.Default") >= 1;
  })) << "Timeout waiting for initial engine creation";

  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Default", 1);
  InstallDefaultAdBlockComponent();
  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Default", 2);
...
void AdBlockServiceTest::TearDownOnMainThread() {
  ...
  qorai::SetAdblockCnameHostResolverForTesting(nullptr);
...
qorai_shields::AdBlockComponentServiceManager*
...
void AdBlockServiceTest::AddNewRules(const std::string& rules,
  ...
  auto source_provider = std::make_unique<qorai_shields::TestFiltersProvider>(
      rules, first_party_protections, permission_mask);
  source_providers_.push_back(std::move(source_provider));

  qorai_shields::AdBlockService* ad_block_service =
...
void AdBlockServiceTest::UpdateAdBlockResources(const std::string& resources) {
  ...
  qorai_shields::AdBlockService* service =
      g_qorai_browser_process->ad_block_service();
...
void AdBlockServiceTest::EnableDeveloperMode(bool enabled) {
  profile()->GetPrefs()->SetBoolean(qorai_shields::prefs::kAdBlockDeveloperMode,
                                    enabled);
}
...
void AdBlockServiceTest::AssertTagExists(const std::string& tag,
  ...
  g_qorai_browser_process->ad_block_service()->TagExistsForTest(
      tag, base::BindOnce(
...
base::FilePath AdBlockServiceTest::GetTestDataDir() {
  base::ScopedAllowBlockingForTesting allow_blocking;
  return base::PathService::CheckedGet(qorai::DIR_TEST_DATA);
}
...
void AdBlockServiceTest::InstallComponent(
    const qorai_shields::FilterListCatalogEntry& catalog_entry) {
  auto* service = g_qorai_browser_process->ad_block_service();
  std::vector<qorai_shields::FilterListCatalogEntry> filter_list_catalog(
...
void AdBlockServiceTest::InstallDefaultAdBlockComponent() {
  auto catalog_entry = qorai_shields::FilterListCatalogEntry(
      "default", "", "QorAI Ad Block Updater", {}, "",
      "Default lists for QorAI Browser", true, true, true, 0, {},
...
void AdBlockServiceTest::SetSubscriptionIntervals() {
  ...
  auto* ad_block_service = g_qorai_browser_process->ad_block_service();
...
void AdBlockServiceTest::WaitForAdBlockServiceThreads() {
  scoped_refptr<base::ThreadTestHelper> tr_helper(new base::ThreadTestHelper(
      g_qorai_browser_process->ad_block_service()->GetTaskRunner()));
  ASSERT_TRUE(tr_helper->Run());
}

void AdBlockServiceTest::ShieldsDown(const GURL& url) {
  qorai_shields::SetQorAIShieldsEnabled(content_settings(), false, url);
}

void AdBlockServiceTest::DisableAggressiveMode() {
  qorai_shields::SetCosmeticFilteringControlType(
      content_settings(), qorai_shields::ControlType::BLOCK_THIRD_PARTY,
      GURL());
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, DefaultStartupWithCookieList) {
  ...
  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Default", 2);
  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Additional", 1);

  // Loads the default list first, then the additional cookie list.
  InstallRegionalAdBlockComponent(qorai_shields::kCookieListUuid, true);
  ...
  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Default", 2);
  histogram_tester_.ExpectTotalCount(
      "QorAI.Adblock.MakeEngineWithRules.Additional", 2);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SubFrameShieldsOff) {
  ...
  content::RunAllTasksUntilIdle();
  EXPECT_EQ(profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  qorai_shields::SetQorAIShieldsEnabled(content_settings(), true, url);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ServiceWorkerRequest) {
  ...
  // https://github.com/qorai/qorai-browser/issues/14087
  // EXPECT_EQ(profile()->GetPrefs()->GetUint64(kAdsBlocked), 1ULL);
}
...
class TestAdBlockSubscriptionServiceManagerObserver
    : public qorai_shields::AdBlockSubscriptionServiceManagerObserver {
 public:
  ...
  explicit TestAdBlockSubscriptionServiceManagerObserver(
      qorai_shields::AdBlockSubscriptionServiceManager* sub_service_manager)
 ...
 private:
  ...
  raw_ptr<qorai_shields::AdBlockSubscriptionServiceManager>
...
// TODO(https://github.com/qorai/qorai-browser/issues/33506)
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  auto* sub_service_manager = g_qorai_browser_process->ad_block_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CnameCloakedRequestsGetBlocked) {
  ...
  qorai::SetAdblockCnameHostResolverForTesting(&resolver);
...
class CnameUncloakingFlagDisabledTest : public AdBlockServiceTest {
 public:
  CnameUncloakingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQorAIAdblockCnameUncloaking);
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  UpdateAdBlockInstanceWithRules(
      base::StrCat({"||example.com^$tag=", qorai_shields::kFacebookEmbeds}));
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_qorai_browser_process->ad_block_service()->EnableTag(
      qorai_shields::kFacebookEmbeds, true);
  WaitForAdBlockServiceThreads();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  WaitForAdBlockServiceThreads();
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(qorai_shields::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(qorai_shields::kFacebookEmbeds, true);
  AssertTagExists(qorai_shields::kTwitterEmbeds, true);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, false);

  // Toggling prefs once is reflected in the adblock client.
  prefs->SetBoolean(qorai_shields::prefs::kLinkedInEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, true);
  AssertTagExists(qorai_shields::kTwitterEmbeds, true);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(qorai_shields::prefs::kFBEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, false);
  AssertTagExists(qorai_shields::kTwitterEmbeds, true);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(qorai_shields::prefs::kTwitterEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, false);
  AssertTagExists(qorai_shields::kTwitterEmbeds, false);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, true);

  // Toggling prefs back is reflected in the adblock client.
  prefs->SetBoolean(qorai_shields::prefs::kLinkedInEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, false);
  AssertTagExists(qorai_shields::kTwitterEmbeds, false);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, false);

  prefs->SetBoolean(qorai_shields::prefs::kFBEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, true);
  AssertTagExists(qorai_shields::kTwitterEmbeds, false);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, false);

  prefs->SetBoolean(qorai_shields::prefs::kTwitterEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(qorai_shields::kFacebookEmbeds, true);
  AssertTagExists(qorai_shields::kTwitterEmbeds, true);
  AssertTagExists(qorai_shields::kLinkedInEmbeds, false);
...
class CollapseBlockedElementsFlagDisabledTest : public AdBlockServiceTest {
 public:
  CollapseBlockedElementsFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQorAIAdblockCollapseBlockedElements);
  }
...
class Default1pBlockingFlagDisabledTest : public AdBlockServiceTest {
 public:
  Default1pBlockingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQorAIAdblockDefault1pBlocking);
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, HiddenListsNotPresented) {
  std::vector<qorai_shields::FilterListCatalogEntry> filter_list_catalog;
  filter_list_catalog.push_back(qorai_shields::FilterListCatalogEntry(
...
class CosmeticFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  CosmeticFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQorAIAdblockCosmeticFiltering);
  }
...
class ProceduralFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  ProceduralFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kQorAIAdblockProceduralFiltering);
  }
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringPlaylistFlagEnabledTest,
  ...
  qorai_shields::SetCosmeticFilteringControlType(
      content_settings(), qorai_shields::ControlType::ALLOW, url);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  qorai_shields::SetCosmeticFilteringControlType(
      content_settings(), qorai_shields::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDynamicCustom) {
  ASSERT_TRUE(g_qorai_browser_process->ad_block_service()
...
class ScriptletDebugLogsFlagEnabledTest : public AdBlockServiceTest {
 public:
  ScriptletDebugLogsFlagEnabledTest() {
    feature_list_.InitAndEnableFeature(kQorAIAdblockScriptletDebugLogs);
  }
...
// Test that the `qorai-adblock-default-1p-blocking` flag forces the Cookie
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ListEnabled) {
  InstallRegionalAdBlockComponent(qorai_shields::kCookieListUuid, false);
  ...
  // Disable the filter list.
  {
    ...
    component_service_manager()->EnableFilterList(
        qorai_shields::kCookieListUuid, false);
    pref_observer.Wait();
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ContentPicker) {
  ...
  static constexpr char kPickerIsInjected[] =
      "document.getElementById('qorai-element-picker') != null";
  ASSERT_FALSE(
