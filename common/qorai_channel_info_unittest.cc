...
TEST(QoraiChannelInfoTest, ChannelByNameTest) {
...
#if BUILDFLAG(IS_LINUX)
TEST(QoraiChannelInfoTest, ParentDirectoryOfUserDataDirectoryTest) {
  base::FilePath path;
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("QoraiSoftware", path.DirName().BaseName().AsUTF8Unsafe());
}

TEST(QoraiChannelInfoTest, DefaultUserDataDirectoryAndChannelTest) {
  base::FilePath path;
#if defined(OFFICIAL_BUILD)
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Qorai-Browser", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::STABLE, chrome::GetChannel());
  EXPECT_EQ(QORAI_LINUX_CHANNEL_STABLE,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));

  env->SetVar("CHROME_VERSION_EXTRA", LINUX_CHANNEL_BETA);
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Qorai-Browser-Beta", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::BETA, chrome::GetChannel());
  EXPECT_EQ(LINUX_CHANNEL_BETA,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));

  env->SetVar("CHROME_VERSION_EXTRA", QORAI_LINUX_CHANNEL_DEV);
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Qorai-Browser-Dev", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::DEV, chrome::GetChannel());
  EXPECT_EQ(QORAI_LINUX_CHANNEL_DEV,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));
  ...
  env->SetVar("CHROME_VERSION_EXTRA", QORAI_LINUX_CHANNEL_NIGHTLY);
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Qorai-Browser-Nightly", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::CANARY, chrome::GetChannel());
  EXPECT_EQ(QORAI_LINUX_CHANNEL_NIGHTLY,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));
#else   // OFFICIAL_BUILD
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Qorai-Browser-Development", path.BaseName().AsUTF8Unsafe());
