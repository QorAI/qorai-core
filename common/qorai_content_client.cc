...
QoraiContentClient::QoraiContentClient() = default;

QoraiContentClient::~QoraiContentClient() = default;

base::RefCountedMemory* QoraiContentClient::GetDataResourceBytes(
    int resource_id) {
  if (resource_id == IDR_FLAGS_UI_APP_JS) {
    ...
    const std::string flags_js =
        ...
        resource_bundle.LoadDataResourceString(
            IDR_FLAGS_UI_QORAI_FLAGS_OVERRIDES_JS);
...
void QoraiContentClient::AddAdditionalSchemes(Schemes* schemes) {
  ChromeContentClient::AddAdditionalSchemes(schemes);
  schemes->standard_schemes.push_back(content::kQoraiUIScheme);
  schemes->secure_schemes.push_back(content::kQoraiUIScheme);
  schemes->cors_enabled_schemes.push_back(content::kQoraiUIScheme);
  schemes->savable_schemes.push_back(content::kQoraiUIScheme);
}

void QoraiContentClient::AddContentDecryptionModules(
