...
#if BUILDFLAG(IS_MAC)
  return base::apple::PathForFrameworkBundleResource("qorai_resources.pak");
#else
  base::FilePath pak_path;
  base::PathService::Get(base::DIR_ASSETS, &pak_path);
  pak_path = pak_path.AppendASCII("qorai_resources.pak");
  return pak_path;
#endif  // OS_MAC
}
#endif  // OS_ANDROID
...
base::FilePath GetScaledResourcesPakFilePath(
  ...
  const char* pak_file = (scale_factor == ui::k100Percent)
                             ? "qorai_100_percent.pak"
                             : "qorai_200_percent.pak";
#if BUILDFLAG(IS_MAC)
...
#if BUILDFLAG(IS_ANDROID)
  ui::QoraiLoadMainAndroidPackFile("assets/qorai_resources.pak",
                                   base::FilePath());
  // qorai_100_percent.pak is excluded now from the Android build because
