...
QoraiAudioFarblingHelper::QoraiAudioFarblingHelper(double fudge_factor,
                                                   uint64_t seed,
                                                   bool max)
    : fudge_factor_(fudge_factor), seed_(seed), max_(max) {}

QoraiAudioFarblingHelper::~QoraiAudioFarblingHelper() = default;

void QoraiAudioFarblingHelper::FarbleAudioChannel(base::span<float> dst) const {
  if (max_) {
    uint64_t v = seed_;
    for (auto& value : dst) {
      v = lfsr_next(v);
...
void QoraiAudioFarblingHelper::FarbleFloatTimeDomainData(
...
void QoraiAudioFarblingHelper::FarbleByteTimeDomainData(
    const float* input_buffer,
...
void QoraiAudioFarblingHelper::FarbleConvertToByteData(
    const float* source,
    unsigned char* destination,
    size_t len,
    double min_decibels,
...
void QoraiAudioFarblingHelper::FarbleConvertFloatToDb(const float* source,
                                                      float* destination,
                                                      size_t len) const {
  if (max_) {
    uint64_t v = seed_;
