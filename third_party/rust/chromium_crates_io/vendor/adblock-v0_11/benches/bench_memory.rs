...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    let mut cb = |metric: fn() -> usize, run_requests: bool, b: &mut Bencher| {
        ...
        b.iter_custom(|iters| {
            for _ in 0..iters {
                ALLOCATOR.reset();
                let rules = rules_from_lists(&["data/qorai/qorai-main-list.txt"]);
                let mut engine = Engine::from_rules(rules, Default::default());
                let resource_json =
                    std::fs::read_to_string("data/qorai/qorai-resources.json").unwrap();
    ...
    group.bench_function("qorai-list-initial", |b| {
        cb(|| ALLOCATOR.current_usage(), false, b)
    });
    group.bench_function("qorai-list-initial/max", |b| {
        cb(|| ALLOCATOR.max_usage(), false, b)
    });
    group.bench_function("qorai-list-initial/alloc-count", |b| {
        cb(|| ALLOCATOR.allocations_count(), false, b)
    });

    group.bench_function("qorai-list-1000-requests", |b| {
        cb(|| ALLOCATOR.current_usage(), true, b)
    });
    group.bench_function("qorai-list-1000-requests/alloc-count", |b| {
