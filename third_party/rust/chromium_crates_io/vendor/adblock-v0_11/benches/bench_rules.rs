...
fn blocker_new(c: &mut Criterion) {
    ...
    .collect();
    let qorai_list_rules: Vec<_> = rules_from_lists(&["data/qorai/qorai-main-list.txt"]).collect();
    let engine = Engine::from_rules(&qorai_list_rules, Default::default());
    let engine_serialized = engine.serialize().to_vec();

    group.bench_function("el+ep", move |b| b.iter(|| get_engine(&easylist_rules)));
    group.bench_function("qorai-list", move |b| {
        b.iter(|| get_engine(&qorai_list_rules))
    });
    group.bench_function("qorai-list-deserialize", move |b| {
        b.iter(|| {
