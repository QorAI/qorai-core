...
def _AstImportPred(qorai_import, ast_import):
    return qorai_import.import_filename == ast_import.import_filename


# Predicate for a definition name (doesn't check type).
def _AstDefinitionNamePred(qorai_definition, ast_definition):
    return qorai_definition.mojom_name == ast_definition.mojom_name


# Predicate for definition name and type.
def _AstDefinitionPred(qorai_definition, ast_definition):
    return _AstDefinitionNamePred(qorai_definition, ast_definition) and \
              isinstance(qorai_definition, type(ast_definition))


# Returns required action from a definition attributes list.
def _GetQoraiDefinitionAction(qorai_definition):
    if isinstance(qorai_definition, ast.Const):
        # Const can only be added.
        return _DEFINITION_ADD

    if qorai_definition.attribute_list:
        for attribute in qorai_definition.attribute_list:
            if attribute.key.name == 'QoraiAdd':
                return _DEFINITION_ADD
            if attribute.key.name == 'QoraiExtend':
                return _DEFINITION_EXTEND

    raise ValueError(
        "Definition should have [QoraiAdd] or [QoraiExtend] attribute: %s" %
        qorai_definition.mojom_name)


# Validates that a definition with same name doesn't exist in the list.
def _CheckDefinitionDoesntExist(qorai_definition, ast_items):
    if any(
            _AstDefinitionNamePred(qorai_definition, item)
...
def _FindMatchingDefinition(qorai_definition, ast_definitions):
    for ast_definition in ast_definitions:
        if _AstDefinitionPred(qorai_definition, ast_definition):
...
def _ExtendAstDefinition(qorai_definition, ast_definition):
    if isinstance(qorai_definition, ast.Enum):
        for item in qorai_definition.enum_value_list:
            _CheckDefinitionDoesntExist(item, ast_definition.enum_value_list)
            ast_definition.enum_value_list.Append(item)
    elif isinstance(qorai_definition, (ast.Interface, ast.Struct, ast.Union)):
        items_to_append = []
        for item in reversed(qorai_definition.body.items):
            if isinstance(item, (ast.Const, ast.Enum)):
                # Handle nested types.
                _ApplyQoraiDefinition(item, ast_definition.body)
            else:
...
def _ApplyQoraiDefinition(qorai_definition, ast_definitions):
    definition_action = _GetQoraiDefinitionAction(qorai_definition)
    if definition_action == _DEFINITION_ADD:
        _CheckDefinitionDoesntExist(qorai_definition, ast_definitions)
        if isinstance(ast_definitions, ast.NodeListBase):
            ast_definitions.Insert(qorai_definition)
        else:
            ast_definitions.insert(0, qorai_definition)
    elif definition_action == _DEFINITION_EXTEND:
        ast_definition_to_extend = _FindMatchingDefinition(
            qorai_definition, ast_definitions)
        if not ast_definition_to_extend:
            raise ValueError("Trying to extend non-existent definition: %s" %
                             qorai_definition.mojom_name)
        _ExtendAstDefinition(qorai_definition, ast_definition_to_extend)
    else:
        raise ValueError("Unknown definition action requested: %s" %
                         qorai_definition.mojom_name)


# Applies changes to original mojom ast using qorai ast.
def _ApplyQoraiAstChanges(qorai_ast, parsed_ast):
    # Make sure the module name is correct.
    if qorai_ast.module != parsed_ast.module:
        raise ValueError(
            f"Mojo module ids are not equal while trying to patch: "
            f"{qorai_ast.module.mojom_namespace} vs "
            f"{parsed_ast.module.mojom_namespace}. "
            "(Maybe missing an attribute?)")

    # Add new imports.
    for qorai_import in qorai_ast.import_list:
        if not any(
                _AstImportPred(qorai_import, imp)
                for imp in parsed_ast.import_list):
            parsed_ast.import_list.Append(qorai_import)
    ...
    for qorai_definition in reversed(qorai_ast.definition_list):
        _ApplyQoraiDefinition(qorai_definition, parsed_ast.definition_list)
...
#
# To be more strict with patching, attributes [QoraiAdd] or [QoraiExtend] should
# be used.
def _PatchInQoraiMojomAst(mojom_abspath, parsed_ast, enabled_features):
    ...
    # Build qorai/chromium_src path.
    chromium_src_abspath = os.path.join(chromium_original_dir, 'qorai',
                                        'chromium_src')
    if not os.path.isdir(chromium_src_abspath):
        raise RuntimeError(
            "Could not find qorai/chromium_src. %s is not a dir" %
            chromium_src_abspath)

    # Relative path.
    mojom_relpath = mojom_abspath[len(chromium_original_dir) + 1:]

    # Build possible qorai/chromium_src/**/*.mojom path.
    qorai_mojom_abspath = os.path.join(chromium_src_abspath, mojom_relpath)
    if not os.path.isfile(qorai_mojom_abspath):
        # Nothing to patch.
        return

    # Open and parse qorai/chromium_src/**/*.mojom file.
    with codecs.open(qorai_mojom_abspath, encoding='utf-8') as f:
        qorai_ast = parser.Parse(f.read(), qorai_mojom_abspath)
        conditional_features.RemoveDisabledDefinitions(qorai_ast,
                                                       enabled_features)

        _ApplyQoraiAstChanges(qorai_ast, parsed_ast)
...
def _ParseAstHelper(mojom_abspath, enabled_features):
    mojom_abspath, parsed_ast = orig_parse_ast_helper(mojom_abspath,
                                                      enabled_features)
    _PatchInQoraiMojomAst(mojom_abspath, parsed_ast, enabled_features)
