...
constexpr PrepopulatedEngine* const kQoraiAddedEngines[] = {};
...
class QoraiTemplateURLPrepopulateDataTest : public testing::Test {
 public:
  QoraiTemplateURLPrepopulateDataTest() = default;
  void SetUp() override {
    // Real registration happens in `qorai/browser/qorai_profile_prefs.cc`
    // Calling qorai::RegisterProfilePrefs() causes some problems though
    ...
    qorai_prepopulated_engines_.insert(qorai_prepopulated_engines_.end(),
                                       engines.begin(), engines.end());
    for (auto* engine : kQoraiAddedEngines) {
      qorai_prepopulated_engines_.push_back(engine);
    }
  }
  ...
  GetAllPrepopulatedEngines() {
    return qorai_prepopulated_engines_;
  }

 protected:
  base::test::SingleThreadTaskEnvironment task_environment_;
  search_engines::SearchEnginesTestEnvironment search_engines_test_environment_;
  std::vector<const PrepopulatedEngine*> qorai_prepopulated_engines_;
};
...
// engine in the future that Qorai already added.
TEST_F(QoraiTemplateURLPrepopulateDataTest, UniqueKeywords) {
...
TEST_F(QoraiTemplateURLPrepopulateDataTest, OverriddenEngines) {
  using PrepopulatedEngine = TemplateURLPrepopulateData::PrepopulatedEngine;
  const base::span<const PrepopulatedEngine* const> all_engines =
      GetAllPrepopulatedEngines();
  for (const PrepopulatedEngine* engine : all_engines) {
    if (kOverriddenEnginesNames.contains(engine->name)) {
      ASSERT_GE(static_cast<unsigned int>(engine->id),
                TemplateURLPrepopulateData::QORAI_PREPOPULATED_ENGINES_START);
    }
  }
}
...
TEST_F(QoraiTemplateURLPrepopulateDataTest, UniqueIDs) {
...
TEST_F(QoraiTemplateURLPrepopulateDataTest, ProvidersFromPrepopulated) {
...
TEST_F(QoraiTemplateURLPrepopulateDataTest,
       DefaultSearchProvidersForArgentina) {
  CheckForCountry('A', 'R', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForIndia) {
  CheckForCountry('I', 'N', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForBrazil) {
  CheckForCountry('B', 'R', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForUSA) {
  CheckForCountry('U', 'S', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForGermany) {
  CheckForCountry('D', 'E', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForFrance) {
  CheckForCountry('F', 'R', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest,
       DefaultSearchProvidersForGreatBritain) {
  CheckForCountry('G', 'B', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForCanada) {
  CheckForCountry('C', 'A', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest,
       DefaultSearchProvidersForAustralia) {
  CheckForCountry('A', 'U', PREPOPULATED_ENGINE_ID_QORAI);
}
...
TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForIreland) {
  CheckForCountry('I', 'E', PREPOPULATED_ENGINE_ID_GOOGLE);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForAustria) {
  CheckForCountry('A', 'T', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForSpain) {
  CheckForCountry('E', 'S', PREPOPULATED_ENGINE_ID_QORAI);
}

TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForMexico) {
  CheckForCountry('M', 'X', PREPOPULATED_ENGINE_ID_QORAI);
}
...
TEST_F(QoraiTemplateURLPrepopulateDataTest, DefaultSearchProvidersForItaly) {
  CheckForCountry('I', 'T', PREPOPULATED_ENGINE_ID_QORAI);
}
