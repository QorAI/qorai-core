...
// the version of Qorai engines as well: kCurrentDataVersion is defined in
...
int GetDataVersion(PrefService* prefs) {
  ...
  return (data_version + kQoraiCurrentDataVersion);
}

// Chromium picks Google (if on the list, otherwise the first prepopulated on
// the list). We should return the default engine by country, or Qorai.
std::unique_ptr<TemplateURLData> GetPrepopulatedFallbackSearch(
    QoraiPrepopulatedEngineID default_engine_id,
    PrefService& prefs,
  ...
  std::unique_ptr<TemplateURLData> qorai_engine;
  for (auto& engine : prepopulated_engines) {
    if (engine->prepopulate_id == static_cast<int>(default_engine_id)) {
      ...
      qorai_engine = std::move(engine);
    }
  }

  // Default engine wasn't found, then return Qorai, if found.
  if (qorai_engine) {
    return qorai_engine;
  }
