...
def _LoadPolicies(orig_func):
    ...
    # Qorai uses the group name "QoraiSoftware". The child element for the
    # group is the policy itself (those are the yaml files in the folder).
    #
    # Qorai specific entries are get copied into place by `update_policy_files`.
    # We copy the files from:
    # `//qorai/components/policy/resources/templates/policy_definitions/QoraiSoftware` # pylint: disable=line-too-long
    # to:
    # `//components/policy/resources/templates/policy_definitions`
    ...
    qorai_policies = []
    qorai_policy_section = policy_definition_yaml['QoraiSoftware']
    assert qorai_policy_section, "'policy_definitions > QoraiSoftware' entries not found (failed to copy?)"  # pylint: disable=line-too-long

    qorai_policy_items = qorai_policy_section['policies']
    for key, _ in qorai_policy_items.items():
        qorai_policies.append(key)

    # 2) "policies"
    # This has the contents of:
    # `//components/policy/resources/templates/policies.yaml`
    # This is where we need to inject the Qorai specific names. The policies
    ...
    for i, entry in enumerate(qorai_policies):
...
def sync_policy_files():
    ...
    # Qorai's policy definitions to Chromium's policy definitions.
    with open("gen/qorai_policies_sync_config.json", "r") as f:
        qorai_policies = json.load(f)

        for policy in qorai_policies["policies"]:
            copy_only_if_modified(f'{qorai_policies["copy_from"]}/{policy}',
                                  f'{qorai_policies["copy_to"]}/{policy}')
