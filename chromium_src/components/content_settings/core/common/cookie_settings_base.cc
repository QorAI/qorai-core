...
bool QoraiIsAllowedThirdParty(const GURL& url,
...
bool CookieSettingsBase::IsFullCookieAccessAllowed(
  ...
  if (QoraiIsAllowedThirdParty(url, first_party_url, this))
    return true;
...
// everything" path.
#define QORAI_COOKIE_SETTINGS_BASE_DECIDE_ACCESS                              \
...
// enable 1PES mode.
#define QORAI_COOKIE_SETTINGS_BASE_GET_COOKIES_SETTINGS_INTERNAL_IS_EXPLICIT_SETTING \
  if (false)
...
#undef QORAI_COOKIE_SETTINGS_BASE_DECIDE_ACCESS
#undef QORAI_COOKIE_SETTINGS_BASE_GET_COOKIES_SETTINGS_INTERNAL_IS_EXPLICIT_SETTING
