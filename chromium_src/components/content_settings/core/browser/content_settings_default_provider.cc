...
#if !BUILDFLAG(IS_IOS)
#define QORAI_DISCARD_OR_MIGRATE_OBSOLETE_PREFERENCES           \
...
#define GetValue(...)                                  \
  ...
  if (IsQoraiValidDefaultValue(content_type, value)) { \
    return value.Clone();                              \
  }

namespace {

bool IsQoraiValidDefaultValue(ContentSettingsType content_type,
...
#undef QORAI_DISCARD_OR_MIGRATE_OBSOLETE_PREFERENCES
#undef GetValue
