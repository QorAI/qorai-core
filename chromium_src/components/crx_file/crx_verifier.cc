...
// The Qorai publisher key that is accepted in addition to upstream's
...
constexpr uint8_t kQoraiPublisherKeyHash[] = {
...
auto GetQoraiPublisherKeyHash() {
  static auto qorai_publisher_key = std::to_array(kQoraiPublisherKeyHash);
  return base::span(qorai_publisher_key);
}

// Used in the patch in crx_verifier.cc.
bool IsQoraiPublisher(base::span<const uint8_t> key_hash) {
  return GetQoraiPublisherKeyHash() == key_hash;
}
...
void SetQoraiPublisherKeyHashForTesting(base::span<const uint8_t> test_key) {
  GetQoraiPublisherKeyHash().copy_from(test_key);
}
