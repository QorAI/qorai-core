...
#include "qorai/components/translate/core/common/qorai_translate_constants.h"
...
// Redirect the translate script request to the Qorai endpoints.
GURL ChromiumTranslateScript::AddHostLocaleToUrl(const GURL& url) {
  ...
  if (result.host_piece() == google_translate_script.host_piece()) {
    const GURL qorai_translate_script(kQoraiTranslateScriptURL);
    GURL::Replacements replaces;
    replaces.SetHostStr(qorai_translate_script.host_piece());
    replaces.SetPathStr(qorai_translate_script.path_piece());
...
void TranslateScript::Request(RequestCallback callback, bool is_incognito) {
  if (!IsQoraiTranslateGoAvailable()) {
...
void TranslateScript::OnScriptFetchComplete(bool success,
  ...
  const std::string new_data = base::StrCat(
      {absl::StrFormat("const useGoogleTranslateEndpoint = %s;",
       ...
       absl::StrFormat("const qoraiTranslateStaticPath = '%s';",
                       kQoraiTranslateStaticPath),
       ui::ResourceBundle::GetSharedInstance().LoadDataResourceString(
           IDR_QORAI_TRANSLATE_JS),
       data});
