...
#define QORAI_SYNC_SCHEDULER_IMPL_HANDLE_FAILURE \
  HandleQoraiConfigurationFailure(model_neutral_state);
...
#undef QORAI_SYNC_SCHEDULER_IMPL_HANDLE_FAILURE

#include "qorai/components/sync/engine/qorai_sync_server_commands.h"
...
void SyncSchedulerImpl::HandleQoraiConfigurationFailure(
...
void SyncSchedulerImpl::PermanentlyDeleteAccountImpl(
  ...
  QoraiSyncServerCommands::PermanentlyDeleteAccount(&cycle,
                                                    std::move(callback));
}
