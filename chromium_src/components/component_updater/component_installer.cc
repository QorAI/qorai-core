...
bool ComponentInstallerPolicy::IsQoraiComponent() const {
  return false;
}
...
void ComponentInstaller::Register(
  ...
  if (installer_policy_) {
    ...
    if (kDisallowedComponents.contains(id)) {
      VLOG(1) << "Skipping registration of Qorai-unsupported component " << id
              << ".";
      return;
    }
  }
...
bool ComponentInstaller::IsQoraiComponent() const {
  return installer_policy_->IsQoraiComponent();
}
