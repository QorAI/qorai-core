...
mangle((element) => {
  ...
  h2.textContent = 'List of Qorai URLs'
})

// Rewrite chrome://chrome-urls -> qorai://chrome-urls
mangle((element) => {
  ...
  anchor.textContent = 'qorai://chrome-urls'
}, x => x.text.includes('href="#"'))

// Rewrite standard chrome URLs to use qorai: scheme (these appear under the
// "List of Qorai URLs" header) and rewrite internal debugging page URLs to use
// qorai: scheme (these appear under the "Internal Debugging Page URLs" header
...
mangleAll((element) => {
  ...
  anchor.textContent = '\${info.url.url.replace(/chrome:/, "qorai:")}'
}, x => x.text.includes('href="${info.url.url}"'))

// Rewrite inactive chrome URLs to use qorai: scheme (these also appear under
// the "List of Qorai URLs" header) and rewrite internal debugging page URLs to
// use qorai: scheme (these appear under the "Internal Debugging Page URLs"
...
mangleAll((element) => {
  ...
  listItem.textContent = '\${info.url.url.replace(/chrome:/, "qorai:")}'
}, x => x.text.includes('<li>${info.url.url}</li>'))

// Rewrite command URLs to use qorai: scheme (these appear under the
...
mangle((element) => {
  ...
  listItem.textContent = '\${url.url.replace(/chrome:/, "qorai:")}'
