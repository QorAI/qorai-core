...
#define QORAI_MAKE_LOCAL_DEVICE_SPECIFICS \
  specifics->mutable_qorai_fields()->set_is_self_delete_supported(true);
...
// breaks the ability to remove other device in sync chain for Qorai
#define QORAI_DEVICE_INFO_SYNC_BRIDGE_APPLY_SYNC_CHANGES_SKIP_NEXT_IF if (false)

#define QORAI_SKIP_EXPIRE_OLD_ENTRIES return;

#define QORAI_ON_READ_ALL_METADATA_CLEAR_PROGRESS_TOKEN         \
...
#undef QORAI_ON_READ_ALL_METADATA_CLEAR_PROGRESS_TOKEN
#undef QORAI_SKIP_EXPIRE_OLD_ENTRIES
#undef QORAI_DEVICE_INFO_SYNC_BRIDGE_APPLY_SYNC_CHANGES_SKIP_NEXT_IF
#undef RefreshLocalDeviceInfoIfNeeded
#undef QORAI_MAKE_LOCAL_DEVICE_SPECIFICS
...
std::unique_ptr<QoraiDeviceInfo> QoraiSpecificsToModel(
  ...
  // extra patch
  return std::make_unique<QoraiDeviceInfo>(
      ...
      specifics.has_qorai_fields() &&
          specifics.qorai_fields().has_is_self_delete_supported() &&
          specifics.qorai_fields().is_self_delete_supported());
}
...
std::vector<std::unique_ptr<QoraiDeviceInfo>>
DeviceInfoSyncBridge::GetAllQoraiDeviceInfo() const {
  std::vector<std::unique_ptr<QoraiDeviceInfo>> list;
  for (const auto& data : all_data_) {
    list.push_back(QoraiSpecificsToModel(data.second.specifics()));
  }
  return list;
}
...
std::vector<std::unique_ptr<QoraiDeviceInfo>>
DeviceInfoTracker::GetAllQoraiDeviceInfo() const {
