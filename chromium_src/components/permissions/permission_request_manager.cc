...
#define QORAI_PERMISSION_REQUEST_MANAGER_GET_REQUESTING_ORIGIN \
  if (!ShouldBeGrouppedInRequests(request.get()))

// |tab_is_hidden_| should be updated after upstream sets.
#define QORAI_PERMISSION_REQUEST_MANAGER_ON_VISIBILITY_CHANGED \
  UpdateTabIsHiddenWithTabActivationState();

#include <components/permissions/permission_request_manager.cc>

#undef QORAI_PERMISSION_REQUEST_MANAGER_ON_VISIBILITY_CHANGED
#undef QORAI_PERMISSION_REQUEST_MANAGER_GET_REQUESTING_ORIGIN
...
bool PermissionRequestManager::ShouldGroupRequests(PermissionRequest* a,
  ...
  url::Origin origin_a;
  url::Origin origin_b;
  if (a->request_type() == RequestType::kQoraiEthereum ||
      a->request_type() == RequestType::kQoraiSolana ||
      a->request_type() == RequestType::kQoraiCardano) {
    if (a->request_type() == b->request_type() &&
        qorai_wallet::ParseRequestingOriginFromSubRequest(
