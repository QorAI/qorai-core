...
#define CreateJavaDelegate                                      \
  Java_PermissionDialogController_createDialog_QoraiImpl(       \
...
#include "qorai/components/permissions/android/jni_headers/QoraiPermissionDialogDelegate_jni.h"
#include "qorai/components/permissions/permission_lifetime_utils.h"
#include "qorai/components/permissions/permission_widevine_utils.h"
...
void SetLifetimeOptions(const base::android::JavaRef<jobject>& j_delegate) {
  ...
  Java_QoraiPermissionDialogDelegate_setLifetimeOptionsText(
      env, j_delegate,
  ...
  Java_QoraiPermissionDialogDelegate_setLifetimeOptions(
      env, j_delegate,
...
void PermissionDialogJavaDelegate::ApplyLifetimeToPermissionRequests(
  ...
  const int selected_lifetime_option =
      Java_QoraiPermissionDialogDelegate_getSelectedLifetimeOption(env,
                                                                   j_delegate_);
...
void PermissionDialogJavaDelegate::ApplyDontAskAgainOption(
  ...
  const bool dont_ask_again =
      Java_QoraiPermissionDialogDelegate_getDontAskAgain(env, j_delegate_);
...
void PermissionDialogJavaDelegate::
  ...
  if (HasWidevinePermissionRequest(
    ...
    Java_QoraiPermissionDialogDelegate_setIsWidevinePermissionRequest(
        env, j_delegate, true);
  }
#endif
...
#define QORAI_PERMISSION_DIALOG_DELEGATE_ACCEPT                               \
...
#define QORAI_PERMISSION_DIALOG_DELEGATE_CANCEL                               \
  java_delegate_->ApplyLifetimeToPermissionRequests(env, permission_prompt_); \
  java_delegate_->ApplyDontAskAgainOption(env, permission_prompt_);
...
#undef QORAI_PERMISSION_DIALOG_DELEGATE_CANCEL
#undef QORAI_PERMISSION_DIALOG_DELEGATE_ACCEPT
