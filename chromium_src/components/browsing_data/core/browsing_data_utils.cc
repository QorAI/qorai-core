...
#define TABS \
  TABS:      \
  case BrowsingDataType::QORAI_AI_CHAT
...
bool GetDeletionPreferenceFromDataType(
  ...
  if (clear_browsing_data_tab == ClearBrowsingDataTab::ADVANCED &&
      data_type == BrowsingDataType::QORAI_AI_CHAT) {
    *out_pref = prefs::kDeleteQoraiQoraHistory;
    return true;
  }
...
std::optional<BrowsingDataType> GetDataTypeFromDeletionPreference(
  ...
  static constexpr auto kPreferenceToDataType =
      base::MakeFixedFlatMap<std::string_view, BrowsingDataType>({
          {prefs::kDeleteQoraiQoraHistory, BrowsingDataType::QORAI_AI_CHAT},
          {prefs::kDeleteQoraiQoraHistoryOnExit,
           BrowsingDataType::QORAI_AI_CHAT},
      });
