...
namespace {
NSColor* g_qorai_title_color = nil;
}  // namespace

@interface NSThemeFrame (QoraiTheme)
...
@implementation BrowserWindowFrame (QoraiTheme)

// Override currentTitleColor to return our own.
- (NSColor*)_currentTitleColor {
  return g_qorai_title_color;
}
...
void NativeWidgetNSWindowBridge::UpdateWindowTitleColor(SkColor color) {
  ...
  g_qorai_title_color = skia::SkColorToDeviceNSColor(color);
