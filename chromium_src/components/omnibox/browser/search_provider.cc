...
// Pass additional argument to check if it's Qorai Search's rich suggestion
#define ParseSuggestResults(root_list, input, scheme_classifier,            \
  ...
  ParseSuggestResults(root_list, input, scheme_classifier,                  \
                      ...
                      IsQoraiRichSuggestion(is_keyword_result))

#include <components/omnibox/browser/search_provider.cc>

#undef ParseSuggestResults

bool SearchProvider::IsQoraiRichSuggestion(bool is_keyword) {
...
class QoraiSearchProvider* SearchProvider::AsQoraiSearchProvider() {
  NOTREACHED() << "We replaces SearchProvider with QoraiSearchProvider";
}
