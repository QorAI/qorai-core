...
bool IsQoraiComponent(const Component* component) {
  CHECK(component);
  const auto& crx_component = component->crx_component();
  if (!crx_component || !crx_component->installer) {
    return false;
  }
  return crx_component->installer->IsQoraiComponent();
}
...
void SequentialUpdateChecker::CheckForUpdates(
  ...
  std::ranges::stable_partition(
      ...
      [&](const std::string& id) {
        return IsQoraiComponent(update_context_->components[id].get());
      });
