...
class HSTSPartitionHashHelper {
 public:
  static std::string GetPartitionDomain(const SchemefulSite& schemeful_site) {
    DCHECK(base::FeatureList::IsEnabled(features::kQoraiPartitionHSTS));
...
std::optional<TransportSecurityState::HashedHost> GetPartitionHashForAddingHSTS(
  ...
  if (!base::FeatureList::IsEnabled(features::kQoraiPartitionHSTS)) {
    return std::nullopt;
  }
...
bool TransportSecurityState::DeleteDynamicDataForHost(const std::string& host) {
  ...
  if (qorai_deleted)
    DirtyNotify();
  return chromium_deleted || qorai_deleted;
}
