...
float FarbleDeviceMemory(blink::ExecutionContext* context) {
  ...
  QoraiFarblingLevel farbling_level = qorai::GetQoraiFarblingLevelFor(
      context, ContentSettingsType::QORAI_WEBCOMPAT_DEVICE_MEMORY,
      QoraiFarblingLevel::OFF);
  // If Qorai Shields are down or anti-fingerprinting is off for this site,
...
float NavigatorDeviceMemory::deviceMemory(ScriptState* script_state) const {
  ...
  return qorai::FarbleDeviceMemory(context);
}
