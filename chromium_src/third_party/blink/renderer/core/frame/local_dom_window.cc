...
#define QORAI_LOCAL_DOM_WINDOW_CAN_EXECUTE_SCRIPTS                 \
...
#undef QORAI_LOCAL_DOM_WINDOW_CAN_EXECUTE_SCRIPTS
#undef outerHeight
#undef outerWidth
#undef screenX
#undef screenY
...
using qorai::BlockScreenFingerprinting;
using qorai::FarbleInteger;
using qorai::FarbleKey;
...
int LocalDOMWindow::outerWidth() const {
  ...
  return BlockScreenFingerprinting(context) && top_window
             ? FarbleInteger(context, qorai::FarbleKey::kWindowInnerWidth,
...
int LocalDOMWindow::outerHeight() const {
  ...
  auto* top_window = DynamicTo<LocalDOMWindow>(top());
  return BlockScreenFingerprinting(context) && top_window
             ? FarbleInteger(context, qorai::FarbleKey::kWindowInnerHeight,
...
int LocalDOMWindow::screenX() const {
  ...
  return BlockScreenFingerprinting(context)
             ? FarbleInteger(context, qorai::FarbleKey::kWindowScreenX, 0, 0, 8)
...
int LocalDOMWindow::screenY() const {
  ...
  ExecutionContext* context = GetExecutionContext();
  return BlockScreenFingerprinting(context)
             ? FarbleInteger(context, qorai::FarbleKey::kWindowScreenY, 0, 0, 8)
