...
#include "qorai/components/qorai_geolocation_permission/common/qorai_geolocation_permission.mojom-blink.h"
#endif
...
bool SetEnableHighAccuracy(LocalFrame* frame, bool enable_high_accuracy) {
  if (frame->Client()->GetRemoteNavigationAssociatedInterfaces()) {
    mojo::AssociatedRemote<
        geolocation::mojom::blink::QoraiGeolocationPermission>
        qorai_geolocation_permission_binding;
    frame->Client()->GetRemoteNavigationAssociatedInterfaces()->GetInterface(
        &qorai_geolocation_permission_binding);
    CHECK(qorai_geolocation_permission_binding.is_bound());
    qorai_geolocation_permission_binding->SetEnableHighAccuracy(
