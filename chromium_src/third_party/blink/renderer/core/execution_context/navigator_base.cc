...
void ApplyQoraiHardwareConcurrencyOverride(blink::ExecutionContext* context,
  ...
  switch (qorai::GetQoraiFarblingLevelFor(
      context, ContentSettingsType::QORAI_WEBCOMPAT_HARDWARE_CONCURRENCY,
    ...
    case QoraiFarblingLevel::BALANCED: {
      qorai::FarblingPRNG prng =
          qorai::QoraiSessionCache::From(*context).MakePseudoRandomGenerator();
      farbled_value =
...
#define ApplyHardwareConcurrencyOverride                        \
  ApplyQoraiHardwareConcurrencyOverride(GetExecutionContext(),  \
...
String NavigatorBase::userAgent() const {
  if (ExecutionContext* context = GetExecutionContext()) {
    if (!qorai::AllowFingerprinting(
            context, ContentSettingsType::QORAI_WEBCOMPAT_USER_AGENT)) {
      return qorai::QoraiSessionCache::From(*context).FarbledUserAgent(
          context->UserAgent());
    }
  }
