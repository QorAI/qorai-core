...
#define CalculateDeviceWidth(...)                                              \
  CalculateDeviceWidth(__VA_ARGS__, bool early) {                              \
    ...
    return top_frame && qorai::BlockScreenFingerprinting(context, early)       \
               ? qorai::FarbleInteger(context,                                 \
                                      qorai::FarbleKey::kWindowInnerWidth,     \
...
#define CalculateDeviceHeight(...)                                         \
  CalculateDeviceHeight(__VA_ARGS__, bool early) {                         \
    ...
    return top_frame && qorai::BlockScreenFingerprinting(context, early)   \
               ? qorai::FarbleInteger(                                     \
                     context, qorai::FarbleKey::kWindowInnerHeight,        \
