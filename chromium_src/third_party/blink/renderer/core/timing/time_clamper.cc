...
constexpr int kQoraiTimerResolutionMicroseconds = 1000;

bool ShouldRound() {
  return base::FeatureList::IsEnabled(features::kQoraiRoundTimeStamps);
}
...
base::TimeDelta TimeClamper::MaybeRoundTimeDelta(base::TimeDelta value) {
  return ShouldRound() ? value.RoundToMultiple(base::Microseconds(
                             kQoraiTimerResolutionMicroseconds))
                       : value;
}

// static
int TimeClamper::FineResolutionMicroseconds() {
  return ShouldRound() ? kQoraiTimerResolutionMicroseconds
