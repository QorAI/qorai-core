...
from qorai_chromium_utils import get_src_dir
...
def CopyAllFilesToStagingDir(original_function, config, distribution,
    ...
    qorai_extension_locales_src_dir_path = os.path.realpath(
        os.path.join(get_src_dir(), 'qorai', 'components', 'qorai_extension',
                     'extension', 'qorai_extension', '_locales'))
    CopyExtensionLocalization('qorai_extension',
                              qorai_extension_locales_src_dir_path, config,
...
def GetPrevVersion(_, build_dir, _temp_dir, last_chrome_installer, output_name):
    ...
    # in (chrome.7z in) Qorai's installation directory when the delta gets
...
def CopyExtensionLocalization(extension_name, locales_src_dir_path, config,
    ...
    locales_dest_path = os.path.join(
        locales_dest_path, config.get('GENERAL', 'qorai_resources.pak'),
...
def SignAndCopyPreSignedBinaries(skip_signing, output_dir, staging_dir,
    ...
    if not skip_signing:
        ...
        shutil.copy(os.path.join(src_dir, 'qorai.exe'), chrome_dir)
