...
class QoraiChromeTestSetupHelper : public testing::EmptyTestEventListener {
 public:
  ...
  QoraiChromeTestSetupHelper() = default;
  ~QoraiChromeTestSetupHelper() override = default;

  // testing::EmptyTestEventListener:
  void OnTestStart(const testing::TestInfo& test_info) override {
    static const base::NoDestructor<std::vector<TestAdjustments>>
        kTestAdjustments({
            // We allow FileSystem API via qorai://flags, so enable it here for
            // upstream tests.
            {
                .test_patterns =
                    {
            ...
            // Disable farbling in Fingerprint-sensitive Chromium tests.
            {
                ...
                .disable_features = {"QoraiFarbling"},
            },
        });
...
void ChromeTestSuite::Initialize() {
  ...
  listeners.Append(new QoraiChromeTestSetupHelper());
}
