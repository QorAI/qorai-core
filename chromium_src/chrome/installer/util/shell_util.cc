...
#if defined(OFFICIAL_BUILD)
// Add QoraiFile prog id in registry with proper icon.
// This prog id will be referenced from serveral file association reg entry.
#define QORAI_GET_CHROME_PROG_ID_ENTRIES                     \
  app_info.prog_id = installer::GetProgIdForFileType();      \
  app_info.file_type_icon_index = GetIconIndexForFileType(); \
  GetProgIdEntries(app_info, entries);

// Give QoraiXXFile prog id for some file type.(ex, .pdf or .svg) instead of
// QoraiHTML.
#define QORAI_GET_APP_EXT_REGISTRATION_ENTRIES                         \
...
// Give QoraiXXFile prog id for some file type.(ex, .pdf or .svg) instead of
// QoraiHTML.
#define QORAI_GET_SHELL_INTEGRATION_ENTRIES                               \
...
#undef QORAI_GET_SHELL_INTEGRATION_ENTRIES
#undef QORAI_GET_APP_EXT_REGISTRATION_ENTRIES
#undef QORAI_GET_CHROME_PROG_ID_ENTRIES
