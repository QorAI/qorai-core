...
// changes:
//
// - `distribution_data` not appended as Qorai does not record histograms.
// - `kQoraiUninstallSurveyUrl` used instead of `kUninstallSurveyUrl`

constexpr std::wstring_view kQoraiUninstallSurveyUrl(
    L"https://qorai.com/uninstall-survey/?p=qorai_uninstall_survey");
...
void DoPostUninstallOperations(const base::Version& version,
  ...
  const std::wstring survey_url = std::wstring(kQoraiUninstallSurveyUrl);
#if DCHECK_IS_ON()
