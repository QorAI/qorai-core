...
class QoraiSetupInstallWorkerTest : public testing::Test {
 public:
  QoraiSetupInstallWorkerTest()
...
// chromium_src/chrome/install_static/qorai_user_data_dir_win_unittest.cc
...
TEST_F(QoraiSetupInstallWorkerTest, CleanupAlreadyRan) {
  ...
  base::win::RegKey key(
      ...
      KEY_ALL_ACCESS);
  DWORD cleanup_ran = 1;
  key.WriteValue(qorai_vpn::kQoraiVpnOneTimeServiceCleanupValue, cleanup_ran);
...
TEST_F(QoraiSetupInstallWorkerTest, CleanupNotRanYetNoKey) {
  registry_util::RegistryOverrideManager override_manager;
  std::wstring temp;
  ASSERT_NO_FATAL_FAILURE(
      override_manager.OverrideRegistry(HKEY_LOCAL_MACHINE, &temp));
  ...
  base::win::RegKey key(
      ...
      KEY_ALL_ACCESS);
  DWORD cleanup_ran = 0;
  LONG rv = key.ReadValueDW(qorai_vpn::kQoraiVpnOneTimeServiceCleanupValue,
                            &cleanup_ran);
...
TEST_F(QoraiSetupInstallWorkerTest, CleanupNotRanKeyExists) {
  registry_util::RegistryOverrideManager override_manager;
  std::wstring temp;
  ASSERT_NO_FATAL_FAILURE(
      override_manager.OverrideRegistry(HKEY_LOCAL_MACHINE, &temp));
