...
void ChromeAutocompleteProviderClient::OpenQora(const std::u16string& query) {
#if !BUILDFLAG(IS_ANDROID)
  ...
  // * Even if the active browser is changed, it'd be better to open the Qora in
  ...
  ai_chat::mojom::ConversationTurnPtr turn =
      ai_chat::mojom::ConversationTurn::New(
          ...
          false /* from_qorai_search_SERP */, std::nullopt /* model_key */);
...
bool ChromeAutocompleteProviderClient::IsQoraProviderEnabled() {
#if BUILDFLAG(IS_ANDROID)
  return false;
#else
