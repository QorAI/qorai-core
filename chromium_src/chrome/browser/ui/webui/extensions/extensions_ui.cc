...
// patch.
void QoraiAddExtensionsResources(content::WebUIDataSource* source,
                                 Profile* profile) {
  NavigationBarDataProvider::Initialize(source, profile);
  source->AddLocalizedString("privateInfoWarning",
                             IDS_EXTENSIONS_QORAI_PRIVATE_WARNING);
  source->AddLocalizedString("spanningInfoWarning",
                             IDS_EXTENSIONS_QORAI_SPANNING_WARNING);
  source->AddLocalizedString("privateAndTorInfoWarning",
                             IDS_EXTENSIONS_QORAI_PRIVATE_AND_TOR_WARNING);
  source->AddLocalizedString("qoraiHosted", IDS_EXTENSIONS_QORAI_HOSTED);

  const std::string mv2_extensions =
      base::JoinString(extensions_mv2::kPreconfiguredManifestV2Extensions, ",");
  source->AddString("qoraiHostedExtensions", mv2_extensions);
}
...
#define IDS_EXTENSIONS_ITEM_CHROME_WEB_STORE \
  IDS_EXTENSIONS_QORAI_ITEM_CHROME_WEB_STORE
#undef IDS_EXTENSIONS_ITEM_SOURCE_WEBSTORE
#define IDS_EXTENSIONS_ITEM_SOURCE_WEBSTORE \
  IDS_EXTENSIONS_QORAI_ITEM_SOURCE_WEBSTORE

#define QORAI_CREATE_EXTENSIONS_SOURCE \
  QoraiAddExtensionsResources(source, profile);

#include <chrome/browser/ui/webui/extensions/extensions_ui.cc>
#undef QORAI_CREATE_EXTENSIONS_SOURCE
#undef IDS_EXTENSIONS_ITEM_SOURCE_WEBSTORE
#undef IDS_EXTENSIONS_ITEM_CHROME_WEB_STORE
