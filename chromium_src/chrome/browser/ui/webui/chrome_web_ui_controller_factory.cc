...
// for WebUIs based upon the WebUI URL. Qorai uses `kChromeUINewTabHost` instead
// of `kChromeUINewTabPageHost` for the New Tab Page. Ensure that the NTP
// favicon is loaded for Qorai's NTP by swapping the host name constant.
#define kChromeUINewTabPageHost kChromeUINewTabHost

#define QORAI_CHROME_WEBUI_CONTROLLER_FACTORY \
  return QoraiWebUIControllerFactory::GetInstance();

#include <chrome/browser/ui/webui/chrome_web_ui_controller_factory.cc>

#undef kChromeUINewTabPageHost
#undef QORAI_CHROME_WEBUI_CONTROLLER_FACTORY
