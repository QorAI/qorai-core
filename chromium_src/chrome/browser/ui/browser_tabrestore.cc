...
void MaybeRestoreCustomTitleForTab(
  ...
  if (!extra_data.contains(tabs::kQoraiTabCustomTitleExtraDataKey) ||
      extra_data.at(tabs::kQoraiTabCustomTitleExtraDataKey).empty()) {
    return;
  }

  model->SetCustomTitleForTab(
      tab_index,
      base::UTF8ToUTF16(extra_data.at(tabs::kQoraiTabCustomTitleExtraDataKey)));
}
...
content::WebContents* AddRestoredTab(
  ...
  MaybeRestoreCustomTitleForTab(
      static_cast<QoraiTabStripModel*>(browser->tab_strip_model()), tab_index,
      extra_data);
  return web_contents;
}
