...
void QoraiToggleBookmarkBarState(content::BrowserContext* browser_context) {
  ...
  qorai::SetBookmarkState(always_show ? qorai::BookmarkBarState::kAlways
...
#define ToggleBookmarkBarWhenVisible                                       \
  ToggleBookmarkBarWhenVisible(content::BrowserContext* browser_context) { \
    QoraiToggleBookmarkBarState(browser_context);                          \
  }                                                                        \
  void ToggleBookmarkBarWhenVisible_ChromiumImpl
...
#if defined(TOOLKIT_VIEWS)
// Qorai override: return folder icons at 20px like the saved groups icon.
