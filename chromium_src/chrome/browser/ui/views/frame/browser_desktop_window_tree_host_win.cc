  ...
  switch (message) {
    case WM_NCCREATE: {
      ...
      if (!is_cloacked_ && base::FeatureList::IsEnabled(
                               features::kQoraiWorkaroundNewWindowFlash)) {
...
SkColor BrowserDesktopWindowTreeHostWin::GetBackgroundColor(
  ...
  if (requested_color == SK_ColorTRANSPARENT ||
      !base::FeatureList::IsEnabled(features::kQoraiWorkaroundNewWindowFlash)) {
...
SkColor BrowserDesktopWindowTreeHostWin::GetToolbarColor() const {
  CHECK(base::FeatureList::IsEnabled(features::kQoraiWorkaroundNewWindowFlash));
