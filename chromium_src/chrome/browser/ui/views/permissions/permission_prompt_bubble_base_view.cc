...
#include "qorai/browser/ui/geolocation/geolocation_utils.h"
#include "qorai/browser/ui/views/dialog_footnote_utils.h"
...
std::unique_ptr<views::View> CreateGeolocationDescIcon(
  ...
  auto icon_view =
      std::make_unique<views::ImageView>(ui::ImageModel::FromVectorIcon(
          use_high_accuracy ? kQoraWarningTriangleOutlineIcon
                            : kQoraInfoOutlineIcon,
...
void AddGeolocationDescriptionIfNeeded(
  ...
  if (auto* web_contents = delegate->GetAssociatedWebContents()) {
    if (auto* geolocation_tab_helper =
            QoraiGeolocationPermissionTabHelper::FromWebContents(
                web_contents)) {
...
#define QORAI_PERMISSION_PROMPT_BUBBLE_BASE_VIEW                          \
...
#undef QORAI_PERMISSION_PROMPT_BUBBLE_BASE_VIEW
