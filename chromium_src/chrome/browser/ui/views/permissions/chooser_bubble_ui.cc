...
bool IsQoraiPanel(content::WebContents* content) {
  return content->GetVisibleURL().EqualsIgnoringRef(
      GURL(kQoraiUIWalletPanelURL));
}
...
void OnWindowClosing(views::Widget* anchor_widget) {
  ...
  auto* tab_helper =
      qorai_wallet::QoraiWalletTabHelper::FromWebContents(active);
  if (tab_helper) {
...
namespace views {
class QoraiBubbleDialogDelegateView : public views::BubbleDialogDelegateView {
 public:
  QoraiBubbleDialogDelegateView() : BubbleDialogDelegateView() {}
...
Browser* FindBrowserAndAdjustBubbleForQoraiWalletPanel(
    content::WebContents* contents) {
  if (!IsQoraiPanel(contents))
...
#define FindBrowserWithTab FindBrowserAndAdjustBubbleForQoraiWalletPanel
#define GetActiveWebContents                           \
  GetActiveWebContents() && !IsQoraiPanel(contents) && \
      browser->tab_strip_model()->GetActiveWebContents

#define BubbleDialogDelegateView QoraiBubbleDialogDelegateView
