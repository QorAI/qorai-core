...
#define param(TAB, MODEL_INDEX, PINNED) \
  param(std::make_unique<QoraiTab>(this), MODEL_INDEX, PINNED)

#define CompoundTabContainer QoraiCompoundTabContainer
#define TabContainerImpl QoraiTabContainer
...
#define QORAI_TAB_DRAG_CONTEXT_IMPL_CALCULATE_INSERTION_INDEX                \
...
#define QORAI_TAB_DRAG_CONTEXT_IMPL_CALCULATE_BOUNDS_FOR_DRAGGED_VIEWS      \
  if (tabs::utils::ShouldShowVerticalTabs(tab_strip_->GetBrowser())) {      \
...
#undef QORAI_TAB_DRAG_CONTEXT_IMPL_CALCULATE_BOUNDS_FOR_DRAGGED_VIEWS
#undef QORAI_TAB_DRAG_CONTEXT_IMPL_CALCULATE_INSERTION_INDEX
