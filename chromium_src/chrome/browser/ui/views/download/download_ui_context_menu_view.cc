...
ui::SimpleMenuModel* DownloadUiContextMenuView::GetMenuModel() {
  ...
  if (auto index =
          model->GetIndexOfCommandId(QoraiDownloadCommands::REMOVE_FROM_LIST)) {
    return model;
  }
  ...
  if (auto index =
    ...
    model->InsertItemAt(*index + 1, QoraiDownloadCommands::REMOVE_FROM_LIST,
...
bool DownloadUiContextMenuView::IsCommandIdEnabled(int command_id) const {
  if (command_id == QoraiDownloadCommands::REMOVE_FROM_LIST) {
    return true;
  }
