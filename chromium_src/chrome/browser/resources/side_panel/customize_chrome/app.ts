...
class DarkerThemeToggle extends I18nMixinLit(CrLitElement) {
  static get is() {
    return 'qorai-darker-theme-toggle'
  }
  ...
  static override get styles() {
    return css`
      #darker-theme-toggle-container {
        ...
        margin-inline: var(--qora-spacing-xl);
      }
  ...
  override render() {
    return html`
      ...
      >
        <qora-icon name="theme-darker"></qora-icon>
        <span>${this.i18n('CUSTOMIZE_CHROME_DARKER_THEME_TOGGLE_LABEL')}</span>
        <!-- Use cr-toggle instead of qora-toggle in order to inherit style -->
        <cr-toggle
...
declare global {
  interface HTMLElementTagNameMap {
    'qorai-darker-theme-toggle': DarkerThemeToggle
  }
}
