...
mangle(
  (element: DocumentFragment) => {
    ...
    headingEl.insertAdjacentHTML(
      'afterbegin',
      /* html */ `<qora-icon name="themes"></qora-icon>`,
    )
  },
...
mangle(
  (element) => {
    ...
    el.textContent = '$i18n{qoraiCustomizeMenuToolbarLabel}'
  },
  (template) =>
...
// Add Qorai Midnight(Darker) theme option
mangle(
  (element: DocumentFragment) => {
    ...
    appearance.insertAdjacentHTML(
      'beforeend',
      /* html */ `
      <qorai-darker-theme-toggle></qorai-darker-theme-toggle>`
    )
  },
  (template) => template.text.includes('id="appearance"'),
)
