...
TEST_F(ChromeMetricsServiceClientTest, QoraiTestRegisterUKMProviders) {
  ...
  size_t observed_count = chrome_metrics_service_client->GetUkmService()
                              ...
                              .size();
  // In Qorai, we expect 0 UKM providers regardless of feature flag
  EXPECT_EQ(0ul, observed_count);
}

TEST_F(ChromeMetricsServiceClientTest, QoraiRegisterMetricsServiceProviders) {
  ...
  // In Qorai, we expect only 2 metrics providers (the ones added in the
