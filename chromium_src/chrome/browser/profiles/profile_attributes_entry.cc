...
void ProfileAttributesEntry::QoraiMigrateObsoleteProfileAttributes() {
  // Run our migrations
#if !BUILDFLAG(IS_ANDROID)
  const int kPlaceholderAvatarIndex =
      static_cast<int>(profiles::GetPlaceholderAvatarIndex());

  const int kQoraiAvatarIconStartIndex =
      static_cast<int>(profiles::GetDefaultAvatarIconCount() -
                       profiles::kQoraiDefaultAvatarIconsCount);
  // Added 25 July 2024
  // see https://github.com/qorai/qorai-browser/issues/40005
  //
  // Qorai originally allowed folks to pick the Chromium profile icons.
  // We then removed those in favor of our own branded icons in 0.70.x (2019).
  // https://github.com/qorai/qorai-core/pull/3165
  //
  ...
  if (icon_index < kQoraiAvatarIconStartIndex &&
...
#define QORAI_PROFILE_ATTRIBUTES_ENTRY_MIGRATE_OBSOLETE_PROFILE_ATTRIBUTES \
  QoraiMigrateObsoleteProfileAttributes();

#include <chrome/browser/profiles/profile_attributes_entry.cc>
#undef QORAI_PROFILE_ATTRIBUTES_ENTRY_MIGRATE_OBSOLETE_PROFILE_ATTRIBUTES
