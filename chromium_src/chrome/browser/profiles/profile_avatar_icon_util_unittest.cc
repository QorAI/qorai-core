...
TEST(ProfileUtilTest, QoraiAvatarIconChoices) {
  // Test that the avatar icon choices presented to the user are qorai's.
...
TEST(ProfileUtilTest, RandomIconNeverFirstIcon) {
  ...
  // the placeholder index (which for qorai is always the first item).
...
TEST_F(ProfileAvatarSelectorTest, ProfileAvatarSelectorPlaceholder) {
  ...
  const std::string expected_label =
      ui::ResourceBundle::GetSharedInstance().LoadLocalizedResourceString(
          IDS_QORAI_AVATAR_LABEL_PLACEHOLDER);
