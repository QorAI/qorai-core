...
bool IsQoraiVPNConnected(PrefService* local_state) {
  if (!base::FeatureList::IsEnabled(
          qorai_vpn::features::kQoraiVPNDnsProtection)) {
    return false;
  }
  return !local_state->GetString(prefs::kQoraiVpnDnsConfig).empty();
}
...
bool ShouldOverride(net::SecureDnsMode secure_dns_mode,
                    ...
                    bool is_managed) {
  if (!IsQoraiVPNConnected(local_state))
    return false;
...
net::DnsOverHttpsConfig MaybeOverrideDnsConfig(
  ...
  if (ShouldOverride(secure_dns_mode, local_state, management_mode,
    ...
    return net::DnsOverHttpsConfig::FromStringLax(
        local_state->GetString(prefs::kQoraiVpnDnsConfig));
  }
  return doh_config;
}
