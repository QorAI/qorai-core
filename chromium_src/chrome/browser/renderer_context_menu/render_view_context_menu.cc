...
#include "qorai/browser/renderer_context_menu/qorai_spelling_options_submenu_observer.h"
...
#include "qorai/components/ai_chat/content/browser/associated_web_contents_content.h"
...
#include "qorai/browser/ui/ai_rewriter/ai_rewriter_dialog_delegate.h"
...
AutocompleteMatch GetAutocompleteMatchForText(Profile* profile,
  ...
  AutocompleteClassifier classifier(
      ...
      std::make_unique<QoraiAutocompleteSchemeClassifier>(profile));
...
base::OnceCallback<void(QoraiRenderViewContextMenu*)>*
QoraiGetMenuShownCallback() {
...
void RenderViewContextMenu::RegisterMenuShownCallbackForTesting(
  ...
  *QoraiGetMenuShownCallback() = std::move(cb);
}

#define QORAI_APPEND_SEARCH_PROVIDER                                     \
...
#define SpellingOptionsSubMenuObserver QoraiSpellingOptionsSubMenuObserver
...
#undef RenderViewContextMenu
#undef QORAI_APPEND_SEARCH_PROVIDER
...
void OnGetImageForTextCopy(base::WeakPtr<content::WebContents> web_contents,
  ...
  qorai::ShowTextRecognitionDialog(web_contents.get(), image);
}
#endif
...
void OnRewriteSuggestionCompleted(
  ...
  if (!result.has_value()) {
    ...
    // Show the error in Qora side panel UI.
...
QoraiRenderViewContextMenu::QoraiRenderViewContextMenu(
...
QoraiRenderViewContextMenu::~QoraiRenderViewContextMenu() = default;

bool QoraiRenderViewContextMenu::IsCommandIdEnabled(int id) const {
  switch (id) {
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
    ...
    case IDC_AI_CHAT_CONTEXT_QORA_TOOLS:
...
void QoraiRenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  switch (id) {
    case IDC_COPY_CLEAN_LINK: {
      ...
      qorai::CopyLinkWithStrictCleaning(GetBrowser(), link_url);
    }; break;
    case IDC_CONTENT_CONTEXT_FORCE_PASTE: {
...
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
void QoraiRenderViewContextMenu::CopyTextFromImage() {
...
bool QoraiRenderViewContextMenu::IsAIChatEnabled() const {
  return !params_.selection_text.empty() &&
         ...
         GetProfile()->GetPrefs()->GetBoolean(
             ai_chat::prefs::kQoraiAIChatContextMenuEnabled) &&
         !IsInProgressiveWebApp();
}

void QoraiRenderViewContextMenu::ExecuteAIChatCommand(int command) {
  // To do rewrite in-place, the following conditions must be met:
  // 1) Selected content is editable.
  // 2) User has opted in to Qora.
...
void QoraiRenderViewContextMenu::BuildAIChatMenu() {
  if (!IsAIChatEnabled()) {
    return;
  }
  ...
  menu_model_.InsertSubMenuWithStringIdAt(
      *print_index, IDC_AI_CHAT_CONTEXT_QORA_TOOLS,
      IDS_AI_CHAT_CONTEXT_QORA_TOOLS, &ai_chat_submenu_model_);
}

#if BUILDFLAG(ENABLE_CONTAINERS)
void QoraiRenderViewContextMenu::BuildContainersMenu() {
...
Browser* QoraiRenderViewContextMenu::GetBrowserToOpenSettings() {
  return GetBrowser();
}

float QoraiRenderViewContextMenu::GetScaleFactor() {
...
void QoraiRenderViewContextMenu::AddSpellCheckServiceItem(bool is_checked) {
  // Call our implementation, not the one in the base class.
  // Assumption:
  // Use of spelling service is disabled in Qorai profile preferences.
...
// static
void QoraiRenderViewContextMenu::AddSpellCheckServiceItem(
    ui::SimpleMenuModel* menu,
    bool is_checked) {
  // Suppress adding "Spellcheck->Ask Qorai for suggestions" item.
}

void QoraiRenderViewContextMenu::AddAccessibilityLabelsServiceItem(
    bool is_checked) {
  // Suppress adding "Get image descriptions from Qorai"
}

void QoraiRenderViewContextMenu::AppendDeveloperItems() {
  ...
  bool add_block_elements = shields_tab_helper &&
                            shields_tab_helper->GetQoraiShieldsEnabled() &&
                            shields_tab_helper->GetAdBlockMode() !=
                                qorai_shields::mojom::AdBlockMode::ALLOW;
  add_block_elements &=
...
#if BUILDFLAG(ENABLE_CONTAINERS)
void QoraiRenderViewContextMenu::OnContainerSelected(
    const containers::mojom::ContainerPtr& container) {
  // TODO(https://github.com/qorai/qorai-browser/issues/47118)
  // Open |params_.link_url| in the selected container.
  NOTIMPLEMENTED();
}

base::flat_set<std::string>
QoraiRenderViewContextMenu::GetCurrentContainerIds() {
  // TODO(https://github.com/qorai/qorai-browser/issues/47118) If the tab is in
...
void QoraiRenderViewContextMenu::SetAIEngineForTesting(
    std::unique_ptr<ai_chat::EngineConsumer> ai_engine) {
  ai_engine_ = std::move(ai_engine);
}

void QoraiRenderViewContextMenu::InitMenu() {
...
void QoraiRenderViewContextMenu::NotifyMenuShown() {
  auto* cb = QoraiGetMenuShownCallback();
  if (!cb->is_null()) {
