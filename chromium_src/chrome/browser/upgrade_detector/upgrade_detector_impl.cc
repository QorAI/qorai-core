...
// yet - see github.com/qorai/qorai-browser/issues/45086. Under these
// circumstances, the "reinstall" prompt is very confusing, especially because
// it is likely that Qorai is just downloading an update in the background. To
// work around this, we disable outdated build detection until we also have
// background updates on macOS.
#define QORAI_UPGRADE_DETECTOR_IMPL_START_OUTDATED_BUILD_DETECTOR return;
#else
#define QORAI_UPGRADE_DETECTOR_IMPL_START_OUTDATED_BUILD_DETECTOR
#endif  // BUILDFLAG(IS_MAC)

#include <chrome/browser/upgrade_detector/upgrade_detector_impl.cc>

#undef QORAI_UPGRADE_DETECTOR_IMPL_START_OUTDATED_BUILD_DETECTOR
