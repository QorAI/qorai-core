...
from qorai_chromium_utils import get_webui_overridden_but_referenced_files

def is_gen_qorai_dir(out_dir):
    GEN_QORAI = 'gen/qorai'
    dirs = out_dir.split('/')
    # Test gen/qorai/... and <target_toolchain>/gen/qorai/...
    return '/'.join(dirs[:2]) == GEN_QORAI or '/'.join(dirs[1:3]) == GEN_QORAI
...
def main(original_function, argv):
    ...
    if args.in_files is not None:
        for f in args.in_files:
            ...
            # input file."
            if args.root_dir != args.out_dir and is_gen_qorai_dir(args.out_dir):
