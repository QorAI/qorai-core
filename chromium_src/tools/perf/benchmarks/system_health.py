...
def _add_qorai_metrics(options):
    options.ExtendTimelineBasedMetric(['qoraiNavigationMetric'])
    tracing_categories = options.config.chrome_trace_config.category_filter
    tracing_categories.AddIncludedCategory('qorai')
    tracing_categories.AddIncludedCategory('qorai.adblock')
    tracing_categories.AddIncludedCategory('qorai.ads')
    return options
...
def CreateCoreTimelineBasedMeasurementOptions(self, original_method):
    return _add_qorai_metrics(original_method(self))
