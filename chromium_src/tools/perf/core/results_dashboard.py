...
def QoraiAuthTokenGeneratorCallback():
    """A substitution for LuciAuthTokenGeneratorCallback()"""
    vpython_name = 'vpython3.bat' if sys.platform == 'win32' else 'vpython3'
    vpython_path = os.path.join(path_util.GetChromiumSrcDir(), 'qorai',
                                'vendor', 'depot_tools', vpython_name)
    args = [
        vpython_path,
        os.path.join(path_util.GetChromiumSrcDir(), 'qorai', 'tools', 'perf',
                     'dashboard_auth.py')
    ]
...
def SendResults(original_function, *args, **kwargs):
    kwargs['token_generator_callback'] = QoraiAuthTokenGeneratorCallback
