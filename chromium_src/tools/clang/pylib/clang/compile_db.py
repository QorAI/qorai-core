...
def _FilterFlags(original_function, command, additional_filtered_flags):
    ...
    flags_to_restore = [
        ...
        ' -Xclang -iquote../../qorai/chromium_src',
    ]
...
def _ProcessEntry(original_function, entry, filtered_args, target_os):
    ...
    qorai_chromium_src_file = re.sub(r'^(\.\./\.\./|gen/)',
                                     '../../qorai/chromium_src/', entry_file)
    if qorai_chromium_src_file != entry_file:
        abs_qorai_chromium_src_file = os.path.join(entry['directory'],
                                                   qorai_chromium_src_file)
        if os.path.exists(abs_qorai_chromium_src_file):
            entry['command'] = entry['command'].replace(
                entry_file, qorai_chromium_src_file)
            entry['file'] = qorai_chromium_src_file
