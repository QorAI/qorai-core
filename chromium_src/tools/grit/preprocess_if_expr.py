...
from qorai_chromium_utils import get_chromium_src_override, get_webui_overriden_file_name, get_webui_overridden_but_referenced_files, get_src_dir, wspath
...
def maybe_keep_upstream_version(override_in_folder, out_folder, override_file):
    ...
    # The path to the qorai-core override file
...
def run_mangler(out_folder, mangler_file, preprocess_file):
    ...
    tsx = qorai_node.PathInNodeModules('tsx', 'dist', 'cli.mjs')
    ts_config = wspath("//qorai/tsconfig-mangle.json")
    lit_mangler = wspath(
        "//qorai/tools/chromium_src/lit_mangler/lit_mangler_cli.ts")
...
def get_chromium_src_files(in_folder, in_files):
    """Gets all the overrides we have in qorai-core for this target"""
    override_files = []
    lit_mangler_files = []
