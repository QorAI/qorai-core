...
class QoraiFeatureDefinitionExtender:
    ...
    # being moved somewhere. "True" means //qorai/chromium_src/... counterpart
    # must exist.
    KNOWN_FILES = {
    ...
    def GetFeatureDefinitions(self, source_file):
        has_counterpart = self._HasQoraiChromiumSrcConterpart(source_file)
        if not has_counterpart:
            return None

        with open(
                qorai_chromium_utils.wspath(
                    f"//qorai/chromium_src/{source_file}"), "r") as f:
            parsed_json = json_parse.Parse(f.read())
        return parsed_json

    def _HasQoraiChromiumSrcConterpart(self, source_file):
        ...
        if has_counterpart is None:
            raise RuntimeError(
                ...
                f"{qorai_chromium_utils.get_chromium_src_override(__file__)}")
    ...
    def _ValidateKnownFiles(self):
        for source_file, should_exist in self.KNOWN_FILES.items():
            ...
            original_filepath = qorai_chromium_utils.wspath(f"//{source_file}")
            ...
            overridden_filepath = qorai_chromium_utils.wspath(
                f"//qorai/chromium_src/{source_file}")
...
def Load(self, original_method):
    ...
    qorai_extender = QoraiFeatureDefinitionExtender()
    parent_dir_prefix = "../../"
    ...
    for source_file in self._source_files:
        ...
        if source_file.startswith("qorai/") or "/test/" in source_file:
            continue

        feature_definitions = qorai_extender.GetFeatureDefinitions(source_file)
