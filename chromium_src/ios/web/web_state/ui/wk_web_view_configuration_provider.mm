...
// Replace the `WKWebViewConfigurationProvider` constructor call with the Qorai
// subclass inside of `WKWebViewConfigurationProvider::FromBrowserState`
#define SetUserData(key, ...)                                                \
  SetUserData(key, base::WrapUnique(new QoraiWKWebViewConfigurationProvider( \
                       browser_state)));
...
void QoraiWKWebViewConfigurationProvider::ResetWithWebViewConfiguration(
  ...
  // Reset fullscreen to default as it wasn't set in Qorai
