...
import '//resources/qorai/qora.bundle.js';

const leoIcons = (window as any).qoraIcons as Set<string>

// Maps Chromium icons to their equivalent Qorai icons.
const iconMap: { [key: string]: string } = {
    ...
    'tab-search:auto-tab-groups': 'product-qorai-qora',  // tab organization
}

injectStyle(CrIconElement, css`:host {
  --qora-icon-size: var(--iron-icon-width, 24px);
  --qora-icon-color: var(--iron-icon-fill-color, currentColor);
 }`)
...
(CrIconElement.prototype as any).updateIcon_ = function (...args: any) {
    ...
    if (name || leoIcons.has(this.icon)) {
        ...
        let leoIcon = this.shadowRoot!.querySelector('qora-icon')
        if (!leoIcon) {
            leoIcon = document.createElement('qora-icon')
            this.shadowRoot!.append(leoIcon)
        }
        leoIcon.setAttribute('name', name ?? this.icon)
    } else {
        removeAllOfType('qora-icon')
        old.apply(this, args)
    }
}
