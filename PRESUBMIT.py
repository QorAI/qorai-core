...
# Check Qora variables actually exist
def CheckQoraVariables(input_api, output_api):
    ...
    # If no web files were affected, this shouldn't change any Qora variables, so
    # we can skip running qora-check.
    if not any(
    ...
    try:
        parts = [
            qorai_node.PathInNodeModules('@qorai', 'qora', 'src', 'scripts',
                                         'audit-tokens.js')
        ]
        qorai_node.RunNode(parts, include_command_in_error=False)
        return []
...
def CheckPatchFormatted(input_api, output_api):
    cmd = [
        qorai_chromium_utils.wspath(
            '//qorai/build/commands/scripts/format.js'), '--presubmit'
    ]
    ...
    if not input_api.PRESUBMIT_FIX:
        cmd.append('--dry-run')
    try:
        qorai_node.RunNode(cmd, include_command_in_error=False)
        return []
    except RuntimeError as err:
        return [
...
def CheckESLint(input_api, output_api):
    ...
    with qorai_chromium_utils.sys_path('//tools'):
...
def CheckLicense(input_api, output_api):
    """Verifies the Qorai license header."""

    files_to_check = input_api.DEFAULT_FILES_TO_CHECK + (r'.+\.gni?$', )
    files_to_skip = input_api.DEFAULT_FILES_TO_SKIP + (
        r"\.storybook/",
        r"ios/browser/api/qorai_rewards/legacy_database/core_data_models/",
        r'win_build_output/',
    )
    ...
    new_file_license_re = input_api.re.compile((
        r'.*? Copyright \(c\) %(year)s The Qorai Authors\. All rights reserved\.\n'
    ...
    expected_license_template = (
        '%(comment)s Copyright (c) %(year)s The Qorai Authors. All rights reserved.\n'
...
def CheckNewThemeFilesForUpstreamOverride(input_api, output_api):
    ...
    for f in new_sources:
        ...
        upstream_file = f.replace('/qorai/', '/chromium/')
        path = qorai_chromium_utils.wspath(f'//chrome/{upstream_file}')
...
# DON'T ADD NEW QORAI CHECKS AFTER THIS LINE.
#
...
_BANNED_JAVA_FUNCTIONS += (BanRule(
    r'/(QoraiQoraPrefUtils|Utils)\.getProfile\(\)',
...
# Extend BanRule exclude lists with Qorai-specific paths.
...
def CheckForIncludeGuards(original_check, input_api, output_api, **kwargs):
    # Add 'qorai/' prefix for header guard checks to properly validate guards.
    def AffectedSourceFiles(self, original_method, source_file):

        def PrependQorai(affected_file):
            affected_file = copy.copy(affected_file)
            affected_file._path = f'qorai/{affected_file._path}'
            return affected_file

        return [
            PrependQorai(f) for f in filter(self.FilterSourceFile,
...
def CheckJavaStyle(_original_check, input_api, output_api):
    ...
    qorai/tools/android/checkstyle/qorai-style-5.0.xml style file where all
    ...
    with qorai_chromium_utils.sys_path('//tools/android/checkstyle'):
        import checkstyle
    ...
    local_path = os.path.join(input_api.PresubmitLocalPath(), 'qorai')
    style_file = os.path.join(input_api.PresubmitLocalPath(), 'qorai', 'tools',
                              'android', 'checkstyle', 'qorai-style-5.0.xml')
...
def CheckTodoBugReferences(_original_check, input_api, output_api):
    ...
    for f in input_api.AffectedSourceFiles(_FilterFile):
        for line_number, line in f.ChangedContents():
            ...
            if match and 'https://github.com/qorai/qorai-browser/issues/' not in match.group(
                    1):
    ...
    if problems:
        return [
            output_api.PresubmitPromptWarning(
                'TODO comments must be accompanied with a valid qorai-browser '
                'issue. https://github.com/qorai/qorai-browser/issues',
                problems)
        ]
    return []
