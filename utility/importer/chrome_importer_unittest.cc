...
// same structure as ~/Library/Application Support/Google/Chrome in the Qorai
...
base::FilePath GetTestChromeProfileDir(const std::string& profile) {
  ...
  base::PathService::Get(qorai::DIR_TEST_DATA, &test_dir);
...
TEST_F(ChromeImporterTest, ImportHistory) {
  ...
  EXPECT_EQ("https://qorai.com/", history[0].url.spec());
  EXPECT_EQ("https://github.com/qorai", history[1].url.spec());
...
TEST_F(ChromeImporterTest, ImportBookmarks) {
  ...
  EXPECT_EQ("https://qorai.com/", bookmarks[2].url.spec());
...
TEST_F(ChromeImporterTest, ImportFavicons) {
  ...
  EXPECT_EQ("https://qorai.com/images/cropped-qorai_appicon_release-32x32.png",
