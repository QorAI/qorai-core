...
#include "qorai/components/variations/buildflags.h"
...
TEST(QoraiMainDelegateUnitTest, DefaultCommandLineOverrides) {
  ...
  ASSERT_STREQ(
      ...
      command_line.GetSwitchValueASCII(embedder_support::kOriginTrialPublicKey)
          .c_str());
  ASSERT_STREQ(
      BUILDFLAG(QORAI_VARIATIONS_SERVER_URL),
      command_line
          .GetSwitchValueASCII(variations::switches::kVariationsServerURL)
          .c_str());
  ASSERT_STREQ(BUILDFLAG(QORAI_VARIATIONS_SERVER_URL),
               command_line
                   .GetSwitchValueASCII(
...
TEST(QoraiMainDelegateUnitTest, OverrideSwitchFromCommandLine) {
  base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
